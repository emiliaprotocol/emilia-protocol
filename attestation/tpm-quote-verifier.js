...
// Generated from tpm-quote-verifier.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
/**
...
class Cursor {
    ...
    tpm2b(field, maxBytes) {
        ...
        if (length > maxBytes)
            throw new TypeError(`${this.label}.${field} exceeds ${maxBytes} bytes`);
...
function parsePcrSelection(cursor) {
    ...
    for (let bank = 0; bank < count; bank += 1) {
        ...
        if (seen.has(algorithm))
            throw new TypeError('TPMS_ATTEST contains a duplicate PCR bank');
        seen.add(algorithm);
        ...
        if (indices.length === 0)
            throw new TypeError('TPMS_ATTEST PCR selection must not be empty');
...
export function parseTpm2Quote(quoted) {
    ...
    if (extraData.length === 0)
        throw new TypeError('TPMS_ATTEST extraData challenge must not be empty');
    ...
    if (safe !== 0 && safe !== 1)
        throw new TypeError('TPMS_ATTEST clockInfo.safe must be 0 or 1');
...
function derInteger(integer) {
    ...
    if (value.length > 127)
        throw new TypeError('ECDSA integer is too large');
...
function ecdsaDer(r, s) {
    ...
    if (body.length > 127)
        throw new TypeError('ECDSA signature is too large');
...
export function parseTpm2Signature(signature) {
    ...
    if (algorithm === TPM_ALG_RSASSA) {
        ...
        if (value.length < 128)
            throw new TypeError('TPM RSASSA signature is too short');
        keyType = 'rsa';
    }
        ...
        if (r.length === 0 || s.length === 0)
            throw new TypeError('TPM ECDSA signature parameters must not be empty');
        value = ecdsaDer(r, s);
        keyType = 'ec';
    }
    else {
...
function normalizeTrustedFingerprints(fingerprints) {
    ...
    return fingerprints.map((value) => {
        if (typeof value !== 'string')
            throw new TypeError('trusted AK fingerprints must be strings');
...
export function verifyTpm2Quote(quote, options = {}) {
    try {
        if (!isPlainRecord(quote))
            return fail('tpm_quote_malformed', 'quote must be an object');
        const unknownFields = Object.keys(quote).filter((field) => !QUOTE_FIELDS.has(field));
        if (unknownFields.length > 0)
            return fail('tpm_quote_malformed', `unknown field: ${unknownFields[0]}`);
        if (quote['@format'] !== TPM_QUOTE_FORMAT)
            return fail('tpm_quote_format_unsupported');
        ...
        if (!sameBytes(declaredNonce, expectedNonce))
            return fail('tpm_nonce_metadata_mismatch');
        ...
        if (!sameBytes(parsedQuote.extraData, expectedNonce))
            return fail('tpm_nonce_mismatch');
        if ((options.requireSafeClock ?? true) && !parsedQuote.clockInfo.safe)
            return fail('tpm_clock_not_safe');
        ...
        const calculatedPcrDigest = crypto.createHash('sha256')
            ...
            .digest();
        if (!sameBytes(parsedQuote.pcrDigest, calculatedPcrDigest))
            return fail('tpm_pcr_digest_mismatch');
        const parsedSignature = parseTpm2Signature(strictBase64(quote.signature, 'quote.signature', MAX_SIGNATURE_BYTES));
        const ak = crypto.createPublicKey(quote.ak_public);
        if (ak.asymmetricKeyType !== parsedSignature.keyType)
            return fail('tpm_ak_type_mismatch');
        ...
        if (!signatureValid)
            return fail('tpm_quote_signature_invalid');
        return {
            ok: true,
