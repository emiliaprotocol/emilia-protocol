...
function baseRecord(overrides: Record<string, any> = {}): Record<string, any> {
  return assembleRecord(
    { commit: COMMIT_A, package_path: 'packages/verify' },
    { filename: 'pkg-1.0.0.tgz', sha256: HASH_H, bytes: 1024, ...overrides },
  );
}
...
function rebuildReturning(sha256: string): () => { source_commit: string; sha256: string; filename: string; bytes: number } {
