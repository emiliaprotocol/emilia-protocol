...
function emit(packagePath) {
  ...
  return assembleRecord(
    { commit, package_path: packagePath },
    { filename: built.filename, sha256: built.sha256, bytes: built.bytes },
  );
}
...
function main() {
  ...
  if (cmd === 'emit') {
    if (!arg) throw new Error('usage: emit <package_path>');
    process.stdout.write(JSON.stringify(emit(arg), null, 2) + '\n');
    return;
  }

  if (cmd === 'verify') {
    if (!arg) throw new Error('usage: verify <record.json> [--rebuild]');
  ...
  if (cmd === 'demo') {
    ...
    const record = assembleRecord(
      { commit, package_path: packagePath },
      { filename: a.filename, sha256: a.sha256, bytes: a.bytes },
    );
