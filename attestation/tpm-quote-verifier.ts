...
function fail(reason: string, detail?: string): { ok: false; reason: string; detail?: string } {
  return {
    ok: false,
    reason,
...
function hexAlgorithm(value: number): string {
...
function strictBase64(value: unknown, field: string, maxBytes: number): Buffer {
 ...
 */
function strictHex(value: unknown, field: string, bytes: number | null = null): Buffer {
...
class Cursor {
  ...
  constructor(buffer: Buffer, label: string) {
    this.buffer = buffer;
    this.label = label;
    this.offset = 0;
  }
  ...
  tpm2b(field: string, maxBytes: number): Buffer {
...
function parsePcrSelection(cursor: Cursor): Array<{ algorithm: number; bank: string; indices: number[] }> {
  ...
  const selections: Array<{ algorithm: number; bank: string; indices: number[] }> = [];
...
export function parseTpm2Quote(quoted: Buffer): Record<string, any> {
...
function derInteger(integer: Buffer): Buffer {
...
function ecdsaDer(r: Buffer, s: Buffer): Buffer {
...
export function parseTpm2Signature(signature: Buffer): Record<string, any> {
...
function normalizeExpectedSelection(selection: any): number[] {
  ...
  if (selection.sha256.length === 0
      || selection.sha256.some((index: any) => !Number.isInteger(index) || index < 0 || index > 31)
  ...
  return [...selection.sha256].sort((left: number, right: number) => left - right);
}

function normalizePcrValues(values: any, selected: number[], field: string): Buffer[] {
...
function normalizeTrustedFingerprints(fingerprints: any): string[] {
  if (!Array.isArray(fingerprints) || fingerprints.length === 0) {
    throw new TypeError('trustedAkFingerprints must be a non-empty array');
  }
  return fingerprints.map((value: any) => {
 ...
 */
export function verifyTpm2Quote(
  quote: any,
  options: {
    expectedNonce?: string | Buffer;
    trustedAkFingerprints?: string[];
    expectedPcrSelection?: Record<string, number[]>;
    expectedPcrValues?: Record<string, Record<string, string>>;
    requireSafeClock?: boolean;
  } = {},
): Record<string, any> {
  try {
