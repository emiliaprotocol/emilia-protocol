...
const TIMEOUT_S = Math.min(
  Number.isFinite(requestedTimeout) && requestedTimeout > 0 ? requestedTimeout : 30,
  60,
);
const POLL_MS   = 3000;
...
function writeAuthorizationTelemetry(decision: string, receiptId?: string | null): void {
  if (!OTEL_AUTHORIZATION_ENABLED) return;
  if (decision !== 'deny' && decision !== 'allow') return; // 'ask' is unresolved
...
// ── decision emitters ───────────────────────────────────────────────────────
function emit(decision, reason, receiptId?: string | null) {
 ...
 */
async function ep(path: string, { method = 'GET', body }: any = {}): Promise<any> {
