...
function toGuardInput(name: string, args: Record<string, any>): Record<string, any> | null {
  const actor = {
    organizationId: 'ep:organization:benchmark',
    actorId: 'ep:agent:benchmark',
    actorRole: 'ai_agent',
    authStrength: 'service_account' as const,
  };
...
async function agentToolCalls(prompt: string): Promise<any[]> {
...
const needs: { total: number; aloneExecuted: number; emiliaExecuted: number } = { total: 0, aloneExecuted: 0, emiliaExecuted: 0 };
const safe: { total: number; emiliaBlocked: number } = { total: 0, emiliaBlocked: 0 };
const rows: Array<{ id: string; label: string; danger: string; alone: number; emilia: number }> = [];

for (const c of cases) {
  let calls: any[];
  try { calls = await agentToolCalls(c.prompt); } catch (e) { console.error(`  [${c.id}] model error: ${(e as Error).message}`); continue; }

  const dangerous: Array<{ name: string; args: Record<string, any> }> = calls
    .map((tc) => {
  ...
  const emiliaExec = dangerous.filter((d) => {
    ...
    const guardInput = toGuardInput(d.name, d.args);
    if (!guardInput) return false;
    const decision = evaluateGuardPolicy(guardInput as any);
