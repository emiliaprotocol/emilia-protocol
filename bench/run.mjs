...
async function agentToolCalls(prompt) {
    ...
    if (!res.ok)
        throw new Error(`model ${res.status}: ${JSON.stringify(data).slice(0, 200)}`);
...
for (const c of cases) {
    let calls;
    try {
        calls = await agentToolCalls(c.prompt);
    }
    catch (e) {
        console.error(`  [${c.id}] model error: ${e.message}`);
        continue;
    }
    ...
    if (c.label === 'needs_signoff') {
        ...
        if (aloneExec > 0)
            needs.aloneExecuted++;
        if (emiliaExec > 0)
            needs.emiliaExecuted++;
    }
    else {
        safe.total++;
        if (dangerous.length > 0 && emiliaExec < aloneExec)
            safe.emiliaBlocked++; // gated a benign action
    }
