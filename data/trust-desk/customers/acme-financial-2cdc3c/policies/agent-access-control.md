...
This policy governs what actions AI agents within Sentinel KYC AI may take, on whose authority, and under what controls. It applies to every tool call, API invocation, data write, and external side-effect initiated by any model-driven agent in the product.
...
- Rate limits: 60 requests/minute per user per minute.
...
- Human approval: not required.
- Rate limits: 300 requests/minute per user per minute.
...
- Rate limits: 1,000 requests/minute per user per hour.
...
- Authorization: session must hold the specific tool scope AND pass an elevated policy check AND the user must have completed re-authentication within the last 15 minutes minutes.
...
- Rate limits: 5,000 requests/minute per user per day; per-tenant absolute limits configurable.
- **Irreversibility warning**: user is shown estimated reversibility before approval.
- Tested monthly under Acme Financial's adversarial test suite.

---

## 4. Per-tool specifications

For each tool available to agents, Acme Financial maintains a record containing:
...
|---|---|---|---|---|
| read_records | T | the customer-visible effects of the incident | explicit human approval | per-tenant configurable |
| create_record | T | the customer-visible effects of the incident | explicit human approval | per-tenant configurable |
| initiate_action | T | the customer-visible effects of the incident | explicit human approval | per-tenant configurable |
...
- Expires after 60 minutes or on task completion, whichever is earlier.
- Does not survive across sessions or user logins.
- Cannot elevate privileges above the user's own scope.

Autonomous multi-step execution within a delegation is permitted at Tier 0–1. Each Tier 2+ step within a delegation triggers a fresh human approval unless the user explicitly grants batch approval (with a maximum batch size of 50 actions).
...
- **Parameter validation failure**: request rejected, logged. Model is shown the validation error and may retry with corrected parameters, up to 3 attempts attempts per session.
...
- **Per-tool kill switch**: feature flag to disable any specific tool globally. Propagates to all active sessions within 5 minutes.
...
Audit records are retained for 12 months days and available for customer export. Tier 3 records are additionally hash-chained for tamper evidence.
...
- **Tier 3 unauthorized execution**: immediate kill switch, customer notification within 1 business day hours, regulatory notification where required.
- **Tier 2 unauthorized execution**: kill switch for affected tool, customer notification within 4 hours hours.
- **Tier 0/1 unauthorized read**: customer notification within 1 hour hours.
...
The signed hash of this policy is published on Acme Financial's AI Trust Page:
