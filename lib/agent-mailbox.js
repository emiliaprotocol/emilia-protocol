...
// Generated from agent-mailbox.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
/**
...
const FILE_STORE_QUEUES = new Map();
const VERIFIED_ENVELOPE_RESULTS = new WeakMap();
...
export function agentMailboxDigest(value) {
...
function sign(domain, value, privateKey) {
...
function verify(domain, value, signature, publicKey) {
    if (!ed25519Signature(signature))
        return false;
    try {
        const bytes = Buffer.concat([domain, Buffer.from(canonical(value), 'utf8')]);
...
async function serializeFileStore(key, operation) {
    ...
    try {
        ...
        release();
        if (FILE_STORE_QUEUES.get(key) === tail)
            FILE_STORE_QUEUES.delete(key);
    }
}
export function createAgentMailboxEnvelope(input) {
    ...
    if (!MESSAGE_TYPES.has(input.messageType))
        throw new TypeError('mailbox_message_type_invalid');
    ...
    if (!privateKey)
        throw new TypeError('mailbox_ed25519_private_key_required');
    const core = {
    ...
    if (!identifier(envelopeId))
        throw new TypeError('mailbox_envelope_id_invalid');
...
function envelopeRefusal(reason, envelopeDigest = null) {
    return Object.freeze({
...
export function verifyAgentMailboxEnvelope(envelope, options) {
    if (!exactKeys(envelope, ENVELOPE_KEYS))
        return envelopeRefusal('envelope_shape_invalid');
    let envelopeDigest;
    try {
    ...
    if (Date.parse(asOf) > Date.parse(expiresAt))
        return envelopeRefusal('envelope_expired', envelopeDigest);
    if (Date.parse(asOf) < Date.parse(createdAt))
        return envelopeRefusal('envelope_not_yet_valid', envelopeDigest);
    try {
    ...
    if (sender.status !== 'active')
        return envelopeRefusal('sender_key_not_active', envelopeDigest);
    const publicKeySpkiB64u = sender.public_key_spki_b64u;
    const publicKey = publicEd25519(publicKeySpkiB64u);
    if (!publicKey)
        return envelopeRefusal('sender_key_invalid', envelopeDigest);
...
function sameStoredDelivery(left, right) {
    return canonical({ ...left, read_acknowledgement: null })
        === canonical({ ...right, read_acknowledgement: null });
}
function storedDeliverySigningValue(record, binding) {
...
function createReadAcknowledgement(record, input) {
    const unsigned = {
...
export function createMemoryAgentMailboxStore() {
    const records = new Map();
    const key = (recipientId, envelopeId) => `${recipientId}\0${envelopeId}`;
    return Object.freeze({
        durable: false,
        ...
        async put(record) {
            ...
            const sameSequence = [...records.values()].find((candidate) => (candidate.recipient_id === record.recipient_id
        ...
        async acknowledge(recipientId, envelopeId, acknowledgement) {
            ...
            if (record.read_acknowledgement === null) {
                ...
                if (!updated)
                    throw new Error('mailbox_store_acknowledgement_invalid');
...
function storageKey(recipientId, envelopeId) {
    return crypto.createHash('sha256').update(`${recipientId}\0${envelopeId}`, 'utf8').digest('hex');
}
function shapedReadAcknowledgement(value) {
...
function checkedStoredEnvelope(value) {
    try {
        canonical(value);
    }
    catch {
    ...
    if (!exactKeys(value, STORED_ENVELOPE_KEYS))
        return null;
    const record = value;
    ...
    try {
        ...
        if (acknowledgement !== null && (acknowledgement.mailbox_id !== record.delivery_binding.mailbox_id
...
function verifyStoredEnvelope(record, options) {
    ...
    if (!verify(STORED_DELIVERY_DOMAIN, storedDeliverySigningValue(record, binding), signature.value, publicKey))
        return false;
    const senderDirectory = Object.create(null);
    ...
    if (acknowledgement === null)
        return true;
    if (Date.parse(acknowledgement.read_at) > Date.parse(options.asOf))
        return false;
    ...
    return verify(READ_ACKNOWLEDGEMENT_DOMAIN, unsignedAcknowledgement, acknowledgementSignature.value, acknowledgementKey)
...
function checkedStorePutResult(value, pending) {
...
async function writeNewRecord(filename, record) {
    let handle;
    try {
...
async function readStoredEnvelopeFile(recordsDirectory, name) {
...
export function createFileAgentMailboxStore({ directory }) {
    ...
    const filename = (recipientId, envelopeId) => path.join(recordsDirectory, `${storageKey(recipientId, envelopeId)}.json`);
    return Object.freeze({
        durable: true,
        deliveryAtomicity: 'single_process',
        bodyBound: true,
        async put(record) {
            return serializeFileStore(recordsDirectory, async () => {
                ...
                const existingRecords = await Promise.all(names.map((name) => (readStoredEnvelopeFile(recordsDirectory, name))));
                const sameSequence = existingRecords.find((candidate) => (candidate.recipient_id === record.recipient_id
                    && candidate.envelope.sender_id === record.envelope.sender_id
                    && candidate.envelope.thread_id === record.envelope.thread_id
                    && candidate.envelope.sequence === record.envelope.sequence));
                if (sameSequence) {
                ...
                if (outcome === 'stored')
                    return { outcome, record: copyRecord(record) };
        ...
        async list(recipientId) {
            return serializeFileStore(recordsDirectory, async () => {
                await ready;
                const names = (await readdir(recordsDirectory)).filter((name) => name.endsWith('.json'));
                const records = await Promise.all(names.map((name) => (readStoredEnvelopeFile(recordsDirectory, name))));
                return records
                    .filter((record) => record.recipient_id === recipientId)
                    .sort((left, right) => left.envelope.sequence - right.envelope.sequence)
                    .map(copyRecord);
...
export function verifyAgentMailboxDeliveryReceipt(receipt, options) {
    const refusal = (reason) => Object.freeze({
        verification_profile: AGENT_MAILBOX_DELIVERY_VERIFICATION_PROFILE,
        verified: false, accepted: false, duplicate: false, reason, authorizes: false,
    });
    if (!identifier(options?.expectedRecipientId))
        return refusal('delivery_receipt_recipient_required');
    ...
    if (!publicKey)
        return refusal('delivery_receipt_key_invalid');
    ...
    if (receiptId !== expectedReceiptId)
        return refusal('delivery_receipt_id_mismatch');
...
export function createAgentMailbox(options) {
    ...
    if (!privateKey)
        throw new TypeError('mailbox_service_ed25519_key_required');
    ...
    const mailboxVerificationKeys = new Map();
    ...
    const timestamp = () => {
        ...
        if (!value)
            throw new Error('mailbox_trusted_time_unavailable');
        return value;
    };
    const receipt = (input) => createDeliveryReceipt({
        mailboxId: options.mailboxId,
        keyId: options.keyId,
        privateKey,
        ...input,
    });
    const verifiedList = async (recipientId, asOf) => {
        const listed = await options.store.list(recipientId);
        if (!Array.isArray(listed))
            throw new Error('mailbox_store_result_invalid');
        const records = [];
        const byEnvelopeId = new Map();
        const byThreadSequence = new Map();
        for (const candidate of listed) {
            ...
            if (envelopeMatch !== undefined || sequenceMatch !== undefined) {
                if (envelopeMatch === recordBytes && sequenceMatch === recordBytes)
                    continue;
                throw new Error('mailbox_store_result_invalid');
            }
    ...
    return Object.freeze({
        ...
        async deliver(envelope, { recipientId }) {
            if (!identifier(recipientId))
                throw new TypeError('mailbox_recipient_invalid');
            ...
            const stored = {
                ...
                delivery_binding: {
                    ...
                    signature: {
                        algorithm: 'Ed25519',
                        value: sign(STORED_DELIVERY_DOMAIN, storedDeliverySigningValue(storedDelivery, deliveryBinding), privateKey),
                    },
                },
            };
        ...
        async acknowledge({ recipientId, envelopeId }) {
            ...
            if (!existing)
                return Object.freeze({ acknowledgement: null, authorizes: false });
            ...
            const returnedValue = await options.store.acknowledge(recipientId, envelopeId, pendingAcknowledgement);
...
export function extractMailboxActionProposal(envelope, options) {
    const refusal = (reason) => Object.freeze({ valid: false, authorizes: false, reason });
    ...
    if (actionDigest !== payload.action_digest)
        return refusal('action_digest_mismatch');
    return Object.freeze({
        valid: true,
        authorizes: false,
        reason: null,
...
function admissionRefusal(reason, state = 'REFUSED') {
    return Object.freeze({
        admitted: false,
        state,
...
export async function admitMailboxActionProposal(input) {
    ...
    if (checkedAdmission.accepted !== true)
        return admissionRefusal('admission_not_accepted');
