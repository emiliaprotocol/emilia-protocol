...
let getLoggerConfig: (() => LoggerBootstrapConfig) | null;
try {
...
const loggerConfig: LoggerBootstrapConfig = typeof getLoggerConfig === 'function'
  ? getLoggerConfig()
...
const LEVELS: Record<LogLevel, number> = { error: 50, warn: 40, info: 30, debug: 20 };
...
function normalizeFields(fields: unknown): unknown {
  ...
  if (fields !== null && typeof fields === 'object') return redactSensitive(fields);
  return fields;
}
 ...
 */
function emit(level: LogLevel, context: LogContext, msg: string, fields?: unknown): void {
  const safeFields = normalizeFields(fields);
  const fieldsObject: Record<string, unknown> = safeFields === undefined
    ? {}
...
function createLogger(context: LogContext = {}): Logger {
  return {
    error(msg: string, fields?: unknown)  { emit('error', context, msg, fields); },
    warn(msg: string, fields?: unknown)   { emit('warn',  context, msg, fields); },
    info(msg: string, fields?: unknown)   { emit('info',  context, msg, fields); },
    debug(msg: string, fields?: unknown)  { emit('debug', context, msg, fields); },
