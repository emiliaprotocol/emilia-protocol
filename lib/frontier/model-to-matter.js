...
export async function evaluateRegisteredModelToMatterPresentation(input = {}) {
    ...
    try {
        base = await evaluateRegisteredPresentation(challenge, graph, profile, {
            ...
            verifiers: graphVerifiers(action, profile, input.as_of, revokedEvidenceDigests),
            as_of: input.as_of,
 ...
 */
export function createModelToMatterExecutor({ profile, challengeStore, clearanceStore, revocationProvider, relianceProgram, challengeTtlSec = 300, now = Date.now, allowEphemeralState = false, } = {}) {
    let pinnedProfile;
