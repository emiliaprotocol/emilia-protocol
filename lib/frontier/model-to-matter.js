...
const ACTION_TYPES = Object.freeze(['science.bio.experiment.execute']);
 ...
 * Storage errors propagate; there is no in-memory fallback.
 */
export async function evaluateRegisteredModelToMatterPresentation(input = {}) {
  ...
  if (!profile.allowed_action_types.includes(action.action_type)) {
    return clearanceResult('malformed', { action_digest: actionDigest, reasons: ['action_type is not allowed by the profile'] });
  }
  if (challenge?.action_digest !== actionDigest) {
    return {
      ...clearanceResult('action_mismatch', {
        action_digest: actionDigest,
  ...
  if ([...revokedEvidenceDigests].some((digest) => !validDigest(digest))) {
    return clearanceResult('malformed', {
      action_digest: actionDigest,
  ...
  const base = await evaluateRegisteredPresentation(
    challenge,
    graph,
    profile,
    {
  ...
  if (base.verdict === 'admissible') {
    const firstClearance = await input.clearanceStore.consume(`model-to-matter:${actionDigest}`);
  ...
  return clearanceResult(base.verdict, {
    action_digest: actionDigest,
    replay_digest: base.replay_digest,
...
export function createModelToMatterExecutor({
  ...
  async function issueChallenge(action, options = {}) {
    ...
    return createRegisteredModelToMatterChallenge(actionSnapshot, pinnedProfile, {
      ...clone(options),
...
export function verifyModelToMatterEffect(effect, opts = {}) {
  ...
  let expectedAction;
  try { expectedAction = modelToMatterActionDigest(opts.expectedAction); } catch { return { ...verified, reason: 'expected_action_invalid' }; }
