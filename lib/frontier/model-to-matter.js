...
// Generated from model-to-matter.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
/**
...
import { canonicalize, verifyOutcomeObservationSet, } from '../../packages/verify/index.js';
import { predictedEffectsDigest } from '../../packages/verify/effect-predicates.js';
import { createRegisteredEvidenceChallenge, evaluateRegisteredPresentation, } from '../negotiate/evidence-challenge.js';
import { artifactDigest, EVIDENCE_GRAPH_VERSION, } from '../evidence/evidence-graph.js';
import { trustProgramDigest } from '../../packages/gate/trust-program.js';
import { computeCaid, verifyCaid } from '../../caid/impl/js/caid.mjs';
import { signAgile, verifyAgileSignatureSet, ML_DSA_65_PUBLIC_KEY_BYTES, } from '@emilia-protocol/verify/pq-signature-agility';
...
export const M2M_V2_REQUIRED_ALGORITHMS = Object.freeze(['Ed25519', 'ML-DSA-65']);
...
function publicKeyToB64u(privateKey) {
    ...
    if (publicKey.asymmetricKeyType !== 'ed25519')
        throw new Error('Model-to-Matter evidence requires an Ed25519 key');
...
function unsigned(document) {
    if (!isObject(document))
        throw new Error('signed document must be an object');
...
function verifySignature(document, domain, digestField) {
    ...
    let digest;
    try {
        digest = signedBodyDigest(domain, document);
    }
    catch {
        return fail('uncanonicalizable');
    }
    if (digest !== sig[digestField])
    ...
    try {
        ...
        const ok = crypto.verify(null, signingBytes(domain, unsigned(document)), key, Buffer.from(sig.signature_b64u, 'base64url'));
 ...
 * longer verifies.
 */
function signingBytesV2(domain, body) {
...
function signedBodyDigestV2(domain, document) {
 ...
 */
async function verifySignatureV2(document, domain, digestField) {
    const sig = document?.signature;
    const fail = (reason) => ({ verified: false, reason });
    if (!sig
    ...
    let digestValue;
    try {
        digestValue = signedBodyDigestV2(domain, document);
    }
    catch {
        return fail('uncanonicalizable');
    }
    if (digestValue !== sig[digestField])
        return fail('digest_mismatch');
    const keyId = keyIdFor(sig.public_key);
    if (sig.key_id !== undefined && sig.key_id !== keyId)
        return fail('key_id_mismatch');
    ...
    if (sig.pq_key_id !== undefined && sig.pq_key_id !== pqKeyId)
        return fail('pq_key_id_mismatch');
    let setResult;
    try {
        setResult = await verifyAgileSignatureSet(new Uint8Array(signingBytesV2(domain, unsigned(document))), sig.signatures, [
...
function assertAction(action) {
    ...
    if (action['@version'] !== M2M_ACTION_VERSION)
        throw new Error(`action @version must be ${M2M_ACTION_VERSION}`);
    ...
    if (Number.isNaN(strictInstantMs(action.requested_at)))
        throw new Error('requested_at must be a valid RFC 3339 instant');
    if (action.max_executions !== 1)
        throw new Error('max_executions must equal 1');
    return action;
}
...
function validatePin(pin, type) {
    if (!isObject(pin))
        throw new Error(`accepted_issuers.${type} pin must be an object`);
...
function assertPhysicalStatePolicy(policy) {
    ...
    policy.executor_public_keys.forEach((key) => validateEd25519PublicKey(key, 'physical_state_policy.executor_public_keys entry'));
    return policy;
}
function assertProfile(profile) {
    ...
    if (profile.policy_id !== profile.profile_id)
        throw new Error('policy_id must equal profile_id');
    ...
    if (profile.requirement !== expectedRequirement)
        throw new Error('profile requirement cannot be weakened');
    ...
    if (!Object.hasOwn(ASSURANCE_RANK, profile.required_human_assurance))
        throw new Error('required_human_assurance is invalid');
    if (profile.require_action_agreement !== true)
        throw new Error('profile action agreement cannot be weakened');
    if (!isObject(profile.accepted_issuers))
        throw new Error('accepted_issuers must be an object');
    ...
    for (const type of M2M_EVIDENCE_TYPES) {
        ...
        if (!Array.isArray(pins) || pins.length === 0)
            throw new Error(`accepted_issuers.${type} requires at least one pin`);
 ...
 */
export function createModelToMatterEvidenceRequirement(profile) {
...
function assertCompiledRelianceProgram(value, evidenceRequirementDigest) {
    ...
    const stages = new Map(value.program.stages.map((stage) => [stage.stage_id, stage]));
...
function createRequirementBinding(profile, relianceProgram) {
...
function requirementBindingRefusal(binding, actionDigest, actionCaid, asOf) {
    if (!binding)
        return null;
...
function clearanceBinding(binding) {
    return binding ? {
...
function boundReplayDigest(replayDigest, binding) {
    if (!binding || !validDigest(replayDigest))
        return replayDigest;
...
function assertEvidenceBody(body, version = M2M_EVIDENCE_VERSION) {
    ...
    if (body['@version'] !== version)
        throw new Error(`evidence @version must be ${version}`);
    if (!M2M_EVIDENCE_TYPES.includes(body.evidence_type))
    ...
    switch (body.evidence_type) {
        ...
        case 'human_authorization':
            ...
            assertString(claims.decision, 'claims.decision');
            if (!Object.hasOwn(ASSURANCE_RANK, claims.assurance_class))
                throw new Error('claims.assurance_class is invalid');
            break;
        case 'physical_state_attestation':
            ...
            if (typeof claims.match !== 'boolean')
                throw new Error('claims.match must be a boolean');
            break;
        default:
            throw new Error('unsupported evidence_type');
    }
 ...
 */
export async function signModelToMatterEvidenceV2(input, keys) {
    if (!isObject(input))
        throw new Error('evidence input must be an object');
...
function physicalMeasurementBody(artifact) {
 ...
 */
export function physicalStateMeasurementDigest(artifact) {
 ...
 */
function acceptModelToMatterEvidence(artifact, opts, evidenceDigest) {
    ...
    try {
        assertEvidenceBody(unsigned(artifact), artifact['@version']);
    }
    catch {
    ...
    if (issuedAt > asOf)
        return { ...verified, reason: 'not_yet_valid' };
    if (asOf >= expiresAt)
    ...
    if (artifact.evidence_type === 'physical_state_attestation') {
        ...
        if (measuredAt > asOf)
            return { ...verified, reason: 'measurement_not_yet_valid' };
        ...
        if (measuredAt !== issuedAt)
            return { ...verified, reason: 'measurement_issuance_mismatch' };
        ...
        try {
            measurementDigest = physicalStateMeasurementDigest(artifact);
        }
        catch {
            return { ...verified, reason: 'evidence_body_invalid' };
        }
    ...
    if (!claimsMatchAction(artifact.evidence_type, artifact.claims, expectedAction, opts.requiredHumanAssurance ?? 'class_a')) {
    ...
    if (revoked)
        return { ...verified, revoked: true, reason: 'revoked' };
    return {
        ...verified,
        accepted: true,
        reason: null,
...
export function verifyModelToMatterEvidence(artifact, opts = {}) {
    ...
    if (!sig.verified)
        return output({ reason: sig.reason });
 ...
 */
export async function verifyModelToMatterEvidenceV2(artifact, opts = {}) {
    const output = (overrides = {}) => ({
        verified: false, accepted: false, revoked: false, establishes_physical_truth: false, reason: null, ...overrides,
    });
 ...
 */
export async function verifyModelToMatterEvidenceAny(artifact, opts = {}) {
    if (isObject(artifact) && artifact['@version'] === M2M_EVIDENCE_V2_VERSION) {
...
export function buildModelToMatterGraph(action, evidenceArtifacts) {
    ...
    if (!Array.isArray(evidenceArtifacts))
        throw new Error('evidenceArtifacts must be an array');
    const seen = new Set();
    const nodes = evidenceArtifacts.map((artifact) => {
        const type = artifact?.evidence_type;
        if (!M2M_EVIDENCE_TYPES.includes(type))
            throw new Error(`unsupported evidence type ${type}`);
        if (seen.has(type))
            throw new Error(`duplicate evidence type ${type}`);
        seen.add(type);
...
function graphIsSafeToEvaluate(graph) {
    try {
        if (!isObject(graph) || graph['@version'] !== EVIDENCE_GRAPH_VERSION)
            return false;
        ...
        for (const node of graph.nodes) {
            ...
            if (!validDigest(node.id) || !M2M_EVIDENCE_TYPES.includes(node.type)
                || seen.has(node.type) || !isObject(node.artifact))
                return false;
            seen.add(node.type);
            if (artifactDigest(node.artifact) !== node.id)
                return false;
        }
        return true;
    }
...
export async function createRegisteredModelToMatterChallenge(action, profile, opts = {}) {
    ...
    if (!profile.allowed_action_types.includes(action.action_type))
        throw new Error('action_type is not allowed by the profile');
    return createRegisteredEvidenceChallenge(action, profile, opts);
}
function graphVerifiers(action, profile, asOf, revokedEvidenceDigests, retiredPhysicalMeasurementDigests) {
...
export async function evaluateRegisteredModelToMatterPresentation(input = {}) {
    ...
    const bindingRefusal = requirementBindingRefusal(requirementBinding, actionDigest, actionCaid, input.as_of);
    if (bindingRefusal) {
    ...
    const retiredPhysicalMeasurementDigests = new Set(input.retiredPhysicalMeasurementDigests);
    ...
    let base;
    try {
        base = await evaluateRegisteredPresentation(challenge, graph, profile, {
            challengeStore: input.challengeStore,
            verifiers: graphVerifiers(action, profile, input.as_of, revokedEvidenceDigests, retiredPhysicalMeasurementDigests),
            as_of: input.as_of,
 ...
 */
export function createModelToMatterExecutor({ profile, challengeStore, clearanceStore, revocationProvider, physicalMeasurementProvider, relianceProgram, challengeTtlSec = 300, now = Date.now, allowEphemeralState = false, } = {}) {
    let pinnedProfile;
    let pinnedRelianceProgram = null;
    let pinnedRequirementBinding = null;
    try {
    ...
    function executorClearance(baseVerdict, data = {}) {
    ...
    async function issueChallenge(action, options = {}) {
        ...
        if (!isObject(options))
            throw new Error('Model-to-Matter challenge options must be an object');
        assertOnlyKeys(options, new Set(['nonce', 'challenge_id', 'obtain_hints']), 'challenge options');
        const asOf = currentInstant();
        if (!asOf)
            throw new Error('Model-to-Matter executor clock is invalid');
        let actionSnapshot;
        try {
        ...
        const bindingRefusal = requirementBindingRefusal(pinnedRequirementBinding, modelToMatterActionDigest(actionSnapshot), modelToMatterCaid(actionSnapshot).caid, asOf);
        if (bindingRefusal) {
    ...
    async function evaluate(presentation = {}) {
        if (reconciliationFreeze)
            return frozenClearance();
        const asOf = currentInstant();
        if (!asOf)
            return executorClearance('refused', { reasons: ['executor clock is unavailable or invalid'] });
        try {
        ...
        try {
            ...
            if (!(provided instanceof Set))
                throw new Error('revocation provider did not return a Set');
        ...
        try {
            ...
            if (!(provided instanceof Set))
                throw new Error('physical measurement provider did not return a Set');
    ...
    async function run(presentation, effect) {
        if (typeof effect !== 'function')
            throw new Error('Model-to-Matter run() requires an effect function');
        let actionSnapshot;
        let challengeSnapshot;
        let graphSnapshot;
        try {
...
function assertEffectBody(body, version = M2M_EFFECT_VERSION) {
    assertNoRawContent(body);
    assertOnlyKeys(body, new Set([
    ...
    if (body['@version'] !== version)
        throw new Error(`effect @version must be ${version}`);
    assertDigest(body.action_digest, 'action_digest');
    ...
    if (Number.isNaN(strictInstantMs(body.executed_at)))
        throw new Error('executed_at must be a valid RFC 3339 instant');
    if (!EFFECT_STATUSES.has(body.status))
        throw new Error('effect status must be completed, failed, aborted, or indeterminate');
...
export function signModelToMatterEffect(input, privateKey) {
    ...
    if (input.clearance.action_digest !== actionDigest)
        throw new Error('clearance binds a different action');
    if (input.clearance.action_caid !== actionCaid)
        throw new Error('clearance binds a different CAID');
    if (input.executor_id !== input.action.executor.executor_id)
        throw new Error('executor_id does not match the action');
    const body = {
 ...
 * custody rationale.
 */
export async function signModelToMatterEffectV2(input, keys) {
    if (!isObject(input))
        throw new Error('effect input must be an object');
    assertAction(input.action);
    if (input.clearance?.['@version'] !== M2M_CLEARANCE_VERSION
 ...
 */
function acceptModelToMatterEffect(effect, opts, effectDigest) {
    const output = (overrides = {}) => ({
        verified: false,
        accepted: false,
        establishes_physical_truth: false,
    ...
    try {
        assertEffectBody(unsigned(effect), effect['@version']);
    }
    catch {
        return { ...verified, reason: 'effect_body_invalid' };
    }
    let expectedAction;
    let expectedCaid;
    try {
    ...
    if (effect.action_digest !== expectedAction)
        return { ...verified, reason: 'action_binding_mismatch' };
    if (effect.action_caid !== expectedCaid)
        return { ...verified, reason: 'caid_binding_mismatch' };
    ...
    if (!pin)
        return { ...verified, reason: 'executor_key_not_pinned' };
    return {
        ...verified,
        accepted: true,
        reason: null,
...
export function verifyModelToMatterEffect(effect, opts = {}) {
    ...
    if (!isObject(effect) || effect['@version'] !== M2M_EFFECT_VERSION)
        return output({ reason: 'unsupported_version' });
 ...
 */
export async function verifyModelToMatterEffectV2(effect, opts = {}) {
    const output = (overrides = {}) => ({
        verified: false, accepted: false, establishes_physical_truth: false, reason: null, ...overrides,
    });
...
export async function verifyModelToMatterEffectAny(effect, opts = {}) {
    if (isObject(effect) && effect['@version'] === M2M_EFFECT_V2_VERSION) {
 ...
 */
export function verifyModelToMatterOutcome(input, opts = {}) {
    ...
    if (!effect.accepted)
        return refuse('executor_effect_not_verified', { effect });
