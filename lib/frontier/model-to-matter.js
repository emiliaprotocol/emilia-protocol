...
function assertEvidenceBody(body) {
    ...
    if (body['@version'] !== M2M_EVIDENCE_VERSION)
        throw new Error(`evidence @version must be ${M2M_EVIDENCE_VERSION}`);
...
export function verifyModelToMatterEvidence(artifact, opts = {}) {
    ...
    try {
        assertEvidenceBody(unsigned(artifact));
    }
    catch {
...
export async function evaluateRegisteredModelToMatterPresentation(input = {}) {
    ...
    try {
        base = await evaluateRegisteredPresentation(challenge, graph, profile, {
            ...
            verifierFactory: ({ as_of }) => graphVerifiers(action, profile, as_of, revokedEvidenceDigests, retiredPhysicalMeasurementDigests),
            as_of: input.as_of,
...
function assertEffectBody(body) {
    ...
    if (body['@version'] !== M2M_EFFECT_VERSION)
        throw new Error(`effect @version must be ${M2M_EFFECT_VERSION}`);
    assertDigest(body.action_digest, 'action_digest');
...
export function verifyModelToMatterEffect(effect, opts = {}) {
    ...
    try {
        assertEffectBody(unsigned(effect));
    }
    catch {
