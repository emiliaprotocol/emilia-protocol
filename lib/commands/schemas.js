...
function requireField(obj, field, label) {
  if (!obj[field] && obj[field] !== 0 && obj[field] !== false) {
    throw new ProtocolWriteError(`${label || field} is required`, 'VALIDATION_ERROR');
  }
}
...
function requireString(obj, field, label) {
  ...
  if (typeof obj[field] !== 'string') {
    throw new ProtocolWriteError(`${label || field} must be a string`, 'VALIDATION_ERROR');
  }
}

function requireArray(obj, field, label) {
  if (!Array.isArray(obj[field]) || obj[field].length === 0) {
    throw new ProtocolWriteError(`${label || field} must be a non-empty array`, 'VALIDATION_ERROR');
  }
}
...
export function validateConfirmReceipt(input) {
  ...
  if (typeof input.confirm !== 'boolean') {
    throw new ProtocolWriteError('confirm must be a boolean', 'VALIDATION_ERROR');
  }
}
...
export function validateAppealDispute(input) {
  ...
  if (input.reason && input.reason.trim().length < 10) {
    throw new ProtocolWriteError('Appeal reason must be at least 10 characters', 'VALIDATION_ERROR');
  }
}
