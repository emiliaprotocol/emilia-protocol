// Generated from scoring.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
/**
...
export function computeReceiptComposite(receipt) {
    ...
    for (const [signal, weight] of Object.entries(EMILIA_WEIGHTS)) {
        if (signal === 'consistency')
            continue; // consistency is entity-level, not receipt-level
...
export function computeSubmitterWeight(submitterScore, submitterEstablished = false) {
    ...
    if (!submitterEstablished)
        return 0.1;
    if (submitterScore == null || isNaN(submitterScore))
        return 0.5;
    ...
    if (claims.price_honored != null) {
        if (typeof claims.price_honored === 'boolean') {
            ...
            if (quoted != null && charged != null && quoted > 0) {
                ...
                if (overcharge <= 0)
                    scores.price_integrity = 100;
                else if (overcharge <= 0.02)
                    scores.price_integrity = 90;
                else if (overcharge <= 0.05)
                    scores.price_integrity = 70;
                else if (overcharge <= 0.10)
                    scores.price_integrity = 40;
                else
                    scores.price_integrity = 10;
            }
        }
    }
