...
// Generated from privacy.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
/**
...
function assertExactFields(value, allowed) {
    for (const field of Object.keys(value)) {
        if (!allowed.has(field))
            throw new Error(`privacy profile forbids artifact field: ${field}`);
    }
}
function assertPrivacyKeyId(value) {
    if (typeof value !== 'string' || !TOKEN_RE.test(value))
        throw new Error('privacy_key_id must be a stable token');
}
...
function assertInstant(value, label) {
    if (!Number.isFinite(canonicalUtcMs(value)))
...
function assertSafeAppealUrl(value) {
    ...
    try {
        ...
        throw new Error('appeal_url must be a generic HTTPS route');
    }
 ...
 * @param {string} [options.patientIdentifier]
 * @param {string} [options.relyingPartyId]
 * @param {string} [options.privacyKeyId]
 * @param {Buffer|Uint8Array} [options.sectorSecret]
 * @returns {string}
 */
export function derivePairwisePatientRef({ patientIdentifier, relyingPartyId, privacyKeyId, sectorSecret } = {}) {
    if (typeof patientIdentifier !== 'string' || patientIdentifier.length === 0)
        throw new Error('patientIdentifier is required');
    if (typeof relyingPartyId !== 'string' || relyingPartyId.length === 0)
...
export function commitRxEvidence({ evidenceType, record, privacyKeyId, sectorSecret } = {}) {
    if (typeof evidenceType !== 'string' || !TOKEN_RE.test(evidenceType))
        throw new Error('evidenceType must be a stable token');
    assertPrivacyKeyId(privacyKeyId);
    if (record === undefined)
...
export function assertRxArtifactPrivacy(body) {
    ...
    if (!allowed)
        throw new Error('unsupported Rx artifact type');
    assertExactFields(body, allowed);
    if (!SHA256_RE.test(body.action_hash || ''))
        throw new Error('action_hash must be sha256');
    assertPrivacyKeyId(body.privacy_key_id);
    assertInstant(body.issued_at, 'issued_at');
    if (type === 'EP-RX-CONSENT-v1') {
        if (!RX_PAIRWISE_REF_RE.test(body.subject_ref || ''))
            throw new Error('subject_ref must be pairwise');
        if (!RX_PRIVATE_DIGEST_RE.test(body.consent_digest || ''))
            throw new Error('consent_digest must be a keyed commitment');
    }
    else if (type === 'EP-RX-CLINICAL-v1') {
        if (!RX_PRIVATE_DIGEST_RE.test(body.evidence_digest || ''))
            throw new Error('evidence_digest must be a keyed commitment');
        if (typeof body.criteria !== 'string' || !TOKEN_RE.test(body.criteria))
            throw new Error('criteria must be a coded token');
    }
    else if (type === 'EP-RX-BENEFIT-v1') {
        if (!SHA256_RE.test(body.policy_hash || ''))
            throw new Error('policy_hash must be sha256');
    }
    else {
        if (!RX_PRIVATE_DIGEST_RE.test(body.reason_digest || ''))
            throw new Error('reason_digest must be a keyed commitment');
        if (typeof body.reason_code !== 'string' || !TOKEN_RE.test(body.reason_code))
            throw new Error('reason_code must be a coded token');
 ...
 * @param {*} [options.packet]
 * @param {*} [options.result]
 * @param {number} [options.now]
 * @param {Buffer|Uint8Array} [options.privacyKey]
 * @param {string} [options.privacyKeyId]
...
export function buildPrivateRxAppealBundle({ challenge, packet, result, now = Date.now(), privacyKey, privacyKeyId, retentionDays = 30 } = {}) {
    ...
    if (!/^rx_(?:do_not_rely_[a-z_]+|rely)$/.test(result.verdict || ''))
        throw new Error('result verdict is not closed');
    if (result.determination !== 'approve' && result.determination !== 'deny')
        throw new Error('result determination must be approve or deny');
    assertPrivacyKeyId(privacyKeyId);
    const action = packet.action && typeof packet.action === 'object' && !Array.isArray(packet.action) ? packet.action : {};
    if (!ACTION_TOKEN_RE.test(action.action_type || ''))
        throw new Error('action_type must be a stable token');
    if (!SHA256_RE.test(action.action_hash || ''))
        throw new Error('action_hash must be sha256');
    if (action.policy_hash != null && !SHA256_RE.test(action.policy_hash))
        throw new Error('policy_hash must be sha256');
...
export function evaluateRxDisclosure({ bundle, profile, now = Date.now() } = {}) {
    ...
    try {
        ...
        for (const value of Object.values(bundle.evidence_refs)) {
            if (value !== null && !RX_PRIVATE_DIGEST_RE.test(value || ''))
                throw new Error('bundle evidence reference is not keyed');
        }
        ...
        if (expectedDigest !== bundle.bundle_digest)
    ...
    if (nowMs >= purgeAfter)
        return deny('refuse_expired', 'bundle is at or past its purge boundary');
