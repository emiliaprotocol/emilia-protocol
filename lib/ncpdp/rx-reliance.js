...
export function verifyRxArtifact(artifact, opts = {}) {
  ...
  if (Number.isFinite(opts.maxStalenessSec)) {
    const at = artifact.issued_at ? Date.parse(artifact.issued_at) : NaN;
    if (!Number.isFinite(opts.now) || opts.maxStalenessSec < 0 || Number.isNaN(at) || at > opts.now || (opts.now - at) > opts.maxStalenessSec * 1000) {
...
export { commitRxEvidence, derivePairwisePatientRef, RX_PRIVACY_VERSION };
