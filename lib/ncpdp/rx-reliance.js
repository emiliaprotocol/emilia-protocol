...
export function evaluateRxReliance({ challenge, packet, now } = {}, opts = {}) {
  ...
  if (req.benefit_policy_hash) {
    ...
    if (benefitArtifact.policy_hash !== req.benefit_policy_hash) { checks.benefit = 'policy_mismatch'; return deny('rx_do_not_rely_policy_mismatch', 'the RTBP benefit check cites a different formulary policy than the challenge', { base: base.verdict }); }
