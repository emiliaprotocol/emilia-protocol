...
// Generated from rx-reliance.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
/**
 * @license Apache-2.0
...
import { assertRxArtifactPrivacy, assertRxSignedArtifactPrivacy, buildPrivateRxAppealBundle, commitRxEvidence, derivePairwisePatientRef, evaluateRxDisclosure, RX_DISCLOSURE_PROFILE_VERSION, RX_DISCLOSURE_VERDICTS, RX_PRIVATE_BUNDLE_VERSION, RX_PRIVACY_VERSION, } from './privacy.js';
...
/**
 * Sign an Rx evidence artifact (patient consent / clinical evidence / benefit
 * status / signed denial). The body carries only privacy-safe commitments and
 * policy digests, never PHI.
...
export function signRxArtifact(body, privateKey) {
    ...
    if (publicKeyObject.asymmetricKeyType !== 'ed25519')
        throw new Error('Rx artifacts require an Ed25519 signing key');
...
export function verifyRxArtifact(artifact, opts = {}) {
    ...
    if (!artifact || typeof artifact !== 'object')
        return { ...out, reason: 'absent' };
    if (opts.expectType && artifact['@type'] !== opts.expectType)
        return { ...out, reason: 'wrong_type' };
    ...
    try {
        ...
        return { ...out, reason: 'privacy_profile_violation' };
    }
    const { signature: _s, ...body } = artifact;
    let digest;
    try {
        digest = `sha256:${sha256hex(artifactSigningBytes(body))}`;
    }
    catch {
        return { ...out, reason: 'uncanonicalizable' };
    }
    if (digest !== sig.digest)
        return { ...out, reason: 'digest_mismatch' };
    if (opts.expectActionHash && artifact.action_hash !== opts.expectActionHash)
        return { ...out, reason: 'action_binding_mismatch' };
    let ok = false;
    try {
        ...
        if (pk.asymmetricKeyType !== 'ed25519')
            return { ...out, reason: 'signature_invalid' };
    ...
    if (pinned.length === 0 || !pinned.includes(sig.public_key))
        return { ...out, reason: 'issuer_key_not_pinned' };
...
export const RX_ARTIFACT_V2_REQUIRED_ALGORITHMS = Object.freeze(['Ed25519', 'ML-DSA-65']);
...
function artifactV2SigningBytes(unsigned, requiredAlgorithms = RX_ARTIFACT_V2_REQUIRED_ALGORITHMS) {
    ...
    return Buffer.from(RX_ARTIFACT_V2_DOMAIN + canonicalize({ ...unsigned, required_algorithms: [...requiredAlgorithms] }), 'utf8');
}
/**
 ...
 * never produced.
 */
export async function signRxArtifactV2(body, signer) {
    if (!body || typeof body !== 'object' || typeof body['@type'] !== 'string') {
        throw new Error('artifact body needs an @type');
    }
    assertRxArtifactPrivacy(body);
 ...
 */
export async function verifyRxArtifactV2(artifact, opts = {}) {
    const out = { verified: false, accepted: false, reason: null };
    if (!artifact || typeof artifact !== 'object')
        return { ...out, reason: 'absent' };
    if (opts.expectType && artifact['@type'] !== opts.expectType)
    ...
    let bytes;
    try {
        bytes = artifactV2SigningBytes(body);
    }
    catch {
        return { ...out, reason: 'uncanonicalizable' };
    }
...
export function evaluateRxReliance({ challenge, packet, now } = {}, opts = {}) {
    ...
    if (!challenge || challenge['@type'] !== RX_CHALLENGE_VERSION)
        return deny('rx_do_not_rely_missing_prescriber_authority', 'no pinned EP-RX-EVIDENCE-CHALLENGE-v1');
    if (!packet || challenge && packet['@type'] !== RX_PACKET_VERSION)
        return deny('rx_do_not_rely_missing_prescriber_authority', 'no EP-RX-RELIANCE-PACKET-v1 supplied');
    if (!determination)
        return deny('rx_do_not_rely_malformed_packet', 'packet determination must be approve or deny');
    ...
    if (challenge.required === null)
        return deny('rx_do_not_rely_malformed_packet', 'challenge required profile must not be null');
    ...
    if (determination === 'deny') {
        if (req.signed_denial_required !== false) {
            ...
            if (!d.accepted)
                return deny('rx_do_not_rely_signed_denial_required', `denial reason not signed/accepted: ${d.reason}`, { base: base.verdict });
        }
    ...
    if (req.patient_consent) {
        ...
        if (!c.accepted)
            return deny('rx_do_not_rely_missing_patient_consent', `patient consent not present/accepted: ${c.reason}`, { base: base.verdict });
    }
    if (req.clinical_evidence) {
        ...
        if (!c.accepted)
            return deny('rx_do_not_rely_missing_clinical_evidence', `diagnosis/lab evidence not present/accepted: ${c.reason}`, { base: base.verdict });
    }
    if (req.benefit_policy_hash) {
        const b = verifyRxArtifact(packet.benefit_check, {
            expectType: 'EP-RX-BENEFIT-v1',
            expectActionHash: actionHash,
            pinnedKeys: challenge.accepted_benefit_keys || [],
            now: nowMs,
            maxStalenessSec: req.benefit_freshness_sec,
        });
        if (!b.accepted) {
            checks.benefit = 'stale';
            return deny('rx_do_not_rely_stale_benefit', `RTBP benefit/formulary check is missing, untrusted, or stale: ${b.reason}`, { base: base.verdict });
        }
        const benefitArtifact = packet.benefit_check;
        if (benefitArtifact.policy_hash !== req.benefit_policy_hash) {
            checks.benefit = 'policy_mismatch';
            return deny('rx_do_not_rely_policy_mismatch', 'the RTBP benefit check cites a different formulary policy than the challenge', { base: base.verdict });
        }
...
export { commitRxEvidence, derivePairwisePatientRef, evaluateRxDisclosure, RX_DISCLOSURE_PROFILE_VERSION, RX_DISCLOSURE_VERDICTS, RX_PRIVACY_VERSION, };
