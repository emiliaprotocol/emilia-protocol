...
function keyedDigest(secret: unknown, purpose: string, value: unknown): string {
...
function assertSafeAppealUrl(value: string): void {
  let url: URL;
...
export function derivePairwisePatientRef({ patientIdentifier, relyingPartyId, privacyKeyId, sectorSecret }: {
  ...
  privacyKeyId?: string;
  sectorSecret?: Buffer | Uint8Array;
} = {}): string {
...
export function assertRxArtifactPrivacy(body: any): boolean {
  assertPlainObject(body, 'artifact');
  const type = body['@type'];
  const allowed = (ARTIFACT_FIELDS as Record<string, Set<string>>)[type];
...
export function assertRxSignedArtifactPrivacy(artifact: any): boolean {
...
function safeArtifact(artifact: any): any {
  if (artifact == null) return null;
  assertRxSignedArtifactPrivacy(artifact);
  return structuredClone(artifact);
}

function projectChecks(checks: any): Record<string, unknown> {
...
function finiteNonNegative(value: any): number | null {
 ...
 * @returns {Object}
 */
export interface RxDisclosureResult {
  verdict: string;
  disclose: boolean;
  reasons: string[];
...
export function evaluateRxDisclosure({ bundle, profile, now = Date.now() }: {
  ...
  const deny = (verdict: string, reason: string): RxDisclosureResult => ({ verdict, disclose: false, reasons: [reason], checks });
  ...
  try {
    ...
    if (Object.values(bundle.checks as Record<string, any>).some((value: any) => !PROJECTED_CHECK_VALUES.has(value)
      && !(typeof value === 'string' && /^rx_(?:do_not_rely_[a-z_]+|rely)$/.test(value)))) {
      throw new Error('bundle checks are not closed values');
    }
    for (const value of Object.values(bundle.evidence_refs as Record<string, any>)) {
  ...
  for (const artifact of Object.values(bundle.signed_artifacts as Record<string, any>)) {
