...
/** Shape of an entry from EP_ERROR_CODES (lib/errors/taxonomy.ts). */
export interface EPErrorCode {
  code: string;
  status: number;
  message: string;
}
 ...
 *
 * @param errorCode  An entry from EP_ERROR_CODES (must have code, status, message).
 * @param detail  Optional human-readable detail for this specific occurrence.
 * @param extras  Optional additional fields merged into the error body
 *                (e.g. { field: 'entity_id' } for validation errors).
 */
export function epError(
  errorCode: EPErrorCode,
 ...
 */
export function epMissingField(fieldName: string): NextResponse {
