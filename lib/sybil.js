...
export async function runReceiptFraudChecks(supabase, entityId, submittedBy) {
  ...
  // Closed loops are warnings (bidirectional scoring is legitimate in some cases)
  // Velocity spikes block the receipt
  const blocked = flags.some(f => f.reason === 'velocity_spike');

  if (flags.length > 0) {
    // Log the fraud flags for later analysis
    try {
...
/**
 * Compute effective establishment status.
 * Requires BOTH receipt count >= 5 AND unique submitters >= 3.
 * This replaces the simple receipt-count check.
 */
