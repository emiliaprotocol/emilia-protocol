 ...
 * @param {import('@supabase/supabase-js').SupabaseClient} supabase - Supabase service client
...
export async function checkRegistrationLimits(supabase, ownerId) {
  ...
  if (dailyError) throw new Error(`registration daily-count unavailable: ${dailyError.message || dailyError.code || 'database error'}`);
  ...
  const { count: totalCount, error: totalError } = await supabase
    ...
    .select('id', { count: 'exact', head: true })
    .eq('owner_id', ownerId);
  if (totalError) throw new Error(`registration total-count unavailable: ${totalError.message || totalError.code || 'database error'}`);
 ...
 * @returns {Promise<{ flagged: boolean, reason?: string, detail?: string }>}
 */
export async function detectClosedLoop(supabase, entityId, submittedBy) {
  ...
  const { data: reverseReceipts, error } = await supabase
    .from('receipts')
    .select('id')
    .eq('entity_id', submittedBy)
    .eq('submitted_by', entityId)
    .limit(1);
  if (error) throw new Error(`closed-loop check unavailable: ${error.message || error.code || 'database error'}`);
...
export async function detectVelocitySpike(supabase, submittedBy) {
  ...
  const { count: hourlyCount, error } = await supabase
    .from('receipts')
    .select('id', { count: 'exact', head: true })
    .eq('submitted_by', submittedBy)
    .gte('created_at', hourAgo);
  if (error) throw new Error(`velocity check unavailable: ${error.message || error.code || 'database error'}`);
...
export async function analyzeReceiptGraph(supabase, entityId) {
  ...
  if (submittersError) throw new Error(`receipt-graph check unavailable: ${submittersError.message || submittersError.code || 'database error'}`);
  ...
  if (uniqueSubmitters >= 2 && uniqueSubmitters <= 3 && totalReceipts >= 20) {
    ...
    if (submitterIds.length > 0) {
      const { count: intraGroupReceipts, error: intraGroupError } = await supabase
        .from('receipts')
        .select('id', { count: 'exact', head: true })
        .in('entity_id', submitterIds)
        .in('submitted_by', [entityId, ...submitterIds]);
      if (intraGroupError) throw new Error(`cluster check unavailable: ${intraGroupError.message || intraGroupError.code || 'database error'}`);
 ...
 * @returns {Promise<{ updated: number, failed: number, partial_failure?: boolean, failed_ids?: string[] }>}
 */
