  ...
  try {
    ...
    for (const batch of chunk(ids, 50)) {
      try {
        ...
        failed += batch.length;
        console.error('Retroactive weight update chunk failed:', e.message, { entityId, submittedBy, newWeight, batchSize: batch.length });
      }
    }
  } catch (e) {
    console.error('Retroactive graph weight fetch failed:', e.message);
...
export async function runReceiptFraudChecks(supabase, entityId, submittedBy) {
  ...
  if (graphWeight < 1.0) {
    ...
    if (retroResult.failed > 0) {
      console.error('Partial retroactive weight failure:', retroResult);
    }
  }
