...
// Integer-scaled confidence weights for deterministic arithmetic in computeWeightedVote.
// Values are CONFIDENCE_WEIGHT * 10 — exact integers, no floating-point rounding.
// Division by 10 in output restores the [0, 1] scale for API compatibility.
const CONFIDENCE_WEIGHT_INT = {
  pending:      0,
  insufficient: 0,
  provisional:  3,
  emerging:     6,
...
// Integer behavior scores for deterministic sentiment computation.
// These are the exact rational equivalents of the previous float formula
// `(raw + 0.2) / 1.2 * 100`, computed as integers to avoid repeating-decimal
// IEEE 754 accumulation errors (e.g., 0.7/1.2 = 0.5833... is non-terminating).
//
// Derivation (raw values from original BEHAVIOR_SENTIMENT):
//   completed:         (1.0  + 0.2) / 1.2 * 100 = 100
//   retried_same:      (0.5  + 0.2) / 1.2 * 100 =  70  (exact: 70/120 * 120 = 70)
//   retried_different: (0.15 + 0.2) / 1.2 * 100 =  29  (35/120 * 100 ≈ 29.17 → floor to 29)
//   abandoned:         (0.0  + 0.2) / 1.2 * 100 =  17  (20/120 * 100 ≈ 16.67 → ceil  to 17)
//   disputed:          (-0.2 + 0.2) / 1.2 * 100 =   0
//
// Threshold translations (sentiment < 0.35 → uphold, > 0.65 → dismiss):
//   0.35 * 100 = 35  →  score < 35  ⟹ uphold
//   0.65 * 100 = 65  →  score > 65  ⟹ dismiss
const BEHAVIOR_SCORE_INT = {
  completed:         100,
  retried_same:       70,
  retried_different:  29,
  abandoned:          17,
  disputed:            0,
};
...
export async function findVouchers(disputedEntityId, supabase) {
  ...
  // confidence tier DESC (integer weight, not float score — avoids IEEE 754
  // non-determinism from accumulated float comparisons), then entity_id UUID
  // lexicographically as a final deterministic tiebreaker.
  vouchers.sort((a, b) => {
    if (b.receipt_count !== a.receipt_count) return b.receipt_count - a.receipt_count;
    const wA = CONFIDENCE_WEIGHT_INT[a.confidence] ?? 0;
    const wB = CONFIDENCE_WEIGHT_INT[b.confidence] ?? 0;
 ...
 * @param {object} supabase - Supabase client
 * @returns {Promise<{ sentiment: number|null, receipt_count: number, behavior_breakdown: object }>}
...
async function fetchVoucherSentiment(voucherEntityId, disputedEntityId, supabase) {
  ...
  // Integer accumulators — eliminates IEEE 754 repeating-decimal accumulation errors.
  // Primary weight = 10 (represents 1.0x); secondary weight = 4 (represents 0.4x).
  // BEHAVIOR_SCORE_INT values are pre-computed integers in [0, 100].
  // sentiment = numerator / denominator / 100  →  [0.0, 1.0]
  let numerator = 0;
  let denominator = 0;
  ...
  for (const receipt of (submittedAbout || [])) {
    ...
    const score = BEHAVIOR_SCORE_INT[behavior];
  ...
  if (denominator === 0 || receiptCount === 0) {
    // No behavioral data — sentiment is unknown, don't include in vote
    return { sentiment: null, receipt_count: 0, behavior_breakdown: {} };
  }

  // Divide integers, then normalize to [0, 1]. Both operations are on integers
  // or exact rational results — deterministic for any identical input set.
  const sentiment = (numerator / denominator) / 100;
 ...
 *   uphold_fraction: number|null,  // weighted_uphold / (weighted_uphold + weighted_dismiss)
 * }}
 */
export function computeWeightedVote(voucherSentiments) {
  ...
  for (const { voucher, sentiment } of voucherSentiments) {
    ...
    // Use integer confidence weight (0, 3, 6, or 10) for deterministic accumulation.
    // Dividing by 10 in the output object restores the [0, 1] scale for API compat.
    const w = CONFIDENCE_WEIGHT_INT[voucher.confidence] ?? 0;

    // Sentiment thresholds (scaled: BEHAVIOR_SCORE_INT < 35 → uphold, > 65 → dismiss):
    // These are equivalent to the original float thresholds 0.35 / 0.65 since
    // sentiment = BEHAVIOR_SCORE_INT-derived value / 100.
  ...
  if (totalDeciding === 0 || participatingCount < MIN_VOUCHERS_FOR_DECISION) {
    return {
      ...
      confidence: 0,
      // Divide integer weights by 10 to restore [0, 1] scale for API compatibility
      weighted_uphold: Math.round((weightedUphold / 10) * 1000) / 1000,
      weighted_dismiss: Math.round((weightedDismiss / 10) * 1000) / 1000,
      weighted_abstain: Math.round((weightedAbstain / 10) * 1000) / 1000,
  ...
  /** @type {'uphold_dispute'|'dismiss_dispute'|'inconclusive'} */
  let recommendation;
...
export async function adjudicateDispute(disputeId, supabaseOverride?) {
  ...
  if (vouchers.length === 0) {
    ...
    /** @type {{
     *   recommendation: 'inconclusive'|'uphold_dispute'|'dismiss_dispute',
     *   confidence: number,
     *   voucher_count: number,
     *   participating_count: number,
     *   weighted_vote: { uphold_fraction: number|null, weighted_uphold: number, weighted_dismiss: number, weighted_abstain: number },
     *   graph_note: string,
     * }} */
    const result = {
      recommendation: 'inconclusive',
...
async function persistAdjudicationResult(supabase, disputeId, result, triggeredBy) {
  try {
    ...
    if (!isMissingColumn) {
      logger.error('Adjudication result persistence failed:', err.message);
    } else {
      logger.warn('Adjudication columns not yet present (run migration 025_dispute_adjudication.sql)');
    }
  }
}
