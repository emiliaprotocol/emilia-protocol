...
export async function handleSignoffDecision(request, params, decision) {
  try {
    ...
    const parsed = await readLimitedJson(request, MAX_SIGNOFF_DECISION_BYTES, { invalidValue: {} });
    ...
    if (decision === 'approved' && requiredAssurance === 'A') {
      return epProblem(
        403,
        'insufficient_assurance',
        'This receipt requires Class-A WebAuthn/passkey approval; bearer-key signoff is not sufficient',
      );
    }
