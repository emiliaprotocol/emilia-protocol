...
/**
 * Error thrown when a protocol write operation fails.
 *
 * @extends Error
 * @property {string} name - Always 'ProtocolWriteError'
 * @property {string} code - Machine-readable error code (e.g. 'VALIDATION_ERROR', 'EVENT_PERSISTENCE_FAILED')
 * @property {number} status - HTTP status code for the error (default 400)
 */
export class ProtocolWriteError extends Error {
  /**
   * @param {string} message - Human-readable error description
   * @param {string} [code='PROTOCOL_WRITE_ERROR'] - Machine-readable error code
...
const VALIDATORS = {
  ...
  [COMMAND_TYPES.CONSUME_HANDSHAKE_BINDING](command) {
    ...
    if (!input) throw new ProtocolWriteError('input is required', 'VALIDATION_ERROR');
    if (!input.handshake_id) throw new ProtocolWriteError('input.handshake_id is required', 'VALIDATION_ERROR');
    if (!input.consumed_by) throw new ProtocolWriteError('input.consumed_by is required', 'VALIDATION_ERROR');
    if (!input.consumed_for) throw new ProtocolWriteError('input.consumed_for is required', 'VALIDATION_ERROR');
  },
  ...
  [COMMAND_TYPES.EXPIRE_RECEIPTS](command) {
    ...
    if (!Array.isArray(input.receipt_ids) || input.receipt_ids.length === 0) {
      throw new ProtocolWriteError('input.receipt_ids must be a non-empty array', 'VALIDATION_ERROR');
    }
  },
  ...
  [COMMAND_TYPES.ESCALATE_DISPUTES](command) {
    ...
    if (!Array.isArray(input.dispute_ids) || input.dispute_ids.length === 0) {
      throw new ProtocolWriteError('input.dispute_ids must be a non-empty array', 'VALIDATION_ERROR');
    }
  },
  ...
  [COMMAND_TYPES.EXPIRE_CONTINUITY_CLAIMS](command) {
    ...
    if (!Array.isArray(input.continuity_ids) || input.continuity_ids.length === 0) {
      throw new ProtocolWriteError('input.continuity_ids must be a non-empty array', 'VALIDATION_ERROR');
    }
  },
};
