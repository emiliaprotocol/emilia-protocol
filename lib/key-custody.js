...
// Generated from key-custody.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
//
...
export const HYBRID_CUSTODY_ALGORITHMS = Object.freeze(['Ed25519', 'ML-DSA-65']);
...
export function createHybridCustodySigner({ classical, pq }) {
    ...
    const publicKeySpkiB64u = typeof classical.publicKeySpkiB64u === 'function'
        ? () => classical.publicKeySpkiB64u.call(classical)
...
export function isHybridCustodySigner(signer) {
    const s = signer;
    return !!s
 ...
 */
export async function resolveHybridPublicKeys(signer) {
...
export function resolveIssuerSigner(config = getKeyCustodyConfig()) {
    ...
    if (mode === 'kms' || mode === 'hsm') {
        ...
        if (!_registeredCustodySigner) {
            const err = new Error(`EP_KEY_CUSTODY_MODE=${mode} but no custody signer is registered. `
...
export const CUSTODY_BOUNDARY_LABELS = Object.freeze(['kms', 'hsm']);
function boundaryLabel(custody) {
    return typeof custody === 'string' && CUSTODY_BOUNDARY_LABELS.includes(custody);
}
/**
