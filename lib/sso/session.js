...
function sessionKey() {
  ...
  // lib/crypto/secret-box.js. The derivation is dev-only convenience.
  if (!explicit && process.env.NODE_ENV === 'production') {
    throw new Error('sso/session: SSO_SESSION_SECRET is required in production — refusing to sign sessions with a derived/predictable key.');
  }
  const base = explicit
 ...
 * Revocation is DEFENSE-IN-DEPTH: if the store is unavailable (not yet migrated /
 * DB hiccup) the check fails OPEN so signature+expiry remain the primary gate and
 * a transient DB outage can't lock everyone out.
 */
