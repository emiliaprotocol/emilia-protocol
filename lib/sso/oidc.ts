...
import { safePinnedFetch } from './pinned-fetch.js';
...
export async function exchangeCode({
  ...
  // exchangeCode has no @param JSDoc, so `fetchImpl`'s type comes from its default
  // value (safePinnedFetch). That function's own Promise executor return isn't
  // inferrable by tsc, so the import's declared return collapses to `unknown` —
...
> {
  ...
  try {
    ...
    // `sub` is REQUIRED by OIDC Core, but jose only verifies sig/iss/aud/exp, so
    // a signed token without it would validate here and mint a session with an
    // undefined subject. Refuse it — the subject is the stable identity anchor.
    if (!payload.sub) {
      return { valid: false, error: 'ID token missing required sub claim' };
    }
