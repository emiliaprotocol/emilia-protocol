...
export interface SsoIdentity {
  ...
  email?: string;
  protocol: 'saml' | 'oidc';
  directory?: { matched: boolean; active: boolean; user_id?: string };
}
 ...
 */
export async function verifySession(token?: string | null): Promise<jose.JWTPayload | null> {
...
async function isSessionRevoked(payload: jose.JWTPayload): Promise<boolean> {
...
export async function revokeSession(
  jti: string,
  { subject = null, tenant = null, expiresAt = null }: { subject?: string | null; tenant?: string | null; expiresAt?: string | null } = {},
): Promise<boolean> {
  if (!jti) return false;
  try {
    const { getServiceClient } = await import('@/lib/supabase');
 ...
 */
export async function revokeAllSessionsForSubject(subject: string | null | undefined, tenant?: unknown): Promise<boolean> {
...
export async function readSessionFromRequest(request: Request): Promise<jose.JWTPayload | null> {
