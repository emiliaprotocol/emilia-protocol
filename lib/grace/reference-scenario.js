...
// Generated from reference-scenario.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
...
import { buildCurtailmentControlledAction, buildGraceOutcomePredictions, buildCurtailmentPresentation, createCurtailmentAction, executeGraceCurtailment, graceDigest, } from './mobile-grid.js';
import { predictedEffectsDigest } from '../../packages/verify/effect-predicates.js';
import { createCosaReferenceActuator, createEphemeralMemoryStore, createReferenceMeter, verifyReferenceMeterStatement, } from './reference-adapters.js';
...
export function createGraceReferenceRuntime() {
    if (process.env.NODE_ENV === 'production') {
        throw new Error('createGraceReferenceRuntime is a demo-only harness over process-local consumption '
...
export async function executeGraceReferenceInput(input, state, overrides = {}) {
...
export async function runGraceReferenceScenario() {
    ...
    const substitution = await executeGraceReferenceInput({ ...input, action: changed }, createGraceReferenceRuntime());
