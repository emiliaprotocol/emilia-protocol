...
// Generated from forecast-evidence.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
/**
...
function stepMilliseconds(value) {
...
export function forecastDigest(value) {
...
export function validateForecastEvidence(value) {
    const reasons = [];
    const body = unsignedBody(value);
    if (!exact(body, BODY_KEYS))
        return { valid: false, reasons: ['forecast_shape_invalid'], body: null };
    try {
        canonical(body);
    }
    catch {
        reasons.push('forecast_not_canonicalizable');
    }
    ...
    if (exact(body.forecast, FORECAST_KEYS)) {
        ...
        if (exact(body.forecast.action_window, WINDOW_KEYS)) {
            ...
            if (Array.isArray(body.forecast.points) && body.forecast.points.length > 0
                && body.forecast.points.every((point) => exact(point, POINT_KEYS)
                    && decimal(point.p10) && decimal(point.p50) && decimal(point.p90))) {
                const inWindow = body.forecast.points.filter((point) => instant(point.at)
                ...
                if (inWindow.length > 0 && decimal(window.p10) && decimal(window.p50) && decimal(window.p90)) {
                    for (const quantile of ['p10', 'p50', 'p90']) {
                        const minimum = inWindow.reduce((current, point) => (compareDecimal(point[quantile], current) < 0 ? point[quantile] : current), inWindow[0][quantile]);
 ...
 */
export function assessForecastForGrace(value, { policy, expected_action_digest, expected_action_window, action_target_mw, now, }) {
    const checked = validateForecastEvidence(value);
    const reasons = [...checked.reasons];
    if (!validateForecastPolicy(policy))
        reasons.push('forecast_policy_invalid');
    ...
    if (decimal(policy.max_interval_width_mw) && Array.isArray(body.forecast.points)) {
        ...
        add(reasons, body.forecast.points.every((point) => decimal(point.p10) && decimal(point.p90)
    ...
    try {
        evidenceDigest = forecastDigest(value);
    }
    catch { /* structural reason already recorded */ }
    return {
...
export function buildTimesFmForecastEvidence({ forecast_id, generated_at, action_digest, action_window, source_id, source_observations, source_frequency, model_version, checkpoint_id, checkpoint_digest, adapter_version, adapter_config, points, step, backtest, } = {}) {
    ...
    const ordered = points?.filter((point) => instant(point?.at)
    ...
    if (!Array.isArray(ordered) || ordered.length === 0)
        throw new TypeError('forecast points must cover the action window');
    const minimum = (quantile) => ordered.reduce((current, point) => compareDecimal(point[quantile], current) < 0 ? point[quantile] : current, ordered[0][quantile]);
    const body = {
    ...
    if (!checked.valid)
        throw new TypeError(`invalid TimesFM forecast evidence: ${checked.reasons.join(',')}`);
    return body;
}
/**
 ...
 */
export function reconcileForecastObservation(value, { observed_value_mw, observation_digest, observed_at, } = {}) {
    const checked = validateForecastEvidence(value);
