...
export function checkOrderWithinEnvelope(order: any, envelope: any, opts: any = {}): { within: boolean, violations: string[] } {
  ...
  if (!validEnvelopeWindow) violations.push('envelope: invalid participation window');
...
export async function runSettlementOnce(
  ...
  if (reserved !== true) return { settled: false, key, reason: 'settlement_already_consumed' };
