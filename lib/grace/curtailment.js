...
// Generated from curtailment.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
/**
...
// Use the same calendar round-trip validation as the Model-to-Matter profile.
// Date.parse alone normalizes impossible dates and accepts timezone-less input.
// Canonical GRACE actions impose a narrower UTC/millisecond format separately.
...
export function checkOrderWithinEnvelope(order, envelope, opts = {}) {
    const violations = [];
    if (envelope?.['@version'] !== FLEX_ENVELOPE_VERSION)
        violations.push('envelope: unknown version');
    ...
    const requiredBound = (name) => {
        ...
        if (!Number.isFinite(v) || v <= 0) {
            violations.push(`envelope: ${name} missing or unparseable (fail closed)`);
            return null;
        }
        return v;
    };
    ...
    const spentOf = (name) => {
        ...
        const v = Number(raw);
        if (!Number.isFinite(v) || v < 0) {
            violations.push(`opts: ${name} unparseable (fail closed)`);
            return null;
        }
        return v;
    };
    ...
    if (!Number.isFinite(mw) || mw <= 0)
        violations.push('order: mw missing or non-positive');
    else if (maxEventMw !== null && mw > maxEventMw) {
        violations.push(`order: ${mw} MW exceeds envelope per-event cap ${maxEventMw} MW`);
    }
    const rawFloor = bounds?.min_notice_minutes;
    const floor = typeof rawFloor === 'number' || typeof rawFloor === 'string' ? Number(rawFloor) : NaN;
    const noticeMin = Number(order?.notice_minutes);
    if (!Number.isFinite(floor) || floor <= 0) {
        violations.push('envelope: min_notice_minutes missing or unparseable (fail closed)');
    }
    else if (!Number.isFinite(noticeMin) || noticeMin < floor) {
        violations.push(`order: ${noticeMin} min notice below envelope floor ${floor} min`);
    }
    const start = strictInstantMs(order?.window?.start), end = strictInstantMs(order?.window?.end);
    const eStart = strictInstantMs(bounds?.window?.start);
    const eEnd = strictInstantMs(bounds?.window?.end);
    const validEnvelopeWindow = Number.isFinite(eStart) && Number.isFinite(eEnd) && eStart < eEnd;
    if (!validEnvelopeWindow)
        violations.push('envelope: invalid participation window');
    let durationHours = null; // an invalid window is already a refusal; no budget math without it
    if (!(start < end))
        violations.push('order: invalid window');
    else {
        durationHours = (end - start) / 3600000;
        if (validEnvelopeWindow && (start < eStart || end > eEnd)) {
...
export function computeCompliance(order, meterStatement) {
    ...
    if (!Number.isFinite(ordered) || ordered <= 0)
        return { computable: false, reason: 'order lacks mw' };
...
export function settlementEntitlementKey(claim) {
    ...
    if (typeof entitlement !== 'string' || entitlement.length === 0)
        return { key: null, reason: 'entitlement_id_missing' };
    if (typeof event !== 'string' || event.length === 0)
        return { key: null, reason: 'event_id_missing' };
    if (typeof windowDigest !== 'string' || !SHA256_DIGEST.test(windowDigest))
        return { key: null, reason: 'meter_window_digest_malformed' };
...
export function checkSettlementConsumption(claim, consumedKeys) {
    if (!claim || typeof claim !== 'object')
        return { settled: false, key: null, reason: 'claim_missing' };
    if (!(consumedKeys instanceof Set))
        return { settled: false, key: null, reason: 'consumption_registry_missing' };
    const { key, reason } = settlementEntitlementKey(claim);
    if (key === null)
        return { settled: false, key: null, reason };
    if (consumedKeys.has(key))
        return { settled: false, key, reason: 'settlement_already_consumed' };
 ...
 */
export function isGraceCustodyStore(store) {
 ...
 */
export async function runSettlementOnce(claim, store, settle, { allowEphemeralState = false } = {}) {
    if (!claim || typeof claim !== 'object')
        return { settled: false, key: null, reason: 'claim_missing' };
    const { key, reason } = settlementEntitlementKey(claim);
    if (key === null)
    ...
    if (typeof settle !== 'function')
        return { settled: false, key, reason: 'settlement_effect_missing' };
    let reserved;
    try {
        reserved = await store.reserve(key);
    }
    catch {
        return { settled: false, key, reason: 'consumption_registry_unavailable' };
    }
    if (reserved !== true)
        return { settled: false, key, reason: 'settlement_already_consumed' };
    let committed = false;
    try {
        ...
        if ((await store.commit(key)) !== true)
            throw new Error('settlement_consumption_commit_failed');
        committed = true;
        ...
        if (!committed) {
            try {
                ...
                if (error && typeof error === 'object') {
                    error.consumption_error = String(commitError?.message ?? commitError);
                }
            }
        }
        throw error;
