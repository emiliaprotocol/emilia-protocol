...
// Generated from reference-adapters.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
import { GRACE_COSA_ACK_VERSION, GRACE_DISPATCH_VERSION, GRACE_METER_VERSION, graceDigest, signGraceArtifact, validateCurtailmentAction, verifyGraceArtifact, } from './mobile-grid.js';
...
export function createCosaReferenceActuator({ privateKey, keyId, actuatorId = 'cosa:reference:actuator-1', clock = () => new Date().toISOString(), } = {}) {
        ...
        async dispatch(request) {
            ...
            if (prior) {
                if (prior.request_digest !== requestDigest)
                    throw new Error('COSA idempotency key was reused for different bytes');
                return structuredClone(prior.ack);
            }
            const dispatchedAt = clock();
            if (!canonicalInstant(dispatchedAt))
                throw new Error('COSA adapter clock is not canonical UTC');
...
export function createReferenceMeter({ privateKey, keyId, meterId = 'meter:reference:facility-1', baselineMw = '64.000', complianceFactor = 0.996, clock = () => new Date().toISOString(), } = {}) {
    ...
    return {
        ...
        async observe({ action, acknowledgment, } = {}) {
            ...
            if (!canonicalInstant(observedAt))
                throw new Error('reference meter clock is not canonical UTC');
