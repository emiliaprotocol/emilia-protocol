 ...
 */
export function verifyOperatorAuth(token, opts = {}) {
  if (!token) {
  ...
  // Sensitive routes pass requireOperatorIdentity: once per-operator keys are
  // configured, the shared secret is refused there so every sensitive action
  // ties to a NAMED operator in the audit trail. Until operator keys exist,
  // legacy is still accepted so the route is never bricked (safe migration).
  if (opts.requireOperatorIdentity && getOperatorKeys().size > 0) {
 ...
 */
export function authenticateOperator(request, opts = {}) {
