...
import type { GuardActionType, GuardAmlContext } from './guard-policies.js';
...
/** Spec object each thin adapter route passes to runGuardPrecheck(). */
export interface GuardPrecheckSpec {
  /** Short slug for audit, e.g. 'gov.benefit-bank-change' */
  adapterName: string;
  /** GUARD_ACTION_TYPES.* value */
  actionType: GuardActionType;
  /** Default policy_id when body omits one */
  policyId: string;
  /** Body field that names target_resource_id */
  targetResourceField: string;
  /** Used when body.target_changed_fields omitted */
  defaultChangedFields?: string[];
  /** Default actor role when body omits */
  actorRole?: string;
}

/**
 * readLimitedJson()'s documented discriminated-union return shape
 * (lib/http/body-limit.ts). Cast to this at the call site below: that file
 * is owned by a different, concurrently-running conversion pass and its
 * exported function is not yet annotated with a real TS return type.
 */
type ReadLimitedJsonResult =
  | { ok: true; value: any }
 ...
 */
export async function runGuardPrecheck(request: Request, spec: GuardPrecheckSpec): Promise<NextResponse> {
  try {
    ...
    // resolveGuardChangedFields() (lib/guard-action-inputs.ts, owned by a
    // different conversion pass) returns `unknown` — deliberately, since it
    // hands back whatever the caller supplied unvalidated. This call-site
    // signature cast (not a persisted `any`) restores the real
    // string[]-in/string[]-out shape this adapter relies on below; the shape
    // is actually enforced immediately after by validateGuardActionInput().
    const changedFields = (resolveGuardChangedFields as (body: any, defaults: string[]) => string[])(body, spec.defaultChangedFields || []);
    ...
    // extractGuardActionDetails's `changedFields` parameter (lib/guard-action-inputs.ts)
    // is typed `unknown`, so the real string[] `changedFields` value here needs
    // no cast to pass through it.
    ...
    // resolveGuardEnforcementMode returns plain `string` (body.mode is
    // unconstrained caller input); widen the frozen enum's literal-union
    // array to string[] for the membership check below, which is exactly
    // what validates `mode` against the enum at runtime.
    if (!(Object.values(ENFORCEMENT_MODES) as string[]).includes(mode)) {
...
function buildAmlContext(body: any): GuardAmlContext | undefined {
...
function amlCounterpartyKey(name: unknown): string {
...
async function resolveAmlContext(supabase: SupabaseClient, body: any): Promise<GuardAmlContext | undefined> {
  ...
  const authoritative: number[] = (data || []).map((r: any) => Number(r.amount)).filter(Number.isFinite);
...
async function recordAmlHistory(
  supabase: SupabaseClient,
  body: any,
  spec: GuardPrecheckSpec,
  receiptId: string,
  aml: GuardAmlContext | undefined,
): Promise<boolean> {
