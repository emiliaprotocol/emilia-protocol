 ...
 * logger.js retains only bootstrap-only NODE_ENV/version reads to avoid a
 * logger/environment module cycle; next.config.js is also exempt.
 *
 ...
 *
 * @returns {{ trustedIssuerKeys: string[], govStrict: boolean, rpId: string|null, allowedOrigins:string[], expectedPolicyHash: string|null }}
 */
export function getGovVerifierConfig() {
  ...
  try {
    ...
    logger.warn(`ENV WARNING: EP_WEBAUTHN_ALLOWED_ORIGINS invalid (${e.message}), ignoring`);
  }
  return {
    trustedIssuerKeys,
...
export function getPinnedQuorumPolicies() {
  ...
  try {
    ...
    logger.warn(`ENV WARNING: EP_QUORUM_POLICIES invalid (${e.message}), ignoring`);
    return {};
  }
}
 ...
 *
 * @returns {{ key: Buffer, keyId: string }|null}
 */
 ...
 *
 * @returns {Record<string, { approver_id: string, public_key: string, key_class?: 'A'|'B' }>}
 */
 ...
 * @returns {'shadow'|'warn'|'enforce_critical'|'enforce_default'}
 */
export function authorityEnforcementMode() {
  ...
  return allowed.includes(raw) ? raw : 'shadow';
}
