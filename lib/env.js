// Generated from env.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
/**
...
function parseStrictEnvJson(raw, label) {
    ...
    if (!strict.ok)
        throw new Error(`${label} must be strict JSON: ${strict.reason}`);
...
export function getSsoOriginConfig() {
    return {
 ...
 */
export function getDemoSigningKey() {
...
export function getBlockchainConfig() {
    ...
    if (!network && !walletPrivateKey && !signingKeyId && signingMode === 'env')
        return null;
    ...
    if (trustedKeysRaw) {
        try {
            ...
            for (const [kid, val] of Object.entries(parsed)) {
                if (kid === '__proto__' || kid === 'constructor' || kid === 'prototype')
                    continue;
                if (typeof kid !== 'string' || typeof val !== 'string' || !val)
                    continue;
                clean[kid] = val;
            }
            trustedKeys = clean;
...
export function isAgentRecordSigningKeyId(value) {
...
export function getAgentRecordSigningConfig() {
    ...
    if (!isAgentRecordSigningKeyId(signingKeyId)) {
        throw Object.assign(new Error('EP_AGENT_RECORD_SIGNING_KEY_ID must use the safe Agent Record key-id shape'), { code: 'agent_record_operator_key_id_invalid' });
    }
 ...
 */
export function getAgentRecordCreationCapability() {
...
function splitKeyList(raw) {
    ...
    if (raw.trim().startsWith('[')) {
        ...
        if (!Array.isArray(parsed))
            throw new Error('trusted issuer keys must be a JSON array or comma-separated list');
...
export function getGovVerifierConfig() {
    ...
    try {
        ...
        if (isProduction())
            throw new Error(`EP_TRUSTED_ISSUER_KEYS invalid: ${e.message}`);
    ...
    try {
        ...
        if (isProduction())
            throw new Error(`EP_WEBAUTHN_ALLOWED_ORIGINS invalid: ${e.message}`);
...
export function getPinnedQuorumPolicies() {
    ...
    try {
        ...
        for (const [action, policy] of Object.entries(parsed)) {
            if (['__proto__', 'constructor', 'prototype'].includes(action))
                continue;
        ...
        if (isProduction())
            throw new Error(`EP_QUORUM_POLICIES invalid: ${e.message}`);
 ...
 */
export function getCryptoProfileId() {
 ...
 */
export function getRxPrivacyConfig() {
    const encoded = process.env.EP_RX_PRIVACY_KEY_B64U || '';
    const keyId = process.env.EP_RX_PRIVACY_KEY_ID || '';
    if (!/^[A-Za-z0-9_-]+$/.test(encoded) || !/^[A-Za-z][A-Za-z0-9._:-]{0,127}$/.test(keyId))
        return null;
    let key;
    try {
...
export function getPinnedApproverKeys() {
    ...
    try {
        ...
        if (isProduction())
            throw new Error(`EP_PINNED_APPROVER_KEYS invalid JSON: ${e.message}`);
        envLogger.warn(`ENV WARNING: EP_PINNED_APPROVER_KEYS invalid JSON (${e.message}), ignoring`);
        return {};
    }
    if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) {
        if (isProduction())
            throw new Error('EP_PINNED_APPROVER_KEYS must be a JSON object keyed by approver_key_id');
 ...
 * Default 'enforce_default': every action fails closed when authority is not
 * `authorized`, including `registry_unavailable` and a revoked authority. An
 * unset or unrecognized value is treated as 'enforce_default', so a typo or a
 * missing variable can never silently disable enforcement.
 *
 * 'shadow' and 'warn' remain available as an EXPLICIT, non-production opt-out
 * while an authority registry is being populated: they resolve real authority
 * and bind it into the receipt, but never block. Deployments that set them are
 * declaring that unauthorized actions are allowed to proceed, so they are not
 * a supported production configuration.
 */
export function authorityEnforcementMode() {
    const raw = process.env.EP_AUTHORITY_ENFORCEMENT;
    const allowed = ['shadow', 'warn', 'enforce_critical', 'enforce_default'];
...
export function getOperatorKeys() {
    ...
    try {
        ...
        if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed))
            throw new Error('EP_OPERATOR_KEYS must be an object');
        const keys = new Map();
...
export function getOperatorRoles() {
    ...
    try {
        ...
        if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed))
            throw new Error('EP_OPERATOR_ROLES must be an object');
        return new Map(Object.entries(parsed).filter(([id, role]) => id && id.length <= 128
