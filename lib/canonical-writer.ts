...
import { createReceipt, type CreateReceiptResult } from '@/lib/create-receipt';
...
/** Actor shape accepted by the dispute/report write paths (auth-projection actor). */
export interface WriterActor {
  id: string;
  entity_id?: string;
  [key: string]: unknown;
}
...
async function persistEvent(event: Record<string, unknown>): Promise<void> {
  try {
    ...
    if (!isMissingTable) logger.warn('Event persistence failed (non-truth-bearing, degrading gracefully):', e.message);
  }
}

function emitEvent(type: string, payload: Record<string, unknown>): Record<string, unknown> {
  const event = {
    type,
...
async function withRetry<T>(fn: () => Promise<T>, maxAttempts: number = 3, baseDelayMs: number = 200): Promise<T | undefined> {
 ...
 */
async function recomputeAndPersistScores(supabase: any, entityId: string): Promise<any> {
...
async function updateReceiptDisputeStatus(
  supabase: any,
  receiptId: string,
  disputeStatus: string | null,
  graphWeight?: number,
): Promise<void> {
  const patch: { dispute_status: string | null; graph_weight?: number } = { dispute_status: disputeStatus };
...
async function materializeTrustProfile(entityDbId: string): Promise<void> {
...
export interface CanonicalSubmitReceiptParams {
  entity_id: string;
  ...
  request_bilateral?: boolean;
}

/** Raw (pre-normalization) auto-receipt input; see canonicalSubmitAutoReceipt. */
export interface CanonicalSubmitAutoReceiptParams {
  entity_id: string;
  transaction_ref: string;
  transaction_type?: string;
  delivery_accuracy?: number | null;
  ...
  agent_satisfaction?: number | null;
  agent_behavior?: string | null;
  outcome?: { completed?: boolean; error_occurred?: boolean };
  claims?: Record<string, unknown> | null;
  evidence?: Record<string, unknown>;
  context?: unknown;
  [key: string]: unknown;
...
export async function canonicalSubmitReceipt(
  params: CanonicalSubmitReceiptParams,
  submitterEntity: any,
...
export async function canonicalSubmitAutoReceipt(
  raw: CanonicalSubmitAutoReceiptParams,
  submitterEntity: any,
): Promise<CreateReceiptResult> {
...
export interface BilateralConfirmResult {
  error?: string;
  status?: number;
  receipt_id?: string;
  bilateral_status?: string;
  provenance_tier?: string;
}

/** Confirm or dispute a bilateral receipt. */
export async function canonicalBilateralConfirm(
  receiptId: string,
  confirmingEntityId: string,
  confirm: boolean,
): Promise<BilateralConfirmResult> {
  const supabase = getServiceClient();
  const now = new Date().toISOString();
...
export async function canonicalFileDispute(params: FileDisputeParams, filerEntity: WriterActor): Promise<FileDisputeResult> {
  const supabase = getServiceClient();
...
export async function canonicalRespondDispute(
  disputeId: string,
  responderId: string,
  response: string,
...
export async function canonicalResolveDispute(
  disputeId: string,
  resolution: string,
  rationale: string,
  operatorId: string,
 ...
 */
export async function canonicalWithdrawDispute(disputeId: string, withdrawerEntity: WriterActor): Promise<WithdrawDisputeResult> {
  const supabase = getServiceClient();
  const now = new Date().toISOString();
...
export async function canonicalAppealDispute(
  disputeId: string,
  appealerEntity: WriterActor,
  reason: string,
  evidence: Record<string, unknown> | null | undefined,
  ...
  if (updateError) {
    logger.error('Appeal update error:', updateError);
...
export async function canonicalResolveAppeal(
  disputeId: string,
  resolution: string,
  rationale: string,
  operatorId: string,
...
export async function canonicalFileReport(params: FileReportParams): Promise<FileReportResult> {
  const supabase = getServiceClient();
  const { randomBytes } = await import('crypto');
  ...
  // Older deployments may not have migration 030's reporter_ip_hash column yet.
  // Filing the report is more important than IP-throttling metadata, so retry
  // once with the base schema and keep the route live while migration drift is
  // corrected.
  if (insertError && /reporter_ip_hash/i.test(String(insertError.message || insertError.details || insertError.hint || ''))) {
    const baseReportRow = { ...reportRow };
    delete baseReportRow.reporter_ip_hash;
