 ...
 *
 * Delegates to blockchain.js buildMerkleTree on EP-MERKLE-v2 (domain-separated:
 * 0x00 leaf / 0x01 branch, positional) — the same anchoring layer used for
 * receipt anchors. v2 closes the leaf/branch second-preimage class on this last
 * live default-v1 caller. Safe to flip: commitment roots are computed at
 * generation and never re-derived against a stored v1 root at verify time.
 *
 * @param {string[]} commitments  Array of 64-char hex commitment strings
 * @returns {{ root: string|null, layers: string[][], leafCount: number }}
 */
export function buildCommitmentTree(commitments) {
  if (!commitments || commitments.length === 0) {
    return { root: null, layers: [], leafCount: 0 };
  }
  return buildMerkleTree(commitments, { v2: true });
}
 ...
 * @returns {Promise<{ provable: boolean, measured_value: number, receipts_for_claim: ZkClaimReceipt[] }>}
 */
async function evaluateClaim(entityId, claim, supabase) {
  ...
  const allReceipts = /** @type {ZkClaimReceipt[]} */ (receipts || []);
 ...
 */
/** @deprecated Use generateCommitmentProof — ZK naming was inaccurate (these are HMAC commitments, not zk-SNARKs) */
 ...
 * @param {import('@supabase/supabase-js').SupabaseClient} [supabase]
 */
export async function generateCommitmentProof(entityId, claim, supabase) {
 ...
 */
/** @deprecated Use verifyCommitmentProof — ZK naming was inaccurate (these are HMAC commitments, not zk-SNARKs) */
 ...
 * @param {import('@supabase/supabase-js').SupabaseClient} [supabase]
 */
export async function verifyCommitmentProof(proofId, supabase) {
...
/**
 * Reconstruct the public claim object ({ type, threshold, domain? }) from a
 * stored zk_proofs row.
 *
 * @param {{ claim_type: string, claim_threshold: number|string, claim_domain?: string }} proof
 * @returns {{ type: string, threshold: number, domain?: string }}
 */
function buildClaimObject(proof) {
  /** @type {{ type: string, threshold: number, domain?: string }} */
  const claim = {
    type: proof.claim_type,
    threshold: Number(proof.claim_threshold),
  };
