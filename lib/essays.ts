...
export function essayMdToHtml(md) {
  ...
  for (const line of lines) {
    if (line.startsWith('```')) {
      if (!inCode) {
        inCode = true;
        codeLang = sanitizeCodeLanguage(line.slice(3));
        codeLines = [];
      } else {
...
function inlineFormat(text) {
  // Escape before adding the small set of markup we support. Essays are
  // committed build-time inputs today, but a future content pipeline must not
  // silently turn this renderer into an HTML/script execution boundary.
