...
//
// Narrow authentication bridge for the policy-rollout Accountable Signoff
// flow. Normal EP entity keys retain the existing authenticateRequest path.
// Tenant control-plane keys are admitted only when they are the generated
// ept_* form, hold admin permission, and are later constrained by the route to
// policy_rollout receipt creation/signoff request.
...
export async function authenticateGuardRequest(request, dependencies = {}) {
  ...
  if (!Array.isArray(cloud.permissions)
      || (!cloud.permissions.includes('admin')
        && !cloud.permissions.includes('policy_rollout'))) {
    return {
      error: 'Cloud API key requires policy_rollout permission for rollout authorization',
