...
function normalizeTableName(table) {
  ...
  if (t.includes('.')) t = /** @type {string} */ (t.split('.').pop()); // drop schema prefix
  ...
  return new Proxy(client, {
    get(target, prop, receiver) {
      if (prop === 'schema' && originalSchema) {
        return (/** @type {string} */ schemaName) => createWriteGuard(originalSchema(schemaName));
      }
      if (prop === 'from') {
          ...
          if (TRUST_TABLES.includes(normalizeTableName(table))) {
            ...
            return new Proxy(query, {
              get(qTarget, qProp, qReceiver) {
                if (blockedOps.has(/** @type {string} */ (qProp))) {
                  return (/** @type {any[]} */ ...args) => {
                    throw new Error(
                      `WRITE_DISCIPLINE_VIOLATION: Direct ${/** @type {any} */ (qProp)}() on trust table "${table}" is forbidden. ` +
