...
async function resolvesPublicly(hostname) {
  ...
  try {
    const records = /** @type {import('node:dns').LookupAddress[] | import('node:dns').LookupAddress} */ (
 ...
 */
async function validatePublicFetchUrl(urlStr) {
  let parsed;
  try {
...
export async function fetchMcpServerMeta(serverUrl) {
  ...
  const validated = typeof serverUrl === 'string'
  ...
  try {
    ...
    const baseUrl = /** @type {string} */ (validated.url).replace(/\/+$/, '');
 ...
 * @returns {Promise<{ software_meta: object, raw: object|null }>}
 */
