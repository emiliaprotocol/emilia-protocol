...
export function resolveBlockchainSigner(config = {}) {
  ...
  if (EXTERNAL_MODES.has(mode)) {
    if (!config.signingKeyId) {
      const error = new Error('EP_BLOCKCHAIN_SIGNING_KEY_ID is required for external blockchain custody');
    ...
    if (config.signingKeyId && registeredProvider.keyId !== config.signingKeyId) {
      const error = new Error('registered blockchain signer key id does not match EP_BLOCKCHAIN_SIGNING_KEY_ID');
  ...
  if (mode !== 'env') {
    const error = new Error(`Unsupported EP_BLOCKCHAIN_SIGNING_MODE=${mode}`);
