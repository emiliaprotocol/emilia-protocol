...
function mobileDecisionEvidence(challenge, result, auditEntry) {
  if (result?.decision === 'approved') return result.class_a;
  if (result?.decision !== 'denied') return null;
  return {
  ...
  if (enrollments.length > 0) {
    ...
    const service = createMobileCeremonyService({
      ...
      commitDecision: ({ challenge, result, auditEntry }) => commitMobileActionDecision(supabase, {
        ...
        decisionEvidence: mobileDecisionEvidence(challenge, result, auditEntry),
        auditEntry,
      }),
    });
