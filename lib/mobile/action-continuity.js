...
// Generated from action-continuity.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
import crypto from 'node:crypto';
import { canonicalize } from '../../caid/impl/js/caid.mjs';
import { MOBILE_ACTION_CAID_TYPE, _internals as actionIdentityInternals, buildMobileActionIdentity as computeMobileActionIdentity, mobileActionFingerprint, } from '../../packages/mobile/action-identity.js';
...
export function materialFieldDiff(current = {}, previous = {}) {
    ...
    for (const field of fields) {
        ...
        if (had && has && before[field] === after[field])
            continue;
        changes.push({
            field,
...
export function normalizeSystemAlignments(input = []) {
    ...
    return input.slice(0, 32).flatMap((item) => {
        if (!record(item) || !boundedString(item.system, 128))
            return [];
        ...
        return [{
                ...
                reason: boundedString(item.reason, 256) ? item.reason : (verdict === 'INDETERMINATE' && requested === 'EQUIVALENT_UNDER_PROFILE'
...
export function deriveMobileActionContinuity(input = {}) {
    ...
    if (effect === 'refused')
        return { state: 'REFUSED', retry_safe: false, quorum };
    if (effect === 'indeterminate')
        return { state: 'INDETERMINATE', retry_safe: false, quorum };
    if (effect === 'consumed')
        return { state: 'CONSUMED', retry_safe: false, quorum };
    ...
    if (approved >= required)
        return { state: 'AUTHORIZED', retry_safe: true, quorum };
    if (input.status === 'denied' || denied > 0)
        return { state: 'DENIED', retry_safe: true, quorum };
    if (input.status === 'withdrawn' || withdrawn > 0)
        return { state: 'WITHDRAWN', retry_safe: true, quorum };
    if (approved > 0)
        return { state: 'QUORUM_PENDING', retry_safe: true, quorum };
    if (input.status === 'expired')
        return { state: 'EXPIRED', retry_safe: true, quorum };
    if (input.status === 'cancelled')
        return { state: 'CANCELLED', retry_safe: true, quorum };
...
function providerSigningBytes(body) {
    ...
    if (!canonical.ok)
        throw new TypeError('provider outcome is not canonicalizable');
...
export function verifyMobileProviderOutcome(evidence, { expected = {}, executorKeys = {}, notBefore = null, now = new Date().toISOString(), } = {}) {
    ...
    try {
        ...
        if (key.asymmetricKeyType !== 'ed25519'
            || !crypto.verify(null, providerSigningBytes(body), key, Buffer.from(evidence.proof.signature_b64u, 'base64url'))) {
...
export const MOBILE_PROVIDER_OUTCOME_V2_REQUIRED_ALGORITHMS = Object.freeze(['Ed25519', 'ML-DSA-65']);
...
function providerOutcomeAlgorithmSetV2(algorithms) {
    return Array.isArray(algorithms)
        && algorithms.length === MOBILE_PROVIDER_OUTCOME_V2_REQUIRED_ALGORITHMS.length
        && algorithms.every((a, i) => a === MOBILE_PROVIDER_OUTCOME_V2_REQUIRED_ALGORITHMS[i]);
}
function providerOutcomeBodyV2(input) {
    return {
...
function providerSigningBytesV2(body, requiredAlgorithms = MOBILE_PROVIDER_OUTCOME_V2_REQUIRED_ALGORITHMS) {
...
export function mobilePqExecutorKeyId(pqPublicKeyRawB64u) {
 ...
 * signAgileSet throw.
 */
export async function buildMobileProviderOutcomeV2(input = {}) {
    if (!boundedString(input.operationId) || !CAID.test(input.actionCaid || '')
        || !SHA256.test(input.actionDigest || '') || !boundedString(input.consumptionNonce)
        || !boundedString(input.executorId) || !['executed', 'refused'].includes(input.outcome)
        || !boundedString(input.observedAt, 64)
 ...
 */
export async function verifyMobileProviderOutcomeV2(evidence, { expected = {}, executorKeys = {}, notBefore = null, now = new Date().toISOString(), } = {}, options = {}) {
    const refused = (reason) => ({ valid: false, reason, outcome: null, evidence_digest: null });
    ...
    let bytes;
    try {
        bytes = providerSigningBytesV2(body);
    }
    catch {
        return refused('provider_outcome_signature_invalid');
    }
