...
export interface AndroidSigningCertificate {
  hex: string;
  base64: string;
  base64url: string;
  assetLinks: string;
...
export interface MobileConfig {
  ...
  androidSigningCertificateSha256Hex: string;
  androidKeyHashes: string[];
  androidOrigins: string[];
  androidCertificateDigests: string[];
  androidAssetLinksFingerprints: string[];
  androidConfigured: boolean;
  appleEnvironment: 'production' | 'development';
  appleAllowedValidationCategories: number[];
  appleAllowedBundleVersions: string[];
  appleRequireRuntimeSignals: boolean;
  androidAllowedVersionCodes: number[];
  androidMinimumSdkVersion: number;
  androidRequirePlayProtect: boolean;
...
export function normalizeAndroidSigningCertificate(
  value: unknown,
  name: string = 'Android signing certificate',
  ...
  return Object.freeze({
    ...
    assetLinks: (normalizedHex.toUpperCase().match(/../g) as string[]).join(':'),
  });
}

function assertLegacyCertificatePins(env: NodeJS.ProcessEnv, certificate: AndroidSigningCertificate): void {
...
function positiveIntegerList(value: string | undefined, fallback: string): number[] {
...
export function mobileAndroidOrigin(keyHash: string): string {
...
}: { env?: NodeJS.ProcessEnv; production?: boolean } = {}): MobileConfig {
