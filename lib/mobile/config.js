...
export function normalizeAndroidSigningCertificate(value, name = 'Android signing certificate') {
    ...
    if (!bytes || bytes.length !== 32)
        throw new TypeError(`${name} must be one SHA-256 digest`);
    const normalizedHex = bytes.toString('hex');
    return Object.freeze({
        hex: normalizedHex,
        base64: bytes.toString('base64'),
        base64url: bytes.toString('base64url'),
        assetLinks: normalizedHex.toUpperCase().match(/../g).join(':'),
    });
}
...
export function mobileAndroidOrigin(keyHash) {
    if (!B64U_SHA256.test(keyHash || ''))
        throw new TypeError('Android APK key hash must be an unpadded base64url SHA-256 value');
...
export function getMobileConfig({ env = process.env, production = env.NODE_ENV === 'production', } = {}) {
    ...
    if (!PROFILE_ID.test(profileId))
        throw new TypeError('MOBILE_PROFILE_ID is malformed');
    if (!RP_ID.test(rpId) || rpId !== rpId.toLowerCase())
        throw new TypeError('MOBILE_RP_ID must be a lowercase DNS name');
    let parsedOrigin;
    try {
        parsedOrigin = new URL(iosOrigin);
    }
    catch {
        throw new TypeError('MOBILE_IOS_ORIGIN must be an HTTPS origin');
    }
    ...
    const androidSigningCertificate = normalizeAndroidSigningCertificate(canonicalCertificate, 'MOBILE_ANDROID_SIGNING_CERT_SHA256');
    assertLegacyCertificatePins(env, androidSigningCertificate);
    if (!/^[A-Z0-9]{10}$/.test(appleTeamId))
        throw new TypeError('MOBILE_APPLE_TEAM_ID must be a 10-character Team ID');
    const appleAllowedValidationCategories = positiveIntegerList(env.MOBILE_APPLE_ALLOWED_VALIDATION_CATEGORIES, production ? '2,4' : '3');
    ...
    return Object.freeze({
        ...
        androidMinimumSdkVersion: boundedInteger(env.MOBILE_ANDROID_MIN_SDK_VERSION, production ? 33 : 26, 'MOBILE_ANDROID_MIN_SDK_VERSION', 26, 100),
        androidRequirePlayProtect: boolean(env.MOBILE_ANDROID_REQUIRE_PLAY_PROTECT, production),
        maxChallengeAgeMs: boundedInteger(env.MOBILE_CHALLENGE_TTL_MS, 300_000, 'MOBILE_CHALLENGE_TTL_MS', 30_000, 600_000),
        pairingTtlMs: boundedInteger(env.MOBILE_PAIRING_TTL_MS, 10 * 60_000, 'MOBILE_PAIRING_TTL_MS', 60_000, 3_600_000),
        sessionTtlMs: boundedInteger(env.MOBILE_SESSION_TTL_MS, 30 * 24 * 60 * 60_000, 'MOBILE_SESSION_TTL_MS', 300_000, 90 * 24 * 60 * 60_000),
    });
}
