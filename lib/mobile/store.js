...
export const MOBILE_DENIAL_EVIDENCE_VERSION = 'EP-MOBILE-DENIAL-EVIDENCE-v1';
const MOBILE_DENIAL_EVIDENCE_MEMBERS = new Set([
  '@version',
  'challenge_id',
  'action_hash',
  'profile_hash',
...
export async function resolveMobileAction(supabase, { entityRef, approverId, actionReference }) {
  ...
  return { ...data, presentation: normalizeMobilePresentation(data.presentation) };
}
...
function mobileApprovalEvidenceMatches(evidence, {
  actionHash,
  approverId,
  contextHash,
}) {
  return mobileRecord(evidence?.context)
    && mobileRecord(evidence?.signoff)
    && evidence.context.action_hash === actionHash
    && evidence.context.decision === 'approved'
    && evidence.context.approver === approverId
    && evidence.signoff.key_class === 'A'
    && evidence.signoff.context_hash === contextHash;
}

function mobileDenialEvidenceMatches(evidence, {
  challengeId,
  actionHash,
  profileHash,
  approverId,
  ...
  return mobileRecord(evidence)
    && Object.keys(evidence).length === MOBILE_DENIAL_EVIDENCE_MEMBERS.size
    && Object.keys(evidence).every((key) => MOBILE_DENIAL_EVIDENCE_MEMBERS.has(key))
    && evidence['@version'] === MOBILE_DENIAL_EVIDENCE_VERSION
    && evidence.challenge_id === challengeId
    && evidence.action_hash === actionHash
    && evidence.profile_hash === profileHash
    && evidence.decision === 'denied'
    && evidence.approver_id === approverId
    && evidence.device_key_id === deviceKeyId
    && evidence.context_hash === contextHash;
}
...
function recoveredMobileResult({ challenge, action, evidenceRecord, session }) {
  ...
  if (challenge.decision === 'approved') {
    ...
    if (!mobileRecord(context) || !mobileRecord(signoff) || !mobileRecord(binding)
        || !mobileRecord(signoff.webauthn)) return null;
    const issuedAt = mobileInstant(context.issued_at);
    const contextExpiresAt = mobileInstant(context.expires_at);
    if (issuedAt === null || contextExpiresAt === null
        || issuedAt > challengeCreatedAt || contextExpiresAt !== challengeExpiresAt
        || context.action_hash !== challenge.action_hash
        || context.decision !== 'approved'
        || context.approver !== challenge.approver_id
        || binding.platform !== session.platform
        || binding.app_id !== session.appId
        || binding.device_key_id !== session.deviceKeyId
        || !MOBILE_SHA256.test(binding.profile_hash || '')
        || signoff.approver_key_id !== binding.device_key_id
        || signoff.signed_at !== context.issued_at) return null;
    contextHash = `sha256:${crypto.createHash('sha256')
      .update(canonicalEvidenceJson(context), 'utf8')
      .digest('hex')}`;
    profileHash = binding.profile_hash;
    if (!mobileApprovalEvidenceMatches(evidence, {
      actionHash: challenge.action_hash,
      approverId: challenge.approver_id,
      contextHash,
    })) return null;
  } else {
    contextHash = evidence.context_hash;
    profileHash = evidence.profile_hash;
    if (!MOBILE_SHA256.test(profileHash || '') || !MOBILE_SHA256.test(contextHash || '')
        || !mobileDenialEvidenceMatches(evidence, {
  ...
  if (challenge.decision === 'denied') {
    recovered.denial_evidence = structuredClone(evidence);
  }
  return recovered;
}
...
export async function commitMobileActionDecision(supabase, {
  ...
  const evidenceMatches = decision === 'approved'
    ? mobileApprovalEvidenceMatches(resultEvidence, {
      actionHash,
      approverId: suppliedEntry.approver_id,
      contextHash: suppliedEntry.context_hash,
    })
    : decision === 'denied' && mobileDenialEvidenceMatches(resultEvidence, {
      challengeId,
      actionHash,
...
export async function createGraceMobileActionGroup(supabase, {
  ...
  const { data, error } = await supabase.rpc('create_grace_mobile_action_group', {
