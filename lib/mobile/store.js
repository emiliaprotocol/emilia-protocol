...
// Generated from store.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
import crypto from 'node:crypto';
import { canonicalEvidenceJson, createAtomicEvidenceLog, verifyEvidenceRecord, } from '../../packages/gate/evidence.js';
import { normalizeMobilePresentation } from '../../packages/mobile/presentation.js';
import { buildDecisionPassport, buildMobileActionIdentity, deriveMobileActionContinuity, materialFieldDiff, mobileActionFingerprint, normalizeSystemAlignments, verifyMobileProviderOutcome, } from './action-continuity.js';
 ...
 */
export function mobilePairingIdentityChallenge(code) {
    return crypto.createHash('sha256')
        .update(`${MOBILE_PAIRING_IDENTITY_PROFILE}\0${code.trim().toUpperCase()}`, 'utf8')
        .digest('base64url');
}
export async function listMobilePairingIdentityCredentials(supabase, { organizationId, approverId, directoryUserId, now = new Date().toISOString(), }) {
    const { data, error } = await supabase
        ...
        .is('revoked_at', null);
    if (error)
        throw databaseError('mobile pairing credential directory failed', error);
 ...
 */
export async function loadMobilePairingIdentityContext(supabase, { code, credentialId, now = new Date().toISOString() }) {
    const { data: pairing, error: pairingError } = await supabase
        ...
        .maybeSingle();
    if (pairingError)
        throw databaseError('mobile pairing identity lookup failed', pairingError);
    if (!pairing || pairing.consumed_at || !pairing.organization_id || !pairing.directory_user_id
        || Date.parse(pairing.expires_at) <= Date.parse(now))
        return null;
    const { data: entity, error: entityError } = await supabase
        ...
        .maybeSingle();
    if (entityError)
        throw databaseError('mobile pairing tenant lookup failed', entityError);
    ...
    const { data: directoryUser, error: directoryUserError } = await supabase
        ...
        .maybeSingle();
    if (directoryUserError)
        throw databaseError('mobile pairing directory-user lookup failed', directoryUserError);
    if (!directoryUser || directoryUser.active !== true || directoryUser.user_name !== pairing.approver_id)
        return null;
    const { data: directoryToken, error: directoryTokenError } = await supabase
        ...
        .limit(1)
        .maybeSingle();
    if (directoryTokenError)
        throw databaseError('mobile pairing directory-token lookup failed', directoryTokenError);
    if (!directoryToken)
        return null;
    const { data: credential, error: credentialError } = await supabase
        ...
        .maybeSingle();
    if (credentialError)
        throw databaseError('mobile pairing credential lookup failed', credentialError);
    const instant = Date.parse(now);
    const validFrom = credential?.valid_from ? Date.parse(credential.valid_from) : Number.NEGATIVE_INFINITY;
    const validTo = credential?.valid_to ? Date.parse(credential.valid_to) : Number.POSITIVE_INFINITY;
    if (!credential
...
export function createMobileStateBackend(supabase) {
    ...
    return {
        ...
        async addIfAbsent(key, value) {
            ...
            if (error)
                throw databaseError('mobile state insert failed', error);
            return data === true;
        },
        async compareAndSet(key, expected, replacement) {
            ...
            if (error)
                throw databaseError('mobile state compare-and-set failed', error);
            return data === true;
        },
        async has(key) {
            const { data, error } = await supabase
                ...
                .maybeSingle();
            if (error)
                throw databaseError('mobile state lookup failed', error);
            return Boolean(data);
        },
    };
}
export function createMobileCounterStore(supabase, namespace = 'mobile') {
    ...
    return {
        ...
        async advance(key, next) {
            if (typeof key !== 'string' || !Number.isSafeInteger(next) || next < 0)
                return false;
            ...
            if (error)
                throw databaseError('mobile counter advance failed', error);
            return data === true;
        },
    };
}
function createMobileEvidenceBackend(supabase, entityRef) {
    ...
    return {
        ...
        async readHead(streamId) {
            if (streamId !== entityRef)
                throw new Error('mobile evidence stream mismatch');
            const { data, error } = await supabase
                ...
                .maybeSingle();
            if (error)
                throw databaseError('mobile evidence head lookup failed', error);
            if (!data)
                return null;
        ...
        async getById(streamId, recordId) {
            ...
            const { data, error } = await supabase
                ...
                .maybeSingle();
            if (error)
                throw databaseError('mobile evidence record lookup failed', error);
        ...
        async appendIfHead(streamId, expectedHeadHash, record) {
            ...
            if (error)
                throw databaseError('mobile evidence append failed', error);
            return data === true;
        },
        async readAll(streamId) {
            ...
            if (error)
                throw databaseError('mobile evidence history lookup failed', error);
...
export function createMobileEnrollmentDirectory(supabase, entityRef, sessionId) {
    ...
    return {
        ...
        async enrollAtomically({ enrollment, event }) {
            ...
            if (error)
                throw databaseError('mobile enrollment insert failed', error);
            return data === true;
        },
        async active() {
            ...
            const { data, error } = await supabase
                ...
                .gte('valid_to', now);
            if (error)
                throw databaseError('mobile enrollment directory unavailable', error);
            return data || [];
        },
        async platformKey(attestationKeyId, platform) {
            if (!['ios', 'android'].includes(platform))
                throw new TypeError('mobile platform key lookup is malformed');
            const { data, error } = await supabase
                .from('mobile_enrollments')
                ...
                .maybeSingle();
            if (error)
                throw databaseError('mobile platform key lookup failed', error);
            return data || null;
        },
    };
}
export async function createPairing(supabase, { code, entityRef, organizationId, approverId, directoryUserId, profileId, allowedApps, expiresAt, sessionExpiresAt, }) {
    ...
    if (error)
        throw databaseError('mobile pairing creation failed', error);
    if (data !== true)
        throw new Error('mobile pairing creation refused');
    return true;
}
export async function exchangePairing(_supabase, _input) {
    throw new Error('unverified_mobile_pairing_exchange_disabled');
}
export async function exchangePairingVerified(supabase, { code, token, platform, appId, credentialId, approverId, newSignCount, identityProofDigest, }) {
    ...
    if (error)
        throw databaseError('verified mobile pairing exchange failed', error);
...
export async function authenticateMobileToken(supabase, authorization) {
    ...
    if (!match || !MOBILE_TOKEN.test(match[1]))
        return null;
    ...
    const { data, error } = await supabase
        ...
        .maybeSingle();
    if (error)
        throw databaseError('mobile session lookup failed', error);
    if (!data)
        return null;
    ...
    if (touchError)
        throw databaseError('mobile session update failed', touchError);
    if (touched !== true)
        return null;
    return data;
}
export async function revokeMobileSession(supabase, { sessionId, entityRef }) {
    if (typeof sessionId !== 'string' || !sessionId || typeof entityRef !== 'string' || !entityRef)
        return false;
    ...
    if (error)
        throw databaseError('mobile session revocation failed', error);
    return data === true;
}
...
// members are checked ad hoc below (mobileRecord/mobileExactMembers/regexes)
// rather than through a static shape, so `any` reflects its real contract.
...
async function listMobileActionContinuity(supabase, { entityRef, approverId, pendingOnly, }) {
    if (!supabase?.rpc)
        throw new TypeError('Supabase client is required');
    ...
    if (error)
        throw databaseError('mobile action inbox unavailable', error);
...
export async function resolveMobileAction(supabase, { entityRef, approverId, actionReference }) {
    ...
    if (!data || data.status !== 'pending' || Date.parse(data.expires_at) <= Date.now())
        return null;
    return data;
}
...
function mobileInstant(value) {
    if (typeof value !== 'string' || !MOBILE_CANONICAL_INSTANT.test(value))
        return null;
...
function mobileDecisionEvidenceMatches(evidence, { decision, actionHash, profileHash, approverId, deviceKeyId, contextHash, platform = null, appId = null, }) {
...
function mobileDecisionAuditMatches(entry, { challengeId, actionHash, decision, verdict, }) {
...
function recoveredMobileResult({ challenge, action, evidenceRecord, session }) {
    ...
    if (!mobileRecord(challenge) || !mobileRecord(action) || !mobileRecord(evidence))
        return null;
    ...
    if (!['approved', 'denied'].includes(challenge.decision)
        ...
        || action.decision_verdict !== 'verified')
        return null;
    const context = evidence.context;
    const binding = context?.mobile_binding;
    ...
    if (!verifyEvidenceRecord(evidenceRecord, { atomicRequired: true, expectedEntry }))
        return null;
    const recovered = {
        valid: true,
        verdict: 'verified',
...
export async function lookupMobileCeremonyResult(supabase, { entityRef, sessionId, approverId, platform, appId, deviceKeyId, challengeId, }) {
    if (!supabase?.from || typeof entityRef !== 'string' || !entityRef
        ...
        || typeof challengeId !== 'string' || !MOBILE_LOOKUP_ID.test(challengeId))
        return null;
    const { data: challenge, error: challengeError } = await supabase
        ...
        .maybeSingle();
    if (challengeError)
        throw databaseError('mobile ceremony result lookup failed', challengeError);
    ...
    const { data: action, error: actionError } = await supabase
        ...
        .maybeSingle();
    if (actionError)
        throw databaseError('mobile ceremony result lookup failed', actionError);
    if (!action)
        return null;
    const { data: evidence, error: evidenceError } = await supabase
        ...
        .maybeSingle();
    if (evidenceError)
        throw databaseError('mobile ceremony result lookup failed', evidenceError);
    if (!evidence?.record)
        return null;
...
export async function registerMobileActionChallenge(supabase, { entityRef, sessionId, actionReference, approverId, challengeId, actionHash, decision, expiresAt, }) {
    ...
    if (error)
        throw databaseError('mobile action challenge registration failed', error);
    return data === true;
}
export async function commitMobileActionDecision(supabase, { entityRef, sessionId, challengeId, actionHash, decision, verdict, decisionEvidence, auditEntry, }) {
    ...
    for (let attempt = 0; attempt < 32; attempt++) {
        ...
        if (data?.reason === 'head_changed')
            continue;
        if (['action_conflict', 'session_inactive'].includes(data?.reason))
            return false;
...
export async function createDemoAction(supabase, row) {
    ...
    if (error)
        throw databaseError('mobile demo action creation failed', error);
    if (data !== true)
        throw new Error('mobile demo action creation refused');
    return row.action_reference;
}
export async function createGraceMobileActionGroup(supabase, { assignments, entityRef, initiatorId, action, presentation, policy, policyId, expiresAt, }) {
    ...
    if (error)
        throw databaseError('GRACE mobile action group creation failed', error);
    if (data !== true)
        throw new Error('GRACE mobile action group creation refused');
    return snapshot;
}
export async function supersedeMobileAction(supabase, { entityRef, currentActionReference, assignments, initiatorId, action, presentation, policy, policyId, expiresAt, }) {
    if (!Array.isArray(assignments) || assignments.length === 0) {
    ...
    const { data: current, error: currentError } = await supabase
        ...
        .maybeSingle();
    if (currentError)
        throw databaseError('mobile action supersession failed', currentError);
    if (!current?.presentation)
        throw new Error('mobile action supersession refused: not_found');
    const currentPresentation = normalizeMobilePresentation(current.presentation);
    const identity = buildMobileActionIdentity({
        actionReference: assignments[0].action_reference,
        action,
    });
    const changes = materialFieldDiff(normalizedPresentation.material_fields, currentPresentation.material_fields);
    ...
    if (error)
        throw databaseError('mobile action supersession failed', error);
    if (data?.ok !== true)
        throw new Error(`mobile action supersession refused: ${data?.reason || 'unknown'}`);
    return { ...data, identity, changes };
}
export async function withdrawMobileAction(supabase, { entityRef, sessionId, actionReference, }) {
    ...
    if (error)
        throw databaseError('mobile approval withdrawal failed', error);
    return data || { ok: false, reason: 'unknown' };
}
export async function consumeMobileAction(supabase, { entityRef, actionReference, operationId, consumptionNonce, executorId, }) {
    ...
    if (error)
        throw databaseError('mobile action consumption failed', error);
    return data || { ok: false, reason: 'unknown' };
}
export async function markMobileActionIndeterminate(supabase, { entityRef, operationId, }) {
    ...
    if (error)
        throw databaseError('mobile action indeterminate transition failed', error);
    return data || { ok: false, reason: 'unknown' };
}
export async function resolveMobileOperation(supabase, { entityRef, operationId, actionReference = null }) {
    const { data, error } = await supabase
        ...
        .maybeSingle();
    if (error)
        throw databaseError('mobile action operation lookup failed', error);
    if (!data)
        return null;
    const { data: revision, error: revisionError } = await supabase
        ...
        .maybeSingle();
    if (revisionError)
        throw databaseError('mobile action operation lookup failed', revisionError);
    if (!revision)
        return null;
    if (actionReference !== null) {
        const { data: action, error: actionError } = await supabase
            ...
            .maybeSingle();
        if (actionError)
            throw databaseError('mobile action operation lookup failed', actionError);
        if (!action)
            return null;
    }
...
export async function resolveMobileExecutorKey(supabase, { entityRef, executorId, executorKeyId = null }) {
    let query = supabase
        ...
        .eq('status', 'active');
    if (executorKeyId !== null)
        query = query.eq('key_id', executorKeyId);
    const { data, error } = await query.maybeSingle();
    if (error)
        throw databaseError('mobile executor key lookup failed', error);
    return data || null;
}
export async function reconcileMobileActionOperation(supabase, { entityRef, operation, evidence, }) {
    ...
    if (error)
        throw databaseError('mobile action reconciliation failed', error);
    return data || { ok: false, reason: 'unknown' };
}
export async function registerMobileExecutorKey(supabase, { entityRef, executorId, keyId, publicKey, }) {
    ...
    if (error)
        throw databaseError('mobile executor key registration failed', error);
    return data === true;
}
export async function recordMobileActionAlignment(supabase, { entityRef, actionReference, alignment, }) {
    const [normalized] = normalizeSystemAlignments([alignment]);
    if (!normalized)
        throw new TypeError('mobile action alignment is malformed');
    ...
    if (error)
        throw databaseError('mobile action alignment recording failed', error);
    return data === true;
}
