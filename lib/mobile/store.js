...
function recoveredMobileResult({ challenge, action, evidenceRecord, session }) {
  ...
  if (!mobileRecord(challenge) || !mobileRecord(action) || !mobileRecord(evidence)
      || !mobileRecord(context) || !mobileRecord(signoff) || !mobileRecord(binding)
  ...
  if (issuedAt === null || challengeCreatedAt === null || challengeExpiresAt === null
      || contextExpiresAt === null || consumedAt === null || decidedAt === null
      || issuedAt > challengeCreatedAt || challengeCreatedAt > consumedAt
      || consumedAt !== decidedAt || decidedAt > challengeExpiresAt
      || contextExpiresAt !== challengeExpiresAt) return null;
  ...
  if (!['approved', 'denied'].includes(challenge.decision)
      ...
      || context.decision !== challenge.decision
  ...
  const contextHash = `sha256:${crypto.createHash('sha256')
    .update(canonicalEvidenceJson(context), 'utf8')
    .digest('hex')}`;
  if (signoff.context_hash !== contextHash) return null;
...
export async function commitMobileActionDecision(supabase, {
  ...
  if (!decisionEvidence || typeof decisionEvidence !== 'object' || Array.isArray(decisionEvidence)) {
    throw new TypeError('portable Class-A decision evidence is required');
  }
