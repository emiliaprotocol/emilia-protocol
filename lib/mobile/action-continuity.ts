...
function canonicalDigest(value: any) {
  const result = canonicalize(value);
  if (!result.ok) {
    throw new TypeError(`value is not canonicalizable: ${(result as { ok: false; refusals: string[] }).refusals.join(', ')}`);
  }
 ...
 */
export function buildMobileActionIdentity({ actionReference, action }: any = {}) {
...
export function materialFieldDiff(current: any = {}, previous: any = {}) {
 ...
 */
export function normalizeSystemAlignments(input: any = []) {
...
export function deriveMobileActionContinuity(input: any = {}) {
...
function providerSigningBytes(body: any) {
...
export function buildMobileProviderOutcome(input: any = {}) {
...
export function verifyMobileProviderOutcome(evidence: any, {
  expected = {},
  executorKeys = {},
  notBefore = null,
