...
function cborRecord(value: any): any {
  if (value instanceof Map) return Object.fromEntries(value);
  return value && typeof value === 'object' && !Array.isArray(value) ? value : null;
}

function decodeOne(bytes: Buffer): any {
...
export function decodeAppleAuthenticatorExtensions(authenticatorData: Buffer): any {
...
export function verifyAppleRuntimeSignals(authenticatorData: Buffer, config: any = {}): any {
  try {
...
function certificateIsCurrent(certificate: any, now: number): boolean {
  const validFrom = Date.parse(certificate.validFrom);
  const validTo = Date.parse(certificate.validTo);
  return Number.isFinite(validFrom) && Number.isFinite(validTo) && now >= validFrom && now <= validTo;
}

export function inspectAppleAppAttestation(attestation: any, {
  clock = Date.now,
  certificateFactory = (raw: any) => new X509Certificate(raw),
}: any = {}): any {
  ...
  if (leaf.ca !== false || intermediate.ca !== true
      ...
      || !['prime256v1', 'P-256'].includes((leaf.publicKey?.asymmetricKeyDetails?.namedCurve as string) || '')) {
    throw new Error('App Attest certificate profile is invalid');
  }
  return { authenticatorData: decoded.authData };
}

export function inspectAppleAppAssertion(assertion: any): any {
...
function serviceAccountFromEnvironment(value: any): any {
...
/**
 * @param {object} [opts]
 * @param {string} [opts.serviceAccount]
...
export function createGooglePlayIntegrityDecoder({
  ...
  async function accessToken(): Promise<string> {
    const now = clock();
    if (cached && cached.expiresAt - 60 > now) return cached.value as string;
  ...
  return async function decodeIntegrityToken(integrityToken: string): Promise<any> {
 ...
 * @param {object} [opts]
 * @param {*} [opts.response]
 * @param {*} [opts.expectedChallenge]
 * @param {*} [opts.expectedOrigin]
 * @param {*} [opts.expectedRPID]
 * @param {boolean} [opts.requireUserVerification]
 */
 ...
 * @param {object} [opts]
 * @param {*} [opts.config]
 * @param {*} [opts.playDecoder]
 * @param {typeof inspectAppleAppAttestation} [opts.inspectAppleAttestation]
 */
export function createPlatformEnrollmentVerifier({
  ...
  return async function verifyPlatformEnrollment(input: any): Promise<any> {
 ...
 * @param {*} [opts.counterStore]
 * @param {*} [opts.playDecoder]
 * @param {typeof inspectAppleAppAssertion} [opts.inspectAppleAssertion]
 */
export function createProductionAttestationVerifier({
  ...
  return async function verifyPlatformAssertion(input: any): Promise<any> {
    if (input.platform === 'android') {
      ...
      try {
        ...
        if (expectedKeyId !== input.expected_attestation_key_id || requestHash.length !== 32
            ...
            || !['prime256v1', 'P-256'].includes((publicKey.asymmetricKeyDetails?.namedCurve as string) || '')
            || !verify(
              'sha256',
              requestHash,
              publicKey,
    ...
    const apple = createAppleAppAttestVerifier({
      ...
      counterStore,
      verifyAssertion: (async ({ assertionObject, expectedBinding, clientDataHash, keyId, environment }: any): Promise<any> => {
