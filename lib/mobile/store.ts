...
import type { MobileEnrollment } from '../../packages/mobile/index.js';
import {
...
/** Row shape returned by the `mobile_sessions` lookup in {@link authenticateMobileToken}. */
...
/** Generic JSON-compatible value used for opaque action/policy/evidence payloads. */
...
export function createMobileStateBackend(supabase: SupabaseClient) {
  if (!supabase?.from || !supabase?.rpc) throw new TypeError('Supabase client is required');
  return {
    durable: true,
    async addIfAbsent(key: string, value: Json): Promise<boolean> {
    ...
    async compareAndSet(key: string, expected: Json | null, replacement: Json): Promise<boolean> {
...
export function createMobileCounterStore(supabase: SupabaseClient, namespace = 'mobile') {
  if (!supabase?.rpc) throw new TypeError('Supabase client is required');
  return {
    durable: true,
    async advance(key: string, next: number): Promise<boolean> {
...
function createMobileEvidenceBackend(supabase: SupabaseClient, entityRef: string) {
  ...
  return {
    durable: true,
    async readHead(streamId: string): Promise<{ seq: number; hash: string } | null> {
      ...
      if (!data) return null;
      return { seq: (data.record as Json | null)?.seq as number, hash: data.record_hash };
    },
    async getById(streamId: string, recordId: string): Promise<Json | null> {
      if (streamId !== entityRef) throw new Error('mobile evidence stream mismatch');
      const { data, error } = await supabase
        .from('mobile_evidence_records')
        .select('record')
      ...
      return (data?.record as Json | null) || null;
    },
    async appendIfHead(streamId: string, expectedHeadHash: string | null, record: Json & { hash: string }): Promise<boolean> {
      if (streamId !== entityRef) throw new Error('mobile evidence stream mismatch');
    ...
    async readAll(streamId: string): Promise<Json[]> {
      if (streamId !== entityRef) throw new Error('mobile evidence stream mismatch');
      const { data, error } = await supabase
        .from('mobile_evidence_records')
        .select('record')
      ...
      return (data || []).map((row) => row.record as Json);
    },
  };
}

function mobileEvidenceRecordId(): string {
  return `mar_${crypto.randomUUID().replaceAll('-', '')}`;
}

export function createMobileAuditLog(supabase: SupabaseClient, entityRef: string) {
  const backend = createMobileEvidenceBackend(supabase, entityRef);
  return createAtomicEvidenceLog(backend, {
    streamId: entityRef,
    recordIdFactory: mobileEvidenceRecordId as () => `${string}-${string}-${string}-${string}-${string}`,
  });
}

export function createMobileEnrollmentDirectory(supabase: SupabaseClient, entityRef: string, sessionId: string) {
  ...
  return {
    durable: true,
    async enrollAtomically({ enrollment, event }: { enrollment: MobileEnrollment; event: Json }): Promise<boolean> {
    ...
    async active(): Promise<MobileEnrollment[]> {
    ...
    async platformKey(attestationKeyId: string, platform: string): Promise<Json | null> {
...
export async function revokeMobileSession(
  supabase: SupabaseClient,
  { sessionId, entityRef }: { sessionId: string; entityRef: string },
): Promise<boolean> {
...
function mobileContinuitySnapshot(row: any): Json {
...
async function listMobileActionContinuity(supabase: SupabaseClient, {
  entityRef,
  approverId,
  pendingOnly,
}: {
...
export async function listMobileActionHistory(
  supabase: SupabaseClient,
  { entityRef, approverId }: { entityRef: string; approverId: string },
): Promise<Json[]> {
...
export async function resolveMobileAction(
  supabase: SupabaseClient,
  { entityRef, approverId, actionReference }: { entityRef: string; approverId: string; actionReference: string },
): Promise<Json | null> {
  let rows: Json[];
  try {
...
function mobileContextHash(context: any): string | null {
  try {
...
function mobileDecisionEvidenceMatches(evidence: any, {
  decision,
  actionHash,
  profileHash,
  approverId,
...
function mobileDecisionAuditMatches(entry: any, {
  challengeId,
  actionHash,
  decision,
  verdict,
...
interface MobileCeremonySession {
  entityRef: string;
  sessionId: string;
  approverId: string;
  platform: string;
...
interface MobileCeremonyResult {
  valid: boolean;
  verdict: string;
  decision: string;
  reason: string | null;
...
function recoveredMobileResult({ challenge, action, evidenceRecord, session }: {
  ...
  action: any;
  evidenceRecord: any;
  session: MobileCeremonySession;
}): MobileCeremonyResult | null {
  ...
  const recovered: MobileCeremonyResult = {
    valid: true,
    verdict: 'verified',
    decision: challenge.decision,
    reason: null,
...
export async function lookupMobileCeremonyResult(supabase: SupabaseClient, {
  entityRef,
  sessionId,
  approverId,
  platform,
  ...
  deviceKeyId: string;
  challengeId: string;
}): Promise<MobileCeremonyResult | null> {
...
export async function registerMobileActionChallenge(supabase: SupabaseClient, {
  entityRef,
  sessionId,
  actionReference,
  approverId,
...
export async function commitMobileActionDecision(supabase: SupabaseClient, {
  entityRef,
  sessionId,
  challengeId,
  actionHash,
  ...
  decision: string;
  verdict: string;
  decisionEvidence: any;
  auditEntry: any;
}): Promise<{ committed: true; audit_record: Json } | false> {
...
export async function createDemoAction(supabase: SupabaseClient, row: any): Promise<string> {
...
export async function createGraceMobileActionGroup(supabase: SupabaseClient, {
  assignments,
  entityRef,
  initiatorId,
  action,
...
export async function supersedeMobileAction(supabase: SupabaseClient, {
  entityRef,
...
export async function withdrawMobileAction(supabase: SupabaseClient, {
  entityRef,
  sessionId,
  actionReference,
}: {
...
export async function consumeMobileAction(supabase: SupabaseClient, {
  entityRef,
  actionReference,
  operationId,
  consumptionNonce,
...
export async function markMobileActionIndeterminate(supabase: SupabaseClient, {
  entityRef,
  operationId,
}: {
  entityRef: string;
...
export async function resolveMobileOperation(
  supabase: SupabaseClient,
  { entityRef, operationId, actionReference = null }: { entityRef: string; operationId: string; actionReference?: string | null },
): Promise<Json | null> {
  const { data, error } = await supabase
...
export async function resolveMobileExecutorKey(
  supabase: SupabaseClient,
  { entityRef, executorId, executorKeyId = null }: { entityRef: string; executorId: string; executorKeyId?: string | null },
): Promise<Json | null> {
  let query = supabase
...
export async function reconcileMobileActionOperation(supabase: SupabaseClient, {
  entityRef,
  operation,
  evidence,
}: {
  ...
  const verified = verifyMobileProviderOutcome(evidence, {
    ...
    executorKeys: pin ? { [pin.executor_id as string]: pin } : {},
...
export async function registerMobileExecutorKey(supabase: SupabaseClient, {
  entityRef,
  executorId,
  keyId,
  publicKey,
...
export async function recordMobileActionAlignment(supabase: SupabaseClient, {
  entityRef,
  actionReference,
  alignment,
}: {
