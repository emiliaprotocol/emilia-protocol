...
export function inspectAppleAppAttestation(attestation, {
  ...
  if (leaf.ca !== false || intermediate.ca !== true
      ...
      || !['prime256v1', 'P-256'].includes(/** @type {string} */ (leaf.publicKey?.asymmetricKeyDetails?.namedCurve))) {
...
export function createPlatformEnrollmentVerifier({
  ...
  const play = config.androidCertificateDigests.length > 0
    ? createPlayIntegrityAttestationVerifier(/** @type {any} */ ({
...
export function createProductionAttestationVerifier({
  ...
  return async function verifyPlatformAssertion(input) {
    if (input.platform === 'android') {
      ...
      try {
        ...
        if (expectedKeyId !== input.expected_attestation_key_id || requestHash.length !== 32
            ...
            || !['prime256v1', 'P-256'].includes(/** @type {string} */ (publicKey.asymmetricKeyDetails?.namedCurve))
            || !verify(
              'sha256',
              requestHash,
              publicKey,
    ...
    const apple = createAppleAppAttestVerifier({
      ...
      counterStore,
      verifyAssertion: /** @type {(input: { assertionObject: *, expectedBinding: *, clientDataHash: Uint8Array, keyId: string, environment: * }) => Promise<object>} */ (async ({ assertionObject, expectedBinding, clientDataHash, keyId, environment }) => {
