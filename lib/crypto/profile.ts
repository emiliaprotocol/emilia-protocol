...
import { getCryptoProfileId } from '../env.js';
 ...
 */
export function getActiveCryptoProfile(profileId?: string): any {
  const id = (profileId || getCryptoProfileId() || 'default').trim();
  const profile = (PROFILES as Record<string, any>)[id];
  if (!profile) {
    const err = new Error(
      `Unknown EP_CRYPTO_PROFILE "${id}". Allowed: ${CRYPTO_PROFILE_IDS.join(
        ', ',
      )}. Refusing to run with an undeclared crypto boundary (fail closed).`,
 ...
 */
export function isAlgAllowed(alg: string, profile?: any): boolean {
  const p = profile || getActiveCryptoProfile();
  return p.sign_algs.includes(alg);
}
 ...
 */
export function assertAlgAllowed(
  alg: string,
  profile?: any,
): { ok: true; profile: string } {
  const p = profile || getActiveCryptoProfile();
  if (!isAlgAllowed(alg, p)) {
    const err = new Error(
      `Algorithm "${alg}" is not permitted under crypto profile "${p.id}" (allowed: ${p.sign_algs.join(
        ', ',
      )}). ` +
        (p.fips_boundary
 ...
 */
export function assertProfileSatisfied({
  custodyMode,
  profileId,
}: any = {}): {
  ok: boolean;
  ...
  if (
    profile.requires_custody &&
    !['kms', 'hsm'].includes(custodyMode ?? '')
  ) {
    reasons.push(
      `crypto profile "${profile.id}" requires signing in a validated module ` +
        `(EP_KEY_CUSTODY_MODE=kms or hsm), but custody mode is "${
          custodyMode || 'unset'
        }".`,
    );
  }
