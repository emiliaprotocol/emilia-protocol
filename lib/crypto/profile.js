...
// Generated from profile.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
//
...
export function getActiveCryptoProfile(profileId) {
    ...
    if (!profile) {
        const err = new Error(`Unknown EP_CRYPTO_PROFILE "${id}". Allowed: ${CRYPTO_PROFILE_IDS.join(', ')}. Refusing to run with an undeclared crypto boundary (fail closed).`);
 ...
 */
export function isAlgAllowed(alg, profile) {
 ...
 */
export function assertAlgAllowed(alg, profile) {
    const p = profile || getActiveCryptoProfile();
    if (!isAlgAllowed(alg, p)) {
        const err = new Error(`Algorithm "${alg}" is not permitted under crypto profile "${p.id}" (allowed: ${p.sign_algs.join(', ')}). ` +
            (p.fips_boundary
...
export function assertProfileSatisfied({ custodyMode, profileId, } = {}) {
    ...
    const reasons = [];
    if (profile.requires_custody &&
        !['kms', 'hsm'].includes(custodyMode ?? '')) {
        reasons.push(`crypto profile "${profile.id}" requires signing in a validated module ` +
