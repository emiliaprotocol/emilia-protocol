 ...
 * @param {AdmissibilityProfile} profile
 ...
 * @returns {{ verdict:string, profile_hash:string, replay_digest:string,
 *             requirement_results:{evidence:string,satisfied:boolean,reason:string}[],
 *             evaluated_at:string }}
 */
export function evaluateAdmissibilityProfile(profile, evidenceBundle, ctx = {}) {
  const nowMs = typeof ctx.now === 'number' ? ctx.now : Date.parse(ctx.now);
  ...
  for (const req of profile.requires) {
    ...
    if (fact) {
      facts.push(fact);
      if (Number.isFinite(req.max_staleness_sec)) freshnessSec[req.evidence] = req.max_staleness_sec;
