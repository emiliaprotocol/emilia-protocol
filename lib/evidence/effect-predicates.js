 ...
 * - Unknown predicate ops and unknown members of a signed prediction entry
 *   are malformed (the signer may have intended a constraint this evaluator
 *   would silently ignore). Observed entries MAY carry extra metadata; only
 *   the compared fields are read.
...
export function validatePredictedEffects(predicted) {
  ...
  predicted.forEach((entry, i) => {
    ...
    if (typeof entry.effect_type !== 'string' || !entry.effect_type) return bad(`${at}.effect_type must be a non-empty string`);
    if (typeof entry.target !== 'string' || !entry.target) return bad(`${at}.target must be a non-empty string`);
    ...
    if (p.op === 'eq') {
      ...
      if (typeof p.value !== 'string') return bad(`${at}.predicate.value must be a string`);
    }
    if (p.op === 'lte' || p.op === 'gte') {
      if (typeof p.value === 'number') return bad(`${at}.predicate.value is a number; comparison values MUST be strings (canonicalization malleability)`);
      if (typeof p.value !== 'string' || !isDecimalString(p.value)) return bad(`${at}.predicate.value must be a decimal string`);
    }
    if (p.op === 'range') {
      let broke = false;
      for (const f of ['min', 'max']) {
        ...
        else if (typeof p[f] !== 'string' || !isDecimalString(p[f])) { bad(`${at}.predicate.${f} must be a decimal string`); broke = true; }
      }
      if (!broke && compareDecimalStrings(p.min, p.max) === 1) bad(`${at}.predicate range has min > max`);
    }
    if (p.op === 'set_eq') {
      if (!Array.isArray(p.values)) return bad(`${at}.predicate.values must be an array of strings`);
      for (const v of p.values) {
        ...
        if (typeof v !== 'string') return bad(`${at}.predicate.values must contain only strings`);
      }
    }
    if (p.op === 'count_lte') {
      if (typeof p.value === 'number') return bad(`${at}.predicate.value is a number; comparison values MUST be strings (canonicalization malleability)`);
      if (typeof p.value !== 'string' || !/^(0|[1-9][0-9]*)$/.test(p.value)) return bad(`${at}.predicate.value must be a non-negative integer string`);
    }
  });
 ...
 *                           executor-attested; extra members are ignored.
