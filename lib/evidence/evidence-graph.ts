...
export function evaluateEvidenceGraph(graphDoc, policy, opts = {}) {
  ...
  const structuralFail = (why, code) => {
    ...
    // widening that shared type from this batch.
    const res = /** @type {*} */ (evaluateAdmissibility({ components: [/** @type {*} */ ({ type: 'evidence_graph', verified: false })] }, policy, { as_of: opts.as_of }));
  ...
  const result = /** @type {*} */ (evaluateAdmissibility(
    ...
    if (policyPredictionsMalformed) {
      const evaluation = {
        ...
        reasons: suppliedPolicyPredictions
          ...
          ? /** @type {{ reasons: string[] }} */ (policyPredictionValidation).reasons
