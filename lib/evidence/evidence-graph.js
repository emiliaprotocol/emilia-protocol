...
// Generated from evidence-graph.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
/**
...
import { evaluatePredictedEffects, predictedEffectsDigest, validatePredictedEffects, } from './effect-predicates.js';
...
export function evaluateEvidenceGraph(graphDoc, policy, opts = {}) {
    ...
    try {
        if (graphDoc && typeof graphDoc === 'object')
            graphDoc = JSON.parse(canon(graphDoc));
        if (policy && typeof policy === 'object')
            policy = JSON.parse(canon(policy));
    }
    catch {
        graphDoc = null;
        policy = null;
    ...
    const structuralFail = (why, code) => {
        ...
        const res = evaluateAdmissibility({ components: [{ type: 'evidence_graph', verified: false }] }, policy, opts.as_of === undefined ? {} : { as_of: opts.as_of });
    ...
    if (canonicalInputFailure)
        return structuralFail('graph or policy contains non-canonical JSON state', 'malformed_graph');
    if (!graphDoc || typeof graphDoc !== 'object')
        return structuralFail('graph is not an object', 'malformed_graph');
    if (graphDoc['@version'] !== EVIDENCE_GRAPH_VERSION)
        return structuralFail(`unexpected @version (want ${EVIDENCE_GRAPH_VERSION})`, 'malformed_graph');
    if (!policy || typeof policy !== 'object' || Array.isArray(policy))
        return structuralFail('no relying-party policy supplied', 'malformed_graph');
    ...
    if (!nodes || nodes.length === 0)
        return structuralFail('graph has no nodes', 'malformed_graph');
    for (const e of edges) {
        if (!EDGE_RELS.includes(e?.rel))
            return structuralFail(`unknown edge rel "${e?.rel}"`, 'malformed_graph');
    }
    ...
    const facts = nodes.map((n, idx) => {
        ...
        byId.set(id, rec);
        if (!id) {
            reasons.push(`malformed_graph: node ${row.label} has no digest id`);
            return row;
        }
        if (n.artifact == null)
            return row; // unresolved: shape-only disclosure — fails closed if required
        ...
        if (typeof v !== 'function') {
            reasons.push(`no verifier registered for node type "${n.type}"`);
            return row;
        }
        let res;
        try {
            ...
            reasons.push(`verifier threw for ${row.label}: ${e.message}`);
            return row;
        }
        ...
        if (explicitSignatureFailure)
            reasons.push(`executor_signature_invalid: ${row.label} reported a failed executor signature check`);
        if (explicitPinFailure)
            reasons.push(`executor_key_not_pinned: ${row.label} reported an unpinned executor key`);
    ...
    const edgeRows = edges.map((e) => {
        ...
        if (!from || !to) {
            reasons.push(`unbacked_edge_claim: edge ${e.rel} references a node not in the graph`);
            unbacked++;
            return row;
        }
        if (!from.verified || !to.verified)
            return row; // unresolved endpoints: edge simply doesn't count
        const checker = opts.edgeCheckers?.[e.rel];
        row.backed = checker
            ? checker(from.artifact, to.artifact, e) === true
            : edgeBackedByBytes(from.artifact, to.id);
        if (!row.backed) {
            reasons.push(`unbacked_edge_claim: "${from.node.type}" -${e.rel}-> "${to.node.type}" is not present in the source artifact's bytes`);
            unbacked++;
        }
        return row;
    });
    ...
    const result = evaluateAdmissibility({ action_digest: graphDoc.action_digest, components: effectiveFacts }, policy, opts.as_of === undefined ? {} : { as_of: opts.as_of });
    // 5) A lying graph is unverifiable regardless of what else passed.
    if (unbacked > 0 && result.verdict === 'admissible')
    ...
    if (floorSec !== null) {
        for (const f of effectiveFacts) {
            if (f.type !== CEREMONY_EVIDENCE_TYPE || !f.verified)
                continue;
    ...
    const outcomeRows = [];
    for (const f of effectiveFacts) {
        if (f.type !== EFFECT_ATTESTATION_TYPE || !f.verified)
            continue;
        const eff = f.effect || {};
        const observed = eff.observed_effect_digest;
        let approved = null;
        let approvalLinkageFailure = null;
        ...
        if (policyPredictionsMalformed) {
            const evaluation = {
                ...
                reasons: suppliedPolicyPredictions
                    ...
                    ? policyPredictionValidation.reasons
                        .map((reason) => `malformed predicted_effects: ${reason}`)
...
export function verifyRelianceResult(doc, pinnedVerifierKeys = []) {
    ...
    let key;
    try {
        key = crypto.createPublicKey({ key: Buffer.from(doc.verifier_key, 'base64url'), type: 'spki', format: 'der' });
    }
    catch {
        return { verified: false, accepted: false, checks };
    }
...
export const RELIANCE_RESULT_V2_REQUIRED_ALGORITHMS = Object.freeze(['Ed25519', 'ML-DSA-65']);
function relianceResultAlgorithmSetV2(algorithms) {
 ...
 */
export async function signRelianceResultV2(result, policy, signer, opts = {}) {
    opts = opts && typeof opts === 'object' && !Array.isArray(opts) ? opts : {};
    ...
    const pub = crypto.createPublicKey(signer.privateKey).export({ type: 'spki', format: 'der' }).toString('base64url');
 ...
 */
export async function verifyRelianceResultV2(doc, pinnedVerifierKeyPairs = [], options = {}) {
    const checks = {
    ...
    checks.issuer_pinned = pinnedVerifierKeyPairs.some((p) => p?.verifier_key === doc.proof.verifier_key && p?.pq_verifier_key === doc.proof.pq_verifier_key);
