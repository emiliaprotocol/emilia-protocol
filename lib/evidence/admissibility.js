 ...
 * @param {object} [opts]  { verifiers?, keysByType?, as_of? } — forwarded to the chain
...
export function evaluateChainAdmissibility(aec, policy, opts = {}) {
  const chain = verifyAuthorizationChain(aec, {
    verifiers: opts.verifiers, keysByType: opts.keysByType, requirement: policy?.requirement,
  });
  ...
  result.chain = { allow: chain.allow, reasons: chain.reasons, requirement_source: chain.requirement_source };
  return result;
}
