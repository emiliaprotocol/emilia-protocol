      ...
      try {
        ...
        if (error) {
          ...
          if (isMissingTable) {
            ...
            logger.error('[EP Attribution] Failed to write delegation signal:', error.message);
          }
        } else {
        ...
        logger.error('[EP Attribution] Unexpected error writing principal signal:', err.message);
      }
    }
  }
...
export async function getDelegationJudgmentScore(principalId, supabase) {
  ...
  try {
    ...
    if (error) {
      ...
      if (isMissingTable) {
        logger.warn('[EP Attribution] principal_delegation_signals table not yet created.');
      }
