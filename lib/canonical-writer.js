 ...
 * Useful as a secondary in-request audit trail; not durable across cold starts.
 */
const eventLog = [];
 ...
 * Degrades gracefully if the table does not yet exist (pre-migration).
 */
async function persistEvent(event) {
  try {
    ...
    if (!isMissingTable) console.warn('Event persistence failed:', e.message);
  }
}
...
async function materializeTrustProfile(entityDbId) {
  ...
  try {
    ...
    try {
      ...
      console.error('Trust materialization failed after 3 attempts:', e.message);
      return;
    }
  ...
  if (resolution === 'appeal_reversed') {
    ...
    if (dispute.resolution === 'upheld') {
      ...
      try {
        ...
        console.warn('Score recomputation after appeal reversal failed:', e.message);
      }
      await materializeTrustProfile(dispute.entity_id);
    }
