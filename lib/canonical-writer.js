...
async function materializeTrustProfile(entityDbId) {
  ...
  if (scoreError) {
    throw new ProtocolWriteError(
      ...
      { code: 'MATERIALIZATION_SCORE_FAILED', cause: /** @type {any} */ (scoreError) }
    );
  }
  ...
  if (receiptsError) {
    throw new ProtocolWriteError(
      ...
      { code: 'MATERIALIZATION_RECEIPTS_FAILED', cause: /** @type {any} */ (receiptsError) }
    );
  }
  ...
  if (entityError) {
    throw new ProtocolWriteError(
      ...
      { code: 'MATERIALIZATION_ENTITY_FAILED', cause: /** @type {any} */ (entityError) }
    );
  }
  ...
  await withRetry(async () => {
    ...
    if (updateError) {
      throw new ProtocolWriteError(
        ...
        { code: 'MATERIALIZATION_UPDATE_FAILED', cause: /** @type {any} */ (updateError) }
      );
    }
  });
...
export async function canonicalSubmitReceipt(params, submitterEntity) {
  const result = await createReceipt(/** @type {any} */ ({
