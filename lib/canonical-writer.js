// Generated from canonical-writer.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
/**
...
async function persistEvent(event) {
    try {
        ...
        if (!isMissingTable)
            logger.warn('Event persistence failed (non-truth-bearing, degrading gracefully):', e.message);
    }
}
...
async function recomputeAndPersistScores(supabase, entityId) {
    ...
    if (scoreError) {
        throw new ProtocolWriteError(`Score recomputation failed: ${scoreError.message}`, { code: 'SCORE_RECOMPUTATION_FAILED', cause: scoreError });
    }
    if (newScore !== null && newScore !== undefined) {
        const { error: updateError } = await supabase
            ...
            .eq('id', entityId);
        if (updateError) {
            throw new ProtocolWriteError(`Score update failed: ${updateError.message}`, { code: 'SCORE_UPDATE_FAILED', cause: updateError });
        }
    }
    return newScore;
}
...
async function updateReceiptDisputeStatus(supabase, receiptId, disputeStatus, graphWeight) {
    ...
    if (graphWeight !== undefined)
        patch.graph_weight = graphWeight;
...
async function materializeTrustProfile(entityDbId) {
    ...
    if (scoreError) {
        throw new ProtocolWriteError(`Trust materialization failed — score recomputation error: ${scoreError.message}`, { code: 'MATERIALIZATION_SCORE_FAILED', cause: scoreError });
    }
    const { data: receipts, error: receiptsError } = await supabase
        ...
        .limit(200);
    if (receiptsError) {
        throw new ProtocolWriteError(`Trust materialization failed — receipt fetch error: ${receiptsError.message}`, { code: 'MATERIALIZATION_RECEIPTS_FAILED', cause: receiptsError });
    }
    ...
    const { data: entity, error: entityError } = await supabase
        ...
        .single();
    if (entityError) {
        throw new ProtocolWriteError(`Trust materialization failed — entity fetch error: ${entityError.message}`, { code: 'MATERIALIZATION_ENTITY_FAILED', cause: entityError });
    }
    ...
    await withRetry(async () => {
        const { error: updateError } = await supabase
            ...
            .eq('id', entityDbId);
        if (updateError) {
            throw new ProtocolWriteError(`Trust materialization failed — snapshot update error: ${updateError.message}`, { code: 'MATERIALIZATION_UPDATE_FAILED', cause: updateError });
        }
    });
...
export async function canonicalSubmitReceipt(params, submitterEntity) {
    ...
    if (result.error)
        return result;
    emitEvent(result.deduplicated ? WRITE_EVENTS.RECEIPT_DEDUPLICATED : WRITE_EVENTS.RECEIPT_SUBMITTED, {
...
export async function canonicalSubmitAutoReceipt(raw, submitterEntity) {
    ...
    if (!agentBehavior && raw.outcome) {
        if (raw.outcome.completed === true)
            agentBehavior = 'completed';
        else if (raw.outcome.error_occurred === true)
            agentBehavior = 'abandoned';
    }
...
export async function canonicalBilateralConfirm(receiptId, confirmingEntityId, confirm) {
    ...
    const { data: receipt, error: receiptError } = await supabase
        ...
        .single();
    if (receiptError)
        return { error: 'Could not load bilateral receipt state', status: 503 };
    if (!receipt)
    ...
    if (deadlineMs !== null && deadlineMs < Date.parse(now)) {
        const { data: expired, error: expireError } = await supabase
            ...
            .maybeSingle();
        if (expireError)
            return { error: 'Could not persist bilateral receipt expiry', status: 503 };
        if (!expired)
            return { error: 'Receipt state changed before expiry could commit', status: 409 };
    ...
    if (transitionError)
        return { error: 'Could not persist bilateral receipt transition', status: 503 };
    if (!committed)
        return { error: 'Receipt state changed and is no longer pending confirmation', status: 409 };
    if (confirm) {
...
export async function canonicalFileDispute(params, filerEntity) {
    ...
    if (filerEntity.id === receipt.entity_id)
        filedByType = 'receipt_subject';
    else if (filerEntity.id === receipt.submitted_by)
        filedByType = 'affected_entity';
    ...
    const { data: dispute, error: insertError } = await supabase
        ...
        .single();
    if (insertError)
        return { error: 'Failed to file dispute', status: 500, _raw: insertError };
...
export async function canonicalFileReport(params) {
    ...
    if (insertError)
        return { error: 'Failed to file report', status: 500, _raw: insertError };
