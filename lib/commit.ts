...
/** kid → base64 public key */
const _trustedKeyRegistry: Map<string, string> = new Map();
 ...
 */
function registerTrustedKey(kid: string, publicKeyBase64: string): void {
  _trustedKeyRegistry.set(kid, publicKeyBase64);
}

/**
 * Look up a trusted public key by kid.
 * @returns Base64 public key or undefined if not found
 */
function getTrustedKey(kid: string): string | undefined {
 ...
 */
function getAllTrustedKeys(): Array<{ kid: string; publicKeyBase64: string }> {
...
export interface CommitSigningKeypair {
 ...
 *
 */
function getSigningKeypair(): CommitSigningKeypair {
...
let _cachedKeypair: CommitSigningKeypair | null = null;
function ensureKeypair(): CommitSigningKeypair {
  if (!_cachedKeypair) {
...
/**
 * Normalize an instant (Date | ISO string | ms number) to canonical RFC3339
 * millisecond UTC ("...Z"). Sign side passes Date objects; verify side reads the
 * value back from a Postgres timestamptz column, which PostgREST serializes as
 * "...+00:00". Without this, the two sides canonicalize to different bytes and a
 * legitimately signed commit fails verification. new Date(x).toISOString() maps
 * every parseable form to the same "Z" string.
 */
function normalizeInstant(value: unknown): unknown {
  if (value == null) return value;
  const d = value instanceof Date ? value : new Date(value as string | number);
  if (Number.isNaN(d.getTime())) return value; // leave unparseable input to fail elsewhere
  return d.toISOString();
}

/**
 * Recursively sort object keys so nested objects (scope, context) canonicalize
 * identically whether they came straight from JS (insertion order) or were read
 ...
 */
function signPayload(payload: string): { signature: string; publicKeyBase64: string } {
 ...
 * @param payload - The canonical JSON string
 ...
 */
function requireServiceClient(): ReturnType<typeof getServiceClient> {
  try {
...
export class CommitError extends Error {
  ...
  constructor(message: string, status = 500, code = 'COMMIT_ERROR') {
    super(message);
...
export interface IssueCommitParams {
  ...
  /** Other side of interaction */
  counterparty_entity_id?: string | null;
  /** If acting under delegation */
  delegation_id?: string | null;
  /** install | connect | delegate | transact */
  action_type: string;
  /** Action scope details */
  scope?: Record<string, unknown> | null;
  /** Advisory only. This field informs policy evaluation.
   * EP does not enforce, hold, or settle monetary value. */
  max_value_usd?: number | null;
  /** Context for trust evaluation */
  context?: Record<string, unknown> | null;
  /** Policy name or custom policy object */
  policy?: string | Record<string, unknown> | null;
  /** Custom expiry in milliseconds (default: 10 min) */
  expiry_ms?: number;
}

/** A signed EP Commit record, as stored in and read back from the `commits` table. */
...
/** The custody-boundary signer returned by lib/key-custody.js's resolveIssuerSigner(). */
interface CustodySigner {
  keyId: string;
  sign(data: Buffer): Promise<string>;
  publicKeySpkiB64u: string | (() => string | Promise<string>);
}
...
export async function issueCommit({
...
}: IssueCommitParams): Promise<CommitRecord> {
  // ── Validate inputs ──
  if (!entity_id) {
  ...
  // Resolve the signer through the custody boundary. KMS/HSM mode returns a
  // registered custody signer (and fails closed if none is registered);
  // local-dev/env returns null → the built-in env-key signer below. The kid is
  // part of the signed payload, so it must reflect the actual signing key.
  let custodySigner: CustodySigner | null = null;
  try {
    const custodyConfig = typeof getKeyCustodyConfig === 'function' ? getKeyCustodyConfig() : null;
    if (custodyConfig) custodySigner = resolveIssuerSigner(custodyConfig) as CustodySigner | null;
  } catch (e: any) {
    // A genuine custody MISCONFIG (kms/hsm mode without a registered signer or a
    // key id) must fail closed — surface it as the issuance error type. Anything
    // else (e.g. config unavailable under a partial test mock) falls back to the
    // built-in env-key signer below.
    if (['custody_signer_not_registered', 'missing_custody_key_id', 'unknown_key_custody_mode'].includes(e.code)) {
      throw new ProtocolWriteError(`commit signing custody error: ${e.message}`, 500 as any);
    }
    custodySigner = null;
  }
  const kid = custodySigner ? custodySigner.keyId : 'ep-signing-key-1';
  ...
  const canonicalFields = {
    ...
    context,
    nonce,
    expires_at: normalizeInstant(expires_at),
    created_at: normalizeInstant(now),
  };
  ...
  if (custodySigner) {
    // Sign through KMS/HSM. Bridge formats to the commit registry's convention:
    // base64 raw signature + base64 raw 32-byte public key (registered by kid so
    // verifyCommit resolves it).
    const sigB64u = await custodySigner.sign(Buffer.from(canonicalPayload, 'utf8'));
    signature = Buffer.from(sigB64u, 'base64url').toString('base64');
    const spkiB64u = typeof custodySigner.publicKeySpkiB64u === 'function'
      ? await custodySigner.publicKeySpkiB64u()
      : custodySigner.publicKeySpkiB64u;
    if (!spkiB64u) {
      throw new ProtocolWriteError('custody signer did not expose a public key (getPublicKey required to register the verification key)', 500 as any);
    }
    const rawPub = Buffer.from(spkiB64u, 'base64url');
    publicKeyBase64 = (rawPub.length === 32 ? rawPub : rawPub.subarray(rawPub.length - 32)).toString('base64');
    registerTrustedKey(kid, publicKeyBase64);
  } else {
  ...
  if (error) {
    throw new ProtocolWriteError(
      ...
      { status: 500, code: 'COMMIT_STORAGE_FAILED', cause: error as any }
    );
  }
...
export async function verifyCommit(commit_id_or_token: string): Promise<{
  ...
  if (fetchError) {
    throw new ProtocolWriteError(
      ...
      { status: 500, code: 'COMMIT_VERIFY_DB_ERROR', cause: fetchError as any }
    );
  }
  ...
  if (commit.nonce) {
    ...
    if (dupeError) {
      throw new ProtocolWriteError(
        `Commit replay check failed: ${dupeError.message}`,
        { status: 503, code: 'COMMIT_REPLAY_CHECK_FAILED', cause: dupeError as any }
      );
    }
  ...
  // ── Emergency commit-key revocation (T6) ──
  // A revoked signing kid is treated as compromised: EVERY commit bearing it is
  // rejected, regardless of an otherwise-valid signature, so a leaked key cannot
  // continue to sustain authorizations. Legitimate commits are re-issued under a
  // fresh kid. A revocation lookup failure is indistinguishable from a hidden
  // compromised key, so verification must fail closed.
  const { data: revokedKey, error: revokedKeyError } = await supabase
  ...
  if (revokedKeyError) {
    throw new ProtocolWriteError(
      `Commit signing-key revocation check failed: ${revokedKeyError.message}`,
      { status: 503, code: 'COMMIT_KEY_REVOCATION_CHECK_FAILED', cause: revokedKeyError as any }
    );
  }
  if (revokedKey) {
    reasons.push('kid_revoked');
    return {
      valid: false,
      status: commit.status,
      decision: commit.decision,
  ...
  const canonicalFields = {
    ...
    nonce: commit.nonce,
    expires_at: normalizeInstant(commit.expires_at),
    created_at: normalizeInstant(commit.created_at),
  };
 ...
 *
 * @param commit_id - Commit to revoke
 * @param reason - Why the commit is being revoked
 */
export async function revokeCommit(
  commit_id: string,
  reason: string,
): Promise<{ success: boolean; commit_id: string; reason?: string }> {
  if (!commit_id) {
  ...
  if (fetchError) {
    throw new ProtocolWriteError(
      ...
      { status: 500, code: 'REVOCATION_FETCH_FAILED', cause: fetchError as any }
    );
  }
  if (!commit) {
 ...
 *
 * @param commit_id - Commit to fulfill
 * @returns The updated commit record
 */
export async function fulfillCommit(commit_id: string): Promise<CommitRecord> {
  if (!commit_id) {
    throw new CommitError('commit_id is required', 400, 'VALIDATION_ERROR');
  }
  ...
  if (fetchError) {
    throw new ProtocolWriteError(
      ...
      { status: 500, code: 'FULFILLMENT_FETCH_FAILED', cause: fetchError as any }
    );
  }
  if (!commit) {
    throw new CommitError('Commit not found', 404, 'NOT_FOUND');
 ...
 *
 * @returns Commit record or null if not found
 */
export async function getCommitStatus(commit_id: string): Promise<CommitRecord | null> {
  if (!commit_id) {
    throw new CommitError('commit_id is required', 400, 'VALIDATION_ERROR');
  }
  ...
  if (error) {
    throw new ProtocolWriteError(
      ...
      { status: 500, code: 'COMMIT_STATUS_FETCH_FAILED', cause: error as any }
    );
  }
  if (!commit) {
    return null;
...
/**
 * Bind a receipt to an ACTIVE commit for post-action accountability.
 * This links the pre-action authorization (commit) to the post-action record (receipt).
 *
 * The link is append-only: once a commit reaches a terminal state
 * (fulfilled/revoked/expired) its receipt is immutable and cannot be rebound.
 */
export async function bindReceiptToCommit(
  commit_id: string,
  receipt_id: string,
): Promise<{ success: boolean; commit_id: string; receipt_id: string }> {
  ...
  // receipt_id must be selected: the first-write-wins idempotency guard below
  // reads commit.receipt_id. Selecting only status left it undefined, so the
  // guard was dead — a legitimate re-bind of the SAME receipt (idempotent retry
  // of the bind->fulfill flow) fell through to the `receipt_id IS NULL` mutation
  // and 409'd instead of returning success. (Bug surfaced by lib/ type-checking.)
  const { data: commit, error: fetchError } = await supabase
    .from('commits')
  ...
  if (fetchError) {
    throw new ProtocolWriteError(
      ...
      { status: 500, code: 'BIND_RECEIPT_FETCH_FAILED', cause: fetchError as any }
    );
  }
  if (!commit) {
    throw new CommitError('Commit not found', 404, 'NOT_FOUND');
  ...
  // Receipts may only be bound to an ACTIVE commit. Once a commit reaches a
  // terminal state the authorization->receipt link is append-only: rebinding
  // would overwrite the receipt already recorded against a completed
  // authorization, breaking the integrity of the pre-auth -> outcome chain.
  if (commit.status !== 'active') {
    throw new CommitError(
      `Cannot bind receipt to commit in '${commit.status}' state — terminal states are immutable`,
      409, 'INVALID_STATE_FOR_RECEIPT'
    );
  }

  // First-write-wins: the link is append-only even WHILE the commit is
  // active. Re-binding the SAME receipt is an idempotent retry (the
  // bind->fulfill flow may legitimately be re-run when fulfillment fails);
  // binding a DIFFERENT receipt over one already recorded is refused.
  if (commit.receipt_id) {
    if (commit.receipt_id === receipt_id) {
      return { success: true, commit_id, receipt_id };
    }
    throw new CommitError(
      'Cannot bind receipt: a different receipt is already bound to this commit',
      409, 'RECEIPT_ALREADY_BOUND'
    );
  }

  // Guard the MUTATION, not just the response: the UPDATE is atomic on
  // status='active' AND receipt_id IS NULL, so a commit that was fulfilled,
  // revoked, or bound concurrently (after the read above) can never have its
  // receipt overwritten — Postgres re-evaluates the predicate under the row
  // lock, so the second of two concurrent binds matches 0 rows. A 0-row
  // result means the commit left active or lost the bind race — reject.
  const { data: updatedRows, error: updateError } = await supabase
    .from('commits')
    .update({ receipt_id })
    .eq('commit_id', commit_id)
    .eq('status', 'active')
    .is('receipt_id', null) // first-write-wins: never overwrite a bound receipt
    .select('commit_id');
  ...
  if (!updatedRows || updatedRows.length === 0) {
    throw new CommitError(
      'Cannot bind receipt: commit is no longer active or a receipt is already bound',
      409, 'INVALID_STATE_FOR_RECEIPT'
    );
  }
