...
export async function runGuardPrecheck(request, spec) {
  try {
    ...
    const parsed = await readLimitedJson(request, MAX_GUARD_PRECHECK_BYTES, { invalidValue: {} });
    ...
    const actionDetails = {
      ...
      target_changed_fields: body.target_changed_fields || spec.defaultChangedFields || [],
    ...
    // authenticateRequest does not expose actorRole / authStrength today, and no
    // current policy branches on authStrength; body.actor_role + spec.actorRole
    // are advisory hints, not security controls. authStrength is set to the
    // WEAKEST value ('password') as a fail-SAFE default — an unproven request
    // escalates to signoff rather than being assumed MFA the moment a policy
    // starts gating on it. Never trust a body-supplied strength. Replace when the
    // auth layer grows verified role/strength fields. (Parallel to
