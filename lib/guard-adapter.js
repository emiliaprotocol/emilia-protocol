...
export async function runGuardPrecheck(request, spec) {
  try {
    ...
    // resolveGuardChangedFields's `defaults` parameter (lib/guard-action-inputs.ts,
    // an unannotated `= []` default) currently infers as `never[]` under native
    // .ts noImplicitAny:false, rejecting any real string[] argument. That file
    // is owned by a different conversion pass; this call-site signature cast
    // (not a persisted `any`) restores the real string[]-in/string[]-out shape
    // without changing runtime behavior.
    const changedFields = (/** @type {(body: any, defaults: string[]) => string[]} */ (resolveGuardChangedFields))(body, spec.defaultChangedFields || []);
    ...
    // has the same unannotated-`= []`-default never[] inference as
    // resolveGuardChangedFields above; call-site-only workaround, see comment there.
    const actionDetails = extractGuardActionDetails(body, /** @type {any} */ (changedFields));
