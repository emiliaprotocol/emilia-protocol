...
function derEcdsaToRawP256(der) {
    ...
    if ((sequenceLength & 0x80) !== 0 || sequenceLength !== der.length - i)
        return null;
    const readInt = () => {
        if (der[i++] !== 0x02)
            return null; // INTEGER
        const len = der[i++];
        if (!Number.isInteger(len) || len < 1 || len > 33 || i + len > der.length)
            return null;
        ...
        if (val.length > 32)
            return null; // not a P-256 component
...
export async function verifyReceipt(doc, publicKeyBase64url, opts = {}) {
    ...
    try {
        ...
        const key = await subtle.importKey('spki', bufferSource(b64uToBytes(publicKeyBase64url)), { name: 'Ed25519' }, false, ['verify']);
        checks.signature = await subtle.verify({ name: 'Ed25519' }, key, bufferSource(b64uToBytes(doc.signature.value)), bufferSource(payloadBytes));
    }
    catch (e) {
...
const FLAG_BE = 0x08;
const FLAG_BS = 0x10;
function authenticatorMetadata(authData, opts) {
...
function counterPolicyError(metadata, opts) {
...
function isMlDsa65Spki(spki) {
    if (spki.length !== ML_DSA_65_SPKI_BYTES)
        return false;
    for (let i = 0; i < ML_DSA_65_SPKI_PREFIX.length; i++) {
        if (spki[i] !== ML_DSA_65_SPKI_PREFIX[i])
            return false;
    }
    return true;
}
...
export async function verifyWebAuthnSignoff(signoff, approverPublicKeySpkiB64u, opts = {}) {
    ...
    try {
        ...
        const key = await subtle.importKey('spki', bufferSource(spkiBytes), { name: 'ECDSA', namedCurve: 'P-256' }, false, ['verify']);
        checks.signature = await subtle.verify({ name: 'ECDSA', hash: 'SHA-256' }, key, bufferSource(rawSig), bufferSource(signedData));
    }
    catch (e) {
...
export async function verifyCommitmentProof(proof, publicKeyBase64url, options = {}) {
    ...
    try {
        const key = await subtle.importKey('spki', bufferSource(b64uToBytes(publicKeyBase64url)), { name: 'Ed25519' }, false, ['verify']);
        const ok = await subtle.verify({ name: 'Ed25519' }, key, bufferSource(b64uToBytes(proof.signature.value)), bufferSource(utf8(canonicalize(proof.commitment))));
        if (!ok)
