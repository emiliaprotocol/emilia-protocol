...
// Sign an arbitrary payload with the DEMO Ed25519 key, using the same recursive
// canonicalization as /r/example — so any demo artifact (e.g. the /demo crash
// receipts) verifies against the already-published demo public key. DEMO ONLY;
// never signs production data (production receipts use EP_OPERATOR_KEYS).
export function signDemoPayload(payload) {
  const canonical = canonicalize(payload);
  return crypto.sign(null, Buffer.from(canonical, 'utf8'), getPrivateKey()).toString('base64url');
}
