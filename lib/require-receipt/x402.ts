 ...
 * @param {object} o
 * @param {string|null} [o.resource] the protected resource URL
 * @param {string|null} [o.action] the action_type the proof must be bound to
    ...
    accepts: [
      {
        ...
        network: 'offline',
        resource,
        description: description || 'Present verifiable EMILIA authorization evidence at the relying party\'s required assurance tier for this exact action.',
...
export function decodeX402Payment(headerValue) {
  return parseReceiptCarrier(headerValue);
}
 ...
 * @returns {{ ok:boolean, valid:boolean, profile?:string|null, reason?:string, detail?:string,
 *            checks?:object, errors?:string[], settlement?:object, receipt?:object }}
 */
