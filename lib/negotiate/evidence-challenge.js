...
function portableIdentifier(kind, value) {
    if (absoluteUri(value))
        return value;
...
function authoritativeInstant(epochMs) {
    if (!Number.isSafeInteger(epochMs) || epochMs < 0) {
        throw new Error('authoritative challenge owner time is invalid');
    }
    const date = new Date(epochMs);
    if (Number.isNaN(date.valueOf()))
        throw new Error('authoritative challenge owner time is outside the timestamp range');
...
function evaluateEvidencePresentation(challenge, presentation, policy, opts) {
    ...
    const verifiers = typeof opts.verifierFactory === 'function'
        ? opts.verifierFactory({ as_of: opts.as_of })
        : opts.verifiers;
