...
import { canonicalizeFinite } from './canonical-json.js';

// ─── Shared types ───────────────────────────────────────────────────────────

export type GuardActionType = typeof GUARD_ACTION_TYPES[keyof typeof GUARD_ACTION_TYPES];
export type GuardDecisionValue = typeof GUARD_DECISIONS[keyof typeof GUARD_DECISIONS];
export type EnforcementMode = typeof ENFORCEMENT_MODES[keyof typeof ENFORCEMENT_MODES];

/** Optional AML context for a guard action. See lib/aml/screening.ts screenAml(). */
export interface GuardAmlContext {
...
/** Input to evaluateGuardPolicy(). Pure policy-decision input — no I/O. */
export interface GuardPolicyInput {
  ...
  currency?: string;
  riskFlags?: string[];
  authStrength?: 'password' | 'mfa' | 'phishing_resistant_mfa' | 'service_account';
  initiatorId?: string;
  approverId?: string;
  /** Optional AML context (counterpartyName, counterpartyCountry, amount, recentAmounts). */
  aml?: GuardAmlContext;
  [key: string]: unknown;
}

/** The decision produced by evaluateGuardPolicy()/applyEnforcementMode(). */
export interface GuardPolicyDecision {
  decision: string;
  reasons: string[];
  signoffRequired: boolean;
  signoffTier?: 'single' | 'dual';
  requiredAssurance?: 'A';
  aml_signals?: string[];
  observed_decision?: string;
}

/** PIP-007 §1 initiator escalation attestation context. */
export interface InitiatorAttestationContext {
  actionType?: string;
  policyId?: string;
  targetChangedFields?: string[];
}

/** PIP-007 §1 initiator escalation attestation. */
...
function paymentSignoffTier(amount: number | undefined): 'single' | 'dual' | null {
 ...
 */
const CLASS_A_ACTIONS: readonly string[] = Object.freeze([
 ...
 */
function stampAssurance(decision: GuardPolicyDecision, input: GuardPolicyInput): GuardPolicyDecision {
 ...
 * ALLOW.
 */
const MINT_CRITICAL_FLOOR_ACTIONS: readonly string[] = Object.freeze([
  GUARD_ACTION_TYPES.BENEFIT_BANK_ACCOUNT_CHANGE,
  GUARD_ACTION_TYPES.BENEFICIARY_CREATION,
  GUARD_ACTION_TYPES.LARGE_PAYMENT_RELEASE,
]);
 ...
 */
function applyCriticalKeyClassFloor(decision: GuardPolicyDecision, input: GuardPolicyInput): GuardPolicyDecision {
...
export function guardPolicyContentHash(): string {
 ...
 */
export function computeGuardPolicyHash(policyId: string | null | undefined): string {
 ...
 *
 * @param input - the policy input. `aml` is optional AML context
 *   (counterpartyName, counterpartyCountry, amount, recentAmounts). When
 *   present, sanctions hits fail closed and structuring/velocity escalate to
 *   signoff.
 */
export function evaluateGuardPolicy(input: GuardPolicyInput): GuardPolicyDecision {
...
function basePolicy(input: GuardPolicyInput): GuardPolicyDecision {
...
function capStatement(s: string | null | undefined): string {
  const text = String(s || '');
 ...
 */
function ruleBasis(policyId: string | null | undefined, ruleName: string): string {
 ...
 *
 * @param decision
 * @param ctx.actionType          - GUARD_ACTION_TYPES.* value
 * @param ctx.policyId            - the evaluated policy id
 * @param ctx.targetChangedFields - fields the action changes
 */
export function buildInitiatorAttestation(
  decision: GuardPolicyDecision,
  { actionType, policyId, targetChangedFields }: InitiatorAttestationContext = {},
): InitiatorAttestation | undefined {
 ...
 *
 * @param base
 * @param mode - one of ENFORCEMENT_MODES
 */
export function applyEnforcementMode(base: GuardPolicyDecision, mode: string): GuardPolicyDecision {
 ...
 *
 * @param action
 * @returns hex-encoded sha256
 */
export function hashCanonicalAction(action: Record<string, unknown>): string {
  return crypto.createHash('sha256').update(canonicalizeFinite(action)).digest('hex');
}
