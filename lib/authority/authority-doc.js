...
/**
 * EP-AUTHORITY-DOC-v1 — trust establishment without prior federation.
 *
 * THE HOLE THIS FILLS
 * -------------------
 * Every evidence format in the agent-action space resolves issuer trust the
 * same way: "pinned out-of-band". Verification is specified everywhere;
 * ACCEPTANCE is specified nowhere — two organizations with no prior
 * relationship cannot begin trusting each other's receipts without a human
 * doing key ceremony by email.
 *
 * THE DESIGN (three moves)
 * 1. Make the bootstrap VERIFIABLE: an Authority Document — a signed,
 ...
 *    PREVIOUS document (TUF-style root continuity), so first contact is the
 *    only leap and everything after is mechanical.
 ...
 *    admissibility layer. A young, unendorsed issuer can be admissible for
 *    low-value actions while missing_evidence for money movement — and its
 *    acceptance widens as its verifiable history accrues. Nothing here
 *    creates trust from nothing; it makes the bootstrap checkable and the
 *    risk priced by the relying party's own policy.
 */
 ...
 * @param {object} p {org:{name,domain}, issuer_keys:[{key,custody_class,valid_from,valid_to,roles?,revoked_at?}], issued_at}
 * @param {KeyObject} rootPrivateKey    signs THIS document
...
export function resolveIssuerKeyAt(docs, kid, atISO) {
  ...
  for (let i = docs.length - 1; i >= 0; i--) {
    ...
    for (const k of entries) {
      ...
      return { key: k.key, custody_class: k.custody_class ?? null, doc_seq: docs[i].seq };
    }
