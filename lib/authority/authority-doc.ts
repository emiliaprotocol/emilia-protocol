...
export const authorityIssuerKeyId = (b64url) => (
  `ep:authority-issuer-key:sha256:${sha256hex(Buffer.from(b64url, 'base64url'))}`
);
...
function terminalRevocationMs(docs, kid) {
  ...
  for (const doc of docs) {
    if (!Array.isArray(doc?.issuer_keys)) continue;
    for (const entry of doc.issuer_keys) {
      if (entry?.kid !== kid || entry.revoked_at === undefined) continue;
      const candidate = authorityInstantMs(entry.revoked_at);
      if (Number.isFinite(candidate)) revokedAt = Math.min(revokedAt, candidate);
    }
  }
  return revokedAt;
}
...
export function verifyAuthorityChain(docs) {
  ...
  for (let i = 0; i < docs.length; i++) {
    ...
    try {
      if (toKey(d.root_key).asymmetricKeyType !== 'ed25519') throw new Error('wrong algorithm');
    } catch {
    ...
    for (const entry of d.issuer_keys) {
      ...
      try {
        if (toKey(entry.key).asymmetricKeyType !== 'ed25519') throw new Error('wrong algorithm');
      } catch {
...
export function resolveIssuerKeyAt(docs, kid, atISO) {
  ...
  if (!Array.isArray(docs) || !Number.isFinite(at)) return null;
  ...
  if (at >= terminalRevocationMs(docs, kid)) return null;
  ...
  if (!effectiveDoc || !Array.isArray(effectiveDoc.issuer_keys)) return null;
  ...
  if (!Number.isFinite(from) || !Number.isFinite(to) || at < from || at > to) return null;
  return {
    key: entry.key,
    ...
    pq_key: (entry as { pq_key?: string }).pq_key ?? null,
...
export const AUTHORITY_DOC_V2_REQUIRED_ALGORITHMS = Object.freeze(['Ed25519', 'ML-DSA-65'] as const);

interface AuthorityIssuerKeyEntryV2 extends AuthorityIssuerKeyEntry {
  /** ML-DSA-65 raw public key, base64url. */
  pq_key: string;
}

interface AuthorityDocV2Proof {
  profile: string;
  required_algorithms: string[];
  signatures: { alg: string; sig: string; key_id?: string }[];
}

interface AuthorityDocV2 {
  '@version': string;
  org: AuthorityOrg;
  seq: number;
  prev_doc_digest: string | null;
  root_key: string;
  root_pq_key: string;
  issuer_keys: AuthorityIssuerKeyEntryV2[];
  issued_at: string;
  proof: AuthorityDocV2Proof;
  continuity_proof?: AuthorityDocV2Proof;
  endorsements?: AuthorityEndorsementV2[];
}

interface AuthorityEndorsementV2 {
  by_org: string;
  by_key: string;
  by_pq_key: string;
  doc_digest: string;
...
function authorityDocAlgorithmSetV2(algorithms: unknown): algorithms is string[] {
...
export function authorityIssuerKeyIdV2(edKeyB64u: string, pqKeyB64u: string): string {
...
export function docCoreDigestV2(doc: AuthorityDocV2): string {
  const { proof, continuity_proof, endorsements, ...core } = doc as any;
  return `sha256:${sha256hex(canon(core))}`;
}

function docSelfSigningBytesV2(core: Record<string, unknown>, requiredAlgorithms: readonly string[] = AUTHORITY_DOC_V2_REQUIRED_ALGORITHMS): Buffer {
...
function continuitySigningBytesV2(docDigest: string, requiredAlgorithms: readonly string[] = AUTHORITY_DOC_V2_REQUIRED_ALGORITHMS): Buffer {
  if (!authorityDocAlgorithmSetV2(requiredAlgorithms)) {
...
function toPqPublicB64u(value: Uint8Array | string, label: string): string {
...
export interface AuthorityDocV2IssuerKeyInput {
  kid?: string;
  key: string;
  pq_key: Uint8Array | string;
  usages?: string[];
  custody_class?: string;
  valid_from: string;
  valid_to: string;
...
export interface AuthorityDocV2Signer {
  /** Ed25519 root private key. */
 ...
 */
export async function createAuthorityDocV2(
  p: { org: AuthorityOrg; issuer_keys: AuthorityDocV2IssuerKeyInput[]; issued_at: string },
  signer: AuthorityDocV2Signer,
  prev: { doc: AuthorityDocV2; continuitySigner: AuthorityDocV2Signer } | null = null,
): Promise<AuthorityDocV2> {
  const rootPub = crypto.createPublicKey(signer.privateKey as any).export({ type: 'spki', format: 'der' }).toString('base64url');
...
export async function endorseAuthorityDocV2(doc: AuthorityDocV2, endorserOrg: string, signer: AuthorityDocV2Signer): Promise<AuthorityEndorsementV2> {
  const endorserKey = crypto.createPublicKey(signer.privateKey as any).export({ type: 'spki', format: 'der' }).toString('base64url');
...
async function verifySetV2(bytes: Buffer, proof: AuthorityDocV2Proof | undefined, keys: AgileVerificationKey[], options: AgilityOptions): Promise<boolean> {
  if (!proof || proof.profile !== AUTHORITY_DOC_V2_VERSION || !authorityDocAlgorithmSetV2(proof.required_algorithms)
      || !Array.isArray(proof.signatures)) return false;
  let result;
  try {
 ...
 */
export async function verifyAuthorityChainV2(docs: unknown, options: AgilityOptions = {}): Promise<{ verified: boolean; head: AuthorityDocV2 | null; breaks: number[]; reasons: string[] }> {
  const reasons: string[] = [];
  if (!Array.isArray(docs) || docs.length === 0) return { verified: false, head: null, breaks: [], reasons: ['empty chain'] };
  let parsedDocs: AuthorityDocV2[];
  try {
  ...
  const breaks: number[] = [];
  const registryIdentityByKid = new Map<string, string | null>();
  for (let i = 0; i < parsedDocs.length; i++) {
    ...
    if (d['@version'] !== AUTHORITY_DOC_V2_VERSION) { reasons.push(`doc ${i}: bad version`); return { verified: false, head: null, breaks, reasons }; }
    if (d.seq !== i) { reasons.push(`doc ${i}: seq ${d.seq} != ${i}`); return { verified: false, head: null, breaks, reasons }; }
    if (!d.org || !isStableAuthorityIdentifier(d.org.domain)
        || (d.org.id !== undefined && !isStableAuthorityIdentifier(d.org.id))) {
      reasons.push(`doc ${i}: organization identifier or domain is unstable`);
    ...
    let rootKeyOk = false;
    try { rootKeyOk = toKey(d.root_key).asymmetricKeyType === 'ed25519'; } catch { rootKeyOk = false; }
    ...
    for (const entry of d.issuer_keys) {
      ...
      try { issuerKeyOk = toKey(entry.key).asymmetricKeyType === 'ed25519'; } catch { issuerKeyOk = false; }
      ...
      registryIdentityByKid.set(entry.kid, registryIdentity);
      if (entry.usages !== undefined
          && (!Array.isArray(entry.usages) || entry.usages.some((usage: unknown) => !isStableAuthorityIdentifier(usage))
            || new Set(entry.usages).size !== entry.usages.length)) {
        reasons.push(`doc ${i}: issuer key "${entry.kid}" has invalid usages`);
        return { verified: false, head: null, breaks, reasons };
      }
    }
    const { proof, continuity_proof, endorsements, ...core } = d as any;
    ...
    if (d.prev_doc_digest !== docCoreDigestV2(prev)) { reasons.push(`doc ${i}: prev digest mismatch (fork/equivocation)`); return { verified: false, head: null, breaks, reasons }; }
    const issuedAt = authorityInstantMs(d.issued_at);
    const previousIssuedAt = authorityInstantMs(prev.issued_at);
    if (!(issuedAt > previousIssuedAt)) {
      reasons.push(`doc ${i}: issued_at must be later than doc ${i - 1}`);
...
export async function verifyEndorsementV2(e: AuthorityEndorsementV2, doc: AuthorityDocV2, options: AgilityOptions = {}): Promise<boolean> {
  if (!e || e.doc_digest !== docCoreDigestV2(doc)) return false;
