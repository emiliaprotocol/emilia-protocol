...
import { resolveAuthority, type AuthorityStore, type AuthorityResolveInput } from './store.js';
import { authorityBinding } from './resolver.js';
import { applyAuthorityEnforcement } from './enforcement.js';

export interface ResolveAndBindAuthorityOptions {
  /** whether this action fails closed under enforce_critical */
  isCritical?: boolean;
  /** one of AUTHORITY_ENFORCEMENT_MODES (server-pinned) */
  mode?: string;
}

export interface ResolveAndBindAuthorityResult {
  result: any;
  enforcement: ReturnType<typeof applyAuthorityEnforcement>;
  binding: any;
}
 ...
 *
 * @param store   a store (supabaseAuthorityStore | snapshotStore)
 * @param input   resolver input (organization_id, principal_id/approver_id,
 *                action_type, amount, currency, policy_hash, issued_at, ...)
 * @param opts
 */
export async function resolveAndBindAuthority(
  store: AuthorityStore,
  input: AuthorityResolveInput,
  { isCritical = false, mode = 'shadow' }: ResolveAndBindAuthorityOptions = {},
): Promise<ResolveAndBindAuthorityResult> {
