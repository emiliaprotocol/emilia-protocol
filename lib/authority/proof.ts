...
import {
  ...
  authorityProofV2SignedBytes,
  authorityProofV2Digest,
...
export type {
  AuthorityProofV2,
  AuthorityProofV2Signature,
  PinnedRegistryKeyV2,
  AuthorityProofV2Options,
  VerifyAuthorityProofV2Result,
} from '../../packages/verify/authority-proof.js';
...
/**
 * A proof as received for verification: field values are attacker-controlled
 * until each is checked, hence `unknown`.
 */
...
export interface AuthorityDocumentRef {
  head_digest: string;
  head_seq: number;
  issuer_kid: string;
}

export interface AuthorityProofLimits {
  max_amount_usd?: number | null;
  currency?: string;
}

export interface AuthorityProofValidity {
  from?: string | null;
  to?: string | null;
}
...
export interface SignAuthorityProofArgs {
  authority_id?: string | null;
  /** the approver/principal the grant is for */
  subject?: string | null;
  organization_id?: string;
  /** stable identity of the registry issuer */
  registry_issuer_id?: string;
  authority_document?: AuthorityDocumentRef;
  role?: string | null;
  /** action scopes granted */
  scope?: string[];
...
// Type aliases (not interfaces) so they carry implicit index signatures and
// stay assignable to the index-signature-bearing untrusted AuthorityProof
// shape -- sign -> verify must round-trip without casts.
export type AuthorityProofSignature = {
  algorithm: 'Ed25519';
  key_id: string;
  public_key: string;
  proof_digest: string;
...
export type SignedAuthorityProof = {
  '@type': string;
  authority_id: string | null;
  subject: string | null;
  organization_id?: string;
  ...
  issued_at: string;
  limitations: string[];
  signature: AuthorityProofSignature;
};
...
export interface VerifyAuthorityProofOptions {
  pinnedRegistryKeys?: PinnedRegistryKey[];
  /** if set, the proof's registry_head must match (equivocation check) */
  expectRegistryHead?: string;
  /** if set, the proof's registry_epoch must be >= this (staleness check) */
  expectMinEpoch?: number;
}

type Checks = Record<string, boolean>;

interface VerifySignatureFailure {
  verified: false;
  accepted: false;
  checks: Checks;
  reason: string;
...
interface VerifySignatureSuccess {
  verified: true;
  accepted: false;
  checks: Checks;
  key_id: string;
...
export type VerifyAuthorityProofSignatureResult = VerifySignatureFailure | VerifySignatureSuccess;

interface VerifyProofFailure {
  verified: false;
  accepted: false;
  checks: Checks;
  reason: string;
...
export type VerifyAuthorityProofResult = VerifyProofFailure | VerifyProofSuccess;

function sha256hex(bytes: Buffer): string {
  return crypto.createHash('sha256').update(bytes).digest('hex');
}
function publicKeyToB64u(key: crypto.KeyObject): string {
  ...
  return crypto.createPublicKey(key as unknown as crypto.PublicKeyInput).export({ type: 'spki', format: 'der' }).toString('base64url');
}
...
function signingBytes(unsignedProof: Record<string, unknown>): Buffer {
  return Buffer.from(AUTHORITY_PROOF_DOMAIN + canonicalize(unsignedProof), 'utf8');
}
function unsigned(proof: AuthorityProof | Record<string, unknown>): Record<string, unknown> {
  if (!proof || typeof proof !== 'object' || Array.isArray(proof)) throw new Error('proof must be an object');
  const { signature: _sig, ...body } = proof as Record<string, unknown>;
  return body;
}

/** Digest of the signed proof body, excluding the signature envelope. */
export function authorityProofDigest(proof: AuthorityProof | Record<string, unknown>): string {
...
export function signAuthorityProof(args: SignAuthorityProofArgs, privateKey: crypto.KeyObject): SignedAuthorityProof {
  ...
  const signature: AuthorityProofSignature = {
    algorithm: 'Ed25519',
...
// ── EP-AUTHORITY-PROOF-v2 issuer (hybrid) ───────────────────────────────────
// The issuer half lives here (the v2 sibling of signAuthorityProof above); the
// verifier half is the re-exported package verifier. The signed bytes are
// produced by the package's authorityProofV2SignedBytes, so a lib-issued proof
// is byte-identical to what the package verifier recomputes.

export type AuthorityProofV2SignatureBlock = {
  profile: string;
...
export type SignedAuthorityProofV2 = Omit<SignedAuthorityProof, 'signature'> & {
  signature: AuthorityProofV2SignatureBlock;
};

export interface AuthorityProofV2Signer {
  /** Ed25519 registry issuer private key. */
...
function pqRegistryKeyIdOf(rawB64u: string): string {
 ...
 */
export async function signAuthorityProofV2(
  args: SignAuthorityProofArgs,
  signer: AuthorityProofV2Signer,
  opts: { deterministic?: boolean } = {},
): Promise<SignedAuthorityProofV2> {
  ...
  const body: Omit<SignedAuthorityProofV2, 'signature'> = {
  ...
  const ordered = AUTHORITY_PROOF_V2_REQUIRED_ALGORITHMS.map((alg) => {
    ...
    if (!s) throw new Error(`signAuthorityProofV2: signing produced no ${alg} leg`);
    return s;
  });

  const digest = authorityProofV2Digest(body as unknown as Parameters<typeof authorityProofV2Digest>[0]);
  return Object.freeze({
    ...body,
    signature: {
 ...
 */
export function verifyAuthorityProofSignature(proof: AuthorityProof | SignedAuthorityProof): VerifyAuthorityProofSignatureResult {
  const checks: Checks = {
  ...
  const fail = (reason: string, extra: { proof_digest?: string } = {}): VerifySignatureFailure => ({
    verified: false,
    accepted: false,
    checks,
    reason,
 ...
 */
export function verifyAuthorityProof(proof: AuthorityProof | SignedAuthorityProof | null | undefined, opts: VerifyAuthorityProofOptions = {}): VerifyAuthorityProofResult {
  ...
  const fail = (reason: string, extra: { checks?: Checks; proof_digest?: string } = {}): VerifyProofFailure => ({
    verified: false,
    accepted: false,
  ...
  if (Number.isSafeInteger(opts.expectMinEpoch) && !(Number.isSafeInteger(proof.registry_epoch) && (proof.registry_epoch as number) >= (opts.expectMinEpoch as number))) {
