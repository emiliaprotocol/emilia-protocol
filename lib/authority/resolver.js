...
// Generated from resolver.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
/**
...
const VERDICT = Object.freeze(Object.fromEntries(AUTHORITY_VERDICTS.map((v) => [v, v])));
...
function meetsAssurance(have, required) {
    if (!required)
        return true;
    if (!Object.hasOwn(ASSURANCE_RANK, have) || !Object.hasOwn(ASSURANCE_RANK, required))
        return false;
...
function checkDelegation(record, resolveParent, atISO) {
    ...
    for (let depth = 0; depth < MAX_DELEGATION_DEPTH; depth++) {
        if (!child.delegation_parent)
            return { ok: true };
        const parent = normalizeAuthorityRecord(resolveParent ? resolveParent(child.delegation_parent) : null);
        if (!parent)
            return { ok: false, detail: 'delegation_parent_missing' };
        if (!parent.authority_id || seen.has(parent.authority_id))
            return { ok: false, detail: 'delegation_cycle' };
        ...
        if (parentFrom !== null && parentFrom > at)
            return { ok: false, detail: 'delegation_parent_not_yet_valid' };
        if (parentTo !== null && parentTo < at)
            return { ok: false, detail: 'delegation_parent_expired' };
        if (Array.isArray(parent.action_scopes)) {
            if (!Array.isArray(child.action_scopes))
...
export function evaluateAuthorityVerdict(ctx, input) {
    ...
    if (!record)
        return out(fail(VERDICT.unknown_authority, 'no_authority_record'));
    const facts = {
    ...
    if (record.revoked_at)
        return out({ ...facts, ...fail(VERDICT.revoked_authority, 'revoked_at') });
    ...
    if (!deleg.ok)
        return out({ ...facts, ...fail(VERDICT.delegation_broken, deleg.detail) });
