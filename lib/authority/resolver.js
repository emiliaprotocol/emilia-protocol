...
function checkDelegation(record, resolveParent, atISO) {
  ...
  for (let depth = 0; depth < MAX_DELEGATION_DEPTH; depth++) {
    ...
    const parent = resolveParent ? resolveParent(child.delegation_parent) : null;
    if (!parent) return { ok: false, detail: 'delegation_parent_missing' };
    if (seen.has(parent.authority_id)) return { ok: false, detail: 'delegation_cycle' };
    ...
    if (parent.valid_from && Date.parse(parent.valid_from) > at) return { ok: false, detail: 'delegation_parent_not_yet_valid' };
    if (parent.valid_to && Date.parse(parent.valid_to) < at) return { ok: false, detail: 'delegation_parent_expired' };

    // Containment: the child may not claim a scope the parent lacks, nor a
    // higher ceiling than the parent grants. A null parent scope = unbounded
    // scope (a root); a null parent ceiling = unbounded amount.
    if (Array.isArray(parent.action_scopes) && Array.isArray(child.action_scopes)) {
    ...
    if (typeof parent.max_amount_usd === 'number'
      && typeof child.max_amount_usd === 'number'
      && child.max_amount_usd > parent.max_amount_usd) {
...
export function evaluateAuthorityVerdict(ctx, input) {
  const at = input.issued_at || new Date(0).toISOString();
  ...
  if (record.valid_to && Date.parse(record.valid_to) < Date.parse(at)) {
    return out({ ...facts, ...fail(VERDICT.expired_authority, 'valid_to_passed') });
  }
  if (record.valid_from && Date.parse(record.valid_from) > Date.parse(at)) {
  ...
  if (typeof input.amount === 'number' && typeof record.max_amount_usd === 'number') {
    const ceilingCurrency = record.currency || 'USD';
    const amountCurrency = input.currency || ceilingCurrency;
  ...
  if (record.policy_hash && input.policy_hash && record.policy_hash !== input.policy_hash) {
