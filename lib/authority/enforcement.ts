 ...
 *                     denied. Never blocks. An explicit, non-production opt-out
 *                     for populating a registry, never the default.
 ...
 *                     This is the default: an unset or unrecognized mode
 *                     enforces rather than observes, so a typo or a missing
 *                     environment variable can never disable enforcement.
 *
...
export function isAuthorityEnforcementMode(mode: any): boolean {
  return (ORDER as readonly string[]).includes(mode);
}

/** Map a resolver verdict to a stable admissibility code. */
export function authorityAdmissibilityCode(verdict: string): string {
 ...
 */
export function applyAuthorityEnforcement({
  verdict,
  isCritical,
  mode,
}: any): {
  ...
  verdict: string;
  block: boolean;
  admissibility: 'admissible' | 'observed' | 'not_admissible';
  code: string;
  warn: boolean;
  reason: string;
} {
  // Fail closed on an unrecognized mode: an unreadable rollout setting must not
  // be an implicit licence to proceed without authority.
  const m = isAuthorityEnforcementMode(mode)
    ? mode
    : AUTHORITY_ENFORCEMENT_MODES.ENFORCE_DEFAULT;
  ...
  const willBlock =
    m === AUTHORITY_ENFORCEMENT_MODES.ENFORCE_DEFAULT ||
  ...
  const warn =
    m === AUTHORITY_ENFORCEMENT_MODES.WARN ||
    (m === AUTHORITY_ENFORCEMENT_MODES.ENFORCE_CRITICAL && !isCritical);
  return {
    mode: m,
    verdict,
