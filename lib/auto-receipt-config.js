  ...
  try {
    ...
    if (error) {
      ...
      if (error.code === '42703' || error.code === '42P01') {
        console.warn('[EP auto-receipt] DB columns not yet available — using in-memory fallback');
    ...
    console.warn('[EP auto-receipt] DB unavailable, using in-memory fallback:', err.message);
    return _memoryGet(entityId);
  }
}
...
export async function setAutoReceiptConfig(entityId, { enabled, redact_fields = [], privacy_mode = DEFAULT_PRIVACY_MODE } = {}) {
  ...
  try {
    ...
    if (error) {
      if (error.code === '42703' || error.code === '42P01') {
        console.warn('[EP auto-receipt] DB columns not yet available — persisting to in-memory fallback');
    ...
    console.warn('[EP auto-receipt] DB unavailable, persisting to in-memory fallback:', err.message);
    return _memorySet(entityId, { enabled, redact_fields, privacy_mode, last_updated: now });
  }
}
