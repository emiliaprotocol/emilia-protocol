...
/**
 * The closed set of action fields this profile renders, in fixed order. The
 * renderer NEVER reads anything outside this set, so adding noise fields to an
 * action cannot change the rendering, and the rendering is total over the
 * action object (a pure function of it). A `null` value renders the literal
 * absence marker — it is still part of the deterministic output.
 */
 ...
 * runtime, in any locale. The rendering is derived ONLY from the action's
 * RENDER_FIELDS and binds to the frozen actionHash() of the SAME action. The
...
export function renderAction(action) {
  ...
  const lines = RENDER_FIELDS.map(([key, label]) => ({
    label,
 ...
 *           algorithm?: string }} [args.signer] - optional signer.
 * @returns {object} an EP-DISPLAY-ATTESTATION-v1 object.
 */
export function buildDisplayAttestation({ action, signer } = {}) {
