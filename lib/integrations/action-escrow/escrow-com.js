...
function validateCreateRequest(request) {
  ...
  if (!validText(request.description, 256)
      ...
      || request.buyerCustomer.toLowerCase() === request.sellerCustomer.toLowerCase()
...
function releaseStateResult(environment, request, transaction, milestone) {
  ...
  return closedResult(
    ...
    phase === 'disbursed' ? 'released' : 'release_submitted',
    {
      ...
      provider_milestone_id: request.milestoneId,
...
export function createEscrowComAdapter({
  ...
  const origin = validatePinnedOrigin(ESCROW_ORIGINS[environment], {
  ...
  async function createTransaction(input) {
    ...
    if (prior.kind !== 'not_found') {
      return providerFailure(environment, 'create_transaction_preflight', prior);
    }
  ...
  async function releaseMilestone(input) {
    ...
    if (milestonePhase(before.milestone) !== 'not_accepted') {
      return releaseStateResult(environment, request, before.transaction, before.milestone);
    }
    const isAuthenticatedBuyer = before.transaction.parties.some(
      (party) => party.role === 'buyer'
        && comparableCustomer(party.customer, email) === email.toLowerCase(),
    );
    ...
    if (!after.result && milestonePhase(after.milestone) !== 'not_accepted') {
      return releaseStateResult(environment, request, after.transaction, after.milestone);
    }
    ...
    return closedResult(environment, 'indeterminate', {
      ...
      provider_transaction_id: request.transactionId,
      provider_milestone_id: request.milestoneId,
      http_status: attempted.status ?? null,
    });
  }
