...
const SENSITIVE_METADATA_KEY =
  /(?:^|[_-])(?:api[_-]?(?:key|token)|access[_-]?token|refresh[_-]?token|oauth[_-]?token|password|client[_-]?secret|secret)(?:$|[_-])|^(?:credential|credentials|authorization)$/i;
 ...
 *
 * @typedef {Object} ExternalCustodianAdapter
 * @property {'external_custodian'} kind
 * @property {string} provider
 * @property {'sandbox'|'production'} environment
 * @property {Readonly<Record<string, unknown>>} customer_diligence
 * @property {Readonly<{
 ...
 * }>} capabilities
 * @property {(request: object) => Promise<CustodianCreateResult>} createTransaction
 * @property {(request: object) => Promise<CustodianReconcileResult>} reconcileTransaction
 * @property {(request: object) => Promise<CustodianReleaseResult>} releaseMilestone
 * @property {(request: object) => Promise<CustodianDisbursementResult>} requestMilestoneDisbursement
 */
...
function bindClosedOperation(definition, method) {
  ...
  return async (...args) => {
    ...
    if (!isPlainRecord(result) || !allowedKinds.includes(result.kind)) {
 ...
 * @returns {ExternalCustodianAdapter}
 */
