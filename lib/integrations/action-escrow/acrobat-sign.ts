...
function normalizeStrongEtag(value) {
  if (typeof value !== 'string' || /^W\//i.test(value)) return null;
...
function metadataEtag(response) {
  ...
  if (rawEtag === null) return { valid: true, value: null };
...
function normalizeExpected(expected) {
  ...
  participantSets.sort((left, right) => (
    left.order - right.order || left.set_id.localeCompare(right.set_id)
  ));
  return {
...
function normalizeParticipantSets(value) {
  ...
  for (const set of sets) {
    ...
    if (typeof memberStatus !== 'string') return null;
...
function normalizeAgreement(value, response) {
  ...
  if (!etag.valid || !sets) return null;
  return {
...
function normalizeEventInstant(value) {
  if (!validString(value, 64)) return null;
...
function normalizeAgreementEvents(value) {
  ...
  events.sort((left, right) => (
    left.event_at_epoch_ms - right.event_at_epoch_ms
  ...
  if (latestVersions.size !== 1) return null;
...
export function createAcrobatSignAdapter({
  ...
  async function fetchFinalEvidence({
    ...
    const initialSnapshot = await fetchAgreementSnapshot(
      encodedAgreementId,
      deadline,
      'fetch_agreement',
    );
    ...
    if (!participantSetsMatch(
      agreement.participant_sets,
      normalizedExpected.participantSets,
    )) {
      return closedResult('mismatch', {
        operation: 'fetch_final_evidence',
    ...
    const initialEvents = await fetchEventSnapshot(
      encodedAgreementId,
      deadline,
    ...
    const documentResponse = await call(
      `/api/rest/v6/agreements/${encodedAgreementId}/combinedDocument`
        + `?versionId=${encodeURIComponent(initialEvents.events.version_id)}`,
      'application/pdf',
      documentLimit,
      deadline,
    );
    ...
    const finalSnapshot = await fetchAgreementSnapshot(
      encodedAgreementId,
      deadline,
      'refetch_agreement',
    );
