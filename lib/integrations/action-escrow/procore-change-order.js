...
// Generated from procore-change-order.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
...
const PROJECT_RECORD_CLAIM_BOUNDARY = 'The provider reported these project facts at the observation time. '
...
function normalizeChangeOrder(value, expectedId) {
    ...
    const total = decimal(value.grand_total ?? value.total_amount ?? value.amount);
    const updatedAt = optionalString(value.updated_at, 64);
    const updatedAtMs = strictInstantMs(updatedAt);
    const contractId = identifier(value.contract_id
        ?? value.commitment_id
...
export function verifyProcoreChangeOrderEvidence(value, expected = {}) {
    try {
        ...
        const normalizedChangeOrder = normalizeChangeOrder(value.change_order, expectedChangeOrderId);
...
export function createProcoreChangeOrderAdapter({ apiOrigin = 'https://api.procore.com', accessToken, companyId, fetch: fetchImpl, timeoutMs = DEFAULT_TIMEOUT_MS, maxMetadataBytes = DEFAULT_MAX_METADATA_BYTES, clock = () => new Date().toISOString(), } = {}) {
    const origin = validateProcoreOrigin(apiOrigin);
    const normalizedCompanyId = identifier(companyId);
    if (!normalizedCompanyId)
        throw new TypeError('companyId is invalid');
    ...
    async function call(path, deadline) {
        ...
        return requestBounded(fetchImpl, `${origin}${path}`, {
            method: 'GET',
            headers: {
    ...
    async function fetchLineItems(resource, projectId, changeOrderId, deadline) {
        ...
        while (items.length <= MAX_LINE_ITEMS) {
            ...
            if (!fetched.ok) {
                return { ok: false, result: fetched.result };
            }
    ...
    async function fetchSnapshot(resource, projectId, changeOrderId, deadline) {
        ...
        if (!order.ok) {
            return { ok: false, result: order.result };
        }
        ...
        const lineItems = await fetchLineItems(resource, projectId, changeOrderId, deadline);
        if (!lineItems.ok) {
            return { ok: false, result: lineItems.result };
        }
        return {
            ok: true,
            value: {
    ...
    async function fetchChangeOrderEvidence(input = {}) {
        ...
        const { projectId, changeOrderId, changeOrderType, expected, } = input;
        ...
        const initial = await fetchSnapshot(resource, normalizedProjectId, normalizedChangeOrderId, deadline);
        if (!initial.ok)
            return initial.result;
        if (!expectationsMatch(initial.value.change_order, normalizedExpected)) {
            return closedResult(initial.value.change_order.status === normalizedExpected.status
                ? 'mismatch'
                : 'not_final', {
                operation: 'fetch_change_order_evidence',
        ...
        const final = await fetchSnapshot(resource, normalizedProjectId, normalizedChangeOrderId, deadline);
        if (!final.ok)
            return final.result;
