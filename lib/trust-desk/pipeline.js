...
export async function runPipeline({ engagement, persist = true }) {
  ...
  const persistStatus = (status, extra) => {
    if (persist && id && getEngagement(id)) setStatus(id, status, extra);
  };

  try {
    // ── 1. EXTRACT ──
    persistStatus(STATUS.EXTRACTING);
    let extraction;
    try {
    ...
    persistStatus(STATUS.MINTING);
    const policiesDir = path.join(process.cwd(), 'data', 'trust-desk', 'customers', slug, 'policies');
    const policies = mintPolicies({ intake, slug, outDir: policiesDir });
    const minted = mintTrustPage({ engagement, answers, verification, policies, slug });
    ...
    persistStatus(STATUS.PUBLISHED, {
      slug,
    ...
    persistStatus(STATUS.FAILED, { error: err.message });
    return {
      ok: false,
      outcome: 'failed',
      engagement_id: id,
...
function finishEscalated({ id, slug, persist, reason, detail, engagement, t0, verification, answers }) {
  const perQuestionEscalations = (answers || [])
    .filter((a) => a.status !== 'answered')
    .map((a) => ({ id: a.id, reason: a.escalation_reason }));

  if (persist && id && getEngagement(id)) {
    setStatus(id, STATUS.ESCALATED, {
