...
export async function setStatus(engagementId, status, extra = {}) {
    if (storeBackend() === 'supabase')
        return (await sb()).setStatus(engagementId, status, extra);
    const current = fileGet(engagementId);
    if (!current)
        return null;
 ...
 */
export async function compareAndSetStatus(engagementId, expectedStatus, status, extra = {}) {
    if (storeBackend() === 'supabase') {
...
export async function listEngagements() {
    if (storeBackend() === 'supabase')
        return (await sb()).listEngagements();
    ensureDir();
    return fs
