 ...
 * @license Apache-2.0
 *
 * Builds the published trust-page document (data/trust-desk/customers/
 * <slug>.json) from a completed pipeline run. Every claim carries a NON-NULL
 * content_hash bound to a real artifact, and a STORED signed envelope
 * (reproducible HMAC) so a buyer can verify the same signature the page shows.
 *
 * This is the file the existing renderer (app/trust-desk/c/[slug]) and
 * customers.js loader consume — the minter just produces it deterministically
 * instead of a human hand-authoring JSON.
 */
 ...
 * @param {Array}  opts.policies     mintPolicies() result
 * @param {string} opts.slug
 * @param {number} [opts.expiryMonths=6]
 * @returns {{ slug, jsonPath, claims, expires_at, published_at }}
 */
export function mintTrustPage({ engagement, answers, verification, policies, slug, expiryMonths = 6 }) {
  ...
  // ── Build the answers sidecar (buyer-facing Q&A artifact) ──
  fs.mkdirSync(CUSTOMER_DIR, { recursive: true });
  const customerSubdir = path.join(CUSTOMER_DIR, slug);
  fs.mkdirSync(customerSubdir, { recursive: true });
  ...
  fs.writeFileSync(path.join(customerSubdir, 'answers.json'), JSON.stringify(answersPayload, null, 2));

  // ── Assemble claims ──
  const rawClaims = [];

  // 1) Questionnaire response claim
  const qContentHash = hashClaim(answersPayload);
  rawClaims.push({
    ...
    summary:
      `Answered ${answered.length} of ${answers.length} questions ` +
  ...
  for (const p of policies) {
    rawClaims.push({
      ...
      bullets: sectionHeadings(p.path),
  ...
  // ── Write the published trust-page document ──
  const doc = {
    slug,
  ...
  const jsonPath = path.join(CUSTOMER_DIR, `${slug}.json`);
  ...
  return { slug, jsonPath, claims, expires_at: expiresAt, published_at: publishedAt };
}
