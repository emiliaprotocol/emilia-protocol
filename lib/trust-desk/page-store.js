// Generated from page-store.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
/**
...
export async function getPublishedPage(slug) {
    ...
    if (storeBackend() === 'supabase') {
        ...
        const getArtifact = (sourceFile) => {
            if (!sourceFile)
                return null;
            if (sourceFile.endsWith('/answers.json'))
    ...
    const getArtifact = (sourceFile) => {
        ...
        if (!p.startsWith(CUSTOMER_DIR + path.sep) || !fs.existsSync(p))
            return null;
...
export async function getPageMonitor(slug) {
    if (storeBackend() === 'supabase')
        return (await sb()).getPageMonitor(slug);
    try {
...
export async function setPageMonitor(slug, monitor) {
    if (storeBackend() === 'supabase')
        return (await sb()).setPageMonitor(slug, monitor);
    const dir = path.join(CUSTOMER_DIR, slug);
