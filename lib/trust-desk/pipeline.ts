  ...
  try {
    ...
    if (requiresReview()) {
      ...
      await notifyInternal(
        `Trust Desk: engagement ${id} (${intake.company || 'unknown company'}) is prepared and `
      ...
      return {
        ...
        escalated_questions: answers.filter((a: any) => a.status !== 'answered').map((a: any) => ({
...
export async function publishReviewed({
  ...
  reviewer,
  note,
}: { engagementId: string; reviewer: string; note?: string }): Promise<any> {
  const t0 = Date.now();
  ...
  try {
    const claimed = await compareAndSetStatus(
      engagementId,
      STATUS.AWAITING_REVIEW,
      STATUS.MINTING,
      { reviewer: reviewer.trim(), approved_at: approvedAt },
    );
    ...
    return {
      ...
      decision: verification.decision,
      counts: verification.counts,
      policies: policies.map((p: any) => ({ doc_id: p.doc_id, content_hash: p.content_hash })),
      claims: minted.claims.map((c: any) => ({ id: c.id, claim_id: c.claim_id, content_hash: c.content_hash })),
      expires_at: minted.expires_at,
      ep_receipt: epReceipt,
      duration_ms: Date.now() - t0,
      notify,
 ...
 * decision to publish.
 */
export async function rejectReviewed({
  engagementId,
  reviewer,
  reason,
}: { engagementId: string; reviewer: string; reason: string }): Promise<any> {
  if (!reviewer?.trim()) return { ok: false, error: 'reviewer_required' };
  ...
  const claimed = await compareAndSetStatus(
    ...
    STATUS.AWAITING_REVIEW,
    STATUS.REJECTED,
    { reviewer: reviewer.trim(), reason: reason.trim(), rejected_at: rejectedAt },
  );
  if (!claimed.ok) {
    return {
      ok: false,
