// Generated from resource-budget.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
/**
...
export function assertQuestionnaireWithinBudget(questions) {
    const count = Array.isArray(questions) ? questions.length : 0;
    if (count > TRUST_DESK_RESOURCE_LIMITS.maxQuestions) {
        throw new TrustDeskResourceLimitError('question_count_exceeded', `questionnaire has ${count} questions; maximum is ${TRUST_DESK_RESOURCE_LIMITS.maxQuestions}`, TRUST_DESK_RESOURCE_LIMITS.maxQuestions, count);
    }
    let totalChars = 0;
    for (const question of questions || []) {
        const chars = String(question?.text || '').length;
        if (chars > TRUST_DESK_RESOURCE_LIMITS.maxQuestionChars) {
            throw new TrustDeskResourceLimitError('question_length_exceeded', `one question has ${chars} characters; maximum is ${TRUST_DESK_RESOURCE_LIMITS.maxQuestionChars}`, TRUST_DESK_RESOURCE_LIMITS.maxQuestionChars, chars);
        }
        totalChars += chars;
        if (totalChars > TRUST_DESK_RESOURCE_LIMITS.maxTotalQuestionChars) {
            throw new TrustDeskResourceLimitError('question_text_budget_exceeded', `question text totals ${totalChars} characters; maximum is ${TRUST_DESK_RESOURCE_LIMITS.maxTotalQuestionChars}`, TRUST_DESK_RESOURCE_LIMITS.maxTotalQuestionChars, totalChars);
        }
    }
}
export function createTrustDeskLlmBudget({ maxCalls = TRUST_DESK_RESOURCE_LIMITS.maxLlmCalls, maxEstimatedTokens = TRUST_DESK_RESOURCE_LIMITS.maxEstimatedTokens, maxWallClockMs = TRUST_DESK_RESOURCE_LIMITS.maxLlmWallClockMs, now = Date.now, } = {}) {
    ...
    return {
        reserve({ system = '', user = '', maxTokens = 900 } = {}) {
...
function estimateInputTokens(system, user) {
