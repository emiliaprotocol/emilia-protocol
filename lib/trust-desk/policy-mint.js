// Generated from policy-mint.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
/**
...
function fallbackFor(name, vars) {
    if (/_EMAIL$/.test(name))
        return vars.SECURITY_LEAD_EMAIL || 'security@vendor.example';
    if (/_SLA$|_WINDOW$|_TTL$/.test(name))
        return 'as specified in the MSA';
    if (/_CADENCE$|_FREQUENCY$/.test(name))
        return 'periodic';
    if (/_RETENTION$/.test(name))
        return 'per the data retention policy';
    if (/_REGION$/.test(name))
        return 'United States';
    if (/_DPA$|_DPA_DATE$/.test(name))
        return 'on file';
    if (/_PROVIDER$|_TOOL$|_DB$/.test(name))
        return 'as disclosed in the subprocessor map';
    if (/_NAME$/.test(name))
        return vars.SECURITY_LEAD_NAME || 'Security Team';
    if (/_TITLE$/.test(name))
        return 'Authorized Representative';
    if (/_URL$/.test(name))
        return vars.TRUST_PAGE_URL || 'https://www.emiliaprotocol.ai/trust-desk';
    if (/_CADENCE|_SLA|_LIMIT|_RATE/.test(name))
        return 'as configured per tenant';
