// Generated from llm.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
/**
...
class LlmOwnedTimeoutError extends Error {
    enforcesBudgetDeadline;
    constructor(enforcesBudgetDeadline, cause) {
 ...
 */
export async function llmJSON({ system, user, maxTokens = 900, temperature = 0, validate, budget, } = {}) {
    const provider = activeProvider();
    if (!provider)
        return { ok: false, reason: 'no_provider', provider: null };
    ...
    if (!budget)
        return { ok: false, reason: 'budget_required', provider };
...
async function withAbortTimeout(timeout, operation) {
    ...
    try {
        ...
        if (deadlineElapsed)
            throw new LlmOwnedTimeoutError(timeout.enforcesBudgetDeadline, err);
        throw err;
    }
    finally {
        clearTimeout(timer);
