// Generated from answerer.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
/**
...
import { assertQuestionnaireWithinBudget, createTrustDeskLlmBudget, } from './resource-budget.js';
...
export async function answerQuestion(q, ctx) {
    const boundedCtx = {
        ...(ctx || {}),
        llmBudget: ctx?.llmBudget || createTrustDeskLlmBudget(),
    };
    switch (q.bucket) {
        case BUCKET.AI_TEMPLATE_MATCH:
            return await answerFromTemplate(q, boundedCtx);
        case BUCKET.SOC2_OVERLAP:
            return answerFromSoc2(q, boundedCtx);
        case BUCKET.CUSTOMER_SPECIFIC:
            return answerFromIntakeFact(q, boundedCtx);
        case BUCKET.AI_SPECIFIC:
            return answerWithLlm(q, boundedCtx);
        case BUCKET.NOVEL:
        default:
...
export async function answerAll(questions, ctx, concurrency = 8) {
    ...
    const llmBudget = ctx?.llmBudget || createTrustDeskLlmBudget();
    const boundedCtx = { ...(ctx || {}), llmBudget };
    ...
    for (let start = 0; start < questions.length; start += concurrency) {
        ...
        const results = await Promise.all(batch.map((q) => answerQuestion(q, boundedCtx)));
        results.forEach((r, j) => {
            out[start + j] = r;
        });
    }
...
async function answerFromTemplate(q, ctx) {
    ...
    if (!tpl || !tpl.content)
        return escalate(q, 'template_missing');
    ...
    const groundingPool = substitute(section
        ? `## ${section.heading}\n${section.body}\n\n${otherSections(tpl, section)}`
        : tpl.content, ctx.policyVars).slice(0, 8000);
...
async function composeFromPolicy(q, policyText, ctx) {
    ...
    const res = await llmJSON({
        system: 'You answer one enterprise AI security questionnaire question on behalf of a vendor, using '
        ...
        budget: ctx.llmBudget,
    });
    if (!res.ok)
        return { ok: false, reason: `llm_${res.reason}` };
    const { answer, supporting_quotes: quotes, confidence } = res.data;
    const conf = clamp01(confidence);
    const trimmed = String(answer || '').trim();
    if (!trimmed)
        return { ok: false, reason: 'policy_does_not_answer' };
    ...
    if (grounded.length === 0)
        return { ok: false, reason: 'no_verifiable_quote' };
    return {
        ok: true,
        answer: trimmed,
        confidence: conf,
 ...
 */
function isGrounded(quote, policyText) {
...
function otherSections(tpl, chosen) {
    return (tpl.sections || [])
...
function answerFromSoc2(q, ctx) {
    ...
    const answer = `${phrasing} Controls relevant to this question are covered under our SOC 2 program ` +
...
function answerFromIntakeFact(q, ctx) {
    ...
    const answer = `Per information provided by ${intake.company || 'the vendor'}: ${String(value).trim()}. ` +
...
async function answerWithLlm(q, ctx) {
    if (!llmAvailable())
        return escalate(q, 'no_llm_provider');
    const tpl = q.matched_template ? getTemplate(q.matched_template) : null;
    const grounding = tpl
        ? tpl.sections.slice(0, 8).map((s) => `## ${s.heading}\n${s.body}`).join('\n\n').slice(0, 6000)
        : '';
    const res = await llmJSON({
        system: 'You answer enterprise AI security questionnaire questions for a vendor. ' +
    ...
    if (!res.ok)
        return escalate(q, `llm_${res.reason}`);
