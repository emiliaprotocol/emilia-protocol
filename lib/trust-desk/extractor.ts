...
function preflightDocxZip(buf: Buffer): void {
  ...
  const parsedEntries: DocxZipEntry[] = [];
  ...
  for (const entry of parsedEntries) {
    ...
    if (entry.compressionMethod === 0) {
      ...
      try {
        ...
        if (result.engine.bytesWritten !== entry.compressedBytes) {
          throw new ExtractionUnsupportedError(
            'docx',
            'deflate stream length does not match its ZIP claim — escalate to a human reviewer',
          );
        }
        ...
        throw new ExtractionUnsupportedError(
          'docx',
          'deflate stream is invalid or exceeds the parser budget — escalate to a human reviewer',
        );
      }
    }
    ...
    if (actualUncompressed !== entry.uncompressedBytes
      ...
      throw new ExtractionUnsupportedError(
        'docx',
        'actual ZIP expansion does not match its size claim or parser budget — escalate to a human reviewer',
      );
    }
