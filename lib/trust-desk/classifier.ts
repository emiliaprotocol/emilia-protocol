      ...
      batch.map(async (i) => {
        ...
        if (res.ok) {
          ...
          out[i] = {
            ...
            classify_note: /** @type {LlmFailureResult} */ (res).reason,
          };
          if (
            /** @type {LlmFailureResult} */ (res).reason
            && /** @type {LlmFailureResult} */ (res).reason.startsWith('provider_error')
          ) {
            logger.warn('trust-desk classify: llm refine failed', {
              id: q.id,
              reason: /** @type {LlmFailureResult} */ (res).reason,
            });
          }
        }
      }),
