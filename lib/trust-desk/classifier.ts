...
/**
 * `strict` is off in tsconfig.lib.json, which disables control-flow narrowing
 * on the boolean `ok` discriminant returned by llmJSON(); the explicit cast
 * below on the failure branch stands in for that narrowing.
 */
type LlmFailureResult = { ok: false; reason: string; provider: string | null; raw?: string };
...
export function heuristicClassify(q: any): any {
  const text = q.text || '';
      ...
      batch.map(async (i) => {
        ...
        if (res.ok) {
          ...
          out[i] = {
            ...
            classify_note: (res as LlmFailureResult).reason,
          };
          if (
            (res as LlmFailureResult).reason
            && (res as LlmFailureResult).reason.startsWith('provider_error')
          ) {
            logger.warn('trust-desk classify: llm refine failed', {
              id: q.id,
              reason: (res as LlmFailureResult).reason,
            });
          }
        }
      }),
