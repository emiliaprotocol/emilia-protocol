// Generated from classifier.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
/**
 ...
 */
export async function classifyQuestions(questions, intake = {}, { llmBudget } = {}) {
    ...
    const refined = await refineAmbiguous(heuristic, intake, llmBudget || createTrustDeskLlmBudget());
    return refined;
}
...
// ── LLM refinement ──────────────────────────────────────────────────────────
async function refineAmbiguous(classified, intake, llmBudget) {
    ...
    for (let start = 0; start < ambiguousIdx.length; start += CONCURRENCY) {
        ...
        await Promise.all(batch.map(async (i) => {
            ...
            const res = await llmJSON({
                system: 'You classify enterprise AI security questionnaire questions. Respond ONLY with JSON: ' +
                ...
                maxTokens: 150,
                validate: (o) => o && typeof o.bucket === 'string' && Object.values(BUCKET).includes(o.bucket),
                budget: llmBudget,
            });
            if (res.ok) {
                const matched = res.data.matched_template && TEMPLATE_IDS.includes(res.data.matched_template)
                ...
                out[i] = {
                    ...
                    classify_note: res.reason,
                };
                if (res.reason
                    && res.reason.startsWith('provider_error')) {
                    logger.warn('trust-desk classify: llm refine failed', {
                        id: q.id,
                        reason: res.reason,
                    });
                }
