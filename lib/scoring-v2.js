 ...
 *    The behavioral signal (did the agent come back?) is the ONLY Phase 1 signal
 *    that's truly self-enforcing. It should be primary, not a 10% tiebreaker.
...
/**
 * Phase 1 weights: behavioral signals are primary because they're the only
 * signals that are truly self-enforcing (you rate what you eat).
 *
...
export const EP_WEIGHTS_V2 = {
  // TIER 1: Self-enforcing (unfakeable in Phase 1)
  behavioral:   0.40,  // Did they come back? Switch? Abandon? Dispute?
  consistency:  0.20,  // Low variance = reliable. Mathematical, not self-reported.
...
export function computeTrustProfile(receipts, entityMeta = {}) {
  ...
  // === BEHAVIORAL ANALYSIS (Tier 1 — self-enforcing) ===
  ...
  for (const r of window) {
    ...
    const w = submitterWeight * timeWeight;
    effectiveEvidence += w;
  ...
  return {
    ...
    profile: {
      // Tier 1: Self-enforcing behavioral signals (the real differentiator)
      behavioral: {
