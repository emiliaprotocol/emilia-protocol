...
export function computeTrustProfile(receipts, entityMeta = {}) {
  ...
  for (const r of window) {
    ...
    const w = submitterWeight * timeWeight * graphWeight * provenanceWeight;
    effectiveEvidence += w;
  ...
  // The dampening threshold applies to ESTABLISHED evidence, not total.
  // This prevents 200 fake identities from crossing the trust barrier.
  const dampeningEvidence = Math.min(effectiveEvidence, establishedEvidence + Math.min(effectiveEvidence - establishedEvidence, 2.0));
  if (dampeningEvidence < 5.0) {
    score = 50 + (score - 50) * (dampeningEvidence / 5.0);
  }
