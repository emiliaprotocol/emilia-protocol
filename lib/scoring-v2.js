...
/**
 * @typedef {'behavioral'|'consistency'|'delivery'|'product'|'price'|'returns'} ScoringDimension
 */

/**
 * @typedef {Object} ScoringWeights
 * @property {number} behavioral
 ...
 *
 * @param {ScoringWeights} weights - { behavioral, consistency, delivery, product, price, returns }
 ...
 * @param {ScoringWeights|null} [weights] - Custom scoring weights. If null/undefined, uses EP_WEIGHTS_V2
...
export function computeTrustProfile(receipts, entityMeta = {}, disputedReceiptIds = new Set(), weights = null) {
  ...
  /** @type {Object.<string, {sum: number, weight: number}>} */
  const signalAccum = {
...
function detectScoreAnomaly(receipts, now) {
   ...
   * @param {(r: *) => number} scoreGetter
   */
  function windowStats(receipts, scoreGetter) {
    const scores = receipts.map(scoreGetter).filter(Number.isFinite);
    if (scores.length === 0) return null;
    const mean = scores.reduce((a, b) => a + b, 0) / scores.length;
    const variance = scores.reduce((/** @type {number} */ s, /** @type {number} */ v) => s + Math.pow(v - mean, 2), 0) / scores.length;
    const stdDev = Math.sqrt(variance);
    const stderr = stdDev / Math.sqrt(scores.length);
    return { mean, variance, stdDev, stderr, n: scores.length };
  }

  const recent = windowStats(recentReceipts, (/** @type {{ composite_score?: number|null }} */ r) => safeScore(r.composite_score));
  const older = windowStats(olderReceipts, (/** @type {{ composite_score?: number|null }} */ r) => safeScore(r.composite_score));
...
/**
 * @typedef {Object} ScoringBehavioralProfile
 * @property {number} [score]
 * @property {number|null} [completion_rate]
 * @property {number|null} [retry_rate]
 * @property {number|null} [abandon_rate]
 * @property {number|null} [dispute_rate]
 * @property {number} [total_observed]
 */

/**
 * @typedef {Object.<string, number|null|undefined>} ScoringSignalProfile
 */

/**
 * @typedef {Object} ScoringAnomalyResult
 * @property {'declining'|'improving'} type
 * @property {number} delta
 * @property {string} [period]
 * @property {number} [recent_avg]
 * @property {number} [older_avg]
 * @property {number} [recent_n]
 * @property {number} [older_n]
 * @property {number} [recent_stddev]
 * @property {number} [significance]
 * @property {'severe'|'moderate'} alert
 */

/**
 * @typedef {Object} ScoringNestedProfile
 * @property {ScoringBehavioralProfile} [behavioral]
 * @property {ScoringSignalProfile} [signals]
 * @property {number} [consistency]
 * @property {Object} [provenance]
 */

/**
 * @typedef {Object} TrustProfileResult
 * @property {number} score
 * @property {'pending'|'insufficient'|'provisional'|'emerging'|'confident'} confidence
 * @property {boolean} [established]
 * @property {number} [effectiveEvidence]
 * @property {number} [qualityGatedEvidence]
 * @property {number} [uniqueSubmitters]
 * @property {number} receiptCount
 * @property {string|null} [last_receipt_at]
 * @property {number} [dispute_dampened_count]
 * @property {string} [weights_version]
 * @property {Object} [weights_used]
 * @property {ScoringNestedProfile|null} profile
 * @property {ScoringAnomalyResult|null} [anomaly]
 */

/**
 * @typedef {Object} TrustPolicyDefinition
 * @property {number} [min_score]
 * @property {'pending'|'insufficient'|'provisional'|'emerging'|'confident'} [min_confidence]
 * @property {number} [min_receipts]
 * @property {number} [max_dispute_rate]
 * @property {Object.<string, number>} [signal_minimums]
 * @property {number} [min_completion_rate]
 * @property {boolean} [reject_anomaly]
 * @property {number} [max_days_since_last_receipt]
 */
 ...
 *
 * @param {TrustProfileResult} profile - Trust profile from computeTrustProfile()
 * @param {TrustPolicyDefinition} policy - Trust policy
