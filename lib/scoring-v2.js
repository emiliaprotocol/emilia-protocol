// Generated from scoring-v2.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
/**
...
export function validateScoringWeights(weights) {
    ...
    if (errors.length > 0)
        return { valid: false, weights: null, errors };
    // Check bounds
    for (const key of required) {
...
export function computeTrustProfile(receipts, entityMeta = {}, disputedReceiptIds = new Set(), weights = null) {
    ...
    if (signals.delivery != null) {
        score += signals.delivery * w.delivery;
        totalW += w.delivery;
    }
    if (signals.product != null) {
        score += signals.product * w.product;
        totalW += w.product;
    }
    if (signals.price != null) {
        score += signals.price * w.price;
        totalW += w.price;
    }
    if (signals.returns != null) {
        score += signals.returns * w.returns;
        totalW += w.returns;
    }
    ...
    const qualityGatedEvidence = Math.min(effectiveEvidence, establishedEvidence + Math.min(Math.max(0, effectiveEvidence - establishedEvidence), MAX_UNESTABLISHED_AGGREGATE_CONTRIBUTION));
    ...
    let confidence;
    if (qualityGatedEvidence === 0)
        confidence = 'pending';
    else if (qualityGatedEvidence < 1)
        confidence = 'insufficient';
    else if (!established)
        confidence = 'provisional';
    else if (qualityGatedEvidence < 20)
        confidence = 'emerging';
    else
...
function detectScoreAnomaly(receipts, now) {
    ...
    if (recentReceipts.length < 5 || olderReceipts.length < 5)
        return null;
    ...
    if (Math.abs(delta) < 10 || significance < 2.0)
        return null;
...
export function evaluateTrustPolicy(profile, policy) {
    ...
    if (policy.signal_minimums && profile.profile?.signals) {
        const signalScores = profile.profile.signals;
