...
// Generated from davinci-pas-binding.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
...
export const DAVINCI_PAS_ACTION_TYPE = 'health.medical-prior-authorization-review.1';
export const DAVINCI_PAS_BINDING_TYPE = 'EP-DAVINCI-PAS-REVIEW-BINDING-v1';
export const DAVINCI_PAS_PROFILE_ID = 'davinci-pas-review-binding.v1';
...
const CLAIM_RESPONSE_PROFILE = 'http://hl7.org/fhir/us/davinci-pas/StructureDefinition/profile-claimresponse|2.2.1';
 ...
 */
function canonicalizePasValue(value) {
    let nodes = 0;
    let stringBytes = 0;
    ...
    function serialize(current, depth) {
        ...
        if (typeof current === 'number') {
            if (!Number.isFinite(current))
                throw new TypeError('PAS value contains a non-finite number');
            return JSON.stringify(current);
        }
        if (typeof current === 'string') {
            if (!validUnicode(current))
                throw new TypeError('PAS value contains invalid Unicode');
        ...
        if (seen.has(current))
            throw new TypeError('PAS value contains a cycle or alias');
        seen.add(current);
...
export function digestPasValue(value) {
...
function canonicalClone(value) {
    return JSON.parse(canonicalizePasValue(value));
}
...
function hasAllFields(value, required) {
...
function isDecisionOutcome(value) {
...
function profileIncludes(resource, profile) {
...
function extensionsAt(value, url) {
    return extensionList(value).filter((entry) => entry.url === url);
}
function codingHas(value, system, code) {
    return isPlainRecord(value)
        && Array.isArray(value.coding)
        && value.coding.some((coding) => isPlainRecord(coding)
            && coding.system === system
            && coding.code === code);
}
function identifierPresent(value) {
...
function claimItemSequences(items, reasons) {
...
function projectDecision(response, expectedSequences, reasons) {
    ...
    for (const item of response.item) {
        ...
        for (const adjudication of item.adjudication) {
            if (!isPlainRecord(adjudication)
                || !codingHas(adjudication.category, ADJUDICATION_SYSTEM, 'submitted'))
                continue;
        ...
        const code = codes[0];
        const outcome = REVIEW_ACTION_TO_OUTCOME[code];
...
function validatePasResources(claim, response, reasons) {
    ...
    if (!isPlainRecord(claim) || !isPlainRecord(response))
        return null;
    try {
    ...
    if (!profileIncludes(claim, CLAIM_PROFILE))
        addReason(reasons, 'claim_profile_mismatch');
    ...
    if (claim.use !== 'preauthorization')
        addReason(reasons, 'claim_use_mismatch');
    if (response.use !== 'preauthorization')
        addReason(reasons, 'claim_response_use_mismatch');
    if (claim.status !== 'active')
        addReason(reasons, 'claim_status_not_active');
    if (response.status !== 'active')
        addReason(reasons, 'claim_response_status_not_active');
    if (!canonicalEqual(claim.type, response.type))
        addReason(reasons, 'claim_response_type_mismatch');
    if (!identifierPresent(claim.identifier))
        addReason(reasons, 'claim_identifier_required');
    if (typeof claim.id !== 'string' || !FHIR_ID_RE.test(claim.id)) {
        addReason(reasons, 'claim_id_invalid');
    }
    if (response.outcome !== 'complete')
        addReason(reasons, 'claim_response_outcome_not_complete');
    if (Array.isArray(response.error) && response.error.length > 0) {
        addReason(reasons, 'claim_response_error_present');
    }
    if (!referenceOf(claim.provider))
        addReason(reasons, 'claim_provider_reference_required');
    const claimPatient = referenceOf(claim.patient);
    const responsePatient = referenceOf(response.patient);
    if (!claimPatient)
        addReason(reasons, 'claim_patient_reference_required');
    if (!responsePatient)
        addReason(reasons, 'claim_response_patient_reference_required');
    ...
    if (!claimInsurer)
        addReason(reasons, 'claim_insurer_reference_required');
    if (!responseInsurer)
        addReason(reasons, 'claim_response_insurer_reference_required');
    ...
    if (reasons.length > 0 || !decision)
        return null;
    return { claim, response, decision };
}
function reviewIsAdverse(outcome) {
    return outcome !== 'approved';
}
function validateReviewer(response, decision, reviewer, policyDigest, responseDigest, reasons) {
    if (!reviewIsAdverse(decision.outcome))
        return null;
    ...
    if (!isPlainRecord(identity)) {
        ...
        if (identity.status !== 'accepted')
            addReason(reasons, 'reviewer_identity_not_accepted');
        if (identity.subject_ref !== reviewerRef)
            addReason(reasons, 'reviewer_identity_subject_mismatch');
        if (!validDigest(identity.evidence_digest))
            addReason(reasons, 'reviewer_identity_digest_invalid');
    ...
    if (!isPlainRecord(authority)) {
        ...
        if (authority.status !== 'accepted')
            addReason(reasons, 'reviewer_authority_not_accepted');
        if (authority.subject_ref !== reviewerRef)
            addReason(reasons, 'reviewer_authority_subject_mismatch');
        if (!validDigest(authority.evidence_digest))
            addReason(reasons, 'reviewer_authority_digest_invalid');
        if (authority.scope !== REVIEWER_AUTHORITY_SCOPE)
    ...
    if (reasons.length > 0
        ...
        || !validDigest(authority.evidence_digest))
        return null;
    return {
...
function materialProjection(input) {
    const reasons = [];
    if (!isPlainRecord(input))
        return { ok: false, reasons: ['binding_input_required'] };
    ...
    if (!isPlainRecord(policy)) {
        ...
        if (!validToken(policy.policy_version))
            addReason(reasons, 'policy_version_invalid');
        if (!validDigest(policy.policy_digest))
            addReason(reasons, 'policy_digest_invalid');
    }
    ...
    const reviewerProjection = validateReviewer(resources.response, resources.decision, input.reviewer, policy.policy_digest, responseDigest, reasons);
    if (reasons.length > 0)
        return { ok: false, reasons };
    const action = {
        action_type: DAVINCI_PAS_ACTION_TYPE,
 ...
 */
export function buildDavinciPasReviewBinding(input) {
    try {
...
function findForbiddenPortableField(value) {
...
function validatePortableShape(binding, reasons) {
    try {
    ...
    if (findForbiddenPortableField(binding))
        addReason(reasons, 'portable_phi_field');
    ...
    if (!hasAllFields(binding, TOP_LEVEL_FIELDS))
        addReason(reasons, 'portable_output_missing_field');
    if (!hasOnlyFields(binding.ig, IG_FIELDS))
        addReason(reasons, 'portable_output_unknown_field');
    if (!hasAllFields(binding.ig, IG_FIELDS))
        addReason(reasons, 'portable_output_missing_field');
    ...
    if (!isPlainRecord(binding.action))
        return false;
    ...
    for (const field of [
        ...
        if (!validDigest(binding.action[field]))
            addReason(reasons, 'portable_digest_invalid');
    }
    if (!validDigest(binding.action_digest))
        addReason(reasons, 'action_digest_invalid');
 ...
 */
export function canonicalizeDavinciPasMaterialAction(input) {
 ...
 */
export function verifyDavinciPasReviewBinding(binding, context) {
    const reasons = [];
    let portableBinding;
    try {
        if (validatePortableShape(binding, reasons))
    ...
    if (portableBinding) {
        ...
        if (caidVerification.valid !== true)
            addReason(reasons, 'caid_mismatch');
