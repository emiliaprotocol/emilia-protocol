...
import { signAgile, verifyAgileSignatureSet, } from '@emilia-protocol/verify/pq-signature-agility';
 ...
 */
export const HEALTHCARE_ASSURANCE_ASSERTION_V2_VERSION = 'EMILIA-HEALTHCARE-ASSURANCE-ASSERTION-v2';
export const HEALTHCARE_ASSURANCE_PACKET_V2_VERSION = 'EMILIA-HEALTHCARE-CONSEQUENCE-ASSURANCE-PACKET-v2';
export const HEALTHCARE_ASSURANCE_V2_REQUIRED_ALGORITHMS = Object.freeze(['Ed25519', 'ML-DSA-65']);
...
function hybridSignerShape(value) {
    return isPlainObject(value)
...
export function assuranceProofV2Shape(value) {
    return exactKeys(value, ['profile', 'required_algorithms', 'signatures'])
        ...
        && HEALTHCARE_ASSURANCE_V2_REQUIRED_ALGORITHMS.every((alg, index) => value.required_algorithms[index] === alg)
 ...
 */
async function signAssuranceValue(domain, value, signer, fips) {
    if (fips) {
 ...
 */
export async function signAssuranceValueV2(domain, value, signer) {
...
async function signedAssuranceAssertion(role, body, signer, fips) {
    const assertion = {
...
export async function signedAssuranceAssertionV2(role, body, signer) {
    const assertion = {
...
export function createHealthcareConsequenceControl(options) {
    ...
    async function exportAssurancePacket(input) {
        ...
        try {
            receiptAssertion = await signedAssuranceAssertion('receipt', assertionBody('receipt', options.assurance.relying_party_id, input.tenant_id, input.operation_id, proposal.caid, proposal.action_digest, execution.payload.approval_evidence_digest, receipt), options.assurance.signers.receipt, options.assurance.fips);
            aebAssertion = await signedAssuranceAssertion('aeb', assertionBody('aeb', options.assurance.relying_party_id, input.tenant_id, input.operation_id, proposal.caid, proposal.action_digest, execution.payload.aeb_evaluation_digest, aeb), options.assurance.signers.aeb, options.assurance.fips);
            if (provider) {
                providerAssertion = await signedAssuranceAssertion('provider', assertionBody('provider', options.assurance.relying_party_id, input.tenant_id, input.operation_id, proposal.caid, proposal.action_digest, provider.evidence_digest, provider), options.assurance.signers.provider, options.assurance.fips);
            }
        }
        catch {
        ...
        try {
            packet.proof = await signAssuranceValue('packet:evaluator', packet, options.assurance.signers.evaluator, options.assurance.fips);
        }
        catch (error) {
            return refusal('healthcare_assurance_signing_failed', {
    ...
    async function exportAssurancePacketV2(input) {
        ...
        async function reassertV2(role) {
        ...
        let receiptAssertionV2;
        let aebAssertionV2;
        let providerAssertionV2;
        try {
 ...
 */
function assuranceAssertion(value, role, version = HEALTHCARE_ASSURANCE_ASSERTION_VERSION, proofShapeCheck = assuranceProofShape) {
 ...
 */
export function checkHealthcareAssurancePacketInternalConsistency(packet, options = {}) {
    ...
    const receipt = assuranceAssertion(packet.protocol_evidence?.receipt, 'receipt', assertionVersion, proofShapeCheck);
    const aeb = assuranceAssertion(packet.protocol_evidence?.aeb, 'aeb', assertionVersion, proofShapeCheck);
    ...
    const provider = assuranceAssertion(packet.protocol_evidence?.provider, 'provider', assertionVersion, proofShapeCheck);
 ...
 */
export async function verifyPinnedSignatureV2(domain, value, proof, pin) {
    if (!assuranceProofV2Shape(proof) || !isPlainObject(pin))
        return false;
 ...
 */
export async function verifyHealthcareAssurancePacketOfflineV2(packet, trust) {
    const reasons = [
    ...
    const packetProof = packetForSignature.proof;
    delete packetForSignature.proof;
    if (!(await verifyPinnedSignatureV2('packet:evaluator', packetForSignature, packetProof, trust.evaluator))) {
        reasons.push('evaluator_signature_invalid');
    }
    for (const role of ['receipt', 'aeb', 'provider']) {
        const assertion = packet.protocol_evidence?.[role];
        ...
        const proof = unsigned.proof;
        delete unsigned.proof;
        if (!(await verifyPinnedSignatureV2(`assertion:${role}`, unsigned, proof, trust[role]))) {
            reasons.push(`${role}_signature_invalid`);
        }
    }
    return { valid: reasons.length === 0, reasons: [...new Set(reasons)] };
 ...
 * (fail-closed).
 */
export async function verifyHealthcareAssurancePacketOfflineAny(packet, trust) {
