...
function getSeverity(eventType) {
  return SEVERITY_MAP[/** @type {keyof typeof SEVERITY_MAP} */ (eventType)] ?? 'low';
}
 ...
 * @param {string} source
 * @param {{host: string, index: string}} config
 */
function formatSplunk(eventType, data, severity, source, { host, index }) {
