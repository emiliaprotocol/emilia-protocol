...
describe('tenant-scoped large-payment approval queue', () => {
  ...
  it('fails closed for receipt lookup, timeline lookup, and thrown storage errors', async () => {
    const warnings = [];
    const log = { warn(...args) { warnings.push(args); } };

    const receiptLookup = await loadTenantApprovalQueue({
      supabase: {
        from() {
          return queryResult({ data: null, error: { message: 'receipt lookup denied' } }, []);
        },
      },
      tenantId: TENANT_ID,
      log,
    });
    expect(receiptLookup).toEqual({
      approvals: [],
      error: 'receipt lookup denied',
    });

    let query = 0;
    const timelineLookup = await loadTenantApprovalQueue({
      supabase: {
        from() {
          query += 1;
          if (query === 1) {
            return queryResult({ data: [{ target_id: 'tr_owned' }], error: null }, []);
          }
          return queryResult({ data: null, error: { message: 'timeline denied' } }, []);
        },
      },
      tenantId: TENANT_ID,
      log,
    });
    expect(timelineLookup).toEqual({
      approvals: [],
      error: 'timeline denied',
    });

    const thrown = await loadTenantApprovalQueue({
      supabase: {
        from() {
    ...
    expect(thrown).toEqual({
      approvals: [],
      error: 'Approval queue query failed.',
    });
    expect(warnings.map((entry) => entry[0])).toEqual([
      '[approval queue] tenant receipt lookup failed:',
      '[approval queue] tenant timeline lookup failed:',
      '[approval queue] query threw:',
    ]);
  });

  it('returns early when the tenant owns no usable receipt ids', async () => {
    let queryCount = 0;
    const result = await loadTenantApprovalQueue({
      supabase: {
        from() {
    ...
    expect(result).toEqual({ approvals: [], error: null });
  ...
  it('uses only explicit safe fallbacks for malformed optional audit fields', () => {
    const receiptId = 'fallbacks';
    const createdEvent = created(receiptId, {
      ...
      after_state: {
        ...
        payment_destination_hash: null,
        expires_at: '2026-07-19T13:00:00.000Z',
        canonical_action: {
          amount: 42,
          currency: 'EUR',
          counterparty_name: 'Fallback Treasury',
          target_resource_id: 'wire:fallback',
          payment_destination_hash: `sha256:${'2'.repeat(64)}`,
        },
      },
    });
    const requestEvent = requested(receiptId);
    requestEvent.after_state.approver_id = null;
    requestEvent.after_state.expires_at = null;
    requestEvent.after_state.quorum = { approver_id: 'ep:approver:quorum' };
    const consumedEvent = {
      event_type: 'guard.trust_receipt.consumed',
      target_id: receiptId,
      actor_id: 'ep:executor:payments',
      after_state: { action_hash: `sha256:${receiptId}` },
      created_at: '2026-07-19T10:03:00.000Z',
    };

    const [request] = replayApprovalQueue(
      [createdEvent, requestEvent, consumedEvent],
      [receiptId],
      TENANT_ID,
      new Date(NOW),
    );
    ...
    expect(request).toMatchObject({
      ...
      payment_destination_hash: `sha256:${'2'.repeat(64)}`,
      expires_at: '2026-07-19T13:00:00.000Z',
      approver_id: 'ep:approver:quorum',
      status: 'consumed',
      consumed_at: '2026-07-19T10:03:00.000Z',
    });
  });

  it('rejects invalid tenant scopes and malformed receipt timelines', () => {
    expect(replayApprovalQueue([], [], '', NOW)).toEqual([]);
    expect(replayApprovalQueue([], [], 'x'.repeat(257), NOW)).toEqual([]);
    expect(replayApprovalQueue(undefined, null, TENANT_ID, NOW)).toEqual([]);

    const noRequest = created('no_request');
    const noCreator = requested('no_creator');
    const malformedAction = created('malformed_action', {
      after_state: { canonical_action: [] },
    });
    const malformedRequest = requested('malformed_action', {
      approver_id: null,
      expires_at: 'not-a-date',
    });

    const requests = replayApprovalQueue(
      [noRequest, noCreator, malformedAction, malformedRequest],
      ['no_request', 'no_creator', 'malformed_action'],
      TENANT_ID,
      Number.NaN,
    );
