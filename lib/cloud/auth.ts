...
// and stays directly assignable to authorize.ts's CloudAuthResultLike
// (which has an `[key: string]: unknown` member) without per-call-site casts.
 ...
 */
export async function authenticateCloudRequest(request: Request): Promise<CloudAuthContext | null> {
  try {
    ...
    if (error) {
      logger.error('[cloud/auth] Key lookup error:', error.message);
      return null;
    }
    ...
    logger.error('[cloud/auth] Unexpected error during authentication:', err.message);
    return null;
  }
}
