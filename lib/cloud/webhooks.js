 ...
 * Throws if the URL is invalid or targets a private host.
 *
...
async function attemptDelivery(supabase, endpoint, delivery) {
  // DNS rebinding protection: re-validate (and re-resolve) the URL at delivery time
  try {
  ...
  try {
    ...
    const response = await fetch(endpoint.url, {
      ...
      // Never follow redirects: a 3xx to a private host would bypass the
      // pre-flight SSRF validation. Treat any redirect as a delivery failure.
      redirect: 'manual',
    });
