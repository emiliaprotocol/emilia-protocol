 ...
 */
function normalizeEvent(row: any, source: string): any {
  return {
 ...
 * @param {string} [filters.tenant_id] - Required. Scopes all queries to this tenant.
 ...
 */
export async function queryEvents(filters: any = {}): Promise<any> {
  if (!filters.tenant_id) {
    throw new Error('tenant_id is required for queryEvents — cross-tenant event access is forbidden');
  }
  const supabase = getGuardedClient();
  const limit = Math.min(Math.max(parseInt(/** @type {any} */ (filters.limit), 10) || 50, 1), 500);
  const offset = Math.max(parseInt(/** @type {any} */ (filters.offset), 10) || 0, 0);
  ...
  const queries = tables.map(async ({ name, selectCols }) => {
    try {
      ...
      logger.warn(`[event-explorer] Query on ${name} failed:`, e.message);
      return { rows: [], count: 0 };
    }
  });
  ...
  allEvents.sort((a, b) => (new Date(b.created_at) as any) - (new Date(a.created_at) as any));
 ...
 */
export async function getTimeline(handshakeId: string, tenantId: string): Promise<any> {
  if (!handshakeId) {
  ...
  // Fail-closed: if no binding exists, or binding has no tenant_id,
  // we cannot confirm ownership — deny access rather than leak data.
  ...
  if (!binding || !binding.tenant_id) {
    throw new Error('Handshake tenant ownership cannot be verified — access denied');
  }
  ...
  events.sort((a, b) => (new Date(a.created_at) as any) - (new Date(b.created_at) as any));
 ...
 */
export async function searchEvents(query: string, filters: any = {}): Promise<any> {
  if (!query || typeof query !== 'string' || query.trim().length === 0) {
    throw new Error('query is required and must be a non-empty string');
  }
  if (!filters.tenant_id) {
    throw new Error('tenant_id is required for searchEvents — cross-tenant event access is forbidden');
  }
  ...
  const searches = tables.map(async (table) => {
    try {
      ...
      logger.warn(`[event-explorer] Search on ${table} failed:`, e.message);
      return [];
    }
  });
  ...
  allMatches.sort((a, b) => (new Date(b.created_at) as any) - (new Date(a.created_at) as any));
 ...
 *
 * @param {object} [dateRange] - { tenant_id, from, to } ISO timestamps
 * @returns {Promise<{ score: number, total_events: number, handshake_event_count: number,
 *   signoff_event_count: number, handshakes_checked: number, terminal_handshakes_checked: number,
 *   anomalies: object[] }>}
 */
export async function verifyIntegrity(dateRange: any = {}): Promise<any> {
  if (!dateRange.tenant_id) {
    throw new Error('tenant_id is required for verifyIntegrity — cross-tenant integrity checks are forbidden');
  }
  const supabase = getGuardedClient();
  const anomalies: any[] = [];
  ...
  let heQuery = supabase
    .from('handshake_events')
    .select('*')
    .eq('tenant_id', dateRange.tenant_id)
    .order('created_at', { ascending: true });
