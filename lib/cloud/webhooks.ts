 ...
 */
export async function validateWebhookUrl(urlString: string): Promise<{ parsed: URL, addresses: any[] }> {
  let parsed: URL;
  try {
 ...
 */
export async function registerEndpoint(tenantId: string, url: string, events: string[]): Promise<any> {
 ...
 */
export async function disableEndpoint(endpointId: string): Promise<any> {
  const supabase = getServiceClient();
 ...
 */
export function computeSignature(secret: string, timestamp: number, payload: any): string {
...
function deliverToPinnedAddress(url: URL, pinnedAddress: string, pinnedFamily: number, { headers, body, timeoutMs }: any): Promise<any> {
  return new Promise((resolve, reject) => {
    const req = https.request(
      ...
      (res) => {
        // Node sets statusCode before this callback fires and it never changes
        // afterward; read it once so a single already-guaranteed number backs
        // both the reported status and the 2xx check below.
 ...
 */
export async function deliverWebhook(endpointId: string, eventType: string, payload: any): Promise<any> {
  const supabase = getServiceClient();
 ...
 * @returns {Promise<{ delivery: object } | { error: string, status: number }>}
 */
async function attemptDelivery(supabase: any, endpoint: any, delivery: any): Promise<any> {
 ...
 */
async function handleFailure(supabase: any, endpoint: any, delivery: any, responseStatus: any, responseBody: string): Promise<any> {
 ...
 */
export async function retryFailedDeliveries(): Promise<any> {
  const supabase = getServiceClient();
  ...
  for (const delivery of deliveries) {
    ...
    const result = await attemptDelivery(supabase, endpoint, delivery);
    // attemptDelivery's return type is a discriminated union ({delivery} | {error}); this
    // cast preserves the pre-existing runtime optional-chaining access unchanged.
    if (/** @type {*} */ (result).delivery?.status === 'delivered') {
      succeeded++;
    } else {
      failed++;
    }
