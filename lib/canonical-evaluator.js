...
import { computeTrustProfile, evaluateTrustPolicy, TRUST_POLICIES, EP_WEIGHTS_V2, validateScoringWeights } from '@/lib/scoring-v2';
  ...
  if (typeof policyInput === 'string') {
    ...
    if (policyInput.length <= 4096) {
      try {
        ...
        // Malformed JSON falls through to the 'standard' default rather
        // than throwing — but log so operators see the parse failure
        // instead of silently getting standard-policy semantics they
        // didn't ask for. Use logger so SIEM picks it up if configured.
        logger.warn(
          '[canonical-evaluator] policy JSON parse failed; falling back to standard policy:',
          { error: e.message, policyPrefix: policyInput.slice(0, 80) }
        );
      }
    }
 ...
 * @param {object} options.scoringWeights - Custom scoring weights (optional).
 *   When provided, returns both canonical_score (protocol defaults) and policy_score
 *   (custom weights). Must pass validateScoringWeights() or will be rejected.
