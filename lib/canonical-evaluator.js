...
// Software-specific policy templates (same as install-preflight)
...
export async function fetchEstablishment(entityDbId) {
  const supabase = getServiceClient();
  try {
    const { data: estData } = await supabase.rpc('is_entity_established', { p_entity_id: entityDbId });
...
export function resolvePolicy(policyInput) {
  ...
  if (typeof policyInput === 'string') {
    ...
    try {
      ...
      if (typeof parsed === 'object') return { resolved: parsed, name: 'custom' };
    } catch {}
...
export async function canonicalEvaluate(entityIdOrSlug, options = {}) {
  ...
  if (hasFreshSnapshot && !hasContext) {
    ...
    profile = computeTrustProfile(fetched.receipts, entity);
  ...
  if (entity.principal_id) {
    try {
      ...
      // EP-IX tables may not exist yet — graceful degradation
      if (!e.message?.includes('does not exist')) {
        console.warn('EP-IX continuity lookup failed:', e.message);
      }
    }
  }
