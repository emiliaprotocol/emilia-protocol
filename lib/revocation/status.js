...
export async function buildRevokerAuthorityCertificate(input) {
    closedObject(input, 'certificate input', AUTHORITY_INPUT_KEYS, AUTHORITY_INPUT_KEYS.filter((key) => key !== 'targetRegistry'));
...
export async function buildStatusArtifact(input) {
    closedObject(input, 'status input', STATUS_INPUT_KEYS, STATUS_INPUT_KEYS.filter((key) => key !== 'previousStatus' && key !== 'targetRegistry'));
    validateAuthorityPin(input.authorityPin);
