// Generated from chain.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
/**
...
// The EP-PROVENANCE-CHAIN-v2 hybrid verifier and its delegation-proof bytes
// helper are COMPOSED from packages/verify (never copied), matching the
// EP-REVOCATION-v2 twin discipline (lib/revocation/revocation.ts): one
// verification body in the repository, so this twin cannot drift from the
// published one on the hybrid path.
import { verifyProvenanceOfflineV2, delegationProofV2Bytes, PROVENANCE_V2_VERSION, PROVENANCE_V2_REQUIRED_ALGORITHMS, } from '../../packages/verify/provenance.js';
import { signAgileSet } from '../../packages/verify/pq-signature-agility.js';
...
function scopePermits(scope, actionType) {
    ...
    for (const grant of scope) {
        ...
        if (typeof grant === 'string' && grant.endsWith('.*')) {
            ...
            if (actionType === prefix || actionType.startsWith(prefix + '.'))
                return true;
        }
    }
    return false;
...
function verifyDetachedSignature(att) {
    try {
        if (!att?.signed_payload_b64u || !att?.signature_b64u || !att?.public_key)
            return false;
...
function constraintsMonotonic(parentC, childC) {
    ...
    for (const k of Object.keys(p)) {
        ...
        if (typeof pv === 'number' && typeof cv === 'number') {
            ...
            if (!cv.every((x) => pset.has(canonicalize(x))))
                return false;
        }
 ...
 */
export function assembleProvenance({ rootSignoff, delegationChain = [], actionApproval = undefined, execution, agentIdentity = undefined, liability = undefined, metadata = undefined, }) {
    ...
    if (agentIdentity)
        doc.agent_identity = agentIdentity;
    if (liability)
        doc.liability = liability;
    ...
    for (const link of chain) {
        ...
        if (prevDelegatee !== null) {
            ...
            if (!boundByParentRef || !boundByDelegator) {
                ...
                errors.push(`delegation ${link.delegation_id}: parent_ref "${link.parent_ref}" / delegator "${link.delegator}" `
        ...
        if (link.proof) {
            ...
            const presentedBytes = (() => {
                try {
                    return Buffer.from(link.proof.signed_payload_b64u || '', 'base64url');
                }
                catch {
...
// ── EP-PROVENANCE-CHAIN-v2 (hybrid delegation-link proofs) ─────────────────
/**
 * Issuer-side counterpart to packages/verify/src/provenance.ts's
 * verifyDelegationProofSetV2: signs one delegation link's `proof_set` under
 * BOTH Ed25519 and ML-DSA-65 over the exact bytes the verifier independently
 * recomputes (delegationProofV2Bytes, imported above -- never reimplemented).
 * Throws on issuer-side misuse; never silently downgrades to one algorithm.
 */
export async function signDelegationLinkV2(link, signer) {
    if (!signer?.private_key || signer.private_key.type !== 'private'
        || signer.private_key.asymmetricKeyType !== 'ed25519') {
        throw new Error('signDelegationLinkV2: signer.private_key must be an Ed25519 private KeyObject');
    }
    if (!signer?.pq_private_key) {
        throw new Error('signDelegationLinkV2: signer.pq_private_key is required');
    }
    const bytes = delegationProofV2Bytes(link, PROVENANCE_V2_REQUIRED_ALGORITHMS);
    const signatures = await signAgileSet(new Uint8Array(bytes), [
        { alg: 'Ed25519', private_key: signer.private_key },
        { alg: 'ML-DSA-65', private_key: signer.pq_private_key },
    ]);
    return {
        ...link,
        proof_set: {
            required_algorithms: [...PROVENANCE_V2_REQUIRED_ALGORITHMS],
            signatures,
        },
    };
}
/**
 * Assemble an EP-PROVENANCE-CHAIN-v2 document. Identical composition to
 * assembleProvenance above (same rootSignoff/actionApproval/execution
 * validation; adds no new trust over the embedded EP-RECEIPT-v1 receipts),
 * stamped with the v2 version marker so a deployed v1 verifier refuses it
 * cleanly on the version marker rather than accepting it on the leg it
 * happens to understand. Delegation chain entries are expected to already
 * carry a `proof_set` (see signDelegationLinkV2) or be intentionally
 * unsigned for a caller that will pass allowUnsignedDelegations.
 */
export function assembleProvenanceV2({ rootSignoff, delegationChain = [], actionApproval = undefined, execution, agentIdentity = undefined, liability = undefined, metadata = undefined, }) {
    if (!rootSignoff?.receipt || !rootSignoff?.verification) {
        throw new Error('assembleProvenanceV2 requires rootSignoff.{receipt,verification}');
    }
    if (!execution?.action_hash || typeof execution.irreversible !== 'boolean') {
        throw new Error('assembleProvenanceV2 requires execution.{action_hash,irreversible}');
    }
    if (execution.irreversible && !actionApproval?.receipt) {
        throw new Error('assembleProvenanceV2: an irreversible execution requires actionApproval.receipt (fail-closed)');
    }
    const chain = delegationChain.map((link, i) => ({
        sequence: link.sequence ?? i,
        ...link,
    ...
    doc.provenance_metadata = {
        chain_depth: chain.length,
        assembled_at: new Date().toISOString(),
        note: 'Hybrid (Ed25519 + ML-DSA-65) delegation-link proofs over EP-RECEIPT-v1 receipts. No new trust over the receipts themselves.',
        ...(metadata || {}),
    };
    return doc;
}
export { verifyProvenanceOfflineV2, PROVENANCE_V2_VERSION, PROVENANCE_V2_REQUIRED_ALGORITHMS };
const provenance = {
    assembleProvenance,
