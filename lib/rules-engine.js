// Generated from rules-engine.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
/**
 * EP Rules Engine v0
 * @license Apache-2.0
...
export function evaluateAction(input) {
    ...
    if (typeof input.context?.destination_age_days === 'number' &&
    ...
    if (input.approver && input.actor) {
        ...
        if (input.policy?.requires_cross_department_approval &&
        ...
        if (Array.isArray(input.approver.manager_chain) &&
    ...
    /** @type {{actor_id?: string, role?: string, department?: string, assurance_level?: 'low'|'medium'|'high', mfa_verified?: boolean, session_age_seconds?: number, device_trust?: 'unknown'|'personal'|'managed'}} */
    const actor = input.actor ?? {};
    if (ctx.business_hours === false)
        risk_score += RISK_WEIGHTS.AFTER_HOURS;
    if (actor.device_trust && actor.device_trust !== 'managed')
        risk_score += RISK_WEIGHTS.UNMANAGED_DEVICE;
    if ((actor.session_age_seconds ?? 0) > 3600)
        risk_score += RISK_WEIGHTS.STALE_SESSION;
    if ((ctx.velocity_same_actor_24h ?? 0) >= 5)
        risk_score += RISK_WEIGHTS.HIGH_VELOCITY;
    if ((ctx.prior_changes_target_30d ?? 0) >= 2)
        risk_score += RISK_WEIGHTS.REPEATED_TARGET_CHANGES;
    if (typeof ctx.destination_age_days === 'number' && ctx.destination_age_days < 30) {
        risk_score += RISK_WEIGHTS.NEW_DESTINATION;
    }
    if (typeof amountUsd === 'number' && amountUsd >= 10_000)
        risk_score += RISK_WEIGHTS.AMOUNT_GTE_10K;
    if (typeof amountUsd === 'number' && amountUsd >= 50_000)
        risk_score += RISK_WEIGHTS.AMOUNT_GTE_50K;
    if ((ctx.prior_denials_actor_30d ?? 0) > 0)
        risk_score += RISK_WEIGHTS.PRIOR_DENIALS;
...
function checkHardDeny(input) {
    ...
    const ctx = input.context ?? {};
    if (!actor.actor_id)
        return REASON_CODES.ACTOR_MISSING;
    if (!auth.authority_id)
        return REASON_CODES.AUTHORITY_MISSING;
    if (auth.revoked === true)
        return REASON_CODES.AUTHORITY_REVOKED;
    if (auth.expires_at && new Date(auth.expires_at).getTime() < Date.now()) {
        return REASON_CODES.AUTHORITY_EXPIRED;
    }
    if (actor.mfa_verified === false)
        return REASON_CODES.MFA_REQUIRED;
    if (actor.assurance_level === 'low')
        return REASON_CODES.ASSURANCE_TOO_LOW;
    if (ctx.watchlist_hit === true)
        return REASON_CODES.WATCHLIST_HIT;
    if (typeof action.amount_usd === 'number' &&
