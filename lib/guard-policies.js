...
export const GUARD_POLICY_CONTENT = Object.freeze({
  policy_engine: 'EP-GUARD-POLICY-v1',
 ...
 * @returns {{ decision: string, reasons: string[], signoffRequired: boolean, aml_signals?: string[] }}
 */
export function evaluateGuardPolicy(input) {
  const base = stampAssurance(basePolicy(input), input);
...
export function buildInitiatorAttestation(decision, { actionType, policyId, targetChangedFields } = {}) {
  ...
  if (actionType === GUARD_ACTION_TYPES.LARGE_PAYMENT_RELEASE && decision.signoffTier) {
    return { escalation_trigger: 'magnitude', policy_basis: ruleBasis(policyId, `payment-threshold-${decision.signoffTier}`), statement };
  }

  // Caseworker override (a deterministic rule with no substantive category).
  if (actionType === GUARD_ACTION_TYPES.CASEWORKER_OVERRIDE) {
    return { escalation_trigger: 'policy_rule', policy_basis: ruleBasis(policyId, 'caseworker-override'), statement };
  }
