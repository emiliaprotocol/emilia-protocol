// Generated from guard-policies.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
/**
...
function paymentSignoffTier(amount) {
    if (typeof amount !== 'number' || Number.isNaN(amount))
        return null;
    if (amount >= DUAL_AUTH_THRESHOLD_USD)
        return 'dual';
    if (amount >= SIGNOFF_THRESHOLD_USD)
        return 'single';
    return null;
}
/**
...
function stampAssurance(decision, input) {
    if (!decision?.signoffRequired)
        return decision;
    const touchesMoneyDestination = (input.targetChangedFields || []).some((f) => MONEY_DESTINATION_FIELDS.includes(f));
...
function applyCriticalKeyClassFloor(decision, input) {
    if (!decision || decision.decision !== GUARD_DECISIONS.ALLOW)
        return decision;
    if (!MINT_CRITICAL_FLOOR_ACTIONS.includes(input.actionType))
        return decision;
    return {
        ...decision,
...
function basePolicy(input) {
    ...
    if (input.actionType === GUARD_ACTION_TYPES.BENEFIT_ADDRESS_CHANGE) {
        const touchesBenefitIdentityRouting = (input.targetChangedFields || []).some((f) => BENEFIT_IDENTITY_ROUTING_FIELDS.includes(f));
    ...
    if (input.actionType === GUARD_ACTION_TYPES.GOV_DISBURSEMENT_RELEASE
        ...
        if (tier === 'dual') {
            reasons.push(`Government disbursement of $${input.amount} >= $${DUAL_AUTH_THRESHOLD_USD} requires dual authorization.`);
        }
        else {
    ...
    if (input.actionType === GUARD_ACTION_TYPES.LARGE_PAYMENT_RELEASE) {
        ...
        if (tier === 'dual') {
            reasons.push(`Payment release of $${input.amount} >= $${DUAL_AUTH_THRESHOLD_USD} requires dual authorization (a second, senior approver).`);
            return {
                decision: GUARD_DECISIONS.ALLOW_WITH_SIGNOFF,
                reasons,
                signoffRequired: true,
        ...
        if (tier === 'single') {
            reasons.push(`Payment release of $${input.amount} >= $${SIGNOFF_THRESHOLD_USD} requires accountable signoff.`);
            return {
                decision: GUARD_DECISIONS.ALLOW_WITH_SIGNOFF,
                reasons,
                signoffRequired: true,
...
function capStatement(s) {
    const text = String(s || '');
    if (text.length <= ATTESTATION_STATEMENT_MAX)
        return text;
...
function denyReason(flag) {
    if (flag === 'impossible_travel')
        return 'Impossible travel detected.';
    if (flag === 'known_compromised_device')
        return 'Known compromised device.';
