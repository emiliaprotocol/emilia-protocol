...
function decodeBase64Strict(value, label) {
  ...
  const normalized = value.replace(/=+$/u, '').replace(/\+/gu, '-').replace(/\//gu, '_');
 ...
 * @returns {{ keyObject: import('crypto').KeyObject, canonicalDer: Buffer }}
 */
function ed25519PublicKey(publicKey) {
  const bytes = decodeBase64Strict(publicKey, 'publicKey');
  if (bytes.length !== 32 && bytes.length !== 44) {
    throw new Error(`publicKey must be a 32-byte raw key or 44-byte SPKI DER, got ${bytes.length} bytes`);
  }
 ...
 * @property {string} [signature_input_digest]
 * @property {string} [signature_version]
 */

/**
 * @param {ReceiptEvidence} [evidence]
 * @returns {Object}
 */
function unsignedEvidence(evidence) {
  if (!evidence || typeof evidence !== 'object' || Array.isArray(evidence)) return {};
  return Object.fromEntries(Object.entries(evidence).filter(([key]) => !AUTH_EVIDENCE_FIELDS.has(key)));
}

/**
 * @typedef {Object} IdentifiedSubmissionSignals
 * @property {*} [delivery_accuracy]
 * @property {*} [product_accuracy]
 * @property {*} [price_integrity]
 * @property {*} [return_processing]
 * @property {*} [agent_satisfaction]
 */

/**
 * @typedef {Object} IdentifiedSubmissionInput
 * @property {string} [targetEntityRef]
 * @property {string} [transactionRef]
 * @property {string} [transactionType]
 * @property {IdentifiedSubmissionSignals} [signals]
 * @property {string} [agentBehavior]
 * @property {Object} [claims]
 * @property {ReceiptEvidence} [evidence]
 * @property {*} [context]
 * @property {boolean} [requestBilateral]
 */
 ...
 * construct.
 *
 * @param {IdentifiedSubmissionInput} [input]
 */
export function buildIdentifiedSubmissionDigest(input = {}) {
  const payload = {
  ...
  if (typeof canonical !== 'string') throw new Error('identified submission is not canonical JSON');
  return {
    payload,
    canonical,
...
export function verifyReceiptSignature(receiptHash, signature, publicKey) {
  if (typeof receiptHash !== 'string' || !SHA256_HEX_RE.test(receiptHash)) {
    return { valid: false, reason: 'receiptHash must be a lowercase SHA-256 hex digest' };
  }
 ...
 * @param {string} receiptHash - The canonical submission digest
 * @param {ReceiptEvidence} evidence - The evidence object from the receipt submission
 * @param {string} trustedPublicKey - Authenticated entity's enrolled Ed25519 key
 * @returns {{ tier: string, warning?: string }}
 */
export function resolveProvenanceTier(claimedTier, receiptHash, evidence, trustedPublicKey) {
  ...
  if (!sig) {
    return {
      tier: 'self_attested',
      warning: 'identified_signed tier claimed but signature is missing from evidence; downgraded to self_attested',
    };
  }
  ...
  if (!trustedPublicKey) {
    return {
      tier: 'self_attested',
      warning: 'identified_signed tier has no enrolled submitter key; downgraded to self_attested',
    };
  }
  ...
  try {
    ...
    if (evidence?.public_key) {
      ...
      if (!presented.canonicalDer.equals(trusted.canonicalDer)) {
        return {
          tier: 'self_attested',
          warning: 'identified_signed evidence key does not match the authenticated submitter key; downgraded to self_attested',
        };
      }
    }
    ...
    return {
      tier: 'self_attested',
      warning: `identified_signed signature verification failed${result.reason ? `: ${result.reason}` : ''}; downgraded to self_attested`,
    };
  } catch (err) {
    return {
      tier: 'self_attested',
      warning: `identified_signed key verification error: ${err.message}; downgraded to self_attested`,
    };
  }
}
