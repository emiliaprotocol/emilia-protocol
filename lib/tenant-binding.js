 ...
 *
 * @param {{entity?: object|string}} auth   authenticateRequest() result
...
export function canReadReceipt(auth, { organizationId, creatorActorId } = {}) {
  ...
  if (callerOrg) return callerOrg === organizationId;        // bound caller: same-org only
  const callerId = callerEntityId(auth);                     // unbound caller: creator only
  return Boolean(callerId) && callerId === creatorActorId;
}
