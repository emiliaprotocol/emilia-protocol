...
const VALID_PRESENTATION_TYPES = new Set<string>([
  'self_asserted', 'verifiable_credential', 'certificate', 'attestation', 'delegation_proof',
]);
const VALID_ACTION_TYPES = new Set<string>(['install', 'connect', 'delegate', 'transact']);
 ...
 */
export function validateHandshakeCreate(body: any) {
  ...
  const result = validateBody(body, {
    mode:       (v: any) => validate(v, 'mode').required().string().oneOf(VALID_MODES).result,
    policy_id:  (v: any) => validate(v, 'policy_id').required().string().result,
    parties:    (v: any) => validate(v, 'parties').required().isArray().result,
  });
 ...
 */
export function validatePresent(body: any) {
  const result = validateBody(body, {
    party_role:        (v: any) => validate(v, 'party_role').required().string().oneOf(VALID_PARTY_ROLES).result,
    presentation_type: (v: any) => validate(v, 'presentation_type').required().string().oneOf(VALID_PRESENTATION_TYPES).result,
    claims:            (v: any) => validate(v, 'claims').required().isObject().result,
  });
 ...
 */
export function validateSignoffAttest(body: any) {
  return validateBody(body, {
    humanEntityRef:  (v: any) => validate(v, 'humanEntityRef').required().string().result,
    authMethod:      (v: any) => validate(v, 'authMethod').required().string().result,
    assuranceLevel:  (v: any) => validate(v, 'assuranceLevel').required().string().result,
    channel:         (v: any) => validate(v, 'channel').required().string().result,
    attestationHash: (v: any) => validate(v, 'attestationHash').required().string().result,
  });
}
 ...
 */
export function validatePolicyCreate(body: any) {
  const result = validateBody(body, {
    name:            (v: any) => validate(v, 'name').required().string().minLength(1).maxLength(255).result,
    description:     (v: any) => validate(v, 'description').required().string().maxLength(1000).result,
    min_score:       (v: any) => validate(v, 'min_score').required().isNumber().result,
    min_confidence:  (v: any) => validate(v, 'min_confidence').required().isNumber().result,
    min_receipts:    (v: any) => validate(v, 'min_receipts').required().isNumber().result,
    max_dispute_rate:(v: any) => validate(v, 'max_dispute_rate').required().isNumber().result,
  });
