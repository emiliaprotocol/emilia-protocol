...
export function screenSanctions(
  name: string,
  opts: any = {},
): { hit: boolean; topScore: number; matches: any[]; country_blocked: boolean } {
...
export function detectStructuring(
  ...
  threshold: number = CTR_THRESHOLD_USD,
): { structuring: boolean; reason?: string; score: number } {
  const justUnder = (x: number): boolean => x >= threshold * 0.9 && x < threshold;
...
export function detectVelocity(
  recentAmounts: number[] = [],
  maxInWindow: number = 10,
): { high_velocity: boolean; count: number } {
 ...
 */
export function screenAml(aml: any): {
  recommendation: 'allow' | 'signoff' | 'deny';
  risk: 'none' | 'elevated' | 'blocked';
  signals: string[];
