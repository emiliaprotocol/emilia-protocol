// Generated from screening.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
/**
...
export function screenAml(aml) {
    ...
    if (aml.counterpartyName || aml.counterpartyCountry) {
        ...
        detail.sanctions = s;
        if (s.matches.length)
            signals.push(`sanctions_match:${s.matches[0].name} (${s.matches[0].program}, score ${s.matches[0].score})`);
        if (s.country_blocked)
            signals.push(`embargoed_jurisdiction:${aml.counterpartyCountry}`);
        if (s.hit) {
    ...
    if (typeof aml.amount === 'number') {
        ...
        if (st.structuring)
            signals.push(`structuring:${st.reason}`);
        const vel = detectVelocity(aml.recentAmounts);
        detail.velocity = vel;
        if (vel.high_velocity)
            signals.push(`high_velocity:${vel.count} transfers in window`);
