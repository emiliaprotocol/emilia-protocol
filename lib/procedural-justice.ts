...
export const OPERATOR_ROLES: Record<string, OperatorRoleDefinition> = {
  reporter: {
 ...
 */
export function requireDualControl(
  action: string,
  firstOperatorId: string,
  secondOperatorId: string | null | undefined,
  firstOperatorRole: string,
  ...
  // (secondOperatorRole is guaranteed defined here: dual-control actions always
  // supply it alongside secondOperatorId, checked above.)
  if (!hasPermission(secondOperatorRole as string, action)) {
...
export interface VisibilityTierDefinition {
  level: number;
  description: string;
  includes: string[];
  excludes: string[];
...
export const VISIBILITY_TIERS: Record<string, VisibilityTierDefinition> = {
  public_summary: {
    level: 0,
...
export const DISPUTE_STATES: Record<string, StateDefinition> = {
  open: {
 ...
 */
export const CONTINUITY_STATES: Record<string, StateDefinition> = {
  pending: {
...
export function validateTransition(
  stateMachine: Record<string, StateDefinition>,
  currentState: string,
  targetState: string,
 ...
 */
export interface AbusePatternDefinition {
  description: string;
  detection: string;
  threshold: number;
  action: string;
...
export const ABUSE_PATTERNS: Record<string, AbusePatternDefinition> = {
 ...
 *
 * Consumer-facing appeal surfaces may choose graceful degradation, but trust-
 * bearing write paths MUST pass { failClosed: true } so unavailable counters
 * cannot be interpreted as evidence that an action is safe.
 */
export interface CheckAbuseParams {
  entity_id?: string;
  report_type?: string;
...
export async function checkAbuse(
  ...
  actionType: string,
  params: CheckAbuseParams,
  { failClosed = false }: { failClosed?: boolean } = {},
): Promise<CheckAbuseResult> {
  const now = new Date();

  function handleCheckFailure(error: unknown): void {
  ...
  if (actionType === 'report') {
    ...
    } catch (error) { handleCheckFailure(error); }
 ...
 */
export interface RecordOperatorActionParams {
  operatorId: string;
  operatorRole: string;
  targetType: string;
  targetId: string;
...
export async function recordOperatorAction(
  supabase: SupabaseClient,
  params: RecordOperatorActionParams,
