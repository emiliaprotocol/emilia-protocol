...
// =============================================================================
// ESTABLISHMENT THRESHOLDS — quality gate for trust status
// =============================================================================

/**
 * Minimum quality-gated effective evidence for an entity to leave the
 * dampening zone. Below this threshold, the raw score is linearly compressed
 * toward 50 (neutral). The value 5.0 corresponds to approximately 5 bilateral
 * receipts from established submitters, making it non-trivially expensive for
 * a new Sybil cluster to escape neutral territory.
 */
export const DAMPENING_THRESHOLD = 5.0;

/**
 * Minimum quality-gated evidence required for an entity to be considered
 * "established". Matches DAMPENING_THRESHOLD intentionally — an entity that
 * has escaped dampening has also crossed the establishment gate.
 * Kept as a separate export so the two invariants can diverge independently
 * if the protocol is updated.
 */
export const ESTABLISHMENT_EVIDENCE_GATE = 5.0;

/**
 * Minimum number of distinct submitters required for establishment.
 * Three is the minimum quorum that prevents a single-party sockpuppet attack:
 * with only 2 required, one entity controlling two accounts can self-establish.
 * At 3, the attacker needs to compromise or collude with at least one external
 * established entity, raising the attack cost above the trust laundering benefit.
 */
export const ESTABLISHMENT_MIN_SUBMITTERS = 3;

/**
 * Maximum total contribution from unestablished submitters toward the
 * quality-gated evidence total. Without this cap, 200 fake unestablished
 * identities each contributing 0.01 evidence could cross the dampening
 * threshold as a coordinated swarm. The cap binds the entire unestablished
 * cohort, not each individual — one established submitter plus 200 fake ones
 * is treated identically to one established submitter plus one unestablished one.
 */
export const MAX_UNESTABLISHED_AGGREGATE_CONTRIBUTION = 2.0;
 ...
 */
export const EP_WEIGHTS_V2: Record<string, number> = {
 ...
 */
export const WEIGHT_BOUNDS: Record<string, { min: number; max: number }> = {
 ...
 *
 * @param weights - { behavioral, consistency, delivery, product, price, returns }
 */
...
export type ValidateScoringWeightsResult =
  | { valid: true; weights: ScoringWeights; errors: never[] }
  | { valid: false; weights: null; errors: string[] };

export function validateScoringWeights(weights: Record<string, any>): ValidateScoringWeightsResult {
  const errors: string[] = [];
  const required: Array<keyof ScoringWeights> = ['behavioral', 'consistency', 'delivery', 'product', 'price', 'returns'];
  ...
  return { valid: true, weights: cleaned as unknown as ScoringWeights, errors: [] };
}
 ...
 * signals, v2 claims/evidence, behavioral and provenance metadata all merged
 * into one loosely-shaped record) — typed as a permissive record throughout
 * this file rather than a strict interface.
 */
type TrustReceipt = Record<string, any>;

export interface TrustProfileBehavioral {
  score: number;
...
export interface TrustProfileProvenance {
  breakdown: Record<string, number>;
  bilateral_rate: number | null;
}

export interface TrustProfileDimensions {
  behavioral: TrustProfileBehavioral;
  signals: TrustProfileSignals;
  consistency: number;
  provenance: TrustProfileProvenance;
}

export type TrustConfidence = 'pending' | 'insufficient' | 'provisional' | 'emerging' | 'confident';

export interface ScoreAnomaly {
  type: 'declining' | 'improving';
  delta: number;
  period: string;
  recent_avg: number;
  older_avg: number;
  ...
  significance: number;
  alert: 'moderate' | 'severe';
}

export interface TrustProfileResult {
  score: number;
  confidence: TrustConfidence;
  profile: TrustProfileDimensions | null;
  anomaly: ScoreAnomaly | null;
  effectiveEvidence: number;
  established: boolean;
  dispute_dampened_count: number;
  qualityGatedEvidence?: number;
  uniqueSubmitters?: number;
  receiptCount?: number;
  last_receipt_at?: unknown;
  weights_version?: 'policy' | 'ep-v2-default';
 ...
 *
 * @param receipts - Receipt history, most recent first
 * @param entityMeta - { created_at, total_receipts }
 * @param disputedReceiptIds - Set of receipt IDs currently under active dispute.
 *   Receipts in this set receive DISPUTE_DAMPENING_FACTOR (0.3x) weight.
 * @param weights - Custom scoring weights. If null/undefined, uses EP_WEIGHTS_V2
...
export function computeTrustProfile(
  receipts: TrustReceipt[] | null | undefined,
  entityMeta: Record<string, any> = {},
  disputedReceiptIds: Set<unknown> = new Set(),
  weights: Record<string, number> | null = null,
): TrustProfileResult {
  const w: Record<string, number> = weights || EP_WEIGHTS_V2;
  const weightsVersion: 'policy' | 'ep-v2-default' = weights ? 'policy' : 'ep-v2-default';
  const now = Date.now();
  ...
  const BEHAVIOR_VALUES: Record<string, number> = {
    completed: 95,
    retried_same: 75,
  ...
  const signalAccum: Record<string, { sum: number; weight: number }> = {
  ...
  const submitterContributions = new Map<any, number>();
  ...
  const qualityGatedEvidence = Math.min(
    effectiveEvidence,
    establishedEvidence + Math.min(Math.max(0, effectiveEvidence - establishedEvidence), MAX_UNESTABLISHED_AGGREGATE_CONTRIBUTION)
  );
  if (qualityGatedEvidence < DAMPENING_THRESHOLD) {
    score = 50 + (score - 50) * (qualityGatedEvidence / DAMPENING_THRESHOLD);
  }

  const uniqueSubmitters = new Set(window.map(r => r.submitted_by).filter(Boolean)).size;
  const established = qualityGatedEvidence >= ESTABLISHMENT_EVIDENCE_GATE && uniqueSubmitters >= ESTABLISHMENT_MIN_SUBMITTERS;
 ...
 *
 * @param receipts - Recent receipts
 * @param now - Current timestamp in ms
 * @returns Anomaly data or null if none detected
 */
function detectScoreAnomaly(receipts: TrustReceipt[], now: number): ScoreAnomaly | null {
  ...
  const safeScore = (v: any): number => (Number.isFinite(v) ? Math.max(0, Math.min(100, v)) : 50);

  // Compute mean, variance, stdDev, and standard error for a set of receipts.
  function windowStats(
    receipts: TrustReceipt[],
    scoreGetter: (r: TrustReceipt) => number,
  ): { mean: number; variance: number; stdDev: number; stderr: number; n: number } | null {
  ...
  const alert: 'severe' | 'moderate' | null = (Math.abs(delta) >= 20 && significance >= 3.0 && minN >= 10) ? 'severe'
  ...
  return {
    type: delta < 0 ? 'declining' as const : 'improving' as const,
    delta,
    period: '7d vs 30d',
 ...
 *
 * @param profile - Trust profile from computeTrustProfile()
 * @param policy - Trust policy
 */
export interface TrustPolicy {
  min_score?: number;
  min_confidence?: TrustConfidence;
  min_receipts?: number;
  max_dispute_rate?: number;
  signal_minimums?: Record<string, number>;
  min_completion_rate?: number;
  reject_anomaly?: boolean;
  max_days_since_last_receipt?: number;
  software_requirements?: Record<string, any>;
}
...
export function evaluateTrustPolicy(
  profile: TrustProfileResult | null | undefined,
  policy: TrustPolicy,
  ...
  // Confidence level
  const confLevels: TrustConfidence[] = ['pending', 'insufficient', 'provisional', 'emerging', 'confident'];
  ...
  // Minimum receipts
  if (policy.min_receipts != null && (profile.receiptCount as number) < policy.min_receipts) {
  ...
  if (policy.signal_minimums && profile.profile?.signals) {
    const signalScores = profile.profile.signals as unknown as Record<string, number | null>;
    for (const [signal, min] of Object.entries(policy.signal_minimums)) {
      const actual = signalScores[signal];
  ...
  if (policy.max_days_since_last_receipt != null && profile.last_receipt_at) {
    const daysSince = (Date.now() - new Date(profile.last_receipt_at as string | number | Date).getTime()) / 86400000;
...
export const TRUST_POLICIES: Record<string, TrustPolicy> = {
