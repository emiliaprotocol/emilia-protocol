 ...
 *
 * @param params
 * @param params.challengeId - The challenge to revoke
 * @param params.reason - Reason for revocation
 * @param params.actor - The actor requesting revocation
 * @returns The updated challenge record
 */
export async function revokeChallenge({ challengeId, reason, actor }: RevokeChallengeParams): Promise<any> {
  if (!challengeId) {
 ...
 * @param challengeId - The challenge whose attestation to resolve
 * @param humanEntityRef - The caller's entity ref
 * @returns The resolved signoff_id
 */
async function resolveSignoffIdForActor(supabase: any, challengeId: string, humanEntityRef: string): Promise<string> {
 ...
 *
 * @param params
 * @param params.signoffId - The attestation to revoke
 * @param params.challengeId - Challenge to resolve the caller's attestation from
 * @param params.reason - Reason for revocation
 * @param params.actor - The actor requesting revocation
 * @returns The updated attestation record
 */
export async function revokeAttestation({ signoffId, challengeId, reason, actor }: RevokeAttestationParams): Promise<any> {
  ...
  const resolvedSignoffId = signoffId
    // signoffId and challengeId can't both be falsy here: the guard above throws
    // MISSING_SIGNOFF_ID in that case, so reaching this branch guarantees challengeId.
    || await resolveSignoffIdForActor(supabase, challengeId as string, actor.entity_id);
