...
async function acquireDeduplicationLock(entityId, submitterId, transactionRef) {
  ...
  if (_dedupUseRedis) {
    try {
      ...
      const res = await fetch(/** @type {string} */ (UPSTASH_URL), {
        method: 'POST',
        headers: {
      ...
      const release = acquired
        ? async () => {
            try {
              await fetch(/** @type {string} */ (UPSTASH_URL), {
                method: 'POST',
                headers: {
                  Authorization: `Bearer ${UPSTASH_TOKEN}`,
                  'Content-Type': 'application/json',
 ...
 * @returns {Promise<{ allowed: boolean, count?: number, limit?: number, unavailable?: boolean }>}
 */
async function checkEntityDailyQuota(supabase, entityId) {
  ...
  try {
    ...
    if (!Number.isSafeInteger(count) || /** @type {number} */ (count) < 0) {
    ...
    const safeCount = /** @type {number} */ (count);
 ...
 * @param {*} [params.context] - Free-form transaction context recorded on the receipt
 * @param {string} [params.provenanceTier] - Requested provenance tier (defaults to 'self_attested')
 * @param {boolean} [params.requestBilateral] - If true, sets bilateral_status to 'pending_confirmation'
