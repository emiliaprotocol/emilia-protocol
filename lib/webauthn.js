...
export function getRpConfig() {
  return {
    ...
    rpID: process.env.WEBAUTHN_RP_ID
      || (process.env.NODE_ENV === 'development' ? 'localhost' : 'emiliaprotocol.ai'),
...
export function coseToSpkiP256(coseKeyBytes) {
  const decoded = new Decoder({ mapsAsObjects: false }).decode(
    coseKeyBytes instanceof Uint8Array ? coseKeyBytes : new Uint8Array(coseKeyBytes),
  );
