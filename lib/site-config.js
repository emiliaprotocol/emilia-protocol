...
export const ENTITY = {
  // Operating entity. CA SOS file no. B20260099176, amended 4/22/2026.
  // Name is "Future Enterprises Corporation" (plural) — confirmed against the
  // CA Certificate of Amendment. A dedicated Delaware C-Corp for EMILIA is
  // being formed via Stripe Atlas; update legalName + entityType + jurisdiction
  // here once that EIN is issued (one-line swap, no other changes needed).
  legalName: 'Future Enterprises Corporation',
  entityType: 'California corporation (dedicated EMILIA entity forming)',
  jurisdiction: 'California, USA',
  // Public site shows city/state only. Full mailing address + EIN live on
  // procurement / SAM.gov forms — never in this public file.
  address: 'Los Angeles, California, USA — full mailing address on request',
  // CA SOS entity number (public, optional). NEVER put the EIN here.
  registrationNumber: 'B20260099176',
...
export const SUB_PROCESSORS = [
  ...
  { name: 'OpenAI, L.L.C.', purpose: 'AI Trust Desk — LLM answer drafting (fallback)', region: 'United States', data: 'Questionnaire question text, vendor product description (inference only; not used for training)' },
  ...
  // TODO[founder]: add error tracking (Sentry) + analytics provider if/when added.
];
