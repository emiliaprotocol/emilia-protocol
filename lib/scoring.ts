/**
 * EMILIA Protocol — Compatibility Scoring Engine (v1)
 *
 * STATUS (2026-03-23):
 *   STILL ACTIVE — computeReceiptComposite, computeReceiptHash, behaviorToSatisfaction,
 *     and computeScoresFromClaims are imported by lib/create-receipt.js, app/ routes, and tests.
 *   DEAD IN PRODUCTION — computeEmiliaScore and computeMatchScore are only called by
 *     tests/scoring.test.js. No app/ or lib/ code imports them. They remain here for
 *     test coverage and potential future use; do not delete without confirming test migration.
 *
 * For trust decisions, use lib/scoring-v2.js (trust profiles + policy evaluation).
 *
 * @license Apache-2.0
 */
 ...
 * into one loosely-shaped record) — typed as a permissive record rather than
 * a strict interface throughout this file.
 */
export type ScoringReceipt = Record<string, any>;

// =============================================================================
// SCORE WEIGHTS — the published formula
// =============================================================================
export const EMILIA_WEIGHTS: Record<string, number> = {
 ...
 *
 * @param receipt - { delivery_accuracy, product_accuracy, price_integrity, return_processing, agent_satisfaction }
 * @returns 0-100 composite score
 */
export function computeReceiptComposite(receipt: ScoringReceipt): number {
  let totalWeight = 0;
  let weightedSum = 0;
...
// =============================================================================
/**
 * Compute an entity's compatibility score from their receipt history.
 ...
 * 
 * @deprecated Test-only remnant. No production (app/ or lib/) code imports this function.
 *   It is retained solely for test coverage in tests/scoring.test.js.
 *   For production trust decisions, use lib/scoring-v2.js.
 *
 * @param receipts - Array of receipt objects, most recent first.
 *   Each receipt may include: submitter_score (0-100) for weighting
 */
export interface EmiliaScoreBreakdown {
...
export interface EmiliaScoreResult {
  score: number;
  breakdown: EmiliaScoreBreakdown | null;
  receiptCount: number;
...
export function computeEmiliaScore(receipts: ScoringReceipt[] | null | undefined): EmiliaScoreResult {
  ...
  // Compute effective evidence FIRST — sum of all receipt weights
  // This matches the legacy SQL compatibility-score path.
  // The canonical trust-profile logic lives in scoring-v2.js.
  let effectiveEvidence = 0;

  const now = Date.now();

  // Pre-compute per-receipt weights
  const receiptWeights = window.map(r => {
    ...
    return submitterWeight * timeWeight * graphWeight;
  });

  effectiveEvidence = receiptWeights.reduce((sum, w) => sum + w, 0);
  ...
  for (const signal of Object.keys(EMILIA_WEIGHTS)) {
    ...
    for (let i = 0; i < window.length; i++) {
      const value = window[i][signal];
      if (value != null && !isNaN(value)) {
        weightedSum += value * receiptWeights[i];
        totalWeight += receiptWeights[i];
      }
    }
 ...
 * 
 * @param submitterScore - The submitter's EMILIA score (0-100)
 * @param submitterEstablished - Whether submitter has 5+ receipts from 3+ submitters
 * @returns Weight multiplier (0.1 to 1.0)
 */
export function computeSubmitterWeight(
  submitterScore: number | null | undefined,
  submitterEstablished: boolean = false,
): number {
 ...
 * @returns Weight multiplier (0.05 to 1.0)
 */
export function computeTimeDecay(createdAt: string | number | Date | null | undefined, now: number): number {
 ...
 *   For production trust decisions, use lib/scoring-v2.js.
 *
 * @param cosineSimilarity - 0-1, from pgvector
 * @param emiliaScore - 0-100, entity's compatibility score
 * @param minScore - minimum required score (from the need)
 * @returns match score (0-1), or null if below minimum
 */
 ...
 *
 * @param behavior - One of: completed, retried_same, retried_different, abandoned, disputed
 * @returns 0-100 satisfaction score, or null for an unrecognized behavior
 */
export function behaviorToSatisfaction(behavior: string | null | undefined): number | null {
  const BEHAVIOR_SCORES: Record<string, number> = {
  ...
  return (behavior != null ? BEHAVIOR_SCORES[behavior] : undefined) ?? null;
}
 ...
 *
 * @param claims - Structured claims from v2 receipt
 * @returns Computed signal scores { delivery_accuracy, product_accuracy, ... }
 */
export interface ComputedClaimScores {
...
export function computeScoresFromClaims(claims: Record<string, any> | null | undefined): ComputedClaimScores {
  if (!claims || typeof claims !== 'object') return {};

  const scores: ComputedClaimScores = {};
 ...
 * @param previousHash - hash of the previous receipt for this entity
