 ...
 * @returns {{url:string|null, kind:string, minRank:number, shadow:boolean, revealRefs:boolean, timeoutMs:number}}
 */
function readConfig() {
...
function inferKind(explicit, url) {
  ...
  const host = url ? (safeParseUrl(url)?.hostname || '') : '';
  if (host.includes('discord')) return 'discord';
  if (host.includes('office.com') || host.includes('webhook.office') || host.includes('teams')) {
    return 'teams';
  }
  return 'slack';
}
