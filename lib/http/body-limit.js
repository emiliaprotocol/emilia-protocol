...
export async function readLimitedJson(request, maxBytes, { emptyValue = {}, invalidValue } = {}) {
  // Unit-test doubles in this repo often provide only `{ json() }`. Real
  // runtime requests always have Headers + ReadableStream, so production still
  // goes through the byte-enforcing path below.
  if ((!request?.headers || typeof request.headers.get !== 'function') && typeof request?.json === 'function') {
    try {
