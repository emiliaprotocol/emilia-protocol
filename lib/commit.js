...
export async function issueCommit({
  ...
  try {
    ...
    if (['custody_signer_not_registered', 'missing_custody_key_id', 'unknown_key_custody_mode'].includes(e.code)) {
      throw new ProtocolWriteError(`commit signing custody error: ${e.message}`, 500);
    }
    custodySigner = null;
  }
  ...
  if (custodySigner) {
    ...
    if (!spkiB64u) {
      throw new ProtocolWriteError('custody signer did not expose a public key (getPublicKey required to register the verification key)', 500);
    }
