...
function deepCanonicalize(value) {
  ...
  for (const key of Object.keys(value).sort()) {
    if (value[key] !== undefined) out[key] = deepCanonicalize(value[key]);
  }
  return out;
}
 ...
 * @param {string|null} [params.principal_id] - Human/org principal
 * @param {string|null} [params.counterparty_entity_id] - Other side of interaction
 * @param {string|null} [params.delegation_id] - If acting under delegation
 * @param {string} params.action_type - install | connect | delegate | transact
 * @param {Object} [params.scope] - Action scope details
 * @param {number|null} [params.max_value_usd] - Advisory only. This field informs policy evaluation.
 *   EP does not enforce, hold, or settle monetary value.
...
export async function issueCommit({
  ...
  const evaluation = await canonicalEvaluate(entity_id, /** @type {*} */ ({
    context,
    policy,
  ...
  let custodySigner = null;
  try {
    const custodyConfig = typeof getKeyCustodyConfig === 'function' ? getKeyCustodyConfig() : null;
    if (custodyConfig) custodySigner = resolveIssuerSigner(custodyConfig);
  } catch (e) {
    ...
    if (['custody_signer_not_registered', 'missing_custody_key_id', 'unknown_key_custody_mode'].includes(e.code)) {
      throw new ProtocolWriteError(`commit signing custody error: ${e.message}`, /** @type {*} */ (500));
    }
    custodySigner = null;
  }
  ...
  if (custodySigner) {
    ...
    if (!spkiB64u) {
      throw new ProtocolWriteError('custody signer did not expose a public key (getPublicKey required to register the verification key)', /** @type {*} */ (500));
    }
  ...
  if (error) {
    throw new ProtocolWriteError(
      ...
      { status: 500, code: 'COMMIT_STORAGE_FAILED', cause: /** @type {any} */ (error) }
    );
  }
...
export async function verifyCommit(commit_id_or_token) {
  ...
  if (fetchError) {
    throw new ProtocolWriteError(
      ...
      { status: 500, code: 'COMMIT_VERIFY_DB_ERROR', cause: /** @type {any} */ (fetchError) }
    );
  }
  ...
  if (commit.nonce) {
    ...
    if (dupeError) {
      throw new ProtocolWriteError(
        ...
        { status: 503, code: 'COMMIT_REPLAY_CHECK_FAILED', cause: /** @type {any} */ (dupeError) }
      );
    }
  ...
  if (revokedKeyError) {
    throw new ProtocolWriteError(
      ...
      { status: 503, code: 'COMMIT_KEY_REVOCATION_CHECK_FAILED', cause: /** @type {any} */ (revokedKeyError) }
    );
  }
  if (revokedKey) {
...
export async function revokeCommit(commit_id, reason) {
  ...
  if (fetchError) {
    throw new ProtocolWriteError(
      ...
      { status: 500, code: 'REVOCATION_FETCH_FAILED', cause: /** @type {any} */ (fetchError) }
    );
  }
  if (!commit) {
...
export async function fulfillCommit(commit_id) {
  ...
  if (fetchError) {
    throw new ProtocolWriteError(
      ...
      { status: 500, code: 'FULFILLMENT_FETCH_FAILED', cause: /** @type {any} */ (fetchError) }
    );
  }
  if (!commit) {
    throw new CommitError('Commit not found', 404, 'NOT_FOUND');
...
export async function getCommitStatus(commit_id) {
  ...
  if (error) {
    throw new ProtocolWriteError(
      ...
      { status: 500, code: 'COMMIT_STATUS_FETCH_FAILED', cause: /** @type {any} */ (error) }
    );
  }
  if (!commit) {
    return null;
...
export async function bindReceiptToCommit(commit_id, receipt_id) {
  ...
  if (fetchError) {
    throw new ProtocolWriteError(
      ...
      { status: 500, code: 'BIND_RECEIPT_FETCH_FAILED', cause: /** @type {any} */ (fetchError) }
    );
  }
  if (!commit) {
    throw new CommitError('Commit not found', 404, 'NOT_FOUND');
