...
export async function bindReceiptToCommit(commit_id, receipt_id) {
  ...
  // status='active', so a commit that was fulfilled/revoked concurrently
  // (after the read above) can never have its receipt overwritten. A 0-row
  // result means the commit is no longer active — reject rather than persist.
  const { data: updatedRows, error: updateError } = await supabase
    .from('commits')
    .update({ receipt_id })
    .eq('commit_id', commit_id)
    .eq('status', 'active') // optimistic concurrency: only bind while still active
    .select('commit_id');
  ...
  if (!updatedRows || updatedRows.length === 0) {
    throw new CommitError(
      'Cannot bind receipt: commit is no longer active — terminal states are immutable',
      409, 'INVALID_STATE_FOR_RECEIPT'
    );
  }
