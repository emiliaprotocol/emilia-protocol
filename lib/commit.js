...
export async function issueCommit({
  ...
  if (evaluation.policyResult) {
    ...
    // No policy specified — decide based on trust score thresholds
    const score = evaluation.score ?? 0;
    if (score >= 0.6) decision = 'allow';
...
export async function verifyCommit(commit_id_or_token) {
  ...
  if (supabase) {
    ...
    if (error?.code === '42P01') {
      return { valid: false, status: 'unknown', reasons: ['commits table not yet created'] };
    }
    commit = data;
  }

  if (!commit) {
    return { valid: false, status: 'not_found', reasons: ['Commit not found'] };
  }
  ...
  if (commit.status !== 'active') {
    reasons.push(`Commit is ${commit.status}`);
    return {
      valid: false,
  ...
  if (commit.nonce) {
    ...
    if (!dupeError && dupeRows && dupeRows.length > 0) {
      reasons.push('Nonce reuse detected');
      return {
        valid: false,
        status: commit.status,
        decision: commit.decision,
