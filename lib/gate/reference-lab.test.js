...
describe('EMILIA Gate reference lab', () => {
  for (const profileId of Object.keys(GATE_REFERENCE_PROFILES)) {
    it(`${profileId} executes once and refuses every hostile path`, async () => {
      ...
      expect(result.authorization.policy_id).toBe(result.profile.policy);
      expect(result.authorization.policy_hash).toBe(result.profile.policy_hash);
      expect(result.authorization.tenant_id).toBe(result.profile.tenant_id);
      expect(result.ceremony.context_policies).toEqual([result.profile.policy]);
