...
const MAX_SIGNOFF_DECISION_BYTES = 32 * 1024;
...
export async function handleSignoffDecision(request, params, decision) {
  try {
    ...
    const parsed = await readLimitedJson(request, MAX_SIGNOFF_DECISION_BYTES, /** @type {*} */ ({ invalidValue: {} }));
    ...
    // Filter by signoff_id at the SQL layer rather than scanning every
    // `guard.signoff.requested` event in JS. The previous query was a
    // full-table scan over the requested-event partition; at any volume
    // this is a DoS vector. The `->>` path operator extracts the JSONB
    // value and Supabase routes it as a server-side filter.
    ...
    const expectedApproverId = requestEvent.after_state.approver_id
      || requestEvent.after_state.quorum?.approver_id
      || null;
    const expectedActionHash = requestEvent.after_state.action_hash;
    const requiredAssurance = requestEvent.after_state.required_assurance || null;
    const expiresAt = requestEvent.after_state.expires_at;

    // SoD on string identities. The old `auth.entity === initiatorId` compared
    // the entity ROW OBJECT to a string — always false, so this guard never
    // fired. authEntityId() makes the comparison real.
    const approverEntityId = authEntityId(auth);
    if (approverEntityId === initiatorId) {
      return epProblem(
        403,
    ...
    if (!expectedApproverId) {
      return epProblem(
        409,
        'approver_not_bound',
        'Signoff request is missing an intended approver and cannot be safely decided',
      );
    }
    if (approverEntityId !== expectedApproverId) {
      return epProblem(
        403,
        'approver_mismatch',
        'Authenticated approver does not match the approver bound to this signoff request',
      );
    }
    ...
    if (requiredAssurance === 'A') {
      return epProblem(
        403,
        'insufficient_assurance',
        'This receipt requires a Class-A WebAuthn/passkey decision; bearer-key approval or rejection is not sufficient',
      );
    }
    ...
    const { error: insertErr } = await supabase.from('audit_events').insert({
      ...
      after_state: {
        ...
        approver_id: approverEntityId,
        approved_action_hash: body.approved_action_hash,
        comment: typeof body.comment === 'string' ? body.comment.slice(0, 500) : null,
        decided_at: decidedAt,
        // Honest labeling per the EP draft §5.1: this path authenticates the
        // approver by bearer EP API key and the server records the decision —
        // operator-custodied assurance, key_class C. The WebAuthn path
        // (/approve-webauthn) records key_class A with the device assertion.
        key_class: 'C',
      },
    });

    if (insertErr) {
      // Postgres unique_violation (SQLSTATE 23505) on
      // guard_signoff_decided_once means another approver/rejecter raced
      // this one. Return 409 — the signoff has already been decided.
