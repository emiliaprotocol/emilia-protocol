...
function evidenceEnvelope(stored, now) {
  ...
  const body = {
    ...
    limitations: [...RELEASE_LOCK_LIMITATIONS, authenticityLimitation],
    evidence: stored,
  };
...
export function createReleaseLockService({
  ...
  async function createLock({ organizationId, contractorEntityId, input }) {
    ...
    const invitations = rawInvitations.map((invite) => ({
      ...
      role: invite.role,
      contact_binding_id: contacts.find((entry) => entry.role === invite.role).contact_binding_id,
  ...
  async function actionCheckOptions({
    ...
    const built = actionCheckBuilder({
      lockId,
      version: context.version,
      round,
      role: context.role,
  ...
  async function recordEffectOutcome(effect, outcome) {
    ...
    return call('release_lock_record_effect_result', {
  ...
  async function reconcile({ effectReference }) {
    ...
    try {
      ...
      return call('release_lock_record_reconciliation', {
        p_effect_reference: effectReference,
        p_status: status,
        p_provider_result: outcome.result,
      });
    } catch (error) {
      try {
        await call('release_lock_record_reconciliation', {
          p_effect_reference: effectReference,
          p_status: 'indeterminate',
          p_provider_result: effectFailureResult(effect, 'unknown_effect', error),
        });
      } catch {
