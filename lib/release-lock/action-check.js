...
function optionId(questionId, label) {
  return `opt_${canonicalDigest({ question_id: questionId, label }).slice(-24)}`;
}

function question(questionId, stem, correct, decoys, randomInt) {
  const labels = shuffled([...new Set([correct, ...decoys])].slice(0, 4), randomInt);
  ...
  const options = labels.map((label) => ({
    option_id: optionId(questionId, label),
    label,
  }));
  return {
    prompt: {
    ...
    answer: {
      question_id: questionId,
      option_id: optionId(questionId, correct),
    },
  };
}
...
function coQuestions(action, randomInt) {
...
function drawQuestions(action, randomInt) {
...
export function buildReleaseLockActionCheck({
  ...
  const pairs = shuffled(
    round === 'CO_ACCEPTED' ? coQuestions(action, randomInt) : drawQuestions(action, randomInt),
    randomInt,
  );
  const promptSet = {
