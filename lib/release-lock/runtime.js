...
/**
 * Mirrors the documented options of createEscrowComAdapter (see
 * lib/integrations/action-escrow/escrow-com.ts). Declared explicitly here
 * because the adapter's own destructured-parameter type currently only
 * infers the properties that carry defaults (timeoutMs, maxResponseBytes);
 * passing the full option set through a variable typed against this shape
 * (rather than an inline object literal) avoids tripping that gap while the
 * adapter itself is still being converted to real TypeScript.
 *
 * @typedef {object} EscrowComCustodianOptions
 * @property {'sandbox'|'production'} environment
 * @property {string} email
 * @property {string} apiKey
 * @property {typeof fetch} fetch
 * @property {object} customerDiligence
 * @property {(binding: object) => Promise<boolean>} claimEffectBinding
...
function resolveEnvironmentCustodianAdapter(request) {
  ...
  try {
    /** @type {EscrowComCustodianOptions} */
