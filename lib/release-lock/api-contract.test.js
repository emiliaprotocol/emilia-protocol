...
function routeFiles(root) {
  return fs.readdirSync(root, { recursive: true, withFileTypes: true })
    .filter((entry) => entry.isFile() && entry.name === 'route.js')
...
describe('Release Lock API contract', () => {
  ...
  it('exchanges raw invitation input only into the strict session cookie', () => {
    const exchange = fs.readFileSync(
      path.join(publicRoot, 'invitations/exchange/route.js'),
      'utf8',
    );
    ...
    const pairingExchange = fs.readFileSync(
      path.join(publicRoot, 'pairings/exchange/route.js'),
      'utf8',
    );
  ...
  it('exposes both approval rounds without a public effect retry/refund route', () => {
    ...
    expect(paths).toContain(
      '[lockId]/rounds/[round]/action-check/options/route.js',
    );
    expect(paths).toContain('[lockId]/rounds/[round]/approvals/route.js');
    expect(paths).toContain('[lockId]/rounds/[round]/pairings/route.js');
    expect(paths.some((file) => /(^|\/)(retry|refund|release)\/route\.js$/.test(file)))
      .toBe(false);
    expect(routeFiles(internalRoot).map((file) => path.relative(internalRoot, file)))
      .toEqual(['reconcile/route.js']);
  });

  it('keeps organization writes bound to the existing authenticated-org pattern', () => {
    for (const relative of [
      'route.js',
      '[lockId]/draw-release/route.js',
      '[lockId]/amendments/route.js',
      '[lockId]/evidence/route.js',
    ]) {
