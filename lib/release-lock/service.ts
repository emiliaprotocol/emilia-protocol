...
function exactObject(value: any, allowed: any, required: any = allowed) {
...
function requiredText(value: any, code: any = 'invalid_request') {
...
function currentMillis(now: any) {
...
function evidenceEnvelope(stored: any, now: any, extraLimitations: any = []) {
...
function rpPolicy(provider: any) {
...
function assertCredentialPolicy(credential: any, policy: any) {
...
function contactsForProvider(normalized: any) {
...
export function createReleaseLockService({
  ...
  async function call(name: any, args: any) {
    try {
      const result = await (rpc as any)(name, args);
  ...
  async function createLock({ organizationId, contractorEntityId, input }: any) {
    ...
    const normalized = validateChangeOrderInput(input, ({
      now: nowMs,
      cryptoSuite,
      contractorEntityId,
    }) as any);
    ...
    const invitations = rawInvitations.map((invite) => ({
      ...
      role: invite.role,
      contact_binding_id: (
        contacts.find((entry) => entry.role === invite.role) as any
      ).contact_binding_id,
  ...
  async function exchangeInvitation(input: any) {
  ...
  async function createPairing({ rawSessionToken, lockId, round }: any) {
  ...
  async function exchangePairing(input: any) {
  ...
  async function resolveSession(rawSessionToken: any, lockId: any) {
  ...
  async function participantView({ rawSessionToken, lockId }: any) {
  ...
  async function beginRegistration({ rawSessionToken, lockId }: any) {
    const session = await resolveSession(rawSessionToken, lockId);
    const policy = rpPolicy(rpConfigProvider);
    const generated = await registrationOptions(({
      session: {
        ...session,
  ...
  async function stageDraw({
    ...
    const normalized = validateDrawReleaseInput(input, ({
      now,
  ...
  function effectFailureResult(effect: any, status: any, error: any) {
    return {
  ...
  async function recordEffectOutcome(effect: any, outcome: any) {
  ...
  async function executeReservedEffect(effect: any) {
  ...
  async function approve({
    ...
    const verified = await actionCheckVerifier(({
      challenge,
  ...
  async function evidence({ organizationId, lockId }: any) {
  ...
  async function participantEvidence({ rawSessionToken, lockId }: any) {
  ...
  async function reconcile({ effectReference }: any) {
