 ...
 * @param {number|null} [opts.maxExpiresAt]
...
export function validateChangeOrderInput(input, {
  ...
  if (!amendment && (invitationExpiresAtMs <= nowMs || invitationExpiresAtMs > lockExpiresAtMs)) {
    throw releaseLockRefusal(
      400,
  ...
  if (!amendment
      && (invitationExpiresAtMs > Date.parse(contractor.contact.verification_expires_at)
        || invitationExpiresAtMs > Date.parse(customer.contact.verification_expires_at))) {
    throw releaseLockRefusal(
      400,
      'invalid_invitation_expiry',
 ...
 * @param {object} [opts]
 * @param {number|(() => number)} [opts.now]
 * @param {number} [opts.maxExpiresAt]
 */
