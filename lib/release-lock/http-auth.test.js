...
describe('Release Lock organization authorization', () => {
  ...
  it('pins route permission classes and keeps reconciliation operator-only', () => {
    ...
    const mutationFiles = [
      'app/api/v1/release-locks/route.ts',
      'app/api/v1/release-locks/[lockId]/draw-release/route.ts',
      'app/api/v1/release-locks/[lockId]/amendments/route.ts',
    ];
    ...
    expect(readFileSync(
      new URL('app/api/v1/release-locks/[lockId]/evidence/route.ts', root),
      'utf8',
    )).toContain("requiredPermission: 'read'");
    const reconcile = readFileSync(
      new URL('app/api/internal/release-lock/reconcile/route.ts', root),
      'utf8',
    );
    expect(reconcile).toContain('authenticateOperator');
    expect(reconcile).toContain("hasPermission(operator.role as string, 'release_lock.reconcile')");
