...
export class ReleaseLockError extends Error {
  constructor(status, code, detail, { cause = null, expose = true } = {}) {
    super(detail);
...
export function releaseLockRefusal(status, code, detail, options = {}) {
  return new ReleaseLockError(status, code, detail, options);
}

export function mapReleaseLockRpcError(error, operation = 'release_lock_storage') {
  const text = [
    error?.message,
    error?.details,
    error?.hint,
...
export function isReleaseLockError(error) {
