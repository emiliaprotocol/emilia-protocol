...
export type ScimValidationResult =
  | { ok: true }
  | { ok: false; error: ScimValidationError };

interface JsonBoundsLimits {
  maxDepth: number;
  maxNodes: number;
  ...
  maxKey: number;
  maxString: number;
  maxTotalString: number;
}

function validationFailure(detail: string, scimType: string = 'invalidValue'): ScimValidationResult {
  return { ok: false, error: { status: 400, detail, scimType } };
}

function schemaFailure(label: string, error: z.ZodError): ScimValidationResult {
...
function validateJsonBounds(root: unknown, limits: JsonBoundsLimits): string | null {
...
function validateResourceEnvelope(body: unknown, label: string): ScimValidationResult {
...
function validateRawExtensions(body: Record<string, any>, label: string): ScimValidationResult {
...
function isBooleanPatchValue(value: unknown): boolean {
...
function validatePatchTargetValue(path: string, value: unknown, index: number): ScimValidationResult | null {
...
export function validateScimGroup(
  body: Record<string, any>,
  { requireDisplayName = true }: { requireDisplayName?: boolean } = {},
): ScimValidationResult {
...
export function validateScimPatch(body: Record<string, any>): ScimValidationResult {
...
export function scimError(status: number, detail: string, scimType?: string): Record<string, any> {
  const body: Record<string, any> = { schemas: [SCIM.ERROR], status: String(status), detail };
...
export function toScimUser(row: Record<string, any>, baseUrl: string = ''): Record<string, any> {
...
export function fromScimUser(body: Record<string, any>): Record<string, any> {
...
export function toScimGroup(row: Record<string, any>, baseUrl: string = ''): Record<string, any> {
  const resource: Record<string, any> = {
    schemas: [SCIM.GROUP],
    id: row.id,
...
export function fromScimGroup(body: Record<string, any>): Record<string, any> {
  return {
...
export function listResponse(
  resources: unknown[],
  { totalResults, startIndex = 1, itemsPerPage }: ListResponseOptions = {},
): Record<string, any> {
  return {
    schemas: [SCIM.LIST],
...
export type ScimFilter =
  ...
  | { unsupported: true; raw: string; attribute?: undefined; operator?: undefined; value?: undefined };
 ...
 *
 * Returns null when no filter is present; an `unsupported` marker the route
 * turns into a precise 400 for anything outside the supported subset.
 */
export function parseFilter(filter: unknown): ScimFilter | null {
  ...
  let value: string | boolean | undefined = m[3];
...
export type ApplyPatchResult =
  | { resource: Record<string, any>; error?: undefined }
  | { error: ScimValidationError; resource?: undefined };
 ...
 */
export function applyPatch(resource: Record<string, any>, body: Record<string, any>): ApplyPatchResult {
...
function setAttr(resource: Record<string, any>, path: string, value: unknown, verb: string = 'replace'): void {
...
function removeAttr(resource: Record<string, any>, path: string): void {
  const p = normalizePath(path);
  ...
  if (memberMatch) {
    ...
    resource.members = (resource.members || []).filter((m: any) => String(m.value) !== id);
    return;
  }
...
export function serviceProviderConfig(baseUrl: string = ''): Record<string, any> {
  return {
    schemas: [SCIM.SPC],
...
export function resourceTypes(baseUrl: string = ''): Record<string, any> {
  const types = [
    {
...
function mergeMultiValued(existing: unknown, value: unknown, verb: string): any[] {
  const incoming: any[] = Array.isArray(value) ? value : [value];
  if (verb !== 'add') return incoming;
  const current: any[] = Array.isArray(existing) ? existing : [];
...
function str(v: unknown): string { return typeof v === 'string' ? v.trim() : (v == null ? '' : String(v)); }
...
function iso(t: unknown): string | undefined {
  if (!t) return undefined;
  try { return new Date(t as string | number | Date).toISOString(); } catch { return undefined; }
}
function stripServerFields(body: Record<string, any>): Record<string, any> {
  const clone = structuredCloneSafe(body || {});
  delete clone.id;
  delete clone.meta;
  return clone;
}
function structuredCloneSafe(obj: Record<string, any>): Record<string, any> {
