...
/** Result of an authorization check — see authorizeCommitAccess / authorizeCommitIssuance. */
export interface CommitAuthResult {
  authorized: boolean;
  reason?: string;
}

/** Minimal shape of a commit row needed for the access check. */
export interface CommitAccessRecord {
  entity_id: string;
 ...
 *   1. The issuing entity  (commit.entity_id === authenticated entity id)
 *   2. The principal on the commit (commit.principal_id === authenticated entity id)
 *
 * @param auth  - The auth object from authenticateRequest()
 * @param commit - The commit record from the database
 * @param action - Human-readable action for the error message (e.g. "view", "revoke")
 */
export function authorizeCommitAccess(auth: unknown, commit: CommitAccessRecord, action: string): CommitAuthResult {
  const callerEntityId = authEntityId(auth);
  const isIssuer = commit.entity_id === callerEntityId;
  const isPrincipal = commit.principal_id && commit.principal_id === callerEntityId;
 ...
 *
 * @param auth - The auth object from authenticateRequest()
 * @param targetEntityId - The entity_id the commit is being issued for
 * @param delegationId - Optional delegation_id supplied in the request
 * @param actionType - The action_type for scope checking
 */
export async function authorizeCommitIssuance(
  auth: unknown,
  targetEntityId: string,
  delegationId: string | null,
  actionType: string | null,
): Promise<CommitAuthResult> {
  ...
  if (result.agent_entity_id !== callerEntityId) {
    return {
      authorized: false,
