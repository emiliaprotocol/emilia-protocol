...
export function verifyMerkleProof(leafHash, proof, expectedRoot) {
  ...
  for (const step of proof) {
    ...
    // Since hashPair sorts inputs canonically, position is informational only
    // but we still validate it's a known value to catch malformed proofs
...
export async function anchorToBase(batchId, merkleRoot) {
  ...
  if (!privateKey) {
    if (isProduction()) {
      ...
      throw new Error(
        'EP_WALLET_PRIVATE_KEY is not configured. ' +
        'Blockchain anchoring is required in production. ' +
        'Set EP_WALLET_PRIVATE_KEY or remove blockchain anchoring claims from your deployment.'
      );
    }
    logger.warn('EP_WALLET_PRIVATE_KEY not set — skipping on-chain anchor (non-production only)');
    return {
...
export async function runAnchorBatch(supabase) {
  ...
  // 5. Store batch record — abort if this fails.
  // The on-chain tx is committed but the DB record must exist before we
  // mark any receipts as anchored, otherwise receipts end up with a
  // dangling anchor_batch_id that joins to nothing.
  ...
  if (batchErr) {
    // The on-chain transaction is already committed. Log the tx hash so
    // ops can reconcile manually, then abort — do NOT mark receipts.
    logger.error(
      `Anchor batch on-chain (txHash: ${anchorResult.transactionHash}) but DB record failed — receipts NOT marked. Manual reconciliation required.`,
      batchErr,
    );
    throw new Error(
      `Anchor batch DB record failed: ${batchErr.message}. On-chain tx: ${anchorResult.transactionHash}`,
    );
  }

  // 6. Bulk-update all receipts in a single RPC call (migration 075).
