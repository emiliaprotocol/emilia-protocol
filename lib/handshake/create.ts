...
export type HandshakeMode = 'basic' | 'mutual' | 'selective' | 'delegated';
export type HandshakePartyRole = 'initiator' | 'responder' | 'verifier' | 'delegate';

export interface HandshakeParty {
  /** Party role. */
  role: HandshakePartyRole;
  /** Entity reference identifier */
  entity_ref: string;
  /** Required assurance level. */
  assurance_level?: 'low' | 'medium' | 'substantial' | 'high';
  /** Delegation chain for delegated mode. */
  delegation_chain?: object | string | null;
}

export interface HandshakeBindingOverride {
  expires_at?: string | Date;
  session_ref?: string;
}

export interface InitiateCommandInput {
  mode: HandshakeMode;
  policy_id: string;
  ...
  binding: HandshakeBindingOverride | null;
...
export interface InitiateHandshakeResult {
  /** UUID of the created handshake */
  handshake_id: string;
  /** Handshake mode ('unilateral', 'mutual', 'delegated') */
  mode: string;
  /** Policy identifier */
  policy_id: string;
  /** Policy version */
  policy_version: string | null;
  /** Pinned integer policy version row */
  policy_version_number?: number | null;
  /** Hash of the bound action, if any */
  action_hash?: string | null;
  /** Cryptographic snapshot of policy rules at initiation */
  policy_hash?: string | null;
  /** Always 'initiated' for new handshakes */
  status: string;
  /** Created party records */
  parties: Array<Record<string, any>>;
  /** Created binding record including binding_hash */
  binding: Record<string, any>;
  /** True if this was an idempotent return of an existing handshake */
  idempotent?: boolean;
}
 ...
 * @param {HandshakeBindingOverride|null} [params.binding=null] - Override binding fields (nonce, expires_at, session_ref)
...
export async function initiateHandshake({
  ...
  metadata?: Record<string, unknown>;
  actor?: ActorRef;
}): Promise<InitiateHandshakeResult> {
  ...
  for (const party of parties) {
    ...
    const assuranceRank: Record<string, number> = ASSURANCE_RANK;
    if (party.assurance_level && !assuranceRank[party.assurance_level]) {
      throw new HandshakeError(
  ...
  const initiatorParty = parties.find((p) => p.role === 'initiator') as HandshakeParty;
  ...
  if (mode === 'delegated') {
    ...
    const delegateParty = parties.find((p) => p.role === 'delegate') as HandshakeParty;
  ...
  const action_hash = sha256(canonicalize(actionIntent));
 ...
 *
 * @param {{ actor: string|object, input: InitiateCommandInput }} command - The protocol write command
 * @returns {Promise<{ result: InitiateHandshakeResult, aggregateId: string, _protocolEventWritten?: boolean }>}
 * @throws {HandshakeError} DB_ERROR on database failures
 */
export async function _handleInitiateHandshake(command: { actor: ActorRef; input: InitiateCommandInput }): Promise<{
  result: InitiateHandshakeResult;
  aggregateId: string;
  ...
  let policy_hash: string | null = null;
  let policy_version_number: number | null = null;
  if (policy_id) {
  ...
  const bindingMaterial = buildBindingMaterial({
    ...
    context_hash,
    nonce,
    // The expiry is server-derived above. Do not let the optional binding
    // envelope override the value that is persisted and later verified.
    expires_at,
  });
  ...
  const { data: rpcResult, error: rpcError } = await supabase.rpc('create_handshake_atomic', {
    ...
    //
    // Store the EXACT canonical form the hash is computed over. Previously this
    // used a local key-sort reviver with NO NFC normalization, while payload_hash
    // used deepSortKeys WITH NFC, so the stored payload_json did not re-hash to the
    // stored payload_hash for any Unicode-non-NFC input. Use deepSortKeys for both.
    p_protocol_event_payload: (command.input && typeof command.input === 'object')
      ? deepSortKeys(command.input)
      : {},
    p_protocol_event_payload_hash: computePayloadHash((command.input || {}) as unknown as Record<string, unknown>) || sha256(JSON.stringify({})),
