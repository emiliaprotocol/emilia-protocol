...
export interface VerifyHandshakeResult {
  /** The verified handshake UUID */
  handshake_id: string;
  /** Verification outcome */
  outcome: 'accepted' | 'rejected' | 'partial' | 'expired';
  /** Array of reason codes explaining the outcome */
  reason_codes: string[];
  /** Minimum assurance level achieved across all parties (omitted on early-rejection paths) */
  assurance_achieved?: string | null;
  /** Policy version used for verification (omitted on early-rejection paths) */
  policy_version?: string | null;
  /** When the binding was consumed (present on already-consumed rejections) */
  consumed_at?: string | null;
  /** Binding expiry (present on binding_expired rejections) */
  expires_at?: string | null;
  /** Authoritative DB clock at rejection (present on binding_expired rejections) */
  server_now?: string | null;
}

type VerifyRecord = Record<string, any>;
type VerifyRecords = Array<VerifyRecord>;

interface VerifyBindingRecord {
...
export async function _handleVerifyHandshake(command: {
  ...
  const parties: VerifyRecords = Array.isArray(ctx.parties) ? ctx.parties : [];
  const presentations: VerifyRecords = Array.isArray(ctx.presentations) ? ctx.presentations : [];
  const binding: VerifyBindingRecord | null = ctx.binding || null;
  ...
  let selfAssertedPolicyRoles: Record<string, boolean> | null = null;
    ...
    if (pres.verified === false) {
      ...
      if (pres.issuer_status === 'self_asserted' && handshake.policy_id) {
        if (selfAssertedPolicyRoles === null) {
          try {
            const pol = await resolvePolicy(supabase, { policy_id: handshake.policy_id });
            selfAssertedPolicyRoles = {};
            const reqParties: Record<string, any> = pol?.rules?.required_parties || {};
  ...
  // Determine outcome
  let outcome: 'accepted' | 'rejected' | 'partial' | 'expired';
  ...
  // Assurance achieved
  let assurance_achieved: string | null = null;
