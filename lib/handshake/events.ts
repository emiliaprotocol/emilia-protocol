 ...
 */
export const HANDSHAKE_EVENT_TYPES: string[] = [
  'initiated',
  'presentation_added',
  'status_changed',
  'verified',
...
const VALID_EVENT_TYPES: Set<string> = new Set(HANDSHAKE_EVENT_TYPES);
...
export class HandshakeEventError extends Error {
  ...
  constructor(message: string, code: string = 'HANDSHAKE_EVENT_ERROR') {
    super(message);
 ...
 * @returns {string}
 */
export function generateIdempotencyKey(handshake_id: string, event_type: string, discriminator: string = ''): string {
 ...
 *
 * @param {any} supabase - Supabase service client
 * @param {object} params
 ...
 */
export async function recordHandshakeEvent(supabase: SupabaseClient, {
  handshake_id,
  event_type,
  event_payload = {},
  actor_id = 'system',
 ...
 * @throws {HandshakeEventError} On validation failure or DB error
 */
export async function getHandshakeEvents(supabase: SupabaseClient, handshake_id: string): Promise<any[]> {
  if (!handshake_id) {
    throw new HandshakeEventError('handshake_id is required', 'MISSING_HANDSHAKE_ID');
  }
