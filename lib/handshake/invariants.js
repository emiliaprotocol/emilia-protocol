...
/** @typedef {Record<string, any>} InvariantRecord */
/** @typedef {{ ok: boolean, code: string, message: string }} InvariantResult */
...
/** @param {string} code @returns {InvariantResult} */
function pass(code) {
  return { ok: true, code, message: 'ok' };
}

/** @param {string} code @param {string} message @returns {InvariantResult} */
...
export function checkAssuranceLevel(achievedLevel, requiredLevel, assuranceRank) {
  ...
  const rank = /** @type {Record<string, number>} */ (assuranceRank || ASSURANCE_RANK);
...
export function runAllInvariants(context) {
  ...
  const handshakeWithBinding = /** @type {{ binding?: { expires_at: string } }} */ ({ ...handshake, binding });
  ...
  results.push(
    checkAllPartiesPresent(
      handshake,
      /** @type {Array<{ party_role: string }>} */ (parties),
      /** @type {Array<{ party_role: string }>} */ (presentations),
      policy,
    ),
  );
  ...
  results.push(
    checkBindingValid(
      /** @type {{ payload_hash?: string, nonce?: string }|null} */ (binding),
  ...
  for (const pres of presentations) {
    if (pres.issuer_ref) {
      results.push(
        checkIssuerTrusted(
          pres,
          /** @type {Array<{ key_id: string, status?: string }>} */ (authorities),
        ),
      );
    }
  }
  ...
  results.push(
    checkResultImmutability(
      /** @type {{ outcome?: string }|null} */ (existingResult),
    ),
  );
