// Generated from binding.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
/**
...
export function validateBindingCompleteness(binding) {
    ...
    if (binding.binding_material_version !== BINDING_MATERIAL_VERSION) {
        throw new Error(`BINDING_INVARIANT_VIOLATION: binding_material_version mismatch. ` +
    ...
    const visit = (current) => {
        ...
        if (typeof current === 'number') {
            if (!Number.isFinite(current))
                return invalid(`non-finite number ${current}`);
            return current;
        }
        ...
        if (ancestors.has(current))
            return invalid('cyclic reference');
        ancestors.add(current);
        try {
            if (Array.isArray(current)) {
                if (Object.getPrototypeOf(current) !== Array.prototype)
                    return invalid('unsafe array prototype');
                ...
                return Array.from({ length: current.length }, (_, index) => {
                    ...
                    if (!descriptor || !('value' in descriptor))
                        return invalid('array accessors are not permitted');
                    return visit(descriptor.value);
                });
            }
            const prototype = Object.getPrototypeOf(current);
            if (prototype !== Object.prototype && prototype !== null)
                return invalid('only plain objects are permitted');
            const entries = [];
            const normalizedKeys = new Set();
            for (const key of Reflect.ownKeys(current)) {
                if (typeof key !== 'string')
                    return invalid('symbol members are not permitted');
                ...
                if (normalizedKeys.has(normalizedKey))
                    return invalid('member names collide after NFC normalization');
