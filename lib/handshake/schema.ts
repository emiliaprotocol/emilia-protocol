 ...
 *
 * VALID_MODES is imported from invariants.js — single source of truth.
 *
 * @license Apache-2.0
 */

import { VALID_MODES, VALID_PARTY_ROLES as _ROLES, VALID_DISCLOSURE_MODES as _DISC } from './invariants.js';

// Re-use sets from invariants — never redefine them here.
const VALID_PARTY_ROLES = _ROLES;
const VALID_DISCLOSURE_MODES = _DISC;
 ...
 */
export function validateInitiateBody(
  body: any,
): { valid: false; error: string; sanitized?: undefined } | { valid: true; sanitized: any } {
  ...
  if (!body.policy_id || typeof body.policy_id !== 'string') {
    return {
      valid: false,
      error: 'policy_id is required and must be a string',
    };
  }
  if (!Array.isArray(body.parties) || body.parties.length < 2) {
    return {
      valid: false,
      error: 'parties is required and must contain at least 2 entries',
    };
  }
  ...
  for (let i = 0; i < body.parties.length; i++) {
    ...
    if (!p.role || !VALID_PARTY_ROLES.has(p.role)) {
      return {
        valid: false,
        error: `parties[${i}].role must be one of: ${[
          ...VALID_PARTY_ROLES,
        ].join(', ')}`,
      };
    }
    if (!p.entity_ref || typeof p.entity_ref !== 'string') {
      return {
        valid: false,
        error: `parties[${i}].entity_ref is required and must be a string`,
      };
    }
  }
  ...
  if (body.resource_ref && typeof body.resource_ref !== 'string') {
    return {
      valid: false,
      error: 'resource_ref must be a string',
    };
  }
  if (body.intent_ref && typeof body.intent_ref !== 'string') {
    return {
      valid: false,
      error: 'intent_ref must be a string',
    };
  }
  if (
    body.binding_ttl_ms !== undefined &&
    (typeof body.binding_ttl_ms !== 'number' || body.binding_ttl_ms <= 0)
  ) {
    return {
      valid: false,
      error: 'binding_ttl_ms must be a positive number',
    };
  }
 ...
 */
export function validatePresentBody(
  body: any,
): { valid: false; error: string; sanitized?: undefined } | { valid: true; sanitized: any } {
  if (!body || typeof body !== 'object') {
    return { valid: false, error: 'Request body must be a JSON object' };
  ...
  if (!body.party_role || !VALID_PARTY_ROLES.has(body.party_role)) {
    return {
      valid: false,
      error: `party_role must be one of: ${[...VALID_PARTY_ROLES].join(', ')}`,
    };
  }
  if (
    !body.presentation_type ||
    !VALID_PRESENTATION_TYPES.has(body.presentation_type)
  ) {
    return {
      valid: false,
      error: `presentation_type must be one of: ${[
  ...
  if (
    ...
    return {
      valid: false,
      error: 'claims is required and must be a non-array object',
    };
  }
  if (
    body.disclosure_mode &&
    !VALID_DISCLOSURE_MODES.has(body.disclosure_mode)
  ) {
    return {
      valid: false,
      error: `disclosure_mode must be one of: ${[
