...
// Redeclared locally rather than imported: importing the ActorRef type
// alongside resolveActorRef as a value from the same '@/lib/actor' specifier
...
export interface HandshakeConsumptionRecord {
  /** Auto-generated consumption record ID */
  id: string;
  /** The consumed handshake UUID */
  handshake_id: string;
  /** Binding hash verified at consumption time */
  binding_hash: string;
  /** Type of consuming artifact (e.g. 'commit_issue') */
  consumed_by_type: string;
  /** ID of the consuming artifact */
  consumed_by_id: string;
  /** Entity reference of the consuming actor */
  actor_entity_ref: string;
  /** Action label for the consumption */
  consumed_by_action: string | null;
  /** ISO-8601 creation timestamp */
  created_at: string;
  [key: string]: unknown;
}

export interface ConsumeHandshakeArgs {
  /** The handshake to consume */
  handshake_id: string;
  /** Binding hash for integrity check */
  binding_hash: string;
  /** Type of consuming artifact (e.g., 'commit_issue') */
  consumed_by_type: string;
  /** ID of the consuming artifact */
  consumed_by_id: string;
  /** Authenticated actor performing consumption */
  actor: ActorRef;
  /** WHAT action the consumption is for (LOCK 100 B.1) */
  consumed_by_action?: string | null;
}
 ...
 *
 * @param opts
 * @returns The created consumption record
...
export async function consumeHandshake({
...
}: ConsumeHandshakeArgs): Promise<HandshakeConsumptionRecord> {
  ...
  // Audit-fix (HIGH): the binding-mark is now inside consume_handshake_atomic
  // (migration 085). The JS-side UPDATE used to run in a separate transaction,
  // which meant a JS-side failure after RPC success would permanently diverge
  // handshake_consumptions (populated) from handshake_bindings.consumed_at
  // (still NULL) — weakening verify.js's HARD GATE. With the UPDATE inside
  // the RPC, consumption row insert and binding.consumed_at are atomic.
 ...
 *
 * @param handshake_id - The handshake UUID to check
 * @returns True if a consumption record exists for this handshake
 */
export async function isHandshakeConsumed(handshake_id: string): Promise<boolean> {
  const supabase = getServiceClient();
