/// <reference path="./_external-modules.d.ts" />
/**
 ...
 * @param {string|object|null} [actor=null]
 */
export async function getHandshake(handshakeId: string, actor: ActorRef = null): Promise<Record<string, any> | null> {
  if (!handshakeId) {
  ...
  const handshakeRow: Record<string, any> = handshake;

  // Scope read access to party members (Finding 11)
  if (actor && actor !== 'system') {
    const actorId = resolveActorRef(actor);
 ...
 * @param {HandshakeFilter} [filters={}]
 * @param {string|object|null} [actor=null]
 */
export async function listHandshakes(filters: HandshakeFilter = {}, actor: ActorRef = null): Promise<{ handshakes: any[] }> {
  const supabase = getServiceClient();

  // Scope reads to actor membership (Finding 11)
  const actorId = actor
    ? resolveActorRef(actor)
    : null;
  ...
  if (filters.entity_ref) {
    ...
    const ids = (partyRows || []).map((r: { handshake_id: string }) => r.handshake_id);
