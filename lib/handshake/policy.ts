...
const VALID_ASSURANCE_VALUES: Set<string> = new Set(['low', 'medium', 'substantial', 'high']);
 ...
 */
export function validatePolicyRules(rules: Record<string, any>): { valid: boolean; errors: string[] } {
  const errors: string[] = [];
 ...
 * @param {any} supabase
 * @param {{ policy_key?: string, policy_version?: number|string|null, policy_id?: string }} identifiers
 */
 ...
 */
export function getRequiredPartiesForMode(policy: Record<string, any> | null): string[] {
 ...
 */
export function checkClaimsAgainstPolicy(
  normalizedClaims: Record<string, unknown>,
  policyRequirements: { required_claims?: string[]; minimum_assurance?: string } | null | undefined,
): { satisfied: boolean; missing: string[] } {
  if (!policyRequirements || !Array.isArray(policyRequirements.required_claims)) {
    return { satisfied: true, missing: [] };
  }

  const claims: Record<string, unknown> = normalizedClaims || {};
