...
export async function _handleInitiateHandshake(command) {
  ...
  // NOTE: initiator_entity_ref is intentionally NOT included in CANONICAL_BINDING_FIELDS
  // or the binding_hash computation. The initiator is already captured within party_set_hash
  // (which hashes all role:entity_ref pairs including initiator). Adding it to the canonical
  // fields would change binding hashes for all existing handshakes (breaking change).
  // This field exists purely for audit/query convenience.
  const initiatorParty = parties.find((p) => p.role === 'initiator');
