...
export async function _handleInitiateHandshake(command) {
  ...
  const { data: rpcResult, error: rpcError } = await supabase.rpc('create_handshake_atomic', {
    ...
    // Audit-fix (H7): use computePayloadHash, which uses deepSortKeys. The
    // previous JSON.stringify(obj, Object.keys(obj).sort()) silently dropped
    // nested keys, making the protocol_events audit log forgeable in the
    // nested-metadata dimension.
