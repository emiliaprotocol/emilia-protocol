...
/**
 * @typedef {'basic'|'mutual'|'selective'|'delegated'} HandshakeMode
 * @typedef {'initiator'|'responder'|'verifier'|'delegate'} HandshakePartyRole
 *
 * @typedef {Object} HandshakeParty
 * @property {HandshakePartyRole} role - Party role.
 * @property {string} entity_ref - Entity reference identifier
 * @property {'low'|'medium'|'substantial'|'high'} [assurance_level] - Required assurance level.
 * @property {object|string|null} [delegation_chain] - Delegation chain for delegated mode.
 *
 * @typedef {Object} HandshakeBindingOverride
 * @property {string|Date} [expires_at]
 * @property {string} [session_ref]
 *
 * @typedef {Object} InitiateCommandInput
 * @property {HandshakeMode} mode
 * @property {string} policy_id
 * @property {string|null} policy_version
 * @property {string|null} interaction_id
 * @property {HandshakeParty[]} parties
 * @property {string} payload_hash
 * @property {string} nonce
 * @property {string} expires_at
 * @property {Record<string, unknown>} metadata
 * @property {HandshakeBindingOverride|null} binding
 * @property {string|null} idempotency_key
 * @property {string|null} action_type
 * @property {string|null} resource_ref
 * @property {string|null} intent_ref
 ...
 * @property {number|null} [policy_version_number] - Pinned integer policy version row
 * @property {string|null} [action_hash] - Hash of the bound action, if any
 * @property {string|null} [policy_hash] - Cryptographic snapshot of policy rules at initiation
...
export async function initiateHandshake({
  ...
  for (const party of parties) {
    ...
    const assuranceRank = /** @type {Record<string, number>} */ (ASSURANCE_RANK);
  ...
  const initiatorParty = /** @type {HandshakeParty} */ (parties.find((p) => p.role === 'initiator'));
  ...
  if (mode === 'delegated') {
    ...
    const delegateParty = /** @type {HandshakeParty} */ (parties.find((p) => p.role === 'delegate'));
...
export async function _handleInitiateHandshake(command) {
  ...
  if (policy_id) {
    ...
    let policy;
    try {
      policy = await resolvePolicy(supabase, /** @type {any} */ ({ policy_id }));
    } catch (err) {
      throw new HandshakeError(
