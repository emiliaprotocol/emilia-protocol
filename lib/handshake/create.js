...
export async function initiateHandshake({
  ...
  const actorEntityId = typeof actor === 'object'
    ? (actor.entity_id || actor.id || actor)
    : actor;
...
export async function _handleInitiateHandshake(command) {
  ...
  if (policy_id) {
    try {
      ...
      if (policy && policy.rules) {
        policy_hash = sha256(JSON.stringify(policy.rules, Object.keys(policy.rules).sort()));
      }
    } catch {
  ...
  const sortedPartyRefs = parties.map(p => `${p.role}:${p.entity_ref}`).sort();
  const party_set_hash = sha256(JSON.stringify(sortedPartyRefs));
  ...
  const context_hash = sha256(JSON.stringify(contextMaterial, Object.keys(contextMaterial).sort()));
  ...
  // ENFORCEMENT: Assert all canonical fields are present
  const materialKeys = Object.keys(bindingMaterial);
  const missingFields = CANONICAL_BINDING_FIELDS.filter(f => !materialKeys.includes(f));
  if (missingFields.length > 0) {
    throw new Error(`BINDING_INVARIANT_VIOLATION: Missing canonical binding fields: ${missingFields.join(', ')}`);
  }

  // Also assert no extra fields leaked in (envelope must be exact)
  const extraFields = materialKeys.filter(f => !CANONICAL_BINDING_FIELDS.includes(f));
  if (extraFields.length > 0) {
    throw new Error(`BINDING_INVARIANT_VIOLATION: Unexpected fields in binding material: ${extraFields.join(', ')}`);
  }

  const binding_hash = sha256(JSON.stringify(bindingMaterial, Object.keys(bindingMaterial).sort()));
  ...
  // If this fails, the operation must fail. An unlogged initiation is not allowed.
