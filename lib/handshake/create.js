...
export async function _handleInitiateHandshake(command) {
  ...
  // Compute policy_hash: cryptographic snapshot of policy rules at initiation time.
  // If the policy is later modified, verification will detect the mismatch.
  ...
  const handshakeRecord = {
    mode, policy_id, policy_id_legacy: String(policy_id), policy_version, interaction_id,
    action_type: action_type || null,
    resource_ref: resource_ref || null,
    intent_ref: intent_ref || null,
  ...
  // Delegated to binding.js — the single source of truth for binding computation.
  ...
  // LOCK 100: Store initiator_entity_ref on the binding record for audit.
  const initiatorParty = parties.find((p) => p.role === 'initiator');
  ...
  // Parallelize: parties, binding, and event inserts are independent after handshake_id is known.
  // This cuts 3 serial roundtrips down to 1 parallel batch.
  const { requireHandshakeEvent } = await import('./events.js');

  const [partiesResult, bindingResult, eventResult] = await Promise.allSettled([
    supabase.from('handshake_parties').insert(partyRecords),
    supabase.from('handshake_bindings').insert(bindingRecord),
    requireHandshakeEvent({
      handshake_id,
      event_type: 'handshake_created',
      actor: command.actor,
      detail: { mode, policy_id, party_count: parties.length, action_type: action_type || null },
    }),
  ]);

  // Check for failures in the parallel batch
  if (partiesResult.status === 'rejected' || partiesResult.value?.error) {
    const msg = partiesResult.reason?.message || partiesResult.value?.error?.message || 'unknown';
    throw new HandshakeError(`Failed to create handshake parties: ${msg}`, 500, 'DB_ERROR');
  }
  if (bindingResult.status === 'rejected' || bindingResult.value?.error) {
    const msg = bindingResult.reason?.message || bindingResult.value?.error?.message || 'unknown';
    throw new HandshakeError(`Failed to create handshake binding: ${msg}`, 500, 'DB_ERROR');
  }
  if (eventResult.status === 'rejected') {
    throw eventResult.reason; // requireHandshakeEvent throws its own typed errors
  }
