...
import { computePolicyHash, hashBinding, canonicalizeBinding } from './binding.js';
 ...
 * @param {string|null} [options.payload_hash=null] - Expected payload hash for binding integrity
 ...
 *
 * @param {{ actor: string|object, input: { handshake_id: string, payload_hash?: string|null, nonce?: string|null, action_hash?: string|null, policy_hash?: string|null } }} command
...
export async function _handleVerifyHandshake(command) {
  ...
  const resultRecord = {
    ...
    reason_codes,
    assurance_achieved,
    // Use the full binding_hash (canonical binding material hash), not payload_hash.
    // binding_hash is the integrity fingerprint used by checkNoDuplicateResult (invariant 7).
  ...
  const eventType = outcome === 'accepted' ? 'verified' : outcome === 'expired' ? 'expired' : 'rejected';
  ...
  // Update handshake status (AFTER event is durably recorded)
  ...
  // Update party verified_status (parallel — independent per party)
  ...
  if (outcome === 'accepted' && binding) {
    await supabase
      ...
      .update({
        ...
        consumed_by: resolveActorRef(command.actor),
