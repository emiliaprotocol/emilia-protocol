 ...
 * @property {string|null} [assurance_achieved] - Minimum assurance level achieved across all parties (omitted on early-rejection paths)
 * @property {string} [policy_version] - Policy version used for verification (omitted on early-rejection paths)
 * @property {string|null} [consumed_at] - When the binding was consumed (present on already-consumed rejections)
 * @property {string|null} [expires_at] - Binding expiry (present on binding_expired rejections)
 * @property {string|null} [server_now] - Authoritative DB clock at rejection (present on binding_expired rejections)
 */

/** @typedef {Record<string, any>} VerifyRecord */
/** @typedef {Array<VerifyRecord>} VerifyRecords */
 ...
 * @returns {Promise<{ result: VerifyHandshakeResult, aggregateId: string, _protocolEventWritten?: boolean }>}
 * @throws {HandshakeError} DB_ERROR on database failures
...
export async function _handleVerifyHandshake(command) {
  ...
  //
  // Audit-fix (silent-failure-hunter CRITICAL-1): previously this call did
  // `const { data } = await ...` without inspecting `error`. A transient DB
  // failure returned `{ data: null, error: <failure> }`, and the optional-chain
  // `existingBinding?.consumed_at` silently evaluated falsy, letting the flow
  // fall through to the full verify pipeline. The comment claimed "reject
  // already-consumed bindings before any processing," but a DB flap turned
  // this gate into a no-op. Now we surface the error as DB_ERROR.
  ...
  if (existingBindingError) {
    throw new HandshakeError(
      `Failed to load binding (HARD GATE): ${existingBindingError.message}`,
      500, 'DB_ERROR',
    );
  }
  ...
  // Audit-fix (H8): use load_verify_context RPC (migration 082) to load the
  // handshake row + parties + presentations + binding in a single snapshot-
  // consistent read. The previous four-parallel-queries approach could see
  // mutually-inconsistent state under concurrent writes (late presentation
  // arrives between reads, binding marked consumed, party status flipped),
  // producing reason_codes that reflect a state that never actually existed.
  //
  // The RPC raises P0002 HANDSHAKE_NOT_FOUND if the handshake is missing.
  // Audit-fix (C4): we also now surface DB errors explicitly rather than
  // defaulting to empty arrays — an empty parties set previously meant
  // "no required roles" → verification could emerge as 'accepted'.
  const { data: ctx, error: ctxError } = await supabase.rpc('load_verify_context', {
    p_handshake_id: handshake_id,
  });

  if (ctxError) {
    if (ctxError.code === 'P0002' || ctxError.message?.includes('HANDSHAKE_NOT_FOUND')) {
      throw new HandshakeError('Handshake not found', 404, 'NOT_FOUND');
    }
    throw new HandshakeError(`Failed to load verify context: ${ctxError.message}`, 500, 'DB_ERROR');
  }
  ...
  const parties = /** @type {VerifyRecords} */ (Array.isArray(ctx.parties) ? ctx.parties : []);
  const presentations = /** @type {VerifyRecords} */ (Array.isArray(ctx.presentations) ? ctx.presentations : []);
  const binding = /** @type {{ expires_at: string|number|Date, consumed_at?: string|number|Date|null, nonce?: string|null, payload_hash?: string|null, binding_hash?: string|null }|null} */ (ctx.binding || null);
  ...
  //
  // Audit-fix (HIGH — H4 completion): the prior H4 fix (present.js) set
  // `verified = false` + `issuer_status = 'self_asserted'` for issuer-less
  // presentations. This caused every policy that previously relied on
  // self-asserted identity-only flows to fail verification with no opt-out.
  //
  // Policies can now explicitly opt in per-role via:
  //     rules.required_parties.<role>.allow_self_asserted = true
  //
  // With opt-in, a self-asserted presentation does NOT trigger
  // unverified_presentation_<role>. Without opt-in, it still fails (the
  // conservative default from H4). Policies that need self-asserted
  // initiators must now declare it explicitly — which is the whole point.
  //
  // We defer the policy lookup to keep the pattern cheap: look up the policy
  // ONCE here if needed, but Check 6 below also resolves it. To avoid a
  // double-fetch, we read self-asserted-policy config only if we find a
  // self-asserted presentation in the loop.
  /** @type {Record<string, boolean>|null} */
  let selfAssertedPolicyRoles = null;
  for (const pres of presentations) {
    if (pres.revocation_status === 'revoked') {
      reason_codes.push(`issuer_revoked_${pres.party_role}`);
    }
    if (pres.verified === false) {
      // If this is a self-asserted presentation, check the per-role opt-in.
      if (pres.issuer_status === 'self_asserted' && handshake.policy_id) {
        if (selfAssertedPolicyRoles === null) {
          try {
            const pol = await resolvePolicy(supabase, /** @type {any} */ ({ policy_id: handshake.policy_id }));
            selfAssertedPolicyRoles = {};
            const reqParties = /** @type {Record<string, any>} */ (pol?.rules?.required_parties || {});
            for (const [role, def] of Object.entries(reqParties)) {
              selfAssertedPolicyRoles[role] = def?.allow_self_asserted === true;
            }
          } catch {
            // Policy resolution error is surfaced by Check 6; treat as "not allowed" here.
            selfAssertedPolicyRoles = {};
          }
        }
        if (selfAssertedPolicyRoles[pres.party_role] === true) {
          // Policy explicitly allows self-asserted for this role. Skip the reject.
          continue;
        }
      }
  ...
  if (handshake.policy_id) {
    try {
      policy = await resolvePolicy(supabase, /** @type {any} */ ({ policy_id: handshake.policy_id }));
    } catch (policyErr) {
      // Audit-fix (silent-failure-hunter HIGH-2): previously this catch
      // swallowed the error with no logging, making post-mortem of a stuck
      // handshake impossible. Log with handshake_id + policy_id + err so ops
      // can distinguish "policy genuinely missing" from "Postgres had a blip."
      logger.error(
        {
          err: policyErr,
          handshake_id,
          policy_id: handshake.policy_id,
        },
        'Policy resolution failed during handshake verify',
      );
  ...
  // Determine outcome
  /** @type {'accepted'|'rejected'|'partial'|'expired'} */
  let outcome;
  ...
  if (presentations.length > 0) {
    ...
    if (verifiedPres.length === presentations.length && parties.length > 0) {
      const assuranceLevels = parties
        ...
        .map((p) => (/** @type {Record<string, number>} */ (ASSURANCE_RANK))[p.assurance_level] || 0);
  ...
  if (writeError) {
    // Audit-fix (H3): migration 081 raises P0004 INVALID_STATE_TRANSITION if
    // the handshake was finalized or terminal (revoked/expired) before we
    // got the lock. Surface this as a structured 409 rather than a generic
    // DB_ERROR so upstream code can distinguish "lost the race" from
    // "something else broke."
    if (writeError.code === 'P0004' || writeError.message?.includes('INVALID_STATE_TRANSITION')) {
      throw new HandshakeError(
        'Handshake is already in a terminal state and cannot be re-verified',
        409, 'INVALID_STATE_FOR_VERIFY',
      );
    }
    if (writeError.code === 'P0002' || writeError.message?.includes('HANDSHAKE_NOT_FOUND')) {
      throw new HandshakeError('Handshake not found', 404, 'HANDSHAKE_NOT_FOUND');
    }
