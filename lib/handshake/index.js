...
/** @typedef {{ entity_ref?: string, status?: string, mode?: string }} HandshakeFilter */
...
export async function getHandshake(handshakeId, actor = null) {
  ...
  const handshakeRow = /** @type {Record<string, any>} */ (handshake);
...
export async function listHandshakes(filters = {}, actor = null) {
  ...
  if (filters.entity_ref) {
    ...
    const ids = /** @type {Array<{handshake_id: string}>} */ (partyRows || [])
      .map((r) => r.handshake_id);
