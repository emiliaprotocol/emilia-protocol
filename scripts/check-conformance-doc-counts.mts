...
const suiteFiles: string[] = [...LIVE_SUITE_FILES];
if (suiteFiles.length === 0) {
  throw new Error('conformance/suites.mjs LIVE_SUITE_FILES is empty');
}

const tableLabels: Map<string, string> = new Map([
...
const counts: SuiteCount[] = [];
for (const suiteFile of suiteFiles) {
  const suitePath: string = resolve(root, 'conformance/vectors', suiteFile);
  const suite: SuiteVector = JSON.parse(readFileSync(suitePath, 'utf8'));
...
const total: number = counts.reduce((sum: number, s: SuiteCount) => sum + s.count, 0);
const summary: string = `${total} vectors · ${suiteFiles.length} suites`;
const failures: string[] = [];

const readDoc = (path: string): string => readFileSync(resolve(root, path), 'utf8');
...
const conformance: string = docs['CONFORMANCE.md'];
const esc = (s: string): string => s.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
for (const { suiteFile, count } of counts) {
  const label: string | undefined = tableLabels.get(suiteFile);
  if (!label) {
    failures.push(`no CONFORMANCE.md table label registered for ${suiteFile}`);
    continue;
  }
  const re: RegExp = new RegExp(`${esc(label)}\\s+—\\s+${String(count).padStart(1)}\\s+vectors`);
