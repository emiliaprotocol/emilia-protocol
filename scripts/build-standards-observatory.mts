...
const write: boolean = args.has('--write');
const check: boolean = args.has('--check');
if (write === check) {
...
const PRIVATE_RECON_INDEX: string = 'docs/strategy-private/observatory/recon-index.v1.json';
...
function readJson(relative: string): Record<string, any> {
  try {
    return JSON.parse(read(relative)) as Record<string, any>;
  } catch (error) {
    throw new Error(`${relative}: ${(error as any).message}`);
  }
}
...
function assertUnique(values: any[], label: string): void {
  assert(values.length === new Set(values).size, `${label} contains duplicates`);
}

function stableJson(value: Record<string, any>): string {
...
const catalog: Record<string, any> = readJson(INPUT_PATHS.catalog);
const sourceLock: Record<string, any> = readJson(INPUT_PATHS.sourceLock);
const recon: Record<string, any> = readJson(INPUT_PATHS.recon);
...
const dimensionIds: any[] = catalog.dimensions.map((item: any) => item.id);
const sourceIds: any[] = catalog.sources.map((item: any) => item.id);
const eventIds: any[] = catalog.events.map((item: any) => item.id);
const frontierIds: any[] = catalog.frontiers.map((item: any) => item.id);
...
const allowedValues: Set<string> = new Set(['yes', 'partial', 'no', 'unknown']);
...
const locksById: Map<any, Record<string, any>> = new Map(sourceLock.sources.map((item: any) => [item.id, item]));
for (const source of catalog.sources) {
  const lock: any = locksById.get(source.id);
  assert(lock as any, `${source.id}: no source lock`);
  assert((lock as any).revision === source.revision, `${source.id}: source revision differs from lock`);
  assert((lock as any).source_url === source.source_url, `${source.id}: source URL differs from lock`);
  assert((lock as any).quote?.text === source.quote.text, `${source.id}: quote differs from lock`);
  assert((lock as any).quote_verified === true, `${source.id}: quote is not verified`);
  assert(/^[0-9a-f]{64}$/.test((lock as any).sha256), `${source.id}: invalid source digest`);
}
...
const serializedCatalog: string = JSON.stringify(catalog);
...
const sourceLockPublic: Map<any, Record<string, any>> = new Map(sourceLock.sources.map((item: any) => [item.id, {
...
;
const curatedSources: Array<Record<string, any>> = catalog.sources.map((source: any) => ({ ...source, evidence_lock: sourceLockPublic.get(source.id) }));
...
const snapshot: Record<string, any> = { ...core, snapshot_sha256: sha256(Buffer.from(JSON.stringify(core), 'utf8')) };
const snapshotBody: string = stableJson(snapshot);

function renderLlmsText(): string {
  const lines: string[] = [
...
const llmsBody: string = renderLlmsText();
const outputBodies: Map<string, string> = new Map([
  [OUTPUT_PATHS.snapshot, snapshotBody],
  [OUTPUT_PATHS.publicJson, snapshotBody],
  [OUTPUT_PATHS.llmsText, llmsBody],
]);

const generatedFrom: Array<Record<string, any>> = Object.values(INPUT_PATHS).map((relative) => {
  const bytes: Buffer = fs.readFileSync(absolute(relative));
...
const manifest: Record<string, any> = {
  ...
  outputs: [...outputBodies].map(([relative, body]: [string, string]) => ({ path: relative, bytes: Buffer.byteLength(body), sha256: sha256(Buffer.from(body)) })),
};
