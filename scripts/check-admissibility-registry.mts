...
const VECTORS: string = path.join(ROOT, 'conformance', 'vectors');
const REGISTRY: string = path.join(ROOT, 'admissibility', 'registry.json');

const errors: string[] = [];
const fail = (claimId: string, msg: string): number => errors.push(`[${claimId}] ${msg}`);

function nonEmptyString(v: any): boolean {
  return typeof v === 'string' && v.trim().length > 0;
}

function suiteHasRefusalCase(suiteFile: string): { ok: boolean; reason?: string; refusals?: number } {
  const p: string = path.join(VECTORS, suiteFile);
  if (!fs.existsSync(p)) return { ok: false, reason: `suite not found: conformance/vectors/${suiteFile}` };
  let json: Record<string, any>;
  try {
    json = JSON.parse(fs.readFileSync(p, 'utf8')) as Record<string, any>;
  } catch (e) {
    return { ok: false, reason: `suite unreadable (${(e as any).message})` };
  }
  const vectors: any[] = Array.isArray(json?.vectors) ? json.vectors : [];
  const refusals: number = vectors.filter((v: any) => v?.expect?.valid === false).length;
  if (refusals === 0) {
...
try {
  registry = JSON.parse(fs.readFileSync(REGISTRY, 'utf8')) as Record<string, any>;
} catch (e) {
  console.error(`FATAL: cannot read ${REGISTRY}: ${(e as any).message}`);
  process.exit(2);
}

const DOCTRINE: string[] = ['signed_is_not_trusted', 'verified_is_not_accepted', 'accepted_requires_pinned_policy'];
...
const langs: any[] = Array.isArray(registry.conformance_langs) ? registry.conformance_langs : [];
if (langs.length === 0) fail('registry', 'conformance_langs must be non-empty');
const claims: any[] = Array.isArray(registry.claims) ? registry.claims : [];
if (claims.length === 0) fail('registry', 'no claims registered');

const seen: Set<string> = new Set();
for (const c of claims) {
  const id: string = c?.claim_id || '(missing claim_id)';
  if (seen.has(id)) fail(id, 'duplicate claim_id');
  seen.add(id);

  if (!nonEmptyString(c.claim)) fail(id, 'missing claim text');

  // 1. pinned authority
  if (!nonEmptyString(c.pinned_authority)) fail(id, 'invariant 1: pinned_authority missing');
  ...
  if (nv.suite) {
    ...
    if (!r.ok) fail(id, `invariant 3: ${r.reason}`);
  } else if (nv.test) {
    if (!fs.existsSync(path.join(ROOT, nv.test))) fail(id, `invariant 3: negative test not found: ${nv.test}`);
  } else {
  ...
  if (parity.length === 0) {
    ...
    if (stray.length) fail(id, `invariant 4: parity lists unknown language(s): ${stray.join(', ')}`);
  ...
  if (!nonEmptyString(c.acceptance_semantics)) fail(id, 'invariant 5: acceptance_semantics missing');
}
