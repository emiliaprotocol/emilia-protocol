...
try {
  ...
  try {
    ...
    const m2 = ((e2 as any).stdout || '').match(/Tests\s+(\d+)\s+passed/);
...
const mcpSrc: string = readFileSync(join(root, 'mcp-server/index.js'), 'utf8');
const mcpTools: number = (mcpSrc.match(/    name: 'ep_/g) || []).length;

const scoringSrc: string = readFileSync(join(root, 'lib/scoring-v2.ts'), 'utf8');
const pm: RegExpMatchArray | null = scoringSrc.match(/export const TRUST_POLICIES(?::[^=]*)?\s*=\s*\{/);
