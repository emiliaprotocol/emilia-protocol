#!/usr/bin/env node
// Generated from reconcile-load-test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
/**
...
async function check1_handshakeVsCreatedEvents() {
    ...
    record('1. Handshakes vs handshake_created events', pass, `handshakes=${handshakes}, handshake_created events=${events}`);
}
async function check2_bindingsVsHandshakes() {
    ...
    record('2. Handshake bindings vs handshakes (1:1)', pass, `bindings=${bindings}, handshakes=${handshakes}`);
}
async function check3_partiesVsHandshakes() {
    ...
    if (hsErr)
        throw new Error(`check3: ${hsErr.message}`);
    ...
    if (ids.length > 0) {
        ...
        for (let i = 0; i < ids.length; i += 500) {
            ...
            if (error)
                throw new Error(`check3 parties: ${error.message}`);
            partyCount += count ?? 0;
        }
    }
    const pass = partyCount >= expectedMin && (mutualCount === 0 || partyCount === expectedMutualParties + nonMutual);
    record('3. Handshake parties (2x for mutual)', pass, `parties=${partyCount}, mutual_hs=${mutualCount}, non_mutual=${nonMutual}, expected_min=${expectedMin}`);
}
async function check4_verifiedVsAcceptedResults() {
    ...
    record('4. Verified handshakes vs accepted results', pass, `verified_hs=${verified}, accepted_results=${accepted}`);
}
async function check5_challengesVsChallengeEvents() {
    ...
    record('5. Signoff challenges vs challenge_issued events', pass, `challenges=${challenges}, challenge_issued_events=${events}`);
}
async function check6_attestationsVsApprovedEvents() {
    ...
    record('6. Signoff attestations vs signoff_approved events', pass, `attestations=${attestations}, approved_events=${events}`);
}
async function check7_noDuplicateConsumptions() {
    ...
    if (error) {
        ...
        if (e1)
            throw new Error(`check7: ${e1.message}`);
        ...
        const { data: rows, error: e2 } = await supabase
            ...
            .gte('consumed_at', SINCE);
        if (e2)
            throw new Error(`check7 fetch: ${e2.message}`);
        const seen = new Set();
        const dupes = [];
        for (const r of rows || []) {
            if (seen.has(r.signoff_id))
                dupes.push(r.signoff_id);
            seen.add(r.signoff_id);
        }
        const pass = dupes.length === 0;
        record('7. No duplicate signoff consumptions', pass, pass
    ...
    record('7. No duplicate signoff consumptions', pass, pass ? `duplicates=0` : `VIOLATION: ${dupeCount} duplicate signoff_ids`);
}
async function check8_orphanedBindings() {
    ...
    const { data: orphans, error } = await supabase
        ...
        .gte('bound_at', SINCE);
    if (error)
        throw new Error(`check8: ${error.message}`);
    ...
    if (bindingHsIds.length > 0) {
        ...
        for (let i = 0; i < uniqueIds.length; i += 500) {
            ...
            const { data: found, error: fErr } = await supabase
                ...
                .in('handshake_id', chunk);
            if (fErr)
                throw new Error(`check8 lookup: ${fErr.message}`);
    ...
    record('8. No orphaned bindings', pass, pass
...
async function check9_partialTerminalStates() {
    ...
    const { data: verifiedHs, error: vErr } = await supabase
        ...
        .gte('created_at', SINCE);
    if (vErr)
        throw new Error(`check9: ${vErr.message}`);
    ...
    if (verifiedIds.length > 0) {
        for (let i = 0; i < verifiedIds.length; i += 500) {
            ...
            const { data: resultRows, error: rErr } = await supabase
                ...
                .in('handshake_id', chunk);
            if (rErr)
                throw new Error(`check9 results: ${rErr.message}`);
    ...
    record('9. No partial terminal states (verified without result)', pass, pass
...
async function check10_missingRequiredEvents() {
    ...
    for (const status of terminalStatuses) {
        const { data: hsRows, error: hsErr } = await supabase
            ...
            .gte('created_at', SINCE);
        if (hsErr)
            throw new Error(`check10 (${status}): ${hsErr.message}`);
        ...
        for (let i = 0; i < hsIds.length; i += 500) {
            ...
            const { data: eventRows, error: evErr } = await supabase
                ...
                .in('handshake_id', chunk);
            if (evErr)
                throw new Error(`check10 events (${status}): ${evErr.message}`);
    ...
    record('10. No missing required events for terminal states', pass, details.join(', '));
}
async function check11_protocolEventsCount() {
    ...
    const pass = count > 0;
    record('11. Protocol events count (sanity)', pass, `protocol_events=${count} (since ${SINCE})`);
}
// ---------------------------------------------------------------------------
// Main
// ---------------------------------------------------------------------------
