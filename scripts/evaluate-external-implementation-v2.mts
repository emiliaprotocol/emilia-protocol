...
export function evaluateExternalImplementationV2({
  manifestPath,
  sourcePath,
  runnerPath,
  attestationPath,
...
function cliOptions(argv: string[]): {
  ...
  for (const key of allowed) {
    if (!values.get(key)) {
      throw new Error(
        'usage: evaluate-external-implementation-v2 --manifest FILE --source CHECKOUT '
...
if (process.argv[1]
  ...
  try {
    ...
    console.log(
      `EXTERNAL CONFORMANCE V2: PASS (${report.conformance.suites} suites, `
