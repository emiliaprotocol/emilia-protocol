...
// Generated from verify-clean-room-submission-v2.mts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
...
const EXPECTED_MANIFEST_SHA256 = '17fabd094e0fe63d291b68de5f0190da150e74557995a342aff3f27994b163db';
const EXPECTED_MANIFEST_CLAIM_SHA256 = 'd1b045f9f4f7633330e0e029973df0d9fe6c37aaaf79f5c25563fa8248a25591';
...
function immutableOid(value, label) {
...
export function validateBundleDefinitionV2(bundle) {
    exactKeys(bundle, ['@version', 'claim_scope', 'source_manifest', 'runner_protocol', 'suites', 'totals'], ['@version', 'claim_scope', 'source_manifest', 'runner_protocol', 'suites', 'totals'], 'bundle');
    ...
    exactKeys(bundle.source_manifest, ['path', 'sha256', 'manifest_sha256'], ['path', 'sha256', 'manifest_sha256'], 'bundle.source_manifest');
    ...
    if (sha256String(bundle.source_manifest.manifest_sha256, 'bundle.source_manifest.manifest_sha256') !== EXPECTED_MANIFEST_CLAIM_SHA256) {
        throw new Error('bundle source conformance manifest claim hash is not pinned');
    }
    exactKeys(bundle.runner_protocol, ['version', 'invocation', 'result_shape', 'result_comparison', 'complete_suite_required'], ['version', 'invocation', 'result_shape', 'result_comparison', 'complete_suite_required'], 'bundle.runner_protocol');
    ...
    if (bundle.totals.suites !== 21 || bundle.totals.vectors !== 329) {
        throw new Error('bundle must pin exactly 21 suites and 329 vectors');
    }
    ...
    const paths = new Set();
    let vectorCount = 0;
    let authorityCompanion = null;
    for (const [index, suite] of bundle.suites.entries()) {
        const label = `bundle.suites[${index}]`;
        exactKeys(suite, ['path', 'sha256', 'vectors', 'result_contract', 'execution_path', 'execution_sha256'], ['path', 'sha256', 'vectors', 'result_contract'], label);
        const suitePath = safeRelativePath(suite.path, `${label}.path`);
        if (paths.has(suitePath))
            throw new Error(`bundle contains duplicate suite path ${suitePath}`);
        paths.add(suitePath);
    ...
    if (vectorCount !== 329 || vectorCount !== bundle.totals.vectors) {
        throw new Error('bundle suite total is not exactly 329 vectors');
    }
...
function vectorExpectations(suitePath, suite) {
    ...
    for (const vector of suite.vectors) {
        ...
        if (expectations.has(vector.id))
            throw new Error(`${suitePath}: duplicate vector id ${vector.id}`);
        expectations.set(vector.id, vector.expect);
    }
    return expectations;
}
function validateSourceManifest(bundle, bytes, manifest) {
    ...
    if (manifest.totals?.suites !== 21 || manifest.totals?.vectors !== 329) {
        throw new Error('current conformance manifest is not the pinned 21-suite/329-vector set');
    }
...
export function loadPinnedKitV2({ root = ROOT } = {}) {
    ...
    const manifestPath = resolveUnder(absoluteRoot, bundle.source_manifest.path, 'source manifest path');
    const { bytes: manifestBytes, value: manifest } = readJson(manifestPath, 'current conformance manifest');
    ...
    for (const suiteRef of bundle.suites) {
        ...
        if (executionPath !== suiteRef.path) {
            const target = resolveUnder(absoluteRoot, executionPath, `execution companion ${executionPath}`);
    ...
    const authority = contracts.find((contract) => contract.path === 'conformance/vectors/authority-document-proof-join.v1.json');
...
export function validateResultRowsV2(contract, rows) {
    ...
    for (const row of rows) {
        ...
        if (seen.has(row.id))
            throw new Error(`${contract.path}: duplicate vector id ${row.id}`);
        ...
        const difference = compareTypedResult(contract.expectations.get(row.id), row.result);
        if (difference)
            throw new Error(`${contract.path}#${row.id}: ${difference}`);
        seen.add(row.id);
        normalized.push({ id: row.id, result: row.result });
    }
    for (const id of contract.expectations.keys()) {
        if (!seen.has(id))
            throw new Error(`${contract.path}: runner omitted vector id ${id}`);
    }
    return normalized;
}
...
export function verifySubmissionManifestV2(manifest, kit) {
    exactKeys(manifest, ['@version', 'implementation', 'runner', 'kit', 'construction'], ['@version', 'implementation', 'runner', 'kit', 'construction'], 'submission');
    ...
    exactKeys(manifest.implementation, implementationFields, implementationFields, 'submission.implementation');
    for (const key of [
        'implementation_id',
        'organization',
        'team_id',
    ...
    safeRelativePath(manifest.implementation.source_tree_path, 'submission.implementation.source_tree_path', true);
    validateStringArray(manifest.implementation.dependencies, 'submission.implementation.dependencies', true);
    exactKeys(manifest.runner, ['protocol', 'artifact_sha256', 'fixed_arguments'], ['protocol', 'artifact_sha256', 'fixed_arguments'], 'submission.runner');
    ...
    exactKeys(manifest.construction, [
    ...
    validateStringArray(manifest.construction.specification_inputs, 'submission.construction.specification_inputs');
    if (nonEmptyString(manifest.construction.statement, 'submission.construction.statement').length < 40) {
        throw new Error('submission construction statement is not substantive');
    }
}
export function verifyRunnerArtifactV2(runner, runnerPath) {
    const target = fs.realpathSync(path.resolve(runnerPath));
    const stat = fs.statSync(target);
    if (!stat.isFile())
        throw new Error('runner artifact is not a file');
    if ((stat.mode & 0o111) === 0)
        throw new Error('runner artifact is not executable');
...
function expectedAttestationClaim(manifest) {
    return {
        ...
        authority_document_execution_companion_sha256: manifest.kit.authority_document_execution_companion_sha256,
        reference_source_access: 'none',
        emilia_affiliation: 'none',
    };
}
export function verifyIndependentAttestationV2(manifest, attestation, trustedAttestors) {
    ...
    exactKeys(attestation, ['@version', 'claim', 'attestor', 'signature'], ['@version', 'claim', 'attestor'], 'independent attestation');
    ...
    if (nonEmptyString(attestation.claim.statement, 'independent attestation.claim.statement').length < 40) {
        throw new Error('construction claim refused: attestor statement is not substantive');
    }
    const reviewedAt = nonEmptyString(attestation.claim.reviewed_at, 'independent attestation.claim.reviewed_at');
    if (!Number.isFinite(Date.parse(reviewedAt))) {
        throw new Error('construction claim refused: reviewed_at is not an ISO timestamp');
    }
    exactKeys(attestation.attestor, ['key_id', 'organization', 'team_id'], ['key_id', 'organization', 'team_id'], 'independent attestation.attestor');
    ...
    exactKeys(attestation.signature, ['algorithm', 'value_base64url'], ['algorithm', 'value_base64url'], 'independent attestation.signature');
    if (attestation.signature.algorithm !== 'Ed25519') {
        throw new Error('construction claim refused: signature algorithm must be Ed25519');
    }
    nonEmptyString(attestation.signature.value_base64url, 'independent attestation.signature.value_base64url');
    exactKeys(trustedAttestors, ['@version', 'keys'], ['@version', 'keys'], 'trusted attestors');
    ...
    const pin = trustedAttestors.keys.find((entry) => entry?.key_id === attestation.attestor.key_id);
    if (!pin)
        throw new Error('construction claim refused: attestor key is not pinned');
    exactKeys(pin, ['key_id', 'organization', 'team_id', 'independent', 'public_key_spki_base64url'], ['key_id', 'organization', 'team_id', 'independent', 'public_key_spki_base64url'], 'trusted attestor pin');
    ...
    if (!crypto.verify(null, Buffer.from(canonicalizeV2(unsigned), 'utf8'), publicKey, signature)) {
...
function parseRunnerOutput(stdout, suitePath) {
    try {
...
export function verifyCleanRoomSubmissionV2({ manifestPath, runnerPath, attestationPath, trustedAttestorsPath, emitPath, requireAcceptance = false, root = ROOT, }) {
    ...
    try {
        for (const [index, contract] of kit.contracts.entries()) {
            ...
            const suiteTarget = path.join(temporary, `${String(index).padStart(2, '0')}-${path.basename(contract.executionPath)}`);
            fs.writeFileSync(suiteTarget, contract.executionBytes, { mode: 0o444 });
            fs.chmodSync(suiteTarget, 0o444);
            let stdout;
            try {
                stdout = execFileSync(runner.path, [...manifest.runner.fixed_arguments, suiteTarget], {
                ...
                throw new Error(`${contract.path}: runner failed: ${String(error?.stderr || errorMessage(error)).trim()}`);
            }
            ...
            verifyRunnerArtifactV2(manifest.runner, runner.path);
            const rows = validateResultRowsV2(contract, parseRunnerOutput(stdout, contract.path));
    ...
    if (suites.length !== 21 || vectorCount !== 329) {
        throw new Error('external clean-room evaluation did not complete all 21 suites and 329 vectors');
    }
    const report = {
...
function cliOptions(argv) {
    ...
    if (!manifestPath || !runnerPath) {
        throw new Error('usage: verify-clean-room-submission-v2 --manifest FILE --runner EXECUTABLE '
...
if (process.argv[1]
    ...
    try {
        ...
        console.log(`CLEAN-ROOM V2: PASS (${report.conformance.suites} suites, `
