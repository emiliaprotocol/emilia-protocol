...
// Generated from migration-reconcile.mts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
//
...
if (reconcileRef) {
    ...
    try {
        migrationFiles = execFileSync('git', ['ls-tree', '-r', '--name-only', reconcileRef, '--', 'supabase/migrations'], { cwd: ROOT, encoding: 'utf8', maxBuffer: 4 * 1024 * 1024 }).split('\n').filter((file) => file.endsWith('.sql'));
        if (migrationFiles.length === 0)
            throw new Error('no migration files found at pinned commit');
        sql = migrationFiles.map((file) => execFileSync('git', ['show', `${reconcileRef}:${file}`], { cwd: ROOT, encoding: 'utf8', maxBuffer: 16 * 1024 * 1024 })).join('\n');
    }
    catch (error) {
