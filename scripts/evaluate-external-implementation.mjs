...
// Generated from evaluate-external-implementation.mts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
...
if (!fs.statSync(runnerReal).isFile())
    throw new Error('external runner is not a file');
...
if (sourceCommit !== pin.source.commit)
    throw new Error(`external source commit drift: ${sourceCommit}`);
const sourceTree = git('rev-parse', `${sourceCommit}:${pin.source.tree_path}`);
if (sourceTree !== pin.source.tree_oid)
    throw new Error(`external source tree drift: ${sourceTree}`);
function pinnedEvidenceFile(relativePath, expectedHash, label) {
    const target = path.resolve(ROOT, relativePath || '');
    if (target !== ROOT && !target.startsWith(`${ROOT}${path.sep}`))
        throw new Error(`${label} escapes the evaluator repository`);
    const bytes = fs.readFileSync(target);
    if (sha256(bytes) !== expectedHash)
        throw new Error(`${label} hash mismatch`);
    return { target, bytes };
}
const statementFile = pinnedEvidenceFile(pin.construction_evidence?.statement, pin.construction_evidence?.statement_sha256, 'construction statement');
const publicKeyFile = pinnedEvidenceFile(pin.construction_evidence?.public_key, pin.construction_evidence?.public_key_sha256, 'construction statement public key');
const statementGate = strictParseGate(statementFile.bytes.toString('utf8'));
if (!statementGate.ok)
    throw new Error(`construction statement: ${statementGate.reason}`);
const statement = JSON.parse(statementFile.bytes.toString('utf8'));
...
for (const suiteRef of bundle.suites) {
    ...
    if (sha256(bytes) !== suiteRef.sha256)
    ...
    if (!outputGate.ok)
        throw new Error(`external runner output ${suiteRef.path}: ${outputGate.reason}`);
    ...
    for (const result of results) {
        ...
        if (result.valid !== expected.get(result.id))
            throw new Error(`external divergence ${suiteRef.path}#${result.id}`);
        seen.add(result.id);
    }
