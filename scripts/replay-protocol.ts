...
interface ProtocolEvent {
  ...
  aggregate_id: string;
  command_type: string;
  payload_json: string | Record<string, any>;
  created_at: string;
...
interface VerifyReport {
  totalEvents: number;
  validHashes: number;
  invalidHashes: Array<{ event_id: any; expected?: string; actual?: string; error?: string }>;
  validChains: number;
...
interface ReplayResult {
  ...
  replayed: number;
  errors: Array<{ event_id: any; error: string }>;
}
...
interface ReconstitutionReport {
  deterministic: boolean;
  totalEventsReplayed: number;
  replayErrors: Array<{ event_id: any; error: string }>;
  projectionsRebuilt: { [key: string]: number };
  exactMatches: number;
  driftedRecords: any[];
  orphanedInCurrent: any[];
  orphanedInReplay: any[];
  eventIntegrity: {
    totalEvents: number;
    validHashes: number;
    invalidHashes: any[];
 ...
 */
export function computePayloadHash(payloadJson: string | Record<string, any>): string {
  const canonical =
 ...
 */
export function applyEvent(projections: ProjectionMap, event: ProtocolEvent): void {
 ...
 */
export function verifyEvents(events: ProtocolEvent[]): VerifyReport {
  const report: VerifyReport = {
    totalEvents: events.length,
    validHashes: 0,
    invalidHashes: [] as Array<{ event_id: any; expected?: string; actual?: string; error?: string }>,
    validChains: 0,
    brokenChains: [] as Array<{ event_id: any; parent_event_hash: any }>,
    signedEvents: 0,
    signatureErrors: [] as any[],
  };

  // Build an index of event_id -> payload_hash for chain verification
  const hashIndex: Map<string, any> = new Map();
 ...
 */
export function rebuildProjections(events: ProtocolEvent[], aggregateTypeFilter?: string | null): ReplayResult {
  const projections: ProjectionMap = {};
  let replayed: number = 0;
  const errors: Array<{ event_id: any; error: string }> = [];
  ...
  const sorted: ProtocolEvent[] = [...filtered].sort(
    (a: ProtocolEvent, b: ProtocolEvent) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime(),
  );
 ...
 */
export function diffProjections(replayedState: ProjectionMap, currentState: ProjectionMap): DiffResult {
  const result: DiffResult = {
    matching: [] as Array<{ aggregate_type: string; aggregate_id: string }>,
    drifted: [] as Array<{ aggregate_type: string; aggregate_id: string; diffs: Array<{ field: string; replayed: any; current: any }> }>,
    orphanedInCurrent: [] as Array<{ aggregate_type: string; aggregate_id: string }>,
    orphanedInReplay: [] as Array<{ aggregate_type: string; aggregate_id: string }>,
  };
  ...
  for (const aggType of allTypes) {
    const replayed: any = replayedState[aggType] || {};
    const current: any = currentState[aggType] || {};
    ...
    for (const id of allIds) {
      const rRow: any = replayed[id];
      const cRow: any = current[id];
      ...
      const diffs: Array<{ field: string; replayed: any; current: any }> = [];
      const keysToCompare: Set<string> = new Set([...Object.keys(rRow), ...Object.keys(cRow)]);
      const ignoreKeys: Set<string> = new Set(['updated_at', 'trust_materialized_at', 'id']);

      for (const key of keysToCompare) {
        if (ignoreKeys.has(key)) continue;
        const rVal: string | null = normalizeForDiff(rRow[key]);
        const cVal: string | null = normalizeForDiff(cRow[key]);
        if (rVal !== cVal) {
...
function normalizeForDiff(val: any): string | null {
...
const PROJECTION_TABLES: Record<string, string> = {
  receipt: 'receipts',
  dispute: 'disputes',
...
const PROJECTION_ID_COLUMN: Record<string, string> = {
  receipt: 'receipt_id',
  dispute: 'dispute_id',
  report: 'report_id',
  commit: 'commit_id',
 ...
 */
export function verifyCommitChainIntegrity(commitProjections: any[]): CommitChainReport {
  const report: CommitChainReport = {
  ...
  const nonceSet: Map<any, any> = new Map(); // nonce -> commit_id
 ...
 */
export function reconstitute(events: ProtocolEvent[], currentState: ProjectionMap): ReconstitutionReport {
...
async function main(): Promise<void> {
  ...
  const flags: { verify: boolean; rebuild: boolean; diff: boolean; dryRun: boolean; reconstitute: boolean } = {
  ...
  const aggregateTypeIdx: number = args.indexOf('--aggregate-type');
  const aggregateTypeFilter: string | null =
  ...
  const { data: events, error: eventsError }: { data: ProtocolEvent[] | null; error: any } = await supabase
  ...
  const filtered: ProtocolEvent[] = aggregateTypeFilter
    ? (events || []).filter((e: ProtocolEvent) => e.aggregate_type === aggregateTypeFilter)
    : (events || []);
  ...
  if (flags.verify) {
    ...
    const report: VerifyReport = verifyEvents(filtered);
  ...
  if (flags.rebuild) {
    ...
    const { projections, replayed, errors }: ReplayResult = rebuildProjections(filtered, null);
  ...
  if (flags.diff) {
    ...
    // Rebuild from events
    const { projections: replayed }: ReplayResult = rebuildProjections(filtered, null);

    // Load current state from projection tables
    const currentState: ProjectionMap = {};
    const typesToDiff: string[] = aggregateTypeFilter
      ? [aggregateTypeFilter]
      : Object.keys(PROJECTION_TABLES);

    for (const aggType of typesToDiff) {
      const table: string | undefined = PROJECTION_TABLES[aggType];
      const idCol: string | undefined = PROJECTION_ID_COLUMN[aggType];
      if (!table || !idCol) continue;

      const { data: rows, error: fetchError }: { data: any[] | null; error: any } = await supabase
        .from(table)
        .select('*')
        .order('created_at', { ascending: true });
      ...
      for (const row of (rows || [])) {
        const id: any = row[idCol];
        if (id) currentState[aggType][id] = row;
      }
    }

    const diffResult: DiffResult = diffProjections(replayed, currentState);
  ...
  if (flags.reconstitute) {
    ...
    const report: ReconstitutionReport = reconstitute(filtered, currentState);
