#!/usr/bin/env node
// Generated from check-docs-secrets.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
/**
...
const PATTERNS = [
    ...
    },
    {
        ...
        validate: (match, line) => {
            ...
            if (/sha256|sha512|hash|checksum|integrity/i.test(line))
                return false;
            // Skip if it looks like a base64 image or data URI
            if (/data:image|data:application/i.test(line))
                return false;
            try {
                const decoded = Buffer.from(match, "base64").toString("utf8");
                // Flag if decoded contains key-like patterns
                return (/password|secret|token|key|auth|credential/i.test(decoded) ||
...
const HOSTNAME_PATTERN = /(?:https?:\/\/)([a-z0-9][a-z0-9.-]*\.[a-z]{2,})/gi;
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            for (const pattern of PATTERNS) {
                ...
                while ((m = pattern.regex.exec(line)) !== null) {
                    ...
                    if (pattern.validate && !pattern.validate(matchStr, line))
                        continue;
            ...
            while ((hm = HOSTNAME_PATTERN.exec(line)) !== null) {
                ...
                if (SAFE_HOSTS.has(cleanHost))
                    continue;
                // Check if any safe host is a suffix (e.g. *.github.com)
                const isSafeSuffix = [...SAFE_HOSTS].some((safe) => cleanHost.endsWith("." + safe) || cleanHost === safe);
                if (isSafeSuffix)
                    continue;
                // Skip common documentation TLDs used as examples
                if (cleanHost.endsWith(".example") ||
                ...
                if (PLACEHOLDER_HOST_PATTERNS.some((p) => p.test(cleanHost)))
                    continue;
                // Skip hostnames the surrounding line clearly marks as illustrative.
                if (/\bplaceholder\b|\bexample\b|\byour\b|\breplace\b/i.test(line))
                    continue;
