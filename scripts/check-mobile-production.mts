...
const config: any = getMobileConfig({ env: process.env, production: true });
...
assert(config.androidKeyHashes.every((value: string) => /^[A-Za-z0-9_-]{43}$/.test(value)), 'Android package key pins are valid SHA-256 values');
assert(config.androidCertificateDigests.every((value: string) => /^[A-Za-z0-9_-]{43}={0,2}$/.test(value)), 'Play certificate pins are valid SHA-256 values');
...
const encodedAccount: string = required('GOOGLE_PLAY_SERVICE_ACCOUNT_JSON');
const accountText: string = encodedAccount.startsWith('{')
  ? encodedAccount
  : Buffer.from(encodedAccount, 'base64').toString('utf8');
const account: any = JSON.parse(accountText);
...
const assetLinkPins: any = config.androidAssetLinksFingerprints;
...
const nonexistentSessionId: string = '00000000-0000-0000-0000-000000000000';
const readinessNow: string = new Date().toISOString();
...
const invalidStateInsert: any = await supabase.rpc('mobile_state_add_if_absent', {
  p_state_key: null,
  p_state_value: null,
});
...
const fetchJson = async (path: string): Promise<any> => {
  const response: Response = await fetch(new URL(path, `${config.iosOrigin}/`), { redirect: 'manual' });
...
const apple: any = await fetchJson('/.well-known/apple-app-site-association');
const appleId: string = `${config.appleTeamId}.${config.iosBundleId}`;
assert(apple?.applinks?.details?.some((item: any) => item?.appIDs?.includes(appleId)), 'AASA serves the permanent Apple application identifier');

const android: any = await fetchJson('/.well-known/assetlinks.json');
const target: any = Array.isArray(android)
  ? android.find((item: any) => item?.target?.package_name === config.androidPackageName)
  : null;
assert(Boolean(target), 'Digital Asset Links serves the permanent Android package');
const servedPins: any[] = target.target.sha256_cert_fingerprints || [];
