...
// Generated from assemble-conformance-case.mts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
...
function validateBaseManifest(manifest) {
    ...
    assert(manifest.claim_scope === 'same-team consistency over the frozen external clean-room bundle; not independent implementation evidence', 'base manifest claim scope drifted');
    ...
    assert(manifest.totals?.vectors === manifest.suites.reduce((sum, suite) => sum + suite.vectors, 0), 'base manifest vector total mismatch');
    const ids = new Set();
...
function validatePin(pin) {
    ...
    assert(pin.construction_evidence.third_party_attestation === false
        && pin.construction_evidence.strict_clean_room_acceptance === false, 'v1 pin cannot claim strict clean-room acceptance without a separately validated independent-attestation object');
}
...
export function assembleConformanceCase({ manifestBytes, pinBytes, externalConformanceBytes, externalHostilityBytes, expectedEvaluatorCommit, }) {
    ...
    validatePin(pin);
    const externalConformanceFileHash = validateExternalConformance(externalConformance, normalizedConformanceBytes, pin, normalizedPinBytes, manifest, expectedEvaluatorCommit);
    const externalHostilityFileHash = validateHostility(externalHostility, normalizedHostilityBytes, pin, externalConformance.build.runner_sha256);
