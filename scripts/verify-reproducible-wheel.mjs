...
// Generated from verify-reproducible-wheel.mts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
...
try {
    for (let index = 0; index < packageArgs.length; index += 1) {
        ...
        if (!fs.existsSync(pyproject))
            throw new Error(`missing pyproject.toml: ${packageArgs[index]}`);
        ...
        for (const name of firstNames) {
            ...
            let artifactHash;
            try {
                artifactHash = assertPythonArtifactBytesMatch(firstBytes, secondBytes);
            }
            catch {
                throw new Error(`${packageArgs[index]} is not reproducible: ${name} bytes differ`);
            }
