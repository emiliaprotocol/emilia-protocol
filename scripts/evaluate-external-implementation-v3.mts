...
function readManifest(target: string): any {
  try {
...
function gitBytes(checkout: string, ...args: string[]): Buffer {
  return execFileSync('git', ['-C', checkout, ...args], {
    encoding: null,
...
function resolveImplementationRoot(
  sourceRoot: string,
  treePath: string,
): string {
...
export function evaluateExternalImplementationV3({
  manifestPath,
  sourcePath,
  runnerPath,
  attestationPath,
  ...
  if (!dockerImage && allowUnsafeLocalExecution !== true) {
    throw new Error(
      'external runner execution refused: explicit unsafe-local-execution acknowledgement is required',
    );
  }
  ...
  if (treeOid !== manifest.implementation?.source_tree_oid) {
    throw new Error(
      `external source tree drift: expected ${manifest.implementation?.source_tree_oid}, got ${treeOid}`,
    );
  }
...
function cliOptions(argv: string[]): {
  ...
  for (const key of [...allowed].filter((entry) =>
    !['--allow-unsafe-local-execution', '--docker-image'].includes(entry))) {
    if (!values.get(key)) {
      throw new Error(
        'usage: evaluate-external-implementation-v3 --manifest FILE --source CHECKOUT '
  ...
  return {
    ...
    sourcePath: values.get('--source') as string,
    runnerPath: values.get('--runner') as string,
    attestationPath: values.get('--attestation') as string,
    trustedAttestorsPath: values.get('--trusted-attestors') as string,
    emitPath: values.get('--emit') as string,
...
if (process.argv[1]
  ...
  try {
    ...
    console.log(
      `EXTERNAL CONFORMANCE V3: PASS (${report.conformance.suites} suites, `
