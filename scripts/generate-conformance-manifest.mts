...
const relative = (absolute: string): string => path.relative(ROOT, absolute).split(path.sep).join('/');
...
function filesUnder(root: string, predicate: (path: string) => boolean = () => true): string[] {
  const out: string[] = [];
...
function sourceTree(paths: string[]): SourceTree {
  const members: SourceTreeMember[] = paths
  ...
  return {
    ...
    tree_sha256: sha256(Buffer.from(canonicalize(members as unknown), 'utf8')),
  };
}
...
const implementations: Implementation[] = [
  {
    ...
    sources: [
      ...
      path.join(ROOT, 'lib/evidence/effect-predicates.ts'),
      path.join(ROOT, 'lib/evidence/evidence-graph.ts'),
    ],
  },
  {
...
let suiteRefs: unknown[];
if (cleanRoom) {
  bundlePath = BUNDLE_PATH;
  bundleBytes = fs.readFileSync(bundlePath);
  const bundle = parseStrictJson(bundleBytes.toString('utf8'), 'conformance bundle') as any;
...
const suites: ManifestSuite[] = [];
let vectorCount = 0;
for (const suiteRef of suiteRefs) {
  const suiteRefTyped = suiteRef as Record<string, unknown>;
  const suitePath = path.resolve(ROOT, suiteRefTyped.path as string);
  const bytes = fs.readFileSync(suitePath);
  const suiteSha256 = sha256(bytes);
  if (suiteRefTyped.sha256 && suiteSha256 !== suiteRefTyped.sha256) throw new Error(`conformance vector drift: ${suiteRefTyped.path}`);
  const suite = parseStrictJson(bytes.toString('utf8'), `suite ${suiteRefTyped.path}`) as Record<string, unknown>;
  if (!Array.isArray(suite.vectors)) throw new Error(`suite has no vectors: ${suiteRefTyped.path}`);
  vectorCount += suite.vectors.length;
  const suiteFile = path.basename(suiteRefTyped.path as string);
  ...
  const manifestSuite: ManifestSuite = { path: suiteRefTyped.path as string, sha256: suiteSha256, vectors: suite.vectors.length };
...
for (const implementation of implementations) {
  ...
  for (const suite of suites) {
    ...
    try {
      ...
      throw new Error(`${implementation.id} failed ${suite.path}: ${(error as any).stderr || (error as Error).message}`);
    }
