...
//   --check --drift-report <f>  also record the comparison in <f> for CI's
//                               volatile-evidence policy
//                               (scripts/ci/volatile-evidence.mjs). A stale
//                               artifact still fails: the outputs are rendered
//                               only from checked-in inputs, so the LLM
//                               context has no advisory drift. Its test counts
//                               come from lib/proof-stats.json, whose measured
//                               counts main refreshes after merge.
...
for (let index = 2; index < process.argv.length; index += 1) {
  ...
  if (argument === '--write') write = true;
    ...
    if (argument === '--out-dir') outDir = value;
    else driftReport = value;
    index += 1;
  } else usage();
}
if (write === check || (outDir && !write) || (driftReport && !check)) usage();
...
const INPUTS: string[] = Object.values(PATHS);
...
function comma(value: number | string): string {
...
const source = readJson(PATHS.source) as any;
const proofStats = readJson(PATHS.proofStats) as any;
const conformance = readJson(PATHS.conformance) as any;
const external = readJson(PATHS.external) as any;
const claimSource = readJson(PATHS.claimSource) as any;
const securityCase = readJson(PATHS.securityCase) as any;
const observatory = readJson(PATHS.observatory) as any;
...
assert(
  Object.values(proofStats.formalEvidenceCoverage).reduce(
    (total: number, category: any) => total + category.count,
    0,
  ) === securityCase.claim_count,
  'formal evidence taxonomy does not cover the security-case claim inventory',
);
...
for (const [index, document] of canonicalReadingPath.documents.entries()) {
  ...
  assert(
    standardsStatus.active_datatracker.some(
      (standard: any) => standard.draft === document.draft && standard.revision === document.revision,
    ),
    `canonical reading path revision is not active: ${document.draft}-${document.revision}`,
  );
}
...
for (const group of source.source_precedence || []) {
  for (const item of group.sources || []) {
    ...
    if (GENERATED_PATHS.has(item)) continue;
    assert(fs.existsSync(absolute(item)), `missing precedence source: ${item}`);
  }
}

interface GeneratedFromItem {
  path: string;
  sha256: string;
  bytes: number;
}

const generatedFrom: GeneratedFromItem[] = INPUTS.map((relative) => {
  const bytes = fs.readFileSync(absolute(relative));
  return { path: relative, sha256: sha256(bytes), bytes: bytes.length };
});
const inputDigest: string = sha256(Buffer.from(JSON.stringify(generatedFrom), 'utf8'));
const externalScope: string = external.conformance.vectors === conformance.totals.vectors
  ? 'current_vector_set'
  : 'time_pinned_prior_vector_set';
const hostilityCases: number = external.hostility.structured_cases + external.hostility.raw_parser_cases;

const securityClaims: unknown[] = (claimSource.claims as any[]).map((claim: any) => ({
  claim_id: (claim as any).claim_id,
  statement: (claim as any).statement,
  acceptance_roots: (claim as any).acceptance_roots || [],
  enforcement_path: (claim as any).enforcement_path || [],
  language_coverage: (claim as any).language_coverage || {},
  vectors: ((claim as any).vectors || []).map((vector: any) => ({
    suite: vector.suite,
    case_id: vector.case_id,
    polarity: vector.polarity,
  })),
  formal: (claim as any).formal || [],
  assumptions: (claim as any).assumptions || [],
  exclusions: (claim as any).exclusions || [],
}));
...
const context: any = {
  ...
  provenance: {
    ...
    freshness_scope:
      'Checks these artifacts byte for byte against the checked-in inputs listed in generated_from. It does not re-measure the automated test counts: they come from lib/proof-stats.json, which main refreshes after each merge, so they may lag the source revision by one refresh cycle. npm run check:proof-stats -- --drift-report <file> re-runs the suite, fails on every other proof field, and reports that lag.',
  },
  ...
  current_evidence: {
    ...
    formal: {
      ...
      tla_inventory_scope:
        'established core model set; selected model/runtime scenario conformance is reported separately',
...
function repoHref(relative: string): string {
...
function pathLink(relative: string, web: boolean): string {
  return `[${relative}](${web ? repoHref(relative) : relative})`;
}

function renderFull(web: boolean = false): string {
  const lines: string[] = [];
  ...
  lines.push('');
  lines.push(`Generated from repository evidence. Test-count snapshot: ${proofStats.generatedAt}. Input digest: sha256:${inputDigest}.`);
  lines.push(`Do not edit this file directly. Source: ${pathLink('docs/ai/context-source.v1.json', web)}. Freshness check: \`npm run check:llm-context\` (compares these artifacts with their checked-in inputs).`);
  lines.push('');
  lines.push('The automated test counts come from lib/proof-stats.json, which main refreshes after each merge, so they may lag the source revision by one refresh cycle. The other figures are rendered from checked-in evidence files, and CI fails a change that leaves the security case, the formal traces, the conformance manifest or this file out of date.');
  lines.push('');
  lines.push('## Read This First');
  lines.push('');
  lines.push('EMILIA is the authority control plane for autonomous work. A human or institution defines a finite operating mandate once; agents may work unattended inside it; Gate enforces each consequential unit of work on covered executor paths. Native artifacts verify under native rules; CAID performs material-action matching; AEC evaluates whether the matched bundle satisfies a relying-party evidence requirement; the executor makes the separate authorization decision. Identity, delegated machine scope, machine policy, human approval when required, execution evidence, transparency, and reliance remain separate layers.');
  lines.push('');
  ...
  lines.push('');
  lines.push(`- Automated tests: ${comma(proofStats.tests.total)} cases across ${comma(proofStats.tests.files)} files at the test-count snapshot; ${proofStats.tests.policy}.`);
  ...
  lines.push('');
  for (const item of source.canonical_definitions) lines.push(`- **${item.term}:** ${item.definition}`);
  lines.push('');
  ...
  lines.push('');
  for (const claim of securityClaims) lines.push(`- \`${(claim as any).claim_id}\`: ${(claim as any).statement}`);
  lines.push('');
  ...
  lines.push('');
  for (const item of source.excluded_as_current_authority) lines.push(`- ${item}`);
  lines.push('');
  ...
  lines.push('');
  for (const entry of source.code_entry_points) lines.push(`- ${pathLink(entry.path, web)} - ${entry.purpose}.`);
  lines.push('');
  lines.push('## Reproduce Before Repeating');
  lines.push('');
  for (const entry of source.commands) lines.push(`- \`${entry.command}\` - ${entry.purpose}.`);
  lines.push('');
  lines.push('## Rules For Answering About EMILIA');
  lines.push('');
  for (const rule of source.answering_rules) lines.push(`- ${rule}`);
  lines.push('');
...
function renderIndex(): string {
  const lines: string[] = [
    ...
    '',
    `This is a generated discovery index following the llms.txt proposal. Test-count snapshot: ${proofStats.generatedAt}; main refreshes the test counts after each merge, so they may lag the source revision by one refresh cycle. For substantive analysis, load the full or machine-readable context below before drawing conclusions from individual repository files.`,
    '',
    ...
    `- [Canonical four-document reading path](${BASE_URL}/llms-full.txt#canonical-four-document-reading-path): ${canonicalReadingPath.documents.map((document: any) => document.label).join(' -> ')}.`,
...
const outputs: Map<string, string> = new Map([
  ['AI_CONTEXT.md', renderFull(false)] as [string, string],
  ['public/llms.txt', renderIndex()] as [string, string],
  ['public/llms-full.txt', renderFull(true)] as [string, string],
...
if (write) {
  ...
  if (stale.length) {
    // No advisory drift here, with or without --drift-report: every output is
    // rendered from checked-in inputs, so a stale one means a change to an
    // input (the proof fields included) that the pull request did not
    // regenerate, and `npm run sync:llm-context` fixes it without the suite.
    console.error(`LLM CONTEXT: FAIL - stale generated artifact(s): ${stale.join(', ')}`);
    console.error('Fix: npm run sync:llm-context, then commit the four artifacts.');
    process.exit(1);
  } else {
