...
import {
  sha256V3,
  validateBundleDefinitionV3,
} from './verify-clean-room-submission-v3.mjs';
...
function safeKitPath(value: string): string {
...
function readAt(commit: string, filePath: string): Buffer {
  return git(['show', `${commit}:${safeKitPath(filePath)}`], { encoding: null }) as Buffer;
}

function modeAt(commit: string, filePath: string): string {
...
function trackedFile(commit: string, filePath: string): CollectedFile {
...
export function collectCleanRoomKitV3Files(
  ref = 'HEAD',
): { commit: string; files: CollectedFile[] } {
  const commit = String(git(['rev-parse', '--verify', `${ref}^{commit}`])).trim();
  if (!/^[0-9a-f]{40}$/.test(commit)) throw new Error(`could not resolve immutable commit ${ref}`);
  ...
  for (const [filePath, expectedHash] of expectedHashes) {
    ...
    if (actual !== expectedHash) {
      throw new Error(
        `v3 kit pin mismatch at ${commit}:${safe}: expected ${expectedHash}, got ${actual}`,
      );
    }
...
function buildArchiveCommit(
  ...
  return String(git(
    ['commit-tree', tree, '-m', `Source-free clean-room kit v3 from ${sourceCommit}`],
    { env: identity },
  )).trim();
}

function writeArchive(commit: string, files: CollectedFile[], target: string): void {
  git([
    'archive',
    '--format=tar.gz',
    `--prefix=${KIT_PREFIX}`,
...
function archiveMembers(target: string): string[] {
...
export function buildCleanRoomKitV3({
  ref = 'HEAD',
  output,
}: {
  ref?: string;
  ...
  const target = path.resolve(
    output ?? path.join(ROOT, 'release-artifacts/emilia-clean-room-kit-v3.tar.gz'),
  );
  ...
  try {
    ...
    const authority = bundle.suites.find((suite: any) => suite.execution_path);
...
function cliOptions(argv: string[]): { ref?: string; output?: string } {
...
if (process.argv[1]
  ...
  try {
    ...
    console.log(
      `CLEAN-ROOM KIT V3: PASS (${report.files.length} files; `
