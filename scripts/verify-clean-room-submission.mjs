...
// Generated from verify-clean-room-submission.mts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
...
function validateManifest(manifest) {
    ...
    if (manifest?.['@version'] !== 'EP-CLEAN-ROOM-SUBMISSION-v1')
        problems.push('unexpected @version');
    ...
    for (const field of ['implementation_id', 'organization', 'language', 'version', 'source_repository', 'source_commit', 'license_spdx', 'build_instructions']) {
        if (typeof impl?.[field] !== 'string' || impl[field].trim() === '')
            problems.push(`implementation.${field} is required`);
    }
    ...
    if (typeof independence?.claimed !== 'boolean')
        problems.push('independence.claimed must be boolean');
    if (!Array.isArray(independence?.authors) || independence.authors.length === 0)
        problems.push('independence.authors is required');
    else if (independence.authors.some((value) => typeof value !== 'string' || value.trim() === ''))
        problems.push('independence.authors must contain non-empty strings');
    if (!Array.isArray(independence?.specification_inputs) || independence.specification_inputs.length === 0)
        problems.push('independence.specification_inputs is required');
    else if (independence.specification_inputs.some((value) => typeof value !== 'string' || value.trim() === ''))
        problems.push('independence.specification_inputs must contain non-empty strings');
    if (!/^[0-9a-f]{64}$/.test(independence?.specification_bundle_sha256 || ''))
        problems.push('independence.specification_bundle_sha256 is invalid');
    if (!['none', 'reference_port'].includes(independence?.reference_source_access))
        problems.push('independence.reference_source_access is invalid');
    if (!['none', 'maintainer', 'contractor'].includes(independence?.emilia_affiliation))
        problems.push('independence.emilia_affiliation is invalid');
    if (independence?.claimed && independence.reference_source_access !== 'none')
        problems.push('a clean-room claim requires reference_source_access:none');
    if (independence?.claimed && (typeof independence.statement !== 'string' || independence.statement.length < 20))
        problems.push('a clean-room claim requires a substantive statement');
    const runner = manifest?.runner;
    exact(runner, ['protocol', 'artifact_argument_index', 'artifact_sha256', 'fixed_arguments'], 'runner');
    if (runner?.protocol !== 'EP-CONFORMANCE-FILE-RUNNER-v1')
        problems.push('runner.protocol is invalid');
    if (!Number.isInteger(runner?.artifact_argument_index) || runner.artifact_argument_index < -1)
        problems.push('runner.artifact_argument_index is invalid');
    if (!/^[0-9a-f]{64}$/.test(runner?.artifact_sha256 || ''))
        problems.push('runner.artifact_sha256 is invalid');
    if (!Array.isArray(runner?.fixed_arguments) || runner.fixed_arguments.some((value) => typeof value !== 'string'))
        problems.push('runner.fixed_arguments is invalid');
    ...
    if (manifest?.attestation)
        exact(manifest.attestation, ['algorithm', 'key_id', 'attestor_relationship', 'signature_base64url'], 'attestation');
    if (problems.length)
        throw new Error(`manifest refused: ${problems.join('; ')}`);
}
function verifyRunnerBinding(manifest) {
    ...
    if (index >= commandArgs.length)
        throw new Error('runner binding refused: artifact_argument_index is out of range');
    ...
    if (actual !== manifest.runner.artifact_sha256)
        throw new Error('runner binding refused: executable artifact hash mismatch');
...
function verifyPinnedBundle(bundlePath, expectedVersion, memberField) {
    ...
    if (bundle['@version'] !== expectedVersion)
        throw new Error(`unsupported ${expectedVersion} bundle`);
...
function verifyAttestation(manifest, trusted) {
    if (!manifest.attestation)
        return { status: manifest.independence.claimed ? 'self_attested_unverified' : 'not_claimed' };
    const pin = trusted?.keys?.find((key) => key.key_id === manifest.attestation.key_id);
    if (!pin)
        return { status: 'attestor_unpinned', key_id: manifest.attestation.key_id };
    if (manifest.attestation.algorithm !== 'Ed25519')
        return { status: 'invalid_attestation', reason: 'algorithm' };
    ...
    try {
        ...
        if (key.asymmetricKeyType !== 'ed25519')
            return { status: 'invalid_attestation', reason: 'key_type' };
...
const specificationBundle = verifyPinnedBundle(SPECIFICATION_BUNDLE_PATH, 'EP-CLEAN-ROOM-SPECIFICATION-BUNDLE-v1', 'documents');
...
if (manifest.independence.claimed && attestation.status === 'invalid_attestation')
    throw new Error('clean-room attestation signature is invalid');
if (requireExternal) {
    const problems = [];
    if (!manifest.independence.claimed)
        problems.push('independence must be claimed');
    if (manifest.independence.reference_source_access !== 'none')
        problems.push('reference source access must be none');
    if (manifest.independence.emilia_affiliation !== 'none')
        problems.push('EMILIA-affiliated implementations are not external');
    if (/emilia/i.test(manifest.implementation.organization))
        problems.push('implementation organization is not external to EMILIA');
    if (!/^[0-9a-f]{40,64}$/.test(manifest.implementation.source_commit))
        problems.push('source_commit must be an immutable 40-64 character hexadecimal commit');
    if (attestation.status !== 'third_party_attested' || attestation.independent !== true)
        problems.push('an independently pinned third-party attestation is required');
    if (manifest.attestation?.attestor_relationship !== 'independent')
        problems.push('attestor_relationship must be independent');
    ...
    if (/emilia/i.test(String(attestation.organization || '')))
        problems.push('EMILIA cannot attest external independence');
    ...
    if (builtInRunnerHashes.includes(runnerBinding.sha256))
        problems.push('same-team reference runner cannot satisfy external acceptance');
    if (problems.length)
        throw new Error(`external clean-room acceptance refused: ${problems.join('; ')}`);
}
const suiteResults = [];
let vectorCount = 0;
for (const suiteRef of bundle.suites) {
    ...
    if (actualHash !== suiteRef.sha256)
        throw new Error(`vector bundle drift: ${suiteRef.path}`);
    const suiteText = new TextDecoder('utf-8', { fatal: true }).decode(suiteBytes);
    const suiteGate = strictParseGate(suiteText);
    if (!suiteGate.ok)
        throw new Error(`frozen vector JSON refused: ${suiteRef.path}: ${suiteGate.reason}`);
    ...
    try {
        ...
        throw new Error(`runner emitted invalid JSON for ${suiteRef.path}: ${error.message}`);
    }
    if (!Array.isArray(results))
        throw new Error(`runner output is not an array for ${suiteRef.path}`);
    ...
    for (const result of results) {
        ...
        if (seen.has(result.id))
            throw new Error(`duplicate runner result ${result.id} in ${suiteRef.path}`);
        seen.add(result.id);
        if (!expected.has(result.id))
            throw new Error(`runner added unknown vector ${result.id} in ${suiteRef.path}`);
        const expectedValue = expected.get(result.id);
        if (result.valid !== expectedValue)
            failures.push({ id: result.id, expected: expectedValue, actual: result.valid });
    }
    for (const id of expected.keys())
        if (!seen.has(id))
            throw new Error(`runner omitted vector ${id} in ${suiteRef.path}`);
    if (failures.length)
        throw new Error(`conformance failures in ${suiteRef.path}: ${JSON.stringify(failures)}`);
