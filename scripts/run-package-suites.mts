...
const PACKAGES_DIR: string = fileURLToPath(new URL('../packages/', import.meta.url));
...
function discoverTestablePackages(): string[] {
  const names: string[] = [];
  for (const entry of readdirSync(PACKAGES_DIR, { withFileTypes: true })) {
    ...
    if (manifest.scripts?.test) names.push(entry.name);
  }
  return names.sort();
}

const testable: string[] = discoverTestablePackages();
const testableSet: Set<string> = new Set(testable);
...
const stale: string[] = Object.keys(COVERED_ELSEWHERE).filter((name: string) => !testableSet.has(name));
if (stale.length > 0) {
  console.error(
    `COVERED_ELSEWHERE lists ${stale.join(', ')}, which no longer declare a test script.\n` +
      'Remove the entry, or restore the suite it was standing in for.',
  );
  process.exit(1);
}

const toRun: string[] = testable.filter((name: string) => !(name in COVERED_ELSEWHERE));
