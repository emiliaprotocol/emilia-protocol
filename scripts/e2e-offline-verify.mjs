...
// Generated from e2e-offline-verify.mts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
...
const signoffId = process.argv[2];
...
if (error)
    throw error;
const decision = decisions?.[0]?.after_state;
if (!decision) {
...
const signoff = { context: decision.context, webauthn: decision.webauthn };
const ok = verifyWebAuthnSignoff(signoff, cred.public_key_spki, { rpId: 'localhost' });
...
const forged = JSON.parse(JSON.stringify(signoff));
forged.context.action_hash = 'f'.repeat(64); // point the approval at a different action
const bad = verifyWebAuthnSignoff(forged, cred.public_key_spki, { rpId: 'localhost' });
