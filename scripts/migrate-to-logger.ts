...
for (const filePath of targets) {
  ...
  const libLogger: string = path.join(ROOT, 'lib', 'logger.js');
  let relPath: string = path.relative(path.dirname(filePath), libLogger);
  // Normalise to forward slashes and ensure ./ prefix
  relPath = relPath.replace(/\\/g, '/');
  if (!relPath.startsWith('.')) relPath = './' + relPath;

  // Replace console calls
  let updated: string = src
  ...
  const importLine: string = `import { logger } from '${relPath}';`;
