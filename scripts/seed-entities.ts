 ...
 *
 * Usage:
 *   node scripts/seed-entities.ts https://emiliaprotocol.ai
 *   node scripts/seed-entities.ts http://localhost:3000
 *
...
interface EntityDefinition {
  ...
  pricing_amount_cents: number;
}

const BASE_URL: string = process.argv[2] || 'http://localhost:3000';
...
async function seed(): Promise<void> {
  ...
  for (const entity of entities) {
    try {
      const res: Response = await fetch(`${BASE_URL}/api/entities/register`, {
        method: 'POST',
      ...
      console.error(`Failed to register ${entity.entity_id}:`, message);
    }
  }
