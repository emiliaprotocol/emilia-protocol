...
const VERIFY: string = path.join(ROOT, 'scripts/verify-clean-room-submission.mjs');
const REFERENCE: string = path.join(ROOT, 'conformance/clean-room/reference-port.manifest.json');
const SPECIFICATION_BUNDLE: string = path.join(ROOT, 'conformance/clean-room/specification-bundle.v1.json');
const specHash: string = crypto.createHash('sha256').update(fs.readFileSync(SPECIFICATION_BUNDLE)).digest('hex');
const dir: string = fs.mkdtempSync(path.join(os.tmpdir(), 'ep-clean-room-contract-'));

function writeJson(name: string, value: any): string {
