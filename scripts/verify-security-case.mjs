...
if (!sourceCase.release_artifacts || typeof sourceCase.release_artifacts !== 'object')
...
for (const claim of sourceCase.claims ?? []) {
    ...
    if (!Array.isArray(claim?.acceptance_roots) || claim.acceptance_roots.length === 0 || claim.acceptance_roots.some((v) => !nonEmpty(v)))
        fail(id, 'acceptance_roots must contain non-empty strings');
    if (!Array.isArray(claim?.assumptions) || claim.assumptions.length === 0 || claim.assumptions.some((v) => !nonEmpty(v)))
        fail(id, 'assumptions must be explicit non-empty strings');
    if (!Array.isArray(claim?.exclusions) || claim.exclusions.length === 0 || claim.exclusions.some((v) => !nonEmpty(v)))
        fail(id, 'exclusions must be explicit non-empty strings');
    if (!Array.isArray(claim?.enforcement_path) || claim.enforcement_path.length === 0)
    ...
    if (!Array.isArray(claim?.implementations) || claim.implementations.length === 0)
    ...
    if (claim.operational_evidence !== undefined && !Array.isArray(claim.operational_evidence))
    ...
    if (!Array.isArray(claim.release_artifacts) || claim.release_artifacts.length === 0)
