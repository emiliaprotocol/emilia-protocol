#!/usr/bin/env node
// Generated from generate-proof-metrics.mts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
/**
...
if (!totalChecks) {
    console.error('FAIL: could not determine test count');
    process.exit(1);
}
const mcpSrc = readFileSync(join(root, 'mcp-server/index.js'), 'utf8');
const mcpTools = (mcpSrc.match(/    name: 'ep_/g) || []).length;
const scoringSrc = readFileSync(join(root, 'lib/scoring-v2.ts'), 'utf8');
const pm = scoringSrc.match(/export const TRUST_POLICIES(?::[^=]*)?\s*=\s*\{/);
let policyCount = 0;
if (pm) {
    const b = scoringSrc.substring(scoringSrc.indexOf(pm[0]), scoringSrc.indexOf('};', scoringSrc.indexOf(pm[0])) + 2);
