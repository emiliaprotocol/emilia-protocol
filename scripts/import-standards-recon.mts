...
function parseArgs(argv: string[]): ParsedArgs {
  const parsed: ParsedArgs = { inputs: [], output: DEFAULT_OUTPUT, declared: null, asOf: null };
  for (let index = 0; index < argv.length; index += 1) {
    const value = argv[index];
    if (value === '--input') parsed.inputs.push(argv[++index]);
    else if (value === '--emit') parsed.output = path.resolve(argv[++index]);
    else if (value === '--declared') parsed.declared = Number(argv[++index]);
...
function listJsonl(input: string): string[] {
...
function toolUses(record: any): any[] {
...
function parseFile(file: string): unknown[] {
  ...
  for (let lineNumber = 0; lineNumber < lines.length; lineNumber += 1) {
    ...
    for (const tool of toolUses(record)) {
      ...
      if (tool.name !== 'StructuredOutput') continue;

      const input = tool.input;
      if (!input || typeof input !== 'object' || !input.name || !input.title || !input.layer) continue;
...
function main(): void {
  ...
  const catalog: Record<string, any> = JSON.parse(fs.readFileSync(CATALOG, 'utf8'));
  const files = [...new Set(args.inputs.flatMap(listJsonl))].sort();
  const reports = files.flatMap(parseFile).sort((left: any, right: any) => (left.id as string).localeCompare(right.id as string));
  const duplicateIds = reports.filter((report: any, index) => index > 0 && (report as any).id === (reports[index - 1] as any).id).map((report: any) => report.id);
  if (duplicateIds.length) throw new Error(`duplicate report id(s): ${[...new Set(duplicateIds)].join(', ')}`);
  ...
  const output = {
    ...
    metrics: {
      ...
      fetch_failures_in_recovered_reports: reports.filter((report: any) => (report as any).fetch_failed).length,
