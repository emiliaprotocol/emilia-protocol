...
// Generated from generate-conformance-manifest.mts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
...
if (emitIndex >= 0 && !argv[emitIndex + 1])
...
function filesUnder(root, predicate = () => true) {
    ...
    const walk = (current) => {
        for (const entry of fs.readdirSync(current, { withFileTypes: true }).sort((a, b) => a.name.localeCompare(b.name))) {
            if (entry.name === '__pycache__' || entry.name === 'dist' || entry.name.startsWith('.'))
                continue;
...
if (cleanRoom) {
    ...
    if (bundle['@version'] !== 'EP-CLEAN-ROOM-VECTOR-BUNDLE-v1')
        throw new Error('unsupported conformance bundle');
...
for (const suiteRef of suiteRefs) {
    const suiteRefTyped = suiteRef;
    const suitePath = path.resolve(ROOT, suiteRefTyped.path);
    const bytes = fs.readFileSync(suitePath);
    const suiteSha256 = sha256(bytes);
    if (suiteRefTyped.sha256 && suiteSha256 !== suiteRefTyped.sha256)
        throw new Error(`conformance vector drift: ${suiteRefTyped.path}`);
    const suite = parseStrictJson(bytes.toString('utf8'), `suite ${suiteRefTyped.path}`);
    if (!Array.isArray(suite.vectors))
        throw new Error(`suite has no vectors: ${suiteRefTyped.path}`);
    vectorCount += suite.vectors.length;
    const suiteFile = path.basename(suiteRefTyped.path);
    ...
    const manifestSuite = { path: suiteRefTyped.path, sha256: suiteSha256, vectors: suite.vectors.length };
...
if (check) {
    ...
    if (expected !== output)
        throw new Error(`${relative(DEFAULT_OUTPUT)} is stale; regenerate its conformance manifest`);
}
if (emitPath) {
