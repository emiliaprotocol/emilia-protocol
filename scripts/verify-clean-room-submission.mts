...
const SPECIFICATION_BUNDLE_PATH: string = path.join(ROOT, 'conformance/clean-room/specification-bundle.v1.json');
...
const manifestPath: string | null = option('--manifest');
const emitPath: string | null = option('--emit');
const trustedPath: string | null = option('--trusted-attestors');
const requireExternal: boolean = argv.includes('--require-external');
const command: string | null = separator >= 0 ? argv[separator + 1] : null;
const commandArgs: string[] = separator >= 0 ? argv.slice(separator + 2) : [];
...
function validateManifest(manifest: any): void {
  ...
  const exact = (value: any, allowed: string[], label: string): void => {
...
function verifyRunnerBinding(manifest: any): { path: string; sha256: string } {
...
function frozenMemberPath(memberPath: string, bundleLabel: string): string {
...
function verifyPinnedBundle(bundlePath: string, expectedVersion: string, memberField: string): { bundle: any; sha256: string } {
...
function verifyAttestation(manifest: any, trusted: any): any {
...
const manifestAbsolute: string = path.resolve(manifestPath as string);
const manifest: any = readJson(manifestAbsolute, 'manifest');
validateManifest(manifest);
const runnerBinding: { path: string; sha256: string } = verifyRunnerBinding(manifest);
...
const trusted: any = trustedPath ? readJson(path.resolve(trustedPath), 'trusted attestors') : null;
const attestation: any = verifyAttestation(manifest, trusted);
...
const suiteResults: Array<{ path: string; sha256: string; vectors: number; status: string }> = [];
let vectorCount: number = 0;
for (const suiteRef of bundle.suites) {
  const suitePath: string = frozenMemberPath(suiteRef.path, 'vector bundle');
  const suiteBytes: Buffer = fs.readFileSync(suitePath);
  const actualHash: string = sha256(suiteBytes);
  if (actualHash !== suiteRef.sha256) throw new Error(`vector bundle drift: ${suiteRef.path}`);
  const suiteText: string = new TextDecoder('utf-8', { fatal: true }).decode(suiteBytes);
  ...
  const suite: any = JSON.parse(suiteText);
  let output: string;
  try {
    output = execFileSync(command as string, [...commandArgs, suitePath], {
      cwd: ROOT,
      encoding: 'utf8',
    ...
    throw new Error(`runner failed for ${suiteRef.path}: ${(error as any).stderr || (error as any).message}`);
  }
  let results: any;
  try {
  ...
  } catch (error) { throw new Error(`runner emitted invalid JSON for ${suiteRef.path}: ${(error as any).message}`); }
  ...
  const failures: Array<{ id: string; expected: boolean; actual: boolean }> = [];
  for (const result of results) {
    ...
    const expectedValue = expected.get(result.id) as boolean;
    if (result.valid !== expectedValue) failures.push({ id: result.id, expected: expectedValue, actual: result.valid });
  }
...
const unsignedManifest: any = { ...manifest };
