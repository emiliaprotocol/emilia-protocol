...
const SCAN_GLOBS_INCLUDE: string[] = [
  'app/',
  'components/',
  'lib/',
  'tests/',
...
const SCAN_EXTENSIONS: Set<string> = new Set(['.js', '.mjs', '.cjs', '.ts', '.tsx', '.json', '.yaml', '.yml', '.md']);

function walk(dirOrFile: string, acc: string[] = []): string[] {
  const abs: string = path.join(ROOT, dirOrFile);
...
const files: string[] = SCAN_GLOBS_INCLUDE.flatMap((g: string) => walk(g));

const violations: Violation[] = [];
for (const relPath of files) {
  const abs: string = path.join(ROOT, relPath);
  let src: string;
  try {
