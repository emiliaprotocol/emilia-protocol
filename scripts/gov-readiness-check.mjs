...
// Generated from gov-readiness-check.mts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
//
...
check('async Gate providers cannot bypass a guarded action', () => {
    ...
    for (const needle of [
        ...
        if (!gate.includes(needle))
            throw new Error(`packages/gate/src/index.ts missing ${needle}`);
    }
...
check('inline/self-signed receipt acceptance is demo-only', () => {
    ...
    for (const rel of runtimeFiles) {
        const src = read(rel);
        if (!src.includes('allowInlineKey: true'))
            continue;
        if (!rel.startsWith('app/api/demo/'))
            offenders.push(rel);
    }
    if (offenders.length)
        throw new Error(`allowInlineKey:true outside demo routes: ${offenders.join(', ')}`);
...
check('crypto profile is declared and fail-closed', () => {
    ...
    if (process.env.EP_CRYPTO_PROFILE) {
        ...
        if (!sat.ok)
            throw new Error(sat.reasons.join('; '));
...
check('incident drill is runnable', () => {
    ...
    if (!pkg.scripts?.['gov:drill:key-compromise'])
        throw new Error('package.json missing gov:drill:key-compromise');
