...
export function auditClaimText(text, file = '<text>', expectations = {}) {
    ...
    for (const match of text.matchAll(/\b(\d+)\s+(?:cross-language\s+)?conformance\s+suites?\b/gi)) {
        ...
        if (Number(match[1]) !== expected)
            findings.push(finding(file, text, match, `${scope} conformance suite count is ${expected}`));
    }
    const countPairs = [
    ...
    for (const rule of countPairs) {
        for (const match of text.matchAll(rule)) {
            ...
            if (/\b(?:historical|legacy|signed statement|bound to (?:commit|its input)|input set)\b/i.test(nearby))
                continue;
            const external = isExternalBaselineClaim(text, match.index);
            const expectedPairSuites = external ? externalSuites : suites;
            const expectedPairVectors = external ? externalVectors : vectors;
            const scope = external ? 'externally pinned' : 'current';
            if (Number(match[1]) !== expectedPairSuites)
                findings.push(finding(file, text, match, `${scope} conformance suite count is ${expectedPairSuites}`));
            if (Number(match[2]) !== expectedPairVectors)
                findings.push(finding(file, text, match, `${scope} conformance vector count is ${expectedPairVectors}`));
        }
    }
    for (const match of text.matchAll(/\ball\s+(\d+)(?:\/\d+)?\s+(?:published\s+|current\s+)?vectors?\b/gi)) {
        ...
        if (/\b(?:legacy|predates|signed statement)\b/i.test(nearby))
            continue;
        const external = isExternalBaselineClaim(text, match.index);
        const expectedCount = external ? externalVectors : vectors;
        const scope = external ? 'externally pinned' : 'current';
        if (Number(match[1]) !== expectedCount)
            findings.push(finding(file, text, match, `${scope} conformance vector count is ${expectedCount}`));
    }
    ...
    const floorWordBefore = (idx) => /\b(?:over|at least|more than|at minimum|minimum of|upwards of|north of)\s+$/i.test(text.slice(Math.max(0, idx - 24), idx));
    const countOk = (n, plusSign, numIdx, actual) => (plusSign === '+' || floorWordBefore(numIdx)) ? actual >= n : actual === n;
    for (const match of text.matchAll(/\b(\d{3,}|\d{1,3}(?:,\d{3})+)(\+?)\*{0,2}\s+automated\s+test(?:s|\s+cases)\b([^\n]{0,80})/gi)) {
        ...
        if (!countOk(number(match[1]), match[2], numIdx, tests))
            findings.push(finding(file, text, match, `current automated-test case count is ${tests}`));
        ...
        if (files) {
            ...
            if (!countOk(number(files[1]), files[2], fileIdx, testFiles))
                findings.push(finding(file, text, match, `current automated-test file count is ${testFiles}`));
        }
    }
    for (const match of text.matchAll(/\bautomated\s+test(?:s|\s+cases)\s*\|\s*(\d{3,}|\d{1,3}(?:,\d{3})+)(\+?)\s+across\s+(\d+)(\+?)\s+files\b/gi)) {
        ...
        if (!countOk(number(match[3]), match[4], fileIdx, testFiles))
            findings.push(finding(file, text, match, `current automated-test file count is ${testFiles}`));
    }
...
if (isMain()) {
    if (manifest?.['@version'] !== 'EP-CONFORMANCE-MANIFEST-v1')
        throw new Error('unsupported conformance manifest');
    if (pin?.['@version'] !== 'EP-EXTERNAL-IMPLEMENTATION-PIN-v1')
        throw new Error('unsupported external implementation pin');
    const findings = auditRepository();
    if (findings.length) {
        console.error(`PUBLIC CONFORMANCE CLAIMS: FAIL (${findings.length} finding(s))`);
        for (const item of findings)
            console.error(`${item.file}:${item.line}: ${item.message}: ${JSON.stringify(item.match)}`);
        process.exitCode = 1;
    }
    else {
