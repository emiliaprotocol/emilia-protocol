...
// Generated from check-preprint-sync.mts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
...
export function normalizeExtractedText(text) {
    return text
        .normalize('NFKC')
        .replace(/[\uFB00-\uFB06]/g, (value) => LIGATURES.get(value) ?? value)
        .replace(/\s+/g, ' ')
        .trim();
}
...
function currentDraftRevision(standardsStatus, draft) {
    const entry = standardsStatus.active_datatracker?.find((item) => item.draft === draft);
    if (!entry || !/^\d{2}$/.test(entry.revision)) {
        throw new Error(`standards/STATUS.json has no two-digit active revision for ${draft}`);
    }
    return entry.revision;
}
export function deriveEvidence({ canonicalMarkdown, manifest, proofStats, proofStatus, standardsStatus, external, }) {
    const failures = [];
    const suiteCount = manifest.suites?.length;
    const vectorCount = manifest.suites?.reduce((total, suite) => total + suite.vectors, 0);
    if (suiteCount !== manifest.totals?.suites) {
        failures.push(`conformance manifest lists ${suiteCount} suites but totals.suites is ${manifest.totals?.suites}`);
    }
    if (vectorCount !== manifest.totals?.vectors) {
        failures.push(`conformance manifest sums to ${vectorCount} vectors but totals.vectors is ${manifest.totals?.vectors}`);
    }
...
function requireMatch(failures, text, regex, message) {
    if (!regex.test(text)) {
        failures.push(message);
    }
}
function auditConformanceClaims(failures, text, label, evidence, includeHeaderClaim) {
    ...
    for (const pattern of claimPatterns) {
        ...
        for (const match of matches) {
            if (Number(match[1]) !== suites || Number(match[2]) !== vectors) {
                failures.push(`${label} claims ${match[1]} suites / ${match[2]} vectors; evidence is ${suites} / ${vectors}`);
            }
        }
    }
    ...
    for (const match of agreementClaims) {
        if (Number(match[1]) !== vectors) {
            failures.push(`${label} claims agreement across ${match[1]} vectors; evidence is ${vectors}`);
        }
    }
    if (includeHeaderClaim) {
        ...
        for (const match of headerClaims) {
            if (Number(match[1]) !== suites || Number(match[2]) !== vectors) {
                failures.push(`${label} header claims ${match[1]} suites / ${match[2]} vectors; evidence is ${suites} / ${vectors}`);
            }
        }
    }
}
function auditDraftClaims(failures, text, label, drafts) {
    for (const [draft, expectedRevision] of Object.entries(drafts)) {
        ...
        for (const revision of new Set(matches)) {
            if (revision !== expectedRevision) {
                failures.push(`${label} cites stale ${draft}-${revision}; evidence is ${draft}-${expectedRevision}`);
            }
        }
    }
}
function auditComposedTamarinBlock(failures, text, label, evidence) {
    ...
    if (verified !== evidence.tamarin.verified || falsified !== evidence.tamarin.falsified) {
        failures.push(`${label} composed Tamarin block is ${verified} verified / ${falsified} falsified; evidence is ${evidence.tamarin.verified} / ${evidence.tamarin.falsified}`);
    }
}
function auditFormalAndExternalClaims(failures, text, label, evidence) {
    const { tla, alloy, external } = evidence;
    requireMatch(failures, text, new RegExp(`${escapeRegex(tla.states)} states \\(${escapeRegex(tla.distinctStates)} distinct\\).*?${tla.invariants} invariants`), `${label} does not carry the current TLA+ state and invariant counts`);
    requireMatch(failures, text, new RegExp(`four Alloy models \\(${escapeRegex(alloy.version)}, SAT4J\\), with ${alloy.assertions} assertions`), `${label} does not carry the current Alloy version and assertion count`);
    requireMatch(failures, text, new RegExp(`pinned ${external.suites}-suite\\/${external.vectors}-vector clean-room bundle`), `${label} does not carry the pinned external verifier corpus`);
    requireMatch(failures, text, new RegExp(`${external.structuredCases} structured attacks plus ${external.rawParserCases} raw-parser refusals`), `${label} does not carry the external hostility split`);
    requireMatch(failures, text, new RegExp(`${external.hostilityCases}-case hostility campaign`), `${label} does not carry the aggregate external hostility count`);
...
export function auditPreprintClaims({ tex, pdfText, staging, evidence }) {
    ...
    const source = normalizeExtractedText(tex);
    const pdf = normalizeExtractedText(pdfText);
    const stagingText = normalizeExtractedText(staging);
    ...
    requireMatch(failures, stagingText, new RegExp(`Conformance ${evidence.conformance.suites} suites \\/ ${evidence.conformance.vectors} vectors`, 'i'), 'STAGING.md does not carry the current conformance totals');
    requireMatch(failures, stagingText, new RegExp(`${evidence.tamarin.verified} composed obligations \\+ ${evidence.tamarin.falsified} deliberate falsifications`), 'STAGING.md does not carry the current composed Tamarin totals');
    requireMatch(failures, stagingText, new RegExp(`${escapeRegex(evidence.tla.states)} states \\/ ${evidence.tla.invariants} invariants`), 'STAGING.md does not carry the current TLA+ totals');
    requireMatch(failures, stagingText, new RegExp(`${evidence.alloy.assertions} assertions across four CI-gated models.*?${escapeRegex(evidence.alloy.version)}`), 'STAGING.md does not carry the current Alloy totals and version');
    requireMatch(failures, stagingText, new RegExp(`Rust external verifier \\/ ${evidence.external.vectors} vectors \\/ ${evidence.external.hostilityCases} hostility cases`), 'STAGING.md does not carry the current external verifier totals');
...
export function extractPdfText(pdfPath) {
    ...
    if (result.status !== 0) {
        throw new Error(`pdftotext failed (${result.status}): ${result.stderr.trim()}`);
    }
    return result.stdout;
}
export function checkRepository(root) {
    const canonicalMarkdown = readFileSync(resolve(root, 'papers/authorization-receipts-preprint.md'), 'utf8');
...
if (invokedPath === fileURLToPath(import.meta.url)) {
    ...
    try {
        ...
        if (failures.length > 0) {
            ...
            console.log(`PREPRINT SYNC: OK (${evidence.conformance.suites} suites / ${evidence.conformance.vectors} vectors; source, PDF, staging, and evidence agree)`);
        }
    }
    catch (error) {
        console.error(`PREPRINT SYNC: FAIL\n- ${error.message}`);
