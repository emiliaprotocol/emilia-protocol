...
for (const required of [
  ...
  'AE-CHALLENGE neither defines nor implies that protocol',
  'Future Work',
  ...
  'finite reissue budget bound at least to the authenticated issuer',
  'A fresh challenge identifier or nonce does not reset the automatic',
  'Capacity policy therefore distinguishes initial issuance, presentation claim, and',
  'MUST NOT relabel an automatic',
  'does not permanently prohibit a new',
  'MUST NOT rely on <tt>retry_timing</tt> as an enforcement control',
...
for (const required of [
  ...
  'finite reissue budget bound at least to the authenticated issuer',
  'fresh challenge identifier or nonce does not reset the automatic',
  'Capacity policy therefore distinguishes initial issuance, presentation claim, and',
  'MUST NOT relabel an automatic',
  'does not permanently prohibit a new',
  'MUST NOT rely on retry_timing as an enforcement control',
...
for (const required of [
  ...
  'retained_units',
  'capacity_limits',
  "ChallengeCapacityPhase = 'issuance' | 'claim' | 'followup'",
  "'followup'",
