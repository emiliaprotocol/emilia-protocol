...
const pubB64url = (publicKey.export({ type: 'spki', format: 'der' }) as Buffer).toString('base64url');
...
interface Payload {
  ...
  claim: {
    ...
    outcome: string;
    approver: string;
    context: { amount: number; destination: string; currency: string };
  };
}
...
const sigValue = (crypto.sign(null, Buffer.from(canonicalize(payload as unknown), 'utf8'), privateKey) as Buffer).toString('base64url');
...
const doc: ReceiptDoc = {
  ...
  anchor: buildReceiptAnchorV2(payload as any),
};
...
const jsResult = verifyReceipt(doc as any, pubB64url) as VerifyResult;
if (!jsResult.valid) {
