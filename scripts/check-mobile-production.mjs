...
const assetLinkPins = required('MOBILE_ANDROID_ASSETLINKS_CERT_SHA256')
  .split(',').map((item) => item.trim().toUpperCase()).filter(Boolean);
assert(assetLinkPins.length > 0, 'Android Digital Asset Links signing pins are configured');
...
for (const [table, column] of [
  ['mobile_kv_state', 'state_key'],
  ['mobile_pairings', 'code_hash'],
  ['mobile_sessions', 'session_id'],
  ['mobile_enrollments', 'device_key_id'],
  ['mobile_counters', 'counter_key'],
  ['mobile_audit_records', 'record_id'],
  ['mobile_evidence_records', 'record_id'],
  ['mobile_actions', 'action_reference'],
  ['mobile_action_challenges', 'challenge_id'],
]) {
...
assert(assetLinkPins.every((pin) => servedPins.includes(pin)), 'Digital Asset Links serves every pinned Android signing certificate');
