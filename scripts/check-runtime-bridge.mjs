...
// Generated from check-runtime-bridge.mts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
import { readFileSync } from 'node:fs';
import { FORMAL_RUNTIME_CONFIG, FORMAL_RUNTIME_INVARIANT_MAP, } from '../packages/gate/formal-runtime-map.js';
...
for (const item of FORMAL_RUNTIME_INVARIANT_MAP) {
    if (!configured.has(item.formal))
        errors.push(`${item.formal} is not declared in ${FORMAL_RUNTIME_CONFIG}`);
}
for (const invariant of Object.values(RUNTIME_INVARIANTS)) {
    if (!mappedRuntime.has(invariant))
        errors.push(`${invariant} has no formal source mapping`);
}
if (mappedFormal.size !== FORMAL_RUNTIME_INVARIANT_MAP.length)
    errors.push('formal runtime map contains duplicate theorem names');
if (errors.length) {
