...
const root = new URL('../standards/staged/NEXT-BOUNDED-CAPABILITY-03/', import.meta.url);
const basename = 'draft-schrock-ep-bounded-capability-receipts-03';

function invariant(condition, message) {
  if (!condition) throw new Error(`Bounded Capability -03 packet: ${message}`);
}
...
invariant(xml.includes(`docName="${basename}"`), 'docName must identify -03');
...
for (const required of [
  ...
  invariant(xmlText.includes(required), `XML is missing required -03 text: ${required}`);
}
for (const required of [
  'revocation_mode',
  'direct or cascade',
...
console.log('Bounded Capability -03: source, renders, implementation boundary, metadata, and checksums PASS.');
