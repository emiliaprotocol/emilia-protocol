...
const BASE: string = process.env.EP_LOCAL_BASE || 'http://localhost:3000';
...
async function post(path: string, body: any, headers: { [key: string]: string } = {}): Promise<{ ok: boolean; status: number; json: any }> {
...
async function main(): Promise<void> {
  ...
  const apiKey: string = `ep_live_${crypto.randomBytes(32).toString('hex')}`;
  const apiKeyHash: string = sha256Hex(apiKey);
  const entityId: string = `realdevice-initiator-${RUN}`;
  ...
  const approverId: string = `ep:approver:realdevice-${RUN}`;
  const signoffUrl: string = `${BASE}/signoff/${req.json.signoff_id}?approver=${encodeURIComponent(approverId)}`;
