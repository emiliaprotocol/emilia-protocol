...
// Generated from build-standards-observatory.mts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
...
for (const event of catalog.events) {
    for (const sourceId of event.source_ids || [])
        assert(sourceIds.includes(sourceId), `${event.id}: unknown source ${sourceId}`);
}
for (const conflict of catalog.operative_conflicts || []) {
    for (const sourceId of conflict.source_ids || [])
        assert(sourceIds.includes(sourceId), `${conflict.id}: unknown source ${sourceId}`);
}
