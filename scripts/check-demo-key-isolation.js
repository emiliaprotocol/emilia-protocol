#!/usr/bin/env node
// Generated from check-demo-key-isolation.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
/**
...
function walk(dirOrFile, acc = []) {
    ...
    for (const entry of fs.readdirSync(abs)) {
        if (entry === 'node_modules' || entry === '.next' || entry === 'dist' || entry === '.vercel')
            continue;
        if (entry.startsWith('.'))
            continue;
