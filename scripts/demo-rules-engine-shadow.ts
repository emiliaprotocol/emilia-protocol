...
interface ScenarioBody {
  ...
  prior_changes_target_30d?: number;
  risk_flags?: string[];
  velocity_same_actor_24h?: number;
  prior_denials_actor_30d?: number;
  actor_role?: string;
...
const ENFORCEMENT_MODES = { OBSERVE: 'observe', WARN: 'warn', ENFORCE: 'enforce' } as const;

// Mirrors the adapter inside app/api/v1/trust-receipts/route.js
interface RulesEngineInputParams {
  body: ScenarioBody;
  actor_id: string;
  mode: string;
}

function buildRulesEngineInput({ body, actor_id, mode }: RulesEngineInputParams): any {
  return {
...
const actor_id: string = 'ap_demo_actor_alice';
const mode: string = ENFORCEMENT_MODES.ENFORCE;
...
for (const scenario of SCENARIOS) {
  ...
  const input: any = buildRulesEngineInput({ body: scenario.body, actor_id, mode });
  const result: any = evaluateAction(input);
