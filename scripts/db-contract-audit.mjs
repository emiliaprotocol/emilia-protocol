...
// Generated from db-contract-audit.mts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
//
...
const aclServiceRole = (acl) => /(^|[,{])service_role=[A-Za-z]*X/.test(acl);
...
export function evaluateContract(snap, schemaContract = defaultContract) {
    ...
    for (const assertion of schemaContract.requiredReconcileAssertions || []) {
        if (qualifiedFunctions.has(assertion))
            pass();
        else
            fail(`RECONCILIATION SECURITY ASSERTION failed: ${assertion}`);
    }
    ...
    for (const table of schemaContract.rlsRequired) {
        if (!tables.has(table)) {
            fail(`RLS-required table missing: ${table}`);
            continue;
        }
        if (rls.get(table)?.enabled === true)
            pass();
        else
    ...
    for (const table of schemaContract.forceRlsRequired || []) {
        if (!tables.has(table)) {
            fail(`FORCE-RLS-required table missing: ${table}`);
            continue;
        }
        if (rls.get(table)?.forced === true)
            pass();
        else
            fail(`RLS NOT forced: ${table}`);
    }
    // 5. No anon/authenticated/PUBLIC READ policy on sensitive tables
    for (const table of schemaContract.noAnonRead) {
        const bad = (policiesByTable.get(table) || []).filter((policy) => READ_CMDS.has(policy.cmd) && hasUntrustedRole(policy.roles));
        if (bad.length === 0)
            pass();
        else
            fail(`ANON-READ exposure on ${table}: ${bad.map((p) => `${p.name}[${p.cmd}→${(p.roles || []).join(',')}]`).join('; ')}`);
    }
    // 6. No anon/authenticated/PUBLIC WRITE policy on sensitive tables
    for (const table of schemaContract.noAnonWrite) {
        const bad = (policiesByTable.get(table) || []).filter((policy) => WRITE_CMDS.has(policy.cmd) && hasUntrustedRole(policy.roles));
        if (bad.length === 0)
            pass();
        else
            fail(`ANON-WRITE exposure on ${table}: ${bad.map((p) => `${p.name}[${p.cmd}→${(p.roles || []).join(',')}]`).join('; ')}`);
    }
    ...
    for (const table of schemaContract.tableGrantsNoPublic || []) {
        const bad = tableGrants.filter((grant) => grant.t === table
            && hasUntrustedRole([grant.grantee])
            && TABLE_PRIVILEGES.has(String(grant.privilege || '').toUpperCase()));
        if (bad.length === 0)
            pass();
        else
            fail(`PUBLIC TABLE GRANT on ${table}: ${bad.map((g) => `${g.grantee}.${g.privilege}`).join(', ')}`);
    }
    for (const table of schemaContract.tableGrantsNoServiceRoleDirect || []) {
        ...
        if (bad.length === 0)
            ...
            fail(`DIRECT SERVICE-ROLE TABLE GRANT on ${table}: ${bad.map((g) => g.privilege).join(', ')}`);
    }
    for (const table of schemaContract.tableWriteGrantsNoServiceRole || []) {
        ...
        if (bad.length === 0)
            ...
            fail(`DIRECT SERVICE-ROLE WRITE GRANT on ${table}: ${bad.map((g) => g.privilege).join(', ')}`);
    }
    ...
    for (const { table, column } of sensitiveColumns) {
        const bad = columnGrants.filter((grant) => grant.t === table && grant.c === column && hasUntrustedRole([grant.grantee]));
        if (bad.length === 0)
            pass();
        else
            fail(`PUBLIC COLUMN GRANT on ${table}.${column}: ${bad.map((g) => `${g.grantee}.${g.privilege}`).join(', ')}`);
    }
    ...
    for (const table of schemaContract.serviceRolePoliciesRequired || []) {
        ...
        if (good)
            ...
            fail(`SERVICE-ROLE POLICY missing or widened: ${table}`);
    }
    ...
    for (const name of schemaContract.definerRpcsServiceRoleOnly) {
        ...
        if (exposed.length === 0)
            ...
            fail(`RPC executable by untrusted role: ${name} (${exposed.length}/${overloads.length} overloads acl=${exposed.map((fn) => fn.acl).join(' | ')})`);
    }
    ...
    for (const signature of schemaContract.requiredDefinerRpcSignatures || []) {
        ...
        const overload = (fnsByName.get(name) || []).find((fn) => String(fn.args || '').replace(/\s+/g, '')
