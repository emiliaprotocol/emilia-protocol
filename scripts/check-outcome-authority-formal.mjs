...
// Generated from check-outcome-authority-formal.mts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
...
export function runOutcomeAuthorityFormalGate() {
    ...
    for (const [name, result] of Object.entries(formal.obligations)) {
        ...
        requireCondition(result.mutation_counterexample, `${name} has no mutation counterexample and may be vacuous`);
    }
    const cases = JSON.parse(fs.readFileSync(CASES_PATH, 'utf8'));
    for (const vector of cases.vectors ?? []) {
        const actual = evaluateFormalCase(vector);
        requireCondition(actual === vector.expect?.valid, `${vector.id}: model returned ${actual}; expected ${vector.expect?.valid}`);
    }
    ...
    for (const claimId of CLAIM_IDS) {
        ...
        for (const required of REQUIRED_EXCLUSIONS) {
            requireCondition(exclusions.includes(required), `${claimId} must explicitly exclude ${required}`);
        }
        const bounded = (claim.formal ?? []).find((entry) => entry.method === 'bounded_exhaustive_state_exploration');
        requireCondition(bounded?.status === 'partial', `${claimId} must mark bounded formal scope partial`);
        requireCondition(Array.isArray(bounded.obligations) && bounded.obligations.length > 0, `${claimId} must name exact bounded formal obligations`);
    }
    return {
        verified: true,
        claims: CLAIM_IDS,
...
if (invokedAsScript) {
    try {
        ...
        console.log(`OUTCOME/AUTHORITY FORMAL GATE: PASS `
