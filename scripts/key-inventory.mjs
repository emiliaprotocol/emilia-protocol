...
// Generated from key-inventory.mts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
//
...
const main = async () => {
    ...
    if (ke) {
        console.error('FATAL reading api_keys:', ke.message);
        process.exit(2);
    }
    ...
    for (const k of active) {
        if (!activeByEntity.has(k.entity_id))
            activeByEntity.set(k.entity_id, []);
        activeByEntity.get(k.entity_id).push(k);
    }
    const multiActive = [...activeByEntity.entries()].filter(([, ks]) => ks.length > 1);
    const neverUsedActive = active.filter((k) => !k.last_used_at);
    const orphanEntity = (keys || []).filter((k) => !entById.has(k.entity_id)); // key for a non-existent entity = red flag
    const byOrg = new Map();
    for (const k of active) {
    ...
    for (const [org, n] of [...byOrg.entries()].sort((a, b) => b[1] - a[1]))
        console.log(`      ${n.toString().padStart(4)}  ${org}`);
    ...
    for (const [eid, ks] of multiActive.slice(0, 25)) {
        ...
        for (const k of ks)
            console.log(`            ${k.key_prefix}… created=${k.created_at?.slice(0, 10)} (${days(k.created_at)}d) last_used=${k.last_used_at ? k.last_used_at.slice(0, 10) : 'NEVER'} label=${k.label || '-'}`);
    }
    console.log(`    keys for non-existent entity (orphans): ${orphanEntity.length}` + (orphanEntity.length ? '  ← RED FLAG' : '  ✓'));
    for (const k of orphanEntity.slice(0, 25))
        console.log(`        ${k.key_prefix}… entity_id=${k.entity_id} created=${k.created_at?.slice(0, 10)}`);
    // age distribution of active keys
    const ages = active.map((k) => days(k.created_at)).filter((d) => d != null);
    ages.sort((a, b) => a - b);
    if (ages.length) {
    ...
    const out = { generated_at: new Date().toISOString(), totals: { total: (keys || []).length, active: active.length, revoked: revoked.length, neverUsedActive: neverUsedActive.length }, multiActive: multiActive.map(([eid, ks]) => ({ entity_id: eid, count: ks.length })), orphanEntity: orphanEntity.map((k) => ({ id: k.id, entity_id: k.entity_id, key_prefix: k.key_prefix })) };
    const dump = process.env.KEY_INVENTORY_OUT;
    if (dump) {
