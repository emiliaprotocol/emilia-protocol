...
function pointerValue(root, fragment) {
  ...
  if (!fragment.startsWith('#/')) throw new SchemaError('unsupported schema fragment: ' + fragment);
  let value = root;
...
function resolveSchemaReference(reference, context) {
  ...
  if (!root) throw new SchemaError('unresolved schema reference: ' + reference);
  ...
  if (schema.type === 'array') {
    ...
    if (schema.uniqueItems) {
      ...
      for (const child of value) {
        ...
        if (seen.has(key)) throw new SchemaError(valuePath + ': duplicate array item');
        seen.add(key);
      }
    }
    if (schema.items) {
...
export async function loadRefereePack(manifestPath) {
  ...
  for (const [name, schema] of Object.entries(loaded)) {
    ...
    if (schema.$id) store.set(schema.$id, schema);
  }
...
export function sanitizedChildEnvironment(source = process.env) {
  ...
  if (source.SystemRoot) result.SystemRoot = source.SystemRoot;
  return result;
}
...
export async function digestExecutable(executable) {
  ...
  const stat = await lstat(resolved);
  if (!stat.isFile()) throw new RefereeError('COMMAND_FAILED', 'executable is not a regular file');
...
async function invokeExternal(options) {
  ...
  return new Promise((resolve) => {
    ...
    const finish = (code) => {
      ...
      if (!errorCode && code !== 0) errorCode = 'COMMAND_FAILED';
      resolve({
        error: errorCode,
    ...
    child.stderr.on('data', (chunk) => {
      ...
      if (stderrBytes > options.limits.max_stderr_bytes) failAndKill('STDERR_LIMIT');
    });
    child.once('error', () => {
...
export async function runExternalCase(options) {
  ...
  for (let run = 0; run < options.limits.deterministic_runs; run += 1) {
    ...
    if (invocation.error) return emptyRow(options.fixture, invocation.error);
...
async function verifyCheckedInEntrypoint(entrypoint, commandArgv, workspace) {
  ...
  const present = commandArgv.slice(0, 2).some((argument) => {
    if (path.isAbsolute(argument)) return path.resolve(argument) === absolute;
...
function takeOption(argv, index, name) {
  if (index + 1 >= argv.length) throw new RefereeError('USAGE', name + ' requires a value');
  return argv[index + 1];
}
...
function oneSource(direct, environmentName, label) {
  ...
  if (!value) throw new RefereeError('USAGE', label + ' is required');
  return value;
}
...
export async function cliMain(argv = process.argv.slice(2)) {
  ...
  const executableSha256 = oneSource(
    cli.executable_sha256,
    cli.executable_sha256_env,
    '--executable-sha256',
  );
  ...
  if (report.summary.outcome !== 'PASS') process.exitCode = 1;
}
