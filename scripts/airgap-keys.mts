...
const secret: string = process.env.SUPABASE_JWT_SECRET || crypto.randomBytes(32).toString('hex');
...
const serviceRole: string = await sign('service_role');
const anon: string = await sign('anon');
