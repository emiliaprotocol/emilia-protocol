...
export const sha256 = (bytes: Buffer | string): string => crypto.createHash('sha256').update(bytes).digest('hex');
...
function strictJson(bytes: Buffer | string, label: string): Record<string, any> {
  const input: Buffer = Buffer.isBuffer(bytes) ? bytes : Buffer.from(bytes);
  let text: string;
  try {
  ...
  try {
    ...
    fail(`${label}: ${(error as any).message}`);
  }
}
...
function without(object: Record<string, any>, field: string): Record<string, any> {
  const clone: Record<string, any> = structuredClone(object);
  delete clone[field];
  return clone;
}

function verifyCanonicalSelfHash(object: Record<string, any>, field: string, label: string): void {
...
function verifyInsertionOrderSelfHash(object: Record<string, any>, field: string, label: string): void {
  assert(SHA256.test(object[field] || ''), `${label} has no valid ${field}`);
  const actual = sha256(Buffer.from(JSON.stringify(without(object, field)), 'utf8'));
  assert(actual === object[field], `${label} ${field} mismatch`);
}

function validateBaseManifest(manifest: Record<string, any>): void {
...
function validatePin(pin: Record<string, any>): void {
...
function validateExternalConformance(report: Record<string, any>, reportBytes: Buffer, pin: Record<string, any>, pinBytes: Buffer, manifest: Record<string, any>, expectedEvaluatorCommit: string): string {
  ...
  const expectedSuites: Map<any, any> = new Map(manifest.suites.map((suite: any) => [suite.path, suite]));
  const seen: Set<any> = new Set();
  for (const suite of report.suites) {
    const expected: any = expectedSuites.get(suite?.path);
    assert((expected as any) && !seen.has(suite.path), 'external suite evidence contains an unknown or duplicate suite');
    seen.add(suite.path);
    assert(suite.status === 'pass', `external suite ${suite.path} did not pass`);
    assert((suite.sha256 as string) === (expected as any).sha256 && (suite.vectors as number) === (expected as any).vectors, `external suite ${suite.path} drifted from base manifest`);
  }
  return sha256(reportBytes);
}

function validateHostility(report: Record<string, any>, reportBytes: Buffer, pin: Record<string, any>, expectedRunnerHash: string): string {
...
export function assembleConformanceCase({
  ...
  pinBytes: Buffer | string;
  externalConformanceBytes: Buffer | string;
  externalHostilityBytes: Buffer | string;
  expectedEvaluatorCommit: string;
}): Record<string, any> {
  ...
  const strictAccepted: boolean = false;
  ...
  const result: Record<string, any> = {
    ...
    external_implementations: [{
      ...
      construction_evidence: {
        ...
        strict_clean_room_verdict: (strictAccepted as boolean) ? 'accepted' : 'refused',
    ...
    totals: {
      ...
      strict_external_clean_room_acceptances: (strictAccepted as boolean) ? 1 : 0,
...
if (isMain()) {
  ...
  const manifestPath: string | null = option(argv, '--manifest');
  const pinPath: string | null = option(argv, '--pin');
  const externalConformancePath: string | null = option(argv, '--external-conformance');
  const externalHostilityPath: string | null = option(argv, '--external-hostility');
  const evaluatorCommit: string | null = option(argv, '--evaluator-commit');
  const emitPath: string | null = option(argv, '--emit');
  ...
  const result = assembleConformanceCase({
    manifestBytes: fs.readFileSync(path.resolve(ROOT, manifestPath as string)),
    pinBytes: fs.readFileSync(path.resolve(ROOT, pinPath as string)),
    externalConformanceBytes: fs.readFileSync(path.resolve(ROOT, externalConformancePath as string)),
    externalHostilityBytes: fs.readFileSync(path.resolve(ROOT, externalHostilityPath as string)),
    expectedEvaluatorCommit: evaluatorCommit as string,
  });
  const target: string = path.resolve(ROOT, emitPath as string);
