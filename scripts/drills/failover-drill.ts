...
const JSON_OUTPUT: boolean = args.includes('--json');
const TIMEOUT_MS_IDX: number = args.indexOf('--timeout-ms');
const DB_TIMEOUT_MS: number = TIMEOUT_MS_IDX !== -1
 ...
 */
function withSimulatedTimeout(promiseFn: () => Promise<any>, timeoutMs: number): Promise<any> {
 ...
 */
async function cleanup(supabase: SupabaseClient<any, 'public', any>, drillPrefix: string): Promise<void> {
...
async function testCreateFailover(): Promise<TestResult> {
  const testName: string = 'DB connection loss during handshake create';
  const start: number = Date.now();
  const supabase: SupabaseClient<any, 'public', any> = getClient();
  const drillId: string = `drill-create-${uuid().slice(0, 8)}`;
  ...
  try {
    ...
    const handshakeId: string = uuid();
    const result: any = await withSimulatedTimeout(async () => {
    ...
    let partialStateClean: boolean = true;
    let retryClean: boolean = true;
    ...
    if (result.error?.code === 'SIMULATED_TIMEOUT') {
      ...
      const { data: orphan }: { data: any } = await supabase
        .from('handshakes')
...
async function testConsumeFailover(): Promise<TestResult> {
  const testName: string = 'DB connection loss during consume';
  const start: number = Date.now();
  const supabase: SupabaseClient<any, 'public', any> = getClient();
  const drillId: string = `drill-consume-${uuid().slice(0, 8)}`;
  ...
  try {
    ...
    const handshakeId: string = uuid();
    const bindingHash: string = crypto.createHash('sha256')
    ...
    let recoverableState: boolean = true;
    let retryCorrect: boolean = true;

    // Wait for in-flight write
    await new Promise((r: (value: void) => void) => setTimeout(r, 1000));

    // Check: event should exist regardless
    const { data: eventCheck }: { data: any } = await supabase
      .from('handshake_events')
      .select('event_id')
      .eq('handshake_id', handshakeId)
      .eq('event_type', 'handshake_verified');

    const eventExists: boolean = eventCheck && eventCheck.length > 0;
    log(`    Event exists: ${eventExists}`);

    // Check: consumption may or may not exist depending on timeout
    const { data: consumptionCheck }: { data: any } = await supabase
      .from('handshake_consumptions')
      .select('consumption_id')
      .eq('handshake_id', handshakeId);

    const consumptionExists: boolean = consumptionCheck && consumptionCheck.length > 0;
    ...
    const retryConsumptionId: string = uuid();
    const { data: retryData, error: retryError }: { data: any; error: any } = await supabase
      .from('handshake_consumptions')
      .insert({
    ...
    const { data: finalCount }: { data: any } = await supabase
      .from('handshake_consumptions')
      .select('consumption_id')
      .eq('handshake_id', handshakeId);

    const consumptionCount: number = finalCount ? finalCount.length : 0;
    const noDoubleConsume: boolean = consumptionCount <= 1;
...
async function testEventWriteFailure(): Promise<TestResult> {
  const testName: string = 'Event write failure blocks mutation';
  const start: number = Date.now();
  const supabase: SupabaseClient<any, 'public', any> = getClient();
  const drillId: string = `drill-eventfail-${uuid().slice(0, 8)}`;
  ...
  try {
    ...
    const { error: eventError }: { error: any } = await supabase
    ...
    const eventWriteFailed: boolean = !!eventError;
    ...
    const { data: orphanHandshake }: { data: any } = await supabase
      .from('handshakes')
      .select('handshake_id')
      .eq('handshake_id', handshakeId)
      .maybeSingle();

    const noStateChange: boolean = !orphanHandshake;
    log(`    No handshake state created: ${noStateChange} (expected: true)`);

    // Verify: no event was persisted
    const { data: orphanEvent }: { data: any } = await supabase
      .from('protocol_events')
      .select('event_id')
      .eq('aggregate_id', handshakeId)
      .maybeSingle();

    const noEventPersisted: boolean = !orphanEvent;
...
async function main(): Promise<void> {
  ...
  const allPassed: boolean = results.every((r: TestResult) => r.passed);
  ...
  if (JSON_OUTPUT) {
    ...
    for (const r of results) {
      const status: string = r.passed ? 'PASS' : 'FAIL';
