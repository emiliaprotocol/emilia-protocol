...
import { existsSync, linkSync, lstatSync, mkdirSync, mkdtempSync, readdirSync, readFileSync, realpathSync, renameSync, rmSync, writeFileSync, } from "node:fs";
...
function readLockOwner(participantPath) {
    try {
        const parsed = JSON.parse(readFileSync(join(participantPath, "owner.json"), "utf8"));
        const token = participantPath.slice(participantPath.lastIndexOf("-") + 1);
        if (parsed.version !== PROOF_STATS_LOCK_VERSION ||
            parsed.token !== token ||
            ...
            (parsed.ticket !== null &&
                (!Number.isSafeInteger(parsed.ticket) || parsed.ticket < 1))) {
            return null;
        }
        return parsed;
    }
...
function writeLockOwner(participantPath, owner) {
    const ownerPath = join(participantPath, "owner.json");
    const temporaryPath = join(participantPath, `.owner-${randomBytes(8).toString("hex")}.tmp`);
...
export function resolveProofStatsLockQueue(cwd = process.cwd()) {
    ...
    if (commonDirectory.error || commonDirectory.status !== 0) {
        throw new Error(`proof-stats run lock could not resolve the shared Git directory: ${commonDirectory.error?.message || commonDirectory.stderr || "unknown error"}`);
    }
    ...
    const hostScope = createHash("sha256")
        ...
        .digest("hex")
        .slice(0, 16);
    return join(absoluteCommonDirectory, "emilia-run-locks", hostScope, "proof-stats-run-v1");
}
export function acquireProofStatsRunLock({ cwd = process.cwd(), timeoutMs = PROOF_STATS_LOCK_TIMEOUT_MS, pollMs, } = {}) {
    const effectivePollMs = pollMs ?? Math.min(PROOF_STATS_LOCK_POLL_MS, timeoutMs);
    if (!Number.isSafeInteger(timeoutMs) || timeoutMs < 1) {
        throw new Error("proof-stats run lock timeout must be a positive integer");
    }
    if (!Number.isSafeInteger(effectivePollMs) ||
        effectivePollMs < 1 ||
        effectivePollMs > timeoutMs) {
        throw new Error("proof-stats run lock poll interval must be a positive integer no greater than its timeout");
    }
    ...
    const versionStagingPath = join(queuePath, `.version-${randomBytes(16).toString("hex")}.tmp`);
    ...
    try {
        ...
        for (const participant of lockParticipants(queuePath)) {
            if (participant === participantPath)
                continue;
            ...
            if (contender?.ticket)
                maxTicket = Math.max(maxTicket, contender.ticket);
        }
        ...
        throw new Error(`proof-stats run lock timed out after ${timeoutMs}ms; ${blocker}`);
    }
    catch (error) {
        releaseEntry();
        throw error;
 ...
 */
export const ADVISORY_PROOF_STATS_FIELDS = Object.freeze([
    "tests.files",
    "tests.total",
]);
 ...
 * stableGeneratedAt).
 */
export function proofStatsDriftFields(measured, recorded, prefix = "") {
 ...
 * starts from.
 */
export function proofStatsFileText(stats, previousText) {
    let previous = null;
    ...
    const generatedAt = stableGeneratedAt(stats, previous);
...
function generateProofStats() {
    ...
    const driftReportIndex = process.argv.indexOf("--drift-report");
    const driftReport = driftReportIndex === -1 ? undefined : process.argv[driftReportIndex + 1];
    if (driftReportIndex !== -1) {
        if (!check)
            throw new Error("drift-report is check-mode only");
    ...
    if (securityCasePreverified) {
        ...
        const expectedSha = process.env.SECURITY_CASE_PREVERIFIED_SHA || "";
    ...
    if (bootstrapDerivedEvidence) {
        ...
        const recorded = JSON.parse(readFileSync("lib/proof-stats.json", "utf8"));
        if (!Number.isSafeInteger(recorded.tests?.total) ||
        ...
        j = JSON.parse(readFileSync(reportPath, "utf8"));
    ...
    if (check) {
        ...
        const recorded = isPlainObject(current) ? { ...current } : {};
        ...
        const fields = isPlainObject(current)
            ? proofStatsDriftFields(measured, recorded)
            : ["(root)"];
        if (!matches && fields.length === 0)
            fields.push("(root)");
        // Deny by default: anything outside the measured test counts.
        const denied = fields.filter((field) => !ADVISORY_PROOF_STATS_FIELDS.includes(field));
        if (driftReport) {
        ...
        if (!matches) {
            const lagOnly = Boolean(driftReport) && denied.length === 0;
            console.error(lagOnly
            ...
            if (!lagOnly) {
                ...
                console.error("(Docs state the count as a floor, so no doc edits are needed — only the generated files.)");
                process.exitCode = 1;
            }
        }
        else {
        ...
        const previousText = existsSync("lib/proof-stats.json")
            ? readFileSync("lib/proof-stats.json", "utf8")
            : undefined;
        const text = proofStatsFileText(stats, previousText);
 ...
 */
export function deriveSourceProofStats(root = process.cwd()) {
    const sourcePath = (relative) => join(root, relative);
    const cfg = readFileSync(sourcePath("formal/ep_handshake.cfg"), "utf8");
    const composedLifecycleCfg = readFileSync(sourcePath("formal/ep_composed_trust_lifecycle.cfg"), "utf8");
    const als = readFileSync(sourcePath("formal/ep_relations.als"), "utf8");
    const fedAls = readFileSync(sourcePath("formal/ep_federation.als"), "utf8");
    const quorumAls = readFileSync(sourcePath("formal/ep_quorum.als"), "utf8");
    const delegationAls = readFileSync(sourcePath("formal/ep_delegation.als"), "utf8");
    const redTeam = readFileSync(sourcePath("docs/conformance/RED_TEAM_CASES.md"), "utf8");
    const tamarinSummary = readFileSync(sourcePath("formal/tamarin/results/ep_reliance_composed.summary.txt"), "utf8");
    const conformance = JSON.parse(readFileSync(sourcePath("conformance/conformance-manifest.json"), "utf8"));
    const external = JSON.parse(readFileSync(sourcePath("conformance/external/rust-cleanroom-jdieselny.v1.json"), "utf8"));
    const securityCase = JSON.parse(readFileSync(sourcePath("security/security-case.json"), "utf8"));
    const claimSource = JSON.parse(readFileSync(sourcePath("security/claims.v1.json"), "utf8"));
    const scenarioConformanceBytes = readFileSync(sourcePath("formal/results/formal-runtime-scenario-conformance.v2.json"));
    ...
    ].map((file) => createHash("sha256").update(readFileSync(sourcePath(file))).digest("hex"));
