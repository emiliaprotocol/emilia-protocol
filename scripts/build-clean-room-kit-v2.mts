...
export function collectCleanRoomKitV2Files(
  ref = 'HEAD',
  ...
  for (const [filePath, expectedHash] of expectedHashes) {
    ...
    if (actual !== expectedHash) {
      throw new Error(
        `v2 kit pin mismatch at ${commit}:${safe}: expected ${expectedHash}, got ${actual}`,
      );
    }
...
function buildArchiveCommit(
  ...
  return String(git(
    ['commit-tree', tree, '-m', `Source-free clean-room kit v2 from ${sourceCommit}`],
    { env: identity },
  )).trim();
}
...
export function buildCleanRoomKitV2({
  ref = 'HEAD',
  output,
}: {
  ref?: string;
  ...
  const target = path.resolve(
    output ?? path.join(ROOT, 'release-artifacts/emilia-clean-room-kit-v2.tar.gz'),
  );
...
if (process.argv[1]
  ...
  try {
    ...
    console.log(
      `CLEAN-ROOM KIT V2: PASS (${report.files.length} files; `
