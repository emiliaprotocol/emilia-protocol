...
function loadEnvFile(): void {
  const envPath: string = path.join(ROOT, '.env.local');
  ...
  for (const line of lines) {
    ...
    if (eqIdx === -1) continue;
    const key: string = trimmed.slice(0, eqIdx);
    let value: string = trimmed.slice(eqIdx + 1);
...
const SUPABASE_KEY: string | undefined = process.env.SUPABASE_SERVICE_ROLE_KEY;
...
function parseSinceArg(): string {
  ...
  let sinceRaw: string = '1h';
  const idx: number = args.indexOf('--since');
  if (idx !== -1 && args[idx + 1]) {
    sinceRaw = args[idx + 1];
  }

  const match: RegExpMatchArray | null = sinceRaw.match(/^(\d+)(m|h|d)$/);
  if (!match) {
  ...
  const ms: number = num * (unit === 'm' ? 60_000 : unit === 'h' ? 3_600_000 : 86_400_000);
 ...
 */
async function countRows(table: string, { column, value, timeColumn = 'created_at', since = SINCE }: { column?: string; value?: any; timeColumn?: string; since?: string } = {}): Promise<number> {
...
async function check1_handshakeVsCreatedEvents(): Promise<void> {
  const [handshakes, events]: number[] = await Promise.all([
    countRows('handshakes'),
    countRows('handshake_events', { column: 'event_type', value: 'handshake_created' }),
  ]);
  const pass: boolean = handshakes === events;
  record(
...
async function check2_bindingsVsHandshakes(): Promise<void> {
  const [bindings, handshakes]: number[] = await Promise.all([
    countRows('handshake_bindings', { timeColumn: 'bound_at' }),
    countRows('handshakes'),
  ]);
  const pass: boolean = bindings === handshakes;
  record(
...
async function check3_partiesVsHandshakes(): Promise<void> {
  // For mutual mode, expect 2x parties per handshake.
  // handshake_parties lacks created_at, so we filter via recent handshake IDs.
  const [mutualCount, totalHandshakes]: number[] = await Promise.all([
    countRows('handshakes', { column: 'mode', value: 'mutual' }),
    countRows('handshakes'),
  ]);

  const expectedMutualParties: number = mutualCount * 2;
  const nonMutual: number = totalHandshakes - mutualCount;
  const expectedMin: number = expectedMutualParties + nonMutual; // at least 1 party per non-mutual
  ...
  const ids: any[] = (recentHandshakeIds || []).map((r: any) => r.handshake_id);
  ...
  const pass: boolean = partyCount >= expectedMin && (mutualCount === 0 || partyCount === expectedMutualParties + nonMutual);
  record(
...
async function check4_verifiedVsAcceptedResults(): Promise<void> {
  const [verified, accepted]: number[] = await Promise.all([
  ...
  const pass: boolean = verified === accepted;
  record(
...
async function check5_challengesVsChallengeEvents(): Promise<void> {
  const [challenges, events]: number[] = await Promise.all([
    countRows('signoff_challenges', { timeColumn: 'issued_at' }),
    countRows('signoff_events', { column: 'event_type', value: 'challenge_issued' }),
  ]);
  const pass: boolean = challenges === events;
  record(
...
async function check6_attestationsVsApprovedEvents(): Promise<void> {
  const [attestations, events]: number[] = await Promise.all([
    countRows('signoff_attestations', { timeColumn: 'approved_at' }),
    countRows('signoff_events', { column: 'event_type', value: 'signoff_approved' }),
  ]);
  const pass: boolean = attestations === events;
  record(
...
async function check7_noDuplicateConsumptions(): Promise<void> {
  ...
  if (error) {
    ...
    const pass: boolean = dupes.length === 0;
    record(
  ...
  // RPC path
  const dupeCount: number = (data as { dupe_count?: number } | null)?.dupe_count ?? 0;
  const pass: boolean = dupeCount === 0;
  record(
    '7. No duplicate signoff consumptions',
    pass,
...
async function check8_orphanedBindings(): Promise<void> {
  ...
  let orphanCount: number = 0;
  const bindingHsIds: any[] = (orphans || []).map((b: any) => b.handshake_id);
  if (bindingHsIds.length > 0) {
    const uniqueIds: any[] = [...new Set(bindingHsIds)];
    for (let i: number = 0; i < uniqueIds.length; i += 500) {
      const chunk: any[] = uniqueIds.slice(i, i + 500);
      const { data: found, error: fErr } = await supabase
        .from('handshakes')
        .select('handshake_id')
        .in('handshake_id', chunk);
      if (fErr) throw new Error(`check8 lookup: ${fErr.message}`);
      const foundSet: Set<any> = new Set((found || []).map((r: any) => r.handshake_id));
  ...
  const pass: boolean = orphanCount === 0;
  record(
...
async function check9_partialTerminalStates(): Promise<void> {
  ...
  const verifiedIds: any[] = (verifiedHs || []).map((r: any) => r.handshake_id);
  let missingResults: number = 0;

  if (verifiedIds.length > 0) {
    for (let i: number = 0; i < verifiedIds.length; i += 500) {
      const chunk: any[] = verifiedIds.slice(i, i + 500);
      const { data: resultRows, error: rErr } = await supabase
        .from('handshake_results')
        .select('handshake_id')
        .in('handshake_id', chunk);
      if (rErr) throw new Error(`check9 results: ${rErr.message}`);
      const resultSet: Set<any> = new Set((resultRows || []).map((r: any) => r.handshake_id));
      for (const id of chunk) {
  ...
  const pass: boolean = missingResults === 0;
  record(
...
async function check10_missingRequiredEvents(): Promise<void> {
  // Handshakes in terminal states (verified, rejected, expired, revoked)
  // must have a matching event.
  const terminalStatuses: string[] = ['verified', 'rejected', 'expired', 'revoked'];
  ...
  for (const status of terminalStatuses) {
    ...
    const hsIds: any[] = (hsRows || []).map((r: any) => r.handshake_id);
    if (hsIds.length === 0) continue;

    let missingCount: number = 0;
    for (let i: number = 0; i < hsIds.length; i += 500) {
      const chunk: any[] = hsIds.slice(i, i + 500);
      ...
      const eventSet: Set<any> = new Set((eventRows || []).map((r: any) => r.handshake_id));
      for (const id of chunk) {
  ...
  const pass: boolean = totalMissing === 0;
  record(
...
async function check11_protocolEventsCount(): Promise<void> {
  const count: number = await countRows('protocol_events');
  // This check is informational — we report the count.
  // A zero count during a load test window would be suspicious.
  const pass: boolean = count > 0;
  record(
...
async function main(): Promise<void> {
  ...
  for (const check of checks) {
    try {
      ...
      const name: string = (check as any).name.replace(/^check\d+_/, '').replace(/_/g, ' ');
      record(name, false, `ERROR: ${(err as any).message}`);
    }
  }
  ...
  for (const r of results) {
    const stat: string = r.status === 'PASS' ? ' PASS' : ' FAIL';
    const check: string = r.name.padEnd(56).slice(0, 56);
    const detail: string = r.detail.padEnd(62).slice(0, 62);
    console.log(`│ ${stat} │ ${check} │ ${detail} │`);
  }
  ...
  const failed: number = results.filter((r) => r.status === 'FAIL').length;
  console.log('');
