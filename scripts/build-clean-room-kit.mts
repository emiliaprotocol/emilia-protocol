...
function git(args: string[], options: Record<string, any> = {}): any {
...
function assertSafePath(value: unknown): string {
  if (typeof value !== 'string' || value.length === 0 || value.includes('\\')) {
    throw new Error(`unsafe kit path: ${JSON.stringify(value)}`);
  }
  const normalized: string = path.posix.normalize(value);
...
function readAt(commit: string, filePath: string): Buffer {
  return git(['show', `${commit}:${assertSafePath(filePath)}`], { encoding: null }) as Buffer;
}

function parseJsonAt(commit: string, filePath: string): Record<string, any> {
  try {
    return JSON.parse((readAt(commit, filePath)).toString('utf8')) as Record<string, any>;
  } catch (error) {
    throw new Error(`cannot parse ${filePath} at ${commit}: ${(error as any).message}`);
  }
}

function verifyDeclaredFile(commit: string, entry: Record<string, any>, source: string): { filePath: string; sourcePath: string } {
...
export function collectCleanRoomKitFiles(ref: string = 'HEAD'): { commit: string; files: Array<{ path: string; sourcePath: string; mode: string; bytes: number; sha256: string }> } {
  const commit: string = (git(['rev-parse', '--verify', `${ref}^{commit}`], { encoding: 'utf8' }) as string).trim();
...
function buildArchiveCommit(sourceCommit: string, files: Array<{ path: string; sourcePath: string; mode: string }>, temporary: string): string {
  const indexPath: string = path.join(temporary, 'archive.index');
  const env: Record<string, string> = { ...process.env as Record<string, string>, GIT_INDEX_FILE: indexPath };
  git(['read-tree', '--empty'], { env, encoding: 'utf8' });
  for (const file of files) {
    const bytes: Buffer = readAt(sourceCommit, file.sourcePath);
    const oid: string = (git(['hash-object', '-w', '--stdin'], { input: bytes, encoding: 'utf8' }) as string).trim();
    git(['update-index', '--add', '--cacheinfo', `${file.mode},${oid},${file.path}`], { env, encoding: 'utf8' });
  }
  const tree: string = (git(['write-tree'], { env, encoding: 'utf8' }) as string).trim();
  const timestamp: string = (git(['show', '-s', '--format=%ct', sourceCommit], { encoding: 'utf8' }) as string).trim();
...
function archive(commit: string, paths: string[], output: string): void {
  git([
    'archive',
    '--format=tar.gz',
    `--prefix=${KIT_PREFIX}`,
...
function archiveFiles(output: string): string[] {
  const entries: string[] = (execFileSync('tar', ['-tzf', output], {
    encoding: 'utf8',
...
export function buildCleanRoomKit({ ref = 'HEAD', output }: { ref?: string; output?: string } = {}): Record<string, any> {
...
function parseArgs(argv: string[]): { ref?: string; output?: string } {
  const options: { ref?: string; output?: string } = {};
