...
const MONEY_PATTERNS: PatternConfig[] = [
...
const INFRA_PATTERNS: PatternConfig[] = [
...
function classifyCommand(cmd: string): any {
  for (const p of MONEY_PATTERNS) {
    if (p.re.test(cmd)) {
      ...
      const base: any = evaluateGuardPolicy({ actionType: GUARD_ACTION_TYPES.AI_AGENT_PAYMENT_ACTION, riskFlags: [] } as any);
...
function canonicalize(v: any): string {
  ...
  if (typeof v === 'object') return `{${Object.keys(v).sort().map((k: string) => JSON.stringify(k) + ':' + canonicalize(v[k])).join(',')}}`;
  return JSON.stringify(v);
}

// ── Stable agent-gate signing identity (generated once, gitignored) ───────────
function agentKey(): crypto.KeyObject {
  const dir: string = join(ROOT, '.emilia');
  const path: string = join(dir, 'agent-gate-key.json');
  if (existsSync(path)) {
    const jwk: any = JSON.parse(readFileSync(path, 'utf8'));
    return crypto.createPrivateKey({ key: jwk, format: 'jwk' });
  }
  const { privateKey }: { privateKey: crypto.KeyObject } = crypto.generateKeyPairSync('ed25519');
  if (!existsSync(dir)) mkdirSync(dir, { recursive: true });
  writeFileSync(path, JSON.stringify(privateKey.export({ format: 'jwk' })), { mode: 0o600 });
  return privateKey;
}

function signReceipt(verdict: any, subject: string, command: string): any {
  const priv: crypto.KeyObject = agentKey();
  ...
  const pub: string = crypto.createPublicKey(priv as any).export({ type: 'spki', format: 'der' }).toString('base64url');
  const payload: any = {
  ...
  const value: string = crypto.sign(null, Buffer.from(canonicalize(payload), 'utf8'), priv).toString('base64url');
...
// ── Main ──────────────────────────────────────────────────────────────────────
function blockedDecision(d: string): boolean {
  return d === GUARD_DECISIONS.ALLOW_WITH_SIGNOFF || d === GUARD_DECISIONS.DENY;
}

function failClosed(message: string, detail: string = ''): void {
...
async function main(): Promise<void> {
  ...
  const hookMode: boolean = process.argv.includes('--hook');
  let command: string | undefined = arg('--command');
  let subject: string = 'agent:local';
  let verdict: any;
  ...
  if (hookMode) {
    ...
    verdict = classifyCommand(command as string);
  ...
  const icon: string = verdict.decision === GUARD_DECISIONS.ALLOW ? '✅ ALLOW' : verdict.decision === GUARD_DECISIONS.DENY ? '⛔ DENY' : '✋ HOLD — human signoff required';
  const receipt: any = signReceipt(verdict, subject, (command || actionJson || '') as string);
  if (process.argv.includes('--json')) {
    ...
    process.stdout.write(
      ...
      verdict.reasons.map((r: string) => `  reason:   ${r}`).join('\n') + '\n' +
...
main().catch((err: any) => failClosed('Unhandled gate error.', String(err?.message ?? err)));
