...
const SECRET_KEYS: string[] = ['api_key_hash', 'private_key_encrypted', 'api_key'];
const hasSecret = (v: unknown): boolean => !!(v && typeof v === 'object' && SECRET_KEYS.some((k) => k in (v as Record<string, unknown>)));
const idOf = (v: unknown): string | null => (v && typeof v === 'object' ? ((v as any).entity_id || (v as any).id || null) : v as string);

const exposedEntities: Set<any> = new Set();
...
if (evErr) { console.error('audit_events scan failed:', evErr.message); process.exit(1); }

const evBad: Array<{ id: any; type: any; field: string }> = [];
for (const e of events || []) {
  const a: any = e.after_state || {};
  for (const f of ['actor_id', 'initiator_id', 'approver_id']) {
    if (hasSecret(a[f])) { evBad.push({ id: e.id, type: e.event_type, field: f }); exposedEntities.add(idOf(a[f])); }
  }
}
console.log(`audit_events: ${evBad.length} row(s) with a serialized entity (REPORT ONLY — append-only log).`);
for (const r of evBad) console.log(`  • ${r.type}.${r.field}  (id ${r.id})`);
if (evBad.length) {
...
if (credErr) { console.error('approver_credentials scan failed:', credErr.message); process.exit(1); }

const credBad: Array<{ id: any; approver_id: any; clean: string | null }> = [];
for (const c of creds || []) {
  let parsed: any = c.attested_by;
  if (typeof parsed === 'string') { try { parsed = JSON.parse(parsed); } catch { parsed = c.attested_by; } }
  if (hasSecret(parsed)) { credBad.push({ id: c.id, approver_id: c.approver_id, clean: idOf(parsed) }); exposedEntities.add(idOf(parsed)); }
}
...
if (exposedEntities.size === 0) {
  ...
  for (const id of exposedEntities) {
    ...
    const tag: string = /e2e|realdevice|test|throwaway/i.test(id || '') ? 'TEST (likely safe to ignore)' : 'REVIEW — rotate its API key if this is real';
