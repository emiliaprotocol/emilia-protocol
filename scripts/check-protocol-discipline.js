#!/usr/bin/env node
// Generated from check-protocol-discipline.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
/**
 ...
 */
function isGeneratedRuntimeCompanion(filePath) {
 ...
 */
function stripSourceExt(p) {
    return p.replace(/\.(js|mjs|ts|mts|tsx)$/, '');
}
function setHasByBase(set, filePath) {
    if (set.has(filePath))
        return true;
    const base = stripSourceExt(filePath);
    for (const entry of set) {
        if (stripSourceExt(entry) === base)
            return true;
    }
    return false;
}
function checkTrustTableWrites() {
    ...
    for (const filePath of routeFiles) {
        if (setHasByBase(TRUST_WRITE_ALLOWLIST, filePath))
            continue;
...
function checkEnvReads() {
    ...
    for (const filePath of files) {
        if (setHasByBase(ENV_ALLOWLIST, filePath))
            continue;
        const content = fs.readFileSync(filePath, 'utf-8');
        const lines = content.split('\n');
        for (let i = 0; i < lines.length; i++) {
...
function extractHandlerBodies(content) {
    ...
    for (const method of HTTP_METHODS) {
        ...
        const funcPattern = new RegExp(`export\\s+(async\\s+)?function\\s+${method}\\s*\\(`);
...
function checkHandlerComplexity() {
    ...
    for (const filePath of routeFiles) {
        ...
        const cap = override?.cap ?? HANDLER_COMPLEXITY_THRESHOLD;
...
function checkHandshakeTableWrites() {
    ...
    for (const filePath of allFiles) {
        if (setHasByBase(HANDSHAKE_WRITE_ALLOWLIST, filePath))
            continue;
        if (isInsideHandshakeLib(filePath))
            continue;
        const content = fs.readFileSync(filePath, 'utf-8');
        const lines = content.split('\n');
        for (let i = 0; i < lines.length; i++) {
...
function checkHandshakeEnv() {
    ...
    if (fs.existsSync(handshakeDir))
        filesToScan.push(...collectFiles(handshakeDir, /\.js$/));
...
function checkHandshakeRoutePolicyLogic() {
    ...
    if (!fs.existsSync(handshakeRouteDir))
        return violations;
