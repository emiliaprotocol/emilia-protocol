...
const honest: any = verifyReceipt(r.document, r.public_key);
console.log('   →', honest);
if (!honest.valid) {
...
const tampered: any = JSON.parse(JSON.stringify(r.document));
tampered.payload.claim.context.change.after_bank_hash = 'sha256:EVIL_DESTINATION';
const tamperedResult: any = verifyReceipt(tampered, r.public_key);
...
const riskTamper: any = JSON.parse(JSON.stringify(r.document));
riskTamper.payload.claim.context.risk_signals = ['BENIGN'];
const riskResult: any = verifyReceipt(riskTamper, r.public_key);
