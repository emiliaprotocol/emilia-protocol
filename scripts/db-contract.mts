...
const PROJECT_LABEL: string = (process.env.NEXT_PUBLIC_SUPABASE_URL || process.env.SCHEMA_GATE_DB_URL || '')
...
const main = async (): Promise<void> => {
  ...
  const { passCount, gaps, failures }: EvaluationResult = evaluateContract(snap, contract);
...
main().catch((e: any) => { console.error('FATAL:', e); process.exit(2); });
