// Generated from stripe-setup.mts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
...
if (!process.env.STRIPE_SECRET_KEY) {
    for (const f of ['.env.local', '.env']) {
        ...
        for (const line of readFileSync(f, 'utf8').split('\n')) {
            ...
            if (val && !(m[1] in process.env))
                process.env[m[1]] = val; // skip blanks
        }
    }
}
...
if (key.startsWith('sk_live_'))
    console.log('⚠  LIVE key — creating real, chargeable products.\n');
...
async function findOrCreatePrice(product, p) {
    ...
    const match = prices.data.find((pr) => pr.unit_amount === p.amount
...
async function findOrCreateLink(product, price) {
    ...
    if (existingId) {
        try {
            ...
            const existingPriceId = l?.line_items?.data?.[0]?.price?.id;
            if (l?.active && existingPriceId === price.id)
                return l;
            if (l?.active)
                await stripe.paymentLinks.update(existingId, { active: false });
        }
