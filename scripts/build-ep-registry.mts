...
export function buildProfilesManifest(): Record<string, any> {
  const profiles: Array<Record<string, any>> = PROFILE_DESCRIPTORS.map((dsc: any) => ({ ...dsc, content_hash: sha256(canonicalize(dsc)) }));
  const registry_hash: string = sha256(canonicalize(profiles.map((p: any) => p.content_hash)));
  return {
...
export function buildActionsManifest(): Record<string, any> {
  return {
    '@version': 'EP-REGISTRY-v1',
...
const SERIALIZE = (obj: Record<string, any>): string => JSON.stringify(obj, null, 2) + '\n';

export function manifestPaths(): { profiles: string; actions: string } {
  return {
