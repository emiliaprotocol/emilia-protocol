#!/usr/bin/env node
// Generated from emilia-gate.mts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
/**
...
function parseBoundaryJson(raw, maxBytes = 1024 * 1024) {
    if (typeof raw !== 'string' || Buffer.byteLength(raw, 'utf8') > maxBytes)
        throw new Error('JSON input is too large');
...
async function main() {
    ...
    if (hookMode) {
        let evt = {};
        try {
            evt = parseBoundaryJson(readStdin() || '{}');
        }
        catch {
            failClosed('Could not parse the Claude Code hook event.');
        }
        if (evt.tool_name && evt.tool_name !== 'Bash')
            process.exit(0);
        command = evt.tool_input?.command || '';
        subject = 'agent:claude-code';
        if (!command)
            failClosed('Bash hook event did not include a command.');
    ...
    if (actionJson) {
        let input;
        try {
            input = parseBoundaryJson(actionJson);
        }
        catch {
            failClosed('Could not parse the action as strict JSON.');
        }
