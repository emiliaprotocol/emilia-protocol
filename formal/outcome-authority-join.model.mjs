...
const EXACT_BINDING_FIELDS = Object.freeze([
  ...
  'source_keys_distinct',
  'control_domains_distinct',
  'source_key_current',
...
export function normalizeOutcomeState(state = {}) {
  return {
    ...
    source_keys_distinct: state.source_keys_distinct ?? true,
    control_domains_distinct: state.control_domains_distinct ?? true,
    source_key_current: state.source_key_current ?? true,
    observation_window_valid: state.observation_window_valid ?? true,
    source_quorum_met: state.source_quorum_met ?? true,
