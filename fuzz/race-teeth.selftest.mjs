...
// Generated from race-teeth.selftest.mts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
//
...
function spendSeq(store, operationId, amount, acc) {
    const s = { token: null };
    return [
        async () => { const r = await store.reserveSpend({ operationId, amount }); if (r.ok)
            s.token = r.reservation_token; },
        async () => { if (!s.token)
            return; const c = await store.commitSpend({ operationId, reservationToken: s.token }); if (c.ok)
...
test('concurrent driver DETECTS over-commit in a non-atomic store (teeth)', async () => {
    ...
    assert.throws(() => invariant(store.getState().consumed <= budget, 'total-committed<=budget', `consumed=${store.getState().consumed} budget=${budget}`), InvariantViolation, 'the concurrent driver failed to surface the non-atomic over-commit');
...
test('concurrent driver PASSES against the shipped atomic store (clean signal)', async () => {
    ...
    const seqs = Array.from({ length: 10 }, (_, i) => {
        ...
        return [
            ...
            async () => { if (!s.token)
                return; const c = await store.commitSpend({ capabilityId, operationId: `op${i}`, reservationToken: s.token, now: clock }); if (c.ok)
                acc.committed += 40; },
        ];
    });
    await concurrent(seqs);
