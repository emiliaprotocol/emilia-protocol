...
export function invariant(condition, name, detail) {
    if (!condition)
        throw new InvariantViolation(name, detail);
}
/**
...
export async function interleave(ops, rng) {
    ...
    while (cursors.length > 0) {
        ...
        cursor.pos += 1;
        stepCount += 1;
        if (cursor.pos >= cursor.steps.length)
...
function parseArgs(argv) {
    const args = { target: null, seed: 1, seeds: undefined, iterations: 200 };
    const rest = argv.slice(2);
    for (let i = 0; i < rest.length; i += 1) {
        const token = rest[i];
        if (token === '--seed')
            args.seed = Number(rest[(i += 1)]);
        else if (token === '--seeds')
            args.seeds = rest[(i += 1)];
        else if (token === '--iterations')
            args.iterations = Number(rest[(i += 1)]);
        else if (!args.target)
            args.target = token;
    }
    return args;
...
function seedRange(spec, fallbackSeed) {
    ...
    if (!match)
        throw new Error(`--seeds must look like A..B, got ${spec}`);
...
async function main() {
    ...
    if (!args.target || !TARGETS[args.target]) {
        process.stderr.write(`usage: node fuzz/harness.mjs <${Object.keys(TARGETS).join('|')}> `
    ...
    let totalOps = 0;
    process.stdout.write(`fuzz target=${target.name} seeds=${seeds[0]}..${seeds[seeds.length - 1]} `
    ...
    process.stdout.write(`OK  ${totalIterations} iterations across ${seeds.length} seed(s), `
