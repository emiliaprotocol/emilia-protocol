...
export default {
  ...
  async iterate({ rng, iteration }) {
    ...
    // own internal state.
    const committedOps: Array<{ operationId: string; token: string; amount: number }> = [];
    ...
    const ops: Array<Array<() => Promise<void>>> = [];
    for (let i = 0; i < opCount; i += 1) {
      ...
      const state: { reserved: boolean; token: string | null | undefined } = { reserved: false, token: null };
      ...
      const reserveStep = async () => {
        ...
        if (r.ok) {
          ...
          state.token = r.reservation_token as string;
      ...
      const commitStep = async () => {
        ...
        committedOps.push({ operationId, token: state.token as string, amount });
