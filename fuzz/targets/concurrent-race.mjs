...
// Generated from concurrent-race.mts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
//
...
export function checkInvariants({ store, capabilityId, budget, acc }) {
    ...
    invariant(acc.committed <= budget, 'total-committed<=budget', `independently-tracked committed sum=${acc.committed} budget=${budget}`);
    invariant(s.consumed_amount <= budget, 'total-committed<=budget', `store consumed=${s.consumed_amount} budget=${budget}`);
    invariant(s.consumed_amount === acc.committed, 'consumed==sum-of-committed-amounts', `store consumed=${s.consumed_amount} tracked=${acc.committed}`);
    invariant(s.consumed_amount + s.reserved_amount <= budget, 'consumed+reserved<=budget', `consumed=${s.consumed_amount} reserved=${s.reserved_amount} budget=${budget}`);
}
export default {
