...
Internet-Draft            Presentation Binding                 July 2026
   ...
   residue — a compromised or malicious client that renders honestly to
   a verifier's re-derivation while showing the human something else out
...
2.  The Deterministic Renderer
   ...
   action that says "$82,000" cannot produce a rendering that re-derives
   — the deviation is detected offline, by anyone, without trusting the
   surface.  This turns "render from the exact hashed bytes, never a re-
   described copy" from an unverifiable interface rule into a verifiable
   property.
