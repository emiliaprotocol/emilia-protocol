...
5.  Verification Procedure
   ...
   2.  The verifier obtains the EP authorization receipt through
       whatever distribution channel the deployment provides (held
       locally, conveyed with the statement, or fetched from a
       transparency service when a statement_digest is used -- the
       acquisition transport is out of scope for this profile) and
       confirms the obtained receipt hashes to the authority-reference
       digest.  Digest equality, not the channel, is what the profile
       relies on.
...
6.  Verdict-Completeness
   ...
   from silence: fail closed, but do not record it as a fact.  A minimal
   observed-absence statement format (typed payload naming the sources
   searched, the query, and the observation time, signed by the
   observer, with verified-versus-accepted semantics) ships as a
   deterministic vector with the reference implementation
   (examples/scitt/observed-absence-vector.mjs).  Auditors and
   counterparties rely on refusals as much as approvals; this profile
   makes both -- and honest observed-absence -- first-class.
