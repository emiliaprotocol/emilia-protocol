...
| C13 | The checked-in report is byte-stable under the current generator. | Local reproducibility claim | `generate-report.mjs`; report-determinism self-test; `node generate-report.mjs --check` | Supported. `report.json` SHA-256 is `3563151f6716c3922bc25eae5311778cffb4fb6c9425735930eb72e03cebc259`. |
| C14 | The self-test file contains 40 passing tests in the current local run. | Time-bounded local test result | `evaluate.selftest.mjs`; `node --test evaluate.selftest.mjs` on 2026-09-01 | Supported for the tested working tree. This is same-team test evidence, not independent implementation evidence. |
...
| C18 | The evaluator rejects duplicate object members, impossible calendar dates, non-canonical array indexes, unsafe numeric tokens, mutable programmatic accessors, and inputs over explicit resource limits without producing a false result. | Local defensive-behavior claim | `evaluate.mjs`; strict-parser, timestamp, pointer, numeric-alias, inert-input, and bounded-input self-tests | Supported for the exercised local cases. This is not an external security assessment. |
