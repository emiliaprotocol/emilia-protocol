...
- `node scripts/check-authorization-receipts-11.mjs` verifies the isolated
  ...
  21-case vector inventory, packed-vector byte identity, implementation
  guardrails, and checksums.
- `node --test packages/verify/authorization-bundle.test.js` passes 23 tests:
  the 21 generated hostile cases, inventory integrity, and atomic-store helper
  behavior for first bind, idempotent same-grant retry, and conflicting grants.
