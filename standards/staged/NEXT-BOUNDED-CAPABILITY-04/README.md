...
The publication red team also closes four composition attacks: independently transitive but heterogeneous relations cannot be chained, off-domain values cannot inherit a finite-domain proof, a self-authenticated but untrusted runner cannot establish the property, and local-only evidence cannot establish authority across a multi-hop delegation path. It aligns operation identity with the capability-scoped runtime key, defines replay-preserving `not_entered` restoration, makes issuance consumption plus root registration idempotent and reconcilable, and separates budget reservation from freshness of mutable external predicates. A second full-document review added a bounded provider-entry deadline and explicit expiry-versus-commit rule, a concrete-profile requirement for split-domain issuance reconciliation, an Ed25519 operation-bound holder proof alongside the preimage baseline, and explicit cascade-contention and numeric-range limits. The packet checker executes paired positive and negative decision-model controls, including holder-proof field-substitution attacks. These are editorial controls over the normative contract, not an implementation-conformance claim.
