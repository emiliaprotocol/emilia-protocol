...
| `action-evidence-boundary-06` | Owns executor-side ordering: native verification, exact-action match, evidence satisfaction, local authorization, durable custody, invocation, outcome classification, and reconciliation. Outputs admission/refusal and lifecycle records. | Boundary-observed action, native verifier results, CAID mapping when a cross-format join is needed, AEC result when policy requires several evidence legs, local policy, durable state, provider evidence. | Defines no token, receipt, policy language, universal taxonomy, provider truth, or complete-mediation claim. | 6 drafts / 4 parties. | Runtime waist. Keep independent; AEB consumes other formats and must not absorb them. AEB-05 defined the neutral native-compilation contract; AEB-06 makes CAID and AEC conditional and keeps semantic-loss disclosure and stable native replay identity. |
