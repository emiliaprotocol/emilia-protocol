...
| `action-evidence-boundary-07` | Owns executor-side ordering: native verification, exact-action match, evidence satisfaction, local authorization, same-action fencing, durable custody, invocation, outcome classification, and reconciliation. Outputs admission/refusal and lifecycle records. | Boundary-observed action, native verifier results or a pinned gateway's native authorization handoff, CAID mapping when a cross-format join is needed, AEC result when policy requires several evidence legs, local policy, durable state, provider evidence affirmed by a relying-party-configured verifier. | Defines no token, receipt, handoff encoding, policy language, universal taxonomy, provider truth, or complete-mediation claim. | 6 drafts / 4 parties. | Runtime waist. Keep independent; AEB consumes other formats and must not absorb them. AEB-05 defined the neutral native-compilation contract and AEB-06 made CAID and AEC conditional. AEB-07 adds the durable same-action in-flight fence, pre-entry recovery that releases only on proof of non-entry, verifier-affirmed terminal provider evidence, one native replay identity derived from the pinned authority namespace and the native authorization identifier, and the contents and verification of a native authorization handoff; it keeps semantic-loss disclosure and extends it to native projections. |
...
| `canonical-action-identifier-03` | Owns typed material-action identity and loss-aware, profile-pinned cross-format mapping. Outputs CAID plus `EQUIVALENT_UNDER_PROFILE`, `NOT_EQUIVALENT`, or `INDETERMINATE`. | Independently verified native representations, action-type definitions, canonicalization suite, exact mapping profile. | Carries no identity, trust, authority, authorization, execution, safety, or legal-reliance semantics. | 2 drafts / 2 parties. | Join semantics are the one layer EMILIA must defend. Keep independent from every artifact verifier and policy engine. CAID-03 binds external enum values to immutable, digest-pinned value-set snapshots that resolve locally and fail closed, and makes the enum definition forms, own-member field presence, and the unpaired-surrogate refusal exact, without changing the Action Object, identifier syntax, digest suites, or mapping algorithm. |
...
| `ep-architecture-03` | Defines the system map, non-collapsing vocabulary, layer boundaries, and applicability limits. | Component contracts and deployment assumptions. | Defines no universal token, policy language, execution engine, settlement network, or consensus system. | 0 direct; architecture ordinarily earns informative rather than normative use. | Navigation document, not a wire format. Keep; do not use it as a substitute for mechanism specifications. Architecture-03 makes provider entry the transition from reserved to consumed authority before provider invocation, defines admission-control domains and monotonic epochs, defines serialized emergency freeze, restoration, and reconciliation for the three freeze-versus-entry races, states the disconnected-edge stale-admission window, rejects immediate global-freeze claims, and adds idempotency, wrong-holder, receipt-absence, and unsigned-event claim boundaries. |
...
| `ep-authorization-evidence-chain-06` | Composes heterogeneous, natively verified and action-matched evidence against a relying-party requirement. Outputs `SATISFIED` or `UNSATISFIED` plus replayable evaluation. | Native verifier outputs, CAID matches, evidence roles, requirement expression, freshness/status pins. | Satisfaction is not authorization, policy for the protected application, execution, or outcome. Defines no component receipt. | 0 under current name; 4 drafts / 4 parties still pin the superseded Action Evidence Graph name. | Living successor to AEG. Keep as the composition waist and publish a visible predecessor pointer rather than creating a second graph contract. AEC-06 adds an explicit pre-execution Authorization Bundle component, kept separate from the terminal Trust Receipt component, without changing the EP-AEC-v1 envelope or converting evidence satisfaction into execution authority. |
