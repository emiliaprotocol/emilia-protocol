...
- **Follow-up #1:** 7 days after initial — short bump, link to `/govguard`.
...
- GovGuard URL: `https://emiliaprotocol.ai/govguard`
...
**SUBJECT:** Preventing benefit-routing changes from executing — 60-day observe-mode pilot
...
EMILIA GovGuard is a pre-execution control layer designed for exactly this control gap. Live example of a vendor bank-change attempt that GovGuard required two named human approvers to release: https://emiliaprotocol.ai/govguard
...
EMILIA GovGuard inserts that control as a pre-execution check. Output is a publicly verifiable authorization receipt (live example: https://emiliaprotocol.ai/govguard) — the same evidence packet your auditors and Joint Legislative Audit Committee can re-verify months later without trusting our infrastructure.

Pilot scope: 1 disbursement workflow, 60 days, observe mode, weekly fraud/control report. $25K. Open-source protocol, no vendor lock-in.
...
GovGuard intercepts the change *before* it commits. Risk signals fire (new destination, after-hours, no prior change in 30 days), the change is held, two-party named approval is required by policy. Live example of exactly this scenario: https://emiliaprotocol.ai/govguard

This is a 60-day observe-mode pilot, then optional enforcement, on one workflow (e.g., centralized vendor data updates above $25K next-payment exposure). $25K. Apache 2.0 protocol — no lock-in.
...
That gap is where vendor impersonation fraud succeeds. EMILIA GovGuard closes it — pre-execution. Live example of a blocked-then-approved vendor bank change (the kind of incident that costs agencies six figures per occurrence): https://emiliaprotocol.ai/govguard

I'd value 20 minutes of your time to ask one question: in CA's current vendor payment integrity stack, what would it take to slot in a pre-execution control layer on one workflow as a 60-day observe-mode pilot?
...
A 60-day observe-mode pilot on one workflow (e.g., new-vendor onboarding above $50K projected annual spend, or any vendor-bank-account modification) costs $25K. Apache 2.0 — your team can self-host.
...
EMILIA GovGuard is a pre-execution control layer. Before a routing change commits: verified identity, verified authority, exact-action context bound to a one-time cryptographic ceremony, named human signoff when policy requires it. Live receipt example: https://emiliaprotocol.ai/govguard

LA County is the right size for a meaningful pilot — large enough to surface real signal, small enough operationally to ship a 60-day observe-mode trial without procurement-protest exposure. $25K, one workflow, weekly fraud/control report. Apache 2.0.
...
EMILIA GovGuard is a pre-execution control layer. Pre-action: verified actor identity, authority chain, policy-pinned action context, named human signoff when policy requires it. Output: a tamper-evident receipt your investigators can verify offline. Live example of a vendor bank-change blocked-then-approved scenario: https://emiliaprotocol.ai/govguard

A 60-day observe-mode pilot on one workflow (provider data changes, grant disbursement approvals, or benefit-routing changes) is $25K. Apache 2.0 protocol, formally verified, can be self-hosted.
...
EMILIA GovGuard inserts pre-execution verification at the exact moment the change attempts to commit. Live example of a vendor bank-change attempt blocked until two named approvers signed off: https://emiliaprotocol.ai/govguard
...
EMILIA GovGuard is a pre-execution control layer that closes this gap. Live example showing a vendor bank-change attempt blocked until two named approvers signed off: https://emiliaprotocol.ai/govguard
...
EMILIA GovGuard is a pre-execution control layer that addresses this gap. Live example: https://emiliaprotocol.ai/govguard
...
EMILIA GovGuard is a pre-execution control layer that closes that gap. Live example: https://emiliaprotocol.ai/govguard

20-minute scoping call to discuss a 60-day observe-mode pilot on one workflow — supplier onboarding, master-contract supplier modifications, or vendor-bank-account changes? $25K, Apache 2.0 protocol, formally verified.
...
EMILIA GovGuard intercepts the change pre-execution. Live demo of the exact scenario: https://emiliaprotocol.ai/govguard
...
EMILIA GovGuard inserts a pre-execution control at exactly that boundary. Live example: https://emiliaprotocol.ai/govguard
...
EMILIA GovGuard sits between the case-system save and the payment system. Live example: https://emiliaprotocol.ai/govguard
...
EMILIA GovGuard intercepts high-risk action attempts (refund-destination changes, taxpayer-record modifications, payment redirects) *before* they execute. Live example: https://emiliaprotocol.ai/govguard
...
EMILIA GovGuard is a pre-execution control layer for high-risk action authorization. Live example showing a fraudulent bank-account change blocked until two named human approvers signed off: https://emiliaprotocol.ai/govguard

A 60-day observe-mode pilot on one workflow (UI benefit-routing changes is the natural fit) is $25K. Apache 2.0 protocol — EDD can self-host. Formally verified.
