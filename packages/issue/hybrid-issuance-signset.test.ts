...
import {
  ...
  createHybridReceipt,
  createHybridReceiptFromSignSet,
...
async function signSet(bytes: Uint8Array | Buffer) {
  ...
  return [
    ...
    {
      alg: 'ML-DSA-65',
      sig: Buffer.from(ml_dsa65.sign(
        new Uint8Array(buf),
        new Uint8Array(Buffer.from(bundle['ml-dsa-65'].private_key, 'base64url')),
        { extraEntropy: false },
      )).toString('base64url'),
...
test('mints a verifiable EP-RECEIPT-HYBRID-v1 receipt from a signSet signer', async () => {
  const doc: any = await createHybridReceiptFromSignSet({ payload: PAYLOAD as Record<string, any>, signSet });
  ...
  assert.deepEqual(doc.signatures.map((s: any) => s.alg), [...HYBRID_RECEIPT_REQUIRED_ALGORITHMS]);
  assert.deepEqual(doc.signatures.map((s: any) => s.key_id), [bundle.ed25519.key_id, bundle['ml-dsa-65'].key_id]);
...
test('the required-algorithm SET is inside the bytes the signer was handed', async () => {
  const seen: Buffer[] = [];
  const recording = async (bytes: Uint8Array | Buffer) => {
    seen.push(Buffer.from(bytes));
    return signSet(bytes);
  };
  const doc: any = await createHybridReceiptFromSignSet({ payload: PAYLOAD as Record<string, any>, signSet: recording });
  ...
  assert.equal(
    ml_dsa65.verify(new Uint8Array(Buffer.from(doc.signatures[1].sig, 'base64url')), new Uint8Array(rebuilt), PQ_PUBLIC),
    true,
  );
});

test('is byte-identical to the key-material path for the same payload and keys', async () => {
  const fromSigner = await createHybridReceiptFromSignSet({ payload: PAYLOAD as Record<string, any>, signSet });
...
test('carries optional metadata and refuses metadata outside the canonicalization profile', async () => {
  const doc: any = await createHybridReceiptFromSignSet({
    payload: PAYLOAD as Record<string, any>,
    signSet,
  ...
  await assert.rejects(
    () => createHybridReceiptFromSignSet({ payload: PAYLOAD as Record<string, any>, signSet, metadata: { n: 1.5 } as any }),
...
test('a narrowed, widened, empty, or malformed set is a THROW, never a receipt', async () => {
  ...
  await assert.rejects(
    () => createHybridReceiptFromSignSet({ payload: PAYLOAD as Record<string, any>, signSet: async () => [] }),
    /no Ed25519 leg/,
  );
  await assert.rejects(
    () => createHybridReceiptFromSignSet({ payload: PAYLOAD as Record<string, any>, signSet: async () => ({ ok: true } as any) }),
    /one signature per required algorithm/,
  );
  await assert.rejects(
    () => createHybridReceiptFromSignSet({ payload: PAYLOAD as Record<string, any>, signSet: undefined as any }),
    /signSet must be a dual-signer/,
  );
});
...
test('a payload outside the canonicalization profile refuses BEFORE the signer is called', async () => {
  ...
  await assert.rejects(
    () => createHybridReceiptFromSignSet({
      payload: { amount: 1.5 } as Record<string, any>,
