 ...
 */
export function generateIssuerKeyBundle({ approverId = 'ep:approver:local', approverKeyId = 'ep:key:local-approver#1', logKeyId = formatLogKeyId('local'), validFrom = '2026-01-01T00:00:00Z', validTo = '2036-01-01T00:00:00Z', } = {}) {
...
export function validateInitiatorAttestation(attestation) {
    ...
    if (statement !== undefined) {
        if (typeof statement !== 'string')
            throw new Error('initiatorAttestation.statement must be a string');
    ...
    if (policyBasis !== undefined)
        validated.policy_basis = policyBasis;
    if (statement !== undefined)
        validated.statement = statement;
...
export function validateAgentBinding(binding) {
    ...
    if (delegation !== undefined) {
        ...
        if (hash !== undefined)
            validatedDelegation.hash = hash;
        if (observedAt !== undefined)
            validatedDelegation.observed_at = observedAt;
        validatedDelegation = Object.freeze(validatedDelegation);
    }
    if (statement !== undefined) {
        if (typeof statement !== 'string')
            throw new Error('agentBinding.statement must be a string');
        if (statement.length > ATTESTATION_STATEMENT_MAX) {
    ...
    if (validatedDelegation !== undefined)
        validated.delegation = validatedDelegation;
    if (statement !== undefined)
        validated.statement = statement;
    return Object.freeze(validated);
}
...
export function buildContexts({ action, policyHash, approvers, requiredApprovals, issuedAt, expiresAt, prevReceiptHash, initiatorAttestation, agentBinding }) {
    if (!action || typeof action !== 'object')
        throw new Error('buildContexts requires an action');
    if (!action.policy_id)
        throw new Error('action.policy_id is required');
    ...
    return approvers.map((approver, i) => {
        ...
        if (prevReceiptHash)
            ctx.prev_receipt_hash = prevReceiptHash;
        if (attestation !== undefined)
            ctx.initiator_attestation = attestation;
        if (binding !== undefined)
            ctx.agent_binding = binding;
        return ctx;
    });
}
...
export function softwareSignerFromPrivateKey({ privateKey, privateKeyB64u, approverKeyId, signedAt, keyClass = 'B' }) {
    ...
    if (!key)
        throw new Error('softwareSignerFromPrivateKey requires privateKey or privateKeyB64u');
    return {
        approverKeyId,
        keyClass,
        signedAt,
...
export async function collectSignoffs(contexts, signers) {
    ...
    for (let i = 0; i < contexts.length; i++) {
        ...
        if (keyClass === 'A') {
            if (typeof s.signWebAuthn !== 'function')
                throw new Error(`Class A signer ${s.approverKeyId} needs signWebAuthn`);
            signoff.webauthn = await s.signWebAuthn(digest);
            signoff.signature = signoff.webauthn.signature;
        }
        else {
            if (typeof s.sign !== 'function')
                throw new Error(`signer ${s.approverKeyId} needs sign()`);
...
export function merkleProof(leaves, leafIndex) {
    if (!Array.isArray(leaves) || leaves.length === 0)
        throw new Error('merkleProof: no leaves');
    let level = [...leaves];
    let index = leafIndex;
    const path = [];
...
const leafHashV2 = (canonicalPayload) => crypto.createHash('sha256')
...
export function merkleProofV2(leaves, leafIndex) {
    if (!Array.isArray(leaves) || leaves.length === 0)
        throw new Error('merkleProofV2: no leaves');
    let level = [...leaves];
    let index = leafIndex;
    const path = [];
    while (level.length > 1) {
 ...
 * @param {object} args.log
 */
export async function issueAuthorizationReceipt({ receiptId, action, policy, policyHash: explicitPolicyHash, approvers, requiredApprovals, issuedAt = new Date().toISOString(), expiresAt, expiresInSeconds = 3600, prevReceiptHash, initiatorAttestation, agentBinding, signers, committedAt, log, }) {
    if (!receiptId)
        receiptId = `ep:receipt:${crypto.randomBytes(10).toString('base64url')}`;
 ...
 */
export async function issueFromKeyBundle({ keys, action, policy, policyHash: explicitPolicyHash, receiptId, issuedAt = new Date().toISOString(), expiresAt, expiresInSeconds = 3600, initiatorAttestation, agentBinding, }) {
 ...
 */
export const LOG_CHECKPOINT_HYBRID_REQUIRED_ALGORITHMS = Object.freeze(['Ed25519', 'ML-DSA-65']);
...
function checkpointSetMatchesRegistered(algorithms) {
...
export function logCheckpointSignedFields(checkpoint) {
    ...
    for (const key of CHECKPOINT_SIGNED_KEYS) {
        if (!(key in fields))
            throw new Error(`logCheckpointSignedFields: checkpoint is missing "${key}"`);
    }
 ...
 */
export async function assembleAuthorizationReceiptHybrid({ hybridSignSet, ...args }) {
