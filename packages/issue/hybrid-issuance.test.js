...
// Generated from hybrid-issuance.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
/**
...
import { HYBRID_RECEIPT_PROFILE, HYBRID_RECEIPT_REASONS, HYBRID_RECEIPT_REQUIRED_ALGORITHMS, createHybridReceipt, generateHybridIssuerKeyBundle, hybridSignedBytes, loadAgilityModule, signingKeysFromHybridBundle, verificationKeysFromHybridBundle, verifyHybridReceipt, } from './dist/hybrid-issuance.js';
...
async function issue(payload = PAYLOAD, metadata) {
...
test('createHybridReceipt mints both legs over one set of canonical bytes', async () => {
    ...
    const direct = await agility.verifyAgileSignatureSet(message, doc.signatures, [
...
test('createHybridReceipt refuses issuer-side misuse rather than downgrading', async () => {
    await assert.rejects(() => createHybridReceipt({ payload: PAYLOAD, keys: { ...signingKeys, mldsaSecretKey: 'AAAA' } }), /mldsaSecretKey must be 4032 raw bytes/);
    await assert.rejects(() => createHybridReceipt({
        payload: PAYLOAD,
    ...
    await assert.rejects(() => createHybridReceipt({ payload: { amount: 1.5 }, keys: signingKeys }), /canonicalization profile/);
    // No agility module means no receipt, never a single-signature one.
    await assert.rejects(() => createHybridReceipt({ payload: PAYLOAD, keys: signingKeys, agility: {} }), new RegExp(HYBRID_RECEIPT_REASONS.AGILITY_MODULE_UNAVAILABLE));
});
// --- verification -----------------------------------------------------------
...
test('the surviving leg does not verify over a narrowed algorithm set', async () => {
    ...
    const result = await agility.verifyAgileSignatureSet(narrowedBytes, [doc.signatures[0]], [{ alg: 'Ed25519', public_key: verificationKeys.ed25519PublicKey }], { policy: 'hybrid_all', requiredAlgorithms: ['Ed25519'] });
    assert.equal(result.verified, false);
...
test('structural refusals are named and nothing throws on hostile input', async () => {
    ...
    const halfKeys = await verifyHybridReceipt(doc, { ed25519PublicKey: verificationKeys.ed25519PublicKey });
...
test('an EP-RECEIPT-v1 verifier refuses a hybrid receipt cleanly', async () => {
    const doc = await issue();
    const result = verifyReceipt(doc, verificationKeys.ed25519PublicKey);
