...
import type { QualificationDecision } from '../verify/gate-qualification.js';
import {
...
import {
  ...
  type InvocationAuthorityCustodyV2,
  type ObservedEffectRelationV2,
  type ProtectedAdapterV2,
...
function invocationToken(index = 9): string {
  return `admission-invocation:v2:${Buffer.alloc(32, index).toString('base64url')}`;
}

function admissionInput(
  role: AdmissionSnapshotInput['inputs'][number]['role'],
...
function qualified(
  value: Readonly<AdmissionSnapshot>,
  overrides: Partial<QualificationDecision> = {},
  ...
  return {
    ...
    payload_digests: {
      ...
      qualification_statement:
        value.body.qualification_statement_payload_digest,
      qualification_status_head:
        value.body.qualification_status.head_payload_digest,
...
function bundle(
  value: Readonly<AdmissionSnapshot>,
  overrides: {
    qualification?: Partial<QualificationDecision>;
    aeb?: Partial<GateQualificationBundleV2['aeb']>;
    aec?: Partial<GateQualificationBundleV2['aec']>;
    localPolicy?: Partial<GateQualificationBundleV2['localPolicy']>;
  } = {},
...
function relation(
  ...
  observedRelation: ObservedEffectRelationV2['relation'] =
...
function matchingObservation(
  ...
  return {
    ...
    qualification_status_authority_id:
      value.body.qualification_status.authority_id,
    ...
    qualification_status_expires_at:
      value.body.qualification_status.expires_at,
...
function memoryStore(
  value?: Readonly<AdmissionSnapshot>,
): AdmissionStore & { readonly testOnly: true } {
  let ownerIndex = 0;
...
function executionInput(value: Readonly<AdmissionSnapshot>) {
...
function harness(options: {
  ...
  authorityCustody?: InvocationAuthorityCustodyV2;
  remeasure?: (
...
describe('Gate Qualification v2 pure composition', () => {
  it('consumes the real qualification verifier result and all structural legs', () => {
    ...
    assert.equal(
      decision.programDigest,
      value.body.authorization_policy_digest,
    );
    assert.equal(decision.effectKey,
      JSON.stringify([value.body.tenant_id, value.body.operation_id]));
    ...
    assert.ok(denied.reasons.includes(
      'qualification_not_qualified:qualification_revoked',
    ));
  });

  it('validates the immutable snapshot digest and every typed evidence binding', () => {
    const value = snapshot();
    const tampered = structuredClone(value) as AdmissionSnapshot;
    (tampered.body as { action_digest: string }).action_digest = d('attacker');
    ...
    assert.ok(qualificationMismatch.reasons.includes(
      'qualification_runtime_measurement_binding_mismatch',
    ));
    ...
    assert.ok(replayedRequestDecision.reasons.includes(
      'qualification_protected_request_binding_mismatch',
    ));
...
describe('Gate Qualification v2 custody orchestration', () => {
  it('runs shadow mode with no store, adapter, credentials, reserve, or invocation', async () => {
    ...
    if (result.status !== 'shadow') assert.fail('expected shadow result');
    ...
    assert.throws(() => new GateQualificationV2({
      ...
      protectedAdapter: {} as ProtectedAdapterV2,
    } as never), /shadow mode accepts no AdmissionStore or protected adapter/);
  });

  it('requires the authoritative atomic/currentness/exclusive store contract and durability outside testOnly', () => {
    const store = memoryStore();
    const protectedAdapter: ProtectedAdapterV2 = {
      custody: 'protected',
      credentialsExposed: false,
      async invoke() { return {}; },
    ...
    const dependencies = {
      ...
      invocationRemeasurer: {
        ...
        async remeasure(value: Readonly<AdmissionSnapshot>) {
          return bundle(value);
        },
      },
      authorityCustody: createMemoryInvocationAuthorityCustodyV2(),
      providerEvidenceVerifier: {
        async verify(): Promise<ProviderEvidenceVerificationV2> {
          return { ok: false, reason: 'unused' };
        },
      },
      observedEffectRelator: {
        async relate(): Promise<ObservedEffectRelationV2> {
  ...
  it('reserves then begins and invokes only from the immutable snapshot returned by begin', async () => {
    const value = snapshot();
    const callerSnapshot = structuredClone(value) as AdmissionSnapshot;
    const base = memoryStore(value);
    let begunSnapshot: Readonly<AdmissionSnapshot> | null = null;
    const store: AdmissionStore = {
      ...base,
      async reserve(input) {
        const result = await base.reserve(input);
        (callerSnapshot.body.provider as { account_id: string }).account_id =
          'account:attacker';
        return result;
      },
      async beginInvocationWithPreparedToken(input) {
        const result = await base.beginInvocationWithPreparedToken(input);
        if (result.ok) begunSnapshot = result.snapshot;
        return result;
      },
    };
    let adapterInput: Readonly<ProtectedInvocationV2> | null = null;
    const h = harness({
      store,
      async invoke(input) {
        adapterInput = input;
  ...
  it('stages restart authority before an unacknowledged begin commit and never releases consumed rights', async () => {
    ...
    const crashingStore: AdmissionStore = {
      ...base,
      async beginInvocationWithPreparedToken(input) {
        const result = await base.beginInvocationWithPreparedToken(input);
        if (result.ok) {
    ...
    assert.equal(
      authorityPresentAtBeginCommit,
      true,
      'custody must linearize before beginInvocation',
    );
  ...
  it('records COMMITTED and DIVERGED independently', async () => {
    ...
    const result = await h.gate.execute(executionInput(value));
    assert.equal(result.status, 'committed');
    if (result.status !== 'committed') assert.fail('expected committed result');
  ...
  for (const failure of ['crash', 'timeout'] as const) {
    it(`turns adapter ${failure} into INDETERMINATE and reconciles without retransmit`, async () => {
      ...
      const h = harness({
        ...
        async invoke() {
          providerEffects += 1;
          if (failure === 'crash') throw new Error('adapter crashed');
      ...
      assert.ok(indeterminate?.resources.every(
  ...
  it('admits a remedy only as a fresh admission, operation, and CAID', async () => {
    ...
    const h = harness();
    assert.equal((await h.gate.execute(executionInput(original))).status,
      'committed');
    ...
    if (refused.status !== 'refused') assert.fail('expected refusal');
