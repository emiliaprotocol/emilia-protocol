...
// Generated from action-escrow-evidence.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
import assert from 'node:assert/strict';
import test from 'node:test';
import { ACTION_ESCROW_CONTRACTOR_EVIDENCE_PACKAGE_VERSION, ACTION_ESCROW_EVIDENCE_PACKAGE_VERSION, buildActionEscrowEvidencePackage, parseActionEscrowEvidencePackage, verifyActionEscrowEvidencePackage, } from './action-escrow-evidence.js';
import { ACTION_ESCROW_CONTRACTOR_TEMPLATE_VERSION, } from './action-escrow-verifiers.js';
...
test('contractor package re-performs the bound project sidecar and refuses profile confusion', async (t) => {
    ...
    assert.equal(verified.project_record_snapshot_digest, PROJECT_RECORD_SNAPSHOT_DIGEST);
...
test('component verifiers must echo the exact agreement, binding, and action they checked', async (t) => {
    ...
    for (const [name, verifierName, field, reason] of cases) {
        await t.test(name, async () => {
            ...
            const result = await verify(buildActionEscrowEvidencePackage(inputs(), { now: NOW }), {
...
test('early-stage package cannot smuggle funding or release artifacts', async () => {
    ...
    const releaseResult = await verify(buildActionEscrowEvidencePackage(releaseSmuggling, { now: NOW }));
...
test('raw parser refuses duplicate members, invalid Unicode, and oversized input', () => {
    ...
    assert.equal(parseActionEscrowEvidencePackage(`{"x":"${String.fromCharCode(0xd800)}"}`).reason, 'unpaired Unicode surrogate');
