...
export function activateProtectionPlan(args, now = new Date()) {
    if (args[0] !== 'activate')
    ...
    if (!strict.ok)
        throw new TypeError(`strict JSON required: ${strict.reason}`);
...
if (process.argv[1] && fileURLToPath(import.meta.url) === realpathSync(process.argv[1])) {
    try {
        ...
        if (result.output === '-')
            ...
            process.stdout.write(`${JSON.stringify({ status: 'ACTIVATED', output: result.output, activation_id: result.activation.activation_id })}\n`);
    }
    catch (error) {
