...
// Generated from action-refusal-statement-v2.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
//
...
import { ACTION_REFUSAL_CLAIM_BOUNDARY, ACTION_REFUSAL_STATEMENT_VERSION, ACTION_REFUSAL_STATEMENT_V2_VERSION, ACTION_REFUSAL_STATEMENT_V2_DOMAIN, ACTION_REFUSAL_V2_REQUIRED_ALGORITHMS, actionRefusalV2SignedPayload, signActionRefusalStatement, signActionRefusalStatementV2, verifyActionRefusalStatement, verifyActionRefusalStatementV2, verifyActionRefusalStatementAny, } from './action-refusal-statement.js';
...
test('v1 byte-identity regression: a v1 statement verifies and re-signs identically', () => {
    const a = signActionRefusalStatement(input(), { issuer_id: ISSUER_ID, key_id: KEY_ID, private_key: ed.privateKey });
    const b = signActionRefusalStatement(input(), { issuer_id: ISSUER_ID, key_id: KEY_ID, private_key: ed.privateKey });
...
test('the v2 verifier refuses a v1 statement on the version marker', async () => {
    const v1 = signActionRefusalStatement(input(), { issuer_id: ISSUER_ID, key_id: KEY_ID, private_key: ed.privateKey });
    const res = await verifyActionRefusalStatementV2(v1, opts());
    assert.equal(res.verified, false);
    assert.equal(res.checks.version, false);
    assert.ok(/unsupported version/.test(res.reason), res.reason);
});
...
test('SET NARROWING: narrowing required_algorithms fails BOTH structurally and cryptographically', async () => {
    ...
    const { proof, ...bodyNoProof } = stmt;
    const narrowedBytes = Buffer.from(ACTION_REFUSAL_STATEMENT_V2_DOMAIN + canonicalize({ ...bodyNoProof, required_algorithms: ['Ed25519'] }), 'utf8');
