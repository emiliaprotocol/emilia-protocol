...
// Generated from provider-outcome-binding.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
import assert from 'node:assert/strict';
import crypto from 'node:crypto';
import test from 'node:test';
import { buildOutcomeObservationV2 } from '@emilia-protocol/verify/outcome-binding';
import { PROVIDER_OUTCOME_CONTEXT_VERSION, buildProviderOutcomeBinding, providerOutcomeContextDigest, providerOutcomeObservationEffects, verifyProviderOutcomeBinding, } from './provider-outcome-binding.js';
...
async function fixture(overrides = {}) {
    const providerContext = { ...context, ...overrides };
...
function options(expectedContext = context) {
    return {
...
test('provider context digest is deterministic and changes with exact action context', () => {
    ...
    assert.notEqual(providerOutcomeContextDigest(context), providerOutcomeContextDigest({ ...context, account: 'account:other' }));
});
test('verifies a complete hybrid-signed provider outcome under external pins', async () => {
    ...
    const verified = await verifyProviderOutcomeBinding(binding, observation, options(providerContext));
...
test('refuses digest-only evidence and missing external context', async () => {
    ...
    const missingObservation = await verifyProviderOutcomeBinding(binding, digestOnly, options());
    ...
    const missingContext = await verifyProviderOutcomeBinding(binding, observation, { ...options(), expected_context: undefined });
...
test('every exact provider-context substitution is refused', async (t) => {
    ...
    for (const [field, replacement] of substitutions) {
        await t.test(String(field), async () => {
            ...
            const verified = await verifyProviderOutcomeBinding(binding, observation, options(changed));
            assert.equal(verified.status, 'CONFLICTED');
