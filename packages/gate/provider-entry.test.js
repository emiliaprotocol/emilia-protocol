...
import { composeProviderEntryGuards, createOrganizationStatusProviderEntryGuard, evaluateProviderEntryGuard, providerEntryContext, requiredProviderEntryControlDomain, createDefaultActionRiskManifest, createEg1Harness, createGate, } from './index.js';
...
test('composed guards stop at the first refusal and preserve prior evidence', async () => {
    let second = 0;
    const guard = composeProviderEntryGuards(async () => ({ ok: true, evidence: { first: true } }), async () => { second += 1; return { ok: false, reason: 'panic' }; }, async () => { throw new Error('must not run'); });
