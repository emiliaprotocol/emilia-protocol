...
import {
  ...
  COVERAGE_SOURCE_INVENTORY_V3_VERSION,
  COVERAGE_RECONCILIATION_REPORT_VERSION,
  coveragePopulationRoot,
  signCoverageSourceInventory,
  signCoverageSourceInventoryV3,
  verifyCoverageSourceInventory,
  verifyCoverageSourceInventoryV3,
  runCoverageReconciliationV3,
...
async function buildSystemV3(material: ReturnType<typeof keys>) {
...
function sourceVerifyOpts(material: ReturnType<typeof keys>, overrides: any = {}) {
  return {
...
test('LEG STRIPPING: removing the ML-DSA leg refuses structurally', async () => {
  const material = keys();
  const inventory: any = structuredClone(await buildSystemV3(material));
  inventory.proof.signatures = inventory.proof.signatures.filter((s: any) => s.alg === 'Ed25519');
  const res = await verifyCoverageSourceInventoryV3(inventory, systemRecords(), sourceVerifyOpts(material));
  assert.equal(res.accepted, false);
});
...
test('WRONG-LENGTH SIGNATURE: a truncated Ed25519 leg refuses', async () => {
  const material = keys();
  const inventory: any = structuredClone(await buildSystemV3(material));
  const leg = inventory.proof.signatures.find((s: any) => s.alg === 'Ed25519');
