...
// Generated from action-escrow-package.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
import assert from 'node:assert/strict';
import crypto from 'node:crypto';
import test from 'node:test';
import { ACTION_ESCROW_PROFILE_VERSION, computeActionEscrowReleaseBindingMomentDigest, computeActionEscrowResolutionNonce, createActionEscrowKernel, } from './action-escrow.js';
import { assembleActionEscrowEvidencePackage, buildActionEscrowEvidencePackageFromKernel, } from './action-escrow-package.js';
...
import { computeDocumentActionBindingDigest, computeReleaseActionDigest, signDocumentActionBinding, } from '../verify/document-action-binding.js';
...
function releaseTemplate(profileDigest, agreementDigest, terms, amount = '18400.00') {
    return {
...
function signedBinding({ issuerKey, profileDigest, agreementDigest, bindingId = 'binding:package-assembler:v1', amount = '18400.00', supersedesDigest, } = {}) {
...
function harness() {
        ...
        async verifyDocumentActionBinding(artifact, expected) {
            ...
            return {
                ...
                ...(artifact?.supersedes_digest === undefined
                    ? {}
                    : {
                        supersedes_document_action_binding_digest: artifact.supersedes_digest,
                    }),
            };
        },
    ...
    const bindingsFor = (selectedBinding = binding) => ({
        agreement_digest: agreementDigest,
        document_action_binding_digest: computeDocumentActionBindingDigest(selectedBinding),
        milestone_id: MILESTONE_ID,
        release_action_digest: computeReleaseActionDigest(selectedBinding.release_action.template),
        parties: PARTIES,
        profile,
    });
...
async function lifecycle(fixture) {
    ...
    result = await fixture.kernel.beginAcceptance(fixture.inputFor('begin-acceptance'));
    ...
    result = await fixture.kernel.acceptAgreement(fixture.inputFor('accept-contractor', {
        ...
        agreement_acceptance: acceptanceArtifact('party:contractor', fixture.bindingDigest),
    }));
    records.push(result.record);
    result = await fixture.kernel.acceptAgreement(fixture.inputFor('accept-homeowner', {
        party_id: 'party:homeowner',
        agreement_acceptance: acceptanceArtifact('party:homeowner', fixture.bindingDigest),
    }));
    records.push(result.record);
    ...
    for (const party of PARTIES) {
        result = await fixture.kernel.approveRelease(fixture.inputFor(`approve-${partySuffix(party.party_id)}`, {
            party_id: party.party_id,
            resolution: resolutionArtifact(party, fixture.resolutionBindingInput(evidence.evidence_digest)),
        }));
        assert.equal(result.ok, true, result.code);
    }
    records.push(result.record);
...
test('assembles lifecycle snapshots mechanically into the shipped evidence package', async () => {
    ...
    for (const record of records) {
        const pkg = assembleActionEscrowEvidencePackage(fixture.assemblyInput(record), { now: NOW });
    ...
    const releasedPackage = buildActionEscrowEvidencePackageFromKernel(fixture.assemblyInput(released), { now: NOW });
    const releaseOperation = released.operations.find((entry) => entry.operation === 'release');
    assert.deepEqual(releasedPackage.release.reservation, {
    ...
    assert.deepEqual(releasedPackage.release.provider_request, released.release.provider_request);
    assert.deepEqual(releasedPackage.release.provider_statement, released.release.provider_statement);
...
test('derives pending and effective amendment evidence from the kernel chain', async () => {
    ...
    for (const party of PARTIES) {
        await fixture.kernel.acceptAgreement(fixture.inputFor(`accept-${partySuffix(party.party_id)}`, {
            party_id: party.party_id,
            agreement_acceptance: acceptanceArtifact(party.party_id, fixture.bindingDigest),
        }));
    }
    ...
    const amendedActionDigest = computeReleaseActionDigest(amendedBinding.release_action.template);
    let result = await fixture.kernel.proposeAmendment(fixture.inputFor('propose-amendment', {
        party_id: 'party:homeowner',
        command_authorization: commandArtifact('propose_amendment', 'party:homeowner'),
    ...
    const pendingPackage = assembleActionEscrowEvidencePackage(fixture.assemblyInput(result.record), { now: NOW });
    assert.equal(pendingPackage.stage, 'amendment_pending');
    assert.deepEqual(pendingPackage.amendments, []);
    assert.deepEqual(pendingPackage.state_record.snapshot.pending_amendment.document_action_binding.artifact, amendedBinding);
    const amendedInput = (id, party) => fixture.inputFor(id, {
        party_id: party.party_id,
        agreement_acceptance: acceptanceArtifact(party.party_id, amendedBindingDigest),
    }, amendedBinding);
    result = await fixture.kernel.acceptAmendment(amendedInput('amend-contractor', PARTIES[0]));
    assert.equal(result.state, 'amendment_pending');
    result = await fixture.kernel.acceptAmendment(amendedInput('amend-homeowner', PARTIES[1]));
    assert.equal(result.state, 'effective');
    const pkg = assembleActionEscrowEvidencePackage(fixture.assemblyInput(result.record), { now: NOW });
    ...
    assert.deepEqual(pkg.state_record.statement.payload.amendment_digests, result.record.superseded_bindings.map(canonicalDigest));
});
test('refuses malformed stages, missing artifacts, overrides, and state mismatch', async (t) => {
    ...
    const milestone = records.find((record) => (record.state === 'milestone_submitted'
        && record.release_approvals.length === 0));
    await t.test('stage substitution', () => {
        const record = structuredClone(funded);
        record.state = 'released';
        assert.throws(() => assembleActionEscrowEvidencePackage(fixture.assemblyInput(record, {
            operatorStateStatement: fixture.stateStatementFor(record),
        }), { now: NOW }), /state does not match its history/);
    });
    await t.test('missing required artifact', () => {
        ...
        assert.throws(() => assembleActionEscrowEvidencePackage(fixture.assemblyInput(record, {
            operatorStateStatement: fixture.stateStatementFor(record),
        }), { now: NOW }), /required funding statement is missing/);
    });
    await t.test('caller override', () => {
        assert.throws(() => assembleActionEscrowEvidencePackage({
            ...fixture.assemblyInput(released),
            stage: 'draft',
        }, { now: NOW }), /caller overrides are not accepted/);
    });
    await t.test('operator statement from another revision', () => {
        assert.throws(() => assembleActionEscrowEvidencePackage({
            ...fixture.assemblyInput(released),
            operatorStateStatement: fixture.stateStatementFor(milestone),
        }, { now: NOW }), /operator state statement mismatch/);
    });
});
test('hostile artifact and provider substitutions fail before package construction', async (t) => {
    ...
    await t.test('final PDF substitution', () => {
        ...
        }, { now: NOW }), /final PDF does not match/);
    });
    await t.test('binding substitution', () => {
        ...
        assert.throws(() => assembleActionEscrowEvidencePackage(fixture.assemblyInput(record, {
            operatorStateStatement: fixture.stateStatementFor(record),
        }), { now: NOW }), /document-action binding is inconsistent/);
    });
    await t.test('agreement acceptance substitution', () => {
        ...
        assert.throws(() => assembleActionEscrowEvidencePackage(fixture.assemblyInput(record, {
            operatorStateStatement: fixture.stateStatementFor(record),
        }), { now: NOW }), /agreement acceptance is malformed or inconsistent/);
    });
    await t.test('release statement substitution', () => {
        ...
        assert.throws(() => assembleActionEscrowEvidencePackage(fixture.assemblyInput(record, {
            operatorStateStatement: fixture.stateStatementFor(record),
        }), { now: NOW }), /provider release statement is malformed or inconsistent/);
    });
    await t.test('party role substitution', () => {
        ...
        assert.throws(() => assembleActionEscrowEvidencePackage(fixture.assemblyInput(record, {
            operatorStateStatement: fixture.stateStatementFor(record),
        }), { now: NOW }), /parties digest mismatch/);
    });
    await t.test('verification profile substitution', () => {
        ...
        }, { now: NOW }), /verification-profile reference mismatch/);
    });
    await t.test('provider credential leakage', () => {
        ...
        }, { now: NOW }), /provider credentials or secrets/);
        const record = structuredClone(released);
        ...
        for (const field of ['api_token', 'session_token']) {
            ...
            assert.throws(() => assembleActionEscrowEvidencePackage(fixture.assemblyInput(secretRecord, {
...
test('assembler refuses pruned operation history and regressing timestamps', async (t) => {
    ...
    await t.test('pruned operation', () => {
        ...
        assert.throws(() => assembleActionEscrowEvidencePackage(fixture.assemblyInput(record, {
            operatorStateStatement: fixture.stateStatementFor(record),
        }), { now: NOW }), /revision does not match its complete operation history/);
    });
    await t.test('compensated approval-operation pruning', () => {
        const record = structuredClone(released);
        const approvalIndex = record.operations.findIndex((operation) => operation.operation === 'approve_release');
        record.operations.splice(approvalIndex, 1);
        record.revision -= 1;
        assert.throws(() => assembleActionEscrowEvidencePackage(fixture.assemblyInput(record, {
            operatorStateStatement: fixture.stateStatementFor(record),
        }), { now: NOW }), /approve_release operation coverage does not match its artifacts/);
    });
    await t.test('operation time regression', () => {
        ...
        assert.throws(() => assembleActionEscrowEvidencePackage(fixture.assemblyInput(record, {
            operatorStateStatement: fixture.stateStatementFor(record),
        }), { now: NOW }), /operation history is not time-monotonic/);
    });
    await t.test('state-history time regression', () => {
        ...
        assert.throws(() => assembleActionEscrowEvidencePackage(fixture.assemblyInput(record, {
            operatorStateStatement: fixture.stateStatementFor(record),
        }), { now: NOW }), /state history is not bound to its operation log/);
    });
    await t.test('milestone evidence observed after submission', () => {
        ...
        assert.throws(() => assembleActionEscrowEvidencePackage(fixture.assemblyInput(record, {
            operatorStateStatement: fixture.stateStatementFor(record),
        }), { now: '2026-07-17T12:00:02.000Z' }), /milestone evidence was not valid at its submission operation/);
    });
    await t.test('approval expired before release reservation', () => {
        ...
        assert.throws(() => assembleActionEscrowEvidencePackage(fixture.assemblyInput(record, {
            operatorStateStatement: fixture.stateStatementFor(record),
        }), { now: NOW }), /release approval was not valid at its operation/);
    });
});
