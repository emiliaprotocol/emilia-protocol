...
// Generated from remedy-program-postgres.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
import assert from 'node:assert/strict';
import { createHash } from 'node:crypto';
import { readFileSync } from 'node:fs';
import test from 'node:test';
import { REMEDY_PROGRAM_POSTGRES_SQL, createRemedyProgramPostgresStore, } from './remedy-program-postgres.js';
const MIGRATION = readFileSync(new URL('../../supabase/migrations/20260721223000_remedy_program_store.sql', import.meta.url), 'utf8');
...
function authorizedAttempt(suffix, overrides = {}) {
    return {
...
function fakePostgres() {
    ...
    let remedyOperations = new Map();
    let remedyActions = new Map();
    let remedyCaids = new Map();
    let snapshot = null;
    let nextResult = null;
    let nextError = null;
    let nextEnvelopeMutation = null;
    let failCommit = false;
    let releases = 0;
    const transactionLog = [];
    const key = (tenantId, instanceId) => JSON.stringify([tenantId, instanceId]);
    const copyRecords = () => new Map([...records].map(([recordKey, value]) => [recordKey, structuredClone(value)]));
    const copyEvents = () => new Map([...events].map(([recordKey, value]) => [recordKey, structuredClone(value)]));
    const claimKey = (tenantId, value) => JSON.stringify([tenantId, value]);
    const attempts = (state) => [
        ...(state.active_remedy === null ? [] : [state.active_remedy]),
        ...state.remedies,
    ];
    const newClaims = (current, candidate) => {
        const currentIds = new Set(current?.used_evidence_ids ?? []);
        const currentDigests = new Set(current?.used_evidence_digests ?? []);
        const currentOperations = new Set(current === null ? [] : attempts(current).map((attempt) => attempt.remedy_operation_id));
        return {
            evidenceIds: candidate.used_evidence_ids.filter((value) => !currentIds.has(value)),
            evidenceDigests: candidate.used_evidence_digests.filter((value) => !currentDigests.has(value)),
            attempts: attempts(candidate).filter((attempt) => !currentOperations.has(attempt.remedy_operation_id)),
        };
    };
    const replayReason = (tenantId, claims) => {
        if (claims.evidenceIds.some((value) => evidenceIds.has(claimKey(tenantId, value)))
            || claims.evidenceDigests.some((value) => evidenceDigests.has(claimKey(tenantId, value)))) {
            return 'evidence_replayed';
        }
        if (claims.attempts.some((attempt) => (remedyOperations.has(claimKey(tenantId, attempt.remedy_operation_id))
    ...
    const reserveClaims = (tenantId, instanceId, claims) => {
        for (const value of claims.evidenceIds)
            evidenceIds.set(claimKey(tenantId, value), instanceId);
    ...
    const refusal = (tenantId, instanceId, reason) => ({
        rowCount: 1,
        rows: [{
                ok: false,
                reason,
...
test('migration uses composite tenant keys, immutable ledgers, and DB-clock custody', () => {
    ...
    assert.match(MIGRATION, /FOREIGN KEY \(tenant_id, instance_id\)\s+REFERENCES remedy_program_private\.instances\(tenant_id, instance_id\) ON DELETE RESTRICT/);
    ...
    for (const table of [
        ...
        assert.match(MIGRATION, new RegExp(`BEFORE UPDATE OR DELETE ON remedy_program_private\\.${table}`));
    }
    ...
    assert.match(MIGRATION, /p_event_at::pg_catalog\.timestamptz\s+> v_recorded_at \+ pg_catalog\.make_interval\(mins => 5\)/);
...
test('migration is RPC-only service_role with FORCE RLS and pinned search paths', () => {
    ...
    for (const table of [
        ...
        assert.match(MIGRATION, new RegExp(`REVOKE ALL ON TABLE remedy_program_private\\.${table}\\s+FROM PUBLIC, anon, authenticated, service_role;`));
    }
    ...
    for (const name of ['create', 'get', 'compare_and_swap']) {
        ...
        assert.match(MIGRATION, new RegExp(`REVOKE ALL ON FUNCTION remedy_program_private\\.remedy_program_${name}\\([\\s\\S]*?FROM PUBLIC, anon, authenticated, service_role;`));
        assert.match(MIGRATION, new RegExp(`GRANT EXECUTE ON FUNCTION remedy_program_private\\.remedy_program_${name}\\([\\s\\S]*?TO service_role;`));
    }
});
test('same instance id is isolated across tenants and cannot be read cross-tenant', async () => {
    ...
    assert.deepEqual(await store.create(tenantOne), { ok: true, state: tenantOne });
    assert.deepEqual(await store.create(tenantTwo), { ok: true, state: tenantTwo });
    assert.deepEqual(await store.get({ tenantId: 'tenant-3', instanceId: 'shared-remedy' }), { ok: false, reason: 'instance_not_found' });
    assert.deepEqual((await store.get({ tenantId: 'tenant-2', instanceId: 'shared-remedy' })).state, tenantTwo);
...
test('CAS is tenant-bound, revision-fenced, canonical, and append-only', async () => {
    const pg = fakePostgres();
    const store = createRemedyProgramPostgresStore({ pool: pg.pool });
    const created = await store.create(initialState('tenant-a', 'cas-1'));
    ...
    assert.equal(record.state_digest, `sha256:${createHash('sha256').update(record.state_json, 'utf8').digest('hex')}`);
    assert.deepEqual(JSON.parse(record.state_json), next);
    assert.notEqual(record.recorded_at, next.updated_at);
    assert.deepEqual(pg.events.get(pg.key('tenant-a', 'cas-1')).map((event) => ({
...
test('tenant-global evidence ids and digests are one-use, including original reconciliation', async () => {
    ...
    assert.equal(pg.evidenceDigests.get(JSON.stringify(['tenant-global', reconciliationDigest])), 'evidence-a');
});
test('remedy operation, action digest, and CAID remain reserved after proved_no_effect', async () => {
    ...
    assert.equal(pg.remedyOperations.get(JSON.stringify(['tenant-remedy-global', reserved.remedy_operation_id])), 'operation-a');
});
test('state bindings and application event clocks fail closed', async () => {
    const pg = fakePostgres();
    const store = createRemedyProgramPostgresStore({ pool: pg.pool });
    await store.create(initialState('tenant-clock', 'clock-1'));
    ...
    assert.deepEqual(await store.create(future), { ok: false, reason: 'clock_forward_skew' });
    await assert.rejects(() => store.create({ ...initialState('tenant-x', 'bound-1'), revision: 1 }), /binding does not match/);
...
test('modified and ambiguous database envelopes are rejected and rolled back', async () => {
    const pg = fakePostgres();
    const store = createRemedyProgramPostgresStore({ pool: pg.pool });
    await store.create(initialState('tenant-closed', 'closed-1'));
    const mutations = [
        (result) => ({ ...result, rows: [{ ...result.rows[0], tenant_id: 'tenant-other' }] }),
        (result) => ({ ...result, rows: [{ ...result.rows[0], revision: '7' }] }),
        (result) => ({ ...result, rows: [{ ...result.rows[0], state_digest: `sha256:${'0'.repeat(64)}` }] }),
        (result) => ({ ...result, rows: [{ ...result.rows[0], recorded_at: 'not-a-db-time' }] }),
        (result) => ({ ...result, rows: [{ ...result.rows[0], injected: true }] }),
    ];
    for (const mutation of mutations) {
        pg.mutateNextEnvelope(mutation);
        await assert.rejects(() => store.get({ tenantId: 'tenant-closed', instanceId: 'closed-1' }), /Postgres returned|outcome is malformed/);
        assert.equal(pg.transactionLog.at(-1), 'ROLLBACK');
    }
    pg.returnNext({ rowCount: 0, rows: [] });
    await assert.rejects(() => store.get({ tenantId: 'tenant-closed', instanceId: 'closed-1' }), /outcome is ambiguous/);
    assert.equal(pg.transactionLog.at(-1), 'ROLLBACK');
    pg.throwNext(new Error('database unavailable'));
    await assert.rejects(() => store.get({ tenantId: 'tenant-closed', instanceId: 'closed-1' }), /database unavailable/);
    assert.equal(pg.transactionLog.at(-1), 'ROLLBACK');
});
...
test('requires a transaction-capable pool', () => {
    assert.throws(() => createRemedyProgramPostgresStore({ pool: { query: async () => ({}) } }), /transaction-capable pg pool/);
});
