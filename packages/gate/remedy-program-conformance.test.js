...
// Generated from remedy-program-conformance.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
import assert from 'node:assert/strict';
import { createHash } from 'node:crypto';
import { readFileSync } from 'node:fs';
import test from 'node:test';
import { REMEDY_PROGRAM_VERSION, createRemedyMemoryStore, createRemedyProgramKernel, } from './remedy-program.js';
...
function isMaterializationToken(value) {
    if (!isRecord(value))
        return false;
...
function materialize(value, label) {
    if (Array.isArray(value))
        return value.map((entry, index) => materialize(entry, `${label}[${index}]`));
    if (!isRecord(value))
        return value;
    ...
    return Object.fromEntries(Object.entries(value).map(([key, entry]) => [key, materialize(entry, `${label}.${key}`)]));
}
...
function assertExpectations(result, expectations, label) {
    ...
    for (const [path, expected] of Object.entries(expectations)) {
        assert.deepEqual(valueAtPath(result, path), materialize(expected, `${label}.expect.${path}`), `${label} expectation failed at ${path}`);
    }
}
function verifiedOriginal(input) {
    return {
        ok: true,
        ...input.original,
...
function verifiedRevocation(input) {
    return {
        ok: true,
...
function verifiedDispute(input) {
    return {
        ok: true,
        ...input.dispute,
...
function verifiedAuthorization(input) {
    return {
        ok: true,
        ...input.authorization,
...
function verifiedOriginalReconciliation(input) {
    return {
        ok: true,
        evidence_id: input.evidence.evidence_id,
        evidence_digest: input.evidence.evidence_digest,
...
function kernelFor(vector) {
    const rejectedAuthorizationEvidence = new Set(vector.verifier_policy?.reject_authorization_evidence_ids ?? []);
    return createRemedyProgramKernel({
        ...
        verifyRemedyAuthorization: (input) => (rejectedAuthorizationEvidence.has(input.authorization.evidence_id)
...
async function dispatch(subject, op, input) {
    switch (op) {
...
function validateCatalog() {
    ...
    for (const [index, vector] of catalog.vectors.entries()) {
        ...
        if (vector.verifier_policy === undefined)
            expectedKeys.delete('verifier_policy');
        assertExactKeys(vector, expectedKeys, `vector ${index}`);
        ...
        if (vector.verifier_policy !== undefined) {
            ...
            assertExactKeys(vector.verifier_policy, POLICY_KEYS, `${vector.id} verifier policy`);
            assert.ok(Array.isArray(vector.verifier_policy.reject_authorization_evidence_ids));
            assert.ok(vector.verifier_policy.reject_authorization_evidence_ids.every((entry) => typeof entry === 'string' && entry.length > 0));
        }
        for (const [stepIndex, step] of vector.steps.entries()) {
            ...
            if (step.patch === undefined)
                expectedStepKeys.delete('patch');
            assertExactKeys(step, expectedStepKeys, `${vector.id} step ${stepIndex}`);
    ...
    for (const vector of catalog.vectors) {
        await t.test(vector.id, async () => {
            ...
            for (const [index, step] of vector.steps.entries()) {
                ...
                const input = materialize(deepMerge(fixture, step.patch), `${vector.id}.steps[${index}].input`);
