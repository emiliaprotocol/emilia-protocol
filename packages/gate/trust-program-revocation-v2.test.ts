...
function authorizationBinding(overrides: Record<string, unknown> = {}) {
  return {
...
function ed25519KeyId(pubDer: Buffer): string {
  return `ep:revoker-key:sha256:${crypto.createHash('sha256').update(pubDer).digest('hex')}`;
}
function pqKeyId(pubRaw: Uint8Array): string {
...
function revokerFixture() {
  ...
  async function v2Statement(
    target: { target_type: 'commit'; target_id: string; action_hash: string },
    overrides: { requiredAlgorithms?: readonly string[]; signatures?: any[] } = {},
  ) {
    const body = {
    ...
    const bytes = new Uint8Array(revocationV2SignedPayload(body as any, requiredAlgorithms as any));
  ...
  function v1Statement(target: { target_type: 'commit'; target_id: string; action_hash: string }) {
    const signedFields = {
...
describe('EP-REVOCATION-v2 router adoption in the Trust Program revocation target', () => {
  it('the v2 loader uses bundle-analyzable imports and resolves the published router', async () => {
    const source = await readFile(
      new URL('./src/trust-program-revocation.ts', import.meta.url),
      'utf8',
    );
  ...
  it('stripped leg: dropping the ML-DSA signature while keeping required_algorithms full is refused, never a classical-only pass', async () => {
    ...
    const stripped = {
      ...full,
      proof: { ...full.proof, signatures: full.proof.signatures.filter((s: any) => s.alg !== 'ML-DSA-65') },
    };
    const result = await verifyTrustProgramRevocationStatement({
  ...
  it('narrowed set: claiming required_algorithms=["Ed25519"] only is refused structurally, not silently accepted', async () => {
    ...
    const narrowed = {
      ...
      proof: {
        ...
        signatures: full.proof.signatures.filter((s: any) => s.alg === 'Ed25519'),
      },
    };
    const result = await verifyTrustProgramRevocationStatement({
  ...
  it('wrong-length signature on the ML-DSA leg refuses, never crashes', async () => {
    ...
    const tampered = {
      ...
      proof: {
        ...stmt.proof,
        signatures: stmt.proof.signatures.map((s: any) => (
