...
test('CF-1: a gate that trusts the WRONG authority cannot earn the badge', async () => {
  ...
  const gate = createTrustedActionFirewall({ trustedKeys: [wrongHarness.publicKey], approverKeys: wrongHarness.approverKeys, rpId: wrongHarness.rpId, allowedOrigins: wrongHarness.allowedOrigins });
...
test('CF-1: an action the manifest does not declare consequential fails the declaration check', async () => {
  const harness = createEg1Harness();
  const manifest = createDefaultActionRiskManifest();
  const gate = createTrustedActionFirewall({ trustedKeys: [harness.publicKey], approverKeys: harness.approverKeys, rpId: harness.rpId, allowedOrigins: harness.allowedOrigins });
  const wrongHarness = createEg1Harness();
  const wrongGate = createTrustedActionFirewall({ trustedKeys: [wrongHarness.publicKey], approverKeys: wrongHarness.approverKeys, rpId: wrongHarness.rpId, allowedOrigins: wrongHarness.allowedOrigins });
