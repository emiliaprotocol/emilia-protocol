...
// Generated from fido-ap2-bridge.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
...
import { composeQualificationDecisionV2, } from './gate-qualification-v2.js';
import { createFidoAp2AdmissionInput, createFidoAp2QualificationBundle, digestFidoAp2EffectRequest, } from './fido-ap2-bridge.js';
...
import { AEB_NATIVE_VERIFICATION_ATTESTATION_VERSION, adapterPinDigest, canonicalizeAeb, createAebNativeVerificationAttestationAdapter, digestAeb, evaluateAebEvidence, registryEntryDigest, signAebNativeVerificationAttestation, unifiedRegistryDigest, } from '@emilia-protocol/verify/aeb-adapter-contract';
import { createFidoAp2AebAdapter, createFidoAp2NativeSourceBinding, createFidoAp2PinnedProfile, FIDO_AP2_ACTION_TYPE, FIDO_AP2_AEB_CONFIG_VERSION, FIDO_AP2_AEB_TRUST_ROOT_VERSION, FIDO_AP2_CAID_ACTION_DEFINITIONS, FIDO_AP2_CAID_MAPPER_ID, FIDO_AP2_CAID_PROFILE_ID, FIDO_AP2_NATIVE_PROTOCOL_ID, FIDO_AP2_SOURCE_REVISION, projectFidoAp2PaymentAction, } from '@emilia-protocol/verify/fido-ap2-bridge';
...
function compactJwt(label, payload) {
    return [
...
function rawAp2(options = {}) {
    ...
    const checkoutMandateToken = canonicalSdJwt(`checkout-mandate:${options.checkoutTokenIssue ?? seed}`, checkoutHashAlgorithm === 'sha-256'
        ? checkoutMandate
        : { ...checkoutMandate, _sd_alg: checkoutHashAlgorithm });
    const paymentMandateToken = canonicalSdJwt(`payment-mandate:${seed}`, paymentMandate);
...
function evidenceInput(role, payloadDigest) {
    return {
        role,
...
function providerRequestBytes(paymentMandateToken, provider = PROVIDER) {
...
function exactProviderRequestVerifier(expectedBytes, expectedPaymentToken, expectedPaymentDigest, expectedProvider = PROVIDER, onVerify) {
...
function authenticatedStatuses(evaluation, status) {
    const byRole = new Map(evaluation.legs.map((leg) => [leg.evidence_role, leg]));
...
function fixture(options = {}) {
    ...
    const sourceCommitments = {
        checkout_mandate_token_digest: ap2.source_binding.checkout_mandate_token_digest,
        payment_mandate_token_digest: ap2.source_binding.payment_mandate_token_digest,
    ...
    const entries = {
        [profile.registry_entry_ref]: registryEntry(profile.registry_entry_ref, 'mapping-profile', profile.version, { profile_digest: profile.profile_digest }),
        'role:ap2-native-authorization': registryEntry('role:ap2-native-authorization', 'evidence-role', '1', { role: 'ap2-native-authorization', subject_kinds: ['system'] }),
        'role:human_authorization': registryEntry('role:human_authorization', 'evidence-role', '1', { role: 'human_authorization', subject_kinds: ['human'] }),
    };
    const registry = {
    ...
    nativePin.config_digest = adapterPinDigest(nativeAdapter.id, nativePin);
    const pinnedConfig = {
    ...
    const controls = {
        ...
        executor_id: EXECUTOR_ID,
        provider: PROVIDER,
        gate_trust_configuration_digest: evaluated.record.evaluator.pinned_config_digest,
        executor_adapter_digest: EXECUTOR_ADAPTER_DIGEST,
        ...
        provider_request_verifier: exactProviderRequestVerifier(effectBytes, ap2.payment_mandate_token, ap2.source_binding.payment_mandate_token_digest),
    };
    return {
        ap2,
        action,
...
function payloadFor(snapshot, role) {
    const value = snapshot.body.inputs.find((entry) => entry.role === role)
        ?.payload_digest;
    assert.ok(value);
    return value;
}
function qualificationParts(admissionInput) {
...
function enrolledCounterHeads(snapshot) {
...
describe('FIDO/AP2 Gate bridge v1 over official AP2 v0.2', () => {
    it('builds the canonical admission from current mandates, exact source tokens, and trusted controls', () => {
        ...
        assert.equal(FIDO_AP2_SOURCE_REVISION, 'google-agentic-commerce/AP2@e1ea56db72a6385bce3e5c1112b3a56ce60acb43');
        ...
        assert.match(f.ap2.checkout_mandate_token, /^[^.]+\.[^.]+\.[A-Za-z0-9_-]+~$/);
        assert.match(f.ap2.payment_mandate_token, /^[^.]+\.[^.]+\.[A-Za-z0-9_-]+~$/);
        assert.deepEqual(f.action, projectFidoAp2PaymentAction(projectionInput(f.ap2)));
        assert.equal(f.raw.nativeAttestation.protocol_id, NATIVE_PROTOCOL);
        assert.equal(snapshot.body.admitted_at, NOW);
        assert.equal(snapshot.body.effect_request_digest, digestFidoAp2EffectRequest(f.effectBytes));
        ...
        const counters = snapshot.body.resource_reservations.filter((resource) => resource.kind === 'monotonic_counter');
        assert.equal(counters.length, 1);
        assert.equal(counters[0].expected_value, 41);
        assert.equal(counters[0].next_value, 42);
        assert.deepEqual(leases.map((lease) => lease.digest).sort(), f.statuses.map((status) => status.head_digest).sort());
    });
    it('re-derives every economic field from the AP2 payloads held by Gate', () => {
        ...
        const payment = f.controls.ap2_source.payment_mandate_payload;
    ...
    it('refuses AP2 payload bytes spliced under authenticated token commitments', () => {
        ...
        const tampered = structuredClone(f.controls.ap2_source.payment_mandate_payload);
        const amount = tampered.payment_amount;
        amount.amount = 9_999_999;
        assert.throws(() => projectFidoAp2PaymentAction({
            checkout_mandate: f.controls.ap2_source.checkout_mandate_payload,
    ...
    it('refuses a signed amount splice even when the pinned native verifier attests to it', () => {
        ...
        assert.equal(f.raw.humanArtifact.source_commitments.payment_mandate_token_digest, f.ap2.source_binding.payment_mandate_token_digest);
        assert.throws(() => createFidoAp2AdmissionInput(f.raw, f.controls), /signed AEB evaluation is not execution-authorizing|trusted AP2 projected-action binding/);
    });
    it('accepts a zero-counter platform passkey only under the pinned non-reliance policy', () => {
        ...
        const snapshot = createAdmissionSnapshot(createFidoAp2AdmissionInput(f.raw, f.controls));
        assert.equal(snapshot.body.resource_reservations.filter((resource) => resource.kind === 'monotonic_counter').length, 0);
        assert.equal(snapshot.body.resource_reservations.filter((resource) => resource.kind === 'replay').length, 3);
        assert.throws(() => fixture({ signCount: 0, enrolledSignCount: 0, counterHead: 0 }), /sign_count_not_above_enrollment/);
    });
    it('keeps the payment replay identity stable when only the CheckoutMandate token is reissued', () => {
        ...
        assert.notEqual(reissuedCheckout.ap2.checkout_mandate_token, first.ap2.checkout_mandate_token);
        assert.equal(reissuedCheckout.ap2.payment_mandate_token, first.ap2.payment_mandate_token);
        const replayIds = (value) => new Map(createAdmissionSnapshot(createFidoAp2AdmissionInput(value.raw, value.controls))
        ...
        assert.notEqual(reissuedReplayIds.get('ap2-checkout-mandate-token'), firstReplayIds.get('ap2-checkout-mandate-token'));
        assert.equal(reissuedReplayIds.get('ap2-payment-mandate-token'), firstReplayIds.get('ap2-payment-mandate-token'));
    });
    it('retains the digest of the complete signed evaluation record and composes the derived AEB leg', () => {
        ...
        const aebInput = parts.snapshot.body.inputs.find((entry) => entry.role === 'aeb');
        assert.ok(aebInput);
    ...
    it('refuses request-side clock, trust configuration, status, and verifier injection', () => {
        for (const [key, value] of [
            ...
            const f = fixture();
            f.raw[key] = value;
            assert.throws(() => createFidoAp2AdmissionInput(f.raw, f.controls), /is not allowed/);
        }
    });
    it('refuses a forged signed evaluation and trusted actor mismatch', () => {
        const forged = fixture();
        const signature = Buffer.from(forged.raw.evaluation.signature.value, 'base64url');
        signature[signature.length - 1] ^= 0x01;
        forged.raw.evaluation.signature.value = signature.toString('base64url');
        assert.throws(() => createFidoAp2AdmissionInput(forged.raw, forged.controls), /signed AEB evaluation is not execution-authorizing/);
        for (const actor of ['initiator_id', 'executor_id']) {
            const f = fixture();
            const controls = {
                ...f.controls,
                [actor]: `${actor}:attacker`,
            };
            assert.throws(() => createFidoAp2AdmissionInput(f.raw, controls), new RegExp(`server-owned ${actor.replace('_id', '')} binding`));
        }
    });
    it('binds exact effect bytes, payment token, provider identity, and provider adapter', () => {
        ...
        const controls = {
            ...f.controls,
            provider_request_verifier: exactProviderRequestVerifier(f.effectBytes, f.ap2.payment_mandate_token, f.ap2.source_binding.payment_mandate_token_digest, PROVIDER, (input) => { verified = input; }),
        };
        createFidoAp2AdmissionInput(f.raw, controls);
        assert.ok(verified);
        assert.deepEqual(Buffer.from(verified.effect_request_bytes), Buffer.from(f.effectBytes));
        assert.equal(verified.payment_mandate_token, f.ap2.payment_mandate_token);
        assert.equal(verified.payment_mandate_token_digest, f.ap2.source_binding.payment_mandate_token_digest);
        ...
        assert.throws(() => createFidoAp2AdmissionInput(wrongBytes.raw, {
            ...
            provider_request_verifier: exactProviderRequestVerifier(changedBytes, wrongBytes.ap2.payment_mandate_token, wrongBytes.ap2.source_binding.payment_mandate_token_digest),
    ...
    it('refuses authenticated-status identity mismatch and expired source authority', () => {
        ...
        assert.throws(() => createFidoAp2AdmissionInput(outlivesSource.raw, outlivesSource.controls), /closed source freshness does not cover admission/);
    });
    ...
    it('atomically advances WebAuthn counters across distinct admissions', async () => {
        ...
        if (!firstReservation.ok)
            assert.fail(firstReservation.reason);
        const fork = (suffix, expectedValue, nextValue) => {
    ...
    it('reaches AdmissionStore reserve and beginInvocation with authenticated external leases', async () => {
        ...
        if (!reserved.ok)
            assert.fail(reserved.reason);
        assert.equal(reserved.record.state, 'RESERVED');
        assert.equal(reserved.record.resources.filter((resource) => resource.kind === 'external_lease').length, 2);
        const begun = await store.beginInvocation({
            tenant_id: TENANT_ID,
            admission_id: ADMISSION_ID,
    ...
    it('remains synchronous, pure, and deterministic for identical trusted inputs', () => {
        ...
        assert.throws(() => {
            first.inputs[0].subject = 'attacker:mutated';
        }, TypeError);
    ...
    it('conformance corpus counts and executable test references cannot drift', () => {
        const corpus = JSON.parse(readFileSync(new URL('../../conformance/vectors/fido-ap2-bridge.v1.json', import.meta.url), 'utf8'));
        assert.equal(corpus.determinism.vector_count, corpus.vectors.length);
        const vectorIds = corpus.vectors.map((vector) => vector.id);
        assert.equal(new Set(vectorIds).size, vectorIds.length);
        assert.deepEqual(vectorIds.map((id) => Number(id.slice(0, 2))), Array.from({ length: vectorIds.length }, (_, index) => index + 1));
        const catalogIds = corpus.test_catalog.map((entry) => entry.id);
        assert.deepEqual(catalogIds, Array.from({ length: catalogIds.length }, (_, index) => `T${String(index + 1).padStart(2, '0')}`));
        const catalog = new Map(corpus.test_catalog.map((entry) => [entry.id, entry]));
        for (const observation of corpus.coverage_observations) {
            for (const testId of observation.test_ids) {
                assert.equal(catalog.has(testId), true, `${observation.requirement}: unknown test_catalog ID ${testId}`);
            }
        }
        for (const vector of corpus.vectors) {
            assert.equal(catalog.has(vector.coverage.test_id), true, vector.id);
            assert.deepEqual(vector.reason_codes, [...new Set(vector.reason_codes)].sort(), vector.id);
        }
        for (const entry of corpus.test_catalog) {
