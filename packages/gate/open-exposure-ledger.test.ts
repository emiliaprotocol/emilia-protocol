...
const DIGEST_0 = `sha256:${'0'.repeat(64)}` as const;
const CAID = `caid:1:payment.release.1:jcs-sha256:${'A'.repeat(43)}` as const;
...
function beginInput(input: OpenExposureReserveInput): OpenExposureBeginInput {
  return {
...
test('open exposure refuses ghost state without invoking accessors', async () => {
  ...
  await assert.rejects(
    ledger.registerCeiling(value, auth('tenant-a', 'POLICY_ADMIN', 'policy-admin')),
  ...
  const denied = await ledger.registerCeiling(
    ceiling('TENANT', '*', 100n),
    credentials,
  );
...
async function configuredLedger(
  extraCeilings: OpenExposureCeilingInput[] = [],
  clock: () => string = () => DEFAULT_NOW,
) {
...
test('reservation composes immutable execution and authorization pins into record and history', async () => {
  ...
  const input = reservation(
    'exposure-binding',
    `open-exposure-op:v1:${'b'.repeat(32)}`,
    20n,
  );
  ...
  const history = await ledger.history(
    { tenantId: input.tenantId, exposureId: input.exposureId },
    auth('tenant-a', 'READER', 'risk-reader'),
  );
...
test('reservation rejects malformed immutable pins and invocation beyond a window or authorization expiry', async () => {
  ...
  const base = reservation(
    'exposure-invalid-binding',
    `open-exposure-op:v1:${'v'.repeat(32)}`,
    20n,
  );
  await assert.rejects(
    ledger.reserve({ ...base, caid: 'caid:invalid' }, auth('tenant-a', 'ORIGIN', 'origin-a')),
  ...
  await assert.rejects(
    ledger.reserve({ ...base, invokeBy: '2026-08-01T00:00:00.001Z' }, auth('tenant-a', 'ORIGIN', 'origin-a')),
    (error: unknown) => error instanceof TypeError
      && 'code' in error && error.code === 'invalid_time',
  );
});
...
test('parallel beginInvocation issues exactly one bound one-use effect-entry permit', async () => {
  const ledger = await configuredLedger();
  const input = reservation(
    'exposure-permit-race',
    `open-exposure-op:v1:${'p'.repeat(32)}`,
    25n,
  );
  ...
  const replay = await ledger.beginInvocation(
    request,
    auth('tenant-a', 'EXECUTOR', 'executor-a'),
  );
  ...
  if (history.ok) {
    ...
    assert.equal(
      history.entries[1].invocationPermitDigest,
      authorized[0].record.invocationPermitDigest,
    );
  }
});
...
test('beginInvocation uses the injected monotonic clock, rejects expiry, and rechecks every immutable pin', async () => {
  ...
  const expired = reservation(
    'exposure-expired',
    `open-exposure-op:v1:${'e'.repeat(32)}`,
    10n,
  );
  assert.equal((await expiredLedger.reserve(expired, auth('tenant-a', 'ORIGIN', 'origin-a'))).ok, true);
  assert.deepEqual(
    await expiredLedger.beginInvocation(beginInput(expired), auth('tenant-a', 'EXECUTOR', 'executor-a')),
    { ok: false, reason: 'invocation_expired' },
  );
  ...
  const bound = reservation(
    'exposure-pin-check',
    `open-exposure-op:v1:${'m'.repeat(32)}`,
    10n,
  );
  const clockGuard = reservation(
    'exposure-clock-guard',
    `open-exposure-op:v1:${'g'.repeat(32)}`,
    10n,
  );
  ...
  assert.deepEqual(
    await ledger.beginInvocation(
      { ...beginInput(bound), actionDigest: DIGEST_0 },
      auth('tenant-a', 'EXECUTOR', 'executor-a'),
    ),
    { ok: false, reason: 'immutable_binding_conflict' },
  );
  const read = await ledger.read(
    { tenantId: bound.tenantId, exposureId: bound.exposureId },
    auth('tenant-a', 'READER', 'risk-reader'),
  );
  assert.equal(read.ok, true);
  if (read.ok) assert.equal(read.record?.status, 'RESERVED');
  ...
  await assert.rejects(
    ledger.beginInvocation(beginInput(clockGuard), auth('tenant-a', 'EXECUTOR', 'executor-a')),
    (error: unknown) => error instanceof TypeError
...
test('tenant-wide read, history, sum, aging, and deadline surfaces are READER-only', async () => {
  const ledger = await configuredLedger();
  const input = reservation(
    'exposure-reader-only',
    `open-exposure-op:v1:${'q'.repeat(32)}`,
    10n,
  );
  assert.equal((await ledger.reserve(input, auth('tenant-a', 'ORIGIN', 'origin-a'))).ok, true);
  ...
  assert.deepEqual(
    await ledger.read({ tenantId: input.tenantId, exposureId: input.exposureId }, executor),
    { ok: false, reason: 'wrong_authority' },
  );
  assert.deepEqual(
    await ledger.history({ tenantId: input.tenantId, exposureId: input.exposureId }, reconciler),
    { ok: false, reason: 'wrong_authority' },
  );
