...
// Generated from bounded-execution-program.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
...
test('signed bytes are immutable and the program validity window is enforced', () => {
    ...
    assert.equal(verifyBoundedExecutionProgram(altered, verificationOptions(material)).reason, 'program_signature_invalid');
    assert.equal(verifyBoundedExecutionProgram(artifact, {
        ...verificationOptions(material),
...
test('program construction rejects unknown fields, cycles, and invalid budgets', () => {
    ...
    assert.throws(() => signBoundedExecutionProgram(cyclic, material.signer), /program graph contains a cycle/);
    const overBudget = program();
    overBudget.budgets[1].limit = 2;
    assert.throws(() => signBoundedExecutionProgram(overBudget, material.signer), /node charge exceeds its program budget/);
    const unbounded = program();
    delete unbounded.max_total_occurrences;
    assert.throws(() => signBoundedExecutionProgram(unbounded, material.signer), /program occurrence ceiling is invalid/);
    const excessive = program();
    excessive.max_total_occurrences = 1_000_001;
    assert.throws(() => signBoundedExecutionProgram(excessive, material.signer), /program occurrence ceiling is invalid/);
    const missingConcurrency = program();
    delete missingConcurrency.max_concurrent_effects;
    assert.throws(() => signBoundedExecutionProgram(missingConcurrency, material.signer), /program concurrent-effect ceiling is invalid/);
    const excessiveConcurrency = program();
    excessiveConcurrency.max_concurrent_effects = 1_000_001;
    assert.throws(() => signBoundedExecutionProgram(excessiveConcurrency, material.signer), /program concurrent-effect ceiling is invalid/);
});
test('profile actions require a pinned profile and exact actions require both identifiers', () => {
    ...
    assert.throws(() => signBoundedExecutionProgram(missingProfile, material.signer), /program action is invalid/);
    const partialExact = program();
    partialExact.nodes[0].action = { mode: 'exact', caid: C('A') };
    assert.throws(() => signBoundedExecutionProgram(partialExact, material.signer), /program action is invalid/);
});
test('budget_id is the independent dimension while duplicate unit labels remain valid', () => {
    ...
    source.nodes = source.nodes.map((node, index) => ({
        ...
        depends_on: index === 0
            ? []
            : [{ node_id: `${source.nodes[index - 1].node_id}+v1`, outcomes: ['COMMITTED'] }],
        charges: [
