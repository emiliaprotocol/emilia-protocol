...
// Generated from recovery-admission.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
import assert from 'node:assert/strict';
import { generateKeyPairSync } from 'node:crypto';
import test from 'node:test';
import { RECOVERY_CAPABILITY_CLAIM_BOUNDARY, RECOVERY_CAPABILITY_STATUS_VERSION, RECOVERY_CAPABILITY_VERSION, RECOVERY_RESERVATION_STATUS_VERSION, evaluateRecoveryAdmission, recoveryCapabilityDigest, signRecoveryCapability, verifyRecoveryCapability, } from './recovery-admission.js';
...
function verificationContext(material, input = localInput()) {
    return {
...
function currentStatus(input, overrides = {}) {
    return {
...
function reservationStatus(input, overrides = {}) {
...
test('LOCAL_ATOMIC is signed, pinned, current, and limited to the same transaction', async () => {
    ...
    assert.equal(artifact.admission_snapshot_digest, localInput().admission_snapshot_digest);
    ...
    let resolved = 0;
    const decision = await evaluateRecoveryAdmission(artifact, verificationContext(material), {
        current_status_resolver(input) {
            resolved += 1;
...
test('RESERVED_COMPENSATION routes only with exact capacity and requires fresh action admission', async () => {
    ...
    const decision = await evaluateRecoveryAdmission(artifact, verificationContext(material, reservedInput()), {
        current_status_resolver: (input) => currentStatus(input),
        reservation_verifier: (input) => reservationStatus(input),
    });
    assert.equal(decision.recovery_route_accepted, true);
...
test('IRREVERSIBLE has null recovery and always routes to fresh authority', async () => {
    ...
    const decision = await evaluateRecoveryAdmission(artifact, verificationContext(material, irreversibleInput()), {
        current_status_resolver(input) {
            statusCalls += 1;
            return currentStatus(input);
        },
    ...
    assert.equal(decision.retry_permitted, false);
    assert.equal(decision.fresh_action_admission_required, true);
    assert.throws(() => signRecoveryCapability({ ...irreversibleInput(), recovery: {} }, material.signer), /irreversible recovery must be null/);
});
test('tampering and validly signed unknown presenter status both refuse', () => {
    ...
    const withPresenterStatus = signRiskBody(RECOVERY_CAPABILITY_VERSION, {
...
test('verification requires the complete RP-pinned expected policy snapshot', () => {
    ...
    assert.equal(verifyRecoveryCapability(artifact, incomplete).reason, 'verification_context_required');
    const missingRecovery = structuredClone(context);
    delete missingRecovery.expected_policy.recovery;
    assert.equal(verifyRecoveryCapability(artifact, missingRecovery).reason, 'verification_context_required');
    const openRecovery = structuredClone(context);
    openRecovery.expected_policy.recovery.surprise = true;
    assert.equal(verifyRecoveryCapability(artifact, openRecovery).reason, 'verification_context_required');
    assert.equal(verifyRecoveryCapability(artifact, { ...context, trusted_keys: {} }).reason, 'verification_context_required');
    assert.equal(verifyRecoveryCapability(artifact, { ...context, surprise: true }).reason, 'verification_context_required');
});
...
test('validity, action-capability expiry, and local adapter equality fail closed', () => {
    ...
    assert.throws(() => signRecoveryCapability(adapterMismatch, material.signer), /local recovery adapter must match the provider adapter binding/);
});
test('construction rejects unknown keys, symbols, accessors, cycles, and bad mode shapes', () => {
    const material = keyMaterial();
    const missingAdmission = localInput();
    delete missingAdmission.admission_id;
    assert.throws(() => signRecoveryCapability(missingAdmission, material.signer), /capability input must be a closed JSON object/);
    assert.throws(() => signRecoveryCapability({
        ...localInput(),
        admission_id: '',
    }, material.signer), /admission_id is invalid/);
    assert.throws(() => signRecoveryCapability({
        ...localInput(),
        admission_snapshot_digest: 'sha256:not-a-digest',
    }, material.signer), /admission_snapshot_digest is invalid/);
    assert.throws(() => signRecoveryCapability({ ...localInput(), surprise: true }, material.signer), /capability input must be a closed JSON object/);
    assert.throws(() => signRecoveryCapability({
        ...localInput(),
        recovery: { ...localInput().recovery, surprise: true },
    }, material.signer), /local recovery shape is invalid/);
    const symbolInput = localInput();
    symbolInput[Symbol('hidden')] = true;
    assert.throws(() => signRecoveryCapability(symbolInput, material.signer), /outside strict JSON|closed JSON object/);
    let getterCalls = 0;
    ...
    assert.throws(() => signRecoveryCapability(accessorInput, material.signer), /outside strict JSON|closed JSON object/);
    assert.equal(getterCalls, 0);
    const cyclicInput = localInput();
    cyclicInput.recovery.cycle = cyclicInput.recovery;
    assert.throws(() => signRecoveryCapability(cyclicInput, material.signer), /outside strict JSON|local recovery shape is invalid/);
    assert.throws(() => signRecoveryCapability({ ...localInput(), mode: 'OTHER' }, material.signer), /recovery mode is invalid/);
    assert.throws(() => signRecoveryCapability({
        ...reservedInput(),
        recovery: { ...reservedInput().recovery, units: 0 },
    }, material.signer), /reservation units are invalid/);
});
test('hostile artifact JSON and unknown nested members are refused without execution', () => {
    const material = keyMaterial();
    const context = verificationContext(material);
    const artifact = structuredClone(signRecoveryCapability(localInput(), material.signer));
    let getterCalls = 0;
    ...
    assert.equal(getterCalls, 0);
    const symbolArtifact = structuredClone(signRecoveryCapability(localInput(), material.signer));
    symbolArtifact.recovery[Symbol('hidden')] = true;
    assert.equal(verifyRecoveryCapability(symbolArtifact, context).accepted, false);
    const cyclicArtifact = structuredClone(signRecoveryCapability(localInput(), material.signer));
...
test('stale, revoked, future, mismatched, malformed, and throwing status refuse', async () => {
    const material = keyMaterial();
    const artifact = signRecoveryCapability(localInput(), material.signer);
    const evaluateWith = (resolver) => evaluateRecoveryAdmission(artifact, verificationContext(material), { current_status_resolver: resolver });
    ...
    assert.equal((await evaluateRecoveryAdmission(artifact, verificationContext(material), {})).reason, 'current_status_resolver_required');
});
test('reservation absence, mismatches, expiry, malformed output, and exceptions refuse', async () => {
    ...
    for (const [field, value] of mismatches) {
        const decision = await evaluateRecoveryAdmission(artifact, context, {
            current_status_resolver: goodStatus,
            reservation_verifier: (input) => reservationStatus(input, { [field]: value }),
        });
