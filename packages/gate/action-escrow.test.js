...
// Generated from action-escrow.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
        ...
        async verifyDocumentActionBinding(artifact, expected) {
            ...
            return {
                ...
                ...(expected.supersedes_document_action_binding_digest === undefined
                    ? {}
                    : {
                        supersedes_document_action_binding_digest: expected.supersedes_document_action_binding_digest,
                    }),
...
test('runs the complete milestone lifecycle and releases exactly once', async () => {
    ...
    assert.deepEqual(provider.calls.map((call) => call.method), ['release', 'getRelease'], 'POST acknowledgement is never substituted for authoritative GET reconciliation');
    ...
    const completed = await kernel.complete(common('complete', {
        party_id: 'ep:principal:client',
        command_authorization: commandAuthorization('complete', 'ep:principal:client'),
    }));
...
test('pre-funding amendment supersedes the exact binding and invalidates the old action', async () => {
    ...
    const pending = await kernel.proposeAmendment(common('propose-amendment', {
        party_id: 'ep:principal:client',
        command_authorization: commandAuthorization('propose_amendment', 'ep:principal:client'),
    ...
    assertState(await kernel.acceptAmendment(pendingCommon('amend-client', 'ep:principal:client')), 'amendment_pending');
    const effective = await kernel.acceptAmendment(pendingCommon('amend-contractor', 'ep:principal:contractor'));
...
test('dispute, amendment, and cancellation refuse raw party identifiers without command proof', async () => {
    ...
    refused = await setup.kernel.openDispute(common('cross-party-dispute', {
        party_id: 'ep:principal:client',
        reason: 'Milestone evidence is incomplete',
        command_authorization: commandAuthorization('open_dispute', 'ep:principal:contractor'),
    }));
    ...
    const disputed = await setup.kernel.openDispute(common('signed-dispute', {
        party_id: 'ep:principal:client',
        reason: 'Milestone evidence is incomplete',
        command_authorization: commandAuthorization('open_dispute', 'ep:principal:client'),
    }));
    ...
    refused = await setup.kernel.cancel(common('changed-cancel-reason', {
        party_id: 'ep:principal:client',
        reason: 'Cancel before acceptance',
        command_authorization: commandAuthorization('cancel', 'ep:principal:client', { details_digest: digest('9') }),
    }));
    assert.equal(refused.code, 'command_authorization_refused');
