...
// Generated from remedy-program-receipt.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
...
function resign(receipt, privateKey) {
...
test('issues an offline-verifiable receipt over one exact Remedy Program snapshot', async () => {
    ...
    assert.deepEqual(Object.values(verified.checks), Array(Object.keys(verified.checks).length).fill(true));
    ...
    assert.equal(cryptoVerify(null, remedyProgramReceiptSigningBytes(receipt), keys.publicKey, signature), true);
    assert.equal(cryptoVerify(null, canonicalBodyOnly, keys.publicKey, signature), false);
    assert.equal(remedyProgramReceiptSigningBytes(receipt).subarray(0, Buffer.byteLength(ACTION_REMEDY_RECEIPT_DOMAIN)).toString(), ACTION_REMEDY_RECEIPT_DOMAIN);
});
...
test('rejects malformed, noncanonical, non-Ed25519, and substituted signatures', async () => {
    ...
    assert.equal(verifyRemedyProgramReceipt(substituted, options).reason, 'receipt_signature_invalid');
...
test('detects content tampering independently of signature verification', async () => {
    ...
    assert.equal(verifyRemedyProgramReceipt(digestTamper, options).reason, 'receipt_content_digest_mismatch');
    const factTamper = structuredClone(receipt);
    factTamper.payload.original_effect.consequence_digest = HASH('c');
    assert.equal(verifyRemedyProgramReceipt(factTamper, options).reason, 'receipt_content_digest_mismatch');
    const signedContradiction = structuredClone(receipt);
    signedContradiction.payload.original_effect.consequence_digest = HASH('c');
    assert.equal(verifyRemedyProgramReceipt(resign(signedContradiction, keys.privateKey), options).reason, 'receipt_expected_binding_mismatch');
});
...
test('production issuance requires KMS/HSM custody and self-checks external signatures', async () => {
    ...
    await assert.rejects(() => issueRemedyProgramReceipt(input, {
        ...
        signer: {
            ...
            sign: async (bytes) => cryptoSign(null, bytes, keys.privateKey),
        },
    ...
    await assert.rejects(() => issueRemedyProgramReceipt(input, {
        ...
        signer: {
            ...
            custody: 'kms',
            publicKey: publicKeyB64u(keys.publicKey),
            sign: async (bytes) => cryptoSign(null, bytes, wrong.privateKey),
        },
