...
// Generated from admission-store-postgres.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
...
function redigestSnapshot(snapshot) {
    snapshot.snapshot_digest = `sha256:${crypto.createHash('sha256')
        ...
        .update('\0')
        .update(canonical(snapshot.body))
        .digest('hex')}`;
    return snapshot;
}
...
function admissionInput(admissionId, operationId, now, overrides = {}) {
    const admittedAt = iso(now - 60_000);
    const validUntil = iso(now + 10 * 60_000);
    const role = (name, index) => ({
        role: name,
...
test('SQL contract is singleton-bound, append-only, and has permanent operation/resource fences', async () => {
    ...
    assert.doesNotMatch(sql.match(/CREATE OR REPLACE FUNCTION public\.ep_gate_advance_monotonic_counters[\s\S]*?\n\$\$;/)?.[0] ?? '', /INSERT INTO public\.ep_gate_monotonic_counters/);
...
test('adapter is explicitly durable, local_atomic, single-tenant, and deployment-bound', async () => {
    const query = async () => ({ rowCount: 1, rows: [{ result: null }] });
    ...
    await assert.rejects(store.read({ tenant_id: 'tenant:other', admission_id: 'admission:x' }), /tenant_id does not match/);
});
...
test('real PostgreSQL enforces reserve, supersession, currentness, crash recovery, and consumed truth', {
    ...
    try {
      ...
      deployment_id, authority_id, sequence, head_payload_digest, observed_at, expires_at
        ...
        async function assertDirectReserveRejected(snapshot, label) {
            const admissionId = String(snapshot.body.admission_id);
            const operationId = String(snapshot.body.operation_id);
            await assert.rejects(pool.query('SELECT public.ep_gate_admission_reserve($1, $2, $3::jsonb, $4)', ['deployment:gate-v2', 'tenant:gate-v2', JSON.stringify(snapshot), HASH('e')]), undefined, label);
        ...
        function hostileSnapshot(suffix, mutate) {
            const value = structuredClone(createAdmissionSnapshot(admissionInput(`admission:hostile-${suffix}`, `operation:hostile-${suffix}`, now)));
            mutate(value);
        ...
        await assertDirectReserveRejected(hostileSnapshot('unknown', (snapshot) => {
            snapshot.presenter_selected_mode = 'bypass';
        ...
        await assertDirectReserveRejected(hostileSnapshot('missing-role', (snapshot) => {
            snapshot.body.inputs = snapshot.body.inputs.filter((entry) => entry.role !== 'authorization');
        ...
        await assertDirectReserveRejected(hostileSnapshot('duplicate-role', (snapshot) => {
            snapshot.body.inputs.push({
                ...snapshot.body.inputs.find((entry) => entry.role === 'aeb'),
        ...
        await assertDirectReserveRejected(hostileSnapshot('capability', (snapshot) => {
            const capability = snapshot.body.resource_reservations.find((resource) => resource.kind === 'capability');
        ...
        await assertDirectReserveRejected(hostileSnapshot('binding', (snapshot) => {
            const candidate = snapshot.body.inputs.find((entry) => entry.role === 'candidate_manifest');
        ...
        async function seedCurrentness(value) {
      ...
      ON CONFLICT (deployment_id, authority_id) DO UPDATE SET
        ...
        expires_at = EXCLUDED.expires_at`, [
            ...
            for (const lease of value.resource_reservations.filter((resource) => resource.kind === 'external_lease')) {
        ...
        async function assertCurrentnessRefused(suffix, mutateHead) {
            const value = admissionInput(`admission:currentness-${suffix}`, `operation:currentness-${suffix}`, now);
            const snapshot = createAdmissionSnapshot(value);
            const ownerDigest = HASH('d');
            const reserved = await pool.query('SELECT public.ep_gate_admission_reserve($1, $2, $3::jsonb, $4) AS result', ['deployment:gate-v2', 'tenant:gate-v2', JSON.stringify(snapshot), ownerDigest]);
            ...
            const begun = await pool.query(`SELECT public.ep_gate_admission_begin_invocation(
          $1, $2, $3, 0, $4, $5
        ) AS result`, ['deployment:gate-v2', 'tenant:gate-v2', value.admission_id, ownerDigest, HASH('c')]);
        ...
        for (const role of ['aeb', 'aec', 'local_policy', 'authorization']) {
        ...
        const liveInput = admissionInput('admission:not-yet-expired', 'operation:not-yet-expired', Date.now());
        ...
        const abandonedInput = admissionInput('admission:abandoned-before-invocation', 'operation:abandoned-before-invocation', Date.now());
      ...
      EXTRACT(EPOCH FROM ($1::timestamptz - clock_timestamp()))))`, [
        ...
        const counterInput = (suffix, expectedValue, nextValue) => {
            const value = admissionInput(`admission:counter-${suffix}`, `operation:counter-${suffix}`, now);
        ...
        missingCounterInput.resource_reservations = missingCounterInput.resource_reservations.map((resource) => resource.kind === 'monotonic_counter'
      ...
      WHERE deployment_id = $1 AND resource_id = $2`, [
        ...
        await pool.query('SELECT public.ep_gate_provision_monotonic_counter($1, $2, $3, $4)', ['deployment:gate-v2', 'tenant:gate-v2', 'webauthn-sign-count:test-credential', 41]);
        await assert.rejects(pool.query('SELECT public.ep_gate_provision_monotonic_counter($1, $2, $3, $4)', ['deployment:gate-v2', 'tenant:gate-v2', 'webauthn-sign-count:test-credential', 41]), /monotonic counter already provisioned/);
        ...
        await pool.query('SELECT public.ep_gate_provision_monotonic_counter($1, $2, $3, $4)', ['deployment:gate-v2', 'tenant:gate-v2', 'webauthn-sign-count:rollback-proof', 7]);
        const rollbackInput = counterInput('rollback-proof', 7, 8);
        rollbackInput.resource_reservations = rollbackInput.resource_reservations.map((resource, index) => resource.kind === 'monotonic_counter'
        ...
        const rollbackHead = await pool.query(`SELECT current_value::text
      FROM public.ep_gate_monotonic_counters
      WHERE deployment_id = $1 AND resource_id = $2`, [
        ...
        const reserves = await Promise.all(Array.from({ length: 24 }, () => (store.reserve(admissionInput('admission:reserve-race', raceOperation, now)))));
        assert.equal(reserves.filter((result) => result.ok).length, 1);
        assert.equal(reserves.filter((result) => !result.ok).every((result) => (result.reason === 'admission_exists' || result.reason === 'operation_exists')), true);
        ...
        const lossyQuery = async (text, params) => {
