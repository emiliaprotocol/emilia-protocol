...
// Generated from admission-store.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
import test from 'node:test';
import assert from 'node:assert/strict';
import crypto from 'node:crypto';
import { ADMISSION_CURRENTNESS_VERSION, AdmissionStoreValidationError, createAdmissionSnapshot, createMemoryAdmissionStore, verifyAdmissionJournal, } from './admission-store.js';
...
function input(role, suffix = role) {
    return {
        role,
...
function currentnessOracle() {
    return {
        read: async (current) => matchingObservation(current.body),
    };
}
function remedySnapshot(target, targetSnapshotDigest, relationOverrides = {}) {
    const relation = {
...
async function beginTarget(store, target) {
    const reserved = await store.reserve(target);
    if (!reserved.ok)
        assert.fail(`target reservation failed: ${reserved.reason}`);
    ...
    if (!begun.ok)
        assert.fail(`target invocation failed: ${begun.reason}`);
    return begun;
}
test('snapshot is closed, canonical, sorted, immutable and deadline bounded', () => {
    ...
    assert.throws(() => { a.body.caid = caid('attacker'); }, TypeError);
    assert.throws(() => createAdmissionSnapshot(snapshot({ expires_at: '2026-07-26T12:20:00.000Z' })), (error) => error instanceof AdmissionStoreValidationError && error.code === 'expiration_exceeds_evidence');
});
