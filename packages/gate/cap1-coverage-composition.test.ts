...
import {
  CAP1_COVERAGE_COMPOSITION_CLAIM_BOUNDARY,
  CAP1_DOCUMENT_DIGEST_PROFILE,
  CAP1_OBSERVED_BUNDLE_SOURCE_LOCK,
  cap1CoverageSubjectDigest,
  verifyCap1CoverageComposition,
  type Cap1CoveragePopulationPin,
} from './cap1-coverage-composition.js';
import {
...
const systemPin: Cap1CoveragePopulationPin = {
...
const receiptPin: Cap1CoveragePopulationPin = {
...
async function fixture() {
  ...
  const options = {
    ...
    claim_class_id: 'coverage.cap1.absence',
    examination_profile: EXAMINATION_PROFILE,
    verify_cap1: async (candidate: unknown) => ({
...
test('accepts native CAP-1 plus strict set evidence only under the hybrid reconciliation binding', async () => {
  ...
  assert.equal(result.accepted, true, result.reason as string);
  assert.equal(result.claim_boundary, CAP1_COVERAGE_COMPOSITION_CLAIM_BOUNDARY);
  assert.equal((result.cap1 as any).document_digest_profile, CAP1_DOCUMENT_DIGEST_PROFILE);
  assert.deepEqual((result.reconciliation as any).required_algorithms, ['Ed25519', 'ML-DSA-65']);
  assert.ok((result.nonclaims as string[]).includes('source_population_completeness'));
  assert.ok((result.nonclaims as string[]).includes('honest_enumeration'));
});
...
test('refuses a CAP-1 subject that does not bind the supplied sets and program', async () => {
  ...
  (input.cap1_document as any).subject.digest.value = '0'.repeat(64);
...
test('refuses hybrid leg stripping after CAP-1 and set evidence both pass', async () => {
  const { input, options } = await fixture();
  const stripped: any = structuredClone(input.coverage_attestation);
  assert.equal(stripped['@version'], COVERAGE_RECONCILIATION_ATTESTATION_V3_VERSION);
  stripped.proof.signatures = stripped.proof.signatures.filter((entry: any) => entry.alg === 'Ed25519');
