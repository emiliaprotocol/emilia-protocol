...
// Generated from action-escrow-custodian.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
import assert from 'node:assert/strict';
import crypto from 'node:crypto';
import test from 'node:test';
import { hashCanonical } from './execution-binding.js';
import { createActionEscrowCustodianBridge, createActionEscrowCustodianStatementVerifier, } from './action-escrow-custodian.js';
...
function expectedObservation() {
    return {
        ...
        provider_transaction_id: request.release_action_template.custodian_transaction_id,
        provider_milestone_id: request.release_action_template.custodian_milestone_id,
...
test('wrong amount or beneficiary refuses before the provider mutation', async () => {
    ...
    await assert.rejects(bridge.release({
        ...request,
        release_action_template: {
...
test('tampered observations and request substitutions are refused', async () => {
    ...
    await assert.rejects(bridge.release({ ...request, document_action_binding_digest: digest('9') }), /invalid kernel release request/);
});
test('unknown or unavailable provider state remains indeterminate', async () => {
    ...
    await assert.rejects(bridge.getRelease({ ...request, method: 'GET' }), /indeterminate/);
});
