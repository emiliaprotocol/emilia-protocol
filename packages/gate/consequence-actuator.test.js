...
// Generated from consequence-actuator.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
import assert from 'node:assert/strict';
import { generateKeyPairSync, randomBytes } from 'node:crypto';
import { describe, it } from 'node:test';
import { CONSEQUENCE_ACTUATOR_ENVELOPE_VERSION, CONSEQUENCE_ACTUATOR_SQL, ConsequenceActuator, createMemoryConsequenceActuatorStore, createPostgresConsequenceActuatorStore, signConsequenceExecutionEnvelope, } from './consequence-actuator.js';
...
function harness() {
    ...
    const calls = { count: 0 };
    const providerCall = async (binding) => {
        calls.count += 1;
...
function signEnvelope(signer, body, keyId = 'actuator-key-1') {
...
describe('credential-owning consequence actuator', () => {
    it('requires production store capabilities and an explicit memory-store test opt-in', () => {
        ...
        assert.throws(() => new ConsequenceActuator(baseOptions), /durable.*atomic.*ownership-fenced.*permanent.*testOnly/i);
        assert.doesNotThrow(() => new ConsequenceActuator({ ...baseOptions, testOnly: true }));
        const capableStore = {
            durable: true,
            atomic: true,
        ...
        assert.doesNotThrow(() => new ConsequenceActuator({ ...baseOptions, store: capableStore }));
        for (const capability of [
            ...
            assert.throws(() => new ConsequenceActuator({
                ...baseOptions,
                store: { ...capableStore, [capability]: false },
                testOnly: true,
            }), /durable.*atomic.*ownership-fenced.*permanent/i, capability);
        }
    });
    ...
    it('atomically allows one winner under a concurrent race and refuses replay', async () => {
        ...
        assert.deepEqual(raced.filter((result) => !result.ok).map((result) => result.reason), ['envelope_replayed', 'envelope_replayed']);
        assert.equal(calls.count, 1);
        const replay = await actuator.execute(input);
        assert.deepEqual({ ok: replay.ok, reason: replay.ok ? undefined : replay.reason, invoked: replay.invoked }, { ok: false, reason: 'envelope_replayed', invoked: false });
    });
    it('refuses a correctly signed envelope for the wrong target before invocation', async () => {
        ...
        if (result.ok)
            assert.fail('wrong-target envelope executed');
    ...
    it('refuses stale envelopes and mismatched live attempt/action/idempotency bindings', async () => {
        ...
        if (staleResult.ok)
            assert.fail('stale envelope executed');
        ...
        if (wrongAttempt.ok)
            assert.fail('wrong attempt executed');
        ...
        if (wrongAction.ok)
            assert.fail('wrong action executed');
        ...
        if (wrongIdempotencyKey.ok)
            assert.fail('wrong idempotency key executed');
    ...
    it('verifies only the constructor-pinned Ed25519 key and key identifier', async () => {
        ...
        if (wrongKey.ok)
            assert.fail('substituted key executed');
        ...
        if (wrongKeyId.ok)
            assert.fail('substituted key identifier executed');
    ...
    it('permanently fences an indeterminate provider invocation', async () => {
        ...
        if (indeterminate.ok)
            assert.fail('provider error returned success');
        ...
        if (replay.ok)
            assert.fail('indeterminate envelope replayed');
...
describe('Postgres consequence actuator store', () => {
    function mockPool(results, principal = 'tenant_1_consequence_executor') {
        const calls = [];
        return {
            calls,
            pool: {
    ...
    function statefulPool(principal = 'tenant_1_consequence_executor') {
    ...
    function productionActuator(signer, executorPool, perform) {
    ...
    it('allows only one provider entry across concurrent production replicas', async () => {
        ...
        assert.equal(raced.filter((result) => result.ok).length, 1);
        assert.equal(raced.filter((result) => !result.ok && result.reason === 'envelope_replayed').length, 1);
    ...
    it('refuses the same envelope after a production actuator restart', async () => {
        ...
        assert.equal(replay.ok, false);
        if (replay.ok)
            assert.fail('restarted actuator replayed an envelope');
        assert.equal(replay.reason, 'envelope_replayed');
        assert.equal(replay.invoked, false);
        assert.equal(providerCalls, 1);
    ...
    it('requires a dedicated non-privileged executor principal and matching pool', () => {
        ...
        assert.throws(() => createPostgresConsequenceActuatorStore({
            tenantId: 'tenant-1',
    ...
    it('binds reserve to the exact RPC and positional arguments', async () => {
        ...
        assert.match(mock.calls[0].text, /^SELECT envelope_digest\nFROM consequence_actuator_private\.reserve_envelope\(/);
    ...
    it('distinguishes a zero-row replay from ambiguous acknowledgements', async () => {
        ...
        for (const result of [
            ...
            await assert.rejects(() => store.reserve(row), /ambiguous acknowledgement/);
        }
    });
    it('refuses cross-tenant bindings before touching the executor pool', async () => {
        ...
        await assert.rejects(() => store.reserve(reservation({ tenantId: 'tenant-2' })), /does not match the store tenant/);
