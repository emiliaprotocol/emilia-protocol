...
// Generated from breakglass-v2.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
//
...
import { BREAKGLASS_V2_VERSION, BREAKGLASS_V2_REQUIRED_ALGORITHMS, mintBreakGlassAuthorizationV2, verifyBreakGlassV2, verifyBreakGlassStatement, runBreakGlassStatement, mintBreakGlassAuthorization, } from './breakglass.js';
...
function v2Policy(signers, minimumThreshold = 2) {
    return {
...
describe('EP-GATE-BREAKGLASS-v2 hybrid M-of-N roster', () => {
    ...
    it('M-of-N with one hybrid signer missing a leg refuses the whole grant, never a classical-only pass for that signer', async () => {
        ...
        const strippedBob = {
            ...grant,
            signatures: grant.signatures.map((s) => (s.kid === 'kid-bob' ? { ...s, signatures: s.signatures.filter((x) => x.alg !== 'ML-DSA-65') } : s)),
        };
    ...
    it('wrong-length signature on a signer\'s ML-DSA leg refuses, never crashes', async () => {
        ...
        const tampered = {
            ...grant,
            signatures: grant.signatures.map((s) => (s.kid === 'kid-alice'
                ? {
                    ...s,
                    signatures: s.signatures.map((x) => (x.alg === 'ML-DSA-65' ? { ...x, sig: Buffer.from(crypto.randomBytes(16)).toString('base64url') } : x)),
                }
                : s)),
        };
    ...
    it('never throws on hostile input (absent, malformed JSON, threshold-1 grant)', async () => {
        for (const bad of [null, undefined, '', '{not json']) {
            const result = await verifyBreakGlassV2(bad, { policy: { minimum_threshold: 2, roster: [] } });
            assert.equal(result.valid, false);
        }
    });
