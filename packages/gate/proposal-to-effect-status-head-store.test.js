...
// Generated from proposal-to-effect-status-head-store.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
import assert from 'node:assert/strict';
import test from 'node:test';
import { statusArtifactDigest, } from '@emilia-protocol/verify/status';
import { PROPOSAL_TO_EFFECT_STATUS_HEAD_SQL, createPostgresProposalToEffectStatusHeadStore, } from './proposal-to-effect-status-head-store.js';
...
function status(sequence, previousStatusDigest) {
    return {
...
function row(current, predecessor) {
    return {
...
test('atomically accepts the genesis status under the fixed tenant, relying party, and target', async () => {
    ...
    let previous = Symbol('unobserved');
...
test('re-verifies an already accepted head and confirms it still wins the database comparison', async () => {
    ...
    assert.equal(pg.calls.some(({ text }) => text === PROPOSAL_TO_EFFECT_STATUS_HEAD_SQL.compareAndAdvance), true);
});
...
test('fails closed on a compare-and-advance conflict and releases clients after database errors', async () => {
    ...
    await assert.rejects(() => failingStore.accept({
        target: TARGET,
        status: candidate,
        verify: () => verified(candidate),
...
test('rejects invalid fixed scopes and malformed authenticated rows', async () => {
    assert.throws(() => createPostgresProposalToEffectStatusHeadStore({
    ...
    await assert.rejects(() => store.accept({
        target: TARGET,
