...
// Generated from autonomy-control-plane-profile.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
...
test('compiles a closed profile into one existing Trust Program per exact child action', () => {
    ...
    assert.equal(compiled.programs[1]?.stages.some((stage) => stage.stage_id === 'canary-evidence'), true);
...
test('refuses child action, audience, expiry, or per-child budget widening', () => {
    for (const mutate of [
        (value) => { value.children[0].action_type = 'code.delete'; },
        (value) => { value.children[0].audience = 'repo:other'; },
        (value) => { value.children[0].expires_at = '2026-07-28T12:00:00Z'; },
        (value) => { value.children[0].budget.cents = 1001; },
    ]) {
        const candidate = profile();
        mutate(candidate);
...
test('requires pinned, freshness-bounded fitness evidence for every child', () => {
    const candidate = profile();
    candidate.children[0].fitness.max_age_sec = 0;
...
test('executes every published autonomy control-plane conformance vector', () => {
    const suite = JSON.parse(fs.readFileSync(new URL('../../conformance/vectors/autonomy-control-plane.v1.json', import.meta.url), 'utf8'));
