...
// Generated from proposal-to-effect-status.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
import assert from 'node:assert/strict';
import crypto, {} from 'node:crypto';
import test from 'node:test';
import { REVOCER_AUTHORITY_DOMAIN, REVOCER_AUTHORITY_VERSION, STATUS_DOMAIN, STATUS_VERSION, revokerAuthorityCertificateDigest, statusArtifactDigest, } from '@emilia-protocol/verify/status';
import { createProposalToEffectStatusVerifier, } from './proposal-to-effect-status.js';
...
const EXPECTED = Object.freeze({
    ...
    replay_unit: `sha256:${'b'.repeat(64)}`,
});
...
function signBody(body, domain, signer) {
    return crypto.sign(null, Buffer.from(`${domain}${jcs(body)}`, 'utf8'), signer.privateKey).toString('base64url');
}
function certificateBody(revoker = revokerKeys) {
    return {
...
function signCertificate(body = certificateBody()) {
...
function statusBody(certificate, overrides = {}) {
    return {
...
function signStatus(body, signer = revokerKeys) {
    const unsigned = structuredClone(body);
    delete unsigned.proof;
    return {
        ...unsigned,
...
function statusHeadStore(head = null, predecessor = null) {
...
async function check(statusArtifact, statusVerifier = verifier()) {
...
test('rejects the legacy caller-controlled previous-head resolver configuration', () => {
    assert.throws(() => createProposalToEffectStatusVerifier({
        authorityPin,
        targetMapper: () => TARGET,
        certificateResolver: async () => certificate,
...
test('gives durable custody no presenter-supplied predecessor input', async () => {
    ...
    const result = await check(signStatus(statusBody(certificate)), verifier({ statusHeadStore: observingStore }));
...
test('requires authenticated local unconsumed state and never infers it from status', async () => {
    ...
    assert.deepEqual([consumed.valid, consumed.outcome, consumed.status], [false, 'indeterminate', null]);
    assert.deepEqual([unknown.valid, unknown.outcome, unknown.status], [false, 'indeterminate', null]);
});
