...
function normalizeProgram(input) {
    ...
    if (!positiveInteger(input.max_concurrent_effects, EXECUTION_PROGRAM_LIMITS.maxConcurrentEffects)) {
        throw new TypeError('program concurrent-effect ceiling is invalid');
    }
...
function validatePayload(value, version = BOUNDED_EXECUTION_PROGRAM_VERSION) {
...
export async function signBoundedExecutionProgramV2(input, signer, options = {}) {
    const normalized = normalizeProgram(input);
    const body = {
 ...
 */
export async function verifyBoundedExecutionProgramV2(artifact, options = {}) {
    const refuse = (reason, verified = false, programDigest = null) => ({
        accepted: false,
        verified,
        reason,
    ...
    const signed = await verifyRiskBodyV2(artifact, BOUNDED_EXECUTION_PROGRAM_V2_VERSION, options.trusted_keys, options);
    if (!signed.valid || !signed.body) {
        return refuse(signed.reason === 'issuer_untrusted' ? 'program_issuer_untrusted' : 'program_signature_invalid');
    }
    const { issuer, ...payload } = signed.body;
