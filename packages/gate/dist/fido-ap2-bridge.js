...
const AP2_SOURCE_REVISION = 'google-agentic-commerce/AP2@e1ea56db72a6385bce3e5c1112b3a56ce60acb43';
...
function strictJsonInput(value, seen = new WeakSet(), depth = 0) {
        ...
        if (Array.isArray(value)) {
            ...
            for (let index = 0; index < value.length; index += 1) {
                ...
                if (!descriptor || !Object.hasOwn(descriptor, 'value')
                    || !strictJsonInput(descriptor.value, seen, depth + 1))
                    return false;
            }
            return true;
        }
...
function cloneStrictJsonInput(value, name) {
    if (!strictJsonInput(value))
        fail(`${name} is outside the strict JSON domain`);
    let cloned;
    try {
    ...
    if (!strictJsonInput(cloned))
        fail(`${name} is outside the strict JSON domain`);
    return cloned;
}
function freezeStrictJson(value) {
...
function base64urlBytes(value, name, maxBytes) {
...
function emptyReasons(value, name) {
...
function bridgeDigest(domain, ...parts) {
...
export function digestFidoAp2EffectRequest(value) {
...
function trustedNow(controls) {
    ...
    if (!Number.isFinite(ms))
        fail('trusted clock returned an invalid instant');
    return { iso: new Date(ms).toISOString(), ms };
}
function validateProvider(raw, name) {
...
function sameProvider(left, right, name) {
...
function validateEvaluation(raw, admittedAtMs) {
    ...
    const consumptionNonce = string(record.consumption_nonce, 'evaluation.record.consumption_nonce');
    ...
    const requirementRef = string(record.requirement_ref, 'evaluation.record.requirement_ref');
    const requirementDigest = digest(record.requirement_digest, 'evaluation.record.requirement_digest');
    digest(record.registry_digest, 'evaluation.record.registry_digest');
    const caid = string(record.caid, 'evaluation.record.caid');
    const evidenceDigest = digest(record.evidence_digest, 'evaluation.record.evidence_digest');
    ...
    const evaluatorKeyId = string(evaluator.key_id, 'evaluation.record.evaluator.key_id');
    const pinnedConfigDigest = digest(evaluator.pinned_config_digest, 'evaluation.record.evaluator.pinned_config_digest');
    ...
    for (const [index, value] of record.legs.entries()) {
        ...
        if (byRole.has(role))
            fail(`duplicate AEB evidence role ${role}`);
        byRole.set(role, leg);
    ...
    const humanArtifactRef = string(humanLeg.artifact_ref, 'human AEB artifact reference');
    ...
    const nativeAdapterVersion = string(nativeLeg.adapter_version, 'native AEB adapter version');
    const nativeArtifactRef = string(nativeLeg.artifact_ref, 'native AEB artifact reference');
    ...
    exact(composition.requirement_expression, REQUIRED_COMPOSITION, 'evaluation composition requirement');
    ...
    const constraints = object(record.authority_constraints, 'evaluation.record.authority_constraints');
    ...
    for (const [role, leg] of byRole) {
        ...
        if (typeof freshness.age_seconds !== 'number'
            ...
            || Math.abs(freshness.age_seconds - ((evaluatedAt.ms - checkedAt.ms) / 1_000)) > 0.001) {
...
function verifyEvaluationAtAdmission(record, pinnedConfigRaw, currentStatuses, humanArtifact, nativeAttestation, expectedAction, admittedAt, evaluation) {
    const pinnedConfigObject = object(cloneStrictJsonInput(pinnedConfigRaw, 'trusted pinnedConfig'), 'trusted pinnedConfig');
    const relyingPartyId = string(pinnedConfigObject.relying_party_id, 'pinnedConfig.relying_party_id');
    exact(relyingPartyId, evaluation.relyingPartyId, 'pinned AEB relying-party binding');
    exact(aebDigest(pinnedConfigObject, 'trusted pinnedConfig'), evaluation.pinnedConfigDigest, 'signed AEB pinned-config digest');
    ...
    return pinnedConfigObject;
}
function resolveTrustedStatuses(controls, evaluation, now) {
    ...
    for (const [index, raw] of cloned.entries()) {
        ...
        if (!match || Object.hasOwn(current, ref))
            fail('authenticated status identity mismatch');
        ...
        if (expires.ms <= now.ms)
            fail('authenticated status is expired');
        current[ref] = structuredClone(status);
    ...
    if (expected.size !== 0)
        fail('authenticated status set is incomplete');
    return { current, leases };
}
function aebDigest(value, name) {
    try {
...
function validateHumanArtifact(raw) {
    ...
    const relyingPartyId = string(artifact.relying_party_id, 'humanArtifact.relying_party_id');
    ...
    const effectRequestDigest = digest(artifact.effect_request_digest, 'humanArtifact.effect_request_digest');
    const provider = validateProvider(artifact.provider, 'humanArtifact.provider');
    const commitments = object(artifact.source_commitments, 'humanArtifact.source_commitments');
    closed(commitments, SOURCE_COMMITMENT_KEYS, 'humanArtifact.source_commitments');
    const checkoutMandateTokenDigest = digest(commitments.checkout_mandate_token_digest, 'humanArtifact.source_commitments.checkout_mandate_token_digest');
    const paymentMandateTokenDigest = digest(commitments.payment_mandate_token_digest, 'humanArtifact.source_commitments.payment_mandate_token_digest');
    const nativeVerificationAttestationDigest = digest(commitments.native_verification_attestation_digest, 'humanArtifact.source_commitments.native_verification_attestation_digest');
    ...
    if (!/^[A-Z]{3}$/.test(currency))
        fail('humanArtifact action currency is invalid');
    for (const field of [
    ...
    const sourceExpires = instant(action.source_expires_at, 'humanArtifact action source_expires_at');
    ...
    exact(disclosure.checkout_payload_jwt_commitment, action.checkout_payload_jwt_digest, 'humanArtifact checkout payload disclosure');
    ...
    exact(context.effect_request_digest, effectRequestDigest, 'humanArtifact context effect request');
    sameProvider(validateProvider(context.provider, 'humanArtifact context provider'), provider, 'humanArtifact context provider');
    exact(aebDigest(context.source_commitments, 'humanArtifact context source commitments'), aebDigest(commitments, 'humanArtifact source commitments'), 'humanArtifact context source commitments');
    ...
    const credentialId = string(webauthn.credential_id, 'humanArtifact WebAuthn credential_id');
    const authenticatorData = base64urlBytes(webauthn.authenticator_data, 'humanArtifact WebAuthn authenticator_data', 64);
    if (authenticatorData.length !== 37) {
        fail('humanArtifact WebAuthn authenticator_data is outside closed v1');
    }
    base64urlBytes(webauthn.client_data_json, 'humanArtifact WebAuthn client_data_json', 8 * 1024);
    base64urlBytes(webauthn.signature, 'humanArtifact WebAuthn signature', 128);
...
function validateNativeAttestation(raw, artifact, expectedNativeArtifactDigest) {
    ...
    exact(attestation['@version'], 'EP-AEB-NATIVE-VERIFICATION-ATTESTATION-v1', 'nativeAttestation.@version');
    exact(attestation.protocol_id, FIDO_AP2_NATIVE_PROTOCOL_ID, 'nativeAttestation.protocol_id');
    ...
    const nativeArtifactDigest = digest(attestation.native_artifact_digest, 'nativeAttestation native_artifact_digest');
    exact(nativeArtifactDigest, expectedNativeArtifactDigest, 'nativeAttestation source commitment binding');
    ...
    if (verifiedAt.ms >= expiresAt.ms)
        fail('nativeAttestation interval is invalid');
    ...
    exact(mapping.normalized_action_digest, artifact.actionDigest, 'nativeAttestation mapping action');
    ...
    exact(attestationDigest, artifact.nativeVerificationAttestationDigest, 'human/native attestation digest binding');
    return {
        attestationDigest,
...
function validateTrustedControls(controls) {
    ...
    const gateTrustConfigurationDigest = digest(controls.gate_trust_configuration_digest, 'trusted Gate trust configuration');
    const executorAdapterDigest = digest(controls.executor_adapter_digest, 'trusted executor adapter digest');
    ...
    exact(digest(verifier.implementation_digest, 'provider-request verifier implementation'), executorAdapterDigest, 'provider-request verifier implementation binding');
    ...
    try {
        accepted = verifier.verify({
            ...
            payment_mandate_token_digest: source.payment_mandate_token_digest,
...
function makeResource(kind, resourceId, expiresAt, context) {
    return {
        kind,
        resource_id: resourceId,
        reservation_id: `fido-ap2-reservation:${bridgeDigest('reservation', kind, resourceId, expiresAt, ...context)}`,
...
function deriveResources(bindings, artifact, evaluation, provider, expiresAt, webauthnCounterHead) {
    ...
    const paymentId = `ap2-payment-mandate-token:${bridgeDigest('replay/ap2-payment-mandate-token', artifact.paymentMandateTokenDigest)}`;
    const counterId = `webauthn-sign-count:${bridgeDigest('webauthn/sign-count-head', artifact.relyingPartyId, artifact.credentialId)}`;
    const counter = {
        kind: 'monotonic_counter',
        resource_id: counterId,
        reservation_id: `fido-ap2-reservation:${bridgeDigest('reservation/monotonic-counter', counterId, String(webauthnCounterHead), String(artifact.signCount), ...context)}`,
        digest: bridgeDigest('resource/monotonic-counter', counterId, String(webauthnCounterHead), String(artifact.signCount), ...context),
        expires_at: expiresAt,
...
function insertAebInput(raw, aeb) {
    if (!Array.isArray(raw))
        fail('bindings.inputs must be an array');
 ...
 */
export function createFidoAp2AdmissionInput(raw, controls) {
    ...
    if (expires.ms <= admitted.ms)
        fail('admission expiration is invalid');
    ...
    const relyingPartyId = string(bindings.relying_party_id, 'bindings.relying_party_id');
    ...
    exact(effectRequestDigest, trusted.effectRequestDigest, 'server-owned provider-request digest binding');
    exact(bindings.executor_adapter_digest, trusted.executorAdapterDigest, 'server-owned executor-adapter binding');
    ...
    exact(artifact.effectRequestDigest, effectRequestDigest, 'artifact effect-request binding');
    ...
    exact(pinnedConfig.relying_party_id, relyingPartyId, 'Gate/pinned-config relying-party binding');
    ...
    exact(evaluation.humanEvidenceDigest, artifact.artifactDigest, 'AEB human artifact digest binding');
    exact(evaluation.nativeEvidenceDigest, nativeAttestation.attestationDigest, 'AEB native AP2 attestation binding');
    exact(evaluation.nativeSubjectId, nativeAttestation.subjectId, 'AEB native verifier subject binding');
    exact(evaluation.nativeSubjectKind, nativeAttestation.subjectKind, 'AEB native verifier kind binding');
    const base = structuredClone(bindings);
...
function rolePayload(snapshot, role) {
    const values = snapshot.body.inputs
        .filter((entry) => entry.role === role)
        .map((entry) => entry.payload_digest);
    if (values.length !== 1)
        fail(`admission ${role} input is not singular`);
    return values[0];
}
function validateQualificationBinding(raw, snapshot) {
    const qualification = object(raw, 'qualification');
    const payloads = object(qualification.payload_digests, 'qualification.payload_digests');
    exact(payloads.candidate_manifest, snapshot.body.candidate_manifest_digest, 'qualification candidate manifest binding');
    exact(payloads.runtime_measurement, snapshot.body.runtime_measurement_digest, 'qualification runtime binding');
    exact(payloads.protected_request_digest, snapshot.body.effect_request_digest, 'qualification protected request binding');
    exact(payloads.qualification_statement, snapshot.body.qualification_statement_payload_digest, 'qualification statement binding');
    exact(payloads.qualification_status_head, snapshot.body.qualification_status.head_payload_digest, 'qualification status binding');
    digest(payloads.campaign_head, 'qualification campaign head');
    digest(payloads.qualification_graph, 'qualification graph');
    return structuredClone(raw);
}
function validateRequirementBinding(raw, snapshot, role) {
...
function validateLocalPolicyBinding(raw, snapshot) {
    const leg = object(raw, 'localPolicy');
    if (leg.decision !== 'allow' && leg.decision !== 'deny') {
        fail('localPolicy.decision is invalid');
    }
    string(leg.policyId, 'localPolicy.policyId');
    exact(leg.policyDigest, snapshot.body.authorization_policy_digest, 'local-policy program binding');
    ...
    exact(leg.evidenceDigest, rolePayload(snapshot, 'local_policy'), 'local-policy evidence binding');
    return structuredClone(raw);
}
function validateAdmissionAebBinding(snapshot, evaluation) {
    ...
    if (!input)
        fail('admission AEB input is missing');
    ...
    exact(input.trust_configuration_digest, evaluation.pinnedConfigDigest, 'admission AEB trust binding');
    if (Date.parse(input.valid_until) > evaluation.legExpiresAtMs) {
        fail('admission AEB input outlives the verified AEB leg');
    }
    const replay = snapshot.body.resource_reservations.filter((resource) => resource.kind === 'replay');
    if (replay.length !== 3
    ...
    const leases = snapshot.body.resource_reservations.filter((resource) => resource.kind === 'external_lease'
        && resource.resource_id.startsWith('aeb-status-head:sha256:'));
    if (leases.length !== 2) {
        fail('admission authenticated-status leases are incomplete');
    }
    const providerOperations = snapshot.body.resource_reservations.filter((resource) => resource.kind === 'provider_operation'
 ...
 */
export function createFidoAp2QualificationBundle(raw, controls) {
    ...
    closed(input, QUALIFICATION_BUNDLE_INPUT_KEYS, 'qualification bundle input');
    const snapshot = createAdmissionSnapshot(createFidoAp2AdmissionInput(input.bridgeInput, controls));
    const bridgeInput = object(input.bridgeInput, 'qualification bridge input');
    const evaluation = validateEvaluation(bridgeInput.evaluation, Date.parse(snapshot.body.admitted_at));
    validateAdmissionAebBinding(snapshot, evaluation);
    return {
        qualification: validateQualificationBinding(input.qualification, snapshot),
        aeb: {
            decision: 'allow',
