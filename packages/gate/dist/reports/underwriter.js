 ...
 */
export function buildUnderwriterAttestation(entries = [], { insured, policyRef = null, periodStart, periodEnd, now = Date.now, } = {}) {
    ...
    entries.forEach((e, index) => {
        ...
        if (!e || typeof e !== 'object') {
            warnings.push({ ...ref, reason: 'not_an_object' });
            return;
        }
        const t = toMs(e.at);
        if (t == null) {
            warnings.push({ ...ref, reason: 'unparseable_at' });
            return;
        }
        if (t < startMs || t > endMs)
            return;
        if (typeof e.hash !== 'string' || e.hash.length === 0) {
            warnings.push({ ...ref, reason: 'missing_hash' });
            return;
        }
        if (e.kind !== 'decision' && e.kind !== 'execution') {
            warnings.push({ ...ref, reason: 'unknown_kind' });
            return;
        }
        if (e.kind === 'decision' && typeof e.allow !== 'boolean') {
            warnings.push({ ...ref, reason: 'decision_missing_allow' });
            return;
        }
    ...
    for (const { entry: d, t } of decisions) {
        if (firstAt === null || t < firstAt.t)
            firstAt = { t, at: d.at };
        if (lastAt === null || t > lastAt.t)
            lastAt = { t, at: d.at };
    }
