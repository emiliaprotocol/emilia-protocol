...
export const ACTION_ESCROW_CONTRACTOR_EVIDENCE_PACKAGE_VERSION = 'EP-ACTION-ESCROW-CONTRACTOR-EVIDENCE-PACKAGE-v1';
...
function projectRecordBytes(value, maxBytes) {
    ...
    const bytes = Buffer.from(value);
    if (bytes.length === 0 || bytes.length > maxBytes) {
        throw new TypeError(`action-escrow evidence: projectRecordBytes must be between 1 and ${maxBytes} bytes`);
    }
    return bytes;
}
/**
 ...
 */
export function parseActionEscrowEvidencePackage(raw, { maxBytes = DEFAULT_MAX_PACKAGE_BYTES, } = {}) {
    ...
    if (!gated.ok)
        return { ok: false, reason: gated.reason, value: null };
    try {
 ...
 */
export function buildActionEscrowEvidencePackage({ agreementId, stage, binding, documentBytes: rawDocumentBytes, documentFileName = null, documentExecution = null, agreementAcceptances = [], releaseApprovals = [], fundingStatement = null, milestones = [], release = null, stateRecord, amendments = [], verificationProfile, projectRecordBytes: rawProjectRecordBytes = null, projectRecordFileName = null, projectRecordProvider = null, projectRecordSnapshotDigest = null, } = {}, { now = 0, maxDocumentBytes = DEFAULT_MAX_DOCUMENT_BYTES, maxProjectRecordBytes = DEFAULT_MAX_PROJECT_RECORD_BYTES, } = {}) {
    ...
    if (hasProjectRecord) {
        if (!Number.isSafeInteger(maxProjectRecordBytes)
            ...
            throw new TypeError('action-escrow evidence: maxProjectRecordBytes must be a positive safe integer');
        }
        if (typeof projectRecordProvider !== 'string'
            || projectRecordProvider.length === 0
            || projectRecordProvider.length > 128
            // HASH.test() ToString-coerces its argument at runtime regardless;
            // String() here makes that coercion explicit for the type checker.
            || !HASH.test(String(projectRecordSnapshotDigest))) {
            throw new TypeError('action-escrow evidence: project-record provider and snapshot digest are required');
        }
        validFileName(projectRecordFileName, 'projectRecordFileName');
        projectBytes = projectRecordBytes(rawProjectRecordBytes, maxProjectRecordBytes);
    }
        ...
        ...(hasProjectRecord
            ? {
                project_record: {
                    ...
                    // projectBytes is non-null here: hasProjectRecord true means the
                    // `if (hasProjectRecord)` block above already assigned it.
...
async function callVerifier(verifier, value, context) {
    if (typeof verifier !== 'function')
        return { valid: false, reason: 'verifier_required' };
    try {
 ...
 * @param {*} pkg
 */
export async function verifyActionEscrowEvidencePackage(pkg, { documentBytes: rawDocumentBytes, projectRecordBytes: rawProjectRecordBytes, verifyBinding, verifyProjectRecord, verifyProfile, verifyDocumentExecution, verifyAgreementAcceptance, verifyReleaseApproval, verifyFunding, verifyMilestone, verifyRelease, verifyAmendment, verifyState, expectedAgreementId, now, maxDocumentBytes = DEFAULT_MAX_DOCUMENT_BYTES, maxProjectRecordBytes = DEFAULT_MAX_PROJECT_RECORD_BYTES, } = {}) {
    const checks = {
        structure: false,
    ...
    try {
        ...
        checks.structure = exactKeys(pkg, allowedTop, requiredTop)
            ...
            && (!contractorPackage
                || (exactKeys(pkg.project_record, PROJECT_RECORD_KEYS, new Set([
                    'media_type',
                    'digest',
                    'byte_length',
                    'provider',
        ...
        if (!checks.package_digest)
            return resultFailure('package_digest_mismatch', checks);
        ...
        if (!checks.time)
            return resultFailure('invalid_or_future_assembled_at', checks);
        if (!Number.isSafeInteger(maxDocumentBytes) || maxDocumentBytes <= 0) {
        ...
        if (!checks.document)
            return resultFailure('document_bytes_mismatch', checks);
        ...
        const contractorBinding = HASH.test(pkg.binding?.release_action?.template?.project_record_snapshot_digest);
        ...
        if (contractorPackage) {
            ...
            let sourceBytes;
            try {
                sourceBytes = projectRecordBytes(rawProjectRecordBytes, maxProjectRecordBytes);
            }
            catch {
            ...
            const projectResult = await callVerifier(verifyProjectRecord, projectRecord, {
        ...
        const requiredReleaseParties = normalizedRequiredParties(profileResult.required_release_parties);
        ...
        if (pkg.document_execution !== null) {
            const executionResult = await callVerifier(verifyDocumentExecution, pkg.document_execution, {
                agreementId: pkg.agreement_id,
                bindingDigest: bindingResult.binding_digest,
        ...
        // requiredParties is non-null here: checks.binding (verified above, with
        // an early return when false) requires requiredParties !== null.
        const requiredAgreementByIdentity = new Map(requiredParties.map((entry) => [`${entry.role ?? ''}\u0000${entry.party_id}`, entry]));
        ...
        for (const acceptance of pkg.agreement_acceptances) {
            ...
            const result = await callVerifier(verifyAgreementAcceptance, acceptance.evidence, {
        ...
        if (pkg.funding_statement !== null) {
            ...
            if (!checks.funding)
                return resultFailure('funding_statement_missing', checks);
        }
        ...
        if (stageRequiresMilestone(pkg.stage))
            milestonesValid = milestonesValid && milestoneIds.size > 0;
        checks.milestones = milestonesValid;
        if (!checks.milestones)
            return resultFailure('milestone_verification_failed', checks);
        // requiredReleaseParties is non-null here: checks.profile (verified
        // above, with an early return when false) requires it !== null.
        const requiredReleaseByIdentity = new Map(requiredReleaseParties.map((entry) => [`${entry.role ?? ''}\u0000${entry.party_id}`, entry]));
        ...
        for (const [approvalIndex, approval] of pkg.release_approvals.entries()) {
            ...
            const result = await callVerifier(verifyReleaseApproval, approval.evidence, {
            ...
            if (result.valid !== true
                ...
                || result.milestone_evidence_digests.some((digest, index) => digest !== milestoneEvidenceDigests[index])
                || typeof result.principal_key_id !== 'string'
                || result.principal_key_id.length === 0
        ...
        if (hasReleaseArtifacts) {
            ...
            if (!checks.release)
                return resultFailure('release_artifact_missing', checks);
        }
        return {
            valid: true,
            reason: 'verified',
            ...
            project_record_snapshot_digest: bindingResult.project_record_snapshot_digest ?? null,
...
//# sourceMappingURL=action-escrow-evidence.js.map