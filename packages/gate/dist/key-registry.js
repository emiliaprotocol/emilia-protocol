...
export function createKeyRegistry(entries = []) {
    ...
    function normalize(e) {
        if (!e || !e.key || typeof e.key !== 'string')
            throw new Error('key registry entry requires a base64url SPKI key');
        const normalized = {
    ...
    for (const e of entries)
        list.push(normalize(e));
    /** Is this entry usable to verify a receipt issued at `atMs`? */
    function entryActiveAt(entry, atMs) {
        if (entry.revoked_at != null)
            return false; // HARD revocation: never trust a revoked key
        if (!Number.isFinite(atMs))
            return false;
        if (entry.not_before != null && atMs < entry.not_before)
            return false;
        if (entry.not_after != null && atMs > entry.not_after)
            return false;
        return true;
    }
    return {
        ...
        revoke(kid, at) {
            ...
            let n = 0;
            for (const e of list) {
                if (e.kid === kid && e.revoked_at == null) {
                    e.revoked_at = revokedAt;
                    n += 1;
                }
            }
            if (n === 0)
                throw new Error(`key registry: no active key with kid "${kid}" to revoke`);
            return n;
        },
