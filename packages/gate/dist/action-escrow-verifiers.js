 ...
 */
import { verifyDocumentActionBinding, } from '@emilia-protocol/verify/document-action-binding';
 ...
 */
export const ACTION_ESCROW_AGREEMENT_DIGEST_VERSION = 'EP-ACTION-ESCROW-AGREEMENT-DIGEST-v1';
export const ACTION_ESCROW_CONTRACTOR_TEMPLATE_VERSION = 'EP-ACTION-ESCROW-CONTRACTOR-TEMPLATE-v1';
...
function exactExpected(value) {
    ...
    return {
        ...
        supersedes_document_action_binding_digest: value.supersedes_document_action_binding_digest ?? null,
    };
}
export function validateActionEscrowReleaseTemplate(template, { profileDigest, agreementId, agreementDigest, milestoneId, documentDigest, materialTerms, contractorProjectSource = false, } = {}) {
...
function materialTermsMatchAction(binding, template) {
    if (!Array.isArray(binding?.material_terms))
        return false;
    ...
    const projectRecordBound = Object.hasOwn(template, 'project_record_snapshot_digest');
 ...
 */
function createDocumentBindingVerifier({ issuerKeys, resolveDocumentBytes, allowedMediaTypes = ['application/pdf'], allowedPartyRoles = ['client', 'contractor'], now = Date.now, } = {}, { contractorProjectSource, }) {
    ...
    return async function verifyForKernel(binding, untrustedExpected) {
        try {
            ...
            if (!expected)
                return refusal('invalid_expected_binding_context');
            ...
            const options = {
                ...
                expectedSupersedesDigest: expected.supersedes_document_action_binding_digest,
            };
            const authenticated = verifyDocumentActionBinding(binding, options);
            if (!authenticated.valid)
                return refusal(authenticated.reason);
            ...
            const actionTemplate = exactActionTemplate(binding, expected, contractorProjectSource);
            if (!actionTemplate
                || !materialTermsMatchAction(binding, actionTemplate)) {
            ...
            if (!documentBytes)
                return refusal('document_bytes_unavailable');
            ...
            return deepFreeze({
                ...
                ...(HASH.test(actionTemplate.project_record_snapshot_digest)
                    ? {
                        project_record_snapshot_digest: actionTemplate.project_record_snapshot_digest,
                    }
                    : {}),
                ...(verified.supersedes_digest === null
                    ? {}
                    : {
                        supersedes_document_action_binding_digest: verified.supersedes_digest,
                    }),
            });
        }
        catch {
...
//# sourceMappingURL=action-escrow-verifiers.js.map