...
function validateProfile(profile) {
    ...
    for (const field of ['profile_id', 'verifier_id', 'evidence_type', 'gate_id', 'environment_id', 'audience']) {
        if (!string(profile[field]))
            return `profile_${field}_invalid`;
    }
    if (!string(profile.nonce, 1024))
        return 'profile_nonce_invalid';
    ...
    if (!isPlainObject(profile.required_measurements)
        ...
        || Object.keys(profile.required_measurements).length > 32)
        return 'profile_measurements_invalid';
    for (const [name, value] of Object.entries(profile.required_measurements)) {
        if (!string(name, 128) || !digest(value))
            return 'profile_measurements_invalid';
    }
    try {
...
function verifierFor(verifiers, verifierId) {
    let verifier;
    if (verifiers instanceof Map)
        verifier = verifiers.get(verifierId);
...
export async function verifyDeploymentAttestation(evidence, options = {}) {
    ...
    if (invalid)
        return { ...fail('refuse_profile_invalid'), reason: invalid };
    ...
    try {
        claims = JSON.parse(canonicalize(claims));
    }
    catch {
        return {
            ...fail('refuse_evidence_invalid', profileHash, { profile: true, verifier: true }),
    ...
    for (const [name, expected] of Object.entries(profile.required_measurements)) {
        if (!Object.hasOwn(claims.measurements, name))
            missing.push(name);
        else if (claims.measurements[name] !== expected)
...
//# sourceMappingURL=deployment-attestation.js.map