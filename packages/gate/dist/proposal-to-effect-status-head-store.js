 ...
 */
import { statusArtifactDigest, } from '@emilia-protocol/verify/status';
export const PROPOSAL_TO_EFFECT_STATUS_HEAD_STORE_VERSION = 'EP-GATE-PTE-STATUS-HEAD-PG-v1';
...
function snapshotJson(value, seen = new WeakSet()) {
    ...
    if (!dataRecord(value))
        throw new TypeError('status head is not a plain data object');
    const snapshot = {};
...
function targetSnapshot(value) {
    if (!dataRecord(value)
        ...
        || !TARGET_TYPES.has(value.type)
        || !TARGET_USAGES.has(value.usage)
...
function storedHead(result) {
    const rows = exactRowCount(result, 'load status head');
    if (rows === 0 && (!result.rows || result.rows.length === 0))
        return null;
...
function candidateMetadata(status, verification) {
...
function verificationRefusal(verification) {
    return {
        accepted: false,
        source: null,
 ...
 */
export function createPostgresProposalToEffectStatusHeadStore({ pool, tenantId, relyingPartyId, } = {}) {
    if (!pool || typeof pool.connect !== 'function') {
        throw new TypeError('createPostgresProposalToEffectStatusHeadStore requires an ep_aeb_executor pg pool');
    }
    ...
    const store = {
        ...
        async accept(input) {
            ...
            try {
                const current = storedHead(await client.query(PROPOSAL_TO_EFFECT_STATUS_HEAD_SQL.get, [tenantId, relyingPartyId, target.type, target.id, target.digest, target.usage]));
                ...
                const result = await client.query(PROPOSAL_TO_EFFECT_STATUS_HEAD_SQL.compareAndAdvance, [
                    tenantId,
                    relyingPartyId,
                    target.type,
                    target.id,
...
//# sourceMappingURL=proposal-to-effect-status-head-store.js.map