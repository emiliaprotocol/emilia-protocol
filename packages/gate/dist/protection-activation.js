...
import { PROTECTION_PLAN_VERSION, } from './protection-plan.js';
...
export const PROTECTION_ACTIVATION_CLAIM_BOUNDARY = 'customer_pinned_gateway_configuration_not_per_action_authority_connector_coverage_deployment_or_effect_truth';
const BODY_KEYS = [
    '@version',
    'activation_id',
    'tenant_id',
...
function validPlan(plan) {
    if (!riskExact(plan, [
        ...
        || typeof plan.activation.limitation !== 'string')
        return false;
...
export function signProtectionActivation(input, signer) {
...
export function verifyProtectionActivation(artifact, options = {}) {
    ...
    try {
        ...
        return refuse(error?.message || 'protection_activation_invalid');
    }
    const verified = verifyRiskBody(artifact, PROTECTION_ACTIVATION_VERSION, options.trusted_keys);
    if (!verified.valid || !verified.body)
        return refuse(verified.reason || 'protection_activation_invalid');
    const body = verified.body;
    const expected = options.expected;
    if (!expected)
        return refuse('protection_activation_expected_context_required');
    ...
    if (!Number.isFinite(now))
        return refuse('protection_activation_now_invalid');
    if (now < riskInstant(body.valid_from))
        return refuse('protection_activation_not_yet_valid');
    if (now >= riskInstant(body.expires_at))
        return refuse('protection_activation_expired');
    return riskFreeze({
        accepted: true,
        reason: null,
        activation: body,
...
//# sourceMappingURL=protection-activation.js.map