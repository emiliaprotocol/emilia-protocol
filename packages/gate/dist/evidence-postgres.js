...
function verifyHistory(records, head) {
    ...
    for (let index = 0; index < records.length; index++) {
        ...
        if (sequences.has(record.seq))
            return { ok: false, at: record.seq, reason: 'fork_detected' };
        sequences.add(record.seq);
        if (record.seq !== index)
            return { ok: false, at: index, reason: 'sequence_gap_or_rollback' };
...
export function createPostgresEvidenceBackend({ query, tenantId, gateId, } = {}) {
    ...
    async function readHead(streamId) {
        ...
        if (rows.length > 1)
            throw new Error('readHead: Postgres returned multiple scoped heads');
    ...
    async function getById(streamId, recordId) {
        ...
        const rows = queryRows(await runQuery(EVIDENCE_SQL.getById, [...scopeParams(streamId), recordId]), 'getById');
        if (rows.length > 1)
            throw new Error('getById: Postgres returned duplicate scoped record IDs');
    ...
    async function appendIfHead(streamId, expectedHeadHash, record) {
        ...
        const rows = queryRows(await runQuery(EVIDENCE_SQL.appendIfHead, [
            ...params,
            expectedHeadHash,
            persisted,
            canonicalBody,
    ...
    async function readSnapshot(streamId) {
        ...
        if (rows.length !== 1)
            throw new Error('verify: Postgres did not return one scoped snapshot');
        const recordRows = parseJson(rows[0].record_rows, 'verify');
        if (!Array.isArray(recordRows))
            throw new Error('verify: Postgres returned malformed evidence history');
        return {
    ...
    return {
        ...
        async health() {
            ...
            if (rows.length !== 1)
                throw new Error('health: Postgres did not return one readiness row');
            const checks = {
...
//# sourceMappingURL=evidence-postgres.js.map