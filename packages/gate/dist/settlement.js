...
function validateProfile(profile) {
    ...
    if (profile['@version'] !== SETTLEMENT_PROFILE_VERSION || !string(profile.profile_id))
        return 'profile_identity_invalid';
    for (const field of ['require_witness', 'require_outcome', 'require_coverage']) {
        if (profile[field] !== true && profile[field] !== false)
            return `profile_${field}_invalid`;
    }
    ...
    if (profile.require_coverage && (profile.required_coverage_state !== 'gated'
        || !string(profile.required_surface_id)))
        return 'profile_coverage_binding_invalid';
    if (!profile.require_coverage && (profile.required_coverage_state !== undefined
        || profile.required_surface_id !== undefined))
        return 'profile_coverage_fields_forbidden';
    try {
        canonicalize(profile);
    }
    catch {
        return 'profile_canonicalization_invalid';
    }
    return null;
}
...
async function invokeVerifier(verifier, artifact, context) {
    if (typeof verifier !== 'function')
        return { accepted: false, reason: 'pinned_verifier_missing' };
    try {
        const result = await verifier(artifact, Object.freeze({ ...context }));
        if (!isPlainObject(result))
            return { accepted: false, reason: 'verifier_result_invalid' };
...
export async function evaluateSettlementEligibility(bundle = {}, options = {}) {
    ...
    try {
        profileInput = canonicalSnapshot(options.profile);
    }
    catch {
        profileInputInvalid = true;
    }
    ...
    if (invalid)
        return refused('refuse_profile_invalid', invalid, null, baseChecks);
    ...
    if (profile.require_witness) {
        ...
        if (hasTrustedWitnessAcceptance) {
            ...
            try {
                expectedStatementDigest = networkWitnessDigest(evidence.witness);
            }
            catch {
