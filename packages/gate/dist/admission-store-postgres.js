...
export function createAdmissionPostgresStore(options) {
    ...
    const store = {
        ...
        async beginInvocation(input) {
            ...
            try {
                const value = await rpc('admission begin invocation', ADMISSION_POSTGRES_SQL.beginInvocation, [
                    deploymentId,
                    tenantId,
                    cas.admission_id,
                    cas.expected_revision,
                ...
                try {
                    ...
                    throw new AdmissionPostgresAmbiguousBeginError(cas.admission_id, 'beginInvocation acknowledgement and authoritative readback are both unavailable', { cause: readError });
                }
                if (current
                    ...
                    if (!snapshot) {
                        throw new AdmissionPostgresAmbiguousBeginError(cas.admission_id, 'beginInvocation committed but its authoritative snapshot is unavailable', { cause: error });
                    }
                    return {
                        ok: true,
                        snapshot,
                ...
                throw new AdmissionPostgresAmbiguousBeginError(cas.admission_id, 'beginInvocation outcome is not provably attributable to this invocation token', { cause: error });
            }
        },
