    ...
    function copy(current, depth) {
        ...
        if (typeof current === 'string') {
            ...
            if (stringBytes > MAX_STRING_BYTES)
                fail(`${label} exceeds string limits`);
            return current;
        }
        ...
        if (seen.has(current))
            fail(`${label} contains an alias or cycle`);
        seen.add(current);
...
function expectedCore(record, overrides = {}) {
    return {
        agreement_digest: record.agreement_digest,
        document_action_binding_digest: overrides.document_action_binding_digest ?? record.document_action_binding_digest,
...
function sameParties(left, right) {
    ...
    return expected.size === left.length && right.every((party) => (exactKeys(party, PARTY_KEYS) && expected.has(partyIdentity(party))));
}
function validatePartiesAndProfile(record) {
    ...
    validateStringArray(record.profile.required_acceptance_party_ids, ids, 'required acceptance party roster');
    validateStringArray(record.profile.required_release_approver_party_ids, ids, 'required release party roster');
...
function validateHistory(record) {
    ...
    for (const [index, entry] of record.history.entries()) {
        ...
        if (entry.operation === 'release') {
            ...
            releaseHistoryCounts.set(entry.idempotency_key, (releaseHistoryCounts.get(entry.idempotency_key) ?? 0) + 1);
        }
    ...
    if (previous !== record.state)
        fail('kernel state does not match its history');
 ...
 * }} [options]
 */
function validateBindingContainer(container, record, { bindingDigest = record.document_action_binding_digest, actionDigest = record.release_action_digest, supersedesDigest = null, documentDigest = null, documentLength = null, } = {}) {
    ...
    if (!exactKeys(verification, BINDING_VERIFICATION_KEYS, requiredVerification)
        ...
        || !sameCanonical(verification.release_action_template, releaseActionTemplate)
    ...
    return {
        ...
        documentDigest: bindingDocumentDigest,
        projectRecordSnapshotDigest: releaseActionTemplate.project_record_snapshot_digest ?? null,
    };
}
function validateAcceptances(entries, record, core, { requireComplete, requireIncomplete = false, } = {}) {
    if (!Array.isArray(entries))
        fail('agreement acceptances are malformed');
...
function validateMilestone(record) {
    if (!exactKeys(record.milestone_evidence, CONTAINER_KEYS)
        ...
        || !exactKeys(record.milestone_evidence.verification, MILESTONE_VERIFICATION_KEYS)
        || !verificationMatchesCore(record.milestone_evidence.verification, expectedCore(record))
        || !validDigest(record.milestone_evidence.verification.evidence_digest)
        || !record.parties.some((party) => party.party_id
    ...
    const submission = record.operations.find((operation) => operation.operation === 'submit_milestone');
    if (!submission
...
function validateReleaseApprovals(record, milestoneVerification) {
    if (!Array.isArray(record.release_approvals))
        fail('release approvals are malformed');
    const parties = new Map(record.parties.map((party) => [party.party_id, party]));
    ...
    const seenKeys = new Set();
    const result = [];
    const approvalOperations = record.operations.filter((operation) => operation.operation === 'approve_release');
...
function validateRelease(record) {
    ...
    if (request.method !== 'POST'
        ...
        || !sameCanonical(request.release_action_template, record.document_action_binding.verification.release_action_template)
    ...
    const execution = record.operations.find((entry) => (entry.operation === 'release'
        && entry.idempotency_key === release.operation_idempotency_key));
    if (!execution)
        fail('release execution record is missing');
    ...
    if (release.status === 'reserved') {
        ...
        if (release.reconciled_at === null)
            fail('reconciled release is missing its instant');
...
function validateAncillaryState(record) {
    if (record.state === 'disputed') {
        ...
        validateCommandAuthorization(record.dispute.authorization, record, 'open_dispute', record.dispute.party_id, { reason: record.dispute.reason });
    }
    ...
    if (record.state === 'cancelled') {
        if (!exactKeys(record.cancellation, CANCELLATION_KEYS)
            || !record.parties.some((party) => party.party_id === record.cancellation.party_id)
        ...
        validateCommandAuthorization(record.cancellation.authorization, record, 'cancel', record.cancellation.party_id, { reason: record.cancellation.reason });
    }
    ...
    if (record.state === 'completed') {
        ...
        validateCommandAuthorization(record.completion.authorization, record, 'complete', record.completion.party_id, { meaning: 'administrative_archive_only' });
    }
...
function validatePendingAmendment(record) {
    ...
    if (record.pending_amendment === null) {
        if (record.state === 'amendment_pending')
            fail('pending amendment is missing');
        return;
    }
    if (!pendingAllowed
        ...
        || record.pending_amendment.document_action_binding_digest
            === record.document_action_binding_digest
        || !record.parties.some((/** @type {{party_id: string}} */ party) => party.party_id
    ...
    if (record.state === 'amendment_pending'
        && ['release_reserved', 'release_indeterminate'].includes(record.pending_amendment.from_state)) {
    ...
    validateBindingContainer(record.pending_amendment.document_action_binding, record, {
    ...
    const pendingCore = expectedCore(record, {
        document_action_binding_digest: record.pending_amendment.document_action_binding_digest,
        release_action_digest: record.pending_amendment.release_action_digest,
    });
    validateAcceptances(record.pending_amendment.agreement_acceptances, record, pendingCore, { requireComplete: false, requireIncomplete: true });
    validateCommandAuthorization(record.pending_amendment.proposal_authorization, record, 'propose_amendment', record.pending_amendment.proposer_party_id, {
        next_document_action_binding_digest: record.pending_amendment.document_action_binding_digest,
...
function validateLifecycle(record) {
    ...
    const agreementAcceptances = validateAcceptances(record.agreement_acceptances, record, expectedCore(record), {
...
function validateLifecycleOperationCoverage(record) {
    ...
    const expectedAmendmentAcceptances = record.superseded_bindings.length * acceptanceCount
    ...
    requireExact('begin_acceptance', record.history.some((/** @type {{to: string}} */ entry) => entry.to === 'awaiting_acceptance')
        ? 1 : 0);
    requireExact('accept_agreement', expectedInitialAcceptances);
    requireExact('request_funding', record.history.some((/** @type {{to: string}} */ entry) => entry.to === 'awaiting_funding')
        ? 1 : 0);
    ...
    requireExact('propose_amendment', record.superseded_bindings.length + (record.pending_amendment === null ? 0 : 1));
...
function validateKernelRecord(record) {
    ...
    if (record.escrow_key !== expectedEscrowKey)
        fail('kernel escrow key mismatch');
...
function validateDocumentExecution(value, expected, required) {
    if (value === null) {
        if (required)
            fail('required document-execution artifact is missing');
        return null;
    }
    if (!isRecord(value)
...
function stateStatementDigest(statement) {
    ...
    const bytes = Buffer.from(ACTION_ESCROW_STATE_STATEMENT_DOMAIN + canonicalize(body), 'utf8');
 ...
 */
function validateStateStatement(statement, record, agreementId, amendmentDigests, assembledAt) {
    if (!exactKeys(statement, STATE_STATEMENT_KEYS)
        ...
        || statement.payload.amendment_digests.some((/** @type {string} */ digest, /** @type {number} */ index) => digest
...
function evaluationInstant(value) {
    ...
    if (!Number.isFinite(parsed))
        fail('now must be a valid instant');
    return parsed;
}
/**
 ...
 */
function assembleEvidencePackage(input, options, { contractorProjectSource }) {
    ...
    const stateStatement = canonicalCopy(input.operatorStateStatement, 'operatorStateStatement');
    const verificationProfile = canonicalCopy(input.verificationProfile, 'verificationProfile');
    ...
    const execution = validateDocumentExecution(documentExecution, {
    ...
    validateStateStatement(stateStatement, record, binding.agreementId, supersessions.amendmentDigests, assembledAt);
...
export function assembleActionEscrowContractorEvidencePackage(input = {}, options = {}) {
    return assembleEvidencePackage(input, options, {
        contractorProjectSource: true,
    });
}
export const buildActionEscrowEvidencePackageFromKernel = assembleActionEscrowEvidencePackage;
export default {
...
//# sourceMappingURL=action-escrow-package.js.map