...
export const STATE_DOMAIN_MIGRATION_CLAIM_BOUNDARY = 'receipted_source_freeze_sealed_import_external_fence_activation_and_tombstone_not_destination_safety_external_truth_or_physical_exclusivity';
const SNAPSHOT_KEYS = [
...
export function signStateDomainMigrationReceipt(input, signer) {
    const body = {
...
export function verifyStateDomainMigrationReceipt(artifact, options = {}) {
    const refuse = (reason) => riskFreeze({
        accepted: false,
        reason,
        receipt: null,
        receipt_digest: null,
    ...
    try {
        ...
        return refuse(error?.message || 'state_domain_migration_receipt_invalid');
    }
    const verified = verifyRiskBody(artifact, STATE_DOMAIN_MIGRATION_RECEIPT_VERSION, options.trusted_keys);
    if (!verified.valid || !verified.body)
        return refuse(verified.reason || 'state_domain_migration_receipt_invalid');
    const body = verified.body;
    const expected = options.expected;
    if (!expected)
        return refuse('state_domain_migration_expected_context_required');
...
function failed(reason, phase, state = 'REFUSED') {
    return riskFreeze({ ok: false, reason, phase, state, receipt: null });
}
async function invoke(adapter, input, phase, receiver = undefined) {
    if (typeof adapter !== 'function')
        return { ok: false, reason: `${phase}_adapter_required` };
    try {
...
export async function migrateStateDomain(input = {}) {
    ...
    if (freeze.ok !== true)
        return failed(freeze.reason || 'source_freeze_refused', 'source_freeze');
    const rawSnapshot = await invoke(input.source.readFrozenSnapshot, context, 'source_snapshot', input.source);
    try {
        validateSnapshot(rawSnapshot);
    }
    catch (error) {
        return failed(error?.message || 'state_domain_snapshot_invalid', 'source_snapshot');
    }
    ...
    if (fenceVerification.verified !== true) {
        return failed(fenceVerification.reason || fenced.reason || 'external_fence_unverified', 'external_fence');
    }
...
//# sourceMappingURL=state-domain-migration.js.map