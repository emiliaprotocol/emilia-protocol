...
function transportSelectorMatches(action, selector = {}) {
...
export function resolveActionControl(manifest, selector = {}) {
    ...
    if (!hasCompleteTransportShape) {
        if (identityGroups.length < 2)
            return { status: 'none', action: null };
        const identityIntersection = actions.filter((action) => (identityGroups.every((group) => group.includes(action))));
