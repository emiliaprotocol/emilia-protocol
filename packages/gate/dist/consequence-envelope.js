...
import { AGILE_SIGNATURE_ALGORITHMS, SIGNATURE_AGILITY_VERSION, signAgileSet, verifyAgileSignatureSet, } from '@emilia-protocol/verify/pq-signature-agility';
import { canonicalizeAeb, digestAeb, } from '@emilia-protocol/verify/aeb-adapter-contract';
...
function validBody(value) {
    ...
    if (!identifier(value.envelope_id) || !identifier(value.state_domain_id)
        ...
        || !validParent(value.parent_allocation))
        return false;
    if (!isObject(value.validity) || !exactKeys(value.validity, ['not_before', 'not_after'])
        ...
        || Date.parse(value.validity.not_before) >= Date.parse(value.validity.not_after))
        return false;
    if (!isObject(value.issuer) || !exactKeys(value.issuer, ['id', 'key_id'])
        || !identifier(value.issuer.id) || !identifier(value.issuer.key_id))
        return false;
    if (!isObject(value.signature_profile)
        ...
        || value.signature_profile.required_algorithms.some((algorithm, index) => algorithm !== CONSEQUENCE_ENVELOPE_REQUIRED_ALGORITHMS[index])
        || !digest(value.contract_digest))
        return false;
    return consequenceEnvelopeContractDigest(value) === value.contract_digest;
}
export function consequenceEnvelopeContractDigest(input) {
    return digestAeb({
...
export function consequenceEnvelopeSignedBytes(body) {
    return Buffer.from(`${CONSEQUENCE_ENVELOPE_DOMAIN}${canonicalizeAeb(body)}`, 'utf8');
}
export function consequenceEnvelopeDigest(body) {
    return digestAeb({ domain: `${CONSEQUENCE_ENVELOPE_VERSION}:artifact`, body });
}
export async function issueConsequenceEnvelope(draft, options) {
    const unsigned = frozenClone({
        ...draft,
    ...
    const body = {
        ...
        contract_digest: consequenceEnvelopeContractDigest(unsigned),
    };
    if (!validBody(body))
        throw new TypeError('consequence_envelope_malformed');
    if (!Array.isArray(options?.signing_keys)
        || options.signing_keys.length !== CONSEQUENCE_ENVELOPE_REQUIRED_ALGORITHMS.length
        || options.signing_keys.some((key, index) => key.alg !== CONSEQUENCE_ENVELOPE_REQUIRED_ALGORITHMS[index]))
        throw new TypeError('consequence_envelope_algorithm_set_mismatch');
    const signatures = await signAgileSet(consequenceEnvelopeSignedBytes(body), options.signing_keys, options);
    return frozenClone({ '@version': CONSEQUENCE_ENVELOPE_VERSION, body, signatures });
}
export async function verifyConsequenceEnvelope(value, options) {
    const refuse = (reason) => ({
        verified: false,
        reason,
    ...
    if (!isObject(value) || !exactKeys(value, ['@version', 'body', 'signatures'])
        ...
        return refuse('consequence_envelope_malformed');
    if (!instant(options?.now))
        return refuse('consequence_envelope_clock_invalid');
    const body = value.body;
    if (Date.parse(options.now) < Date.parse(body.validity.not_before))
        return refuse('consequence_envelope_not_yet_valid');
    if (Date.parse(options.now) >= Date.parse(body.validity.not_after))
        return refuse('consequence_envelope_expired');
    const signatures = await verifyAgileSignatureSet(consequenceEnvelopeSignedBytes(body), value.signatures, options.verification_keys, {
        ...options,
        policy: 'hybrid_all',
        requiredAlgorithms: [...CONSEQUENCE_ENVELOPE_REQUIRED_ALGORITHMS],
    });
    if (signatures.verified !== true)
        return refuse(`consequence_envelope_signature_refused:${signatures.reason ?? 'unknown'}`);
    return {
        verified: true,
        reason: null,
...
function impactProfile(id, version, unit, definition, derive) {
...
export const FINANCE_CUMULATIVE_EXPOSURE_PROFILE = impactProfile('EP-CONSEQUENCE-IMPACT-FINANCE-MINOR-UNITS-v1', '1', 'currency-minor-unit', { action_type: 'finance.vendor-payment.1', material_field: 'amount_minor', rule: 'positive_integer' }, (action) => {
    if (!isObject(action) || action.action_type !== 'finance.vendor-payment.1'
        || !identifier(action.currency))
        return { ok: false, reason: 'consequence_impact_action_invalid' };
...
export const GRID_ACTIVE_POWER_PROFILE = impactProfile('EP-CONSEQUENCE-IMPACT-GRID-ACTIVE-POWER-v1', '1', 'watt', {
 ...
 */
export const GRACE_CURTAILMENT_IMPACT_PROFILE = impactProfile('ep:impact-profile:grace-curtailment:v1', '1', 'watt', {
...
export function createMemoryConsequenceEnvelopeStore() {
    let binding = null;
    const rows = new Map();
    let held = 0n;
    let committed = 0n;
    ...
    return {
        ...
        async reserve(input) {
            ...
            if (prior)
                return { reserved: false, reason: 'consequence_envelope_operation_conflict' };
            ...
            const owner = `owner:consequence-envelope:${++ownerCounter}:${crypto.randomBytes(12).toString('base64url')}`;
        ...
        async transition(input) {
            ...
            const { owner: _owner, expected_state: _expectedState, next_state: _nextState, ...presentedBinding } = input;
        ...
        async recover(input) {
            ...
            if (!row || row.binding.action_digest !== input.action_digest
                || !['ENTERED', 'INDETERMINATE'].includes(row.state))
                return null;
...
function secureStore(value, allowTestStore) {
...
export async function createConsequenceEnvelopeBoundary(options) {
    ...
    if (!bound)
        throw new TypeError('consequence_envelope_store_binding_mismatch');
    ...
    async function reserveUnits(input) {
        if (!identifier(input?.operation_id) || !digest(input?.action_digest)
            || !decimal(input?.impact_units))
            return refuse('consequence_envelope_reservation_invalid');
        if (input.state_domain_id !== body.state_domain_id)
            return refuse('consequence_envelope_state_domain_mismatch');
        if (input.expected_epoch !== body.epoch)
            return refuse('consequence_envelope_epoch_mismatch');
        const current = now();
        if (!instant(current))
            return refuse('consequence_envelope_clock_invalid');
        if (Date.parse(current) < Date.parse(body.validity.not_before))
            return refuse('consequence_envelope_not_yet_valid');
        if (Date.parse(current) >= Date.parse(body.validity.not_after))
            return refuse('consequence_envelope_expired');
        const binding = {
            envelope_id: body.envelope_id,
            envelope_digest: verified.envelope_digest,
    ...
    const boundary = {
        ...
        async reserve(input) {
            ...
            if (!actionDigest)
                return refuse('consequence_envelope_action_invalid');
            const impact = options.profile.derive(input.action);
            if (!impact.ok || impact.impact_units <= 0n)
                return refuse(impact.ok ? 'consequence_impact_amount_invalid' : impact.reason);
            return reserveUnits({
                operation_id: input.operation_id,
                state_domain_id: input.state_domain_id,
        ...
        async reconcile(input) {
            ...
            if (!reservation)
                return refuse('consequence_envelope_recovery_binding_mismatch');
...
//# sourceMappingURL=consequence-envelope.js.map