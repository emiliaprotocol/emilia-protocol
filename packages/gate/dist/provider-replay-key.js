 ...
 */
function validCaid(value) {
    if (typeof value !== 'string' || value.length === 0 || value.length > 512)
        return false;
    if (value.startsWith('caid:'))
        return CAID_STRING_RE.test(value);
    return ACTION_TYPE_RE.test(value) || PROFILE_ACTION_RE.test(value);
}
function validateSlotSpec(slotSpec) {
    if (!isPlainObject(slotSpec)) {
        return refuse('slot_spec_invalid', 'slot_spec must be a plain object');
    }
    const { slot_id: slotId, charset, encoded_length: encodedLength, max_length: maxLength, prefix = '', charset_applies_to_prefix: charsetAppliesToPrefix = false, min_entropy_bits: minEntropyBits = DEFAULT_MIN_ENTROPY_BITS, forbid_leading: forbidLeading = [], forbid_trailing: forbidTrailing = [], forbid_substrings: forbidSubstrings = [], } = slotSpec;
    ...
    if (charsetAppliesToPrefix) {
        ...
        for (const character of prefix) {
            if (!allowed.has(character)) {
                return refuse('slot_prefix_charset_violation', `prefix character ${JSON.stringify(character)} is outside the slot charset`);
            }
        }
    }
    if (prefix.length + encodedLength > maxLength) {
        return refuse('slot_length_exceeds_max', `prefix (${prefix.length}) plus encoded_length (${encodedLength}) exceeds max_length (${maxLength})`);
    }
 ...
 */
function carriedEntropyBits(radix, length) {
    let capacity = 1n;
...
function encodeToCharset(macHex, charset, length) {
...
export function deriveProviderReplayKey(first, caidArg, providerEnvArg, attemptGroupArg, slotSpecArg) {
    const args = typeof first === 'string'
        ? {
        ...
        : (isPlainObject(first) ? first : {});
    const { authorization_digest: authorizationDigest, caid, provider_env: providerEnv, attempt_group: attemptGroup, slot_spec: slotSpec, } = args;
    ...
    if (entropyBits < spec.minEntropyBits) {
        return refuse('slot_capacity_insufficient', `slot carries ${entropyBits} bits, below the ${spec.minEntropyBits} bits this slot requires`);
    }
    let preimage;
    try {
 ...
 */
export function matchesProviderReplayKey(candidate, input) {
 ...
 */
export function authorizationInstanceDigest({ authorization_digest: authorizationDigest, material_action: materialAction, profile, }) {
    if (typeof authorizationDigest !== 'string' || !DIGEST_RE.test(authorizationDigest)) {
...
export const PROVIDER_CARRIAGE_TABLE = Object.freeze([
    Object.freeze({
...
export function getCarriageRow(id) {
 ...
 */
export function deriveMcpToolCallReplayKey({ authorization_digest: authorizationDigest, target, tool, args, call_id: callId, server_env: serverEnv, }) {
    ...
    if (instance.ok !== true) {
        return refuse(instance.reason === 'authorization_digest_invalid'
            ? 'authorization_digest_invalid'
            : 'derivation_input_uncanonicalizable', instance.detail);
    }
...
export function createMcpReplayLedger({ windowMs = 15 * 60 * 1000 } = {}) {
    ...
    function prune(now) {
        ...
        for (const [digest, entries] of byContent) {
            ...
            if (kept.length === 0)
                byContent.delete(digest);
            else
                byContent.set(digest, kept);
        }
    }
    return {
        windowMs,
        ...
        begin({ key, contentDigest, now = Date.now() }) {
        ...
        complete({ key, result, now = Date.now() }) {
            const existing = byKey.get(key);
            if (!existing)
                return false;
...
//# sourceMappingURL=provider-replay-key.js.map