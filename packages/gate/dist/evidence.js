...
export function canonicalEvidenceJson(v) {
    ...
    if (Array.isArray(v))
        return `[${v.map(canonicalEvidenceJson).join(',')}]`;
    if (typeof v === 'object') {
    ...
    return {
        ...
        verify() {
            ...
            for (const r of records) {
                const { hash, ...body } = r;
                if (body.prev_hash !== p)
                    return { ok: false, at: r.seq, reason: 'prev_hash_mismatch' };
                if (sha256hex(canonical(body)) !== hash)
                    return { ok: false, at: r.seq, reason: 'hash_mismatch' };
                p = hash;
            }
    ...
    try {
        ...
        if ((atomicRequired && !hasRecordId)
            || (hasRecordId && (typeof record.record_id !== 'string'
                || record.record_id.length < 16 || record.record_id.length > 256)))
            return false;
        ...
        if (expectedEntry !== undefined) {
            if (!expectedEntry || typeof expectedEntry !== 'object' || Array.isArray(expectedEntry))
                return false;
            const entry = Object.fromEntries(Object.entries(record).filter(([key]) => !LOG_RESERVED_FIELDS.has(key)));
            if (canonical(entry) !== canonical(expectedEntry))
                return false;
        }
        return true;
    }
...
function assertLogEntry(entry) {
    ...
    while (stack.length) {
        ...
        if (++nodes > 50000 || depth > 64)
            throw new Error('atomic evidence entry exceeds resource limits');
        if (value === null || typeof value === 'boolean')
            continue;
        if (typeof value === 'number') {
            if (!Number.isSafeInteger(value))
                throw new Error('atomic evidence entry contains a non-safe integer');
            continue;
        }
        if (typeof value === 'string') {
            stringBytes += Buffer.byteLength(value, 'utf8');
            if (stringBytes > 1024 * 1024)
                throw new Error('atomic evidence entry exceeds string limit');
            continue;
        }
        if (typeof value !== 'object')
            throw new Error('atomic evidence entry is not canonical JSON');
        if (seen.has(value))
            throw new Error('atomic evidence entry contains a cycle or alias');
        seen.add(value);
 ...
 * semantics.
 */
export function createAtomicEvidenceLog(backend, { streamId = 'emilia-gate', maxRetries = 32, recordIdFactory = () => crypto.randomUUID(), } = {}) {
    ...
    return {
        ...
        async record(entry) {
            ...
            for (let attempt = 0; attempt < maxRetries; attempt++) {
                ...
                if (!validHead(head))
                    throw new Error('atomic evidence backend returned a malformed head');
        ...
        async all() {
            if (typeof backend.readAll !== 'function')
                throw new Error('atomic evidence backend does not expose readAll()');
            const records = await backend.readAll(streamId);
            if (!Array.isArray(records))
                throw new Error('atomic evidence backend returned malformed history');
            return structuredClone(records);
        },
        async verify() {
            try {
                if (typeof backend.readAll !== 'function')
                    return { ok: false, reason: 'read_all_unavailable' };
                const records = await backend.readAll(streamId);
                if (!Array.isArray(records))
                ...
                if (canonical(head) !== canonical(expectedHead))
...
export function createMemoryAtomicEvidenceBackend() {
    ...
    const state = (streamId) => {
        if (!streams.has(streamId))
            streams.set(streamId, { records: [], byId: new Map() });
    ...
    return {
        ...
        async appendIfHead(streamId, expectedHeadHash, record) {
            ...
            if (actual !== expectedHeadHash || s.byId.has(record.record_id)
                || record.seq !== s.records.length || record.prev_hash !== (head?.hash ?? 'genesis'))
                return false;
