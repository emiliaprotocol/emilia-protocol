...
function validManifest(value, tenantId, caseSetId) {
    if (!exactKeys(value, MANIFEST_KEYS)
        ...
        return false;
    const unique = (selector) => (new Set(value.legs.map(selector)).size === value.legs.length);
...
function validState(value, tenantId, caseSetId) {
    ...
    const legIds = value.manifest.legs.map((leg) => leg.leg_id);
    if (value.observations.some((entry, index) => entry.leg_id !== legIds[index])) {
        return false;
    }
    const statuses = value.observations.map((entry) => entry.status);
    if (value.status === 'open')
        return statuses.every((status) => status === 'pending');
    if (value.status === 'completed')
        return statuses.every((status) => status === 'executed');
    return statuses.includes('indeterminate') && statuses.every((status) => status !== 'pending');
}
...
function originalWithoutEvidence(value) {
...
export function createRemedyCaseSetCoordinator(options) {
    if (!isDataRecord(options)
        ...
        || !Object.entries(options.trustedReceiptKeys).every(([keyId, key]) => validId(keyId) && publicKey(key) !== null)
    ...
    const load = async (caseSetId) => {
        try {
            const result = await store.get({ tenantId, caseSetId });
            if (!isDataRecord(result))
                return fail('store_response_invalid');
            if (result.ok === true)
                return checkedState(result.state, caseSetId);
            if (result.reason === 'case_set_not_found')
    ...
    async function create(input) {
        ...
        if (existing.reason !== 'case_set_not_found')
            return existing;
        const at = clock();
        ...
        const state = {
            ...
            observations: manifest.legs.map((entry) => ({
                leg_id: entry.leg_id,
                status: 'pending',
                case_revision: null,
        ...
        if (!validState(state, tenantId, value.caseSetId))
            return fail('case_set_manifest_invalid');
        try {
            const stored = await store.create(deepFreeze(canonicalCopy(state)));
            if (!isDataRecord(stored))
                return fail('store_response_invalid');
            if (stored.ok === true)
                return checkedState(stored.state ?? state, value.caseSetId);
            if (stored.reason !== 'case_set_exists')
                return fail('store_operation_failed');
    ...
    function verifyChild(child, leg, previous) {
        try {
            ...
            if (previous.status !== 'pending') {
                if (observation.case_revision < previous.case_revision)
                    return fail('child_revision_regression');
    ...
    async function recordChildren(input) {
        let value;
        try {
            value = canonicalCopy(input);
        }
        ...
        if (state.owner_token_digest !== ownerDigest(value.ownerToken))
            return fail('ownership_conflict');
        if (state.revision !== value.expectedRevision)
            return fail('state_transition_conflict');
        if (value.children.length !== state.manifest.legs.length)
        ...
        if (state.manifest.legs.some((leg) => !byLeg.has(leg.leg_id))) {
            return fail('case_set_incomplete');
        }
        const observations = [];
        ...
        if (!Number.isFinite(at))
            return fail('clock_invalid');
        if (at < instant(state.updated_at))
        ...
        if (next.manifest_digest !== state.manifest_digest || !same(next.manifest, state.manifest)
            || !validState(next, tenantId, value.caseSetId))
            return fail('case_set_state_invalid');
        try {
            const stored = await store.compareAndSwap({
                ...
                state: deepFreeze(canonicalCopy(next)),
            });
            if (!isDataRecord(stored))
                return fail('store_response_invalid');
            if (stored.ok === true)
                return checkedState(stored.state ?? next, value.caseSetId);
            if (stored.reason === 'revision_conflict')
                return fail('state_transition_conflict');
            if (stored.reason === 'ownership_conflict')
                return fail('ownership_conflict');
            return fail('store_operation_failed');
        }
        catch {
...
//# sourceMappingURL=remedy-case-set.js.map