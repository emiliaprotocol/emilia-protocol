...
function boundedComponent(value, field) {
...
function documentDigest(version, value) {
    return `sha256:${hashCanonical({ version, value })}`;
}
export function mongoFilterDigest(filter) {
    return documentDigest(MONGODB_FILTER_BINDING_VERSION, filter);
}
export function mongoUpdateDigest(update) {
...
const mongoConnectors = new WeakMap();
...
export function createMongoConnector({ client, cluster } = {}) {
...
export async function guardMongoMutation(gate, connector, { op, params = {}, receipt = null } = {}) {
    const configured = mongoConnectors.get(connector);
    if (!configured)
        throw new TypeError('guardMongoMutation requires a configured MongoDB connector');
