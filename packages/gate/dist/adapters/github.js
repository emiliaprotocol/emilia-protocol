...
const githubAllowanceConnectors = new WeakMap();
...
export async function createGithubAllowanceConnector({ octokit, } = {}) {
 ...
 */
export async function createGithubDeploymentProtectionConnector({ octokit, } = {}) {
 ...
 */
export function guardGithubDeploymentProtectionRule({ connector, params, operationId, ...allowanceOptions }) {
    ...
    return executeWithGateAllowance({
        ...
        executeAction: async (verifiedAction) => {
            ...
            const response = await configured.octokit.request('POST /repos/{owner}/{repo}/actions/runs/{run_id}/deployment_protection_rule', {
            ...
            if (response?.status !== 204)
                throw new Error('github_deployment_review_outcome_indeterminate');
            return response;
        },
    });
}
 ...
 */
export function guardGithubAllowanceMutation({ connector, params, operationId, ...allowanceOptions }) {
    const configured = githubAllowanceConnectors.get(connector);
    if (!configured)
        throw new TypeError('guardGithubAllowanceMutation requires a configured GitHub allowance connector');
