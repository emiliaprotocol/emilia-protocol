...
export function createAdapter({ system, ops }) {
    if (!system || !ops || typeof ops !== 'object')
        throw new Error('createAdapter requires { system, ops }');
    ...
    async function guard(gate, client, { op, params = {}, receipt = null } = {}) {
        if (!gate || typeof gate.run !== 'function')
            throw new Error(`${system} adapter requires an EMILIA Gate (with .run)`);
        const spec = ops[op];
        if (!spec)
            throw new Error(`${system} adapter: unknown op "${op}" (expected one of: ${OPS.join(', ')})`);
        if (!receipt) {
            const refused = await gate.run({ selector: spec.selector, receipt: null, observedAction: null }, () => { throw new Error('unreachable_adapter_effect'); });
        ...
        const actuator = canonicalActuatorObject(typeof spec.actuator === 'function' ? spec.actuator(input, observedAction) : observedAction);
        const out = await gate.run({ selector: spec.selector, receipt, observedAction }, () => spec.perform(client, actuator));
        if (!out.ok) {
