 ...
 */
export function classifyRetention(entries = [], { hotDays = 365, coldDays = 2190, now = Date.now(), legalHold, } = {}) {
    ...
    for (const e of entries) {
        ...
        if (e.hash && held.has(e.hash)) {
            buckets.legal_hold.push(tagged);
            continue;
        }
        const age = ageDays(e.at, now);
        if (age == null) {
            buckets.unknown.push(tagged);
            continue;
        }
        if (age <= hotDays)
            buckets.hot.push(tagged);
        else if (age <= coldDays)
            buckets.cold.push(tagged);
        else
            buckets.expired.push(tagged);
    }
    return {
        ...buckets,
        summary: {
