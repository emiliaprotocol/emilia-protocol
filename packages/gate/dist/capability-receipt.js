...
function capabilityStateDomainDigest() {
    return `sha256:${sha256Hex(Buffer.concat([
        Buffer.from('EP-CAPABILITY-STATE-DOMAIN-v1\0', 'utf8'),
        randomBytes(32),
    ]))}`;
}
function normalizeOptionalStateDomainDigest(value) {
    if (value === undefined || value === null)
        return null;
    return validateActionDigest(value);
}
function capabilityBudgetGuarantee(store, binding) {
    ...
    let expectedStateDomainDigest;
    try {
        expectedStateDomainDigest = normalizeOptionalStateDomainDigest(binding.expected_state_domain_digest);
    }
    catch {
        return { ok: false, reason: 'capability_execution_domain_invalid' };
    }
...
async function verifyPerActionHumanAuthorization({ required, artifact, pins, verifier, action, actionDigest, }) {
...
function normalizeAllowanceStatusAdvance(value) {
    if (!isRecord(value)
    ...
    const assertion = normalizeAllowanceStatusAssertion(Object.fromEntries(['allowance_profile_id', 'allowance_digest', 'revision', 'status_epoch', 'status_head_digest']
...
function allowanceStatusRefusal(current, asserted) {
    if (current.status !== 'active')
        return `allowance_${current.status}`;
 ...
 */
function reservedAllowanceStatusAssertion(state, operation) {
...
function exactAllowanceStatus(current, next) {
...
function allowanceStatusAdvanceConflict(current, next) {
 ...
 */
function capabilityCaidActionFenceDigest(caid) {
 ...
 */
export function verifyCapabilityScope(capability, action, operationId, { resolveCaid = null, verifyActionProfile = null, } = {}) {
    try {
        ...
        async reserveSpend({ capabilityId, capabilityFingerprint, operationNamespace = capabilityId, operationId, actionDigest, actionFenceDigest = actionDigest, amount, currency, controlDomainId, allowanceStatus, now = Date.now }) {
            ...
            try {
                ...
                if (isActionFenceConflict(error) || isLiveActionUniqueViolation(error)) {
                    let holder = null;
                    try {
                        const holderResult = await transaction((query) => query(CAPABILITY_SQL.readActionHolder, [operationNamespace, actionFenceDigest]));
...
export async function executeWithCapability({ capabilityReceipt, secret, action, store, executeAction, gate = null, selector = {}, observedAction = null, trustedIssuerKeys = [], verifyBaseReceipt = null, resolveCaid = null, verifyActionProfile = null, executionDomain = null, requireHumanAuthorization = false, humanAuthorization = undefined, humanAuthorizationPins = undefined, verifyHumanAuthorization = undefined, providerEntryGuard = null, allowanceStatus, controlDomainId, operationId = null, now = Date.now, thresholdSecretVerified = false, } = {}) {
    ...
    if (!domain.ok)
        return { ok: false, reason: domain.reason };
    ...
    try {
        ...
        return {
            ok: false,
            reason: error?.message || 'capability_action_invalid',
            authorization,
            ...composition,
        };
    }
    ...
    let providerEntryEvidence = undefined;
 ...
 * @param {number|(() => number)} [options.now]
 */
export async function reconcileCapabilityOperation({ store, capabilityId, operationNamespace = capabilityId, operationId, action, evidence, verifyEvidence, now = Date.now, } = {}) {
...
export const CAPABILITY_V2_REQUIRED_ALGORITHMS = Object.freeze(['Ed25519', 'ML-DSA-65']);
...
function capabilityV2AlgorithmSetRegistered(algorithms) {
...
function capabilityPqKeyId(publicKeyRawB64u) {
    try {
...
function capabilityEdKeyId(publicKeyB64u) {
    try {
...
function capabilityV2AgilityPassthrough(opts) {
    const out = {};
...
function capabilityUnsignedBodyV2(receipt, capability, requiredAlgorithms) {
    return {
 ...
 */
export function capabilityV2SignedPayload(receipt, capability, requiredAlgorithms = CAPABILITY_V2_REQUIRED_ALGORITHMS) {
...
export async function mintCapabilityReceiptV2(baseReceipt, options = {}) {
    ...
    if (!issuerPrivateKey)
        throw new TypeError('mintCapabilityReceiptV2 requires issuerPrivateKey');
    ...
    if (pqPrivateKey === undefined)
        throw new TypeError('mintCapabilityReceiptV2 requires pqPrivateKey');
    ...
    if (!edId)
        throw new TypeError('mintCapabilityReceiptV2 issuerPrivateKey is not Ed25519');
...
function createPrivateKeyObject(material) {
    if (material && typeof material === 'object' && material.type === 'private') {
        return material;
    }
    return createPrivateKey(material);
}
function capabilitySignatureV2(capabilityReceipt) {
    const signature = capabilityReceipt?.capability_signature;
...
export async function verifyCapabilityReceiptV2(capabilityReceipt, { trustedIssuerKeys = [], allowUntrustedIssuer: _allowUntrustedIssuer = false, mldsaBackend, mldsaBackendLoader, } = {}) {
    try {
        ...
        if (!signature)
            return { ok: false, reason: 'capability_signature_envelope_invalid' };
        ...
        if (!signatures || signatures.length === 0)
            return { ok: false, reason: 'capability_signature_legs_missing' };
        ...
        for (const s of signatures) {
            ...
            if (presented.has(s.alg))
                return { ok: false, reason: 'capability_signature_leg_duplicate' };
            presented.add(s.alg);
        }
        for (const alg of presented) {
            if (!CAPABILITY_V2_REQUIRED_ALGORITHMS.includes(alg)) {
        ...
        for (const alg of CAPABILITY_V2_REQUIRED_ALGORITHMS) {
            if (!presented.has(alg))
                return { ok: false, reason: 'capability_signature_leg_stripped' };
        }
 ...
 */
export async function verifyCapabilityReceiptAny(capabilityReceipt, options = {}) {
    if (isRecord(capabilityReceipt) && capabilityReceipt['@version'] === CAPABILITY_RECEIPT_V2_VERSION) {
