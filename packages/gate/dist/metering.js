...
export function meterUsage(entries = [], { periodStart, periodEnd, retentionYearsDefault = DEFAULT_RETENTION_YEARS, } = {}) {
    ...
    if (endMs < startMs)
        throw new Error('meterUsage: periodEnd must be >= periodStart');
    ...
    entries.forEach((e, index) => {
        ...
        if (t < startMs || t >= endMs)
            return; // outside [start, end)
        if (typeof e.kind !== 'string' || e.kind.length === 0) {
            warnings.push({ index, reason: 'missing_kind' });
            return;
        }
        if (e.kind !== 'decision')
            return; // execution/other records are not billable
        if (e.reason === 'not_guarded')
            return; // pass-throughs are free by design
        protectedActions += 1;
