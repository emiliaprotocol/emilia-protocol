...
export const CLAIM_ASSURANCE_ADMISSIBILITY_RESULT_VERSION = 'EP-CLAIM-ASSURANCE-ADMISSIBILITY-v1';
 ...
 */
export function claimAssuranceResultCandidate(value) {
    if (!isObject(value))
        return null;
    try {
        ...
        if (!typeDescriptor)
            return null;
        if (typeDescriptor.enumerable !== true || !Object.hasOwn(typeDescriptor, 'value'))
            return candidate;
 ...
 */
export function validateClaimAssuranceAdmissibilityResult(value, { expectedProfile = null, expectedActionDigest = null, requireAdmissible = false, } = {}) {
    let block;
    try {
    ...
    if (!isObject(block))
        return fail('result_not_object');
    if (!sameKeys(block, TOP_LEVEL_KEYS))
        return fail('result_shape_not_closed');
    ...
    if (profile.version !== '1')
        return fail('profile_version_invalid');
    if (!validDigest(block.profile_hash))
        return fail('profile_hash_invalid');
    for (const field of [
        'replay_digest', 'assurance_record_digest', 'claim_case_digest', 'action_digest',
    ]) {
        if (!validDigest(block[field]))
    ...
    if (typeof block.profile_satisfied !== 'boolean')
        return fail('profile_satisfied_invalid');
    if (block.authorizes_action !== false)
        return fail('authorizes_action_must_be_false');
    const asOf = instantMilliseconds(block.as_of);
    const evaluatedAt = instantMilliseconds(block.evaluated_at);
    if (asOf === null)
        return fail('as_of_invalid');
    if (evaluatedAt === null)
        return fail('evaluated_at_invalid');
    if (asOf > evaluatedAt)
        return fail('as_of_after_evaluated_at');
    ...
    const permittedAdmissibilityByClaim = {
        VERIFIED: ['admissible', 'stale'],
        DIVERGED: ['conflicted'],
        UNVERIFIED: ['unverifiable'],
        INDETERMINATE: ['missing_evidence', 'stale', 'unverifiable'],
    };
    if (!permittedAdmissibilityByClaim[block.claim_assurance_verdict]
        ?.includes(block.verdict)) {
...
//# sourceMappingURL=claim-assurance-result.js.map