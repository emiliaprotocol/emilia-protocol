 ...
 */
import { RISK_CAID, RISK_DIGEST, riskClone, riskDigest, riskExact, riskFreeze, riskIdentifier, riskInstant, riskRecord, signRiskBody, verifyRiskBody, signRiskBodyV2, verifyRiskBodyV2, } from './reliance-risk-crypto.js';
import { COVERAGE_RECONCILIATION_ATTESTATION_VERSION, COVERAGE_RECONCILIATION_ATTESTATION_V3_VERSION, signCoverageReconciliationAttestation, signCoverageReconciliationAttestationV3, } from './coverage-reconciliation-attestation.js';
...
export const COVERAGE_CLASSIFICATION_RULES = riskFreeze({
 ...
 */
export function resolveCoverageClassificationRule(classification, ruleId) {
...
const RECORD_KEYS_WITH_RULE = [...RECORD_KEYS, 'classification_rule_id'];
 ...
 */
export function assertCoveragePopulationConservation(joins, counts) {
...
function validateSourceBody(value, expectedVersion = COVERAGE_SOURCE_INVENTORY_VERSION) {
...
export function runCoverageReconciliation(input, options, signer) {
    ...
    assertCoveragePopulationConservation(joins, {
        system_record_count: systemBody.record_count,
        receipt_record_count: receiptBody.record_count,
    });
...
export async function signCoverageSourceInventoryV3(input, records, signer, options = {}) {
    const normalized = normalizeRecords(input.inventory_kind, records);
    const body = {
...
export async function verifyCoverageSourceInventoryV3(artifact, records, options = {}) {
    const refuse = (reason, verified = false, inventoryDigest = null) => ({
        accepted: false,
        verified,
        reason,
 ...
 */
export function verifyCoverageReconciliationReportBindingV3(report, attestation) {
    if (!riskRecord(report)
        || report['@version'] !== COVERAGE_RECONCILIATION_REPORT_VERSION) {
        return { accepted: false, reason: 'coverage_report_invalid', report_hash: null };
    }
 ...
 */
export async function runCoverageReconciliationV3(input, options, signer) {
    if (!riskRecord(input)
        || !riskIdentifier(input.run_id)
        || !riskIdentifier(input.attestation_id)
        || !riskIdentifier(input.relying_party_id)
    ...
    const systemVerification = await verifyCoverageSourceInventoryV3(input.system_of_record.artifact, input.system_of_record.records, {
        trusted_keys: options.trusted_keys,
        now: options.now,
        expected_inventory_kind: 'system_of_record',
        expected_source_system_id: options.system_of_record_pin.source_system_id,
    ...
    const receiptVerification = await verifyCoverageSourceInventoryV3(input.receipt_population.artifact, input.receipt_population.records, {
        trusted_keys: options.trusted_keys,
        now: options.now,
        expected_inventory_kind: 'receipt_population',
        expected_source_system_id: options.receipt_population_pin.source_system_id,
