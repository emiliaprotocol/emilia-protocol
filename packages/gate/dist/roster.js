...
export function importRoster(idpUsers, { source, importedAt, allowEmpty = false, } = {}) {
    if (!Array.isArray(idpUsers))
        throw new Error('roster import: idpUsers must be an array');
    ...
    if (!Number.isFinite(atMs))
        throw new Error('roster import: importedAt is not a valid time');
    const warnings = [];
    const candidates = [];
...
export function diffRoster(previous, next) {
    ...
    for (const [principal, g] of nxt) {
        if (!prev.has(principal))
            added.push({ principal, kids: [...g.kids].sort() });
    }
    for (const [principal, g] of prev) {
        if (!nxt.has(principal)) {
            removed.push({ principal, kids: [...g.kids].sort() });
            continue;
        }
        if (g.active && !nxt.get(principal).active)
            deactivated.push({ principal, kids: [...g.kids].sort() });
    }
    const byPrincipal = (a, b) => (a.principal < b.principal ? -1 : a.principal > b.principal ? 1 : 0);
    added.sort(byPrincipal);
    removed.sort(byPrincipal);
    deactivated.sort(byPrincipal);
...
export function applyRosterToRegistry(roster, registry, { revokedAt } = {}) {
    ...
    for (const e of status) {
        if (e.revoked || done.has(e.kid) || activeByKid.has(e.kid))
            continue;
