...
const ALLOWANCE_INPUT_KEYS = ALLOWANCE_BODY_KEYS.filter((key) => !['@version', 'claim_boundary', 'issuer'].includes(key));
...
function capabilityIssuerKeyDigest(key) {
    const publicKey = typeof key === 'object' && !Buffer.isBuffer(key) && key.type === 'public'
        ? key
        : createPublicKey(key);
...
function uniqueBoundedStrings(value, label, max, predicate) {
...
function normalizeConstraints(value) {
    if (!riskExact(value, CONSTRAINT_KEYS))
        throw new TypeError('allowance constraints are not closed');
    const materialFields = uniqueBoundedStrings(value.material_fields, 'allowance constraints.material_fields', MAX_FIELDS, (entry) => FIELD_NAME.test(entry));
    const allowedTargets = uniqueBoundedStrings(value.allowed_targets, 'allowance constraints.allowed_targets', MAX_TARGETS, (entry) => riskIdentifier(entry));
    ...
    for (const field of Object.keys(value.allowed_values).sort()) {
        ...
        allowedValues[field] = uniqueBoundedStrings(value.allowed_values[field], `allowance constraints.allowed_values.${field}`, MAX_TARGETS, (entry) => riskIdentifier(entry));
    }
    const fieldNames = [
...
function normalizeAllowanceInput(value) {
    if (!riskExact(value, ALLOWANCE_INPUT_KEYS))
        throw new TypeError('allowance input is not closed');
    for (const key of [
        'allowance_id',
        'tenant_id',
        'subject_id',
        ...
        if (!riskIdentifier(value[key]))
            throw new TypeError(`allowance ${key} is invalid`);
    }
...
function nowMilliseconds(now) {
    ...
    if (!Number.isSafeInteger(value) || value < 0)
        throw new TypeError('allowance clock is invalid');
    return value;
}
function withoutProof(artifact) {
    if (!riskRecord(artifact))
        throw new TypeError('allowance artifact is invalid');
    const { proof: _proof, ...body } = artifact;
    return body;
}
/** Sign a closed Gate allowance with a customer-controlled Ed25519 key. */
export function signGateAllowance(input, signer) {
...
export function allowanceDigest(artifact) {
...
export function issueGateAllowance({ authorizationReceipt, allowance, predecessorAllowance, signer, capabilityIssuerPrivateKey, capabilityRevocationMode, capabilityId, secret, } = {}) {
    ...
    if (Number(allowance.revision) > 1) {
        ...
        const normalizedPredecessor = normalizeAllowanceInput(Object.fromEntries(ALLOWANCE_INPUT_KEYS.map((key) => [key, predecessorBody[key]])));
    ...
    const boundCapabilityId = capabilityId ??
...
function validateActionAgainstAllowance(action, operationId, body) {
    if (!riskRecord(action))
    ...
    if (action.action_type !== body.action_type)
        return { ok: false, reason: 'allowance_action_type_mismatch' };
    ...
    if (!validPositiveSafeInteger(amount))
        return { ok: false, reason: 'allowance_amount_invalid' };
    ...
    if (currency !== constraints.currency)
        return { ok: false, reason: 'allowance_currency_mismatch' };
    ...
    for (const [field, permitted] of Object.entries(constraints.allowed_values)) {
 ...
 */
export async function executeWithGateAllowance({ allowance, capabilityReceipt, secret, action, operationId, store, executeAction, verifyAuthorizationReceipt, verifyAllowanceStatus, trustedAllowanceKeys, trustedCapabilityIssuerKeys = [], expected, providerEntryGuard = null, controlDomainId, now = Date.now, } = {}) {
    if (!expected)
    ...
    if (!verifiedAllowance.accepted)
        return { ok: false, reason: verifiedAllowance.reason };
    const verifiedArtifact = verifiedAllowance.allowance;
    ...
    try {
        statusVerification = await verifyAllowanceStatus(riskClone(verifiedArtifact), {
    ...
    if (!riskRecord(capability)
        ...
        || capabilityIssuerKeyDigest(createPublicKey({
