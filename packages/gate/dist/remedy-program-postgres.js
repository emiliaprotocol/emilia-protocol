...
export function createRemedyProgramPostgresStore({ pool } = {}) {
    if (!pool || typeof pool.connect !== 'function') {
        throw new TypeError('createRemedyProgramPostgresStore requires a transaction-capable pg pool');
    }
        ...
        try {
            ...
            if (began) {
                try {
                    ...
                    throw new AggregateError([error, rollbackError], 'remedy-program transaction and rollback both failed');
                }
            }
            throw error;
        }
    ...
    return Object.freeze({
        ...
        async create(state) {
            ...
            return transaction(false, async (client) => definitiveRow(await client.query(REMEDY_PROGRAM_POSTGRES_SQL.create, [
                tenantId,
                instanceId,
                encoded.stateJson,
                encoded.stateDigest,
                encoded.eventAt,
            ]), 'create', tenantId, instanceId, new Set([
        ...
        async get(input) {
            ...
            return transaction(true, async (client) => definitiveRow(await client.query(REMEDY_PROGRAM_POSTGRES_SQL.get, [tenantId, instanceId]), 'get', tenantId, instanceId, new Set(['instance_not_found'])));
        },
        async compareAndSwap(input) {
            ...
            return transaction(false, async (client) => definitiveRow(await client.query(REMEDY_PROGRAM_POSTGRES_SQL.compareAndSwap, [
                tenantId,
                instanceId,
                expectedRevision,
                nextRevision,
...
//# sourceMappingURL=remedy-program-postgres.js.map