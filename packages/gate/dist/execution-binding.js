    ...
    while (stack.length) {
        ...
        if (typeof v === 'string') {
            if (!validUnicodeString(v))
                throw new TypeError('value contains invalid Unicode');
            stringBytes += Buffer.byteLength(v, 'utf8');
            if (stringBytes > MAX_JSON_STRING_BYTES)
                throw new TypeError('value exceeds the EP canonical JSON string limit');
            continue;
        }
        if (typeof v === 'number') {
            if (!Number.isSafeInteger(v))
                throw new TypeError('value contains a non-safe integer');
            continue;
        }
        ...
        if (seen.has(v))
            throw new TypeError('value contains a cycle or alias');
        seen.add(v);
        if (Array.isArray(v)) {
...
function canonicalizeValidated(v) {
    if (v === null)
        return 'null';
    if (Array.isArray(v))
        return `[${v.map(canonicalizeValidated).join(',')}]`;
    if (typeof v === 'object') {
...
export function verifyExecutionBinding({ requirement, receipt, observedAction } = {}) {
    ...
    for (const field of requiredFields) {
        ...
        const key = typeof field === 'symbol' ? field : String(field);
        ...
        if (expected.state === 'valid' && actual.state === 'valid'
            && !equalValue(expected.value, actual.value))
            mismatched.push(key);
    }
    ...
    try {
        assertCanonicalJson(signedValues);
    }
    catch {
        for (const field of signedAccepted) {
            if (!invalidSigned.includes(field))
                invalidSigned.push(field);
        }
    }
    try {
        assertCanonicalJson(observedValues);
    }
    catch {
        for (const field of observedAccepted) {
            if (!invalidObserved.includes(field))
