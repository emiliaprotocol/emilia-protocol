...
export function createRuntimeMonitor({ now = Date.now, onDivergence = null, authorizeRecovery = null, } = {}) {
    ...
    function transition(cycleId, event, details = {}) {
        ...
        if (!cycle) {
            return fail({ cycle_id: cycleId, ...details }, RUNTIME_INVARIANTS.WRITE_BYPASS, 'known runtime cycle', 'unknown runtime cycle');
        }
        if (event === 'decision') {
    ...
    return {
        ...
        beginCheck({ action = null, receipt_id = null } = {}) {
            ...
            while (cycles.size > MAX_CYCLES)
                cycles.delete(cycles.keys().next().value);
            return cycle_id;
        },
        preflight({ hasReceipt } = {}) {
            if (mode === RUNTIME_MONITOR_MODES.NORMAL || hasReceipt === true)
                return { ok: true };
            return { ok: false, reason: 'runtime_safe_mode_signoff_required', mode };
        },
        minimumAssuranceTier(declaredTier) {
            if (mode === RUNTIME_MONITOR_MODES.NORMAL || declaredTier === undefined || declaredTier === null)
                return declaredTier;
        ...
        executionSkipped(cycleId) {
            ...
            if (!cycle || cycle.phase !== 'consumed') {
                return fail(cycle || { cycle_id: cycleId }, RUNTIME_INVARIANTS.WRITE_BYPASS, 'consumed → execution skipped', `${cycle?.phase || 'unknown'} → execution skipped`);
            }
        ...
        recover(input = {}) {
            if (mode === RUNTIME_MONITOR_MODES.NORMAL)
                return { ok: true, mode };
