...
const CUSTODY_REASONS = Object.freeze({
 ...
 */
function normalizeOperatorMode(config) {
    let value = config;
    if (config && typeof config === 'object' && !Array.isArray(config)) {
        value = config.hybrid_issuance;
    }
    ...
    if (value === false)
        value = 'disabled';
    if (typeof value !== 'string' || !HYBRID_ISSUANCE_MODES.includes(value)) {
        throw new Error(`hybrid_issuance must be one of ${HYBRID_ISSUANCE_MODES.join(', ')}; got ${JSON.stringify(value)}`);
    }
    return value;
}
/**
 ...
 */
export function resolveHybridReceiptProfile(config, custody) {
...
let cachedIssuance;
function isIssuanceModule(value) {
    const m = value;
...
export async function loadHybridIssuanceModule() {
    if (cachedIssuance !== undefined)
...
async function resolveIssuance(injected) {
 ...
 */
export async function issueUnderHybridProfile(args) {
    ...
    if (!profile.issues_hybrid)
        return refuse(HYBRID_PROFILE_REASONS.HYBRID_ISSUANCE_DISABLED);
    if (!hasSigningMaterial(args))
 ...
 */
async function mintHybrid(module, args) {
    ...
    try {
        const receipt = useSigner
            ? await module.createHybridReceiptFromSignSet({
                payload,
                signSet: (bytes, context) => hybridSigner.signSet(bytes, context),
...
function hasSigningMaterial(args) {
    if (args.hybridSigner && typeof args.hybridSigner.signSet === 'function')
        return true;
...
export async function acceptUnderHybridProfile(args) {
    ...
    if (version === HYBRID_RECEIPT_PROFILE_ID) {
        if (!profile.issues_hybrid)
            return refuse(HYBRID_PROFILE_REASONS.HYBRID_RECEIPT_NOT_ACCEPTED);
        ...
        if (result?.verified !== true)
            return refuse(String(result?.reason ?? 'signature_invalid'), result ?? null);
        return { ok: true, profile: HYBRID_RECEIPT_PROFILE_ID, detail: result };
    }
    if (version === CLASSICAL_RECEIPT_PROFILE_ID) {
        if (profile.requires_hybrid)
            return refuse(HYBRID_PROFILE_REASONS.HYBRID_REQUIRED);
        if (typeof verifyClassical !== 'function')
            return refuse(HYBRID_PROFILE_REASONS.CLASSICAL_VERIFIER_MISSING);
        const result = await verifyClassical(receipt);
        if (result?.valid !== true)
            return refuse(String(result?.error ?? 'signature_invalid'), result ?? null);
...
//# sourceMappingURL=hybrid-receipt-profile.js.map