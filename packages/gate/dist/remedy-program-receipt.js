...
const REMEDY_PROGRAM_PROFILE_V1_ONLY = new Set([REMEDY_PROGRAM_VERSION]);
const REMEDY_PROGRAM_PROFILE_V1_OR_V2 = new Set([
...
function canonicalCopy(value, label) {
    try {
        ...
        if (error instanceof TypeError && /prototype-named field/.test(error.message))
            throw error;
...
function validOutcomeEvidence(value) {
...
function ownerDigest(value) {
    if (value.consequence_mode === 'receipt-program'
        ...
        return value.capability_template_digest;
    if (value.consequence_mode === 'action-escrow'
        ...
        return value.escrow_profile_digest;
    return null;
}
 ...
 */
function validStateUnder(state, allowedProfileVersions) {
    if (!exactKeys(state, STATE_KEYS)
        ...
        || !state.used_evidence_digests.every((entry) => (typeof entry === 'string' && DIGEST.test(entry)))
        ...
        || !DIGEST.test(state.create_request_digest))
        return false;
    ...
    return attempts.every((attempt) => (attempt.original_operation_id === state.original.operation_id
        && attempt.destination_binding_digest === state.destination_binding_digest
        && attempt.unit === state.unit));
}
function selectAttempt(state, remedyOperationId) {
    const attempts = [
        ...(state.active_remedy === null ? [] : [state.active_remedy]),
        ...state.remedies,
    ].filter((attempt) => attempt.remedy_operation_id === remedyOperationId);
    if (attempts.length === 0)
        throw new TypeError('remedy operation is absent from the state snapshot');
    if (attempts.length !== 1)
        throw new TypeError('remedy operation is not unique in the state snapshot');
...
function expectedBindings(state, attempt) {
    return {
...
function snapshotAndAttempt(value, remedyOperationId, allowedProfileVersions = REMEDY_PROGRAM_PROFILE_V1_ONLY) {
    const state = canonicalCopy(value, 'remedy state snapshot');
    if (!validStateUnder(state, allowedProfileVersions))
        throw new TypeError('remedy state snapshot is invalid');
    if (!validId(remedyOperationId))
        throw new TypeError('remedyOperationId is invalid');
...
export function expectedRemedyProgramReceiptBindings(state, remedyOperationId) {
...
function payloadFor(state, attempt) {
    return {
        case: {
...
export function remedyProgramReceiptSigningBytes(receipt) {
    if (!isDataRecord(receipt))
        throw new TypeError('remedy receipt signing body is invalid');
    return Buffer.from(ACTION_REMEDY_RECEIPT_DOMAIN + canonicalize(signingBody(receipt)), 'utf8');
}
...
function receiptExpected(receipt) {
    return {
...
function configureSigner(options, context) {
    ...
    if (hasPrivateKey) {
        ...
        return Object.freeze({
            ...
            sign: async (bytes) => crypto.sign(null, bytes, key),
        });
    }
    const signer = options.signer;
    if (!isDataRecord(signer))
        throw new TypeError('remedy receipt signer must be a data object');
 ...
 */
export async function issueRemedyProgramReceipt(input = {}, options = {}) {
    ...
    const context = canonicalCopy(options.context, 'remedy receipt context');
    ...
    const signer = configureSigner(options, context);
 ...
 */
export function verifyRemedyProgramReceipt(receipt, { trustedKeys, expectedIssuer, state, expected, } = {}) {
    const checks = {
        structure: false,
        payload: false,
    ...
    try {
        const snapshot = canonicalCopy(receipt, 'remedy receipt');
        ...
        checks.key = key !== null;
        if (!checks.key)
            return refusal('receipt_key_untrusted', checks);
        ...
        if (!checks.signature)
            return refusal('receipt_signature_invalid', checks);
        let stateCopy;
        try {
        ...
        try {
            ...
            derivedAttempt = selectAttempt(stateCopy, expectedCopy.remedy_operation_id);
        }
        catch {
            return refusal('receipt_expected_binding_mismatch', checks);
        }
        checks.expected_bindings = sameCanonical(receiptExpected(snapshot), expectedCopy)
            && sameCanonical(expectedBindings(stateCopy, derivedAttempt), expectedCopy)
            && sameCanonical(payloadFor(stateCopy, derivedAttempt), snapshot.payload);
...
export const ACTION_REMEDY_RECEIPT_V2_REQUIRED_ALGORITHMS = Object.freeze(['Ed25519', 'ML-DSA-65']);
 ...
 */
export function expectedRemedyProgramReceiptV2Bindings(state, remedyOperationId) {
    const selected = snapshotAndAttempt(state, remedyOperationId, REMEDY_PROGRAM_PROFILE_V1_OR_V2);
    return deepFreeze(expectedBindings(selected.state, selected.attempt));
}
function remedyV2AlgorithmSetRegistered(algorithms) {
...
function remedyPqKeyId(publicKeyRawB64u) {
    try {
...
function remedyAgilityPassthrough(opts) {
    const out = {};
 ...
 */
export function remedyProgramReceiptV2SigningBytes(receipt, requiredAlgorithms = ACTION_REMEDY_RECEIPT_V2_REQUIRED_ALGORITHMS) {
    if (!isDataRecord(receipt))
        throw new TypeError('remedy receipt v2 signing body is invalid');
    ...
    return Buffer.from(ACTION_REMEDY_RECEIPT_V2_DOMAIN + canonicalize({
        ...signingBody(receipt),
        required_algorithms: [...requiredAlgorithms],
    }), 'utf8');
}
function configureHybridSigner(options, context) {
    const hasKeys = options.keys !== undefined;
    const hasSigner = options.signer !== undefined;
    if (hasKeys === hasSigner)
        throw new TypeError('configure exactly one remedy receipt v2 signer');
    const allowEphemeralState = options.allowEphemeralState === true;
    if (hasKeys) {
        if (!allowEphemeralState) {
        ...
        return {
            ...
            signSet: async (bytes) => signAgileSet(new Uint8Array(bytes), signingKeys),
        };
    }
    const signer = options.signer;
    if (!isDataRecord(signer))
        throw new TypeError('remedy receipt v2 signer must be a data object');
    ...
    return {
        ...
        signSet: async (bytes) => {
            ...
            if (!Array.isArray(set)
                || !ACTION_REMEDY_RECEIPT_V2_REQUIRED_ALGORITHMS.every((alg, index) => (set[index]?.alg === alg && typeof set[index]?.sig === 'string'))) {
 ...
 */
export async function issueRemedyProgramReceiptV2(input = {}, options = {}) {
    if (!exactKeys(input, new Set(['state', 'remedyOperationId']))) {
        throw new TypeError('remedy receipt input must contain exactly state and remedyOperationId');
    }
    const selected = snapshotAndAttempt(input.state, input.remedyOperationId, REMEDY_PROGRAM_PROFILE_V1_OR_V2);
    const context = canonicalCopy(options.context, 'remedy receipt context');
    if (!validIssuer(context)) {
        throw new TypeError('remedy receipt context must contain exact pinned issuer fields');
    }
    ...
    const signer = configureHybridSigner(options, context);
    if (signer.keyId !== context.key_id) {
        throw new TypeError('remedy receipt context key_id does not match signer keyId');
    }
 ...
 */
export async function verifyRemedyProgramReceiptV2(receipt, { trustedKeys, expectedIssuer, state, expected, mldsaBackend, mldsaBackendLoader, } = {}) {
    const checks = {
        structure: false,
        payload: false,
        content_digest: false,
    ...
    try {
        ...
        if (!checks.algorithm_set)
            return refusal('receipt_algorithm_set_unsupported', checks);
        ...
        for (const alg of ACTION_REMEDY_RECEIPT_V2_REQUIRED_ALGORITHMS) {
            if (!presented.has(alg))
                return refusal('receipt_signature_leg_missing', checks);
        }
        for (const alg of presented) {
            if (!ACTION_REMEDY_RECEIPT_V2_REQUIRED_ALGORITHMS.includes(alg)) {
                return refusal('receipt_signature_set_invalid', checks);
            }
        }
        ...
        let setResult;
        try {
            setResult = await verifyAgileSignatureSet(new Uint8Array(remedyProgramReceiptV2SigningBytes(snapshot, ACTION_REMEDY_RECEIPT_V2_REQUIRED_ALGORITHMS)), signatures, [
        ...
        if (!checks.signature) {
            return refusal(`receipt_signature_invalid:${String(setResult?.reason ?? 'signature_set_unverified')}`, checks);
        }
 ...
 */
export async function verifyRemedyProgramReceiptStatement(receipt, options = {}) {
...
//# sourceMappingURL=remedy-program-receipt.js.map