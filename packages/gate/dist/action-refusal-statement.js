...
export const ACTION_REFUSAL_V2_REQUIRED_ALGORITHMS = Object.freeze(['Ed25519', 'ML-DSA-65']);
const REFUSAL_PROOF_V2_KEYS = [
    'profile', 'required_algorithms', 'public_key', 'key_id',
    'pq_public_key', 'pq_key_id', 'signatures',
];
function refusalV2AlgorithmSetRegistered(algorithms) {
...
function refusalPqKeyId(publicKeyRawB64u) {
    try {
...
function refusalEdKeyId(publicKeyB64u) {
    try {
...
function refusalAgilityPassthrough(opts) {
    const out = {};
 ...
 */
export function actionRefusalV2SignedPayload(body, requiredAlgorithms = ACTION_REFUSAL_V2_REQUIRED_ALGORITHMS) {
    ...
    return Buffer.from(ACTION_REFUSAL_STATEMENT_V2_DOMAIN + canonicalize({ ...body, required_algorithms: [...requiredAlgorithms] }), 'utf8');
}
/**
 ...
 */
export async function signActionRefusalStatementV2(input, signer, options = {}) {
    ...
    if (edKey.asymmetricKeyType !== 'ed25519')
        throw new TypeError('action refusal v2 Ed25519 key is invalid');
    ...
    if (!edId || !pqId)
        throw new TypeError('action refusal v2 public key material is invalid');
 ...
 */
export async function verifyActionRefusalStatementV2(statement, options = {}) {
    const checks = {
        version: false,
        structure: false,
    ...
    const refuse = (reason, refusalDigest = null) => riskFreeze({
        accepted: false,
        verified: false,
        reason,
        refusal_digest: refusalDigest,
    ...
    try {
        if (!riskRecord(statement) || !riskRecord(statement.proof)
            || !riskRecord(statement.issuer)) {
            return refuse('refusal_shape_invalid');
        }
        const { proof, ...bodyNoProof } = statement;
        checks.version = bodyNoProof['@version'] === ACTION_REFUSAL_STATEMENT_V2_VERSION;
        if (!checks.version)
            return refuse(`unsupported version: ${String(bodyNoProof['@version'])}`);
        ...
        if (!checks.structure)
            return refuse('refusal_proof_envelope_invalid');
        ...
        if (!checks.issuer_pin)
            return refuse('refusal_issuer_untrusted', refusalDigest);
        ...
        if (!checks.issuer_key_bound)
            return refuse('refusal_issuer_key_unbound', refusalDigest);
        checks.algorithm_set = refusalV2AlgorithmSetRegistered(proof.required_algorithms);
        if (!checks.algorithm_set)
            return refuse('refusal_algorithm_set_invalid', refusalDigest);
        const signatures = Array.isArray(proof.signatures) ? proof.signatures : null;
        if (!signatures || signatures.length === 0)
            return refuse('refusal_signature_legs_missing', refusalDigest);
        ...
        for (const s of signatures) {
            ...
            if (presented.has(s.alg))
                return refuse('refusal_signature_leg_duplicate', refusalDigest);
            presented.add(s.alg);
        }
        for (const alg of presented) {
            if (!ACTION_REFUSAL_V2_REQUIRED_ALGORITHMS.includes(alg)) {
        ...
        for (const alg of ACTION_REFUSAL_V2_REQUIRED_ALGORITHMS) {
            if (!presented.has(alg))
                return refuse('refusal_signature_leg_stripped', refusalDigest);
        }
        ...
        if (riskInstant(bodyNoProof.refused_at) > now + skew * 1000)
            return refuse('refusal_from_future', refusalDigest);
        if (now >= riskInstant(bodyNoProof.expires_at))
            return refuse('refusal_expired', refusalDigest);
        checks.time_window = true;
        const mismatch = expectedMismatch(bodyNoProof, options.expected);
        if (mismatch)
            return refuse(mismatch, refusalDigest);
 ...
 */
export async function verifyActionRefusalStatementAny(statement, options = {}) {
    if (riskRecord(statement) && statement['@version'] === ACTION_REFUSAL_STATEMENT_V2_VERSION) {
