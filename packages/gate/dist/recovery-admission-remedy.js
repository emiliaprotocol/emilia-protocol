...
function receiptReason(reason) {
    if (reason === 'receipt_state_snapshot_mismatch')
        return 'remedy_receipt_state_mismatch';
    if (reason === 'receipt_expected_binding_mismatch')
        return 'remedy_receipt_binding_mismatch';
    return 'remedy_receipt_invalid';
}
function ownerAllowed(owners, remedy) {
 ...
 */
export function createRecoveryAdmissionRemedyBridge(options) {
    ...
    async function reserve(input) {
        ...
        const authorizationInputs = admission.body.inputs.filter((entry) => entry.role === 'authorization');
...
//# sourceMappingURL=recovery-admission-remedy.js.map