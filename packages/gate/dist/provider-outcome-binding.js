 ...
 */
import { verifyOutcomeObservationV2, } from '@emilia-protocol/verify/outcome-binding';
import { RISK_CAID, RISK_DIGEST, riskClone, riskDigest, riskExact, riskFreeze, riskIdentifier, riskInstant, riskRecord, } from './reliance-risk-crypto.js';
...
export const PROVIDER_OUTCOME_BINDING_CLAIM_BOUNDARY = 'verified_source_observation_bound_to_one_exact_provider_context_not_provider_truth_not_external_effect_not_coverage_not_claim_adjudication';
...
const SOURCE_KEYS = ['role', 'source_id', 'source_class'];
const SOURCE_KEYS_WITH_FACILITY = [...SOURCE_KEYS, 'facility_id'];
...
function validContext(value) {
    return riskExact(value, CONTEXT_KEYS)
        ...
        && PROVIDER_OUTCOMES.includes(value.outcome)
...
function fullV2Observation(value) {
...
export function providerOutcomeContextDigest(context) {
 ...
 */
export function providerOutcomeObservationEffects(context) {
...
function result(checks, status, reason, binding, observation, sourceErrors = []) {
 ...
 */
export async function verifyProviderOutcomeBinding(binding, outcomeObservation, options) {
    ...
    try {
        ...
        checks.source_identity_exact = sameSourceIdentity(observation.source, options.expected_source);
        ...
        const sourceVerification = await verifyOutcomeObservationV2(observation, {
            ...(options.agility ?? {}),
        ...
        if (!checks.outcome_observation_v2) {
            ...
            return result(checks, stale ? 'INCOMPLETE' : 'CONFLICTED', stale ? 'outcome_source_not_current' : 'outcome_observation_not_verified', binding, outcomeObservation, sourceVerification.errors);
        }
...
//# sourceMappingURL=provider-outcome-binding.js.map