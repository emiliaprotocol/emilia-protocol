...
// Generated from allowance.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
import assert from 'node:assert/strict';
import crypto, { generateKeyPairSync, sign } from 'node:crypto';
import test from 'node:test';
import { GATE_ALLOWANCE_CLAIM_BOUNDARY, GATE_ALLOWANCE_VERSION, allowanceDigest, executeWithGateAllowance, issueGateAllowance, signGateAllowance, verifyGateAllowance, } from './allowance.js';
import { capabilityActionDigest, capabilityBaseReceiptDigest, createMemoryCapabilityStore, delegateCapabilityReceipt, reconcileCapabilityOperation, } from './capability-receipt.js';
...
function authorizationReceipt(keys) {
    ...
    return {
        ...
        signature: {
            algorithm: 'Ed25519',
            value: sign(null, Buffer.from(canonicalize(payload)), keys.pair.privateKey).toString('base64url'),
        },
        public_key: keys.publicKey,
    };
}
function allowanceInput(receipt, keys) {
    return {
...
function verificationOptions(keys) {
    return {
...
function payout(operationId, overrides = {}) {
    return {
        action_type: 'stripe.payout.create',
        amount: 2_500,
        currency: 'USD',
...
test('a Gate allowance is a signed, closed, context-pinned artifact', () => {
    ...
    assert.equal(verifyGateAllowance(substituted, verificationOptions(keys)).reason, 'allowance_signature_invalid');
    assert.equal(verifyGateAllowance(artifact, {
...
test('issue binds the authorization receipt and wires the allowance to the capability ledger', () => {
    ...
    assert.equal(issued.allowance.capability_id, issued.capabilityReceipt.capability.id);
    assert.equal(issued.allowance.capability_issuer_key_digest, `sha256:${crypto
        .createHash('sha256')
...
test('one allowance binds exactly one capability envelope and refuses delegation', async () => {
    ...
    assert.throws(() => issueGateAllowance({
        authorizationReceipt: receipt,
        allowance: input,
        signer: keys.signer,
        capabilityIssuerPrivateKey: keys.pair.privateKey,
...
test('allowance execution fences one material action across wrapper operation ids', async () => {
    ...
    assert.equal(first.action_fence_digest, capabilityActionDigest({
...
test('concurrent draws linearize and post-entry uncertainty consumes replay authority', async () => {
    ...
    assert.equal(concurrent.filter((entry) => !entry.ok)[0].reason, 'budget_exceeded');
...
test('a provider success followed by commit failure remains reconcilable without retry', async () => {
    ...
    assert.equal(durableBoundary.getOperation(action.operation_id, input.capability_id, `${input.tenant_id}/${input.allowance_id}`).outcome, 'indeterminate');
    assert.equal(durableBoundary.getOperation(action.operation_id, input.capability_id, `${input.tenant_id}/${input.allowance_id}`).reconciliation_outcome, 'executed');
});
