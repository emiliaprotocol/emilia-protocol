...
// Generated from consequence-actuator-v2.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
//
...
import { CONSEQUENCE_ACTUATOR_ENVELOPE_V2_VERSION, CONSEQUENCE_ACTUATOR_ENVELOPE_VERSION, signConsequenceExecutionEnvelope, signConsequenceExecutionEnvelopeV2, verifyConsequenceExecutionEnvelope, verifyConsequenceExecutionEnvelopeV2, } from './consequence-actuator.js';
...
async function sign(m, overrides = {}) {
    return signConsequenceExecutionEnvelopeV2(payloadV2(overrides), {
...
test('the v1 verifier refuses a v2 envelope cleanly on shape', async () => {
    ...
    const verified = verifyConsequenceExecutionEnvelope(envelope, {
        pins: { ...pinsV2(m), envelopeKeyId: 'actuator-key-v2' },
        expected: expected(),
        now: NOW,
    });
...
test('the v1 verifier still accepts a v1 envelope, unchanged', () => {
    const m = material();
    const envelope = signConsequenceExecutionEnvelope({ '@version': CONSEQUENCE_ACTUATOR_ENVELOPE_VERSION, ...payloadV2({ '@version': CONSEQUENCE_ACTUATOR_ENVELOPE_VERSION }) }, { privateKey: m.ed.privateKey, keyId: 'actuator-key-v1' });
    const verified = verifyConsequenceExecutionEnvelope(envelope, {
        pins: { ...pinsV2(m), envelopeKeyId: 'actuator-key-v1' },
        expected: expected(),
        now: NOW,
    });
    assert.equal(verified.ok, true, verified.reason);
...
test('the v2 verifier refuses a v1 envelope on the version marker', async () => {
    const m = material();
    const envelope = signConsequenceExecutionEnvelope({ '@version': CONSEQUENCE_ACTUATOR_ENVELOPE_VERSION, ...payloadV2({ '@version': CONSEQUENCE_ACTUATOR_ENVELOPE_VERSION }) }, { privateKey: m.ed.privateKey, keyId: 'actuator-key-v2' });
    const verified = await verifyConsequenceExecutionEnvelopeV2(envelope, { pins: pinsV2(m), expected: expected(), now: NOW });
    assert.equal(verified.ok, false);
    assert.equal(verified.reason, 'malformed_envelope');
});
test('LEG STRIPPING: removing the ML-DSA leg refuses structurally', async () => {
    const m = material();
    const signed = await sign(m);
...
test('WRONG-LENGTH SIGNATURE: a truncated Ed25519 signature refuses', async () => {
    ...
    const signed = await sign(m);
    const envelope = structuredClone(signed);
    envelope.proof.signatures = envelope.proof.signatures.map((s) => (s.alg === 'Ed25519' ? { ...s, sig: s.sig.slice(0, -4) } : s));
    const verified = await verifyConsequenceExecutionEnvelopeV2(envelope, { pins: pinsV2(m), expected: expected(), now: NOW });
    assert.equal(verified.ok, false);
});
...
test('NO ML-DSA BACKEND is a refusal, never a pass on the classical leg', async () => {
    ...
    assert.equal(verified.ok, false);
    assert.equal(verified.reason, 'pq_backend_unavailable');
});
