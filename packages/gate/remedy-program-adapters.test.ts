...
const CAID = (char: string, action = 'payments.refund') => (
...
type SigningIdentity = ReturnType<typeof keyPair> & {
  authorityId: string;
  keyId: string;
};

function signingIdentity(authorityId: string, keyId: string): SigningIdentity {
  return { ...keyPair(), authorityId, keyId };
}

function signEvidence(kind: string, payload: Record<string, unknown>, signer: SigningIdentity) {
  const unsigned = {
  ...
  const signature = crypto.sign(
    null,
    remedyProgramEvidenceSigningBytes(body),
    signer.privateKey,
  ).toString('base64url');
  return {
    ...body,
...
function signRevocation({
  ...
  statement.proof.signature_b64u = crypto.sign(
    null,
...
class EvidenceSource {
  ...
  put(tenantId: string, evidenceId: string, evidence: unknown) {
  ...
  async get({ tenantId, evidenceId, evidenceDigest }: Record<string, string>) {
...
function disputePayload(h: ReturnType<typeof harness>, overrides = {}) {
  const { values } = h;
  return {
...
async function createAndDispute(h: ReturnType<typeof harness>, artifact?: unknown) {
...
function authorizationPayload(h: ReturnType<typeof harness>, overrides = {}) {
  const { values } = h;
  return {
...
async function authorize(h: ReturnType<typeof harness>, artifact?: unknown) {
...
function providerPayload(h: ReturnType<typeof harness>, {
  evidenceId,
  outcome,
  observedAt,
  reconciliation,
...
test('real adapters preserve original effect across late revocation and reconcile a signed provider timeout', async () => {
  ...
  const timeoutRef = h.source.put(
    h.values.tenantId,
    timeoutPayload.evidence_id,
    signEvidence('provider_outcome', timeoutPayload, h.signers.provider),
  );
  ...
  const executedRef = h.source.put(
    h.values.tenantId,
    executedPayload.evidence_id,
    signEvidence('provider_outcome', executedPayload, h.signers.provider),
  );
...
test('tenant, authority key, and exact action substitutions fail closed', async (t) => {
  await t.test('wrong tenant', async () => {
    const h = harness();
    const artifact = signEvidence(
      'dispute',
      disputePayload(h, { tenant_id: 'tenant-attacker' }),
      h.signers.dispute,
    );
  ...
  await t.test('wrong remedy action', async () => {
    ...
    const artifact = signEvidence(
      ...
      authorizationPayload(h, { remedy_action_digest: HASH('9') }),
      h.signers.remedy,
    );
