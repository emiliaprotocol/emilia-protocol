...
test('runtime grants keep the application role non-destructive', () => {
  ...
  assert.throws(
    () => actionEscrowRuntimeGrantDdl('runtime"; DROP TABLE x;--'),
...
test('atomically creates, reads, and compare-and-swaps expected revisions', async () => {
  ...
  assert.deepEqual(
    (await store.readHistory('agreement-1')).map((entry) => ({
...
test('history verification refuses mutation, gaps, and digest substitution', async () => {
  ...
  await assert.rejects(
    store.readHistory('agreement-history'),
...
test('normal reads refuse truncated, reordered, or state-divergent journals', async (t) => {
  async function fixture() {
    ...
    await store.compareAndSwap(
      'agreement-journal',
      0,
      '{"revision":1,"state":"awaiting_acceptance"}',
    );
    return { pg, store };
  }
  ...
  await t.test('truncated', async () => {
    ...
    await assert.rejects(
      store.read('agreement-journal'),
      /state and append-only journal do not agree/,
    );
  });
...
test('database time fencing refuses rollback across separate store processes', async () => {
  ...
  assert.deepEqual(
    await restarted.compareAndSwap('agreement-restart-clock', 0, '{"revision":1}'),
    { applied: false, revision: null },
  );
  assert.deepEqual(
    await first.read('agreement-restart-clock'),
