...
import {
  signRelianceRefusal,
  relianceRefusalClass,
  MAPPED_RELIANCE_VERDICTS,
} from './reliance-refusal-bridge.js';
...
test('an allow verdict can never be signed as a refusal', () => {
  assert.throws(
    () => signRelianceRefusal(context({ decision: { verdict: 'rely', allow: true } }), SIGNER),
    /cannot be signed for an allow verdict/,
  );
  // allow:true must be refused even when the verdict string looks like a denial
  assert.throws(
    () => signRelianceRefusal(context({ decision: { verdict: 'do_not_rely_unsigned', allow: true } }), SIGNER),
    /cannot be signed for an allow verdict/,
  );
});
...
test('every mapped verdict yields semantics the statement validator accepts', () => {
  for (const verdict of MAPPED_RELIANCE_VERDICTS) {
    const statement = signRelianceRefusal(
      context({ decision: { verdict, allow: false } }),
      SIGNER,
    );
    const result = verifyActionRefusalStatement(statement, { trusted_keys: TRUSTED, now: '2026-07-29T00:00:01Z' });
...
test('an unmapped verdict fails closed to indeterminate rather than borrowing semantics', () => {
  ...
  const statement = signRelianceRefusal(
    context({ decision: { verdict: 'do_not_rely_some_future_verdict', allow: false } }),
    SIGNER,
  );
...
test('explicit failed requirement ids are preserved', () => {
  const statement = signRelianceRefusal(
    context({ failed_requirement_ids: ['admissibility-01', 'admissibility-02'] }),
    SIGNER,
  );
...
test('the bridge refuses to invent a failed requirement identifier', () => {
  assert.throws(
    () => signRelianceRefusal(context({ failed_requirement_ids: undefined }), SIGNER),
    /requires explicit failed requirement ids/,
  );
});
