...
// Generated from receipt-program-v2.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
//
...
import { RECEIPT_PROGRAM_VERSION, RECEIPT_PROGRAM_CERTIFICATE_VERSION, RECEIPT_PROGRAM_V2_VERSION, RECEIPT_PROGRAM_CERTIFICATE_V2_VERSION, RECEIPT_PROGRAM_V2_REQUIRED_ALGORITHMS, issueReceiptProgramCertificateV2, verifyReceiptProgramCertificate, verifyReceiptProgramCertificateV2, verifyReceiptProgramCertificateStatement, } from './receipt-program.js';
...
async function issued() {
    ...
    return { issuer, certificate: certificate };
}
/** Mint a v1 certificate by hand: the v1 signer lives inside the kernel. */
function mintV1Certificate(privateKey, publicKeyB64u) {
...
describe('EP-RECEIPT-PROGRAM-CERTIFICATE-v2 hybrid execution certificate', () => {
    it('valid v2 roundtrip: both legs verify under the pinned key pair', async () => {
        ...
        assert.deepEqual(certificate.signature.required_algorithms, [...RECEIPT_PROGRAM_V2_REQUIRED_ALGORITHMS]);
        assert.deepEqual(certificate.signature.signatures.map((s) => s.alg), [...RECEIPT_PROGRAM_V2_REQUIRED_ALGORITHMS]);
    ...
    it('stripped leg: dropping the ML-DSA signature refuses, never a classical-only pass', async () => {
        ...
        const stripped = {
            ...
            signature: {
                ...
                signatures: certificate.signature.signatures.filter((s) => s.alg !== 'ML-DSA-65'),
            },
        };
    ...
    it('narrowed set: claiming required_algorithms=["Ed25519"] is refused structurally AND cryptographically', async () => {
        ...
        const narrowed = {
            ...
            signature: {
                ...
                signatures: certificate.signature.signatures.filter((s) => s.alg === 'Ed25519'),
            },
        };
        assert.equal((await verifyReceiptProgramCertificateV2(narrowed, options)).reason, 'certificate_algorithm_set_unsupported');
        const setIntact = {
            ...narrowed,
            signature: {
                ...narrowed.signature,
        ...
        assert.equal((await verifyReceiptProgramCertificateV2(setIntact, options)).reason, 'certificate_signature_leg_missing');
    });
    it('wrong-length signature on the ML-DSA leg refuses, never crashes', async () => {
        ...
        const tampered = {
            ...
            signature: {
                ...certificate.signature,
                signatures: certificate.signature.signatures.map((s) => (s.alg === 'ML-DSA-65' ? { ...s, sig: crypto.randomBytes(16).toString('base64url') } : s)),
            },
        };
    ...
    it('a v1-marked program inside a v2 certificate is refused by the shared body, at issuance', async () => {
        ...
        await assert.rejects(issueReceiptProgramCertificateV2(coreInput(RECEIPT_PROGRAM_VERSION), { keys: issuer.keys }), /self-verification failed: certificate_program_invalid/);
    });
...
describe('EP-RECEIPT-PROGRAM-CERTIFICATE-v2 FIPS consult and signer configuration', () => {
    it('a denied FIPS policy refuses issuance BEFORE the signer runs, named reason, never a silent sign', async () => {
        ...
        await assert.rejects(issueReceiptProgramCertificateV2(coreInput(), { signer, fipsPosture: DENYING_POSTURE }), /fips_policy_denied:Ed25519:ed25519_boundary_undeclared/);
    ...
    it('an ACTIVE posture refuses the ML-DSA leg until the unvalidated implementation is acknowledged', async () => {
        ...
        await assert.rejects(issueReceiptProgramCertificateV2(coreInput(), { keys: issuer.keys, fipsPosture: active }), /fips_policy_denied:ML-DSA-65:mldsa_implementation_unvalidated/);
    ...
    it('exactly one signer must be configured, and an incomplete set refuses issuance', async () => {
        const issuer = issuerFixture();
        await assert.rejects(issueReceiptProgramCertificateV2(coreInput(), {}), /exactly one receipt program certificate v2 signer/);
        const signer = {
            keyId: KEY_ID,
            custody: 'hsm',
            publicKeys: issuer.hybridPin[KEY_ID],
        ...
        await assert.rejects(issueReceiptProgramCertificateV2(coreInput(), { signer }), /malformed signature set/);
    });
    it('an injected signSet signer that supplies both legs issues a verifiable certificate', async () => {
        ...
        const certificate = await issueReceiptProgramCertificateV2(coreInput(), { signer });
        const verified = await verifyReceiptProgramCertificateV2(certificate, {
            trustedCertificateKeys: issuer.hybridPin,
            expectedContext: CONTEXT,
        });
