...
import { CLAIM_ASSURANCE_PROFILE_VERSION, CLAIM_CASE_VERSION, claimAssuranceArtifactDigest, claimAssuranceProfileHash, evaluateClaimAssurance, } from '@emilia-protocol/verify/claim-assurance';
import { CLAIM_ASSURANCE_GATE_PRESENTATION_VERSION, CLAIM_ASSURANCE_ADMISSIBILITY_VERSION, buildReliancePacket, createClaimAssuranceAdmissibilityVerifier, createTrustedActionFirewall, createEg1Harness, EG1_DEFAULT_SELECTOR, hashCanonical, validateClaimAssuranceAdmissibilityResult, } from './index.js';
...
function claimCase(pinnedProfile, actionDigest, artifacts = [artifact()], asOf = AS_OF) {
    return {
...
function verifierFor(pinnedProfile, overrides = {}) {
...
test('re-performs a Claim Case and returns an exact-action, non-authorizing admissibility block', async () => {
    ...
    const pinnedProfile = profile();
    const actionDigest = `sha256:${hashCanonical(harness.action)}`;
