...
function payloadFields(overrides: Record<string, any> = {}) {
  return {
    provider_id: 'escrow.com',
    environment: 'sandbox' as const,
    statement_type: 'release' as const,
...
async function buildV2(overrides: Record<string, any> = {}) {
...
function expected(overrides: Record<string, any> = {}) {
...
function verifyOpts(overrides: Record<string, any> = {}) {
  return {
...
test('a real hybrid custodian observation verifies under both pinned keys (valid roundtrip)', async () => {
  ...
  assert.equal(res.valid, true, (res as any).reason);
  assert.equal((res as any).status, 'released');
});
...
test('LEG STRIPPING: removing the ML-DSA leg refuses structurally', async () => {
  ...
  const res = await verifyActionEscrowCustodianStatementV2(stmt, verifyOpts());
  assert.equal(res.valid, false);
  assert.equal((res as any).reason, 'custodian_signature_leg_stripped');
});
...
test('SET NARROWING: narrowing required_algorithms to Ed25519-only refuses structurally', async () => {
  ...
  const res = await verifyActionEscrowCustodianStatementV2(stmt, verifyOpts());
  assert.equal(res.valid, false);
  assert.equal((res as any).reason, 'custodian_algorithm_set_invalid');
});
...
test('DUPLICATE ALGORITHM: two entries for one algorithm refuse', async () => {
  ...
  const res = await verifyActionEscrowCustodianStatementV2(stmt, verifyOpts());
  assert.equal(res.valid, false);
  assert.equal((res as any).reason, 'custodian_signature_leg_duplicate');
});
...
test('an unpinned operator key confers nothing', async () => {
  ...
  assert.equal(res.valid, false);
  assert.equal((res as any).reason, 'custodian_operator_key_not_pinned');
});
...
test('expected-binding mismatch refuses on custodian_observation_binding_mismatch', async () => {
  ...
  assert.equal(res.valid, false);
  assert.equal((res as any).reason, 'custodian_observation_binding_mismatch');
});
