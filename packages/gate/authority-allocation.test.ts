...
import {
  AUTHORITY_ALLOCATION_DDL,
  AUTHORITY_ALLOCATION_SQL,
  AUTHORITY_ALLOCATION_VERSION,
  AuthorityAllocationValidationError,
  createMemoryAuthorityAllocationStore,
  createPostgresAuthorityAllocationStore,
  isDurableAuthorityAllocationStore,
...
const PIN: AuthorityAllocationPin = {
...
function snapshot(
  overrides: Partial<AuthorityAllocationSnapshot> = {},
...
function reservation(
  ...
  allocationId = 'branch:a',
  budget = { cents: 40, calls: 1 },
) {
  return {
    relying_party_id: PIN.relying_party_id,
    parent_id: 'authority:treasury',
...
test('rejects stale or missing relying-party authority pins', () => {
  assert.equal(validationCode(() => validateAuthorityAllocationSnapshot(
    snapshot(),
    { ...PIN, authority_epoch: 18 },
  )), 'authority_pin_mismatch');
  assert.equal(validationCode(() => validateAuthorityAllocationSnapshot(
    snapshot(),
    { ...PIN, authority_head: undefined as unknown as string },
  )), 'missing_authority_pin');
});
