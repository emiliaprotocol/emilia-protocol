...
// Generated from custody.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
import { test } from 'node:test';
import assert from 'node:assert/strict';
import crypto from 'node:crypto';
import { createGate, createEg1Harness, createKeyRegistry, classifyRetention, buildRetentionExport, createDurableConsumptionStore, createMemoryBackend, } from './index.js';
...
test('key registry: supplied invalid or non-RFC3339 dates throw instead of becoming unwindowed', () => {
    ...
    for (const not_after of ['not-a-date', '01/02/2030', 1893456000000, null, undefined]) {
        assert.throws(() => createKeyRegistry([{ kid: 'eternal-if-normalized', key: h.publicKey, not_after }]), /not_after.*RFC3339/);
    }
    const registry = createKeyRegistry();
    assert.throws(() => registry.add({ kid: 'bad-rotation', key: h.publicKey, not_before: 'tomorrow' }), /not_before.*RFC3339/);
});
