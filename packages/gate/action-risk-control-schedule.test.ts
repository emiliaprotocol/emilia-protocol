...
} from './action-risk-control-schedule.js';
...
function keyMaterial(seed: number, issuerId: string, keyId: string) {
...
const scheduleArtifact: any = await signActionRiskControlSchedule(scheduleSource(), scheduleKeys.signer);

async function qualificationStatus(
  schedule: any = scheduleArtifact,
...
function qualificationStatusHead(status: any, overrides: Record<string, unknown> = {}) {
  return {
...
async function evaluationOptions(overrides: Record<string, unknown> = {}) {
...
test('real Ed25519 and ML-DSA-65 schedule plus independent current status evaluate ELIGIBLE', async () => {
  const result: any = await evaluateActionRiskControlSchedule(scheduleArtifact, await evaluationOptions());
...
test('published JSON Schema accepts the signed artifact and refuses unknown members', () => {
  const schema = JSON.parse(readFileSync(
    new URL('../../public/schemas/ep-action-risk-control-schedule.schema.json', import.meta.url),
    'utf8',
  ));
...
test('published qualification-status JSON Schema matches the signed artifact shape', async () => {
  const schema = JSON.parse(readFileSync(
    new URL('../../public/schemas/ep-action-risk-qualification-status.schema.json', import.meta.url),
    'utf8',
  ));
  const ajv = new Ajv2020({ allErrors: true, strict: false, validateFormats: false });
  const validate = ajv.compile(schema);
...
test('tampering any signed schedule field fails before control evaluation', async () => {
  ...
  const result: any = await evaluateActionRiskControlSchedule(tampered, await evaluationOptions());
...
test('removing either hybrid signature never leaves a valid schedule', async () => {
  for (const kept of ['Ed25519', 'ML-DSA-65']) {
    ...
    stripped.proof.signatures = stripped.proof.signatures.filter((signature: any) => signature.alg === kept);
    const result: any = await evaluateActionRiskControlSchedule(stripped, await evaluationOptions());
    assert.equal(result.outcome, 'NOT_ELIGIBLE');
...
test('every schedule signature key id is required and must equal proof and issuer key ids', async () => {
  ...
  const missingResult: any = await evaluateActionRiskControlSchedule(missing, await evaluationOptions());
  ...
  const mismatchResult: any = await evaluateActionRiskControlSchedule(mismatch, await evaluationOptions());
  ...
  const reorderedResult: any = await evaluateActionRiskControlSchedule(reordered, await evaluationOptions());
...
test('wrong or absent externally supplied schedule pins fail closed', async () => {
  ...
  const wrongResult: any = await evaluateActionRiskControlSchedule(
    scheduleArtifact,
    await evaluationOptions({ trusted_schedule_keys: wrong.pins }),
  );
  assert.equal(wrongResult.outcome, 'NOT_ELIGIBLE');
  assert.equal(wrongResult.reason, 'signature_invalid');

  const absentResult: any = await evaluateActionRiskControlSchedule(
    scheduleArtifact,
    await evaluationOptions({ trusted_schedule_keys: undefined }),
  );
...
test('a signed trust reference cannot substitute for the caller trust root', async () => {
  ...
  source.trust_pin_references = source.trust_pin_references.map((reference) => (
    reference.purpose === 'SCHEDULE_ISSUER' ? { ...reference, key_digest: D('e') } : reference
  ));
  const artifact = await signActionRiskControlSchedule(source, scheduleKeys.signer);
  const result: any = await evaluateActionRiskControlSchedule(artifact, {
...
test('all schedule context identities are required and exact', async () => {
  ...
  for (const [label, override, reason] of cases) {
    const result: any = await evaluateActionRiskControlSchedule(scheduleArtifact, await evaluationOptions(override));
...
test('not-yet-active and expired schedules are NOT_ELIGIBLE', async () => {
  const before: any = await evaluateActionRiskControlSchedule(
    scheduleArtifact,
    await evaluationOptions({ now: '2026-09-04T11:59:59Z' }),
  );
  assert.equal(before.outcome, 'NOT_ELIGIBLE');
  assert.equal(before.reason, 'schedule_not_yet_issued');

  const expired: any = await evaluateActionRiskControlSchedule(
    scheduleArtifact,
    await evaluationOptions({ now: '2026-10-04T12:00:00Z' }),
  );
...
test('every control group is substitution-resistant', async () => {
  const cases: Array<[string, (observed: any) => void, string]> = [
  ...
  for (const [label, mutate, reason] of cases) {
    const observed: any = observedControls();
    mutate(observed);
    const result: any = await evaluateActionRiskControlSchedule(
      scheduleArtifact,
      await evaluationOptions({ observed_controls: observed }),
    );
    assert.equal(result.outcome, 'NOT_ELIGIBLE', label);
    assert.equal(result.reason, reason, label);
  }
...
test('unknown observed fields and invalid control-domain independence fail closed', async () => {
  const unknown: any = observedControls();
  unknown.provider_binding.policy = 'attacker-controlled';
  const unknownResult: any = await evaluateActionRiskControlSchedule(
    scheduleArtifact,
    await evaluationOptions({ observed_controls: unknown }),
  );
  assert.equal(unknownResult.outcome, 'NOT_ELIGIBLE');
  assert.equal(unknownResult.reason, 'control_observation_invalid');

  const collapsed: any = observedControls();
  collapsed.outcome_binding.require_control_domain_independence = false;
  const collapsedResult: any = await evaluateActionRiskControlSchedule(
    scheduleArtifact,
    await evaluationOptions({ observed_controls: collapsed }),
  );
...
test('missing qualification evidence is INDETERMINATE and supplies the required handling', async () => {
  const result: any = await evaluateActionRiskControlSchedule(
    scheduleArtifact,
    await evaluationOptions({ qualification_status: undefined }),
  );
...
test('a verified status without relying-party last-seen head state is INDETERMINATE', async () => {
  const result: any = await evaluateActionRiskControlSchedule(
    scheduleArtifact,
    await evaluationOptions({ qualification_status_head: undefined }),
  );
  assert.equal(result.outcome, 'INDETERMINATE');
...
test('qualification status trust roots are external, pinned, and digest-bound', async () => {
  const absent: any = await evaluateActionRiskControlSchedule(
    scheduleArtifact,
    await evaluationOptions({ trusted_status_keys: undefined }),
  );
  ...
  const wrongResult: any = await evaluateActionRiskControlSchedule(
    scheduleArtifact,
    await evaluationOptions({ trusted_status_keys: wrong.pins }),
  );
  assert.equal(wrongResult.outcome, 'NOT_ELIGIBLE');
...
test('a tampered or wrong-authority qualification status is NOT_ELIGIBLE', async () => {
  const status: any = await qualificationStatus();
  const tampered = mutable(status);
  tampered.sequence = 8;
  const tamperedResult: any = await evaluateActionRiskControlSchedule(
    scheduleArtifact,
    await evaluationOptions({ qualification_status: tampered }),
  );
  ...
  const alternateResult: any = await evaluateActionRiskControlSchedule(
    scheduleArtifact,
    await evaluationOptions({
...
test('qualification status signature legs also require the proof and issuer key id', async () => {
  ...
  const missingResult: any = await evaluateActionRiskControlSchedule(
    scheduleArtifact,
    await evaluationOptions({ qualification_status: missing }),
  );
  assert.equal(missingResult.outcome, 'NOT_ELIGIBLE');
  assert.equal(missingResult.reason, 'signature_key_id_required');
  ...
  const mismatchResult: any = await evaluateActionRiskControlSchedule(
    scheduleArtifact,
    await evaluationOptions({ qualification_status: mismatch }),
  );
  assert.equal(mismatchResult.outcome, 'NOT_ELIGIBLE');
  assert.equal(mismatchResult.reason, 'signature_key_id_mismatch');
});
...
test('qualification status schedule, tenant, and requirements substitutions are refused', async () => {
  ...
  for (const [label, overrides, reason] of cases) {
    ...
    const result: any = await evaluateActionRiskControlSchedule(
      scheduleArtifact,
      await evaluationOptions({ qualification_status: status }),
    );
    assert.equal(result.outcome, 'NOT_ELIGIBLE', label);
    assert.equal(result.reason, reason, label);
  }
...
test('old sequence and stale observation are INDETERMINATE, never eligible', async () => {
  ...
  const sequenceResult: any = await evaluateActionRiskControlSchedule(
    scheduleArtifact,
    await evaluationOptions({ qualification_status: oldSequence }),
  );
  ...
  const staleResult: any = await evaluateActionRiskControlSchedule(
    scheduleArtifact,
    await evaluationOptions({
...
test('future qualification observations are NOT_ELIGIBLE', async () => {
  ...
  const result: any = await evaluateActionRiskControlSchedule(
    scheduleArtifact,
    await evaluationOptions({ qualification_status: future }),
  );
  assert.equal(result.outcome, 'NOT_ELIGIBLE');
...
test('the qualification authority can report NOT_ELIGIBLE or INDETERMINATE without authorizing', async () => {
  for (const outcome of ['NOT_ELIGIBLE', 'INDETERMINATE'] as const) {
...
test('qualification head schedule, tenant, authority, time, and shape are exact', async () => {
  ...
  const unknown: any = qualificationStatusHead(status);
  unknown.untrusted = true;
  const unknownResult: any = await evaluateActionRiskControlSchedule(
    scheduleArtifact,
    await evaluationOptions({ qualification_status: status, qualification_status_head: unknown }),
  );
  assert.equal(unknownResult.outcome, 'NOT_ELIGIBLE');
...
test('signing refuses unknown fields, unsafe ceilings, unsorted sources, and self-inconsistent trust references', async () => {
  const unknown: any = scheduleSource();
  ...
  wrongReference.trust_pin_references = wrongReference.trust_pin_references.map((reference) => (
    reference.purpose === 'QUALIFICATION_STATUS'
      ? { ...reference, authority_id: 'authority:qualification:other' }
      : reference
  ));
  await assert.rejects(
    signActionRiskControlSchedule(wrongReference, scheduleKeys.signer),
    /qualification status reference must match/,
  );
  ...
  await assert.rejects(
    signActionRiskControlSchedule(decorativeReference, scheduleKeys.signer),
    /closed, bounded v1 object/,
  );
});
...
test('qualification status is closed and uses only the three evaluation outcomes', async () => {
  ...
  await assert.rejects(
    signActionRiskQualificationStatus(unknown, statusKeys.signer),
    /closed, bounded v1 object/,
  );
  const invalidOutcome = { ...unknown, policy_number: undefined, outcome: 'COVERED' };
  delete invalidOutcome.policy_number;
  await assert.rejects(
    signActionRiskQualificationStatus(invalidOutcome, statusKeys.signer),
    /closed, bounded v1 object/,
  );
});
...
test('absence of the ML-DSA verifier is INDETERMINATE and never a classical-only pass', async () => {
  const result: any = await evaluateActionRiskControlSchedule(
    scheduleArtifact,
    { ...(await evaluationOptions()), mldsaBackendLoader: async () => null } as any,
  );
  assert.equal(result.outcome, 'INDETERMINATE');
