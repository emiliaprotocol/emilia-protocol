...
// Generated from action-evidence-packet.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
...
import { ACTION_EVIDENCE_COMPONENT_ROLES, ACTION_EVIDENCE_MANIFEST_VERSION, ACTION_EVIDENCE_PACKET_CLAIM_BOUNDARY, actionEvidenceArtifactDigest, buildActionEvidencePacket, verifyActionEvidencePacket, } from './action-evidence-packet.js';
...
function providerContext(outcome = 'COMMITTED') {
    return {
...
async function makeFixture(input = {}) {
    ...
    const observations = await Promise.all(SOURCES.map((source, index) => (buildOutcomeObservationV2({
    ...
    const artifacts = Object.fromEntries(ACTION_EVIDENCE_COMPONENT_ROLES
    ...
    const components = Object.fromEntries(ACTION_EVIDENCE_COMPONENT_ROLES.map((role) => [
        role,
...
function componentVerifiers(overrides = {}) {
...
function options(context, input = {}) {
    return {
        expected_context: context,
        now: NOW,
        verify_schedule: input.schedule ?? (async (request) => ({
            verification: 'VERIFIED',
            currentness: 'CURRENT',
            artifact_digest: actionEvidenceArtifactDigest(request.artifact),
            subject_digest: request.subject_digest,
...
test('returns TECHNICALLY_COMPLETE only after every trusted adapter reports an exact current binding', async () => {
    ...
    assert.notEqual(fixture.observations[0].observed_until, fixture.observations[1].observed_until, 'independent sources may use different signed observation intervals');
});
test('the published schema accepts a built packet and preserves the source-cardinality bound', async () => {
    const fixture = await makeFixture();
    const schema = JSON.parse(readFileSync(new URL('../../public/schemas/ep-action-evidence-packet.schema.json', import.meta.url), 'utf8'));
...
test('missing artifacts and missing native verifiers are INCOMPLETE', async (t) => {
    ...
    await t.test('missing attachment', async () => {
        ...
        delete packet.attachments[fixture.manifest.components.aeb.artifact_digest];
    ...
    await t.test('missing verifier', async () => {
        ...
        delete verifiers.qualification_status_head;
        const verified = await verifyActionEvidencePacket(fixture.packet, options(fixture.context, { components: verifiers }));
        assert.equal(verified.result, 'INCOMPLETE');
...
test('manifest, attachment, role, and expected-context substitutions are CONFLICTED', async (t) => {
    ...
    await t.test('subject changed outside the relying party', async () => {
        ...
        const verified = await verifyActionEvidencePacket(fixture.packet, options(changed));
        assert.equal(verified.result, 'CONFLICTED');
    ...
    await t.test('unreferenced attachment smuggling', async () => {
        ...
        packet.attachments[actionEvidenceArtifactDigest(extra)] = extra;
        const verified = await verifyActionEvidencePacket(packet, options(fixture.context));
        assert.equal(verified.result, 'CONFLICTED');
...
test('uncertain currentness and a coherent indeterminate outcome remain INDETERMINATE', async (t) => {
    ...
    await t.test('provider, OEL, and observed effect remain indeterminate together', async () => {
        ...
        const verified = await verifyActionEvidencePacket(pending.packet, options(pending.context));
        assert.equal(verified.result, 'INDETERMINATE');
...
test('the verified schedule controls provider observation windows and freshness', async (t) => {
    const scheduleVerifier = (requirements) => async (request) => ({
        verification: 'VERIFIED',
        currentness: 'CURRENT',
        artifact_digest: actionEvidenceArtifactDigest(request.artifact),
        subject_digest: request.subject_digest,
    ...
    await t.test('a caller may tighten the signed maximum age', async () => {
        const fixture = await makeFixture();
        const verified = await verifyActionEvidencePacket(fixture.packet, options(fixture.context, { maximum_observation_age_ms: 30_000 }));
        assert.equal(verified.result, 'INCOMPLETE');
        assert.ok(verified.reasons.includes('provider_outcome_outcome_observation_stale'));
    });
    await t.test('the signed opening boundary is enforced around provider entry', async () => {
        const fixture = await makeFixture();
        const verified = await verifyActionEvidencePacket(fixture.packet, options(fixture.context, { provider_entry_at: fixture.context.observed_at }));
        assert.equal(verified.result, 'CONFLICTED');
    ...
    await t.test('a source interval must contain the shared signed provider event', async () => {
        ...
        const verified = await verifyActionEvidencePacket(fixture.packet, options(fixture.context, { maximum_observation_age_ms: 180_000 }));
        assert.equal(verified.result, 'CONFLICTED');
        assert.ok(verified.reasons.includes('provider_observation_outside_schedule_window'));
    });
    ...
    await t.test('a schedule result without its signed window is invalid', async () => {
        const fixture = await makeFixture();
        const malformed = structuredClone(OUTCOME_REQUIREMENTS);
        delete malformed.observation_window;
        const verified = await verifyActionEvidencePacket(fixture.packet, options(fixture.context, { schedule: scheduleVerifier(malformed) }));
        assert.equal(verified.result, 'INCOMPLETE');
...
test('packet chronology is internally consistent without treating assembled_at as authenticated freshness', async (t) => {
    ...
    await t.test('provider event cannot occur after assembly', () => {
        ...
        assert.throws(() => buildActionEvidencePacket({
            manifest: manifest,
...
test('TECHNICALLY_COMPLETE requires the schedule quorum and independent control domains', async (t) => {
    ...
    await t.test('one missing pinned source cannot satisfy two-of-two', async () => {
        ...
        const verified = await verifyActionEvidencePacket(fixture.packet, options(fixture.context, { source_keys: oneSource }));
        assert.equal(verified.result, 'INCOMPLETE');
    ...
    await t.test('two verified sources in one control domain are not independent', async () => {
        ...
        const verified = await verifyActionEvidencePacket(fixture.packet, options(fixture.context, { source_keys: sameDomain }));
        assert.equal(verified.result, 'INCOMPLETE');
    ...
    await t.test('a schedule quorum larger than the verified source set is not met', async () => {
        const threeOfThree = {
            required_sources: [
                { role: 'executor', source_class: 'executor.adapter' },
