...
import { AEB_NATIVE_AUTHORIZATION_GATEWAY_KEY_VERSION, AEB_NATIVE_AUTHORIZATION_PINS_VERSION, AEB_NATIVE_AUTHORIZATION_SOURCE_PIN_VERSION, AEB_NATIVE_AUTHORIZATION_STATUS_VERSION, digestAebNativeAuthorizationAction, issueAebNativeAuthorizationHandoff, verifyAebNativeAuthorizationHandoff, } from '@emilia-protocol/verify/aeb';
import { loadDefaultAgilityMldsaBackend } from '@emilia-protocol/verify/pq-signature-agility';
import { consequenceBoundaryActionFenceHolderKey, consequenceBoundaryProviderIdempotencyKey, createConsequenceBoundary, createNativeConsequenceBoundary, nativeConsequenceBoundaryActionFenceKey, nativeConsequenceBoundaryAttemptReservationKeys, nativeConsequenceBoundaryProviderIdempotencyKey, nativeConsequenceBoundaryReservationKey, } from './consequence-boundary.js';
...
function nativeRecovery(f, operationId, attempt, outcome, recoveryAuthorization = 'recovery:approved') {
    return {
        operation_id: operationId,
        handoff: f.handoff,
        action: ACTION,
...
test('B7: a lost RESERVED to INVOKING write is recoverable; an applied one needs a provider outcome', async () => {
    ...
    // be reconciled and locked the action forever.
    {
        ...
        assert.equal(first.invoked, false);
        const recovered = await h.boundary.reconcile(nativeRecovery(f, 'operation:native:b7', first.attempt, { state: 'FAILED', evidence: failedEvidence(7), reason: 'not_found' }));
        assert.equal(recovered.reason, 'attempt_never_entered_provider');
        const fresh = await h.boundary.run({
    ...
    }
    {
        ...
        assert.equal(first.reason, 'attempt_start_unconfirmed');
        assert.equal([...attempts.rows.values()][0].state, 'INVOKING');
        const unproven = await h.boundary.reconcile(nativeRecovery(f, 'operation:native:b7:applied', first.attempt, { state: 'INDETERMINATE', reason: 'provider_lookup_unavailable' }));
        ...
        const closed = await h.boundary.reconcile(nativeRecovery(f, 'operation:native:b7:applied', first.attempt, { state: 'FAILED', evidence: failedEvidence(8), reason: 'not_found' }));
...
test('composed: a pre-entry lockout is recovered by reconcile and burns only that evaluation', async () => {
    ...
    attempts.transition = pinnedTransition;
