...
import { AEB_NATIVE_AUTHORIZATION_GATEWAY_KEY_VERSION, AEB_NATIVE_AUTHORIZATION_PINS_VERSION, AEB_NATIVE_AUTHORIZATION_SOURCE_PIN_VERSION, AEB_NATIVE_AUTHORIZATION_STATUS_VERSION, issueAebNativeAuthorizationHandoff, verifyAebNativeAuthorizationHandoff, } from '@emilia-protocol/verify/aeb';
import { loadDefaultAgilityMldsaBackend } from '@emilia-protocol/verify/pq-signature-agility';
import { consequenceBoundaryProviderIdempotencyKey, createConsequenceBoundary, createNativeConsequenceBoundary, nativeConsequenceBoundaryActionFenceKey, nativeConsequenceBoundaryProviderIdempotencyKey, } from './consequence-boundary.js';
...
function makeBoundary({ f = fixture(), aebStore = durableAebStore(), attempts = attemptStore(), consequenceEnvelope = undefined, localAuthorize = () => true, invoke = async () => ({ state: 'EXECUTED', evidence: executedEvidence(), result: { id: 'effect-1' } }), } = {}) {
...
function nativeFixture(system = 'authzen') {
