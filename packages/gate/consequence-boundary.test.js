...
function makeBoundary({ f = fixture(), aebStore = durableAebStore(), attempts = attemptStore(), consequenceEnvelope = undefined, localAuthorize = () => true, invoke = async () => ({ state: 'EXECUTED', evidence: executedEvidence(), result: { id: 'effect-1' } }), attemptPrefix = '', } = {}) {
...
function makeNativeBoundary({ f = nativeFixture(), aebStore = durableAebStore(), attempts = attemptStore(), localAuthorize = () => true, resolveStatus = () => f.status, resolveHistoricalPins = () => f.pins, providerOutcomeVerify = () => true, trustSnapshotId = 'native-trust-snapshot:test:1', localAuthorizationProgramDigest = digestAeb({
...
test('direct native security callbacks are pinned when the boundary is constructed', async () => {
    ...
    }
    {
        ...
        h.configuration.provider_outcomes.verify = () => true;
...
test('B7: a lost RESERVED to INVOKING write is recoverable; an applied one needs a provider outcome', async () => {
    ...
    }
    {
        ...
        assert.equal([...attempts.rows.values()][0].state, 'INVOKING');
        const unproven = await h.boundary.reconcile(nativeRecovery(f, 'operation:native:b7:applied', first.attempt, { state: 'INDETERMINATE', reason: 'provider_lookup_unavailable' }, 'recovery:approved', 'terminal'));
        ...
        const lost = await h.boundary.reconcile(nativeRecovery(f, 'operation:native:b7:applied', first.attempt, { state: 'FAILED', evidence: failedEvidence(8), reason: 'not_found' }));
        ...
        const closed = await h.boundary.reconcile(nativeRecovery(f, 'operation:native:b7:applied', first.attempt, { state: 'FAILED', evidence: failedEvidence(8), reason: 'not_found' }, 'recovery:approved', 'terminal'));
