...
// Generated from store-postgres.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
/**
...
import { createPostgresBackend, CONSUMPTION_TABLE, CONSUMPTION_TABLE_DDL, CONSUMPTION_SQL, PG_CONSUMPTION_VERSION, } from './store-postgres.js';
    ...
    async function query(text, params) {
        ...
        switch (text) {
            ...
            case CONSUMPTION_SQL.compareAndSet: {
                ...
                if (table.get(key)?.state !== expected)
            ...
            case CONSUMPTION_SQL.cleanupExpired: {
                ...
                for (const [k, v] of table) {
                    if (v.state.startsWith('committed:') && v.expires_at != null && v.expires_at <= nowArg) {
                        table.delete(k);
                        n += 1;
                    }
                }
...
test('stale release cannot delete a reservation whose ownership changed', async () => {
    ...
    assert.equal(await backend.compareAndSet('rcpt_fenced', 'reserved:v2:stale-owner-token01', 'reserved:v2:newer-owner-token01'), true);
...
test('clock regression fails closed before any expiry-bearing write or cleanup', async () => {
    ...
    clock.t = 49_999;
    await assert.rejects(backend.addIfAbsent('clock-b', 'committed:v2', { ttlSeconds: 60 }), /clock regression refused/);
