...
// Generated from action-fence-runtime-regression.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
...
function action(operationId, overrides = {}) {
    return {
...
function mintFixture(capabilityId, scopedAction = action(`${capabilityId}-scope`)) {
    const keys = issuer();
...
async function registerFixture(store, capabilityId) {
...
async function createPostgresHarness(t, label) {
...
for (const amount of [0, -1]) {
    ...
    test(`PostgreSQL reserveSpend rejects a ${label} amount at the API boundary`, {
        ...
        await assert.rejects(() => harness.store.reserveSpend({
            capabilityId: fixture.capabilityId,
            capabilityFingerprint: fixture.capabilityFingerprint,
            operationId,
            actionDigest: capabilityActionDigest(action(operationId)),
        ...
        assert.equal(harness.transactionCount(), transactionsBeforeReserve, 'invalid amounts must be rejected before opening a PostgreSQL transaction');
    });
}
...
async function assertWrongDigestRefused(store, storeKind, path) {
...
for (const path of ['pre-entry', 'indeterminate', 'not-entered']) {
