...
function claimsProvider(overrides: Record<string, unknown>) {
  const native = provider();
  return {
    providerId: native.providerId,
    profileId: native.profileId,
    verifyProjection(value: unknown, context: any) {
      const result = native.verifyProjection(value, context);
      if (result.state !== 'VERIFIED' || !result.claims) return result;
      return {
        ...result,
...
test('fails closed when the action omits or changes the signed context binding', () => {
  ...
  delete (omitted as any).trusted_context;
...
test('returns INDETERMINATE for stale signer status and stale source keyring state', () => {
  ...
  assert.deepEqual(
    staleSigner({ evidence: candidate.evidence, action: candidate.action }),
    {
      state: 'INDETERMINATE',
  ...
  }) as any, candidate.policy);
  assert.deepEqual(
    staleKeyring({ evidence: candidate.evidence, action: candidate.action }),
    { state: 'INDETERMINATE', reason: 'keyring_status_stale', authorizes: false },
  );
});
...
test('refuses revoked signers, untrusted delivered content, and forbidden exclusions', () => {
  ...
  assert.equal(
    strictPolicy({ evidence: strictCandidate.evidence, action: strictCandidate.action }).reason,
    'projection_exclusion_policy_mismatch',
  );
});
...
test('hostile reflective inputs fail closed without escaping the verifier', () => {
  const candidate = fixture();
  const hostile = new Proxy(candidate as any, {
    ownKeys() {
...
test('provider failure is INDETERMINATE and never becomes authorization', () => {
  ...
  const candidate = fixture();
  assert.deepEqual(
    evaluator(unavailable as any, candidate.policy)({ evidence: candidate.evidence, action: candidate.action }),
    { state: 'INDETERMINATE', reason: 'provider_verification_unavailable', authorizes: false },
  );
});
