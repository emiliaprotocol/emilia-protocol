...
// Generated from loss-experience-feed-v2.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
//
...
import { LOSS_EXPERIENCE_FEED_CLAIM_BOUNDARY, LOSS_EXPERIENCE_FEED_V2_VERSION, signLossExperienceFeed, signLossExperienceFeedV2, verifyLossExperienceFeed, verifyLossExperienceFeedV2, } from './loss-experience-feed.js';
...
test('a real hybrid feed verifies under both pinned keys', async () => {
    ...
    const verified = await verifyLossExperienceFeedV2(feed, verifyOptions(m));
...
test('the v1 verifier refuses a v2 feed cleanly on the version marker', async () => {
    const m = material();
    const feed = await signLossExperienceFeedV2(input(), m.signer);
    const verified = verifyLossExperienceFeed(feed, verifyOptions(m));
...
test('the v1 verifier still accepts a v1 feed, unchanged', () => {
    ...
    const verified = verifyLossExperienceFeed(feed, {
        ...verifyOptions(m),
...
test('LEG STRIPPING: removing the ML-DSA leg refuses structurally', async () => {
    const m = material();
    const feed = await signLossExperienceFeedV2(input(), m.signer);
    const stripped = { ...feed, proof: { ...feed.proof, signatures: feed.proof.signatures.filter((s) => s.alg === 'Ed25519') } };
    const verified = await verifyLossExperienceFeedV2(stripped, verifyOptions(m));
    assert.equal(verified.accepted, false);
});
test('LEG STRIPPING: removing the Ed25519 leg refuses too', async () => {
    const m = material();
    const feed = await signLossExperienceFeedV2(input(), m.signer);
    const stripped = { ...feed, proof: { ...feed.proof, signatures: feed.proof.signatures.filter((s) => s.alg === 'ML-DSA-65') } };
    const verified = await verifyLossExperienceFeedV2(stripped, verifyOptions(m));
    assert.equal(verified.accepted, false);
});
test('SET NARROWING: dropping the PQ leg and narrowing required_algorithms fails', async () => {
    ...
    const narrowed = {
        ...
        proof: {
            ...
            signatures: feed.proof.signatures.filter((s) => s.alg === 'Ed25519'),
        },
    };
    const verified = await verifyLossExperienceFeedV2(narrowed, verifyOptions(m));
    assert.equal(verified.accepted, false);
});
test('WRONG-LENGTH SIGNATURE: a truncated ML-DSA signature refuses', async () => {
    ...
    const tampered = {
        ...
        proof: {
            ...feed.proof,
            signatures: feed.proof.signatures.map((s) => (s.alg === 'ML-DSA-65' ? { ...s, sig: s.sig.slice(0, -8) } : s)),
        },
    };
    const verified = await verifyLossExperienceFeedV2(tampered, verifyOptions(m));
    assert.equal(verified.accepted, false);
});
test('ED448 MASQUERADE: an Ed448 key pinned as the Ed25519 half refuses', async () => {
    ...
    const verified = await verifyLossExperienceFeedV2(feed, {
        ...verifyOptions(m),
        trusted_keys: {
            [m.signer.key_id]: {
...
test('NO ML-DSA BACKEND is a refusal, never a pass on the classical leg', async () => {
    const m = material();
    const feed = await signLossExperienceFeedV2(input(), m.signer);
    const verified = await verifyLossExperienceFeedV2(feed, { ...verifyOptions(m), mldsaBackendLoader: async () => null });
    assert.equal(verified.accepted, false);
});
