...
test('run() consumes an indeterminate attempt when the executor throws', async () => {
    ...
    await assert.rejects(() => g.run({ selector: PAY, receipt: r }, async () => {
...
test('receiptAssuranceTier is cryptographically verified, not payload-inferred (DoD audit)', () => {
    ...
    assert.equal(receiptAssuranceTier(signoffDoc, { approverKeys: { device: { approver_id: s.signoff.context.approver, public_key: s.approver_public_key } }, ...WEBAUTHN_SCOPE }), 'class_a');
...
test('default product pack guards the seven high-risk action families', () => {
    ...
    assert.deepEqual(guarded.map((a) => a.action_type).sort(), [
        'data.export',
        'deploy.production',
