...
import {
  createGate, createEvidenceLog, MemoryConsumptionStore, createRuntimeMonitor,
  createMemoryCapabilityStore, mintCapabilityReceipt, CAPABILITY_SCOPE_PROFILE, capabilityActionDigest,
} from './index.js';
...
function fixture({ delayAt = '', delayMs = 120_000, maxAgeSec = 900, releaseFails = false, onReserved = () => {} } = {}) {
  ...
  runtimeMonitor.beginExecution = (cycleId, authorization) => {
    ...
    if (delayAt === 'runtime-monitor') clock += delayMs;
    return result;
  };
  class Store extends MemoryConsumptionStore {
    async reserve(key) {
      ...
      if (reserved) {
        if (delayAt === 'reserve') clock += delayMs;
        onReserved();
      }
      return reserved;
    }
    async release(key) {
      ...
      if (releaseFails) throw new Error('backend unavailable');
  ...
  const log = createEvidenceLog({
    strict: true,
    sink: async (record) => {
      if (delayAt === 'decision-log' && record.kind === 'decision' && record.allow) clock += delayMs;
    },
  });
  ...
  return {
    ...
    calls: () => calls,
    advance: (ms) => { clock += ms; },
    invoke: (receipt, effect = async () => 'settled') => gate.run(
      { selector: { action_type: ACTION }, receipt },
      async () => { calls += 1; return effect(); },
    ),
  };
}
