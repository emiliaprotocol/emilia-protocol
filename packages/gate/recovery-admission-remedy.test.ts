...
test('captures the checked admission before any awaited store call', async () => {
  ...
  const readBarrier = new Promise<void>((resolve) => { readReached = resolve; });
  ...
  await readBarrier;
  (mutable.admission as AdmissionSnapshotInput).action_digest = d('swapped-after-check');
  (mutable.admission as AdmissionSnapshotInput).tenant_id = 'tenant:swapped';
  releaseRead();
...
test('rechecks the exact remedy claim revision before reservation', async () => {
  const f = await fixture();
  let reads = 0;
  const remedyProgramStore: RemedyProgramStore = {
    ...
    async get(input) {
      ...
      reads += 1;
      if (reads < 2 || !result.ok || !result.state) return result;
      return {
        ok: true,
        state: {
