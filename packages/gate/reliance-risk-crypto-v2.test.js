...
// Generated from reliance-risk-crypto-v2.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
//
...
import { RISK_HYBRID_PROFILE, RISK_HYBRID_REQUIRED_ALGORITHMS, signRiskBody, verifyRiskBody, signRiskBodyV2, verifyRiskBodyV2, } from './reliance-risk-crypto.js';
import { canonicalize } from './execution-binding.js';
import { GATE_ALLOWANCE_VERSION, signGateAllowance, verifyGateAllowance, } from './allowance.js';
...
function buildV2(fields = { claim: 'reliance-ok', amount: 42 }) {
...
function buildV1(fields = { claim: 'reliance-ok', amount: 42 }) {
...
test('the v2 verifier refuses a v1 artifact on the version marker', async () => {
    const res = await verifyRiskBodyV2(buildV1(), V1, PINS);
    assert.equal(res.valid, false);
    assert.equal(res.reason, 'artifact_signature_envelope_invalid');
});
...
test('an existing caller (Gate allowance v1) still verifies unchanged through the helper', () => {
    ...
    assert.equal(signed['@version'], GATE_ALLOWANCE_VERSION);
...
test('SET NARROWING: dropping the PQ leg AND narrowing required_algorithms fails structurally AND cryptographically', async () => {
    ...
    const survivingEd = stmt.proof.signatures.find((s) => s.alg === 'Ed25519');
    ...
    assert.equal(crypto.verify(null, narrowedBytes, ed.publicKey, Buffer.from(survivingEd.sig, 'base64url')), false, 'narrowing the committed set must break the surviving signature');
});
...
test('DUPLICATE ALGORITHM: two entries for one algorithm refuse', async () => {
    const stmt = mutable(await buildV2());
    const edLeg = stmt.proof.signatures.find((s) => s.alg === 'Ed25519');
...
test('pinning the Ed25519 half but not the ML-DSA half refuses (both halves required)', async () => {
    const res = await verifyRiskBodyV2(await buildV2(), V2, {
        [KEY_ID]: { issuer_id: ISSUER, public_key: edPubB64u },
    });
    assert.equal(res.valid, false);
    assert.equal(res.reason, 'issuer_untrusted');
});
