...
export interface RemedyProgramStore {
  ...
  get(input: { tenantId: string; instanceId: string }): Promise<RemedyProgramResult>;
...
function snapshot<T>(value: T, label: string): Readonly<T> {
  try {
    canonicalize(value);
...
function isFailure(value: Readonly<DataRecord> | Failure): value is Failure {
  return value.ok === false && typeof value.reason === 'string';
}

function pass(fields: DataRecord): Readonly<DataRecord> {
  return deepFreeze(fields);
}

function withoutOk(projection: DataRecord): DataRecord {
  const result: DataRecord = {};
  for (const [key, value] of Object.entries(projection)) {
    if (key !== 'ok') result[key] = clone(value);
  }
  return result;
}
...
function validOriginalReconciliation(value: unknown): value is DataRecord {
...
function validDispute(value: unknown): value is DataRecord {
...
function validStoredOutcome(value: unknown, reconciliation: boolean): value is DataRecord {
...
function validRemedyOwner(value: DataRecord): boolean {
...
function validAttempt(value: unknown): value is DataRecord {
  ...
  if (finalized !== (typeof value.finalize_request_digest === 'string' && DIGEST.test(value.finalize_request_digest))
      ...
      || finalized !== (typeof value.outcome === 'string')) return false;
  ...
  if (reconciled && !validStoredOutcome(value.reconciliation, true)) return false;
  return true;
}

function validState(value: unknown, tenantId?: string, instanceId?: string): value is DataRecord {
  if (!exactKeys(value, STATE_KEYS)
      ...
      || !value.used_evidence_digests.every((entry: unknown) => typeof entry === 'string' && DIGEST.test(entry))
  ...
  if (value.original_reconciliation !== null
      && !validOriginalReconciliation(value.original_reconciliation)) return false;
  if (value.original_reconciliation !== null
  ...
  const committed = value.remedies
    .filter((attempt: DataRecord) => attempt.status === 'executed')
    .reduce((total: number, attempt: DataRecord) => total + attempt.units, 0);
  if (committed !== value.remedied_units || committed > value.max_remedy_units) return false;
  ...
  if (value.status === 'effect_indeterminate' && effectiveOriginalOutcome !== 'indeterminate') return false;
  if (value.status === 'original_proved_no_effect') {
    if (effectiveOriginalOutcome !== 'proved_no_effect'
        ...
        || value.remedied_units !== 0) return false;
  } else if (effectiveOriginalOutcome === 'proved_no_effect') return false;
  if (!['effect_indeterminate', 'original_proved_no_effect', 'disputed', 'resolved_no_remedy'].includes(value.status)
      && effectiveOriginalOutcome !== 'executed') return false;
  if (value.status === 'disputed' && effectiveOriginalOutcome === 'proved_no_effect') return false;
  if (['remedy_authorized', 'remedy_claimed', 'remedy_indeterminate'].includes(value.status)
      !== (value.active_remedy !== null)) return false;
  return true;
}
...
function validEvidencePair(id: unknown, evidenceDigest: unknown): boolean {
  return validId(id) && typeof evidenceDigest === 'string' && DIGEST.test(evidenceDigest);
}

function evidenceUsed(state: DataRecord, id: string, evidenceDigest: string): boolean {
  return state.used_evidence_ids.includes(id) || state.used_evidence_digests.includes(evidenceDigest);
}

function consumeEvidence(state: DataRecord, id: string, evidenceDigest: string): void {
  state.used_evidence_ids.push(id);
  state.used_evidence_digests.push(evidenceDigest);
}

function expectedContext(state: DataRecord): Readonly<DataRecord> {
  return snapshot({
  ...
  }, 'verifier expectation') as Readonly<DataRecord>;
}
 ...
 */
export function createRemedyMemoryStore(): RemedyProgramStore {
  ...
  const remedyOperations = new Set<string>();
  const remedyActions = new Set<string>();
  const remedyCaids = new Set<string>();
  const claimKey = (tenantId: string, value: string) => `${tenantId.length}:${tenantId}${value}`;
  const attempts = (state: DataRecord): DataRecord[] => [
    ...(state.active_remedy === null ? [] : [state.active_remedy]),
    ...state.remedies,
  ];
  const claimEvidence = (state: DataRecord) => {
    for (const id of state.used_evidence_ids) evidenceIds.add(claimKey(state.tenant_id, id));
    for (const value of state.used_evidence_digests) {
      evidenceDigests.add(claimKey(state.tenant_id, value));
    }
  };
  const claimAttempts = (state: DataRecord) => {
  ...
  return Object.freeze({
    ...
    async create(state: RemedyProgramState) {
      let candidate: Readonly<DataRecord>;
      try {
        candidate = snapshot(state, 'remedy state') as Readonly<DataRecord>;
      } catch {
        return fail('store_state_invalid');
      }
      if (!validState(candidate)) return fail('store_state_invalid');
      const key = storageKey(candidate.tenant_id, candidate.instance_id);
      if (records.has(key)) return fail('instance_exists');
      if (candidate.used_evidence_ids.some((id: string) => evidenceIds.has(claimKey(candidate.tenant_id, id)))
          || candidate.used_evidence_digests.some((value: string) => (
            evidenceDigests.has(claimKey(candidate.tenant_id, value))
          ))) return fail('evidence_replayed');
      records.set(key, clone(candidate));
      claimEvidence(candidate as DataRecord);
      claimAttempts(candidate as DataRecord);
    ...
    async compareAndSwap({ tenantId, instanceId, expectedRevision, state }: DataRecord) {
      ...
      const newEvidenceIds = candidate.used_evidence_ids.filter((id: string) => (
        !current.used_evidence_ids.includes(id)
      ));
      const newEvidenceDigests = candidate.used_evidence_digests.filter((value: string) => (
        !current.used_evidence_digests.includes(value)
      ));
      if (newEvidenceIds.some((id: string) => evidenceIds.has(claimKey(tenantId, id)))
          || newEvidenceDigests.some((value: string) => evidenceDigests.has(claimKey(tenantId, value)))) {
        return fail('evidence_replayed');
      }
      const currentOperations = new Set(attempts(current).map((attempt) => attempt.remedy_operation_id));
      const newAttempts = attempts(candidate as DataRecord).filter((attempt) => (
        !currentOperations.has(attempt.remedy_operation_id)
      ));
      if (newAttempts.some((attempt) => (
  ...
  } as RemedyProgramStore);
}

function verifierInput(fields: DataRecord): Readonly<DataRecord> {
  return snapshot(fields, 'verifier input') as Readonly<DataRecord>;
}
...
export function createRemedyProgramKernel(options: RemedyProgramKernelConfig) {
  ...
  for (const key of [
    ...
    if (typeof options[key] !== 'function') throw new TypeError(`${key} callback required`);
  }
  ...
  const checkedState = (value: unknown, tenantId: string, instanceId: string): RemedyProgramResult => {
    let state: Readonly<DataRecord>;
    try {
      state = snapshot(value, 'stored remedy state') as Readonly<DataRecord>;
    } catch {
      return fail('store_state_invalid');
    }
    return validState(state, tenantId, instanceId)
      ? pass({ ok: true, state }) as RemedyProgramResult
      : fail('store_state_invalid');
  };

  const normalizeStoreResult = (
    value: unknown,
    tenantId: string,
    instanceId: string,
    proposedState?: DataRecord,
  ): RemedyProgramResult => {
    if (!isDataRecord(value)) return fail('store_response_invalid');
    if (value.ok === true) return checkedState(value.state ?? proposedState, tenantId, instanceId);
    if (value.applied === true && proposedState) return checkedState(value.state ?? proposedState, tenantId, instanceId);
    ...
    if (value.ok === false || value.applied === false) return fail('store_operation_failed');
  ...
  const load = async (
    ...
    if (!validContext(tenantId) || !validId(instanceId)) return fail('remedy_lookup_invalid');
    try {
      const result = normalizeStoreResult(
        await store.get({ tenantId, instanceId }), tenantId, instanceId,
      );
  ...
  const commit = async (state: DataRecord, next: DataRecord): Promise<RemedyProgramResult> => {
    try {
      const result = normalizeStoreResult(await store.compareAndSwap({
        tenantId: state.tenant_id,
        instanceId: state.instance_id,
        expectedRevision: state.revision,
        state: next as RemedyProgramState,
  ...
  const operationInput = (input: unknown, keys: Set<string>, reason: string): Readonly<DataRecord> | Failure => {
    let value: Readonly<DataRecord>;
    try {
      value = snapshot(input, reason) as Readonly<DataRecord>;
    } catch {
      return fail(reason);
    }
  ...
  const transitionTime = (state: DataRecord): number | Failure => {
    const current = clock();
    if (!Number.isFinite(current)) return fail('clock_invalid');
    if (current < instant(state.updated_at)) return fail('clock_regression');
    return current;
  };

  const touch = (state: DataRecord, at: number): DataRecord => {
  ...
  const invoke = async (
    callback: (input: Readonly<DataRecord>) => unknown | Promise<unknown>,
    input: DataRecord,
  ): Promise<Readonly<DataRecord> | null> => {
    try {
      const result = await callback(verifierInput(input));
      return snapshot(result, 'verifier projection') as Readonly<DataRecord>;
    } catch {
      return null;
    }
  };

  async function create(input: unknown): Promise<RemedyProgramResult> {
    const parsed = operationInput(input, CREATE_KEYS, 'create_input_invalid');
    if (isFailure(parsed)) return parsed;
    const value = parsed as Readonly<DataRecord>;
    ...
    if (existing.ok) {
      return existing.state!.create_request_digest === requestDigest
        ? pass({ ok: true, idempotent: true, state: existing.state }) as RemedyProgramResult
        : fail('instance_exists');
    }
    if (existing.reason !== 'instance_not_found') return existing;
    const current = clock();
    if (!Number.isFinite(current)) return fail('clock_invalid');
    if (instant(value.original.occurred_at) > current) return fail('original_effect_time_invalid');
    ...
    const state: DataRecord = {
      ...
      created_at: at,
      updated_at: at,
      original: withoutOk(verified as DataRecord),
      remedy_profile_digest: value.remedyProfileDigest,
      destination_binding_digest: value.destinationBindingDigest,
      max_remedy_units: value.maxRemedyUnits,
      unit: value.unit,
    ...
    if (!validState(state, value.tenantId, value.instanceId)) return fail('original_effect_invalid');
    try {
      const result = normalizeStoreResult(
        await store.create(snapshot(state, 'initial remedy state') as RemedyProgramState),
        value.tenantId,
        value.instanceId,
        state,
      );
      if (result.reason !== 'instance_exists') return result;
      ...
      return raced.ok && raced.state!.create_request_digest === requestDigest
        ? pass({ ok: true, idempotent: true, state: raced.state }) as RemedyProgramResult
        : fail('instance_exists');
    } catch {
      return fail('store_unavailable');
    }
  ...
  async function reconcileOriginalEffect(input: unknown): Promise<RemedyProgramResult> {
    const parsed = operationInput(
      input,
      ORIGINAL_RECONCILIATION_INPUT_KEYS,
      'original_reconciliation_input_invalid',
    );
    if (isFailure(parsed)) return parsed;
    const value = parsed as Readonly<DataRecord>;
    ...
    const requestDigest = digest(value);
    if (state.original.outcome !== 'indeterminate') return fail('original_effect_already_conclusive');
    ...
    next.original_reconciliation = {
      ...withoutOk(verified as DataRecord), request_digest: requestDigest,
    };
  ...
  async function recordRevocation(input: unknown): Promise<RemedyProgramResult> {
    ...
    if (state.revocation !== null) {
      return state.revocation.request_digest === requestDigest
        ? pass({ ok: true, idempotent: true, code: 'late_revocation_recorded', state }) as RemedyProgramResult
        : fail('revocation_already_recorded');
    }
    if (evidenceUsed(state, value.evidence.id, value.evidence.digest)) return fail('evidence_replayed');
    const at = transitionTime(state);
    if (typeof at !== 'number') return at;
    ...
    const committed = await commit(state, next);
    return committed.ok
      ? pass({ ok: true, code: 'late_revocation_recorded', state: committed.state }) as RemedyProgramResult
      : committed;
  }

  async function openDispute(input: unknown): Promise<RemedyProgramResult> {
    ...
    if (value.dispute.requested_units > state.remaining_units) return fail('dispute_limit_exceeded');
    ...
    next.dispute = { ...withoutOk(verified as DataRecord), request_digest: requestDigest };
    consumeEvidence(next, verified.evidence_id, verified.evidence_digest);
    const committed = await commit(state, next);
    return committed.ok ? pass({ ok: true, state: committed.state }) as RemedyProgramResult : committed;
  }

  const allAttempts = (state: DataRecord): DataRecord[] => [
    ...(state.active_remedy ? [state.active_remedy] : []), ...state.remedies,
  ];

  async function authorizeRemedy(input: unknown): Promise<RemedyProgramResult> {
    ...
    if (!validRemedyOwner(value.authorization)) return fail('remedy_owner_invalid');
    const loaded = await load(value.tenantId, value.instanceId);
    if (!loaded.ok) return loaded;
    const state = loaded.state as DataRecord;
    const requestDigest = digest(value);
  ...
  async function claimRemedy(input: unknown): Promise<RemedyProgramResult> {
    ...
    if (prior?.claim_request_digest === requestDigest) {
      ...
      return pass({ ok: true, idempotent: true, state }) as RemedyProgramResult;
    }
    if (state.status === 'remedy_indeterminate') return fail('remedy_indeterminate');
    if (!state.active_remedy) return prior ? fail('remedy_already_finalized') : fail('remedy_not_authorized');
    if (state.active_remedy.remedy_operation_id !== value.remedyOperationId) {
      return fail('remedy_operation_mismatch');
    }
    if (state.active_remedy.status === 'claimed') return fail('remedy_claim_owned');
  ...
  const outcomeExpected = (state: DataRecord, attempt: DataRecord): DataRecord => ({
  ...
  const outcomeBound = (
    ...
    value: DataRecord,
    attempt: DataRecord,
  ): boolean => verified.evidence_id === value.evidence.evidence_id
    && verified.evidence_digest === value.evidence.evidence_digest
  ...
  async function finalizeRemedy(input: unknown): Promise<RemedyProgramResult> {
    ...
    if (attempt.remedy_operation_id !== value.remedyOperationId) return fail('remedy_operation_mismatch');
    if (attempt.status !== 'claimed' || state.status !== 'remedy_claimed') return fail('remedy_not_claimed');
    if (attempt.claim_token_digest !== tokenDigest(value.claimToken)) return fail('remedy_claim_owned');
    if (evidenceUsed(state, value.evidence.evidence_id, value.evidence.evidence_digest)) {
      return fail('evidence_replayed');
    }
    const at = transitionTime(state);
    ...
    if (instant(value.evidence.observed_at) < instant(attempt.authorized_at)
        || instant(value.evidence.observed_at) > at) return fail('remedy_outcome_time_invalid');
    ...
    if (!outcomeBound(verified as DataRecord, value as DataRecord, attempt)) {
    ...
    completed.outcome_evidence = withoutOk(verified as DataRecord);
  ...
  async function reconcileRemedy(input: unknown): Promise<RemedyProgramResult> {
    ...
    if (instant(value.evidence.observed_at) < instant(attempt.outcome_evidence.observed_at)
        || instant(value.evidence.observed_at) > at) return fail('remedy_reconciliation_time_invalid');
    const verified = await invoke(verifyRemedyOutcome, {
      evidence: clone(value.evidence), outcome: value.outcome,
    ...
    completed.status = value.outcome;
    completed.outcome = value.outcome;
    completed.reconciliation = withoutOk(verified as DataRecord);
  ...
  async function resolveDispute(input: unknown): Promise<RemedyProgramResult> {
    ...
    if (typeof verifyResolution !== 'function') return fail('resolution_verifier_unavailable');
    ...
    if (instant(value.resolution.resolved_at) < instant(state.dispute.opened_at)
        || instant(value.resolution.resolved_at) > at) return fail('resolution_time_invalid');
    ...
    next.resolution = { ...withoutOk(verified as DataRecord), request_digest: requestDigest };
    consumeEvidence(next, verified.evidence_id, verified.evidence_digest);
    const committed = await commit(state, next);
    return committed.ok ? pass({ ok: true, state: committed.state }) as RemedyProgramResult : committed;
  }

  async function status(input: unknown): Promise<RemedyProgramResult> {
