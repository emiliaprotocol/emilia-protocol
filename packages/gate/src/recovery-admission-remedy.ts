...
export type RecoveryAdmissionRemedyRefusalReason =
  ...
  | 'remedy_claim_owned'
  | 'remedy_claim_currentness_mismatch'
...
function snapshotOf(
  ...
  try {
    if (dataRecord(value) && Object.hasOwn(value, 'snapshot_digest')) {
      ...
      const recreated = createAdmissionSnapshot(body as unknown as AdmissionSnapshotInput);
      return recreated.snapshot_digest === (value as AdmissionSnapshot).snapshot_digest
        ? recreated
        : null;
    }
...
function claimTokenDigest(token: string): string {
...
export function createRecoveryAdmissionRemedyBridge(
  ...
  async function reserve(
    ...
    let receipt: unknown;
    let executionProgram: RecoveryAdmissionRemedyProgramInput | undefined;
    let capturedAdmission: AdmissionSnapshotInput | AdmissionSnapshot;
    try {
      ...
      reserved = executionProgram === undefined
        ...
        : await admissionStore.reserveExecutionProgramAdmission({
          ...
          admission: (() => {
            ...
            return body as AdmissionSnapshotInput;
          })(),
        });
    } catch {
      return { ok: false, reason: 'remedy_current_state_unavailable' };
