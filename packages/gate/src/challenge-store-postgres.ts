...
import type {
  ChallengeCapacityBucket,
  ChallengeOwnerBackend,
  ChallengeOwnerRecord,
  ChallengeOwnerTransaction,
...
function exactOne(result: QueryResult, operation: string) {
...
function encodeRecord(record: ChallengeOwnerRecord) {
...
function decodeRecord(value: unknown): ChallengeOwnerRecord {
  ...
  try {
    ...
    return parsed as ChallengeOwnerRecord;
  } catch {
 ...
 */
export function createPostgresChallengeOwnerBackend({
  pool,
  ownerId: configuredOwnerId,
}: {
  pool?: PgPool;
  ownerId?: string;
} = {}): ChallengeOwnerBackend & { health: () => Promise<{ ok: true; version: string }> } {
  ...
  async function transaction<T>(work: (tx: ChallengeOwnerTransaction) => Promise<T>) {
    ...
    try {
      ...
      began = true;
      const tx: ChallengeOwnerTransaction = {
        async authoritativeNowMs() {
          const row = exactOne(
            await client.query(AE_CHALLENGE_POSTGRES_SQL.authoritativeNow, []),
            'authoritative time',
          );
        ...
        async lockCapacity(buckets: ChallengeCapacityBucket[]) {
          ...
          const rows: Record<string, { used: number; limit: number }> = Object.create(null);
  ...
  return Object.freeze({
    ...
    async health() {
      ...
      try {
        ...
        return { ok: true as const, version: AE_CHALLENGE_POSTGRES_OWNER_VERSION };
      } finally {
        client.release();
      }
    },
