...
export type RemedyCaseSetPgClient = {
  query: (text: string, params?: any[]) => Promise<QueryResult>;
  release: () => void;
};

export type RemedyCaseSetPgPool = {
  connect: () => Promise<RemedyCaseSetPgClient>;
};

export interface RemedyCaseSetPostgresStore extends RemedyCaseSetStore {
  readonly durable: true;
  create(state: RemedyCaseSetState): Promise<RemedyCaseSetResult>;
  get(input: Readonly<{
    tenantId: string;
    caseSetId: string;
  }>): Promise<RemedyCaseSetResult>;
...
function exactKeys(value: unknown, expected: ReadonlySet<string>): value is RecordLike {
...
function assertJsonData(value: unknown, seen = new Set<object>()): void {
  ...
  if (typeof value === 'number') {
    if (!Number.isFinite(value)) throw new TypeError('remedy case-set state contains a non-JSON number');
    return;
  }
  ...
  if (Array.isArray(value)) {
    ...
    for (const child of Object.values(value)) assertJsonData(child, seen);
  }
  seen.delete(value);
}
...
function assertCaseSetId(value: unknown): asserts value is string {
...
function digestJson(value: string) {
  return `sha256:${createHash('sha256').update(value, 'utf8').digest('hex')}`;
}

function validObservation(value: unknown) {
...
function encodeState(
  ...
  if (!exactKeys(state, STATE_KEYS)
      ...
      || state.manifest.legs.some(
        (leg: unknown) => !isDataRecord(leg)
      ...
      || state.observations.some(
        (observation: RecordLike, index: number) => (
          observation.leg_id !== state.manifest.legs[index].leg_id
        ),
      )) {
    throw new TypeError('remedy case-set state binding is invalid');
  }
  const statuses = state.observations.map((entry: RecordLike) => entry.status);
  if ((state.status === 'open' && !statuses.every((status: string) => status === 'pending'))
      || (state.status === 'completed'
        && !statuses.every((status: string) => status === 'executed'))
      || (state.status === 'indeterminate'
        && (!statuses.includes('indeterminate')
          || !statuses.every((status: string) => status !== 'pending')))) {
  ...
  return {
    state: state as RemedyCaseSetState,
    stateJson,
...
function strictRows(result: QueryResult, operation: string, maximum: 0 | 1) {
  if (!result
      || !Number.isSafeInteger(result.rowCount)
      || (result.rowCount as number) < 0
      || (result.rowCount as number) > maximum
...
function decodeCurrentRow(
  ...
  try {
    ...
    if (canonicalize(parsed) !== row.state_json) throw new Error('non-canonical state');
  } catch {
  ...
  if (expected && (
    encoded.stateJson !== expected.stateJson
...
function validateTenantContext(result: QueryResult, tenantId: string) {
...
function transactionClock(result: QueryResult) {
  ...
  return {
    value: row.recorded_at as string,
...
function fail(reason: string): RemedyCaseSetResult {
  return { ok: false, reason };
}

function assertNotForwardSkewed(eventAt: number, databaseNow: number) {
...
export function createRemedyCaseSetPostgresStore(
  { pool }: { pool?: RemedyCaseSetPgPool } = {},
): RemedyCaseSetPostgresStore {
  if (!pool || typeof pool.connect !== 'function') {
    throw new TypeError(
      'createRemedyCaseSetPostgresStore requires a transaction-capable pg pool',
    );
  }
  ...
  async function transaction<T>(
    ...
    tenantId: string,
    work: (
      client: RemedyCaseSetPgClient,
      clock: Readonly<{ value: string; milliseconds: number }>,
    ) => Promise<T>,
  ): Promise<T> {
    ...
    try {
      ...
      validateTenantContext(
        await client.query(REMEDY_CASE_SET_POSTGRES_SQL.setTenant, [tenantId]),
        tenantId,
      );
      const clock = transactionClock(
        await client.query(REMEDY_CASE_SET_POSTGRES_SQL.clock),
      );
      ...
      if (began) {
        try {
          ...
          throw new AggregateError(
            [error, rollbackError],
            'remedy case-set transaction and rollback both failed',
          );
        }
      }
      throw error;
  ...
  return Object.freeze({
    durable: true,
    async create(state: RemedyCaseSetState) {
      const tenantId = (state as RecordLike)?.tenant_id;
      const caseSetId = (state as RecordLike)?.case_set_id;
      ...
      return transaction(false, tenantId, async (client, clock) => {
        ...
        const rows = strictRows(
          await client.query(REMEDY_CASE_SET_POSTGRES_SQL.create, [
            tenantId,
            caseSetId,
            encoded.status,
        ...
        if (rows.length === 0) return fail('case_set_exists');
        const stored = decodeCurrentRow(
          rows[0],
          tenantId,
          caseSetId,
          encoded,
        ...
        validateEvent(
          await client.query(REMEDY_CASE_SET_POSTGRES_SQL.appendEvent, [
            tenantId,
            caseSetId,
            0,
            null,
    ...
    async get(input: { tenantId: string; caseSetId: string }) {
      const { tenantId, caseSetId } = lookupInput(input);
      return transaction(true, tenantId, async (client) => {
        const rows = strictRows(
          await client.query(REMEDY_CASE_SET_POSTGRES_SQL.get, [tenantId, caseSetId]),
          'get',
          1,
        );
    ...
    async compareAndSwap(input: {
      ...
      const {
        tenantId, caseSetId, expectedRevision, ownerTokenDigest, state,
      } = input;
      assertTenantId(tenantId);
      assertCaseSetId(caseSetId);
      ...
      return transaction(false, tenantId, async (client, clock) => {
        const rows = strictRows(
          await client.query(
            REMEDY_CASE_SET_POSTGRES_SQL.getForUpdate,
            [tenantId, caseSetId],
          ),
          'getForUpdate',
          1,
        );
        if (rows.length === 0) return fail('case_set_not_found');
        ...
        if (current.status === 'completed') return fail('case_set_terminal');
        ...
        if (encoded.status === 'open') return fail('case_set_reopen_refused');
        if (encoded.eventAtMs < current.eventAtMs) return fail('clock_regression');
        assertNotForwardSkewed(encoded.eventAtMs, clock.milliseconds);

        const updatedRows = strictRows(
          await client.query(REMEDY_CASE_SET_POSTGRES_SQL.compareAndSwap, [
            tenantId,
            caseSetId,
            expectedRevision,
            ownerTokenDigest,
