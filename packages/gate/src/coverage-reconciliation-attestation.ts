...
import {
  ...
  signRiskBody, verifyRiskBody, signRiskBodyV2, verifyRiskBodyV2,
  type RiskRecord, type TrustedRiskKeys, type TrustedRiskKeysV2, type RiskHybridSigner, type RiskV2Options,
...
function validate(value: unknown, expectedVersion: string = COVERAGE_RECONCILIATION_ATTESTATION_VERSION): asserts value is RiskRecord {
...
export function signCoverageReconciliationAttestationV3(
  input: RiskRecord,
  signer: RiskHybridSigner,
  options: RiskV2Options = {},
): Promise<RiskRecord> {
  const body: RiskRecord = { '@version': COVERAGE_RECONCILIATION_ATTESTATION_V3_VERSION, ...input };
...
export async function verifyCoverageReconciliationAttestationV3(attestation: unknown, options: {
  trusted_keys?: TrustedRiskKeysV2; now?: string | number; expected_program?: RiskRecord;
  expected_census_digest?: string; expected_relying_party_id?: string;
  expected_coverage_report_hash?: string;
} & RiskV2Options = {}) {
  const refuse = (reason: string, verified = false, attestationDigest: string | null = null) => ({
  ...
  try { validate(payload, COVERAGE_RECONCILIATION_ATTESTATION_V3_VERSION); } catch {
    return refuse('population_conservation_or_schema_invalid', true, signed.artifact_digest);
  }
  const now = options.now === undefined ? Date.now() : (typeof options.now === 'string' ? Date.parse(options.now) : Number(options.now));
  if (!Number.isFinite(now)) return refuse('verification_time_invalid', true, signed.artifact_digest);
...
export async function verifyCoverageReconciliationAttestationAny(
  attestation: unknown,
  options: Parameters<typeof verifyCoverageReconciliationAttestation>[1]
    & Parameters<typeof verifyCoverageReconciliationAttestationV3>[1] = {},
) {
  if (attestation !== null && typeof attestation === 'object'
      && (attestation as RiskRecord)['@version'] === COVERAGE_RECONCILIATION_ATTESTATION_V3_VERSION) {
