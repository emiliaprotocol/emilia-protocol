...
import {
  ...
  type MemoryProjectionNativeSourceResult,
  type MemoryProjectionVerificationMaterial,
...
export type NativeSourceVerifier = (
  input: NativeSourceClassificationInput,
) => MemoryProjectionNativeSourceResult;

export type NativeSourceEnvelopeValidator = (
  sourceEnvelope: Readonly<RecordLike>,
) => void;
...
export function isDataRecord(value: unknown): value is RecordLike {
...
export function absoluteUri(value: unknown): value is string {
...
export function decodeCanonicalSourceEnvelope(bytes: Uint8Array): RecordLike {
  let value: unknown;
  try {
...
function defaultVerifier(input: NativeSourceClassificationInput): MemoryProjectionNativeSourceResult {
  return Object.freeze({
    valid: true as const,
    formatVersion: 1,
    sealedObjectDigest: input.sealedObjectDigest,
    derivedTrust: 'unverified' as const,
...
export function createNativeMemoryProjection(
  input: NativeMemoryProjectionInput,
  ...
  if (typeof verifier !== 'function') throw new TypeError('native source verifier invalid');

  const sourceObjectBytes = input.sources.map((source) => canonicalBytes(source.sourceEnvelope));
  const nativeResults = sourceObjectBytes.map((bytes) => classify(
    verifier,
    input.providerId,
    input.sourceProfile,
    bytes,
    input.validateSourceEnvelope,
  ));
