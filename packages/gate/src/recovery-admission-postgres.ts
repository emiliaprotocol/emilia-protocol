...
import {
  ...
  type RecoveryAdmissionDecision,
  type RecoveryAdmissionDependencies,
  type RecoveryCapabilityVerificationContext,
  type VerifiedRecoveryCapability,
} from './recovery-admission.js';
import type {
  AdmissionDigest,
  AdmissionProviderOutcome,
  AdmissionRecord,
  AdmissionSnapshot,
  AdmissionStore,
...
export interface RecoveryAdmissionPostgresClient {
  ...
  ): Promise<RecoveryAdmissionPostgresQueryResult>;
...
interface RecoveryAdmissionCapabilityBinding {
  readonly capability: Readonly<VerifiedRecoveryCapability>;
  readonly tenant_id: string;
  readonly admission_id: string;
  readonly admission_snapshot_digest: AdmissionDigest;
  readonly operation_id: string;
  readonly action_caid: string;
  readonly action_digest: AdmissionDigest;
  readonly adapter_digest: AdmissionDigest;
  readonly state_domain_digest: AdmissionDigest;
  readonly max_transaction_ms: number;
...
export interface RecoveryAdmissionPostgresExecutionOptions<TResult> {
   ...
   */
  readonly perform: (
    client: RecoveryAdmissionPostgresClient,
    invocation: Readonly<RecoveryAdmissionPostgresInvocation>,
  ) => Promise<RecoveryAdmissionPostgresPerformance<TResult>>
    | RecoveryAdmissionPostgresPerformance<TResult>;
  readonly validatePrecommit: (
    client: RecoveryAdmissionPostgresClient,
    performed: Readonly<RecoveryAdmissionPostgresPerformance<TResult>>,
    invocation: Readonly<RecoveryAdmissionPostgresInvocation>,
  ) => Promise<boolean> | boolean;
  readonly recheckCurrent: (
    client: RecoveryAdmissionPostgresClient,
    invocation: Readonly<RecoveryAdmissionPostgresInvocation>,
  ) => Promise<boolean> | boolean;
...
export type RecoveryAdmissionPostgresFailureReason =
  | 'evaluation_failed'
  | 'recovery_admission_refused'
  | 'route_mismatch'
  | 'recovery_binding_invalid'
  | 'pool_guarantee_mismatch'
  | 'state_domain_digest_mismatch'
  | 'adapter_digest_mismatch'
  | 'admission_store_guarantee_mismatch'
  | 'admission_read_failed'
  | 'admission_not_found'
  | 'admission_snapshot_read_failed'
  | 'admission_snapshot_not_found'
  | 'admission_binding_mismatch'
  | 'admission_not_reserved'
  | 'begin_invocation_refused'
  | 'begin_invocation_ambiguous'
  | 'deadline_expired'
  | 'transaction_timeout'
  | 'clock_invalid'
  | 'connection_failed'
  ...
  | 'currentness_recheck_failed'
  | 'rollback_failed'
  | 'commit_acknowledgement_failed'
  | 'provider_outcome_recording_failed';

type PreInvocationFailureReason = Extract<
  RecoveryAdmissionPostgresFailureReason,
  | 'evaluation_failed'
  | 'recovery_admission_refused'
  | 'route_mismatch'
  | 'recovery_binding_invalid'
...
type ProvenNotCommittedReason = Extract<
  RecoveryAdmissionPostgresFailureReason,
  | 'deadline_expired'
  | 'transaction_timeout'
  | 'clock_invalid'
...
type IndeterminateReason = Exclude<
  RecoveryAdmissionPostgresFailureReason,
  PreInvocationFailureReason | ProvenNotCommittedReason
> | ProvenNotCommittedReason;

export type RecoveryAdmissionPostgresExecutionResult<TResult> =
  | Readonly<{
      outcome: 'COMMITTED';
      invoked: true;
      result: TResult;
...
function digest(value: unknown): value is AdmissionDigest {
...
function evidenceFields(evidenceDigest: string | undefined) {
  return evidenceDigest === undefined ? {} : { evidence_digest: evidenceDigest };
}

function notInvoked(
  reason: PreInvocationFailureReason,
): RecoveryAdmissionPostgresExecutionResult<never> {
  return Object.freeze({ outcome: 'NOT_INVOKED' as const, invoked: false as const, reason });
}
...
function committed<TResult>(
  result: TResult,
  evidenceDigest: string,
): RecoveryAdmissionPostgresExecutionResult<TResult> {
  return Object.freeze({
...
function assertCallbacks<TResult>(
  options: RecoveryAdmissionPostgresExecutionOptions<TResult>,
): void {
  if (!isRecord(options)
...
function capabilityBinding(
  decision: Readonly<RecoveryAdmissionDecision>,
): RecoveryAdmissionCapabilityBinding | 'route_mismatch' | 'recovery_binding_invalid' {
  if (decision.recovery_route_accepted !== true
      || decision.route !== 'LOCAL_ATOMIC'
      || decision.scope !== 'INTRA_TRANSACTION_ONLY'
      || decision.retry_permitted !== false) return 'route_mismatch';
  ...
  if (!isRecord(capability)
      ...
      || !validInstant(decision.status.valid_until)) return 'recovery_binding_invalid';
  ...
  return Object.freeze({
    ...
    deadline: Math.min(
      ...
      Date.parse(capability.action_capability_expires_at),
      Date.parse(decision.status.valid_until),
    ),
  });
}
...
function snapshotMatches(
  snapshot: unknown,
  binding: RecoveryAdmissionCapabilityBinding,
): snapshot is Readonly<AdmissionSnapshot> {
  if (!(isRecord(snapshot)
    ...
    && snapshot.body.executor_adapter_digest === binding.adapter_digest)) return false;
  try {
    const derived = deriveRecoveryAdmissionSnapshotBindings(
      snapshot.body as Readonly<AdmissionSnapshot['body']>,
    );
...
function recordMatches(
  record: unknown,
  binding: RecoveryAdmissionCapabilityBinding,
): record is Readonly<AdmissionRecord> {
...
function poolFailure(
  pool: unknown,
  binding: RecoveryAdmissionCapabilityBinding,
): PreInvocationFailureReason | null {
  if (!isRecord(pool) || typeof pool.connect !== 'function') return 'pool_guarantee_mismatch';
  if (pool.localAtomic !== true
      || pool.policyBoundToSingleTransaction !== true
      || pool.externalEffectsForbidden !== true) return 'pool_guarantee_mismatch';
  if (pool.stateDomainDigest !== binding.state_domain_digest) {
    return 'state_domain_digest_mismatch';
  }
  if (pool.adapterDigest !== binding.adapter_digest) return 'adapter_digest_mismatch';
  return null;
}
 ...
 */
export async function executeRecoveryAdmissionPostgresLocalAtomic<TResult>(
  options: RecoveryAdmissionPostgresExecutionOptions<TResult>,
  ...
  if (decision.route === 'REFUSED') return notInvoked('recovery_admission_refused');
  const binding = capabilityBinding(decision);
  if (typeof binding === 'string') return notInvoked(binding);

  const selectedPoolFailure = poolFailure(options.pool, binding);
  if (selectedPoolFailure) return notInvoked(selectedPoolFailure);
  ...
  if (reservedRecord === null) return notInvoked('admission_not_found');
  if (!recordMatches(reservedRecord, binding)) return notInvoked('admission_binding_mismatch');

  let reservedSnapshot: Readonly<AdmissionSnapshot> | null;
  try {
  ...
  if (reservedSnapshot === null) return notInvoked('admission_snapshot_not_found');
  ...
  const invokingRecord = begun.record as Readonly<AdmissionRecord>;
  const invocationToken = options.invocationToken;
  const invocation = Object.freeze({
    decision,
    capability: binding.capability,
    snapshot: begun.snapshot as Readonly<AdmissionSnapshot>,
  ...
  const persistOutcome = async <T>(
    ...
    result: RecoveryAdmissionPostgresExecutionResult<T>,
  ): Promise<RecoveryAdmissionPostgresExecutionResult<T>> => {
    try {
      const recorded = await admissionStore.recordProviderOutcome({
        ...
        invocation_token: invocationToken,
        value,
        evidence_digest: (evidenceDigest as AdmissionDigest | undefined) ?? null,
  ...
  const persistIndeterminate = (
    ...
    'INDETERMINATE',
    evidenceDigest,
    indeterminate(true, reason, evidenceDigest),
  );

  let client: RecoveryAdmissionPostgresClient;
  try {
  ...
  const timingFailure = (): ProvenNotCommittedReason | null => {
    let current: number;
    try {
      current = now();
    } catch {
    ...
    if (!Number.isFinite(current) || current < transactionStartedAt) return 'clock_invalid';
    if (current >= binding.deadline) return 'deadline_expired';
  ...
  const rollback = async (
    reason: ProvenNotCommittedReason,
  ): Promise<RecoveryAdmissionPostgresExecutionResult<never>> => {
    try {
    ...
    if (!evidenceDigest) return await persistIndeterminate('evidence_required');
    return await persistOutcome(
      'PROVEN_NOT_COMMITTED',
      evidenceDigest,
      provenNotCommitted(reason, evidenceDigest),
    );
  };
  ...
  try {
    ...
    if (timing) return await rollback(timing);

    let performed: RecoveryAdmissionPostgresPerformance<TResult>;
    try {
      ...
      performed = rawPerformed as RecoveryAdmissionPostgresPerformance<TResult>;
    } catch {
      return await rollback('perform_failed');
    }
    ...
    return await persistOutcome(
      'COMMITTED',
      committedEvidenceDigest,
      committed(performed.result, committedEvidenceDigest),
    );
  } finally {
    try {
