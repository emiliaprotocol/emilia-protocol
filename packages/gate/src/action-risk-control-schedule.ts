...
export const ACTION_RISK_CONTROL_SCHEDULE_CLAIM_BOUNDARY =
  'technical_control_requirements_only_not_policy_coverage_premium_liability_action_authorization_or_effect_proof';
export const ACTION_RISK_QUALIFICATION_STATUS_CLAIM_BOUNDARY =
  'technical_qualification_observation_only_not_policy_coverage_premium_liability_or_action_authorization';
...
export const ACTION_RISK_INDETERMINATE_HANDLING =
  'REFUSE_RETRY_PRESERVE_OPEN_EXPOSURE_REQUIRE_RECONCILIATION';
export const ACTION_RISK_DIVERGENT_HANDLING =
...
const ACTION_KEYS = ['action_class', 'caid_profile_id', 'caid_profile_digest'] as const;
const PROVIDER_KEYS = ['provider_id', 'account_id', 'environment', 'adapter_digest'] as const;
...
const CONTROL_KEYS = ['aeb_digest', 'aec_digest', 'local_policy_digest'] as const;
const MEDIATION_KEYS = ['surface_inventory_digest', 'refusal_probe_evidence_digest'] as const;
const LOSS_ALLOCATION_KEYS = ['program_digest'] as const;
...
const OUTCOME_SOURCE_KEYS = ['role', 'source_class'] as const;
const OUTCOME_WINDOW_KEYS = [
  'opens_before_provider_entry_sec',
  'closes_after_provider_entry_sec',
  'max_observation_age_sec',
] as const;
const HANDLING_KEYS = ['indeterminate', 'divergent'] as const;
const TRUST_REFERENCE_KEYS = ['purpose', 'authority_id', 'key_id', 'key_digest'] as const;
...
const STATUS_BODY_KEYS = ['@version', ...STATUS_SOURCE_KEYS, 'issuer'] as const;
...
const HYBRID_SIGNATURE_KEYS = ['alg', 'sig', 'key_id'] as const;
...
function validateBoundHybridProof(value: unknown): asserts value is RiskRecord {
  if (!riskRecord(value)
      ...
      || !value.proof.required_algorithms.every(
        (algorithm: unknown, index: number) => algorithm === RISK_HYBRID_REQUIRED_ALGORITHMS[index],
      )) {
...
function validAction(value: unknown): value is RiskRecord {
...
function validProviderBinding(value: unknown): value is RiskRecord {
...
function validQualification(value: unknown): value is RiskRecord {
...
function validControlBindings(value: unknown): value is RiskRecord {
...
function validCompleteMediation(value: unknown): value is RiskRecord {
  return riskExact(value, MEDIATION_KEYS)
    && digest(value.surface_inventory_digest)
    && digest(value.refusal_probe_evidence_digest);
}

function validLossAllocation(value: unknown): value is RiskRecord {
  return riskExact(value, LOSS_ALLOCATION_KEYS) && digest(value.program_digest);
}

function validOpenExposure(value: unknown): value is RiskRecord {
  if (!riskExact(value, OPEN_EXPOSURE_KEYS)
      ...
      || !boundedInteger(value.reconciliation_deadline_sec, 1, MAX_SECONDS)) return false;
  try {
...
function validOutcomeBinding(value: unknown): value is RiskRecord {
  if (!riskExact(value, OUTCOME_BINDING_KEYS)
      ...
      || !boundedInteger(value.observation_window.max_observation_age_sec, 1, MAX_SECONDS)) return false;
  ...
  for (const source of value.required_sources) {
    ...
    if (seen.has(identity) || (previous !== null && previous > identity)) return false;
    seen.add(identity);
    previous = identity;
  }
  return true;
...
function validHandling(value: unknown): value is RiskRecord {
...
function validTrustReferences(value: unknown): value is RiskRecord[] {
  if (!denseArray(value, 2, 2)) return false;
  const seen = new Set<string>();
  ...
  for (const reference of value) {
    if (!riskExact(reference, TRUST_REFERENCE_KEYS)
        ...
        || !digest(reference.key_digest)) return false;
...
function validateScheduleSource(value: unknown): asserts value is RiskRecord {
...
function findTrustReference(body: RiskRecord, purpose: string): RiskRecord | undefined {
  return body.trust_pin_references.find((reference: RiskRecord) => reference.purpose === purpose);
}

function validateScheduleBody(value: unknown): asserts value is RiskRecord {
...
function validateObservedControls(value: unknown): asserts value is RiskRecord {
...
function validOutcome(value: unknown): value is ActionRiskControlOutcome {
  return typeof value === 'string'
    && (ACTION_RISK_CONTROL_OUTCOMES as readonly string[]).includes(value);
}

function validateQualificationStatusSource(value: unknown): asserts value is RiskRecord {
...
function validateQualificationStatusBody(value: unknown): asserts value is RiskRecord {
...
function validateQualificationStatusHead(value: unknown): asserts value is RiskRecord {
...
export function actionRiskHybridTrustPinDigest(
  keyId: string,
  pin: { issuer_id: string; public_key: string; pq_public_key: string },
): string {
...
export function actionRiskControlScheduleDigest(artifact: unknown): string {
...
export function actionRiskQualificationStatusDigest(artifact: unknown): string {
  validateBoundHybridProof(artifact);
  const { proof: _proof, ...body } = artifact;
...
export async function signActionRiskControlSchedule(
  input: unknown,
  signer: ActionRiskControlScheduleSigner,
...
export async function signActionRiskQualificationStatus(
  input: unknown,
  signer: ActionRiskQualificationStatusSigner,
  options: RiskV2Options = {},
): Promise<RiskRecord> {
 ...
 */
export async function evaluateActionRiskControlSchedule(
  artifact: unknown,
  options: ActionRiskControlScheduleEvaluationOptions = {},
): Promise<RiskRecord> {
  const result = (
    outcome: ActionRiskControlOutcome,
    reason: string,
    fields: {
      ...
      statusHeadSequence?: number | null;
  ...
  try {
    ...
    return result(
      'NOT_ELIGIBLE',
      error instanceof ActionRiskControlScheduleValidationError ? error.code : 'artifact_signature_envelope_invalid',
    );
  }
  ...
  const signed = await verifyRiskBodyV2(
    ...
    options.trusted_schedule_keys,
    options,
  );
  if (!signed.valid || !signed.body || !signed.artifact_digest) {
    return result(
      signed.reason === 'pq_backend_unavailable' ? 'INDETERMINATE' : 'NOT_ELIGIBLE',
      signed.reason ?? 'schedule_invalid',
    );
  }
  try {
    ...
    return result(
      'NOT_ELIGIBLE',
      error instanceof ActionRiskControlScheduleValidationError ? error.code : 'schedule_schema_invalid',
      { scheduleVerified: true, scheduleDigest: signed.artifact_digest },
    );
  }
  ...
  if (!Number.isFinite(now)) return result('NOT_ELIGIBLE', 'verification_time_invalid', base);
  if (now < riskInstant(body.issued_at)) return result('NOT_ELIGIBLE', 'schedule_not_yet_issued', base);
  if (now < riskInstant(body.valid_from)) return result('NOT_ELIGIBLE', 'schedule_not_yet_valid', base);
  if (now >= riskInstant(body.expires_at)) return result('NOT_ELIGIBLE', 'schedule_expired', base);

  if (options.observed_controls === undefined) {
    return result('NOT_ELIGIBLE', 'control_observation_required', base);
  }
  let observed: RiskRecord;
  try {
    ...
    return result(
      'NOT_ELIGIBLE',
      error instanceof ActionRiskControlScheduleValidationError ? error.code : 'control_observation_invalid',
      base,
    );
  }
  ...
  for (const [reason, expected, actual] of comparisons) {
    if (!sameCanonical(expected, actual)) return result('NOT_ELIGIBLE', reason, base);
  }
  ...
  if (!statusPin) return result('NOT_ELIGIBLE', 'qualification_status_trust_pin_required', base);
  ...
  const statusSigned = await verifyRiskBodyV2(
    options.qualification_status,
    ACTION_RISK_QUALIFICATION_STATUS_VERSION,
    options.trusted_status_keys,
    options,
  );
  ...
  try {
    ...
    return result(
      'NOT_ELIGIBLE',
      error instanceof ActionRiskControlScheduleValidationError ? error.code : 'qualification_status_schema_invalid',
      { ...base, statusVerified: true, statusDigest: statusSigned.artifact_digest },
    );
  }
  ...
  try {
    ...
    return result(
      'NOT_ELIGIBLE',
      error instanceof ActionRiskControlScheduleValidationError ? error.code : 'qualification_status_head_invalid',
      statusBase,
    );
  }
