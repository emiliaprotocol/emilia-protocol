...
function validateBody(
  value: unknown,
  version: string = LOSS_EXPERIENCE_FEED_VERSION,
...
export async function signLossExperienceFeedV2(
  input: RiskRecord,
  signer: LossExperienceFeedSignerV2,
  options: RiskV2Options = {},
) {
  const body: RiskRecord = { '@version': LOSS_EXPERIENCE_FEED_V2_VERSION, ...input };
 ...
 */
export async function verifyLossExperienceFeedV2(
  feed: unknown,
  options: VerifyLossExperienceFeedOptionsV2 = {},
) {
  const refuse = (reason: string, verified = false, feedDigest: string | null = null) => ({
    accepted: false,
    verified,
