...
export const RISK_HYBRID_REQUIRED_ALGORITHMS = Object.freeze(['Ed25519', 'ML-DSA-65'] as const);
...
export interface RiskHybridSignature { alg: string; sig: string; key_id?: string; }
...
const RISK_PROOF_V2_KEYS = ['profile', 'required_algorithms', 'key_id', 'body_digest', 'signatures'] as const;
 ...
 */
function riskV2SigningBytes(
  version: string,
  body: RiskRecord,
  requiredAlgorithms: readonly string[] = RISK_HYBRID_REQUIRED_ALGORITHMS,
): Buffer {
 ...
 */
export async function signRiskBodyV2(
  version: string,
  bodyInput: RiskRecord,
 ...
 */
export async function verifyRiskBodyV2(
  artifact: unknown,
  version: string,
  ...
  const refuse = (reason: string) => ({ valid: false, reason, body: null as RiskRecord | null, artifact_digest: null as string | null });
  try {
    if (!riskRecord(artifact) || !riskRecord(artifact.proof) || !riskRecord(artifact.issuer)) return refuse('artifact_shape_invalid');
    const proof = artifact.proof;
    const { proof: _proof, ...body } = artifact;
    ...
    if (!algorithmSetMatchesRegistered(proof.required_algorithms)) return refuse('algorithm_set_invalid');
    // 3. Exactly one well-formed signature per required algorithm, no duplicate,
    //    no algorithm outside the registered set.
    if (!Array.isArray(proof.signatures) || proof.signatures.length === 0) return refuse('signature_set_invalid');
    ...
    for (const s of proof.signatures) {
      ...
      if (presented.has(s.alg)) return refuse('signature_set_invalid');
      presented.add(s.alg);
    }
    for (const alg of presented) {
      if (!(RISK_HYBRID_REQUIRED_ALGORITHMS as readonly string[]).includes(alg)) return refuse('signature_set_invalid');
    }
    for (const alg of RISK_HYBRID_REQUIRED_ALGORITHMS) {
      if (!presented.has(alg)) return refuse('signature_set_incomplete');
    }
    ...
    if (edBytes.length === 0 || edBytes.toString('base64url') !== pin.public_key) return refuse('pinned_key_invalid');
    const edKey = crypto.createPublicKey({ key: edBytes, type: 'spki', format: 'der' });
    if (edKey.asymmetricKeyType !== 'ed25519') return refuse('pinned_key_invalid');
