 ...
 * @returns {Uint8Array}
 */
function bytes(value, label, length: number | null = null) {
...
export function deriveZkRangeBases(domain = ZK_RANGE_RECEIPT_VERSION) {
  if (typeof domain !== 'string' || domain.length === 0) throw new TypeError('ZK range domain is required');
  ...
  if (Buffer.from(valBase).equals(Buffer.from(randBase))) throw new Error('ZK range bases unexpectedly collide');
...
export async function mintZkRangeReceipt({
  ...
  domain = ZK_RANGE_RECEIPT_VERSION,
  backend = null,
}: {
  value?: number;
  max?: number;
  ...
  nonce?: string;
  domain?: string;
  backend?: { batchRangeProof?: Function; batchVerifyProof?: Function } | null;
} = {}) {
  const hiddenValue = safeU64(value, 'value');
  const upperBound = safeU64(max, 'max');
  if (hiddenValue > upperBound) throw new TypeError('value exceeds the public range bound');
  if (typeof actionPredicate !== 'string' || actionPredicate.length === 0) throw new TypeError('actionPredicate is required');
  if (typeof issuerPublicKey !== 'string' || issuerPublicKey.length === 0) throw new TypeError('issuerPublicKey is required');
  ...
  if (!result || !(result.proof instanceof Uint8Array) || !Array.isArray(result.comms) || result.comms.length !== 2) throw new Error('ZK range backend returned a malformed proof');
  const first = bytes(result.comms[0], 'first commitment', 32);
  const second = bytes(result.comms[1], 'second commitment', 32);
  if (!equalBytes(second, linkedSecondCommitment(upperBound, first, bases.valBase))) throw new Error('ZK range backend returned unlinked commitments');
...
export async function verifyZkRangeReceipt(receipt, { backend = null } = {}) {
  try {
    if (!receipt || receipt['@version'] !== ZK_RANGE_RECEIPT_VERSION || receipt.scheme !== ZK_RANGE_SCHEME) return { ok: false, reason: 'malformed_zk_range_receipt' };
    const statement = receipt.statement;
    const max = safeU64(statement?.max, 'statement.max');
    const numBits = rangeBits(max);
    if (statement.num_bits !== numBits) return { ok: false, reason: 'range_bits_mismatch' };
    publicStatement({
    ...
    if (!Array.isArray(commitments) || commitments.length !== 2) return { ok: false, reason: 'commitments_malformed' };
    if (!equalBytes(commitments[1], linkedSecondCommitment(max, commitments[0], bases.valBase))) return { ok: false, reason: 'commitment_relation_invalid' };
    ...
    if (expectedBinding !== receipt.binding) return { ok: false, reason: 'zk_binding_invalid' };
    const engine = requireBackend(backend || await loadBulletproofBackend());
