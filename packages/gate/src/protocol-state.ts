 ...
 */
export type DbHandshakeStatus = 'initiated' | 'pending_verification' | 'verified' | 'rejected' | 'expired' | 'revoked';

/** Pre-initiation. No persisted handshake exists yet. */
export type None = { readonly tag: 'none'; readonly dbStatus: null };
/** A handshake row was created (T1). DB status 'initiated'. */
export type Initiated = { readonly tag: 'initiated'; readonly dbStatus: 'initiated' };
/** A presentation was added and verification is pending (T2). DB 'pending_verification'. */
export type PendingVerification = { readonly tag: 'pending_verification'; readonly dbStatus: 'pending_verification' };
/** Verification accepted (T3). DB 'verified'. Only state from which consume is legal. */
export type Verified = { readonly tag: 'verified'; readonly dbStatus: 'verified' };
/** Verification rejected (T4). Terminal. DB 'rejected'. */
export type Rejected = { readonly tag: 'rejected'; readonly dbStatus: 'rejected' };
/** Binding expired (T7). Terminal. DB 'expired'. */
export type Expired = { readonly tag: 'expired'; readonly dbStatus: 'expired' };
/** Revoked by a party (T6). Terminal. DB 'revoked'. */
export type Revoked = { readonly tag: 'revoked'; readonly dbStatus: 'revoked' };
/**
 ...
 */
export type Consumed = { readonly tag: 'consumed'; readonly dbStatus: 'verified' };

/** Any state in the lifecycle. */
export type ProtocolState = None | Initiated | PendingVerification | Verified | Rejected | Expired | Revoked | Consumed;
/** States a still-active handshake can be revoked or expired from (T6/T7 domain). */
export type Active = Initiated | PendingVerification | Verified;

/** T0: genesis — the empty pre-initiation state. */
export function genesis(): None {
  return { tag: 'none', dbStatus: null };
}

/** T1 Initiate: none -> initiated. Maps to create.js status:'initiated'. */
export function initiate(_s: None): Initiated {
  return { tag: 'initiated', dbStatus: 'initiated' };
}

/** T2 Present: initiated -> pending_verification. Maps to present.js. */
export function present(_s: Initiated): PendingVerification {
...
export function verifyAccept(_s: PendingVerification): Verified {
  return { tag: 'verified', dbStatus: 'verified' };
}

/** T4 VerifyReject: pending_verification -> rejected (terminal). verify.js outcome='rejected'. */
export function verifyReject(_s: PendingVerification): Rejected {
 ...
 */
export function consume(_s: Verified): Consumed {
 ...
 */
export function revoke(_s: Active): Revoked {
 ...
 */
export function expire(_s: Active): Expired {
