...
export const RELIANCE_COMPILATION_RECORD_VERSION =
  'EP-RELIANCE-PROGRAM-COMPILATION-RECORD-v1';
export const RELIANCE_COMPILER_PROFILE =
  'EP-RELIANCE-PROGRAM-COMPILER-v1';
...
export const RELIANCE_COMPILATION_CLAIM_BOUNDARY =
  'This record identifies one deterministic mapping from a verified Reliance Program source to a Gate Trust Program. It does not establish policy truth, legal sufficiency, authorization, provider entry, execution, or outcome.';
...
function unsignedRecord(record: JsonRecord): JsonRecord {
  const { record_digest: _recordDigest, ...body } = record;
  return body;
}

function validateCompiled(compiled: unknown): asserts compiled is CompiledRelianceProgram {
  ...
  const expectedTrace = new Map<string, string>();
  for (const stage of compiled.program.stages) {
    for (const requirement of stage.requirements) {
      expectedTrace.set(
        `${stage.stage_id}\0${requirement.requirement_id}`,
...
function buildRecordBody(compiled: CompiledRelianceProgram): Omit<RelianceCompilationRecord, 'record_digest'> {
  const requirementCount = compiled.program.stages.reduce(
    (total: number, stage: JsonRecord) => total + stage.requirements.length,
    0,
  );
  return {
 ...
 */
export function createRelianceProgramCompilationRecord(
  compiled: unknown,
): RelianceCompilationRecord {
...
function validateRecordShape(record: unknown): asserts record is RelianceCompilationRecord {
 ...
 */
export function verifyRelianceProgramCompilationRecord(
  record: unknown,
  compiled: unknown,
): JsonRecord {
  try {
...
export function renderRelianceProgramCompilationRecord(record: unknown): string {
  validateRecordShape(record);
  const computedDigest = digest(unsignedRecord(record));
  if (computedDigest !== record.record_digest) {
  ...
  for (const entry of record.trace) {
    lines.push(
      `- \`${entry.stage_id}/${entry.requirement_id}\` pins \`${entry.profile_id}\` at \`${entry.profile_hash}\``,
    );
  }
