...
export const NETWORK_WITNESS_V2_REQUIRED_ALGORITHMS = Object.freeze(['Ed25519', 'ML-DSA-65'] as const);
...
function validateBody(body, expectedVersion = NETWORK_WITNESS_VERSION) {
  ...
  if (body['@version'] !== expectedVersion) return 'version_invalid';
...
function acceptanceResult(verified, {
  ...
  acceptanceVersion = NETWORK_WITNESS_ACCEPTANCE_VERSION,
}) {
  return Object.freeze({
    ...verified,
...
// ===========================================================================
// EP-GATE-NETWORK-WITNESS-v2 -- the hybrid (Ed25519 + ML-DSA-65) witness
// ===========================================================================
//
// The hybrid application of the reference migration documented under
// "PATTERN: the reference hybrid migration" (EP-REVOCATION-v2) in
// docs/protocol/pq-hybrid-program.md. ADDITIVE: signNetworkWitnessStatement /
// verifyNetworkWitnessStatement above are untouched and keep the flat single
// Ed25519 `signature`. The v2 profile carries a set-committed hybrid `proof`
// under a distinct -v2 marker, so a deployed v1 verifier handed a v2 statement
// refuses on its shape/version check before inspecting any signature.

type NetworkWitnessV2Checks = {
  shape: boolean;
  pin: boolean;
 ...
 */
function signingBytesV2(
  body,
  requiredAlgorithms: readonly string[] = NETWORK_WITNESS_V2_REQUIRED_ALGORITHMS,
) {
  ...
  return Buffer.from(
    NETWORK_WITNESS_V2_DOMAIN + canonicalize({ required_algorithms: [...requiredAlgorithms], body }),
    'utf8',
  );
}
...
function witnessAgilityPassthrough(options: NetworkWitnessV2VerifyOptions): AgilityOptions {
 ...
 */
export async function signNetworkWitnessStatementV2(
  input,
  keys: NetworkWitnessV2Keys,
  options: AgilityOptions = {},
) {
  if (!keys || !keys.edPrivateKey) throw new TypeError('edPrivateKey is required');
  if (keys.pqPrivateKey === undefined || keys.pqPrivateKey === null) throw new TypeError('pqPrivateKey is required');
  ...
  const signatures = await signAgileSet(new Uint8Array(signingBytesV2(body)), [
    { alg: 'Ed25519', private_key: keys.edPrivateKey as any, key_id: keyId },
 ...
 */
export async function verifyNetworkWitnessStatementV2(
  statement: any,
  options: NetworkWitnessV2VerifyOptions = {},
): Promise<any> {
  const fail = (reason: string, checks: Partial<NetworkWitnessV2Checks> = {}) => ({
    verified: false,
    accepted: false,
    reason,
    checks: {
  ...
  try {
    ...
    for (const s of proof.signatures) {
      ...
      if (presented.has(s.alg)) return fail('signature_set_invalid', { shape: true, algorithm_set: true });
      presented.add(s.alg);
    }
    for (const alg of presented) {
      if (!(NETWORK_WITNESS_V2_REQUIRED_ALGORITHMS as readonly string[]).includes(alg)) {
        return fail('signature_set_invalid', { shape: true, algorithm_set: true });
      }
    }
    for (const alg of NETWORK_WITNESS_V2_REQUIRED_ALGORITHMS) {
      if (!presented.has(alg)) return fail('signature_set_incomplete', { shape: true, algorithm_set: true });
    }
    const pin = findPin(options.pinnedWitnesses, body.witness);
    ...
    try {
      ...
      if (!edBytes) throw new TypeError('invalid base64url key');
      const edKey = crypto.createPublicKey({ key: edBytes, type: 'spki', format: 'der' });
      if (edKey.asymmetricKeyType !== 'ed25519') throw new TypeError('witness key must be Ed25519');
    } catch {
      return fail('pinned_key_invalid', {
        shape: true, pin: true, algorithm_set: true, legs: true, action_binding: true, freshness: true, config_binding: true,
      });
    ...
    try {
      setResult = await verifyAgileSignatureSet(
        new Uint8Array(signingBytesV2(body)),
        proof.signatures,
        [
...
export async function acceptNetworkWitnessStatementV2(
  statement: any,
  options: NetworkWitnessV2AcceptOptions = {},
) {
