...
function validateSurface(surface) {
  ...
  for (const field of ['surface_id', 'action_family', 'gate_id', 'environment_id']) {
    if (!string(surface[field])) return `surface_${field}_invalid`;
  }
  if (!digest(surface.deployment_profile_hash) || !digest(surface.probe_action_digest)) {
    return 'surface_digest_invalid';
  }
  if (surface.required !== true && surface.required !== false) return 'surface_required_invalid';
...
function validateInventory(inventory) {
  if (!exactKeys(inventory, new Set(['@version', 'inventory_id', 'surfaces']))) return 'inventory_shape_invalid';
  if (inventory['@version'] !== COVERAGE_INVENTORY_VERSION) return 'inventory_version_invalid';
  if (!string(inventory.inventory_id) || !Array.isArray(inventory.surfaces)
      || inventory.surfaces.length === 0 || inventory.surfaces.length > 10_000) return 'inventory_invalid';
  const ids = new Set();
  for (const surface of inventory.surfaces) {
    const invalid = validateSurface(surface);
    if (invalid) return invalid;
    if (ids.has(surface.surface_id)) return 'surface_id_duplicate';
    ids.add(surface.surface_id);
  }
  try { canonicalize(inventory); } catch { return 'inventory_canonicalization_invalid'; }
  return null;
}
...
function validateProbeBody(body, expectedVersion = ENFORCEMENT_PROBE_VERSION) {
  if (!exactKeys(body, new Set(['@version', 'probe', 'test']))) return 'probe_shape_invalid';
  if (body['@version'] !== expectedVersion) return 'probe_version_invalid';
  if (!exactKeys(body.probe, new Set(['id', 'key_id']))) return 'probe_identity_shape_invalid';
  if (!string(body.probe.id) || !string(body.probe.key_id)) return 'probe_identity_invalid';
  ...
  for (const field of ['surface_id', 'gate_id', 'environment_id', 'action_family', 'nonce']) {
    if (!string(body.test[field], 1024)) return `probe_${field}_invalid`;
  }
  if (!digest(body.test.action_digest) || !Number.isFinite(strictInstantMs(body.test.tested_at))) {
    return 'probe_binding_invalid';
  }
  if (!PROBE_RESULTS.includes(body.test.result)) return 'probe_result_invalid';
  if (!Number.isSafeInteger(body.test.response_status)
      || body.test.response_status < 100 || body.test.response_status > 599) return 'probe_status_invalid';
  if (body.test.result === 'blocked_without_receipt' && body.test.response_status !== 428) {
    return 'probe_block_status_invalid';
  }
  try { canonicalize(body); } catch { return 'probe_canonicalization_invalid'; }
  return null;
}
...
export function parseEnforcementProbeStatement(raw, { maxBytes = 64 * 1024 } = {}) {
  ...
  if (!strictJsonGate(raw).ok) return null;
  try {
...
export function verifyEnforcementProbe(
  statement,
  options: {
    pinnedProbes?: any;
    expectedSurface?: any;
  ...
  const refuse = (reason: string): ProbeRefusal => ({ accepted: false, verified: false, reason });
  try {
    if (!exactKeys(statement, new Set(['@version', 'probe', 'test', 'signature']))) return refuse('probe_shape_invalid');
    ...
    if (!pin || !string(pin.public_key, 4096)) return refuse('probe_key_unpinned');
    ...
    if (isPlainObject(expected)) {
      if (body.test.surface_id !== expected.surface_id || body.test.gate_id !== expected.gate_id
          ...
          || body.test.action_digest !== expected.probe_action_digest) return refuse('probe_context_mismatch');
    }
    ...
    if (!Number.isFinite(now) || !Number.isSafeInteger(maxAgeSec) || maxAgeSec < 0
        || !Number.isSafeInteger(maxFutureSkewSec) || maxFutureSkewSec < 0) return refuse('probe_profile_invalid');
    const testedAt = strictInstantMs(body.test.tested_at);
    if (testedAt > now + maxFutureSkewSec * 1000) return refuse('probe_from_future');
    if (now - testedAt > maxAgeSec * 1000) return refuse('probe_stale');
    const computed = probeDigest(body);
    if (computed !== signature.statement_digest) return refuse('probe_digest_mismatch');
    let key;
    try {
      ...
      if (key.asymmetricKeyType !== 'ed25519') throw new TypeError('probe key must be Ed25519');
    } catch { return refuse('probe_pinned_key_invalid'); }
...
export function probeV2Bytes(body, requiredAlgorithms = ENFORCEMENT_PROBE_V2_REQUIRED_ALGORITHMS) {
  ...
  return Buffer.from(
    `${PROBE_V2_DOMAIN}${canonicalize({ ...body, required_algorithms: [...requiredAlgorithms] })}`,
    'utf8',
  );
}

/** Mint a real hybrid probe statement. Throws on issuer misuse (never on caller input; there is none). */
export async function signEnforcementProbeV2(input, signers, options: AgilityOptions = {}) {
  const keyId = input?.key_id;
  if (!string(keyId)) throw new TypeError('signEnforcementProbeV2: key_id is required');
  const body = {
 ...
 */
export async function verifyEnforcementProbeV2(
  statement,
  options: {
    pinnedProbes?: any;
    expectedSurface?: any;
  ...
  const refuse = (reason: string): ProbeRefusal => ({ accepted: false, verified: false, reason });
  try {
    if (!exactKeys(statement, new Set(['@version', 'probe', 'test', 'proof']))) return refuse('probe_shape_invalid');
    ...
    if (!probeV2AlgorithmSetRegistered(proof.required_algorithms)) return refuse('probe_algorithm_set_invalid');
    const signatures = Array.isArray(proof.signatures) ? proof.signatures : null;
    if (!signatures || signatures.length === 0) return refuse('probe_signature_legs_missing');
    const presented = new Set<string>();
    for (const s of signatures) {
      if (!isPlainObject(s) || typeof s.alg !== 'string' || typeof s.sig !== 'string') return refuse('probe_signature_leg_malformed');
      if (presented.has(s.alg)) return refuse('probe_signature_leg_duplicate');
      presented.add(s.alg);
    }
    for (const alg of presented) {
      if (!ENFORCEMENT_PROBE_V2_REQUIRED_ALGORITHMS.includes(alg)) return refuse('probe_signature_leg_unexpected');
    }
    for (const alg of ENFORCEMENT_PROBE_V2_REQUIRED_ALGORITHMS) {
      if (!presented.has(alg)) return refuse('probe_signature_leg_stripped');
    }
    const pin = findProbePin(options.pinnedProbes, body.probe.id, body.probe.key_id);
    if (!pin || !string(pin.public_key, 4096) || !string(pin.pq_public_key, 8192)) return refuse('probe_key_unpinned');
    if (!Array.isArray(pin.surface_ids) || !pin.surface_ids.includes(body.test.surface_id)) {
      return refuse('probe_surface_unpinned');
    }
    const expected = options.expectedSurface;
...
export async function verifyEnforcementProbeAny(statement, options: any = {}): Promise<ProbeVerificationResult> {
...
function snapshotEvidenceList(items) {
  ...
  for (const item of items) {
    try { snapshots.push(canonicalSnapshot(item)); } catch { /* invalid evidence is ignored */ }
  }
  return snapshots;
}
...
export async function evaluateGateCoverage(
  input: { inventory?: any; deployments?: any[]; probes?: any[]; witnesses?: any[] } = {},
  options: {
    now?: number;
  ...
  if (!Number.isFinite(now)) throw new TypeError('coverage now must be finite');

  const lists: Record<string, any[]> = {};
  for (const field of ['deployments', 'probes', 'witnesses']) {
    let value;
    try { value = input[field]; } catch { value = null; }
  ...
  const deployments: { profile: any; evidence: any }[] = [];
  ...
  for (const candidate of deployments) {
    let candidateHash;
    try { candidateHash = deploymentProfileDigest(candidate.profile); } catch { continue; }
    if (!inventory.surfaces.some((surface) => surface.deployment_profile_hash === candidateHash)) continue;
  ...
  for (const statement of probes) {
    ...
    if (fresh.accepted) {
      if (!freshProbes.has(fresh.surface_id)) freshProbes.set(fresh.surface_id, []);
    ...
    if (historical.accepted) {
      if (!historicalProbes.has(historical.surface_id)) historicalProbes.set(historical.surface_id, []);
  ...
  for (const acceptance of trustedWitnessAcceptances) {
    ...
    if (trusted.accepted === true) trustedByDigest.set((trusted as any).statement_digest, trusted);
  }
  for (const statement of witnesses) {
    let previouslyAccepted = null;
    try { previouslyAccepted = trustedByDigest.get(networkWitnessDigest(statement)) ?? null; } catch { /* ingest below */ }
  ...
  const rows: CoverageSurfaceRow[] = [];
  for (const surface of inventory.surfaces) {
    const deployment = deploymentResults.get(deploymentKey(
      surface.gate_id, surface.environment_id, surface.deployment_profile_hash,
    )) ?? null;
