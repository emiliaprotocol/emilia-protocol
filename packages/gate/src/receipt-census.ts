...
const BODY_KEYS = ['@version', 'census_id', 'relying_party_id', 'period', 'program_digest', 'minimum_bucket_count', 'buckets', 'suppressed', 'source_inventory_digest', 'generated_at', 'claim_boundary'] as const;
...
export function createReceiptCensus(input: RiskRecord): RiskRecord {
  ...
  const visible = input.buckets
    ...
    .sort((left: RiskRecord, right: RiskRecord) => bucketKey(left).localeCompare(bucketKey(right)));
...
export function validateReceiptCensus(value: unknown) {
  try {
