 ...
 */
function evaluateFieldOriginBody(
  context: FieldOriginVerificationContext,
  signedBody: RiskRecord,
  artifactDigest: string,
): RiskRecord {
  const fail = (
    reason: string,
    verified = true,
    digest: string | null = artifactDigest,
  ) => riskFreeze({
    accepted: false,
    verified,
    reason,
  ...
  let body: RiskRecord;
  try { body = normalizeBody(signedBody); } catch (error) {
    return fail(
  ...
  for (const path of leafPaths) {
    if (!rules.has(path)) return fail(`field_origin_field_unprofiled:${path}`);
    if (!fields.has(path)) return fail(`field_origin_unknown:${path}`);
  }
  for (const rule of profile.fields) {
    ...
    if (!resolved.found) {
      if (rule.required) return fail(`field_origin_field_missing:${rule.path}`);
      if (field) return fail(`field_origin_field_unobserved:${rule.path}`);
      continue;
    }
    if (!field) return fail(`field_origin_unknown:${rule.path}`);
    ...
    if (field.origin_class === 'derived_via_versioned_transform') {
      if (!field.transform) return fail(`field_origin_transform_required:${rule.path}`);
    ...
    if (field.snapshot.kind === 'mutable_snapshot') {
      ...
      if (ageMs < 0) return fail(`field_origin_snapshot_from_future:${rule.path}`);
...
export const FIELD_ORIGIN_V2_REQUIRED_ALGORITHMS = Object.freeze(['Ed25519', 'ML-DSA-65'] as const);
...
function fieldOriginV2AlgorithmSetRegistered(algorithms: unknown): algorithms is string[] {
...
function fieldOriginPqKeyId(publicKeyRawB64u: unknown): string {
  try {
...
function fieldOriginEdKeyId(publicKeyB64u: unknown): string {
  try {
...
function fieldOriginAgilityPassthrough(opts: any): AgilityOptions {
 ...
 */
export function fieldOriginV2SignedPayload(
  body: RiskRecord,
  requiredAlgorithms: readonly string[] = FIELD_ORIGIN_V2_REQUIRED_ALGORITHMS,
): Buffer {
  ...
  return Buffer.from(
    FIELD_ORIGIN_EVIDENCE_V2_DOMAIN + canonicalize({ ...body, required_algorithms: [...requiredAlgorithms] }),
    'utf8',
  );
}
 ...
 */
export async function signFieldOriginEvidenceV2(
  input: unknown,
  signer: {
    issuer_id: string;
    key_id: string;
  ...
  const { proof: _proof, ...bodyV1 } = v1 as RiskRecord;
  ...
  if (!edId || !pqId) refuse('field_origin_signer_invalid', 'field-origin v2 public key material is invalid');
...
function normalizeFieldOriginV2Context(
  value: unknown,
): { policyContext: FieldOriginVerificationContext; hybridKeys: FieldOriginV2TrustedKeys } | null {
  let context: any;
  try { context = strictJsonClone(value); } catch { return null; }
  if (!riskExact(context, CONTEXT_KEYS)
      || !riskRecord(context.trusted_keys)
  ...
  try {
    ...
    const hybridKeys: FieldOriginV2TrustedKeys = {};
    ...
    for (const [keyId, pin] of entries) {
      if (!riskIdentifier(keyId)
          ...
          || !riskIdentifier((pin as any).issuer_id)
          || typeof (pin as any).public_key !== 'string'
          || fieldOriginEdKeyId((pin as any).public_key) === ''
          || typeof (pin as any).pq_public_key !== 'string'
          || fieldOriginPqKeyId((pin as any).pq_public_key) === '') {
        return null;
      }
      hybridKeys[keyId] = {
        issuer_id: (pin as any).issuer_id,
        public_key: (pin as any).public_key,
        pq_public_key: (pin as any).pq_public_key,
      };
    }
    const policyContext = {
      trusted_keys: {} as TrustedRiskKeys,
 ...
 */
export async function verifyFieldOriginEvidenceV2(
  artifact: unknown,
  rawContext?: FieldOriginV2VerificationContext,
  options: {
    mldsaBackend?: AgilityOptions['mldsaBackend'];
    mldsaBackendLoader?: AgilityOptions['mldsaBackendLoader'];
  } = {},
): Promise<RiskRecord> {
  const fail = (reason: string, verified = false, digest: string | null = null) => riskFreeze({
    accepted: false,
    verified,
    reason,
    artifact_digest: digest,
  ...
  if (!normalized) return fail('field_origin_verification_context_required');
  const { policyContext, hybridKeys } = normalized;

  let snapshot: any;
  try { snapshot = strictJsonClone(artifact); } catch { return fail('field_origin_evidence_invalid'); }
  if (!riskRecord(snapshot) || !riskRecord(snapshot.proof) || !riskRecord(snapshot.issuer)) {
    return fail('field_origin_evidence_invalid');
  }
  const { proof, ...bodyNoProof } = snapshot as RiskRecord;
  ...
  if (!signatures || signatures.length === 0) return fail('field_origin_signature_legs_missing', false, artifactDigest);
  ...
  for (const s of signatures) {
    ...
    if (presented.has(s.alg)) return fail('field_origin_signature_leg_duplicate', false, artifactDigest);
    presented.add(s.alg);
  }
  for (const alg of presented) {
    if (!(FIELD_ORIGIN_V2_REQUIRED_ALGORITHMS as readonly string[]).includes(alg)) {
  ...
  for (const alg of FIELD_ORIGIN_V2_REQUIRED_ALGORITHMS) {
    if (!presented.has(alg)) return fail('field_origin_signature_leg_stripped', false, artifactDigest);
  }

  let bytes: Buffer;
  try { bytes = fieldOriginV2SignedPayload(bodyNoProof, FIELD_ORIGIN_V2_REQUIRED_ALGORITHMS); }
  catch { return fail('field_origin_evidence_invalid', false, artifactDigest); }
 ...
 */
export async function verifyFieldOriginEvidenceAny(
  artifact: unknown,
  rawContext?: any,
  options: any = {},
): Promise<RiskRecord> {
  if (riskRecord(artifact) && (artifact as RiskRecord)['@version'] === FIELD_ORIGIN_EVIDENCE_V2_VERSION) {
    return verifyFieldOriginEvidenceV2(artifact, rawContext, options);
  }
  return verifyFieldOriginEvidence(artifact, rawContext as FieldOriginVerificationContext);
}
