...
type Signer = { issuer_id: string; key_id: string; private_key: crypto.KeyLike };
...
function milliseconds(now: number | (() => number)): number {
  const value = typeof now === 'function' ? now() : now;
  if (!Number.isSafeInteger(value) || value < 0) throw new TypeError('cross-rail clock must return a non-negative safe integer');
  return value;
}

function sortedUniqueIdentifiers(value: unknown): string[] {
...
function normalizeDecisionInput(input: unknown): RiskRecord {
...
export function signHumanInterruptionDecision(input: unknown, signer: Signer): RiskRecord {
...
export function verifyHumanInterruptionDecision(
  artifact: unknown,
  { trusted_keys, now = Date.now, expected }: {
...
type PermitBrokerState = {
  ...
  maxTtlMs: number;
  maxActive: number;
  permits: Map<string, RiskRecord>;
};
const permitBrokers = new WeakMap<object, PermitBrokerState>();

function pruneExpired(state: PermitBrokerState, instant: number): void {
  for (const [handle, artifact] of state.permits) {
    if (riskInstant(artifact.expires_at) <= instant) state.permits.delete(handle);
  }
}

export function createRailEntryPermitBroker({
  signer,
  now = Date.now,
  max_ttl_ms = 30_000,
  max_active_permits = 1_000,
}: {
  signer?: Signer;
  now?: number | (() => number);
  max_ttl_ms?: number;
  ...
  if (key.asymmetricKeyType !== 'ed25519') throw new TypeError('rail-entry permit key must be Ed25519');
  const publicKey = crypto.createPublicKey(key as any).export({ type: 'spki', format: 'der' }).toString('base64url');
...
function mintPermit(broker: object, input: RiskRecord): RiskRecord {
  const state = permitBrokers.get(broker);
  if (!state) throw new TypeError('configured rail-entry permit broker required');
  const instant = milliseconds(state.now);
  pruneExpired(state, instant);
  if (state.permits.size >= state.maxActive) throw new Error('rail_entry_permit_capacity_exceeded');
  ...
  if (!Number.isFinite(notAfter) || notAfter <= instant) throw new Error('rail_entry_permit_parent_expired');
...
function consumePermit(broker: object, handle: string, expected: RiskRecord): RiskRecord {
  const state = permitBrokers.get(broker);
  if (!state || typeof handle !== 'string') return { ok: false, reason: 'rail_entry_permit_unknown' };
  const instant = milliseconds(state.now);
  pruneExpired(state, instant);
  const artifact = state.permits.get(handle);
  if (!artifact) return { ok: false, reason: 'rail_entry_permit_unknown' };
...
type ConnectorState = {
  ...
  actionClass: string;
  actionType: string;
  project: (request: RiskRecord) => unknown;
  resolveCaid: (action: RiskRecord) => unknown;
  invoke: (providerRequest: RiskRecord, context: RiskRecord) => Promise<unknown> | unknown;
};
const connectors = new WeakMap<object, ConnectorState>();

export function createCrossRailConnector({
  connector_id, rail, action_class, action_type, project_request, resolve_caid, invoke,
}: {
  connector_id?: string;
  rail?: string;
  action_class?: string;
...
function projectConnectorRequest(state: ConnectorState, request: unknown): RiskRecord {
  if (!riskRecord(request)) throw new TypeError('cross-rail request must be a strict JSON object');
  ...
  if (typeof caid !== 'string' || !RISK_CAID.test(caid)) throw new TypeError('cross-rail CAID resolution failed');
  return {
    action,
...
function reasonClass(reason: unknown): string {
  if (reason === 'effect_indeterminate' || reason === 'capability_commit_indeterminate') return 'indeterminate';
  if (typeof reason !== 'string') return 'internal';
  if (reason.includes('status') || reason.includes('expired') || reason.includes('current')) return 'status';
  if (reason.includes('provider') || reason.includes('permit')) return 'provider';
  if (reason.includes('scope') || reason.includes('action') || reason.includes('target') || reason.includes('amount')) return 'scope';
  return 'authorization';
}

async function emitObservation(observe: unknown, state: ConnectorState, mode: string, result: RiskRecord, now: number | (() => number)) {
  ...
  try { await observe(event); } catch { /* Observation cannot rewrite an effect outcome. */ }
}
 ...
 */
export async function executeCrossRailAllowance(options: RiskRecord = {}): Promise<RiskRecord> {
  const state = connectors.get(options.connector);
  if (!state) return { ok: false, reason: 'cross_rail_connector_required' };
  if (!permitBrokers.has(options.permit_broker)) return { ok: false, reason: 'rail_entry_permit_broker_required' };
  let projection: RiskRecord;
  try { projection = projectConnectorRequest(state, options.request); }
  catch { return { ok: false, reason: 'cross_rail_projection_invalid' }; }
  ...
  let humanAuthorizationDigest: string | null = null;
  if (decision.mode === 'require_human') {
    ...
    } catch { return { ok: false, reason: 'human_authorization_verification_failed' }; }
  ...
  if (!riskRecord(authorizationReceipt)) return { ok: false, reason: 'cross_rail_authorization_receipt_required' };
  ...
  const permitState: { current: RiskRecord | null } = { current: null };
  ...
  const result = await executeWithGateAllowance({
    ...
    expected: options.expected as any,
    now: options.now ?? Date.now,
    providerEntryGuard: (context: RiskRecord) => {
      ...
      try {
        ...
        return { ok: false, reason: (error as Error)?.message || 'rail_entry_permit_mint_failed' };
      }
    },
    executeAction: async (_verifiedAction: RiskRecord, context: RiskRecord) => {
      const permit = permitState.current;
      if (!permit) throw new Error('rail_entry_permit_missing');
