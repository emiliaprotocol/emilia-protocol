...
import {
  ...
  type BoundedExecutionReportVerificationContext,
...
export const BOUNDED_EXECUTION_ACCEPTANCE_PROFILE_VERSION =
  'EP-BOUNDED-EXECUTION-ACCEPTANCE-PROFILE-v1';
export const BOUNDED_EXECUTION_EVIDENCE_PACK_VERSION =
  'EP-BOUNDED-EXECUTION-EVIDENCE-PACK-v1';
export const BOUNDED_EXECUTION_ACCEPTANCE_CLAIM_BOUNDARY =
  'rp_acceptance_of_gate_recorded_program_outcomes_only_not_legal_compliance_not_external_effect_truth_not_program_safety_not_complete_mediation';
...
const PACK_CONTEXT_KEYS = ['profile_context', 'report_context'] as const;
...
export type BoundedExecutionAcceptanceVerdict =
...
function safeInteger(value: unknown, minimum: number, maximum = MAX_OCCURRENCES): value is number {
  return Number.isSafeInteger(value) && Number(value) >= minimum && Number(value) <= maximum;
}

function canonicalInstant(value: unknown, field: string): string {
  if (typeof value !== 'string') refuse('instant_invalid', `${field} must be canonical RFC 3339`);
...
function normalizeRequiredNodes(value: unknown): RiskRecord[] {
  ...
  const nodes = value.map((entry) => {
    ...
    if (seen.has(entry.node_id)) refuse('required_node_duplicated', 'required node is duplicated');
    seen.add(entry.node_id);
    return {
      node_id: entry.node_id,
      min_terminal_occurrences: entry.min_terminal_occurrences,
      accepted_outcomes: normalizeStringSet(
        entry.accepted_outcomes,
        TERMINAL_OUTCOMES,
        'accepted_outcomes',
      ),
...
function normalizeProfileInput(value: unknown): RiskRecord {
  ...
  return {
    ...
    accepted_program_statuses: normalizeStringSet(
      value.accepted_program_statuses,
      PROGRAM_STATUSES,
      'accepted_program_statuses',
    ),
...
function normalizeProfileBody(
  value: unknown,
  version: string = BOUNDED_EXECUTION_ACCEPTANCE_PROFILE_VERSION,
): RiskRecord {
  ...
  const input: RiskRecord = {};
  for (const key of PROFILE_INPUT_KEYS) input[key] = value[key];
...
function normalizeProfileContext(
  value: unknown,
): BoundedExecutionAcceptanceProfileContext | null {
  let context: unknown;
  ...
  if (!riskExact(context, PROFILE_CONTEXT_KEYS)
      ...
      || !RISK_DIGEST.test(context.expected_program_digest)) return null;
  try { canonicalInstant(context.now, 'now'); } catch { return null; }
  return context as BoundedExecutionAcceptanceProfileContext;
}
...
function normalizeProfileContextV2(
  value: unknown,
): BoundedExecutionAcceptanceProfileContextV2 | null {
  let context: unknown;
  try { context = strictJsonClone(value); } catch { return null; }
  if (!riskExact(context, PROFILE_CONTEXT_KEYS)
      || !riskRecord(context.trusted_keys)
      ...
      || !RISK_DIGEST.test(context.expected_program_digest)) return null;
  try { canonicalInstant(context.now, 'now'); } catch { return null; }
  return context as BoundedExecutionAcceptanceProfileContextV2;
}

export function signBoundedExecutionAcceptanceProfile(
  input: unknown,
...
export function verifyBoundedExecutionAcceptanceProfile(
  artifact: unknown,
  rawContext?: BoundedExecutionAcceptanceProfileContext,
): RiskRecord {
  const fail = (
    reason: string,
    verified = false,
  ...
  let profile: RiskRecord;
  try { profile = normalizeProfileBody(signed.body); } catch (error) {
    return fail(
      error instanceof BoundedExecutionAcceptanceValidationError
        ? error.code : 'profile_schema_invalid',
      true,
  ...
  for (const [condition, reason] of checks) {
    if (condition) return fail(reason, true, signed.artifact_digest);
  }
  const now = Date.parse(context.now);
  if (now < Date.parse(profile.valid_from)) return fail('profile_not_active', true, signed.artifact_digest);
  if (now >= Date.parse(profile.expires_at)) return fail('profile_expired', true, signed.artifact_digest);
  return riskFreeze({
    accepted: true,
    verified: true,
    reason: null,
...
export const BOUNDED_EXECUTION_ACCEPTANCE_PROFILE_V2_VERSION =
  'EP-BOUNDED-EXECUTION-ACCEPTANCE-PROFILE-v2';
...
export async function signBoundedExecutionAcceptanceProfileV2(
  input: unknown,
  signer: BoundedExecutionAcceptanceProfileSignerV2,
  ...
  return signRiskBodyV2(
    BOUNDED_EXECUTION_ACCEPTANCE_PROFILE_V2_VERSION,
    {
 ...
 */
export async function verifyBoundedExecutionAcceptanceProfileV2(
  artifact: unknown,
  rawContext?: BoundedExecutionAcceptanceProfileContextV2,
): Promise<RiskRecord> {
  const fail = (
    reason: string,
    verified = false,
  ...
  let profile: RiskRecord;
  try { profile = normalizeProfileBody(signed.body, BOUNDED_EXECUTION_ACCEPTANCE_PROFILE_V2_VERSION); } catch (error) {
    return fail(
      error instanceof BoundedExecutionAcceptanceValidationError
        ? error.code : 'profile_schema_invalid',
      true,
...
function evaluationFailure(reason: string): RiskRecord {
  return riskFreeze({
    valid: false,
    verdict: null,
    reason,
 ...
 */
function evaluateBoundedExecutionReportBody(
  ...
  let clonedReport: unknown;
  try { clonedReport = strictJsonClone(reportArtifact); } catch {
    return evaluationFailure('report_artifact_invalid');
  }
  if (!riskRecord(clonedReport)) return evaluationFailure('report_artifact_invalid');
  ...
  for (const [condition, reason] of bindingChecks) {
    if (condition) return evaluationFailure(reason);
  }
  const buckets = new Map<string, RiskRecord>();
  let unresolved = 0;
  let reserved = 0;
  ...
  if (indeterminateReasons.length > 0) {
    return riskFreeze({
      valid: true,
      verdict: 'INDETERMINATE' as BoundedExecutionAcceptanceVerdict,
      reason: null,
  ...
  for (const requirement of profile.required_nodes) {
    ...
    for (const terminal of bucket.terminal_recorded_outcomes) {
      ...
      if (requirement.accepted_outcomes.includes(terminal.outcome)) acceptedCount += 1;
  ...
  return riskFreeze({
    ...
    verdict: (reasons.length === 0
      ? 'RECORDED_PROCESS_ACCEPTED'
      : 'RECORDED_PROCESS_NOT_ACCEPTED') as BoundedExecutionAcceptanceVerdict,
    reason: null,
    reasons,
    profile_digest: verifiedProfile.profile_digest,
    report_digest: reportDigest,
...
export function evaluateBoundedExecutionAcceptance(
  profileArtifact: unknown,
  profileContext: BoundedExecutionAcceptanceProfileContext | undefined,
  reportArtifact: unknown,
  reportContext?: BoundedExecutionReportVerificationContext,
): RiskRecord {
...
export function buildBoundedExecutionEvidencePack(input: unknown): RiskRecord {
  if (!riskExact(input, PACK_INPUT_KEYS)) {
    refuse('pack_input_invalid', 'evidence pack input must be closed');
  }
  let profileArtifact: unknown;
...
export function verifyBoundedExecutionEvidencePack(
  artifact: unknown,
  context?: {
  ...
  const fail = (reason: string) => riskFreeze({
    valid: false,
    verdict: null,
    reason,
    reasons: [],
  ...
  if (!riskExact(context, PACK_CONTEXT_KEYS)) return fail('verification_context_required');
  let clonedPack: unknown;
  try { clonedPack = strictJsonClone(artifact); } catch { return fail('package_invalid'); }
  if (!riskRecord(clonedPack)) return fail('package_invalid');
  ...
  if (riskDigest(body) !== statedDigest) return fail('package_digest_mismatch');
  const verifiedProfile = verifyBoundedExecutionAcceptanceProfile(
    pack.profile_artifact,
  ...
  if (evaluation.valid !== true) return fail(evaluation.reason ?? 'report_verification_failed');
