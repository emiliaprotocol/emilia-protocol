...
function normalizedRequiredParties(value) {
  ...
  const result: Array<{ party_id: string; role: string | null }> = [];
...
export function buildActionEscrowEvidencePackage({
  ...
  if (!ACTION_ESCROW_EVIDENCE_STAGES.includes(stage as string)) {
...
export async function verifyActionEscrowEvidencePackage(pkg, {
  ...
  try {
    ...
    const amendmentResults: Array<Record<string, any>> = [];
    ...
    const milestoneEvidenceDigests: string[] = [];
