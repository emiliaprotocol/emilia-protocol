...
export const TRUSTED_CONTEXT_BINDING_V2_REQUIRED_ALGORITHMS = Object.freeze(['Ed25519', 'ML-DSA-65'] as const);
...
function algorithmSetMatchesTrustedContextV2(algorithms: unknown): algorithms is string[] {
 ...
 */
function trustedContextBindingV2SigningInput(
  binding: RecordLike,
  requiredAlgorithms: readonly string[] = TRUSTED_CONTEXT_BINDING_V2_REQUIRED_ALGORITHMS,
): Buffer {
...
export async function signTrustedContextBindingV2(
  input: SignTrustedContextBindingV2Input,
 ...
 */
export async function verifyTrustedContextBindingV2(binding: unknown, options: VerifyTrustedContextBindingV2Options) {
  const refuse = (reason: string) => Object.freeze({ state: 'NOT_VERIFIED' as const, reason, authorizes: false as const });
  if (!isDataRecord(binding) || !exactKeys(binding, BINDING_V2_KEYS)) return refuse('context_binding_invalid');
  ...
  if (binding.nonce !== options.expectedNonce) return refuse('context_binding_nonce_mismatch');
  ...
  if (!Number.isFinite(now) || issued > now || now > expires) return refuse('context_binding_expired');

  const presented = new Set<string>();
  for (const s of binding.proof.signatures as AgileSignature[]) {
    if (!s || typeof s !== 'object' || typeof s.alg !== 'string') return refuse('context_binding_invalid');
    if (presented.has(s.alg)) return refuse('context_binding_duplicate_algorithm');
    presented.add(s.alg);
  }
  for (const alg of TRUSTED_CONTEXT_BINDING_V2_REQUIRED_ALGORITHMS) {
    if (!presented.has(alg)) return refuse(`context_binding_missing_${alg}_signature`);
  }
  for (const alg of presented) {
    if (!(TRUSTED_CONTEXT_BINDING_V2_REQUIRED_ALGORITHMS as readonly string[]).includes(alg)) {
