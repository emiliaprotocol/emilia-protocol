...
export const ADAPTER_MANIFEST_CLAIM_BOUNDARY =
  'signed_adapter_revision_and_receipt_references_not_external_system_behavior_deployment_or_conformance_truth';
...
const EXTERNAL_KEYS = ['name', 'revision', 'digest', 'uri'] as const;
const IMPLEMENTATION_KEYS = ['artifact_digest', 'source_commit'] as const;
const CONFORMANCE_KEYS = ['profile_id', 'profile_revision', 'receipt_digest', 'passed_at'] as const;
const STATUS = new Set(['active', 'withdrawn', 'test_only']);
const COMMIT = /^(?:[0-9a-f]{40}|[0-9a-f]{64})$/;

function canonicalOperations(value: unknown): value is string[] {
...
function validHttpsUri(value: unknown): boolean {
  try {
...
function validateBody(value: unknown): asserts value is RiskRecord {
  if (!riskRecord(value)) throw new TypeError('adapter_manifest_shape_invalid');
...
export function signAdapterManifest(input: RiskRecord, signer: {
  issuer_id: string;
  key_id: string;
...
export function loadAdapterManifestRegistry({
  manifests,
  trusted_keys,
  now = Date.now(),
}: {
  ...
  if (!Number.isFinite(at)) throw new TypeError('adapter registry time is invalid');
  ...
  const resolve = (request: RiskRecord = {}) => {
    const manifest = byId.get(request.adapter_id);
    if (!manifest) return riskFreeze({ ok: false, reason: 'adapter_not_registered', manifest: null });
