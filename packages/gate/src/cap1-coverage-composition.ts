...
import {
  ...
  verifyCoverageReconciliationReportBindingV3,
...
export const CAP1_COVERAGE_COMPOSITION_CLAIM_BOUNDARY =
  'authenticated_cap1_statement_and_supplied_set_commitments_under_pinned_reconciliation_program_not_source_population_completeness_or_honest_enumeration_or_examination_quality';
...
const EXAMINED_SET_RESULT_KEYS = ['verdict', 'verified'] as const;
...
export type Cap1NativeVerifier = (
  document: unknown,
) => Cap1NativeVerificationResult | Promise<Cap1NativeVerificationResult>;
...
export type Cap1ExaminedSetVerifier = (
  document: unknown,
  evidence: unknown,
) => Cap1ExaminedSetVerificationResult | Promise<Cap1ExaminedSetVerificationResult>;
...
function validExaminationProfile(value: unknown): value is RiskRecord {
...
function sameProgram(actual: unknown, expected: RiskRecord): boolean {
...
function validPopulationPin(value: unknown): value is Cap1CoveragePopulationPin {
...
function populationCommitment(pin: Cap1CoveragePopulationPin): RiskRecord {
  return {
...
export function cap1CoverageSubjectDigest(input: Parameters<typeof cap1CoverageSubjectCommitment>[0]) {
  return riskFreeze({
    algorithm: 'SHA-256',
...
function cap1CountsMatch(
  document: RiskRecord,
  systemPin: Cap1CoveragePopulationPin,
  receiptPin: Cap1CoveragePopulationPin,
): boolean {
  if (!Array.isArray(document.strata)) return false;
  const system = document.strata.find(
    (stratum: unknown) => riskRecord(stratum) && stratum.id === systemPin.stratum_id,
  );
  const receipt = document.strata.find(
    (stratum: unknown) => riskRecord(stratum) && stratum.id === receiptPin.stratum_id,
  );
...
function cap1SubjectMatches(
  document: RiskRecord,
  program: RiskRecord,
  claimClassId: string,
  examinationProfile: RiskRecord,
  systemPin: Cap1CoveragePopulationPin,
  receiptPin: Cap1CoveragePopulationPin,
): boolean {
  if (!riskRecord(document.subject)
      || document.subject.kind !== 'coverage-population-set'
      || !riskRecord(document.subject.digest)) return false;
...
function reportPopulationMatches(
  value: unknown,
  expected: Cap1CoveragePopulationPin,
): boolean {
...
function attestationPopulationMatches(
  value: unknown,
  expected: Cap1CoveragePopulationPin,
): boolean {
...
function validNativeResult(value: unknown): value is Cap1NativeVerificationResult {
...
function validExaminedSetResult(
  value: unknown,
  systemPin: Cap1CoveragePopulationPin,
  receiptPin: Cap1CoveragePopulationPin,
): value is Cap1ExaminedSetVerificationResult {
  if (!riskExact(value, EXAMINED_SET_RESULT_KEYS)
      || value.verdict !== 'SATISFIED'
      || !Array.isArray(value.verified)
      || value.verified.length !== 2) return false;
  const expected = [systemPin, receiptPin];
  return expected.every((pin) => value.verified.some((entry: unknown) => (
    riskExact(entry, ['stratum', 'eligible_set_digest', 'examined_set_digest', 'result_bindings'])
 ...
 */
export async function verifyCap1CoverageComposition(
  input: {
  ...
  options: Cap1CoverageCompositionOptions,
) {
  const refuse = (reason: string, cap1DocumentDigest: string | null = null) => riskFreeze({
    accepted: false,
    verified: false,
    reason,
  ...
  let cap1Document: RiskRecord;
  let cap1DocumentDigest: string;
  try {
    cap1Document = riskClone(input.cap1_document) as RiskRecord;
  ...
  let native: Cap1NativeVerificationResult;
  try {
  ...
  let examinedSetVerification: unknown;
  try {
    examinedSetVerification = await options.verify_examined_set_evidence(
      riskClone(cap1Document),
      riskClone(input.examined_set_evidence),
    );
  } catch {
  ...
  if (!validExaminedSetResult(
    examinedSetVerification,
    options.system_of_record,
    options.receipt_population,
  )) {
  ...
  if (!cap1SubjectMatches(
    cap1Document,
    options.expected_program,
    options.claim_class_id,
    options.examination_profile,
    options.system_of_record,
    options.receipt_population,
  )) {
  ...
  const reportBinding = verifyCoverageReconciliationReportBindingV3(
  ...
  const attestation = await verifyCoverageReconciliationAttestationV3(
    input.coverage_attestation,
    {
