...
function actionDigest(action: Record<string, any>): string {
...
export function signExecutionValueAttestation(
  input: Omit<ExecutionValuePayload, 'version'>,
  privateKey: crypto.KeyLike,
): ExecutionValueAttestation {
  const payload = {
    version: EXECUTION_VALUE_ATTESTATION_VERSION,
    ...input,
  } as ExecutionValuePayload;
  ...
  if (!preflight.ok) throw new TypeError(preflight.reason);
...
function validatePayload(
  payload: any,
  version: string = EXECUTION_VALUE_ATTESTATION_VERSION,
  ...
  if (payload.version !== version) return { ok: false, reason: 'execution_value_version_unsupported' };
  if (!DIGEST.test(payload.action_digest)) return { ok: false, reason: 'execution_value_action_digest_invalid' };
  if (!CURRENCY.test(payload.asset_currency) || payload.quote_currency !== 'USD') return { ok: false, reason: 'execution_value_currency_invalid' };
  if (!Number.isSafeInteger(payload.value_minor) || payload.value_minor < 0) return { ok: false, reason: 'execution_value_amount_invalid' };
  if (typeof payload.source !== 'string' || payload.source.length === 0 || payload.source.length > 256) return { ok: false, reason: 'execution_value_source_invalid' };
  if (!KEY_ID.test(payload.key_id)) return { ok: false, reason: 'execution_value_key_id_invalid' };
  const observed = validIso(payload.observed_at);
  const expires = validIso(payload.expires_at);
  if (observed === null || expires === null || expires <= observed) return { ok: false, reason: 'execution_value_window_invalid' };
  return { ok: true };
}

export function verifyExecutionValueAttestation(
  attestation: unknown,
  {
    action,
    trustedKeys,
...
): { ok: boolean; reason: string; payload?: ExecutionValuePayload } {
  if (!attestation || typeof attestation !== 'object' || Array.isArray(attestation)) {
    return { ok: false, reason: 'execution_value_attestation_required' };
  }
  let normalized: any;
  try { normalized = JSON.parse(canonicalize(attestation)); } catch { return { ok: false, reason: 'execution_value_attestation_malformed' }; }
  if (!exactKeys(normalized, ['payload', 'signature'])) return { ok: false, reason: 'execution_value_attestation_malformed' };
  ...
  if (!trustedKeys || typeof trustedKeys !== 'object') return { ok: false, reason: 'execution_value_key_unpinned' };
  const key = trustedKeys[payload.key_id];
  if (typeof key !== 'string' || key.length === 0) return { ok: false, reason: 'execution_value_key_unpinned' };
  ...
  try {
    ...
    signatureOk = crypto.verify(
      ...
      publicKey,
      Buffer.from(normalized.signature.value, 'base64url'),
    );
  } catch { signatureOk = false; }
  if (!signatureOk) return { ok: false, reason: 'execution_value_signature_invalid' };
  if (payload.action_digest !== actionDigest(action)) return { ok: false, reason: 'execution_value_action_mismatch' };
  ...
  if (!Number.isFinite(at) || Number(at) < observed - 1_000 || Number(at) > expires
      || Number(at) - observed > maxAgeMs) return { ok: false, reason: 'execution_value_stale' };
  if (!Number.isSafeInteger(maxValueMinor) || maxValueMinor < 0) return { ok: false, reason: 'execution_value_policy_invalid' };
  if (payload.value_minor > maxValueMinor) return { ok: false, reason: 'execution_value_limit_exceeded' };
...
export const EXECUTION_VALUE_V2_REQUIRED_ALGORITHMS = Object.freeze(['Ed25519', 'ML-DSA-65'] as const);
 ...
 */
function executionValueV2SigningBytes(
  payload: ExecutionValuePayloadV2,
  requiredAlgorithms: readonly string[] = EXECUTION_VALUE_V2_REQUIRED_ALGORITHMS,
): Buffer {
...
export async function signExecutionValueAttestationV2(
  input: Omit<ExecutionValuePayloadV2, 'version'>,
  keys: {
    privateKey: crypto.KeyLike;
...
): Promise<ExecutionValueAttestationV2> {
  const payload = {
    version: EXECUTION_VALUE_ATTESTATION_V2_VERSION,
    ...input,
  } as ExecutionValuePayloadV2;
 ...
 */
export async function verifyExecutionValueAttestationV2(
  attestation: unknown,
  {
    action,
    trustedKeys,
...
): Promise<{ ok: boolean; reason: string; payload?: ExecutionValuePayloadV2 }> {
  if (!attestation || typeof attestation !== 'object' || Array.isArray(attestation)) {
    return { ok: false, reason: 'execution_value_attestation_required' };
  }
  let normalized: any;
  try { normalized = JSON.parse(canonicalize(attestation)); } catch { return { ok: false, reason: 'execution_value_attestation_malformed' }; }
  if (!exactKeys(normalized, ['payload', 'proof'])) return { ok: false, reason: 'execution_value_attestation_malformed' };
  const payload = normalized.payload;
  ...
  for (const sig of proof.signatures) {
    ...
    if (presented.has(sig.alg)) return { ok: false, reason: 'execution_value_proof_duplicate_algorithm' };
    presented.add(sig.alg);
  }
  for (const alg of EXECUTION_VALUE_V2_REQUIRED_ALGORITHMS) {
    if (!presented.has(alg)) return { ok: false, reason: `execution_value_proof_missing_${alg}` };
  }
  for (const alg of presented) {
    if (!(EXECUTION_VALUE_V2_REQUIRED_ALGORITHMS as readonly string[]).includes(alg)) {
...
function directUsdMinor(action: Record<string, any>): number | null {
  if (Number.isSafeInteger(action.amount_minor) && action.amount_minor >= 0) return action.amount_minor;
  if (Number.isSafeInteger(action.amount) && action.amount >= 0
      && action.amount <= Math.floor(Number.MAX_SAFE_INTEGER / 100)) return action.amount * 100;
  return null;
}
 ...
 */
export function createExecutionValueProviderEntryGuard({
  maxValueMinor,
  trustedKeys,
  allowedSources,
  resolveAttestation,
  ...
  if (!Number.isSafeInteger(maxValueMinor) || maxValueMinor < 0) throw new TypeError('maxValueMinor must be a non-negative safe integer');
  ...
  return async (context) => {
    ...
    if (!action) return { ok: false, reason: 'execution_value_action_required', status: 409 };
    const currency = typeof action.currency === 'string' ? action.currency.toUpperCase() : null;
    if (!currency || !CURRENCY.test(currency)) return { ok: false, reason: 'execution_value_currency_required', status: 409 };
    if (currency === 'USD') {
      const valueMinor = directUsdMinor(action);
      if (valueMinor === null) return { ok: false, reason: 'execution_value_amount_required', status: 409 };
      if (valueMinor > maxValueMinor) return { ok: false, reason: 'execution_value_limit_exceeded', status: 409 };
      return {
        ok: true,
        evidence: {
          version: EXECUTION_VALUE_ATTESTATION_VERSION,
    ...
    try { attestation = await resolveAttestation(context); } catch {
    ...
    const verified = verifyExecutionValueAttestation(attestation, {
      action: action as Record<string, any>, trustedKeys, allowedSources, maxValueMinor, maxAgeMs, now,
    });
    return verified.ok
