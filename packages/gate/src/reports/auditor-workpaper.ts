 ...
 */
export function buildAuditWorkpaper(entries: Obj[] = [], {
  ...
  if (!Number.isInteger(sampleSize) || (sampleSize as number) < 1) {
  ...
  const warnings: Obj[] = [];
  const population: Obj[] = []; // guarded decision entries, in supplied (log) order
  let inWindow = 0;
  let outsideWindow = 0;
  let notGuarded = 0;
  let executions = 0;
  ...
  const fullPopulation = (sampleSize as number) >= population.length;
