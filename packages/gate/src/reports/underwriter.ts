...
function sortedObject<V>(map: Map<string, V>): Record<string, V> {
  ...
  for (const k of [...map.keys()].sort()) out[k] = map.get(k) as V;
  return out;
}
 ...
 */
export function buildUnderwriterAttestation(entries: any[] = [], {
  ...
  const warnings: { index: number; seq: number | null; reason: string }[] = [];
  const inScope: { entry: any; t: number }[] = [];
  ...
  let firstAt: { t: number; at: any } | null = null;
  let lastAt: { t: number; at: any } | null = null;
