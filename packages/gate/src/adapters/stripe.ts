...
function boundedRefundOperationId(value: unknown): string {
...
function refundIdempotencyKey(operationId: string): string {
...
const OPS = {
  ...
  'refund.create': {
    ...
    perform: (stripe, p) => stripe.refunds.create(
      { payment_intent: p.payment_intent, amount: p.amount },
      { idempotencyKey: refundIdempotencyKey(p.operation_id) },
    ),
  },
  'bank_account.change': {
