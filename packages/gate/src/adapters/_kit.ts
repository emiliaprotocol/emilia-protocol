...
export function createAdapter({ system, ops }) {
  ...
  async function guard(
    ...
    if (!receipt) {
      ...
      const e = new Error(`EMILIA Gate refused ${system}:${op} — ${refused.authorization.reason}`) as Error & {
        code?: string; status?: any; gate?: any; challenge?: any;
      };
    ...
    if (!out.ok) {
      const e = new Error(`EMILIA Gate refused ${system}:${op} — ${out.authorization.reason}`) as Error & {
        code?: string; status?: any; gate?: any; challenge?: any;
      };
      e.code = 'EMILIA_RECEIPT_REQUIRED';
      e.status = out.status;
