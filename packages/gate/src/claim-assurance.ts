...
export const CLAIM_ASSURANCE_GATE_PRESENTATION_VERSION =
  'EP-CLAIM-ASSURANCE-GATE-PRESENTATION-v1' as const;
export const CLAIM_ASSURANCE_ADMISSIBILITY_VERSION =
...
export type ClaimAssuranceVerdict =
  | 'VERIFIED'
  | 'UNVERIFIED'
  | 'DIVERGED'
  | 'INDETERMINATE';
...
export interface ClaimAssuranceEvidenceVerifierRegistration
  extends ClaimAssuranceVerifierDescriptor {
  verify(
    input: Readonly<ClaimAssuranceEvidenceVerifierInput>,
  ): ClaimAssuranceEvidenceVerifierResult;
}
...
export interface ClaimAssuranceEvaluationRecord {
  ...
  requirement_results: Array<{ reasons: string[] }>;
  evidence_results: Array<{ reasons: string[] }>;
  reasons: string[];
...
export type EvaluateClaimAssurance = (
  claimCase: unknown,
  options: Readonly<ClaimAssuranceEvaluationOptions>,
) => Readonly<ClaimAssuranceEvaluationRecord>;
...
export interface ClaimAssuranceGateVerifierInput {
  pinned_profile: { id?: unknown; profile_hash?: unknown };
  presented: unknown;
  receipt?: unknown;
  selector?: unknown;
...
export type ClaimAssuranceAdmissibilityVerdict =
  | 'admissible'
  | 'missing_evidence'
  | 'stale'
  | 'conflicted'
...
export interface ClaimAssuranceAdmissibilityBlock {
  ...
  admissibility_profile: { id: string; version: '1' };
...
function parsePresentation(value: unknown): ClaimAssuranceGatePresentation {
  const presentation = strictJsonClone(value) as Record<string, unknown>;
  ...
  return presentation as unknown as ClaimAssuranceGatePresentation;
}

function evaluatedAt(clock: () => number): { text: string; milliseconds: number } {
  const supplied = clock();
  if (!Number.isFinite(supplied)) fail('clock must return finite epoch milliseconds');
  const milliseconds = Math.floor(supplied / 1_000) * 1_000;
  const instant = new Date(milliseconds);
  if (!Number.isFinite(instant.getTime())) fail('clock returned an invalid instant');
  return {
...
function allReasons(record: Readonly<ClaimAssuranceEvaluationRecord>): string[] {
  return [...new Set([
    ...record.reasons,
...
function claimVerdictToAdmissibility(
  record: Readonly<ClaimAssuranceEvaluationRecord>,
): ClaimAssuranceAdmissibilityVerdict {
  if (record.verdict === 'VERIFIED') {
    return record.profile_satisfied === true && record.authorizes_action === false
      ? 'admissible'
      : 'unverifiable';
  }
  if (record.verdict === 'DIVERGED') return 'conflicted';
  if (record.verdict === 'UNVERIFIED') return 'unverifiable';
  ...
  if (unavailable) return 'unverifiable';
  if (reasons.includes('EVIDENCE_STALE')) return 'stale';
  if (reasons.some((reason) => [
    'REQUIRED_EVIDENCE_MISSING',
    'INSUFFICIENT_DISTINCT_SOURCES',
  ].includes(reason))) return 'missing_evidence';
  return 'unverifiable';
}

function freezeRegistry(
  registry: ClaimAssuranceEvidenceVerifierRegistration[],
): ClaimAssuranceEvidenceVerifierRegistration[] {
  if (!Array.isArray(registry)) fail('verifierRegistry must be an array');
  ...
  }))) as unknown as ClaimAssuranceEvidenceVerifierRegistration[];
}
 ...
 */
export function createClaimAssuranceAdmissibilityVerifier({
  pinnedProfile,
  pinnedProfileHash,
  ...
  maxCaseAgeSec,
  now = Date.now,
}: ClaimAssuranceAdmissibilityOptions): (
  input: ClaimAssuranceGateVerifierInput,
) => Promise<Readonly<ClaimAssuranceAdmissibilityBlock>> {
  const profile = Object.freeze(strictJsonClone(pinnedProfile)) as ClaimAssuranceProfile;
  const computedProfileHash = `sha256:${hashCanonical(profile)}` as Sha256Digest;
  if (pinnedProfileHash !== computedProfileHash) fail('pinnedProfileHash does not match pinnedProfile');
  ...
  return async function verifyClaimAssuranceForGate(
    input: ClaimAssuranceGateVerifierInput,
  ): Promise<Readonly<ClaimAssuranceAdmissibilityBlock>> {
    ...
    const actionDigest = `sha256:${hashCanonical(input.observed_action)}` as Sha256Digest;
    ...
    return Object.freeze({
      ...
      admissibility_profile: { id: profile.profile_id, version: '1' as const },
...
export {
  ...
  type ClaimAssuranceResultValidation,
  type ClaimAssuranceResultValidationOptions,
