...
export const ACTION_ESCROW_CUSTODIAN_OBSERVATION_V2_VERSION =
  'EP-ACTION-ESCROW-CUSTODIAN-OBSERVATION-v2';
 ...
 */
export function custodianObservationV2Bytes(
  payload,
  ...
  if (!observationV2AlgorithmSetRegistered(requiredAlgorithms)) {
    throw new TypeError(
      'custodianObservationV2Bytes: algorithm set is not the registered EP-ACTION-ESCROW-CUSTODIAN-OBSERVATION-v2 set',
    );
  }
  return Buffer.from(
    `${DOMAIN_V2}${canonicalize({ ...payload, required_algorithms: [...requiredAlgorithms] })}`,
    'utf8',
  );
}
 ...
 */
export async function signActionEscrowCustodianObservationV2(
  fields: Omit<Record<string, any>, '@version'>,
 ...
 */
export async function verifyActionEscrowCustodianStatementV2(
  statement,
  {
    operatorKeys,
    providerId,
  ...
  try {
    ...
    if (!validateObservationV2Payload(payload)) return { valid: false, reason: 'malformed_custodian_observation' };
    ...
    if (!signatures || signatures.length === 0) return { valid: false, reason: 'custodian_signature_legs_missing' };
    ...
    for (const s of signatures) {
      ...
      if (presented.has(s.alg)) return { valid: false, reason: 'custodian_signature_leg_duplicate' };
      presented.add(s.alg);
    }
    ...
    for (const alg of ACTION_ESCROW_CUSTODIAN_OBSERVATION_V2_REQUIRED_ALGORITHMS) {
      if (!presented.has(alg)) return { valid: false, reason: 'custodian_signature_leg_stripped' };
    }
 ...
 * check.
 */
export async function verifyActionEscrowCustodianStatementAny(
  statement,
  options: {
    operatorKeys?: Record<string, any>;
    providerId?: string;
