...
export const BREAKGLASS_V2_REQUIRED_ALGORITHMS = Object.freeze(['Ed25519', 'ML-DSA-65'] as const);

type BreakGlassV2Signer = { kid: string; ed: { privateKey: crypto.KeyObject }; pq: { secretKey: Uint8Array | string } };
...
type BreakGlassV2Policy = { minimum_threshold: number; roster: BreakGlassV2RosterEntry[] };
type BreakGlassV2IssuerKeys = Record<string, { key: string; pq_key: string }>
  | Array<{ kid: string; key: string; pq_key: string }>;
 ...
 */
export async function mintBreakGlassAuthorizationV2(signers: BreakGlassV2Signer[], {
  scope, window: win, reason, incident_ref, threshold,
}: {
  scope?: { action_types: string[] };
  window?: { not_before: number | string; expires_at: number | string };
  ...
  if (exp <= nbf) throw new Error('breakglass v2: window.expires_at must be after window.not_before');
  if (!isNonEmptyString(reason)) throw new Error('breakglass v2: reason is required');
  if (!isNonEmptyString(incident_ref)) throw new Error('breakglass v2: incident_ref is required');
  ...
  const signatures: Array<{ kid: string; signatures: AgileSignature[] }> = [];
  for (const s of signers) {
...
function issuerKeyPairForV2(issuerKeys: BreakGlassV2IssuerKeys | undefined, kid: string): { key: string; pq_key: string } | null {
  if (!issuerKeys) return null;
  if (Array.isArray(issuerKeys)) {
...
/** Roster entry normalized to its verified Ed25519 + ML-DSA-65 public keys. */
type PinnedRosterEntryV2 = {
  kid: string;
  principal_id: string;
  publicKey: crypto.KeyObject;
  pqPublicKeyB64u: string;
  fingerprint: string;
};

function normalizePinnedPolicyV2(policy: unknown, issuerKeys: BreakGlassV2IssuerKeys | undefined):
  { ok: false; reason: string } | { ok: true; minimum_threshold: number; roster: Map<string, PinnedRosterEntryV2> } {
  if (!isPlainObject(policy)) return { ok: false, reason: 'missing_policy' };
  const p = policy as BreakGlassV2Policy;
  if (!Number.isSafeInteger(p.minimum_threshold) || p.minimum_threshold < 2 || !Array.isArray(p.roster)) {
    return { ok: false, reason: 'invalid_policy' };
  }
  try {
    const roster = new Map<string, PinnedRosterEntryV2>();
    for (const entry of p.roster) {
      ...
      const pair: { key: string; pq_key: string } | null = (typeof entryKey === 'string' && entryKey.length > 0
        && typeof entryPqKey === 'string' && entryPqKey.length > 0)
        ? { key: entryKey, pq_key: entryPqKey }
        : issuerKeyPairForV2(issuerKeys, entry.kid);
      if (!pair) return { ok: false, reason: 'invalid_policy' };
...
async function verifyBreakGlassV2Internal(grantJson, {
  policy, issuerKeys, now = Date.now, actionType,
}: {
  ...
  if (!doc || typeof doc !== 'object' || Array.isArray(doc)) return refuse('grant_malformed');
  if (doc['@version'] !== BREAKGLASS_V2_VERSION) return refuse('unsupported_version');
  ...
  if (!isNonEmptyString(p.reason)) return refuse('missing_reason');
  if (!isNonEmptyString(p.incident_ref)) return refuse('missing_incident_ref');
  if (!algorithmSetMatchesRegisteredV2(p.required_algorithms)) return refuse('unsupported_algorithm_set');
  ...
  const signerEntries: PinnedRosterEntryV2[] = [];
  for (const kid of kids) {
    const entry = pinned.roster.get(kid);
    if (!entry) return refuse('signer_not_in_roster', { kid });
    signerEntries.push(entry);
  ...
  for (let i = 0; i < sigs.length; i++) {
    ...
    try {
      setResult = await verifyAgileSignatureSet(
        new Uint8Array(msg),
        s.signatures as AgileSignature[],
        [
    ...
    if (setResult?.verified !== true) {
      ...
      if (reason === 'missing_required_algorithm') return refuse('signer_leg_missing', { kid: s.kid });
...
export async function verifyBreakGlassV2(grantJson, options: {
  ...
  try { return await verifyBreakGlassV2Internal(grantJson, options); } catch { return refuse('grant_malformed'); }
}
...
export async function verifyBreakGlassStatement(grantJson, options: {
  ...
  const doc = typeof grantJson === 'string' ? (() => { try { return JSON.parse(grantJson); } catch { return null; } })() : grantJson;
  if (doc && typeof doc === 'object' && !Array.isArray(doc) && doc['@version'] === BREAKGLASS_V2_VERSION) {
    return verifyBreakGlassV2(grantJson, options as any);
  }
  return verifyBreakGlass(grantJson, options as any);
}
...
export async function runBreakGlassStatement({
  ...
  if (typeof effect !== 'function') throw new Error('runBreakGlassStatement: effect function is required');
  ...
  try {
    ...
    const verification = await verifyBreakGlassStatement(grant, { policy, issuerKeys, actionType, now } as any);
    return { ok: false, reason: verification.reason, verification, consumption: null, evidence: null };
  }

  const verification = await verifyBreakGlassStatement(snapshot, { policy, issuerKeys, actionType, now } as any);
  if (!verification.valid) {
    return { ok: false, reason: verification.reason, verification, consumption: null, evidence: null };
  }
  if (!isSecureConsumptionStore(store)) {
  ...
  try {
    ...
    if (!verifyEvidenceRecord(evidenceRecord, ({
      atomicRequired: evidence.atomicAppend === true,
      expectedEntry: entry,
    }) as any)) throw new Error('malformed evidence acknowledgement');
  } catch {
