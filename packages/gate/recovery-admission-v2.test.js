...
// Generated from recovery-admission-v2.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
//
...
function verificationContext(m, input = localInput()) {
    return {
...
test('a real hybrid capability verifies under both pinned keys', async () => {
    ...
    const verified = await verifyRecoveryCapabilityV2(artifact, verificationContext(m));
...
test('the v1 verifier refuses a v2 capability cleanly on the version marker', async () => {
    ...
    const verified = verifyRecoveryCapability(artifact, v1Context);
...
test('LEG STRIPPING: removing the ML-DSA leg refuses structurally', async () => {
    ...
    const verified = await verifyRecoveryCapabilityV2(stripped, verificationContext(m));
    assert.equal(verified.accepted, false);
});
...
test('SET NARROWING: dropping the PQ leg and narrowing required_algorithms fails', async () => {
    ...
    const verified = await verifyRecoveryCapabilityV2(narrowed, verificationContext(m));
    assert.equal(verified.accepted, false);
});
test('WRONG-LENGTH SIGNATURE: a truncated Ed25519 signature refuses', async () => {
    ...
    const verified = await verifyRecoveryCapabilityV2(tampered, verificationContext(m));
    assert.equal(verified.accepted, false);
});
test('ED448 MASQUERADE: an Ed448 key pinned as the Ed25519 half refuses', async () => {
    ...
    const verified = await verifyRecoveryCapabilityV2(artifact, {
        ...ctx,
        trusted_keys: {
            [m.signer.key_id]: {
                issuer_id: m.signer.issuer_id,
...
test('NO ML-DSA BACKEND is a refusal, never a pass on the classical leg', async () => {
    const m = material();
    const artifact = await signRecoveryCapabilityV2(localInput(), m.signer);
    const verified = await verifyRecoveryCapabilityV2(artifact, { ...verificationContext(m), mldsaBackendLoader: async () => null });
    assert.equal(verified.accepted, false);
});
