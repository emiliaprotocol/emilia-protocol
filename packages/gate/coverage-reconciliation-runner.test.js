...
import { COVERAGE_RECONCILIATION_REPORT_VERSION, COVERAGE_SOURCE_INVENTORY_VERSION, coveragePopulationRoot, runCoverageReconciliation, signCoverageSourceInventory, verifyCoverageReconciliationReportBinding, verifyCoverageSourceInventory, } from './coverage-reconciliation-runner.js';
...
test('reconciles exact CAID/action pairs and signs a report-bound attestation', () => {
    ...
    assert.deepEqual(result.report.findings.receipt_without_effect.map((entry) => entry.record_id), ['gate:receipt:104']);
