...
test('reconciles exact CAID/action pairs and signs a report-bound attestation', () => {
    ...
    assert.equal(result.report.joins.matched + result.report.joins.effect_without_receipt
        + result.report.joins.excluded + result.report.joins.exception
        + result.report.joins.system_indeterminate, result.report.system_of_record.count);
    assert.equal(result.report.joins.matched + result.report.joins.receipted_without_observation
        + result.report.joins.indeterminate, result.report.receipt_population.count);
    assert.deepEqual(result.report.findings.effect_without_receipt.map((entry) => entry.record_id), ['pas:effect:101']);
...
test('demotes excluded and exception records with unresolved rule ids to system_indeterminate', () => {
    ...
    assert.deepEqual(result.report.findings.system_indeterminate.map((entry) => [entry.record_id, entry.reclassification_reason]), [
...
test('classification_rule_id is covered by the signed population root and forbidden off rule-bearing records', () => {
    ...
    assert.notEqual(coveragePopulationRoot('system_of_record', tampered), coveragePopulationRoot('system_of_record', systemRecords()));
    assert.throws(() => coveragePopulationRoot('system_of_record', [{
            record_id: 'pas:effect:100', caid: C('A'), action_digest: D('a'), classification: 'effect',
            classification_rule_id: 'ep:coverage:excluded:out-of-scope-action-class:1',
        }]), /record is invalid/i);
