...
// Generated from action-refusal-statement.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
import assert from 'node:assert/strict';
import crypto from 'node:crypto';
import { readFileSync } from 'node:fs';
import test from 'node:test';
import { ACTION_REFUSAL_CLAIM_BOUNDARY, ACTION_REFUSAL_STATEMENT_VERSION, acceptActionRefusalStatement, actionRefusalStatementDigest, createMemoryActionRefusalReplayStore, signActionRefusalStatement, verifyActionRefusalStatement, } from './action-refusal-statement.js';
...
test('tampering any exact-action or pinned-program binding invalidates the statement', () => {
    const { statement, trusted_keys, expected } = fixture();
    for (const mutate of [
        (value) => { value.action_digest = D('9'); },
        (value) => { value.caid = value.caid.replace(/A$/, 'B'); },
        (value) => { value.program.source_digest = D('9'); },
        (value) => { value.failed_requirement_ids[0] = 'attacker-requirement'; },
        (value) => { value.challenge_digests[0] = D('9'); },
        (value) => { value.semantics.authorization = 'AUTHORIZED'; },
    ]) {
...
test('the deterministic conformance vector verifies and its mutations fail as declared', async () => {
    const vector = JSON.parse(readFileSync(new URL('../../conformance/vectors/action-refusal-statement.v1.json', import.meta.url), 'utf8'));
    ...
    assert.equal((await acceptActionRefusalStatement(positive.statement, replayOptions)).reason, vector.replay_expect.reason);
});
