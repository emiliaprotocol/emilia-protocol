...
// Generated from execution-program-admission-store.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
import assert from 'node:assert/strict';
import crypto, { generateKeyPairSync } from 'node:crypto';
import test from 'node:test';
import { ADMISSION_CURRENTNESS_VERSION, EXECUTION_PROGRAM_REPORT_SNAPSHOT_VERSION, EXECUTION_PROGRAM_STATUS_VERSION, createAdmissionSnapshot, createExecutionProgramAdmissionBinding, createMemoryAdmissionStore, executionProgramReportSnapshotMarker, } from './admission-store.js';
...
function program(version = 1, predecessor = null) {
    return {
...
function admissionInput(node, sequence) {
...
function programStatusObservation(reference, status, sequence, observedAt, expiresAt) {
    return {
    ...
    return createMemoryAdmissionStore({
        ...
        executionProgramStatusOracle: {
            read: async (reference) => {
                ...
                return programStatusObservation(reference, 'ACTIVE', observedAt, new Date(observedAt).toISOString(), new Date(observedAt + 5_400_000).toISOString());
            },
        },
        ...overrides,
    });
...
async function register(reference = store()) {
    ...
    if (!registered.ok)
        throw new Error('program registration failed');
...
test('program-linked admissions cannot bypass the program-aware begin or release paths', async () => {
    ...
    assert.equal(reserved.snapshot.body.resource_reservations.filter((resource) => resource.kind === 'execution_program').length, 1);
...
test('registration uses only the store clock and active program-authorizer pins', async () => {
    ...
    }), { ok: false, reason: 'context_binding_required' });
    const attackerArtifact = signBoundedExecutionProgram(program(), attacker.signer);
    assert.deepEqual(await reference.registerExecutionProgram(attackerArtifact, MATERIAL.context()), { ok: false, reason: 'program_issuer_untrusted' });
    ...
    assert.deepEqual(await inactive.registerExecutionProgram(artifact, MATERIAL.context()), { ok: false, reason: 'program_issuer_untrusted' });
    const late = store({ now: '2026-07-29T21:00:00.000Z' });
    assert.deepEqual(await late.registerExecutionProgram(artifact, MATERIAL.context()), { ok: false, reason: 'program_expired' });
    const registered = await reference.registerExecutionProgram(artifact, MATERIAL.context());
    assert.equal(registered.ok, true);
    if (!registered.ok)
        return;
...
test('signed supersession refuses reserved work and closes the predecessor after release', async () => {
    ...
    if (!reserved.ok)
        return;
    const next = signBoundedExecutionProgram(program(2, fixture.registered.program.program_digest), fixture.material.signer);
...
test('supersession freezes identity and intent bindings and requires version plus one with fresh authorization', async () => {
    ...
    for (const entry of cases) {
        ...
        const result = await fixture.store.supersedeExecutionProgram(artifact, (entry.context ?? MATERIAL.context(2)));
...
test('concurrent occurrence reservations linearize to one winner', async () => {
    ...
    assert.deepEqual([left, right].filter((result) => !result.ok).map((result) => result.reason), ['program_occurrence_exhausted']);
...
test('store-authoritative suspension and revocation are current at reserve and begin', async () => {
    ...
    const reference = store({
        ...
        executionProgramStatusOracle: {
            read: async (programReference) => programStatusObservation(programReference, status, sequence, new Date(now).toISOString(), new Date(now + 60_000).toISOString()),
        },
    });
    const fixture = await register(reference);
    assert.deepEqual(await reference.reserveExecutionProgramAdmission({
...
test('program status currentness fails closed and releases a reserved begin', async () => {
    ...
    const reference = store({
        executionProgramStatusOracle: {
            read: async (programReference) => programStatusObservation(programReference, 'ACTIVE', 1, NOW, stale ? '2026-07-29T19:59:59.999Z' : '2026-07-29T20:01:00.000Z'),
        },
    });
    const fixture = await register(reference);
...
test('program operations fail closed when the store has no status oracle', async () => {
    ...
    const artifact = signBoundedExecutionProgram(program(), MATERIAL.signer);
    assert.deepEqual(await absentAtRegistration.registerExecutionProgram(artifact, MATERIAL.context()), { ok: false, reason: 'program_status_indeterminate' });
    let statusAvailable = true;
    configuration.executionProgramStatusOracle = {
        read: async (reference) => statusAvailable
            ? programStatusObservation(reference, 'ACTIVE', 1, NOW, '2026-07-29T21:00:00.000Z')
            : null,
    };
    const missingAfterRegistration = createMemoryAdmissionStore(configuration);
    const registered = await missingAfterRegistration.registerExecutionProgram(artifact, MATERIAL.context());
    assert.equal(registered.ok, true);
    if (!registered.ok)
        return;
...
test('program status source is constructor-pinned against options mutation', async () => {
    const configuration = {
        ...
        executionProgramVerificationPolicy: MATERIAL.policy,
        executionProgramStatusOracle: {
            read: async (reference) => programStatusObservation(reference, 'ACTIVE', 1, NOW, '2026-07-29T21:00:00.000Z'),
        },
    };
    const reference = createMemoryAdmissionStore(configuration);
    configuration.executionProgramStatusOracle = {
        read: async (statusReference) => programStatusObservation(statusReference, 'REVOKED', 1, NOW, '2026-07-29T21:00:00.000Z'),
    };
    const registered = await reference.registerExecutionProgram(signBoundedExecutionProgram(program(), MATERIAL.signer), MATERIAL.context());
    assert.equal(registered.ok, true);
    if (!registered.ok)
        return;
    const reserved = await reference.reserveExecutionProgramAdmission({
    ...
    const reference = store({
        ...
        executionProgramStatusOracle: {
            read: async (programReference) => {
                const observationValue = programStatusObservation(programReference, 'ACTIVE', 1, new Date(now).toISOString(), new Date(now + 1_000).toISOString());
                now += 2_000;
...
test('program verification callbacks may read the store without deadlocking mutation serialization', async () => {
    ...
    reentrantProgram.nodes[0].action = {
        mode: 'profile',
    ...
    const configuration = {
        ...
        executionProgramStatusOracle: {
            read: async (programReference) => {
                ...
                return programStatusObservation(programReference, 'ACTIVE', 1, NOW, '2026-07-29T21:00:00.000Z');
            },
        },
    ...
    if (!registered.ok)
        return;
    registeredDigest = registered.program.program_digest;
