...
// Generated from remedy-program-adapters-v2.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
//
...
function adapterOptions(authority, evidence, over = {}) {
    return {
        tenantId: TENANT,
...
function disputeInput(evidenceDigest) {
    return {
        dispute: {
            evidence_id: 'dispute-evidence-1',
...
async function hybridFixture() {
    ...
    const evidence = await signRemedyProgramEvidenceV2({ kind: 'dispute', issuer: { authority_id: AUTHORITY_ID, key_id: KEY_ID }, payload: disputePayload() }, issuer.keys);
    const authority = {
...
function mintV1Evidence(privateKey) {
    const unsigned = {
...
async function verifyDisputeWith(evidence, authority, over = {}) {
    const digest = remedyProgramEvidenceDigest(evidence);
    const adapters = createRemedyProgramAdaptersV2(adapterOptions(authority, evidence, over));
...
describe('EP-GATE-REMEDY-EVIDENCE-v2 hybrid evidence envelope', () => {
    it('valid v2 roundtrip: the hybrid adapters accept a correctly signed dispute', async () => {
        ...
        assert.deepEqual(evidence.signature.required_algorithms, [...REMEDY_PROGRAM_EVIDENCE_V2_REQUIRED_ALGORITHMS]);
        assert.deepEqual(evidence.signature.signatures.map((s) => s.alg), [...REMEDY_PROGRAM_EVIDENCE_V2_REQUIRED_ALGORITHMS]);
        const result = await verifyDisputeWith(evidence, authority);
        assert.equal(result.ok, true);
        assert.equal(result.dispute_id, 'dispute-1');
    });
    it('v1-refuses-v2 and v2-refuses-v1: neither factory accepts the other profile', async () => {
        ...
        assert.equal((await v1Adapters.verifyDispute(disputeInput(remedyProgramEvidenceDigest(evidence)))).ok, false);
    ...
    it('an authority pinned WITHOUT the ML-DSA half is rejected at construction, never partial credit', async () => {
        ...
        assert.throws(() => createRemedyProgramAdaptersV2(adapterOptions(classicalOnly, evidence)), /configuration invalid/);
    });
    it('stripped leg: dropping the ML-DSA signature refuses, never a classical-only pass', async () => {
        ...
        const stripped = {
            ...
            signature: {
                ...evidence.signature,
                signatures: evidence.signature.signatures.filter((s) => s.alg !== 'ML-DSA-65'),
            },
        };
    ...
    it('narrowed set: claiming required_algorithms=["Ed25519"] refuses structurally AND cryptographically', async () => {
        ...
        const narrowed = {
            ...
            signature: {
                ...
                signatures: evidence.signature.signatures.filter((s) => s.alg === 'Ed25519'),
            },
        };
    ...
    it('wrong-length signature on the ML-DSA leg refuses, never crashes', async () => {
        ...
        const tampered = {
            ...
            signature: {
                ...evidence.signature,
                signatures: evidence.signature.signatures.map((s) => (s.alg === 'ML-DSA-65' ? { ...s, sig: crypto.randomBytes(16).toString('base64url') } : s)),
            },
        };
    ...
    it('Ed448-masquerade: a non-Ed25519 SPKI as the classical half is rejected at construction', async () => {
        ...
        assert.throws(() => createRemedyProgramAdaptersV2(adapterOptions(masqueradedAuthority, evidence)), /configuration invalid/);
    });
    ...
    it('never throws on hostile evidence', async () => {
        ...
        for (const bad of [null, 42, [], { version: REMEDY_PROGRAM_EVIDENCE_V2_VERSION }]) {
            const adapters = createRemedyProgramAdaptersV2(adapterOptions(authority, bad));
    ...
    it('refuses to issue an envelope without both halves of the key material', async () => {
        const issuer = keyFixture();
        await assert.rejects(signRemedyProgramEvidenceV2({ kind: 'dispute', issuer: { authority_id: AUTHORITY_ID, key_id: KEY_ID }, payload: disputePayload() }, { ed: issuer.keys.ed, pq: { secretKey: issuer.keys.pq.secretKey, publicKey: 'not-a-key' } }), /ML-DSA-65 public key/);
    });
    it('the revocation seam is a ROUTER: a v1 revocation statement still verifies under the v2 adapters', async () => {
        ...
        assert.equal(result.ok, true);
        assert.equal(result.authority_id, 'revoker:compliance');
