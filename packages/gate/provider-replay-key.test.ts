...
test('the positional signature and the object signature agree', () => {
  const positional = deriveProviderReplayKey(
    AUTH,
    'payment.release.1',
    'stripe:live:acct_authorized',
    '1',
    PROVIDER_SLOT_SPECS['stripe.idempotency-key'],
  );
...
test('a completely non-object argument refuses rather than throwing', () => {
  for (const junk of [null, undefined, 42, 'string', [], () => {}, Symbol('x')]) {
    let outcome;
    assert.doesNotThrow(() => { outcome = deriveProviderReplayKey(junk as any); });
...
test('the carriage table has the five carrier rows and every row resolves', () => {
  ...
  for (const row of PROVIDER_CARRIAGE_TABLE) {
    ...
    for (const field of ['slot_length', 'slot_charset', 'retention', 'scope', 'intermediary_rewrite', 'echo', 'mismatch_behaviour'] as const) {
