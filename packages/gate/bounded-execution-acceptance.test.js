...
// Generated from bounded-execution-acceptance.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
...
import { BOUNDED_EXECUTION_ACCEPTANCE_CLAIM_BOUNDARY, BOUNDED_EXECUTION_ACCEPTANCE_PROFILE_VERSION, BOUNDED_EXECUTION_EVIDENCE_PACK_VERSION, buildBoundedExecutionEvidencePack, evaluateBoundedExecutionAcceptance, signBoundedExecutionAcceptanceProfile, verifyBoundedExecutionAcceptanceProfile, verifyBoundedExecutionEvidencePack, } from './bounded-execution-acceptance.js';
...
const VECTOR_PATH = fileURLToPath(new URL('../../conformance/vectors/bounded-execution-report.v1.json', import.meta.url));
const vectors = JSON.parse(readFileSync(VECTOR_PATH, 'utf8'));
const known = vectors.known_answer;
const ACCEPTANCE_VECTOR_PATH = fileURLToPath(new URL('../../conformance/vectors/bounded-execution-acceptance.v1.json', import.meta.url));
const acceptanceVectors = JSON.parse(readFileSync(ACCEPTANCE_VECTOR_PATH, 'utf8'));
function reportHarness(mutate) {
...
test('accepts the recorded process only when the RP-required terminal outcome exists', () => {
    const run = profileHarness();
    const result = evaluateBoundedExecutionAcceptance(run.artifact, run.context, known.report_artifact, known.report_verification_context);
...
test('preserves unresolved or reserved work as indeterminate', () => {
    ...
    const unresolvedResult = evaluateBoundedExecutionAcceptance(run.artifact, run.context, unresolved.artifact, unresolved.context);
    ...
    const reservedResult = evaluateBoundedExecutionAcceptance(run.artifact, run.context, reserved.artifact, reserved.context);
...
test('a disallowed terminal outcome cannot be hidden by an accepted one', () => {
    ...
    const result = evaluateBoundedExecutionAcceptance(run.artifact, run.context, report.artifact, report.context);
    assert.equal(result.verdict, 'RECORDED_PROCESS_NOT_ACCEPTED');
...
test('pack verification refuses tampering, substituted programs, and untrusted profile keys', () => {
    ...
    const mismatch = evaluateBoundedExecutionAcceptance(substituted.artifact, substituted.context, known.report_artifact, known.report_verification_context);
...
test('replays the deterministic portable evidence-pack known answer', () => {
    ...
    const verified = verifyBoundedExecutionAcceptanceProfile(vector.profile_artifact, vector.profile_verification_context);
    assert.equal(verified.accepted, true);
    assert.equal(verified.profile_digest, vector.profile_digest);
    const evaluation = evaluateBoundedExecutionAcceptance(vector.profile_artifact, vector.profile_verification_context, vector.report_artifact, vector.report_verification_context);
