...
// Generated from trusted-context-v2.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
//
...
import { TRUSTED_CONTEXT_BINDING_V2_VERSION, canonicalContextRecordDigest, signTrustedContextBindingV2, trustedContextActionSubjectDigest, verifyTrustedContextBindingV2, } from './trusted-context.js';
...
async function buildBinding(m, overrides = {}) {
...
test('the v2 verifier refuses a stale @version marker cleanly', async () => {
    const m = material();
    const binding = structuredClone(await buildBinding(m));
...
test('LEG STRIPPING: removing the ML-DSA leg refuses structurally', async () => {
    const m = material();
    const binding = structuredClone(await buildBinding(m));
    binding.proof.signatures = binding.proof.signatures.filter((s) => s.alg === 'Ed25519');
    const result = await verifyTrustedContextBindingV2(binding, verifyOptions(m));
    assert.equal(result.state, 'NOT_VERIFIED');
...
test('LEG STRIPPING: removing the Ed25519 leg refuses too', async () => {
    const m = material();
    const binding = structuredClone(await buildBinding(m));
    binding.proof.signatures = binding.proof.signatures.filter((s) => s.alg === 'ML-DSA-65');
    const result = await verifyTrustedContextBindingV2(binding, verifyOptions(m));
    assert.equal(result.state, 'NOT_VERIFIED');
});
...
test('WRONG-LENGTH SIGNATURE: a truncated Ed25519 signature refuses', async () => {
    const m = material();
    const binding = structuredClone(await buildBinding(m));
    binding.proof.signatures = binding.proof.signatures.map((s) => (s.alg === 'Ed25519' ? { ...s, sig: s.sig.slice(0, -4) } : s));
    const result = await verifyTrustedContextBindingV2(binding, verifyOptions(m));
    assert.equal(result.state, 'NOT_VERIFIED');
});
