...
// Generated from referee.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
...
function runnerOutput(overrides = {}) {
    return {
...
function runnerPin(script = successfulRunnerScript()) {
    return {
...
function executableDigest(executable) {
    const digest = crypto.createHash('sha256').update(readFileSync(executable)).digest('hex');
    return `sha256:${digest}`;
}
function evaluation(output = runnerOutput(), overrides = {}) {
    return {
...
function successfulRunnerScript(output = runnerOutput()) {
    return [
        "let raw = '';",
...
describe('offline EMILIA Referee core', () => {
    ...
    it('refuses unknown keys at every closed schema boundary', () => {
        assert.throws(() => evaluateReferee({ ...evaluation(), ambient_trust: true }), (error) => error instanceof RefereeValidationError
            && error.code === 'unknown_key');
        assert.throws(() => evaluateReferee({
            ...evaluation(),
            runner_pin: { ...runnerPin(), shell: true },
        }), (error) => error instanceof RefereeValidationError
            && error.code === 'unknown_key');
        assert.throws(() => evaluateReferee({
            ...evaluation(),
            ...
            && error.code === 'unknown_key');
        assert.throws(() => evaluateReferee(evaluation({
            ...runnerOutput(),
            authorized: true,
        })), (error) => error instanceof RefereeValidationError
            && error.code === 'unknown_key');
    ...
    it('refuses accessor-backed schemas and JSON deeper than 64 containers', () => {
        const accessorBacked = evaluation();
        Object.defineProperty(accessorBacked, 'version', {
            enumerable: true,
            get: () => { throw new Error('accessor must not run'); },
        });
        assert.throws(() => evaluateReferee(accessorBacked), (error) => error instanceof RefereeValidationError
        ...
        assert.throws(() => evaluateReferee(evaluation(runnerOutput(), {
    ...
    it('refuses invalid executable pins, lossy JSON, and invalid verification enums', () => {
        ...
        assert.throws(() => evaluateReferee({
            ...evaluation(),
            request: runnerRequest({ input: { omitted: undefined } }),
        }), (error) => error instanceof RefereeValidationError
            && error.code === 'invalid_json');
        assert.throws(() => evaluateReferee(evaluation(runnerOutput({
            native_verification: 'AUTHORIZED',
        }))), (error) => error instanceof RefereeValidationError
            && error.code === 'invalid_native_verification');
        assert.throws(() => evaluateReferee(evaluation(runnerOutput({
            rp_acceptance: 'AUTHORIZED',
        }))), (error) => error instanceof RefereeValidationError
...
describe('pinned JSON subprocess protocol runner', () => {
    ...
    it('refuses unknown invocation and option keys', async () => {
        await assert.rejects(runPinnedProtocolRunner({
            runner_pin: runnerPin(),
            request: runnerRequest(),
            timeout_ms: 1_000,
            env: process.env,
        ...
        await assert.rejects(runPinnedProtocolRunner({
            ...
            request: runnerRequest(),
            timeout_ms: 1_000,
        }, { signal: undefined, shell: true }), (error) => error instanceof RefereeValidationError
            && error.code === 'unknown_key');
    });
