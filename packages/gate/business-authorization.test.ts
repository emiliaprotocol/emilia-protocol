...
test('real crypto: exact business policy, tenant, and approver are recorded and execute', async () => {
  ...
  const out = await gate.run(
    { selector: SELECTOR, receipt: mint(harness), observedAction: ACTION },
    async () => 'executed',
  );
...
for (const scenario of [
  ...
  test(`real crypto: ${scenario[0]} refuses before reservation and execution`, async () => {
    ...
    let effects = 0;
    const out = await gate.run(
      { selector: SELECTOR, receipt: mint(harness, scenario[1]), observedAction: ACTION },
      async () => { effects++; },
    );
...
test('a partial business pin is rejected when the gate is constructed', () => {
  ...
  assert.throws(
    () => createGate({ manifest, allowEphemeralStore: true }),
    /business_authorization_incomplete/,
  );
});
