 ...
 * @param {any} options.gate configured EMILIA Gate
 * @param {(action: object) => string|object} options.resolveCaid synchronous pinned CAID resolver
 * @param {string} options.operationIdField dot-path to the stable operation id in observed action
 * @param {any} [options.certificatePrivateKey] test/demo-only Ed25519 operator key
 * @param {object} [options.certificateSigner] external KMS/HSM signer
 * @param {object} options.certificateContext pinned issuer, tenant, environment, audience, and key id
 * @param {((result: any) => any|Promise<any>)|null} [options.projectResult] pinned disclosure projection
 * @param {number} [options.effectTimeoutMs=30000] provider deadline in milliseconds
 * @param {boolean} [options.allowEphemeralState=false] explicit test/demo opt-in
 ...
 * @param {{trustedCertificateKeys?: Record<string, string>, resolveCaid?: ((action: any) => any)|null, expectedContext?: object|null, certificateEvidence?: any, verifyCertificateInclusion?: (((record: any, expectation: any) => any)|null), requireAtomicCertificateEvidence?: boolean}} [options]
 */
export function verifyReceiptProgramCertificate(certificate, {
  trustedCertificateKeys = {},
  resolveCaid = null,
