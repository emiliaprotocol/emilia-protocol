...
import { createGate, createEvidenceLog } from './index.js';
...
test('CR-1: production + no store -> throws (fail closed)', () => {
  const saved = process.env.NODE_ENV;
  process.env.NODE_ENV = 'production';
  try {
    assert.throws(() => createGate({ trustedKeys: [TRUSTED] }), /no consumption store/);
...
test('CR-2: strict evidence + failing sink -> allow downgraded to refusal', async () => {
  ...
  const gate = createGate({ trustedKeys: [TRUSTED], log: failingLog });
...
test('CR-2: observe-mode (non-strict) sink failure does NOT block', async () => {
  ...
  const gate = createGate({ trustedKeys: [TRUSTED], log: bestEffort });
  const out = await gate.check({ selector: { action_type: 'payment.release' }, receipt: mint({ action: 'payment.release' }) });
  assert.equal(out.allow, true);
});

test('HI-5: receipt without receipt_id -> refused (no hash fallback)', async () => {
  const gate = createGate({ trustedKeys: [TRUSTED] });
