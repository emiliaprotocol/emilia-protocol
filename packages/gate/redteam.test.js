// Generated from redteam.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
/**
...
import { createGate, createEvidenceLog, MemoryConsumptionStore, createDurableConsumptionStore, createMemoryBackend, } from './index.js';
...
test('CR-1: unset NODE_ENV + no store -> throws unless ephemeral use is explicit', () => {
    ...
    try {
        assert.throws(() => createGate({ trustedKeys: [TRUSTED] }), /durable.*ownership-fenced.*permanent/i);
...
test('CR-1: an explicit memory store is refused in production without the opt-in', () => {
    ...
    try {
        assert.throws(() => createGate({ trustedKeys: [TRUSTED], store: new MemoryConsumptionStore() }), /durable.*ownership-fenced.*permanent/i);
...
test('CR-1: durable, ownership-fenced, and permanent are independently mandatory', () => {
    ...
    for (const capability of ['durable', 'ownershipFenced', 'permanentConsumption']) {
        ...
        assert.throws(() => createGate({ trustedKeys: [TRUSTED], store }), /durable.*ownership-fenced.*permanent/i, capability);
    }
    assert.throws(() => createGate({ trustedKeys: [TRUSTED], store: secureStore({ ttlSeconds: 60 }) }), /durable.*ownership-fenced.*permanent/i);
});
