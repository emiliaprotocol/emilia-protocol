...
// Generated from reliance-program-v2.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
//
...
function source(version = RELIANCE_PROGRAM_SOURCE_V2_VERSION) {
    return {
        '@version': version,
...
async function signed() {
    ...
    const envelope = await signRelianceProgramV2(source(), issuer.keys);
...
describe('EP-RELIANCE-PROGRAM-v2 hybrid relying-party source', () => {
    it('valid v2 roundtrip: signs, verifies, and compiles to a v2 Trust Program', async () => {
        ...
        assert.deepEqual(envelope.proof.signatures.map((s) => s.alg), [...RELIANCE_PROGRAM_V2_REQUIRED_ALGORITHMS]);
    ...
    it('v1-refuses-v2: the SYNC v1 verifier refuses structurally, never a crash and never partial credit', async () => {
        ...
        assert.throws(() => compileRelianceProgram(envelope, { trustedKeys: issuer.classicalPin, profiles: [profile()] }), RelianceProgramValidationError);
    });
    ...
    it('stripped leg: dropping the ML-DSA signature refuses, never a classical-only pass', async () => {
        ...
        const stripped = {
            ...
            proof: {
                ...envelope.proof,
                signatures: envelope.proof.signatures.filter((s) => s.alg !== 'ML-DSA-65'),
            },
        };
    ...
    it('narrowed set: claiming required_algorithms=["Ed25519"] is refused structurally AND cryptographically', async () => {
        ...
        const narrowed = {
            ...
            proof: {
                ...
                signatures: envelope.proof.signatures.filter((s) => s.alg === 'Ed25519'),
            },
        };
        assert.equal((await verifyRelianceProgramV2(narrowed, { trustedKeys: issuer.hybridPin })).reason, 'algorithm_set_unsupported');
        ...
        assert.equal((await verifyRelianceProgramV2(setIntact, { trustedKeys: issuer.hybridPin })).reason, 'signature_leg_missing');
    });
    it('wrong-length signature on the ML-DSA leg refuses, never crashes', async () => {
        ...
        const tampered = {
            ...
            proof: {
                ...envelope.proof,
                signatures: envelope.proof.signatures.map((s) => (s.alg === 'ML-DSA-65' ? { ...s, sig: crypto.randomBytes(16).toString('base64url') } : s)),
            },
        };
    ...
    it('a v1-marked source cannot be signed or verified under the v2 profile', async () => {
        const issuer = issuerFixture();
        await assert.rejects(signRelianceProgramV2(source(RELIANCE_PROGRAM_SOURCE_VERSION), issuer.keys), RelianceProgramValidationError);
        const envelope = await signRelianceProgramV2(source(), issuer.keys);
        const downgraded = structuredClone(envelope);
        downgraded.source['@version'] = RELIANCE_PROGRAM_SOURCE_VERSION;
        assert.equal((await verifyRelianceProgramV2(downgraded, { trustedKeys: issuer.hybridPin })).reason, 'source_schema_invalid');
    });
