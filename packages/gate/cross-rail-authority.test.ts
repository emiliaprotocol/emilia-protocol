...
function decisionInput(connectorId: string, projectedAction = action(), request = { amount: 2_500, currency: 'USD', destination: 'acct_known' }, mode = 'standing_policy') {
  return {
...
function issuedAllowance(connectorId: string, keys: ReturnType<typeof signer>, actionType = 'stripe.payout.create') {
...
function executionFixture({ mode = 'standing_policy', rail = 'stripe' } = {}) {
  ...
  const decision = signHumanInterruptionDecision(
    decisionInput(connectorId, projectedAction, { amount: 2_500, currency: 'USD', destination: 'acct_known' }, mode),
    authority.signer,
  );
...
function executeOptions(fixture: ReturnType<typeof executionFixture>, extra: Record<string, any> = {}) {
