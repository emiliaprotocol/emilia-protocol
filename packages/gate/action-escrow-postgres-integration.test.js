...
// Generated from action-escrow-postgres-integration.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
import assert from 'node:assert/strict';
import test from 'node:test';
import { ACTION_ESCROW_PROFILE_VERSION, createActionEscrowKernel, } from './action-escrow.js';
import { ACTION_ESCROW_STATE_SQL, createActionEscrowPostgresStore, } from './action-escrow-postgres.js';
...
test('the production kernel persists and races transitions through the Postgres contract', async () => {
    ...
    const attempts = await Promise.all(Array.from({ length: 32 }, (_, index) => kernel.beginAcceptance(input(`begin-${index}`))));
    assert.equal(attempts.filter((result) => result.code === 'acceptance_requested').length, 1);
