...
import {
  ...
  capabilityV2SignedPayload,
  mintCapabilityReceipt,
  mintCapabilityReceiptV2,
  verifyCapabilityReceipt,
  verifyCapabilityReceiptV2,
  verifyCapabilityReceiptAny,
...
function mintOptions(overrides: any = {}) {
  return {
...
test('a real hybrid capability envelope verifies under both pinned keys', async () => {
  ...
  assert.equal((res as any).issuer_pq_public_key, pqPubB64u);
});
...
test('v1 byte-identity regression: a v1 envelope verifies and re-mints identically', () => {
  ...
  assert.equal(a.capabilityReceipt.capability_signature.value, b.capabilityReceipt.capability_signature.value,
    'v1 Ed25519 signing must stay byte-identical');
...
test('LEG STRIPPING: removing the ML-DSA leg (set intact) refuses structurally', async () => {
  const cr: any = structuredClone((await buildV2()).capabilityReceipt);
  cr.capability_signature.signatures = cr.capability_signature.signatures.filter((s: any) => s.alg === 'Ed25519');
...
test('LEG STRIPPING: removing the Ed25519 leg refuses too', async () => {
  const cr: any = structuredClone((await buildV2()).capabilityReceipt);
  cr.capability_signature.signatures = cr.capability_signature.signatures.filter((s: any) => s.alg === 'ML-DSA-65');
  const res = await verifyCapabilityReceiptV2(cr, { trustedIssuerKeys: PINS });
  assert.equal(res.ok, false);
  assert.equal(res.reason, 'capability_signature_leg_stripped');
});
...
test('ALGORITHM RELABELLING: calling the Ed25519 leg "Ed448" refuses (closed registry)', async () => {
  const cr: any = structuredClone((await buildV2()).capabilityReceipt);
  cr.capability_signature.signatures = cr.capability_signature.signatures.map(
    (s: any) => (s.alg === 'Ed25519' ? { ...s, alg: 'Ed448' } : s),
  );
  const res = await verifyCapabilityReceiptV2(cr, { trustedIssuerKeys: PINS });
  assert.equal(res.ok, false);
...
test('SWAPPED LEGS: the ML-DSA signature relabelled as Ed25519 refuses', async () => {
  const cr: any = structuredClone((await buildV2()).capabilityReceipt);
  const pqLeg = cr.capability_signature.signatures.find((s: any) => s.alg === 'ML-DSA-65');
  cr.capability_signature.signatures = [{ ...pqLeg, alg: 'Ed25519' }, pqLeg];
  const res = await verifyCapabilityReceiptV2(cr, { trustedIssuerKeys: PINS });
  assert.equal(res.ok, false);
  assert.ok(/capability_signature_invalid/.test(res.reason as string), res.reason as string);
});
