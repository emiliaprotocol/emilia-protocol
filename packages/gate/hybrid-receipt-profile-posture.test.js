...
// Generated from hybrid-receipt-profile-posture.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
/**
...
import { CLASSICAL_RECEIPT_PROFILE_ID, DUAL_ISSUANCE_RESULT_ID, HYBRID_PROFILE_REASONS, HYBRID_RECEIPT_PROFILE_ID, acceptUnderHybridProfile, issueUnderHybridProfile, loadHybridIssuanceModule, resolveHybridIssuancePosture, resolveHybridReceiptProfile, } from './dist/hybrid-receipt-profile.js';
...
function verifyV1Only(receipt) {
    ...
    try {
        const ok = crypto.verify(null, Buffer.from(canonicalize(receipt.payload), 'utf8'), crypto.createPublicKey({ key: Buffer.from(CLASSICAL_PUBLIC_SPKI_B64U, 'base64url'), format: 'der', type: 'spki' }), Buffer.from(receipt.signature.value, 'base64url'));
...
function actionDigestOf(payload) {
...
test('an explicit operator setting wins over the resolved default, in BOTH directions', () => {
    ...
    for (const [setting, mode] of [['enabled', 'enabled'], ['dual', 'dual'], ['required', 'required'], [true, 'enabled']]) {
...
test('required semantics are UNCHANGED by custody resolution', async () => {
    ...
    const refusedAccept = await acceptUnderHybridProfile({
        profile,
        receipt: issueClassical({ payload: PAYLOAD }),
...
test('the dual classical twin is BYTE-IDENTICAL to what a non-hybrid deployment produces', async () => {
    ...
    const classicalOnly = off.ok ? off.receipt : null;
    ...
    assert.equal(dual.ok && dual.profile, DUAL_ISSUANCE_RESULT_ID);
    const twin = dual.classical_receipt;
    ...
    assert.equal(crypto.verify(null, recomputed, crypto.createPublicKey({ key: Buffer.from(CLASSICAL_PUBLIC_SPKI_B64U, 'base64url'), format: 'der', type: 'spki' }), Buffer.from(twin.signature.value, 'base64url')), true);
    ...
    assert.equal(canonicalize(again.hybrid_receipt), canonicalize(dual.hybrid_receipt));
});
// ===========================================================================
...
test('a v1-ONLY verifier accepts the classical twin unchanged and refuses the hybrid twin by version', async () => {
    ...
    assert.deepEqual(dual.hybrid_receipt.signatures.map((s) => s.alg), ['Ed25519', 'ML-DSA-65']);
});
// ===========================================================================
...
function makeSignSetSigner(overrides = {}) {
    return {
        async signSet(bytes) {
...
test('dual mints through a custody signer, and both legs are real signatures over identical bytes', async () => {
    ...
    assert.equal(crypto.verify(null, signedBytes, SIGNER_ED.publicKey, Buffer.from(doc.signatures[0].sig, 'base64url')), true);
    assert.equal(ml_dsa65.verify(new Uint8Array(Buffer.from(doc.signatures[1].sig, 'base64url')), new Uint8Array(signedBytes), PQ_PAIR.publicKey), true);
