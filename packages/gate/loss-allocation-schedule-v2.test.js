...
// Generated from loss-allocation-schedule-v2.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
//
...
import { LOSS_ALLOCATION_SCHEDULE_CLAIM_BOUNDARY, LOSS_ALLOCATION_SCHEDULE_V2_VERSION, lossAllocationScheduleDigest, signLossAllocationSchedule, signLossAllocationScheduleV2, verifyLossAllocationSchedule, verifyLossAllocationScheduleV2, } from './loss-allocation-schedule.js';
...
async function verifyContext(m, artifact) {
    return {
...
test('a real hybrid schedule verifies under both pinned keys', async () => {
    const m = material();
    const artifact = await signLossAllocationScheduleV2(schedule(), m.signer);
    assert.equal(artifact['@version'], LOSS_ALLOCATION_SCHEDULE_V2_VERSION);
    const verified = await verifyLossAllocationScheduleV2(artifact, await verifyContext(m, artifact));
...
test('the v1 verifier refuses a v2 schedule cleanly on the version marker', async () => {
    const m = material();
    const artifact = await signLossAllocationScheduleV2(schedule(), m.signer);
    const verified = verifyLossAllocationSchedule(artifact, await verifyContext(m, artifact));
...
test('LEG STRIPPING: removing the ML-DSA leg refuses structurally', async () => {
    ...
    const verified = await verifyLossAllocationScheduleV2(stripped, await verifyContext(m, artifact));
    assert.equal(verified.accepted, false);
});
...
test('SET NARROWING: dropping the PQ leg and narrowing required_algorithms fails', async () => {
    ...
    const verified = await verifyLossAllocationScheduleV2(narrowed, await verifyContext(m, artifact));
    assert.equal(verified.accepted, false);
});
test('WRONG-LENGTH SIGNATURE: a truncated Ed25519 signature refuses', async () => {
    ...
    const verified = await verifyLossAllocationScheduleV2(tampered, await verifyContext(m, artifact));
    assert.equal(verified.accepted, false);
});
test('ED448 MASQUERADE: an Ed448 key pinned as the Ed25519 half refuses', async () => {
    ...
    const verified = await verifyLossAllocationScheduleV2(artifact, {
        ...ctx,
        trusted_keys: {
            [m.signer.key_id]: {
...
test('NO ML-DSA BACKEND is a refusal, never a pass on the classical leg', async () => {
    ...
    const artifact = await signLossAllocationScheduleV2(schedule(), m.signer);
    const ctx = await verifyContext(m, artifact);
    const verified = await verifyLossAllocationScheduleV2(artifact, { ...ctx, mldsaBackendLoader: async () => null });
    assert.equal(verified.accepted, false);
});
