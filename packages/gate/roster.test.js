// Generated from roster.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
/**
...
test('inactive user is carried active:false and their key is never pinned', () => {
    ...
    assert.deepEqual(roster.signers.find((s) => s.principal === 'mallory'), { principal: 'mallory', kid: 'km', publicKey: 'pk_km', active: false });
...
test('apply rejects a non-roster and a registry that cannot revoke', () => {
    ...
    assert.throws(() => applyRosterToRegistry(roster, { add() { }, status() { return []; } }), /add\/revoke\/status/);
