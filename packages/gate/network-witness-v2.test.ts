...
import {
  ...
  NETWORK_WITNESS_V2_DOMAIN,
  NETWORK_WITNESS_V2_REQUIRED_ALGORITHMS,
...
function pin() {
  return {
    witness_id: 'witness:edge-1',
    key_id: undefined as unknown as string,
...
async function buildV2() {
  const stmt: any = await signNetworkWitnessStatementV2(INPUT, KEYS);
  const p = pin();
  p.key_id = stmt.proof.key_id;
  return { stmt, p };
}

function vopts(p: any, extra: Record<string, unknown> = {}) {
...
test('v1 signing is byte-identical and still verifies (regression)', () => {
  const a: any = signNetworkWitnessStatement(INPUT, ed.privateKey);
  const b: any = signNetworkWitnessStatement(INPUT, ed.privateKey);
...
test('acceptNetworkWitnessStatementV2 consumes the sequence exactly once', async () => {
  ...
  const first: any = await acceptNetworkWitnessStatementV2(stmt, options);
  assert.equal(first.accepted, true, first.reason ?? '');
  assert.equal(first.acceptance_version, NETWORK_WITNESS_ACCEPTANCE_V2_VERSION);
  const replay: any = await acceptNetworkWitnessStatementV2(stmt, options);
