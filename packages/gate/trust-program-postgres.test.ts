...
} from './trust-program-postgres.js';
  ...
  const client = {
    async query(text, params = []) {
      ...
      if (text === TRUST_PROGRAM_POSTGRES_SQL.create) {
        ...
        if (records.has(instanceId)) return refusal(instanceId, 'instance_exists');
        const record = {
          instance_id: instanceId,
          revision: 0,
      ...
      if (text === TRUST_PROGRAM_POSTGRES_SQL.compareAndSwap) {
        ...
        if (!current) return refusal(instanceId, 'instance_not_found');
        if (current.revision !== expectedRevision) return refusal(instanceId, 'revision_conflict');
