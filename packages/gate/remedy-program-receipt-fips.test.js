...
// Generated from remedy-program-receipt-fips.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
//
...
describe('EP-ACTION-REMEDY-RECEIPT-v1 opt-in FIPS consult', () => {
    it('with no fipsPosture configured, issuance succeeds exactly as before (byte-identical default)', async () => {
        ...
        const receipt = await issueRemedyProgramReceipt({ state: remedyState(), remedyOperationId: 'refund-op-1' }, { context: CONTEXT, privateKey: keys.privateKey, allowEphemeralState: true });
    ...
    it('a denied FIPS policy refuses issuance BEFORE the signer runs, named reason, never a silent sign', async () => {
        ...
        await assert.rejects(issueRemedyProgramReceipt({ state: remedyState(), remedyOperationId: 'refund-op-1' }, { context: CONTEXT, signer, allowEphemeralState: true, fipsPosture: DENYING_POSTURE }), /fips_policy_denied:ed25519_boundary_undeclared/);
    ...
    it('an inactive FIPS posture (the normal case) permits issuance even when fipsPosture is supplied', async () => {
        ...
        const receipt = await issueRemedyProgramReceipt({ state: remedyState(), remedyOperationId: 'refund-op-1' }, { context: CONTEXT, privateKey: keys.privateKey, allowEphemeralState: true, fipsPosture: inactive });
        assert.equal(receipt.signature.algorithm, 'Ed25519');
    });
});
