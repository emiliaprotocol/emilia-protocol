...
function pins(overrides: Partial<DiscoveryPermitTrustPinsInput> = {}): DiscoveryPermitTrustPinsInput {
  return {
...
function mockTransport(
  ...
  options: {
    ...
    calls?: Array<{ url: string; init: RequestInit; approved: readonly string[] }>;
  } = {},
): AddressPinnedTransport {
  return {
    ...
    async fetchPinned(url, init, context) {
      ...
      if (!reply) throw new Error(`unexpected URL ${url}`);
...
function resolver(
  trustPins: DiscoveryPermitTrustPinsInput,
  transport: AddressPinnedTransport,
  limits: { max_body_bytes?: number; max_json_depth?: number } = {},
) {
...
test('constructor snapshots origin, URLs, digests, max age, redirect map, and transport methods', async () => {
  ...
  const calls: Array<{ url: string; init: RequestInit; approved: readonly string[] }> = [];
  ...
  (transport as any).fetchPinned = async () => {
...
test('executor CAID and action are snapshotted before any asynchronous transport work', async () => {
  const mutableAction: { action_type: string; amount: string; currency: string } = { ...ACTION };
  const trustPins = pins();
  const delegate = mockTransport(happyReplies(trustPins));
  const transport: AddressPinnedTransport = {
...
test('transaction-scoped trust pins are rejected before address resolution or fetch', async () => {
  ...
  await assert.rejects(
    ...
    (error: any) => error?.code === 'transaction_fields_not_allowed',
  );
...
test('address resolution must be public and the transport must attest the connected address', async () => {
  ...
  await assert.rejects(
    () => resolver(pins(), privateTransport).resolve({ caid: CAID, action: ACTION }),
    (error: any) => error?.code === 'address_not_public',
  );
  ...
  await assert.rejects(
    () => resolver(
      pins(),
      mockTransport(replies, {
        addresses: ['93.184.216.34'],
        connectedAddress: '93.184.216.35',
      }),
    ).resolve({ caid: CAID, action: ACTION }),
    (error: any) => error?.code === 'connected_address_not_approved',
  );
});
...
test('only constructor-pinned exact manual redirects are followed', async () => {
  ...
  const result = await instance.resolve({ caid: CAID, action: ACTION });
  assert.deepEqual(
    result.provenance.discovery.redirect_chain,
    [DISCOVERY_URL, REDIRECTED_DISCOVERY_URL],
  );
  ...
  await assert.rejects(
    () => unmapped.resolve({ caid: CAID, action: ACTION }),
    (error: any) => error?.code === 'redirect_not_pinned',
  );
  ...
  await assert.rejects(
    () => wrongTarget.resolve({ caid: CAID, action: ACTION }),
    (error: any) => error?.code === 'redirect_target_mismatch',
  );
});
...
test('body bytes, JSON depth, duplicate names, and content type are bounded before use', async () => {
  ...
  await assert.rejects(
    () => oversized.resolve({ caid: CAID, action: ACTION }),
    (error: any) => error?.code === 'body_too_large',
  );
  ...
  await assert.rejects(
    () => wrongType.resolve({ caid: CAID, action: ACTION }),
    (error: any) => error?.code === 'content_type_invalid',
  );
  ...
  await assert.rejects(
    () => duplicate.resolve({ caid: CAID, action: ACTION }),
    (error: any) => error?.code === 'json_not_strict',
  );
  ...
  await assert.rejects(
    () => deep.resolve({ caid: CAID, action: ACTION }),
    (error: any) => error?.code === 'json_depth_exceeded',
  );
});
...
test('resolver records raw and canonical digests and preserves closed dispositions', async () => {
  for (const [status, issued_at, disposition] of [
    ...
    assert.notEqual(
      result.provenance.discovery.raw_digest,
      result.provenance.discovery.canonical_digest,
    );
  }
});

test('source, schema, mapping, CAID, and action substitutions fail closed', async () => {
  const substitutions: Array<[string, Parameters<typeof bodies>[1]]> = [
  ...
  for (const [code, overrides] of substitutions) {
    ...
    await assert.rejects(
      () => resolver(trustPins, mockTransport({
