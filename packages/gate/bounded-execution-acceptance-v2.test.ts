...
function input(m: ReturnType<typeof material>) {
  return {
...
function context(m: ReturnType<typeof material>) {
  return {
...
test('a real hybrid acceptance profile verifies under both pinned keys', async () => {
  const m = material();
  const artifact: any = await signBoundedExecutionAcceptanceProfileV2(input(m), m.signer);
  ...
  const verified: any = await verifyBoundedExecutionAcceptanceProfileV2(artifact, context(m) as any);
...
test('the v1 verifier refuses a v2 profile cleanly on the version marker', async () => {
  const m = material();
  const artifact: any = await signBoundedExecutionAcceptanceProfileV2(input(m), m.signer);
  const verified: any = verifyBoundedExecutionAcceptanceProfile(artifact, context(m) as any);
...
test('the v1 verifier still accepts a v1 profile, unchanged', () => {
  ...
  const verified: any = verifyBoundedExecutionAcceptanceProfile(artifact, v1Context as any);
  assert.equal(verified.accepted, true, verified.reason ?? '');
});
...
test('LEG STRIPPING: removing the ML-DSA leg refuses structurally', async () => {
  ...
  const verified: any = await verifyBoundedExecutionAcceptanceProfileV2(stripped, context(m) as any);
  assert.equal(verified.accepted, false);
});
...
test('SET NARROWING: dropping the PQ leg and narrowing required_algorithms fails', async () => {
  ...
  const verified: any = await verifyBoundedExecutionAcceptanceProfileV2(narrowed, context(m) as any);
  assert.equal(verified.accepted, false);
});
...
test('WRONG-LENGTH SIGNATURE: a truncated ML-DSA signature refuses', async () => {
  ...
  const verified: any = await verifyBoundedExecutionAcceptanceProfileV2(tampered, context(m) as any);
  assert.equal(verified.accepted, false);
});
...
test('ED448 MASQUERADE: an Ed448 key pinned as the Ed25519 half refuses', async () => {
  ...
  const verified: any = await verifyBoundedExecutionAcceptanceProfileV2(artifact, {
    ...context(m),
    trusted_keys: {
      [m.keyId]: {
