...
import {
  BOUNDED_EXECUTION_REPORT_CLAIM_BOUNDARY,
  BOUNDED_EXECUTION_REPORT_OUTSIDE_PLAN_CLAIM,
...
const REPORT_VECTOR_PATH = fileURLToPath(new URL(
  ...
  return {
    ...
    nodes: [
      ...
      },
      {
        ...
        depends_on: [{ node_id: 'verify', outcomes: ['COMMITTED'] as const }],
        max_occurrences: 1,
        charges: [{ budget_id: 'attempts', amount: 1 }],
      },
      {
        ...
        depends_on: [{ node_id: 'inspect', outcomes: ['COMMITTED'] as const }],
        max_occurrences: 2,
        charges: [{ budget_id: 'attempts', amount: 1 }],
      },
      {
        ...
        depends_on: [{ node_id: 'remediate', outcomes: ['COMMITTED'] as const }],
        max_occurrences: 2,
        charges: [{ budget_id: 'attempts', amount: 1 }],
      },
    ],
...
function occurrence(
  nodeId: string,
  occurrenceId: string,
  state: ExecutionProgramOccurrence['state'],
  minute: number,
): ExecutionProgramOccurrence {
...
function reportSnapshot(
  runtimeState: ExecutionProgramRuntimeState,
  occurrences: ExecutionProgramOccurrence[],
): ExecutionProgramReportSnapshot {
...
function harness() {
  const authorizer = keyMaterial(
    'customer:example-security',
  ...
  const context = {
    ...
    expected_occurrence_inventory_digest:
      boundedExecutionOccurrenceInventoryDigest(occurrences),
...
test('signs a canonical report with deterministic per-node occurrence buckets and budget use', () => {
  ...
  assert.equal(
    artifact.occurrence_inventory_digest,
    boundedExecutionOccurrenceInventoryDigest([...occurrences].reverse()),
  );
  assert.deepEqual(artifact.budget_usage, [{
    budget_id: 'attempts', unit: 'attempt', limit: 10,
    reserved: 1, consumed: 4, remaining: 5,
  }]);
  assert.deepEqual(artifact.node_buckets.map((entry: any) => entry.node_id), [
...
test('requires a fully pinned RP verification context and rejects substitutions', () => {
  ...
  assert.equal(verifyBoundedExecutionReport(artifact, undefined).reason,
    'verification_context_required');
  const substitutions: Array<[string, unknown, string]> = [
...
test('rejects mutation, signature transplant, non-Ed25519 pins, and validly signed unknown fields', () => {
  ...
  assert.equal(verifyBoundedExecutionReport(algorithm, first.context).reason,
    'artifact_signature_envelope_invalid');
  ...
  assert.equal(verifyBoundedExecutionReport(unknown, first.context).reason,
...
test('fails closed on runtime, occurrence, budget, and closed-schema inconsistencies', () => {
  ...
  assert.throws(() => signBoundedExecutionReport({
    ...run.input,
    report_snapshot: reportSnapshot(run.runtimeState, run.occurrences.map((entry, index) => (
      index === 0 ? { ...entry, node_id: 'unknown-node' } : entry
    ))),
  }, {
    relying_party_id: run.reporter.signer.issuer_id,
    key_id: run.reporter.signer.key_id,
  ...
  assert.throws(() => signBoundedExecutionReport({
    ...run.input,
    report_snapshot: reportSnapshot(
      { ...run.runtimeState, total_occurrences: run.runtimeState.total_occurrences + 1 },
      [...run.occurrences, structuredClone(run.occurrences[0])],
    ),
  }, {
    relying_party_id: run.reporter.signer.issuer_id,
    key_id: run.reporter.signer.key_id,
...
test('rejects accessor-bearing hostile data without evaluating the accessor', () => {
  const run = harness();
  let evaluated = false;
  const hostile = structuredClone(run.occurrences[0]) as any;
...
test('binds runtime status and both directions of program supersession', () => {
  ...
  const supersededState: ExecutionProgramRuntimeState = {
    ...run.runtimeState,
    status: 'SUPERSEDED',
    status_sequence: 1,
...
test('replays the same-team deterministic report known answer and hostile vectors', () => {
  ...
  assert.ok(suite.claim_boundary.does_not_establish.some((entry: string) => (
    entry.includes('independent or cross-language conformance')
  )));
  ...
  assert.equal(
    Buffer.from(known.canonical_signed_body_utf8).toString('base64url'),
    known.canonical_signed_body_b64u,
  );
  assert.equal(
    Buffer.from(`${BOUNDED_EXECUTION_REPORT_VERSION}\0${known.canonical_signed_body_utf8}`)
      .toString('base64url'),
  ...
  for (const entry of suite.hostile_mutations.artifact_cases) {
    assert.equal(verifyBoundedExecutionReport(
      entry.artifact,
      known.report_verification_context,
  ...
  for (const entry of suite.hostile_mutations.construction_cases) {
    assert.throws(() => signBoundedExecutionReport({
      ...
      private_key: null,
    }), (error: any) => error?.code === entry.expected_error_code, entry.id);
  }
});
