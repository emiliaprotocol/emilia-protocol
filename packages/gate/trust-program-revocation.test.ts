...
const RECEIPT_CONTEXT: ReceiptContext = Object.freeze({
  issuer: 'emilia-test',
  tenant: 'tenant-a',
  environment: 'test',
...
const OTHER_RECEIPT_CONTEXT: ReceiptContext = Object.freeze({
  ...RECEIPT_CONTEXT,
  tenant: 'tenant-b',
});
...
function derivationInput(
  receiptContext = RECEIPT_CONTEXT,
  binding = authorizationBinding(receiptContext),
) {
  return {
...
test('target derivation refuses malformed, open, prototype-bearing, unsorted, cross-context, and mixed-owner bindings', () => {
  assert.throws(() => deriveTrustProgramRevocationTarget(derivationInput(
    RECEIPT_CONTEXT,
    authorizationBinding(RECEIPT_CONTEXT, { unexpected: true }),
  )), /closed execution authorization binding/);

  const inherited = Object.assign(
    Object.create({ inherited_target: 'attacker-controlled' }),
  ...
  assert.throws(() => deriveTrustProgramRevocationTarget(derivationInput(
    RECEIPT_CONTEXT,
    prototypeNamed,
  )), /prototype-named field|closed execution authorization binding/);

  assert.throws(() => deriveTrustProgramRevocationTarget(derivationInput(
    RECEIPT_CONTEXT,
    authorizationBinding(RECEIPT_CONTEXT, {
  ...
  assert.throws(() => deriveTrustProgramRevocationTarget(derivationInput(
    RECEIPT_CONTEXT,
    authorizationBinding(RECEIPT_CONTEXT, { escrow_profile_digest: DIGEST('6') }),
  )), /exactly one consequence owner/);

  assert.throws(() => deriveTrustProgramRevocationTarget(derivationInput(
    OTHER_RECEIPT_CONTEXT,
    authorizationBinding(RECEIPT_CONTEXT),
  )), /receipt context digest mismatch/);

  assert.throws(() => deriveTrustProgramRevocationTarget({
    ...derivationInput(),
    target_id: 'statement-controlled-target',
  } as any), /closed derivation input/);
});
...
test('verifies a real Ed25519 statement with the actual portable verifier and rejects target or tenant substitution', () => {
  ...
  const targetB = deriveTrustProgramRevocationTarget(
...
async function createReadyKernel(store: any, instanceId: string) {
...
test('a barrier race proves exactly one of claim or revocation invalidation linearizes first', async () => {
  ...
  const store = {
    ...
    async compareAndSwap(input: any) {
      await awaitRace();
