...
// Generated from loss-allocation-schedule.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
...
import { LOSS_ALLOCATION_SCHEDULE_CLAIM_BOUNDARY, LOSS_ALLOCATION_SCHEDULE_VERSION, LossAllocationScheduleValidationError, createLossAllocationAdmissibilityProfilePin, lossAllocationRulesDigest, lossAllocationScheduleDigest, lossAllocationScheduleProfileReference, signLossAllocationSchedule, verifyLossAllocationSchedule, } from './loss-allocation-schedule.js';
import { hashCanonical } from './execution-binding.js';
import { RELIANCE_PROGRAM_SOURCE_VERSION, compileRelianceProgram, signRelianceProgram, } from './reliance-program.js';
...
function profileVerification(artifact, trusted_keys, expectedProgram = program()) {
    return {
        trusted_keys,
...
test('refuses tampering, untrusted issuers, and missing or substituted relying-party/program pins', () => {
    ...
    assert.equal(verifyLossAllocationSchedule(artifact, { ...options, trusted_keys: {} }).reason, 'issuer_untrusted');
...
test('refuses duplicate and conflicting rules keyed by one failure class', () => {
    ...
    assert.equal(validationCode(() => signLossAllocationSchedule(duplicate, signer)), 'duplicate_failure_rule');
    const conflict = schedule();
    conflict.rules.push({ ...structuredClone(conflict.rules[0]), terms_digest: D('f') });
    assert.equal(validationCode(() => signLossAllocationSchedule(conflict, signer)), 'conflicting_failure_rule');
});
test('requires a digest-bound current status and refuses not-yet-valid, stale, and revoked schedules', () => {
    ...
    assert.equal(verifyLossAllocationSchedule(artifact, { ...base, now: NOW }).reason, 'schedule_status_required');
    assert.equal(verifyLossAllocationSchedule(artifact, {
...
test('produces compact and non-circular Admissibility Profile digest pins', () => {
    ...
    assert.equal(createLossAllocationAdmissibilityProfilePin(rebound, {
        ...
        evaluationMaxAgeSec: 300,
    }, profileVerification(rebound, trusted_keys, program(D('7'), D('8')))).profile.profile_hash, profileHash, 'final program digests must not create a profile-pin cycle');
...
test('the generated pin compiles through unchanged Reliance Program v1 before final digest binding', () => {
    ...
    assert.equal(createLossAllocationAdmissibilityProfilePin(finalArtifact, {
        profileId: pin.profile.id,
        evaluationMaxAgeSec: 300,
    }, profileVerification(finalArtifact, trusted_keys, finalProgram)).profile.profile_hash, pin.profile.profile_hash);
...
test('closed schemas, decimal monetary caps, timestamps, and claim boundary fail before signing', () => {
    ...
    assert.equal(validationCode(() => signLossAllocationSchedule(extra, signer)), 'schedule_schema_invalid');
    const numericMoney = schedule();
    numericMoney.rules[0].allocation.max_amount_minor = 25_000_000;
    assert.equal(validationCode(() => signLossAllocationSchedule(numericMoney, signer)), 'failure_rule_invalid');
    const impossibleTime = schedule();
    impossibleTime.expires_at = '2026-02-30T12:00:00.1Z';
    assert.equal(validationCode(() => signLossAllocationSchedule(impossibleTime, signer)), 'schedule_validity_invalid');
    const overstated = schedule();
    overstated.claim_boundary = 'this_moves_money';
    assert.equal(validationCode(() => signLossAllocationSchedule(overstated, signer)), 'claim_boundary_invalid');
    const reversed = schedule();
    reversed.rules.reverse();
    assert.equal(validationCode(() => signLossAllocationSchedule(reversed, signer)), 'rules_not_canonical_order');
});
test('checked-in vector verifies deterministic Ed25519 bytes and named status refusals', () => {
    const vector = JSON.parse(readFileSync(fileURLToPath(new URL('../../conformance/vectors/loss-allocation-schedule.v1.json', import.meta.url)), 'utf8'));
