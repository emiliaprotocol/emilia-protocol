...
function signedBinding(overrides: any = {}) {
...
function expected(binding: any, overrides: any = {}) {
  return {
    agreement_digest: agreementDigest,
...
function verifier(bytes: Buffer = documentBytes) {
...
function contractorVerifier(bytes: Buffer = documentBytes) {
...
test('changed document bytes and agreement aliases fail closed', async () => {
  const binding = signedBinding();
  assert.equal(
    (await verifier(Buffer.from('%PDF-1.7\nchanged\n'))(binding, expected(binding))).valid,
    false,
  );
  assert.equal(
    (await verifier()(binding, expected(binding, {
      agreement_digest: `sha256:${'8'.repeat(64)}`,
    }))).reason,
    'kernel_binding_context_mismatch',
  );
});
...
test('supersession is explicit and mapped to the kernel amendment join', async () => {
  ...
  assert.equal(result.valid, true);
  assert.equal(
    result.supersedes_document_action_binding_digest,
