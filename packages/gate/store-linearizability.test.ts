...
class ReplicatedLinearizableBackend {
  ...
  #writableNode() {
    if (this.unavailable) throw new Error('cluster_unavailable');
  ...
  #write(kind, key, args) {
    ...
    this.nextFault = null;
    if (fault === 'before') throw new Error('connection_lost_before_linearization');
    ...
    if (changed) {
      if (next === undefined) node.state.delete(key);
      else node.state.set(key, next);
      this.log.push({ index: this.log.length + 1, kind, key, before: current, after: next });
      node.applied = this.log.length;
    }
    if (fault === 'after' && changed) throw new Error('connection_lost_after_linearization');
    return changed;
  }

  replicateOne(index) {
    const node = this.nodes[index];
    if (node.applied >= this.log.length) return false;
    const entry = this.log[node.applied];
    if (entry.after === undefined) node.state.delete(entry.key);
    else node.state.set(entry.key, entry.after);
    node.applied += 1;
    return true;
  }
  ...
  attemptPromotion(index) {
    if (this.nodes[index].applied !== this.log.length) return false;
    this.active = index;
    return true;
  }
  ...
  rollbackReplica(index, applied) {
    if (index === this.active) throw new Error('active_rollback_refused');
    const target = Math.max(0, Math.min(applied, this.log.length));
    const state = new Map();
    for (const entry of this.log.slice(0, target)) {
      if (entry.after === undefined) state.delete(entry.key);
      else state.set(entry.key, entry.after);
    }
  ...
  authoritativeState() {
    ...
    for (const entry of this.log) {
      ...
      if (entry.kind === 'add') assert.equal(state.has(entry.key), false, 'add linearized only from absent');
      if (entry.kind === 'cas') assert.equal(state.get(entry.key), entry.before, 'CAS linearized from its expected value');
      if (entry.kind === 'delete') assert.equal(state.get(entry.key), entry.before, 'delete linearized from its expected value');
      if (entry.after === undefined) state.delete(entry.key);
      else state.set(entry.key, entry.after);
...
async function tryReserve(candidate) {
  ...
  try {
    ...
    if (won) candidate.phase = 'reserved';
    return won;
  } catch {
    return false;
  }
...
function assertScheduleInvariants(cluster, workers, effectCount, effectStarted, committedSeen) {
  ...
  if (effectStarted) assert.ok(value !== undefined, 'an attempted effect was reopened by deleting its reservation');
  if (committedSeen) assert.equal(value, COMMITTED, 'committed consumption regressed');
  ...
  for (let step = 0; step < steps; step += 1) {
    ...
    try {
      switch (choice) {
        ...
        case 2:
          if (candidate.phase === 'effect') {
            try {
              ...
              try { await candidate.store.commit(RECEIPT); } catch { /* frozen is safe */ }
        ...
        case 3:
          if (candidate.phase === 'reserved') {
            try { await candidate.store.release(RECEIPT); candidate.phase = 'idle'; } catch { candidate.phase = 'lost'; }
          }
          break;
        case 4:
        ...
        case 7: {
          ...
          if (!promoted) assert.notEqual(cluster.active, target, 'stale promotion changed the active writer');
          break;
        }
        case 8: {
          ...
          await assert.rejects(
            workers[0].store.has(RECEIPT),
            /stale_replica_refused/,
          );
        ...
        case 9: {
          const target = (cluster.active + 1) % cluster.nodes.length;
          cluster.rollbackReplica(target, Math.max(0, cluster.log.length - 1));
          if (cluster.nodes[target].applied !== cluster.log.length) assert.equal(cluster.attemptPromotion(target), false);
          break;
        }
        case 10:
        ...
        case 11:
          ...
          if (candidate.phase === 'effect') {
            try { await candidate.store.commit(RECEIPT); } catch { candidate.phase = 'indeterminate'; }
          } else {
            await tryReserve(candidate);
          }
          break;
