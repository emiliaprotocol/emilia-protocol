...
function createFakePostgres() {
  ...
  const stateFor = (tenant, gate, stream, create = false) => {
    ...
    if (!scopes.has(key) && create) scopes.set(key, { head: null, rows: [] });
  ...
  async function query(text, params = []) {
    ...
    if (controls.failReads && text !== EVIDENCE_SQL.appendIfHead) throw new Error('pg_unavailable');

    if (text === EVIDENCE_SQL.health) return result([clone(controls.health)]);
    ...
    if (text !== EVIDENCE_SQL.appendIfHead) throw new Error(`fake pg: unrecognized query: ${text}`);
    ...
    return withScopeLock(key, async () => {
      ...
      if ((current.head?.hash ?? null) !== expectedHeadHash) return result([{ appended: false }]);
...
test('postgres evidence: constructor, health, head, all, and empty verification are strict', async () => {
  ...
  assert.throws(
    () => createPostgresEvidenceBackend({ query: async () => ({}), tenantId: 'tenant', gateId: '' }),
    /gateId/,
  );
...
test('postgres evidence: two concurrent appenders produce one monotonically increasing chain', async () => {
  ...
  await Promise.all(Array.from({ length: 64 }, (_, index) => (
    (index % 2 === 0 ? first : second).record({ type: 'decision', index })
  )));
...
test('postgres evidence: tenant and gate scopes isolate identical streams and record IDs', async () => {
  ...
  const records = await Promise.all(scopes.map((backend, index) => (
    createAtomicEvidenceLog(backend, {
