...
import {
  ...
  type ConsequenceBoundaryNotEnteredMarker,
...
function durableAebStore(
  ...
  return {
    ...
    async commit(key: string) {
      if (db.operations.get(key) !== 'RESERVED' || !ownsRow(key)) { owned.delete(key); return false; }
...
function attemptStore() {
  const rows = new Map<string, {
    ...
    evidence?: ConsequenceBoundaryProviderEvidence | ConsequenceBoundaryNotEnteredMarker;
  }>();
  return {
    durable: true as const,
    ownershipFenced: true as const,
    ...
    notEnteredMarker: true as const,
    rows,
    ...
    async transition(input: ConsequenceBoundaryAttemptReference & any) {
      ...
      if (input.next_state === 'RELEASED') row.evidence = structuredClone(input.evidence);
      return true;
    },
...
function makeBoundary({
  ...
  attemptPrefix = '',
  providerOutcomeVerify = affirm as null | ((context: any) => unknown),
  ...
  const boundary = createConsequenceBoundary({
    ...
    invoke,
    ...(providerOutcomeVerify ? { provider_outcomes: { verify: providerOutcomeVerify as any } } : {}),
    now: () => NOW,
  } as any);
...
test('INDETERMINATE is closed to replay but can be reconciled through separately authorized custody recovery', async () => {
  ...
  const tamperedBinding = await h.boundary.reconcile({
    ...
    outcome: { state: 'EXECUTED', evidence_kind: 'provider_outcome' as const, evidence: executedEvidence(), result: { id: 'effect:wrong-key' } },
  ...
  const refusedRecovery = await h.boundary.reconcile({
    ...
    outcome: { state: 'EXECUTED', evidence_kind: 'provider_outcome' as const, evidence: executedEvidence(), result: { id: 'effect:recovered' } },
...
test('unknown provider outcome keeps aggregate capacity unavailable and reconciliation never reexecutes', async () => {
  ...
  const reconciled = await h.boundary.reconcile({
    ...
    artifacts: f.artifacts,
    attempt: first.attempt,
    outcome: { state: 'EXECUTED', evidence_kind: 'provider_outcome' as const, evidence: executedEvidence(), result: { id: 'effect:reconciled' } },
    recovery_authorization: 'recovery:approved',
  });
  assert.equal(reconciled.state, 'EXECUTED');
  assert.equal(providerCalls, 1);
...
function makeNativeBoundary({
  ...
  providerOutcomeVerify = affirm as (context: any) => unknown,
  ...
  const configuration = {
    ...
    provider_outcomes: {
      ...
      verify: providerOutcomeVerify as any,
    },
    now,
  };
...
test('direct native authenticated reconciliation closes uncertainty without re-invocation', async () => {
  ...
  const reconciled = await h.boundary.reconcile({
    ...
    attempt: first.attempt,
    outcome: {
      state: 'EXECUTED', evidence_kind: 'provider_outcome' as const,
      evidence: executedEvidence(),
...
test('reconciliation resumes after the first terminal write failed, and nothing is closed before it', async () => {
  ...
  const reconciliation = {
    ...
    attempt: first.attempt,
    outcome: {
      state: 'EXECUTED' as const, evidence_kind: 'provider_outcome' as const,
      evidence: executedEvidence(),
      result: { id: 'recovered' },
    },
    recovery_authorization: 'recovery:approved',
...
test('direct native security callbacks are pinned when the boundary is constructed', async () => {
...

  {
    ...
    (h.configuration.provider_outcomes as any).verify = affirm;
    const result = await h.boundary.run(nativeInput(
      f,
...
test('a fresh native authorization cannot re-enter the provider while the same action is INDETERMINATE', async () => {
  ...
  const reconciled = await h.boundary.reconcile({
    ...
    action: ACTION,
    attempt: first.attempt,
    outcome: { state: 'EXECUTED', evidence_kind: 'provider_outcome' as const, evidence: providerOutcomeFor(1), result: {} },
    recovery_authorization: 'recovery:approved',
  });
  assert.equal(reconciled.state, 'EXECUTED');
  ...
  for (const terminal of ['FAILED', 'EXECUTED'] as const) {
    ...
    const reconciled = await h.boundary.reconcile({
      ...
      outcome: terminal === 'FAILED'
        ? { state: 'FAILED', evidence_kind: 'provider_outcome' as const, evidence: providerOutcomeFor(1), reason: 'provider_declined' }
        : { state: 'EXECUTED', evidence_kind: 'provider_outcome' as const, evidence: providerOutcomeFor(1), result: {} },
      recovery_authorization: 'recovery:approved',
    });
...
test('one exact issuer under two declared namespaces, or mixed declarations, is refused at construction', async () => {
  {
    ...
    assert.throws(
      ...
      /native_consequence_boundary_configuration_invalid: native_pins_issuer_namespace_conflict/,
    );
  }
  {
...
test('reconciling a never-entered RELEASED attempt is refused and cannot burn a later attempt', async () => {
  ...
  const recovery = {
    ...
    outcome: {
      state: 'FAILED' as const, evidence_kind: 'provider_outcome' as const,
  ...
  const reconciled = await h.boundary.reconcile({
    ...recovery,
    mode: 'pre_entry',
    outcome: { ...recovery.outcome, evidence_kind: 'pre_entry_lookup' as const },
  });
...
test('a reconcile outcome that conflicts with the terminal record changes nothing', async () => {
  ...
  const flipped = await h.boundary.reconcile({
    ...base,
    outcome: { state: 'FAILED', evidence_kind: 'provider_outcome' as const, evidence: providerOutcomeFor(2), reason: 'declined' },
  });
...
test('hostile in-process run and reconcile inputs refuse with a reason instead of throwing', async () => {
  ...
  const good = {
    ...
    action: ACTION,
    attempt: uncertain.attempt,
    outcome: { state: 'EXECUTED', evidence_kind: 'provider_outcome' as const, evidence: executedEvidence(), result: {} },
    recovery_authorization: 'recovery:approved',
  };
...
test('an unconfirmed fence release after FAILED stays reconcilable and never opens a second entry early', async () => {
  ...
  const reconciliation = {
    ...
    action: ACTION,
    attempt: first.attempt,
    outcome: { state: 'FAILED' as const, evidence_kind: 'provider_outcome' as const, evidence: providerOutcomeFor(42), reason: 'declined' },
    recovery_authorization: 'recovery:approved',
  };
  ...
  const reconciled = await h.boundary.reconcile({
    ...reconciliation,
    outcome: { state: 'FAILED' as const, evidence_kind: 'provider_outcome' as const, evidence: providerOutcomeFor(1), reason: 'declined' },
  });
 ...
 */
function labelled(outcome: unknown, mode: 'pre_entry' | 'terminal' | undefined) {
  if (!outcome || typeof outcome !== 'object' || Object.hasOwn(outcome, 'evidence_kind')) return outcome;
  const state = (outcome as { state?: unknown }).state;
  if (state !== 'EXECUTED' && state !== 'FAILED') return outcome;
...
function nativeRecovery(
  ...
  mode: 'pre_entry' | 'terminal' | undefined = 'pre_entry',
) {
  return {
    operation_id: operationId,
    handoff: f.handoff,
...
test('B4: a transient store error while handing back a pre-entry refusal is recoverable in one authorized reconcile', async () => {
  ...
  for (const lookup of ['FAILED', 'INDETERMINATE'] as const) {
    ...
    // pre-entry stop.
    const terminal = await h.boundary.reconcile(nativeRecovery(
      f, `operation:native:b4:${lookup}`, first.attempt,
      { state: 'FAILED', evidence: failedEvidence(1), reason: 'not_found' },
...
test('B7: a lost RESERVED to INVOKING write is recoverable; an applied one needs a provider outcome', async () => {
  ...
  // be reconciled and locked the action forever.
  {
    ...
    const recovered = await h.boundary.reconcile(nativeRecovery(
      f, 'operation:native:b7', b7Attempt.binding,
 ...
 */
function lostStartAttemptStore(failedReads: number) {
  const attempts = attemptStore();
  const transition = attempts.transition.bind(attempts);
  const state = attempts.state.bind(attempts);
  let loseStartAck = true;
 ...
 */
function nonExistenceVerifier(context: any) {
...
function keyCancelledEvidence(providerIdempotencyKey: string) {
  return {
...
test('T1 NA12 native: a start write that lands with an unreadable record is held without a not-entered write, and closes only on authenticated non-existence', async () => {
  {
    ...
    assert.equal(fenced.reason, 'native_action_in_flight');
    const lookup = await h.boundary.reconcile(nativeRecovery(
      f, 'operation:native:na12', first.attempt,
      { state: 'FAILED', evidence: failedEvidence(12), reason: 'not_found' },
    ));
    assert.deepEqual([lookup.state, lookup.reason], ['INDETERMINATE', 'recovery_lost_to_live_attempt']);
    const notFound = await h.boundary.reconcile(nativeRecovery(
      f, 'operation:native:na12', first.attempt,
      { state: 'FAILED', evidence: failedEvidence(12), reason: 'not_found' },
      'recovery:approved', 'terminal',
    ));
  ...
  }
  {
    ...
    const h = makeNativeBoundary({
      ...
      attempts,
      resolveStatus: (handoff: any) => nativeStatusFor(f, handoff),
      invoke: async () => { calls += 1; return { state: 'EXECUTED' as const, evidence: providerOutcomeFor(calls), result: {} }; },
    });
...
test('T1 NX1 native: a delayed not-entered write can never land on an attempt that entered the provider', async () => {
  ...
  for (const lookup of ['INDETERMINATE', 'FAILED'] as const) {
    const f = nativeFixture();
    const attempts = attemptStore();
    const transition = attempts.transition.bind(attempts);
    const state = attempts.state.bind(attempts);
    let armed = true;
    let failReads = 0;
    let pendingMarker: null | (() => Promise<unknown>) = null;
    const notEnteredWrites: string[] = [];
    attempts.transition = async (entry: any) => {
      ...
      if (entry.next_state === 'RELEASED') notEnteredWrites.push(entry.expected_state);
    ...
    const h = makeNativeBoundary({
      ...
      invoke: async () => {
        calls += 1;
        if (pendingMarker) await pendingMarker();
        return { state: 'EXECUTED' as const, evidence: providerOutcomeFor(calls), result: {} };
      },
    });
    ...
    const recovery = await h.boundary.reconcile(nativeRecovery(
      f, 'operation:native:nx1', structuredClone(row.binding),
      lookup === 'FAILED'
        ? { state: 'FAILED', evidence: failedEvidence(1), reason: 'not_found' }
        : { state: 'INDETERMINATE', reason: 'provider_lookup_unavailable' },
    ));
...
test('S5c NA10 native: a terminal outcome the verifier refuses leaves an INVOKING record unfrozen and the live run finishes it', async () => {
  ...
  const h = makeNativeBoundary({
    ...
    providerOutcomeVerify: (context: any) => (context.purpose === 'provider_outcome'
  ...
  const row = [...h.attempts.rows.values()][0];
  const junk = await h.boundary.reconcile(nativeRecovery(
    f, 'operation:native:na10', row.binding,
    { state: 'FAILED', evidence: failedEvidence(10), reason: 'not_found' },
    'recovery:approved', 'terminal',
  ));
...
test('restart reconciliation completes in one call with one credential bound to the attempt', async () => {
  ...
  for (const terminal of ['FAILED', 'EXECUTED'] as const) {
    ...
    const reconciled = await after.boundary.reconcile({
      ...
      attempt: uncertain.attempt,
      outcome: terminal === 'FAILED'
        ? { state: 'FAILED', evidence_kind: 'provider_outcome' as const, evidence: providerOutcomeFor(1), reason: 'declined' }
        : { state: 'EXECUTED', evidence_kind: 'provider_outcome' as const, evidence: providerOutcomeFor(1), result: {} },
...
test('C1 composed: a fresh evaluation cannot re-enter the provider while the same action is INDETERMINATE', async () => {
  ...
  const reconciled = await h.boundary.reconcile({
    ...
    attempt: uncertain.attempt,
    outcome: { state: 'EXECUTED', evidence_kind: 'provider_outcome' as const, evidence: executedEvidence(), result: { n: 1 } },
    recovery_authorization: 'recovery:approved',
  });
  assert.equal(reconciled.state, 'EXECUTED');
...
test('composed: an authenticated FAILED in run or reconcile releases the fence; concurrency enters once', async () => {
  ...
  }
  {
    ...
    const failed = await h.boundary.reconcile({
      ...
      artifacts: first.artifacts,
      attempt: uncertain.attempt,
      outcome: { state: 'FAILED', evidence_kind: 'provider_outcome' as const, evidence: failedEvidence(1), reason: 'declined' },
      recovery_authorization: 'recovery:approved',
    });
    assert.equal(failed.state, 'FAILED');
...
test('composed: a pre-entry lockout is recovered by pre-entry reconcile and leaves only that evaluation spent', async () => {
  ...
  const terminal = await h.boundary.reconcile({
    ...recovery,
    outcome: { state: 'FAILED' as const, evidence_kind: 'provider_outcome' as const, evidence: executedEvidence(), reason: 'not_found' },
  });
  assert.equal(terminal.state, 'INDETERMINATE');
  assert.equal(terminal.reason, 'pre_entry_recovery_required');
  assert.equal(aebStore.operations.get(holderKey), 'RESERVED');
  ...
  const recovered = await h.boundary.reconcile({ ...recovery, mode: 'pre_entry' as const });
  assert.equal(recovered.state, 'REFUSED');
  assert.equal(recovered.reason, 'attempt_never_entered_provider');
...
test('R1 N1a/RR4 native: a pre-entry recovery that loses its transition to the live run releases nothing', async () => {
  for (const lookup of ['FAILED', 'INDETERMINATE'] as const) {
    ...
    const h = makeNativeBoundary({
      ...
      invoke: async () => {
        calls += 1;
        if (calls === 1) { atProvider.resolve(); await providerGo.promise; }
        return { state: 'EXECUTED' as const, evidence: providerOutcomeFor(calls), result: {} };
      },
    });
    ...
    const recovery = await h.boundary.reconcile(nativeRecovery(
      f, `operation:native:r1:${lookup}`, structuredClone(row.binding),
      lookup === 'FAILED'
...
test('R1 native: once pre-entry recovery linearizes first, the live run never invokes and hands back its rows', async () => {
  ...
  const row = [...attempts.rows.values()][0];
  const recovery = await h.boundary.reconcile(nativeRecovery(
    f, 'operation:native:r1-win', structuredClone(row.binding),
    { state: 'INDETERMINATE', reason: 'provider_lookup_unavailable' },
  ));
...
test('R1 pre-entry mode input: an unknown mode is refused, and the verifier sees the lookup purpose', async () => {
  ...
  const h = makeNativeBoundary({
    ...
    providerOutcomeVerify: (context: any) => {
  ...
  let fail = true;
  aebStore.release = async (key: string) => {
    if (fail && key.startsWith('aeb-native-action-holder:')) { fail = false; throw new Error('reset'); }
    return release(key);
  };
  let lookups = 0;
  ...
  const bogus = await k.boundary.reconcile({
    ...nativeRecovery(g, 'operation:native:mode', stopped.attempt,
      { state: 'FAILED', evidence: failedEvidence(21), reason: 'not_found' }),
    mode: 'both',
  });
  assert.equal(bogus.state, 'REFUSED');
  assert.equal(bogus.reason, 'native_reconciliation_input_invalid');
  const recovered = await k.boundary.reconcile(nativeRecovery(
    g, 'operation:native:mode', stopped.attempt,
    { state: 'FAILED', evidence: failedEvidence(21), reason: 'not_found' },
  ));
  assert.equal(recovered.reason, 'attempt_never_entered_provider');
...
test('R1 N1b native: a verifier that accepts lookups only as pre-entry evidence keeps a live call fenced', async () => {
  ...
  const h = makeNativeBoundary({
    ...
    providerOutcomeVerify: (context: any) => (context.purpose === 'pre_entry_lookup'
      || context.outcome.evidence.evidence_id.startsWith('provider-evidence:call-'))
      ? affirm(context)
      : false,
    invoke: async () => {
  ...
  const row = [...h.attempts.rows.values()][0];
  assert.equal(row.state, 'INVOKING');
  const asLookup = await h.boundary.reconcile(nativeRecovery(
    f, 'operation:native:n1b', structuredClone(row.binding),
    { state: 'FAILED', evidence: failedEvidence(31), reason: 'not_found' },
  ));
  assert.equal(asLookup.reason, 'recovery_lost_to_live_attempt');
  const asTerminal = await h.boundary.reconcile(nativeRecovery(
    f, 'operation:native:n1b', structuredClone(row.binding),
    { state: 'FAILED', evidence: failedEvidence(31), reason: 'not_found' },
    'recovery:approved', 'terminal',
  ));
...
test('R2 native: terminal reconciliation closes nothing until the terminal record is confirmed', async () => {
  ...
  attempts.reconcile = async (entry: any) => {
    if (failTerminal) throw new Error('attempt store unavailable');
    return terminal(entry);
  };
  const h = makeNativeBoundary({
    f,
  ...
  assert.equal(first.reason, 'provider_outcome_indeterminate');
  const reconciliation = nativeRecovery(
    f, 'operation:native:r2', first.attempt,
...
test('R3/R4 N6b native: a truthy non-true start answer and a lost start acknowledgement follow the durable record', async () => {
  {
    ...
    const transition = attempts.transition.bind(attempts);
    attempts.transition = async (entry: any) => (
      entry.next_state === 'INVOKING' ? { ok: false, reason: 'cas_conflict' } as any : transition(entry)
    );
    let calls = 0;
    const h = makeNativeBoundary({
      f,
      attempts,
      invoke: async () => { calls += 1; return { state: 'EXECUTED' as const, evidence: providerOutcomeFor(1), result: {} }; },
    });
  ...
  }
  {
    ...
    attempts.transition = async (entry: any) => {
      ...
      if (entry.next_state === 'INVOKING') throw new Error('acknowledgement lost');
      return answer;
    };
    let calls = 0;
    const h = makeNativeBoundary({
...
test('R8 N10 native: a recovery racing a still-reserving run does not strand that run\'s holder', async () => {
  ...
  let blocked = false;
  aebStore.reserve = async (key: string, fences: readonly string[]) => {
  ...
  const row = [...h.attempts.rows.values()][0];
  const recovery = await h.boundary.reconcile(nativeRecovery(
    f, 'operation:native:n10', structuredClone(row.binding),
    { state: 'INDETERMINATE', reason: 'provider_lookup_unavailable' },
  ));
  assert.equal(recovery.reason, 'attempt_never_entered_provider');
  holderGo.resolve();
...
test('R5 recovery-claim scopes name exactly the rows both boundaries derive', () => {
  ...
  const nativeScope = (reservation: string) => ({
    boundary: 'native',
    boundaryId: NATIVE_BOUNDARY_ID,
  ...
  assert.notEqual(
    consequenceBoundaryRecoveryClaimKey({ ...nativeScope('action-fence-holder'), attemptId: 'native-attempt:claim:2' }),
    nativeKeys.holder,
  );
  ...
  const composedScope = (reservation: string) => ({
    boundary: 'composed',
    boundaryId: COMPOSED_BOUNDARY_ID,
  ...
  assert.equal(
    consequenceBoundaryRecoveryAttemptIdentity({ ...nativeScope('operation'), attemptId: shared }),
    `native:${NATIVE_BOUNDARY_ID}:${shared}`,
  );
  assert.equal(
    consequenceBoundaryRecoveryAttemptIdentity({ ...composedScope('operation'), attemptId: shared }),
    `composed:${COMPOSED_BOUNDARY_ID}:${shared}`,
  );
  ...
  assert.notEqual(
    consequenceBoundaryRecoveryAttemptIdentity(otherNative),
    consequenceBoundaryRecoveryAttemptIdentity(nativeScope('action-fence-holder')),
  );
  ...
  } as any), /action_fence_holder_binding_invalid/);
  assert.throws(() => nativeConsequenceBoundaryAttemptReservationKeys({
    ...
    action_digest: digestAebNativeAuthorizationAction(ACTION),
    attempt_id: 'native-attempt:claim:1',
  } as any), /native_attempt_reservation_binding_invalid/);
});

function makeComposedRace(
...
function composedRecovery(
  f: ReturnType<typeof fixture>,
  attempt: unknown,
  outcome: unknown,
  mode?: 'pre_entry' | 'terminal',
) {
  return {
    evaluation: f.evaluation,
    action: ACTION,
...
test('R1 N1d/RR3 composed: a pre-entry recovery that loses its transition to the live run releases nothing', async () => {
  ...
  await attempts.liveArrived.promise;
  const row = [...attempts.rows.values()][0];
  const recovery = await h.boundary.reconcile(composedRecovery(
    first, structuredClone(row.binding),
    { state: 'FAILED', evidence: executedEvidence(), reason: 'not_found' },
    'pre_entry',
  ));
...
test('R2 N1c/RR2 composed: reconcile releases nothing before the terminal record is confirmed', async () => {
  for (const failure of ['terminal', 'freeze'] as const) {
    ...
    attempts.transition = async (entry: any) => (
      failRecovery && failure === 'freeze' && entry.next_state === 'INDETERMINATE' ? false : transition(entry)
    );
    attempts.reconcile = async (entry: any) => (
      failRecovery && failure === 'terminal' ? false : terminal(entry)
    );
    const atProvider = deferred();
    const providerGo = deferred();
    let calls = 0;
    ...
    const recovery = await h.boundary.reconcile(composedRecovery(
      first, structuredClone(row.binding),
      { state: 'FAILED', evidence: executedEvidence(), reason: 'declined' },
    ));
    assert.equal(recovery.state, 'INDETERMINATE', failure);
    assert.equal(
      recovery.reason,
      failure === 'terminal' ? 'attempt_terminal_record_unconfirmed' : 'attempt_freeze_unconfirmed',
      failure,
    );
    const reservationKey = aebReservationKey(first.evaluation);
...
test('R2 N2 composed: a 0.26.0-style attempt store never releases a RESERVED attempt on a terminal outcome', async () => {
  ...
  const { state: _dropped, ...rest } = full as any;
  const legacy = {
    ...rest,
    rows: full.rows,
    async transition(entry: any) {
  ...
  assert.equal(row.state, 'RESERVED');
  assert.equal(row.evidence, undefined);
  const terminal = await h.boundary.reconcile(composedRecovery(
    first, stopped.attempt, { state: 'FAILED', evidence: executedEvidence(), reason: 'not_found' },
  ));
  assert.equal(terminal.state, 'INDETERMINATE');
  ...
  failHolderRelease = false;
  const recovered = await h.boundary.reconcile(composedRecovery(
    first, stopped.attempt, { state: 'INDETERMINATE', reason: 'provider_lookup_unavailable' }, 'pre_entry',
  ));
  assert.equal(recovered.state, 'REFUSED');
  assert.equal(recovered.reason, 'attempt_never_entered_provider');
  assert.equal(row.state, 'RELEASED');
  ...
  const { state: _unused, ...reservedRest } = reserved as any;
  let stall = true;
  const stalledStore = {
    ...reservedRest,
    rows: reserved.rows,
    async transition(entry: any) {
      if (stall && entry.next_state === 'INVOKING') await new Promise(() => {});
  ...
  const refusedTerminal = await g.boundary.reconcile(composedRecovery(
    second, structuredClone(stuck.binding), { state: 'FAILED', evidence: executedEvidence(), reason: 'not_found' },
  ));
  assert.equal(refusedTerminal.reason, 'attempt_terminal_record_unconfirmed');
  assert.equal(stuck.state, 'RESERVED');
  stall = false;
  const preEntry = await g.boundary.reconcile(composedRecovery(
    second, structuredClone(stuck.binding), { state: 'INDETERMINATE', reason: 'provider_lookup_unavailable' },
    'pre_entry',
  ));
...
test('R3 RR1 composed: a lost acknowledgement on the start write proceeds as the owner and releases nothing', async () => {
  ...
  const release = aebStore.release.bind(aebStore);
  aebStore.release = async (key: string) => { releases.push(key); return release(key); };
  attempts.transition = async (entry: any) => {
    const answer = await transition(entry);
...
test('R6 N4 composed: recovery of one attempt never commits another attempt\'s evaluation reservation', async () => {
  ...
  assert.equal(aebStore.operations.get(reservationKey), 'RESERVED');
  const recovery = await h.boundary.reconcile(composedRecovery(
    first, lost.attempt, { state: 'FAILED', evidence: executedEvidence(), reason: 'declined' },
  ));
  assert.equal(recovery.state, 'INDETERMINATE');
...
test('R8 N5 composed: a failed evaluation release after a pre-entry stop is not reported as a clean refusal', async () => {
  ...
  attempts.transition = async (entry: any) => {
    if (conflictOnce && entry.next_state === 'INVOKING') { conflictOnce = false; return false; }
    return transition(entry);
  };
  const h = makeComposedRace(first, attempts, aebStore);
  const result = await h.boundary.run(input(first));
...
test('R8 composed: once pre-entry recovery linearizes first, the live run hands back the holder and R(E)', async () => {
  ...
  const row = [...attempts.rows.values()][0];
  const recovery = await h.boundary.reconcile(composedRecovery(
    first, structuredClone(row.binding), { state: 'INDETERMINATE', reason: 'provider_lookup_unavailable' },
    'pre_entry',
  ));
  assert.equal(recovery.reason, 'attempt_never_entered_provider');
  const result = await live;
...
test('R9 N9 composed: hostile run and reconcile input refuses with a reason and never throws', async () => {
  ...
  const getter = (key: string, base: Record<string, unknown>) => Object.defineProperty({ ...base }, key, {
    get() { throw new Error('boom'); },
    enumerable: true,
  });
  for (const key of ['artifacts', 'current_statuses', 'additional_replay_keys', 'evaluation', 'action']) {
    const result = await h.boundary.run(getter(key, input(first)) as any);
  ...
  revoked.revoke();
  assert.equal((await h.boundary.run(revoked.proxy as any)).reason, 'execution_input_invalid');
  assert.equal((await h.boundary.run({ ...input(first), additional_replay_keys: [7] } as any)).reason, 'execution_input_invalid');
  assert.equal((await h.boundary.run(null as any)).reason, 'execution_input_invalid');
  ...
  assert.equal((await lostRun.boundary.reconcile(null as any)).reason, 'reconciliation_input_invalid');
});
...
test('R9 N9 composed: a synchronous attempt store answering plain booleans never throws after provider entry', async () => {
  ...
  const sync = {
    ...
    transition: (entry: any) => {
      const row = base.rows.get(entry.attempt_id);
      if (!row || row.owner !== entry.owner || row.state !== entry.expected_state) return false;
  ...
  const h = makeComposedRace(first, sync as any);
  const result = await h.boundary.run(input(first));
  assert.equal(result.state, 'EXECUTED');
  assert.equal(h.calls(), 1);
  ...
  const g = makeComposedRace(second, throwingSync as any);
...
test('the native authority reservation also fences the verify 4.1.0 key: 0.26.0 spends and namespace rotations stay spent', async () => {
  ...
  }
  {
    ...
    const aebStore = durableAebStore();
    let calls = 0;
    const invoke = async () => { calls += 1; return { state: 'EXECUTED' as const, evidence: providerOutcomeFor(calls), result: {} }; };
...
test('S1 NA9 native: a RELEASED record without the not-entered marker is never read as not entered, so one grant never enters twice', async () => {
  ...
  const h = makeNativeBoundary({
    ...
    invoke: async () => {
      calls += 1;
      return calls === 1
        ? { state: 'FAILED' as const, evidence: providerOutcomeFor(calls), reason: 'provider_declined' }
        : { state: 'EXECUTED' as const, evidence: providerOutcomeFor(calls), result: {} };
    },
  });
  ...
  assert.equal(row.state, 'RELEASED');
  const terminal = await h.boundary.reconcile(nativeRecovery(
    f, 'operation:native:na9', row.binding,
    { state: 'FAILED', evidence: providerOutcomeFor(1), reason: 'provider_declined' },
    'recovery:approved', 'terminal',
  ));
  assert.equal(terminal.state, 'INDETERMINATE');
  assert.equal(terminal.reason, 'attempt_record_unproven');
  const preEntry = await h.boundary.reconcile(nativeRecovery(
    f, 'operation:native:na9', row.binding,
    { state: 'FAILED', evidence: failedEvidence(9), reason: 'not_found' },
  ));
  ...
  const keys = nativeConsequenceBoundaryAttemptReservationKeys({
    ...
    action_digest: row.binding.action_digest as any,
    boundary_id: NATIVE_BOUNDARY_ID,
    attempt_id: row.binding.attempt_id,
  });
...
test('S1 native: an attempt store that does not declare the not-entered marker is refused at construction', () => {
  const attempts = attemptStore() as any;
...
test('S1 native: a pre-entry stop whose marker was not stored holds its rows, and recovery cannot prove it', async () => {
  ...
  attempts.transition = async (entry: any) => {
    ...
    if (applied && row && entry.next_state === 'RELEASED') delete row.evidence;
    return applied;
  };
  let lookups = 0;
  const h = makeNativeBoundary({
  ...
  assert.equal([...attempts.rows.values()][0].state, 'RELEASED');
  const recovered = await h.boundary.reconcile(nativeRecovery(
    f, 'operation:native:no-marker', stopped.attempt,
    { state: 'FAILED', evidence: failedEvidence(31), reason: 'not_found' },
  ));
...
test('S1 CA9 composed: pre-entry recovery never calls an attempt that entered and FAILED "never entered"', async () => {
  {
    ...
    const h = makeBoundary({
      ...
      invoke: async () => {
        calls += 1;
        return { state: 'FAILED', evidence: failedEvidence(4), reason: 'declined' } as any;
      },
    });
    const first = await h.boundary.run(input(f));
    assert.equal(first.state, 'INDETERMINATE');
    ...
    for (const mode of ['pre_entry', 'terminal'] as const) {
      const recovered = await h.boundary.reconcile(composedRecovery(
        f, row.binding,
        { state: 'FAILED', evidence: failedEvidence(5), reason: 'not_found' },
        mode,
      ));
  ...
  }
  {
    ...
    const h = makeBoundary({
      f,
      attempts,
      invoke: async () => ({ state: 'FAILED', evidence: failedEvidence(6), reason: 'declined' }) as any,
    });
    const first = await h.boundary.run(input(f));
...
test('S2 N1c composed: terminal reconciliation needs a verifier that affirms the terminal purpose; a live call stays fenced', async () => {
  ...
  assert.throws(
    () => makeBoundary({ providerOutcomeVerify: null }),
    /^TypeError: consequence_boundary_configuration_invalid: provider_outcome_verifier_required$/,
  );
  for (const verifier of ['purpose-honoring', 'always-true'] as const) {
    const f = fixture({
    ...
    const h = makeBoundary({
      ...
      providerOutcomeVerify: verifier === 'always-true'
        ? () => true
        : (context: any) => (context.purpose === 'provider_outcome'
      ...
      invoke: async () => {
        calls += 1;
        if (calls === 1) { atProvider.resolve(); await providerGo.promise; }
        return { state: 'EXECUTED', evidence: executedEvidence(), result: {} } as any;
      },
    });
    const live = h.boundary.run(input(f));
    await atProvider.promise;
    const row = [...h.attempts.rows.values()][0];
    const misuse = await h.boundary.reconcile(composedRecovery(
      f, row.binding,
      { state: 'FAILED', evidence: failedEvidence(3), reason: 'not_found' },
      'terminal',
    ));
...
test('S2 N1b native: only an affirmation of the exact purpose, attempt, and key counts; a plain true never closes a live attempt', async () => {
  {
    ...
    const h = makeNativeBoundary({
      f,
      resolveStatus: (handoff: any) => nativeStatusFor(f, handoff),
      providerOutcomeVerify: (context: any) => { contexts.push(context); return true; },
      invoke: async () => {
        calls += 1;
        if (calls === 1) { atProvider.resolve(); await providerGo.promise; }
        return { state: 'EXECUTED' as const, evidence: providerOutcomeFor(calls), result: {} };
    ...
    await atProvider.promise;
    const row = [...h.attempts.rows.values()][0];
    const misuse = await h.boundary.reconcile(nativeRecovery(
      f, 'operation:native:n1b-true', row.binding,
      { state: 'FAILED', evidence: failedEvidence(2), reason: 'not_found' },
      'recovery:approved', 'terminal',
    ));
  ...
  for (const [label, answer] of [
    ['wrong purpose', (context: any) => ({ ...affirm(context), purpose: 'pre_entry_lookup' })],
    ['other attempt', (context: any) => ({ ...affirm(context), attempt_id: 'native-attempt:other' })],
    ['other key', (context: any) => ({ ...affirm(context), provider_idempotency_key: 'epnb1:other' })],
    ['extra member', (context: any) => ({ ...affirm(context), note: 'extra' })],
    ['string verified', (context: any) => ({ ...affirm(context), verified: 'true' })],
...
test('S3 native: a grant consumed by Gate 0.26.0 under one pinned label is refused under every other pinned label of its namespace', async () => {
  const labelFor = (f: ReturnType<typeof nativeFixture>) => ({
    ...f.handoffInput.native_authorization,
    profile: 'authzen:exact-action-result:2',
  });
  const pinLabel = (f: ReturnType<typeof nativeFixture>) => {
  ...
  };
  {
    ...
    aebStore.db.replayOwners.set(
      aebNativeAuthorizationReplayKey(f.handoff),
      'aeb-native-operation:legacy-0.26.0-label-a',
    );
    let calls = 0;
    const h = makeNativeBoundary({
      f,
...
test('S5a CA7 composed: a refusal before the attempt record is INDETERMINATE when the evaluation release is unconfirmed', async () => {
  for (const path of ['reserve-refused', 'reserve-throws', 'allocation-fails'] as const) {
    for (const releaseWorks of [false, true]) {
      ...
      const release = aebStore.release.bind(aebStore);
      aebStore.release = async (key: string) => {
        if (!releaseWorks && key.startsWith('aeb:')) throw new Error('release lost');
        return release(key);
      };
      const attempts = attemptStore();
      if (path === 'reserve-refused') {
        attempts.reserve = async () => ({ reserved: false as const, reason: 'attempt_exists' });
      }
      ...
      const h = makeBoundary({
        ...
        invoke: async () => { calls += 1; return { state: 'EXECUTED', evidence: executedEvidence(), result: {} } as any; },
      });
      const result = await h.boundary.run(input(f));
  ...
  for (const applied of [true, false]) {
    ...
    aebStore.reserve = async (key: string, fences: readonly string[]) => {
      if (armed && key.startsWith('aeb:')) {
        armed = false;
        if (applied) await reserve(key, fences);
        throw new Error('acknowledgement lost');
      }
      return reserve(key, fences);
    };
...
test('S5d native: a recovery whose claim loses to the live run handing back its own rows counts them as released', async () => {
  ...
  recoveryStore.claimReservation = async (...args: [string, unknown, unknown?]) => {
    go.resolve();
    await liveRun;
    return claim(...args);
  };
  ...
  const row = [...attempts.rows.values()][0];
  const recovered = await recovery.boundary.reconcile(nativeRecovery(
    f, 'operation:native:s5d', row.binding,
    { state: 'FAILED', evidence: failedEvidence(55), reason: 'not_found' },
  ));
  assert.equal(recovered.state, 'REFUSED');
  assert.equal(recovered.reason, 'attempt_never_entered_provider');
...
test('S6 native: did:web host case and spiffe trust-domain case are issuer aliases', () => {
  for (const [label, issuers] of [
    ...
    for (const source of f.pins.accepted_sources) source.authority_namespace = 'namespace:authzen';
...
test('S5d native: a recovery whose claim another recovery overtakes claims again instead of reporting unconfirmed', async () => {
  ...
  while (!crashed) await new Promise((done) => setImmediate(done));
  ...
  recoveryStore.claimReservation = async (key: string, authorization: unknown, scope?: unknown) => {
  ...
  const row = [...attempts.rows.values()][0];
  const recovered = await recovery.boundary.reconcile(nativeRecovery(
    f, 'operation:native:s5d-overtaken', row.binding,
    { state: 'FAILED', evidence: failedEvidence(56), reason: 'not_found' },
  ));
  assert.equal(recovered.state, 'REFUSED');
  assert.equal(recovered.reason, 'attempt_never_entered_provider');
...
function strictVerify(context: any) {
  if (context.outcome.reason === 'not_found' && context.purpose !== 'pre_entry_lookup') return false;
  return affirm(context);
}
...
test('T3 NX3/CX3: an echo verifier cannot turn a lookup labelled pre_entry_lookup into a terminal FAILED', async () => {
  ...
  const echo = (context: any) => affirm(context);
  {
    const f = nativeFixture();
    const atProvider = deferred();
    const providerGo = deferred();
    ...
    const h = makeNativeBoundary({
      f,
      resolveStatus: (handoff: any) => nativeStatusFor(f, handoff),
      providerOutcomeVerify: (context: any) => { verifierCalls += 1; return echo(context); },
      invoke: async () => {
        calls += 1;
        if (calls === 1) { atProvider.resolve(); await providerGo.promise; }
        return { state: 'EXECUTED' as const, evidence: providerOutcomeFor(calls), result: {} };
    ...
    const terminal = await h.boundary.reconcile(nativeRecovery(
      f, 'operation:native:nx3', structuredClone(row.binding), lookup, 'recovery:approved', 'terminal',
    ));
    ...
    const preEntry = await h.boundary.reconcile(nativeRecovery(
      f, 'operation:native:nx3', structuredClone(row.binding),
      { state: 'FAILED', evidence_kind: 'provider_outcome', evidence: failedEvidence(3), reason: 'not_found' },
    ));
  ...
  }
  {
    ...
    const terminal = await h.boundary.reconcile(composedRecovery(
      f, row.binding,
      { state: 'FAILED', evidence_kind: 'pre_entry_lookup', evidence: failedEvidence(4), reason: 'not_found' },
      'terminal',
    ));
    assert.deepEqual([terminal.state, terminal.reason], ['REFUSED', 'evidence_kind_mismatch']);
    const bare = await h.boundary.reconcile({
...
test('T4 PG4c (memory): two native boundaries on one store that reuse an attempt ID never share a row or a credential', async () => {
  ...
  const db = durableAebDatabase();
  const credentialFor = new Map<string, string>();
  const claims: Array<{ identity: string | null; granted: boolean }> = [];
  const authorize = ({ key, authorization, scope }: { key: string; authorization: unknown; scope?: any }) => {
  ...
  const n1 = makeNativeBoundary({
    ...
    createId: () => sharedId,
    resolveStatus: (handoff: any) => nativeStatusFor(f1, handoff),
    invoke: async () => ({ state: 'INDETERMINATE' as const, reason: 'provider_timeout' }),
  });
  const n2 = makeNativeBoundary({
    ...
    resolveStatus: (handoff: any) => nativeStatusFor(f2, handoff),
    invoke: async () => {
      n2Calls += 1;
      if (n2Calls === 1) { n2At.resolve(); await n2Go.promise; }
      return { state: 'EXECUTED' as const, evidence: providerOutcomeFor(n2Calls), result: {} };
    },
  });
  ...
  const n1Scope = (reservation: string) => ({
  ...
  const n2Scope = (reservation: string) => ({
...
test('T4: a boundary ID outside the grammar is refused at construction on both boundaries', () => {
  for (const bad of [null, '', 'has:colon', 'has space', '-lead', 'x'.repeat(129), 7]) {
    assert.throws(
      () => makeNativeBoundary({ boundaryId: bad as any }),
      /^TypeError: native_consequence_boundary_configuration_invalid: boundary_id_invalid$/,
      String(bad),
    );
    assert.throws(
      () => makeBoundary({ boundaryId: bad as any }),
      /^TypeError: consequence_boundary_configuration_invalid: boundary_id_invalid$/,
      String(bad),
    );
  }
  ...
  assert.throws(
    () => createNativeConsequenceBoundary(withoutBoundaryId as any),
    /^TypeError: native_consequence_boundary_configuration_invalid: boundary_id_invalid$/,
  );
  ...
  assert.throws(
    () => createNativeConsequenceBoundary(withoutVerifier as any),
    /^TypeError: native_consequence_boundary_configuration_invalid: provider_outcome_verifier_required$/,
  );
...
test('T6 native: an unknown reserve answer is resolved by a durable read and is never a clean refusal', async () => {
  const unknownAnswers: Array<[string, (apply: () => Promise<unknown>) => Promise<unknown>]> = [
  ...
  for (const target of ['aeb-native-attempt-operation:', 'aeb-native-attempt-authority:', 'aeb-native-action-holder:']) {
    for (const [label, answer] of unknownAnswers) {
      ...
      assert.deepEqual([result.state, result.reason, result.invoked],
        ['INDETERMINATE', 'consumption_reservation_unconfirmed', false], at);
      const row = [...h.attempts.rows.values()][0];
  ...
  // pre-entry recovery: the attempt record carries the not-entered marker.
  {
    ...
    const aebStore = durableAebStore();
    const reserve = aebStore.reserve.bind(aebStore);
    let pending: null | (() => Promise<unknown>) = null;
    aebStore.reserve = async (key: string, fences: readonly string[]) => {
    ...
    assert.equal(fenced.reason, 'native_action_in_flight');
    const recovered = await h.boundary.reconcile(nativeRecovery(
      f, 'operation:native:t6:delayed', result.attempt,
      { state: 'INDETERMINATE', reason: 'provider_lookup_unavailable' },
    ));
  ...
  // Defined conflicts stay clean refusals.
  {
    const f = nativeFixture();
    const aebStore = durableAebStore();
    const h = makeNativeBoundary({ f, aebStore, resolveStatus: (handoff: any) => nativeStatusFor(f, handoff) });
...
test('T6 composed: an unknown action-fence holder answer is never a clean refusal', async () => {
  for (const [label, answer] of [
    ['applied, truthy object', async (apply: () => Promise<unknown>) => { await apply(); return { ok: true }; }],
    ['applied, ack lost', async (apply: () => Promise<unknown>) => { await apply(); throw new Error('ack lost'); }],
    ['not applied, throw', async () => { throw new Error('down'); }],
  ] as const) {
    ...
    aebStore.reserve = async (key: string, fences: readonly string[]) => {
      if (armed && key.startsWith('aeb-action-holder:')) {
        armed = false;
        return (answer as any)(() => reserve(key, fences));
      }
      return reserve(key, fences);
    };
    let calls = 0;
    ...
    const result = await h.boundary.run(input(f));
    assert.deepEqual([result.state, result.reason, result.invoked],
      ['INDETERMINATE', 'consumption_reservation_unconfirmed', false], label);
    const row = [...h.attempts.rows.values()][0];
...
test('T1 CX5 composed: an unconfirmed start is held with no not-entered write, with and without a durable read', async () => {
  for (const noState of [false, true]) {
    ...
    let armed = true;
    const written: string[] = [];
    const store: any = noState ? (() => { const { state: _s, ...rest } = full as any; return { ...rest, rows: full.rows }; })() : full;
    store.transition = async (entry: any) => {
      written.push(`${entry.expected_state}->${entry.next_state}`);
    ...
    if (!noState) {
      store.state = async (entry: any) => {
        if (failReads > 0) { failReads -= 1; throw new Error('read timeout'); }
        return state(entry);
      };
    }
    let calls = 0;
    ...
    const result = await h.boundary.run(input(f));
    assert.deepEqual([result.state, result.reason, result.invoked],
      ['INDETERMINATE', 'attempt_start_unconfirmed', false], String(noState));
