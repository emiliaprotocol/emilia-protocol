...
test('atomic evidence: a backend cannot claim success without persisting the record', async () => {
  ...
  await assert.rejects(
    log.record({ type: 'decision', allow: true }),
    /atomic_evidence_append_(not_observable|indeterminate)/,
  );
...
test('atomic evidence: one record id cannot alias different content', async () => {
  ...
  await assert.rejects(
    second.record({ type: 'decision', allow: true }),
    /conflicting record_id/,
  );
});
...
test('atomic evidence: altered persisted bytes fail verification', async () => {
  ...
  const backend = {
    ...
    async readAll(...args) {
      ...
      if (records[0]) records[0].allow = !records[0].allow;
      return records;
    },
  };
