...
function options(m: ReturnType<typeof material>) {
  return {
    action: ACTION,
...
test('a real hybrid attestation verifies under both pinned keys', async () => {
  const m = material();
  const attestation: any = await signExecutionValueAttestationV2(payloadInput(), { privateKey: m.pair.privateKey, pqPrivateKey: Buffer.from(m.pq.secretKey).toString('base64url') });
...
test('the v1 verifier still accepts a v1 attestation, unchanged', () => {
  const m = material();
  const attestation = signExecutionValueAttestation(payloadInput() as any, m.pair.privateKey);
  const verified = verifyExecutionValueAttestation(attestation, { ...options(m), trustedKeys: { 'oracle-key-v2': m.publicKey } });
  assert.equal(verified.ok, true, verified.reason);
});
...
test('LEG STRIPPING: removing the ML-DSA leg refuses structurally', async () => {
  const m = material();
  const signed: any = await signExecutionValueAttestationV2(payloadInput(), { privateKey: m.pair.privateKey, pqPrivateKey: Buffer.from(m.pq.secretKey).toString('base64url') });
  const attestation = structuredClone(signed);
  attestation.proof.signatures = attestation.proof.signatures.filter((s: any) => s.alg === 'Ed25519');
  const verified = await verifyExecutionValueAttestationV2(attestation, options(m));
  assert.equal(verified.ok, false);
...
test('LEG STRIPPING: removing the Ed25519 leg refuses too', async () => {
  ...
  const signed: any = await signExecutionValueAttestationV2(payloadInput(), { privateKey: m.pair.privateKey, pqPrivateKey: Buffer.from(m.pq.secretKey).toString('base64url') });
  const attestation = structuredClone(signed);
  attestation.proof.signatures = attestation.proof.signatures.filter((s: any) => s.alg === 'ML-DSA-65');
  const verified = await verifyExecutionValueAttestationV2(attestation, options(m));
  assert.equal(verified.ok, false);
});
...
test('WRONG-LENGTH SIGNATURE: a truncated Ed25519 signature refuses', async () => {
  ...
  const signed: any = await signExecutionValueAttestationV2(payloadInput(), { privateKey: m.pair.privateKey, pqPrivateKey: Buffer.from(m.pq.secretKey).toString('base64url') });
  const attestation = structuredClone(signed);
  attestation.proof.signatures = attestation.proof.signatures.map((s: any) => (s.alg === 'Ed25519' ? { ...s, sig: s.sig.slice(0, -4) } : s));
  const verified = await verifyExecutionValueAttestationV2(attestation, options(m));
  assert.equal(verified.ok, false);
});
