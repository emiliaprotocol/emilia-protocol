...
// Generated from trust-program.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
import test from 'node:test';
import assert from 'node:assert/strict';
import { generateKeyPairSync } from 'node:crypto';
import { TRUST_PROGRAM_VERSION, createMemoryTrustProgramStore, createTrustProgramKernel, trustProgramDigest, validateTrustProgram, verifyTrustStageReceipt, } from './trust-program.js';
...
function requirement(requirementId, profile, policyChar) {
    return {
...
function artifact(challenge, requirementId, subject, key, policyChar) {
    return {
...
async function admit(kernel, instanceId, stageId, requirementId, subject, key, policyChar) {
...
test('completes stages with signed predecessor-bound receipts and unlocks parallel branches only after their dependencies', async () => {
    ...
    const identity = await admit(kernel, 'tpi_flow', 'identity', 'buyer_handshake', 'alice', 'key_alice', '1');
    ...
    const compliance = await admit(kernel, 'tpi_flow', 'compliance', 'kyc_evidence', 'kyc-provider', 'key_kyc', '2');
    assert.equal(compliance.stage_completed, true);
    assert.equal(compliance.state.stages.finance.status, 'locked');
    const legal = await admit(kernel, 'tpi_flow', 'legal', 'counsel_signoff', 'counsel', 'key_counsel', '4');
    assert.equal(legal.stage_completed, true);
    assert.equal(legal.state.stages.finance.status, 'collecting');
    assert.deepEqual(legal.state.stages.finance.predecessor_receipt_digests, [compliance.stage_receipt.receipt_digest, legal.stage_receipt.receipt_digest].sort());
});
...
test('tampering with a stage receipt or predecessor list is independently detectable', async () => {
    ...
    const completed = await admit(kernel, 'tpi_receipt', 'identity', 'buyer_handshake', 'alice', 'key_alice', '1');
