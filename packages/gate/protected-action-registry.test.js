// Generated from protected-action-registry.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
/**
...
import { createGate, createProtectedActionRegistry, } from './index.js';
...
const validPayee = (value) => {
    if (!value || typeof value !== 'object' || Array.isArray(value))
        return false;
    const input = value;
    return typeof input.vendor_id === 'string'
        && typeof input.account_digest === 'string'
        && Object.keys(input).every((key) => ['vendor_id', 'account_digest'].includes(key));
};
...
test('duplicate registration and every mutation after seal refuse', () => {
    ...
    assert.throws(() => registry.register('finance.change_payee', validPayee, async () => 'other'), /protected_action_duplicate/);
    registry.seal();
    assert.throws(() => registry.register('finance.read_balance', () => true, async () => 'read'), /protected_action_registry_sealed/);
...
test('runRegistered refuses unsealed, unknown, invalid, and counterfeit registries before handler entry', async () => {
    ...
    unsealed.register('finance.read_balance', () => true, async () => { calls += 1; });
    ...
    sealed.register('finance.change_payee', validPayee, async () => { calls += 1; });
    sealed.seal();
...
test('selector accessors cannot change the manifest-selected handler between preflight and admission', async () => {
    ...
    let reads = 0;
    const registry = createProtectedActionRegistry();
    registry.register('finance.read_balance', () => true, async () => { calls += 1; });
    registry.register('finance.change_payee', validPayee, async () => { calls += 1; });
    registry.seal();
    const gate = createGate({ manifest: MANIFEST, allowEphemeralStore: true });
    const selector = {
        protocol: 'mcp',
...
test('manifest resolution selects the handler and agent-carried names cannot redirect it', async () => {
    ...
    registry.register('finance.read_balance', () => true, async (parameters) => {
        ...
        assert.throws(() => { parameters.agent_selected_action = 'mutated'; }, TypeError);
        return 'balance';
    });
...
test('handler exception keeps authority spent and reports an indeterminate effect', async () => {
    ...
    await assert.rejects(gate.runRegistered({
        selector: { protocol: 'mcp', tool: 'change_payee' },
