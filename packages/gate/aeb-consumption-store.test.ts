...
import {
  ...
  AEB_CONSUMPTION_SQL,
  type AebRecoveryClaimAuthorizer,
  type AebRecoveryClaimScope,
  type PostgresAebDurableConsumptionStore,
...
type OperationState = 'RESERVED' | 'CONSUMED' | 'RELEASED_NOT_ENTERED';
type OperationRow = {
  tenantId: string;
...
type DatabaseState = {
  ...
  replays: Map<string, ReplayRow>;
};
type QueryResult = { rowCount: number; rows: Record<string, unknown>[] };

function operationId(tenantId: string, relyingPartyId: string, operationKey: string) {
  return JSON.stringify([tenantId, relyingPartyId, operationKey]);
}

function replayId(tenantId: string, relyingPartyId: string, replayKey: string) {
...
function createDeterministicFakePool() {
  ...
  function maybeFail(statement: string) {
  ...
  return {
    async connect() {
      let transaction: DatabaseState | null = null;
      ...
      return {
        async query(text: string, params: unknown[] = []): Promise<QueryResult> {
          ...
          if (text === AEB_CONSUMPTION_SQL.hasReplayFence) {
            const [tenantId, relyingPartyId, replayKey] = params as string[];
          ...
          if (text === AEB_CONSUMPTION_SQL.operationState) {
            const [tenantId, relyingPartyId, operationKey] = params as string[];
            const active = transaction ?? committed;
          ...
          if (text === AEB_CONSUMPTION_SQL.reserveOperation) {
            ...
            if (transaction.operations.has(id)) return { rowCount: 0, rows: [] };
          ...
          if (text === AEB_CONSUMPTION_SQL.reserveReplayKeys) {
            const [tenantId, relyingPartyId, operationKey, replayKeys] = params as [string, string, string, string[]];
            const inserted: Record<string, unknown>[] = [];
            for (const replayKey of replayKeys) {
              const id = replayId(tenantId, relyingPartyId, replayKey);
              if (transaction.replays.has(id)) continue;
    ...
    replay(tenantId: string, relyingPartyId: string, replayKey: string) {
...
function tokenFactory(prefix: string) {
  let sequence = 0;
  return () => `${prefix}-${String(++sequence).padStart(24, '0')}`;
}

function makeStore(
  pool: ReturnType<typeof createDeterministicFakePool>,
  options: {
    tenantId?: string;
    relyingPartyId?: string;
    tokenPrefix?: string;
...
): PostgresAebDurableConsumptionStore {
...
test('DDL creates namespaced operation and native replay tables with permanent fences', () => {
  ...
  assert.match(
    AEB_CONSUMPTION_DDL,
    /GRANT EXECUTE ON FUNCTION[\s\S]+release_terminal_operation\(TEXT, TEXT, TEXT, TEXT\)\n  TO ep_aeb_executor;/,
  );
...
test('a replay-key insert error rolls back the operation and every replay reservation', async () => {
  ...
  await assert.rejects(
    () => store.reserve('operation-rollback', ['native-a', 'native-b']),
    /pg_unavailable/,
  );
...
test('recovery verifier and PostgreSQL failures propagate without rotating ownership', async () => {
  ...
  await assert.rejects(
    () => verifierUnavailable.claimReservation(row.key, 'kms-token', row.scope),
    /kms_unavailable/,
  );
  ...
  await assert.rejects(
    () => databaseUnavailable.claimReservation(row.key, 'kms-token', row.scope),
    /pg_unavailable/,
  );
  assert.equal(pool.operation('tenant-a', 'rp-a', row.key)?.ownerToken, originalToken);
  assert.equal(await beforeRestart.release(row.key), true);
...
test('replay lookup database failures propagate and malformed answers fail closed', async () => {
  ...
  await assert.rejects(
    () => malformedStore.hasReplayFence('native-malformed'),
    /lookup replay fence: malformed PostgreSQL result/,
  );
});
...
test('a consumed native replay key conflicts under a new operation without reserving that operation', async () => {
  ...
  assert.equal(
    await store.reserve('operation-new', ['native-fresh', 'native-replay']),
...
test('S4 PGX1: a claim without a scope is refused before the authorizer runs, with a reason', async () => {
  ...
  assert.deepEqual(
    await recovery.claimReservationResult(row.key, 'tenant-secret'),
...
test('S4: a malformed or mismatched scope is a refusal with a reason, never a throw', async () => {
  ...
  for (const [label, scope, reason] of [
    ...
    assert.deepEqual(
      await recovery.claimReservationResult(row.key, 'approved', scope as any),
...
test('S4 PGX3: a native and a composed attempt with the same attempt ID never share a recovery credential', async () => {
  ...
  const nativeScope: AebRecoveryClaimScope = {
  ...
  assert.deepEqual(
    await recovery.claimReservationResult(composedHolder, 'credential:native-attempt', composedScope),
  ...
  assert.deepEqual(
    await recovery.claimReservationResult(composedHolder, 'credential:native-attempt', nativeScope),
    { claimed: false, reason: 'recovery_claim_key_mismatch' },
  );
  assert.deepEqual(
    await recovery.claimReservationResult(
      composedHolder,
      'credential:native-attempt',
      { ...composedScope, boundary: 'native' },
    ),
    { claimed: false, reason: 'recovery_claim_key_mismatch' },
  );
  ...
  assert.deepEqual(
    await recovery.claimReservationResult(nativeHolder, 'credential:native-attempt', nativeScope),
    { claimed: true },
  );
...
test('S4: the composed evaluation row is claimable only while the attempt holds its holder, with a reason otherwise', async () => {
  ...
  assert.deepEqual(
    await recovery.claimReservationResult(evaluationKey, 'approved', scope),
    { claimed: false, reason: 'recovery_claim_owner_marker_absent' },
  );
  ...
  assert.deepEqual(
    await recovery.claimReservationResult(evaluationKey, 'approved', scope),
    { claimed: false, reason: 'recovery_claim_already_owned' },
  );
