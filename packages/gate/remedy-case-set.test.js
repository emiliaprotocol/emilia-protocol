...
// Generated from remedy-case-set.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
import assert from 'node:assert/strict';
import crypto from 'node:crypto';
import test from 'node:test';
import { createRemedyMemoryStore, createRemedyProgramKernel, } from './remedy-program.js';
import { expectedRemedyProgramReceiptBindings, issueRemedyProgramReceipt, } from './remedy-program-receipt.js';
...
function verifierKernel() {
    return createRemedyProgramKernel({
        ...
        verifyRemedyAuthorization: (input) => ({
            ok: true,
            ...input.authorization,
...
async function childCase({ suffix, unit, units, destinationBindingDigest, outcome = 'executed', }) {
...
class DurableCaseSetStore {
    ...
    async compareAndSwap({ tenantId, caseSetId, expectedRevision, ownerTokenDigest, state, }) {
        const key = this.key(tenantId, caseSetId);
        const current = this.records.get(key);
        if (!current)
            return { ok: false, reason: 'case_set_not_found' };
...
async function signedChild(child, receiptKey) {
    const context = {
...
function coordinator(store, receiptKey) {
