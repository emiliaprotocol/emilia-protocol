...
import {
  ...
  type ProposalToEffectPostgresPool,
  type ProposalToEffectPostgresRecoveryAuthorization,
...
function binding(overrides: Partial<ConsequenceAttemptBinding> = {}): ConsequenceAttemptBinding {
  return {
...
function evidence(
  ...
  outcome: AuthenticatedProviderEvidenceBinding['outcome'] = 'COMMITTED',
  overrides: Partial<AuthenticatedProviderEvidenceBinding> = {},
): AuthenticatedProviderEvidenceBinding {
  return {
    ...attempt,
...
type StoredAttempt = ConsequenceAttemptBinding & {
  ...
  evidence_binding_digest: string | null;
...
function fakePostgres() {
  ...
  let evidenceRows = new Map<string, {
    tenant_id: string;
    provider_id: string;
  ...
  const queryPrincipals: string[] = [];
  const discardedConnections: Error[] = [];
  ...
  const namespaceKey = (
    ...
    environment: string,
    attemptId: string,
  ) => JSON.stringify([tenantId, providerId, providerAccountId, environment, attemptId]);
  const evidenceKey = (
    tenantId: string,
    providerId: string,
    providerAccountId: string,
    environment: string,
    value: string,
  ) => JSON.stringify([tenantId, providerId, providerAccountId, environment, value]);
  const cloneAttempts = () => new Map(
    [...attempts].map(([key, value]) => [key, structuredClone(value)]),
  );
  const cloneEvidence = () => new Map(
    [...evidenceRows].map(([key, value]) => [key, structuredClone(value)]),
  );
  const instant = (value: number) => new Date(value).toISOString();
  const success = (applied: boolean, reason: string | null = null) => ({
    rowCount: 1,
    rows: [{ applied, reason }],
  });

  const makePool = (principal: 'executor' | 'recovery'): ProposalToEffectPostgresPool => ({
    async connect() {
      let snapshot: {
      ...
      return {
        async query(text: string, params: readonly unknown[] = []) {
          ...
          let result: { rowCount: number; rows: Array<Record<string, unknown>> };
          if (text === PROPOSAL_TO_EFFECT_POSTGRES_SQL.reserve) {
            const [
              tenantId, providerId, providerAccountId, environment, attemptId,
              operationDigest, requestDigest, actionDigest, configDigest,
              attemptDigest, ownerDigest, leaseSeconds,
            ...
            const bindingConflict = [...attempts.entries()].some(([storedKey, stored]) => (
              storedKey.startsWith(namespacePrefix)
            ...
            if (sameReservation) {
              ...
              attempts.set(key, {
                ...
                request_digest: requestDigest as ConsequenceAttemptBinding['request_digest'],
            ...
            const [
              tenantId, attemptId, ownerDigest, expectedState, nextState, leaseSeconds,
            ] = params as [string, string, string, string, string, number];
            const record = [...attempts.values()].find((candidate) => (
              candidate.tenant_id === tenantId
              && candidate.attempt_id === attemptId
              && candidate.owner_digest === ownerDigest
            ));
            const allowed = (
              (expectedState === 'RESERVED' && nextState === 'INVOKING')
            ...
            if (applied && record!.state === expectedState) {
              record!.state = nextState as StoredAttempt['state'];
            ...
            const [tenantId, attemptId, ownerDigest, leaseSeconds] = params as [
            ...
            const [
              tenantId, providerId, providerAccountId, environment, attemptId,
              ownerDigest, operationDigest, requestDigest, actionDigest, configDigest,
              attemptDigest, operationId, caid, nextState, evidenceId, observedAt,
              outcome, providerEvidenceDigest, evidenceBindingDigest,
            ] = params as string[];
            const record = attempts.get(namespaceKey(
              tenantId!, providerId!, providerAccountId!, environment!, attemptId!,
            ));
            const mappedState = outcome === 'COMMITTED'
              ? 'COMMITTED' : outcome === 'NOT_COMMITTED' ? 'RELEASED' : 'ESCALATED';
            const idKey = evidenceKey(
              tenantId!, providerId!, providerAccountId!, environment!, evidenceId!,
            );
            const digestKey = evidenceKey(
              tenantId!, providerId!, providerAccountId!, environment!, providerEvidenceDigest!,
            );
            const exactEvidence = [...evidenceRows.values()].find((stored) => (
            ...
            const applied = Boolean(
              ...
              && ![...evidenceRows.values()].some((stored) => (
                evidenceKey(
                  stored.tenant_id, stored.provider_id, stored.provider_account_id,
                  stored.environment, stored.evidence_id,
                ) === idKey
                || evidenceKey(
                  stored.tenant_id, stored.provider_id, stored.provider_account_id,
                  stored.environment, stored.evidence_digest,
                ) === digestKey
              )),
            );
            if (applied) {
            ...
            const [
              tenantId, providerId, providerAccountId, environment, attemptId,
              requestDigest, attemptDigest, ownerGeneration, expectedState,
              expectedLeaseExpiresAt, nextOwnerDigest, leaseSeconds,
            ] = params as [string, string, string, string, string, string, string,
              number, StoredAttempt['state'], string, string, number];
            const record = attempts.get(namespaceKey(
              tenantId, providerId, providerAccountId, environment, attemptId,
            ));
        ...
        release(error?: Error) {
          if (error) discardedConnections.push(error);
        },
      };
    },
  });
...
async function reserveOwner(
  store: ReturnType<typeof createProposalToEffectPostgresStore>,
  attempt = binding(),
): Promise<ConsequenceAttemptOwnerHandle> {
  const reserved = await store.reserve(attempt);
  assert.equal(reserved.reserved, true);
  return reserved.owner;
}

async function makeIndeterminate(
  store: ReturnType<typeof createProposalToEffectPostgresStore>,
  attempt = binding(),
) {
...
test('secure DDL separates database roles, tenant principals, and recovery authority', () => {
  ...
  assert.match(
    PROPOSAL_TO_EFFECT_POSTGRES_DDL,
    /recover_attempt\([\s\S]*\) TO proposal_to_effect_recovery;/,
  );
  assert.doesNotMatch(PROPOSAL_TO_EFFECT_POSTGRES_DDL, /TO service_role/);
  assert.match(
    PROPOSAL_TO_EFFECT_POSTGRES_DDL,
    /PRIMARY KEY \(\s*tenant_id, provider_id, provider_account_id, environment, attempt_id\s*\)/,
  );
  for (const column of [
  ...
  assert.equal(
    PROPOSAL_TO_EFFECT_POSTGRES_DDL.match(/HH24:MI:SS\.US/g)?.length,
    2,
    'recovery snapshots must preserve PostgreSQL microseconds for exact lease fencing',
  );
  assert.doesNotMatch(PROPOSAL_TO_EFFECT_POSTGRES_DDL, /HH24:MI:SS\.MS/);
  for (const column of ['operation_id', 'caid', 'action_digest']) {
    assert.match(
      PROPOSAL_TO_EFFECT_POSTGRES_DDL,
      new RegExp(`provider_evidence[\\s\\S]*\\b${column}\\b`),
    );
  }
});
...
test('reserve is namespaced, one-winner, and never sends the opaque owner to PostgreSQL', async () => {
  ...
  const results = await Promise.all(
    Array.from({ length: 24 }, () => store.reserve(attempt)),
  );
  ...
  assert.equal(
    pg.queryParameters.some((params) => params.includes(winner.owner)),
    false,
    'only the keyed owner digest may cross the database boundary',
  );
  const stored = pg.attempts.get(pg.namespaceKey(
    attempt.tenant_id,
    attempt.provider_id,
...
test('concurrent exact reconciliation is idempotent and persists one bound evidence row', async () => {
  ...
  const results = await Promise.all(
    Array.from({ length: 24 }, () => store.reconcile({
      tenant_id: attempt.tenant_id,
      attempt_id: attempt.attempt_id,
      owner,
      expected_state: 'INDETERMINATE',
...
test('restart recovery requires server authorization, rotates custody, and fences stale owners', async () => {
  ...
  const seen: ProposalToEffectPostgresRecoveryAuthorization[] = [];
  ...
  assert.ok(stored);
  stored.last_heartbeat_at = stored.last_heartbeat_at.replace(
    /(\.\d{3})Z$/,
    (_match, milliseconds) => `${milliseconds}123Z`,
  );
  stored.lease_expires_at = stored.lease_expires_at.replace(
    /(\.\d{3})Z$/,
    (_match, milliseconds) => `${milliseconds}456Z`,
  );
  ...
  assert.ok(terminal);
  const {
    last_heartbeat_at: terminalHeartbeat,
    lease_expires_at: terminalLease,
    lease_stale: terminalLeaseStale,
    ...terminalBinding
  } = terminal;
