...
import {
  ...
  FIELD_ORIGIN_EVIDENCE_V2_VERSION,
  FIELD_ORIGIN_EVIDENCE_V2_DOMAIN,
  FIELD_ORIGIN_V2_REQUIRED_ALGORITHMS,
  fieldOriginV2SignedPayload,
  signFieldOriginEvidence,
  signFieldOriginEvidenceV2,
  verifyFieldOriginEvidence,
  verifyFieldOriginEvidenceV2,
...
function rule(path: string, allowedOrigins: string[]) {
  return {
...
test('v1 byte-identity regression: a v1 artifact verifies and re-signs identically', () => {
  const a: any = signFieldOriginEvidence(input(), { issuer_id: RP, key_id: KEY_ID, private_key: ed.privateKey });
  const b: any = signFieldOriginEvidence(input(), { issuer_id: RP, key_id: KEY_ID, private_key: ed.privateKey });
...
test('the v2 verifier refuses a v1 artifact on the version marker', async () => {
  const v1: any = signFieldOriginEvidence(input(), { issuer_id: RP, key_id: KEY_ID, private_key: ed.privateKey });
  const res = await verifyFieldOriginEvidenceV2(v1, ctx());
  assert.equal(res.accepted, false);
  assert.ok(/unsupported_version/.test(res.reason as string), res.reason as string);
});
...
test('SET NARROWING: narrowing required_algorithms fails BOTH structurally and cryptographically', async () => {
  ...
  const narrowedBytes = Buffer.from(
    FIELD_ORIGIN_EVIDENCE_V2_DOMAIN + canonicalize({ ...bodyNoProof, required_algorithms: ['Ed25519'] }),
    'utf8',
  );
...
test('SWAPPED LEGS: the ML-DSA signature relabelled as Ed25519 refuses', async () => {
  ...
  const res = await verifyFieldOriginEvidenceV2(stmt, ctx());
  assert.equal(res.accepted, false);
  assert.ok(/field_origin_signature_invalid/.test(res.reason as string), res.reason as string);
});
