...
// Generated from saas.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
...
import { createVercelManifest, guardVercelMutation, VERCEL_OPS, secretValueDigest, SECRET_VALUE_BINDING_VERSION, } from './vercel.js';
...
test('vercel: env.set refused without receipt, runs with Class-A, drift refused', async () => {
    ...
    await assert.rejects(() => guardVercelMutation(gate, client, { op: 'env.set', params: { project: 'app', key: 'OTHER', target: 'production', value: 's' }, receipt: harness.mint({ outcome: A }) }), (e) => /binding/.test(e.gate.reason));
});
test('vercel: promote-to-production requires quorum', async () => {
    ...
    await assert.rejects(() => guardVercelMutation(gate, client, { op: 'deploy.promote', params: { project: 'app', deployment_id: 'dpl_1' }, receipt: harness.mint({ outcome: A }) }), (e) => /assurance/.test(e.gate.reason));
...
test('linear: bulk delete binds the query hash', async () => {
    ...
    await assert.rejects(() => guardLinearMutation(gate, client, { op: 'issue.bulk_delete', params: { team: 'ENG', query: 'state = Backlog' }, receipt: harness.mint({ outcome: A, extra: { query_hash: qhash } }) }), (e) => /binding/.test(e.gate.reason));
    void action;
});
...
test('salesforce: permission-set assign requires quorum; bulk delete binds SOQL', async () => {
    ...
    await assert.rejects(() => guardSalesforceMutation(gate, client, { op: 'permission_set.assign', params: { user: 'u1', permission_set: 'Admin' }, receipt: harness.mint({ outcome: A }) }), (e) => /assurance/.test(e.gate.reason));
