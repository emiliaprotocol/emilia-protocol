...
// Generated from cloud.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
...
test('k8s: secret delete refused without receipt, runs with Class-A, drift refused', async () => {
    ...
    await assert.rejects(() => guardK8sMutation(gate, client, { op: 'secret.delete', params: { namespace: 'prod', name: 'OTHER' }, receipt: harness.mint({ outcome: A }) }), (e) => /binding/.test(e.gate.reason));
});
test('k8s: namespace delete requires quorum', async () => {
    ...
    await assert.rejects(() => guardK8sMutation(gate, client, { op: 'namespace.delete', params: { namespace: 'prod' }, receipt: harness.mint({ outcome: A }) }), (e) => /assurance/.test(e.gate.reason));
...
test('terraform: destroy binds the plan hash and requires quorum', async () => {
    ...
    await assert.rejects(() => guardTerraformMutation(gate, runner, { op: 'apply.destroy', params: { workspace: 'prod', plan_hash: 'sha256:plan1' }, receipt: harness.mint({ outcome: A }) }), (e) => /assurance/.test(e.gate.reason));
    ...
    await assert.rejects(() => guardTerraformMutation(gate, runner, { op: 'apply.destroy', params: { workspace: 'prod', plan_hash: 'sha256:FULL_TEARDOWN' }, receipt: harness.mint({ outcome: A, ...Q }) }), (e) => /binding/.test(e.gate.reason));
});
test('gcp: bucket delete (Class-A) runs; IAM set-policy requires quorum', async () => {
    ...
    await assert.rejects(() => guardGcpMutation(g2.gate, client, { op: 'iam.set_policy', params: { resource: 'proj/x', member: 'user:m@x', role: 'roles/owner' }, receipt: g2.harness.mint({ outcome: A }) }), (e) => /assurance/.test(e.gate.reason));
