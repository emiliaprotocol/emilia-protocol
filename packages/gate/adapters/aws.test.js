...
// Generated from aws.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
...
function setup(action) {
    ...
    return { harness, gate: createGate({ manifest: createAwsManifest(), trustedKeys: [harness.publicKey], approverKeys: harness.approverKeys, quorumPolicy: harness.quorumPolicy, rpId: harness.rpId, allowedOrigins: harness.allowedOrigins, allowEphemeralStore: true }), aws: fakeAws() };
}
...
test('attach-policy requires quorum and never reaches AWS without it', async () => {
    ...
    await assert.rejects(() => guardAwsMutation(gate, aws, { op: 'iam.attach_policy', params, receipt: harness.mint({ outcome: 'allow_with_signoff' }) }), (e) => /assurance/.test(e.gate.reason));
...
test('create-access-key (Class-A) runs with a valid signoff and refuses a different user (drift)', async () => {
    ...
    await assert.rejects(() => guardAwsMutation(gate, aws, { op: 'iam.create_access_key', params: { user: 'root' }, receipt: harness.mint({ outcome: 'allow_with_signoff' }) }), (e) => /binding/.test(e.gate.reason));
});
