...
// Generated from remedy-program-receipt-v2.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
//
...
function remedyAttempt(overrides = {}) {
    return {
...
function remedyState(overrides = {}) {
    return {
...
async function issued(state = remedyState()) {
    ...
    const receipt = await issueRemedyProgramReceiptV2({ state, remedyOperationId: 'refund-op-1' }, { context: CONTEXT, keys: issuer.keys, allowEphemeralState: true });
    const expected = expectedRemedyProgramReceiptV2Bindings(state, 'refund-op-1');
    return { issuer, receipt, state, expected };
}
function verifyOptions(fixture, over = {}) {
    return {
...
describe('EP-ACTION-REMEDY-RECEIPT-v2 hybrid remedy receipt', () => {
    it('valid v2 roundtrip: both legs verify under the pinned key pair', async () => {
        ...
        assert.deepEqual(fixture.receipt.signature.required_algorithms, [...ACTION_REMEDY_RECEIPT_V2_REQUIRED_ALGORITHMS]);
        assert.deepEqual(fixture.receipt.signature.signatures.map((s) => s.alg), [...ACTION_REMEDY_RECEIPT_V2_REQUIRED_ALGORITHMS]);
    ...
    it('the v1 receipt still refuses a v2-profile state snapshot', async () => {
        ...
        await assert.rejects(issueRemedyProgramReceipt({ state: remedyState({ version: REMEDY_PROGRAM_PROFILE_V2_VERSION }), remedyOperationId: 'refund-op-1' }, { context: CONTEXT, privateKey: keys.privateKey, allowEphemeralState: true }), /state snapshot is invalid/);
    });
    ...
    it('the router keeps the exact v1 verdict for a v1 receipt and hybrid-checks a v2 one', async () => {
        ...
        const v1 = await issueRemedyProgramReceipt({ state, remedyOperationId: 'refund-op-1' }, { context: CONTEXT, privateKey: keys.privateKey, allowEphemeralState: true });
        ...
        const fixture = await issued();
        assert.equal((await verifyRemedyProgramReceiptStatement(fixture.receipt, verifyOptions(fixture))).valid, true);
    });
    it('a key id pinned WITHOUT the ML-DSA half never satisfies a v2 pin', async () => {
        const fixture = await issued();
        const result = await verifyRemedyProgramReceiptV2(fixture.receipt, verifyOptions(fixture, { trustedKeys: fixture.issuer.classicalPin }));
        assert.equal(result.valid, false);
    ...
    it('stripped leg: dropping the ML-DSA signature refuses, never a classical-only pass', async () => {
        ...
        const stripped = {
            ...
            signature: {
                ...
                signatures: fixture.receipt.signature.signatures.filter((s) => s.alg !== 'ML-DSA-65'),
            },
        };
    ...
    it('narrowed set: claiming required_algorithms=["Ed25519"] is refused structurally AND cryptographically', async () => {
        ...
        const narrowed = {
            ...
            signature: {
                ...
                signatures: fixture.receipt.signature.signatures.filter((s) => s.alg === 'Ed25519'),
            },
        };
        assert.equal((await verifyRemedyProgramReceiptV2(narrowed, verifyOptions(fixture))).reason, 'receipt_algorithm_set_unsupported');
        const setIntact = {
            ...narrowed,
            signature: {
                ...narrowed.signature,
        ...
        assert.equal((await verifyRemedyProgramReceiptV2(setIntact, verifyOptions(fixture))).reason, 'receipt_signature_leg_missing');
    });
    it('wrong-length signature on the ML-DSA leg refuses, never crashes', async () => {
        ...
        const tampered = {
            ...
            signature: {
                ...fixture.receipt.signature,
                signatures: fixture.receipt.signature.signatures.map((s) => (s.alg === 'ML-DSA-65' ? { ...s, sig: crypto.randomBytes(16).toString('base64url') } : s)),
            },
        };
    ...
    it('an absent ML-DSA backend is pq_backend_unavailable, never a pass on the Ed25519 leg', async () => {
        const fixture = await issued();
        const result = await verifyRemedyProgramReceiptV2(fixture.receipt, verifyOptions(fixture, { mldsaBackend: {} }));
        assert.equal(result.valid, false);
...
describe('EP-ACTION-REMEDY-RECEIPT-v2 FIPS consult and custody gates', () => {
    ...
    it('a denied FIPS policy refuses issuance BEFORE the signer runs, named reason, never a silent sign', async () => {
        ...
        await assert.rejects(issueRemedyProgramReceiptV2({ state: remedyState(), remedyOperationId: 'refund-op-1' }, { context: CONTEXT, signer, allowEphemeralState: true, fipsPosture: DENYING_POSTURE }), /fips_policy_denied:Ed25519:ed25519_boundary_undeclared/);
        assert.equal(signCalls, 0);
    });
    it('an inactive posture (the normal case) permits BOTH legs with no acknowledgment required', async () => {
        const issuer = issuerFixture();
        const inactive = { ...DENYING_POSTURE, fips_status: 'inactive', fips_mode_active: false };
        const receipt = await issueRemedyProgramReceiptV2({ state: remedyState(), remedyOperationId: 'refund-op-1' }, { context: CONTEXT, keys: issuer.keys, allowEphemeralState: true, fipsPosture: inactive });
    ...
    it('an ACTIVE posture refuses the ML-DSA leg until the unvalidated implementation is acknowledged', async () => {
        const issuer = issuerFixture();
        const active = { ...DENYING_POSTURE, ed25519_in_validated_boundary: true };
        await assert.rejects(issueRemedyProgramReceiptV2({ state: remedyState(), remedyOperationId: 'refund-op-1' }, { context: CONTEXT, keys: issuer.keys, allowEphemeralState: true, fipsPosture: active }), /fips_policy_denied:ML-DSA-65:mldsa_implementation_unvalidated/);
        const receipt = await issueRemedyProgramReceiptV2({ state: remedyState(), remedyOperationId: 'refund-op-1' }, {
            context: CONTEXT,
            keys: issuer.keys,
    ...
    it('local keys without the ephemeral opt-in are refused, and a software signSet signer is refused in production mode', async () => {
        const issuer = issuerFixture();
        await assert.rejects(issueRemedyProgramReceiptV2({ state: remedyState(), remedyOperationId: 'refund-op-1' }, { context: CONTEXT, keys: issuer.keys }), /requires an external signSet signer/);
        const signer = {
            keyId: CONTEXT.key_id,
            custody: 'software',
            publicKeys: issuer.hybridPin[CONTEXT.key_id],
        ...
        await assert.rejects(issueRemedyProgramReceiptV2({ state: remedyState(), remedyOperationId: 'refund-op-1' }, { context: CONTEXT, signer }), /custody must be kms or hsm/);
    });
    it('an injected signSet signer that supplies both legs issues a verifiable receipt', async () => {
        const issuer = issuerFixture();
        const pqSecret = issuer.keys.pq.secretKey;
        const signer = {
            keyId: CONTEXT.key_id,
            custody: 'hsm',
            publicKeys: issuer.hybridPin[CONTEXT.key_id],
        ...
        const state = remedyState();
        const receipt = await issueRemedyProgramReceiptV2({ state, remedyOperationId: 'refund-op-1' }, { context: CONTEXT, signer });
    ...
    it('an injected signer returning an incomplete set refuses issuance, never emits a one-leg receipt', async () => {
        ...
        await assert.rejects(issueRemedyProgramReceiptV2({ state: remedyState(), remedyOperationId: 'refund-op-1' }, { context: CONTEXT, signer }), /malformed signature set/);
    });
});
