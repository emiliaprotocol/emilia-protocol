...
// Generated from bounded-execution-report-v2.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
//
...
import { BOUNDED_EXECUTION_REPORT_V2_VERSION, boundedExecutionOccurrenceInventoryDigest, boundedExecutionRuntimeStateDigest, signBoundedExecutionReport, signBoundedExecutionReportV2, verifyBoundedExecutionReport, verifyBoundedExecutionReportV2, } from './bounded-execution-report.js';
...
function reporterMaterial(issuerId, keyId) {
    const pair = generateKeyPairSync('ed25519');
...
function programInput() {
    return {
        ...
        nodes: [
            ...
            { node_id: 'notify', action: { mode: 'exact', caid: C('D'), action_digest: D('d') }, trust_program_digest: D('7'), depends_on: [{ node_id: 'verify', outcomes: ['COMMITTED'] }], max_occurrences: 1, charges: [{ budget_id: 'attempts', amount: 1 }] },
            { node_id: 'remediate', action: { mode: 'exact', caid: C('B'), action_digest: D('b') }, trust_program_digest: D('5'), depends_on: [{ node_id: 'inspect', outcomes: ['COMMITTED'] }], max_occurrences: 2, charges: [{ budget_id: 'attempts', amount: 1 }] },
            { node_id: 'verify', action: { mode: 'exact', caid: C('C'), action_digest: D('c') }, trust_program_digest: D('6'), depends_on: [{ node_id: 'remediate', outcomes: ['COMMITTED'] }], max_occurrences: 2, charges: [{ budget_id: 'attempts', amount: 1 }] },
        ],
    };
}
...
test('a real hybrid report verifies under both pinned keys', async () => {
    ...
    const res = await verifyBoundedExecutionReportV2(artifact, v2Context);
...
test('the v1 verifier refuses a v2 report (version/envelope marker)', async () => {
    ...
    const res = verifyBoundedExecutionReport(artifact, v1Context);
...
test('v1 signing is byte-identical and still verifies (regression)', () => {
    ...
    const a = signBoundedExecutionReport(h.input, v1Signer);
    const b = signBoundedExecutionReport(h.input, v1Signer);
    assert.equal(a.proof.algorithm, 'Ed25519');
    assert.equal(a.proof.signature_b64u, b.proof.signature_b64u);
    const res = verifyBoundedExecutionReport(a, h.v1Context);
    assert.equal(res.accepted, true, res.reason ?? '');
});
// --- anti-stripping -----------------------------------------------------------
test('LEG STRIPPING: removing the ML-DSA leg refuses structurally', async () => {
    ...
    const res = await verifyBoundedExecutionReportV2(a, v2Context);
    assert.equal(res.accepted, false);
...
test('ED448 MASQUERADE refuses', async () => {
    ...
    const res = await verifyBoundedExecutionReportV2(artifact, ctx);
    assert.equal(res.accepted, false);
...
test('expected report-id mismatch refuses on context', async () => {
    const { artifact, v2Context } = await harnessV2();
    const res = await verifyBoundedExecutionReportV2(artifact, { ...v2Context, expected_report_id: 'report:other' });
    assert.equal(res.accepted, false);
    assert.equal(res.reason, 'report_id_mismatch');
});
test('stale report refuses', async () => {
    const { artifact, v2Context } = await harnessV2();
    const res = await verifyBoundedExecutionReportV2(artifact, { ...v2Context, now: '2026-07-29T21:30:00.000Z' });
    assert.equal(res.accepted, false);
...
test('NO ML-DSA BACKEND is a refusal, never a pass on the classical leg', async () => {
    const { artifact, v2Context } = await harnessV2();
    const res = await verifyBoundedExecutionReportV2(artifact, v2Context, { mldsaBackendLoader: async () => null });
    assert.equal(res.accepted, false);
...
test('malformed input refuses without throwing', async () => {
    ...
    for (const junk of [null, undefined, 'x', 42, [], {}]) {
        const res = await verifyBoundedExecutionReportV2(junk, v2Context);
        assert.equal(res.accepted, false);
    }
});
