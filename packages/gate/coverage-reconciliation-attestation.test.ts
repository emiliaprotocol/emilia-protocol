...
test('refuses the v1 effect-claim outcome names under the v2 contract', () => {
  const { pair } = fixture();
  const legacyNames = coverageInput() as any;
...
test('receipt census suppresses small cells and refuses raw payload fields', () => {
  ...
  } as any, taxonomy), /field|bucket/i);
...
test('receipt census uses one UTF-8 byte order for mixed case and punctuation', () => {
  ...
  const census = createReceiptCensus(input, taxonomy);
  assert.deepEqual(census.buckets.map((bucket: any) => bucket.action_class), ['Z', 'a', 'a.b']);
  assert.equal(validateReceiptCensus(census, taxonomy).valid, true);
});
...
test('checked-in coverage and census vector is deterministic and verifies', () => {
  const vector = JSON.parse(readFileSync(fileURLToPath(new URL(
    '../../conformance/vectors/coverage-reconciliation.v2.json', import.meta.url,
  )), 'utf8'));
