...
import {
  SHEESH_CONTEXT_FRAME_PROFILE,
  SHEESH_PROVIDER_ID,
  SHEESH_SOURCE_PROFILE,
  createSheeshContextProvider,
  createSheeshMemoryProjection,
} from './sheesh-context.js';
import {
  ZEP_CONTEXT_FRAME_PROFILE,
  ZEP_PROVIDER_ID,
  ZEP_SOURCE_PROFILE,
...
function spki(key: typeof ADAPTER.publicKey): string {
...
function verifyFull(output: ReturnType<typeof createSheeshMemoryProjection> | ReturnType<typeof createZepMemoryProjection>, expectedSourceProfile: string, expectedContextFrameProfile: string) {
  return verifyMemoryProjectionRecordV1(
    output.record,
    output.verificationMaterial,
    {
...
test('SHEESH adapter refuses traversal paths and detects changed source bytes', () => {
  ...
  assert.throws(
    ...
    /source object 0 does not match its commitment/,
  );
});

test('a deployment-owned SHEESH verifier can make a bounded trust assertion and is rerun by full verification', () => {
  let verifierRuns = 0;
  const trustedSource = ({ sealedObjectDigest }: { sealedObjectDigest: string }): MemoryProjectionNativeSourceResult => {
    verifierRuns += 1;
    return {
      valid: true,
      formatVersion: 1,
...
test('Zep adapter binds graph identity, episode identity, raw result bytes, and projection bytes', () => {
  ...
  assert.throws(
    () => verifyFull(output, ZEP_SOURCE_PROFILE, ZEP_CONTEXT_FRAME_PROFILE),
    /fragment 0 does not match its commitment/,
  );
});
...
test('Gate pins provider and profile, binds one exact action, and never treats context as authorization', () => {
  ...
  assert.equal(
    wrongProvider.verifyProjection(output.record, {
      verificationTime: VERIFY_AT,
