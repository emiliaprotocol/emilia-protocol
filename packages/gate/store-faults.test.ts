...
test('fault: response loss after commit stays consumed', async () => {
  ...
  const backend = {
    ...
    async compareAndSet(...args) {
      ...
      if (applied) throw new Error('connection_lost_after_commit');
      return applied;
    },
  };
...
test('fault: stale owner cannot erase or commit a replacement owner', async () => {
  ...
  assert.equal(await backend.compareAndSet(
    'receipt-fence',
    'reserved:v2:stale-owner-token-000001',
    'reserved:v2:new-owner-token-0000001',
  ), true);
