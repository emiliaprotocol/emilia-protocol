...
function signEvidence(
  signer: ReturnType<typeof evidenceHarness>['signer'],
  action: Record<string, unknown>,
...
test('signs and verifies exact per-field origin plus snapshot evidence', () => {
  ...
  assert.equal(artifact.fields.find((field: any) => field.path === '/memo').origin_class, 'untrusted_bounded');
  assert.deepEqual(
    artifact.fields.find((field: any) => field.path === '/memo').snapshot,
    immutable(),
  );
...
test('keeps mutable-state snapshot time separate from field origin', () => {
  ...
  assert.equal(
    verifyFieldOriginEvidence(stale, context).reason,
    'field_origin_snapshot_stale:/memo',
  );
  ...
  assert.equal(
    verifyFieldOriginEvidence(snapshotAfterEvidence, {
      ...context,
      now: '2026-08-15T22:32:00.000Z',
    }).reason,
    'field_origin_snapshot_after_evidence:/memo',
  );
});
...
test('Gate names all five hostile field-origin refusals and admits bounded untrusted data', async () => {
  ...
  const injectedEmail = await runCase(
    BASE_ACTION,
    annotationsFor(BASE_ACTION, {
  ...
  const webpageTarget = await runCase(
    BASE_ACTION,
    annotationsFor(BASE_ACTION, {
  ...
  const transformSubstitution = await runCase(
    BASE_ACTION,
    annotationsFor(BASE_ACTION, {
      '/new_account_digest': {
  ...
  const profileDowngrade = await runCase(
    BASE_ACTION,
    annotationsFor(BASE_ACTION, {
      '/vendor_id': { origin_class: 'untrusted_bounded' },
    }),
  ...
  assert.equal(
    positive.result.authorization.evidence.field_origin_program_binding.node_id,
    executionProgram.node_id,
  );
...
test('ORIGIN-LABELS-v1 is a closed versioned vocabulary with one definition per label', () => {
  ...
  for (const target of Object.values(ORIGIN_LABELS_V01_PROFILE_MAP)) {
    assert.ok((ORIGIN_LABELS as readonly string[]).includes(target));
  }
});
...
test('originLabelTrustFloor is taint-preserving and never upgrades a derivation', () => {
  ...
  assert.deepEqual(
    originLabelTrustFloor('derived', ['operator-config', 'user-stated']),
    { floor: 'user-stated', reason: null },
  );
  assert.deepEqual(
    originLabelTrustFloor('derived', ['operator-config', 'retrieved-untrusted']),
    { floor: 'retrieved-untrusted', reason: null },
  );
  assert.deepEqual(
    originLabelTrustFloor('derived', ['model-generated', 'counterparty-document']),
    { floor: 'model-generated', reason: null },
  );
  ...
  assert.deepEqual(
    originLabelTrustFloor('derived', ['user-stated', 'user-stated']),
    { floor: null, reason: 'derivation_source_invalid' },
  );
...
test('every ORIGIN-LABELS-v1 conformance vector reproduces its pinned result', () => {
  const vectorsPath = join(
    ...
    '..', '..', 'conformance', 'origin-labels', 'vectors.json',
  );
  ...
  const kinds = new Set(pack.cases.map((entry: any) => entry.kind));
