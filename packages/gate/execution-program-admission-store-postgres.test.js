...
// Generated from execution-program-admission-store-postgres.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
...
import { ADMISSION_POSTGRES_SQL, createAdmissionPostgresStore, } from './admission-store-postgres.js';
...
import { EXECUTION_PROGRAM_RUNTIME_VERSION, EXECUTION_PROGRAM_REPORT_SNAPSHOT_VERSION, createAdmissionSnapshot, createExecutionProgramAdmissionBinding, executionProgramReportSnapshotMarker, } from './admission-store.js';
...
function admissionInput(admissionId, operationId, now, node, authorizationDigest, subjectId, expiresAt = iso(now + 5 * 60_000)) {
...
test('PostgreSQL adapter exposes the complete durable execution-program surface', async () => {
    ...
    const executionProgramVerifierQuery = async (text, params) => {
...
test('prepared reservation snapshots the exact owner token before any awaited verifier work', async () => {
    ...
    const input = {
        ...
        admission: admissionInput('admission:prepared-owner-token', 'operation:prepared-owner-token', Date.parse(NOW), verified.program.nodes[0], context.expected_authorization_digest, verified.program.subject_id),
...
test('execution-program assertion RPCs require a distinct verifier-service query', async () => {
    ...
    await assert.rejects(store.registerExecutionProgram(artifact, context), /executionProgramVerifierQuery is required/);
});
...
test('real PostgreSQL proves the complete execution-program lifecycle atomically', {
    ...
    const ownerQuery = async (text, params) => {
        const result = await pool.query(text, [...params]);
        return { rowCount: result.rowCount ?? 0, rows: result.rows };
    };
    const queryAs = (role) => (async (text, params) => {
    ...
    const inputFor = (suffix, nodeId, authorization = authorizationDigest, expiry) => admissionInput(`admission:program:${suffix}`, `operation:program:${suffix}`, Date.now(), node(nodeId), authorization, subjectId, expiry);
    const cas = (reserved) => ({
        tenant_id: 'tenant:gate-v2',
    ...
    async function reserveProgram(suffix, nodeId, expiry) {
    ...
    END $$`);
        ...
        for (const signature of assertionSignatures) {
            ...
            const privilege = await pool.query(`SELECT has_function_privilege('gate_program_runtime', $1, 'EXECUTE') AS runtime,
          has_function_privilege('gate_program_verifier', $1, 'EXECUTE') AS verifier`, [signature]);
      ...
      SET maximum_observation_age_ms = 300000 WHERE deployment_id = $1`, ['deployment:gate-v2']);
        ...
        const unrelatedOrdinary = inputFor('ordinary-unrelated', 'inspect', digestOf('unrelated-ordinary-authorization'));
        ...
        if (!unrelatedReserved.ok)
            assert.fail(unrelatedReserved.reason);
      ...
      public.ep_gate_execution_program_expected_binding(
        ...
        if (!inspectReserved.ok)
            assert.fail(inspectReserved.reason);
        ...
        assert.deepEqual(inspectReserved.snapshot.body.resource_reservations.filter((resource) => resource.kind === 'execution_program'), [expectedBinding]);
        ...
        if (!indeterminate.ok)
            assert.fail(indeterminate.reason);
        ...
        if (!remediateReserved.ok)
            assert.fail(remediateReserved.reason);
        ...
        if (!remediateBegun.ok)
            assert.fail(remediateBegun.reason);
        ...
        if (!releaseReserved.ok)
            assert.fail(releaseReserved.reason);
        ...
        if (!statusReserved.ok)
            assert.fail(statusReserved.reason);
        setProgramStatus('SUSPENDED');
        ...
        if (!expiringReserved.ok)
            assert.fail(expiringReserved.reason);
        ...
        assert.deepEqual(raceResults.filter((result) => !result.ok).map((result) => result.reason), ['program_occurrence_exhausted']);
      ...
      ON public.ep_gate_execution_program_occurrences`);
        ...
        await assert.rejects(pool.query('SELECT public.ep_gate_execution_program_read($1, $2, $3)', ['deployment:gate-v2', 'tenant:other', programDigest]), /binding mismatch/);
        const supersessionBlocker = await reserveProgram('supersession-blocker', 'releaseable');
        assert.equal(supersessionBlocker.ok, true);
        if (!supersessionBlocker.ok)
            assert.fail(supersessionBlocker.reason);
        ...
        assert.deepEqual(await store.supersedeExecutionProgram(successorArtifact, successorContext), { ok: false, reason: 'program_reserved_work_exists' });
        ...
        const superseded = await store.supersedeExecutionProgram(successorArtifact, successorContext);
        assert.equal(superseded.ok, true);
        if (!superseded.ok)
            assert.fail(superseded.reason);
