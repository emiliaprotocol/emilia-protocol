...
import {
  ...
  type LegacyExecutionProgramActionMatch,
...
type ExpectedResult = { ok: true; execution_program_binding?: AdmissionResourceReservationInput }
  | { ok: false; reason: string };

interface AbstractAdmission {
  admission_form?: 'input' | 'snapshot';
  tenant_id?: string;
  subject_id?: string;
  ...
  expires_at?: string;
  execution_program_resources?: AdmissionResourceReservationInput[];
  action_match?: LegacyExecutionProgramActionMatch;
  action_match_evidence?: unknown;
  action_match_verification?: Record<string, unknown>;
  sealed_evidence?: Array<{
    role: 'aeb';
    subject_id: string;
...
interface RuntimeOperation {
  op: string;
  program_ref?: string;
  predecessor_program_ref?: string;
  successor_program_ref?: string;
  node_id?: string;
  occurrence_id?: string;
  admission?: AbstractAdmission;
  outcome?: 'COMMITTED' | 'PROVEN_NOT_COMMITTED' | 'INDETERMINATE';
  value?: 'OBSERVED_AS_REQUESTED' | 'DIVERGED' | 'INDETERMINATE';
  admission_id?: string;
  tenant_id?: string;
  authorization_digest?: string;
  context?: Record<string, unknown>;
  now?: string;
  observation?: ExecutionProgramStatusObservation;
}

interface ExpectedState {
  programs: Array<Record<string, unknown>>;
  occurrences: Array<Record<string, unknown>>;
  authorization_claims?: Array<Record<string, unknown>>;
  ordinary_admissions?: Array<Record<string, unknown>>;
}
...
interface ProgramDefinition {
  program_digest: AdmissionDigest;
  program: VerifiedBoundedExecutionProgram;
  authorizer_id: string;
  key_id: string;
}

interface RuntimeStoreConfiguration {
  clock: string;
  execution_program_verification_policy: NonNullable<
    CreateMemoryAdmissionStoreOptions['executionProgramVerificationPolicy']
  >;
  max_execution_program_status_age_ms: number;
  status_oracle: 'none' | 'mutable_authenticated_status_fixture';
...
interface VectorDocument {
  syntax: {
    common: {
      trusted_keys: NonNullable<ExecutionProgramVerificationOptions['trusted_keys']>;
      verification_context: Omit<ExecutionProgramVerificationOptions, 'trusted_keys'>;
    };
    fixtures: Record<string, unknown>;
  };
  runtime: {
    store_configurations: Record<string, RuntimeStoreConfiguration>;
...
interface MaterializedAdmission {
  input: AdmissionSnapshotInput;
  admission: AdmissionSnapshotInput | Readonly<AdmissionSnapshot>;
}
...
const vectors = JSON.parse(fs.readFileSync(VECTOR_URL, 'utf8')) as VectorDocument;
...
function digest(label: string): AdmissionDigest {
...
function currentness(
  body: Readonly<AdmissionSnapshot['body']>,
  now: number,
...
function materializeAdmission(
  traceId: string,
  stepIndex: number,
  definition: ProgramDefinition,
  abstract: AbstractAdmission,
  ...
  if (abstract.admission_form === 'snapshot' && executionProgramResources.length > 0) {
    const earliestBindingDeadline = Math.min(...executionProgramResources.map((resource) => (
      Date.parse(resource.expires_at)
    )));
  ...
  const authorizationDigest = (abstract.authorization_digest
    ?? definition.program.authorization_digest) as AdmissionDigest;
  ...
  const input: AdmissionSnapshotInput = {
    ...
    action_digest: required(abstract.action_digest, 'admission.action_digest') as AdmissionDigest,
    ...
    authorization_policy_digest: required(
      abstract.authorization_policy_digest,
      'admission.authorization_policy_digest',
    ) as AdmissionDigest,
...
function materializeOrdinaryAdmission(
  traceId: string,
  stepIndex: number,
  operation: RuntimeOperation,
  ...
  const materialized = materializeAdmission(traceId, stepIndex, base, {
    ...
    authorization_digest: required(
      operation.authorization_digest,
      'ordinary_reserve.authorization_digest',
    ),
...
function snapshotInput(snapshot: Readonly<AdmissionSnapshot>): AdmissionSnapshotInput {
  const { '@version': _version, ...input } = snapshot.body;
  return input;
}

interface ActionMatchPlan {
  evidence: unknown;
  overrides: Partial<ExecutionProgramActionMatchExpected>;
}

interface TraceHarness {
  store: ExecutionProgramAdmissionStore;
  now(): number;
  setNow(value: string): void;
  setStatus(value: ExecutionProgramStatusObservation): void;
  setActionMatchPlan(value: ActionMatchPlan | null): void;
}

function expectedProfileActionMatch(
  definition: ProgramDefinition,
  nodeId: string,
  admission: AbstractAdmission,
): ExecutionProgramActionMatchExpected | null {
  ...
  if (node?.action.mode !== 'profile' || !evidence) return null;
  return {
    ...
    action_digest: required(admission.action_digest, 'admission.action_digest') as AdmissionDigest,
    ...
    trust_configuration_digest: required(
      admission.trust_configuration_digest,
      'admission.trust_configuration_digest',
    ),
  };
}

function actionMatchPlan(
  definition: ProgramDefinition,
  nodeId: string,
  admission: AbstractAdmission,
  ...
  const overrides: Partial<ExecutionProgramActionMatchExpected> = {};
  for (const key of Object.keys(expected) as Array<keyof ExecutionProgramActionMatchExpected>) {
    if (admission.action_match_verification[key] !== expected[key]) {
      (overrides as Record<string, unknown>)[key] = admission.action_match_verification[key];
    }
  }
  return { evidence: structuredClone(admission.action_match_evidence), overrides };
}

function createTraceStore(trace: RuntimeTrace): TraceHarness {
  let ownerIndex = 1;
  let invocationIndex = 1;
  let now = Date.parse(vectors.runtime.store_configurations[trace.store_configuration_ref].clock);
  let status: ExecutionProgramStatusObservation | null = null;
  const generatedStatus = new Map<string, ExecutionProgramStatusObservation>();
  let matchPlan: ActionMatchPlan | null = null;
  const configuration = vectors.runtime.store_configurations[trace.store_configuration_ref];
  const options: CreateMemoryAdmissionStoreOptions = {
    now: () => now,
  ...
  if (configuration.status_oracle !== 'none') {
    options.executionProgramStatusOracle = {
      read: async (reference) => {
        if (status !== null) return structuredClone(status);
        const previous = generatedStatus.get(reference.program_digest);
        const observedAt = new Date(now).toISOString();
        if (previous?.observed_at === observedAt) return structuredClone(previous);
        const definition = Object.values(vectors.runtime.programs).find(
          (entry) => entry.program_digest === reference.program_digest,
        );
        if (!definition) return null;
        const initial = now === Date.parse(configuration.clock);
        const next: ExecutionProgramStatusObservation = {
...
function projectResult(
  raw: { ok: boolean; reason?: string },
  expected: ExpectedResult,
  binding?: AdmissionResourceReservationInput,
): ExpectedResult {
  if (!raw.ok) return { ok: false, reason: required(raw.reason, 'runtime refusal reason') };
...
function closedRegistrationContext(
  definition: ProgramDefinition,
): ExecutionProgramRegistrationContext {
  return {
...
for (const trace of vectors.runtime.traces) {
  test(`runtime trace replay: ${trace.id} [${trace.classification}]`, async () => {
    ...
    const { store } = harness;
    const handles = new Map<string, AdmissionHandle>();
    const attemptedOccurrences = new Map<string, { programRef: string; occurrenceId: string }>();
    const attemptedOrdinaryAdmissions = new Map<string, {
      tenantId: string;
      authorizationDigest: string;
    }>();

    const readOccurrence = async (operation: RuntimeOperation) => {
    ...
    const projectState = async (expected: ExpectedState): Promise<ExpectedState> => {
      const programs: Array<Record<string, unknown>> = [];
      const authorizationClaims = new Map<string, Record<string, unknown>>();
      const indexedNodeCounts = new Map<string, number>();
      ...
      for (const programRef of Object.keys(vectors.runtime.programs).sort(byteOrder)) {
        ...
        authorizationClaims.set(
          `${state.tenant_id}\0${state.program.authorization_digest}`,
          {
      ...
      const includeBinding = expected.occurrences.some((occurrence) => (
        Object.hasOwn(occurrence, 'admission_binding')
      ));
      const occurrences: Array<Record<string, unknown>> = [];
      const occurrenceAttempts = [...attemptedOccurrences.values()].sort((left, right) => (
        byteOrder(`${left.programRef}\0${left.occurrenceId}`, `${right.programRef}\0${right.occurrenceId}`)
      ));
      for (const attempt of occurrenceAttempts) {
        const definition = required(
          vectors.runtime.programs[attempt.programRef],
          `runtime.programs.${attempt.programRef}`,
        );
        const occurrence = await store.readExecutionProgramOccurrence({
          tenant_id: definition.program.tenant_id,
          program_digest: definition.program_digest,
        ...
        if (includeBinding) {
          ...
          const bindings = snapshot.body.resource_reservations.filter(
      ...
      const projected: ExpectedState = { programs, occurrences };
      ...
      if (Object.hasOwn(expected, 'ordinary_admissions')) {
        const ordinaryAdmissions: Array<Record<string, unknown>> = [];
    ...
    const execute = async (
      operation: RuntimeOperation,
      stepIndex: number,
    ): Promise<{ raw: { ok: boolean; reason?: string }; binding?: AdmissionResourceReservationInput }> => {
      ...
      if (operation.op === 'register') {
        ...
        assert.equal(
          executionProgramDigest(artifact),
          definition.program_digest,
          `${programRef}: signed fixture does not match runtime program_digest`,
        );
        assert.deepEqual(operation.context, closedRegistrationContext(definition));
        return {
          raw: await store.registerExecutionProgram(
            artifact,
            closedRegistrationContext(definition),
          ),
        };
      }
      ...
      if (operation.op === 'reserve') {
        ...
        const materialized = materializeAdmission(
          trace.id,
          stepIndex,
          definition,
          abstractAdmission,
        ...
        const actualBindings = raw.snapshot.body.resource_reservations.filter(
          (resource) => resource.kind === 'execution_program',
        );
        assert.deepEqual(actualBindings, [binding], `${trace.id} step ${stepIndex + 1}: binding drift`);

        const existingBindings = materialized.input.resource_reservations.filter(
          (resource) => resource.kind === 'execution_program',
        );
        ...
        assert.deepEqual(
          raw.snapshot,
          expectedSnapshot,
          `${trace.id} step ${stepIndex + 1}: deterministic snapshot/program binding drift`,
        );
        assert.deepEqual(
          createAdmissionSnapshot(snapshotInput(raw.snapshot)),
          raw.snapshot,
          `${trace.id} step ${stepIndex + 1}: snapshot digest is not reproducible`,
        );
        return { raw, binding };
      }
      ...
      if (operation.op === 'ordinary_reserve') {
        ...
        if (raw.ok) handles.set(input.admission_id, { ownerToken: raw.owner_token });
        return { raw };
      }
      ...
      if (operation.op === 'ordinary_release') {
        ...
        if (!attempted) return { raw: { ok: false, reason: 'admission_not_found' } };
        const handle = handles.get(admissionId);
        const record = await store.read({ tenant_id: attempted.tenantId, admission_id: admissionId });
        if (!handle || !record) return { raw: { ok: false, reason: 'admission_not_found' } };
        return {
      ...
      if (operation.op === 'begin' || operation.op === 'ordinary_begin') {
        ...
        if (raw.ok) selected.handle.invocationToken = raw.invocation_token;
        return { raw };
      }
      ...
      if (operation.op === 'supersede') {
        ...
        const definition = required(
          vectors.runtime.programs[successorRef],
          `runtime.programs.${successorRef}`,
        );
        const artifact = required(
          vectors.syntax.fixtures[successorRef],
          `syntax.fixtures.${successorRef}`,
        );
        assert.equal(
          executionProgramDigest(artifact),
          definition.program_digest,
          `${successorRef}: signed fixture does not match runtime program_digest`,
        );
        assert.deepEqual(operation.context, closedRegistrationContext(definition));
        return {
          raw: await store.supersedeExecutionProgram(
            artifact,
            closedRegistrationContext(definition),
          ),
        };
