...
// Generated from remedy-case-set-postgres.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
...
function caseState({ tenantId = 'tenant-1', caseSetId = 'case-set-1', ownerTokenDigest = HASH('a'), revision = 0, status = revision === 0 ? 'open' : 'indeterminate', } = {}) {
    const manifest = {
        version: VERSION,
        tenant_id: tenantId,
...
function fakePostgres() {
    ...
    const key = (tenantId, caseSetId) => JSON.stringify([tenantId, caseSetId]);
    ...
    function restoreMap(target, snapshot) {
        target.clear();
        for (const [entryKey, value] of snapshot)
            target.set(entryKey, structuredClone(value));
    }
            ...
            return {
                async query(text, params = []) {
                    ...
                    if (text.startsWith('BEGIN ')) {
                        ...
                        const [tenantId, caseSetId, status, ownerTokenDigest, manifestJson, manifestDigest, stateJson, stateDigest, recordedAt,] = params;
                        ...
                        const [tenantId, caseSetId, expectedRevision, ownerTokenDigest, nextRevision, status, stateJson, stateDigest, recordedAt, manifestJson, manifestDigest,] = params;
                        assert.equal(tenantContext, tenantId);
                        const recordKey = key(tenantId, caseSetId);
                        ...
                        const [tenantId, caseSetId, revision, previousRevision, status, stateJson, stateDigest, recordedAt,] = params;
                        assert.equal(tenantContext, tenantId);
                        const recordKey = key(tenantId, caseSetId);
                ...
                release() {
                    if (inTransaction)
                        throw new Error('client released with open transaction');
                },
            };
        },
    };
    return {
        ...
        transformNextResult(target, transform) {
            transformNext = { target, transform };
        },
    };
}
test('DDL is tenant-keyed, RPC-only, and makes manifests, terminal rows, and events immutable', () => {
    assert.match(REMEDY_CASE_SET_POSTGRES_DDL, new RegExp(`CREATE TABLE IF NOT EXISTS ${REMEDY_CASE_SET_TABLE}`));
    ...
    assert.match(REMEDY_CASE_SET_POSTGRES_DDL, new RegExp(`BEFORE UPDATE OR DELETE ON ${REMEDY_CASE_SET_EVENT_TABLE}`));
});
test('concurrent create has one winner and one tenant-scoped conflict', async () => {
    ...
    const results = await Promise.all([first.create(state), second.create(state)]);
    assert.equal(results.filter((result) => result.ok).length, 1);
    assert.equal(results.filter((result) => !result.ok && result.reason === 'case_set_exists').length, 1);
...
test('same case-set id is isolated across tenants and a restarted store resumes exact CAS state', async () => {
    ...
    assert.equal((await firstProcess.create(tenantOne)).ok, true);
    assert.equal((await firstProcess.create(tenantTwo)).ok, true);
    assert.deepEqual(await firstProcess.get({ tenantId: 'tenant-c', caseSetId: 'shared-case' }), { ok: false, reason: 'case_set_not_found' });
    const restarted = createRemedyCaseSetPostgresStore({ pool: pg.pool });
    assert.deepEqual((await restarted.get({ tenantId: 'tenant-b', caseSetId: 'shared-case' })).state, tenantTwo);
    ...
    assert.deepEqual((await restarted.get({ tenantId: 'tenant-b', caseSetId: 'shared-case' })).state, resumed);
});
...
test('CAS refuses a stale revision and preserves the winning update', async () => {
    ...
    assert.deepEqual((await store.get({ tenantId: initial.tenant_id, caseSetId: initial.case_set_id })).state, next);
});
test('completed case sets cannot be reopened or rewritten', async () => {
    ...
    assert.deepEqual((await store.get({ tenantId: initial.tenant_id, caseSetId: initial.case_set_id })).state, terminal);
});
test('malformed PostgreSQL responses and database errors fail closed', async () => {
    ...
    await assert.rejects(() => store.get({ tenantId: initial.tenant_id, caseSetId: initial.case_set_id }), /tenant context/);
    ...
    await assert.rejects(() => store.get({ tenantId: initial.tenant_id, caseSetId: initial.case_set_id }), /malformed|invalid/);
    const unavailable = createRemedyCaseSetPostgresStore({
        pool: { connect: async () => { throw new Error('database unavailable'); } },
    });
    await assert.rejects(() => unavailable.get({ tenantId: initial.tenant_id, caseSetId: initial.case_set_id }), /database unavailable/);
});
