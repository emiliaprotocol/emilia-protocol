...
test('an embedded or cross-operator key cannot establish authority', () => {
  ...
  assert.equal(
    verifyActionEscrowStateStatement(unpinned, opts).reason,
    'malformed_state_statement',
  );
...
test('signature and statement digest substitution are independently refused', () => {
  ...
  assert.equal(
    verifyActionEscrowStateStatement(signatureTamper, opts).reason,
  ...
  assert.equal(
    verifyActionEscrowStateStatement(digestTamper, opts).reason,
    'state_statement_digest_mismatch',
  );
});
...
test('future and impossible timestamps are refused', () => {
  ...
  assert.equal(
    verifyActionEscrowStateStatement(future, opts).reason,
    'state_statement_from_future',
  );
