...
// Generated from trust-program.hostile.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
import test from 'node:test';
import assert from 'node:assert/strict';
import { generateKeyPairSync } from 'node:crypto';
import { TRUST_PROGRAM_VERSION, createMemoryTrustProgramStore, createTrustProgramKernel, validateTrustProgram, verifyTrustStageReceipt, } from './trust-program.js';
...
function requirement(id, policy) {
    return {
        requirement_id: id,
...
function evidence(challenge, id, policy) {
    return {
        evidence_id: id,
...
function kernel(options = {}) {
    ...
    const verifier = async ({ artifact }) => ({ ...artifact });
...
async function completeOne(subject, instanceId) {
...
test('profile is closed and refuses extension fields instead of signing ambiguous policy', () => {
    ...
    nestedConsequences.execution.escrow_profile_digest = HASH('9');
...
test('constructor-owned trust configuration is snapshotted against later mutation', async () => {
    const keys = generateKeyPairSync('ed25519');
    const verifiers = {
        'ep-signoff': async ({ artifact }) => ({ ...artifact }),
    };
...
test('stage receipts are closed and independently tenant-bound', async () => {
    ...
    const subject = createTrustProgramKernel({
        ...
        verifiers: { 'ep-signoff': async ({ artifact }) => ({ ...artifact }) },
...
test('execution rejects evidence that becomes future-dated after a clock rollback', async () => {
    ...
    clock = NOW - 2_000;
    assert.equal((await subject.claimExecution({ instanceId: 'clock_rollback' })).reason, 'clock_regression');
});
test('a small clock rollback is refused before a transition can corrupt durable chronology', async () => {
    ...
    clock = NOW - 500;
    assert.equal((await subject.claimExecution({ instanceId: 'clock_regression' })).reason, 'clock_regression');
...
test('production action and terminal-outcome verifiers are mandatory decision gates', async () => {
    const rejectedStore = createMemoryTrustProgramStore();
    ...
    const outcomeStore = createMemoryTrustProgramStore();
