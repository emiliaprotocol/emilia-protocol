...
function gateTrustingHarness(harness, {
  verifyAdmissibilityPacket = async ({ presented }) => presented,
  requiredAdmissibilityProfile = null,
} = {}) {
...
test('NO profile pinned => behavior byte-for-byte unchanged (allowed, no admissibility gating)', async () => {
  ...
  const withoutProfile = await gateA.run(
    { selector: EG1_DEFAULT_SELECTOR, receipt: receiptA, observedAction: harness.action },
    async () => ({ ran: true }),
  );
  ...
  const baseline = await gateB.run(
    { selector: EG1_DEFAULT_SELECTOR, receipt: receiptB, observedAction: harness.action },
    async () => ({ ran: true }),
  );
...
test('verifyAdmissibilityAgainstPinnedProfile: fail-closed refusals are distinct and named', () => {
  ...
  assert.equal(
    verifyAdmissibilityAgainstPinnedProfile(pin, admissibilityBlock({ profileHash: PINNED_HASH, verdict: 'stale' })).reason,
    'admissibility_not_admissible:stale',
  );
  // unrecognized verdict (outside the closed set) refuses, never passes
  assert.equal(
    verifyAdmissibilityAgainstPinnedProfile(pin, admissibilityBlock({ profileHash: PINNED_HASH, verdict: 'looks_fine' })).reason,
    'admissibility_verdict_unrecognized',
  );
  // pin present but NOTHING presented => refuse
  assert.equal(
    verifyAdmissibilityAgainstPinnedProfile(pin, null).reason,
    'admissibility_profile_pinned_but_absent',
  );
  ...
  assert.equal(
    ...
    'pinned_profile_missing_hash',
  );
});

test('closed verdict set is exactly the five admissibility verdicts', () => {
  assert.deepEqual(
    [...ADMISSIBILITY_VERDICTS].sort(),
    ['admissible', 'conflicted', 'missing_evidence', 'stale', 'unverifiable'].sort(),
  );
});
