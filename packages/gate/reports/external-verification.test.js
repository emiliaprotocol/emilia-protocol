...
// Generated from external-verification.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
import test from 'node:test';
import assert from 'node:assert/strict';
import crypto from 'node:crypto';
import { EXTERNAL_VERIFICATION_STATEMENT_VERSION, externalVerificationDigest, signExternalVerificationStatement, verifyExternalVerificationStatement, } from './external-verification.js';
...
test('malformed version and signature fail closed', () => {
    const k = verifierKey();
    const s = statementFixture(k);
    assert.equal(verifyExternalVerificationStatement({ ...s, '@version': 'NOPE' }, { pinnedVerifierKeys: [{ public_key: k.public_key }] }).reason, 'unsupported_version');
    const noSig = { ...s };
    delete noSig.signature;
    assert.equal(verifyExternalVerificationStatement(noSig, { pinnedVerifierKeys: [{ public_key: k.public_key }] }).reason, 'signature_missing_or_malformed');
});
