...
// Generated from auditor-workpaper.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
/**
...
import { AUDIT_WORKPAPER_VERSION, AUDIT_WORKPAPER_HONESTY_NOTICE, AUDIT_ATTRIBUTES, buildAuditWorkpaper, renderMarkdown, } from './auditor-workpaper.js';
...
test('population: guarded decisions in the half-open window; edges inclusive-start / exclusive-end', async () => {
    ...
    assert.deepEqual(pack.population.items.map((i) => i.at), ['2026-01-01T00:00:00.000Z', '2026-01-15T12:00:00.000Z', '2026-01-20T00:00:00.000Z']);
...
test('sampling: same seed twice -> identical selection; auditor can reproduce it from the documented method', async () => {
    const entries = await chained(Array.from({ length: 12 }, (_, i) => allowEntry(`2026-01-${String(i + 2).padStart(2, '0')}T00:00:00.000Z`)));
...
test('attributes: a clean allow passes A1-A6; degraded evidence fails the named attributes into exceptions', async () => {
    ...
    for (const a of dirty.attributes)
        assert.ok(a.evidence_field.length > 0);
...
test('malformed entries -> integrity_warnings, never sampled', async () => {
    ...
    assert.deepEqual(pack.integrity_warnings.map((w) => w.problem), ['not_an_object', 'missing_or_unparseable_at', 'missing_hash', 'unknown_kind', 'decision_missing_allow']);
...
test('fail closed: missing/invalid options are errors, not guesses', async () => {
    ...
    assert.throws(() => buildAuditWorkpaper(entries, { ...OPTS, periodStart: OPTS.periodEnd, periodEnd: OPTS.periodStart }), /empty or inverted period/);
    assert.throws(() => buildAuditWorkpaper(entries, { ...OPTS, periodStart: OPTS.periodEnd }), /empty or inverted period/);
...
test('renderMarkdown fails closed: wrong @version, altered honesty notice, or machine-filled sign-off', async () => {
    ...
    assert.throws(() => renderMarkdown({ ...pack, '@version': 'EP-GATE-AUDIT-WORKPAPER-v2' }), /requires an EP-GATE-AUDIT-WORKPAPER-v1/);
    assert.throws(() => renderMarkdown({ ...pack, notice: pack.notice.replace('not an audit opinion', 'an audit opinion') }), /honesty notice was altered or removed/);
    assert.throws(() => renderMarkdown({ ...pack, notice: undefined }), /honesty notice was altered or removed/);
    assert.throws(() => renderMarkdown({ ...pack, conclusion: { tested_by: 'machine', reviewed_by: null, conclusion: null } }), /sign-off fields must be null/);
    assert.throws(() => renderMarkdown({ ...pack, conclusion: { tested_by: null, reviewed_by: null, conclusion: 'effective' } }), /sign-off fields must be null/);
});
