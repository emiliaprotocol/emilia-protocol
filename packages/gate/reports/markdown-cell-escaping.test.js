...
function assertTablesIntact(markdown, label) {
  ...
  for (const [i, line] of lines.entries()) {
    ...
    assert.equal(
      width,
      headerWidth,
      `${label}: line ${i + 1} renders ${width} cells against a ${headerWidth}-cell header, `
...
function assertRenderedEscaped(markdown, raw, label) {
  ...
  assert.ok(
    markdown.includes(escaped),
    `${label}: hostile value must render escaped, not mangled or dropped`,
  );
}
...
test('liveCells models Markdown escaping, so the assertions above can fail', () => {
  ...
  assert.throws(
    () => assertTablesIntact('| A | B |\n|---|---|\n| a\\\\|b | c |', 'self-test'),
    /broke out of its cell/,
  );
});
