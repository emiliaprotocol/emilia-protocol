...
// Generated from recovery-admission-remedy.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
import assert from 'node:assert/strict';
import crypto, { generateKeyPairSync, sign as cryptoSign } from 'node:crypto';
import test from 'node:test';
import { ADMISSION_CURRENTNESS_VERSION, EXECUTION_PROGRAM_STATUS_VERSION, createMemoryAdmissionStore, } from './admission-store.js';
import { signBoundedExecutionProgram } from './bounded-execution-program.js';
import { canonicalize } from './execution-binding.js';
import { createRecoveryAdmissionRemedyBridge, } from './recovery-admission-remedy.js';
import { issueRemedyProgramReceipt, remedyProgramReceiptSigningBytes, } from './remedy-program-receipt.js';
...
function admission(kind, authorizationDigest, relation = null) {
...
async function claimedRemedy(remedyStore, claim = true) {
...
async function fixture(options = {}) {
    ...
    if (!originalReserved.ok)
        throw new Error('original admission reservation failed');
    ...
    if (!originalBegun.ok)
        throw new Error('original admission invocation failed');
...
test('fails closed for every signed receipt binding mutation', async (t) => {
    ...
    for (const [name, mutate] of mutations) {
        await t.test(name, async () => {
            ...
            if (!result.ok)
                assert.match(result.reason, /^remedy_receipt_/);
        });
    }
});
...
test('duplicate concurrent reservations linearize in AdmissionStore', async () => {
    ...
    assert.deepEqual([left, right].find((result) => !result.ok), { ok: false, reason: 'admission_exists' });
});
...
function boundedProgram(authorizationDigest) {
    return {
        ...
        nodes: [{
                node_id: 'refund',
                action: { mode: 'exact', caid: REMEDY_CAID, action_digest: REMEDY_ACTION },
