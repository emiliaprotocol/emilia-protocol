...
// Generated from trust-program-v2.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
//
...
function program(version = TRUST_PROGRAM_V2_VERSION) {
    return {
        '@version': version,
...
function mintV1Receipt(privateKey) {
    const body = {
...
describe('EP-GATE-TRUST-STAGE-RECEIPT-v2 hybrid stage receipt', () => {
    ...
    it('v1-refuses-v2: the SYNC v1 verifier refuses a v2 receipt on the version marker, never a crash', async () => {
        ...
        const result = verifyTrustStageReceipt(receipt, { trustedKeys: issuer.classicalPin });
        assert.equal(result.valid, false);
    ...
    it('a key id pinned WITHOUT the ML-DSA half never satisfies a v2 pin', async () => {
        const issuer = issuerFixture();
        const receipt = await signTrustStageReceiptV2({ payload: payload(), context: CONTEXT, keys: issuer.keys });
        const result = await verifyTrustStageReceiptV2(receipt, { trustedKeys: issuer.classicalPin });
        assert.equal(result.valid, false);
    ...
    it('wrong-length signature on the ML-DSA leg refuses, never crashes', async () => {
        ...
        const tampered = {
            ...
            signature: {
                ...receipt.signature,
                signatures: receipt.signature.signatures.map((s) => (s.alg === 'ML-DSA-65' ? { ...s, sig: crypto.randomBytes(16).toString('base64url') } : s)),
            },
        };
    ...
    it('never throws on hostile input', async () => {
        for (const bad of [null, undefined, '', 42, [], { version: TRUST_STAGE_RECEIPT_V2_VERSION }]) {
            assert.equal((await verifyTrustStageReceiptV2(bad, { trustedKeys: {} })).valid, false);
        }
    });
    it('refuses to issue a receipt whose signSet signer returns an incomplete set', async () => {
        ...
        await assert.rejects(signTrustStageReceiptV2({ payload: payload(), context: CONTEXT, signer }), /malformed signature set/);
    });
