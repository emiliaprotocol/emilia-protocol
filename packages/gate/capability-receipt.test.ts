...
test('memory capability operations isolate identical operation ids by capability', async () => {
  ...
  assert.equal(
    store.getOperation('shared-reconciliation', second.capabilityReceipt.capability.id).reconciliation_outcome,
    undefined,
  );
  assert.equal((await store.reconcileSpend({
    capabilityId: second.capabilityReceipt.capability.id,
  ...
  assert.equal(
    store.getOperation('shared-reconciliation', first.capabilityReceipt.capability.id).reconciliation_evidence_digest,
    firstEvidence,
  );
  assert.equal(
    store.getOperation('shared-reconciliation', second.capabilityReceipt.capability.id).reconciliation_evidence_digest,
    secondEvidence,
  );
});
...
test('postgres capability operations use a composite namespace and operation key', async () => {
  ...
  assert.equal(
    operations.get(operationKey(first.capabilityReceipt.capability.id, 'shared-postgres-operation')).reconciliation_evidence_digest,
    firstEvidence,
  );
  assert.equal(
...
test('postgres pre-entry recovery locks capability state before the operation', async () => {
  const capabilityId = 'postgres_recovery_lock_order';
  const operationId = 'postgres_recovery_lock_order_operation';
  const actionDigest = `sha256:${'9'.repeat(64)}`;
  const statements: string[] = [];
  const store = createPostgresCapabilityStore({
    transaction: async (callback) => callback(async (sql) => {
      statements.push(sql);
      if (sql === CAPABILITY_SQL.readState) {
        return { rowCount: 1, rows: [{ capability_id: capabilityId }] };
      }
      if (sql === CAPABILITY_SQL.readOperation) {
        return {
          rowCount: 1,
          rows: [{
            ...
            operation_id: operationId,
            action_digest: actionDigest,
            status: 'provider_entered',
          }],
        };
      }
      throw new Error(`unexpected SQL in recovery lock-order test: ${sql}`);
    }),
  });
  ...
  assert.deepEqual(statements, [CAPABILITY_SQL.readState, CAPABILITY_SQL.readOperation]);
});
...
test('CAID-equivalent wrappers keep exact digests but share one runtime fence', async () => {
  ...
  const firstScope = verifyCapabilityScope(
    minted.capabilityReceipt.capability,
    firstAction,
    firstAction.operation_id,
    { resolveCaid: () => caid },
  );
  const secondScope = verifyCapabilityScope(
    minted.capabilityReceipt.capability,
    secondAction,
    secondAction.operation_id,
    { resolveCaid: () => caid },
  );
...
test('materially different CAIDs derive distinct runtime fences', async () => {
  ...
  const scopes = actions.map((action) => verifyCapabilityScope(
    minted.capabilityReceipt.capability,
    action,
    action.operation_id,
    { resolveCaid },
...
test('a committed freeze defeats a still-fresh ACTIVE observation before provider entry', async () => {
  ...
  assert.equal(effects, 0);
  assert.equal(store.getControlDomain(controlDomainId).status, 'frozen');
...
test('one delegation operation identifier funds only one child digest', async () => {
  ...
  assert.equal(results.filter((result) => (
    !result.ok && ['operation_in_flight', 'operation_already_committed'].includes(result.reason)
  )).length, 1);
...
test('separate authority stores demonstrate the explicit cross-domain non-guarantee', async () => {
  ...
  for (const store of stores) assert.equal(store.registerCapability(parent.capabilityReceipt), true);
...
async function threeHopLineage({
  keys,
  prefix,
  rootRevocationMode,
...
test('ancestor revocation and descendant reservation serialize without reopening committed authority', async () => {
  ...
  assert.equal(reservationFirst.store.getOperation(
    'race_before_revocation',
    secondChildId,
  ).status, 'reserved');
...
test('real PostgreSQL serializes cascade revocation against descendant reservation', {
  ...
  function transactionFor(schema: string, hooks: {
  ...
  async function setupLineage(schema: string, prefix: string) {
...
test('real PostgreSQL action fence is load-bearing across capability rows', {
  ...
  try {
    ...
    const vulnerableDdl = CAPABILITY_STATE_DDL
      ...
      .replace(
        /DO \$capability_action_fence_index_contract\$[\s\S]*?\$capability_action_fence_index_contract\$;/,
        '',
      );
  ...
  try {
    for (const [index, conflictingIndex] of conflictingIndexes.entries()) {
      ...
      try {
        ...
        await assert.rejects(
          client.query(CAPABILITY_STATE_DDL),
          (error: Error & { code?: string }) => error.code === '55000',
          `conflicting index ${index} must be rejected`,
        );
      } finally {
        client.release();
      }
...
test('package DDL quarantines legacy capability ids at the database boundary', {
  ...
  async function directInsertOutcome(schema: string, ddl: string): Promise<string> {
    await pool.query(`CREATE SCHEMA ${schema}`);
    const client = await pool.connect();
    try {
      await client.query(`SET search_path TO ${schema}, public`);
  ...
  try {
    ...
    const vulnerableDdl = CAPABILITY_STATE_DDL.replace(
      /CREATE OR REPLACE FUNCTION ep_require_semantic_capability_fence\(\)[\s\S]*?EXECUTE FUNCTION ep_require_semantic_capability_fence\(\);/,
      '',
    );
    assert.notEqual(vulnerableDdl, CAPABILITY_STATE_DDL, 'mutation must remove the database guard');
    assert.equal(
      await directInsertOutcome(`${schemaPrefix}_mutated`, vulnerableDdl),
      'admitted',
      'the negative control must show that the trigger is load-bearing',
    );
  } finally {
    await pool.query(`DROP SCHEMA IF EXISTS ${schemaPrefix}_shipped CASCADE`);
    await pool.query(`DROP SCHEMA IF EXISTS ${schemaPrefix}_mutated CASCADE`);
...
test('package DDL quarantines an incomplete bootstrap that already defaulted the state flag to true', {
  ...
  async function upgradeOutcome(schema: string, ddl: string): Promise<{
    ready: boolean;
    insert: string;
  }> {
    await pool.query(`CREATE SCHEMA ${schema}`);
    ...
    try {
      ...
      await client.query(ddl);
      const state = await client.query<{ semantic_fence_ready: boolean }>(`
      ...
      try {
        ...
        insert = (error as { code?: string }).code ?? 'unknown_error';
      }
      return {
  ...
  try {
    assert.deepEqual(
      await upgradeOutcome(`${schemaPrefix}_shipped`, CAPABILITY_STATE_DDL),
      { ready: false, insert: '55000' },
    );
    const vulnerableDdl = CAPABILITY_STATE_DDL.replace(
    ...
    assert.deepEqual(
      await upgradeOutcome(`${schemaPrefix}_mutated`, vulnerableDdl),
      { ready: true, insert: 'admitted' },
      'the negative control must reproduce the incomplete-bootstrap bypass',
    );
  } finally {
    await pool.query(`DROP SCHEMA IF EXISTS ${schemaPrefix}_shipped CASCADE`);
    await pool.query(`DROP SCHEMA IF EXISTS ${schemaPrefix}_mutated CASCADE`);
...
test('real PostgreSQL serializes emergency freeze against provider entry and preserves accounting', {
  ...
  try {
    ...
    const entryAfterReleaseStore = createPostgresCapabilityStore({
      transaction: transactionFor({
        before(sql) {
          if (sql === CAPABILITY_SQL.readState) lateEntryReachedLock.release();
        },
      }),
      verifyControlTransition,
    });
