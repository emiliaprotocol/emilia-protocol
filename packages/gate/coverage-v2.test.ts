...
import {
  ...
  ENFORCEMENT_PROBE_V2_VERSION,
  ENFORCEMENT_PROBE_V2_REQUIRED_ALGORITHMS,
  signEnforcementProbe,
  verifyEnforcementProbe,
  signEnforcementProbeV2,
  verifyEnforcementProbeV2,
  verifyEnforcementProbeAny,
  probeV2Bytes,
} from './coverage.js';
...
function probeInput(overrides: any = {}) {
  return {
    probe_id: PROBE_ID,
    key_id: KEY_ID,
...
async function buildV2(overrides: any = {}) {
...
test('a real hybrid probe statement verifies under both pinned keys (valid roundtrip)', async () => {
  ...
  assert.equal(res.accepted, true, (res as any).reason);
});
...
test('LEG STRIPPING: removing the ML-DSA leg refuses structurally', async () => {
  ...
  const res = await verifyEnforcementProbeV2(stmt, verifyOpts());
  assert.equal(res.accepted, false);
  assert.equal((res as any).reason, 'probe_signature_leg_stripped');
});
...
test('SET NARROWING: narrowing required_algorithms to Ed25519-only refuses structurally', async () => {
  ...
  const res = await verifyEnforcementProbeV2(stmt, verifyOpts());
  assert.equal(res.accepted, false);
  assert.equal((res as any).reason, 'probe_algorithm_set_invalid');
  ...
  const { proof, ...body } = structuredClone(await buildV2()) as any;
  const narrowedBytes = Buffer.from(
    `${ENFORCEMENT_PROBE_V2_VERSION}\0${canonicalize({ ...body, required_algorithms: ['Ed25519'] })}`,
    'utf8',
  );
...
test('DUPLICATE ALGORITHM: two entries for one algorithm refuse', async () => {
  ...
  const res = await verifyEnforcementProbeV2(stmt, verifyOpts());
  assert.equal(res.accepted, false);
  assert.equal((res as any).reason, 'probe_signature_leg_duplicate');
});
...
test('WRONG-LENGTH SIGNATURE: a truncated Ed25519 leg refuses', async () => {
  const stmt: any = structuredClone(await buildV2());
  const leg = stmt.proof.signatures.find((s: any) => s.alg === 'Ed25519');
...
test('ALGORITHM RELABELLING: calling the Ed25519 leg "Ed448" refuses (closed registry)', async () => {
  ...
  const res = await verifyEnforcementProbeV2(stmt, verifyOpts());
  assert.equal(res.accepted, false);
  assert.equal((res as any).reason, 'probe_signature_leg_unexpected');
});
...
test('an unpinned probe key confers nothing', async () => {
  ...
  assert.equal(res.accepted, false);
  assert.equal((res as any).reason, 'probe_key_unpinned');
});
