...
const receiptStates = new Map<string, string>();
 ...
 */
function interruptionToolName(interruption: AnyRecord): string | null {
  return (
    interruption?.name ??
 ...
 */
function interruptionArgs(interruption: AnyRecord): any {
  const raw =
...
function interruptionCallId(interruption: AnyRecord): string | null {
  return (
...
function deriveAction(
  actionFor: ((toolName: string, args: any) => any) | undefined,
  ...
  try {
    ...
    if (typeof baseAction !== 'string' || baseAction.length === 0) return null;
...
function isApprovalInterruption(interruption: AnyRecord): boolean {
...
function matchesPendingOccurrence(interruption: AnyRecord, pendingApproval: AnyRecord): boolean {
...
export function requireReceiptForOpenAIAgent(opts: AnyRecord = {}): AnyRecord {
   ...
   * }>}
   */
  async function decide(interruption: AnyRecord, receipt: AnyRecord | null | undefined, { reserveOnly = false }: AnyRecord = {}): Promise<AnyRecord> {
    const toolName = interruptionToolName(interruption);
    const callId = interruptionCallId(interruption);
    const args = interruptionArgs(interruption);
   ...
   */
  function lookupReceipt(receipts: any, interruption: AnyRecord): AnyRecord | null {
    ...
    if (Array.isArray(receipts)) {
      const args = interruptionArgs(interruption);
      const action = args === null ? null : deriveAction(
        actionFor, toolName, args, interruptionCallId(interruption),
      );
      if (!action) return null;
      return receipts.find((r: AnyRecord) => r?.payload?.claim?.action_type === action) ?? null;
    }
   ...
   */
  async function resolve(runResult: AnyRecord, ctx: AnyRecord = {}): Promise<AnyRecord> {
    ...
    const releasePending = async (d: AnyRecord) => {
    ...
    const commitPending = async (d: AnyRecord) => {
      if (!d.receipt_id) return;
      const p = pending.get(d.receipt_id);
      if (p) {
        try {
