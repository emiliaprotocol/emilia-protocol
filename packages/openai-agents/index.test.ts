...
test('the four checks: missing -> blocked, valid -> approved, replay -> rejected, tampered -> rejected', async (t) => {
  ...
  await t.test('3. replayed receipt (same receipt_id again) -> REJECT', async () => {
    ...
    const sibling = await gate.resolve(
      { interruptions: [i2], state: makeFakeState() },
      { receipts: { c2: mintReceipt({ action: exactAction(i1) }) } },
    );
...
test('default configuration still binds full arguments and call identity', async () => {
  ...
  const action = bindToolAction(
    'cancelOrder',
    { orderId: 42 },
    'openai.tool.cancelOrder',
    'default_binding',
  );
...
test('action mapper cannot mutate the executor arguments it is selecting', async () => {
  ...
  const action = bindToolAction(
    'cancelOrder', { orderId: 42 }, 'openai.tool.cancelOrder', 'mapper_mutation',
  );
  const decision = await gate.decide(interruption, mintReceipt({ action }));
  assert.equal(decision.decision, 'approve');
...
test('action binding: a receipt for the WRONG tool is rejected', async () => {
  ...
  const wrong = mintReceipt({ action: bindToolAction(
    'somethingElse', {}, 'openai.tool.somethingElse', 'x1',
  ) });
...
test('runtime interruption drift after durable commit is rejected and burns authority', async () => {
  ...
  const store = {
    ownershipFenced: true,
    async reserve() {
      if (receiptState !== 'free') return false;
    ...
    async release() {
      if (receiptState !== 'reserved') return false;
      receiptState = 'free';
      return true;
    },
  };
  ...
  const result = await gate.resolve(
    { interruptions: [interruption], state },
    { receipts: { drift_after_commit: receipt } },
  );
  ...
  const replay = await gate.resolve(
    { interruptions: [approvedOccurrence], state: replayState },
    { receipts: { drift_after_commit: receipt } },
  );
