...
const sharedStore = {
    ...
    async reserve(id) {
        if (receiptStates.has(id))
            return false;
        receiptStates.set(id, 'reserved');
        return true;
    },
    async commit(id) {
        if (receiptStates.get(id) !== 'reserved')
...
function interruptionArgs(interruption) {
    const raw = interruption?.arguments ??
    ...
    if (Buffer.byteLength(raw, 'utf8') > 1024 * 1024 || !strictJsonGate(raw).ok)
        return null;
    try {
...
function isApprovalInterruption(interruption) {
    ...
    if (!interruption || typeof interruption !== 'object')
        return false;
    if (interruption.type && interruption.type !== 'tool_approval_item')
        return false;
...
export function requireReceiptForOpenAIAgent(opts = {}) {
    ...
    const { actionFor, store = sharedStore, action: _callerSuppliedActionIgnored, otelAuthorization, ...gateOptions } = opts;
     ...
     */
    const emitAuthorizationAttributes = async (d) => {
        ...
        await otel.emitToolAuthorization(otelAuthorization, otel.mapOpenAIAgentsDecision(decision, evidence), { runtime: 'openai-agents', tool_name: d.toolName, tool_call_id: d.callId, decision: d.decision, reason: d.reason });
    };
    /**
    ...
    function lookupReceipt(receipts, interruption) {
        ...
        if (receipts instanceof Map) {
            if (callId != null && receipts.has(callId))
                return receipts.get(callId);
            if (toolName != null && receipts.has(toolName))
        ...
        if (typeof receipts === 'object') {
            if (callId != null && Object.prototype.hasOwnProperty.call(receipts, callId))
                return receipts[callId];
            if (toolName != null && Object.prototype.hasOwnProperty.call(receipts, toolName))
                return receipts[toolName];
            return null;
        }
        return null;
    }
