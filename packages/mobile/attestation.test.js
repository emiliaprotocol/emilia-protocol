...
// Generated from attestation.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
...
test('Play Integrity adapter refuses each relying-party pin failure', async () => {
    ...
    for (const payload of cases)
        assert.equal((await playVerifier(payload)(playRequest)).valid, false);
    ...
    assert.equal((await playVerifier(omittedRiskVerdict, { requireNoCaptureOrControl: true })(playRequest)).valid, false, 'a required capture/control verdict must not pass when Google omitted the evidence');
});
