...
// Generated from presentation.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
...
import { MOBILE_PRESENTATION_VERSION, normalizeControlledMobilePresentation, projectMobileAction, } from './presentation.js';
const ROOT = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '../..');
const vectors = JSON.parse(fs.readFileSync(path.join(ROOT, 'mobile/conformance/mobile-core.v1.json'), 'utf8')).presentation_mapping;
...
test('projects every flat scalar action field losslessly across the shared vectors', () => {
    for (const item of vectors.filter((vector) => vector.expect === 'accept')) {
        ...
        assert.deepEqual(normalizeControlledMobilePresentation(vector.action, presentation(vector.material_fields)).material_fields, vector.material_fields, vector.id);
    }
});
test('refuses omitted, changed, extra, and nested material fields across the shared vectors', () => {
    for (const item of vectors.filter((vector) => vector.expect === 'reject')) {
        const vector = expandVector(item);
        assert.throws(() => normalizeControlledMobilePresentation(vector.action, presentation(vector.material_fields)), TypeError, vector.id);
    }
});
