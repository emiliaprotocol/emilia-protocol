...
function safeJsonNumbers(value: any): boolean {
...
function reply(body: AnyRecord, status = 200): Response {
  return new Response(JSON.stringify(body), { status, headers: JSON_HEADERS });
}

function refused(status: number, verdict: string, reason: string): Response {
  return reply({ ok: false, valid: false, verdict, reason }, status);
}

function protocolStatus(result: AnyRecord): number {
...
async function parseBody(request: Request, maxBodyBytes: number): Promise<AnyRecord> {
...
export function createMobileHttpHandler({
...
}: AnyRecord = {}): (request: Request) => Promise<Response> {
  ...
  return async function handleMobileRequest(request: Request): Promise<Response> {
    let url;
    try {
