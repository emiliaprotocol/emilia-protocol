...
type JsonResult = { ok: boolean; reason?: string };

export function strictJsonGate(raw: any): JsonResult {
  ...
  while (index < raw.length) {
    ...
    if (character === '{') {
      ...
      if (isKey) {
        ...
        const objectFrame = top as { object: true; keys: Set<string>; expectsKey: boolean };
