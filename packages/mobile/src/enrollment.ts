...
function instant(value: any): number | null {
 ...
 */
function boundedBase64url(value: any, maxBytes: number): boolean {
...
function validEnrollmentOrigin(value: any): boolean {
...
function isP256Spki(value: any): boolean {
...
function failure(verdict: string, reason: string): AnyRecord {
  return { ok: false, verdict, reason, enrollment: null };
}

export function buildMobileEnrollmentBinding(challenge: AnyRecord): AnyRecord {
  return {
 ...
 */
export function buildMobileAndroidKeyBinding({ challengeRequestHash, keyId, publicKeySpki }: AnyRecord = {}): AnyRecord {
...
function verifyAndroidDeviceKey(challenge: AnyRecord, response: AnyRecord): AnyRecord | null {
...
export function createMobileEnrollmentService({
  ...
  return {
     ...
     */
    async issue({ approverId, platform, appId, rpId, origin, userName, displayName, caller = null }: AnyRecord = {}): Promise<AnyRecord> {
     ...
     */
    async complete({ challenge, response, caller = null }: AnyRecord = {}): Promise<AnyRecord> {
      ...
      if (platform?.valid !== true || platform.request_hash !== expectedPlatformRequestHash
          ...
          || (challenge.platform === 'android'
            ...
            && platform.platform_public_key !== (androidKey as AnyRecord).publicKeySpki)) {
      ...
      const enrollment = {
        ...
        platform_public_key: challenge.platform === 'android'
          ? (androidKey as AnyRecord).publicKeySpki
