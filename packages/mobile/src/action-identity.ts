...
const { canonicalize, isCanonicalizable } = verifier as Record<string, any>;

export const MOBILE_ACTION_CAID_TYPE = 'emilia.mobile.authorized-action.1';
export const MOBILE_ACTION_CAID_PATTERN =
...
function sourceActionType(action: AnyRecord): string {
...
export function mobileActionFingerprint(actionCaid: string): string | null {
  if (!MOBILE_ACTION_CAID_PATTERN.test(actionCaid || '')) return null;
  try {
...
export function buildMobileActionIdentity({
  actionReference,
  action,
}: {
  ...
  if (!isCanonicalizable(action)) throw new TypeError('action is not canonicalizable');
  ...
  if (fingerprint === null) throw new TypeError('computed mobile action identity is malformed');
...
export function verifyMobileActionIdentity({
  actionReference,
  action,
  actionCaid,
  actionDigest,
