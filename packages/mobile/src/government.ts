...
export function createGovernmentMobileController({
  ...
  const byHash = new Map([...profiles.values()].map((profile: AnyRecord) => [profile.profile_hash, profile]));
  const issueMembers = new Set([
    'profile_id', 'action_reference', 'approver_id', 'decision', 'platform',
    'app_id', 'device_key_id',
  ]);

  async function isAuthorized(input: AnyRecord): Promise<boolean> {
    try {
      ...
      const authorizeFn = authorize as (input: AnyRecord) => any;
  ...
  return {
    async issue(request: AnyRecord, caller: any = null): Promise<AnyRecord> {
    ...
    async verify(presentation: AnyRecord, caller: any = null): Promise<AnyRecord> {
