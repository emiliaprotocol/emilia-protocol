...
function validOrigin(value) {
    if (typeof value !== 'string' || value.length > 512)
        return false;
    if (ANDROID_APK_ORIGIN.test(value))
        return true;
    try {
...
function validB64u(value, maxBytes = 64 * 1024) {
    if (typeof value !== 'string' || !value || value.length > Math.ceil(maxBytes * 4 / 3) + 4 || !B64U.test(value))
        return false;
    try {
...
export function mobileProfileHash(profile) {
    ...
    if (!body || !isCanonicalizable(body))
        throw new TypeError('mobile profile is not canonicalizable');
    return hashCanonical(body);
}
function normalizeEnrollment(enrollment) {
    if (!isRecord(enrollment))
        throw new TypeError('every enrollment must be an object');
    const normalized = {
 ...
 * @returns {Object}
 */
export function createMobileRelianceProfile({ profileId, rpId, allowedOrigins, acceptedApps, enrollments, attestationRequired = true, hardwareBackedRequired = true, strongIntegrityRequired = true, maxChallengeAgeMs = 300_000, counterPolicy = 'registration_baseline', } = {}) {
...
function actionHash(action) {
    if (!isRecord(action) || !isCanonicalizable(action))
        throw new TypeError('action must be a canonicalizable object');
...
export function buildMobileAttestationBinding(challenge) {
    ...
    if (!isRecord(context))
        throw new TypeError('challenge authorization_context is required');
    return {
...
export function createMobileChallenge({ action, actionReference, policy = null, policyId = null, initiatorId, approverId, approverIndex = 1, requiredApprovals = 1, decision, presentation, platform, appId, deviceKeyId, profile, issuedAt, expiresAt, challengeId = randomId('mob_'), nonce = randomId('sig_'), } = {}) {
    ...
    if (!profile.accepted_apps[ /** @type {'ios'|'android'} */(platform)]?.includes(appId))
        throw new TypeError('app is not accepted by the profile');
 ...
 */
export async function verifyMobileCeremony({ challenge, response, profile, now, attestationVerifier, } = {}) {
    const checks = {
        profile: false,
        freshness: false,
        action: false,
    ...
    try {
        ...
        if (!checks.profile)
            return refused('refuse_profile_mismatch', 'challenge does not name the pinned profile', checks);
        ...
        if (!checks.freshness)
            return refused('refuse_challenge_expired', 'challenge is not fresh under the pinned profile', checks);
        const context = challenge.authorization_context;
        ...
        if (!checks.presentation)
            return refused('refuse_display_mismatch', 'presentation bytes do not match the signed display hash', checks);
        ...
        if (!checks.signed_context)
            return refused('refuse_action_mismatch', 'response substituted signed context, nonce, or decision', checks);
        ...
        if (!checks.platform)
            return refused('refuse_platform', 'platform does not match the signed context', checks);
        checks.app = profile.accepted_apps[binding.platform]?.includes(binding.app_id)
            && response.app_id === binding.app_id;
        if (!checks.app)
            return refused('refuse_app', 'app is not accepted by the pinned profile', checks);
        ...
        if (!enrollment || !checks.device_key)
            return refused('refuse_device_key', 'no active pinned enrollment matches the signed mobile binding', checks);
        ...
        if (!checks.origin)
            return refused('refuse_origin', 'WebAuthn origin is not pinned by the reliance profile', checks);
        ...
        if (!checks.webauthn)
            return refused('refuse_webauthn', 'Class-A assertion failed offline verification', checks, { webauthn });
        ...
        if (profile.requirements.attestation_required) {
            ...
            if (!checks.attestation)
                return refused('refuse_attestation', 'platform attestation did not satisfy the pinned profile', checks);
        }
        else {
 ...
 * @returns {Object}
 */
export function createMobileCeremonyService({ challengeStore, auditLog, attestationVerifier, counterStore = null, commitDecision = null, clock = () => new Date().toISOString(), allowEphemeral = false, } = {}) {
    ...
    if (typeof auditLog?.record !== 'function')
        throw new TypeError('auditLog must implement async record()');
    if (!allowEphemeral && challengeStore.durable !== true)
        throw new TypeError('durable challengeStore is required');
        ...
        async verifyAndConsume({ challenge, response, profile }) {
            ...
            if (commitDecision !== null) {
                ...
                try {
                    ...
                    if (committed === false) {
                        result = refused('refuse_replay', 'the protected action was already decided or is not bound to this challenge', result.checks);
                        return recordOrRefuse(challenge, result);
                    }
                }
                catch {
                    result = refused('refuse_store_unavailable', 'the protected action decision could not be committed', result.checks);
                    return recordOrRefuse(challenge, result);
                }
...
export function verifyMobileAck(ack, publicKeySpkiB64u) {
    try {
        if (!isRecord(ack) || ack['@version'] !== MOBILE_ACK_VERSION
            ...
            || !validB64u(ack.signature.value) || !validB64u(publicKeySpkiB64u, 4096))
            return false;
...
function auditRecordMatches(challenge, result) {
    ...
    if (normalizeHash(hashCanonical(body)) !== normalizeHash(hash))
        return false;
 ...
 * @returns {Object}
 */
export function createMobileExecutionRecord({ challenge, result, receiptId, recordedAt, signerPrivateKey, signerKeyId, } = {}) {
    const context = challenge?.authorization_context;
    const binding = context?.mobile_binding;
    if (!validChallengeShape(challenge)
...
export function verifyMobileExecutionRecord(record, publicKeySpkiB64u) {
    try {
        if (!isRecord(record) || record['@version'] !== MOBILE_EXECUTION_RECORD_VERSION
            ...
            || !validB64u(record.signature.value) || !validB64u(publicKeySpkiB64u, 4096))
            return false;
