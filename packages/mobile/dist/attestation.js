...
function unwrapOpaqueToken(token) {
    ...
    if (bytes.length === 0 || bytes.length > 128 * 1024)
        throw new Error('attestation token size is invalid');
    return bytes;
}
 ...
 */
export function createPlayIntegrityAttestationVerifier({ decodeToken, packageName, certificateDigests, requireLicensed = true, requireStrongIntegrity = true, requireNoCaptureOrControl = false, requirePlayProtect = false, allowedVersionCodes = [], minimumSdkVersion = null, maxTokenAgeMs = 120_000, clock = Date.now, } = {}) {
    if (typeof decodeToken !== 'function')
        throw new TypeError('decodeToken must be a function');
    ...
    const pinnedCertificates = certificateDigests.map((digest) => certificateDigestBytes(digest));
     ...
     */
    return async function verifyPlayIntegrity({ format, token, expected_request_hash: expectedRequestHash, expected_app_id: expectedAppId, expected_attestation_key_id: expectedAttestationKeyId, platform, } = {}) {
        try {
            ...
            if (!isRecord(request) || !isRecord(app) || !isRecord(account) || !isRecord(device))
                return { valid: false };
 ...
 */
export function createAppleAppAttestVerifier({ verifyAssertion, appId, attestationKeyId, environment = 'production', counterStore, } = {}) {
    if (typeof verifyAssertion !== 'function')
        throw new TypeError('verifyAssertion must be a cryptographic App Attest verifier');
    ...
    if (!['development', 'production'].includes(environment))
        throw new TypeError('unsupported App Attest environment');
    if (typeof counterStore?.advance !== 'function')
        throw new TypeError('a durable App Attest counterStore is required');
    /**
     ...
     */
    return async function verifyAppleAppAttest({ format, token, expected_request_hash: expectedRequestHash, expected_binding: expectedBinding, expected_app_id: expectedAppId, expected_attestation_key_id: expectedAttestationKeyId, platform, } = {}) {
        try {
            if (format !== 'apple-app-attest' || platform !== 'ios'
                || expectedAppId !== appId || expectedAttestationKeyId !== attestationKeyId)
                return { valid: false };
            ...
            if (!isRecord(result) || result.valid !== true
                ...
                || !Number.isSafeInteger(result.counter) || result.counter < 1)
                return { valid: false };
            if ((await counterStore.advance(attestationKeyId, result.counter)) !== true)
                return { valid: false };
            return {
                valid: true,
