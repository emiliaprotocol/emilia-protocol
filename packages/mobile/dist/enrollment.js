...
function hash(value) {
    if (!isCanonicalizable(value))
        throw new TypeError('enrollment binding is not canonicalizable');
...
function isP256Spki(value) {
    if (!boundedBase64url(value, 4096))
        return false;
    try {
...
function verifyAndroidDeviceKey(challenge, response) {
    ...
    if (!exactMembers(deviceKey, ANDROID_DEVICE_KEY_MEMBERS)
        ...
        || !boundedBase64url(deviceKey.signature, 256))
        return null;
    try {
        ...
        if (derivedKeyId !== deviceKey.key_id)
            return null;
        ...
        if (response.platform_attestation.request_hash !== requestHash)
            return null;
        const key = crypto.createPublicKey({ key: publicKeyBytes, format: 'der', type: 'spki' });
        const valid = crypto.verify('sha256', Buffer.from(canonicalize(binding), 'utf8'), key, Buffer.from(deviceKey.signature, 'base64url'));
 ...
 */
export function createMobileEnrollmentService({ challengeStore, directory, verifyPasskeyRegistration, verifyPlatformEnrollment, authorizeEnrollment, clock = () => new Date().toISOString(), ttlMs = 300_000, enrollmentValidityMs = 31_536_000_000, allowEphemeral = false, } = {}) {
    ...
    if (!allowEphemeral && directory.durable !== true)
        throw new TypeError('durable enrollment directory is required');
    ...
    return {
        ...
        async complete({ challenge, response, caller = null } = {}) {
            ...
            try {
                ...
                if (stored !== true)
                    return failure('refuse_store_unavailable', 'enrollment was not durably stored');
            }
            catch {
