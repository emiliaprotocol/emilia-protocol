export function createGovernmentMobileController({ service, profiles, resolveRequest, authorize, registerChallenge = null, } = {}) {
    ...
    if (!(profiles instanceof Map) || profiles.size === 0)
        throw new TypeError('profiles must be a non-empty Map');
    if (typeof resolveRequest !== 'function')
        throw new TypeError('resolveRequest must read the system of record');
    if (typeof authorize !== 'function')
        throw new TypeError('authorize must enforce the government caller policy');
    ...
    return {
        async issue(request, caller = null) {
            ...
            if (!profile)
                return { ok: false, verdict: 'refuse_profile_mismatch', challenge: null };
            ...
            if (issued.ok !== true || registerChallenge === null)
                return issued;
            try {
