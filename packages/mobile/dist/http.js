...
function safeJsonNumbers(value) {
    ...
    if (Array.isArray(value))
        return value.every(safeJsonNumbers);
    if (record(value))
        return Object.values(value).every(safeJsonNumbers);
    return true;
}
...
function protocolStatus(result) {
    if (result?.verdict === 'refuse_malformed')
        return 400;
    if (result?.verdict === 'refuse_unauthorized')
        return 403;
    if (['refuse_store_unavailable', 'refuse_audit_unavailable'].includes(result?.verdict))
        return 503;
    return 200;
}
async function parseBody(request, maxBodyBytes) {
    ...
    if (!safeJsonNumbers(value))
        return { ok: false, reason: 'JSON numbers must be safe integers' };
 ...
 * }} [options]
 */
export function createMobileHttpHandler({ controller, enrollmentService, authenticate, resolveEnrollmentIdentity, enrollmentConfig, maxBodyBytes = 1_048_576, routePrefix = '/v1/mobile', } = {}) {
    ...
    if (typeof authenticate !== 'function')
        throw new TypeError('authenticate must return the agency principal');
    ...
    return async function handleMobileRequest(request) {
        ...
        if (!routes.has(url.pathname))
            return refused(404, 'refuse_malformed', 'mobile endpoint not found');
        ...
        if (url.protocol !== 'https:')
            return refused(400, 'refuse_malformed', 'HTTPS is required');
        let caller;
        try {
        ...
        if (!parsed.ok)
            return refused(400, 'refuse_malformed', parsed.reason);
