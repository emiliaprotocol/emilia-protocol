...
function canonicalizeApprovalAction(
  value: unknown,
  path = '$',
  depth = 0,
  ...
  if (typeof value !== 'object') throw new Error(`non_json_value:${path}`);
  if (ancestors.has(value)) throw new Error(`cyclic_json:${path}`);
  ancestors.add(value);
  try {
  if (Array.isArray(value)) {
    if (value.length > 64) throw new Error(`json_array_too_large:${path}`);
      ...
      for (let index = 0; index < value.length; index += 1) {
        ...
        members.push(canonicalizeApprovalAction(
          descriptor.value,
          `${path}[${index}]`,
          depth + 1,
          ancestors,
  ...
  for (const key of keys) {
    if (FORBIDDEN_KEYS.has(key)) throw new Error(`forbidden_key:${path}.${key}`);
      ...
      members.push(`${JSON.stringify(key)}:${canonicalizeApprovalAction(
        descriptor.value,
        `${path}.${key}`,
        depth + 1,
        ancestors,
