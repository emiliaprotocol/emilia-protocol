...
function assertClosedJson(value: unknown, path = '$', depth = 0): void {
  if (depth > 32) throw new Error(`json_too_deep:${path}`);
  if (value === null || typeof value === 'string' || typeof value === 'boolean') return;
  if (typeof value === 'number') {
    if (!Number.isFinite(value)) throw new Error(`non_json_number:${path}`);
    return;
  }
  if (Array.isArray(value)) {
    if (value.length > 64) throw new Error(`json_array_too_large:${path}`);
  ...
  if (keys.length > 64) throw new Error(`json_object_too_large:${path}`);
  for (const key of keys) {
    if (FORBIDDEN_KEYS.has(key)) throw new Error(`forbidden_key:${path}.${key}`);
    assertClosedJson(value[key], `${path}.${key}`, depth + 1);
  }
}

function canonicalizeApprovalAction(value: any): string {
  if (value === null || typeof value !== 'object') return JSON.stringify(value);
  if (Array.isArray(value)) return `[${value.map(canonicalizeApprovalAction).join(',')}]`;
  return `{${Object.keys(value).sort()
    .map((key) => `${JSON.stringify(key)}:${canonicalizeApprovalAction(value[key])}`)
    .join(',')}}`;
}

export function approvalActionHash(action: unknown): string {
  assertClosedJson(action);
  return `sha256:${crypto.createHash('sha256').update(canonicalizeApprovalAction(action), 'utf8').digest('hex')}`;
}

export function validateApprovalAuthorization(input: unknown):
  { ok: true; value: { authorization_endpoint: string; flow: typeof EP_APPROVAL_FLOW } }
  | { ok: false; reason: string } {
  if (!isPlainObject(input) || !exactKeys(input, ['authorization_endpoint', 'flow'])) {
    return { ok: false, reason: 'authorization_not_closed' };
  }
  if (input.flow !== EP_APPROVAL_FLOW) return { ok: false, reason: 'unsupported_approval_flow' };
...
function requirePinnedAuthorization(
  discovered: unknown,
  trusted: unknown,
): { authorization_endpoint: string; flow: typeof EP_APPROVAL_FLOW } {
...
async function resolveRequesterAuthorization(input: RequesterAuthorization | undefined): Promise<string> {
...
export function validateRequiredFields(input: unknown):
  ...
  if (new Set(input).size !== input.length) return { ok: false, reason: 'required_fields_duplicate' };
  return { ok: true, value: [...input] };
}

export function validateCaidSelector(input: unknown):
  { ok: true; value: { field: string } } | { ok: false; reason: string } {
...
function validateChallenge(input: unknown): JsonObject {
  if (!isPlainObject(input)) throw new Error('challenge_invalid');
  const allowed = new Set(['action', 'action_hash', 'required_fields', 'caid_selector']);
  if (Object.keys(input).some((key) => !allowed.has(key))) throw new Error('challenge_not_closed');
  if (typeof input.action !== 'string' || !input.action) throw new Error('challenge_action_invalid');
  ...
  if (!fields.ok) throw new Error('reason' in fields ? fields.reason : 'required_fields_invalid');
  let selector: { field: string } | undefined;
  if (input.caid_selector !== undefined) {
    const checked = validateCaidSelector(input.caid_selector);
    if (!checked.ok) throw new Error('reason' in checked ? checked.reason : 'caid_selector_invalid');
...
function validateBoundAction(action: unknown, challenge: JsonObject): JsonObject {
  if (!isPlainObject(action)) throw new Error('action_invalid');
  if (action.action_type !== challenge.action) throw new Error('action_type_mismatch');
...
async function readBoundedJson(response: Response): Promise<JsonObject> {
  ...
  if (!strictJsonGate(text).ok) throw new Error('approval_response_invalid_json');
  const parsed = JSON.parse(text);
  if (!isPlainObject(parsed)) throw new Error('approval_response_invalid');
  return parsed;
}
...
function validateStatus(value: unknown): asserts value is typeof APPROVAL_STATUSES[number] {
  if (typeof value !== 'string' || !(APPROVAL_STATUSES as readonly string[]).includes(value)) {
    throw new Error('approval_status_invalid');
  }
}

function assertReceiptState(body: JsonObject): void {
...
export async function beginReceiptApproval({
  authorization,
  ...
  if (response.status !== 201) throw new Error(`approval_request_failed:${response.status}`);
  const body = await readBoundedJson(response);
  validateRequestId(body.request_id);
  if (body.status !== 'pending') throw new Error('approval_initial_status_invalid');
  ...
  if (body.receipt !== undefined) throw new Error('receipt_on_nonapproved_status');
  return body;
}

export async function pollReceiptApproval({
  authorization,
  trustedAuthorization,
  request_id,
  poll_token,
  ...
  if (!response.ok) throw new Error(`approval_poll_failed:${response.status}`);
  const body = await readBoundedJson(response);
  validateRequestId(body.request_id);
  if (body.request_id !== request_id) throw new Error('approval_request_id_mismatch');
