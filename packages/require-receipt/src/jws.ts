...
function b64u(buf: any): string {
  return Buffer.from(buf).toString('base64url');
}

function decodeCanonicalB64u(value: string, maxBytes: number): Buffer {
...
function decodeStrictJson(bytes: Buffer, label: string): { text: string; value: any } {
  let text;
  try {
 ...
 */
export function deriveKid(publicKeyBase64url: string): string {
  return crypto
    .createHash('sha256')
...
function ed25519PrivateFromKey(privateKey: any): any {
...
function ed25519PublicFromKey(publicKey: any): any {
 ...
 */
export function serializeReceiptJws(doc: AnyRecord, privateKey: any, opts: { kid?: string; publicKey?: string } = {}) {
 ...
 */
export function verifyReceiptJws(jws: string, publicKey: any) {
  ...
  try {
    ...
    return { valid: false, checks, error: `Malformed JWS segment: ${e instanceof Error ? e.message : String(e)}` };
  }
  ...
  try {
    ...
    return { valid: false, checks, header, error: `Signature verification failed: ${e instanceof Error ? e.message : String(e)}` };
  }
  if (!checks.signature) {
  ...
  try {
    ...
    return { valid: false, checks, header, error: `Payload is not valid JSON: ${e instanceof Error ? e.message : String(e)}` };
  }
