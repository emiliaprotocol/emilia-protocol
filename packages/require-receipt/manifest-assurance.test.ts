...
test('FAIL-CLOSED: a guarded action with no assurance_class is rejected', () => {
  ...
  assert.ok(
    r.errors.some((e) => e.includes('assurance_class is required when receipt_required is true')),
    `expected a required-assurance_class error, got: ${JSON.stringify(r.errors)}`,
  );
});
...
test('KEY-CLASS FLOOR: a critical action MUST NOT accept the software tier', () => {
  ...
  assert.equal(r.ok, false);
  assert.ok(
    r.errors.some((e) => e.includes('must be class_a or quorum when risk is critical')),
    `expected a critical-key-class-floor error, got: ${JSON.stringify(r.errors)}`,
  );
});
...
test('FAIL-CLOSED: a guarded action with no execution_binding is rejected', () => {
  ...
  assert.equal(r.ok, false);
  assert.ok(
    r.errors.some((e) => e.includes('execution_binding.required_fields must be a non-empty array')),
    `expected a required-execution_binding error, got: ${JSON.stringify(r.errors)}`,
  );
});
...
test('FAIL-CLOSED: an empty or malformed required_fields list is rejected', () => {
  ...
  for (const execution_binding of bindings) {
    ...
    assert.ok(
      r.errors.some((e) => e.includes('execution_binding.required_fields')),
      `expected an execution_binding error, got: ${JSON.stringify(r.errors)}`,
    );
  }
});
