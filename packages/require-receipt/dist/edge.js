...
function normalizeCaidSelector(value) {
    if (value === undefined)
        return undefined;
...
function addHeader(values, rawName, rawValue) {
...
function oneHeader(headers, name) {
    ...
    if (values.length !== 1 || values[0].includes(','))
        throw new Error('ambiguous_proof_header');
    return values[0];
}
...
function directBodyLength(body) {
...
async function streamLength(stream, maximum) {
...
async function boundedBodyLength(request, headers, maximum) {
    const advertised = parseContentLength(headers);
    if (advertised !== undefined && advertised > maximum)
        throw new Error('request_body_too_large');
    let measured;
    if (request.bodyByteLength !== undefined) {
        if (!Number.isSafeInteger(request.bodyByteLength) || request.bodyByteLength < 0) {
            throw new Error('request_body_unverifiable');
        }
        measured = request.bodyByteLength;
    }
    ...
    if (measured > maximum)
        throw new Error('request_body_too_large');
    if (advertised !== undefined && advertised !== measured)
...
function problemDetail(reason) {
    if (!reason)
        return 'This action requires an accountable, verifiable authorization receipt.';
    return 'The presented authorization receipt could not satisfy this action challenge.';
}
function safeVerifierReason(reason) {
...
function requestTarget(request) {
    ...
    if (!/^[A-Z][A-Z0-9-]{0,31}$/.test(method))
        throw new Error('request_method_invalid');
 ...
 */
export function createReceiptRequiredEdgeHandler(options) {
    ...
    if (typeof options.verifyReceipt !== 'function')
        throw new Error('verifyReceipt_required');
    if (options.consume !== undefined && typeof options.consume !== 'function')
        throw new Error('consume_invalid');
    ...
    if (!HEADER_NAME_PATTERN.test(proofHeader))
        throw new Error('proofHeader_invalid');
    ...
    if (maxReceiptBytes > maxHeaderBytes)
        throw new Error('maxReceiptBytes_exceeds_header_limit');
    if (options.manifestUrl !== undefined) {
        let manifest;
        try {
            manifest = new URL(options.manifestUrl);
        }
        catch {
            throw new Error('manifestUrl_invalid');
        }
    ...
    return async function authorize(request) {
        ...
        try {
            ...
            if (headers.bytes > maxHeaderBytes)
                throw new Error('request_headers_too_large');
            ...
            if (options.projectAction) {
                ...
                if (actionHash && actionHash !== projectedHash)
                    throw new Error('observed_action_hash_mismatch');
            ...
            if (carrier === undefined || carrier.length === 0)
                return refuse();
            if (byteLength(carrier) > maxReceiptBytes)
                throw new Error('receipt_header_too_large');
            const context = {
                action,
            ...
            if (boundAction !== action)
                return refuse('action_binding_mismatch');
            ...
            if (options.consume) {
                let consumed;
                try {
                    consumed = await options.consume(receiptId, context);
                }
                catch {
