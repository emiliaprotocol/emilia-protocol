...
function decodeStrictJson(bytes, label) {
    ...
    if (!strict.ok)
        throw new Error(`${label} is not strict JSON: ${strict.reason}`);
...
export function serializeReceiptJws(doc, privateKey, opts = {}) {
    if (!isPlainObject(doc))
        throw new TypeError('doc must be an EP-RECEIPT-v1 object');
    ...
    if (!isPlainObject(doc.payload))
        throw new Error('doc.payload must be an object');
