...
// Generated from gate.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
...
test('whitespace-only receipt_id -> refused before any reservation', async () => {
    ...
    const result = await g.check(mint('db.records.delete:customers', { receiptId: ' \t\n ' }), { target: 'customers' });
...
test('legacy check-then-add stores are rejected as not fleet-safe', () => {
    assert.throws(() => makeReceiptGate({ action: 'db.records.delete', store: { has() { return false; }, add() { } } }), /legacy \{has, add\} stores are not fleet-safe/);
});
...
function assuranceKit() {
    ...
    function signA(signer, digest, approver, { rpId = 'www.emiliaprotocol.ai', origin = 'https://www.emiliaprotocol.ai', duplicateChallenge = false, } = {}) {
