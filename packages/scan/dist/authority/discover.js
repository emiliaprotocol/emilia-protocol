...
import { describeEnv, describeSecret, redactText, sanitizeArgs, sanitizeForReport, } from './redact.js';
...
function writableByCurrentProcess(value) {
    try {
...
export function configCandidates(options = {}) {
    ...
    for (let depth = 0; depth < 8; depth += 1) {
        project.push({ runtime: 'claude-code', scope: 'project', file: path.join(directory, '.mcp.json') }, { runtime: 'claude-code', scope: 'project', file: path.join(directory, '.claude', 'settings.json') }, { runtime: 'claude-code', scope: 'project', file: path.join(directory, '.claude', 'settings.local.json') }, { runtime: 'cursor', scope: 'project', file: path.join(directory, '.cursor', 'mcp.json') });
...
function normalizeServers(parsed, home) {
    const out = [];
    const push = (rawName, rawDeclaration) => {
    ...
    if (mcpServers && typeof mcpServers === 'object' && !Array.isArray(mcpServers)) {
        for (const [name, declaration] of Object.entries(mcpServers))
...
function normalizePermissions(parsed, home) {
    const raw = parsed.permissions;
    if (!raw || typeof raw !== 'object' || Array.isArray(raw))
        return null;
    const permissions = raw;
    const list = (key) => (Array.isArray(permissions[key])
        ? permissions[key].map((rule) => redactText(String(rule)))
        : []);
    return {
        allow: list('allow'),
        deny: list('deny'),
...
export function credentialFiles(home = os.homedir()) {
    const candidates = [
