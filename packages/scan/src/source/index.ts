...
import type {
  CompositionFinding,
  DynamicRegistration,
  SourceBaselineAction,
  SourceDiscoveryBaseline,
  SourceDiscoveryDiff,
...
export const SOURCE_DISCOVERY_VERSION = 'EP-SOURCE-DISCOVERY-v1' as const;
...
function lineBytes(text: string, index: number, end = index): string {
  const start = text.lastIndexOf('\n', index - 1) + 1;
  const lineEnd = text.indexOf('\n', Math.max(index, end));
  return text.slice(start, lineEnd === -1 ? text.length : lineEnd);
}

function literalAt(value: string): { value: string; consumed: number } | null {
  const match = /^\s*(['"`])([^'"`$\r\n]{1,256})\1/.exec(value);
  return match ? { value: match[2], consumed: match[0].length } : null;
}

function propertyName(value: string): { value: string; offset: number; consumed: number } | null {
...
function pushCandidate(candidates: Candidate[], candidate: Candidate): void {
  if (candidate.name !== null && !SAFE_NAME.test(candidate.name)) candidate.name = null;
...
function parseJsTs(text: string): Candidate[] {
  ...
  for (const match of searchable.matchAll(genkit)) {
    if (!hasGenkit && !/^ai\.defineTool/.test(match[0])) continue;
  ...
  if (!hasLangChain && !hasVercel) return candidates;
...
function parsePython(text: string): Candidate[] {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    for (let next = index + 1; next < Math.min(lines.length, index + 5); next += 1) {
      ...
      if (definition) { functionName = definition[1]; break; }
      if (lines[next].trim() && !lines[next].trim().startsWith('@')) break;
    }
    pushCandidate(candidates, {
...
function parseJava(text: string): Candidate[] {
  const candidates: Candidate[] = [];
...
function capabilityNames(actions: SourceEvidence[], expression: RegExp): string[] {
  return [...new Set(actions.filter((action) => expression.test(action.name)).map((action) => action.name))].sort();
}

function finding(id: string, severity: CompositionFinding['severity'], title: string, groups: string[][], reason: string): CompositionFinding | null {
...
function composition(actions: SourceEvidence[]): CompositionFinding[] {
  ...
  ].filter((item): item is CompositionFinding => item !== null).sort((a, b) => a.id.localeCompare(b.id));
}

function tighten(actions: SourceEvidence[], findings: CompositionFinding[]): void {
...
function proposedBaseline(actions: SourceEvidence[]): SourceDiscoveryBaseline {
  const counts = new Map<string, number>();
  for (const action of actions) counts.set(action.name, (counts.get(action.name) ?? 0) + 1);
  const proposed: SourceBaselineAction[] = actions
...
export function scanSourceDirectory(rootInput: string, options: SourceDiscoveryOptions = {}): SourceDiscoveryReport {
  ...
  if (rootStat.isSymbolicLink() || !rootStat.isDirectory()) throw new Error('source root must be a non-symlink directory');
  const root = fs.realpathSync(rootInput);
  const files: SourceDiscoveryReport['files'] = [];
  const actions: SourceEvidence[] = [];
  const dynamic: DynamicRegistration[] = [];
  const skipped: SourceDiscoveryReport['skipped'] = [];
  let totalBytes = 0;

  function walk(directory: string, depth: number): void {
    if (depth > limits.maxDepth) { skipped.push({ file: path.relative(root, directory) || '.', reason: 'max_depth' }); return; }
    const entries = fs.readdirSync(directory, { withFileTypes: true }).sort((a, b) => a.name.localeCompare(b.name));
    for (const entry of entries) {
      const absolute = path.join(directory, entry.name);
      const relative = path.relative(root, absolute).split(path.sep).join('/');
      const stat = fs.lstatSync(absolute);
      if (stat.isSymbolicLink()) { skipped.push({ file: relative, reason: 'symlink' }); continue; }
      if (stat.isDirectory()) {
      ...
      if (!language || !stat.isFile()) continue;
      if (files.length >= limits.maxFiles) { skipped.push({ file: relative, reason: 'max_files' }); continue; }
      if (stat.size > limits.maxFileBytes) { skipped.push({ file: relative, reason: 'file_too_large' }); continue; }
      if (totalBytes + stat.size > limits.maxTotalBytes) { skipped.push({ file: relative, reason: 'total_bytes' }); continue; }
      ...
      if (bytes.includes(0)) { skipped.push({ file: relative, reason: 'binary' }); continue; }
      let text: string;
      try { text = new TextDecoder('utf-8', { fatal: true }).decode(bytes); }
      catch { skipped.push({ file: relative, reason: 'invalid_utf8' }); continue; }
...
function baselineActions(baseline: unknown): Map<string, SourceBaselineAction> {
  if (!baseline || typeof baseline !== 'object' || !Array.isArray((baseline as any).actions)) {
    throw new Error('baseline must contain an actions array');
  }
  const out = new Map<string, SourceBaselineAction>();
  for (const raw of (baseline as any).actions) {
    const name = typeof raw?.name === 'string' ? raw.name : raw?.match?.tool;
    if (typeof name !== 'string' || !SAFE_NAME.test(name)) continue;
    if (out.has(name)) throw new Error(`baseline contains duplicate action name: ${name}`);
    out.set(name, {
      name,
...
export function diffSourceDiscovery(current: SourceDiscoveryReport, baseline: unknown): SourceDiscoveryDiff {
  const previous = baselineActions(baseline);
  const counts = new Map<string, number>();
  for (const action of current.actions) counts.set(action.name, (counts.get(action.name) ?? 0) + 1);
...
export function sourceDiscoveryExitCode(value: SourceDiscoveryReport | SourceDiscoveryDiff): number {
  if ('requires_review' in value) return value.requires_review ? 1 : 0;
