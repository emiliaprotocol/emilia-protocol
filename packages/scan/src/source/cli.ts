...
type Parsed = { command: 'source' | 'diff'; directory: string; baseline: string | null; json: boolean; out: string | null; help: boolean };

function parse(argv: string[]): Parsed {
  const command = argv[0];
  if (command !== 'source' && command !== 'diff') throw new Error('expected source or diff command');
  ...
  for (let index = 1; index < argv.length; index += 1) {
    ...
    if (argument === '--help' || argument === '-h') help = true;
      ...
      if (argument === '--baseline') {
        if (baseline !== null) throw new Error('duplicate option: --baseline');
        baseline = value;
      } else {
        if (out !== null) throw new Error('duplicate option: --out');
        out = value;
      }
      index += 1;
    } else if (argument.startsWith('-')) throw new Error(`unknown option: ${argument}`);
    else if (directory === null) directory = argument;
    else throw new Error('provide exactly one source directory');
  }
  if (!help && directory === null) throw new Error('source directory is required');
  if (command === 'diff' && !help && baseline === null) throw new Error('diff requires --baseline');
  if (command === 'source' && baseline !== null) throw new Error('--baseline is valid only with diff');
  return { command, directory: directory ?? '.', baseline, json, out, help };
}

function readBaseline(file: string): unknown {
  const stat = fs.lstatSync(file);
  if (stat.isSymbolicLink() || !stat.isFile()) throw new Error('baseline must be a non-symlink regular file');
  if (stat.size > MAX_BASELINE_BYTES) throw new Error('baseline exceeds 8 MiB');
  const raw = fs.readFileSync(file, 'utf8');
  const gate = strictJsonGate(raw);
  if (!gate.ok) throw new Error(`baseline refused: ${gate.reason}`);
  return JSON.parse(raw);
}

function renderText(value: ReturnType<typeof scanSourceDirectory> | ReturnType<typeof diffSourceDiscovery>): string {
...
export function sourceMain(argv: string[], io: SourceIo = {
  stdout: (value) => process.stdout.write(value),
  stderr: (value) => process.stderr.write(value),
}): number {
  try {
    const options = parse(argv);
    if (options.help) { io.stdout(USAGE); return 0; }
