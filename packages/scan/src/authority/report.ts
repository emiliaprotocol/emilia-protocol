...
import type { AuthorityScanResult, AuthoritySignal } from './types.js';

export const AUTHORITY_CLAIM_BOUNDARY =
  'config_derived_reachability_only_not_behavioral_not_exploitability_not_an_authorization_guarantee';
...
function severityTag(severity: AuthoritySignal['severity']): string {
  return severity.toUpperCase();
}

export function renderAuthorityText(input: AuthorityScanResult): string {
  const result = sanitizeForReport(input, 'report') as AuthorityScanResult;
  ...
  if (unsupported.length) {
    ...
    for (const source of unsupported) lines.push(`  ${source.file} - not covered by this report.`);
  }
  if (unreadable.length) lines.push(`Unreadable configuration sources excluded: ${unreadable.length}.`);
  if (malformed.length) lines.push(`Malformed JSON sources excluded: ${malformed.length}.`);
  if (tooLarge.length) lines.push(`Oversized sources excluded: ${tooLarge.length}.`);
  if (symlinks.length) lines.push(`Symlinked configuration sources excluded: ${symlinks.length}.`);
  ...
  for (const finding of [...signals].sort(
    ...
    if (observations.length > 12) lines.push(`    ... and ${observations.length - 12} more`);
    lines.push('');
  ...
  for (const item of AUTHORITY_SCOPE.reports) lines.push(`  - ${item}`);
  lines.push('', 'What it does not report:');
  for (const item of AUTHORITY_SCOPE.does_not_report) lines.push(`  - ${item}`);
...
export function renderAuthorityJson(input: AuthorityScanResult): string {
...
export function authorityExitCode(result: AuthorityScanResult): number {
  if (result.inventory.sources.some((source) => source.status === 'malformed')) return 2;
  if (result.signals.length) return 1;
  if (!result.summary.coverage.computable) return 3;
  return 0;
}

export function writePrivateReport(file: string, output: string): void {
  try {
