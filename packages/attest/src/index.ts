...
function toBuf(input: AttestInput): Buffer {
...
export function sha256Hex(input: AttestInput): string {
  return crypto.createHash('sha256').update(toBuf(input as AttestInput)).digest('hex');
}

/** Constant-time hex-string comparison. */
function hexEqual(a: unknown, b: unknown): boolean {
 ...
 */
export function verifyIdentity({ identity, knownGoodHash }: { identity?: AttestInput; knownGoodHash?: string } = {}): IdentityCheck {
  let computedHash = null;
  try { computedHash = sha256Hex(identity as AttestInput); } catch { return { verified: false, computedHash }; }
...
export function signWorkReceipt({
  ...
  const document = {
    '@version': 'EP-RECEIPT-v1' as const,
    payload,
    signature: { algorithm: 'Ed25519' as const, value: signature },
