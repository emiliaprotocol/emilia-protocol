...
export function classifyToolCall(name, args = {}, opts = {}) {
    ...
    if (override === true)
        return { irreversible: true, reason: 'per_call_override' };
    const ann = annotations[name];
    if (ann) {
        if (ann.irreversible === true)
            return { irreversible: true, reason: 'annotation' };
        if (ann.irreversible === false)
            return { irreversible: false, reason: 'annotation' };
        ...
        if (ann.destructiveHint === true)
            return { irreversible: true, reason: 'destructiveHint' };
    ...
    if (typeof policy === 'function') {
        try {
            ...
            if (p === true)
                return { irreversible: true, reason: 'policy_fn' };
            if (p === false)
                return { irreversible: false, reason: 'policy_fn' };
            ...
            if (p !== null && typeof p === 'object' && typeof p.irreversible === 'boolean') {
                return { irreversible: p.irreversible, reason: 'policy_fn' };
            }
        }
        catch {
...
function extractReceipt(args = {}, meta = {}) {
    const ep = args.__ep || {};
    if (ep.receipt && typeof ep.receipt === 'object')
        return ep.receipt;
    ...
    if (args.emilia_receipt && typeof args.emilia_receipt === 'object')
...
export function refusal(action, reason, extra = {}) {
    ...
    return {
        ...
        required: {
            ...challenge.required,
            how: 'Gate this action first (ep_guard_action / the trust gate), obtain an ' +
...
export function withMcpGuard(handler, options = {}) {
    ...
    const resolveAnnotations = (name) => {
        let fromResolver;
        try {
            fromResolver = typeof getAnnotations === 'function' ? getAnnotations(name) : undefined;
        }
        catch {
            fromResolver = { destructiveHint: true };
        }
 ...
 */
export function withCustomerOwnedProtectionGateway(handler, options = {}) {
    if (typeof handler !== 'function') {
...
export function withMcpReceiptGuard(handler, options = {}) {
    ...
    const { policy, annotations = {}, getAnnotations, defaultIrreversible = true, trustReadOnlyHints = false, executingSystem = 'mcp-server', receiptParams, returnEnvelope = false, } = options;
    const resolveAnnotations = (name) => {
        let fromResolver;
        try {
            fromResolver = typeof getAnnotations === 'function' ? getAnnotations(name) : undefined;
...
function receiptHashOf(doc) {
    ...
    if (doc.public_key !== undefined)
        core.public_key = doc.public_key;
    try {
