...
type ScanReport = {
  ...
  summary?: { dangerous?: number; missing_declaration?: number };
  [key: string]: unknown;
};
type ScanFn = (input: unknown) => ScanReport;
type CorpusEntry = { name: string; slug: string; repo: string; manifest: unknown };

let scan: ScanFn;
let FIRE_DRILL_VERSION: string;
let TAGLINE: string;
let REPRESENTATIVE_CORPUS: CorpusEntry[];
let strictJsonGate: (input: string) => { ok: boolean; reason?: string };
...
try { ({ strictJsonGate } = await import(VERIFY_STRICT_JSON_PACKAGE)); }
catch { ({ strictJsonGate } = await import(VERIFY_STRICT_JSON_FALLBACK)); }
...
export async function handleToolRequest(request: { params: { name: string; arguments?: JsonRecord } }) {
