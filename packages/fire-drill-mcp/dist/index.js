...
let strictJsonGate;
const FIRE_DRILL_PACKAGE = '@emilia-protocol/fire-drill';
const FIRE_DRILL_CORPUS_PACKAGE = '@emilia-protocol/fire-drill/corpus.js';
const VERIFY_STRICT_JSON_PACKAGE = '@emilia-protocol/verify/strict-json';
...
try {
    ...
    ({ scan, FIRE_DRILL_VERSION, TAGLINE } = await import(FIRE_DRILL_FALLBACK));
    ({ REPRESENTATIVE_CORPUS } = await import(FIRE_DRILL_CORPUS_FALLBACK));
}
try {
    ({ strictJsonGate } = await import(VERIFY_STRICT_JSON_PACKAGE));
}
catch {
    ({ strictJsonGate } = await import(VERIFY_STRICT_JSON_FALLBACK));
}
const MAX_INPUT_BYTES = 8 * 1024 * 1024;
export const TOOLS = [
    {
        name: 'fire_drill_scan',
        description: 'Scan an MCP manifest, OpenAPI spec, or tool list for detected high-risk operations that omit a ' +
        ...
        inputSchema: {
            ...
            properties: {
                target: {
                    type: 'object',
                    description: 'The thing to scan, as a JSON object: an MCP manifest ({ tools: [...] }), an OpenAPI spec, ' +
    ...
    },
    {
        ...
        description: 'A representative static declaration corpus, sorted by missing required receipt declarations. ' +
...
export async function handleToolRequest(request) {
    ...
    if (name === 'fire_drill_scan') {
        ...
        if (input === undefined && typeof args.target_json === 'string') {
            try {
                if (Buffer.byteLength(args.target_json, 'utf8') > MAX_INPUT_BYTES)
                ...
                return fail(`target_json refused: ${error instanceof Error ? error.message : String(error)}`);
            }
        }
        ...
        try {
            ...
            if (Buffer.byteLength(encoded, 'utf8') > MAX_INPUT_BYTES)
                throw new Error('target exceeds input limit');
            const report = scan(input);
            return ok({ fire_drill_version: FIRE_DRILL_VERSION, tagline: TAGLINE, report });
        }
        catch (e) {
            return fail(`Scan failed: ${e instanceof Error ? e.message : String(e)}`);
        }
    }
...
export function createServer() {
    const server = new Server({ name: 'emilia-fire-drill', version: FIRE_DRILL_MCP_VERSION }, { capabilities: { tools: {} } });
