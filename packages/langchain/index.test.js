...
test('E-1: each entry point runs EXACTLY ONCE with a valid receipt (no double consumption)', async () => {
    ...
    assert.deepEqual(await guarded.call(viaCall, cfg(mintReceipt({ action: exact(viaCall) }))), { ok: true, ran: viaCall });
    const viaBatch = { to: 'acct_batch' };
    assert.deepEqual(await guarded.batch([viaBatch], cfg(mintReceipt({ action: exact(viaBatch) }))), [{ ok: true, ran: viaBatch }]);
    const viaStream = { to: 'acct_stream' };
    assert.deepEqual(await guarded.stream(viaStream, cfg(mintReceipt({ action: exact(viaStream) }))), { ok: true, ran: viaStream });
...
test('E-1: .batch refuses the whole batch when any element is unauthorized', async () => {
    ...
    await assert.rejects(() => guarded.batch([authorized, unauthorized], [cfg(mintReceipt({ action: exact(authorized) })), cfg(null)]), /EMILIA blocked/);
...
test('E-1: a method reaching the effect via this.invoke lands on the gate', async () => {
    ...
    await assert.rejects(() => guarded.transform({ to: 'acct_attacker' }, cfg(null)), /EMILIA blocked/);
    assert.equal(tool.calls.length, 0);
});
