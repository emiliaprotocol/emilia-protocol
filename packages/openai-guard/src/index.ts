...
function denyResult(reason: string, raw: AnyRecord = {}): AnyRecord {
  return { allow: false, deny: true, signoffRequired: false, decision: 'deny', reason, raw };
}

function normalizedHttpsEndpoint(value: string, allowInsecureHttp = false): string {
...
function snapshotJson(value: any): any {
...
export function requireReceiptForOpenAITool(fn: ToolFunction, opts: AnyRecord = {}): ToolFunction {
  ...
  return async function receiptGuarded(args = {}, call = {}) {
    ...
    if (!result.ok) {
      ...
      const error = new Error(`EMILIA blocked "${boundAction}": ${reason}`) as Error & { emilia?: AnyRecord };
 ...
 */
export async function guard(action: string | AnyRecord, opts: AnyRecord = {}): Promise<AnyRecord> {
  const spec: AnyRecord = typeof action === 'string' ? { action } : { ...(action || {}) };
 ...
 */
export function withGuard(fn: ToolFunction, opts: AnyRecord = {}): ToolFunction {
 ...
 */
export async function runToolCalls(toolCalls: any[] = [], tools: AnyRecord = {}, opts: AnyRecord = {}): Promise<any[]> {
  /** @type {Array<{role:'tool', tool_call_id:string, name:string, content:string}>} */
  const out = [];
  for (const tc of toolCalls as AnyRecord[]) {
