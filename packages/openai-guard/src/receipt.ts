...
function normalizeBase(base: string, allowInsecureHttp = false): string {
...
async function epPost(base: string, pathname: string, apiKey: string, body: AnyRecord, fetchImpl: any, allowInsecureHttp = false): Promise<AnyRecord> {
  ...
  if (!res.ok) {
    const err: Error & { status?: number; body?: unknown } = (
 ...
 */
export function mintReceipt({ apiKey, base = DEFAULT_BASE, fetchImpl, allowInsecureHttp, ...receipt }: AnyRecord = {}): Promise<AnyRecord> {
 ...
 */
export function requestSignoff({ apiKey, base = DEFAULT_BASE, fetchImpl, allowInsecureHttp, receipt_id, comment, expires_in_minutes }: AnyRecord = {}): Promise<AnyRecord> {
 ...
 */
export function approveSignoff({ apiKey, base = DEFAULT_BASE, fetchImpl, allowInsecureHttp, signoff_id, comment }: AnyRecord = {}): Promise<AnyRecord> {
 ...
 * @param {{ apiKey?: string, base?: string, fetchImpl?: typeof fetch, allowInsecureHttp?: boolean, signoff_id?: string, comment?: string }} [o]
 */
export function rejectSignoff({ apiKey, base = DEFAULT_BASE, fetchImpl, allowInsecureHttp, signoff_id, comment }: AnyRecord = {}): Promise<AnyRecord> {
 ...
 */
export async function verifyReceipt(doc: AnyRecord, publicKeyBase64url: string): Promise<any> {
  let verify;
  try {
