...
async function epPost(base, pathname, apiKey, body, fetchImpl, allowInsecureHttp = false) {
    ...
    if (!data || typeof data !== 'object' || Array.isArray(data))
        throw new Error(`EMILIA ${pathname}: malformed response`);
    return data;
}
/**
...
export function mintReceipt({ apiKey, base = DEFAULT_BASE, fetchImpl, allowInsecureHttp, ...receipt } = {}) {
    if (!apiKey)
        throw new Error('mintReceipt: apiKey is required');
    for (const f of ['organization_id', 'action_type', 'target_resource_id']) {
        if (!receipt[f])
            throw new Error(`mintReceipt: ${f} is required`);
    }
...
export function requestSignoff({ apiKey, base = DEFAULT_BASE, fetchImpl, allowInsecureHttp, receipt_id, comment, expires_in_minutes } = {}) {
    if (!apiKey)
        throw new Error('requestSignoff: apiKey is required');
    if (!receipt_id)
        throw new Error('requestSignoff: receipt_id is required');
...
export function approveSignoff({ apiKey, base = DEFAULT_BASE, fetchImpl, allowInsecureHttp, signoff_id, comment } = {}) {
    if (!apiKey)
        throw new Error("approveSignoff: apiKey is required (the approving human's key)");
    if (!signoff_id)
        throw new Error('approveSignoff: signoff_id is required');
...
export function rejectSignoff({ apiKey, base = DEFAULT_BASE, fetchImpl, allowInsecureHttp, signoff_id, comment } = {}) {
    if (!apiKey)
        throw new Error('rejectSignoff: apiKey is required');
    if (!signoff_id)
        throw new Error('rejectSignoff: signoff_id is required');
