...
function snapshotJson(value) {
    if (!isCanonicalizable(value))
        throw new Error('action_binding_invalid');
 ...
 */
export async function guardAction({ actor, entityId, action, context = {}, apiKey, gateUrl = DEFAULT_GATE, fetchImpl, allowInsecureHttp = false, } = {}) {
    if (!action)
    ...
    if (!apiKey)
        return denyResult('api_key_required');
    const entity_id = entityId || actor;
    if (typeof entity_id !== 'string' || !entity_id)
        return denyResult('entity_id_required');
    let endpoint;
    try {
    ...
    if (!raw || typeof raw !== 'object' || Array.isArray(raw))
        return denyResult('malformed_gate_response');
...
export function requireReceiptForOpenAITool(fn, opts = {}) {
    if (typeof fn !== 'function')
        throw new TypeError('requireReceiptForOpenAITool: fn must be a function');
    const { action, actionFor, toolName, getReceipt = (args, call = {}) => call.receipt ?? args?.__ep?.receipt ?? null, store = processLocalStore, ...gateOptions } = opts;
...
export function withGuard(fn, opts = {}) {
    if (typeof fn !== 'function')
        throw new Error('withGuard: first argument must be your tool function');
        ...
        if (decision.signoffRequired) {
            if (typeof onSignoff === 'function') {
                ...
                if (signoff?.approved !== true)
                    throw new Error(`EMILIA did not receive explicit signoff for "${action}"`);
            }
            else {
