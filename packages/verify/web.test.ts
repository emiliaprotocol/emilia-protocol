...
function makeReceipt(payload: any): any {
...
function makeCommitmentProof(): any {
  const { privateKey, publicKey } = crypto.generateKeyPairSync('ed25519');
  const commitment = {
    domain: 'demo',
...
async function assertEquivalentReceipt(doc: any, spki: any, expectValid?: boolean): Promise<void> {
...
async function assertEquivalentSignoff(signoff: any, spki: any, opts: any, expectValid?: boolean): Promise<void> {
...
async function assertEquivalentProof(proof: any, spki: any, opts: any, expectValid?: boolean): Promise<void> {
