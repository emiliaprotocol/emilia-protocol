...
function jwtInput() {
  return {
    ...common(),
    carrier_provenance: {
      source_carrier: 'AIC-JWT-COMPACT' as const,
      compact_token: JWT_COMPACT_TOKEN,
      presented_principal_jwk: PRINCIPAL_JWK,
      downstream_representation: 'DIRECT' as const,
    },
    principal_binding: {
...
function x509Input() {
  return {
    ...
    carrier_provenance: {
      source_carrier: 'AIC-X509-CREDENTIAL-BUNDLE' as const,
      agent_certificate_der: AGENT_CERTIFICATE_DER,
      principal_certificate_der: PRINCIPAL_CERTIFICATE_DER,
    },
    principal_binding: {
...
test('bound mapping requires the same exact action and relying-party admission domain', () => {
  ...
  if (!result.ok) return;
  assert.equal(
    result.authority.mapping_profile_id,
    AIC_JWT_JKT_BOUND_CROSSING_MAPPING_PROFILE,
  );

  const substitutedAction = mapAicJwtJktBoundCrossingAuthority(
    boundJwtInput(),
    {
      ...boundContext(),
  ...
  const substitutedDomain = mapAicJwtJktBoundCrossingAuthority(
    boundJwtInput(),
    {
      ...boundContext(),
      admission_domain: {
  ...
  const substitutedMappingPolicy = mapAicJwtJktBoundCrossingAuthority(
    boundJwtInput(),
    {
      ...boundContext(),
      policy: {
...
test('bound mapping fails closed on non-current, stale, future, or out-of-window source status', () => {
  assert.deepEqual(
    ...
    { ok: false, reason: 'aic_status_not_current' },
  );
  assert.deepEqual(
    mapAicJwtJktBoundCrossingAuthority(
      {
  ...
  assert.deepEqual(
    ...
    { ok: false, reason: 'aic_status_observation_future' },
  );
  assert.deepEqual(
    mapAicJwtJktBoundCrossingAuthority(
      {
...
test('X.509 replay identity comes from exact DER rather than free wrapper labels', () => {
  ...
  if (!original.ok || !relabeledArtifact.ok || !relabeledIssuer.ok) return;
  ...
  assert.notEqual(
    original.authority.authority_instance_digest,
    relabeledArtifact.authority.authority_instance_digest,
  );
  assert.notEqual(
    original.authority.authority_instance_digest,
    relabeledIssuer.authority.authority_instance_digest,
  );
});
...
test('JWT replay identity follows issuer and jti across a re-signed compact token', () => {
  ...
  const original = mapAicJwtJktBoundCrossingAuthority(
    boundJwtInput(),
    boundContext(),
  );
  ...
  if (!original.ok || !resigned.ok) return;
  assert.equal(original.authority.replay_unit, resigned.authority.replay_unit);
  assert.notEqual(
    original.authority.authority_instance_digest,
    resigned.authority.authority_instance_digest,
  );
});
...
test('JWT-SVID projection refuses type confusion, multiple audiences, and authority-semantic loss', () => {
  ...
  assert.deepEqual(
    ...
    { ok: false, reason: 'jwt_svid_source_status_stale' },
  );
  assert.deepEqual(
    projectAicJwtToStrictJwtSvid({
      ...base,
    ...
    { ok: false, reason: 'jwt_svid_source_validity_mismatch' },
  );
  assert.deepEqual(
    projectAicJwtToStrictJwtSvid({
      ...base,
      has_constraints: true,
    } as any, projectionContext()),
