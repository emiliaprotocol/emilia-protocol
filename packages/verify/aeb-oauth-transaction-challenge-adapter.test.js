...
import { OAUTH_TXN_CHALLENGE_AEB_ADAPTER_ID, OAUTH_TXN_CHALLENGE_AEB_ADAPTER_VERSION, OAUTH_TXN_CHALLENGE_CONFIG_VERSION, OAUTH_TXN_CHALLENGE_DRAFT_REVISION, OAUTH_TXN_CHALLENGE_MAPPING_VERSION, OAUTH_TXN_CHALLENGE_MAPPER_ID, OAUTH_TXN_CHALLENGE_REPLAY_NAMESPACE, OAUTH_TXN_CHALLENGE_TRUST_ROOT_VERSION, createOAuthTransactionChallengeActionDefinition, createOAuthTransactionChallengeAebAdapter, } from './aeb-oauth-transaction-challenge-adapter.js';
...
test('the mapping pins a complete semantic-loss declaration with relying-party bases', () => {
    ...
    const definition = complete.definition;
    ...
    assert.ok(definition.semantic_loss_contract.omitted_nonmaterial_fields.every((entry) => typeof entry.path === 'string' && typeof entry.relying_party_basis === 'string'));
...
test('a second access token for the same protected-resource transaction shares one replay unit', () => {
    ...
    const twin = adapter.verifyNative(input(fixture, {
        artifact: {
            ...fixture.artifact,
            access_token_jwt: compactEs256({ alg: 'ES256', typ: 'at+jwt', kid: 'as-es256-1' }, twinClaims, fixture.asKey.privateKey),
        },
    }));
    assert.equal(first.acceptance, 'ACCEPTED');
    assert.equal(twin.acceptance, 'ACCEPTED');
    assert.equal(fixture.config.replay_equivalence, 'nonreusable-protected-resource-transaction');
...
test('authorization-server migration cannot create another spend for one protected-resource transaction', () => {
    ...
    const alternateArtifact = {
        challenge_jwt: compactEs256({ alg: 'ES256', typ: 'txn-authz-challenge+jwt', kid: 'resource-es256-1' }, {
            ...fixture.challengeClaims,
            aud: alternateAuthorizationServer,
            jti: 'challenge-jti-migrated-as',
        }, fixture.resourceKey.privateKey),
        access_token_jwt: compactEs256({ alg: 'ES256', typ: 'at+jwt', kid: 'as-migrated-es256-1' }, {
            ...fixture.accessClaims,
...
test('OAuth transaction source lock is the reviewed -00 draft', () => {
    ...
    assert.equal(OAUTH_TXN_CHALLENGE_REPLAY_NAMESPACE, 'emilia:oauth-txn-challenge:protected-resource-transaction:v1');
});
