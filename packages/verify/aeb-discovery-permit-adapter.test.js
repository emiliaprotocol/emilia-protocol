...
// Generated from aeb-discovery-permit-adapter.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
...
function resolution(status = 'active', issuedAt = '2026-07-24T11:59:00Z') {
    const source = {
        origin: pins.origin,
    ...
    const provenanceFor = (role, url, document) => {
...
function attestation(resolved = resolution(), adapterConfig = config(), options = {}) {
...
function resign(artifact, privateKey = resolverKey.privateKey) {
    const { signature: _signature, ...body } = artifact;
    artifact.signature.value = crypto.sign(null, Buffer.from(`${DISCOVERY_PERMIT_RESOLVER_ATTESTATION_DOMAIN}${canonicalizeDiscoveryPermit(body)}`, 'utf8'), privateKey).toString('base64url');
    return artifact;
}
function input(artifact = attestation(), overrides = {}) {
    return {
        artifact,
...
test('native adapter emits a verified evidence-only AEB leg, never authorization', () => {
    ...
    const mapped = adapter.mapAction({ ...input(), profile: {}, native });
...
test('stale and unknown discovery are indeterminate; deprecated discovery is rejected', () => {
    const adapter = createAebDiscoveryPermitAdapter();
    const stale = adapter.verifyNative(input(attestation(resolution('active', '2026-07-24T11:54:59Z'))));
...
test('adapter refuses unpinned source/config changes and any transaction trust roots', () => {
    ...
    const wrongMapping = adapter.verifyNative(input(attestation(), { adapter_config: wrongConfig }));
    ...
    const trustInjected = adapter.verifyNative(input(attestation(), { trust_roots: [{ issuer: 'attacker' }] }));
...
test('presenter-added trust pin fields invalidate the resolved artifact', () => {
    const adapter = createAebDiscoveryPermitAdapter();
    const artifact = structuredClone(attestation());
...
test('signature, pinned key, source, provenance, evaluation time, and config mutation fail closed', () => {
    const adapter = createAebDiscoveryPermitAdapter();
    const cases = [];
    const signatureMutation = structuredClone(attestation());
...
test('signed stale relabeling and evaluation-time replay cannot retain ACCEPTED', () => {
    const adapter = createAebDiscoveryPermitAdapter();
    const relabeled = structuredClone(attestation(resolution('active', '2026-07-24T11:54:59Z')));
