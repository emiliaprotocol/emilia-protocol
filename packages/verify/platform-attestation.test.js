...
// Generated from platform-attestation.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
...
import { EP_PLATFORM_ATTESTATION_COMPONENT, EP_PLATFORM_ATTESTATION_PROFILE, EP_PLATFORM_ATTESTATION_VERSION, verifyPlatformAttestation, } from './platform-attestation.js';
...
const conformanceSuite = JSON.parse(readFileSync(resolve(here, '../../conformance/vectors/platform-attestation.v1.json'), 'utf8'));
function encodeJson(value) {
    return Buffer.from(JSON.stringify(value), 'utf8').toString('base64url');
}
function signEncoded(headerSegment, payloadSegment, key = signer.privateKey) {
...
function makeEvidence({ mutateHeader, mutatePayload, key = signer.privateKey, } = {}) {
...
function expectDenied(evidence, overrides = {}, reason) {
    ...
    test(v.title, () => {
        ...
        const mutatePayload = (payload) => {
            switch (v.mutation) {
                case 'none': break;
                case 'nonce_mismatch':
                    payload.eat_nonce = 'DifferentNonce-20260721';
                    break;
                case 'action_mismatch':
                    payload.ep_action_digest = `sha256:${'d'.repeat(64)}`;
                    break;
                case 'build_mismatch':
                    payload.measres = [['ep-build', [[OTHER_MEASUREMENT, 'success']]]];
                    break;
                case 'expired':
                    payload.exp = NOW_SEC - 1;
                    break;
                case 'future_issued':
                    payload.iat = NOW_SEC + 1;
                    payload.exp = NOW_SEC + 60;
                    break;
                case 'over_age':
                    payload.iat = NOW_SEC - 121;
                    break;
...
test('rejects duplicate JSON members before interpretation and all unknown fields', () => {
    const header = baseHeader();
    const payload = basePayload();
    const duplicatePayloadText = JSON.stringify(payload).replace(`"iss":"${ISSUER}"`, `"iss":"${ISSUER}","iss":"${ISSUER}"`);
    const duplicatePayloadToken = signEncoded(encodeJson(header), Buffer.from(duplicatePayloadText, 'utf8').toString('base64url'));
    ...
    const duplicateHeaderToken = signEncoded(Buffer.from(duplicateHeaderText, 'utf8').toString('base64url'), encodeJson(payload));
