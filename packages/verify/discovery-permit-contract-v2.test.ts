...
import {
  DISCOVERY_PERMIT_RESOLVER_ATTESTATION_VERSION,
  DISCOVERY_PERMIT_RESOLVER_ATTESTATION_V2_VERSION,
  DISCOVERY_PERMIT_RESOLVER_ATTESTATION_V2_REQUIRED_ALGORITHMS,
  ...
  signDiscoveryPermitResolverAttestationV2,
  verifyDiscoveryPermitResolverAttestationSignature,
  verifyDiscoveryPermitResolverAttestationSignatureV2,
  verifyDiscoveryPermitResolverAttestationStatement,
...
function buildResolution() {
  ...
  const provRec = (role: string, url: string, canonicalDigest: string) => ({
    role, requested_url: url, resolved_url: url, connected_address: '93.184.216.34',
    media_type: 'application/json' as const, byte_length: 10, raw_digest: `sha256:${'d'.repeat(64)}`,
...
test('the v1 verifier refuses a v2 attestation cleanly, without crashing', async () => {
  const attestation = await buildV2Attestation();
  const ok = verifyDiscoveryPermitResolverAttestationSignature(attestation as any, PIN as any);
...
test('LEG STRIPPING: removing the ML-DSA leg refuses', async () => {
  const attestation = await buildV2Attestation();
  const tampered = { ...attestation, signatures: attestation.signatures.filter((s: any) => s.alg === 'Ed25519') };
...
test('LEG STRIPPING: removing the Ed25519 leg refuses too', async () => {
  const attestation = await buildV2Attestation();
  const tampered = { ...attestation, signatures: attestation.signatures.filter((s: any) => s.alg === 'ML-DSA-65') };
  const ok = await verifyDiscoveryPermitResolverAttestationSignatureV2(tampered, PIN);
  assert.equal(ok, false);
});
...
test('WRONG-LENGTH SIGNATURE: a truncated leg refuses', async () => {
  const attestation = await buildV2Attestation();
  const edSig = attestation.signatures.find((s: any) => s.alg === 'Ed25519')!;
  const tampered = {
    ...attestation,
    signatures: attestation.signatures.map((s: any) => (s === edSig ? { ...s, sig: s.sig.slice(0, -4) } : s)),
  };
  const ok = await verifyDiscoveryPermitResolverAttestationSignatureV2(tampered, PIN);
  assert.equal(ok, false);
});
