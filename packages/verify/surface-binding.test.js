...
// Generated from surface-binding.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
/**
...
import { SURFACE_BINDING_VERSION, SURFACE_BINDING_FIELD, normalizeSurfaceDigest, validateSurfaceBinding, bindSurfaceInto, receiptSurfaceBinding, verifySurfaceBinding, } from './surface-binding.js';
import { canonicalize } from './index.js';
const __dirname = path.dirname(fileURLToPath(import.meta.url));
const suite = JSON.parse(readFileSync(path.join(__dirname, '..', '..', 'conformance', 'vectors', 'surface-binding.v1.json'), 'utf8'));
test('suite metadata', () => {
...
test('normalizeSurfaceDigest fail-closed on malformed input', () => {
    ...
    assert.equal(normalizeSurfaceDigest(GOOD_DIGEST.toUpperCase().replace('SHA256:', 'sha256:')), GOOD_DIGEST.slice('sha256:'.length));
});
