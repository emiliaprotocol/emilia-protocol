...
function replayStore({
  ...
  const seen = new Set();
  const consume = (key) => {
    if (throws) throw new Error('replay backend unavailable');
...
test('re-signing different receipt bytes cannot reuse the same scoped nonce', () => {
  ...
  assert.notEqual(
    first.detail.evidence_digests.receipt_digest,
    verifyOrprgJsonJcsPermit(reissued, options()).detail.evidence_digests.receipt_digest,
  );
...
test('raw JSON with duplicate member names is denied before parsing', () => {
  const raw = JSON.stringify(makeReceipt()).replace(
    `{"@version":"${ORPRG_JSON_JCS_PROFILE}",`,
    `{"@version":"${ORPRG_JSON_JCS_PROFILE}","@version":"${ORPRG_JSON_JCS_PROFILE}",`,
  );
...
test('profile and action digest mismatches deny', () => {
  ...
  assert.equal(
    verifyOrprgJsonJcsPermit(wrongProfile, options()).detail.denial_reason_code,
  ...
  assert.equal(
    verifyOrprgJsonJcsPermit(wrongDigest, options()).detail.denial_reason_code,
...
test('scope, audience, and budget are bound to the exact expected action', () => {
  ...
  assert.equal(
    verifyOrprgJsonJcsPermit(overspend, options()).detail.denial_reason_code,
    'SCOPE_VIOLATION',
  );

  const missingRequiredBudget = verifyOrprgJsonJcsPermit(
    makeReceipt((r) => { delete r.receipt_core.scope.budget; }),
