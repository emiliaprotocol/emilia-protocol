...
// Generated from authority-program-v2.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
//
...
import { AUTHORITY_PROGRAM_VERSION, authorityProgramDigest, verifyAuthorityProgram, } from './authority-program.js';
import { AUTHORITY_PROGRAM_V2_VERSION, AUTHORITY_STAGE_RECEIPT_V2_VERSION, AUTHORITY_PROGRAM_V2_REQUIRED_ALGORITHMS, authorityProgramDigestV2, authorityStageReceiptDigestV2, signAuthorityProgramV2, signAuthorityStageReceiptV2, verifyAuthorityProgramV2, verifyAuthorityProgramAny, } from './authority-program.js';
...
const pqB64u = (pk) => Buffer.from(pk).toString('base64url');
const pqSecretB64u = (sk) => Buffer.from(sk).toString('base64url');
...
async function buildReceiptV2(program) {
...
test('the v1 (classical) verifier refuses a v2 hybrid program cleanly on the version marker', async () => {
    const { program, receipt } = await build();
    const res = verifyAuthorityProgram(program, [receipt], {});
...
test('verifyAuthorityProgramAny routes on the program version', async () => {
    const { program, receipt } = await build();
    assert.equal((await verifyAuthorityProgramAny(program, [receipt], optionsFor(program))).valid, true);
});
// --- anti-stripping ----------------------------------------------------------
test('LEG STRIPPING: removing the ML-DSA leg from the program proof refuses (structural: the closed set shape requires both legs present)', async () => {
    const { program, receipt } = await build();
    const stripped = structuredClone(program);
    stripped.proof.signatures = stripped.proof.signatures.filter((s) => s.alg === 'Ed25519');
...
test('SET NARROWING: narrowing the program required_algorithms to Ed25519-only refuses structurally and cryptographically', async () => {
    const { program, receipt } = await build();
    const narrowed = structuredClone(program);
    ...
    const { proof, ...unsigned } = program;
    const narrowedBytes = Buffer.from(`${AUTHORITY_PROGRAM_V2_VERSION}\0${canonicalize({ ...unsigned, required_algorithms: ['Ed25519'] })}`, 'utf8');
...
test('DUPLICATE ALGORITHM: two entries for one algorithm refuse', async () => {
    const { program, receipt } = await build();
    const dup = structuredClone(program);
...
test('WRONG-LENGTH SIGNATURE: a truncated Ed25519 leg on the program refuses', async () => {
    const { program, receipt } = await build();
    const truncated = structuredClone(program);
    const leg = truncated.proof.signatures.find((s) => s.alg === 'Ed25519');
...
test('ED448 MASQUERADE: an Ed448 SPKI pinned as the program Ed25519 half refuses', async () => {
    ...
    options.programPin.public_key = ed448Pub;
...
test('malformed input refuses without throwing', async () => {
    for (const junk of [null, undefined, 'x', 42, [], {}]) {
        const res = await verifyAuthorityProgramV2(junk, [], {});
        assert.equal(res.valid, false);
    }
});
