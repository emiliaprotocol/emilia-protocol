...
// Generated from quorum.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
//
...
const suite = JSON.parse(readFileSync(new URL('../../conformance/vectors/quorum.v1.json', import.meta.url), 'utf8'));
...
test('EP-QUORUM-v1: a happy quorum passes every individual check', () => {
    ...
    for (const [k, v] of Object.entries(checks))
        assert.strictEqual(v, true, `check ${k} should be true`);
});
...
test('EP-QUORUM-v1: roster fields containing separators cannot alias another seat', () => {
    const k = crypto.generateKeyPairSync('ec', { namedCurve: 'P-256' });
    const signoff = mintSignoff(ctx('admin\u0000bob', 'ent_agent_7', '2026-06-11T00:01:00.000Z', 'n1'), k.privateKey);
    const quorum = {
        '@type': 'ep.quorum',
