...
import {
  bindAuthorizationBundleToGrant,
  verifyAuthorizationBundle,
  type AuthorizationBundleGrantBindingState,
...
test('Authorization Bundle publishes a closed hostile-case inventory', () => {
  ...
  assert.equal(vectors.cases.length, 21);
  assert.equal(new Set(vectors.cases.map((entry: Obj) => entry.id)).size, 21);
});
...
for (const vector of vectors.cases as Obj[]) {
  test(`Authorization Bundle hostile case: ${vector.id}`, () => {
    const result = verifyAuthorizationBundle(vector.bundle, {
      ...
      expectedAuthorizationBinding: vector.expected_authorization_binding,
...
test('bundle-to-grant binding is idempotent only for the same grant', () => {
  const first: AuthorizationBundleGrantBindingState = {
