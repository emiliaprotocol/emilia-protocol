...
import { AEB_CONSEQUENCE_CONFORMANCE_REPORT_VERSION, AEB_CONSEQUENCE_CONFORMANCE_VERSION, AEB_CONSEQUENCE_LIMITS, AEB_CONSEQUENCE_LOCAL_ATOMIC_SCOPE, AebConsequenceConformanceError, canonicalizeAebConsequenceConformance, digestAebConsequenceCase, digestAebConsequenceConformance, evaluateAebConsequenceCase, evaluateAebConsequenceSuite, parseAebConsequenceCase, validateAebConsequenceConformanceSuite, validateAebConsequenceResult, validateAebConsequenceSubmission, } from './aeb-consequence-conformance.js';
const suite = JSON.parse(fs.readFileSync(new URL('../../conformance/vectors/aeb-consequence-conformance.v1.json', import.meta.url), 'utf8'));
...
function vector(id) {
    const found = suite.vectors.find((entry) => entry.id === id);
...
test('publishes the closed local_atomic AEB consequence-admission hostile suite', () => {
    ...
    assert.deepEqual(suite.vectors.map((entry) => entry.id), REQUIRED_VECTOR_IDS);
});
...
test('CAID and normalized action must both match exactly', () => {
    ...
    assert.notEqual(sameCaid.input.evidence[0].mapped_action_digest, sameCaid.input.operation.normalized_action_digest);
...
test('local reservation refuses operation replay and native replay across a new wrapper', () => {
    ...
    assert.notEqual(rewrapped.input.evidence[0].wrapper_digest, rewrapped.input.evidence[0].native_replay_unit);
    assert.ok(rewrapped.input.reservation.consumed_native_replay_units.includes(rewrapped.input.evidence[0].native_replay_unit));
...
test('case, nested objects, and result schemas reject unknown keys', () => {
    ...
    for (const malformed of cases) {
        assert.throws(() => parseAebConsequenceCase(malformed), (error) => error instanceof AebConsequenceConformanceError
            && error.code === 'unknown_key');
    }
    const observed = vector('provider_committed_effect_diverged').input;
    assert.throws(() => parseAebConsequenceCase({ ...observed, observation: { ...observed.observation, surprise: true } }), (error) => error instanceof AebConsequenceConformanceError
        && error.code === 'unknown_key');
...
test('schema limits bound strings, arrays, nodes, and depth', () => {
    const input = vector('authorized_admission').input;
    assert.throws(() => parseAebConsequenceCase({ ...input, id: 'x'.repeat(AEB_CONSEQUENCE_LIMITS.max_string_bytes + 1) }), (error) => error instanceof AebConsequenceConformanceError
        && error.code === 'invalid_string');
    assert.throws(() => parseAebConsequenceCase({
        ...input,
        evidence: Array.from({ length: AEB_CONSEQUENCE_LIMITS.max_evidence + 1 }, () => input.evidence[0]),
    }), (error) => error instanceof AebConsequenceConformanceError
        && error.code === 'invalid_array');
    let nested = null;
    for (let index = 0; index <= AEB_CONSEQUENCE_LIMITS.max_depth; index += 1)
        nested = [nested];
...
test('evaluation and digests are deterministic and consume no ambient clock', () => {
    ...
    assert.equal(canonicalizeAebConsequenceConformance({ b: 2, a: 1 }), canonicalizeAebConsequenceConformance({ a: 1, b: 2 }));
...
test('self-attested report binds local_atomic scope, suite, implementation identity, and exact rows', () => {
    ...
    assert.deepEqual(report.rows.map((row) => row.expected), suite.vectors.map((entry) => entry.expected));
