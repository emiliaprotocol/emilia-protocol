...
interface FixtureOptions {
  ...
  subjectId?: string;
  receivingWorkload?: string;
  oauthRequestingWorkload?: string;
  oauthClaimRequestingWorkload?: string;
  witJti?: string;
  wptJti?: string;
  signatureNonce?: string;
  oauthIssuer?: { publicKey: KeyObject; privateKey: KeyObject };
  oauthIssuerUrl?: string;
  oauthIssuerKeyId?: string;
  wptAudience?: string;
  wptTth?: string;
  wptOth?: Obj;
  omitWptTth?: boolean;
...
test('the exact request profile rejects unknown headers and keeps signed Content-Type material', () => {
  ...
  substitutedContentType.input.expected_action = structuredClone(
    substitutedContentType.expectedAction,
  );
...
test('request strings have scalar and byte limits before native verification', () => {
  ...
  assert.equal(
    firstSurrogate.artifact.request.headers['Content-Digest'],
    secondSurrogate.artifact.request.headers['Content-Digest'],
  );
...
test('receiver-scoped replay identity has a compatibility-frozen golden value', () => {
  assert.equal(
    ...
    'sha256:bfd653e378a0c0c6185b413e06d772f616bf4e43cd7f1c95032f6548dcc1ade6',
  );
});
...
test('full adapter verification refuses replay at one workload and permits the unchanged txn at the next hop', () => {
  ...
  assert.notEqual(
    nextHop.config.subject.native_id,
    nextHop.config.oauth_requesting_workload,
  );
...
test('original OAuth requesting workload is constructor-pinned and canonical within the trust domain', () => {
  const fixture = makeFixture();
  const missing = structuredClone(fixture.config) as Partial<WimseOAuthSptAdapterConfig>;
  delete missing.oauth_requesting_workload;
  assert.throws(() => createWimseOAuthSptAebAdapter({
    config: missing as WimseOAuthSptAdapterConfig,
    trust_roots: fixture.trustRoots,
  }), /constructor config/);
...
test('valid token reissuance and issuer migration cannot rekey the same native spend', () => {
  ...
  assert.notEqual(
    ...
    deriveOAuthTransactionTokenReplayUnit(
      'other-trust-domain.example',
      'wimse://other-trust-domain.example/workloads/payment-executor',
      'txn-payment-release-0001',
    ),
  );
