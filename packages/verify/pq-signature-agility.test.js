...
// Generated from pq-signature-agility.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
/**
...
test('signAgile refuses to sign under an unknown algorithm', async () => {
    await assert.rejects(() => signAgile(MESSAGE, { alg: 'RSA-PSS', private_key: edPair.privateKey }), /unknown_algorithm/);
});
...
test('hybrid_all: one tampered leg fails the whole set, reason names the algorithm', async () => {
    ...
    const pq = await pqSig();
    const r = await verifyAgileSignatureSet(MESSAGE, [ed, { ...pq, sig: flipBitB64u(pq.sig) }], [ED_KEY, PQ_KEY], { policy: 'hybrid_all' });
    assert.equal(r.verified, false);
...
test('per_algorithm: top-level verdict is null and per-alg verdicts stay separate', async () => {
    const pq = await pqSig();
    const r = await verifyAgileSignatureSet(MESSAGE, [await edSig(), { ...pq, sig: flipBitB64u(pq.sig) }], [ED_KEY, PQ_KEY], { policy: 'per_algorithm' });
...
test('empty signature set and unknown policy refuse', async () => {
    ...
    const r2 = await verifyAgileSignatureSet(MESSAGE, [await edSig()], [ED_KEY], { policy: 'any_of' });
    assert.equal(r2.verified, false);
...
test('conformance/pq-agility/vectors.json verifies exactly as recorded', async () => {
    ...
    assert.equal(crypto.createHash('sha256').update(message).digest('hex'), doc.canonical_payload_sha256);
...
test('a non-canonical base64url spelling of a valid public key refuses: malformed_key', async () => {
    ...
    const r = await verifyAgileSignature(MESSAGE, { alg: 'Ed25519', sig: (await edSig()).sig }, { alg: 'Ed25519', public_key: spelling });
    assert.equal(r.verified, false);
    assert.equal(r.reason, AGILITY_REASONS.MALFORMED_KEY);
});
