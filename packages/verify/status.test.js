...
// Generated from status.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
import { test } from 'node:test';
import assert from 'node:assert/strict';
import crypto, {} from 'node:crypto';
import { REVOCER_AUTHORITY_DOMAIN, REVOCER_AUTHORITY_VERSION, STATUS_DOMAIN, STATUS_VERSION, revokerAuthorityCertificateDigest, statusArtifactDigest, verifyRevokerAuthorityCertificate, verifyStatusArtifact, } from './status.js';
...
function signCertificate(body = certificateBody(), signer = authorityKeys) {
...
function signStatus(body, signer = revokerKeys, domain = STATUS_DOMAIN) {
    const unsigned = structuredClone(body);
    delete unsigned.proof;
    return {
        ...unsigned,
...
function verify(status, certificate, extra = {}) {
...
test('accepts exact JCS/domain-separated Ed25519 shapes for current non-revocation', () => {
    ...
    noDomain.proof.signature_b64u = crypto.sign(null, Buffer.from(jcs(unsigned), 'utf8'), revokerKeys.privateKey).toString('base64url');
...
test('rejects unknown fields at every signed object boundary', () => {
    ...
    for (const mutate of [
        (value) => { value.unsigned_extension = true; },
        (value) => { value.target.unknown = true; },
        (value) => { value.proof.unknown = true; },
    ]) {
    ...
    for (const mutate of [
        ...
        (value) => { value.revoker_key.unknown = true; },
        (value) => { value.scope.unknown = true; },
        (value) => { value.proof.unknown = true; },
    ]) {
