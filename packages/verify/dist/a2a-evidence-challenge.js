...
export const A2A_AE_CHALLENGE_EXTENSION_URI = 'https://emiliaprotocol.ai/extensions/a2a/authorization-evidence-challenge/v1';
...
export function createA2AAuthorizationChallengeTask(input) {
...
export function verifyA2AAuthorizationChallengeTask(candidate, expectedAction, now) {
    ...
    if (!Number.isFinite(instant(task.status.timestamp)))
        result.reasons.push('task_timestamp_invalid');
    ...
    if (challenge.action_digest !== expectedDigest)
        result.reasons.push('challenge_action_mismatch');
    const nowMs = instant(now);
    if (!Number.isFinite(nowMs))
        result.reasons.push('verification_time_invalid');
    else if (instant(challenge.expires_at) <= nowMs)
        result.reasons.push('challenge_expired');
    result.reasons = [...new Set(result.reasons)].sort();
    result.valid = result.reasons.length === 0;
    return result;
}
//# sourceMappingURL=a2a-evidence-challenge.js.map