 ...
 *    Ed25519 leg alone.
 */
export const DISCOVERY_PERMIT_RESOLVER_ATTESTATION_V2_VERSION = 'EP-DISCOVERY-PERMIT-RESOLVER-ATTESTATION-v2';
export const DISCOVERY_PERMIT_RESOLVER_ATTESTATION_V2_DOMAIN = `${DISCOVERY_PERMIT_RESOLVER_ATTESTATION_V2_VERSION}\0`;
/** The registered required algorithm set, in canonical order. */
export const DISCOVERY_PERMIT_RESOLVER_ATTESTATION_V2_REQUIRED_ALGORITHMS = Object.freeze(['Ed25519', 'ML-DSA-65']);
...
function algorithmSetMatchesRegisteredDPR(algorithms) {
...
function resolverAttestationV2SigningBytes(body) {
    return Buffer.from(`${DISCOVERY_PERMIT_RESOLVER_ATTESTATION_V2_DOMAIN}${canonicalizeDiscoveryPermit(body)}`, 'utf8');
}
export function isDiscoveryPermitResolverAttestationV2(value) {
 ...
 */
export async function signDiscoveryPermitResolverAttestationV2(options, signer) {
    if (typeof options?.resolver_id !== 'string' || options.resolver_id.length === 0) {
        fail('resolver_id_invalid');
    }
    if (!isDigest(options.configuration_digest))
 ...
 */
export async function verifyDiscoveryPermitResolverAttestationSignatureV2(attestation, pin, options = {}) {
    ...
    const { signatures: _signatures, ...body } = attestation;
    const bytes = resolverAttestationV2SigningBytes(body);
    let setResult;
    try {
        setResult = await verifyAgileSignatureSet(new Uint8Array(bytes), attestation.signatures, [
 ...
 */
export async function verifyDiscoveryPermitResolverAttestationStatement(attestation, pin, options = {}) {
    if (isObject(attestation) && attestation['@type'] === DISCOVERY_PERMIT_RESOLVER_ATTESTATION_V2_VERSION) {
        return verifyDiscoveryPermitResolverAttestationSignatureV2(attestation, pin, options);
    }
    return verifyDiscoveryPermitResolverAttestationSignature(attestation, pin);
}
