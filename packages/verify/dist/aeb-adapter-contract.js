...
function evaluateAuthorityConstraints(requirement, legs, initiatorId, executorId) {
    ...
    for (const rule of evidenceBindingTerms(requirement)) {
        ...
        const matched = satisfiedSources.some((source) => source.evidence_bindings?.some((binding) => (binding.role === rule.target_role
        ...
        if (!matched) {
            ...
            if (sourceLegs.some((leg) => leg.verdict === 'INDETERMINATE')
                || targetLegs.some((leg) => leg.verdict === 'INDETERMINATE'))
                indeterminate = true;
        }
    }
...
export const AEB_NATIVE_ATTESTATION_V2_REQUIRED_ALGORITHMS = Object.freeze(['Ed25519', 'ML-DSA-65']);
...
function aebV2SetMatchesRegistered(algorithms) {
...
function aebNativeAttestationV2Body(attestation) {
 ...
 */
export function aebNativeAttestationV2SigningBytes(body, requiredAlgorithms = AEB_NATIVE_ATTESTATION_V2_REQUIRED_ALGORITHMS) {
    ...
    return Buffer.from(`${AEB_NATIVE_VERIFICATION_ATTESTATION_V2_DOMAIN}${canonicalize({
        ...body,
 ...
 */
export async function signAebNativeVerificationAttestationV2(body, signer, options = {}) {
    if (!exactString(signer?.key_id) || !isEd25519PrivateKey(signer?.private_key)
...
function aebNativeAttestationV2Shape(value) {
    if (!isObject(value) || !exactKeys(value, NATIVE_ATTESTATION_V2_KEYS)
        ...
        || !validDigest(value.mapping.resolver_digest) || typeof value.mapping.caid !== 'string'
        || !CAID_RE.test(value.mapping.caid) || !validDigest(value.mapping.normalized_action_digest)
        || !isObject(value.proof) || !exactKeys(value.proof, NATIVE_ATTESTATION_V2_PROOF_KEYS))
        return false;
    return true;
}
/**
 ...
 */
export async function verifyAebNativeVerificationAttestationV2(attestation, pin, options = {}) {
    const checks = {
        version: true,
        structure: true,
        algorithm_set: true,
    ...
    if (!aebV2SetMatchesRegistered(proof.required_algorithms)) {
        fail('algorithm_set', `proof.required_algorithms must be exactly ${JSON.stringify([...AEB_NATIVE_ATTESTATION_V2_REQUIRED_ALGORITHMS])} (set narrowing / widening refused)`);
    }
    ...
    if (!signatures || signatures.length === 0) {
        ...
        if (!malformed) {
            ...
            for (const alg of presented) {
                if (!AEB_NATIVE_ATTESTATION_V2_REQUIRED_ALGORITHMS.includes(alg)) {
    ...
    if (!pinnedEd || !pinnedPq || !exactString(pin?.key_id)
        || !exactString(pin?.pq_key_id)) {
        fail('verifier_key_pinned', 'a pinned Ed25519 + ML-DSA-65 verifier key pair is required (identified but not trusted)');
    }
    ...
    let recomputed = null;
    try {
        recomputed = aebNativeAttestationV2SigningBytes(aebNativeAttestationV2Body(attestation), AEB_NATIVE_ATTESTATION_V2_REQUIRED_ALGORITHMS);
    }
    catch {
        recomputed = null;
    }
    ...
    let setResult;
    try {
        setResult = await verifyAgileSignatureSet(new Uint8Array(recomputed), signatures ?? [], [
    ...
    if (setResult?.verified !== true) {
        ...
        const failedLeg = Array.isArray(setResult?.results)
            ...
            : null;
        fail('signature_valid', `native attestation signature set does not verify under the pinned Ed25519 + ML-DSA-65 keys (${reason})`);
        if (failedLeg?.reason === 'signature_invalid') {
            fail('signature_binds_attestation', 'signature set does not bind the presented attestation bytes (recomputed body mismatch)');
        }
    }
    return done();
}
