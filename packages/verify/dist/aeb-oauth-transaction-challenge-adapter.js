...
export const OAUTH_TXN_CHALLENGE_REPLAY_NAMESPACE = 'emilia:oauth-txn-challenge:protected-resource-transaction:v1';
...
function parseConfig(value) {
    if (!isRecord(value) || !exactKeys(value, CONFIG_KEYS)
        ...
        || !descriptor(value.details_verifier))
        return null;
    return structuredClone(value);
}
function parseRoot(candidate) {
...
function decodeUtf8(segment) {
    if (!canonicalB64url(segment))
        return null;
    try {
...
function verifyJwt(token, root, typ) {
    if (typeof token !== 'string')
        return null;
    const parts = token.split('.');
    if (parts.length !== 3 || !canonicalB64url(parts[2]))
        return null;
    ...
    if ((root.algorithm === 'ES256' && signature.length !== 64)
        || (root.algorithm === 'EdDSA' && signature.length !== 64))
        return null;
...
function validDetails(value) {
    if (!Array.isArray(value) || value.length < 1 || !value.every(isRecord))
        return false;
    try {
        safeDigest(value);
        return true;
...
function exactVerifiedContext(value) {
    if (!isRecord(value) || !exactKeys(value, VERIFIED_CONTEXT_KEYS))
        return false;
    return [...VERIFIED_CONTEXT_KEYS].every((key) => nonEmptyString(value[key]));
}
function expectedActionShape(value, config) {
...
function verifiedContextMatches(value, config) {
    const transaction = value.oauth_transaction;
    const context = transaction.verified_context;
...
function verifyNative(input, pins) {
    ...
    if (!expectedActionShape(input.expected_action, pins.config)) {
        result.reasons = ['oauth-txn:missing_or_ambiguous_exact_action'];
        return result;
    }
    ...
    if (!challenge)
        return reject(result, 'oauth-txn:challenge_invalid');
    if (!access)
        return reject(result, 'oauth-txn:access_token_invalid');
    const challengeClaims = challenge.claims;
    const accessClaims = access.claims;
    ...
    const actorOptional = pins.config.require_actor_context ? new Set() : new Set(['act']);
    ...
    const expectedTxn = input.expected_action.oauth_transaction;
    ...
    if (!Number.isFinite(nowMs)) {
        result.reasons = ['oauth-txn:verification_time_invalid'];
        return result;
    }
    const now = Math.floor(nowMs / 1000);
    const challengeIat = Number(challengeClaims.iat);
    const challengeExp = Number(challengeClaims.exp);
    const accessIat = Number(accessClaims.iat);
    const accessExp = Number(accessClaims.exp);
    ...
    try {
        ...
        result.reasons = ['oauth-txn:details_verifier_error'];
        return result;
    }
...
export function createOAuthTransactionChallengeActionDefinition(actionType, requireActor) {
...
export function createOAuthTransactionChallengeAebAdapter(constructorPins) {
    ...
    return Object.freeze({
        ...
        verifyNative(input) {
            try {
                ...
                const result = fallbackNative(input, pins);
                result.reasons = ['oauth-txn:unexpected_adapter_error'];
                return result;
            }
        },
        mapAction(input) {
...
//# sourceMappingURL=aeb-oauth-transaction-challenge-adapter.js.map