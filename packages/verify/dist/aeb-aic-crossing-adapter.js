...
function validVerifierDescriptor(value) {
    return isRecord(value)
...
function validRelyingPartyPolicy(value) {
    return isRecord(value)
...
function jsonObjectFromSegment(segment) {
...
function inspectPrincipalPublicJwk(value) {
    ...
    if (value.kty === 'EC') {
        ...
        if (!exactKeys(value, keys)
            ...
            || !canonicalBase64urlBytes(y, 1_024))
            return null;
        jwk = { kty: 'EC', crv, x, y };
...
function inspectJwtCarrier(input) {
    ...
    if (!isRecord(provenance)
        ...
        || !['DIRECT', 'SYNTHESIZED-X509'].includes(String(provenance.downstream_representation))
    ...
    if (!header || !payload || header.typ !== 'aic+jwt')
        return null;
    const aic = payload.aic;
    const principal = isRecord(aic) ? aic.principal : null;
    const presentedPrincipal = inspectPrincipalPublicJwk(provenance.presented_principal_jwk);
    ...
    if (confirmation !== undefined
        && (!isRecord(confirmation) || !keyHash(confirmation.jkt, 'jkt')))
        return null;
    return {
        carrierOrigin: 'AIC-JWT',
        representation: provenance.downstream_representation,
...
function inspectX509Carrier(input) {
    const provenance = input.carrier_provenance;
    if (!isRecord(provenance)
        || !exactKeys(provenance, X509_PROVENANCE_KEYS)
        || provenance.source_carrier !== 'AIC-X509-CREDENTIAL-BUNDLE')
        return null;
    const agentDer = canonicalBase64urlBytes(provenance.agent_certificate_der, MAX_AIC_CERTIFICATE_BYTES);
    const principalDer = canonicalBase64urlBytes(provenance.principal_certificate_der, MAX_AIC_CERTIFICATE_BYTES);
...
function temporalDisposition(input, context) {
...
function authorityFrom(input, policy, native) {
    return {
        ...
        replay_unit: digestAebTyped(native.replayIdentity, `${native.mappingProfile}:replay-unit`),
...
function boundDisposition(input, context, expectedMappingProfileId) {
...
export function mapAicJwtJktBoundCrossingAuthority(input, context) {
    ...
    const disposition = boundDisposition(input, context, AIC_JWT_JKT_BOUND_CROSSING_MAPPING_PROFILE);
    if (disposition)
...
export function mapAicX509SpkiBoundCrossingAuthority(input, context) {
    ...
    const disposition = boundDisposition(input, context, AIC_X509_SPKI_BOUND_CROSSING_MAPPING_PROFILE);
    if (disposition)
        return { ok: false, reason: disposition };
    return {
        ok: true,
