...
function boundedDiagnosticText(value) {
    const compact = String(value)
        ...
        .replace(/\s+/g, ' ')
        .trim();
    if (compact.length <= CROSSING_LAB_DIAGNOSTIC_TEXT_LIMIT)
        return compact;
    return `${compact.slice(0, CROSSING_LAB_DIAGNOSTIC_TEXT_LIMIT - 1)}…`;
}
function crossingLabFailureDiagnostics(report) {
    const lines = [];
    for (const row of report.adapter_rows.filter((entry) => !entry.passed)) {
        ...
        const reasonSummary = reasons.length > 0
            ...
            : 'none reported';
        lines.push(`failed adapter row ${boundedDiagnosticText(row.id)}: `
    ...
    if (failedHarness.length > 0)
        lines.push(`failed harness rows: ${failedHarness.join(', ')}`);
    ...
    if (typeof candidateCaid === 'string'
        ...
        lines.push(`candidate mapped CAID ${boundedDiagnosticText(candidateCaid)} differs from workspace evaluation CAID `
...
if (args[0] === 'reliance-gap') {
    ...
    try {
        if (profilePath) {
            ...
            const profiles = names.map((name) => ({
                label: name,
                profile: load(join(profilesDir, name)),
    ...
    const summarize = (label, r) => {
        ...
        for (const gap of r.missing_evidence)
            human.push(`  gap: ${gap.requirement} -- ${gap.how_to_close}`);
    };
    let exitCode;
    if (profilePath) {
        ...
        for (const { label, report: r } of report.reports)
            summarize(label, r);
...
if (args[0] === 'revocation') {
    ...
    for (let i = 0; i < sub.length; i++) {
        if (sub[i] === '--target')
            targetPath = sub[++i];
        else if (sub[i] === '--revoker-keys')
            keysPath = sub[++i];
        else if (sub[i] === '--max-age')
            maxAge = Number(sub[++i]);
        else
            statementPath = sub[i];
    }
    ...
    if (!keysPath)
        console.log('  note: no --revoker-keys supplied; a revocation from an unpinned key confers nothing.');
...
for (let i = 0; i < args.length; i++) {
    if (args[i] === '--key')
        suppliedKey = args[++i];
    else if (args[i] === '--rp-id')
        suppliedRpId = args[++i];
    else if (args[i] === '--allowed-origin')
        suppliedAllowedOrigins.push(args[++i]);
    else if (args[i] === '--verification')
        verificationPath = args[++i];
    else if (args[i] === '--delegation-keys')
        delegationKeysPath = args[++i];
    else
        files.push(args[i]);
}
let delegationKeys = {};
if (delegationKeysPath) {
    try {
        delegationKeys = loadStrictJson(delegationKeysPath);
    }
    catch (err) {
        console.error(`✕ --delegation-keys not readable JSON (${err.message})`);
        process.exit(1);
    }
}
...
function findKey(doc, names) {
    ...
    for (const n of names) {
        if (typeof doc?.[n] === 'string')
            return doc[n];
        if (typeof doc?.context?.[n] === 'string')
            return doc.context[n];
        if (typeof doc?.signer?.[n] === 'string')
            return doc.signer[n];
    }
    return null;
}
