...
import { COSE_ALG_EDDSA, COSE_ALG_ML_DSA_65, COSE_HEADER_EP_REQUIRED_ALGS, COSE_RECEIPT_CONTENT_TYPE, decodeDeterministicCbor8949, encodeDeterministicCbor8949, receiptActionCaid, } from './receipt-cose-encoding.js';
// ---------------------------------------------------------------------------
// Profile constants
// ---------------------------------------------------------------------------
...
const V2_PROFILE_PROTECTED_LABELS = new Set([
    COSE_HEADER_ALG,
    COSE_HEADER_CONTENT_TYPE,
    COSE_HEADER_KID,
    COSE_HEADER_CWT_CLAIMS,
...
function scittV2SetMatchesRegistered(algorithms) {
...
export function epScittV2ProtectedHeader(alg, kid, iss, sub, requiredAlgorithms = EP_SCITT_STATEMENT_V2_REQUIRED_ALGORITHMS) {
    ...
    return new Map([
        ...
        [COSE_HEADER_CWT_CLAIMS, new Map([
                [CWT_CLAIM_ISS, iss],
                [CWT_CLAIM_SUB, sub],
            ])],
...
export async function buildEpScittHybridSignedStatement(receipt, opts, agility = {}) {
    if (!isPlainObject(receipt) || !isPlainObject(receipt.payload)) {
        return refuse('invalid_receipt_document');
    }
    if (typeof opts?.kid !== 'string' || opts.kid.length === 0 || opts.kid.length > 1024) {
    ...
    for (const alg of EP_SCITT_STATEMENT_V2_REQUIRED_ALGORITHMS) {
        const protectedEncoded = encodeDeterministicCbor8949(epScittV2ProtectedHeader(alg, opts.kid, opts.iss, sub));
        if (!protectedEncoded.ok)
            return protectedEncoded;
        const sigStruct = sigStructureBytes(protectedEncoded.value, payload);
        if (!sigStruct.ok)
        ...
        try {
            const agile = await signAgile(new Uint8Array(sigStruct.value), alg === COSE_ALG_EDDSA
                ...
                : { alg: 'ML-DSA-65', private_key: opts.statementPqSecretKey }, agility);
...
function parseScittHybridHalf(statementBytes, expectedAlg, expectedKid) {
    if (!(statementBytes instanceof Uint8Array) || statementBytes.length < 2) {
    ...
    if (!headers.has(COSE_HEADER_CWT_CLAIMS))
        return refuse('cwt_claims_missing');
    const cwt = headers.get(COSE_HEADER_CWT_CLAIMS);
    if (!(cwt instanceof Map))
        return refuse('cwt_claims_malformed');
    for (const label of cwt.keys()) {
        if (!PROFILE_CWT_CLAIM_LABELS.has(label))
            return refuse('unexpected_cwt_claim');
    }
    if (!cwt.has(CWT_CLAIM_ISS))
        return refuse('iss_missing');
    if (!cwt.has(CWT_CLAIM_SUB))
        return refuse('sub_missing');
    const iss = cwt.get(CWT_CLAIM_ISS);
    const sub = cwt.get(CWT_CLAIM_SUB);
    if (!isAcceptableIss(iss))
        return refuse('iss_malformed');
    if (!isCaidString(sub))
        return refuse('sub_malformed');
 ...
 */
export async function verifyEpScittSignedStatementHybrid(pair, opts) {
    const checks = {
        pair_present: false,
        deterministic_encoding: false,
        cose_structure: false,
    ...
    if (!isAcceptableIss(opts?.expectedIss))
    ...
    if (classical.value.iss !== opts.expectedIss)
        return fail('iss_mismatch');
    ...
    if (!recomputed.ok)
        return fail('sub_not_bound_to_payload');
    if (recomputed.value.caid !== classical.value.sub)
        return fail('sub_not_bound_to_payload');
    for (const [half, alg] of [
        ...
        let expected;
        try {
            expected = encodeDeterministicCbor8949(epScittV2ProtectedHeader(alg, opts.expectedKid, opts.expectedIss, recomputed.value.caid));
        }
        catch {
    ...
    const legs = [
        await verifyAgileSignature(classicalSig.value, { alg: 'Ed25519', sig: Buffer.from(classical.value.signature).toString('base64url') }, { alg: 'Ed25519', public_key: opts.statementPublicKeyBase64url }, opts.agility ?? {}),
        await verifyAgileSignature(pqSig.value, { alg: 'ML-DSA-65', sig: Buffer.from(pq.value.signature).toString('base64url') }, { alg: 'ML-DSA-65', public_key: opts.statementPqPublicKeyBase64url }, opts.agility ?? {}),
    ];
    const failedLeg = legs.find((leg) => leg.verified !== true);
    if (failedLeg)
        return fail(`statement_signature_invalid:${failedLeg.alg}:${failedLeg.reason}`);
