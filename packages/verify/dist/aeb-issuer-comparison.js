...
function trimTrailing(value, char) {
    ...
    while (end > 0 && value.charCodeAt(end - 1) === char.charCodeAt(0))
        end -= 1;
 ...
 */
function leadingLabel(value, first, rest, maximum) {
    if (value.length === 0 || !first(value.charCodeAt(0)))
        return -1;
    let index = 1;
    while (index < value.length && index < maximum && rest(value.charCodeAt(index)))
        index += 1;
 ...
 */
export function aebIssuerComparisonForm(issuer) {
    if (typeof issuer !== 'string' || issuer.length > AEB_ISSUER_COMPARISON_MAX_LENGTH)
        return issuer;
    // ^([A-Za-z][A-Za-z0-9+.-]*):
    const schemeLength = leadingLabel(issuer, isAsciiLetter, (code) => isAsciiLetter(code) || isAsciiDigit(code)
        || code === 0x2b || code === 0x2e || code === 0x2d, Number.MAX_SAFE_INTEGER);
    if (schemeLength < 0)
        return issuer;
    ...
    if (protocol === 'urn:') {
        ...
        const nid = leadingLabel(rest, (code) => isAsciiLetter(code) || isAsciiDigit(code), (code) => isAsciiLetter(code) || isAsciiDigit(code) || code === 0x2d, 32);
        return nid >= 0
    ...
    if (protocol === 'did:') {
        // ^([A-Za-z0-9]+):
        const alphanumeric = (code) => isAsciiLetter(code) || isAsciiDigit(code);
    ...
    if (protocol === 'spiffe:') {
        ...
        if (LINE_TERMINATOR.test(remainder))
            return `spiffe:${rest}`;
...
//# sourceMappingURL=aeb-issuer-comparison.js.map