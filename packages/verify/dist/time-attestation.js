...
export function verifyTimeAttestation(att, opts = {}) {
    ...
    if (att['@version'] !== TIME_ATTESTATION_VERSION)
    ...
    if (!pinned)
        fail('tsa_key_pinned', `no pinned key for ts_authority "${authorityId}" (identified but not trusted)`);
    else if (presentedKey && pinned !== presentedKey)
        fail('tsa_key_pinned', `presented TSA key != pinned key for "${authorityId}"`);
    const ms = instantMs(att.time);
    if (ms === null)
        fail('time_present', 'time is absent or not a well-formed RFC 3339 instant');
    const sigOk = pinned && verifyEd25519(timeSignedPayload(att), pinned, proof?.signature_b64u);
    if (!sigOk)
        fail('signature_valid', 'TSA signature does not verify under the pinned key over the recomputed bytes');
    ...
    if (ms !== null) {
        ...
        if (nb !== null && !Number.isNaN(nb) && ms < nb)
            fail('within_bounds', 'attested time is before notBefore');
        if (na !== null && !Number.isNaN(na) && ms > na)
            fail('within_bounds', 'attested time is after notAfter');
    }
...
export const TIME_ATTESTATION_V2_REQUIRED_ALGORITHMS = Object.freeze(['Ed25519', 'ML-DSA-65']);
 ...
 */
export function timeAttestationV2SignedBytes(att, requiredAlgorithms = TIME_ATTESTATION_V2_REQUIRED_ALGORITHMS) {
 ...
 */
export async function verifyTimeAttestationV2(att, opts = {}) {
    opts = opts && typeof opts === 'object' ? opts : {};
    const tsaKeys = opts.tsaKeys || {};
    const checks = {
        version: true,
    ...
    //    attestation claims.
    if (!algorithmSetMatchesRegistered(requiredAlgorithms)) {
        fail('algorithm_set', `proof.required_algorithms must be exactly ${JSON.stringify([...TIME_ATTESTATION_V2_REQUIRED_ALGORITHMS])} (set narrowing / widening refused)`);
    }
    ...
    const authorityId = typeof att.ts_authority_id === 'string' ? att.ts_authority_id : '';
    const pin = authorityId && tsaKeys && typeof tsaKeys === 'object'
        ? tsaKeys[authorityId] || {}
        : {};
    ...
    if (!authorityId) {
        ...
        fail('tsa_key_pinned', `no pinned Ed25519 + ML-DSA-65 key pair for ts_authority "${authorityId}" (identified but not trusted)`);
    }
    else {
    ...
    if (!recomputedBytes) {
        ...
        if (setResult?.verified !== true) {
            ...
            fail('signature_set_valid', `TSA signature set does not verify under the pinned Ed25519 + ML-DSA-65 keys (${reason})`);
        }
    }
    if (typeof opts.expectedHash === 'string') {
        if (hexOf(att.hashed) !== hexOf(opts.expectedHash)) {
 ...
 */
export async function verifyTimeAttestationStatement(att, opts = {}) {
    if (att && typeof att === 'object' && !Array.isArray(att)
        && att['@version'] === TIME_ATTESTATION_V2_VERSION) {
        return verifyTimeAttestationV2(att, opts);
    }
    return verifyTimeAttestation(att, opts);
}
...
function tsKeyId(publicKeyB64u) {
    try {
...
function pqTsKeyId(publicKeyRawB64u) {
 ...
 */
export async function buildTimeAttestationV2({ ts_authority_id: tsAuthorityId, hashed, time = new Date().toISOString(), signer, deterministic = false, } = {}) {
    ...
    if (!derivedTsKeyId)
        throw new Error('buildTimeAttestationV2 requires a valid base64url Ed25519 SPKI public key');
    ...
    const ordered = TIME_ATTESTATION_V2_REQUIRED_ALGORITHMS.map((alg) => {
        ...
        if (!s)
            throw new Error(`buildTimeAttestationV2: signing produced no ${alg} leg`);
        return s;
    });
    return {
        ...att,
...
//# sourceMappingURL=time-attestation.js.map