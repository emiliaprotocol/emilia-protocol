...
function canonicalize(value, seen = new WeakSet()) {
    ...
    if (typeof value === 'string') {
        if (!validUnicodeString(value))
            throw new TypeError('JCS input contains an invalid Unicode scalar');
        return JSON.stringify(value);
    }
    if (typeof value === 'number') {
        if (!Number.isFinite(value))
            throw new TypeError('JCS input contains a non-finite number');
        return JSON.stringify(value);
    }
    ...
    if (Array.isArray(value)) {
        if (!densePlainArray(value))
            throw new TypeError('JCS input contains a non-JSON array');
...
function revokerKeyId(publicKey) {
    const der = canonicalBase64url(publicKey, 44);
    if (!der || !loadEd25519Key(publicKey))
        return null;
...
function validCertificateProof(value) {
...
function validScopeArray(value, allowed) {
    return densePlainArray(value)
        ...
        && value.every((member) => typeof member === 'string' && allowed.includes(member))
        && new Set(value).size === value.length;
}
function certificateStructure(value) {
    if (!exactObject(value, CERTIFICATE_KEYS)
        ...
        || !validCertificateProof(value.proof))
        return false;
    return true;
}
...
function certificateValidity(value, at) {
...
export function revokerAuthorityCertificateDigest(certificate) {
    return digest(certificate);
}
/** Digest of the exact closed, signed status envelope. */
export function statusArtifactDigest(status) {
    return digest(status);
}
function verifyRevokerAuthorityCertificateCore(certificate, options = {}) {
    const checks = {
        structure: false,
        authority: false,
        scope: false,
...
export function verifyRevokerAuthorityCertificate(certificate, options = {}) {
    try {
...
function previousStatusChecks(candidate, previous, certificate, checks, reasons) {
...
function verifyStatusArtifactCore(expectedTarget, status, options = {}) {
    ...
    if (!certificateResult.valid || !certificateStructure(options.certificate)) {
        ...
        for (const reason of certificateResult.reasons)
    ...
    previousStatusChecks(status, options.previousStatus, certificate, result.checks, result.reasons);
 ...
 */
export function verifyStatusArtifact(expectedTarget, status, options = {}) {
    try {
...
export const REVOCER_AUTHORITY_V2_REQUIRED_ALGORITHMS = Object.freeze(['Ed25519', 'ML-DSA-65']);
export const STATUS_V2_REQUIRED_ALGORITHMS = Object.freeze(['Ed25519', 'ML-DSA-65']);
...
const REVOKER_KEY_KEYS_V2 = ['key_id', 'public_key', 'pq_key_id', 'pq_public_key'];
const CERTIFICATE_PROOF_KEYS_V2 = ['key_id', 'pq_key_id', 'signatures'];
...
const STATUS_PROOF_KEYS_V2 = ['key_id', 'pq_key_id', 'signatures'];
function algorithmSetMatchesRegistered(algorithms, registered) {
...
function pqRevokerKeyId(publicKey) {
...
function validAuthorityPinV2(value) {
    return record(value)
        && authorityDomain(value.authority_domain)
        && identifier(value.authority_id)
        && identifier(value.key_id)
...
function certificateStructureV2(value) {
    if (!exactObject(value, CERTIFICATE_KEYS_V2)
        ...
        || !validCertificateProofV2(value.proof))
        return false;
    return true;
}
function validStatusProofV2(value) {
    return exactObject(value, STATUS_PROOF_KEYS_V2)
        && identifier(value.key_id)
        && identifier(value.pq_key_id)
        && densePlainArray(value.signatures);
}
function statusStructureV2(value, registry) {
...
function indeterminateStatusV2() {
    return {
        outcome: 'indeterminate',
        valid: false,
        checks: {
...
async function verifyRevokerAuthorityCertificateV2Core(certificate, options = {}) {
    const checks = {
        structure: false,
        algorithm_set: false,
        authority: false,
    ...
    checks.algorithm_set = algorithmSetMatchesRegistered(certificate.required_algorithms, REVOCER_AUTHORITY_V2_REQUIRED_ALGORITHMS);
    if (!checks.algorithm_set)
        addReason(reasons, 'invalid_revoker_authority_algorithm_set');
    if (!certificateScope(certificate, options.targetRegistry)) {
        addReason(reasons, 'invalid_revoker_authority_scope');
    }
    else {
    ...
    if (!validAuthorityPinV2(pin)) {
        ...
        let bytes = null;
        try {
            bytes = signingBytes(unsigned(certificate), REVOCER_AUTHORITY_V2_DOMAIN);
        }
        catch {
            bytes = null;
        }
        ...
        if (bytes) {
            try {
                setResult = await verifyAgileSignatureSet(new Uint8Array(bytes), certificate.proof.signatures, [
...
export async function verifyRevokerAuthorityCertificateV2(certificate, options = {}) {
    try {
...
async function previousStatusChecksV2(candidate, previous, certificate, checks, reasons, options) {
    if (candidate.sequence === 0) {
        if (candidate.previous_status_digest !== null) {
            addReason(reasons, 'initial_status_has_previous_digest');
            return;
    ...
    try {
        previousBytes = signingBytes(unsigned(previous), STATUS_V2_DOMAIN);
    }
    catch {
        previousBytes = null;
    }
    let previousSigResult = null;
    if (previousBytes) {
        try {
            previousSigResult = await verifyAgileSignatureSet(new Uint8Array(previousBytes), previous.proof.signatures, [
...
async function verifyStatusArtifactV2Core(expectedTarget, status, options = {}) {
    ...
    result.sequence = status.sequence;
    result.next_update = status.next_update;
    result.checks.algorithm_set = algorithmSetMatchesRegistered(status.required_algorithms, STATUS_V2_REQUIRED_ALGORITHMS);
    if (!result.checks.algorithm_set)
        addReason(result.reasons, 'invalid_status_algorithm_set');
    if (!validTarget(expectedTarget, options.targetRegistry) || !targetEqual(expectedTarget, status.target)) {
        addReason(result.reasons, 'status_target_mismatch');
    }
    else {
    ...
    if (status.proof.key_id !== certificate.revoker_key.key_id
        ...
        let bytes = null;
        try {
            bytes = signingBytes(unsigned(status), STATUS_V2_DOMAIN);
        }
        catch {
            bytes = null;
        }
        ...
        if (bytes) {
            try {
                setResult = await verifyAgileSignatureSet(new Uint8Array(bytes), status.proof.signatures, [
                    { alg: 'Ed25519', public_key: certificate.revoker_key.public_key, key_id: certificate.revoker_key.key_id },
                    { alg: 'ML-DSA-65', public_key: certificate.revoker_key.pq_public_key, key_id: certificate.revoker_key.pq_key_id },
                ], {
                    ...agilityPassthrough(options),
    ...
    await previousStatusChecksV2(status, options.previousStatus, certificate, result.checks, result.reasons, options);
    const valid = Object.values(result.checks).every(Boolean);
    if (!valid)
        return result;
    result.valid = true;
 ...
 */
export async function verifyStatusArtifactV2(expectedTarget, status, options = {}) {
    try {
 ...
 */
export async function verifyRevokerAuthorityCertificateStatement(certificate, options = {}) {
    if (certificate && typeof certificate === 'object' && !Array.isArray(certificate)
        && certificate['@version'] === REVOCER_AUTHORITY_V2_VERSION) {
        return verifyRevokerAuthorityCertificateV2(certificate, options);
    }
    return verifyRevokerAuthorityCertificate(certificate, options);
}
/**
 ...
 */
export async function verifyStatusArtifactStatement(expectedTarget, status, options = {}) {
    if (status && typeof status === 'object' && !Array.isArray(status)
        && status['@version'] === STATUS_V2_VERSION) {
        return verifyStatusArtifactV2(expectedTarget, status, options);
    }
    return verifyStatusArtifact(expectedTarget, status, options);
}
