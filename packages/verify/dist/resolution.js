...
export function computeBindingMomentHash(bindingMoment) {
...
export function computeResolutionResponseHash(response) {
    return sha256Canonical(response);
}
/** Return the WebAuthn challenge for an already-built resolution context. */
export function computeResolutionChallenge(context) {
    if (!isRecord(context) || !isCanonicalizable(context))
        return null;
    try {
...
function bindingMomentShapeValid(bindingMoment) {
    ...
    if (!hasExactKeys(bindingMoment, outerAllowed, outerRequired))
        return false;
    ...
    if (!question.options.every((option) => hasExactKeys(option, optionKeys)
        && typeof option.label === 'string' && typeof option.reasoning === 'string'))
        return false;
    if (!Number.isSafeInteger(question.recommended_idx)
        || question.recommended_idx < 0 || question.recommended_idx >= question.options.length)
        return false;
    ...
    if (!hasExactKeys(question.hatches, hatchKeys)
        ...
        || typeof question.hatches.dialogue !== 'boolean')
        return false;
    if (Object.hasOwn(bindingMoment, 'meta')) {
        const metaKeys = new Set(['decision_class', 'calibration_note']);
        if (!hasExactKeys(bindingMoment.meta, metaKeys, new Set()))
            return false;
        if (Object.values(bindingMoment.meta).some((value) => typeof value !== 'string'))
            return false;
    }
    return true;
}
...
function allowedResolutionKeys(outcome) {
    if (outcome === 'approved')
        return new Set(['outcome', 'selected_option']);
    if (outcome === 'declined')
        return new Set(['outcome']);
    if (outcome === 'amended')
        return new Set(['outcome', 'response_hash', 'successor_envelope_hash']);
    if (outcome === 'rejected')
        return new Set(['outcome', 'objection_hash', 'successor_envelope_hash']);
    return null;
}
function resolutionShapeValid(resolution, bindingMoment, currentEnvelopeHash) {
    if (!isRecord(resolution) || !RESOLUTION_OUTCOMES.includes(resolution.outcome))
        return false;
    const allowed = allowedResolutionKeys(resolution.outcome);
    if (!allowed || !Object.keys(resolution).every((key) => allowed.has(key)))
        return false;
    if (resolution.outcome === 'approved') {
        if (!Number.isInteger(resolution.selected_option) || resolution.selected_option < 0)
            return false;
    ...
    if (resolution.outcome === 'declined')
        return Object.keys(resolution).length === 1;
    if (resolution.outcome === 'amended' && !isHash(resolution.response_hash))
        return false;
    if (resolution.outcome === 'rejected'
        && Object.hasOwn(resolution, 'objection_hash')
        && !isHash(resolution.objection_hash))
        return false;
    if (Object.hasOwn(resolution, 'successor_envelope_hash')) {
        if (!isHash(resolution.successor_envelope_hash))
            return false;
        if (resolution.successor_envelope_hash === currentEnvelopeHash)
            return false;
    }
    return true;
}
function structureValid(receipt) {
    if (!hasExactKeys(receipt, RECEIPT_KEYS))
        return false;
    if (receipt.profile !== RESOLUTION_VERSION)
        return false;
    const signoff = receipt.signoff;
    if (!hasExactKeys(signoff, SIGNOFF_KEYS) || signoff['@type'] !== 'ep.signoff')
        return false;
    if (!hasExactKeys(signoff.context, CONTEXT_KEYS))
        return false;
    if (!hasExactKeys(signoff.webauthn, WEBAUTHN_KEYS))
        return false;
...
function refuse(reason, checks, outcome = null) {
    return {
        valid: false,
 ...
 */
export function verifyResolutionReceipt(receipt, opts = {}) {
    const checks = {
        structure: false,
    ...
    try {
        ...
        if (!checks.canonical_profile)
            return refuse('resolution_outside_canonicalization_profile', checks, outcome);
        checks.binding_moment_shape = bindingMomentShapeValid(opts.bindingMoment);
        if (!checks.binding_moment_shape)
            return refuse('malformed_binding_moment', checks, outcome);
        const bindingMoment = opts.bindingMoment;
        ...
        if (!checks.principal_pin)
            return refuse('principal_key_not_pinned_for_role', checks, outcome);
        ...
        if (!checks.initiator_binding)
            return refuse('initiator_binding_mismatch', checks, outcome);
        ...
        if (!checks.nonce_binding)
            return refuse('nonce_binding_mismatch', checks, outcome);
        ...
        if (checks.time_window && opts.evaluationTime !== undefined) {
            ...
            if (!checks.evaluation_time)
                return refuse('resolution_outside_validity_window', checks, outcome);
        }
        if (!checks.time_window)
            return refuse('resolution_outside_validity_window', checks, outcome);
        checks.rp_id = typeof opts.rpId === 'string' && opts.rpId.length > 0;
        if (!checks.rp_id)
            return refuse('rp_id_required', checks, outcome);
        ...
        if (!checks.webauthn)
            return refuse('webauthn_verification_failed', checks, outcome);
