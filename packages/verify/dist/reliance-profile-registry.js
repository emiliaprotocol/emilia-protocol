...
export function signRelianceProfileEntry({ registry_id, profile_id, profile, registry_epoch, issued_at }, privateKey) {
    ...
    if (!v.ok)
        throw new Error(`invalid inner profile: ${v.issues.join('; ')}`);
    if (typeof registry_id !== 'string' || !registry_id)
        throw new Error('registry_id is required');
...
export function verifyRelianceProfileEntry(entry, opts = {}) {
    ...
    if (!entry || typeof entry !== 'object' || entry['@type'] !== PROFILE_REGISTRY_VERSION)
        return fail('unsupported_version');
    checks.version = true;
    const sig = entry.signature;
    if (typeof entry.registry_id !== 'string' || !entry.registry_id)
        return fail('registry_id_missing_or_malformed');
    ...
    if (typeof sig.entry_digest !== 'string' || !SHA256_RE.test(sig.entry_digest))
        return fail('entry_digest_malformed');
    let digest;
    try {
        digest = profileRegistryEntryDigest(entry);
    }
    catch {
        return fail('entry_uncanonicalizable');
    }
    if (digest !== sig.entry_digest)
        return fail('entry_digest_mismatch', { entry_digest: digest });
    ...
    if (!pv.ok)
        return fail('inner_profile_invalid', { entry_digest: digest });
    if (entry.profile_hash !== profileHash(entry.profile))
        return fail('profile_hash_mismatch', { entry_digest: digest });
    ...
    if (!sigOk)
        return { verified: false, accepted: false, profile: null, checks, reason: 'signature_invalid', entry_digest: digest };
...
export const PROFILE_REGISTRY_V2_REQUIRED_ALGORITHMS = Object.freeze(['Ed25519', 'ML-DSA-65']);
/** ML-DSA-65 registry-key identifier: same truncated-digest convention as keyIdFor. */
const pqKeyIdFor = (pubRawB64u) => `ep:reliance-registry-key:ml-dsa-65:sha256:${sha256hex(Buffer.from(pubRawB64u, 'base64url')).slice(0, 16)}`;
 ...
 */
export function entrySigningBytesV2(unsignedEntryV2, requiredAlgorithms = PROFILE_REGISTRY_V2_REQUIRED_ALGORITHMS) {
    ...
    return Buffer.from(PROFILE_REGISTRY_V2_DOMAIN + canonicalize({ ...unsignedEntryV2, required_algorithms: [...requiredAlgorithms] }), 'utf8');
}
/** Digest of the signed v2 entry body, excluding the `proof` envelope. */
export function profileRegistryEntryDigestV2(entry, requiredAlgorithms = PROFILE_REGISTRY_V2_REQUIRED_ALGORITHMS) {
    return `sha256:${sha256hex(entrySigningBytesV2(unsignedV2(entry), requiredAlgorithms))}`;
}
function toRawB64uV2(value, expectedLength, label) {
 ...
 */
export async function signRelianceProfileEntryV2({ registry_id, profile_id, profile, registry_epoch, issued_at }, signer) {
    const v = validateRelianceProfile(profile);
    if (!v.ok)
        throw new Error(`invalid inner profile: ${v.issues.join('; ')}`);
    if (typeof registry_id !== 'string' || !registry_id)
...
function agilityPassthroughV2(opts) {
    const out = {};
 ...
 */
export async function verifyRelianceProfileEntryV2(entry, opts = {}) {
    const checks = {
        version: false,
        entry_digest: false,
        profile_hash: false,
    ...
    if (typeof entry.profile_id !== 'string' || !entry.profile_id)
        return fail('profile_id_missing_or_malformed');
    ...
    if (signatures && signatures.length > 0) {
        ...
        if (!malformed) {
            ...
            const extra = [...presented].filter((alg) => !PROFILE_REGISTRY_V2_REQUIRED_ALGORITHMS.includes(alg));
            legsOk = missing.length === 0 && extra.length === 0;
            if (!legsOk)
                legsReason = missing.length ? `missing_required_algorithm:${missing.join(',')}` : `unexpected_algorithm:${extra.join(',')}`;
        }
    }
    checks.legs_present = legsOk;
    if (!legsOk)
        return { verified: false, accepted: false, profile: null, checks, reason: legsReason, entry_digest: digest };
    ...
    const presentedKeys = [
        { alg: 'Ed25519', public_key: proof.public_key },
        { alg: 'ML-DSA-65', public_key: proof.pq_public_key },
    ];
    let setResult;
    try {
    ...
    const derivedKeyId = keyIdFor(proof.public_key);
    const derivedPqKeyId = pqKeyIdFor(proof.pq_public_key);
    ...
    const resolvedHash = profileHash(resolved);
    const pinned = Array.isArray(opts.pinnedRegistryKeys) ? opts.pinnedRegistryKeys : [];
    const keyMatched = pinned.filter((k) => k
 ...
 */
export async function verifyRelianceProfileEntryStatement(entry, opts = {}) {
    if (entry && typeof entry === 'object' && !Array.isArray(entry) && entry['@type'] === PROFILE_REGISTRY_V2_VERSION) {
