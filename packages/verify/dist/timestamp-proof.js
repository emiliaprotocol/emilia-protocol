...
export function verifyTimestampProof(timestampProof, expectedDigest, pinnedTsaKeys) {
    ...
    if (parsed.error || !parsed.tstInfo || !parsed.signerInfo || !parsed.eContentRaw)
        return refuse(parsed.error ?? 'unparseable_token');
    ...
    if (!sigResult.ok)
        return refuse(sigResult.reason);
    return {
        verified: true,
...
function parseTimeStampToken(der) {
    ...
    if (contentInfo.tag !== 0x10 || !contentInfo.constructed)
    ...
    if (!signedData || signedData.tag !== 0x10)
        return { error: 'unparseable_token' };
    ...
    return { tstInfo: tstInfo, signerInfo: signerInfo, eContentRaw };
}
...
function parseSignerInfo(node) {
    try {
        ...
        if (!digestAlg || digestAlg.tag !== 0x10)
            return { error: 'unparseable_token' };
...
//# sourceMappingURL=timestamp-proof.js.map