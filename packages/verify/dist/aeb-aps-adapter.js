...
export function computeApsPayloadRef(payload) {
    return domainHash('APS-ACTION-PAYLOAD-V1', payload);
}
export function computeApsActionRef(inputObject) {
    return domainHash('APS-ACTION-REF-V2', inputObject);
}
export function computeApsReceiptId(receipt) {
    if (!isRecord(receipt))
        throw new TypeError('APS receipt must be an object');
    const body = {};
    for (const [key, value] of Object.entries(receipt)) {
        if (key !== 'receipt_id' && key !== 'signatures')
            body[key] = value;
    }
...
function parseUtcMillis(value) {
    if (typeof value !== 'string' || !UTC_MILLIS_RE.test(value))
        return NaN;
...
function canonicalStrings(value, allowEmpty = true) {
...
function parseRoots(value) {
    ...
    for (const candidate of value) {
        ...
        roots.push({ ...structuredClone(candidate), key });
    }
    return roots;
}
...
function validActionInput(value) {
...
function materialFromExpected(value, config) {
    if (!isRecord(value) || !exactKeys(value, EXPECTED_ACTION_KEYS)
        ...
        || typeof value.aps_action.nonce !== 'string' || !NONCE_RE.test(value.aps_action.nonce))
        return null;
    try {
...
function validSignatures(receipt, roots) {
    if (!Array.isArray(receipt.signatures) || receipt.signatures.length < 1)
        return false;
    let prior = null;
    ...
    for (const signature of receipt.signatures) {
        if (!isRecord(signature) || !exactKeys(signature, SIGNATURE_KEYS)
            ...
            || typeof signature.value !== 'string' || !HEX_64_RE.test(signature.value))
            return false;
        const identity = `${signature.signer}\0${signature.key_id}`;
        if (prior !== null && utf8Compare(prior, identity) >= 0)
            return false;
        prior = identity;
        ...
        if (!crypto.verify(null, bytes, root.key, Buffer.from(signature.value, 'hex')))
            return false;
        if (signature.signer === receipt.issuer)
            issuerSigned = true;
    }
    return issuerSigned;
}
function validReceiptCommon(receipt, roots) {
    ...
    if (computeApsReceiptId(receipt) !== receipt.receipt_id)
        return 'aps:receipt_id_mismatch';
    if (!validSignatures(receipt, roots))
        return 'aps:receipt_signature_invalid';
    return null;
}
...
function verifyNative(input, pins) {
    ...
    const actionInput = artifact.action_input;
    const intent = artifact.action_intent;
    const decision = artifact.policy_decision;
    const decisionMaterial = artifact.decision_material;
    ...
    if (!exactKeys(intent, INTENT_KEYS)
        ...
        || intent.result.status !== 'declared')
        return reject(result, 'aps:intent_schema_invalid');
    const intentFailure = validReceiptCommon(intent, pins.roots);
    if (intentFailure)
        return reject(result, intentFailure);
    ...
    if (decisionFailure)
        return reject(result, decisionFailure);
...
export function createApsActionDefinition(actionType) {
    if (!ACTION_TYPE_RE.test(actionType))
        throw new TypeError('invalid APS action type');
    return {
...
export function createApsAebAdapter(constructorPins) {
...
//# sourceMappingURL=aeb-aps-adapter.js.map