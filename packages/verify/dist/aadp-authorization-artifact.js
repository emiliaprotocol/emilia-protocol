 ...
 */
import { AUTHORIZATION_BUNDLE_VERSION, verifyAuthorizationBundle, } from './authorization-bundle.js';
import { canonicalizeAeb, digestAeb } from './aeb-adapter-contract.js';
export const AADP_AUTHORIZATION_ARTIFACT_VERSION = 'AADP-AUTHORIZATION-ARTIFACT-v1';
export const AADP_EP_AUTHORIZATION_ARTIFACT_PROFILE = 'EP-AADP-AUTHORIZATION-ARTIFACT-v1';
export const AADP_ACTION_MAPPING_CONFIG_VERSION = 'AADP-ACTION-MAPPING-CONFIG-v1';
export const AADP_ACTION_MAPPING_RECORD_VERSION = 'AADP-ACTION-MAPPING-RECORD-v1';
export const AADP_NATIVE_VERIFIER_DESCRIPTOR_VERSION = 'AADP-NATIVE-VERIFIER-DESCRIPTOR-v1';
export const AADP_NATIVE_VERIFICATION_RECORD_VERSION = 'AADP-NATIVE-VERIFICATION-RECORD-v1';
...
function parseMappingConfiguration(value) {
    ...
    if (!record
        ...
        || record.material_field_map.length === 0)
        return null;
    ...
    for (const candidate of record.material_field_map) {
        ...
        if (!entry
            ...
            || typeof entry.mapped_path !== 'string')
            return null;
...
function parseVerifierDescriptor(value) {
    ...
    if (!record
        ...
        || record.artifact_profile !== AUTHORIZATION_BUNDLE_VERSION)
        return null;
    const implementation = parseImplementation(record.implementation);
    if (!implementation)
        return null;
...
function parseAadpAction(value) {
    ...
    if (!record
        ...
        || !nonEmptyString(record.action_type))
        return null;
...
function parseMappingRecord(value) {
    ...
    if (!implementation
        ...
        || !validDigest(resolverRecord.configuration_digest))
        return null;
    return {
...
export function parseAadpAuthorizationArtifact(value) {
    const record = dataRecord(value);
    if (!record
        ...
        || !['VERIFIED', 'REFUSED', 'UNAVAILABLE'].includes(record.native_verification)
        || !['SATISFIED', 'REFUSE', 'INDETERMINATE'].includes(record.evidence_satisfaction)
        || !validDigest(record.verification_record_digest))
        return null;
    ...
    return {
        ...
        native_verification: record.native_verification,
        evidence_satisfaction: record.evidence_satisfaction,
 ...
 */
export function matchAadpAuthorizationArtifact(presented, expected) {
 ...
 */
export function deriveAadpEpAuthorizationArtifact(input) {
    try {
        ...
        if (verification.verdict !== 'SATISFIED' || artifact === null) {
            return result(verification.verdict === 'REFUSE' ? 'REFUSE' : 'INDETERMINATE', nativeVerification, verification.verdict, verification.reasons.length > 0
...
export function verifyAadpEpAuthorizationArtifact(presented, input) {
...
//# sourceMappingURL=aadp-authorization-artifact.js.map