...
function pubKeyB64u(pub) {
    ...
    if (typeof pub === 'string')
        return pub;
    if (pub && typeof pub === 'object' && typeof pub.public_key === 'string')
...
function validateQuorumPolicy(policy) {
    ...
    if (!policy || typeof policy !== 'object' || Array.isArray(policy))
        return ['quorum_policy must be an object'];
    if (!['threshold', 'ordered'].includes(policy.mode))
        issues.push('quorum_policy.mode must be threshold or ordered');
    ...
    if (policy.distinct_humans === false)
        issues.push('quorum_policy.distinct_humans cannot be false for reliance');
...
function decimalMaterial(candidates) {
    ...
    if (present.length === 0)
        return { value: null, ambiguous: false };
    if (present.some((value) => parseNonNegativeDecimal(value) === null))
...
function isApprovalContext(context) {
    ...
    return Boolean(c && typeof c === 'object'
        && (c.decision === undefined || c.decision === 'approved'));
}
...
export function evaluateReliance(input = {}, opts = {}) {
    ...
    const { action: rawAction = {}, receipt, quorum, authority_proof: authorityProof, revocation_state: revocationState, consumption, relying_party_profile: profile, } = input;
    ...
    if (!receipt || typeof receipt !== 'object')
        return deny('do_not_rely_unsigned', 'no receipt supplied');
    ...
    const rc = verifyTrustReceipt(receipt, receiptVerificationOpts);
    checks.receipt = rc.valid === true;
    if (!rc.valid)
        return deny('do_not_rely_unsigned', `receipt did not verify: ${(rc.errors || []).join('; ') || 'invalid'}`);
    ...
    for (const c of contexts) {
        try {
            ctxByHash.set(crypto.createHash('sha256').update(canonicalize(c), 'utf8').digest('hex'), c);
        }
        catch { /* skip uncanonicalizable */ }
    }
    ...
    for (const s of (Array.isArray(receipt.signoffs) ? receipt.signoffs : [])) {
        ...
        if (!ctx?.approver || !isApprovalContext(ctx))
            continue;
    ...
    if (hasCheckpoint) {
        ...
        if (!issuerOk)
            return deny('do_not_rely_untrusted_issuer', 'transparency checkpoint was not signed by a pinned issuer key');
    }
    else {
        checks.issuer = true;
    }
    ...
    if (requiredAssurance === 'class_a') {
        ...
        if (!hasClassA)
            return deny('do_not_rely_no_class_a', 'a valid Class-A device-bound signoff is required and none is present');
        ...
        if (!quorumOk)
            return deny('do_not_rely_quorum_unsatisfied', 'a satisfied EP-QUORUM-v1 bound to this action is required');
        ...
        if (classASigners.length > 0) {
            ...
            const { q, ok: quorumOk } = boundQuorum();
            if (!quorumOk)
                return deny('do_not_rely_no_class_a', 'required evidence demands a valid Class-A signoff or quorum, but neither is present');
    ...
    if (prof.required_authority === true || requiredEvidence.has('authority_proof')) {
        ...
        const organizationRegistryKeys = acceptedRegistryKeys.filter((key) => key?.organization_id === action.organization_id);
        ...
        if (!p.authority_id)
            return deny('do_not_rely_authority_missing', 'authority proof carries no authority_id');
        ...
        if (p.revocation?.status === 'revoked')
            return deny('do_not_rely_authority_revoked', 'authority was revoked as of the proof check');
    ...
    if (acceptedPolicyHashes.length > 0) {
        ...
        if (!policyOk)
            return deny('do_not_rely_policy_mismatch', 'the action policy hash is not on the accepted list');
    }
    else {
    ...
    if (requiredEvidence.has('revocation_freshness')) {
        ...
        if (hasStatement || hasTarget) {
            ...
            if (!rv.valid)
                return deny('do_not_rely_stale_revocation', 'revocation evidence did not verify under a pinned revoker');
        ...
        if (!fresh)
            return deny('do_not_rely_stale_revocation', 'the revocation check is older than the pinned freshness bound');
    }
    else {
    ...
    if (requiredEvidence.has('consumption_proof')) {
        ...
        if (consumption && typeof consumption === 'object' && Object.hasOwn(consumption, 'proof')) {
            ...
            if (!cp.valid)
                return deny('do_not_rely_already_consumed', `consumption evidence did not verify: ${cp.reason || 'invalid'}`);
            checks.consumption = 'consumed';
            return deny('do_not_rely_already_consumed', 'the authorization has already been consumed');
        }
...
export function validateRelianceProfile(profile) {
    ...
    if (!profile || typeof profile !== 'object' || Array.isArray(profile))
        return { ok: false, issues: ['profile is not an object'] };
    if (profile['@type'] !== RELIANCE_PROFILE_VERSION)
        issues.push(`@type must be ${RELIANCE_PROFILE_VERSION}`);
...
//# sourceMappingURL=reliance.js.map