...
function referenceSet(value) {
    ...
    for (const measurement of value) {
        if (typeof measurement !== 'string' || !SHA256.test(measurement) || result.has(measurement))
            return null;
...
function verifyPlatformAttestationInternal(evidence, options) {
    ...
    try {
        signatureValid = crypto.verify(null, Buffer.from(`${headerSegment}.${payloadSegment}`, 'ascii'), pinnedKey, signature);
    }
    catch {
    ...
    if (payload.iat * 1000 > verificationTimeMs)
        return fail('token_from_future');
    if (verificationTimeMs >= payload.exp * 1000)
        return fail('token_expired');
    if (verificationTimeMs - payload.iat * 1000 > options.maxAgeSeconds * 1000)
 ...
 */
export function verifyPlatformAttestation(evidence, options) {
    try {
...
//# sourceMappingURL=platform-attestation.js.map