 ...
 */
export const NON_REDACTABLE_PATHS = Object.freeze([
    'caid',
    'action.caid',
    'action.action_type',
    'action_digest',
...
function canonicalBase64url(value) {
    if (typeof value !== 'string' || value.length === 0
        || !B64URL_RE.test(value) || value.length % 4 === 1)
        return false;
    try {
...
function nonRedactableConflict(path) {
 ...
 */
export function sdCommitmentDigest(path, salt, value) {
...
function commitSlot(path, salt, value) {
 ...
 */
export function sdPresentationBindingDigest(receipt, disclosed, binding) {
...
function scanMarkerStrings(value, path, hits) {
    if (typeof value === 'string') {
        if (value.startsWith(EP_SD_COMMIT_MARKER_PREFIX))
 ...
 */
export function prepareSelectiveDisclosure(payload, disclosablePaths, salts = {}) {
    try {
        if (!isRecord(payload) || !isStrictCanonicalJson(payload))
            return refuse(['malformed_payload']);
        if (Object.hasOwn(payload, 'disclosure'))
            return refuse(['disclosure_block_present']);
        ...
        if (!isRecord(salts) || !isStrictCanonicalJson(salts))
            return refuse(['malformed_salts']);
        ...
        for (const path of disclosablePaths) {
            if (!validPath(path)) {
                reasons.push(`invalid_path:${String(path).slice(0, 64)}`);
                continue;
            }
            if (seen.has(path)) {
                reasons.push(`duplicate_path:${path}`);
                continue;
            }
            seen.add(path);
            if (nonRedactableConflict(path)) {
                reasons.push(`non_redactable_path:${path}`);
                continue;
            }
            if (!getPath(payload, path).found)
                reasons.push(`unknown_path:${path}`);
        }
        ...
        for (let i = 0; i < sorted.length; i += 1) {
            for (let j = i + 1; j < sorted.length; j += 1) {
                if (pathsOverlap(sorted[i], sorted[j]))
                    reasons.push(`overlapping_paths:${sorted[i]}:${sorted[j]}`);
            }
        }
        if (reasons.length > 0)
        ...
        const prepared = JSON.parse(canonicalizeStrictJson(payload));
        for (const path of sorted) {
            ...
            const salt = provided === undefined
                ...
                : provided;
            if (!canonicalBase64url(salt)) {
                reasons.push(`salt_invalid:${path}`);
                continue;
            }
            ...
            if (saltValues.has(salt)) {
                reasons.push('salt_reuse');
                continue;
            }
            saltValues.add(salt);
 ...
 */
export function createSelectiveDisclosurePresentation(receipt, openings, disclosePaths, binding, opts = {}) {
    try {
        if (!isRecord(receipt) || !isStrictCanonicalJson(receipt))
            return refuse(['malformed_receipt']);
        ...
        if (!Array.isArray(disclosePaths))
            return refuse(['malformed_path_list']);
        const declared = new Set(block.paths.filter((p) => typeof p === 'string'));
        const reasons = [];
        const disclosed = [];
        const seen = new Set();
        for (const path of disclosePaths) {
            ...
            if (seen.has(path)) {
                reasons.push(`duplicate_disclosure:${path}`);
                continue;
            }
            seen.add(path);
            ...
            if (!slot.found || slot.value !== commitSlot(path, opening.salt, opening.value)) {
                reasons.push(`digest_mismatch:${path}`);
                continue;
            }
            disclosed.push({ path, salt: opening.salt, value: opening.value });
        }
        if (reasons.length > 0)
            return refuse(reasons);
...
function verifyEd25519OverDigest(signatureB64u, digest, publicKeySpkiB64u) {
    try {
 ...
 */
export function verifySelectiveDisclosurePresentation(presentation, issuerPublicKeySpkiB64u, expected) {
    const checks = {
    ...
    try {
        ...
        if (pres['@version'] !== EP_SD_PRESENTATION_VERSION)
        ...
        if (receiptResult.valid !== true)
            return result(['receipt_signature_invalid']);
        checks.receipt_signature = true;
        const payload = pres.receipt.payload;
        // 3. The signed disclosure block.
        const block = payload.disclosure;
        if (block === undefined)
            return result(['missing_disclosure_block']);
        if (!isRecord(block) || block['@version'] !== EP_SD_VERSION || block.alg !== 'sha256'
            ...
            || !block.paths.every((p) => typeof p === 'string' && validPath(p))
        ...
        const declaredPaths = block.paths;
        ...
        if (caidCandidate === undefined)
            return result(['missing_caid']);
        ...
        for (const hit of markerHits) {
            if (!slotByPath.has(hit.path))
                commitmentReasons.push(`undeclared_commitment:${hit.path}`);
        }
        if (commitmentReasons.length > 0)
            return result(commitmentReasons, { caid });
        ...
        for (const entry of pres.disclosed) {
            ...
            if (!slotByPath.has(path)) {
                openingReasons.push(`undeclared_disclosure:${path.slice(0, 64)}`);
                continue;
            }
            if (seenPaths.has(path)) {
                openingReasons.push(`duplicate_disclosure:${path}`);
                continue;
            }
            seenPaths.add(path);
            const salt = entry.salt;
            if (!nonEmptyString(salt)) {
                openingReasons.push(`missing_salt:${path}`);
                continue;
            }
            if (!canonicalBase64url(salt)) {
                openingReasons.push(`salt_invalid:${path}`);
                continue;
            }
            if (Buffer.from(salt, 'base64url').length < EP_SD_MIN_SALT_BYTES) {
            ...
            if (seenSalts.has(salt)) {
                openingReasons.push('salt_reuse');
                continue;
            }
            seenSalts.add(salt);
        ...
        for (const path of expected.requiredPaths ?? []) {
            ...
            if (Object.hasOwn(disclosedValues, path))
                continue;
            ...
            if (!isPlainlyReadable)
                requiredReasons.push(`undisclosed_required_field:${path}`);
        }
        ...
        if (!nonEmptyString(binding.audience) || !nonEmptyString(binding.nonce)
            ...
            if (binding.audience !== expected.audience)
                bindingReasons.push('binding_audience_mismatch');
            if (binding.nonce !== expected.nonce)
                bindingReasons.push('binding_nonce_mismatch');
        }
        ...
        if (expected.holderPublicKeySpkiB64u !== undefined || holderProof !== undefined) {
            ...
            const digest = sdPresentationBindingDigest(pres.receipt, pres.disclosed, {
                audience: binding.audience,
                nonce: binding.nonce,
                created_at: binding.created_at,
            });
            if (!verifyEd25519OverDigest(holderProof.signature, digest, holderProof.public_key)) {
...
//# sourceMappingURL=receipt-selective-disclosure.js.map