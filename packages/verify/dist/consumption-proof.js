...
export function verifyConsumptionProof(bundle) {
    ...
    if (!niRes.ok)
        return fail(niRes.reason ?? 'non_inclusion_invalid');
    ...
    if (!incRes.ok)
        return fail(incRes.reason ?? 'inclusion_invalid');
    ...
    if (hexOf(ni.root) === hexOf(inc.root))
    ...
    _rootRec(level, prefixBits) {
        if (level === this.depth) {
            ...
            for (const [keyHex, valueHex] of this.present) {
                if (this._bitsOf(keyHex, this.depth) === prefixBits)
        ...
        for (const [keyHex] of this.present) {
            if (this._bitsOf(keyHex, level).startsWith(prefixBits) || prefixBits === '') {
                any = true;
                break;
            }
        }
...
//# sourceMappingURL=consumption-proof.js.map