...
async function resolveBackend(mldsaBackend, mldsaBackendLoader) {
    if (mldsaBackend !== undefined && mldsaBackend !== null) {
        if (typeof mldsaBackend.verify !== 'function')
            return null; // malformed injection refuses
        return mldsaBackend;
    }
...
export async function signHybrid(message, keys, options = {}) {
    ...
    if (!messageBytes)
        throw new TypeError('signHybrid: message must be a Uint8Array or string');
    if (!keys || !keys.ed25519PrivateKey)
        throw new TypeError('signHybrid: keys.ed25519PrivateKey is required');
    ...
    if (!mldsaSecretKey)
        throw new TypeError('signHybrid: keys.mldsaSecretKey is required');
...
export async function verifyHybrid(message, envelope, keys, options = {}) {
    ...
    const messageBytes = toMessageBytes(message);
    if (!messageBytes)
        return refuse(REASONS.INVALID_INPUT);
    // 1. Envelope shape (fail closed on anything unexpected)
    if (!envelope || typeof envelope !== 'object' || Array.isArray(envelope))
        return refuse(REASONS.INVALID_ENVELOPE);
    const e = envelope;
    if (e.alg !== HYBRID_ALG)
        return refuse(REASONS.INVALID_ENVELOPE);
    if (!e.sigs || typeof e.sigs !== 'object' || Array.isArray(e.sigs))
        return refuse(REASONS.INVALID_ENVELOPE);
    ...
    if (!algosEqual(e.signature_algos, HYBRID_SIGNATURE_ALGOS)) {
    ...
    for (const algo of HYBRID_SIGNATURE_ALGOS) {
        if (typeof sigs[algo] !== 'string' || sigs[algo].length === 0) {
    ...
    //    algorithm.
    if (!keys || !keys.ed25519PublicKey || !keys.mldsaPublicKey)
        return refuse(REASONS.MISSING_KEY);
    const edKey = toEd25519PublicKeyObject(keys.ed25519PublicKey);
    if (edKey === 'algorithm_mismatch')
        return refuse(REASONS.ALGORITHM_KEY_MISMATCH);
    if (!edKey)
        return refuse(REASONS.MISSING_KEY);
    const pqKey = toRawBytes(keys.mldsaPublicKey);
    if (!pqKey || pqKey.length === 0)
        return refuse(REASONS.MISSING_KEY);
    if (pqKey.length !== ML_DSA_65_PUBLIC_KEY_BYTES)
        return refuse(REASONS.PUBLIC_KEY_LENGTH_INVALID);
    ...
    const edSigBytes = new Uint8Array(Buffer.from(sigs['Ed25519'], 'base64url'));
    if (edSigBytes.length !== ED25519_SIGNATURE_BYTES)
        return refuse(REASONS.SIGNATURE_LENGTH_INVALID);
    const pqSigBytes = new Uint8Array(Buffer.from(sigs['ML-DSA-65'], 'base64url'));
    if (pqSigBytes.length !== ML_DSA_65_SIGNATURE_BYTES)
        return refuse(REASONS.SIGNATURE_LENGTH_INVALID);
    ...
    if (!checks.classical_signature)
        return refuse(REASONS.CLASSICAL_INVALID);
    // 6. PQ leg (ML-DSA-65). No backend => REFUSE. Never skip, never pass.
    const backend = await resolveBackend(options.mldsaBackend, options.mldsaBackendLoader);
    if (!backend || typeof backend.verify !== 'function')
        return refuse(REASONS.PQ_BACKEND_UNAVAILABLE);
    let pqOk = false;
    try {
        pqOk = backend.verify(pqSigBytes, new Uint8Array(signingInput), new Uint8Array(pqKey)) === true;
    }
    catch {
        pqOk = false;
    }
    ...
    if (!pqOk)
        return refuse(REASONS.PQ_INVALID);
    return { verified: true, reason: null, checks };
}
//# sourceMappingURL=pq-hybrid.js.map