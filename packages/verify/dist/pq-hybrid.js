...
export async function verifyHybrid(message, envelope, keys, options = {}) {
    ...
    let edOk = false;
    try {
        edOk = crypto.verify(null, signingInput, edKey, Buffer.from(sigs['Ed25519'], 'base64url'));
    }
    catch {
        edOk = false;
    }
    ...
    let pqOk = false;
    try {
        pqOk = backend.verify(new Uint8Array(Buffer.from(sigs['ML-DSA-65'], 'base64url')), new Uint8Array(signingInput), new Uint8Array(pqKey)) === true;
    }
    catch {
        pqOk = false;
    }
