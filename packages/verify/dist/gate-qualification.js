...
import { verifyAgileSignature, ML_DSA_65_SIGNATURE_BYTES, } from './pq-signature-agility.js';
...
export const GATE_QUALIFICATION_HYBRID_REQUIRED_ALGORITHMS = Object.freeze(['Ed25519', 'ML-DSA-65']);
...
function hybridSetMatchesRegistered(algorithms) {
...
const HYBRID_SIGNATURE_BYTES = Object.freeze({
    'Ed25519': 64,
 ...
 */
export async function verifyHybridDsseEnvelope(envelope, expectedPayloadType, trust, options = {}) {
    const checks = {
        structure: false,
        algorithm_set: false,
        legs_present: false,
    ...
    for (const signature of env.signatures) {
        ...
        if (!record(pin) || typeof pin.alg !== 'string' || typeof pin.public_key !== 'string'
            || !GATE_QUALIFICATION_HYBRID_REQUIRED_ALGORITHMS.includes(pin.alg)) {
    ...
    for (const alg of required) {
        if (!presentedAlgs.has(alg))
            errors.push(`missing_required_algorithm:${alg}`);
    }
    ...
    if (checks.legs_present && lengthsOk) {
        ...
        for (const leg of legs) {
            const verdict = await verifyAgileSignature(new Uint8Array(pae), {
                alg: leg.alg,
                sig: Buffer.from(leg.sig, 'base64').toString('base64url'),
                key_id: leg.keyid,
            }, { alg: leg.alg, public_key: leg.publicKey }, options);
