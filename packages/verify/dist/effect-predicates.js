...
function trimTrailingDecimalZeros(value) {
    ...
    while (end > 0 && value.charCodeAt(end - 1) === 0x30)
        end -= 1;
...
export function compareDecimalStrings(a, b) {
    ...
    if (A.int.length !== B.int.length)
        mag = A.int.length < B.int.length ? -1 : 1;
    else if (A.int !== B.int)
        mag = A.int < B.int ? -1 : 1;
    else {
    ...
    predicted.forEach((entry, i) => {
        ...
        for (const k of Object.keys(entry)) {
            if (!ENTRY_KEYS.includes(k))
                return bad(`${at} has unknown member "${k}"`);
        }
        ...
        if (!p || typeof p !== 'object' || Array.isArray(p))
            return bad(`${at}.predicate is not an object`);
        if (!PREDICATE_OPS.includes(p.op))
            return bad(`${at}.predicate.op "${p.op}" is not a known op`);
        const allowed = PREDICATE_KEYS[p.op];
        for (const k of Object.keys(p)) {
            if (!allowed.includes(k))
                return bad(`${at}.predicate (op ${p.op}) has unknown member "${k}"`);
        }
        if (p.op === 'eq') {
            if (typeof p.value === 'number')
                return bad(`${at}.predicate.value is a number; comparison values MUST be strings (canonicalization malleability)`);
            if (typeof p.value !== 'string' || tooLong(p.value))
                return bad(`${at}.predicate.value must be a bounded string`);
        }
        if (p.op === 'lte' || p.op === 'gte') {
            if (typeof p.value === 'number')
                return bad(`${at}.predicate.value is a number; comparison values MUST be strings (canonicalization malleability)`);
            if (typeof p.value !== 'string' || tooLong(p.value) || !isDecimalString(p.value))
                return bad(`${at}.predicate.value must be a bounded decimal string`);
        }
        if (p.op === 'range') {
            let broke = false;
            for (const f of ['min', 'max']) {
                if (typeof p[f] === 'number') {
                    bad(`${at}.predicate.${f} is a number; comparison values MUST be strings (canonicalization malleability)`);
                    broke = true;
                }
                else if (typeof p[f] !== 'string' || tooLong(p[f]) || !isDecimalString(p[f])) {
                    bad(`${at}.predicate.${f} must be a bounded decimal string`);
                    broke = true;
                }
            }
            if (!broke && compareDecimalStrings(p.min, p.max) === 1)
                bad(`${at}.predicate range has min > max`);
        }
        if (p.op === 'set_eq') {
            if (!Array.isArray(p.values) || p.values.length > MAX_OBSERVED_EFFECTS)
                return bad(`${at}.predicate.values must be a bounded array of strings`);
            for (const v of p.values) {
                if (typeof v === 'number')
                    return bad(`${at}.predicate.values contains a number; comparison values MUST be strings (canonicalization malleability)`);
                if (typeof v !== 'string' || tooLong(v))
                    return bad(`${at}.predicate.values must contain only bounded strings`);
            }
        }
        if (p.op === 'count_lte') {
            if (typeof p.value === 'number')
                return bad(`${at}.predicate.value is a number; comparison values MUST be strings (canonicalization malleability)`);
            if (typeof p.value !== 'string' || tooLong(p.value) || !/^(0|[1-9][0-9]*)$/.test(p.value))
                return bad(`${at}.predicate.value must be a bounded non-negative integer string`);
        }
    });
...
function validateObservedEffects(observed) {
    ...
    observed.forEach((entry, i) => {
        ...
        for (const member of Object.keys(entry)) {
            if (!OBSERVED_ENTRY_KEYS.includes(member))
                bad(`${at} has unknown member "${member}"`);
        }
        if (typeof entry.effect_type !== 'string' || !entry.effect_type || tooLong(entry.effect_type)) {
        ...
        if (hasValue === hasValues)
            bad(`${at} must carry exactly one of value or values`);
...
function evaluateEntry(entry, matches) {
    ...
    if (p.op === 'count_lte') {
        ...
        const cmp = compareDecimalStrings(count, p.value);
        return cmp <= 0
            ? inBounds
    ...
    if (matches.length > 1)
        return incomparable(`ambiguous: ${matches.length} observed effects match ${at}`);
    const obs = matches[0];
    if (p.op === 'set_eq') {
        if (!Array.isArray(obs.values))
            return incomparable(`observed effect for ${at} has no values array`);
        for (const v of obs.values) {
            if (typeof v !== 'string')
                return incomparable(`observed values for ${at} contain a non-string (values MUST be strings)`);
        }
    ...
    if (!isDecimalString(obs.value))
        return incomparable(`observed value "${obs.value}" for ${at} is not a decimal string`);
    ...
    if (p.op === 'lte') {
        const cmp = compareDecimalStrings(obs.value, p.value);
        return cmp <= 0
            ? inBounds
    ...
    // range
    const cmpMin = compareDecimalStrings(obs.value, p.min);
    if (cmpMin < 0)
        return divergent(`predicted range [${p.min}, ${p.max}] for ${at}, observed ${obs.value} (below min)`);
    const cmpMax = compareDecimalStrings(obs.value, p.max);
    if (cmpMax > 0)
        return divergent(`predicted range [${p.min}, ${p.max}] for ${at}, observed ${obs.value} (above max)`);
    return inBounds;
}
/**
...
export function evaluatePredictedEffects(predicted, observed) {
    ...
    const results = predicted.map((entry) => {
        const matches = observed.filter((o) => o && typeof o === 'object' && o.effect_type === entry.effect_type && o.target === entry.target);
...
//# sourceMappingURL=effect-predicates.js.map