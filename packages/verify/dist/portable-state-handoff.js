...
function uniqueText(value, maxItems = PORTABLE_STATE_LIMITS.max_objects) {
...
function reason(reasons, value) {
    if (!reasons.includes(value))
        reasons.push(value);
}
export function stateHandoffDigest(value) {
...
function signaturesAreShaped(value) {
    return Array.isArray(value) && value.length > 0 && value.length <= 2 && value.every((entry) => (exact(entry, SIGNATURE_KEYS)
        && ['Ed25519', 'ML-DSA-65'].includes(entry.alg)
        && text(entry.sig, 8192)
        && text(entry.key_id)));
}
function artifactBytes(domain, unsigned) {
...
async function signArtifact(domain, unsigned, signer) {
...
async function verifyArtifact(domain, value, principal, issuedAt, pins, agility) {
    const reasons = [];
    ...
    for (const alg of required) {
        ...
        if (pin.status !== 'active')
            reason(reasons, 'signer_pin_inactive');
        if (!pin.principals.includes(principal))
            reason(reasons, 'signer_principal_mismatch');
        ...
        if (!signature || signature.key_id !== pin.key_id)
            reason(reasons, 'signer_key_id_mismatch');
        activePins.push(pin);
    }
    if (reasons.length > 0)
        return reasons;
    const result = await verifyAgileSignatureSet(artifactBytes(domain, withoutSignatures(value)), value.signatures, activePins, { ...agility, policy: 'hybrid_all', requiredAlgorithms: required });
    if (result.verified !== true)
        reason(reasons, `artifact_signature_invalid:${result.reason ?? 'unknown'}`);
    return reasons;
}
...
function validateDescriptor(value, position) {
...
function expectedSourceActions(manifest) {
    const actions = [PORTABLE_STATE_ACTIONS.EXPORT];
    if (manifest.objects.some((entry) => entry.sensitivity === 'VAULT')) {
        actions.push(PORTABLE_STATE_ACTIONS.KEY_RELEASE);
    }
    return actions;
}
function validateManifestShape(value, reasons) {
    ...
    if (!exact(value.index, INDEX_KEYS)
        ...
        || !digest(value.index.index_digest))
        valid = false;
    if (!exact(value.authority, AUTHORITY_KEYS)
        ...
        || value.authority.recipient_action !== PORTABLE_STATE_ACTIONS.IMPORT)
        valid = false;
    if (Array.isArray(value.objects)) {
        value.objects.forEach((entry, position) => {
            if (!validateDescriptor(entry, position))
                valid = false;
        });
    }
    if (!valid)
...
function vaultSetDigest(manifest) {
...
export function stateActionExpectation(manifest, actionType, importReceiptDigest, retirementSetDigest) {
    ...
    if (actionType === PORTABLE_STATE_ACTIONS.KEY_RELEASE)
        action.vault_set_digest = vaultSetDigest(manifest);
...
function safeManifestDigest(value) {
    try {
        return stateHandoffDigest(value);
    }
    catch {
...
function receiptShapeValid(value) {
    const valid = exact(value, RECEIPT_KEYS)
        ...
        && ['INITIAL', 'RECONCILIATION'].includes(value.receipt_kind)
        && text(value.handoff_id)
        && digest(value.manifest_digest)
        && text(value.payload_profile)
        && text(value.importer_boundary_id)
        && ['ACCEPTED', 'PARTIAL', 'REFUSED', 'INDETERMINATE'].includes(value.result)
        ...
        && value.authority_evidence.every((entry) => exact(entry, EVIDENCE_KEYS)
            && ['SOURCE_RELEASE', 'RECIPIENT_COMMIT'].includes(entry.stage)
            && Object.values(PORTABLE_STATE_ACTIONS).includes(entry.action)
    ...
    if (!valid)
        return false;
    const evidence = value.authority_evidence;
    const unavailable = value.unavailable_objects;
...
function indeterminate(reasons) {
...
async function issueReceipt(partial, signer) {
    ...
    const signatures = await signArtifact(RECEIPT_DOMAIN, partial, signer);
    return { ...partial, signatures };
}
async function failureReceipt(bundle, options, manifestDigest, reasons, sourceEvidence = [], unavailable = []) {
    const manifest = isObject(bundle?.manifest) ? bundle.manifest : null;
    return issueReceipt({
...
async function successReceipt(record, options, kind) {
...
function validateManifestIntrinsic(manifest, reasons) {
    ...
    if (new Set(orderedIds).size !== orderedIds.length)
        reason(reasons, 'duplicate_manifest_object');
    if (!same(orderedIds, manifest.index.ordered_object_ids))
        reason(reasons, 'index_order_mismatch');
    if (stateHandoffDigest(manifest.objects) !== manifest.index.index_digest)
        reason(reasons, 'index_digest_mismatch');
    ...
    if (manifest.scope_digest !== expectedScope)
        reason(reasons, 'scope_digest_mismatch');
}
async function evaluateManifest(value, options, reasons) {
    if (!validateManifestShape(value, reasons))
        return false;
    const manifest = value;
    if (manifest.recipient_agent !== options.expected_recipient_agent)
        reason(reasons, 'recipient_mismatch');
    ...
    if (manifest.relying_party_id !== options.expected_relying_party_id)
        reason(reasons, 'relying_party_mismatch');
    if (!time(options.now))
        reason(reasons, 'verifier_time_invalid');
    if (time(options.now) && Date.parse(options.now) < Date.parse(manifest.created_at)) {
        reason(reasons, 'manifest_not_yet_valid');
    }
    if (time(options.now) && Date.parse(options.now) > Date.parse(manifest.expires_at))
        reason(reasons, 'manifest_expired');
    validateManifestIntrinsic(manifest, reasons);
    const signatureReasons = await verifyArtifact(MANIFEST_DOMAIN, manifest, manifest.source_agent, manifest.created_at, options.source_signer_pins, options.signature_agility);
 ...
 */
export async function importPortableState(bundle, options) {
    ...
    for (const action of expectedSource) {
        ...
        let verified;
        try {
            verified = await options.source_authority_verifier.verify(expected, bundle.source_authority_evidence[action]);
        }
        catch {
    ...
    if (reasons.length > 0)
        return failureReceipt(bundle, options, manifestDigest, reasons, sourceEvidence);
    const byId = new Map();
    ...
    for (const descriptor of manifest.objects) {
        ...
        if (object === undefined) {
            if (descriptor.required)
                reason(reasons, 'required_object_missing');
            else
                unavailable.push({ object_id: descriptor.object_id, reason: 'optional_object_missing' });
            continue;
        }
        let objectDigest;
        try {
        ...
        if (profileResult.status !== 'VALID') {
            ...
            if (profileResult.status === 'INDETERMINATE')
                reason(reasons, 'payload_validation_indeterminate');
            continue;
        }
        let head;
        try {
        ...
        if (head) {
            if (descriptor.generation <= head.generation)
                reason(reasons, 'lineage_rollback');
            else if (descriptor.generation > head.generation + 1)
                reason(reasons, 'lineage_gap');
            else if (descriptor.predecessor_digest !== head.object_digest)
                reason(reasons, 'lineage_fork');
        }
        ...
        if (descriptor.sensitivity === 'VAULT')
            vaultObjects.push({ descriptor, object });
    }
    if (reasons.length > 0)
    ...
    if (vaultObjects.length > 0) {
        ...
        try {
            availability = await (options.verify_vault_availability?.(vaultObjects)
                ?? Promise.resolve({ status: 'INDETERMINATE', reasons: ['vault_key_unavailable'] }));
        }
        catch {
        ...
        if (availability.status !== 'AVAILABLE') {
            ...
            if (availability.status === 'INDETERMINATE')
                reason(reasons, 'vault_availability_indeterminate');
            return failureReceipt(bundle, options, manifestDigest, reasons, sourceEvidence, unavailable);
        }
    }
...
export async function reconcilePortableStateImport(handoffId, manifestDigest, options) {
    const record = options.recipient_boundary.lookupAdmission(handoffId);
    if (!record || record.manifest_digest !== manifestDigest)
        return null;
    return successReceipt(record, options, 'RECONCILIATION');
}
export async function verifyPortableStateImportReceipt(value, pins, agility) {
    if (!receiptShapeValid(value))
        return { valid: false, reasons: ['receipt_schema_invalid'] };
    let reasons;
    try {
        reasons = await verifyArtifact(RECEIPT_DOMAIN, value, value.importer_boundary_id, value.issued_at, pins, agility);
    }
    catch {
 ...
 */
export async function verifyPortableStateImportReceiptForManifest(value, manifestValue, pins, agility) {
    ...
    if (value.handoff_id !== manifest.handoff_id)
        reason(reasons, 'receipt_handoff_mismatch');
    if (value.manifest_digest !== manifestDigest)
        reason(reasons, 'receipt_manifest_digest_mismatch');
    if (value.payload_profile !== manifest.payload_profile)
        reason(reasons, 'receipt_payload_profile_mismatch');
    ...
    for (const objectId of [...value.accepted_object_ids, ...unavailableIds]) {
        if (!descriptorById.has(objectId))
            reason(reasons, 'receipt_object_not_in_manifest');
    }
    ...
    for (const entry of value.authority_evidence) {
        try {
            ...
            if (entry.caid !== expected.caid)
                reason(reasons, 'receipt_authority_caid_mismatch');
        }
        catch {
...
export class InMemoryRecipientStateBoundary {
    #heads = new Map();
    #objects = new Map();
    #admissions = new Map();
    #options;
    ...
    seedHead(objectId, head, object = null) {
        if (!text(objectId) || !Number.isSafeInteger(head.generation) || head.generation < 0
            || !digest(head.object_digest))
            throw new TypeError('invalid state head');
        this.#heads.set(objectId, { ...head });
        this.#objects.set(objectId, structuredClone(object));
    }
    consumptionCount() { return this.#consumptions; }
    loseNextAcknowledgement() { this.#loseAcknowledgement = true; }
    async commitImport(request) {
        if (this.#admissions.has(request.handoff_id))
            return { status: 'REFUSED', reasons: ['handoff_replay'] };
        ...
        const decision = this.#options.authorizeImport(request.import_authority.expected, request.import_authority.evidence);
        if (decision.status !== 'AUTHORIZED')
            return decision;
        if (!digest(decision.receipt_digest))
            return { status: 'REFUSED', reasons: ['authority_receipt_digest_invalid'] };
...
export async function signPortableStateManifest(unsigned, signer) {
    ...
    const signatures = await signArtifact(MANIFEST_DOMAIN, unsigned, signer);
    const signed = { ...unsigned, signatures };
    return signed;
}
export function buildSourceRetirementExpectation(manifest, acceptedImportReceipt) {
    ...
    const retirementSetDigest = stateHandoffDigest(manifest.objects
        ...
        .map((entry) => ({ object_id: entry.object_id, object_digest: entry.object_digest })));
    return stateActionExpectation(manifest, PORTABLE_STATE_ACTIONS.RETIRE_SOURCE, stateHandoffDigest(acceptedImportReceipt), retirementSetDigest);
}
//# sourceMappingURL=portable-state-handoff.js.map