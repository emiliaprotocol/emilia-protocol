...
function canonicalOutcomeSourceKey(publicKey) {
...
function outcomeSourceKeyId(publicKey) {
    const canonical = canonicalOutcomeSourceKey(publicKey);
    if (!canonical)
        throw new Error('outcome source public key must be canonical Ed25519 SPKI');
...
function observationSigningBytes(observation) {
...
export function buildOutcomeObservation({ receipt_id, receipt_digest, action_hash, action_caid, consumption_nonce, operation_id, source, observed_from, observed_until, attested_at, observed_effects, signer, } = {}) {
    const observedValidation = validateObservedEffects(observed_effects);
    if (!observedValidation.ok)
        throw new Error(observedValidation.errors.join('; '));
    if (typeof receipt_id !== 'string' || !receipt_id)
...
export function verifyOutcomeObservation(observation, opts = {}) {
    ...
    if (checks.source_key_pinned && checks.source_metadata_pinned) {
        try {
            ...
            checks.signature = key.asymmetricKeyType === 'ed25519'
                && crypto.verify(null, observationSigningBytes(observation), key, Buffer.from(observation.proof.signature_b64u, 'base64url'));
        }
        catch {
            checks.signature = false;
        }
...
function sourceSelector(prediction) {
    return {
...
export function outcomeBindingSetResultDigest(result) {
 ...
 */
export function verifyOutcomeObservationSet(predictedEffects, observations, opts = {}) {
    const checks = {
    ...
    if (!checks.predictions_valid)
        errors.push(...predictionValidation.reasons);
    ...
    observation_results.forEach((result, index) => {
        if (!result.valid)
            errors.push(...result.errors.map((reason) => `observation[${index}]: ${reason}`));
    });
    const first = observations?.[0];
    checks.exact_bindings = checks.observations_verified
        && observations.every((observation) => (observation.receipt_id === first.receipt_id
    ...
    if (!checks.exact_bindings)
        errors.push('outcome_observations_not_exactly_bound');
    ...
    if (checks.observations_verified && checks.exact_bindings) {
        for (const { selector, predictions } of grouped.values()) {
            const matches = observations.filter((observation) => (observation.source.role === selector.role
    ...
    missing_sources.forEach((selector) => errors.push(`required_outcome_source_missing:${selector.role}:${selector.source_class || '*'}`));
    ...
    if (!requirementsValid)
        errors.push('outcome_source_requirements_malformed');
    if (requirementsValid) {
        for (const requirement of requirements) {
            ...
            const matches = verifiedObservations.filter((observation) => (observation.source.role === role
    ...
    if (!windowsValid)
        errors.push('outcome_observation_windows_malformed');
    if (windowsValid) {
        for (const window of windows) {
            ...
            const matches = verifiedObservations.filter((observation) => (observation.source.role === window.role
                && (sourceClass === undefined || observation.source.source_class === sourceClass)));
            if (matches.length === 0) {
                windowsValid = false;
...
export function verifyOutcomeBindingSetCore(receipt, observations, opts = {}, verifyReceipt) {
    let receipt_result = null;
    try {
    ...
    const base = verifyOutcomeObservationSet(Array.isArray(signedPredictions) ? signedPredictions : [], observations, opts);
    const receiptDigest = safeDigest(receipt);
    const receiptBound = Array.isArray(observations) && observations.length > 0
        && observations.every((observation) => (observation.receipt_id === receipt?.receipt_id
...
export function verifyOutcomeBindingCore(receipt, attestation, opts = {}, verifyReceipt) {
    ...
    if (!receiptCanonical)
        return refuse('receipt_not_canonical_json');
    if (!attestationCanonical)
        return refuse('attestation_not_canonical_json');
