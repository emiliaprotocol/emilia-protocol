...
export const AGENT_CONTINUITY_V2_REQUIRED_ALGORITHMS = Object.freeze(['Ed25519', 'ML-DSA-65']);
...
function unsignedForSignatureV2(envelope) {
 ...
 */
export function continuityV2SigningBytes(unsignedEnvelope, requiredAlgorithms = AGENT_CONTINUITY_V2_REQUIRED_ALGORITHMS) {
    ...
    return Buffer.from(`${AGENT_CONTINUITY_V2_DOMAIN}${canonicalizeAeb({ ...body, required_algorithms: [...requiredAlgorithms] })}`, 'utf8');
}
/** Build and sign an immutable hybrid continuity envelope. Throws on issuer misuse. */
export async function createAgentContinuityEnvelopeV2(options) {
    const body = {
    ...
    const withId = { ...body, continuity_id: continuityId(body) };
    ...
    if (reasons.length > 0)
        throw new Error(`invalid v2 continuity envelope: ${reasons.join(',')}`);
    return candidate;
}
function shapeReasonsV2(value) {
    const reasons = [];
    ...
    if (!isObject(proof) || proof.profile !== AGENT_CONTINUITY_V2_VERSION
        ...
        if (malformed) {
            ...
            for (const alg of presented) {
                if (!AGENT_CONTINUITY_V2_REQUIRED_ALGORITHMS.includes(alg))
                    reasons.push('invalid_proof');
            }
        }
    }
...
export async function verifyAgentContinuityEnvelopeV2(value, options) {
    try {
        const checks = {
            schema: false,
            identity: false,
 ...
 */
export async function verifyAgentContinuityEnvelopeAny(value, options) {
    if (isObject(value) && value['@type'] === AGENT_CONTINUITY_V2_VERSION) {
        return verifyAgentContinuityEnvelopeV2(value, options);
    }
    return verifyAgentContinuityEnvelope(value, options);
}
