...
function parseRoots(value, config) {
    ...
    for (const candidate of value) {
        ...
        let key;
        try {
            key = crypto.createPublicKey({ key: candidate.public_jwk, format: 'jwk' });
        }
        catch {
            return null;
        }
...
function parsePins(value) {
    ...
    if (!config)
        throw new TypeError('invalid WAG constructor config');
    const roots = parseRoots(value?.trust_roots, config);
    if (!roots)
        throw new TypeError('invalid WAG constructor trust roots');
    return {
        config,
        roots,
...
function parseCompactGrant(value) {
    if (typeof value !== 'string')
        return null;
...
function audienceContains(value, candidate) {
    return (Array.isArray(value) ? value : [value]).includes(candidate);
}
function validWimseSubject(subject, expectedAuthority) {
    try {
...
function validMaterialProperty(value) {
    return nonEmptyString(value)
        || (Array.isArray(value) && value.length > 0 && value.every((entry) => nonEmptyString(entry)));
}
function parseArtifact(value) {
    if (!isRecord(value) || !exactKeys(value, ARTIFACT_KEYS)
        ...
        || !exactUrl(value.resource))
        return null;
    return structuredClone(value);
}
function projectAction(artifact, claims, config) {
    if (!nonEmptyString(claims.iss) || !nonEmptyString(claims.sub)
        || !nonEmptyString(claims.jti))
        return null;
    const properties = {};
    for (const name of config.property_claims) {
        if (!Object.hasOwn(claims, name) || !validMaterialProperty(claims[name]))
            return null;
...
function exactExpectedTokenIssuanceAction(value, config) {
    if (!isRecord(value) || !exactKeys(value, ACTION_KEYS)
        ...
        || !exactKeys(value.properties, new Set(config.property_claims)))
        return false;
...
function verifyNative(input, pins) {
    ...
    try {
        signatureValid = crypto.verify('sha256', Buffer.from(grant.signingInput, 'ascii'), { key: root.key, dsaEncoding: 'ieee-p1363' }, grant.signature);
    }
    catch {
    ...
    if (!audienceContains(claims.aud, pins.config.authorization_server_issuer)
        && !audienceContains(claims.aud, pins.config.token_endpoint)) {
        result.acceptance = 'REJECTED';
...
export function createWagActionDefinition(actionType) {
...
export function createWagAebAdapter(constructorPins) {
...
//# sourceMappingURL=aeb-wag-adapter.js.map