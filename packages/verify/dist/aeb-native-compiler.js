...
import { AEB_ADAPTER_VERSION, aebNativeReplayKeys, canonicalizeAeb, digestAeb, evaluateAebEvidence, mappingProfileDigest, } from './aeb-adapter-contract.js';
...
function tryCanonicalClone(value) {
    try {
        return { ok: true, value: JSON.parse(canonicalizeAeb(value)) };
    }
    catch {
...
export function aebNativeDescriptorDigest(descriptorId, descriptor) {
    const { descriptor_digest: _ignored, ...body } = descriptor;
    return digestAeb({ descriptor_id: descriptorId, descriptor: body });
}
function descriptorShapeValid(value) {
    if (!isRecord(value) || !exactKeys(value, DESCRIPTOR_KEYS)
        ...
        || typeof value.descriptor_digest !== 'string' || !DIGEST.test(value.descriptor_digest))
        return false;
    return true;
}
function emptyDescriptorReport(id) {
    return {
        id,
        digest: ZERO_DIGEST,
        pinned: false,
...
function validateNativeDescriptor(leg, descriptors, pins, expectedAction) {
...
function localPolicyInputValid(value) {
...
function profileIsPinned(profileId, profile, pins) {
    if (!profile)
        return false;
    let digest = null;
    try {
...
function semanticOmission(profileId, profileDigest, profilePinned, path, classification, declaration) {
    const basis = {
...
function unknownSemanticLoss(profileId, pins, declaration = 'profile_semantics_unavailable') {
    ...
    return {
        ...
        omissions: [semanticOmission(profileId, profileDigest, profilePinned, '$', 'unknown', declaration)],
    };
}
function semanticLossForProfile(profileId, pins) {
    const profile = pins.profiles?.[profileId];
    ...
    return {
        ...
        omissions: [
            ...material.map((path) => semanticOmission(profileId, profile.profile_digest, true, path, 'material', 'omitted_material_fields')),
            ...nonmaterial.map((path) => semanticOmission(profileId, profile.profile_digest, true, path, 'non_material', 'omitted_nonmaterial_fields')),
        ],
    };
}
function failureReasonsForLegs(legs, predicate) {
    return sortedUnique(legs.filter(predicate).flatMap((leg) => leg.reasons));
}
function aggregateVerified(legs) {
    if (legs.length === 0)
        return { result: 'INDETERMINATE', reasons: ['native_legs_required'] };
...
function aggregateAccepted(legs) {
    if (legs.length === 0)
        return { result: 'INDETERMINATE', reasons: ['native_legs_required'] };
...
function aggregateMatch(legs) {
    if (legs.length === 0)
        return { result: 'INDETERMINATE', reasons: ['native_legs_required'] };
    const semanticRelationUnknown = legs.some((leg) => (leg.semantic_loss.status === 'MATERIAL' || leg.semantic_loss.status === 'UNKNOWN'));
    if (semanticRelationUnknown) {
        return {
            result: 'INDETERMINATE',
            reasons: failureReasonsForLegs(legs, (leg) => (leg.semantic_loss.status === 'MATERIAL' || leg.semantic_loss.status === 'UNKNOWN')),
        };
    }
...
function buildReportCore(input) {
    ...
    const legs = evaluation.record.legs.map((leg, index) => {
        const adapterPin = input.pins?.adapters?.[leg.adapter_id];
        const profile = input.pins?.profiles?.[leg.profile_id];
        const descriptor = validateNativeDescriptor(legsInput[index], input.native_descriptors, input.pins, input.expected_action?.value);
    ...
    if (!requirementPinned)
        compilerReasons.push('requirement_not_pinned');
    if (!expectedActionValid)
        compilerReasons.push('expected_action_invalid');
    for (const { profile_id: profileId, report } of semanticLoss.profiles) {
        if (report.status === 'MATERIAL')
            compilerReasons.push(`semantic_loss_material:${profileId}`);
        if (report.status === 'UNKNOWN')
            compilerReasons.push(`semantic_loss_unknown:${profileId}`);
    }
    compilerReasons.push(...legs.flatMap((leg) => (leg.native_descriptor.pinned
    ...
    const policyInput = policyInputValid
        ? {
 ...
 */
export function compileAebNativeEvidence(input) {
    let core;
    try {
...
//# sourceMappingURL=aeb-native-compiler.js.map