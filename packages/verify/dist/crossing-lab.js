...
const STATUS_REQUIRED_KEYS = ['checked_at', 'expires_at', 'revocation_checked', 'revoked', 'consumed'];
...
function inspectJson(value, depth = 0, state = { nodes: 0 }) {
    state.nodes += 1;
    if (state.nodes > CROSSING_LAB_LIMITS.max_nodes)
        throw new TypeError('JSON node limit exceeded');
    if (depth > CROSSING_LAB_LIMITS.max_depth)
        throw new TypeError('JSON depth limit exceeded');
    ...
    if (!isObject(value) || Object.getPrototypeOf(value) !== Object.prototype)
        throw new TypeError('plain JSON object required');
    const descriptors = Object.getOwnPropertyDescriptors(value);
    if (Object.getOwnPropertySymbols(value).length > 0)
        throw new TypeError('symbol JSON member refused');
    for (const descriptor of Object.values(descriptors)) {
        if (!descriptor.enumerable || !Object.hasOwn(descriptor, 'value'))
            throw new TypeError('canonical JSON properties required');
        inspectJson(descriptor.value, depth + 1, state);
    }
}
export function canonicalizeCrossingLab(value) {
    inspectJson(value);
    return canonicalizeStrictJson(value);
}
export function digestCrossingLab(value) {
...
function assertDirectFile(root, name, maxBytes) {
    if (typeof name !== 'string' || !FILE_RE.test(name) || isAbsolute(name))
        throw new TypeError('workspace file must be one relative filename');
    ...
    if (!stat.isFile() || stat.isSymbolicLink())
        throw new TypeError(`${name} must be a regular non-symlink file`);
    if (stat.size > maxBytes)
        throw new TypeError(`${name} exceeds file-size limit`);
...
function validateWorkspace(workspace) {
    ...
    if (!isObject(workspace.adapter) || !exactKeys(workspace.adapter, ['id', 'module', 'module_digest', 'version'])
        ...
        throw new TypeError('invalid adapter metadata');
    if (!FILE_RE.test(workspace.artifact ?? '') || !DIGEST_RE.test(workspace.artifact_digest ?? ''))
        throw new TypeError('invalid artifact pin');
    if (!DIGEST_RE.test(workspace.expected_action_digest ?? ''))
        throw new TypeError('invalid expected-action pin');
    if (!DIGEST_RE.test(workspace.hostile_expected_action_digest ?? ''))
        throw new TypeError('invalid hostile-action pin');
    if (!RFC3339_RE.test(workspace.evaluated_at ?? '') || !Number.isFinite(Date.parse(workspace.evaluated_at)))
        throw new TypeError('invalid evaluated_at');
    if (!isObject(workspace.config) || workspace.config['@version'] !== 'AEB-ADAPTER-v1')
        throw new TypeError('real AEB-ADAPTER-v1 config required');
    if (!isObject(workspace.evaluation) || !exactKeys(workspace.evaluation, [
        ...
        || !isObject(workspace.evaluation.status) || !DIGEST_RE.test(workspace.evaluation.status_digest ?? ''))
        throw new TypeError('invalid AEB evaluation input');
    ...
    if (declared.size !== CROSSING_LAB_LIMITS.required_file_count)
        throw new TypeError('workspace must declare exactly three distinct required files');
    inspectJson(workspace);
}
function workspacePinErrors(workspace, artifact, adapterBytes) {
    const reasons = [];
    if (sha256Bytes(adapterBytes) !== workspace.adapter.module_digest)
        reasons.push('adapter_module_pin_drift');
    if (digestAeb(artifact) !== workspace.artifact_digest)
        reasons.push('artifact_pin_drift');
    if (digestAeb(workspace.expected_action) !== workspace.expected_action_digest)
        reasons.push('expected_action_pin_drift');
    if (digestAeb(workspace.hostile_expected_action) !== workspace.hostile_expected_action_digest)
        reasons.push('hostile_action_pin_drift');
    if (workspace.hostile_expected_action_digest === workspace.expected_action_digest)
    ...
    for (const [id, pin] of Object.entries(workspace.config?.adapters ?? {})) {
        try {
            if (!isObject(pin) || adapterPinDigest(id, pin) !== pin.config_digest)
                reasons.push(`adapter_config_pin_drift:${id}`);
        }
        catch {
            reasons.push(`adapter_config_pin_drift:${id}`);
        }
    ...
    for (const [id, profile] of Object.entries(workspace.config?.profiles ?? {})) {
        try {
            if (!isObject(profile) || mappingProfileDigest(id, profile) !== profile.profile_digest)
                reasons.push(`mapping_profile_pin_drift:${id}`);
        }
        catch {
            reasons.push(`mapping_profile_pin_drift:${id}`);
        }
    ...
    try {
        if (!isObject(registry) || unifiedRegistryDigest(registry) !== registry.registry_digest)
...
function invokeAdapter(adapterBytes, adapterDigest, method, input) {
    ...
    if (child.error)
        throw new Error(`adapter worker failed: ${child.error.message}`);
    if (child.status !== 0)
        throw new Error(`adapter worker refused: ${(child.stderr || '').trim() || `exit ${child.status}`}`);
...
function closeAdapterResult(method, value) {
    if (!isObject(value))
        throw new TypeError(`${method} returned a non-object`);
    ...
    if (unknown.length > 0)
        throw new TypeError(`${method} returned unknown members: ${unknown.sort().join(',')}`);
    return value;
}
function proxyAdapter(workspace, adapterBytes, transform) {
    ...
    return {
        ...
        verifyNative(input) { return invoke('verifyNative', input); },
        mapAction(input) { return invoke('mapAction', input); },
    };
}
function configWithTrustRoots(config, adapterId, trustRoots) {
    const clone = structuredClone(config);
    const current = clone.adapters[adapterId];
    const pin = { ...current, trust_roots: trustRoots, config_digest: current.config_digest };
    pin.config_digest = adapterPinDigest(adapterId, pin);
    clone.adapters[adapterId] = pin;
    return clone;
}
function evaluate(workspace, artifact, adapter, overrides = {}) {
...
function adapterRow(id, category, rule, result) {
...
function harnessSelfTests(workspace, artifact, adapterBytes) {
...
export function runCrossingLab(workspaceDirectory) {
    ...
    if (pinErrors.length > 0)
        throw new TypeError(`workspace pin verification failed: ${pinErrors.join(',')}`);
    ...
    const replayRow = adapterRow('replay-identity-is-wrapper-independent', 'hostile', 'REWRAPPED_REPLAY_IDENTITY_STABLE', rewrapped);
    ...
    const body = {
        ...
        assurance: {
            ...
            evaluator_key_id: LAB_EVALUATOR_KEY_ID,
...
const SAMPLE_CAID_REFERENCE_SOURCE = readFileSync(fileURLToPath(new URL('../vendor/caid.mjs', import.meta.url)), 'utf8');
...
export function initCrossingLab(targetDirectory) {
...
function recomputeConfigPins(config) {
    for (const [id, pin] of Object.entries(config.adapters))
        pin.config_digest = adapterPinDigest(id, pin);
    for (const [id, profile] of Object.entries(config.profiles))
        profile.profile_digest = mappingProfileDigest(id, profile);
 ...
 */
export function sealCrossingLab(workspaceDirectory) {
    const rootStat = lstatSync(workspaceDirectory);
    if (!rootStat.isDirectory() || rootStat.isSymbolicLink())
        throw new TypeError('workspace must be a non-symlink directory');
    const root = realpathSync(workspaceDirectory);
...
export function writeCrossingLabReport(path, report) {
    writeFileSync(path, `${JSON.stringify(report, null, 2)}\n`, { encoding: 'utf8', mode: 0o600, flag: 'wx' });
}
//# sourceMappingURL=crossing-lab.js.map