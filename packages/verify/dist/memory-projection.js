...
export const MEMORY_PROJECTION_PQ_REQUIRED_ALGORITHMS = Object.freeze(['Ed25519', 'ML-DSA-65']);
...
function pqCosignatureSetMatchesRegistered(algorithms) {
 ...
 */
export function memoryProjectionPqCosignatureSigningBytes(body, requiredAlgorithms = MEMORY_PROJECTION_PQ_REQUIRED_ALGORITHMS) {
 ...
 */
export function memoryProjectionPqCosignatureBody(record) {
    validateRecordShape(record);
    const value = record;
    return {
        ...
        record_proof_signature_b64u: String(value.proof.signature_b64u),
    };
}
/**
 ...
 */
export async function signMemoryProjectionPqCosignature(record, signer, options = {}) {
    ...
    const signatures = await signAgileSet(new Uint8Array(memoryProjectionPqCosignatureSigningBytes(body)), [
 ...
 */
export async function verifyMemoryProjectionPqCosignature(record, cosignature, pin, options = {}) {
    const checks = {
        structure: true,
        version: true,
        algorithm_set: true,
    ...
    const errors = [];
    const fail_ = (key, msg) => { checks[key] = false; errors.push(msg); };
    ...
    if (!pqCosignatureSetMatchesRegistered(proof.required_algorithms)) {
        fail_('algorithm_set', `proof.required_algorithms must be exactly ${JSON.stringify([...MEMORY_PROJECTION_PQ_REQUIRED_ALGORITHMS])} (set narrowing / widening refused)`);
    }
        ...
        if (!malformed) {
            for (const alg of MEMORY_PROJECTION_PQ_REQUIRED_ALGORITHMS) {
                if (!presented.has(alg))
                    fail_('legs_present', `missing required ${alg} signature (leg stripped)`);
            }
            for (const alg of presented) {
                if (!MEMORY_PROJECTION_PQ_REQUIRED_ALGORITHMS.includes(alg)) {
    ...
    if (!pinnedEd || !pinnedPq || typeof pin?.key_id !== 'string' || typeof pin?.pq_key_id !== 'string') {
        fail_('cosigner_key_pinned', 'a pinned Ed25519 + ML-DSA-65 co-signer key pair is required (identified but not trusted)');
    }
    else {
        let edOk = false;
        try {
        ...
        if (!edOk)
            fail_('cosigner_key_pinned', 'pinned Ed25519 co-signer key is not a canonical Ed25519 SPKI');
    ...
    let setResult;
    try {
        setResult = await verifyAgileSignatureSet(new Uint8Array(memoryProjectionPqCosignatureSigningBytes(expectedBody, MEMORY_PROJECTION_PQ_REQUIRED_ALGORITHMS)), signatures ?? [], [
    ...
    if (setResult?.verified !== true) {
        fail_('signature_valid', `co-signature set does not verify under the pinned Ed25519 + ML-DSA-65 keys (${String(setResult?.reason ?? 'signature_set_unverified')})`);
    }
    return done();
}
