...
function canonicalJsonIssue(value) {
    ...
    function visit(current, depth) {
        ...
        if (state.nodes > MAX_NODES)
            return 'canonical_value_too_large';
        if (depth > MAX_DEPTH)
            return 'canonical_value_too_deep';
        if (current === null || typeof current === 'boolean')
            return null;
        if (typeof current === 'string')
            return canonicalString(current) ? null : 'noncanonical_string';
        ...
        for (const key of Object.keys(descriptors)) {
            ...
            if (!canonicalString(key))
                return 'noncanonical_member_name';
            const issue = visit(descriptor.value, depth + 1);
            if (issue)
                return issue;
        }
...
function strictBase64url(value, expectedLength) {
    if (typeof value !== 'string' || value.length === 0
        || !BASE64URL.test(value) || value.length % 4 === 1)
        return null;
...
function canonicalCopy(value) {
    ...
    if (issue)
        throw new TypeError(`value is outside the DAB canonical JSON profile: ${issue}`);
...
function validateTerm(term) {
    ...
    if (term.type === 'decimal')
        return typeof term.value === 'string' && DECIMAL.test(term.value);
    if (term.type === 'integer')
        return Number.isSafeInteger(term.value);
    if (term.type === 'boolean')
        return typeof term.value === 'boolean';
    if (term.type === 'date')
        return strictDate(term.value);
    if (term.type === 'timestamp')
        return Number.isFinite(strictInstant(term.value));
    if (term.type === 'digest')
        return typeof term.value === 'string' && SHA256.test(term.value);
    if (term.type === 'identifier')
        return typeof term.value === 'string' && IDENTIFIER.test(term.value);
...
function validateMaterialTerms(terms) {
    ...
    if (!terms.every(validateTerm))
        return { ok: false, reason: 'invalid_material_terms' };
    const ids = terms.map((term) => term.term_id);
    if (new Set(ids).size !== ids.length)
        return { ok: false, reason: 'duplicate_material_term' };
...
function amountFieldsValid(value) {
    if (Array.isArray(value))
        return value.every(amountFieldsValid);
    if (!isRecord(value))
        return true;
    for (const [key, child] of Object.entries(value)) {
        if (MINOR_AMOUNT_FIELD.test(key))
            return false;
        if (AMOUNT_FIELD.test(key)
            && (typeof child !== 'string' || !AMOUNT.test(child)))
            return false;
        if (!amountFieldsValid(child))
            return false;
    }
...
function validatePartyArray(parties, label) {
    ...
    if (new Set(ids).size !== ids.length)
        return { ok: false, reason: 'duplicate_party_id' };
...
function validateValidity(validity, now) {
    ...
    if (!Number.isFinite(now))
        return { ok: false, reason: 'invalid_evaluation_time' };
    if (now < notBefore)
        return { ok: false, reason: 'binding_not_yet_valid' };
    if (now > notAfter)
...
function validateCore(core, now) {
    if (!exactKeys(core, CORE_KEYS, CORE_REQUIRED))
...
export function computeDocumentActionBindingDigest(binding) {
    try {
        ...
        if (core === null || canonicalJsonIssue(core) !== null)
            return null;
...
export function signDocumentActionBinding(spec, signer) {
    if (!isRecord(spec) || !isRecord(signer))
        throw new TypeError('DAB spec and signer are required');
    const documentBytes = asBytes(spec.document?.bytes);
    if (documentBytes === null)
        throw new TypeError('spec.document.bytes must be a Uint8Array or ArrayBuffer');
    ...
    if (spec.supersedes_digest !== undefined)
        core.supersedes_digest = spec.supersedes_digest;
    const issue = canonicalJsonIssue(core);
    if (issue)
        throw new TypeError(`DAB is outside the canonical JSON profile: ${issue}`);
    const validated = validateCore(core, strictInstant(core.validity.not_before));
    if (!validated.ok)
        throw new TypeError(`invalid DAB spec: ${validated.reason}`);
...
export function verifyDocumentActionBinding(binding, opts = {}) {
    ...
    try {
        ...
        if (issue)
            return { ...result, reason: 'noncanonical_binding' };
        ...
        if (!coreCheck.ok)
            return { ...result, reason: coreCheck.reason };
        ...
        if (opts.requiredMaterialTermIds !== undefined) {
            ...
            if (requiredTermIds === null)
                return { ...result, reason: 'invalid_required_material_terms' };
        ...
        if (opts.documentBytes !== undefined) {
            ...
            if (expectedBytes === null)
                return { ...result, reason: 'invalid_expected_document' };
        ...
        if (opts.releaseActionTemplate !== undefined) {
            ...
            if (expectedActionDigest === null)
                return { ...result, reason: 'invalid_expected_action' };
        ...
        if (opts.expectedRequiredParties !== undefined) {
            ...
            if (!expectedCheck.ok)
                return { ...result, reason: 'invalid_expected_required_parties' };
        ...
        if (Object.hasOwn(opts, 'expectedSupersedesDigest')) {
            ...
            if (expected !== actual)
                return { ...result, reason: 'supersedes_digest_mismatch' };
        }
        return {
            valid: true,
            reason: 'valid',
...
export const DOCUMENT_ACTION_BINDING_V2_REQUIRED_ALGORITHMS = Object.freeze(['Ed25519', 'ML-DSA-65']);
...
function algorithmSetMatchesRegisteredDAB(algorithms) {
...
function validateCoreV2(core, now) {
    if (!exactKeys(core, CORE_KEYS_V2, CORE_REQUIRED_V2))
        return { ok: false, reason: 'malformed_binding' };
 ...
 */
export async function signDocumentActionBindingV2(spec, signer) {
    if (!isRecord(spec) || !isRecord(signer))
        throw new TypeError('DAB spec and signer are required');
    const documentBytes = asBytes(spec.document?.bytes);
    if (documentBytes === null)
 ...
 */
export async function verifyDocumentActionBindingV2(binding, opts = {}) {
    const result = baseResult();
    try {
        ...
        const signatures = binding.issuer_signatures;
        if (!Array.isArray(signatures) || signatures.length !== 2) {
            return { ...result, reason: 'issuer_signature_missing' };
        }
        const presentedAlgs = new Set();
        for (const s of signatures) {
            if (!s || typeof s !== 'object' || Array.isArray(s) || typeof s.alg !== 'string' || typeof s.sig !== 'string') {
                return { ...result, reason: 'malformed_issuer_signature' };
            }
            if (presentedAlgs.has(s.alg))
                return { ...result, reason: 'duplicate_issuer_signature_algorithm' };
            presentedAlgs.add(s.alg);
        }
        for (const alg of DOCUMENT_ACTION_BINDING_V2_REQUIRED_ALGORITHMS) {
            if (!presentedAlgs.has(alg))
                return { ...result, reason: `missing_required_${alg}_signature` };
        }
        const issuerKeys = isRecord(opts.issuerKeys) ? opts.issuerKeys : null;
        const keyId = core.mapping_issuer.key_id;
        const pin = issuerKeys && Object.hasOwn(issuerKeys, keyId) ? issuerKeys[keyId] : null;
 ...
 */
export async function verifyDocumentActionBindingStatement(binding, opts = {}) {
...
//# sourceMappingURL=document-action-binding.js.map