 ...
 */
export function normalizeSurfaceDigest(h) {
...
export function validateSurfaceBinding(binding) {
    ...
    if (b.verifier_hint !== undefined)
        normalized.verifier_hint = b.verifier_hint;
 ...
 */
export function bindSurfaceInto(action, binding) {
    ...
    if (existing !== undefined && canonicalize(existing) !== canonicalize(v.normalized)) {
        throw new Error(`bindSurfaceInto: action already carries a different ${SURFACE_BINDING_FIELD}; refusing to overwrite`);
    }
...
export function receiptSurfaceBinding(receipt) {
    ...
    if (raw === undefined)
        return { strength: 'none', binding: null, errors: [] };
    const v = validateSurfaceBinding(raw);
    if (!v.ok)
        return { strength: 'none', binding: null, errors: v.errors };
 ...
 */
export function verifySurfaceBinding(receipt, evidence, opts) {
...
//# sourceMappingURL=surface-binding.js.map