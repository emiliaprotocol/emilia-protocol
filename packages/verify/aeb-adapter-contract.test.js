...
// Generated from aeb-adapter-contract.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
...
test('signed native bridge composes WIMSE possession and human authorization', () => {
    ...
    s.config.registry.entries['role:workload-possession'] = registryEntry('role:workload-possession', 'evidence-role', '1', { role: 'workload-possession', subject_kinds: ['workload'] });
...
test('AEB freezes indeterminate execution and consumes a satisfied authorization once', () => {
    ...
    assert.equal(authorized.program_digest, satisfied.record.evaluator.pinned_config_digest);
...
test('AEB execution conditions fail closed before authority is reserved', () => {
    ...
    const base = {
        ...
        resolver_profile_digest: `sha256:${'b'.repeat(64)}`,
        reasons: [],
    };
...
test('T6 CX2: a reserve write that lands after the durable read is never reported as a clean refusal', async () => {
    ...
    const store = {
        ...
        async commit() { return false; },
        async release() { return false; },
        state: (key) => states.get(key) ?? 'AVAILABLE',
    };
    ...
    assert.equal(states.get(decision.reservation_key ?? aebReservationKey(result.record)), undefined, 'the durable read ran before the write landed');
    landLater();
    assert.deepEqual([decision.state, decision.reason, decision.invoke_allowed], ['RECONCILIATION_REQUIRED', 'consumption_reservation_unconfirmed', false]);
    assert.equal(states.get(aebReservationKey(result.record)), 'RESERVED', 'the evaluation is RESERVED after the call returned, so the answer could not have been a clean refusal');
