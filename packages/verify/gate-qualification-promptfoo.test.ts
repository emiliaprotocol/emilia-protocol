...
import {
  PROMPTFOO_QUALIFICATION_ADAPTER_VERSION,
  PROMPTFOO_QUALIFICATION_EVIDENCE_VERSION,
  PROMPTFOO_QUALIFICATION_LIMITS,
  PROMPTFOO_QUALIFICATION_RUN_METADATA_VERSION,
...
function immutableRef(kind: string, id: string, digest: string): string {
  return `${kind}:${id}@${digest}`;
}

function attemptRow(
  ordinal: number,
  status: 'PASS' | 'FAIL' | 'ERROR' | 'ABORTED' | 'EXPIRED',
) {
...
function repinArtifact(fixture: ReturnType<typeof makeFixture>) {
...
function repinCampaign(fixture: ReturnType<typeof makeFixture>): void {
  ...
  fixture.pins.challenge_campaign.immutable_ref = immutableRef(
    'campaign',
    fixture.pins.challenge_campaign.id,
    digest,
  );
  fixture.artifact.metadata.emilia_gate_qualification_v2.challenge_campaign.digest = digest;
}

function refusal(fixture: ReturnType<typeof makeFixture>, reason: string) {
  ...
  if (!result.ok) assert.ok(result.reasons.includes(reason), JSON.stringify(result.reasons));
}
...
test('converts a complete pinned Promptfoo run into evaluation-only Qualification v2 evidence', () => {
  ...
  assert.throws(() => {
    (result.evidence as any).authority.authorizes = true;
  }, TypeError);
});
...
test('rejects mutable aliases and unpinned versions', () => {
  {
    ...
    for (const row of fixture.artifact.results.results) row.evaluationId = 'latest';
    repinArtifact(fixture);
...
test('rejects unsupported fields at every interpreted boundary', () => {
  for (const mutate of [
    (fixture: ReturnType<typeof makeFixture>) => { fixture.artifact.unexpected = true; },
    (fixture: ReturnType<typeof makeFixture>) => { fixture.artifact.results.unexpected = true; },
    (fixture: ReturnType<typeof makeFixture>) => { fixture.artifact.results.results[0].unexpected = true; },
...
test('requires bounded costs, latency, quality metrics, and complete payload digests', () => {
  for (const [mutate, reason] of [
    [
      (fixture: ReturnType<typeof makeFixture>) => { delete fixture.artifact.results.results[0].cost; },
      'attempt_cost_invalid:attempt-1',
    ],
    [
      (fixture: ReturnType<typeof makeFixture>) => { fixture.artifact.results.results[0].latencyMs = -1; },
      'attempt_latency_invalid:attempt-1',
    ],
    [
      (fixture: ReturnType<typeof makeFixture>) => { fixture.artifact.results.results[0].score = 2; },
...
test('returns closed refusals instead of throwing on malformed untrusted input', () => {
  ...
  fixture.pins.candidate.immutable_ref = immutableRef(
    'candidate',
    fixture.pins.candidate.id,
    fixture.pins.candidate.manifest_digest,
  );
  assert.doesNotThrow(() => {
...
test('fails closed on aggregate Promptfoo input byte, node, depth, and result-count limits', () => {
  {
    const fixture = makeFixture();
    fixture.artifact.config.description = 'x'.repeat(
      PROMPTFOO_QUALIFICATION_LIMITS.max_input_bytes,
    );
  ...
  {
    const fixture = makeFixture();
    fixture.pins.verifier.trust_config.aggregate = Array.from(
      { length: PROMPTFOO_QUALIFICATION_LIMITS.max_input_nodes },
      () => null,
    );
    assert.deepEqual(adaptPromptfooQualificationArtifact({
      artifact: fixture.artifact,
...

  {
    ...
    for (let depth = 0;
      depth <= PROMPTFOO_QUALIFICATION_LIMITS.max_input_depth;
      depth += 1) {
    ...
    assert.throws(
      () => digestPromptfooQualification(deeplyNested),
      { name: 'TypeError', message: 'input_depth_limit_exceeded' },
    );
  }

  {
    const fixture = makeFixture();
    fixture.artifact.results.results = new Array(
      PROMPTFOO_QUALIFICATION_LIMITS.max_summary_results + 1,
    );
    assert.deepEqual(adaptPromptfooQualificationArtifact({
      artifact: fixture.artifact,
      pins: fixture.pins,
