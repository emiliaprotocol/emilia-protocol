...
const VECTORS_PATH = path.join(
  ...
  '../../conformance/selective-disclosure/vectors.json',
);
const vectors = JSON.parse(fs.readFileSync(VECTORS_PATH, 'utf8'));

function privateKeyFrom(pkcs8B64u: string): crypto.KeyObject {
  ...
  it('re-derives the disclosure-ready receipt and every presentation from fixed seeds', () => {
    ...
    for (const entry of vectors.presentations) {
      ...
      const rebuilt = createSelectiveDisclosurePresentation(
        receipt,
        prep.openings,
        entry.presentation.disclosed.map((d: { path: string }) => d.path),
        entry.presentation.binding,
...
describe('construction invariants', () => {
  ...
  it('refuses overlapping designations', () => {
    const res = prepareSelectiveDisclosure(
      vectors.source_payload,
...
describe('fail-closed behavior (refusal, never a crash)', () => {
  ...
  it('refuses a receipt without a plaintext CAID', () => {
    const prep = prepareSelectiveDisclosure(
      { receipt_id: 'r2', created_at: '2026-08-16T12:00:00Z', field: 'v' },
      ['field'],
