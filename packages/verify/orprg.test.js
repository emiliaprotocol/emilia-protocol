...
test('the action profile is closed and refuses unsafe or ambiguous JSON', () => {
    ...
    const symbolBearingAction = { ...ACTION };
    Object.defineProperty(symbolBearingAction, Symbol.for('hidden_override'), {
        value: { destination_account: 'acct_attacker' },
        enumerable: true,
    });
    assert.equal(computeOrprgActionDigest(symbolBearingAction), null, 'an action with execution-visible data outside the signed JSON domain must be refused');
