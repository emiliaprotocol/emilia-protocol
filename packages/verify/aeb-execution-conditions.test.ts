...
import {
  AEB_EXECUTION_CONDITION_RESOLUTION_VERSION,
  AEB_EXECUTION_CONDITIONS_VERSION,
  AEB_EXECUTION_RESOLVER_PROFILE_VERSION,
  defineAebExecutionConditionsProfile,
  defineAebExecutionResolverProfile,
  evaluateAebExecutionConditions,
  type AebExecutionConditionResolution,
  type AebExecutionConditionsProfile,
  type AebExecutionEnforcementStrength,
  type AebExecutionResolverProfile,
} from './aeb-execution-conditions.js';
...
function sourceId(strength: AebExecutionEnforcementStrength): string {
  return `resolver:${strength}`;
}

function trustDigest(strength: AebExecutionEnforcementStrength): AebDigest {
  return digestAeb({ trust: sourceId(strength), key: '2027-q4' });
}

function buildProfiles(
  strengths: readonly AebExecutionEnforcementStrength[] = ['observed'],
): { resolver: AebExecutionResolverProfile; profile: AebExecutionConditionsProfile } {
...
function resolution(
  resolver: AebExecutionResolverProfile,
  strength: AebExecutionEnforcementStrength = 'observed',
  overrides: Partial<AebExecutionConditionResolution> = {},
): AebExecutionConditionResolution {
...
function evaluate(
  ...
  overrides: Partial<Parameters<typeof evaluateAebExecutionConditions>[1]> = {},
) {
...
test('profiles are RP-owned, content-addressed, canonical, and immutable', () => {
  ...
  assert.deepEqual(
    resolver.sources.map((source) => source.source_id),
    ['resolver:compare-and-set', 'resolver:provider-enforced'],
  );
...
test('observed and leased evidence may admit but may never claim prevention', () => {
  for (const strength of ['observed', 'leased'] as const) {
...
test('valid compare-and-set and provider-enforced paths can support a prevention claim', () => {
  for (const strength of ['compare-and-set', 'provider-enforced'] as const) {
    const { resolver, profile } = buildProfiles([strength]);
