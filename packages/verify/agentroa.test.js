...
// Generated from agentroa.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
...
function sign(body, signer, keyPair) {
    const sig = crypto.sign(null, Buffer.from(canonicalize(body), 'utf8'), keyPair.privateKey).toString('base64url');
...
function resignAER(bundle, mutate, recomputeChainDigest = true) {
    ...
    delete body.signatures;
    mutate(body);
    if (recomputeChainDigest)
        body.chain_summary.chain_digest = digest(bundle.chain);
...
test('verifies every signature, parent digest, and narrowing step in a two-hop chain', () => {
    ...
    duplicate.chain[2] = sign(duplicateBody, duplicateBody.delegating_agent.agent_id, delegateKey);
    resignAER(duplicate, () => { });
...
test('enforces ARA parent digests, identity continuity, policy continuity, and depth', () => {
    for (const [name, mutate, reason, mutateContext = () => { }] of [
        ...
        ['delegator', (bundle) => resignARA(bundle, (ara) => { ara.delegating_agent.agent_id = 'aha:acme/ops/other'; }, 'aha:acme/ops/other'), 'ara_delegator_mismatch', (context) => {
