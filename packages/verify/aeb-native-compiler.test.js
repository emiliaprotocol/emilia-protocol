...
// Generated from aeb-native-compiler.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
import assert from 'node:assert/strict';
import test from 'node:test';
import { AEB_NATIVE_DESCRIPTOR_VERSION, AEB_NATIVE_COMPILER_VERSION, aebNativeDescriptorDigest, compileAebNativeEvidence, } from './aeb-native-compiler.js';
import { adapterPinDigest, digestAeb, mappingProfileDigest, registryEntryDigest, unifiedRegistryDigest, } from './aeb-adapter-contract.js';
...
function nativeAdapter(id, version) {
    return Object.freeze({
        ...
        verifyNative({ artifact, status: currentStatus, trust_roots }) {
            ...
            return {
                ...
                status_digest: statusDigest(currentStatus),
...
function profile(id, mapperId, omittedNonmaterialFields = []) {
    const value = {
...
function refreshProfile(config, id) {
    ...
    config.registry.entries[entryId] = registryEntry(entryId, 'mapping-profile', { profile_digest: pinned.profile_digest });
    config.registry.registry_digest = unifiedRegistryDigest(config.registry);
}
function nativeDescriptor(id, adapter, profileId, pinnedProfile, protocolId, protocolRevision) {
    const value = {
...
function refreshDescriptor(input, id) {
...
function fixture() {
    ...
    const delegationDescriptor = nativeDescriptor(delegationDescriptorId, delegationAdapter, 'profile:delegation', delegationProfile, 'example-delegation', 'draft-07');
    const approvalDescriptor = nativeDescriptor(approvalDescriptorId, approvalAdapter, 'profile:approval', approvalProfile, 'example-approval', 'spec-2.1');
    const entries = {
        [delegationProfile.registry_entry_ref]: registryEntry(delegationProfile.registry_entry_ref, 'mapping-profile', { profile_digest: delegationProfile.profile_digest }),
        [approvalProfile.registry_entry_ref]: registryEntry(approvalProfile.registry_entry_ref, 'mapping-profile', { profile_digest: approvalProfile.profile_digest }),
        'role:delegated-authority': registryEntry('role:delegated-authority', 'evidence-role', { role: 'delegated-authority', subject_kinds: ['workload'] }),
...
test('compiles two native legs without changing native wire artifacts', () => {
    ...
    assert.match(report.legs[0].semantic_loss.omissions[0].basis.binding_digest, /^sha256:[0-9a-f]{64}$/);
...
test('omitted material field fails closed', () => {
    ...
    const approvalArtifact = input.native_legs[1].artifact;
...
test('native replay unit is stable across AEB wrapper references', () => {
    ...
    assert.deepEqual(firstReport.legs.map((leg) => leg.replay_unit), secondReport.legs.map((leg) => leg.replay_unit));
});
test('missing native descriptor fails closed before acceptance or matching', () => {
    ...
    assert.equal(report.legs[0].semantic_loss.omissions[0].basis.declaration, 'native_profile_binding_unestablished');
    assert.equal(report.legs[1].action.mapping, 'MISMATCH');
    assert.equal(report.axes.match.result, 'INDETERMINATE');
    assert.equal(report.axes.satisfied.result, 'INDETERMINATE');
    assert.equal(report.axes.local_authorization.result, 'NOT_EVALUATED');
...
test('relying-party expected action mismatch refuses the match axis', () => {
    const input = fixture();
    const artifact = input.native_legs[0].artifact;
