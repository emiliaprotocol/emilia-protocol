...
const digest = (value: unknown) => `sha256:${crypto.createHash('sha256').update(canonicalizeStrictJson(value)).digest('hex')}`;
const ACTION = digest(action);
const pair = crypto.generateKeyPairSync('ed25519');
const publicKey = pair.publicKey.export({ type: 'spki', format: 'pem' }).toString();
const requirement = (extra: Record<string, any> = {}) => ({
...
function signed(subject: string, extra: Record<string, any> = {}) {
  const payload = {
...
function verify(evidence: any, context: any) {
  const key = context.trust_snapshot.public_key;
  if (!crypto.verify(null, Buffer.from(canonicalizeStrictJson(evidence.payload)), key, Buffer.from(evidence.signature, 'base64url'))) return { valid: false, reason: 'signature_invalid' };
  if (evidence.payload.issuer !== 'issuer:one' || evidence.payload.audience !== 'executor:one') return { valid: false, reason: 'native_scope_invalid' };
  return { valid: true, ...evidence.payload };
}
function registration(override: Record<string, any> = {}) {
...
function evaluator(req = requirement(), overrides: Record<string, any> = {}) {
  assert.equal(typeof (aec as any).createAuthorizationChainEvaluator, 'function', 'current AEC profile API must exist');
  return (aec as any).createAuthorizationChainEvaluator({
    requirement: req,
...
test('AEC05: structured requirement enforces real signatures, native subjects, bindings and emits replay', async () => {
  ...
  assert.deepEqual(result.replay.facts.map((f: any) => f.component_index), [0, 1, 2]);
...
for (const [name, mutate] of [
  ['a repeated native person despite different keys/labels', (c: any) => { c.components[1].evidence = signed('person:one'); c.components[1].label = 'person:two'; }],
  ['an unsigned claimed subject', (c: any) => { c.components[1].evidence.payload.subject_ids = ['person:three']; }],
  ['an unbacked presenter relation', (c: any) => { c.components[2].evidence = signed('policy:one'); c.components[2].label = 'permits approval'; }],
  ['a signed relation to the wrong bytes', (c: any) => { c.components[2].evidence = signed('policy:one', { bindings: [{ relation: 'permits', target_evidence_digest: `sha256:${'f'.repeat(64)}` }] }); }],
  ['an ineligible relation target', (c: any) => { c.components[0].evidence = signed('person:one', { action_digest: `sha256:${'f'.repeat(64)}` }); }],
  ['a false presented evidence digest', (c: any) => { c.components[1].evidence_digest = `sha256:${'f'.repeat(64)}`; }],
  ['future issuance', (c: any) => { c.components[1].evidence = signed('person:two', { issued_at: '2026-09-06T12:01:00Z' }); }],
  ['expired evidence', (c: any) => { c.components[1].evidence = signed('person:two', { expires_at: '2026-09-06T11:59:59Z' }); }],
  ['missing protected time', (c: any) => { c.components[1].evidence = signed('person:two', { issued_at: null }); }],
  ['too-old evidence', (c: any) => { c.components[1].evidence = signed('person:two', { issued_at: '2026-09-06T11:50:00Z' }); }],
  ['wrong native format revision', (c: any) => { c.components[1].evidence = signed('person:two', { format_revision: 'example-signed-v2' }); }],
] as const) {
  test(`AEC05 refuses ${name}`, async () => {
    const chain = fixture(); mutate(chain);
    const result = await evaluator().evaluate(chain, inputs());
...
test('AEC05 pins configuration and refuses transaction-scoped trust or requirement overrides', async () => {
  const req = requirement(); const registrations = { approval: registration(), permit: registration() };
  const ev = evaluator(req, { nativeVerifiers: registrations });
  req.expression = 'permit'; req.role_constraints = []; req.required_bindings = [];
  registrations.approval.verify = () => ({ valid: true } as any);
  const chain = fixture(); chain.components[1].evidence = signed('person:one');
...
test('AEC05 status requires native authentication, active disposition and current bound snapshot', async () => {
  ...
  for (const status of [good, { ...good, authenticated: false }, { ...good, status: 'revoked' }, { ...good, status: 'unknown' }, { ...good, checked_at: '2026-09-06T11:50:00Z' }, { ...good, snapshot_digest: 'bogus' }, { ...good, expires_at: '2026-09-06T11:59:59Z' }]) {
    const chain = fixture(); chain.components[2].evidence = signed('policy:one', { status });
    ...
    if (status === good) assert.equal(result.replay.facts[2].status.snapshot_digest, digest(snapshot));
  }
});
...
test('AEC05 uses only pinned mapping after native verification and checks expected CAID', async () => {
  ...
  let maps = 0;
  const reg = registration({ mapping: { profile: { id: 'mapping:one', revision: '1' }, map: (native: any) => { maps++; return { verdict: native.native_payload.destination === 'merchant:one' ? 'EQUIVALENT_UNDER_PROFILE' : 'NOT_EQUIVALENT', caid }; } } });
...
test('AEC05 produces stable digests and replay re-verifies rather than trusting recorded facts', async () => {
  const ev = evaluator(); const chain = fixture();
  ...
  const forged = structuredClone(a.replay); forged.facts[0].subject_ids = ['invented'];
...
test('AEC05 parses raw JSON strictly and refuses hostile objects and resource overflow', async () => {
  ...
  for (const value of [NaN, 1.25, '\ud800', () => true, undefined]) {
    const chain: any = fixture(); chain.action = { ...action, extra: value };
    assert.equal((await ev.evaluate(chain, inputs())).satisfied, false);
  }
  const accessor: any = fixture(); Object.defineProperty(accessor, 'action', { get() { throw new Error('hostile'); }, enumerable: true });
  assert.equal((await ev.evaluate(accessor, inputs())).satisfied, false);
  const cycle: any = fixture(); cycle.components.push(cycle);
...
test('AEC05 requires explicit boundary action and time and keeps unknown types ineligible', async () => {
  const ev = evaluator();
  for (const input of [{ verificationTime: NOW }, { expectedAction: action }, { ...inputs(), verificationTime: '2026-02-30T12:00:00Z' }, { ...inputs(), expectedAction: { ...action, amount: 101 } }]) assert.equal((await ev.evaluate(fixture(), input)).satisfied, false);
  const chain = fixture(); chain.components[1].type = 'unknown';
  assert.equal((await ev.evaluate(chain, inputs())).satisfied, false);
});
...
test('AEC05 a logged but unsigned receipt context cannot supply another human subject', async () => {
  ...
  const v = structuredClone(corpus.vectors.find((c: any) => c.id === 'accept_pinned_human_receipt'));
  ...
  const checkpoint: any = { tree_size: 1, root_hash: leaf, log_key_id: 'review-test-log', merkle_alg: 'EP-MERKLE-v2' };
  ...
  const ev = (aec as any).createAuthorizationChainEvaluator({
...
test('AEC06 additive Bundle role verifies real native evidence without becoming a terminal receipt', async () => {
  ...
  const v = corpus.cases.find((c: any) => c.id === 'valid-two-of-three-oauth-bound-bundle');
  ...
  const ev = (aec as any).createAuthorizationChainEvaluator(config);
  ...
  assert.equal(result.satisfied, true, JSON.stringify(result));
  const terminal = (aec as any).createAuthorizationChainEvaluator({ ...config, requirement: { ...config.requirement, expression: 'ep-receipt' } });
