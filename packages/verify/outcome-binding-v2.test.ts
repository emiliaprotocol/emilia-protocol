...
import {
  OUTCOME_V2_REQUIRED_ALGORITHMS,
  buildOutcomeAttestation,
  buildOutcomeAttestationV2,
  verifyOutcomeAttestation,
  verifyOutcomeAttestationV2,
...
test('the v1 attestation verifier refuses a v2 attestation cleanly', async () => {
  const attestation = await buildAttestationV2();
  const res = verifyOutcomeAttestation(attestation as any, ATT_OPTS as any);
...
test('the v2 attestation verifier refuses a v1 attestation', () => {
  ...
  return verifyOutcomeAttestationV2(v1 as any, ATT_OPTS as any).then((res) => {
    assert.equal(res.valid, false);
  });
});

test('ATTESTATION LEG STRIPPING: removing the ML-DSA leg refuses', async () => {
  const attestation: any = await buildAttestationV2();
...
test('ATTESTATION WRONG-LENGTH SIGNATURE refuses', async () => {
  const attestation: any = await buildAttestationV2();
  const edSig = attestation.proof.signatures.find((s: any) => s.alg === 'Ed25519');
...
test('the v1 observation verifier refuses a v2 observation cleanly', async () => {
  const observation = await buildObservationV2();
  const res = verifyOutcomeObservation(observation as any, OBS_OPTS as any);
  assert.equal(res.valid, false);
...
test('OBSERVATION LEG STRIPPING: removing the ML-DSA leg refuses', async () => {
  const observation: any = await buildObservationV2();
  observation.proof.signatures = observation.proof.signatures.filter((s: any) => s.alg === 'Ed25519');
  const res = await verifyOutcomeObservationV2(observation, OBS_OPTS);
  assert.equal(res.valid, false);
  assert.equal(res.checks.signature, false);
});
