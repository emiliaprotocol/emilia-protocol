...
// Generated from aeb-native-adapters.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
import assert from 'node:assert/strict';
import crypto from 'node:crypto';
import { readFileSync } from 'node:fs';
import test from 'node:test';
import { adapterPinDigest, canonicalizeAeb, digestAeb, evaluateAebEvidence, mappingProfileDigest, registryEntryDigest, unifiedRegistryDigest, } from './aeb-adapter-contract.js';
import { AEB_NATIVE_CAID_MAPPER_ID, AEB_NATIVE_CAID_MAPPING_VERSION, AGENTROA_AEB_ADAPTER_ID, AGENTROA_AEB_ADAPTER_VERSION, AGENTROA_AEB_CONFIG_VERSION, AGENTROA_AEB_TRUST_ROOT_VERSION, ORPRG_AEB_ADAPTER_ID, ORPRG_AEB_ADAPTER_VERSION, ORPRG_AEB_CONFIG_VERSION, ORPRG_AEB_TRUST_ROOT_VERSION, createAgentRoaAebAdapter, createOrprgAebAdapter, } from './aeb-native-adapters.js';
import { AGENTROA_DRAFT } from './agentroa.js';
import { ORPRG_ACTION_PROFILE, ORPRG_JSON_JCS_PROFILE, computeOrprgActionDigest, } from './orprg.js';
...
function signObject(body, signer, key) {
    const sig = crypto.sign(null, Buffer.from(canonicalizeAeb(body), 'utf8'), key).toString('base64url');
    return { ...structuredClone(body), signatures: [{ signer, alg: 'EdDSA', sig }] };
}
function externalStatus(now, overrides = {}) {
...
function resignAgentAer(fixture, mutate) {
...
function signOrprgReceipt(receipt, key) {
    const unsigned = {
...
function makeOrprgFixture(mutator) {
...
function mappingProfile(protocol, actionType, requiredFields, registryRef = `mapping:${actionType}`) {
    return {
...
function nativeInput(fixture, now, status = externalStatus(now)) {
    return {
...
function evaluateOrprg(fixture, operationId, consumptionNonce) {
...
test('AgentROA refuses forged relying-party roots and exact-action mismatch', () => {
    ...
    const forgedRoots = fixture.trustRoots.map((root) => ({
        ...root,
...
test('AgentROA verifies signed negative evidence but rejects it, and refuses uncertain status', () => {
    ...
    const fixture = makeAgentFixture();
    const unavailable = adapter.verifyNative(nativeInput(fixture, AGENT_NOW, externalStatus(AGENT_NOW, { unavailable: true })));
    ...
    const revoked = adapter.verifyNative(nativeInput(fixture, AGENT_NOW, externalStatus(AGENT_NOW, { revoked: true })));
...
test('ORPRG rejects forged roots and action mismatch before the replay boundary', () => {
    ...
    const forged = adapter.verifyNative({
        ...
        trust_roots: fixture.trustRoots.map((root) => ({
            ...root,
            public_key: spki(attacker.publicKey),
        })),
    });
