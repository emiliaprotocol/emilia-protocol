...
// Generated from portable-state-handoff.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
...
import { InMemoryRecipientStateBoundary, PORTABLE_STATE_ACTIONS, PORTABLE_STATE_AUTHORITY_PROFILE, PORTABLE_STATE_IMPORT_RECEIPT_VERSION, PORTABLE_STATE_LIMITS, PORTABLE_STATE_MANIFEST_VERSION, PORTABLE_STATE_SIGNATURE_PROFILE, STATE_HANDOFF_CAID_DEFINITIONS, buildSourceRetirementExpectation, importPortableState, reconcilePortableStateImport, signPortableStateManifest, stateActionExpectation, stateHandoffDigest, verifyPortableStateImportReceipt, verifyPortableStateImportReceiptForManifest, } from './dist/portable-state-handoff.js';
import { SOMA_COGOBJ_PAYLOAD_PROFILE, SOMA_COGOBJ_VERSION, somaCogobjPayloadAdapter, } from './dist/soma-cogobj-profile.js';
...
function descriptors(objects, optionalIds = []) {
...
async function manifestFor(objects, options = {}) {
    ...
    if (listed.some((entry) => entry.sensitivity === 'VAULT'))
        sourceActions.push(PORTABLE_STATE_ACTIONS.KEY_RELEASE);
    const base = {
...
async function bundleFor(objects, manifestOptions = {}) {
...
function boundary(overrides = {}) {
    return new InMemoryRecipientStateBoundary({
        ...
        authorizeImport(expected, evidence) {
            if (overrides.allow === false || !evidence || evidence.caid !== expected.caid) {
...
function importOptions(target, overrides = {}) {
    return {
        now: NOW,
...
function authorizeBundle(bundle, options) {
...
async function run(bundle, target = boundary(), overrides = {}) {
...
test('accepts only after source release verification and atomic recipient admission', async () => {
    ...
    assert.deepEqual(await verifyPortableStateImportReceiptForManifest(receipt, bundle.manifest, importerPins), { valid: true, reasons: [] });
});
...
test('tampering, required omission, and unlisted payloads refuse before authority consumption', async () => {
    for (const mutate of [
        (bundle) => { bundle.objects[0].content = { changed: true }; },
        (bundle) => { bundle.objects = []; },
        (bundle) => { bundle.objects.push(cogobj({ object_id: 'urn:cogobj:extra' })); },
    ]) {
        const bundle = await bundleFor([cogobj()]);
        mutate(bundle);
        const target = boundary();
...
test('canonical state and receipt collections are bounded before admission', async () => {
    assert.throws(() => stateHandoffDigest(Array.from({ length: PORTABLE_STATE_LIMITS.max_nodes + 1 }, () => null)), /node count exceeds/);
    const bundle = await bundleFor([cogobj()]);
    ...
    tooManyReasons.reasons = Array.from({ length: PORTABLE_STATE_LIMITS.max_reasons + 1 }, (_, index) => `reason-${index}`);
...
test('set and object time ordering is enforced rather than inferred from signatures', async () => {
    ...
    await assert.rejects(() => bundleFor([lateObject]), /object_snapshot_after_manifest_cut/);
    await assert.rejects(() => bundleFor([cogobj()], { overrides: { created_at: '2026-02-30T17:56:00Z' } }), /manifest_schema_invalid/);
});
...
test('lost acknowledgement reconciles the committed record without a blind retry', async () => {
    ...
    assert.equal(target.consumptionCount(), 1);
    bundle.objects[0].content = { tampered_retransmission: true };
    ...
    const reconciled = await reconcilePortableStateImport(bundle.manifest.handoff_id, stateHandoffDigest(bundle.manifest), options);
    ...
    assert.deepEqual(await verifyPortableStateImportReceiptForManifest(reconciled, bundle.manifest, importerPins), { valid: true, reasons: [] });
    assert.equal(target.consumptionCount(), 1);
});
test('rollback and fork refuse, while an unanchored gap is indeterminate', async () => {
    const current = `sha256:${'8'.repeat(64)}`;
...
test('unknown required payload profile stays INDETERMINATE', async () => {
    ...
    const unsigned = { ...original, payload_profile: 'urn:unknown:profile', signatures: undefined };
    ...
    const bundle = { manifest, objects: [object], source_authority_evidence: {} };
...
test('hybrid import receipts verify offline and refuse algorithm-set stripping', async () => {
    ...
    assert.equal(receipt.result, 'ACCEPTED');
    const hybridPins = [importerPins[0], {
            alg: 'ML-DSA-65',
...
test('source retirement is a separate post-import action bound to the accepted receipt', async () => {
    ...
    assert.equal(retirement.action_object.retirement_set_digest, stateHandoffDigest(bundle.manifest.objects.map((entry) => ({
    ...
    assert.equal(partialRetirement.action_object.retirement_set_digest, stateHandoffDigest(partialBundle.manifest.objects
    ...
    assert.throws(() => buildSourceRetirementExpectation(partialBundle.manifest, unsafe), /does not partition/);
});
test('exact-action CAIDs change under recipient, manifest, and action substitution', async () => {
    ...
    const changedUnsigned = { ...manifest, recipient_agent: 'urn:agent:other', signatures: undefined };
    delete changedUnsigned.signatures;
    ...
    delete boundaryUnsigned.signatures;
...
test('runtime CAID definitions match the governed public registry field-for-field', async () => {
    const registry = JSON.parse(await readFile(new URL('../../caid/registry/action-types.json', import.meta.url), 'utf8'));
    for (const runtime of STATE_HANDOFF_CAID_DEFINITIONS) {
        const published = registry.types.find((entry) => entry.action_type === runtime.action_type);
        assert.ok(published, `missing registry definition for ${runtime.action_type}`);
        assert.deepEqual(published.required_fields?.map(({ name, type }) => ({ name, type })), runtime.required_fields.map(({ name, type }) => ({ name, type })));
...
test('manifest-aware receipt verification closes boundary, state-set, and CAID substitutions', async () => {
    ...
    const boundaryCheck = await verifyPortableStateImportReceiptForManifest(wrongBoundary, bundle.manifest, importerPins);
    ...
    const caidCheck = await verifyPortableStateImportReceiptForManifest(changedCaid, bundle.manifest, importerPins);
...
test('payload adapters are pinned exactly once', async () => {
    const bundle = await bundleFor([cogobj()]);
    const duplicate = { ...somaCogobjPayloadAdapter };
...
test('untrusted payload and verifier failures become signed terminal results, never authority', async () => {
    {
        const bundle = await bundleFor([cogobj()]);
        const cyclic = { object_id: cogobj().object_id };
        cyclic.self = cyclic;
...
test('public receipt verifiers fail closed on hostile canonical input without throwing', async () => {
    ...
    const checked = await verifyPortableStateImportReceiptForManifest(receipt, hostileManifest, importerPins);
    assert.equal(checked.valid, false);
