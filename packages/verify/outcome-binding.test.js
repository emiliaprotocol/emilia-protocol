...
// Generated from outcome-binding.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
...
import { buildOutcomeAttestation, buildOutcomeObservation, OUTCOME_BINDING_RESULT_VERSION, outcomeBindingResultCore, outcomeBindingResultDigest, trustReceiptDigest, verifyOutcomeBinding, verifyOutcomeBindingSet, verifyOutcomeBindingResultDigest, verifyOutcomeObservationSet, } from './index.js';
...
function multiSourceObservations(meterValue = '5.2') {
    return [
        ...
        }, [{ effect_type: 'delivered_mw', target: 'meter:plant-7', value: meterValue }], meterPrivateKey),
    ];
}
...
test('one Ed25519 key cannot fill executor and independent-observer roles', () => {
    ...
    hostile[1] = observation({
        role: 'independent_observer', source_id: 'ep:observer:meter-7',
        source_class: 'revenue_meter', facility_id: 'facility:plant-7',
    }, [{ effect_type: 'delivered_mw', target: 'meter:plant-7', value: '5.2' }], executorPrivateKey);
...
test('decimal parsing stays linear on a long zero run with a nonzero tail', () => {
    ...
    assert.ok(elapsedMs < 750, `decimal parsing took ${elapsedMs.toFixed(1)}ms; expected linear-time handling below 750ms`);
});
...
test('serialized real-crypto independent-verification suite exercises the full protocol', () => {
    const suite = JSON.parse(readFileSync(new URL('../../conformance/vectors/outcome-binding.exec.v1.json', import.meta.url), 'utf8'));
...
test('serialized source-policy vectors exercise independence, freshness, windows, and quorum', () => {
    const suite = JSON.parse(readFileSync(new URL('../../conformance/vectors/outcome-binding.sources.v1.json', import.meta.url), 'utf8'));
    assert.equal(suite.count, 8);
    for (const vector of suite.vectors) {
        const result = verifyOutcomeObservationSet(suite.common.predicted_effects, vector.observations, {
