...
test("a lifecycle index cannot claim COMPLETE with an unlabeled evaluation or unknown custody", async () => {
    ...
    const complete = structuredClone(converted.body);
    complete.conversion = { status: "COMPLETE", reason_codes: [] };
    complete.contract_digest = crossingLifecycleIndexV2ContractDigest(complete);
    const result = await verifyAebCrossingLifecycleIndexV2(await signIndexBody(complete), indexOptions({ expected_evaluation: complete.lifecycle.evaluation }));
    assert.equal(result.verified, false);
    ...
    const nativeUnlabeled = structuredClone(converted.body);
    ...
    assert.equal((await verifyAebCrossingLifecycleIndexV2(await signIndexBody(nativeUnlabeled), indexOptions({ expected_evaluation: nativeUnlabeled.lifecycle.evaluation }))).reason, "conversion_report_invalid");
});
