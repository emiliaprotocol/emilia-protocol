...
import { AEB_CROSSING_RECORD_REQUIRED_ALGORITHMS, AEB_CROSSING_RECORD_VERSION, AEB_CROSSING_RECORD_V2_VERSION, AEB_CROSSING_LIFECYCLE_INDEX_V2_VERSION, BCR_CROSSING_MAPPING_PROFILE, WIMSE_OAUTH_CROSSING_MAPPING_PROFILE, crossingRecordContractDigest, crossingRecordV2AdmissionDomainDigest, crossingRecordV2ContractDigest, crossingLifecycleIndexV2AdmissionDomainDigest, crossingLifecycleIndexV2ContractDigest, crossingLifecycleIndexV2SignedBytes, aebCrossingEvaluationReference, issueAebCrossingLifecycleIndexV2, issueAebCrossingRecordV2, issueAebCrossingRecord, mapBcrCrossingAuthority, mapWimseOAuthCrossingAuthority, upgradeAebCrossingRecordV1ToLifecycleIndexV2, verifyAebCrossingLifecycleIndexV2, verifyAebCrossingRecord, verifyAebCrossingRecordV2, } from "./dist/aeb-crossing-record.js";
import { AEB_EVALUATION_VERSION, AEB_EVALUATION_V2_VERSION, adapterPinDigest, aebEvaluationV2Digest, digestAeb, digestAebTyped, evaluateAebEvidence, issueAebEvaluationV2FromV1, mappingProfileDigest, registryEntryDigest, unifiedRegistryDigest, verifyAebEvaluation, verifyAebEvaluationV2, } from "./dist/aeb-adapter-contract.js";
import { loadDefaultAgilityMldsaBackend, signAgileSet, } from "./dist/pq-signature-agility.js";
...
async function issueV2(overrides = {}, contextOverrides = {}) {
    const draft = {
    ...
    return issueAebCrossingRecordV2(draft, {
        action: ACTION,
...
async function issueLifecycleIndex(overrides = {}, contextOverrides = {}) {
    const evaluation = {
...
function evaluationRegistryEntry(entryId, kind, definition) {
    const entry = {
        kind,
        version: "1",
        status: "active",
...
function evaluationConfig() {
    ...
    profile.profile_digest = mappingProfileDigest(EVALUATION_PROFILE_ID, profile);
    const registry = {
        "@version": "EP-EVIDENCE-REGISTRY-v1",
        registry_id: "registry:crossing-test",
        epoch: 1,
        entries: {
            "mapping:test:crossing": evaluationRegistryEntry("mapping:test:crossing", "mapping-profile", { profile_digest: profile.profile_digest }),
            "role:native-authority": evaluationRegistryEntry("role:native-authority", "evidence-role", { role: "native-authority", subject_kinds: ["workload"] }),
        },
    };
    ...
    pin.config_digest = adapterPinDigest(EVALUATION_ADAPTER_ID, pin);
    return {
...
function evaluationFor(operationId, options = {}) {
    ...
    const result = evaluateAebEvidence({
        config,
        adapters: { [EVALUATION_ADAPTER_ID]: evaluationAdapter() },
        ...
        legs: [
            {
                ...
                status: evaluationStatus(options.status),
            },
        ],
        evaluated_at: NOW,
...
function boundLifecycleRecords(evaluationRecord) {
    return {
...
async function signIndexBody(body) {
    return {
        "@version": AEB_CROSSING_LIFECYCLE_INDEX_V2_VERSION,
        body,
        signatures: await signAgileSet(crossingLifecycleIndexV2SignedBytes(body), [...SIGNERS], { deterministic: true, mldsaBackend }),
    };
}
...
test("v2 recomputes an explicit admission-domain commitment", async () => {
    ...
    assert.equal(record.body.admission_domain_digest, crossingRecordV2AdmissionDomainDigest(BOUNDARY));
    assert.equal(record.body.contract_digest, crossingRecordV2ContractDigest(record.body));
...
test("generic v2 issuance refuses action and admission-domain mismatch before signing", async () => {
    ...
    for (const [member, value] of [
        ...
        }), /admission_domain_mismatch/, member);
    }
    await assert.rejects(() => issueV2({}, {
...
test("v2 refuses admission-domain, action, authority, and freshness substitution", async () => {
    const record = await issueV2();
    const mutations = [
        (value) => { value.body.boundary.relying_party_id = "rp:other"; },
        (value) => { value.body.boundary.audience = "erp:other"; },
        (value) => { value.body.boundary.executor_id = "executor:other"; },
        (value) => { value.body.boundary.state_domain_id = "state-domain:other"; },
        (value) => { value.body.action.action_digest = `sha256:${"01".repeat(32)}`; },
        (value) => { value.body.native_authority.authority_instance_digest = `sha256:${"02".repeat(32)}`; },
        (value) => { value.body.native_authority.status.value = "STALE"; },
    ];
...
test("the lifecycle index references separate records without flattening native authority or policy", async () => {
    ...
    assert.equal(index["@version"], AEB_CROSSING_LIFECYCLE_INDEX_V2_VERSION);
    ...
    assert.equal(index.body.admission_domain_digest, crossingLifecycleIndexV2AdmissionDomainDigest(BOUNDARY));
    assert.equal(index.body.contract_digest, crossingLifecycleIndexV2ContractDigest(index.body));
...
test("the lifecycle index refuses impossible ordering before signing", async () => {
    ...
    await assert.rejects(() => issueLifecycleIndex({
        lifecycle: {
            evaluation,
            local_admission_digest: ADMISSION_DIGEST,
            authority_custody: {
...
test("v1 upgrade preserves resolvable lifecycle references and reports unrecoverable state", async () => {
    ...
    assert.equal(converted.body.source_crossing_record.version, AEB_CROSSING_RECORD_VERSION);
    assert.equal(converted.body.lifecycle.evaluation.profile, AEB_EVALUATION_VERSION);
    assert.equal(converted.body.lifecycle.evaluation.digest, source.body.lifecycle_records.evaluation_digest);
    assert.equal(converted.body.lifecycle.authority_custody.phase, "RESERVATION");
    ...
    const terminalConverted = await upgradeAebCrossingRecordV1ToLifecycleIndexV2(terminal, {
        signing_keys: [...SIGNERS],
        source_verification_keys: [...VERIFICATION_KEYS],
        deterministic: true,
        mldsaBackend,
    ...
    assert.ok(terminalConverted.body.conversion.reason_codes.includes("provider_outcome_reference_unavailable"));
    assert.equal(terminalConverted.body.lifecycle.authority_custody.phase, "CONSUMPTION");
});
test("v1 lifecycle upgrade refuses an unverified source", async () => {
    ...
    await assert.rejects(() => upgradeAebCrossingRecordV1ToLifecycleIndexV2(tampered, {
        signing_keys: [...SIGNERS],
        source_verification_keys: [...VERIFICATION_KEYS],
        deterministic: true,
        mldsaBackend,
...
test("crossing records and lifecycle indexes cannot be relabeled across profiles", async () => {
    ...
    for (const crossing of [v1, v2]) {
        const relabeled = structuredClone(crossing);
        relabeled["@version"] = AEB_CROSSING_LIFECYCLE_INDEX_V2_VERSION;
        assert.equal((await verifyAebCrossingLifecycleIndexV2(relabeled, indexOptions)).verified, false);
    }
    const stripped = structuredClone(index);
    stripped.signatures = stripped.signatures.filter((signature) => signature.alg === "Ed25519");
    assert.equal((await verifyAebCrossingLifecycleIndexV2(stripped, indexOptions)).reason, "hybrid_leg_missing");
});
...
async function verifyV2(record, evaluation) {
...
async function verifyV1(record, evaluation) {
    return verifyAebCrossingRecord(record, {
        verification_keys: [...VERIFICATION_KEYS],
        mldsaBackend,
        ...(evaluation === undefined ? {} : { evaluation }),
...
test("v1 and v2 refuse an unrelated evaluation even when the record commits to its digest", async () => {
    ...
    assert.equal((await verifyV1(otherActionRecord, otherAction)).reason, "evaluation_action_mismatch");
});
test("v1 and v2 refuse an evaluation whose digest the record does not commit to", async () => {
    ...
    assert.equal((await verifyV2(recordV2, evaluationForV2)).reason, "evaluation_digest_mismatch");
});
test("v1 and v2 refuse a native authority that matches no evaluated leg", async () => {
    ...
    assert.equal((await verifyV1(bcrRecord, evaluationV1)).reason, "evaluation_authority_unmatched");
    const { record: evaluationForV2 } = evaluationFor(V2_OPERATION);
    const recordV2 = await issueV2({
        lifecycle_records: boundLifecycleRecords(evaluationForV2),
    });
    assert.equal((await verifyV2(recordV2, evaluationForV2)).reason, "evaluation_authority_unmatched");
});
test("an admitted crossing cannot cite an evaluation that was not SATISFIED", async () => {
    ...
    assert.equal((await verifyV2(admittedV2, revokedV2)).reason, "evaluation_verdict_inconsistent");
...
test("an evaluation that verified authority less strongly than the record claims refuses", async () => {
    ...
    assert.equal((await verifyV1(record, weakened)).reason, "evaluation_authority_inconsistent");
    ...
    assert.equal((await verifyV1(admitted, unsatisfiedLeg)).reason, "evaluation_authority_inconsistent");
});
test("hostile evaluation inputs refuse with a reason and never throw", async () => {
    ...
    let getterCalls = 0;
    const accessor = structuredClone(evaluationV1);
    ...
    const missingLegs = structuredClone(evaluationV1);
...
async function issueBoundIndex(evaluation, lifecycleOverrides = {}, overrides = {}) {
    return issueAebCrossingLifecycleIndexV2({
        ...
        execution_authorizing: false,
        ...overrides,
    }, { action: ACTION, admission_domain: BOUNDARY, evaluation }, { signing_keys: [...SIGNERS], deterministic: true, mldsaBackend });
}
function indexOptions(extra = {}) {
    return {
        verification_keys: [...VERIFICATION_KEYS],
        expected_action: ACTION,
        admission_domain: BOUNDARY,
...
test("the lifecycle index reports the evaluation binding as INDETERMINATE without the evaluation", async () => {
    const index = await issueLifecycleIndex();
    const result = await verifyAebCrossingLifecycleIndexV2(index, indexOptions({ expected_evaluation: index.body.lifecycle.evaluation }));
    assert.equal(result.verified, true, JSON.stringify(result));
...
test("the lifecycle index binds AEB-EVALUATION-v1 and -v2 records by the verifier record digest", async () => {
    ...
    const v1Bound = await verifyAebCrossingLifecycleIndexV2(v1Index, indexOptions({ evaluation: evaluationV1 }));
    ...
    assert.notEqual(v2Reference.digest, aebEvaluationV2Digest(v2Body));
    const v2Index = await issueBoundIndex(v2Reference);
    const v2Bound = await verifyAebCrossingLifecycleIndexV2(v2Index, indexOptions({
    ...
    const bodyDigestIndex = await issueBoundIndex({
        profile: AEB_EVALUATION_V2_VERSION,
        digest: aebEvaluationV2Digest(v2Body),
    });
    assert.equal((await verifyAebCrossingLifecycleIndexV2(bodyDigestIndex, indexOptions({ evaluation: evaluationV2 }))).reason, "evaluation_digest_mismatch");
});
test("the lifecycle index refuses an unrelated or relabeled evaluation", async () => {
    ...
    assert.equal((await verifyAebCrossingLifecycleIndexV2(relabeled, indexOptions({ expected_evaluation: relabeledReference }))).verified, true, "the pointer-only check still cannot see the join");
    const relabeledResult = await verifyAebCrossingLifecycleIndexV2(relabeled, indexOptions({
    ...
    const unrelatedIndex = await issueBoundIndex(aebCrossingEvaluationReference(unrelated));
    assert.equal((await verifyAebCrossingLifecycleIndexV2(unrelatedIndex, indexOptions({ evaluation: unrelated }))).reason, "evaluation_operation_mismatch");
    ...
    assert.equal((await verifyAebCrossingLifecycleIndexV2(otherActionIndex, indexOptions({
...
test("the lifecycle index refuses provider entry without an authority reservation", async () => {
    ...
    const result = await verifyAebCrossingLifecycleIndexV2(signed, indexOptions({ expected_evaluation: evaluation }));
    assert.equal(result.verified, false);
...
test("an unverified evaluation label is the flagged 4.1.0 placeholder, never null, and a COMPLETE index needs known custody", async () => {
    ...
    const profile = converted.body.lifecycle.evaluation.profile;
    ...
    const relabeled = structuredClone(converted.body);
    ...
    assert.equal((await verifyAebCrossingLifecycleIndexV2(await signIndexBody(relabeled), indexOptions({ expected_evaluation: relabeled.lifecycle.evaluation }))).reason, "conversion_report_invalid");
    // A null profile is not a lifecycle index, as in 4.1.0.
    const unlabeled = structuredClone(converted.body);
    unlabeled.lifecycle.evaluation.profile = null;
    unlabeled.contract_digest = crossingLifecycleIndexV2ContractDigest(unlabeled);
    assert.equal((await verifyAebCrossingLifecycleIndexV2(await signIndexBody(unlabeled), indexOptions({ expected_evaluation: unlabeled.lifecycle.evaluation }))).reason, "malformed_lifecycle_index");
    ...
    for (const conversion of [
        ...
        const flagged = structuredClone(converted.body);
        ...
        assert.equal((await verifyAebCrossingLifecycleIndexV2(await signIndexBody(flagged), indexOptions({ expected_evaluation: flagged.lifecycle.evaluation }))).reason, "conversion_report_invalid", conversion.status);
    }
    const unknownCustody = structuredClone(converted.body);
    ...
    assert.equal((await verifyAebCrossingLifecycleIndexV2(await signIndexBody(unknownCustody), indexOptions({ expected_evaluation: unknownCustody.lifecycle.evaluation }))).reason, "conversion_report_invalid");
});
test("an unverified conversion label is not compared with a supplied evaluation, so a v2 evaluation still binds", async () => {
    ...
    assert.equal(converted.body.lifecycle.evaluation.profile, AEB_EVALUATION_VERSION);
    assert.ok(converted.body.conversion.reason_codes.includes("evaluation_reference_unverified"));
    const result = await verifyAebCrossingLifecycleIndexV2(converted, indexOptions({ evaluation: evaluationV2 }));
    assert.equal(result.verified, true, JSON.stringify(result));
...
test("v1 upgrade with the bound source evaluation is COMPLETE and carries its profile", async () => {
    ...
    assert.deepEqual(converted.body.lifecycle.evaluation, aebCrossingEvaluationReference(evaluationV1));
    const result = await verifyAebCrossingLifecycleIndexV2(converted, indexOptions({
        expected_action: ACTION,
        evaluation: evaluationV1,
    }));
    assert.equal(result.verified, true, JSON.stringify(result));
    ...
    await assert.rejects(() => upgradeAebCrossingRecordV1ToLifecycleIndexV2(source, {
        signing_keys: [...SIGNERS],
        source_verification_keys: [...VERIFICATION_KEYS],
...
test("v1 upgrade converts MISSING admission with later references to INDETERMINATE instead of throwing", async () => {
    ...
    const result = await verifyAebCrossingLifecycleIndexV2(converted, indexOptions({ expected_evaluation: converted.body.lifecycle.evaluation }));
    assert.equal(result.verified, true, JSON.stringify(result));
});
test("v1 upgrade never reports provider entry without custody or admission as COMPLETE", async () => {
    ...
    const result = await verifyAebCrossingLifecycleIndexV2(converted, indexOptions({ evaluation: evaluationV1 }));
    assert.equal(result.verified, true, JSON.stringify(result));
    assert.equal(result.evaluation_binding, "BOUND");
});
