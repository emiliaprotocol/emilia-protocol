...
function readWorkspace(root: string): any {
  return JSON.parse(readFileSync(join(root, 'workspace.json'), 'utf8'));
}

function writeWorkspace(root: string, workspace: any): void {
  writeFileSync(join(root, 'workspace.json'), `${JSON.stringify(workspace, null, 2)}\n`);
}

const SCAN_PROFILE = 'ccs-wang-draft08-v13';

function writeScanSeed(parent: string): { seedPath: string; manifestPath: string; seed: any } {
...
test('reviewed Scan seed creates an explicit unsealed three-file workspace', () => {
  ...
  assert.throws(
    () => runCrossingLab(target),
    CROSSING_LAB_RUNTIME_SUPPORTED
...
test('Scan seed refuses action, profile, and reviewed-manifest substitution', () => {
  const cases: Array<(fixture: ReturnType<typeof writeScanSeed>) => void> = [
    ({ seedPath }) => {
  ...
  for (const [index, mutate] of cases.entries()) {
    ...
    mutate(fixture);
    assert.throws(
      () => initCrossingLabFromScanSeed(fixture.seedPath, join(parent, 'workspace')),
      /selected action digest|supported crossing profile|Verify version mismatch|profile contract mismatch|reviewed manifest digest/,
    );
...
test('Scan seed refuses duplicate reviewed action ids and selectors', () => {
  const cases: Array<(manifest: any) => void> = [
  ...
  for (const [index, mutate] of cases.entries()) {
    ...
    assert.throws(
      () => initCrossingLabFromScanSeed(fixture.seedPath, join(parent, 'workspace')),
      /duplicate action id or selector/,
    );
  }
});
...
test('Scan seed initializer refuses malformed fields, hard links, and overwrite', () => {
  ...
  assert.throws(
    () => initCrossingLabFromScanSeed(malformed.seedPath, join(malformedParent, 'workspace')),
    /invalid Scan crossing seed/,
  );
});
...
runtimeTest('report keeps native verification, acceptance, mapping, and satisfaction separate', () => {
  ...
  assert.equal(
    rows['replay-identity-is-wrapper-independent'].evaluation.legs[0].replay_unit,
    rows['native-artifact-through'].evaluation.legs[0].replay_unit,
  );
...
runtimeTest('nondeterministic adapter output is refused', () => {
  ...
  const source = readFileSync(adapterPath, 'utf8').replace(
    ...
    "subject: { id: crypto.randomBytes(8).toString('hex'), kind: 'human' },",
  );
  writeFileSync(adapterPath, source);
  sealCrossingLab(root);
  const report = runCrossingLab(root);
...
runtimeTest('module-local state cannot evade the same-process determinism check', () => {
  ...
  const source = readFileSync(adapterPath, 'utf8')
    ...
    .replace(
      ...
      "replay_unit: digest({ protocol: 'example-native', native_id: input.artifact.native_id, call: ++crossingLabCounter }),",
    );
  writeFileSync(adapterPath, source);
  sealCrossingLab(root);
  const report = runCrossingLab(root);
...
runtimeTest('packed package carries the CLI, Crossing Lab library, worker, and declarations', () => {
  ...
  const files = new Set(packed.files.map((entry: any) => entry.path));
