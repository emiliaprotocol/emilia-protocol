...
// Generated from discovery-permit-contract.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
import assert from 'node:assert/strict';
import test from 'node:test';
import { DISCOVERY_PERMIT_BINDING_VERSION, DISCOVERY_PERMIT_DISCOVERY_VERSION, DISCOVERY_PERMIT_RESOLUTION_VERSION, canonicalizeDiscoveryPermit, digestDiscoveryPermit, digestDiscoveryPermitRaw, evaluateDiscoveryPermitContinuity, pinDiscoveryPermitTrust, } from './discovery-permit-contract.js';
...
const DISCOVERY_SCHEMA_DIGEST = `sha256:${'1'.repeat(64)}`;
const PERMIT_SCHEMA_DIGEST = `sha256:${'2'.repeat(64)}`;
const MAPPING_DIGEST = `sha256:${'3'.repeat(64)}`;
function pinsInput(overrides = {}) {
    return {
...
function documents(pins = pinDiscoveryPermitTrust(pinsInput()), overrides = {}) {
    const source = {
        origin: pins.origin,
...
function documentProvenance(role, requestedUrl, resolvedUrl, document) {
...
test('pins and resolution are immutable snapshots with raw and canonical provenance', () => {
    ...
    assert.notEqual(resolution.provenance.discovery.raw_digest, resolution.provenance.discovery.canonical_digest);
...
test('canonicalization is deterministic while the raw digest preserves source bytes', () => {
    ...
    assert.equal(digestDiscoveryPermit(JSON.parse(left)), digestDiscoveryPermit(JSON.parse(right)));
...
test('discovery and permit sources, schema pins, mapping, CAID, and action must agree exactly', () => {
    ...
    for (const [code, overrides] of cases) {
        assert.throws(() => evaluate(overrides), (error) => error?.code === code, code);
    }
});
test('future timestamps, malformed CAIDs, and provenance substitution fail closed', () => {
    assert.throws(() => evaluate({ issued_at: '2026-07-24T12:00:01Z' }), (error) => error?.code === 'issued_at_in_future');
    assert.throws(() => evaluate({ binding: { caid: 'caid:attacker' } }), (error) => error?.code === 'binding_shape_invalid');
    const pins = pinDiscoveryPermitTrust(pinsInput());
    ...
    assert.throws(() => evaluateDiscoveryPermitContinuity({
        pins,
        discovery,
        binding,
        caid: CAID,
        ...
        now: NOW,
        provenance,
    }), (error) => error?.code === 'provenance_digest_mismatch');
});
