// Generated from federation.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
/**
...
import { resolveOperatorKeys, verifyFederatedReceiptOffline, verifyFederatedReceipt, _internals, } from './federation.js';
...
const testNetworkFor = (fetchImpl, { resolveAddresses = async () => [TEST_PUBLIC_ADDRESS], connectedAddress = null, } = {}) => ({
    networkBoundary: {
        resolveAddresses,
...
test('a historical key refuses receipts issued after retirement or without a valid bound issued_at', () => {
    ...
    for (const item of cases) {
        ...
        if (item.issuedAt === undefined)
            delete payload.issued_at;
        else
            payload.issued_at = item.issuedAt;
...
test('a historical key with missing or malformed retired_at fails closed', () => {
    ...
    for (const retiredAt of [undefined, 'not-a-timestamp']) {
        ...
        if (retiredAt === undefined)
            delete doc.historical_keys.ep_operator_a[0].retired_at;
        else
            doc.historical_keys.ep_operator_a[0].retired_at = retiredAt;
...
test('ONLINE: even a "verifying" attacker receipt is not accepted when signer is not pinned', async () => {
    ...
    const fetchImpl = async (url) => {
        if (url.endsWith('/ep-keys.json'))
            return { ok: true, status: 200, json: async () => discoveryDoc(Attacker) };
        return { ok: false, status: 404, json: async () => ({}) };
    };
