...
import {
  AEB_ACCEPTANCE_PROFILE_VERSION,
  aebAcceptanceReservationKey,
  applyAebAcceptanceProfile,
  defineAebAcceptanceProfile,
  verifyAebAcceptanceProfile,
} from './aeb-acceptance-profile.js';
import {
...
const adapterConfig: OasntAdapterConfig = Object.freeze({
...
function status(): AebStatusInput {
  return {
    checked_at: NOW,
...
test('attempt -> challenge -> approve -> consume -> execute -> reconcile is one replay-safe path', () => {
  ...
  let effects = 0;
  if (decision.invoke_allowed) effects += 1;
