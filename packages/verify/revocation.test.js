...
// Generated from revocation.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
//
...
function buildStatement({ revokerId = 'ep:revoker:ig_okafor', revokedAt = '2026-06-20T12:00:00.000Z', reason = 'authority withdrawn', actionHash = TARGET.action_hash, signer }) {
    const signedFields = {
...
test('accepts the historical v1 local key label only with the exact pinned SPKI', () => {
    const historical = JSON.parse(readFileSync(new URL('../../conformance/vectors/revocation.exec.v1.json', import.meta.url), 'utf8')).vectors.find((vector) => vector.id === 'accept_pinned_exact_binding');
...
test('accepts one through nine fractional-second digits and schema enforces the same bounds', () => {
    ...
    const schema = JSON.parse(readFileSync(new URL('../../public/schemas/ep-revocation.schema.json', import.meta.url), 'utf8'));
