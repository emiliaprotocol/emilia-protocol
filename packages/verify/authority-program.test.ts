...
function publicKey(keyPair: crypto.KeyPairKeyObjectResult): string {
  return keyPair.publicKey.export({ type: 'spki', format: 'der' }).toString('base64url');
}

function signBody(body: Record<string, unknown>, domain: string, keyPair: crypto.KeyPairKeyObjectResult): string {
  return crypto.sign(
    null,
    Buffer.from(`${domain}${canonicalize(body)}`, 'utf8'),
    keyPair.privateKey,
  ).toString('base64url');
}

function signProgram(body: Record<string, any>, signer = {
...
function signStage(body: Record<string, any>, organizationId: keyof typeof stageKeys, keyId: string) {
  const unsigned = clone(body);
  delete unsigned.proof;
  const keyPair = stageKeys[organizationId][keyId as keyof typeof stageKeys[typeof organizationId]];
  assert.ok(keyPair);
  return {
    ...unsigned,
    proof: {
...
function stage(stage_id: string, organization_id: string, key_id: string) {
  return {
    type: 'stage',
    stage_id,
...
function makeStageReceipt(program: Record<string, any>, stageNode: Record<string, any>, predecessors: string[]) {
...
function stagesById(program: Record<string, any>) {
...
function optionsFor(bundle: ReturnType<typeof makeBundle>) {
...
function resignReceipt(receipt: Record<string, any>) {
...
test('verifies a signed nested sequence/parallel program as immutable digest joins', () => {
  ...
  assert.deepEqual(result, {
    ...
    stage_receipt_digests: Object.fromEntries(
      bundle.receipts
...
test('fails closed for unsigned, wrongly signed, untrusted, or wrongly pinned programs', () => {
  ...
  wronglySigned.proof.signature_b64u = wronglySigned.proof.signature_b64u.replace(/^./, (c: string) => c === 'A' ? 'B' : 'A');
...
test('hostile JavaScript objects cannot throw through the pure verifier', () => {
  ...
  assert.equal(
    verifyAuthorityProgram(oversized.program, oversized.receipts, oversizedOptions).reason,
    'invalid_program_signature',
  );
});
...
test('requires the exact canonical predecessor receipt digest set', () => {
  const bundle = makeBundle();
  const options = optionsFor(bundle);
  const mutateD = (predecessors: string[]) => bundle.receipts.map((receipt) => (
    receipt.stage_id === 'stage-d'
  ...
  assert.equal(
    verifyAuthorityProgram(bundle.program, mutateD(d.predecessor_receipt_digests.slice(0, 1)), options).reason,
    'predecessor_receipt_digest_mismatch',
  );
  assert.equal(
    verifyAuthorityProgram(bundle.program, mutateD([...d.predecessor_receipt_digests, digest('extra')]), options).reason,
    'predecessor_receipt_digest_mismatch',
  );
  assert.equal(
    verifyAuthorityProgram(bundle.program, mutateD([...d.predecessor_receipt_digests].reverse()), options).reason,
    'predecessor_receipt_digest_mismatch',
  );
});
...
test('rejects the wrong stage organization and receipt replay across every binding axis', () => {
  ...
  wrongOrg[bIndex] = signStage(
    { ...wrongOrg[bIndex], issuer: { organization_id: 'org:gamma', key_id: 'key:gamma' } },
    'org:gamma',
    'key:gamma',
  );
  ...
  assert.equal(
    verifyAuthorityProgram(signedOtherAuthority, bundle.receipts, otherAuthorityOptions).reason,
    'stage_program_digest_mismatch',
  );
});
...
test('requires exact independently verified AEC and explicit AOM joins', () => {
  ...
  const resultSmuggling = {
    ...options,
    verifyAec: (context: Record<string, any>) => ({ ...options.verifyAec(context), operator_override: true }),
  };
  assert.equal(
    verifyAuthorityProgram(bundle.program, bundle.receipts, resultSmuggling).reason,
    'aec_verification_mismatch',
  );
});
...
test('rejects capability broadening and parallel budget claims without authoritative allocation proof', () => {
  ...
  const broadened = {
    ...options,
    verifyCapabilityNarrowing: (context: Record<string, any>) => ({
  ...
  assert.equal(
    verifyAuthorityProgram(bundle.program, bundle.receipts, noAllocationVerifier).reason,
    'parallel_allocation_unproven',
  );
  ...
  assert.equal(
    verifyAuthorityProgram(bundle.program, bundle.receipts, selfAsserted).reason,
    'parallel_allocation_unproven',
  );
});
