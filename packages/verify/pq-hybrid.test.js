// Generated from pq-hybrid.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
/**
...
import { HYBRID_ALG, HYBRID_SIGNATURE_ALGOS, HYBRID_REASONS, ML_DSA_65_SIGNATURE_BYTES, ML_DSA_65_PUBLIC_KEY_BYTES, hybridSigningInput, loadDefaultMldsaBackend, signHybrid, verifyHybrid, } from './pq-hybrid.js';
// --- fixtures ---------------------------------------------------------------
...
test('the Ed25519 sig cryptographically commits to the algo set (core anti-stripping property)', async () => {
    ...
    assert.equal(crypto.verify(null, hybridSigningInput(MESSAGE, ['Ed25519']), edPair.publicKey, edSig), false);
    // ...only over the input committing to the FULL set.
    assert.equal(crypto.verify(null, hybridSigningInput(MESSAGE, [...HYBRID_SIGNATURE_ALGOS]), edPair.publicKey, edSig), true);
});
...
test('signHybrid with no PQ backend throws (never emits a classical-only envelope)', async () => {
    await assert.rejects(signHybrid(MESSAGE, { ed25519PrivateKey: edPair.privateKey, mldsaSecretKey: pqPair.secretKey }, { mldsaBackendLoader: noBackendLoader }), /pq_backend_unavailable/);
});
...
test('signHybrid refuses an Ed448 classical private key (never mints a mislabeled leg)', async () => {
    const ed448 = crypto.generateKeyPairSync('ed448');
    await assert.rejects(signHybrid(MESSAGE, { ed25519PrivateKey: ed448.privateKey, mldsaSecretKey: pqPair.secretKey }), /algorithm_key_mismatch/);
});
