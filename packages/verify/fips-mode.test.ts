...
import {
  FIPS_MODE_VERSION,
  FIPS_POLICY_ALGORITHMS,
  FIPS_REASONS,
  readFipsStatus,
  probeEd25519,
  probeOpensslOperational,
  resetFipsProbeCache,
  getFipsPosture,
  formatFipsPosture,
  assertClassicalFips,
...
function posture(
  ...
  ed25519_operational: boolean | null = true,
  openssl_operational: boolean | null = true,
  ed25519_in_validated_boundary: boolean | null = true,
): FipsPosture {
  return {
...
test('getFipsPosture reports without throwing on this machine', () => {
  ...
  assert.ok(
    p.fips_status === 'active' || p.fips_status === 'inactive' || p.fips_status === 'unavailable',
    `unexpected fips_status ${p.fips_status}`,
  );
...
test('on a working non-FIPS host both probes report true', () => {
  if (readFipsStatus() !== 'inactive') return;
...
test('formatFipsPosture renders a one-line summary and tolerates junk', () => {
  ...
  assert.ok(formatFipsPosture(null as unknown as FipsPosture).includes('malformed'));
});
...
test('assertClassicalFips refuses a posture-shaped impostor, never throws', () => {
  for (const junk of [null, 'active', 42, [], {}, { fips_status: 'yes' }, { fips_status: 'active' }]) {
    const r = assertClassicalFips({ posture: junk as unknown as FipsPosture });
...
test('only the literal true acknowledges', () => {
  for (const truthy of ['true', 1, {}, [], 'yes']) {
    const r = mldsaPolicy({
      posture: FIPS_ON,
      allow_unvalidated_mldsa: truthy as unknown as boolean,
    });
...
test('mldsaPolicy refuses a malformed posture', () => {
  const r = mldsaPolicy({ posture: { nope: true } as unknown as FipsPosture });
  assert.equal(r.permitted, false);
  assert.equal(r.reason, FIPS_REASONS.MALFORMED_POSTURE);
});
...
test('matrix: the boundary field never calls JavaScript ML-DSA an OpenSSL operation', () => {
  for (const p of [FIPS_ON, FIPS_OFF, FIPS_UNKNOWN]) {
    assert.equal(
      checkOperationPolicy('ML-DSA-65', p, { allow_unvalidated_mldsa: true }).boundary,
...
test('a set flag with a dead provider refuses every OpenSSL-backed algorithm', () => {
  ...
  assert.equal(
    checkOperationPolicy('ML-DSA-65', dead).reason,
    FIPS_REASONS.MLDSA_IMPLEMENTATION_UNVALIDATED,
  );
  assert.equal(
    checkOperationPolicy('ML-DSA-65', dead, { allow_unvalidated_mldsa: true }).permitted,
    true,
  );
});
...
test('Ed25519 under active FIPS refuses when the provider has no Ed25519', () => {
  ...
  assert.equal(
    checkOperationPolicy('Ed25519', noEd, { allow_unvalidated_mldsa: true }).permitted,
    false,
  );
});
...
test('the OpenSSL 3.0.x trap: Ed25519 works, and the certificate still says no', () => {
  ...
  assert.equal(
    checkOperationPolicy('Ed25519', trap, { allow_unvalidated_mldsa: true }).permitted,
    false,
  );
});
...
test('an unprobed posture does not block anything when FIPS mode is not active', () => {
  for (const status of ['inactive', 'unavailable'] as const) {
    for (const alg of ['Ed25519', 'ES256', 'SHA-256']) {
      assert.equal(
        checkOperationPolicy(alg, posture(status, null, null)).permitted,
        true,
        `${alg} under ${status}`,
      );
    }
...
test('checkOperationPolicy refuses a malformed posture for a known algorithm', () => {
  const r = checkOperationPolicy('Ed25519', { fips_status: 'active' } as unknown as FipsPosture);
  assert.equal(r.permitted, false);
  assert.equal(r.reason, FIPS_REASONS.MALFORMED_POSTURE);
});
...
test('the flag-set-but-provider-dead state is real, and maps to the named refusal', () => {
  ...
  let observed: { set: boolean; fips: number | null; sha: boolean };
  try {
  ...
  if (observed.sha === false) {
    ...
    assert.equal(
      checkOperationPolicy('Ed25519', observedPosture).reason,
      FIPS_REASONS.ED25519_UNAVAILABLE_IN_PROVIDER,
    );
  }
});
