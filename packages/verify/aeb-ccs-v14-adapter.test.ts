...
import {
  ...
  type CcsV14Receipt,
} from './src/aeb-ccs-v14-adapter.js';
import { canonicalizeFiniteJson } from './src/strict-json.js';
...
function sign(unsigned: Omit<CcsV14Receipt, 'signature'>): CcsV14Receipt {
  return {
    ...unsigned,
...
function mint(args: Record<string, unknown> = ARGS): CcsV14Artifact {
  const response = { issue_url: 'https://github.com/emiliaprotocol/emilia-protocol/issues/538', updated: true };
  const unsigned: Omit<CcsV14Receipt, 'signature'> = {
...
function fixture() {
  const config: CcsV14AebAdapterConfig = {
