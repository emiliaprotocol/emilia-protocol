...
export function verifyResolutionReceipt(receipt, opts = {}) {
  ...
  try {
    ...
    checks.outcome_shape = resolutionShapeValid(context.resolution, opts.bindingMoment, context.envelope_hash);
    ...
    checks.selected_option_binding = outcome !== 'approved'
      || (Number.isInteger(opts.expectedSelectedOption)
        && opts.expectedSelectedOption >= 0
        && context.resolution.selected_option === opts.expectedSelectedOption);

    const hasPinnedKey = isRecord(opts.principalKeys)
      && Object.hasOwn(opts.principalKeys, context.principal_key_id);
    const pin = hasPinnedKey ? opts.principalKeys[context.principal_key_id] : null;
    checks.principal_pin = isRecord(pin)
