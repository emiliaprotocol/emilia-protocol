...
// Generated from document-action-binding.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
...
import { DOCUMENT_ACTION_BINDING_DOMAIN, computeDocumentActionBindingDigest, computeDocumentSha256, computeReleaseActionDigest, signDocumentActionBinding, verifyDocumentActionBinding, } from './document-action-binding.js';
...
test('returns only the verified supersedes digest for amendment-chain joins', () => {
    ...
    assert.equal(verifyDocumentActionBinding(binding, options({
        expectedBindingId: spec.binding_id,
        expectedSupersedesDigest: `sha256:${'ef'.repeat(32)}`,
    })).reason, 'supersedes_digest_mismatch');
});
...
test('required-party omission refuses even under an otherwise valid pinned issuer signature', () => {
    ...
    omitted.required_parties = omitted.required_parties.filter((party) => party.party_id !== 'party:homeowner-jordan');
...
test('document, media type, action, and binding substitutions refuse', () => {
    ...
    assert.equal(verifyDocumentActionBinding(binding, options({ documentBytes: substitutedDocument })).reason, 'document_digest_mismatch');
    assert.equal(verifyDocumentActionBinding(binding, options({ documentMediaType: 'text/plain' })).reason, 'document_media_type_mismatch');
    assert.equal(verifyDocumentActionBinding(binding, options({ releaseActionTemplate: { ...ACTION, amount: '125001.00' } })).reason, 'action_digest_mismatch');
    assert.equal(verifyDocumentActionBinding(binding, options({ expectedBindingId: 'dab:other' })).reason, 'binding_id_mismatch');
});
test('unknown members and vocabulary, omitted terms, and duplicate signatures refuse', () => {
    ...
    assert.equal(verifyDocumentActionBinding(duplicateSignature, options()).reason, 'duplicate_issuer_signatures');
    assert.equal(verifyDocumentActionBinding(binding, options({ allowedPartyRoles: ['contractor'] })).reason, 'unknown_party_role');
    assert.equal(verifyDocumentActionBinding(binding, options({ allowedActionTypes: ['escrow.release'] })).reason, 'unknown_action_type');
    assert.equal(verifyDocumentActionBinding(binding, options({ requiredMaterialTermIds: ['effective_date', 'missing_term'] })).reason, 'required_material_term_missing');
...
test('stale windows and unpinned or attacker-signed issuers refuse', () => {
    assert.equal(verifyDocumentActionBinding(sign(), options({ now: '2026-09-01T00:00:00Z' })).reason, 'binding_expired');
    const attackerBinding = sign(baseSpec(), ATTACKER, 'issuer:attacker');
    assert.equal(verifyDocumentActionBinding(attackerBinding, options()).reason, 'issuer_key_not_pinned');
    assert.equal(verifyDocumentActionBinding(sign(), options({ issuerKeys: {} })).reason, 'issuer_key_not_pinned');
});
...
test('public schema exposes the join, byte length, single pinned issuer signature, and no acceptance claim', () => {
    const schema = JSON.parse(fs.readFileSync(new URL('../../public/schemas/ep-document-action-binding.schema.json', import.meta.url), 'utf8'));
    ...
    assert.match(schema.$defs.canonicalObject.propertyNames.not.pattern, /amount_minor/);
});
test('static real-crypto conformance vectors produce their declared verdicts', () => {
    const suite = JSON.parse(fs.readFileSync(new URL('../../conformance/vectors/document-action-binding.v1.json', import.meta.url), 'utf8'));
    ...
    for (const vector of suite.vectors) {
        ...
        const result = verifyDocumentActionBinding(suite.fixtures.bindings[vector.binding], vectorOptions);
