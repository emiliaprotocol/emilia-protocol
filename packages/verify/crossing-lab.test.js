...
runtimeTest('the replay probe catches an adapter that keys its unit on a wrapper field', () => {
    ...
    writeFileSync(adapterPath, readFileSync(adapterPath, 'utf8').replace("replay_unit: digest({ protocol: 'example-native', native_id: input.artifact.native_id }),", "replay_unit: digest({ protocol: 'example-native', native_id: input.artifact.native_id, presentation_id: input.artifact.presentation_id }),"));
    sealCrossingLab(root);
    const report = runCrossingLab(root);
    assert.equal(report.lab_passed, false);
    const rows = Object.fromEntries(report.adapter_rows.map((entry) => [entry.id, entry]));
...
runtimeTest('the replay probe catches an adapter that collapses distinct authorities', () => {
    const root = freshWorkspace();
    const adapterPath = join(root, 'adapter.mjs');
    writeFileSync(adapterPath, readFileSync(adapterPath, 'utf8').replace("replay_unit: digest({ protocol: 'example-native', native_id: input.artifact.native_id }),", "replay_unit: digest({ protocol: 'example-native' }),"));
    sealCrossingLab(root);
    const report = runCrossingLab(root);
    assert.equal(report.lab_passed, false);
    const rows = Object.fromEntries(report.adapter_rows.map((entry) => [entry.id, entry]));
...
runtimeTest('a vacuous replay probe is refused instead of passing', () => {
    for (const mutate of [
        (workspace) => { workspace.replay_probe.re_presented_artifact = structuredClone(workspace.replay_probe.distinct_authority_artifact); },
        (workspace) => { workspace.replay_probe.distinct_authority_artifact = structuredClone(workspace.replay_probe.re_presented_artifact); },
    ]) {
        const root = freshWorkspace();
        const workspace = readWorkspace(root);
        mutate(workspace);
