...
function ed25519FromSeed(seedHex: string) {
...
function build(receipt: unknown = RECEIPT) {
  const result = buildEpScittSignedStatement(receipt, {
    statementPrivateKey: STATEMENT_KEY.privateKey,
    kid: KID,
    iss: ISS,
  });
  assert.equal(result.ok, true, `build refused: ${(result as any).reason}`);
...
function must<T>(result: { ok: boolean; value?: T; reason?: string }, what: string): T {
...
function forge(
  ...
  signingKey: crypto.KeyObject = STATEMENT_KEY.privateKey,
  unprotected: Map<unknown, unknown> = new Map(),
): Uint8Array {
  const protectedBytes = must<Uint8Array>(encodeDeterministicCbor8949(protectedMap), 'protected');
  const sigStruct = must<Uint8Array>(
...
function baseProtected(): Map<unknown, unknown> {
...
function withClaims(sub: unknown, iss: unknown = ISS): Map<unknown, unknown> {
  const m = baseProtected();
  m.set(COSE_HEADER_CWT_CLAIMS, new Map<unknown, unknown>([[CWT_CLAIM_ISS, iss], [CWT_CLAIM_SUB, sub]]));
  return m;
}
...
test('statement is a tag-18 COSE_Sign1 with an empty unprotected bucket', () => {
  ...
  const decoded = must<unknown>(
    decodeDeterministicCbor8949(built.statement.subarray(1), { textKeysOnly: false }),
    'decode',
  );
  ...
  assert.equal((arr[1] as Map<unknown, unknown>).size, 0);
  // Attached payload, not detached: the receipt's own signature covers it.
  assert.equal(arr[2] instanceof Uint8Array, true);
  assert.deepEqual(Buffer.from(arr[2] as Uint8Array), Buffer.from(built.payload));
});

test('protected header carries exactly alg, content type, kid and CWT Claims', () => {
  const built = build();
  const headers = must<Map<unknown, unknown>>(
    decodeDeterministicCbor8949(built.protectedHeaderBytes, { textKeysOnly: false }) as any,
    'headers',
  );
  assert.deepEqual([...headers.keys()].sort((a: any, b: any) => a - b), [1, 3, 4, 15]);
  assert.equal(headers.get(1), COSE_ALG_EDDSA);
  assert.equal(headers.get(3), EP_STATEMENT_PAYLOAD_CONTENT_TYPE);
  assert.equal(Buffer.from(headers.get(4) as Uint8Array).toString('utf8'), KID);
  ...
  assert.equal((cwt as Map<unknown, unknown>).get(CWT_CLAIM_ISS), ISS);
  assert.equal((cwt as Map<unknown, unknown>).get(CWT_CLAIM_SUB), built.sub);
});
...
test('CWT Claims missing iss or sub is refused with the specific reason', () => {
  ...
  noIss.set(COSE_HEADER_CWT_CLAIMS, new Map<unknown, unknown>([[CWT_CLAIM_SUB, built.sub]]));
  assert.equal(verifyEpScittSignedStatement(forge(noIss, built.payload), PINS).reason, 'iss_missing');

  const noSub = baseProtected();
  noSub.set(COSE_HEADER_CWT_CLAIMS, new Map<unknown, unknown>([[CWT_CLAIM_ISS, ISS]]));
...
test('an unexpected claim inside CWT Claims is refused, not ignored', () => {
  ...
  (m.get(COSE_HEADER_CWT_CLAIMS) as Map<unknown, unknown>).set(6, 1755302400); // iat
  const result = verifyEpScittSignedStatement(forge(m, built.payload), PINS);
...
test('content in the unprotected bucket is refused', () => {
  ...
  const statement = forge(
    withClaims(built.sub),
    built.payload,
    STATEMENT_KEY.privateKey,
    new Map<unknown, unknown>([[394, [new Uint8Array([0xd2])]]]),
  );
  const result = verifyEpScittSignedStatement(statement, PINS);
  assert.equal(result.valid, false);
...
test('a malformed sub or iss is refused before any binding work', () => {
  const built = build();
  assert.equal(
    verifyEpScittSignedStatement(forge(withClaims('not-a-caid'), built.payload), PINS).reason,
    'sub_malformed',
  );
  assert.equal(
    verifyEpScittSignedStatement(forge(withClaims(built.sub, 'no-scheme'), built.payload), PINS).reason,
    'iss_malformed',
  );
  assert.equal(
    verifyEpScittSignedStatement(forge(withClaims(built.sub, 42), built.payload), PINS).reason,
    'iss_malformed',
  );
});
...
test('caller pins on iss and sub are enforced', () => {
  const built = build();
  assert.equal(
    verifyEpScittSignedStatement(built.statement, { ...PINS, expectedIss: 'ep:issuer:other' }).reason,
    'iss_mismatch',
  );
  assert.equal(
  ...
  assert.equal(
    verifyEpScittSignedStatement(built.statement, { ...PINS, expectedIss: ISS, expectedSub: built.sub }).valid,
    true,
  );
});
...
test('build refuses bad inputs with named reasons and never throws', () => {
  ...
  const cases: Array<[unknown, any, string]> = [
...
test('verify refuses hostile bytes with a reason instead of throwing', () => {
  ...
  for (let i = 0; i < built.statement.length; i += 7) {
    ...
    if (result.valid) assert.fail(`corruption at byte ${i} verified`);
    assert.equal(typeof result.reason, 'string');
  }
...
test('registration request is described, not performed', () => {
  const built = build();
  const request = must<any>(
    describeScittRegistrationRequest(built.statement, 'https://example.invalid/entries'),
    'describe',
  );
  ...
  assert.equal(
    request.bodySha256,
    crypto.createHash('sha256').update(built.statement).digest('hex'),
  );
  // Non-https and non-string endpoints refuse.
  assert.equal((describeScittRegistrationRequest(built.statement, 'http://x/y') as any).ok, false);
  assert.equal((describeScittRegistrationRequest(built.statement, '') as any).ok, false);
});
...
test('conformance/scitt-statement/vectors.json replays exactly', () => {
  ...
  for (const vector of suite.vectors) {
    ...
    if (vector.expect.reason) assert.equal(result.reason, vector.expect.reason, vector.id);
    if (vector.expect.iss) assert.equal(result.iss, vector.expect.iss, vector.id);
    if (vector.expect.sub) assert.equal(result.sub, vector.expect.sub, vector.id);
    for (const [name, want] of Object.entries(vector.expect.checks ?? {})) {
      assert.equal((result.checks as any)[name], want, `${vector.id}: check ${name}`);
    }
  }
  ...
  const caid = must<{ caid: string }>(receiptActionCaid(suite.receipt.payload.action), 'caid');
  assert.equal(suite.expected.sub, caid.caid);
  assert.equal(
    suite.expected.payload_sha256,
    crypto.createHash('sha256').update(Buffer.from(suite.receipt_canonical_json, 'utf8')).digest('hex'),
  );
});
