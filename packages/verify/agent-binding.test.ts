...
test('fresh within the window', () => {
  const r = evaluateAgentBinding(
    ...
    { maxAgeSec: 600, at: '2026-06-24T18:05:00Z' },
  );
...
test('stale beyond the window -> fresh:false', () => {
  const r = evaluateAgentBinding(
    ctx({ agent_id: 'a', delegation: { scheme: 'WIMSE', ref: 'x', observed_at: '2026-06-24T18:00:00Z' } }),
    { maxAgeSec: 60, at: '2026-06-24T18:05:00Z' },
  );
...
test('freshness required but no observed_at -> fail-closed', () => {
  const r = evaluateAgentBinding(
    ctx({ agent_id: 'a', delegation: { scheme: 'WIMSE', ref: 'x' } }),
    { maxAgeSec: 600 },
  );
  assert.equal(r.fresh, false);
...
test('observed_at in the future -> not fresh', () => {
  const r = evaluateAgentBinding(
    ctx({ agent_id: 'a', delegation: { scheme: 'WIMSE', ref: 'x', observed_at: '2026-06-24T18:10:00Z' } }),
    { maxAgeSec: 600, at: '2026-06-24T18:00:00Z' },
  );
  assert.equal(r.fresh, false);
