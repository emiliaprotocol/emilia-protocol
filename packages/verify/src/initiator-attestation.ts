...
export function neutralizeStatement(statement: unknown): {
  safe: string; changed: boolean; homoglyph_risk: boolean; escaped_codepoints: number[]; truncated: boolean;
} {
 ...
 */
export function validateInitiatorAttestation(att: unknown): {
  ok: boolean; normalized: Record<string, unknown> | null; errors: string[]; statement_report: ReturnType<typeof neutralizeStatement> | null;
} {
  const errors: string[] = [];
  ...
  if (a['@version'] !== undefined && a['@version'] !== INITIATOR_ATTESTATION_VERSION) {
 ...
 */
export function bindInto(action: Record<string, unknown> | null | undefined, att: unknown) {
