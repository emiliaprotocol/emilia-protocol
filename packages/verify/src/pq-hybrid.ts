 ...
 */
export const HYBRID_SIGNATURE_ALGOS = Object.freeze(['Ed25519', 'ML-DSA-65'] as const);
 ...
 * @returns {Buffer}
 */
export function hybridSigningInput(messageBytes: Uint8Array, signatureAlgos: readonly string[]): Buffer {
 ...
 */
export async function loadDefaultMldsaBackend(): Promise<MldsaBackend | null> {
  try {
    const mod = await import('@noble/post-quantum/ml-dsa.js');
    const impl = (mod as { ml_dsa65?: MldsaBackend }).ml_dsa65;
    ...
    return {
      ...
      sign: (messageBytes: Uint8Array, secretKeyBytes: Uint8Array) => impl.sign!(messageBytes, secretKeyBytes),
      verify: (signatureBytes: Uint8Array, messageBytes: Uint8Array, publicKeyBytes: Uint8Array) => {
        try {
...
async function resolveBackend(
  mldsaBackend: MldsaBackend | null | undefined,
  mldsaBackendLoader: HybridOptions['mldsaBackendLoader'],
): Promise<MldsaBackend | null> {
...
function toRawBytes(key: unknown): Uint8Array | null {
...
function algosEqual(a: unknown, b: readonly string[]): a is string[] {
...
export async function verifyHybrid(
  ...
  const checks: HybridChecks = { envelope: false, algo_set: false, classical_signature: null, pq_signature: null };
  const refuse = (reason: string): HybridVerifyResult => ({ verified: false, reason, checks });
  ...
  const e = envelope as Partial<HybridEnvelope>;
  if (e.alg !== HYBRID_ALG) return refuse(REASONS.INVALID_ENVELOPE);
  if (!e.sigs || typeof e.sigs !== 'object' || Array.isArray(e.sigs)) return refuse(REASONS.INVALID_ENVELOPE);
  ...
  const sigs = e.sigs as Record<string, unknown>;
  ...
  const backend = await resolveBackend(options.mldsaBackend, options.mldsaBackendLoader);
  if (!backend || typeof backend.verify !== 'function') return refuse(REASONS.PQ_BACKEND_UNAVAILABLE);
  let pqOk = false;
  try {
    pqOk = backend.verify(
