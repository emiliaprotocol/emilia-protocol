...
function canonicalCompactJwt(value: string): boolean {
...
function canonicalSingleSdJwtToken(value: unknown, name: string): Buffer {
...
function sourceBytesDigest(label: string, value: unknown): AebDigest {
  const bytes = canonicalSingleSdJwtToken(value, label);
  return domainBytesDigest(label, bytes);
}

function authenticatedCheckoutHashAlgorithm(
  token: unknown,
): FidoAp2NativeSourceBinding['checkout_hash_algorithm'] {
  ...
  try {
    ...
    if (!strictJsonGate(text).ok) throw new TypeError('invalid issuer payload');
  ...
  return algorithm as FidoAp2NativeSourceBinding['checkout_hash_algorithm'];
}

function domainBytesDigest(label: string, bytes: Buffer): AebDigest {
 ...
 */
export function createFidoAp2NativeSourceBinding(
  input: FidoAp2NativeSourceBindingInput,
): FidoAp2NativeSourceBinding {
  const expectedKeys = [
  ...
  const base: Omit<FidoAp2NativeSourceBinding, 'native_artifact_digest'> = {
    source_revision: FIDO_AP2_SOURCE_REVISION,
    checkout_mandate_token_digest: sourceBytesDigest(
      'checkout-mandate-token',
      values.get('checkout_mandate_token'),
    ),
    payment_mandate_token_digest: sourceBytesDigest(
      'payment-mandate-token',
      values.get('payment_mandate_token'),
    ),
    checkout_mandate_payload_digest: digestAeb(checkoutPayload),
    payment_mandate_payload_digest: digestAeb(paymentPayload),
    checkout_hash_algorithm: authenticatedCheckoutHashAlgorithm(
      values.get('checkout_mandate_token'),
    ),
  };
...
interface PinnedConfig {
  ...
  audience: string;
  action_type: typeof FIDO_AP2_ACTION_TYPE;
  rp_id: string;
...
function parseInstant(value: unknown): number {
  ...
  if (date.toISOString().slice(0, 19) !== `${year}-${month}-${day}T${hour}:${minute}:${second}`) return NaN;
  return date.getTime();
}

function isStrictJsonValue(value: unknown, seen = new WeakSet<object>(), depth = 0): boolean {
    ...
    if (Array.isArray(value)) {
      ...
      if (ownKeys.some((key) => key !== 'length'
          && (!/^(0|[1-9][0-9]*)$/.test(String(key))
            || Number(key) >= value.length))) return false;
      for (let index = 0; index < value.length; index += 1) {
        const descriptor = Object.getOwnPropertyDescriptor(value, String(index));
        if (!descriptor || !Object.hasOwn(descriptor, 'value')
            || !isStrictJsonValue(descriptor.value, seen, depth + 1)) return false;
      }
      return true;
    }
    if (!isRecord(value)) return false;
    for (const key of Reflect.ownKeys(value)) {
      if (typeof key !== 'string' || !wellFormedUnicode(key)) return false;
...
function decodeBase64url(value: unknown, maxDecodedBytes = 64 * 1024): Buffer | null {
...
function ap2HashB64url(
  algorithm: FidoAp2NativeSourceBinding['checkout_hash_algorithm'],
  value: string,
): string {
...
function digestOptional(value: unknown): AebDigest {
...
function parseCheckoutMandate(
  ...
  if (!isRecord(value) || !exactKeys(value, CHECKOUT_MANDATE_KEYS)
      ...
      || Number(value.iat) >= Number(value.exp)) return null;
  return value;
}

function parsePaymentMandate(value: unknown): Obj | null {
...
function parseSourceBinding(value: unknown): FidoAp2NativeSourceBinding | null {
  if (!isRecord(value) || !exactKeys(value, new Set([
    ...
    'native_artifact_digest',
      ...
      || !validDigest(value.native_artifact_digest)) return null;
  const base = {
    source_revision: FIDO_AP2_SOURCE_REVISION,
  ...
  return digestAeb(base) === value.native_artifact_digest
    ? value as FidoAp2NativeSourceBinding : null;
}

function parseAp2(value: unknown): ParsedAp2 | null {
  if (!isStrictJsonValue(value) || !isRecord(value)
      || !exactKeys(value, new Set(['checkout_mandate', 'payment_mandate', 'source_binding']))) return null;
  const source = parseSourceBinding(value.source_binding);
  if (!source) return null;
  const checkout = parseCheckoutMandate(
    value.checkout_mandate,
    source.checkout_hash_algorithm,
  );
  const payment = parsePaymentMandate(value.payment_mandate);
  if (!checkout || !payment || payment.transaction_id !== checkout.checkout_hash) return null;
  ...
  return {
    action: {
      ...
      checkout_payload_jwt_digest: domainBytesDigest(
        'checkout-payload-jwt',
        Buffer.from(checkout.checkout_jwt, 'ascii'),
      ),
...
function validAction(value: unknown): value is Obj {
...
export function projectFidoAp2PaymentAction(input: unknown): Obj {
  const parsed = parseAp2(input);
  if (!parsed) throw new TypeError('unsupported or inconsistent closed AP2 payment mandate');
  return { ...parsed.action };
}

function parseConfig(value: unknown): PinnedConfig | null {
  if (!isStrictJsonValue(value) || !isRecord(value) || !exactKeys(value, CONFIG_KEYS)
      ...
      || !value.allowed_origins.every((origin: string) => {
...
function p256PublicKey(value: unknown): { encoded: string; key: KeyObject } | null {
  ...
  try {
    ...
    if (!Buffer.isBuffer(canonical) || !canonical.equals(der) || key.type !== 'public'
        ...
        || !nonEmptyString(jwk.x) || !nonEmptyString(jwk.y)) return null;
    return { encoded: value as string, key };
  } catch {
    return null;
  }
}

function parseRoot(value: unknown, config: PinnedConfig): PinnedRoot | null {
  if (!isStrictJsonValue(value) || !isRecord(value) || !exactKeys(value, ROOT_KEYS)
      ...
      || value.sign_count > 0xffffffff) return null;
...
function parseSourceCommitments(value: unknown): Obj | null {
  if (!isRecord(value) || !exactKeys(value, SOURCE_COMMITMENT_KEYS)
      || !validDigest(value.checkout_mandate_token_digest)
      || !validDigest(value.payment_mandate_token_digest)
      || !validDigest(value.native_verification_attestation_digest)) return null;
  return value;
}

function parseProvider(value: unknown): Obj | null {
  if (!isRecord(value) || !exactKeys(value, PROVIDER_KEYS)
      || !nonEmptyString(value.provider_id) || !nonEmptyString(value.account_id)
      || !nonEmptyString(value.environment)) return null;
  return value;
}

function parseContext(value: unknown): Obj | null {
  if (!isRecord(value) || !exactKeys(value, CONTEXT_KEYS)
      ...
      || !nonEmptyString(value.nonce) || !Number.isFinite(parseInstant(value.expires_at))) return null;
  return value;
}

function parseDisclosure(value: unknown): Obj | null {
  if (!isRecord(value) || !exactKeys(value, DISCLOSURE_KEYS)
      ...
      || !Number.isFinite(parseInstant(value.source_expires_at))) return null;
  return value;
}

function parseArtifact(value: unknown): Obj | null {
  if (!isStrictJsonValue(value) || !isRecord(value) || !exactKeys(value, ARTIFACT_KEYS)
      ...
      || decodeBase64url(value.signoff.webauthn.signature, MAX_SIGNATURE_BYTES) === null) return null;
  return value;
}

function statusDisposition(status: AebStatusInput, now: string, maxAgeSeconds: number): StatusDisposition {
  ...
  if (typeof status.revocation_checked !== 'boolean' || typeof status.revoked !== 'boolean'
      ...
      || (status.unavailable !== undefined && typeof status.unavailable !== 'boolean')) reasons.push('status_malformed');
  ...
  if (!Number.isFinite(nowMs) || !Number.isFinite(checkedMs) || !Number.isFinite(expiresMs)) {
    ...
    if (ageSeconds < 0 || ageSeconds > maxAgeSeconds) reasons.push('status_stale');
  }
...
function verifyWebAuthnAssertion(
  ...
  config: PinnedConfig,
  root: PinnedRoot,
): { verified: boolean; reasons: string[]; sign_count: number | null } {
  const reasons: string[] = [];
  const webauthn = artifact.signoff.webauthn;
  if (webauthn.credential_id !== root.credential_id) reasons.push('credential_id_mismatch');
  ...
  try {
    ...
    if (strictJsonGate(text).ok) {
      ...
      if (isStrictJsonValue(parsed) && isRecord(parsed)
          && exactKeys(parsed, CLIENT_DATA_KEYS, new Set(['crossOrigin']))) clientData = parsed;
    }
  } catch {
    clientData = null;
  }
  ...
  if (clientData.type !== 'webauthn.get') reasons.push('client_data_type_invalid');
  if (clientData.challenge !== expectedChallenge) reasons.push('challenge_binding_mismatch');
  ...
  if (authData.length !== 37) reasons.push('authenticator_data_not_closed_v1');
  if (authData.length < 37) return { verified: false, reasons: [...new Set(reasons)].sort(), sign_count: null };
  const expectedRpIdHash = crypto.createHash('sha256').update(config.rp_id, 'utf8').digest();
  if (!authData.subarray(0, 32).equals(expectedRpIdHash)) reasons.push('rp_id_hash_mismatch');
  const flags = authData[32];
  if ((flags & ~ALLOWED_ASSERTION_FLAGS) !== 0) reasons.push('authenticator_flags_unsupported');
  if ((flags & FLAG_UP) !== FLAG_UP) reasons.push('user_presence_required');
  if ((flags & FLAG_UV) !== FLAG_UV) reasons.push('user_verification_required');
  if ((flags & FLAG_BS) === FLAG_BS && (flags & FLAG_BE) !== FLAG_BE) reasons.push('backup_state_without_eligibility');
  if (!config.allow_backup_eligible && (flags & FLAG_BE) === FLAG_BE) reasons.push('backup_eligible_refused');
  if (!config.allow_backup_state && (flags & FLAG_BS) === FLAG_BS) reasons.push('backup_state_refused');
  ...
  if (!signatureValid) reasons.push('webauthn_signature_invalid');
...
function caidForAction(action: Obj): { caid: string; digest: AebDigest } | null {
  let result: unknown;
  try {
  ...
  if (!isRecord(result) || !nonEmptyString(result.caid) || !validDigest(result.digest)) return null;
  return { caid: result.caid, digest: result.digest };
}

function exactDisclosure(disclosure: Obj, action: Obj): boolean {
...
function exactSourceCommitments(left: Obj, right: Obj): boolean {
  return [...SOURCE_COMMITMENT_KEYS].every((key) => left[key] === right[key]);
}

function exactProvider(left: Obj, right: Obj): boolean {
  return [...PROVIDER_KEYS].every((key) => left[key] === right[key]);
}

function verifyNative(input: Omit<AebAdapterInput, 'profile'>): AebNativeResult {
  ...
  if (artifact.source_revision !== FIDO_AP2_SOURCE_REVISION
      || context.source_revision !== FIDO_AP2_SOURCE_REVISION) reasons.push('source_revision_mismatch');
  ...
  if (artifact.relying_party_id !== config.relying_party_id
      || context.relying_party_id !== config.relying_party_id) reasons.push('relying_party_id_mismatch');
  if (artifact.audience !== config.audience || context.audience !== config.audience) reasons.push('audience_mismatch');
  if (artifact.operation_id !== context.operation_id) reasons.push('operation_id_mismatch');
  if (artifact.effect_request_digest !== context.effect_request_digest) {
    reasons.push('effect_request_digest_mismatch');
  }
  if (!exactProvider(artifact.provider, context.provider)) reasons.push('provider_mismatch');
  if (context.approver_id !== config.subject.id) reasons.push('approver_mismatch');
  if (context.nonce !== config.expected_nonce) reasons.push('nonce_mismatch');
  ...
  if (actionDigest === null || context.normalized_action_digest !== actionDigest) reasons.push('normalized_action_digest_mismatch');
  if (!caid || caid.digest !== actionDigest || context.caid !== caid.caid) reasons.push('caid_mismatch');
  if (disclosureDigest === null || context.disclosure_digest !== disclosureDigest
      || !exactDisclosure(artifact.disclosure, artifact.normalized_action)) reasons.push('disclosure_mismatch');
  ...
  if (!Number.isFinite(nowMs) || nowMs >= contextExpiry) reasons.push('authorization_context_expired');
  if (!Number.isFinite(sourceExpiry) || nowMs >= sourceExpiry
      || contextExpiry > sourceExpiry) reasons.push('ap2_source_expiry_mismatch');
  if (reasons.length > 0) {
    base.acceptance = 'REJECTED';
...
export function createFidoAp2PinnedProfile(): AebPinnedProfile {
  const profile: AebPinnedProfile = {
    ...
    profile_digest: ('sha256:' + '0'.repeat(64)) as AebDigest,
  };
  profile.profile_digest = mappingProfileDigest(FIDO_AP2_CAID_PROFILE_ID, profile);
  return deepFreeze(profile);
}

function exactProfile(value: unknown): value is AebPinnedProfile {
  if (!isStrictJsonValue(value) || !isRecord(value) || !exactKeys(value, PROFILE_KEYS)
      || !isRecord(value.definition) || !exactKeys(value.definition, PROFILE_DEFINITION_KEYS)) return false;
  try {
...
function mapAction(input: AebAdapterInput & { native: AebNativeResult }): AebMappingResult {
...
export function createFidoAp2AebAdapter(): AebAdapter {
  return Object.freeze({
