...
if (args[0] === 'reliance-gap') {
  ...
  let packet;
  try { packet = load(packetPath); } catch (err: any) {
  ...
  try {
    if (profilePath) {
      ...
      const names = readdirSync(profilesDir as string).filter((f: string) => f.endsWith('.json')).sort();
      if (names.length === 0) {
      ...
      const profiles = names.map((name) => ({
        label: name,
        profile: load(join(profilesDir as string, name)),
        path: join(profilesDir as string, name),
      }));
  ...
  const human = [];
  const summarize = (label: string, r: Obj): void => {
  ...
  if (profilePath) {
    ...
    human.push(`${report.all_rely ? 'ALL RELY' : 'GAPS FOUND'}: ${report.summary.filter((s: any) => s.verdict === 'rely').length}/${report.profiles_evaluated} profiles rely on this packet`);
...
if (args[0] === 'aeb-conformance') {
  ...
  } = await import('./aeb-consequence-conformance.js');
  ...
  for (let index = 0; index < sub.length; index += 1) {
    if (sub[index] === '--reference') reference = true;
    else if (sub[index] === '--submission') submissionPath = sub[++index] ?? null;
    else if (sub[index] === '--out') outPath = sub[++index] ?? null;
    else invalid = true;
  }
...
let suppliedRpId: string | null = null;
...
if (delegationKeysPath) {
  ...
  catch (err: any) { console.error(`✕ --delegation-keys not readable JSON (${err.message})`); process.exit(1); }
}

// PIP-007 §2 advisory: print the attestation report when a result carries one.
function printAttestationAdvisory(attestation: Obj): void {
...
function findKey(doc: any, names: string[]): string | null {
...
function printChecks(checks: Obj): void {
