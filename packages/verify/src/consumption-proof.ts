...
interface ConsumptionBundle {
  nonce?: unknown;
  non_inclusion_proof?: SubProof;
  inclusion_proof?: SubProof;
  consistency_proof?: unknown;
  checkpoints?: { h1?: Checkpoint; h2?: Checkpoint };
}
...
function isHex64(h: unknown): h is string {
...
function presentLeaf(keyHex: string, valueHex: string): string {
  return crypto
    .createHash('sha256')
...
function nonceKeyHex(nonce: string): string {
...
function pathBit(keyHex: string, i: number): number {
  const byteIndex = i >> 3;
 ...
 * @returns {string|null}
 */
function foldToRoot(leafHex: string, siblings: unknown, keyHex: string, depth: number): string | null {
 ...
 */
function checkSub(sub: unknown, keyHex: string, label: string): { ok: boolean; reason?: string } {
  if (!sub || typeof sub !== 'object') return { ok: false, reason: `${label}_missing` };
  const proof = sub as SubProof;
  const root = hexOf(proof.root);
 ...
 */
export function verifyConsumptionProof(bundle: unknown): ConsumptionResult {
  const checks: ConsumptionChecks = { non_inclusion: false, inclusion: false, consistency: false };
  const fail = (reason: string): ConsumptionResult => ({ valid: false, checks, reason });

  if (!bundle || typeof bundle !== 'object') return fail('bundle_missing');
  const b = bundle as ConsumptionBundle;
  if (typeof b.nonce !== 'string' || b.nonce.length === 0) return fail('nonce_missing');
  ...
  if (!niRes.ok) return fail(niRes.reason ?? 'non_inclusion_invalid');
  ...
  if (!incRes.ok) return fail(incRes.reason ?? 'inclusion_invalid');
  ...
  if (typeof h1Size !== 'number' || !Number.isInteger(h1Size) || h1Size < 1 || !isHex64(h1Root)) return fail('checkpoint_h1_malformed');
  if (typeof h2Size !== 'number' || !Number.isInteger(h2Size) || h2Size < 1 || !isHex64(h2Root)) return fail('checkpoint_h2_malformed');
  ...
  if (!Array.isArray(b.consistency_proof)) return fail('consistency_proof_missing');
...
function buildEmptyLevels(depth: number): string[] {
...
export class ReferenceConsumptionTree {
  ...
  insert(nonce: string, value?: unknown): { keyHex: string; valueHex: string } {
