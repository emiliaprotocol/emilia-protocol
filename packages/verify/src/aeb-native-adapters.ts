...
import {
  ...
  inspectOrprgJsonJcsPermit,
} from './orprg.js';

type Obj = Record<string, any>;
type AgentRole = 'roa' | 'ara' | 'aer';
...
interface PinnedSubject extends AebEvidenceSubject {
  native_id: string;
}

interface AgentConfig {
  evidence_role: string;
  subject: PinnedSubject & { kind: 'workload' };
  action_type: string;
...
interface OrprgConfig {
  evidence_role: string;
  subject: PinnedSubject & { kind: 'organization' | 'system' };
  action_type: string;
  expected_policy_digest: string;
  expected_epoch: string | number;
...
interface OrprgInspection {
  native_result: Obj;
  pre_replay_verified: boolean;
  /** True only for a native API that completes verification without consume. */
  non_mutating_verification_complete: boolean;
  replay_key: string | null;
  replay_context: Obj | null;
}
...
function validPinnedSubject(value: unknown, kinds: ReadonlySet<string>): value is PinnedSubject {
...
function validEd25519Spki(value: unknown): value is string {
  ...
  try {
    ...
    if (der.length === 0 || der.toString('base64url') !== value) return false;
...
function inputStatusDigest(status: AebStatusInput): AebDigest {
  return safeDigest({
...
function fallbackNative(
  ...
  role = 'invalid-evidence',
  subject: AebEvidenceSubject = { id: 'invalid-evidence', kind: 'system' },
): AebNativeResult {
...
function parseAgentConfig(value: unknown): AgentConfig | null {
  if (!isRecord(value) || !exactKeys(value, AGENT_CONFIG_KEYS)
      ...
      || !isRecord(value.policy) || !exactKeys(value.policy, AGENT_POLICY_KEYS)) return null;
  ...
  if (!nonEmptyString(policy.expected_policy_id)
      ...
      || !isRecord(policy.capability_manifest)) return null;
  const capabilityManifest: Record<string, string[]> = Object.create(null);
  for (const [wildcard, capabilities] of Object.entries(policy.capability_manifest)) {
    if (!nonEmptyString(wildcard) || !uniqueStrings(capabilities)) return null;
...
function agentPins(trustRoots: readonly unknown[]): Record<AgentRole, Record<string, string>> | null {
  if (!Array.isArray(trustRoots) || trustRoots.length === 0) return null;
  const pins: Record<AgentRole, Record<string, string>> = {
  ...
  for (const root of trustRoots) {
    if (!isRecord(root) || !exactKeys(root, AGENT_ROOT_KEYS)
        ...
        || !nonEmptyString(root.signer_id) || !validEd25519Spki(root.public_key)) return null;
    const role = root.role as AgentRole;
    if (Object.hasOwn(pins[role], root.signer_id)) return null;
    pins[role][root.signer_id] = root.public_key;
  }
  if (Object.keys(pins.roa).length === 0 || Object.keys(pins.aer).length === 0) return null;
  return pins;
}

function stripPinnedActionType(value: unknown, actionType: string): Obj | null {
  if (!isRecord(value) || value.action_type !== actionType) return null;
  const native: Obj = {};
  for (const [key, item] of Object.entries(value)) {
    if (key !== 'action_type') native[key] = item;
  }
  return native;
}

function agentReplayUnit(artifact: unknown, fallback: AebDigest): AebDigest {
  if (!isRecord(artifact) || !Array.isArray(artifact.chain) || !isRecord(artifact.chain[0])
      ...
      || !nonEmptyString(artifact.aer.session.session_id)) return fallback;
  return safeDigest({
...
function agentFailureAcceptance(reason: unknown): Acceptance {
...
function verifyAgentNative(input: Omit<AebAdapterInput, 'profile'>): AebNativeResult {
  const config = parseAgentConfig(input?.adapter_config);
  const base = fallbackNative(
    input,
    config?.evidence_role,
    config ? { id: config.subject.id, kind: config.subject.kind } : undefined,
  );
  ...
  const result = verifyAgentROA(input.artifact as Obj, {
...
function parseOrprgConfig(value: unknown): OrprgConfig | null {
  if (!isRecord(value) || !exactKeys(value, ORPRG_CONFIG_KEYS)
      ...
      || value.native_replay_phase !== 'inspection-only') return null;
  return {
    evidence_role: value.evidence_role,
    subject: {
      id: value.subject.id,
...
function orprgIssuerKeys(trustRoots: readonly unknown[]): Obj | null {
  if (!Array.isArray(trustRoots) || trustRoots.length === 0) return null;
  const issuers: Obj = Object.create(null);
  for (const root of trustRoots) {
    if (!isRecord(root) || !exactKeys(root, ORPRG_ROOT_KEYS)
        ...
        || !validEd25519Spki(root.public_key)) return null;
    if (!Object.hasOwn(issuers, root.issuer_id)) issuers[root.issuer_id] = Object.create(null);
    if (Object.hasOwn(issuers[root.issuer_id], root.key_id)) return null;
    issuers[root.issuer_id][root.key_id] = root.public_key;
  }
  return issuers;
}

function orprgFailureAcceptance(code: unknown): Acceptance {
  return ['AMBIGUOUS_CONTEXT', 'REVOCATION_UNKNOWN_OR_STALE', 'ANTI_REPLAY_FAILURE']
    .includes(String(code)) ? 'INDETERMINATE' : 'REJECTED';
}

function nativeInspectionPassed(result: Obj): boolean {
 ...
 */
function inspectOrprgForAeb(input: unknown, options: Obj): OrprgInspection {
...
function verifyOrprgNative(input: Omit<AebAdapterInput, 'profile'>): AebNativeResult {
  ...
  if (inspection.replay_key !== null) {
    base.replay_unit = inspection.replay_key.slice('orprg-replay:'.length) as AebDigest;
  }
...
function parseMappingProfile(
  profile: AebPinnedProfile,
  nativeProtocol: string,
  configActionType: string,
): { mapping: MappingDefinition | null; reasons: string[] } {
  const reasons: string[] = [];
  if (!isRecord(profile)
  ...
  if (definition.native_protocol !== nativeProtocol) reasons.push('mapping_profile_protocol_mismatch');
  if (definition.action_type !== configActionType
      || typeof definition.action_type !== 'string'
      || !ACTION_TYPE.test(definition.action_type)) reasons.push('mapping_profile_action_type_mismatch');
  const matchingDefinitions = definition.definitions.filter((candidate: unknown) => (
    isRecord(candidate) && candidate.action_type === definition.action_type
  ));
  if (matchingDefinitions.length !== 1) reasons.push('mapping_profile_definition_ambiguous');
  if (reasons.length > 0) return { mapping: null, reasons: [...new Set(reasons)].sort() };
  return {
    mapping: {
...
function mapExactNativeAction(
  input: AebAdapterInput & { native: AebNativeResult },
  nativeProtocol: string,
  configActionType: string | null,
  signedNativeAction: unknown,
): AebMappingResult {
  ...
  if (!isRecord(computed) || typeof computed.caid !== 'string'
    ...
    return {
      mapping: 'INDETERMINATE', caid: null, action_digest: null,
      reasons: refusals.map((reason: string) => `caid:${reason}`),
    };
  }
...
export function createAgentRoaAebAdapter(): AebAdapter {
 ...
 */
export function createOrprgAebAdapter(): AebAdapter {
  return Object.freeze({
