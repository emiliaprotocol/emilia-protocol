...
export const PROMPTFOO_QUALIFICATION_ADAPTER_VERSION =
  'EP-GATE-QUALIFICATION-PROMPTFOO-ADAPTER-v1';
export const PROMPTFOO_QUALIFICATION_RUN_METADATA_VERSION =
  'EP-GATE-QUALIFICATION-PROMPTFOO-RUN-v1';
export const PROMPTFOO_QUALIFICATION_EVIDENCE_VERSION =
  'EP-GATE-QUALIFICATION-EVALUATION-EVIDENCE-v2';
...
export type PromptfooQualificationStatus =
  | 'PASS'
  | 'FAIL'
  | 'ERROR'
  | 'ABORTED'
  | 'EXPIRED';
export type PromptfooQualificationJson =
  | null
  | boolean
  | string
  | number
  | PromptfooQualificationJson[]
...
export interface PromptfooQualificationCandidatePin
  extends PromptfooQualificationManifestPin {
  manifest: {
    provider_id: string;
...
export interface PromptfooQualificationHarnessPin
  extends PromptfooQualificationManifestPin {
...
export interface PromptfooQualificationCampaignPin
  extends PromptfooQualificationManifestPin {
  manifest: {
...
interface NormalizedAttempt {
  evidence: PromptfooQualificationAttemptEvidence;
  status: PromptfooQualificationStatus;
  tokenUsage: PromptfooTokenUsage;
  score: number;
...
const MUTABLE_ALIAS_RE =
  /(?:^|[/:@._-])(?:latest|current|default|stable|head|main|master|nightly|canary)(?:$|[/:@._-])/i;
...
const STATUSES = new Set<PromptfooQualificationStatus>([
  'PASS',
  'FAIL',
  'ERROR',
  'ABORTED',
...
type PromptfooQualificationInputLimitReason =
  | 'input_bytes_limit_exceeded'
  | 'input_nodes_limit_exceeded'
  | 'input_depth_limit_exceeded'
  | 'summary_results_limit_exceeded'
  | 'input_not_inspectable';

function dataPropertyValue(
  value: Obj,
  key: string,
): { inspectable: true; value: unknown } | { inspectable: false } {
  const descriptor = Object.getOwnPropertyDescriptor(value, key);
  if (descriptor === undefined) return { inspectable: true, value: undefined };
  if (!Object.hasOwn(descriptor, 'value')) return { inspectable: false };
  return { inspectable: true, value: descriptor.value };
}

function summaryResultsLimitReason(
  value: unknown,
): PromptfooQualificationInputLimitReason | null {
  try {
    if (!isObject(value)) return null;
    const artifactProperty = dataPropertyValue(value, 'artifact');
    if (!artifactProperty.inspectable || !isObject(artifactProperty.value)) return null;
    const resultsProperty = dataPropertyValue(artifactProperty.value, 'results');
    if (!resultsProperty.inspectable || !isObject(resultsProperty.value)) return null;
    const summaryResultsProperty = dataPropertyValue(resultsProperty.value, 'results');
    if (!summaryResultsProperty.inspectable) return null;
...
function aggregateInputLimitReason(
  value: unknown,
): PromptfooQualificationInputLimitReason | null {
  ...
  try {
    while (stack.length > 0) {
      ...
      if (current.value === null) {
        if (addBytes(4)) return 'input_bytes_limit_exceeded';
        continue;
      }
      if (typeof current.value === 'boolean') {
        if (addBytes(current.value ? 4 : 5)) return 'input_bytes_limit_exceeded';
        continue;
      }
      ...
      if (Array.isArray(current.value)) {
        ...
        for (let index = current.value.length - 1; index >= 0; index -= 1) {
          const child = dataPropertyValue(current.value as unknown as Obj, String(index));
          if (!child.inspectable) return 'input_not_inspectable';
...
function adapterInputLimitReason(
  value: unknown,
): PromptfooQualificationInputLimitReason | null {
...
function canonicalize(
  ...
  if (typeof value === 'string') {
    if (!validUnicode(value)) throw new TypeError('invalid Unicode scalar value');
  ...
  if (Array.isArray(value)) {
    ...
    if (!isObject(value)) throw new TypeError('non-plain object');
    const ownKeys = Reflect.ownKeys(value);
    if (ownKeys.some((key) => typeof key !== 'string')) {
    ...
    output = `{${keys.map((key) => {
      const descriptor = Object.getOwnPropertyDescriptor(value, key)!;
      return `${JSON.stringify(key)}:${canonicalize((descriptor as PropertyDescriptor & { value: unknown }).value, seen)}`;
    }).join(',')}}`;
  }
  seen.delete(value);
  return output;
 ...
 * commonly fractional.
 */
export function digestPromptfooQualification(
  value: unknown,
): PromptfooQualificationDigest {
  const limitReason = aggregateInputLimitReason(value);
  if (limitReason !== null) throw new TypeError(limitReason);
  const bytes = Buffer.from(canonicalize(value), 'utf8');
  return `sha256:${crypto.createHash('sha256').update(bytes).digest('hex')}`;
}

function digestPromptBytes(value: string): PromptfooQualificationDigest {
  return `sha256:${crypto.createHash('sha256').update(Buffer.from(value, 'utf8')).digest('hex')}`;
}

function jsonClone(value: unknown): PromptfooQualificationJson {
  return JSON.parse(canonicalize(value)) as PromptfooQualificationJson;
}
...
function validDigest(value: unknown): value is PromptfooQualificationDigest {
...
function validCount(value: unknown): value is number {
  return Number.isSafeInteger(value)
    && (value as number) >= 0
    && (value as number) <= MAX_ATTEMPTS;
}

function validBoundedNumber(
  value: unknown,
  minimum: number,
  maximum: number,
): value is number {
...
function mutableAlias(value: string): boolean {
...
function safeDigest(
  ...
  reasons: string[],
): PromptfooQualificationDigest | null {
  try {
...
function validateManifestPin(
  pin: unknown,
  kind: string,
  reasons: string[],
  allowedKeys: ReadonlySet<string> = MANIFEST_PIN_KEYS,
): pin is Obj {
  ...
  const digest = safeDigest(
    pin.manifest,
    `${kind}_manifest_digest_mismatch`,
    reasons,
  );
...
function addTokenUsage(
  left: PromptfooTokenUsage,
  right: PromptfooTokenUsage,
): PromptfooTokenUsage {
  return {
...
function sameTokenUsage(left: unknown, right: PromptfooTokenUsage): boolean {
...
function challengeDigestPayload(row: Obj): unknown {
 ...
 */
export function promptfooQualificationChallengeDigest(
  row: unknown,
): PromptfooQualificationDigest {
  if (!isObject(row)) throw new TypeError('Promptfoo result row must be an object');
...
function compareRunLineage(
  field: string,
  runValue: Obj | null,
  pin: Obj | null,
  reasons: string[],
): void {
  if (runValue === null || pin === null) return;
...
function statusCounts(
...
): Record<'passed' | 'failed' | 'errors' | 'aborted' | 'expired', number> {
  const counts = {
    passed: 0,
    failed: 0,
    errors: 0,
  ...
  for (const attempt of attempts) {
    if (attempt.status === 'PASS') counts.passed += 1;
    else if (attempt.status === 'FAIL') counts.failed += 1;
    else if (attempt.status === 'ERROR') counts.errors += 1;
    else if (attempt.status === 'ABORTED') counts.aborted += 1;
    else counts.expired += 1;
  }
  return counts;
}

function statusConsistent(
  row: Obj,
  metadata: Obj,
  status: PromptfooQualificationStatus,
  tokenUsage: PromptfooTokenUsage,
...
function validateResultBoundary(
  row: Obj,
  reasons: string[],
): void {
  exactKeys(row, RESULT_KEYS, reasons);
  if (isObject(row.provider)) exactKeys(row.provider, PROVIDER_KEYS, reasons);
  if (isObject(row.prompt)) exactKeys(row.prompt, PROMPT_KEYS, reasons);
  if (isObject(row.response)) exactKeys(row.response, RESPONSE_KEYS, reasons);
  if (isObject(row.gradingResult)) {
    exactKeys(row.gradingResult, GRADING_KEYS, reasons);
  }
  if (isObject(row.metadata)) exactKeys(row.metadata, METADATA_KEYS, reasons);
}
...
function normalizeAttempt(
  ...
  if (!isObject(row.metadata)
      ...
      || !exactKeys(
        row.metadata.emilia_gate_qualification_v2,
  ...
  if (!validId(attemptId) || !validId(challengeId)
      ...
      || (metadata.ordinal as number) < 1
      || (metadata.ordinal as number) > MAX_ATTEMPTS
      || typeof metadata.status !== 'string'
      || !STATUSES.has(metadata.status as PromptfooQualificationStatus)) {
    addReason(reasons, `attempt_metadata_invalid:${index}`);
    return null;
  }
  const typedAttemptId = attemptId;
  const status = metadata.status as PromptfooQualificationStatus;
  ...
  if (!isObject(row.namedScores)
      ...
      || Object.values(row.namedScores).some(
        (value) => !validBoundedNumber(value, 0, 1),
      )) {
  ...
  let requestDigest: PromptfooQualificationDigest | null = null;
  let responseDigest: PromptfooQualificationDigest | null = null;
  let testCaseDigest: PromptfooQualificationDigest | null = null;
  let gradingDigest: PromptfooQualificationDigest | null = null;
  let resultDigest: PromptfooQualificationDigest | null = null;
  try {
    ...
    if (!isObject(row.gradingResult)
        ...
        || (row.gradingResult.namedScores !== undefined
          && (!isObject(row.gradingResult.namedScores)
            || safeDigest(
              row.gradingResult.namedScores,
              `grading_quality_mismatch:${typedAttemptId}`,
              reasons,
            ) !== safeDigest(
              namedScores,
              `grading_quality_mismatch:${typedAttemptId}`,
  ...
  let challengeDigest: PromptfooQualificationDigest;
  try {
  ...
  return {
    evidence: {
      ...
      ordinal: metadata.ordinal as number,
      status,
...
function lineageOutput(pin: Obj): FrozenManifestLineage {
  return {
    id: pin.id as string,
    immutable_ref: pin.immutable_ref as string,
    digest: pin.manifest_digest as PromptfooQualificationDigest,
 ...
 */
export function adaptPromptfooQualificationArtifact(
  options: AdaptPromptfooQualificationOptions,
): PromptfooQualificationAdapterResult {
  const limitReason = adapterInputLimitReason(options);
  if (limitReason !== null) return { ok: false, reasons: [limitReason] };
  try {
...
function adaptPromptfooQualificationArtifactInternal(
  options: AdaptPromptfooQualificationOptions,
): PromptfooQualificationAdapterResult {
  ...
  const pins = pinValue as unknown as PromptfooQualificationPins;
  if (pins['@version'] !== PROMPTFOO_QUALIFICATION_ADAPTER_VERSION) {
    addReason(reasons, 'adapter_version_mismatch');
  }
  if (!validId(pins.eval_id)) addReason(reasons, 'eval_id_invalid');
  ...
  if (pins.output_version !== 3) addReason(reasons, 'output_version_unsupported');
  ...
  if (!Number.isSafeInteger(pins.max_evidence_age_seconds)
      || (pins.max_evidence_age_seconds as number) < 1
      || (pins.max_evidence_age_seconds as number) > MAX_AGE_SECONDS) {
  ...
  const candidateValid = validateManifestPin(
    pins.candidate,
    'candidate',
    reasons,
  );
  const assignmentValid = validateManifestPin(
    pins.assignment,
    'assignment',
    reasons,
  );
  const harnessValid = validateManifestPin(
    pins.harness,
    'harness',
    reasons,
    HARNESS_PIN_KEYS,
  );
  const environmentValid = validateManifestPin(
    pins.environment,
    'environment',
    reasons,
  );
  const campaignValid = validateManifestPin(
    pins.challenge_campaign,
    'campaign',
    reasons,
  );
  ...
  let expectedAttempts: Obj[] = [];
  if (campaignValid) {
    if (!exactKeys(
      pins.challenge_campaign.manifest,
      CAMPAIGN_MANIFEST_KEYS,
      reasons,
    )
        || !validDigest(
          (pins.challenge_campaign.manifest as Obj).challenge_set_digest,
        )
        || !Array.isArray(
          (pins.challenge_campaign.manifest as Obj).attempts,
        )) {
      addReason(reasons, 'campaign_manifest_invalid');
      campaignManifestValid = false;
    } else {
      expectedAttempts = (pins.challenge_campaign.manifest as Obj)
        .attempts as Obj[];
      ...
      for (let index = 0; index < expectedAttempts.length; index += 1) {
        ...
        if (challengeIds.has(attempt.challenge_id)) {
          addReason(
            reasons,
            `duplicate_manifest_challenge_id:${attempt.challenge_id}`,
          );
          campaignManifestValid = false;
        }
      ...
      const challengeSetDigest = safeDigest(
        expectedAttempts,
        'challenge_set_digest_mismatch',
        reasons,
      );
  ...
  if (verifierValid) {
    const trustDigest = safeDigest(
      pins.verifier.trust_config,
      'verifier_trust_config_digest_mismatch',
      reasons,
    );
    ...
    if (!immutableRef(
      ...
      pins.verifier.immutable_ref,
    )) {
      addReason(reasons, 'mutable_or_unpinned_ref:verifier');
    }
  }

  const actualArtifactDigest = safeDigest(
    artifact,
    'artifact_not_canonicalizable',
    reasons,
  );
  ...
  const configDigest = safeDigest(
    artifact.config,
    'harness_config_not_canonicalizable',
    reasons,
  );
  ...
  if (!exactKeys(artifact.metadata, METADATA_KEYS, reasons)
      || !exactKeys(
        artifact.metadata.emilia_gate_qualification_v2,
        RUN_METADATA_KEYS,
        reasons,
      )) {
  ...
  const runCandidate = runLineage(
    run,
    'candidate',
    RUN_LINEAGE_KEYS,
    reasons,
  );
  const runAssignment = runLineage(
    run,
    'assignment',
    RUN_LINEAGE_KEYS,
    reasons,
  );
  const runHarness = runLineage(
    run,
    'harness',
    RUN_HARNESS_KEYS,
    reasons,
  );
  const runEnvironment = runLineage(
    run,
    'environment',
    RUN_LINEAGE_KEYS,
    reasons,
  );
  const runCampaign = runLineage(
    run,
    'challenge_campaign',
    RUN_LINEAGE_KEYS,
    reasons,
  );
  const runVerifier = runLineage(
    run,
    'verifier',
    RUN_VERIFIER_KEYS,
    reasons,
  ...
  compareRunLineage(
    'candidate',
    runCandidate,
    candidateValid ? pins.candidate as unknown as Obj : null,
    reasons,
  );
  compareRunLineage(
    'assignment',
    runAssignment,
    assignmentValid ? pins.assignment as unknown as Obj : null,
    reasons,
  );
  compareRunLineage(
    'harness',
    runHarness,
    harnessValid ? pins.harness as unknown as Obj : null,
    reasons,
  );
  compareRunLineage(
    'environment',
    runEnvironment,
    environmentValid ? pins.environment as unknown as Obj : null,
    reasons,
  );
  compareRunLineage(
    'challenge_campaign',
    runCampaign,
    campaignValid ? pins.challenge_campaign as unknown as Obj : null,
    reasons,
  );
  ...
  if (!Number.isFinite(trustedNow)) addReason(reasons, 'trusted_now_invalid');
  ...
  if (Number.isFinite(completed) && Number.isFinite(trustedNow)) {
    if (completed > trustedNow) addReason(reasons, 'evidence_from_future');
    if (trustedNow - completed
        > (pins.max_evidence_age_seconds as number) * 1000) {
  ...
  for (let index = 0; index < summary.results.length; index += 1) {
    const attempt = normalizeAttempt(
      ...
      index,
      pins.eval_id as string,
      pins.candidate as unknown as PromptfooQualificationCandidatePin,
      pins.quality_metrics as string[],
      started,
      completed,
      reasons,
    );
    if (attempt !== null) normalized.push(attempt);
  }

  const seenAttempts = new Map<string, NormalizedAttempt>();
  ...
  for (let index = 0; index < expectedAttempts.length; index += 1) {
    ...
    if (typeof expected.attempt_id !== 'string') continue;
    const observed = seenAttempts.get(expected.attempt_id);
    if (observed === undefined) {
      addReason(
        reasons,
        `selective_result_omission:${expected.attempt_id}`,
      );
      continue;
    }
  ...
  for (const attempt of normalized) {
    if (!expectedIds.has(attempt.evidence.attempt_id)) {
      addReason(
        reasons,
        `unexpected_attempt:${attempt.evidence.attempt_id}`,
      );
    }
  }
  ...
  let tokenUsage: PromptfooTokenUsage = {
    total: 0,
    prompt: 0,
    completion: 0,
    cached: 0,
  ...
  let latencyTotal = 0;
  let scoreTotal = 0;
  const namedScoreTotals: Record<string, number> = Object.fromEntries(
    (pins.quality_metrics as string[]).map((metric) => [metric, 0]),
  );
  for (const attempt of normalized) {
    ...
    for (const metric of pins.quality_metrics as string[]) {
  ...
  const evidence: PromptfooQualificationEvidenceInput = {
    ...
    source: {
      ...
      promptfoo_version: pins.promptfoo_version as string,
      output_version: 3,
      eval_id: pins.eval_id as string,
      artifact_ref: pins.artifact_ref as string,
      artifact_digest:
        pins.artifact_digest as PromptfooQualificationDigest,
    },
    lineage: {
      candidate: lineageOutput(pins.candidate as unknown as Obj),
      assignment: lineageOutput(pins.assignment as unknown as Obj),
      harness: {
        ...lineageOutput(pins.harness as unknown as Obj),
        config_digest:
          pins.harness.config_digest as PromptfooQualificationDigest,
      },
      environment: lineageOutput(pins.environment as unknown as Obj),
      challenge_campaign: lineageOutput(
        pins.challenge_campaign as unknown as Obj,
      ),
    },
    timing: {
      started_at: run.started_at as string,
      completed_at: run.completed_at as string,
      expires_at: run.expires_at as string,
      evaluated_at: now,
    },
    coverage: {
      complete: true,
    ...
    measurements: {
      ...
      generation_duration_ms:
        typeof stats.generationDurationMs === 'number'
      ...
      named_score_means: Object.fromEntries(
        Object.entries(namedScoreTotals).map(([metric, total]) => [
          metric,
    ...
    verifier: {
      id: pins.verifier.id as string,
      immutable_ref: pins.verifier.immutable_ref as string,
      trust_config: jsonClone(pins.verifier.trust_config),
      trust_config_digest:
        pins.verifier.trust_config_digest as PromptfooQualificationDigest,
    },
  };
