...
export interface ChapTrustRoot {
  ...
  valid_until: string;
  revoked_at?: string;
  identity_binding: { method: string; evidence_digest: AebDigest };
}
...
interface ParsedRoot extends ChapTrustRoot {
  key: KeyObject;
}
...
interface ParsedDecision {
  envelope: Obj;
  params: Obj;
  method: 'decide.approve' | 'decide.override';
  kid: string;
  signature: Buffer;
}
...
function exactKeys(value: Obj, allowed: ReadonlySet<string>, required: readonly string[]): boolean {
...
function canonicalBase64(value: unknown, length?: number): value is string {
...
function safeDigest(value: unknown): AebDigest {
  try { return digestAeb(value); } catch { return digestAeb(null); }
}

function statusDigest(status: unknown): AebDigest {
  return safeDigest(status);
}

function validStatus(value: unknown): value is AebStatusInput {
...
function statusDisposition(
  ...
  if (!validStatus(status)) return { acceptance: 'INDETERMINATE', reasons: ['status_input_invalid'] };
  ...
  if (status.revoked) return { acceptance: 'REJECTED', reasons: ['evidence_revoked'] };
  if (status.consumed) return { acceptance: 'REJECTED', reasons: ['evidence_consumed'] };
  return { acceptance: 'ACCEPTED', reasons: [] };
}

function parseConfig(value: unknown): ChapAdapterConfig | null {
  if (!isRecord(value) || !exactKeys(value, CONFIG_KEYS, [...CONFIG_KEYS])
      ...
      || !positiveInteger(value.max_status_age_seconds)) return null;
  return structuredClone(value) as unknown as ChapAdapterConfig;
}

function parseRoot(value: unknown): ParsedRoot | null {
  if (!isRecord(value)
      ...
      || !DIGEST_RE.test(value.identity_binding.evidence_digest)) return null;
  try {
    ...
    return { ...(structuredClone(value) as unknown as ChapTrustRoot), key };
  } catch { return null; }
}

function parsePins(value: ChapConstructorPins): ParsedPins {
  ...
  const parsedRoots = roots as ParsedRoot[];
...
function parseDecision(value: unknown): ParsedDecision | null {
  if (!isRecord(value) || !exactKeys(value, ENVELOPE_KEYS, [...ENVELOPE_KEYS])
      ...
      || !isRecord(value.params) || !nonEmptyString(value.sig)) return null;
  const method = value.method as ParsedDecision['method'];
  ...
  if (!exactKeys(value.params, allowed, required)
      ...
      || !Number.isFinite(parseInstant(value.params.ts))) return null;
  ...
  if (signatureParts.length !== 3 || signatureParts[0] !== 'ed25519'
      ...
      || !canonicalBase64(signatureParts[2], 64)) return null;
  return {
    envelope: value,
    params: value.params,
    method,
...
function verifyDecisionSignature(decision: ParsedDecision, root: ParsedRoot): boolean {
...
function splitPointer(path: unknown): string[] {
  if (typeof path !== 'string') throw new TypeError('JSON Pointer must be a string');
  if (path === '') return [];
  if (!path.startsWith('/')) throw new TypeError('JSON Pointer must start with slash');
  const segments = path.slice(1).split('/').map((segment) => {
    if (/~(?:[^01]|$)/.test(segment)) throw new TypeError('invalid JSON Pointer escape');
...
function arrayIndex(segment: string, allowAppend: boolean): number | '-' {
  ...
  if (!/^(?:0|[1-9][0-9]*)$/.test(segment)) throw new TypeError('invalid array index');
  const index = Number(segment);
  if (!Number.isSafeInteger(index)) throw new TypeError('array index is not safe');
  return index;
}

function parentAt(document: unknown, segments: string[]): { parent: unknown; key: string | number | '-' } {
  if (segments.length === 0) throw new TypeError('root has no parent');
  let parent = document;
  for (const segment of segments.slice(0, -1)) {
    if (Array.isArray(parent)) {
      const index = arrayIndex(segment, false);
      if (index === '-' || index < 0 || index >= parent.length) throw new TypeError('array path not found');
      parent = parent[index];
    } else if (isRecord(parent)) {
      if (!Object.hasOwn(parent, segment)) throw new TypeError('object path not found');
  ...
  return {
    ...
    key: Array.isArray(parent)
      ? arrayIndex(segments.at(-1) as string, true)
...
function nodeCount(value: unknown): number {
  if (Array.isArray(value)) {
    let total = 1;
    for (const item of value) total += nodeCount(item);
    return total;
  }
  if (isRecord(value)) {
    let total = 1;
    for (const item of Object.values(value)) total += nodeCount(item);
    return total;
  }
  return 1;
}

function applyOnePatch(document: unknown, operation: unknown): unknown {
  ...
  if (operation.op === 'add') {
    if (!Object.hasOwn(operation, 'value')) throw new TypeError('add requires value');
    if (segments.length === 0) return structuredClone(operation.value);
    const { parent, key } = parentAt(document, segments);
    if (Array.isArray(parent)) {
      ...
      } else throw new TypeError('array add index out of range');
    } else if (isRecord(parent) && typeof key === 'string') {
      parent[key] = structuredClone(operation.value);
    } else throw new TypeError('cannot add into scalar');
    return document;
  }
  if (operation.op === 'replace') {
    if (!Object.hasOwn(operation, 'value')) throw new TypeError('replace requires value');
    if (segments.length === 0) return structuredClone(operation.value);
    const { parent, key } = parentAt(document, segments);
    if (Array.isArray(parent)) {
      if (key === '-' || typeof key !== 'number' || key < 0 || key >= parent.length) {
      ...
      parent[key] = structuredClone(operation.value);
    } else throw new TypeError('object replace path not found');
    return document;
  }
  if (operation.op === 'remove') {
    if (segments.length === 0) throw new TypeError('cannot remove root');
    const { parent, key } = parentAt(document, segments);
    if (Array.isArray(parent)) {
      if (key === '-' || typeof key !== 'number' || key < 0 || key >= parent.length) {
      ...
      delete parent[key];
    } else throw new TypeError('object remove path not found');
    return document;
  }
  if (operation.op === 'copy' || operation.op === 'move') {
    if (typeof operation.from !== 'string') throw new TypeError(`${operation.op} requires from`);
  ...
  if (operation.op === 'test') {
    if (!Object.hasOwn(operation, 'value')) throw new TypeError('test requires value');
...
export function applyChapJsonPatch(document: unknown, patch: unknown): unknown {
  ...
  for (const operation of patch) {
    ...
    if (nodeCount(result) > MAX_DOCUMENT_NODES) throw new TypeError('patched document exceeds node limit');
  }
  canonicalizeStrictJson(result);
  return result;
}

function exactExpectedAction(value: unknown, config: ChapAdapterConfig): value is Obj {
  return isRecord(value)
...
function verifyNative(
  ...
  const sender = decision.params.from as string;
...
export function createChapActionDefinition(actionType: string): Obj {
  if (!ACTION_TYPE_RE.test(actionType)) throw new TypeError('invalid CHAP action type');
  return {
...
function validMappingProfile(profile: AebPinnedProfile, config: ChapAdapterConfig): boolean {
  if (!isRecord(profile)
      ...
      || !exactKeys(profile.definition, MAPPING_KEYS, [...MAPPING_KEYS])) return false;
...
export function createChapAebAdapter(constructorPins: ChapConstructorPins): AebAdapter {
