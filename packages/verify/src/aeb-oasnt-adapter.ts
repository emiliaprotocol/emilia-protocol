 ...
 */
export const OASNT_ASSURANCE_LEVELS: Readonly<Record<string, number>> = Object.freeze({
  'software': 10,
  'platform-key': 20,
  'attested-display': 30,
});
...
interface ParsedRoot extends OasntTrustRoot { key: KeyObject }
...
function parseConfig(value: unknown): OasntAdapterConfig | null {
  if (!isRecord(value) || !exactKeys(value, CONFIG_KEYS)
      ...
      || !(value.required_assurance_level === null
        || (typeof value.required_assurance_level === 'string'
          && Object.hasOwn(OASNT_ASSURANCE_LEVELS, value.required_assurance_level)))) return null;
  return structuredClone(value) as unknown as OasntAdapterConfig;
}

function jwkThumbprint(jwk: OasntTrustRoot['public_jwk']): string {
...
function parseRoots(value: readonly unknown[], config: OasntAdapterConfig): ParsedRoot[] | null {
  ...
  for (const candidate of value) {
    if (!isRecord(candidate) || !exactKeys(candidate, ROOT_KEYS)
        ...
        || !DIGEST_RE.test(candidate.enrollment.evidence_digest)) return null;
    const root = structuredClone(candidate) as unknown as OasntTrustRoot;
    if (jwkThumbprint(root.public_jwk) !== root.jwk_thumbprint
        || thumbprints.has(root.jwk_thumbprint)) return null;
    let key: KeyObject;
    try {
...
function parsePins(value: OasntConstructorPins): ParsedPins {
  const config = parseConfig(value?.config);
  if (!config) throw new TypeError('invalid OASNT constructor config');
  const roots = parseRoots(value?.trust_roots, config);
  if (!roots) throw new TypeError('invalid OASNT constructor trust roots');
  return {
    config,
    roots,
...
function validParameters(value: unknown): value is Record<string, string> {
...
function actionEscape(value: string): string {
  return value.replace(/[\\|=&]/g, (match) => `\\${match}`);
}

function displayEscape(value: string): string {
...
export function computeOasntActionDigest(type: string, parameters: Record<string, string>): string {
...
export function computeOasntRequestFingerprint(request: OasntRequestBinding): string {
  if (!isRecord(request) || !exactKeys(request, REQUEST_KEYS)
      || !['method', 'path', 'org_id', 'scope'].every((key) => (
        typeof request[key] === 'string' && !String(request[key]).includes('\u0000')
      ))
...
function parseCompactToken(value: unknown): ParsedToken | null {
  ...
  if (!isRecord(header) || !exactKeys(header, TOKEN_HEADER_KEYS)
      ...
      || !isRecord(claims)
      || !exactKeys(claims, TOKEN_CLAIM_KEYS, new Set(['rqf', 'asl']))) return null;
  return {
    token: value,
    claims,
...
function exactExpectedAction(value: unknown, config: OasntAdapterConfig): value is Obj {
  ...
  if (!isRecord(value) || !exactKeys(value, keys)
      ...
      || !validParameters(value.native_action.parameters)) return false;
  if (!config.require_request_binding) return true;
  try { computeOasntRequestFingerprint(value.request as OasntRequestBinding); return true; } catch { return false; }
}
...
function verifyNative(
  ...
  const tokenThumbprint = claims.cnf.jkt as string;
  ...
  const nativeAction = input.expected_action.native_action as Obj;
  const parameters = nativeAction.parameters as Record<string, string>;
  ...
  if (pins.config.require_request_binding
      && claims.rqf !== computeOasntRequestFingerprint(input.expected_action.request as OasntRequestBinding)) {
    result.acceptance = 'REJECTED';
...
export function createOasntActionDefinition(actionType: string, requireRequestBinding: boolean): Obj {
...
function validMappingProfile(profile: AebPinnedProfile, config: OasntAdapterConfig): boolean {
  if (!isRecord(profile)
...
export function createOasntAebAdapter(constructorPins: OasntConstructorPins): AebAdapter {
