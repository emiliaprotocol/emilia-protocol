 ...
 */
export const AEB_CROSSING_LIFECYCLE_INDEX_V2_VERSION =
  "EP-AEB-CROSSING-LIFECYCLE-INDEX-v2";
export const AEB_CROSSING_LIFECYCLE_INDEX_V2_DOMAIN =
  `${AEB_CROSSING_LIFECYCLE_INDEX_V2_VERSION}\0`;
export const AEB_CROSSING_RECORD_REQUIRED_ALGORITHMS = Object.freeze([
  "Ed25519",
  "ML-DSA-65",
] as const);

export const WIMSE_OAUTH_CROSSING_MAPPING_PROFILE =
  "EP-AEB-CROSSING-WIMSE-OAUTH-v1";
export const BCR_CROSSING_MAPPING_PROFILE = "EP-AEB-CROSSING-BCR-v1";

export type CrossingNativeVerification =
  "VERIFIED" | "FAILED" | "INDETERMINATE";
export type CrossingRpAcceptance = "ACCEPTED" | "REJECTED" | "INDETERMINATE";
export type CrossingActionRelation =
  "EXACT_MATCH" | "MISMATCH" | "INDETERMINATE";
export type CrossingStatus =
  "CURRENT" | "STALE" | "UNAVAILABLE" | "REVOKED" | "INDETERMINATE";
export type CrossingReplay = "FRESH" | "REPLAY" | "INDETERMINATE";
export type CrossingAdmission =
  "ADMIT" | "REFUSE" | "INDETERMINATE" | "NOT_APPLICABLE";
export type CrossingCustody =
  "UNRESERVED" | "RESERVED" | "INVOKING" | "INDETERMINATE" | "TERMINAL";
export type CrossingProviderCommitment =
  "NOT_INVOKED" | "COMMITTED" | "PROVEN_NOT_COMMITTED" | "INDETERMINATE";
export type CrossingObservedEffect =
  "NOT_OBSERVED" | "OBSERVED_AS_REQUESTED" | "DIVERGED" | "INDETERMINATE";
export type CrossingRetry =
  "NOT_APPLICABLE" | "REFUSE" | "REQUIRES_NEW_ADMISSION";
export type CrossingReconciliation =
  "NOT_APPLICABLE" | "REQUIRED" | "REFUSED" | "APPLIED";
export type AdmissionReferenceState =
  "PRESENT" | "MISSING" | "NOT_APPLICABLE" | "INDETERMINATE";
...
export interface AebCrossingRecordBody {
  ...
  action: { caid: string; action_digest: AebDigest };
  boundary: {
...
export type AebCrossingEvaluationProfile =
  | typeof AEB_EVALUATION_VERSION
  | typeof AEB_EVALUATION_V2_VERSION;
...
export type AebCrossingCustodyReferencePhase =
  | "NOT_APPLICABLE"
  | "RESERVATION"
  | "CONSUMPTION"
  | "INDETERMINATE";
...
export type AebCrossingLifecycleIndexConversionStatus =
  | "NATIVE"
  | "COMPLETE"
  | "INDETERMINATE";
...
export interface AebCrossingLifecycleIndexV2Body {
  ...
  source_crossing_record: {
    version:
      | typeof AEB_CROSSING_RECORD_VERSION
      | typeof AEB_CROSSING_RECORD_V2_VERSION
      | null;
    digest: AebDigest | null;
  };
  conversion: {
...
export type AebCrossingLifecycleIndexV2Draft = Omit<
...
export interface AebCrossingLifecycleIndexV2VerifyOptions
  extends AebCrossingRecordVerifyOptions {
...
export interface AebCrossingRecordV1UpgradeOptions
  extends AebCrossingRecordIssueOptions {
...
export type CrossingAuthorityMappingResult =
  | { ok: true; authority: CrossingNativeAuthority }
...
const RP_ACCEPTANCES = new Set<CrossingRpAcceptance>([
  "ACCEPTED",
  "REJECTED",
  "INDETERMINATE",
]);
const ACTION_RELATIONS = new Set<CrossingActionRelation>([
  "EXACT_MATCH",
  "MISMATCH",
  "INDETERMINATE",
]);
const STATUSES = new Set<CrossingStatus>([
  "CURRENT",
  "STALE",
  "UNAVAILABLE",
  "REVOKED",
...
const REPLAYS = new Set<CrossingReplay>(["FRESH", "REPLAY", "INDETERMINATE"]);
const ADMISSIONS = new Set<CrossingAdmission>([
  "ADMIT",
  "REFUSE",
  "INDETERMINATE",
  "NOT_APPLICABLE",
]);
const CUSTODIES = new Set<CrossingCustody>([
  "UNRESERVED",
  "RESERVED",
  "INVOKING",
  "INDETERMINATE",
...
const COMMITMENTS = new Set<CrossingProviderCommitment>([
  "NOT_INVOKED",
  "COMMITTED",
  "PROVEN_NOT_COMMITTED",
  "INDETERMINATE",
]);
const EFFECTS = new Set<CrossingObservedEffect>([
  "NOT_OBSERVED",
  "OBSERVED_AS_REQUESTED",
  "DIVERGED",
  "INDETERMINATE",
]);
const RETRIES = new Set<CrossingRetry>([
  "NOT_APPLICABLE",
  "REFUSE",
  "REQUIRES_NEW_ADMISSION",
]);
const RECONCILIATIONS = new Set<CrossingReconciliation>([
  "NOT_APPLICABLE",
  "REQUIRED",
  "REFUSED",
  "APPLIED",
]);
const ADMISSION_REFERENCE_STATES = new Set<AdmissionReferenceState>([
  "PRESENT",
  "MISSING",
  "NOT_APPLICABLE",
  "INDETERMINATE",
...
function validAuthority(value: unknown): value is CrossingNativeAuthority {
  return (
    isRecord(value) &&
...
function validDigestList(value: unknown): value is AebDigest[] {
  return (
    Array.isArray(value) &&
    value.length > 0 &&
    value.length <= 128 &&
...
function validReferee(value: unknown): value is CrossingRefereeAxes {
  return (
    isRecord(value) &&
...
function algorithmSetMatches(value: unknown): value is string[] {
  return (
    Array.isArray(value) &&
    value.length === AEB_CROSSING_RECORD_REQUIRED_ALGORITHMS.length &&
    value.every(
      (algorithm, index) =>
        algorithm === AEB_CROSSING_RECORD_REQUIRED_ALGORITHMS[index],
    )
  );
}

function signatureArray(value: unknown): value is AgileSignature[] {
  return (
    Array.isArray(value) &&
    value.length === AEB_CROSSING_RECORD_REQUIRED_ALGORITHMS.length &&
    value.every(
...
export function crossingRecordContractDigest(
  body: Pick<
    AebCrossingRecordBody,
    "native_authority" | "action" | "boundary" | "requirements"
  >,
): AebDigest {
...
export function crossingRecordSignedBytes(
  ...
  return Buffer.from(
    `${AEB_CROSSING_RECORD_DOMAIN}${canonicalizeAeb(body)}`,
    "utf8",
  );
}
...
export function crossingLifecycleIndexV2AdmissionDomainDigest(
  boundary: CrossingAdmissionDomain,
): AebDigest {
  return digestAebTyped(
    {
...
export function crossingLifecycleIndexV2ContractDigest(
  body: Pick<
    AebCrossingLifecycleIndexV2Body,
    | "operation_id"
    | "action"
...
export function crossingLifecycleIndexV2SignedBytes(
  body: AebCrossingLifecycleIndexV2Body,
): Uint8Array {
  return Buffer.from(
    `${AEB_CROSSING_LIFECYCLE_INDEX_V2_DOMAIN}${canonicalizeAeb(body)}`,
    "utf8",
  );
}

export function crossingLifecycleIndexV2Digest(
  body: AebCrossingLifecycleIndexV2Body,
): AebDigest {
  return digestAebTyped(
    body,
    `${AEB_CROSSING_LIFECYCLE_INDEX_V2_VERSION}:record`,
  );
}

function validateBody(body: unknown): string | null {
  if (!isRecord(body) || !exactKeys(body, BODY_KEYS)) return "malformed_record";
  if (
  ...
  if (
    !isRecord(body.signature_profile) ||
  ...
  if (!validAuthority(body.native_authority)) return "native_authority_invalid";
  if (
  ...
  if (!digest(body.contract_digest)) return "malformed_record";
  if (
    crossingRecordContractDigest(body as AebCrossingRecordBody) !==
    body.contract_digest
  ) {
    return "contract_digest_mismatch";
  }
  if (
    !isRecord(body.admission_reference) ||
  ...
  if (body.admission_reference.state === "PRESENT") {
    ...
    return "admission_reference_invalid";
  if (
    !isRecord(body.lifecycle_records) ||
  ...
  if (
    !validDigestList(body.evaluated_evidence_digests) ||
    !validDigestList(body.configuration_digests) ||
    !validReferee(body.referee)
  )
    return "malformed_record";

  const authority = body.native_authority as CrossingNativeAuthority;
  const referee = body.referee as CrossingRefereeAxes;
  if (referee.status !== authority.status.value) return "status_inconsistent";
  if (
    referee.admission === "ADMIT" &&
  ...
  if (
    referee.native_verification !== authority.native_verification ||
  ...
  if (
    body.admission_reference.state === "MISSING" &&
    !["INDETERMINATE", "NOT_APPLICABLE"].includes(referee.admission)
  ) {
    return "admission_reference_invalid";
  }
  if (
    body.admission_reference.state === "NOT_APPLICABLE" &&
...
function nullableDigest(value: unknown): value is AebDigest | null {
  return value === null || digest(value);
}

const EVALUATION_VERDICTS = new Set(["SATISFIED", "UNSATISFIED", "INDETERMINATE"]);

interface EvaluationLegFacts {
  evidence_digest: AebDigest;
  verdict: unknown;
  native_verification: unknown;
  acceptance: unknown;
  mapping: unknown;
  caid: unknown;
...
interface EvaluationFacts {
  ...
  caid: string;
  verdict: string;
  /** The evaluation's AEC action commitment over { caid, normalized_action_digest }. */
  action_commitment: AebDigest;
  legs: EvaluationLegFacts[];
}
 ...
 */
function evaluationFacts(value: unknown): EvaluationFacts | null {
  if (!isRecord(value)) return null;
  let profile: AebCrossingEvaluationProfile;
  let caid: unknown;
  let verdict: unknown;
  ...
  if (value["@type"] === AEB_EVALUATION_VERSION) {
    ...
    if (!isRecord(value.action) || !isRecord(value.satisfaction)) return null;
  ...
  if (
    ...
    return null;
  const legs: EvaluationLegFacts[] = [];
  for (const leg of value.legs) {
    if (!isRecord(leg) || !digest(leg.evidence_digest)) return null;
    legs.push({
...
function evaluationActionCommitment(
  caid: string,
  normalizedActionDigest: AebDigest | null,
): string {
...
interface EvaluationJoinTarget {
  digest: AebDigest;
  /** Label to bind; undefined for legacy records, null for an unlabeled index. */
 ...
 */
function evaluationJoinReason(
  evaluation: unknown,
  target: EvaluationJoinTarget,
): string | null {
  let pinned: unknown;
  ...
  if (!facts) return "evaluation_malformed";
  if (evaluationDigest !== target.digest) return "evaluation_digest_mismatch";
  if (
  ...
  if (authority) {
    const candidates = facts.legs.filter(
      (leg) => leg.evidence_digest === authority.evidence_digest,
    );
    if (candidates.length === 0) return "evaluation_authority_unmatched";
    const exactMatch = target.referee?.action_relation === "EXACT_MATCH";
    const admitted = target.referee?.admission === "ADMIT";
    const consistent = candidates.some(
      (leg) =>
        (!admitted || leg.verdict === "SATISFIED") &&
        (authority.native_verification !== "VERIFIED" ||
    ...
    if (!consistent) return "evaluation_authority_inconsistent";
  }
  return null;
}
 ...
 */
export function aebCrossingEvaluationReference(
  evaluation: unknown,
  ...
  const facts = evaluationFacts(pinned);
  if (!facts) throw new CrossingRecordError("evaluation_malformed");
  return { profile: facts.profile, digest: digestAeb(pinned) };
}

function validateLifecycleIndexV2Body(body: unknown): string | null {
  ...
  if (
    !isRecord(lifecycle.evaluation) ||
    !exactKeys(lifecycle.evaluation, LIFECYCLE_EVALUATION_KEYS) ||
    ![AEB_EVALUATION_VERSION, AEB_EVALUATION_V2_VERSION].includes(
      lifecycle.evaluation.profile,
    ) ||
    ...
    return "malformed_lifecycle_index";
  if (
    !isRecord(lifecycle.authority_custody) ||
    ...
    return "malformed_lifecycle_index";
  if (
    lifecycle.authority_custody.phase === "NOT_APPLICABLE" &&
    lifecycle.authority_custody.digest !== null
  )
    return "custody_reference_inconsistent";
  if (
    ["RESERVATION", "CONSUMPTION"].includes(
      lifecycle.authority_custody.phase,
    ) &&
  ...
  if (
    !isRecord(body.source_crossing_record) ||
  ...
  if (
    !isRecord(body.conversion) ||
    !exactKeys(body.conversion, LIFECYCLE_CONVERSION_KEYS) ||
    !["NATIVE", "COMPLETE", "INDETERMINATE"].includes(
      body.conversion.status,
    ) ||
    !Array.isArray(body.conversion.reason_codes) ||
    body.conversion.reason_codes.length > 32 ||
    !body.conversion.reason_codes.every(
      (reason: unknown) => typeof reason === "string" && REASON_CODE.test(reason),
    ) ||
  ...
  if (
    ...
    return "conversion_report_invalid";
  if (
    body.conversion.status === "COMPLETE" &&
  ...
  if (
    body.conversion.reason_codes.includes(UNVERIFIED_EVALUATION_REFERENCE) &&
    (body.conversion.status !== "INDETERMINATE" ||
  ...
  if (!digest(body.contract_digest)) return "malformed_lifecycle_index";
  const typed = body as unknown as AebCrossingLifecycleIndexV2Body;
  if (crossingLifecycleIndexV2ContractDigest(typed) !== typed.contract_digest)
    return "contract_digest_mismatch";

  if (lifecycle.local_admission_digest === null) {
    if (
      !["NOT_APPLICABLE", "INDETERMINATE"].includes(
        lifecycle.authority_custody.phase,
      ) ||
      lifecycle.authority_custody.digest !== null ||
  ...
  if (lifecycle.provider_entry_digest !== null) {
    ...
    if (phase === "NOT_APPLICABLE") return "lifecycle_order_invalid";
...
function mappingCommonValid(input: Record<string, any>): boolean {
  return (
    NATIVE_VERIFICATIONS.has(input.native_verification) &&
...
export function mapWimseOAuthCrossingAuthority(
  input: WimseOAuthCrossingInput,
  ...
  const authority: CrossingNativeAuthority = {
...
export function mapBcrCrossingAuthority(
  input: BcrCrossingInput,
): CrossingAuthorityMappingResult {
  if (
    !isRecord(input) ||
    !mappingCommonValid(input) ||
...
export const WIMSE_OAUTH_CROSSING_ADAPTER: AebCrossingAuthorityAdapter<WimseOAuthCrossingInput> =
  Object.freeze({
...
export const BCR_CROSSING_ADAPTER: AebCrossingAuthorityAdapter<BcrCrossingInput> =
  Object.freeze({
...
export async function issueAebCrossingRecord(
  draft: AebCrossingRecordDraft,
  options: AebCrossingRecordIssueOptions,
): Promise<AebCrossingRecord> {
  const partial = structuredClone(draft) as AebCrossingRecordDraft;
  const body: AebCrossingRecordBody = {
    ...partial,
    signature_profile: {
  ...
  const signatures = await signAgileSet(
    crossingRecordSignedBytes(body),
 ...
 * their decisions.
 */
export async function issueAebCrossingLifecycleIndexV2(
  draft: AebCrossingLifecycleIndexV2Draft,
  context: AebCrossingLifecycleIndexV2Context,
  options: AebCrossingRecordIssueOptions,
): Promise<AebCrossingLifecycleIndexV2> {
  const pinnedContext = JSON.parse(
    canonicalizeAeb(context),
  ) as AebCrossingLifecycleIndexV2Context;
  const pinnedOptions: AebCrossingRecordIssueOptions = {
    ...options,
    signing_keys: options?.signing_keys?.map((key) => ({
      ...key,
      private_key:
        key.private_key instanceof Uint8Array
          ? new Uint8Array(key.private_key)
          : key.private_key,
    })),
  };
  const partial = JSON.parse(
    canonicalizeAeb(draft),
  ) as AebCrossingLifecycleIndexV2Draft;
  if (
    !isRecord(pinnedContext) ||
    !isRecord(pinnedContext.action) ||
    !exactKeys(pinnedContext.action, ACTION_KEYS) ||
  ...
  if (
    ...
    throw new CrossingRecordError("admission_domain_mismatch");
  if (
    !isRecord(pinnedContext.evaluation) ||
  ...
  const bodyWithoutContract = {
    ...
    signature_profile: {
      id: SIGNATURE_AGILITY_VERSION,
      required_algorithms: [...AEB_CROSSING_RECORD_REQUIRED_ALGORITHMS],
    } as AebCrossingLifecycleIndexV2Body["signature_profile"],
    admission_domain_digest: crossingLifecycleIndexV2AdmissionDomainDigest(
      pinnedContext.admission_domain,
    ),
  };
  const body: AebCrossingLifecycleIndexV2Body = {
    ...bodyWithoutContract,
    contract_digest: crossingLifecycleIndexV2ContractDigest(
      bodyWithoutContract,
    ),
  };
  ...
  if (
    ...
    throw new CrossingRecordError("algorithm_set_mismatch");
  const signatures = await signAgileSet(
    crossingLifecycleIndexV2SignedBytes(body),
    pinnedOptions.signing_keys,
    pinnedOptions,
  );
  return {
 ...
 */
export async function upgradeAebCrossingRecordV1ToLifecycleIndexV2(
  source: AebCrossingRecord,
  options: AebCrossingRecordV1UpgradeOptions,
): Promise<AebCrossingLifecycleIndexV2> {
  const pinnedSource = JSON.parse(canonicalizeAeb(source)) as AebCrossingRecord;
  let pinnedEvaluation: unknown;
  ...
  const localAdmissionDigest = sourceBody.admission_reference.state === "PRESENT"
    ...
    : null;
  if (["MISSING", "INDETERMINATE"].includes(
    sourceBody.admission_reference.state,
  ))
    reasonCodes.push("local_admission_reference_indeterminate");

  let custody: AebCrossingCustodyReference = {
  ...
  if (sourceBody.lifecycle_records.consumption_digest !== null) {
    ...
    !["UNRESERVED"].includes(sourceBody.referee.custody)
  ) {
  ...
  if (
    sourceBody.lifecycle_records.provider_entry_digest !== null &&
    sourceBody.referee.admission !== "ADMIT"
  )
    reasonCodes.push("provider_entry_without_admit");

  if (!evaluationBound) reasonCodes.push(UNVERIFIED_EVALUATION_REFERENCE);
  ...
  const conversionStatus: AebCrossingLifecycleIndexConversionStatus =
    reasons.length === 0 ? "COMPLETE" : "INDETERMINATE";
  ...
  if (evaluationBound && boundProfile === undefined)
    throw new CrossingRecordError("source_evaluation_mismatch");
  const evaluation: AebCrossingLifecycleIndexV2Body["lifecycle"]["evaluation"] = {
...
function refusal(
  ...
  evaluationBinding: AebCrossingEvaluationBinding = "INDETERMINATE",
): AebCrossingRecordVerifyResult {
  return {
    verified: false,
    reason,
    execution_authorizing: false,
...
function crossingEvaluationTarget(
  body: AebCrossingRecordBody,
): EvaluationJoinTarget {
  return {
    digest: body.lifecycle_records.evaluation_digest,
    operation_id: body.operation_id,
    action: body.action,
...
export async function verifyAebCrossingRecord(
  value: unknown,
  options: AebCrossingRecordVerifyOptions,
): Promise<AebCrossingRecordVerifyResult> {
  const checks: AebCrossingRecordVerifyResult["checks"] = {
    schema: false,
    algorithm_set: null,
    authority: null,
  ...
  let evaluationBinding: AebCrossingEvaluationBinding = "INDETERMINATE";
  try {
    ...
    if (structural) {
      if (
        structural === "native_authority_invalid" ||
    ...
    const body = value.body as AebCrossingRecordBody;
    const bodyDigest = crossingRecordDigest(body);
    if (suppliedEvaluation !== undefined) {
      const joinReason = evaluationJoinReason(
        suppliedEvaluation,
        crossingEvaluationTarget(body),
      );
...
export async function verifyAebCrossingRecordV2(
  ...
  const refuse = (
    reason: string,
    recordDigest: AebDigest | null = null,
    binding: AebCrossingEvaluationBinding = "INDETERMINATE",
  ...
  try {
    ...
    if (suppliedEvaluation !== undefined) {
      ...
      checks.evaluation_binding = joinReason === null;
      if (joinReason) return refuse(joinReason, bodyDigest, "MISMATCH");
      evaluationBinding = "BOUND";
    }
    if (!signatureArray(value.signatures)) {
      checks.signature_set = false;
...
export async function verifyAebCrossingLifecycleIndexV2(
  value: unknown,
  options: AebCrossingLifecycleIndexV2VerifyOptions,
): Promise<AebCrossingLifecycleIndexV2VerifyResult> {
  const checks: AebCrossingLifecycleIndexV2VerifyResult["checks"] = {
    schema: false,
    algorithm_set: null,
    action: null,
    admission_domain: null,
  ...
  let conversionStatus: AebCrossingLifecycleIndexConversionStatus | null = null;
  let evaluationBinding: AebCrossingEvaluationBinding = "INDETERMINATE";
  const refuse = (
    reason: string,
    recordDigest: AebDigest | null = null,
  ...
  try {
    ...
    if (structural) {
      if (structural === "contract_digest_mismatch")
        checks.contract_digest = false;
      if (
        ["lifecycle_order_invalid", "custody_reference_inconsistent"].includes(
          structural,
        )
      )
    ...
    const body = value.body as AebCrossingLifecycleIndexV2Body;
    ...
    if (!checks.action) return refuse("action_mismatch");
    checks.admission_domain =
      crossingLifecycleIndexV2AdmissionDomainDigest(
        options?.admission_domain,
      ) === body.admission_domain_digest;
    if (!checks.admission_domain) return refuse("admission_domain_mismatch");
    ...
    if (expectedEvaluation !== undefined) {
      ...
      if (!checks.evaluation) return refuse("evaluation_reference_mismatch");
    }
    if (suppliedEvaluation !== undefined) {
      ...
      checks.evaluation_binding = joinReason === null;
      if (joinReason) return refuse(joinReason, null, "MISMATCH");
      if (expectedEvaluation === undefined) checks.evaluation = true;
      evaluationBinding = "BOUND";
    }
