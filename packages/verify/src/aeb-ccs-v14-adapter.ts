...
function finiteClone<T>(value: T): T | null {
  try { return JSON.parse(canonicalizeFiniteJson(value)) as T; } catch { return null; }
}
...
function statusDigest(status: AebAdapterInput['status']): AebDigest {
  return digestAeb({
...
function sha256Finite(value: unknown): string | null {
  try {
...
function parseReceipt(value: unknown): CcsV14Receipt | null {
  if (!isRecord(value) || !exactKeys(value, RECEIPT_KEYS)
      ...
      || !safeNonNegative(value.verified_at) || !safeNonNegative(value.latency_us)) return null;
  return finiteClone(value) as CcsV14Receipt | null;
}

function parseArtifact(value: unknown): CcsV14Artifact | null {
  if (!isRecord(value) || !exactKeys(value, ARTIFACT_KEYS) || !isRecord(value.tool_args)) return null;
...
function parseConfig(value: unknown): CcsV14AebAdapterConfig | null {
  ...
  return finiteClone(value) as CcsV14AebAdapterConfig | null;
}

function parseRoot(value: unknown, config: CcsV14AebAdapterConfig):
  { root: CcsV14Ed25519TrustRoot; raw: Buffer } | null {
  if (!isRecord(value) || !exactKeys(value, ROOT_KEYS)
      ...
      || !FINGERPRINT_RE.test(String(value.public_key_fingerprint_sha256_16))) return null;
  const raw = decodeBase64(value.public_key_raw_base64, 32);
  if (!raw || crypto.createHash('sha256').update(raw).digest('hex').slice(0, 16)
      !== value.public_key_fingerprint_sha256_16) return null;
  const root = finiteClone(value) as CcsV14Ed25519TrustRoot | null;
  return root ? { root, raw } : null;
}

function publicKey(raw: Buffer): crypto.KeyObject {
...
function verifySignature(receipt: CcsV14Receipt, raw: Buffer): boolean {
  try {
...
function statusResult(status: AebAdapterInput['status'], now: string, maxAge: number):
...
function combine(left: Acceptance, right: Acceptance): Acceptance {
...
function canonicalAction(value: unknown, actionType: string): Obj | null {
  if (!isRecord(value) || !exactKeys(value, ACTION_KEYS)
      ...
      || !TOKEN_RE.test(String(value.parameters.tool)) || !isRecord(value.parameters.arguments)) return null;
  return finiteClone({
...
export function createCcsV14AebActionDefinition(actionType: string): Obj {
...
function validProfile(profile: AebPinnedProfile, actionType: string): unknown[] | null {
  ...
  if (!isRecord(profile) || !exactKeys(profile, PROFILE_KEYS)
      ...
      || !isRecord(profile.definition) || !Array.isArray(profile.definition.definitions)) return null;
...
export function createCcsV14AebAdapter(constructorPins: {
  ...
  if (!parsedRoot) throw new TypeError('ccs_v14_adapter_root_invalid');
  ...
  function fallback(input: Omit<AebAdapterInput, 'profile'>): AebNativeResult {
  ...
  function verifyArtifact(value: unknown, now: string):
    { ok: true; artifact: CcsV14Artifact; replay: AebDigest } |
    { ok: false; verified: boolean; acceptance: Acceptance; reason: string } {
    const artifact = parseArtifact(value);
    if (!artifact) return { ok: false, verified: false, acceptance: 'REJECTED', reason: 'ccs:v14_artifact_malformed' };
  ...
  return Object.freeze({
    ...
    verifyNative(input: Omit<AebAdapterInput, 'profile'>): AebNativeResult {
      ...
      try {
        ...
        const verdictAcceptance: Acceptance = verified.artifact.receipt.verdict === 'allow'
          ? 'ACCEPTED' : 'REJECTED';
...
export function ccsV14ArtifactDigest(artifact: CcsV14Artifact): AebDigest {
