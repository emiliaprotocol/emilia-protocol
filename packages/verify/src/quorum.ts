 ...
 */
function parseRequiredAlgorithms(
  value: unknown,
): { ok: true; required: string[] | null } | { ok: false; reason: string } {
  if (value === undefined || value === null) return { ok: true, required: null };
  if (!Array.isArray(value) || value.length === 0) {
    return { ok: false, reason: 'required_algorithms_malformed' };
  }
  const seen = new Set<string>();
  for (const alg of value) {
    if (typeof alg !== 'string' || !(WEBAUTHN_SIGNATURE_ALGORITHMS as readonly string[]).includes(alg)) {
      return { ok: false, reason: `required_algorithms_unknown:${String(alg)}` };
    }
    if (seen.has(alg)) return { ok: false, reason: `required_algorithms_duplicate:${alg}` };
    seen.add(alg);
  }
...
export function verifyQuorum(quorum: QuorumDocument | null | undefined, opts: Record<string, unknown> = {}) {
  ...
  try {
    ...
    const distinctnessKeys: unknown[] = requiredAlgorithms
    ...
    if (!requiredAlgorithms) {
      ...
      const algorithmsByApprover = new Map<string, Set<string>>();
      counted.forEach((_, i) => {
        const alg = countedAlgorithms[i];
        if (alg === null) return; // a credential of no recognized algorithm covers nothing
        const approverKey = JSON.stringify(countedApprovers[i] ?? null);
        const set = algorithmsByApprover.get(approverKey) ?? new Set<string>();
        set.add(alg);
      ...
      for (const [approverKey, algs] of algorithmsByApprover) {
        for (const alg of requiredAlgorithms) {
          if (!algs.has(alg)) missing.push(`${approverKey}:${alg}`);
        }
      }
