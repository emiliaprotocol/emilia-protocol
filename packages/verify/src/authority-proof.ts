...
function unsigned(proof: AuthorityProof): Record<string, unknown> {
...
export function authorityProofDigest(proof: AuthorityProof): string {
 ...
 */
export function verifyAuthorityProof(proof: AuthorityProof | null | undefined, opts: AuthorityProofOptions = {}) {
  ...
  const fail = (reason: string, extra: { checks?: Record<string, boolean>; proof_digest?: string } = {}) => ({ verified: false, accepted: false, checks: { ...checks, ...extra.checks }, reason, ...('proof_digest' in extra ? { proof_digest: extra.proof_digest } : {}) });
...
export const AUTHORITY_PROOF_V2_REQUIRED_ALGORITHMS = Object.freeze(['Ed25519', 'ML-DSA-65'] as const);

const AUTHORITY_PROOF_PQ_KEY_ID_RE = /^ep:authority-registry-key:ml-dsa-65:sha256:[0-9a-f]{64}$/;

export interface AuthorityProofV2Signature { alg?: unknown; sig?: unknown; key_id?: unknown; }
...
const AUTHORITY_PROOF_V2_SIGNATURE_KEYS = new Set<string>([
...
function pqKeyIdFor(publicKeyRawB64u: unknown): string {
  try {
 ...
 */
export function authorityProofV2SignedBytes(
  unsignedBody: Record<string, unknown>,
  requiredAlgorithms: readonly string[] = AUTHORITY_PROOF_V2_REQUIRED_ALGORITHMS,
): Buffer {
  ...
  return Buffer.from(
    AUTHORITY_PROOF_V2_DOMAIN + canonicalize({ ...unsignedBody, required_algorithms: [...requiredAlgorithms] }),
    'utf8',
  );
}

/** Digest of the v2 signed body, excluding the signature envelope. */
export function authorityProofV2Digest(proof: AuthorityProofV2): string {
  return `sha256:${sha256hex(authorityProofV2SignedBytes(unsigned(proof as unknown as AuthorityProof)))}`;
}
 ...
 */
export async function verifyAuthorityProofV2(
  proof: AuthorityProofV2 | null | undefined,
  opts: AuthorityProofV2Options = {},
): Promise<VerifyAuthorityProofV2Result> {
  opts = opts && typeof opts === 'object' ? opts : {};
  const checks: Record<string, boolean> = {
    version: true,
    structure: true,
  ...
  const done = (extra: Partial<VerifyAuthorityProofV2Result> = {}): VerifyAuthorityProofV2Result => {
  ...
  const sig = (proof.signature || null) as AuthorityProofV2SignatureBlock | null;
  ...
  if (!algorithmSetMatchesRegistered(sig?.required_algorithms)) {
    fail('algorithm_set',
      `signature.required_algorithms must be exactly ${JSON.stringify([...AUTHORITY_PROOF_V2_REQUIRED_ALGORITHMS])} (set narrowing / widening refused)`);
  }

  // 3. Exactly one signature per required algorithm; no duplicates, no strays.
  const signatures = Array.isArray(sig?.signatures) ? sig!.signatures as AuthorityProofV2Signature[] : null;
  if (!signatures || signatures.length === 0) {
    ...
    if (!malformed) {
      ...
      for (const alg of presented) {
        if (!(AUTHORITY_PROOF_V2_REQUIRED_ALGORITHMS as readonly string[]).includes(alg)) {
  ...
  try {
    if (presentedEdKey) {
      ...
      if (edObj.asymmetricKeyType === 'ed25519') derivedKeyId = keyIdFor(presentedEdKey);
    }
  } catch {
    derivedKeyId = '';
  }
  ...
  if (Number.isSafeInteger(opts.expectMinEpoch)
    && !(Number.isSafeInteger(proof.registry_epoch) && (proof.registry_epoch as number) >= (opts.expectMinEpoch as number))) {
  ...
  if (!pin) {
    fail('pinned_registry_key',
      'registry issuer Ed25519 + ML-DSA-65 key pair is not pinned for this authority_id (verified is not accepted)');
  }
  ...
  try {
    recomputedBytes = authorityProofV2SignedBytes(unsigned(proof as unknown as AuthorityProof), AUTHORITY_PROOF_V2_REQUIRED_ALGORITHMS);
  } catch {
  ...
  if (setResult?.verified !== true) {
    ...
    fail('signature',
      `registry signature set does not verify under the pinned Ed25519 + ML-DSA-65 keys (${reason})`);
  }
...
function agilityPassthrough(opts: AuthorityProofV2Options): AgilityOptions {
