...
export const POLICY_DECISION_EVIDENCE_V2_REQUIRED_ALGORITHMS = Object.freeze(['Ed25519', 'ML-DSA-65'] as const);
...
export interface PolicyDecisionEvidenceV2Claims
  extends Omit<PolicyDecisionEvidenceClaims, 'ep_version'> {
...
function policyV2SetMatchesRegistered(algorithms: unknown): algorithms is string[] {
...
export function policyDecisionEvidenceV2ProtectedHeader(
  keyId: string,
  pqKeyId: string,
  requiredAlgorithms: readonly string[] = POLICY_DECISION_EVIDENCE_V2_REQUIRED_ALGORITHMS,
): Obj {
...
export function policyDecisionEvidenceV2SigningInput(
...
function v2SignableClaims(value: unknown): value is PolicyDecisionEvidenceV2Claims {
  if (!isRecord(value) || value.ep_version !== POLICY_DECISION_EVIDENCE_V2_VERSION) return false;
...
export async function signPolicyDecisionEvidenceV2(
  claims: PolicyDecisionEvidenceV2Claims,
  signer: PolicyDecisionEvidenceV2Signer,
  options: AgilityOptions = {},
): Promise<PolicyDecisionEvidenceHybridStatement> {
  ...
  const signatures = await signAgileSet(
    new Uint8Array(policyDecisionEvidenceV2SigningInput(protectedB64u, payloadB64u)),
    [
 ...
 */
export async function verifyPolicyDecisionEvidenceV2(
  statement: unknown,
  pin: PolicyDecisionEvidenceV2KeyPin | null | undefined,
  config: PolicyDecisionEvidenceAdapterConfig | null | undefined,
  options: AgilityOptions = {},
): Promise<PolicyDecisionEvidenceV2Result> {
  ...
  const done = (claims?: PolicyDecisionEvidenceV2Claims): PolicyDecisionEvidenceV2Result =>
  ...
  if (!policyV2SetMatchesRegistered(header.required_algorithms)) {
    fail('algorithm_set',
      `protected header required_algorithms must be exactly ${JSON.stringify([...POLICY_DECISION_EVIDENCE_V2_REQUIRED_ALGORITHMS])} (set narrowing / widening refused)`);
  }
  ...
  if (!signatures || signatures.length === 0) {
    ...
    if (!malformed) {
      ...
      for (const alg of presented) {
        if (!(POLICY_DECISION_EVIDENCE_V2_REQUIRED_ALGORITHMS as readonly string[]).includes(alg)) {
  ...
  if (!pinnedEd || !pinnedPq
      || !validIdentifier((pin as PolicyDecisionEvidenceV2KeyPin)?.key_id)
      || !validIdentifier((pin as PolicyDecisionEvidenceV2KeyPin)?.pq_key_id)) {
    fail('engine_key_pinned',
      'a pinned Ed25519 + ML-DSA-65 policy-engine key pair is required (identified but not trusted)');
  } else {
    ...
    if (header.kid !== (pin as PolicyDecisionEvidenceV2KeyPin).key_id) {
      fail('engine_key_pinned', 'protected header kid != pinned Ed25519 key_id');
    }
    if (header.pq_kid !== (pin as PolicyDecisionEvidenceV2KeyPin).pq_key_id) {
  ...
  if (checks.engine_key_pinned) {
    ...
    try {
      expected = Buffer.from(canonicalizeAeb(policyDecisionEvidenceV2ProtectedHeader(
        (pin as PolicyDecisionEvidenceV2KeyPin).key_id,
        (pin as PolicyDecisionEvidenceV2KeyPin).pq_key_id,
  ...
  if (!parsedConfig) {
    fail('claims_valid', 'a valid pinned adapter configuration is required');
      ...
      || !parsedConfig.allowed_engines.includes(rawClaims.engine as PolicyEngineKind)
      || !parsedConfig.allowed_policy_digests.includes(rawClaims.policy_digest as AebDigest)
  ...
  try {
    setResult = await verifyAgileSignatureSet(
      new Uint8Array(policyDecisionEvidenceV2SigningInput(statement.protected, statement.payload)),
      signatures ?? [],
      [
        { alg: 'Ed25519', public_key: pinnedEd, key_id: (pin as PolicyDecisionEvidenceV2KeyPin)?.key_id },
        { alg: 'ML-DSA-65', public_key: pinnedPq, key_id: (pin as PolicyDecisionEvidenceV2KeyPin)?.pq_key_id },
      ],
      {
        ...options,
        policy: 'hybrid_all',
  ...
  if (setResult?.verified !== true) {
    ...
    fail('signature_valid',
      `policy decision signature set does not verify under the pinned Ed25519 + ML-DSA-65 keys (${reason})`);
  ...
  return done(checks.claims_valid ? (rawClaims as unknown as PolicyDecisionEvidenceV2Claims) : undefined);
}
