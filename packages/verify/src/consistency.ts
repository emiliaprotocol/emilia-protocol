...
function hashChildrenV2(left: string, right: string): string {
  return crypto
    .createHash('sha256')
 ...
 */
export function verifyCheckpointConsistency(oldRoot: unknown, oldSize: number, newRoot: unknown, newSize: number, proof: unknown): boolean {
  ...
  const path = (proof as unknown[]).map(hexOf);
 ...
 */
export function buildConsistencyProof(m: number, n: number, leaves: string[]): string[] {
...
function subproof(m: number, d: string[], b: boolean): string[] {
  const n = d.length;
  if (m === n) {
