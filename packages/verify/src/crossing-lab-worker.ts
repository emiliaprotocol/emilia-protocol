...
process.stdin.on('end', async () => {
  try {
    ...
    if (actualDigest !== request.adapter_digest) throw new TypeError('adapter source digest mismatch');
    ...
    if (Buffer.byteLength(output, 'utf8') > 262_144) throw new TypeError('adapter output exceeds limit');
