...
function grantSignedBody(grant: Obj): Obj | null {
 ...
 */
export function computeGrantHash(grant: Obj): string | null {
 ...
 * @returns {boolean}
 */
export function verifyGrantHash(grant: Obj): boolean {
 ...
 */
export function buildConsentGrant(spec: Obj, signer: any): Obj {
...
function refuseGrant(reason: string, checks: Record<string, boolean>): GrantResult {
 ...
 */
export function verifyConsentGrant(grant: Obj, pinnedPrincipalKey: string | undefined, opts: ConsentGrantOptions = {}): GrantResult {
  const checks: Record<string, boolean> = { hash: false, signature: false, within_window: false };
  ...
  if (opts.revocation !== undefined) {
    ...
    const rev = verifyRevocation(target as any, opts.revocation, {
 ...
 */
export function receiptReferencedGrantHash(receipt: Obj, overrideGrantHash?: string): string | null {
 ...
 */
export function receiptGrantBindingStrength(receipt: Obj, overrideGrantHash?: string): 'signed_action' | 'top_level' | 'caller_override' | 'none' {
  if (receipt && typeof receipt === 'object') {
    const action = receipt.action;
...
function refuseComposition(reason: string, checks: Record<string, boolean>): { ok: false; checks: Record<string, boolean>; reason: string } {
 ...
 */
export function verifyReceiptUnderGrant(receipt: Obj, grant: Obj, opts: CompositionOptions = {}) {
...
export const CONSENT_GRANT_V2_REQUIRED_ALGORITHMS = Object.freeze(['Ed25519', 'ML-DSA-65'] as const);

export interface ConsentGrantV2Signature { alg?: unknown; sig?: unknown; key_id?: unknown; }
...
function algorithmSetMatchesRegisteredGrant(algorithms: unknown): algorithms is string[] {
...
function agilityPassthroughGrant(opts: ConsentGrantV2Options): AgilityOptions {
...
function toRawB64uGrant(value: Uint8Array | string, expectedLength: number, label: string): string {
 ...
 */
function grantSignedBodyV2(grant: Obj): Obj {
  if (!grant || typeof grant !== 'object' || Array.isArray(grant)) {
 ...
 */
export function computeGrantHashV2(grant: Obj): string | null {
  try {
 ...
 */
export async function buildConsentGrantV2(spec: Obj, signer: ConsentGrantV2Signer): Promise<Obj> {
  ...
  const ordered = CONSENT_GRANT_V2_REQUIRED_ALGORITHMS.map((alg) => {
    ...
    if (!s) throw new Error(`buildConsentGrantV2: signing produced no ${alg} leg`);
    return s;
  });
 ...
 */
export async function verifyConsentGrantV2(
  grant: Obj,
  pinnedPrincipalKeys: PinnedPrincipalKeysV2 | undefined,
  opts: ConsentGrantV2Options = {},
  ...
  if (!algorithmSetMatchesRegisteredGrant(grant.required_algorithms)) {
    fail('algorithm_set',
      `grant.required_algorithms must be exactly ${JSON.stringify([...CONSENT_GRANT_V2_REQUIRED_ALGORITHMS])} (set narrowing / widening refused)`);
  }

  // 4. exactly one signature per required algorithm.
  const signatures = Array.isArray(grant.signatures) ? grant.signatures as ConsentGrantV2Signature[] : null;
  if (!signatures || signatures.length === 0) {
    ...
    if (!malformed) {
      ...
      for (const alg of presented) {
        if (!(CONSENT_GRANT_V2_REQUIRED_ALGORITHMS as readonly string[]).includes(alg)) {
  ...
  if (!signatures) {
    ...
    if (setResult?.verified !== true) {
      ...
      fail('signature_set_valid',
        `grant signature set does not verify under the pinned Ed25519 + ML-DSA-65 keys (${reason})`);
    }
  }
 ...
 */
export async function verifyConsentGrantStatement(
  grant: Obj,
  pinnedKeys: string | PinnedPrincipalKeysV2 | undefined,
  opts: ConsentGrantOptions | ConsentGrantV2Options = {},
): Promise<GrantResult | GrantV2Result> {
  if (grant && typeof grant === 'object' && !Array.isArray(grant) && grant.profile === CONSENT_GRANT_V2_VERSION) {
    return verifyConsentGrantV2(grant, pinnedKeys as PinnedPrincipalKeysV2 | undefined, opts as ConsentGrantV2Options);
  }
  return verifyConsentGrant(grant, pinnedKeys as string | undefined, opts as ConsentGrantOptions);
}
