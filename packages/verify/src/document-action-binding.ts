 ...
 * @returns {value is Record<string, any>}
 */
function exactKeys(value: unknown, allowed: Set<string>, required: Set<string> = allowed): value is Obj {
...
function canonicalJsonIssue(value: unknown): string | null {
  const seen = new WeakSet();
  const state = { nodes: 0 };

  function visit(current: any, depth: number): string | null {
    state.nodes += 1;
...
function strictBase64url(value: unknown, expectedLength?: number): Buffer | null {
...
function strictDate(value: unknown): boolean {
...
function canonicalCopy(value: unknown): Obj {
...
function sortedBy(items: any[], key: (item: any) => string): any[] {
...
function partySortKey(party: Obj): string {
  return `${party.role}\0${party.party_id}`;
}

function isCanonicallySorted(items: any[], key: (item: any) => string): boolean {
...
function validateTerm(term: unknown): term is Obj {
  if (!isRecord(term)
 ...
 */
function validateMaterialTerms(terms: unknown): { ok: true } | { ok: false; reason: string } {
...
function amountFieldsValid(value: unknown): boolean {
 ...
 */
function validateReleaseAction(releaseAction: unknown): { ok: true; digest: string } | { ok: false; reason: string } {
 ...
 * @returns {{ok:true}|{ok:false,reason:string}}
 */
function validatePartyArray(parties: unknown, label: string): { ok: true } | { ok: false; reason: string } {
 ...
 * @returns {{ok:true}|{ok:false,reason:string}}
 */
function validateValidity(validity: unknown, now: number): { ok: true } | { ok: false; reason: string } {
 ...
 */
function bindingCore(binding: unknown): Obj | null {
...
function signingBytes(core: Obj): Buffer {
 ...
 * @returns {{ok:true}|{ok:false,reason:string}}
 */
function validateCore(core: Obj, now: number): { ok: true } | { ok: false; reason: string } {
...
function normalizedStringSet(value: unknown): Set<string> | null {
 ...
 * @returns {string|null}
 */
export function computeReleaseActionDigest(template: Obj): string | null {
  try {
 ...
 * @returns {string|null}
 */
export function computeDocumentActionBindingDigest(binding: Obj): string | null {
  try {
 ...
 * @returns {object}
 */
export function signDocumentActionBinding(spec: Obj, signer: Obj): Obj {
 ...
 * }}
 */
export function verifyDocumentActionBinding(binding: unknown, opts: DABOptions = {}) {
