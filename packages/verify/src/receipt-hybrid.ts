...
function exactKeys(value: unknown, allowed: Set<string>, optional: Set<string> = new Set()): boolean {
  if (value === null || typeof value !== 'object' || Array.isArray(value)) return false;
  const keys = Object.keys(value);
  if (!keys.every((key) => allowed.has(key))) return false;
  for (const required of allowed) {
    if (!optional.has(required) && !keys.includes(required)) return false;
  }
  return true;
}
 ...
 */
export function hybridReceiptSignedMaterial(
  payload: AnyRecord,
...
export function hybridReceiptSignedBytes(
  payload: AnyRecord,
  requiredAlgorithms: readonly string[] = HYBRID_RECEIPT_REQUIRED_ALGORITHMS,
): Buffer {
...
export async function verifyHybridReceipt(
  doc: unknown,
  keys: HybridReceiptVerificationKeys | null | undefined,
  ...
  const refuse = (
    ...
    setResult: AgileSetResult | null = null,
  ...
  if (!isStrictCanonicalJson(doc)) return refuse(HYBRID_RECEIPT_REASONS.MALFORMED_RECEIPT);
  const d = doc as Partial<HybridReceiptDocument> & AnyRecord;
  ...
  if (!exactKeys(d.profile, PROFILE_KEYS)) return refuse(HYBRID_RECEIPT_REASONS.MALFORMED_RECEIPT);
  ...
  let setResult: AgileSetResult | null;
  try {
 ...
 */
function mapSetFailure(setResult: AgileSetResult | null): [string, string | null] {
 ...
 */
export async function verifyReceiptOfAnyProfile(
  doc: unknown,
  keys: Partial<HybridReceiptVerificationKeys> | null | undefined,
  options: AgilityOptions & { allowLegacyMerkle?: boolean } = {},
): Promise<ReceiptProfileRouteResult> {
  const version = doc && typeof doc === 'object' && !Array.isArray(doc)
    ? (doc as AnyRecord)['@version']
    : null;

  if (version === HYBRID_RECEIPT_PROFILE) {
    const result = await verifyHybridReceipt(doc, keys as HybridReceiptVerificationKeys, options);
    return {
 ...
 */
export function logCheckpointSignedFields(checkpoint: unknown): AnyRecord | null {
  if (!checkpoint || typeof checkpoint !== 'object' || Array.isArray(checkpoint)) return null;
  if (!isStrictCanonicalJson(checkpoint)) return null;
  const source = checkpoint as AnyRecord;
  const fields: AnyRecord = {};
  for (const key of Object.keys(source)) {
    if (key === 'log_signature') continue;
    if (!CHECKPOINT_SIGNED_KEYS.has(key)) return null; // closed set; no smuggled members
    fields[key] = source[key];
  }
  if (!exactKeys(fields, CHECKPOINT_SIGNED_KEYS)) return null;
  if (!Number.isSafeInteger(fields.tree_size) || fields.tree_size < 1) return null;
  if (typeof fields.root_hash !== 'string' || !/^sha256:[0-9a-f]{64}$/.test(fields.root_hash)) return null;
  if (typeof fields.log_key_id !== 'string' || fields.log_key_id.length === 0) return null;
  if (typeof fields.merkle_alg !== 'string' || fields.merkle_alg.length === 0) return null;
  return fields;
}
 ...
 */
export async function verifyLogCheckpointHybridProof(
  heldCheckpoint: unknown,
  proof: unknown,
  keys: HybridReceiptVerificationKeys | null | undefined,
  options: AgilityOptions = {},
): Promise<LogCheckpointHybridVerifyResult> {
  const checks: LogCheckpointHybridChecks = {
    profile: false,
    algorithm_set: null,
  ...
  const refuse = (
    ...
    failedAlgorithm: string | null = null,
    setResult: AgileSetResult | null = null,
  ): LogCheckpointHybridVerifyResult => ({
    verified: false,
    reason,
    failed_algorithm: failedAlgorithm,
    checks,
  ...
  if (!isStrictCanonicalJson(proof)) return refuse(LOG_CHECKPOINT_HYBRID_REASONS.MALFORMED_PROOF);
  const p = proof as Partial<LogCheckpointHybridProof> & AnyRecord;
  if (p['@version'] !== LOG_CHECKPOINT_HYBRID_PROFILE) {
    return refuse(LOG_CHECKPOINT_HYBRID_REASONS.UNKNOWN_PROFILE);
  }
  if (!exactKeys(p, CHECKPOINT_PROOF_KEYS)) return refuse(LOG_CHECKPOINT_HYBRID_REASONS.MALFORMED_PROOF);
  if (!exactKeys(p.profile, PROFILE_KEYS)) return refuse(LOG_CHECKPOINT_HYBRID_REASONS.MALFORMED_PROOF);
  ...
  for (const alg of presented) {
    if (!(LOG_CHECKPOINT_HYBRID_REQUIRED_ALGORITHMS as readonly string[]).includes(alg)) {
      checks.legs_present = false;
