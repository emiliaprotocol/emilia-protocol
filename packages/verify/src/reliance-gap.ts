...
const digestOf = (value: any): string => `sha256:${sha256hex(Buffer.from(canonicalize(value), 'utf8'))}`;

/** Parse a caller-supplied evaluation time. NEVER falls back to the wall clock. */
function parseTimeMs(t: unknown): number | null {
...
function refusal(reason: string): Obj {
  return sortKeysDeep({
 ...
 */
function classifyEvidenceItem(item: any): Obj {
  ...
  const a = item;
  if (a === null || typeof a !== 'object') return { slot: null, artifact: a, declared: 'unknown' };
  if (a['@type'] === 'EP-AUTHORITY-PROOF-v1') return { slot: 'authority_proof', artifact: a, declared: 'authority_proof' };
  ...
  if (Array.isArray(a.contexts) && Array.isArray(a.signoffs)) return { slot: 'receipt', artifact: a, declared: 'receipt' };
  if (typeof a.consumed === 'boolean') return { slot: 'consumption', artifact: a, declared: 'consumption' };
 ...
 * honestly never run).
 */
function buildControlMapping(kernel: Obj): any[] {
...
function resolveProfile(profile: any): Obj {
  let pinned = profile;
  let id = null;
  ...
  let digest = null;
  try { digest = digestOf(pinned); } catch { digest = null; }
 ...
 */
export function buildRelianceGapReport(packet: Obj, profile: Obj, opts: GapOptions = {}): Obj {
  ...
  let actionDigest;
  try { actionDigest = digestOf(packet.action); } catch {
  ...
  evidence.forEach((item: any, index: number) => {
    ...
    if (slot === null) {
      ...
      if (!foreignTypes.includes(declared)) foreignTypes.push(declared);
  ...
  const gaps = new Map<string, Obj>();
  const addGap = (g: Obj): void => { gaps.set(g.requirement, g); };
  if (pinned && typeof pinned === 'object' && pinned['@type'] === RELIANCE_PROFILE_VERSION) {
    ...
    if (!slots.receipt) addGap(PRESENCE_GAPS.receipt);
    if (pinned.required_assurance === 'quorum' && !slots.quorum) addGap(PRESENCE_GAPS.quorum);
    ...
    if (requiredEvidence.has('revocation_freshness') && !slots.revocation_state) addGap(PRESENCE_GAPS.revocation_freshness);
    if (requiredEvidence.has('consumption_proof') && !slots.consumption) addGap(PRESENCE_GAPS.consumption_proof);
  }
  if (kernel.verdict !== 'rely' && (VERDICT_GAPS as Obj)[kernel.verdict]) {
    addGap((VERDICT_GAPS as Obj)[kernel.verdict]);
  }
 ...
 */
export function buildMultiPartyRelianceGapReport(packet: Obj, profiles: any[], opts: GapOptions = {}): Obj {
  ...
  for (const entry of labeled) {
    ...
    if (report.refused === true) return report; // packet-level refusal: same for every profile
