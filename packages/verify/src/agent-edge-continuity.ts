...
function validScope(scope: unknown): scope is ContinuityScope {
  if (!isObject(scope) || !Array.isArray(scope.action_types) || !Array.isArray(scope.resources)) return false;
  if (!scope.action_types.every((value) => typeof value === 'string' && ID_RE.test(value))) return false;
  if (!scope.resources.every((value) => typeof value === 'string' && ID_RE.test(value))) return false;
  return scope.max_amount_minor === undefined
    || (typeof scope.max_amount_minor === 'string' && /^(0|[1-9][0-9]*)$/.test(scope.max_amount_minor));
}

function validClaims(value: unknown): value is AgentContinuityClaims {
  if (!isObject(value) || Object.keys(value).some((key) => !CLAIM_KEYS.has(key))) return false;
  const digestKeys = [
  ...
  for (const key of digestKeys) if (value[key] !== undefined && !validDigest(value[key])) return false;
  const idKeys = ['model_id', 'model_version', 'tool_id', 'from_agent', 'to_agent', 'executor_id'];
  for (const key of idKeys) if (value[key] !== undefined && !validId(value[key])) return false;
  if (value.protocol !== undefined && !['native', 'MCP', 'A2A'].includes(value.protocol as string)) return false;
  if (value.outcome !== undefined && !['COMMITTED', 'NOT_COMMITTED', 'INDETERMINATE'].includes(value.outcome as string)) return false;
  return value.scope === undefined || validScope(value.scope);
}

function edgeClaimReasons(envelope: Pick<AgentContinuityEnvelope,
  'edge' | 'source' | 'destination' | 'initiator_id' | 'executor_id' | 'claims'>): string[] {
  const { edge, source, destination, initiator_id: initiatorId, executor_id: executorId, claims } = envelope;
  const reasons: string[] = [];
  if (edge === 'user-harness') {
    if (source !== initiatorId) reasons.push('initiator_source_mismatch');
    if (!claims.intent_digest || !claims.display_digest) reasons.push('user_intent_binding_missing');
  }
  if (edge === 'harness-model') {
    for (const key of [
      'model_id', 'model_version', 'model_manifest_digest', 'harness_digest',
      'prompt_context_digest', 'output_digest',
    ] as const) if (!claims[key]) reasons.push(`model_provenance_missing:${key}`);
  }
  if (edge === 'model-harness' && !claims.output_digest) reasons.push('model_output_binding_missing');
  if (edge === 'harness-tool') {
    if (claims.protocol !== 'MCP') reasons.push('mcp_protocol_not_declared');
    if (claims.tool_id !== destination) reasons.push('mcp_tool_endpoint_mismatch');
    for (const key of ['tool_id', 'tool_schema_digest', 'request_digest'] as const) {
      if (!claims[key]) reasons.push(`mcp_mapping_missing:${key}`);
    }
  }
  if (edge === 'agent-agent') {
    if (claims.protocol !== 'A2A') reasons.push('a2a_protocol_not_declared');
    if (claims.from_agent !== source || claims.to_agent !== destination) reasons.push('a2a_endpoints_not_bound');
    for (const key of ['delegation_digest', 'scope_digest'] as const) {
      if (!claims[key]) reasons.push(`a2a_mapping_missing:${key}`);
    }
    if (!claims.scope) reasons.push('a2a_scope_missing');
    else if (claims.scope_digest !== digestAeb(claims.scope)) reasons.push('a2a_scope_digest_mismatch');
  }
  if (edge === 'effect') {
    if (claims.executor_id !== executorId || destination !== executorId) reasons.push('effect_executor_mismatch');
    for (const key of ['executor_id', 'effect_digest', 'outcome'] as const) {
      if (!claims[key]) reasons.push(`effect_observation_missing:${key}`);
    }
  }
  return reasons;
}

function shapeReasons(value: unknown): string[] {
  const reasons: string[] = [];
  if (!isObject(value) || value['@type'] !== AGENT_CONTINUITY_VERSION) return ['invalid_type'];
  ...
  if (!isObject(signature) || signature.alg !== 'Ed25519' || !validId(signature.key_id)
      || !validBase64url(signature.value) || Buffer.from(signature.value as string, 'base64url').length !== 64) {
...
function endpointPinReasons(
  envelope: Pick<AgentContinuityEnvelope, 'source' | 'destination' | 'claims'>,
  pins: ContinuityVerifyOptions['endpoint_pins'],
): string[] {
  if (!pins) return [];
  const reasons: string[] = [];
  for (const [endpoint, prefix] of [[envelope.source, 'source'], [envelope.destination, 'destination']] as const) {
    const pin = pins[endpoint];
    for (const kind of ['identity', 'discovery', 'attestation'] as const) {
      const claimed = envelope.claims[`${prefix}_${kind}_digest` as keyof AgentContinuityClaims] as AebDigest | undefined;
      const expected = pin?.[`${kind}_digest`];
      if (claimed !== undefined && expected !== claimed) reasons.push(`endpoint_${kind}_pin_mismatch:${endpoint}`);
      if (expected !== undefined && claimed === undefined) reasons.push(`endpoint_${kind}_evidence_missing:${endpoint}`);
    }
  }
  return reasons;
}

function unsignedForSignature(envelope: AgentContinuityEnvelope): unknown {
...
function publicKeyObject(value: string | KeyObject): KeyObject {
...
function continuityId(body: Omit<AgentContinuityEnvelope, 'continuity_id' | 'signature'>): string {
  return `ec:${digestAeb(body).slice('sha256:'.length)}`;
}

function validTopology(topology: ContinuityTopologyPolicy): boolean {
  if (!isObject(topology) || !Array.isArray(topology.accepted_edges)
      || !isObject(topology.allowed_transitions)) return false;
  ...
  return accepted.size === topology.accepted_edges.length
    ...
    && transitionEntries.every(([parent, children]) => EDGES.has(parent as AgentEdge)
      && accepted.has(parent as AgentEdge)
...
export function createAgentContinuityEnvelope(options: ContinuityBuildOptions): AgentContinuityEnvelope {
  const body: Omit<AgentContinuityEnvelope, 'continuity_id' | 'signature'> = {
  ...
  const candidate: AgentContinuityEnvelope = {
    ...body,
    continuity_id: continuityId(body),
    signature: { alg: 'Ed25519', key_id: options.signer.key_id, value: '' },
  };
  candidate.signature.value = crypto.sign(
    null,
    signingBytes(unsignedForSignature(candidate)),
    options.signer.private_key,
  ).toString('base64url');
  const reasons = shapeReasons(candidate);
  if (reasons.length > 0) throw new Error(`invalid continuity envelope: ${reasons.join(',')}`);
  return candidate;
}

type ContinuityEdgeOptions = Omit<ContinuityBuildOptions, 'edge' | 'claims'>;

export function createUserHarnessContinuity(
  options: ContinuityEdgeOptions & { intent: unknown; display: unknown; scope?: ContinuityScope },
...
export function createHarnessModelContinuity(options: ContinuityEdgeOptions & {
  model_id: string;
  model_version: string;
...
export function createMcpToolContinuity(options: ContinuityEdgeOptions & {
  tool_id: string;
  tool_schema: unknown;
  request: unknown;
  scope?: ContinuityScope;
...
export function createA2AHandoffContinuity(options: ContinuityEdgeOptions & {
  from_agent: string;
  to_agent: string;
  delegation: unknown;
  scope: ContinuityScope;
...
export function createEffectContinuity(options: ContinuityEdgeOptions & {
  executor_id: string;
  effect: unknown;
  outcome: AgentOutcome;
}): AgentContinuityEnvelope {
...
function blankVerification(reason: string): ContinuityVerification {
  return {
    valid: false,
    checks: {
      schema: false,
...
export function verifyAgentContinuityEnvelope(
  value: unknown,
  options: ContinuityVerifyOptions,
  ...
  try {
    ...
    const envelope = value as unknown as AgentContinuityEnvelope;
    ...
    if (pin) {
      try {
        checks.signature = crypto.verify(
          null,
          signingBytes(unsignedForSignature(envelope)),
          publicKeyObject(pin.public_key),
...
function scopeContained(child: ContinuityScope, parent: ContinuityScope): boolean {
  if (!isSubset(child.action_types, parent.action_types) || !isSubset(child.resources, parent.resources)) return false;
  if (parent.max_amount_minor !== undefined) {
    if (child.max_amount_minor === undefined) return false;
    try {
      if (BigInt(child.max_amount_minor) > BigInt(parent.max_amount_minor)) return false;
    } catch {
      return false;
    }
  }
...
function blankGraph(reason: string): ContinuityGraphVerification {
...
export function verifyAgentContinuityGraph(
  values: readonly unknown[],
  options: ContinuityVerifyOptions,
): ContinuityGraphVerification {
  try {
    const checks = {
      schema: true,
      identity: true,
    ...
    const reasons: string[] = [];
    const envelopes: AgentContinuityEnvelope[] = [];
    ...
    for (const value of values) {
      ...
      ] as const) checks[key] = checks[key] && result.checks[key];
      reasons.push(...result.reasons);
      if (!result.checks.schema || !isObject(value)) continue;
      const envelope = value as unknown as AgentContinuityEnvelope;
...
type ExecutionVerifierOptions = Omit<ContinuityVerifyOptions,
  | 'now'
  | 'expected_caid'
...
function executionVerification(
  options: Omit<ContinuityExecutionOptions, 'store'> & { store: unknown },
): ContinuityGraphVerification {
...
function preExecutionGraphValid(
  values: readonly unknown[],
  topology: ContinuityTopologyPolicy,
): boolean {
  const envelopes = values.filter(isObject) as unknown as AgentContinuityEnvelope[];
...
export function authorizeAgentContinuityExecution(
  options: ContinuityExecutionOptions,
): ContinuityExecutionDecision {
...
export async function authorizeAgentContinuityExecutionDurable(
  options: DurableContinuityExecutionOptions,
): Promise<ContinuityExecutionDecision> {
  const continuity = executionVerification(options);
  if (!continuity.valid) {
    return {
      allowed: false,
