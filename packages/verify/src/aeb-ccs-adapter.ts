 ...
 * contracts is silently upgraded or relabeled as another.
 *
...
interface ParsedL1Pins {
  config: CcsL1AebAdapterConfig;
  root: CcsL1Ed25519TrustRoot;
  rootKey: Buffer;
  configDigest: AebDigest;
  rootsDigest: AebDigest;
}

interface VerifiedCcsL1 {
  receipt: CcsL1Receipt;
  replayUnit: AebDigest;
}

function decodeCanonicalBase64(value: unknown, length: number): Buffer | null {
...
function strictFiniteJsonClone<T>(value: T): T | null {
  try { return JSON.parse(canonicalizeFiniteJson(value)) as T; } catch { return null; }
}

function validOptionalHash(value: unknown): value is string {
  return value === '' || (typeof value === 'string' && HEX_64_RE.test(value));
}

function parseL1Receipt(value: unknown): CcsL1Receipt | null {
  if (!isRecord(value) || !exactKeys(value, CCS_L1_RECEIPT_KEYS)
      ...
      || !finiteNonNegative(value.verified_at) || !finiteNonNegative(value.latency_us)) return null;
  return strictFiniteJsonClone(value) as CcsL1Receipt | null;
}

function parseL1Config(value: unknown): CcsL1AebAdapterConfig | null {
  ...
  return strictJsonClone(value) as CcsL1AebAdapterConfig | null;
}

function parseL1Root(value: unknown, config: CcsL1AebAdapterConfig):
  { root: CcsL1Ed25519TrustRoot; key: Buffer } | null {
  ...
  const root = strictJsonClone(value) as CcsL1Ed25519TrustRoot | null;
...
function parseL1Pins(input: {
  ...
  if (!parsedRoot) throw new TypeError('valid issuer-scoped CCS Ed25519 root required');
  return {
    config,
    root: parsedRoot.root,
...
function l1SignatureValid(receipt: CcsL1Receipt): boolean {
  ...
  if (fingerprint !== receipt.public_key_fingerprint) return false;
  let publicKey;
  try {
...
function verifyL1Artifact(value: unknown, pins: ParsedL1Pins, now: string):
  { ok: true; value: VerifiedCcsL1 } |
  { ok: false; verified: boolean; acceptance: Acceptance; reason: string } {
...
function canonicalL1Action(value: unknown, actionType: string): Obj | null {
  if (!isRecord(value) || !exactKeys(value, CCS_L1_ACTION_KEYS)
      ...
      || !validIdentifier(value.parameters.action) || !validCcsToken(value.parameters.tool)) return null;
...
function l1ArgsDigest(value: Obj): string | null {
  try {
...
export function createCcsL1AebActionDefinition(actionType: string): Obj {
  if (!ACTION_TYPE_RE.test(actionType)) throw new TypeError('valid CAID action type required');
  return {
...
function validL1MappingProfile(profile: AebPinnedProfile, actionType: string): unknown[] | null {
  ...
  if (!isRecord(profile) || !exactKeys(profile, CCS_L1_PROFILE_KEYS)
      ...
      || !sameDigest(
        ...
        CCS_L1_OMITTED_NONMATERIAL_FIELDS,
      )
      ...
      || profile.profile_digest !== mappingProfileDigest(CCS_L1_MAPPING_PROFILE_ID, profile)) return null;
  return profile.definition.definitions;
}

function fallbackL1(input: Omit<AebAdapterInput, 'profile'>, pins: ParsedL1Pins): AebNativeResult {
  const evidenceDigest = safeDigest(input.artifact);
  return {
    native_verification: 'FAILED',
    acceptance: 'REJECTED',
