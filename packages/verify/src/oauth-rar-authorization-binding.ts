...
export const OAUTH_RAR_AUTHORIZATION_BINDING_VERSION =
  'EP-OAUTH-RAR-AUTHORIZATION-BINDING-v1';
...
export type OAuthRarAuthorizationBindingVerdict =
  | 'MATCH'
  | 'MISMATCH'
  | 'INDETERMINATE';
...
export function parseOAuthRarAuthorizationBinding(
  value: unknown,
): OAuthRarAuthorizationBinding | null {
  ...
  const normalized: OAuthRarAuthorizationBinding = {
 ...
 * mismatches.
 */
export function matchOAuthRarAuthorizationBinding(
  presented: unknown,
  expected: unknown,
): OAuthRarAuthorizationBindingResult {
