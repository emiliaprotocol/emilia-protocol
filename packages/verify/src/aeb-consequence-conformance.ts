...
export type AebConsequenceErrorCode =
  | 'invalid_object'
  | 'unknown_key'
  | 'missing_key'
  | 'invalid_string'
...
function fail(code: AebConsequenceErrorCode, path: string): never {
...
function canonicalizeInternal(
  ...
  if (depth > AEB_CONSEQUENCE_LIMITS.max_depth) fail('max_depth_exceeded', path);
  state.nodes += 1;
  if (state.nodes > AEB_CONSEQUENCE_LIMITS.max_nodes) fail('max_nodes_exceeded', path);
  ...
  if (typeof value === 'number') {
    if (!Number.isSafeInteger(value)) fail('non_canonical_value', path);
    return JSON.stringify(value);
  }
  if (typeof value !== 'object' || value === undefined) fail('non_canonical_value', path);
  if (state.seen.has(value)) fail('cyclic_value', path);
...
export function canonicalizeAebConsequenceConformance(value: unknown): string {
...
export function digestAebConsequenceConformance(value: unknown): AebConsequenceDigest {
...
export function digestAebConsequenceCase(value: unknown): AebConsequenceDigest {
...
function exactObject(value: unknown, keys: readonly string[], path: string): Record<string, unknown> {
  ...
  for (const key of Object.keys(value)) if (!allowed.has(key)) fail('unknown_key', `${path}.${key}`);
  for (const key of keys) if (!Object.hasOwn(value, key)) fail('missing_key', `${path}.${key}`);
  return value;
}

function boundedString(value: unknown, path: string, pattern?: RegExp): string {
  if (typeof value !== 'string' || value.length === 0 || !validUnicode(value)
      || Buffer.byteLength(value, 'utf8') > AEB_CONSEQUENCE_LIMITS.max_string_bytes
      || (pattern && !pattern.test(value))) fail('invalid_string', path);
  return value;
}

function enumValue<T extends string>(value: unknown, allowed: readonly T[], path: string): T {
  if (typeof value !== 'string' || !allowed.includes(value as T)) fail('invalid_enum', path);
  return value as T;
}

function digestValue(value: unknown, path: string): AebConsequenceDigest {
  if (typeof value !== 'string' || !DIGEST_RE.test(value)) fail('invalid_digest', path);
  return value as AebConsequenceDigest;
}

function caidValue(value: unknown, path: string): string {
  if (typeof value !== 'string' || !CAID_RE.test(value)) fail('invalid_caid', path);
  return value;
}

function instantValue(value: unknown, path: string): string {
  if (typeof value !== 'string' || !RFC3339_SECOND_RE.test(value)) fail('invalid_instant', path);
...
function arrayValue(value: unknown, maximum: number, path: string): unknown[] {
  if (!Array.isArray(value) || value.length > maximum) fail('invalid_array', path);
  return value;
}

function unique(values: readonly string[], path: string): void {
  if (new Set(values).size !== values.length) fail('duplicate_value', path);
}
...
function parseRequirement(value: unknown, path: string): AebConsequenceRequirement {
  ...
  if (!Number.isSafeInteger(minimum) || Number(minimum) < 1
      || Number(minimum) > AEB_CONSEQUENCE_LIMITS.max_evidence) fail('invalid_integer', `${path}.minimum`);
  if (typeof object.distinct_principals !== 'boolean') fail('invalid_boolean', `${path}.distinct_principals`);
  if (typeof object.exclude_initiator !== 'boolean') fail('invalid_boolean', `${path}.exclude_initiator`);
  if (typeof object.exclude_executor !== 'boolean') fail('invalid_boolean', `${path}.exclude_executor`);
  return {
...
function parseOperation(value: unknown, path: string): AebConsequenceOperation {
  ...
  const requirements = arrayValue(
    object.requirements,
    AEB_CONSEQUENCE_LIMITS.max_requirements,
    `${path}.requirements`,
  ).map((entry, index) => parseRequirement(entry, `${path}.requirements[${index}]`));
  if (requirements.length === 0) fail('invalid_array', `${path}.requirements`);
...
function parseStatus(value: unknown, path: string): AebConsequenceStatus {
  const object = exactObject(value, ['verdict', 'authority_pinned', 'checked_at', 'valid_until'], path);
  if (typeof object.authority_pinned !== 'boolean') fail('invalid_boolean', `${path}.authority_pinned`);
  ...
  if (Date.parse(checkedAt) > Date.parse(validUntil)) fail('invalid_combination', path);
  return {
...
function parseEvidence(value: unknown, path: string): AebConsequenceEvidence {
  const object = exactObject(value, [
...
function truthPairValid(provider: ProviderOutcome, effect: EffectRelation): boolean {
  if (provider === 'NOT_INVOKED') return effect === 'NOT_OBSERVED';
  if (provider === 'PROVEN_NOT_COMMITTED') return effect === 'NOT_OBSERVED';
  if (provider === 'INDETERMINATE') return effect === 'INDETERMINATE';
  return provider === 'COMMITTED';
}

function terminalTruthPair(provider: ProviderOutcome, effect: EffectRelation): boolean {
  return (provider === 'PROVEN_NOT_COMMITTED' && effect === 'NOT_OBSERVED')
    || (provider === 'COMMITTED'
      && (effect === 'OBSERVED_AS_REQUESTED' || effect === 'DIVERGED'));
}

function parsePriorOperation(value: unknown, path: string): AebConsequencePriorOperation {
  const object = exactObject(value, [
...
function parseReservation(value: unknown, path: string): AebConsequenceReservationInput {
  const object = exactObject(value, ['atomicity', 'prior_operations', 'consumed_native_replay_units'], path);
  const priorOperations = arrayValue(
    object.prior_operations,
    AEB_CONSEQUENCE_LIMITS.max_prior_operations,
    `${path}.prior_operations`,
  ).map((entry, index) => parsePriorOperation(entry, `${path}.prior_operations[${index}]`));
  unique(priorOperations.map((entry) => entry.operation_id), `${path}.prior_operations`);
  const replayUnits = arrayValue(
    object.consumed_native_replay_units,
    AEB_CONSEQUENCE_LIMITS.max_replay_units,
    `${path}.consumed_native_replay_units`,
  ).map((entry, index) => digestValue(entry, `${path}.consumed_native_replay_units[${index}]`));
...
function parseObservation(value: unknown, path: string): AebConsequenceObservation | null {
  if (value === null) return null;
  ...
  const effect = enumValue(object.effect_relation, ['NOT_OBSERVED', 'OBSERVED_AS_REQUESTED', 'DIVERGED', 'INDETERMINATE'], `${path}.effect_relation`);
  if (!truthPairValid(provider, effect)) fail('invalid_combination', path);
...
function parseReconciliation(value: unknown, path: string): AebConsequenceReconciliation | null {
  if (value === null) return null;
  const object = exactObject(value, [
    'authenticated', 'provider_id', 'operation_id', 'caid',
    'normalized_action_digest', 'provider_outcome', 'effect_relation',
  ], path);
  if (typeof object.authenticated !== 'boolean') fail('invalid_boolean', `${path}.authenticated`);
  const provider = enumValue(object.provider_outcome, ['COMMITTED', 'PROVEN_NOT_COMMITTED', 'INDETERMINATE'], `${path}.provider_outcome`);
  const effect = enumValue(object.effect_relation, ['NOT_OBSERVED', 'OBSERVED_AS_REQUESTED', 'DIVERGED', 'INDETERMINATE'], `${path}.effect_relation`);
  if (!truthPairValid(provider, effect)) fail('invalid_combination', path);
  return {
...
export function parseAebConsequenceCase(value: unknown): AebConsequenceCase {
  ...
  if (object['@version'] !== AEB_CONSEQUENCE_CASE_VERSION) fail('invalid_enum', '$.@version');
  const parsed: AebConsequenceCase = {
  ...
  const prior = parsed.reservation.prior_operations.find(
    (entry) => entry.operation_id === parsed.operation.operation_id,
  );
  if (parsed.mode === 'ADMISSION') {
    if (parsed.observation !== null || parsed.reconciliation !== null) fail('invalid_combination', '$.mode');
  } else {
    if (parsed.reservation.atomicity !== 'local_atomic' || !prior) fail('invalid_combination', '$.reservation');
...
function parseResult(value: unknown, path: string): AebConsequenceResult {
  ...
  const result: AebConsequenceResult = {
  ...
  if (!truthPairValid(result.provider_outcome, result.effect_relation)) fail('invalid_combination', path);
  return deepFreeze(result);
}

export function validateAebConsequenceResult(value: unknown): AebConsequenceValidation<AebConsequenceResult> {
  try {
    canonicalizeAebConsequenceConformance(value);
...
function sortedReasons(values: Iterable<AebConsequenceReason>): AebConsequenceReason[] {
  return [...new Set(values)].sort((left, right) => left.localeCompare(right));
}

function assessEvidence(input: AebConsequenceCase): EvidenceAssessment {
  const reasons = new Set<AebConsequenceReason>();
  const hasFailed = input.evidence.length === 0
    || input.evidence.some((entry) => entry.native_verification === 'FAILED');
  const hasIndeterminateVerification = input.evidence.some(
    (entry) => entry.native_verification === 'INDETERMINATE',
  );
  const verification: AebConsequenceNativeVerification = hasFailed ? 'FAILED'
    : hasIndeterminateVerification ? 'INDETERMINATE' : 'VERIFIED';
  if (hasFailed) reasons.add('native_verification_failed');
  if (hasIndeterminateVerification) reasons.add('native_verification_indeterminate');

  let actionMatch: ActionMatch = 'INDETERMINATE';
  if (verification === 'VERIFIED') {
    const caidMismatch = input.evidence.some((entry) => entry.mapped_caid !== input.operation.caid);
    const normalizedMismatch = input.evidence.some(
      (entry) => entry.mapped_action_digest !== input.operation.normalized_action_digest,
    );
    actionMatch = caidMismatch || normalizedMismatch ? 'MISMATCH' : 'MATCH';
    if (caidMismatch) reasons.add('exact_action_mismatch');
    if (normalizedMismatch) reasons.add('normalized_action_mismatch');
  }
  ...
  if (revoked) reasons.add('evidence_revoked');
  if (unpinned) reasons.add('status_authority_not_pinned');
  if (stale) reasons.add('evidence_stale');
  if (unavailable) reasons.add('status_unavailable');
  ...
  const acceptance: AebConsequenceAcceptance = verification === 'FAILED' || revoked || unpinned
    ? 'REJECTED'
  ...
  const satisfaction: Satisfaction = definitelyUnsatisfied ? 'UNSATISFIED'
  ...
  } else if (satisfaction === 'SATISFIED') authorization = 'AUTHORIZED';
  else if (satisfaction === 'INDETERMINATE') authorization = 'INDETERMINATE';
  else authorization = 'NOT_AUTHORIZED';
...
function result(
  ...
  override: Omit<AebConsequenceResult,
    'verification' | 'acceptance' | 'action_match' | 'satisfaction' | 'authorization' | 'reasons'>,
  additionalReasons: readonly AebConsequenceReason[] = [],
): AebConsequenceResult {
  return deepFreeze({
...
function priorFor(input: AebConsequenceCase): AebConsequencePriorOperation | undefined {
  return input.reservation.prior_operations.find(
    (entry) => entry.operation_id === input.operation.operation_id,
  );
}

function exactPriorBinding(input: AebConsequenceCase, prior: AebConsequencePriorOperation): boolean {
  return prior.caid === input.operation.caid
    && prior.normalized_action_digest === input.operation.normalized_action_digest;
}

function unresolved(provider: ProviderOutcome, effect: EffectRelation): boolean {
  return !terminalTruthPair(provider, effect);
}

export function evaluateAebConsequenceCase(value: unknown): AebConsequenceResult {
  ...
  if (!prior) fail('invalid_combination', '$.reservation.prior_operations');
  ...
  if (input.mode === 'INVOCATION_OBSERVATION') {
    ...
    let reason: AebConsequenceReason;
    if (observation.source === 'TIMEOUT_AFTER_DISPATCH') reason = 'timeout_after_dispatch';
  ...
  const terminal = terminalTruthPair(
    reconciliation.provider_outcome,
    reconciliation.effect_relation,
  );
  if (!terminal) {
    return result(assessment, {
      reservation: 'CONSUMED', custody: 'INVOKING',
...
function claimScopeCopy(): AebConsequenceClaimScope {
  return {
    profile: 'local_atomic',
...
function parseClaimScope(value: unknown, path: string): AebConsequenceClaimScope {
  const object = exactObject(value, ['profile', 'guarantees', 'exclusions'], path);
  const scope: AebConsequenceClaimScope = {
...
function parseSuite(value: unknown): AebConsequenceSuite {
  canonicalizeAebConsequenceConformance(value);
  const object = exactObject(value, ['@version', 'claim_scope', 'vectors'], '$');
  if (object['@version'] !== AEB_CONSEQUENCE_CONFORMANCE_VERSION) fail('invalid_enum', '$.@version');
  const vectors = arrayValue(object.vectors, AEB_CONSEQUENCE_LIMITS.max_vectors, '$.vectors')
    .map((entry, index): AebConsequenceVector => {
      ...
      if (input.id !== id) fail('invalid_combination', `${path}.input.id`);
      return { id, input, expected: parseResult(vectorObject.expected, `${path}.expected`) };
    });
  if (vectors.length === 0) fail('invalid_array', '$.vectors');
...
export function validateAebConsequenceConformanceSuite(
  value: unknown,
): AebConsequenceValidation<AebConsequenceSuite> {
  try {
...
function parseImplementation(value: unknown, path: string): AebConsequenceImplementation {
...
export function evaluateAebConsequenceSuite(
  ...
  const body = {
    '@version': AEB_CONSEQUENCE_CONFORMANCE_REPORT_VERSION as typeof AEB_CONSEQUENCE_CONFORMANCE_REPORT_VERSION,
    ...
    assurance: {
      ...
      statement: 'SELF_ATTESTED_NOT_CERTIFICATION' as const,
    },
  };
  return deepFreeze({
    ...body,
...
function parseNonNegativeInteger(value: unknown, path: string): number {
  if (!Number.isSafeInteger(value) || Number(value) < 0) fail('invalid_integer', path);
  return Number(value);
}

function parseReport(value: unknown): AebConsequenceConformanceReport {
  canonicalizeAebConsequenceConformance(value);
  const object = exactObject(value, [
    '@version', 'suite_digest', 'claim_scope', 'implementation', 'rows',
    'summary', 'assurance', 'report_digest',
  ], '$');
  if (object['@version'] !== AEB_CONSEQUENCE_CONFORMANCE_REPORT_VERSION) fail('invalid_enum', '$.@version');
  const rows = arrayValue(object.rows, AEB_CONSEQUENCE_LIMITS.max_vectors, '$.rows')
    .map((entry, index): AebConsequenceReportRow => {
      const path = `$.rows[${index}]`;
      const row = exactObject(entry, ['id', 'case_digest', 'expected', 'actual', 'pass'], path);
      if (typeof row.pass !== 'boolean') fail('invalid_boolean', `${path}.pass`);
      return {
  ...
  const assuranceObject = exactObject(
    object.assurance,
    ['self_attested', 'certification', 'statement'],
    '$.assurance',
  );
...
export function validateAebConsequenceSubmission(
  suiteValue: unknown,
  submissionValue: unknown,
): AebConsequenceSubmissionValidation {
  try {
    const suite = parseSuite(suiteValue);
    ...
    for (let index = 0; index < suite.vectors.length; index += 1) {
      ...
      if (row.id !== vector.id) return { valid: false, conformant: false, errors: ['row_id_mismatch'] };
