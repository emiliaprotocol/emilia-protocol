...
function referenceTimeMs(value: unknown): number {
...
export interface CurrencyResult {
  authentic_as_of_commit: boolean;
  currency_at_T: { status: CurrencyStatus; evaluated_at: string | null; reason: string };
}
 ...
 * silently dropped.
 */
function headRevokesReceipt(freshHead: Record<string, unknown>, receipt: Record<string, unknown> | null | undefined): boolean {
 ...
 * }}
 */
export function evaluateCurrency(args: CurrencyArgs = {}): CurrencyResult {
  const {
    receipt,
  ...
  const result = (status: CurrencyStatus, reason: string): CurrencyResult => ({
