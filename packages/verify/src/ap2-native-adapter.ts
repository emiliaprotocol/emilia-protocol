...
interface Ap2AebNativeResult extends AebNativeResult {
  /** Exact normalized action produced by the one native verification call. */
  normalized_action: unknown;
...
interface Ap2NativeAdapterConfig {
  source_revision: string;
  evidence_role: string;
  subject: AebEvidenceSubject;
  max_status_age_seconds: number;
  verifier: Ap2NativeVerifierDescriptor;
}
...
function validSubject(value: unknown): value is AebEvidenceSubject {
  return isObject(value) && exactKeys(value, SUBJECT_KEYS)
    && identifier(value.id)
    && ['human', 'workload', 'organization', 'system'].includes(String(value.kind));
}

function validDescriptor(value: unknown): value is Ap2NativeVerifierDescriptor {
...
function parseConfig(value: unknown): Ap2NativeAdapterConfig | null {
  if (!isObject(value) || !exactKeys(value, CONFIG_KEYS)
      ...
      || !validDescriptor(value.verifier)) return null;
  return {
...
function statusDigest(value: unknown): AebDigest {
  if (!isObject(value)) return safeDigest({ malformed_status: true });
  return safeDigest({
...
function statusDisposition(value: unknown, now: string, maxAge: number): { acceptance: Acceptance; reasons: string[] } {
  ...
  if (value.revoked === true) reasons.push('evidence_revoked');
  if (value.consumed === true) reasons.push('evidence_consumed');
  if (value.unavailable === true) reasons.push('status_unavailable');
  if (value.revocation_checked !== true) reasons.push('revocation_not_checked');
  ...
  if (![nowMs, checkedMs, expiresMs].every(Number.isFinite)) {
    ...
    if ((nowMs - checkedMs) / 1000 > maxAge) reasons.push('status_too_old');
  }
...
function fallback(input: Omit<AebAdapterInput, 'profile'>, reason: string): AebNativeResult {
  return {
...
function verifierMatches(config: Ap2NativeAdapterConfig, verifier: Ap2NativeVerifier): boolean {
...
function inspect(
  verifier: Ap2NativeVerifier,
  input: Omit<AebAdapterInput, 'profile'>,
): { config: Ap2NativeAdapterConfig; result: Ap2NativeVerificationResult } | null {
  const config = parseConfig(input?.adapter_config);
  if (!config || !verifierMatches(config, verifier)) return null;
  let result: unknown;
  try {
  ...
  if (!isObject(result) || !exactKeys(result, RESULT_KEYS)
      ...
      || !Array.isArray(result.reasons) || !result.reasons.every(nonEmpty)) return null;
  return { config, result: result as unknown as Ap2NativeVerificationResult };
}

export function createAp2NativeAebAdapter(verifier: Ap2NativeVerifier): AebAdapter {
  if (!validDescriptor({
    id: verifier?.id,
  ...
  return Object.freeze({
    ...
    verifyNative(input: Omit<AebAdapterInput, 'profile'>): AebNativeResult {
      ...
      if (!checked) return fallback(input, 'ap2:pinned_verifier_mismatch');
      ...
      const native: Ap2AebNativeResult = {
        native_verification: 'VERIFIED',
    ...
    mapAction(input: AebAdapterInput & { native: AebNativeResult }) {
      const config = parseConfig(input.adapter_config);
      const native = input.native as Ap2AebNativeResult;
      if (!config || !verifierMatches(config, verifier)
        ...
        return { mapping: 'INDETERMINATE' as const, caid: null, action_digest: null, reasons: ['ap2:pinned_mapping_profile_invalid'] };
      }
      ...
      if (!isObject(computed) || typeof computed.caid !== 'string'
        ...
        return { mapping: 'INDETERMINATE' as const, caid: null, action_digest: null, reasons: ['ap2:caid_mapping_failed'] };
      }
      return {
