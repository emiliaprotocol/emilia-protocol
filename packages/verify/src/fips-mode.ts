 ...
 */
export type ImplementationBoundary =
  | 'openssl_provider'
  | 'javascript_outside_any_validated_module';
 ...
 */
export function readFipsStatus(): FipsStatus {
  try {
    const getFips = (crypto as unknown as { getFips?: () => number | boolean }).getFips;
    if (typeof getFips !== 'function') return 'unavailable';
    const value: number | boolean = getFips.call(crypto);
 ...
 * works at all.
 */
export function probeOpensslOperational(): boolean {
  try {
 ...
 * which algorithms.
 */
export function probeEd25519(): boolean {
  try {
 ...
 */
const PROBE_CACHE = new Map<FipsStatus, { openssl: boolean; ed25519: boolean }>();

function probesFor(status: FipsStatus): { openssl: boolean; ed25519: boolean } {
...
export function resetFipsProbeCache(): void {
  PROBE_CACHE.clear();
}
 ...
 */
export function getFipsPosture(options: FipsPostureOptions = {}): FipsPosture {
  const opts = options && typeof options === 'object' ? options : {};
  const fips_status = readFipsStatus();
  let openssl_version: string | null = null;
  let node_version: string | null = null;
  try {
    const versions = (globalThis as unknown as { process?: { versions?: Record<string, string> } })
      .process?.versions;
    if (versions && typeof versions.openssl === 'string') openssl_version = versions.openssl;
    if (versions && typeof versions.node === 'string') node_version = versions.node;
  } catch {
    // leave both null
  }
...
export function formatFipsPosture(posture: FipsPosture): string {
  if (!isFipsPosture(posture)) return `${FIPS_MODE_VERSION} posture=malformed`;
  return [
    FIPS_MODE_VERSION,
...
function isFipsPosture(value: unknown): value is FipsPosture {
  if (!value || typeof value !== 'object' || Array.isArray(value)) return false;
  const p = value as Partial<FipsPosture>;
  if (p.fips_status !== 'active' && p.fips_status !== 'inactive' && p.fips_status !== 'unavailable') {
    return false;
  }
  if (typeof p.fips_mode_active !== 'boolean') return false;
  if (p.openssl_operational !== null && typeof p.openssl_operational !== 'boolean') return false;
  if (p.ed25519_operational !== null && typeof p.ed25519_operational !== 'boolean') return false;
  if (p.ed25519_in_validated_boundary !== null
    && typeof p.ed25519_in_validated_boundary !== 'boolean') return false;
  return true;
}

function resolvePosture(supplied: unknown): FipsPosture | null {
  if (supplied === undefined) return getFipsPosture();
 ...
 */
export function assertClassicalFips(
  options: { posture?: FipsPosture } = {},
): FipsAssertResult {
  const opts = options && typeof options === 'object' ? options : {};
 ...
 */
export function mldsaPolicy(options: MldsaPolicyOptions = {}): MldsaPolicyResult {
  const opts = options && typeof options === 'object' ? options : {};
  const posture = resolvePosture(opts.posture);
...
function isPolicyAlgorithm(alg: unknown): alg is FipsPolicyAlgorithm {
  return typeof alg === 'string'
    && (FIPS_POLICY_ALGORITHMS as readonly string[]).includes(alg);
}
 ...
 */
export function checkOperationPolicy(
  alg: unknown,
  posture?: FipsPosture,
  options: { allow_unvalidated_mldsa?: boolean } = {},
  ...
  const refuse = (reason: string): OperationPolicyResult => ({
    alg,
    permitted: false,
    reason,
