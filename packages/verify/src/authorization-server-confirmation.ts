...
interface ParsedRoot extends AuthorizationServerConfirmationTrustRoot {
  key: KeyObject;
}
...
function safeDigest(value: unknown): AebDigest {
  try {
    ...
    return `sha256:${'0'.repeat(64)}` as AebDigest;
  }
}
...
function canonicalPublicKey(value: unknown): KeyObject | null {
  if (typeof value !== 'string' || !B64URL_RE.test(value)) return null;
  try {
...
function decodeB64url(value: string, maxBytes: number): Buffer | null {
  if (!B64URL_RE.test(value) || value.length % 4 === 1) return null;
  try {
...
function parseAction(value: unknown, actionType: string): Obj | null {
  if (!isRecord(value) || !exactKeys(value, new Set(['action_type', 'parameters']))
      || value.action_type !== actionType || !isRecord(value.parameters)) return null;
  try {
    canonicalizeAeb(value);
    if (Object.keys(value.parameters).length === 0) return null;
...
function parseConfig(value: unknown): AuthorizationServerConfirmationAdapterConfig | null {
...
function parseRoot(value: unknown, config: AuthorizationServerConfirmationAdapterConfig): ParsedRoot | null {
  if (!isRecord(value) || !exactKeys(value, ROOT_KEYS)
      ...
      || value.issuer !== config.issuer || !nonEmptyIdentifier(value.key_id)) return null;
  const key = canonicalPublicKey(value.public_key);
  if (!key) return null;
  const root = structuredClone(value) as unknown as AuthorizationServerConfirmationTrustRoot;
  return { ...root, key };
}

function parseConstructorPins(value: AuthorizationServerConfirmationConstructorPins): ParsedPins {
  ...
  if (!root) throw new TypeError('invalid Authorization Server confirmation trust root');
  const publicRoot: AuthorizationServerConfirmationTrustRoot = {
...
function parseArtifact(value: unknown): AuthorizationServerConfirmationArtifact | null {
  if (!isRecord(value) || !exactKeys(value, ARTIFACT_KEYS)
      ...
      || Buffer.byteLength(value.grant, 'utf8') > MAX_TOKEN_BYTES) return null;
  try {
    canonicalizeAeb(value.human_evidence);
  } catch {
    return null;
  }
  return structuredClone(value) as unknown as AuthorizationServerConfirmationArtifact;
}

function parseClaims(value: Obj, config: AuthorizationServerConfirmationAdapterConfig): AuthorizationServerConfirmationClaims | null {
  ...
  return structuredClone(value) as unknown as AuthorizationServerConfirmationClaims;
}
...
function verifyGrant(
  ...
  pins: ParsedPins,
  now: string,
): { ok: true; value: VerifiedGrant } | { ok: false; reason: string; acceptance: Acceptance } {
  const artifact = parseArtifact(artifactValue);
  if (!artifact) return { ok: false, reason: 'as-confirmation:artifact_malformed', acceptance: 'REJECTED' };
  ...
  if (!claims) return { ok: false, reason: 'as-confirmation:claims_invalid', acceptance: 'REJECTED' };
  ...
  if (!Number.isFinite(nowMs)) return { ok: false, reason: 'as-confirmation:now_invalid', acceptance: 'INDETERMINATE' };
  const nowSeconds = Math.floor(nowMs / 1000);
  const { iat, nbf, exp } = claims;
  if (exp <= iat || exp <= nbf) return { ok: false, reason: 'as-confirmation:time_window_invalid', acceptance: 'REJECTED' };
...
export function createAuthorizationServerConfirmationActionDefinition(actionType: string): Obj {
...
function mapAction(
  ...
  if (!definitions) return { mapping: 'INDETERMINATE', caid: null, action_digest: null, reasons: ['mapping_profile_invalid'] };
  const grant = verifyGrant(input.artifact, pins, input.now);
  if (!grant.ok) return { mapping: 'INDETERMINATE', caid: null, action_digest: null, reasons: [grant.reason] };
  const expected = parseAction(input.expected_action, pins.config.action_type);
  if (!expected) return { mapping: 'INDETERMINATE', caid: null, action_digest: null, reasons: ['missing_or_ambiguous_exact_action'] };
...
function signableClaims(value: unknown): value is AuthorizationServerConfirmationClaims {
  if (!isRecord(value) || !exactKeys(value, CLAIM_KEYS)
      ...
      || !ACTION_TYPE_RE.test(value.action.action_type)) return false;
  const action = parseAction(value.action, value.action.action_type);
  return action !== null && safeDigest(action) === value.action_digest;
}

export function signAuthorizationServerConfirmation(
  claims: AuthorizationServerConfirmationClaims,
  signer: AuthorizationServerConfirmationSigner,
): string {
...
export function createAuthorizationServerConfirmationAdapter(
  constructorPins: AuthorizationServerConfirmationConstructorPins,
): AebAdapter {
