  ...
  if (Object.hasOwn(workspace, 'replay_probe')) {
    ...
    if (!isObject(probe) || !exactKeys(probe, [
      ...
      're_presented_artifact', 're_presented_artifact_digest',
        ...
        || !DIGEST_RE.test(probe.distinct_authority_artifact_digest ?? '')) throw new TypeError('invalid replay probe');
  }
...
function workspacePinErrors(workspace: Obj, artifact: unknown, adapterBytes: Buffer): string[] {
  ...
  if (isObject(workspace.replay_probe)) {
    const probe = workspace.replay_probe;
    if (digestAeb(probe.re_presented_artifact) !== probe.re_presented_artifact_digest) reasons.push('re_presented_artifact_pin_drift');
    if (digestAeb(probe.distinct_authority_artifact) !== probe.distinct_authority_artifact_digest) reasons.push('distinct_authority_artifact_pin_drift');
    ...
    if (probe.re_presented_artifact_digest === workspace.artifact_digest) reasons.push('re_presented_artifact_not_distinct');
    if (probe.distinct_authority_artifact_digest === workspace.artifact_digest
        || probe.distinct_authority_artifact_digest === probe.re_presented_artifact_digest) reasons.push('distinct_authority_artifact_not_distinct');
  }
...
export function runCrossingLab(workspaceDirectory: string): CrossingLabReport {
  ...
  if (isObject(workspace.replay_probe)) {
    ...
    const rePresentedRow = adapterRow(
      'replay-identity-survives-re-presentation',
      'hostile',
      'RE_PRESENTED_REPLAY_IDENTITY_STABLE',
      rePresented,
    );
    ...
    const distinctRow = adapterRow(
      'distinct-authority-is-a-distinct-replay-unit',
      'hostile',
      'DISTINCT_AUTHORITY_REPLAY_IDENTITY_SEPARATE',
      distinct,
    );
...
export function initCrossingLab(targetDirectory: string): { directory: string; files: string[] } {
  ...
  const signBody = (body: Obj): string => crypto.sign(
    null,
    Buffer.from(canonicalizeCrossingLab(body), 'utf8'),
