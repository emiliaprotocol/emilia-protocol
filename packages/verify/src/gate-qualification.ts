...
export const TERMINAL_OUTCOMES = Object.freeze(['PASS', 'FAIL', 'ABORTED', 'EXPIRED'] as const);
export const QUALIFICATION_DECISIONS = Object.freeze(['QUALIFIED', 'NOT_QUALIFIED', 'INDETERMINATE'] as const);
...
const own = (value: any, key: string): boolean => Object.prototype.hasOwnProperty.call(value, key);

function record(value: any): value is Obj {
  if (value === null || typeof value !== 'object' || Array.isArray(value)) return false;
  const prototype = Object.getPrototypeOf(value);
  return prototype === Object.prototype || prototype === null;
}

function exactObject(value: any, required: string[], optional: string[] = []): value is Obj {
...
function nonEmptyString(value: any, maxBytes: number = GATE_QUALIFICATION_LIMITS.max_string_bytes): value is string {
  return typeof value === 'string' && value.length > 0 && validUnicode(value)
    && Buffer.byteLength(value, 'utf8') <= maxBytes && !/[\u0000-\u001f\u007f]/.test(value);
}

function digest(value: any): value is QualificationDigest {
  return typeof value === 'string' && SHA256.test(value);
}

function safeNonNegativeInteger(value: any): value is number {
  return Number.isSafeInteger(value) && value >= 0;
}

function instantMs(value: any): number {
...
function uri(value: any): boolean {
  if (!nonEmptyString(value)) return false;
  try { return Boolean(new URL(value).protocol); } catch { return false; }
}

function sortedUniqueStrings(value: any, min: number, max: number, predicate: (entry: any) => boolean): boolean {
  if (!Array.isArray(value) || value.length < min || value.length > max || !value.every(predicate)) return false;
  for (let index = 1; index < value.length; index += 1) {
    if (Buffer.compare(Buffer.from(value[index - 1]), Buffer.from(value[index])) >= 0) return false;
  }
  return true;
}
...
function bytesDigest(value: Uint8Array): QualificationDigest {
  return `sha256:${crypto.createHash('sha256').update(value).digest('hex')}`;
}

function canonicalBase64(value: any, maxBytes: number = GATE_QUALIFICATION_LIMITS.max_payload_bytes): Buffer | null {
...
function canonicalBase64url(value: any): Buffer | null {
  if (typeof value !== 'string' || value.length === 0 || value.length % 4 === 1 || !/^[A-Za-z0-9_-]+$/.test(value)) return null;
  const decoded = Buffer.from(value, 'base64url');
  return decoded.toString('base64url') === value ? decoded : null;
}

export function dsseSigningBytes(payloadType: string, payload: Uint8Array): Buffer {
...
function decodeCanonicalEnvelope(envelope: any, expectedPayloadType: string): SchemaResult<{ envelope: DsseEnvelope; payload: Obj; bytes: Buffer; payload_digest: QualificationDigest }> {
  ...
  if (!Array.isArray(envelope.signatures) || envelope.signatures.length === 0
      || envelope.signatures.length > GATE_QUALIFICATION_LIMITS.max_signatures) return { valid: false, reason: 'invalid_envelope_signatures' };
  const keyids = new Set<string>();
  for (const signature of envelope.signatures) {
    if (!exactObject(signature, ['keyid', 'sig']) || !nonEmptyString(signature.keyid, 512)
        || !canonicalBase64(signature.sig, 512) || keyids.has(signature.keyid)) return { valid: false, reason: 'invalid_envelope_signatures' };
    keyids.add(signature.keyid);
  }
  const bytes = canonicalBase64(envelope.payload);
  if (!bytes) return { valid: false, reason: 'invalid_envelope_payload' };
  let text: string;
  let payload: any;
  try {
    text = FATAL_UTF8.decode(bytes);
    payload = JSON.parse(text);
    if (canonicalizeQualification(payload) !== text) return { valid: false, reason: 'non_canonical_payload' };
  } catch {
    return { valid: false, reason: 'invalid_envelope_payload' };
  }
  if (!record(payload)) return { valid: false, reason: 'invalid_envelope_payload' };
  return { valid: true, value: { envelope: envelope as DsseEnvelope, payload, bytes, payload_digest: bytesDigest(bytes) } };
}

function loadEd25519Key(value: string): crypto.KeyObject | null {
  try {
    let key: crypto.KeyObject;
    if (value.includes('-----BEGIN')) key = crypto.createPublicKey(value);
    else {
...
function validTrustPolicy(policy: any): policy is ArtifactTrustPolicy {
  if (!exactObject(policy, ['keys', 'accepted_keyids', 'threshold']) || !record(policy.keys)
      ...
      || policy.threshold < 1 || policy.threshold > GATE_QUALIFICATION_LIMITS.max_signatures) return false;
  ...
  if (keyids.length === 0 || keyids.length > 256
      || !keyids.every((keyid) => nonEmptyString(keyid, 512)
        ...
        && Buffer.byteLength(policy.keys[keyid], 'utf8') <= 8192)) return false;
  if (policy.accepted_keyids.length < policy.threshold
      || new Set(policy.accepted_keyids).size !== policy.accepted_keyids.length
      || !policy.accepted_keyids.every((keyid: any) => typeof keyid === 'string' && own(policy.keys, keyid))) return false;
  ...
  const acceptedFingerprints = new Set(policy.accepted_keyids.map(
    (keyid) => publicKeyFingerprint(loadEd25519Key(policy.keys[keyid])!),
  ));
  return acceptedFingerprints.size >= policy.threshold;
}

interface VerifiedPayload<T = any> {
  payload: T;
  payload_digest: QualificationDigest;
  verified: boolean;
  accepted: boolean;
  reason: string;
}
...
function verifyEnvelope<T>(
  ...
  schema: (value: unknown) => SchemaResult<T>,
): VerifiedPayload<T> {
  const decoded = decodeCanonicalEnvelope(envelope, expectedPayloadType);
  if (!decoded.valid || !decoded.value) return { payload: null as T, payload_digest: null as any, verified: false, accepted: false, reason: decoded.reason ?? 'invalid_envelope' };
  const checked = schema(decoded.value.payload);
  if (!checked.valid || !checked.value) return { payload: null as T, payload_digest: decoded.value.payload_digest, verified: false, accepted: false, reason: checked.reason ?? 'invalid_payload_schema' };
  if (!validTrustPolicy(trust)) return { payload: checked.value, payload_digest: decoded.value.payload_digest, verified: false, accepted: false, reason: 'invalid_trust_policy' };
  const pae = dsseSigningBytes(decoded.value.envelope.payloadType, decoded.value.bytes);
  const acceptedKeyFingerprints = new Set<string>();
  for (const signature of decoded.value.envelope.signatures) {
    if (!own(trust.keys, signature.keyid)) return { payload: checked.value, payload_digest: decoded.value.payload_digest, verified: false, accepted: false, reason: 'untrusted_verification_key' };
    const key = loadEd25519Key(trust.keys[signature.keyid]);
    const sig = canonicalBase64(signature.sig, 512);
    if (!key || !sig || !crypto.verify(null, pae, key, sig)) return { payload: checked.value, payload_digest: decoded.value.payload_digest, verified: false, accepted: false, reason: 'invalid_artifact_signature' };
...
function resourceDescriptor(value: any): boolean {
...
function resourceMatches(value: any, name: string, expected: string): boolean {
  return resourceDescriptor(value) && value.name === name && expected === `sha256:${value.digest.sha256}`;
}

function statement(value: any, predicateType: string): boolean {
...
function validModel(value: any): boolean {
  if (!exactObject(value, ['provider', 'identity', 'version', 'artifact_digest', 'pinning_strength'])
      ...
      || !MODEL_PINNING_STRENGTHS.includes(value.pinning_strength)) return false;
  if (value.artifact_digest !== null && !digest(value.artifact_digest)) return false;
  return value.pinning_strength !== 'IMMUTABLE_DIGEST' || digest(value.artifact_digest);
}

function validStaticCandidate(value: any): boolean {
...
export function validateCandidateManifest(value: any): SchemaResult<Obj> {
  try {
    if (!exactObject(value, ['profile', 'candidate_id', 'static'])
        ...
        || !validStaticCandidate(value.static)) return { valid: false, reason: 'invalid_candidate_manifest' };
    canonicalizeQualification(value);
    return { valid: true, value };
  } catch { return { valid: false, reason: 'invalid_candidate_manifest' }; }
}

function validHiddenChallenges(value: any): boolean {
  if (!record(value) || !own(value, 'scheme')) return false;
...
export function validateEvaluationCampaign(value: any): SchemaResult<Obj> {
  try {
    if (!statement(value, EVALUATION_CAMPAIGN_PREDICATE) || value.subject.length !== 1) return { valid: false, reason: 'invalid_evaluation_campaign' };
    ...
    return { valid: true, value };
  } catch { return { valid: false, reason: 'invalid_evaluation_campaign' }; }
}

function validStringList(value: any): boolean {
  return sortedUniqueStrings(value, 0, GATE_QUALIFICATION_LIMITS.max_test_names, (entry) => nonEmptyString(entry));
}

export function validateTestResultReference(value: any): SchemaResult<Obj> {
  try {
    if (!statement(value, TEST_RESULT_PREDICATE) || value.subject.length !== 1) return { valid: false, reason: 'invalid_test_result' };
    const p = value.predicate;
    if (!exactObject(p, ['result', 'configuration'], ['url', 'passedTests', 'warnedTests', 'failedTests'])
        ...
        || (own(p, 'failedTests') && !validStringList(p.failedTests))) return { valid: false, reason: 'invalid_test_result' };
    return { valid: true, value };
  } catch { return { valid: false, reason: 'invalid_test_result' }; }
}

function validMerkleProof(value: any): boolean {
...
function validTerminalReference(value: any): boolean {
  if (!exactObject(value, [
    ...
    || instantMs(value.started_at) > instantMs(value.finished_at)) return false;
  if (value.outcome === 'PASS' || value.outcome === 'FAIL') return digest(value.test_result_payload_digest);
  return value.test_result_payload_digest === null;
}

function validOutcomeCounts(value: any): boolean {
  return exactObject(value, ['PASS', 'FAIL', 'ABORTED', 'EXPIRED'])
    && TERMINAL_OUTCOMES.every((outcome) => safeNonNegativeInteger(value[outcome]));
}

function terminalReferenceKey(value: Obj): string {
  return `${String(value.batch).padStart(3, '0')}:${String(value.challenge_index).padStart(4, '0')}:${String(value.attempt).padStart(3, '0')}`;
}

function validMeasurements(value: any): boolean {
  if (!Array.isArray(value) || value.length > GATE_QUALIFICATION_LIMITS.max_measurements) return false;
  let prior = '';
  for (const measurement of value) {
    if (!exactObject(measurement, ['name', 'value', 'unit']) || !nonEmptyString(measurement.name)
        || !nonEmptyString(measurement.value) || (measurement.unit !== null && !nonEmptyString(measurement.unit))) return false;
    if (prior && Buffer.compare(Buffer.from(prior), Buffer.from(measurement.name)) >= 0) return false;
    prior = measurement.name;
  }
  return true;
}

export function validateAgentEvaluationEvidence(value: any): SchemaResult<Obj> {
  try {
    if (!statement(value, AGENT_EVALUATION_EVIDENCE_PREDICATE) || value.subject.length !== 1) return { valid: false, reason: 'invalid_agent_evaluation_evidence' };
    const p = value.predicate;
    if (!exactObject(p, [
    ...
    return { valid: true, value };
  } catch { return { valid: false, reason: 'invalid_agent_evaluation_evidence' }; }
}

export function validateQualificationStatement(value: any): SchemaResult<Obj> {
  try {
    if (!statement(value, QUALIFICATION_STATEMENT_PREDICATE) || value.subject.length !== 3) return { valid: false, reason: 'invalid_qualification_statement' };
    const p = value.predicate;
    ...
    return { valid: true, value };
  } catch { return { valid: false, reason: 'invalid_qualification_statement' }; }
}

export function validateQualificationStatus(value: any): SchemaResult<Obj> {
  try {
    if (!exactObject(value, [
      ...
      || instantMs(value.issued_at) >= instantMs(value.valid_until)) return { valid: false, reason: 'invalid_qualification_status' };
    return { valid: true, value };
  } catch { return { valid: false, reason: 'invalid_qualification_status' }; }
}

export function validateRuntimeCandidateMeasurement(value: any): SchemaResult<Obj> {
  try {
    if (!exactObject(value, [
      ...
      || !digest(value.user_input_digest) || !digest(value.protected_request_digest)) return { valid: false, reason: 'invalid_runtime_measurement' };
    return { valid: true, value };
  } catch { return { valid: false, reason: 'invalid_runtime_measurement' }; }
}

function validateStatusObservation(value: any): boolean {
...
export function terminalOutcomesRoot(outcomes: unknown[]): QualificationDigest {
...
export function qualificationMerkleParent(left: QualificationDigest, right: QualificationDigest): QualificationDigest {
  if (!digest(left) || !digest(right)) throw new TypeError('invalid_merkle_digest');
  return qualificationPayloadDigest({ left, right });
}

function verifiesChallengeCommitment(challenges: Obj, reference: Obj): boolean {
  ...
  if (!safeNonNegativeInteger(reference.challenge_index)
      || reference.challenge_index >= challenges.challenge_count) return false;
  const expectedDepth = Math.ceil(Math.log2(challenges.challenge_count));
  if (reference.challenge_proof.length !== expectedDepth) return false;
...
function baseDecision(reason = 'verification_not_run'): QualificationDecision {
  return {
...
function refuse(
  result: QualificationDecision,
  reason: string,
  decision: QualificationDecision['decision'] = 'INDETERMINATE',
...
function validContext(context: any): context is QualificationEvaluationContext {
...
function validBundleShape(bundle: any): bundle is QualificationBundle {
...
function verifyConfigurations(testResult: Obj, campaign: Obj): boolean {
  const configuration = testResult.predicate.configuration;
  if (configuration.length !== 3) return false;
...
function expectedCounts(references: Obj[]): Obj {
  const counts: Obj = { PASS: 0, FAIL: 0, ABORTED: 0, EXPIRED: 0 };
  for (const reference of references) counts[reference.outcome] += 1;
  return counts;
}

function pinningRank(value: string): number {
  return MODEL_PINNING_STRENGTHS.indexOf(value as typeof MODEL_PINNING_STRENGTHS[number]);
}

function verifyArtifactArray<T>(
  envelopes: unknown[],
  payloadType: string,
  trust: ArtifactTrustPolicy,
  schema: (value: unknown) => SchemaResult<T>,
): { ok: true; values: VerifiedPayload<T>[] } | { ok: false; reason: string; verified: boolean; accepted: boolean } {
  const values: VerifiedPayload<T>[] = [];
  const digests = new Set<string>();
  for (const envelope of envelopes) {
    const verified = verifyEnvelope(envelope, payloadType, trust, schema);
    if (!verified.verified) return { ok: false, reason: verified.reason, verified: false, accepted: false };
    if (!verified.accepted) return { ok: false, reason: verified.reason, verified: true, accepted: false };
    if (digests.has(verified.payload_digest)) return { ok: false, reason: 'duplicate_payload_digest', verified: true, accepted: true };
...
function markArtifactFailure(
  result: QualificationDecision,
  failure: { reason: string; verified: boolean; accepted: boolean },
): QualificationDecision {
  if (failure.verified) result.verification = 'VERIFIED';
  if (failure.accepted) result.acceptance = 'ACCEPTED';
  return refuse(result, failure.reason, failure.verified && !failure.accepted ? 'NOT_QUALIFIED' : 'INDETERMINATE');
}

function evaluateInternal(bundle: unknown, context: unknown): QualificationDecision {
  const result = baseDecision();
  if (!validContext(context)) return refuse(result, 'invalid_evaluation_context');
  if (!validBundleShape(bundle)) return refuse(result, 'invalid_qualification_bundle');
  const now = instantMs(context.now);

  const manifestValidation = validateCandidateManifest(bundle.candidate_manifest);
  if (!manifestValidation.valid || !manifestValidation.value) return refuse(result, manifestValidation.reason ?? 'invalid_candidate_manifest');
  const runtimeVerification = verifyEnvelope(
    bundle.runtime_measurement,
    RUNTIME_MEASUREMENT_PAYLOAD_TYPE,
    context.trust.runtime_measurement,
    validateRuntimeCandidateMeasurement,
  );
  if (!runtimeVerification.verified) return markArtifactFailure(result, runtimeVerification);
  if (!runtimeVerification.accepted) return markArtifactFailure(result, runtimeVerification);
  ...
  if (campaigns.ok === false) return markArtifactFailure(result, campaigns);
  ...
  if (testResults.ok === false) return markArtifactFailure(result, testResults);
  ...
  if (evidence.ok === false) return markArtifactFailure(result, evidence);
  ...
  if (!qualification.verified) return markArtifactFailure(result, qualification);
  if (!qualification.accepted) return markArtifactFailure(result, qualification);
  ...
  if (statuses.ok === false) return markArtifactFailure(result, statuses);
  ...
  const campaignPayloads = campaigns.values.map((entry) => entry.payload as Obj);
  for (let index = 0; index < campaignPayloads.length; index += 1) {
    ...
    if (campaign.predecessor_campaign_payload_digest !== predecessor) return refuse(result, 'campaign_predecessor_mismatch');
  ...
  const evidenceByCampaign = new Map<string, VerifiedPayload<Obj>>();
  for (const entry of evidence.values as VerifiedPayload<Obj>[]) {
    const key = entry.payload.predicate.campaign_payload_digest;
    if (evidenceByCampaign.has(key)) return refuse(result, 'duplicate_campaign_evidence');
    evidenceByCampaign.set(key, entry);
  }
  if (evidenceByCampaign.size !== campaigns.values.length) return refuse(result, 'incomplete_campaign_evidence');

  const testResultByDigest = new Map<string, VerifiedPayload<Obj>>();
  for (const entry of testResults.values as VerifiedPayload<Obj>[]) testResultByDigest.set(entry.payload_digest, entry);
  const referencedTestResults = new Set<string>();

  for (let campaignIndex = 0; campaignIndex < campaigns.values.length; campaignIndex += 1) {
    const campaignEntry = campaigns.values[campaignIndex] as VerifiedPayload<Obj>;
    const campaign = campaignEntry.payload.predicate;
    const evidenceEntry = evidenceByCampaign.get(campaignEntry.payload_digest);
    if (!evidenceEntry) return refuse(result, 'missing_campaign_evidence');
    ...
    if (ep.completed_batches < campaign.planned_batches || ep.completed_batches > campaign.maximum_batches
        ...
        || instantMs(ep.completed_at) > instantMs(campaign.not_after)) return refuse(result, 'invalid_campaign_closure');

    const slots = new Map<string, number[]>();
    const merkleLeafIndexes = new Map<string, number>();
    const merkleProofIndexes = new Map<string, number>();
    for (const reference of ep.terminal_outcomes as Obj[]) {
      if (reference.batch > ep.completed_batches || reference.challenge_index >= (
        campaign.hidden_challenges.scheme === 'SALTED_SHA256_SET'
          ? campaign.hidden_challenges.commitments.length
          : campaign.hidden_challenges.challenge_count
      ) || reference.attempt > campaign.attempt_ceiling
        ...
        || instantMs(reference.finished_at) > instantMs(ep.completed_at)) return refuse(result, 'hidden_challenge_commitment_mismatch');
      ...
      if (reference.test_result_payload_digest !== null) {
        ...
        if (!native || referencedTestResults.has(reference.test_result_payload_digest)) return refuse(result, 'test_result_reference_mismatch');
    ...
    if (slots.size !== ep.completed_batches * challengeCount) return refuse(result, 'omitted_terminal_outcome');
    for (const attempts of slots.values()) {
      attempts.sort((left, right) => left - right);
      if (attempts.some((attempt, index) => attempt !== index + 1)) return refuse(result, 'non_contiguous_challenge_attempts');
    }
  }
  if (referencedTestResults.size !== testResultByDigest.size) return refuse(result, 'unreferenced_test_result');
  ...
  const qp = qualification.payload as Obj;
  ...
  if (!resourceMatches(qp.subject[0], 'candidate-manifest', manifestDigest)
      ...
      || instantMs(qp.predicate.timeCreated) < Math.max(...evidence.values.map((entry: VerifiedPayload<Obj>) => instantMs(entry.payload.predicate.completed_at)))
      || instantMs(qp.predicate.timeCreated) > now) return refuse(result, 'qualification_statement_binding_mismatch', 'NOT_QUALIFIED');
  result.checks.qualification_statement_binding = true;

  if (!validateStatusObservation(bundle.qualification_status_observation)) return refuse(result, 'invalid_status_observation');
  const observation = bundle.qualification_status_observation as Obj;
  ...
  for (let index = 0; index < statuses.values.length; index += 1) {
    const entry = statuses.values[index] as VerifiedPayload<Obj>;
  ...
  const headEntry = statuses.values[statuses.values.length - 1] as VerifiedPayload<Obj>;
  ...
  if (observation.authority_id !== context.expected_status_authority_id
      ...
      || head.sequence < context.minimum_status_sequence) return refuse(result, 'qualification_status_observation_mismatch');
...
export function evaluateQualification(bundle: unknown, context: unknown): QualificationDecision {
  try { return evaluateInternal(bundle, context); } catch { return baseDecision('unexpected_verification_error'); }
}

/** Alias emphasizing complete campaign-graph verification. */
export function verifyQualificationGraph(bundle: unknown, context: unknown): QualificationDecision {
