...
const own = (object: any, key: string): boolean => Object.prototype.hasOwnProperty.call(object, key);
...
function nonEmptyString(value: any, maxBytes = MAX_STRING_BYTES): boolean {
...
function exactObject(value: any, required: string[], optional: string[] = []): boolean {
...
function exactArrayOfStrings(value: any, { min = 0, max = MAX_CAPABILITIES, unique = false }: { min?: number; max?: number; unique?: boolean } = {}): boolean {
  if (!Array.isArray(value) || value.length < min || value.length > max) return false;
  if (!value.every((item) => nonEmptyString(item))) return false;
  return !unique || new Set(value).size === value.length;
}

function nonNegativeNumber(value: any): boolean {
  return typeof value === 'number' && Number.isFinite(value) && value >= 0;
}

function nonNegativeInteger(value: any): boolean {
...
function canonicalBase64url(value: any, expectedBytes?: number): Buffer | null {
...
function loadEd25519Key(value: any): crypto.KeyObject | null {
  try {
...
function validSignatureEnvelope(signatures: any): boolean {
  if (!Array.isArray(signatures) || signatures.length === 0 || signatures.length > MAX_SIGNATURES) return false;
  ...
  for (const signature of signatures) {
    if (!exactObject(signature, ['signer', 'alg', 'sig'])
        ...
        || signers.has(signature.signer)) return false;
    signers.add(signature.signer);
  }
  return true;
}

function signingBytes(object: Obj): Buffer {
  const body: Obj = {};
  for (const [key, value] of Object.entries(object)) {
    if (key !== 'signatures') body[key] = value;
  }
  return Buffer.from(canonicalize(body), 'utf8');
}

function verifySignatures(object: Obj, pins: Obj, role: string, expectedSigner: string | null = null): string | null {
  if (!record(pins)) return `missing_${role}_key_pins`;
  ...
  for (const signature of object.signatures) {
    ...
    if (!own(pins, signature.signer)) return `unpinned_${role}_signer`;
    const key = loadEd25519Key(pins[signature.signer]);
    if (!key) return `invalid_${role}_key`;
    let verified = false;
    try {
    ...
    if (!verified) return `invalid_${role}_signature`;
  }
  return null;
}

function validCapability(value: any, { wildcard = true }: { wildcard?: boolean } = {}): boolean {
  if (!nonEmptyString(value, 512) || /\s/.test(value) || !value.includes(':')) return false;
  const stars = [...value].filter((character) => character === '*').length;
  if (stars === 0) return true;
  return wildcard && stars === 1 && value.endsWith('.*');
}

function validCapabilities(value: any): boolean {
  return exactArrayOfStrings(value, { min: 1, max: MAX_CAPABILITIES, unique: true })
    && value.every((capability: any) => validCapability(capability));
}

function validSession(session: any, { aer = false }: { aer?: boolean } = {}): boolean {
  if (!exactObject(session, ['session_id', 'agent_id'], aer
    ? ['device_attestation_ref']
    : ['channel', 'device_attestation_ref'])) return false;
  if (!nonEmptyString(session.session_id) || !AGENT_ID.test(session.agent_id)) return false;
  if (!aer && !CHANNELS.has(session.channel)) return false;
  if (own(session, 'device_attestation_ref')) {
    if (aer && session.device_attestation_ref === null) return true;
    if (!nonEmptyString(session.device_attestation_ref)) return false;
  }
  return true;
}

function validRootScope(scope: any): boolean {
  if (!exactObject(
    scope,
    ['capabilities', 'max_delegation_depth', 'cross_org_permitted'],
    ['data_classification_ceiling', 'budget_ceiling', 'budget_unit', 'price_class', 'slo_class'],
  )) return false;
  if (!validCapabilities(scope.capabilities)
      ...
      || typeof scope.cross_org_permitted !== 'boolean') return false;
  if (own(scope, 'data_classification_ceiling') && !nonEmptyString(scope.data_classification_ceiling)) return false;
  if (own(scope, 'budget_ceiling') !== own(scope, 'budget_unit')) return false;
  if (own(scope, 'budget_ceiling')
      && (!nonNegativeNumber(scope.budget_ceiling) || !nonEmptyString(scope.budget_unit, 64))) return false;
  if (own(scope, 'price_class') && !nonNegativeInteger(scope.price_class)) return false;
  if (own(scope, 'slo_class') && !nonNegativeInteger(scope.slo_class)) return false;
  return true;
}

function validPolicy(policy: any, { root = false }: { root?: boolean } = {}): boolean {
  if (!exactObject(
    policy,
    root ? ['policy_id', 'policy_version', 'policy_digest'] : ['policy_digest', 'policy_version'],
    root ? ['policy_uri'] : [],
  )) return false;
  if ((root && !nonEmptyString(policy.policy_id))
      || !nonEmptyString(policy.policy_version)
      || !SHA256.test(policy.policy_digest)) return false;
...
function validRoot(root: any): boolean {
  if (!exactObject(root, [
    'schema_version',
    'envelope_id',
    'issued_at',
  ...
  if (root.schema_version !== '1.0'
      ...
      || !validPolicy(root.policy, { root: true })) return false;
  if (!exactObject(root.authorization, ['auth_strength', 'approval_state'], ['approval_artifact_ref'])
      ...
      || (own(root.authorization, 'approval_artifact_ref')
        && !nonEmptyString(root.authorization.approval_artifact_ref))) return false;
  if (!exactObject(root.evidence, ['session_hash', 'model_provenance'])
      ...
      || !validSignatureEnvelope(root.signatures)) return false;
  return true;
}

function validDelegatedScope(scope: any): boolean {
  ...
  if (!exactObject(
    scope,
    ['capabilities', 'max_delegation_depth'],
    ['task_context', 'budget_ceiling', 'budget_unit', 'price_class', 'slo_class'],
  )) return false;
  if (!validCapabilities(scope.capabilities)
      || !nonNegativeInteger(scope.max_delegation_depth)
      || scope.max_delegation_depth > MAX_CHAIN_DEPTH) return false;
  if (own(scope, 'task_context') && !nonEmptyString(scope.task_context)) return false;
  if (own(scope, 'budget_ceiling') !== own(scope, 'budget_unit')) return false;
  if (own(scope, 'budget_ceiling')
      && (!nonNegativeNumber(scope.budget_ceiling) || !nonEmptyString(scope.budget_unit, 64))) return false;
  if (own(scope, 'price_class') && !nonNegativeInteger(scope.price_class)) return false;
...
function validARA(ara: any): boolean {
  if (!exactObject(ara, [
    'schema_version',
    'ara_id',
    'issued_at',
  ...
  if (ara.schema_version !== '1.0'
      ...
      || !Number.isFinite(instantMs(ara.issued_at))) return false;
  if (!exactObject(ara.upstream_ref, ['ref_type', 'ref_id', 'ref_digest'])
      ...
      || !SHA256.test(ara.upstream_ref.ref_digest)) return false;
  if (!exactObject(ara.delegating_agent, ['agent_id', 'session_id'])
      || !AGENT_ID.test(ara.delegating_agent.agent_id)
      || !nonEmptyString(ara.delegating_agent.session_id)) return false;
  if (!exactObject(ara.delegated_agent, ['agent_id'], ['capability_declaration_ref'])
      ...
      || (own(ara.delegated_agent, 'capability_declaration_ref')
        && !nonEmptyString(ara.delegated_agent.capability_declaration_ref))) return false;
  return validDelegatedScope(ara.delegated_scope)
    && validPolicy(ara.policy)
    && validSignatureEnvelope(ara.signatures);
}

function validAction(action: any): boolean {
...
function validAER(aer: any): boolean {
  if (!exactObject(aer, [
    'schema_version',
    'aer_id',
    'produced_at',
    ...
    'border_gateway',
    'signatures',
  ], ['deployment_topology', 'denial_reason', 'plan_hash'])) return false;
  if (aer.schema_version !== '1.0'
      ...
      || !validAction(aer.action)) return false;
  if (aer.enforcement_outcome === 'permit' && own(aer, 'denial_reason')) return false;
  if (aer.enforcement_outcome === 'deny'
      && (!own(aer, 'denial_reason') || !DENIAL_REASONS.has(aer.denial_reason))) return false;
  if (!exactObject(aer.policy, ['policy_id', 'policy_digest'])
      || !nonEmptyString(aer.policy.policy_id)
      || !SHA256.test(aer.policy.policy_digest)) return false;
  if (!exactObject(aer.chain_summary, ['chain_depth', 'root_envelope_id', 'chain_digest'])
      ...
      || !SHA256.test(aer.chain_summary.chain_digest)) return false;
  if (!exactObject(aer.border_gateway, ['gateway_id', 'gateway_version'])
      || !nonEmptyString(aer.border_gateway.gateway_id)
      || !nonEmptyString(aer.border_gateway.gateway_version)) return false;
  if (own(aer, 'plan_hash') && !SHA256.test(aer.plan_hash)) return false;
...
function validateManifest(manifest: any): boolean {
  ...
  for (const [wildcard, entries] of Object.entries(manifest) as Array<[string, any]>) {
    if (!validCapability(wildcard) || !wildcard.endsWith('.*')
        || !exactArrayOfStrings(entries, { min: 1, max: MAX_CAPABILITIES, unique: true })
        || !entries.every((entry: any) => validCapability(entry, { wildcard: false }))) return false;
  }
  return true;
}
...
function validateProfile(profile: any): boolean {
  ...
  return nonEmptyString(profile.expected_policy_id)
    ...
    && profile.allowed_topologies.every((topology: any) => TOPOLOGIES.has(topology))
 ...
 */
function resolveCapabilities(capabilities: string[], manifest: Record<string, string[]>): { ok: true; capabilities: Set<string> } | { ok: false; reason: string } {
  ...
  for (const capability of capabilities) {
    ...
    if (!own(manifest, capability)) return { ok: false, reason: 'ambiguous_capability_wildcard' };
    for (const concrete of manifest[capability]) resolved.add(concrete);
  }
  return { ok: true, capabilities: resolved };
}

function organization(agentId: string): string {
  return agentId.slice(4).split('/')[0];
}

function narrowScope(parent: Obj, child: Obj, manifest: Record<string, string[]>): string | null {
  const parentCapabilities = resolveCapabilities(parent.capabilities, manifest);
  if (!parentCapabilities.ok) return parentCapabilities.reason;
  const childCapabilities = resolveCapabilities(child.capabilities, manifest);
  if (!childCapabilities.ok) return childCapabilities.reason;
  for (const capability of childCapabilities.capabilities) {
    if (!parentCapabilities.capabilities.has(capability)) return 'scope_expansion';
  }
  if (own(parent, 'budget_ceiling') && !own(child, 'budget_ceiling')) return 'budget_constraint_omitted';
  if (own(child, 'budget_ceiling')) {
    if (own(parent, 'budget_ceiling') && child.budget_ceiling > parent.budget_ceiling) return 'budget_expansion';
    if (own(parent, 'budget_unit') && child.budget_unit !== parent.budget_unit) return 'budget_unit_mismatch';
  }
  if (own(parent, 'price_class') && !own(child, 'price_class')) return 'price_class_constraint_omitted';
  if (own(child, 'price_class') && own(parent, 'price_class') && child.price_class > parent.price_class) {
    return 'price_class_expansion';
  }
  if (own(parent, 'slo_class') && !own(child, 'slo_class')) return 'slo_constraint_omitted';
  if (own(child, 'slo_class') && own(parent, 'slo_class') && child.slo_class < parent.slo_class) {
    return 'slo_relaxation';
  }
  if (child.max_delegation_depth >= parent.max_delegation_depth) return 'delegation_depth_not_narrowed';
  return null;
}

function refusal(reason: string, extra: Obj = {}): Obj {
  return { ...RESULT_BASE, ...extra, valid: false, reason };
}

function verifyAgentROAInternal(evidence: Obj, context: Obj): Obj {
  if (!exactObject(evidence, ['chain', 'aer'])) return refusal('malformed_agentroa_bundle');
  if (!Array.isArray(evidence.chain)
      || evidence.chain.length === 0
      || evidence.chain.length > MAX_CHAIN_DEPTH + 1) return refusal('malformed_agentroa_chain');
  const [root, ...aras] = evidence.chain;
  if (!validRoot(root)) return refusal('malformed_roa_or_unknown_field');
  const araIds = new Set();
  for (const ara of aras) {
    if (!validARA(ara)) return refusal('malformed_ara_or_unknown_field');
    if (araIds.has(ara.ara_id)) return refusal('duplicate_ara_id');
    araIds.add(ara.ara_id);
  }
  if (!validAER(evidence.aer)) return refusal('malformed_aer_or_unknown_field');

  if (!record(context)) return refusal('malformed_verification_context');
  const verificationTime = instantMs(context.verificationTime);
  if (!Number.isFinite(verificationTime)) return refusal('invalid_verification_time');
  ...
  if (!validateProfile(profile)) return refusal('malformed_policy_profile');
  if (!validAction(context.action)) return refusal('malformed_expected_action');

  const rootSignature = verifySignatures(root, pins.roa, 'roa');
  if (rootSignature) return refusal(rootSignature);
  for (const ara of aras) {
    const signature = verifySignatures(ara, pins.ara, 'ara', ara.delegating_agent.agent_id);
    if (signature) return refusal(signature);
  }
  const aerSignature = verifySignatures(
    evidence.aer,
    pins.aer,
    'aer',
    evidence.aer.border_gateway.gateway_id,
  );
  if (aerSignature) return refusal(aerSignature);

  const rootIssued = instantMs(root.issued_at);
  const rootExpires = instantMs(root.expires_at);
  if (rootIssued >= rootExpires) return refusal('invalid_roa_validity_interval');
  if (verificationTime < rootIssued) return refusal('roa_not_yet_valid');
  if (verificationTime > rootExpires) return refusal('roa_expired');
  if (root.authorization.approval_state === 'pending') return refusal('roa_approval_pending');

  if (root.policy.policy_id !== profile.expected_policy_id) return refusal('policy_id_mismatch');
  if (root.policy.policy_version !== profile.expected_policy_version) return refusal('policy_version_mismatch');
  if (root.policy.policy_digest !== profile.expected_policy_digest) return refusal('policy_digest_mismatch');
  if (aras.length > root.authorized_scope.max_delegation_depth) return refusal('delegation_depth_exceeded');
  ...
  for (let index = 0; index < aras.length; index++) {
    ...
    if (ara.upstream_ref.ref_digest !== sha256(parent)) return refusal('ara_parent_digest_mismatch');
    if (ara.delegating_agent.agent_id !== parentAgent) return refusal('ara_delegator_mismatch');
    if (ara.delegating_agent.session_id !== root.session.session_id) return refusal('ara_session_mismatch');
    if (ara.policy.policy_digest !== root.policy.policy_digest
        || ara.policy.policy_version !== root.policy.policy_version) return refusal('ara_policy_mismatch');
    ...
    if (scopeFailure) return refusal(scopeFailure);
    parent = ara;
  ...
  if (aerTime < previousTime) return refusal('aer_time_before_chain');
  if (aerTime > rootExpires || aerTime > verificationTime) return refusal('aer_time_outside_session');
  if (aer.session.session_id !== root.session.session_id) return refusal('aer_session_mismatch');
  if (aer.session.agent_id !== parentAgent) return refusal('aer_agent_mismatch');
  if (aer.policy.policy_id !== root.policy.policy_id
      || aer.policy.policy_digest !== root.policy.policy_digest) return refusal('aer_policy_mismatch');
  ...
  if (aer.chain_summary.chain_digest !== sha256(evidence.chain)) return refusal('aer_chain_digest_mismatch');
  if (canonicalize(aer.action) !== canonicalize(context.action)) return refusal('aer_action_mismatch');

  const effective = resolveCapabilities(parentScope.capabilities, profile.capability_manifest);
  if (!effective.ok) return refusal(effective.reason);
  ...
  if (aer.enforcement_outcome === 'deny') return refusal('aer_denied', bound);
 ...
 *   }
 */
export function verifyAgentROA(evidence: Obj, context: Obj = {}): Obj {
  try {
