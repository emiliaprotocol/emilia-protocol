...
function hasExactKeys(value: any, allowed: Set<string>, required: Set<string> = allowed): boolean {
...
function validText(value: any, maximum = 512): boolean {
...
function validEpoch(value: any): boolean {
  return (typeof value === 'string' && validText(value, 128))
    || (Number.isSafeInteger(value) && value >= 0);
}

function validBudgetUnit(value: any): boolean {
  return typeof value === 'string' && UNIT.test(value);
}

function validJurisdictions(value: any): boolean {
  if (!Array.isArray(value) || value.length === 0 || value.length > 32) return false;
  if (!value.every((entry) => validText(entry, 64))) return false;
...
function parseInstant(value: any): number {
...
function decodeCanonicalBase64url(value: any, expectedLength?: number): Buffer | null {
...
function actionShapeValid(action: any): boolean {
  if (!hasExactKeys(action, ACTION_KEYS, REQUIRED_ACTION_KEYS)) return false;
  if (typeof action.effect_type !== 'string' || !EFFECT_TYPE.test(action.effect_type)) return false;
  for (const field of ['interface_id', 'target_id', 'audience']) {
    if (!validText(action[field])) return false;
  }
  for (const field of ['tenant_id', 'purpose_id']) {
    if (Object.hasOwn(action, field) && !validText(action[field])) return false;
  }
  if (Object.hasOwn(action, 'jurisdiction') && !validJurisdictions(action.jurisdiction)) return false;
  if (!isRecord(action.request)) return false;
  if (Object.hasOwn(action, 'budget')) {
    if (!hasExactKeys(action.budget, ACTION_BUDGET_KEYS)) return false;
    if (!validBudgetUnit(action.budget.unit)) return false;
    if (!Number.isSafeInteger(action.budget.amount) || action.budget.amount < 0) return false;
  }
 ...
 * ambiguous request.
 */
export function computeOrprgActionDigest(action: any): string | null {
  try {
...
function scopeShapeValid(scope: any): boolean {
  if (!hasExactKeys(scope, SCOPE_KEYS, REQUIRED_SCOPE_KEYS)) return false;
  if (typeof scope.effect_type !== 'string' || !EFFECT_TYPE.test(scope.effect_type)) return false;
  for (const field of ['interface_id', 'target_id', 'audience']) {
    if (!validText(scope[field])) return false;
  }
  for (const field of ['tenant_id', 'purpose_id']) {
    if (Object.hasOwn(scope, field) && !validText(scope[field])) return false;
  }
  if (Object.hasOwn(scope, 'jurisdiction') && !validJurisdictions(scope.jurisdiction)) return false;
  if (Object.hasOwn(scope, 'budget')) {
    if (!hasExactKeys(scope.budget, BUDGET_KEYS)) return false;
    if (!validBudgetUnit(scope.budget.unit)) return false;
    if (!Number.isSafeInteger(scope.budget.limit) || scope.budget.limit < 0) return false;
  }
  return true;
}

function receiptShapeValid(receipt: any): boolean {
  if (!hasExactKeys(receipt, RECEIPT_KEYS)) return false;
  if (receipt['@version'] !== ORPRG_JSON_JCS_PROFILE) return false;
  if (!hasExactKeys(receipt.receipt_core, CORE_KEYS)) return false;
  if (!hasExactKeys(receipt.status, STATUS_KEYS)) return false;
  if (!hasExactKeys(receipt.authenticity, AUTHENTICITY_KEYS)) return false;

  const core = receipt.receipt_core;
  if (!HASH.test(core.policy_digest) || !validEpoch(core.epoch_id)) return false;
  if (!HASH.test(core.action_digest)) return false;
  if (!validText(core.canonicalization_profile, 128)) return false;
  if (!scopeShapeValid(core.scope)) return false;
  if (!hasExactKeys(core.anti_replay, ANTI_REPLAY_KEYS)) return false;
  if (core.anti_replay.mode !== 'single-use'
    || typeof core.anti_replay.nonce !== 'string'
    || !NONCE.test(core.anti_replay.nonce)) return false;

  if (!STATUS_STATES.has(receipt.status.state)) return false;
  if (!Number.isFinite(parseInstant(core.valid_from))
    ...
    || !Number.isFinite(parseInstant(receipt.status.next_update))) return false;
  ...
  if (!validText(authenticity.issuer_id, 1024)
    ...
    || decodeCanonicalBase64url(authenticity.signature, 64) === null) return false;
  return canonicalJsonSafety(receipt);
}

function parseReceipt(input: any): any {
  if (typeof input !== 'string') return input;
  if (Buffer.byteLength(input, 'utf8') > MAX_WIRE_BYTES) return null;
...
function pinnedIssuerKey(issuerKeys: any, issuerId: string, keyId: string): crypto.KeyObject | null {
  if (!isRecord(issuerKeys) || !Object.hasOwn(issuerKeys, issuerId)) return null;
  const issuer = issuerKeys[issuerId];
  if (!isRecord(issuer) || !Object.hasOwn(issuer, keyId)) return null;
...
function verifySignature(receipt: any, key: crypto.KeyObject | null): boolean {
  if (!key) return false;
  ...
  if (canonical === null || signature === null) return false;
  try {
 ...
 * }}
 */
function baseDetail(checks: Obj): Obj {
  return {
...
function deny(code: string, reason: string, detail: Obj): Obj {
...
function allow(detail: Obj, actionDigest: string | null): Obj {
...
function prepareVerification(input: any, options: any): Obj {
  ...
  checks.scope_binding = scopeMatchesAction(
    receipt.receipt_core.scope,
    opts.expectedAction,
  ...
  const issuerKey = pinnedIssuerKey(
    opts.issuerKeys,
    receipt.authenticity.issuer_id,
    receipt.authenticity.key_id,
  );
...
function replayHookShapeValid(hook: any): boolean {
 ...
 */
export function verifyOrprgJsonJcsPermit(input: any, options: any = {}): Obj {
  try {
    ...
    if (!replayHookShapeValid(prepared.antiReplay)) {
      return deny(
        'ANTI_REPLAY_FAILURE',
        'single-use profile requires an atomic durable anti-replay hook',
        prepared.detail,
      );
    }
    ...
    if (isThenable(consumed) || consumed !== true) {
      return deny(
        'ANTI_REPLAY_FAILURE',
        'receipt replayed or durable anti-replay state is unavailable or ambiguous',
        prepared.detail,
      );
    }
 ...
 */
export async function verifyOrprgJsonJcsPermitAsync(input: any, options: any = {}): Promise<Obj> {
  try {
    const prepared = prepareVerification(input, options);
    if (prepared.result) return prepared.result;
    if (!replayHookShapeValid(prepared.antiReplay)) {
...
function captureIssuerKeys(value: any): Obj {
  const captured = Object.create(null);
  if (!isRecord(value)) return Object.freeze(captured);
  for (const [issuerId, issuer] of Object.entries(value) as Array<[string, any]>) {
    if (!isRecord(issuer)) continue;
    const keys = Object.create(null);
    for (const [keyId, publicKey] of Object.entries(issuer) as Array<[string, any]>) {
      if (typeof publicKey === 'string') keys[keyId] = publicKey;
    }
...
function captureReplayHook(value: any): any {
  try {
 ...
 */
export function createOrprgAecVerifier(profile: any = {}): (evidence: any, context?: any) => Obj {
