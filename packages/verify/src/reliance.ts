...
function pubKeyB64u(pub: unknown): string | null {
  // Accept a raw base64url SPKI string or a { public_key } object.
  if (typeof pub === 'string') return pub;
  if (pub && typeof pub === 'object' && typeof (pub as Obj).public_key === 'string') return (pub as Obj).public_key;
  return null;
}

function spkiFingerprint(value: string): string | null {
  try {
...
function validateQuorumPolicy(policy: Obj): string[] {
  const issues: string[] = [];
...
function parseNonNegativeDecimal(value: unknown): { coefficient: bigint; scale: number } | null {
  let text = null;
...
function decimalGreaterThan(left: unknown, right: unknown): boolean | null {
...
function decimalEqual(left: unknown, right: unknown): boolean | null {
...
function exactMaterial(candidates: any[]): { value: any; ambiguous: boolean } {
...
function decimalMaterial(candidates: any[]): { value: any; ambiguous: boolean } {
  const present = candidates.filter((value) => value !== undefined && value !== null);
  if (present.length === 0) return { value: null, ambiguous: false };
...
function isApprovalContext(context: unknown): boolean {
  const c = context as Obj | null;
...
function signedActionMaterial(receipt: Obj, approvalContexts: Obj[]): Obj {
 ...
 */
export function evaluateReliance(input: Obj = {}, opts: Obj = {}) {
  ...
  const deny = (verdict: string, reason: string): Obj => {
    reasons.push(reason);
  ...
  const hexOf = (h: unknown): string => String(h || '').replace(/^sha256:/i, '').toLowerCase();
  const ctxByHash = new Map<string, Obj>();
  for (const c of contexts) {
    try { ctxByHash.set(crypto.createHash('sha256').update(canonicalize(c), 'utf8').digest('hex'), c); } catch { /* skip uncanonicalizable */ }
  }
  const verifiedApprovals: Obj[] = [];
  ...
  let quorumMembers: string[] = []; // verified quorum members, for authority subject binding
  ...
  const boundQuorum = () => {
    ...
    const approvalMembers = members.filter((member: any) => isApprovalContext(member?.signoff?.context));
    const pinnedEntries = (Object.values(approverKeys) as Obj[]).filter((entry: any) => entry && typeof entry === 'object');
    const keysAnchored = approvalMembers.length > 0 && approvalMembers.every((member: any) => {
  ...
  if (requiredAssurance === 'class_a') {
    ...
    quorumMembers = q.members.map((m: any) => m.approver);
  ...
  if (prof.required_authority === true || requiredEvidence.has('authority_proof')) {
    ...
    const organizationRegistryKeys = acceptedRegistryKeys.filter(
      (key: any) => key?.organization_id === action.organization_id,
    );
    ...
    const registryPin = organizationRegistryKeys.find((key: any) => key?.public_key === authorityProof?.signature?.public_key
    ...
    if (!ap.accepted) {
      ...
      const reason = (ap as any).reason as string;
...
export function validateRelianceProfile(profile: Obj): { ok: boolean; issues: string[] } {
  const issues: string[] = [];
  ...
  if (profile.required_evidence !== undefined) {
    if (!Array.isArray(profile.required_evidence)) {
      ...
      for (const item of profile.required_evidence as any[]) {
