...
export interface RevocationTarget { target_type: RevocationTargetType; target_id: string; action_hash: string; }
...
function revocationSignedPayload(stmt: RevocationStatement): Buffer {
  return Buffer.from(
    canonicalize({
...
function exactKeys(value: unknown, allowed: Set<string>): boolean {
  return value !== null && typeof value === 'object' && !Array.isArray(value)
    && Object.keys(value).length === allowed.size
    && Object.keys(value).every((key) => allowed.has(key));
}

function revokerKeyId(publicKeyB64u: unknown): string {
  try {
...
function isLegacyRevokerKeyId(value: unknown): boolean {
 ...
 */
export function verifyRevocation(target: RevocationTarget | null | undefined, statement: RevocationStatement | null | undefined, opts: RevocationOptions = {}) {
  ...
  if (!target || typeof target !== 'object' || Array.isArray(target)) {
    ...
    if (!(typeof statement.target_type === 'string' && TARGET_TYPES.includes(statement.target_type as RevocationTargetType))
 ...
 */
export function isRevoked(target: RevocationTarget | null | undefined, statements: RevocationStatement[] | unknown, opts: RevocationOptions = {}): boolean {
  if (!Array.isArray(statements)) return false;
  return (statements as RevocationStatement[]).some((s) => verifyRevocation(target, s, opts).valid);
}
...
export const REVOCATION_V2_REQUIRED_ALGORITHMS = Object.freeze(['Ed25519', 'ML-DSA-65'] as const);
...
export interface RevocationV2Signature { alg?: unknown; sig?: unknown; key_id?: unknown; }
 ...
 */
export function revocationV2SignedPayload(
  stmt: RevocationV2Statement,
  requiredAlgorithms: readonly string[] = REVOCATION_V2_REQUIRED_ALGORITHMS,
): Buffer {
 ...
 */
export async function verifyRevocationV2(
  target: RevocationTarget | null | undefined,
  statement: RevocationV2Statement | null | undefined,
  opts: RevocationV2Options = {},
  ...
  const proof = (statement.proof || null) as RevocationV2Proof | null;
  ...
  if (!algorithmSetMatchesRegistered(proof?.required_algorithms)) {
    fail('algorithm_set',
      `proof.required_algorithms must be exactly ${JSON.stringify([...REVOCATION_V2_REQUIRED_ALGORITHMS])} (set narrowing / widening refused)`);
  }

  // 4. Exactly one signature per required algorithm.
  const signatures = Array.isArray(proof?.signatures) ? proof!.signatures as RevocationV2Signature[] : null;
  if (!signatures || signatures.length === 0) {
    ...
    if (!malformed) {
      ...
      for (const alg of presented) {
        if (!(REVOCATION_V2_REQUIRED_ALGORITHMS as readonly string[]).includes(alg)) {
  ...
  const revokerId = statement.revoker_id;
  const revokerIdValid = typeof revokerId === 'string' && revokerId.length > 0;
  const pin: RevokerV2KeyPin = revokerIdValid && revokerKeys && typeof revokerKeys === 'object'
    ? revokerKeys[revokerId as string] || {}
    : {};
  ...
  if (!revokerIdValid) {
    ...
    fail('revoker_key_pinned',
      `no pinned Ed25519 + ML-DSA-65 key pair for revoker "${revokerId}" (identified but not trusted)`);
  } else {
  ...
  if (!derivedKeyId || proof?.revoker_key_id !== derivedKeyId
    ...
    || (pin.key_id !== undefined && pin.key_id !== derivedKeyId)) {
    fail('revoker_key_bound',
      'revoker_key_id must be the full SPKI digest of the presented Ed25519 revoker key');
  }
  ...
  if (!derivedPqKeyId || proof?.pq_key_id !== derivedPqKeyId
    ...
    fail('revoker_key_bound',
      'pq_key_id must be the full digest of the presented ML-DSA-65 revoker key');
  }
  ...
  if (setResult?.verified !== true) {
    ...
    const failedLeg = Array.isArray(setResult?.results)
      ...
      : null;
    fail('revoker_signature_valid',
      `revoker signature set does not verify under the pinned Ed25519 + ML-DSA-65 keys (${reason})`);
    ...
    if (failedLeg?.reason === 'signature_invalid') {
      fail('signature_binds_statement',
        'revoker signature set does not bind the presented statement bytes (recomputed payload mismatch)');
    }
  }
...
function pqRevokerKeyId(publicKeyRawB64u: unknown): string {
  try {
...
function agilityPassthrough(opts: RevocationV2Options): AgilityOptions {
...
export async function verifyRevocationStatement(
  target: RevocationTarget | null | undefined,
  statement: RevocationStatement | RevocationV2Statement | null | undefined,
  opts: RevocationV2Options = {},
): Promise<RevocationVerifyResult> {
  if (statement && typeof statement === 'object' && !Array.isArray(statement)
    && (statement as RevocationV2Statement)['@version'] === REVOCATION_V2_VERSION) {
    return verifyRevocationV2(target, statement as RevocationV2Statement, opts);
  }
  return verifyRevocation(target, statement as RevocationStatement, opts as RevocationOptions);
}
 ...
 */
export async function isRevokedAny(
  target: RevocationTarget | null | undefined,
  statements: unknown,
  opts: RevocationV2Options = {},
): Promise<boolean> {
  ...
  for (const s of statements) {
    if ((await verifyRevocationStatement(target, s, opts)).valid) return true;
  }
  return false;
}
