...
import {
  ...
  type StrictCanonicalJsonLimits,
} from './strict-json.js';

export const CLAIM_ASSURANCE_PROFILE_VERSION = 'EP-CLAIM-ASSURANCE-PROFILE-v1' as const;
export const CLAIM_CASE_VERSION = 'EP-CLAIM-CASE-v1' as const;
export const ASSURANCE_RECORD_VERSION = 'EP-ASSURANCE-RECORD-v1' as const;
...
export const EVIDENCE_VERDICTS = ['VERIFIED', 'UNVERIFIED', 'INDETERMINATE'] as const;
export const EVIDENCE_RELATIONSHIPS = ['SUPPORTS', 'CONTRADICTS', 'NEUTRAL'] as const;
...
const PROFILE_LIMITS: StrictCanonicalJsonLimits = {
  maxDepth: 12,
  maxNodes: 512,
  maxStringBytes: 32_768,
};
const CASE_LIMITS: StrictCanonicalJsonLimits = {
...
const ARTIFACT_LIMITS: StrictCanonicalJsonLimits = {
...
const VERIFIER_RESULT_LIMITS: StrictCanonicalJsonLimits = {
  maxDepth: 8,
  maxNodes: 128,
...
function assertExactKeys(
  ...
  for (const key of Object.keys(value).sort()) {
    if (!allowed.has(key)) protocolError(`unknown member ${path}.${key}`);
  }
  for (const key of required) {
    if (!Object.hasOwn(value, key)) protocolError(`${path}.${key} is required`);
  }
}

function assertBoundedString(value: unknown, path: string, maxBytes: number): asserts value is string {
...
function assertIdentifier(value: unknown, path: string): asserts value is string {
  ...
  if (!IDENTIFIER_RE.test(value)) protocolError(`${path} is not a valid identifier`);
}
...
function assertSafeInteger(value: unknown, path: string, min: number, max: number): asserts value is number {
  if (!Number.isSafeInteger(value) || (value as number) < min || (value as number) > max) {
    protocolError(`${path} must be a safe integer between ${min} and ${max}`);
  }
}

function sha256(bytes: string): Sha256Digest {
  return `sha256:${crypto.createHash('sha256').update(bytes, 'utf8').digest('hex')}`;
}

function domainDigest(domain: string, value: unknown, limits: StrictCanonicalJsonLimits): Sha256Digest {
  return sha256(`${domain}\0${canonicalizeStrictJson(value, limits)}`);
}

function verifierKey(value: VerifierDescriptor): string {
...
function validateVerifierDescriptor(value: unknown, path: string): VerifierDescriptor {
  ...
  return object as unknown as VerifierDescriptor;
}

function validateProfile(value: unknown): ClaimAssuranceProfile {
  ...
  const requirementSlots = new Set<string>();
  ...
  return profile as unknown as ClaimAssuranceProfile;
}

function validateClaimStatement(value: unknown, path = 'claim_case.claim'): ClaimStatement {
  ...
  return claim as unknown as ClaimStatement;
}

function validateEvidenceBinding(value: unknown, path: string): EvidenceBinding {
  ...
  if (Object.hasOwn(binding, 'action_digest')) assertDigest(binding.action_digest, `${path}.action_digest`);
  return binding as unknown as EvidenceBinding;
}

function validateClaimCase(value: unknown): ClaimCase {
  ...
  if (Object.hasOwn(claimCase, 'action_digest')) assertDigest(claimCase.action_digest, 'claim_case.action_digest');
  ...
  claimCase.evidence.forEach((raw, index) => {
    ...
    if (evidenceIds.has(item.evidence_id)) protocolError(`duplicate evidence_id ${item.evidence_id}`);
    if (artifactDigests.has(item.artifact_digest)) protocolError(`duplicate artifact_digest ${item.artifact_digest}`);
    evidenceIds.add(item.evidence_id);
    artifactDigests.add(item.artifact_digest);
  });
  return claimCase as unknown as ClaimCase;
}

function validateRegistry(value: unknown): Map<string, EvidenceVerifierRegistration> {
  ...
  const output = new Map<string, EvidenceVerifierRegistration>();
  value.forEach((raw, index) => {
    ...
    if (typeof registration.verify !== 'function') protocolError(`${path}.verify must be a function`);
    const key = verifierKey(descriptor);
    if (output.has(key)) protocolError(`duplicate verifier registration ${descriptor.verifier_id}`);
    ...
    output.set(key, Object.freeze({
      ...
      verify: registration.verify as EvidenceVerifierRegistration['verify'],
    }));
  });
  return output;
}
...
function deepFreeze<T>(value: T): T {
  ...
  if (Array.isArray(value)) {
    ...
    for (const key of Object.keys(value as Obj)) deepFreeze((value as Obj)[key]);
  }
  return Object.freeze(value);
}

function strictClone<T>(value: T, limits: StrictCanonicalJsonLimits): T {
  return JSON.parse(canonicalizeStrictJson(value, limits)) as T;
}

/** SHA-256 over strict canonical artifact bytes. */
export function claimAssuranceArtifactDigest(
  artifact: unknown,
...
export function claimAssuranceProfileHash(profile: unknown): Sha256Digest {
...
export function claimCaseDigest(claimCase: unknown): Sha256Digest {
  validateClaimCase(claimCase);
  return sha256(canonicalizeStrictJson(claimCase, CASE_LIMITS));
}

function malformedVerifierResult(): null {
  return null;
}

function validateVerifierResult(value: unknown): EvidenceVerifierResult | null {
  try {
    ...
    if (!EVIDENCE_VERDICTS.includes(result.verdict as EvidenceVerdict)) return malformedVerifierResult();
    if (!EVIDENCE_RELATIONSHIPS.includes(result.relationship as EvidenceRelationship)) return malformedVerifierResult();
    ...
    if (Date.parse(result.expires_at) <= Date.parse(result.observed_at)) return malformedVerifierResult();
    assertDigest(result.artifact_digest, 'verifier_result.artifact_digest');
    if (!Array.isArray(result.reasons)
        || result.reasons.length > CLAIM_ASSURANCE_LIMITS.max_verifier_reasons) return malformedVerifierResult();
    for (const reason of result.reasons) {
      assertBoundedString(reason, 'verifier_result.reasons[]', CLAIM_ASSURANCE_LIMITS.max_verifier_reason_bytes);
    }
    return result as unknown as EvidenceVerifierResult;
  } catch {
    return malformedVerifierResult();
  }
}

function blankEvidenceResult(item: ClaimEvidenceItem): AssuranceEvidenceResult {
  return {
...
function resultWithVerifier(
  base: AssuranceEvidenceResult,
  result: EvidenceVerifierResult,
): AssuranceEvidenceResult {
  return {
    ...base,
...
function evaluateEvidence(
  ...
  claimCase: ClaimCase,
  profile: ClaimAssuranceProfile,
  registry: Map<string, EvidenceVerifierRegistration>,
  evaluatedAt: string,
): AssuranceEvidenceResult {
  ...
  if (item.binding.subject_digest !== claimCase.subject_digest) bindingReasons.push('SUBJECT_BINDING_MISMATCH');
  if (item.binding.scope_digest !== claimCase.scope_digest) bindingReasons.push('SCOPE_BINDING_MISMATCH');
  if (item.binding.claim_id !== claimCase.claim.claim_id) bindingReasons.push('CLAIM_BINDING_MISMATCH');
  const caseAction = claimCase.action_digest ?? null;
  const itemAction = item.binding.action_digest ?? null;
  if (itemAction !== caseAction) bindingReasons.push('ACTION_BINDING_MISMATCH');
  if (bindingReasons.length > 0) {
    return { ...base, disposition: 'REJECTED', reasons: bindingReasons };
  }

  const matchingRequirements = profile.requirements.filter((requirement) => (
    requirement.evidence_role === item.role
  ...
  if (!registration) return { ...base, disposition: 'INDETERMINATE', reasons: ['VERIFIER_NOT_REGISTERED'] };
  ...
  }) as Readonly<EvidenceVerifierInput>;
  ...
  if (!result) return { ...base, disposition: 'INDETERMINATE', reasons: ['VERIFIER_RESULT_MALFORMED'] };
  const withVerifier = resultWithVerifier(base, result);

  const restatementReasons: string[] = [];
  if (result.subject_digest !== claimCase.subject_digest) restatementReasons.push('VERIFIER_SUBJECT_MISMATCH');
  if (result.scope_digest !== claimCase.scope_digest) restatementReasons.push('VERIFIER_SCOPE_MISMATCH');
  if (result.claim_id !== claimCase.claim.claim_id) restatementReasons.push('VERIFIER_CLAIM_MISMATCH');
  if (result.artifact_digest !== item.artifact_digest) restatementReasons.push('VERIFIER_ARTIFACT_MISMATCH');
...
function requirementResult(
  requirement: ClaimAssuranceRequirement,
  evidenceItems: ClaimEvidenceItem[],
  evidenceResults: AssuranceEvidenceResult[],
): AssuranceRequirementResult {
  ...
  const supports = new Set(relevant
    ...
    .map((result) => result.source_id as string));
...
function normalizeReasons(
  frameworkReasons: readonly string[],
...
function replayDigest(record: Omit<AssuranceRecord, 'replay_digest' | 'record_digest'>): Sha256Digest {
...
function assuranceRecordDigest(record: Omit<AssuranceRecord, 'record_digest'>): Sha256Digest {
 ...
 */
export function evaluateClaimAssurance(
  input: unknown,
  options: ClaimAssuranceOptions,
): Readonly<AssuranceRecord> {
  ...
  if (claimCase.profile_id !== profile.profile_id) protocolError('claim case profile_id does not match pinned profile');
  if (claimCase.profile_hash !== computedProfileHash) protocolError('claim case profile_hash does not match pinned profile');
  if (claimCase.claim.claim_type !== profile.claim_type) protocolError('claim type does not match pinned profile');
  if (claimCase.claim.predicate !== profile.predicate) protocolError('claim predicate does not match pinned profile');
  ...
  const evidenceResults = sortedEvidence.map((item) => (
    evaluateEvidence(item, claimCase, profile, registry, evaluatedAt)
  ));
  const sortedRequirements = [...profile.requirements]
    .sort((left, right) => compareCodeUnits(left.requirement_id, right.requirement_id));
  const requirementResults = sortedRequirements.map((requirement) => (
    requirementResult(requirement, sortedEvidence, evidenceResults)
  ));

  let verdict: ClaimAssuranceVerdict;
  let reasons: string[];
  ...
  const body: Omit<AssuranceRecord, 'replay_digest' | 'record_digest'> = {
  ...
  const withReplay: Omit<AssuranceRecord, 'record_digest'> = {
    ...body,
...
function validateRecordReasons(value: unknown, path: string): asserts value is string[] {
  if (!Array.isArray(value)
  ...
  for (const reason of value) {
    ...
    if (seen.has(reason)) protocolError(`${path} must not contain duplicate reasons`);
    seen.add(reason);
  }
}
...
function validateAssuranceRecordSemantics(value: unknown): AssuranceRecord {
  canonicalizeStrictJson(value, CASE_LIMITS);
  ...
  if (record.action_digest !== null) assertDigest(record.action_digest, 'assurance_record.action_digest');
  ...
  if (!CLAIM_ASSURANCE_VERDICTS.includes(record.verdict as ClaimAssuranceVerdict)) {
  ...
  for (const [index, raw] of record.requirement_results.entries()) {
    ...
    if (typeof result.satisfied !== 'boolean') protocolError(`${path}.satisfied must be boolean`);
    validateRecordReasons(result.reasons, `${path}.reasons`);
    const isSatisfied = result.disposition === 'SATISFIED';
    if (result.satisfied !== isSatisfied) protocolError(`${path}.satisfied contradicts disposition`);
    if (isSatisfied) {
    ...
    if (result.disposition === 'DIVERGED'
        && (result.accepted_supporting_sources === 0
          || result.accepted_contradicting_sources === 0
          || !sameStringArray(result.reasons as string[], ['ACCEPTED_SOURCES_DIVERGED']))) {
  ...
  for (const [index, raw] of record.evidence_results.entries()) {
    ...
    if (result.verifier_verdict !== null
        && !EVIDENCE_VERDICTS.includes(result.verifier_verdict as EvidenceVerdict)) {
      protocolError(`${path}.verifier_verdict is unsupported`);
    }
    if (result.relationship !== null
        && !EVIDENCE_RELATIONSHIPS.includes(result.relationship as EvidenceRelationship)) {
      protocolError(`${path}.relationship is unsupported`);
    }
    if (result.source_id !== null) assertIdentifier(result.source_id, `${path}.source_id`);
    if (result.observed_at !== null) assertCanonicalTimestamp(result.observed_at, `${path}.observed_at`);
    if (result.expires_at !== null) assertCanonicalTimestamp(result.expires_at, `${path}.expires_at`);
    validateRecordReasons(result.reasons, `${path}.reasons`);
    ...
    if (!allNull && !allPresent) protocolError(`${path} has a partial verifier result`);
    if (allPresent) {
      if (Date.parse(result.expires_at as string) <= Date.parse(result.observed_at as string)) {
    ...
    if (result.disposition === 'ACCEPTED') {
      ...
      if (Date.parse(result.observed_at as string) > Date.parse(record.as_of as string)
          || Date.parse(record.evaluated_at as string) >= Date.parse(result.expires_at as string)) {
  ...
  const requirements = record.requirement_results as unknown as AssuranceRequirementResult[];
  const evidence = record.evidence_results as unknown as AssuranceEvidenceResult[];
  ...
  let expectedVerdict: ClaimAssuranceVerdict;
  ...
  if (record.verdict !== expectedVerdict
      ...
      || !sameStringArray(record.reasons as string[], expectedReasons)) {
    protocolError('assurance_record verdict, profile_satisfied, or reasons are inconsistent');
  }
  return record as unknown as AssuranceRecord;
}

function blankIntegrityResult(
  recordDigest: string | null,
  expectedProvided: boolean,
  reason: string,
): AssuranceRecordIntegrityResult {
  return {
 ...
 */
export function inspectAssuranceRecordIntegrity(
  value: unknown,
  options: AssuranceRecordIntegrityOptions = {},
): Readonly<AssuranceRecordIntegrityResult> {
  ...
  const {
    ...
    record_digest: _claimedRecordDigest,
    ...body
  } = record;
 ...
 */
export function verifyAssuranceRecordDigest(value: unknown): {
  ok: boolean;
  record_digest: string | null;
  reason: string | null;
} {
