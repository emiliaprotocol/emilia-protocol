...
const tooLong = (value: unknown): boolean => typeof value === 'string'
 ...
 */
export function predictedEffectsDigest(predictedEffects: any): string {
...
export function isDecimalString(s: unknown): boolean {
 ...
 */
export function compareDecimalStrings(a: unknown, b: unknown): number | null {
...
const PREDICATE_KEYS: Record<string, string[]> = Object.freeze({
  eq: ['op', 'value'],
  lte: ['op', 'value'],
  gte: ['op', 'value'],
  range: ['op', 'min', 'max'],
 ...
 */
export function validatePredictedEffects(predicted: any): { ok: boolean; reasons: string[] } {
  const reasons: string[] = [];
  const bad = (why: string): void => { reasons.push(why); };
...
function validateObservedEffects(observed: any): { ok: boolean; reasons: string[] } {
  const reasons: string[] = [];
  const bad = (why: string): void => { reasons.push(why); };
...
const key = (effectType: string, target: string): string => `${effectType} on ${target}`;
...
function evaluateEntry(entry: any, matches: any[]): any {
  const { predicate: p } = entry;
  const at = key(entry.effect_type, entry.target);
  const inBounds = { outcome: 'in_bounds', reason: null };
  const divergent = (reason: string) => ({ outcome: 'divergent' as const, reason });
  const incomparable = (reason: string) => ({ outcome: 'incomparable' as const, reason });
  ...
  if (p.op === 'count_lte') {
    ...
    const cmp = compareDecimalStrings(count, p.value) as number;
    return cmp <= 0
      ? inBounds
  ...
  if (p.op === 'lte') {
    const cmp = compareDecimalStrings(obs.value, p.value) as number;
    return cmp <= 0
      ? inBounds
  ...
  // range
  const cmpMin = compareDecimalStrings(obs.value, p.min) as number;
  if (cmpMin < 0) return divergent(`predicted range [${p.min}, ${p.max}] for ${at}, observed ${obs.value} (below min)`);
  const cmpMax = compareDecimalStrings(obs.value, p.max) as number;
 ...
 *            reasons: string[] }}
 */
export function evaluatePredictedEffects(predicted: any, observed: any): {
  outcome: 'in_bounds' | 'divergent' | 'incomparable';
  results: Array<{ effect_type: string; target: string; op: string; outcome: string; reason: string | null }>;
  reasons: string[];
} {
  ...
  if (!observedStructural.ok) {
    return {
      outcome: 'incomparable',
      results: [],
    reasons: observedStructural.reasons.map((reason: string) => `malformed observed_effects: ${reason}`),
    };
  }
  const results = predicted.map((entry: any) => {
    const matches = observed.filter(
      (o: any) => o && typeof o === 'object' && o.effect_type === entry.effect_type && o.target === entry.target,
    );
  ...
  const reasons = results.filter((r: any) => r.reason).map((r: any) => r.reason);
  ...
  const outcome = results.some((r: any) => r.outcome === 'divergent') ? 'divergent'
    : results.some((r: any) => r.outcome === 'incomparable') ? 'incomparable'
      : 'in_bounds';
