...
type TimestampResult =
  | { verified: false; tsa_key_id: null; gen_time: null; reason: string }
  | { verified: true; tsa_key_id: string; gen_time: string };
...
function readTLV(buf: Buffer, offset: number): DerNode {
...
function* children(node: DerNode): Generator<DerNode> {
...
function content(node: DerNode): Buffer {
...
function raw(node: DerNode): Buffer {
...
function decodeGeneralizedTime(node: DerNode): { iso: string; ms: number } | null {
...
function keyIdOfSpki(spkiDer: Buffer): string {
...
function loadPinnedKey(pinned: unknown): LoadedKey | null {
  try {
...
function resolveSignatureAlg(sigAlgOid: string, digestName: string | null, keyType: string | undefined): { hash: string; dsaEncoding?: 'der' } | null {
 ...
 */
export function verifyTimestampProof(timestampProof: unknown, expectedDigest: unknown, pinnedTsaKeys: unknown): TimestampResult {
  const refuse = (reason: string): TimestampResult => ({ verified: false, tsa_key_id: null, gen_time: null, reason });
  ...
  const loadedKeys = pinnedList.map(loadPinnedKey).filter((key): key is LoadedKey => key !== null);
  ...
  if (parsed.error || !parsed.tstInfo || !parsed.signerInfo || !parsed.eContentRaw) return refuse(parsed.error ?? 'unparseable_token');
...
function parseTimeStampToken(der: Buffer): ParsedToken {
  ...
  if ('error' in tstInfo && tstInfo.error) return { error: tstInfo.error };
  ...
  if ('error' in signerInfo && signerInfo.error) return { error: signerInfo.error };

  return { tstInfo: tstInfo as TstInfo, signerInfo: signerInfo as SignerInfo, eContentRaw };
}
...
function parseTstInfo(der: Buffer): TstInfo | { error: string } {
  try {
...
function parseSignerInfo(node: DerNode): SignerInfo | { error: string } {
  try {
...
function parseAttributes(setNode: DerNode): Record<string, DerNode[]> {
  const out: Record<string, DerNode[]> = {};
...
function verifySignerInfo(
...
): { ok: true; tsaKeyId: string } | { ok: false; reason: string } {
  ...
  for (const { key, spkiDer } of loadedKeys) {
    ...
    try {
      const verifyOpts: { key: crypto.KeyObject; dsaEncoding?: 'der' } = { key };
...
function derSetHeader(len: number): Buffer {
