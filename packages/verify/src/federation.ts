 ...
 */
export function resolveOperatorKeys(discoveryDoc: Obj, signerId: string): Array<{ public_key: string; status: 'current' | 'historical'; algorithm: string; retired_at?: string }> {
  const candidates: Array<{ public_key: string; status: 'current' | 'historical'; algorithm: string; retired_at?: string }> = [];
 ...
 * }}
 */
export function verifyFederatedReceiptOffline(receipt: Obj, discoveryDoc: Obj, opts: FederationOpts = {}): FederationResult {
...
function isIssuerPinned(signer: string, opts: FederationOpts): boolean {
...
function pinnedSignerIds(trustedIssuers: any): Set<string> {
...
function pinBindingFor(signer: string, opts: FederationOpts): Obj | null {
...
function pinnedPublicKeysFor(signer: string, opts: FederationOpts): Set<string> | null {
...
function pinnedDiscoveryOriginFor(signer: string, opts: FederationOpts): { bound: boolean; origin: string | null } {
  const binding = pinBindingFor(signer, opts);
...
function normalizeStringSet(input: unknown): Set<string> {
  if (input instanceof Set) return input;
  if (Array.isArray(input)) return new Set(input.filter((value): value is string => typeof value === 'string'));
  return new Set();
}
...
export async function verifyFederatedReceipt(receipt: Obj, opts: FederationOpts = {}): Promise<FederationResult> {
  ...
  const bail = (fetched: Obj, error: string): FederationResult => ({
...
function resolveVerifyUrl(discoveryDoc: Obj, verifyUrlBase: string | undefined, keyDiscoveryUrl: string, receiptId: string): string | null {
...
function deriveVerifyBase(keyDiscoveryUrl: string): string | null {
  try {
