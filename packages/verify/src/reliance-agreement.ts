...
type Signer = { party: string; privateKey: crypto.KeyObject };
...
function parseTs(t: unknown): number {
  if (typeof t !== 'string' || t === '') return NaN;
  return Date.parse(t);
}

function pubKeyB64u(ref: unknown): string | null {
  if (typeof ref === 'string') return ref;
  if (ref && typeof ref === 'object' && typeof (ref as Obj).public_key === 'string') return (ref as Obj).public_key;
  return null;
}

function importPinnedKey(b64u: string): crypto.KeyObject | null {
  try {
...
function stripSignatures(agreement: Obj): Obj {
  const { signatures: _sigs, ...body } = agreement;
  return body;
}
function stripSignature(event: Obj): Obj {
  const { signature: _sig, ...body } = event;
  return body;
}

function agreementSigningBytes(unsignedBody: Obj): Buffer {
  return Buffer.from(RELIANCE_AGREEMENT_DOMAIN + canonicalize(unsignedBody), 'utf8');
}
function eventSigningBytes(unsignedBody: Obj): Buffer {
...
export function relianceAgreementDigest(agreement: Obj): string {
...
export function relianceEventDigest(event: Obj): string {
...
export function relianceResultDigest(result: Obj): string {
 ...
 */
export function signRelianceAgreement(payload: Obj, signers: Signer[] = []): Obj {
  ...
  const signatures = (signers || []).map(({ party, privateKey }) => {
    ...
    const publicKey = crypto.createPublicKey(privateKey as any).export({ type: 'spki', format: 'der' }).toString('base64url');
    return {
      party,
      algorithm: 'Ed25519',
 ...
 */
export function signRelianceEvent(payload: Obj, privateKey: crypto.KeyObject): Obj {
...
function checkAmount(terms: Obj, field: string, required: boolean): string | null {
  const v = terms[field];
 ...
 */
export function verifyRelianceAgreement(agreement: Obj, opts: AgreementOptions = {}): Obj {
  ...
  if (!Array.isArray(scope.action_families) || scope.action_families.length === 0
    || !scope.action_families.every((f: any) => typeof f === 'string' && f !== '')) {
    return fail('scope.action_families must be a non-empty array of non-empty strings');
  }
  if (scope.jurisdictions !== undefined
    && (!Array.isArray(scope.jurisdictions) || !scope.jurisdictions.every((j: any) => typeof j === 'string' && j !== ''))) {
  ...
  const amountFields: Array<[string, boolean]> = [['cap_amount', true], ['per_action_cap', false], ['aggregate_cap', false], ['deductible', false]];
  ...
  const byParty = new Map<string, Obj>();
  for (const s of sigs) {
 ...
 */
export function verifyRelianceEvent(event: Obj, opts: EventOptions = {}): Obj {
  const reasons: string[] = [];
  const fail = (reason: string): Obj => { reasons.push(reason); return { valid: false, reasons }; };
  const now = toMs(opts.now);
...
export const RELIANCE_AGREEMENT_V2_REQUIRED_ALGORITHMS = Object.freeze(['Ed25519', 'ML-DSA-65'] as const);
/** The registered required algorithm set, in canonical order (event side). */
export const RELIANCE_EVENT_V2_REQUIRED_ALGORITHMS = Object.freeze(['Ed25519', 'ML-DSA-65'] as const);
...
function algorithmSetMatches(candidate: unknown, registered: readonly string[]): candidate is string[] {
...
function toRawKeyBytesB64u(value: unknown, expectedLength: number, label: string): string {
  let bytes: Buffer;
 ...
 */
function signatureSetStructuralError(entrySignatures: unknown, requiredAlgorithms: readonly string[]): string | null {
  ...
  for (const sig of entrySignatures) {
    if (!sig || typeof sig !== 'object' || Array.isArray(sig)
      || typeof (sig as Obj).alg !== 'string' || typeof (sig as Obj).sig !== 'string') {
      return 'each signatures[] entry must be { alg, sig, key_id? }';
    }
    const alg = (sig as Obj).alg as string;
    if (presented.has(alg)) return `duplicate signature for algorithm "${alg}"`;
    presented.add(alg);
  }
  for (const alg of requiredAlgorithms) {
    if (!presented.has(alg)) return `missing required ${alg} signature (leg stripped)`;
  }
  for (const alg of presented) {
    if (!requiredAlgorithms.includes(alg)) return `unexpected algorithm "${alg}" outside the registered set`;
  }
  return null;
}
 ...
 */
export function agreementV2SigningBytes(
  unsignedBodyV2: Obj,
  requiredAlgorithms: readonly string[] = RELIANCE_AGREEMENT_V2_REQUIRED_ALGORITHMS,
): Buffer {
  ...
  return Buffer.from(
    RELIANCE_AGREEMENT_V2_DOMAIN + canonicalize({ ...unsignedBodyV2, required_algorithms: [...requiredAlgorithms] }),
    'utf8',
  );
}

/** The bytes the relying party's signature set covers for an EP-RELIANCE-EVENT-v2 event. */
export function eventV2SigningBytes(
  unsignedBodyV2: Obj,
  requiredAlgorithms: readonly string[] = RELIANCE_EVENT_V2_REQUIRED_ALGORITHMS,
): Buffer {
  ...
  return Buffer.from(
    RELIANCE_EVENT_V2_DOMAIN + canonicalize({ ...unsignedBodyV2, required_algorithms: [...requiredAlgorithms] }),
    'utf8',
  );
}

/** Digest of the v2 agreement body (domain-separated, signature envelope excluded). */
export function relianceAgreementV2Digest(agreement: Obj): string {
...
export function relianceEventV2Digest(event: Obj): string {
 ...
 */
export async function signRelianceAgreementV2(payload: Obj, signers: RelianceHybridSigner[] = []): Promise<Obj> {
  ...
  for (const s of signers) {
    ...
    const ordered = RELIANCE_AGREEMENT_V2_REQUIRED_ALGORITHMS.map((alg) => {
      ...
      if (!entry) throw new Error(`signRelianceAgreementV2: signing produced no ${alg} leg for '${s.party}' (refusing to emit a half-hybrid agreement)`);
 ...
 * half-hybrid event.
 */
export async function signRelianceEventV2(payload: Obj, signer: RelianceHybridSigner): Promise<Obj> {
  if (!payload || typeof payload !== 'object' || Array.isArray(payload)) {
  ...
  const ordered = RELIANCE_EVENT_V2_REQUIRED_ALGORITHMS.map((alg) => {
    const entry = byAlg.get(alg);
    if (!entry) throw new Error(`signRelianceEventV2: signing produced no ${alg} leg (refusing to emit a half-hybrid event)`);
    return { alg: entry.alg, sig: entry.sig };
  });
  return {
    ...body,
 ...
 * @returns {{valid:boolean, reasons:string[], digest?:string, required_signers?:string[]}}
 */
export async function verifyRelianceAgreementV2(agreement: Obj, opts: RelianceAgreementV2Options = {}): Promise<Obj> {
  const reasons: string[] = [];
  const fail = (reason: string): Obj => { reasons.push(reason); return { valid: false, reasons }; };
  const now = toMs(opts.now);
  const trustedKeys = opts.trustedKeys && typeof opts.trustedKeys === 'object' ? opts.trustedKeys : {};
  ...
  if (!agreement || typeof agreement !== 'object' || Array.isArray(agreement)) return fail('agreement is not an object');
  if (agreement.version !== RELIANCE_AGREEMENT_V2_VERSION) return fail(`version must be ${RELIANCE_AGREEMENT_V2_VERSION}`);
  if (typeof agreement.agreement_id !== 'string' || agreement.agreement_id === '') return fail('agreement_id must be a non-empty string');
  ...
  let bytes: Buffer;
  try { bytes = agreementV2SigningBytes(stripSignatures(agreement)); } catch { return fail('agreement is not JCS-canonicalizable'); }
  const digest = `sha256:${sha256hex(bytes)}`;
  ...
  for (const role of required) {
    if (!byParty.has(role)) return fail(`required signature set from '${role}' is missing (the agreement is not effective)`);
  }
  for (const [role, s] of byParty) {
    const shapeError = signatureSetStructuralError(s.signatures, RELIANCE_AGREEMENT_V2_REQUIRED_ALGORITHMS);
    if (shapeError) return fail(`signature set from '${role}' is malformed: ${shapeError}`);
    const keyId = parties[role].key_id;
    const pin: RelianceV2KeyPin = trustedKeys[keyId] || {};
 ...
 * @returns {{valid:boolean, reasons:string[], agreement_digest?:string, event_digest?:string}}
 */
export async function verifyRelianceEventV2(event: Obj, opts: RelianceEventV2Options = {}): Promise<Obj> {
  const reasons: string[] = [];
  const fail = (reason: string): Obj => { reasons.push(reason); return { valid: false, reasons }; };
  const now = toMs(opts.now);
  const { agreement, relianceResult } = opts;
  ...
  if (!event || typeof event !== 'object' || Array.isArray(event)) return fail('event is not an object');
  if (event.version !== RELIANCE_EVENT_V2_VERSION) return fail(`version must be ${RELIANCE_EVENT_V2_VERSION}`);
  if (typeof event.event_id !== 'string' || event.event_id === '') return fail('event_id must be a non-empty string');
  for (const f of ['agreement_digest', 'action_digest', 'reliance_result_digest']) {
    if (typeof event[f] !== 'string' || !SHA256_RE.test(event[f])) return fail(`${f} must be a sha256:<64 hex> digest`);
  }
  ...
  if (shapeError) return fail(`event signature set is malformed: ${shapeError}`);
  const rpKeyId = agreement.parties.relying_party.key_id;
  const trustedKeys = opts.trustedKeys && typeof opts.trustedKeys === 'object' ? opts.trustedKeys : {};
  const pin: RelianceV2KeyPin = trustedKeys[rpKeyId] || {};
  if (!pin.public_key || !pin.pq_public_key) {
    return fail(`no pinned Ed25519 + ML-DSA-65 key pair for the agreement relying_party (key_id ${rpKeyId})`);
  }
  let bytes: Buffer;
  try { bytes = eventV2SigningBytes(stripSignature(event)); } catch { return fail('event is not JCS-canonicalizable'); }
  let setResult;
  try {
    setResult = await verifyAgileSignatureSet(
      new Uint8Array(bytes),
 ...
 */
export async function verifyRelianceAgreementStatement(agreement: Obj, opts: RelianceAgreementV2Options = {}): Promise<Obj> {
  ...
  return verifyRelianceAgreement(agreement, opts as AgreementOptions);
}

/** Route an event of EITHER version to its verifier. Mirrors verifyRevocationStatement. */
export async function verifyRelianceEventStatement(event: Obj, opts: RelianceEventV2Options = {}): Promise<Obj> {
  if (event && typeof event === 'object' && !Array.isArray(event)
    && event.version === RELIANCE_EVENT_V2_VERSION) {
    return verifyRelianceEventV2(event, opts);
  }
  return verifyRelianceEvent(event, opts as EventOptions);
}
