...
// Generated from aeb-psea-adapter.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
...
import { InMemoryPseaReplayStore, PSEA_AEB_ADAPTER_ID, PSEA_AEB_CAID_MAPPER_ID, PSEA_AEB_CAID_MAPPING_VERSION, PSEA_AEB_CONFIG_VERSION, PSEA_AEB_TRUST_ROOT_VERSION, PSEA_EAT_PROFILE, PSEA_SOURCE_REVISION, canonicalizePsea, createPseaAebAdapter, inspectPseaProof, verifyAndCommitPseaProof, } from './aeb-psea-adapter.js';
...
function signProof(claims, options = {}) {
...
function artifact(claims = baseClaims(), options = {}) {
    return {
...
test('published hostile PSEA vectors fail closed for their declared reason', () => {
    const suite = JSON.parse(readFileSync(new URL('../../conformance/vectors/psea-aeb.v1.json', import.meta.url), 'utf8'));
    ...
    for (const vector of suite.vectors) {
        if (['reject_jti_replay', 'reject_counter_rollback'].includes(vector.id))
            continue;
