...
// Generated from authorization-server-confirmation.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
...
import { AEB_ADAPTER_VERSION, AEB_REGISTRY_VERSION, AEB_REQUIREMENT_VERSION, InMemoryAebConsumptionStore, adapterPinDigest, authorizeAebExecution, canonicalizeAeb, digestAeb, evaluateAebEvidence, mappingProfileDigest, registryEntryDigest, unifiedRegistryDigest, } from './aeb-adapter-contract.js';
import { AUTHORIZATION_SERVER_CONFIRMATION_ADAPTER_ID, AUTHORIZATION_SERVER_CONFIRMATION_ADAPTER_VERSION, AUTHORIZATION_SERVER_CONFIRMATION_ARTIFACT_VERSION, AUTHORIZATION_SERVER_CONFIRMATION_CONFIG_VERSION, AUTHORIZATION_SERVER_CONFIRMATION_MAPPING_VERSION, AUTHORIZATION_SERVER_CONFIRMATION_MAPPER_ID, AUTHORIZATION_SERVER_CONFIRMATION_TOKEN_VERSION, AUTHORIZATION_SERVER_CONFIRMATION_TRUST_ROOT_VERSION, createAuthorizationServerConfirmationActionDefinition, createAuthorizationServerConfirmationAdapter, signAuthorizationServerConfirmation, } from './authorization-server-confirmation.js';
...
const cases = JSON.parse(fs.readFileSync(new URL('./authorization-server-confirmation.v1.json', import.meta.url), 'utf8'));
...
test('AEB/AEC authorizes only when independently verified human and AS legs are digest- and subject-linked', () => {
    ...
    const caid = computeCaid(fixture.action, {
        suite: 'jcs-sha256',
        definitions: fixture.mappingProfile.definition.definitions,
    }).caid;
    const humanAdapter = {
        ...
        verifyNative(input) {
            ...
            return {
                ...
                subject: { id: artifact.approver, kind: 'human' },
    ...
    const entries = {
        'mapping:as-confirmation-payment-release': entry('mapping:as-confirmation-payment-release', 'mapping-profile', { profile_digest: fixture.mappingProfile.profile_digest }),
        'role:human-authorization': entry('role:human-authorization', 'evidence-role', { role: 'human-authorization', subject_kinds: ['human'] }),
        'role:authorization-server-confirmation': entry('role:authorization-server-confirmation', 'evidence-role', { role: 'authorization-server-confirmation', subject_kinds: ['human'] }),
    };
    const registry = {
