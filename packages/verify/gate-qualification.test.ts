...
import {
  ...
  qualificationMerkleParent,
  qualificationPayloadDigest,
  terminalOutcomesRoot,
  validateAgentEvaluationEvidence,
  validateCandidateManifest,
  validateEvaluationCampaign,
  validateQualificationStatement,
...
function descriptor(name: string, digest: string): Obj {
  return { name, digest: { sha256: digest.slice('sha256:'.length) } };
}

function signer(keyid: string): Signer {
...
function publicKey(value: Signer): string {
  return value.publicKey.export({ type: 'spki', format: 'der' }).toString('base64url');
}

function envelope(payload: Obj, value: Signer, payloadType = IN_TOTO_PAYLOAD_TYPE): Obj {
...
function payload(value: Obj): Obj {
  return JSON.parse(Buffer.from(value.payload, 'base64').toString('utf8'));
}

function resign(value: Obj, role: keyof typeof SIGNERS, mutate: (body: Obj) => void, signing = SIGNERS[role]): Obj {
  const body = structuredClone(payload(value));
  mutate(body);
  return envelope(body, signing, value.payloadType);
}

function trust(value: Signer) {
  return { keys: { [value.keyid]: publicKey(value) }, accepted_keyids: [value.keyid], threshold: 1 };
}

function nativeTestResult(manifestDigest: string, campaign: Obj, result: 'PASSED' | 'FAILED', name: string): Obj {
  return {
...
function merkleData(commitments: string[]) {
  const left = qualificationMerkleParent(commitments[0] as any, commitments[1] as any);
  const right = qualificationMerkleParent(commitments[2] as any, commitments[3] as any);
  return {
...
function makeFixture(challengeScheme: 'SALTED_SHA256_SET' | 'MERKLE_SHA256' = 'SALTED_SHA256_SET') {
  ...
  const outcomes = ['PASS', 'FAIL', 'ABORTED', 'EXPIRED'] as const;
...
test('defines the frozen Gate Qualification v2 profiles and deterministic payload digest', () => {
  ...
  assert.equal(
    qualificationPayloadDigest({ b: 2, a: 1 }),
    qualificationPayloadDigest({ a: 1, b: 2 }),
  );

  const symbolBearingPayload: Record<PropertyKey, unknown> = { a: 1 };
  Object.defineProperty(symbolBearingPayload, Symbol.for('hidden_qualification'), {
    value: 'attacker-controlled',
    enumerable: true,
  });
  assert.throws(
    () => canonicalizeQualification(symbolBearingPayload),
    /strict canonical JSON domain/,
    'qualification signatures must refuse members outside the canonical JSON domain',
  );
...
test('qualifies a complete graph containing every terminal outcome without mutating inputs', () => {
  ...
  assert.equal(
    first.payload_digests.protected_request_digest,
    fixture.context.expected_protected_request_digest,
  );
...
test('accepts both salted-set and index-bound Merkle hidden challenge commitments and rejects a forged proof', () => {
  ...
  merkle.bundle.agent_evaluation_evidence[0] = resign(
    merkle.bundle.agent_evaluation_evidence[0],
    'agent_evidence',
    (body) => {
...
test('derives Merkle proof direction and depth from challenge_index and rejects cross-index proof reuse', () => {
  ...
  swapped.bundle.agent_evaluation_evidence[0] = resign(
    swapped.bundle.agent_evaluation_evidence[0],
    'agent_evidence',
    (body) => {
      ...
      body.predicate.terminal_outcomes.sort((left: Obj, right: Obj) =>
        left.challenge_index - right.challenge_index);
      body.predicate.terminal_outcomes_root = terminalOutcomesRoot(body.predicate.terminal_outcomes);
    },
  );
  assert.equal(
    evaluateQualification(swapped.bundle, swapped.context).reason,
    'hidden_challenge_commitment_mismatch',
  );

  const reused = makeFixture('MERKLE_SHA256');
  reused.bundle.agent_evaluation_evidence[0] = resign(
    reused.bundle.agent_evaluation_evidence[0],
    'agent_evidence',
    (body) => {
      body.predicate.terminal_outcomes[1].challenge_commitment =
        body.predicate.terminal_outcomes[0].challenge_commitment;
      body.predicate.terminal_outcomes[1].challenge_proof = structuredClone(
        body.predicate.terminal_outcomes[0].challenge_proof,
      );
      body.predicate.terminal_outcomes_root = terminalOutcomesRoot(body.predicate.terminal_outcomes);
    },
  );
  assert.equal(
    evaluateQualification(reused.bundle, reused.context).reason,
    'duplicate_challenge_leaf_or_proof',
  );
});

test('all accepted schemas are closed at every signed profile layer', () => {
  const fixture = makeFixture();
  const cases: Array<[string, (value: any) => any, Obj]> = [
  ...
  const nested: Obj = structuredClone(fixture.bundle.candidate_manifest);
...
test('signature thresholds count distinct public keys and reject aliases for duplicate key material', () => {
  ...
  distinctCampaign.signatures.push({
    ...
    sig: crypto.sign(
      null,
      dsseSigningBytes(distinctCampaign.payloadType, distinctPayload),
      secondSigner.privateKey,
    ).toString('base64'),
  });
  ...
  aliasedCampaign.signatures.push({
    ...
    sig: crypto.sign(
      null,
      dsseSigningBytes(aliasedCampaign.payloadType, aliasedPayload),
      SIGNERS.campaign.privateKey,
    ).toString('base64'),
  });
...
test('requires an accepted DSSE runtime measurement bound to its authority and mechanism', () => {
  ...
  assert.equal(
    evaluateQualification(tampered.bundle, tampered.context).reason,
    'invalid_artifact_signature',
  );

  const unaccepted = makeFixture();
  const alternate = signer('key:runtime-measurement:verified-only');
  unaccepted.bundle.runtime_measurement = envelope(
    payload(unaccepted.bundle.runtime_measurement),
    alternate,
    RUNTIME_MEASUREMENT_PAYLOAD_TYPE,
  );
  ...
  authority.bundle.runtime_measurement = resign(
    authority.bundle.runtime_measurement,
    'runtime_measurement',
    (body) => { body.authority_id = 'authority:runtime-measurement:other'; },
  );
  assert.equal(
    evaluateQualification(authority.bundle, authority.context).reason,
    'runtime_measurement_authority_mismatch',
  );

  const mechanism = makeFixture();
  mechanism.bundle.runtime_measurement = resign(
    mechanism.bundle.runtime_measurement,
    'runtime_measurement',
    (body) => { body.measurement_mechanism_digest = d('substituted-mechanism'); },
  );
  assert.equal(
    evaluateQualification(mechanism.bundle, mechanism.context).reason,
    'runtime_measurement_mechanism_mismatch',
  );
});

test('fails closed for every static candidate substitution and weak model identity', () => {
  const substitutions: Array<[string, (runtime: Obj) => void]> = [
  ...
  for (const [name, mutate] of substitutions) {
    const fixture = makeFixture();
    fixture.bundle.runtime_measurement = resign(
      fixture.bundle.runtime_measurement,
      'runtime_measurement',
      mutate,
    );
  ...
  (weak.bundle.candidate_manifest.static.model as Obj).artifact_digest = null;
  weak.context.expected_candidate_manifest_digest = qualificationPayloadDigest(weak.bundle.candidate_manifest);
  weak.bundle.runtime_measurement = resign(
    weak.bundle.runtime_measurement,
    'runtime_measurement',
    (body) => {
...
test('refuses cross-assignment and protected-request reuse before qualification can travel', () => {
  ...
  assignment.bundle.runtime_measurement = resign(
    assignment.bundle.runtime_measurement,
    'runtime_measurement',
    (body) => { body.assignment_digest = d('other-assignment'); },
  );
  ...
  request.bundle.runtime_measurement = resign(
    request.bundle.runtime_measurement,
    'runtime_measurement',
    (body) => { body.protected_request_digest = d('other-request'); },
  );
...
test('binds each decision to the exact protected request and rejects replay across valid requests', () => {
  ...
  assert.equal(
    firstDecision.payload_digests.protected_request_digest,
    firstRequest.context.expected_protected_request_digest,
  );
  ...
  secondRequest.bundle.runtime_measurement = resign(
    secondRequest.bundle.runtime_measurement,
    'runtime_measurement',
    (body) => { body.protected_request_digest = secondRequestDigest; },
  );
  ...
  assert.notEqual(
    firstDecision.payload_digests.protected_request_digest,
    secondDecision.payload_digests.protected_request_digest,
  );
  ...
  assert.equal(
    replayed.payload_digests.protected_request_digest,
    firstRequest.context.expected_protected_request_digest,
  );
});
...
function replaceStatus(fixture: ReturnType<typeof makeFixture>, mutate: (status: Obj) => void): void {
  fixture.bundle.qualification_status_chain[0] = resign(
    fixture.bundle.qualification_status_chain[0], 'qualification_status', mutate,
  );
...
test('enforces cardinality and payload-size controls', () => {
  ...
  oversizedManifest.static.dependency_digests = Array.from(
    { length: 1025 }, (_, index) => d(`dependency-${String(index).padStart(4, '0')}`),
  ).sort();
