...
// Generated from provenance-v2.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
//
...
import { PROVENANCE_VERSION, PROVENANCE_V2_VERSION, PROVENANCE_V2_REQUIRED_ALGORITHMS, delegationProofV2Bytes, verifyDelegationProofSetV2, verifyProvenanceOffline, verifyProvenanceOfflineV2, } from './provenance.js';
...
async function signLink(link = LINK) {
...
test('LEG STRIPPING: removing the ML-DSA leg refuses', async () => {
    const link = await signLink();
    link.proof_set.signatures = link.proof_set.signatures.filter((s) => s.alg === 'Ed25519');
...
test('LEG STRIPPING: removing the Ed25519 leg refuses too', async () => {
    const link = await signLink();
    link.proof_set.signatures = link.proof_set.signatures.filter((s) => s.alg === 'ML-DSA-65');
    assert.equal(await verifyDelegationProofSetV2(link, PIN), false);
});
...
test('WRONG-LENGTH SIGNATURE: a truncated leg refuses', async () => {
    const link = await signLink();
    const edSig = link.proof_set.signatures.find((s) => s.alg === 'Ed25519');
...
test('the v1 chain verifier refuses a v2 document on the version marker, without crashing', () => {
    ...
    assert.ok(res.errors.some((e) => e.includes(PROVENANCE_V2_VERSION)));
});
test('the v2 chain verifier refuses a v1 document on the version marker, without crashing', async () => {
    ...
    assert.equal(res.valid, false);
    assert.ok(res.errors.some((e) => e.includes(PROVENANCE_VERSION)));
});
test('the v2 chain verifier refuses malformed input without throwing', async () => {
    for (const junk of [null, undefined, 'x', 42, []]) {
        const res = await verifyProvenanceOfflineV2(junk);
        assert.equal(res.valid, false);
    }
});
