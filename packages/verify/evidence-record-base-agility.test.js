...
// Generated from evidence-record-base-agility.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
//
...
function signedBytes({ hashed, time }) {
    return Buffer.from(canonicalize({ '@version': TIME_ATTESTATION_VERSION, hashed, time, ts_authority_id: TSA }), 'utf8');
}
function ed25519Attestation({ hashed, time = TIME }) {
    return {
...
async function mldsaAttestation({ hashed, time = TIME }) {
...
async function setShapedAttestation({ hashed, time = TIME, algs }) {
...
test('the ML-DSA leg fails closed on a wrong key, a tampered time, and a missing backend', async () => {
    ...
    tampered.archive_timestamps[0].time_attestation.time = '2030-01-01T00:00:00.000Z';
...
test('one broken leg inside a valid-looking set refuses the whole attestation', async () => {
    const rec = record([await setShapedAttestation({ hashed: PROTECTED_HASH, algs: ['Ed25519', 'ML-DSA-65'] })]);
    const sigs = rec.archive_timestamps[0].time_attestation.proof.signatures;
    const pqLeg = sigs.find((s) => s.alg === 'ML-DSA-65');
