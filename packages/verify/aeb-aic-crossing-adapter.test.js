...
import { ML_DSA_65_SECRET_KEY_BYTES, ML_DSA_65_SIGNATURE_BYTES, } from './dist/pq-signature-agility.js';
...
test('pure-JSON RFC 7638 jkt and X.509 SPKI remain distinct native mappings', () => {
    const jwt = mapAicJwtJktBoundCrossingAuthority(boundJwtInput(), boundContext());
    const x509 = mapAicX509SpkiBoundCrossingAuthority(boundX509Input(), boundContext(x509Policy()));
    assert.equal(jwt.ok, true, JSON.stringify(jwt));
    assert.equal(x509.ok, true, JSON.stringify(x509));
    if (!jwt.ok || !x509.ok)
        return;
    assert.equal(jwt.authority.mapping_profile_id, AIC_JWT_JKT_BOUND_CROSSING_MAPPING_PROFILE);
    assert.equal(x509.authority.mapping_profile_id, AIC_X509_SPKI_BOUND_CROSSING_MAPPING_PROFILE);
...
test('native results cannot self-pin relying-party trust or verifier policy', () => {
    ...
    const emptyPolicy = mapAicJwtJktBoundCrossingAuthority(boundJwtInput(), boundContext({
        ...jwtPolicy(),
...
test('public AIC surfaces expose no unbound authority mapping', async () => {
    ...
    assert.equal(typeof subpathSurface.issueAicBoundCrossingRecord, 'function');
});
test('AIC bound issuance refuses an action or boundary mismatch before signing', async () => {
    const issueAicBound = aicAdapterSurface
        .issueAicBoundCrossingRecord;
    assert.equal(typeof issueAicBound, 'function');
    if (typeof issueAicBound !== 'function')
        return;
    const actionMismatch = await issueAicBound(boundJwtInput(), boundContext(), {
        ...boundIssueDraft(),
    ...
    const boundaryMismatch = await issueAicBound(boundJwtInput(), boundContext(), {
        ...boundIssueDraft(),
        boundary: {
            ...ADMISSION_DOMAIN,
...
test('AIC issuance snapshots trusted state and signing options before hostile input', async () => {
    ...
    Object.defineProperty(accessorInput, 'native_verification', {
        ...
        get() {
            ...
            accessorInput.request_binding.projected_admission_domain_digest =
                digestAebTyped(mismatchContext.admission_domain, 'EP-AIC-ADMISSION-DOMAIN-v1');
            mismatchOptions.signing_keys.length = 0;
            return 'VERIFIED';
        },
    });
    const mismatch = await issueAicBoundCrossingRecord(accessorInput, mismatchContext, mismatchDraft, mismatchOptions);
    ...
    const originalBackend = {
        sign(_message, privateKey, options) {
            backendCalls += 1;
    ...
    const issued = await issueAicBoundCrossingRecord(proxiedInput, context, draft, signingOptions);
...
test('native failure short-circuits carrier parsing after structural validation', () => {
    ...
    }, boundContext()), { ok: false, reason: 'aic_native_verification_failed' });
    assert.deepEqual(mapAicX509SpkiBoundCrossingAuthority({
        ...boundX509Input(),
    ...
    }, boundContext(x509Policy())), { ok: false, reason: 'aic_native_verification_indeterminate' });
    assert.deepEqual(projectAicJwtToStrictJwtSvid({
        source: {
            ...jwtInput(),
            native_verification: 'FAILED',
        ...
        projected_algorithm: 'ES256',
        projected_key_id: 'jwt-svid-key-2026-08',
    }, projectionContext()), { ok: false, reason: 'aic_native_verification_failed' });
});
test('JWT-SVID projection digest binds the accepted source evaluation', () => {
    ...
    const original = projectAicJwtToStrictJwtSvid(projectionInput, projectionContext());
    ...
    const changedPolicy = projectAicJwtToStrictJwtSvid(projectionInput, {
        ...projectionContext(),
        source_verification_policy: {
    ...
    assert.notEqual(original.projection.projection_digest, changedEvidence.projection.projection_digest);
    assert.notEqual(original.projection.projection_digest, changedStatusHead.projection.projection_digest);
    assert.notEqual(original.projection.projection_digest, changedPolicy.projection.projection_digest);
});
