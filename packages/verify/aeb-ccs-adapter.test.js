...
const L1_VECTOR = JSON.parse(readFileSync(new URL('../../interop/ccs-aeb/fixtures/ccs-verifier-pypi-1.1.14-reference-signed-001.json', import.meta.url), 'utf8'));
function spki(key) {
...
function l1Profile(actionType = ACTION_TYPE) {
    const pin = {
...
test('CCS 1.1.14 signature, issuer pin, expiry, exact arguments, action, and tool fail independently', () => {
    ...
    const fractionalButTampered = structuredClone(f.input.artifact);
