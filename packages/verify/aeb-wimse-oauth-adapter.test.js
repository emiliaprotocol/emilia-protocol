...
function makeFixture(options = {}) {
    ...
    for (const key of options.principalBindingDelete ?? [])
        delete principalBinding[key];
...
function makeV2Fixture(options = {}) {
...
test('v2 mapper re-verifies principal acceptance instead of trusting a forged native result', () => {
    ...
    const forged = { ...rejected, acceptance: 'ACCEPTED', reasons: [] };
    const mapped = fixture.adapter.mapAction({
        ...fixture.input,
        profile: fixture.profile,
        native: forged,
...
test('v2 checked-in vector covers every principal-confusion class', () => {
    const vectorPath = new URL('../../conformance/vectors/wimse-oauth-principal-aeb.v2.json', import.meta.url);
    const vector = JSON.parse(fs.readFileSync(vectorPath, 'utf8'));
    const ids = new Set(vector.vectors.map((entry) => entry.id));
    for (const id of [
