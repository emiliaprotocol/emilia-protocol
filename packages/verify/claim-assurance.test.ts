...
function profile(minimumDistinctSources = 1): ClaimAssuranceProfile {
  return {
...
function artifact(source_id = 'bank:one', relationship = 'SUPPORTS'): Obj {
  return { source_id, relationship, observed_value: 'acct-ending-1234' };
}

function claimCase(pinnedProfile: ClaimAssuranceProfile, artifacts: Obj[] = [artifact()]): ClaimCase {
  return {
...
function registration(
  transform: (input: Obj) => Partial<EvidenceVerifierResult> = () => ({}),
  ...
  return {
    ...
    implementation_digest: IMPLEMENTATION,
    verify(input) {
      const body = input.artifact as Obj;
      return {
        verdict: 'VERIFIED',
        ...
        reasons: [],
        ...transform(input as unknown as Obj),
      } as EvidenceVerifierResult;
    },
  };
}
...
function evaluate(
  ...
  verifiers: EvidenceVerifierRegistration[] = [registration()],
) {
...
test('emits a deterministic offline VERIFIED record without granting authority', () => {
  ...
  const semanticallyTampered = structuredClone(first) as Obj;
  ...
  const digestTampered = structuredClone(first) as Obj;
...
test('record integrity inspection is strict and explicitly does not re-perform the case', () => {
  ...
  const madeUpVerdict = structuredClone(record) as Obj;
  ...
  const invalidTime = structuredClone(record) as Obj;
  ...
  const invalidReplay = structuredClone(record) as Obj;
...
test('missing, stale, and unavailable required evidence remain distinguishable and INDETERMINATE', () => {
  ...
  const stale = evaluate(
    claimCase(p),
    p,
    [registration(() => ({ expires_at: '2026-08-23T11:59:59Z' }))],
  );
  assert.equal(stale.verdict, 'INDETERMINATE');
  assert.deepEqual(stale.evidence_results[0]?.reasons, ['EVIDENCE_STALE']);

  const expiredAfterSnapshot = evaluate(
    claimCase(p),
    p,
    [registration(() => ({ expires_at: '2026-08-23T12:00:00Z' }))],
  );
  ...
  const expiresExactlyAtEvaluation = evaluate(
    claimCase(p),
    p,
    [registration(() => ({ expires_at: EVALUATED_AT }))],
  );
  ...
  const zeroWidthValidity = evaluate(
    claimCase(p),
    p,
    [registration(() => ({
      observed_at: '2026-08-23T11:59:00Z',
...
test('artifact digest tampering is rejected before trusted verifier execution', () => {
  const p = profile();
  const c = claimCase(p);
  c.evidence[0]!.artifact = { ...c.evidence[0]!.artifact as Obj, observed_value: 'attacker-value' };
  let called = false;
...
test('subject, scope, claim, and exact-action bindings fail closed', () => {
  ...
  for (const [member, replacement, reason] of [
    ...
    const c = claimCase(p);
    (c.evidence[0]!.binding as Obj)[member] = replacement;
    const record = evaluate(c, p);
    assert.equal(record.verdict, 'UNVERIFIED');
  ...
  assert.throws(
    () => evaluateClaimAssurance(wrongExpectedAction, {
      pinned_profile: p,
      pinned_profile_hash: claimAssuranceProfileHash(p),
      verifier_registry: [registration()],
      evaluated_at: EVALUATED_AT,
...
test('verifier output must independently restate the bound claim context', () => {
  ...
  const record = evaluate(
    claimCase(p),
    p,
    [registration(() => ({ subject_digest: `sha256:${'ff'.repeat(32)}` }))],
  );
  assert.equal(record.verdict, 'UNVERIFIED');
...
test('strict versions, keys, timestamps, duplicates, and resource limits are enforced', () => {
  const p = profile();
  const c = claimCase(p);
  (c as Obj).unexpected = true;
  assert.throws(() => evaluate(c, p), /unknown member.*unexpected/);

  const wrongVersion = claimCase(p);
  wrongVersion['@type'] = 'EP-CLAIM-CASE-v0' as typeof CLAIM_CASE_VERSION;
  ...
  (oversized.evidence[0]!.artifact as Obj).blob = 'x'.repeat(270_000);
  oversized.evidence[0]!.artifact_digest = claimAssuranceArtifactDigest(
    oversized.evidence[0]!.artifact,
    { maxStringBytes: 300_000 },
  );
...
test('profile quorum counts distinct accepted source identities', () => {
  ...
  const repeatedSource = evaluate(
    claimCase(p, [artifact('bank:one'), repeatedObservation]),
    p,
  );
  ...
  const distinct = evaluate(
    claimCase(p, [artifact('bank:one'), artifact('bank:two')]),
    p,
  );
...
test('accepted support and contradiction across separate requirements is globally DIVERGED', () => {
  ...
  assert.deepEqual(record.reasons, ['ACCEPTED_SOURCES_DIVERGED']);
  assert.deepEqual(
    new Set(record.requirement_results.map((result) => result.disposition)),
    new Set(['SATISFIED', 'UNVERIFIED']),
  );
});
...
test('deduplicates and caps verifier reasons after preserving the framework reason', () => {
  ...
  const record = evaluate(
    claimCase(p),
    p,
    [registration(() => ({ verdict: 'UNVERIFIED', reasons: verifierReasons }))],
  );
...
test('snapshots evaluated_at before a verifier can mutate caller-owned options', () => {
  ...
  let calls = 0;
  let hostileOptions: ClaimAssuranceOptions;
