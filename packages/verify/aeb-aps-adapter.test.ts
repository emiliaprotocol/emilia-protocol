...
import {
  APS_AEB_ADAPTER_ID,
  APS_AEB_ADAPTER_VERSION,
  APS_AEB_CONFIG_VERSION,
  APS_CAID_MAPPER_ID,
  APS_CAID_MAPPING_VERSION,
  APS_DRAFT_REVISION,
  APS_TRUST_ROOT_VERSION,
  computeApsActionRef,
  computeApsDecisionRef,
  computeApsPayloadRef,
  computeApsReceiptId,
  createApsActionDefinition,
  createApsAebAdapter,
  type ApsAdapterConfig,
  type ApsAuthorityVerifier,
  type ApsTrustRoot,
} from './aeb-aps-adapter.js';
...
function signReceipt(receipt: Obj, signer: string, keyId: string, privateKey: KeyObject): Obj {
...
function makeFixture(): Obj {
  ...
  const authorityVerifier: ApsAuthorityVerifier = {
    ...authorityVerifierDescriptor,
    verify(input) {
      const state = input.authority_state as Obj;
...
test('APS adapter refuses mutation of scope, nonce, issuance time, agent, or profile', () => {
  for (const mutate of [
    (value: Obj) => { value.scope_required = ['commerce:read']; },
    (value: Obj) => { value.nonce = 'f'.repeat(32); },
    (value: Obj) => { value.issued_at = '2026-08-06T12:00:00.001Z'; },
    (value: Obj) => { value.agent_id = 'did:key:z6MkOther'; },
    (value: Obj) => { value.profile = 'aps-action-ref-v1'; },
  ]) {
    const fixture = makeFixture();
    const adapter = createApsAebAdapter({
      config: fixture.config,
...
test('APS adapter refuses a forged decision and a failed authority path', () => {
  ...
  const forged = structuredClone(input(fixture).artifact) as Obj;
  ...
  const refusingVerifier: ApsAuthorityVerifier = {
