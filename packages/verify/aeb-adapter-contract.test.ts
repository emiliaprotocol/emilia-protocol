...
function legacyMalformedUnicodeCanonicalize(value) {
  ...
  return `{${Object.keys(value).sort().map((key) => (
    `${JSON.stringify(key)}:${legacyMalformedUnicodeCanonicalize(value[key])}`
  )).join(',')}}`;
}
...
test('AEB evaluation v2 preserves multi-leg provenance without creating an authorization verdict', () => {
  ...
  assert.deepEqual(
    record.legs.map((item) => item.replay_unit),
    source.legs.map((item) => item.replay_unit),
  );
  assert.deepEqual(
    record.legs.map((item) => item.evidence_digest),
    source.legs.map((item) => item.evidence_digest),
  );
  assert.ok(
    record.legs.every(
      (item) => item.semantic_loss.status === 'NO_MATERIAL_FIELD_LOSS',
    ),
  );
  assert.deepEqual(
    record.legs.find((item) => item.artifact_ref === 'artifact:human-alice')
      ?.native_decision_reference,
    nativeDecisionReferences['artifact:human-alice'],
  );
...
test('AEB evaluation v1 upgrade reports unavailable semantic-loss declarations as indeterminate', () => {
  ...
  assert.ok(
    upgraded.body.legs.every(
      (item) => item.semantic_loss.status === 'INDETERMINATE',
    ),
  );
  assert.ok(
    upgraded.reasons.every((reason) =>
      reason.startsWith('mapping_profile_unavailable:'),
    ),
  );
});
...
test('AEB evaluation v2 refuses provenance, replay-unit, loss-report, and native-decision substitution', () => {
  ...
  for (const mutate of [
    ...
    (candidate) => { (candidate.signature as any).unsigned_note = 'not-covered'; },
  ]) {
...
test('AEB evaluation v2 never infers a native decision reference from an accepted leg', () => {
  ...
  assert.ok(
    record.legs.every((item) => item.native_decision_reference === undefined),
  );
  assert.equal(record.conversion.status, 'COMPLETE');
});
...
test('AEB evaluation v2 bounds conversion reasons for URI-shaped artifact references', () => {
  ...
  assert.ok(record.conversion.reason_codes.every((reason) =>
    /^[A-Za-z0-9_.:-]{1,256}$/.test(reason)));
  const verified = verifyAebEvaluationV2(record, {
    source_evaluation: source,
    evaluator_keys: fixture.config.evaluator_keys,
  });
...
test('AEB evaluation v2 refuses a different private key under the source key id', () => {
  ...
  assert.throws(
    () => issueAebEvaluationV2FromV1(source, {
      profiles: fixture.config.profiles,
...
test('AEB evaluation v2 reports unmatched native decision references', () => {
  ...
  assert.equal(record.conversion.status, 'INDETERMINATE');
  assert.ok(record.conversion.reason_codes.some((reason) =>
    reason.startsWith('native_decision_reference_unmatched:')));
});
...
test('AEB verification fails closed on malformed Unicode in signed-record values and keys', () => {
  ...
  for (const [index, record] of malformedRecords.entries()) {
    ...
    record.signature.value = crypto.sign(
      null,
      Buffer.from(`${AEB_EVALUATION_DOMAIN}${legacyMalformedUnicodeCanonicalize(unsigned)}`, 'utf8'),
