...
test('an online any-of profile refuses weakest-adapter selection when offline authority status is unknown', () => {
  ...
  s.config.registry.entries['role:offline-mandate'] = registryEntry(
    'role:offline-mandate',
    'evidence-role',
    '1',
    { role: 'offline-mandate', subject_kinds: ['human'] },
  );
  s.config.registry.registry_digest = unifiedRegistryDigest(s.config.registry);
  ...
  const offline = leg(
    'offline-mandate',
    CAID,
    'artifact:offline-mandate',
    { id: 'human:offline-approver', kind: 'human' },
  );
