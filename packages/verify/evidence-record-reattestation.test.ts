...
async function makeChain(): Promise<ReattestationEntry[]> {
...
test('a chain crossing an algorithm transition verifies, reporting per-link algorithms', async () => {
  ...
  assert.deepEqual(
    [...r.links].sort((a, b) => a.index - b.index).map((l) => l.alg),
    ['Ed25519', 'ML-DSA-65', 'Ed25519'],
  );
...
test('an unknown signature algorithm in a link refuses with unknown_algorithm', async () => {
  ...
  const r = await verifyReattestationChain(
    RECORD_BYTES, [chain[0]], { 'era-ed-1': { alg: 'Ed448', public_key: edPubB64u } },
  );
  assert.equal(r.verified, false);
...
test('malformed inputs refuse with reasons, never throw', async () => {
  const chain = await makeChain();
  assert.equal((await verifyReattestationChain(null as never, chain, KEYS)).verified, false);
...
function tsaAttestation(hashed: string, time: string, authorityId: string, key: crypto.KeyPairKeyObjectResult) {
