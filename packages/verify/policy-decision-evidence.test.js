...
// Generated from policy-decision-evidence.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
...
import { AEB_ADAPTER_VERSION, AEB_REGISTRY_VERSION, AEB_REQUIREMENT_VERSION, adapterPinDigest, digestAeb, evaluateAebEvidence, mappingProfileDigest, registryEntryDigest, unifiedRegistryDigest, } from './aeb-adapter-contract.js';
import { POLICY_DECISION_EVIDENCE_ADAPTER_ID, POLICY_DECISION_EVIDENCE_ADAPTER_VERSION, POLICY_DECISION_EVIDENCE_CONFIG_VERSION, POLICY_DECISION_EVIDENCE_MAPPING_VERSION, POLICY_DECISION_EVIDENCE_MAPPER_ID, POLICY_DECISION_EVIDENCE_TRUST_ROOT_VERSION, createPolicyDecisionEvidenceActionDefinition, createPolicyDecisionEvidenceAdapter, projectCerbosPolicyDecision, projectOpaPolicyDecision, signPolicyDecisionEvidence, } from './policy-decision-evidence.js';
...
test('OPA ALLOW becomes accepted machine-policy evidence bound to the exact action', () => {
    ...
    const computed = computeCaid(f.action, {
        suite: 'jcs-sha256',
        definitions: profile().definition.definitions,
    });
...
test('native deny and unknown outcomes never become accepted evidence', () => {
    const f = fixture();
    for (const [result, acceptance] of [[false, 'REJECTED'], [{ unexpected: true }, 'INDETERMINATE']]) {
        const claims = projectOpaPolicyDecision({
            ...f.projection,
...
test('a machine-policy ALLOW cannot satisfy a requirement that also requires human authorization', () => {
    ...
    const computed = computeCaid(f.action, {
        suite: 'jcs-sha256',
        definitions: mappingProfile.definition.definitions,
    });
    ...
    const entries = {
        'mapping:policy-decision-payment-release': entry('mapping:policy-decision-payment-release', 'mapping-profile', { profile_digest: mappingProfile.profile_digest }),
        'role:machine-policy-decision': entry('role:machine-policy-decision', 'evidence-role', { role: 'machine-policy-decision', subject_kinds: ['system'] }),
