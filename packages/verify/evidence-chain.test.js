...
// Generated from evidence-chain.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
...
function hydrate(chain) {
    ...
    if ('action_digest' in c)
        c.action_digest = subst(c.action_digest);
    for (const comp of c.components || []) {
        if (comp.evidence && 'action_digest' in comp.evidence)
            comp.evidence.action_digest = subst(comp.evidence.action_digest);
    }
    return c;
}
for (const v of suite.vectors) {
    test(`[aec] ${v.name}`, () => {
        ...
        if (v.expect_requirement_source)
            assert.equal(r.requirement_source, v.expect_requirement_source);
    });
}
