...
import {
  AGENT_CONTINUITY_VERSION,
  AGENT_CONTINUITY_V2_VERSION,
  createAgentContinuityEnvelope,
  createAgentContinuityEnvelopeV2,
  verifyAgentContinuityEnvelope,
  verifyAgentContinuityEnvelopeV2,
  verifyAgentContinuityEnvelopeAny,
} from './agent-edge-continuity.js';
...
test('LEG STRIPPING: removing the ML-DSA leg refuses structurally', async () => {
  const envelope: any = structuredClone(await buildV2());
...
test('SET NARROWING: narrowing required_algorithms to Ed25519-only refuses structurally and cryptographically', async () => {
  ...
  const raw: any = await buildV2();
  const { proof, ...unsigned } = raw;
  const narrowedBytes = Buffer.from(
    `${AGENT_CONTINUITY_V2_VERSION}\0${canonicalizeAeb({ ...unsigned, required_algorithms: ['Ed25519'] })}`,
    'utf8',
  );
...
test('WRONG-LENGTH SIGNATURE: a truncated Ed25519 leg refuses', async () => {
  const envelope: any = structuredClone(await buildV2());
  const leg = envelope.proof.signatures.find((s: any) => s.alg === 'Ed25519');
