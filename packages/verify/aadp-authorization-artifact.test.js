...
import { AADP_ACTION_MAPPING_CONFIG_VERSION, AADP_AUTHORIZATION_ARTIFACT_VERSION, AADP_EP_AUTHORIZATION_ARTIFACT_PROFILE, AADP_NATIVE_VERIFIER_DESCRIPTOR_VERSION, deriveAadpEpAuthorizationArtifact, matchAadpAuthorizationArtifact, parseAadpAuthorizationArtifact, verifyAadpEpAuthorizationArtifact, } from './dist/aadp-authorization-artifact.js';
...
test('AADP hook is closed, profile-neutral, and safely normalized', () => {
    ...
    assert.equal(parseAadpAuthorizationArtifact({ ...hook, artifact_profile: 'example-native-authorization-v1' })
...
test('closed resolver retains every field and rejects ambiguous or dangerous mapped paths', () => {
    ...
    const dangerous = derive({
        mapping: {
            ...mappingConfiguration,
            material_field_map: mappingConfiguration.material_field_map.map((entry) => (entry.source_param === 'payee'
...
test('hostile accessors and proxies produce verdicts without executing getters', () => {
    ...
    assert.doesNotThrow(() => deriveAadpEpAuthorizationArtifact(proxy));
    assert.equal(deriveAadpEpAuthorizationArtifact(proxy).verdict, 'REFUSE');
    const derived = derive();
    assert.ok(derived.artifact);
