...
const hashPair = (a: string, b: string) => { const s = [a, b].sort(); return sha256hex(s[0] + s[1]); };
...
function signB(digestHex: string): string {
...
function signA(digestHex: string, { rpId = 'www.emiliaprotocol.ai', flags = 0x05 } = {}): any {
...
function buildReceipt(mutate: any = {}): any {
  const action = {
    ep_version: '1.0',
...
test('step 3 — an unparseable compromised_at fails closed', () => {
  ...
  assert.equal(r.valid, false);
  assert.ok(r.errors.some((e: string) => /unparseable compromised_at/.test(e)));
});
...
test('opts.now itself fails closed when unparseable', () => {
  ...
  assert.equal(r.valid, false);
  assert.ok(r.errors.some((e: string) => /opts\.now is not a parseable instant/.test(e)));
});
