...
import {
  ...
  A2A_RECEIPT_BINDING_V2_REQUIRED_ALGORITHMS,
  createA2AReceiptPresentation,
  createA2AReceiptPresentationV2,
  verifyA2AReceiptPresentation,
  verifyA2AReceiptPresentationV2,
  verifyA2AReceiptPresentationStatement,
...
const INITIATING_MESSAGE = {
  messageId: 'm1', role: 'ROLE_USER' as const,
  parts: [{ data: ACTION, mediaType: A2A_ACTION_MEDIA_TYPE }],
};
const PROOF_MESSAGE = { messageId: 'm2', role: 'ROLE_USER' as const, taskId: 'task1', contextId: 'ctx1', parts: [{ text: 'proof' }] };
...
function verifyInput(presentation: any) {
  return {
    protocol_version: '1.0',
    target_interface_url: 'https://agent.example/a2a',
    agent_card: AGENT_CARD,
...
test('the v1 verifier refuses a v2 presentation cleanly (shape mismatch), without crashing', async () => {
  const presentation = await buildV2Presentation();
  const res = verifyA2AReceiptPresentation(verifyInput(presentation) as any);
...
test('verifyA2AReceiptPresentationStatement routes v2 to the hybrid verifier', async () => {
  const presentation = await buildV2Presentation();
  const res = await verifyA2AReceiptPresentationStatement(verifyInput(presentation) as any);
  assert.equal(res.valid, true);
});

function withTamperedArtifact(presentation: any, mutate: (artifact: any) => void) {
...
test('LEG STRIPPING: removing the ML-DSA leg refuses', async () => {
  ...
  const tampered = withTamperedArtifact(presentation, (artifact) => {
    artifact.signatures = artifact.signatures.filter((s: any) => s.alg === 'Ed25519');
  });
...
test('WRONG-LENGTH SIGNATURE: a truncated leg refuses', async () => {
  const presentation = await buildV2Presentation();
  const tampered = withTamperedArtifact(presentation, (artifact) => {
    const edSig = artifact.signatures.find((s: any) => s.alg === 'Ed25519');
    artifact.signatures = artifact.signatures.map((s: any) => (s === edSig ? { ...s, sig: s.sig.slice(0, -4) } : s));
  });
  const res = await verifyA2AReceiptPresentationV2(verifyInput(tampered));
  assert.equal(res.valid, false);
  assert.equal(res.checks.signature, false);
...
test('NO ML-DSA BACKEND: not directly exposed on this entry point but a missing PQ leg refuses', async () => {
  ...
  const presentation = await buildV2Presentation();
  const tampered = withTamperedArtifact(presentation, (artifact) => {
    artifact.signatures = artifact.signatures.filter((s: any) => s.alg === 'ML-DSA-65');
  });
  const res = await verifyA2AReceiptPresentationV2(verifyInput(tampered));
  assert.equal(res.valid, false);
});
