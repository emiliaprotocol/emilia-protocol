...
// Generated from a2a-receipt-binding.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
import assert from 'node:assert/strict';
import crypto from 'node:crypto';
import test from 'node:test';
import { A2A_ACTION_MEDIA_TYPE, A2A_PROTOCOL_VERSION, A2A_RECEIPT_BINDING_VERSION, A2A_RECEIPT_EXTENSION_NAME, A2A_RECEIPT_EXTENSION_URI, A2A_RECEIPT_PRESENTATION_VERSION, createA2AReceiptPresentation, verifyA2AReceiptPresentation, } from './a2a-receipt-binding.js';
...
function makeReceipt(issuer) {
    ...
    return {
        ...
        signature: {
            algorithm: 'Ed25519',
            value: crypto.sign(null, Buffer.from(canonicalizeStrictJson(payload), 'utf8'), issuer.privateKey).toString('base64url'),
        },
    };
}
function fixture() {
...
test('refuses unsigned A2A objects, a missing receipt verifier, and forged binders', () => {
    ...
    delete noVerifier.verify_receipt;
    const noVerifierResult = verifyA2AReceiptPresentation(noVerifier);
...
test('fails closed on validity boundaries and hostile non-JSON inputs', () => {
    ...
    let getterRan = false;
    const hostileMessage = { ...f.proofMessage };
