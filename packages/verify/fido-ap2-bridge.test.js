...
function rawAp2(options = {}) {
    ...
    const checkoutMandateToken = canonicalSdJwt(`checkout-mandate:${seed}`, checkoutHashAlgorithm === 'sha-256'
        ? checkout
...
function expectedProjection(raw) {
    const sourceExpiry = new Date(Math.min(raw.checkout_mandate.exp, raw.payment_mandate.exp) * 1_000).toISOString();
    return {
        ...
        checkout_payload_jwt_digest: sourceBytesDigest('checkout-payload-jwt', raw.checkout_mandate.checkout_jwt),
        ...
        payment_instrument_description_digest: digestAeb(raw.payment_mandate.payment_instrument.description ?? null),
...
function evaluate(f, expectedAction = f.expectedAction, profile = createFidoAp2PinnedProfile()) {
...
test('projection implements the current AP2 v0.2 closed CheckoutMandate and PaymentMandate', () => {
    ...
    assert.deepEqual(projectFidoAp2PaymentAction(projectionInput(withTypedOptionals)), expectedProjection(withTypedOptionals));
    const f = fixture();
    ...
    assert.deepEqual(['action_type', ...FIDO_AP2_CAID_ACTION_DEFINITIONS[0].required_fields.map((field) => field.name)], Object.keys(f.expectedAction));
...
test('projection binds verified AP2 token payloads and honors the authenticated hash algorithm', () => {
    ...
    assert.throws(() => projectFidoAp2PaymentAction(spliced), /verified AP2 payment payload binding mismatch/);
    for (const checkoutHashAlgorithm of ['sha-384', 'sha-512']) {
        const fixture = rawAp2({ checkoutHashAlgorithm });
        assert.deepEqual(projectFidoAp2PaymentAction(projectionInput(fixture)), expectedProjection(fixture));
    }
});
test('native source binding commits deterministic canonical AP2 SD-JWT token strings', () => {
    ...
    assert.notEqual(same.checkout_mandate_token_digest, alternate.source_binding.checkout_mandate_token_digest);
    ...
    for (const [checkoutPayload, paymentPayload] of [
        ...
        assert.throws(() => createFidoAp2NativeSourceBinding({
            checkout_mandate_token: raw.checkout_mandate_token,
            payment_mandate_token: raw.payment_mandate_token,
...
test('projection explicitly rejects the legacy CartMandate model', () => {
    ...
    assert.throws(() => projectFidoAp2PaymentAction(legacy), /unsupported or inconsistent closed AP2 payment mandate/);
});
test('projection rejects open or recurring execution and transaction mismatch', () => {
    const attacks = [
        ...
        ({ payment_mandate: payment }) => { payment.recurrence = { frequency: 'monthly' }; },
        ({ payment_mandate: payment }) => { payment.transaction_id = 'checkout:attacker'; },
    ];
    for (const attack of attacks) {
        const raw = rawAp2();
        attack(raw);
        assert.throws(() => projectFidoAp2PaymentAction(projectionInput(raw)), /unsupported or inconsistent closed AP2 payment mandate/);
    }
});
...
test('mutation of every material action field and source digest is refused', () => {
    ...
    for (const field of actionFields) {
        ...
        if (field.endsWith('_digest'))
            f.artifact.normalized_action[field] = digestAeb({ changed: field });
        else if (field === 'amount_minor')
            f.artifact.normalized_action[field] += 1;
        else if (field === 'source_expires_at')
            f.artifact.normalized_action[field] = '2026-07-31T18:03:59.000Z';
        else if (field === 'execution')
            f.artifact.normalized_action[field] = 'scheduled';
        else if (field === 'currency')
            f.artifact.normalized_action[field] = 'EUR';
        else
            f.artifact.normalized_action[field] = `${field}:attacker`;
        assertRefused(f);
    }
    for (const field of [
...
test('post-signing disclosure, scope, provider, and effect tampering is refused', () => {
    for (const [path, value] of [
        ...
        if (path.startsWith('disclosure.'))
            f.artifact.disclosure[path.slice(11)] = value;
        else
            f.artifact[path] = value;
        assertRefused(f);
    }
...
test('every signed WebAuthn context binding is relying-party checked', () => {
    ...
    for (const contextOverrides of wrongBindings)
        assertRefused(fixture({ contextOverrides }));
});
...
test('commitment-only artifact rejects raw AP2, unknown keys, and poisoned values', () => {
    ...
    withRaw.artifact.checkout_mandate = withRaw.checkout_mandate;
    assertRefused(withRaw);
    const unknown = fixture();
    unknown.artifact.source_commitments.unknown = digestAeb({ extension: true });
    ...
    nonJson.artifact.extra = undefined;
...
function twoLegAeb(f, expectedAction = f.expectedAction, caid = f.context.caid) {
    ...
    const entries = {
        [f.profile.registry_entry_ref]: registryEntry(f.profile.registry_entry_ref, 'mapping-profile', f.profile.version, { profile_digest: f.profile.profile_digest }),
...
test('AEB composes separate AP2-native and WebAuthn-human legs for one exact action', () => {
    const f = fixture();
    assert.equal(f.artifact.source_commitments.native_verification_attestation_digest, digestAeb(f.nativeAttestation));
