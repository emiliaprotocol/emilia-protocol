...
import {
  ...
  AUTHORIZATION_BUNDLE_V2_VERSION,
  verifyAuthorizationBundle,
  verifyAuthorizationBundleV2,
  verifyAuthorizationBundleAny,
  signAuthorizationBundleSignoffV2,
  signoffV2SigningBytes,
...
async function buildV2({ context = baseContext(), signers } : { context?: any; signers?: any[] } = {}) {
...
function verifyOpts(context: any, overrides: Record<string, any> = {}) {
  return {
    now: NOW,
    audience: AUDIENCE,
...
test('LEG STRIPPING: removing the ML-DSA leg refuses the signoff (never SATISFIED)', async () => {
  const { bundle, context } = await buildV2();
  const stripped: any = structuredClone(bundle);
  stripped.signoffs[0].proof.signatures = stripped.signoffs[0].proof.signatures.filter((s: any) => s.alg === 'Ed25519');
...
test('LEG STRIPPING: removing the Ed25519 leg refuses too', async () => {
  const { bundle, context } = await buildV2();
  const stripped: any = structuredClone(bundle);
  stripped.signoffs[0].proof.signatures = stripped.signoffs[0].proof.signatures.filter((s: any) => s.alg === 'ML-DSA-65');
  const res = await verifyAuthorizationBundleV2(stripped, verifyOpts(context));
  assert.notEqual(res.verdict, 'SATISFIED');
});
...
test('SET NARROWING: narrowing required_algorithms to Ed25519-only refuses structurally and cryptographically', async () => {
  const { bundle, context } = await buildV2();
  const narrowed: any = structuredClone(bundle);
  ...
  const survivingSig = Buffer.from(bundle.signoffs[0].proof.signatures.find((s: any) => s.alg === 'Ed25519').sig, 'base64url');
  assert.equal(crypto.verify(null, fullBytes, ed.publicKey, survivingSig), true,
    'sanity: the surviving signature DOES verify over the full (unmodified) committed bytes');
  const narrowedBytes = Buffer.from(
    `EP-AUTHORIZATION-BUNDLE-SIGNOFF-v2\0${canonicalizeAeb({ context_hash: contextDigest, required_algorithms: ['Ed25519'] })}`,
    'utf8',
  );
...
test('DUPLICATE ALGORITHM: two entries for one algorithm refuse', async () => {
  const { bundle, context } = await buildV2();
  const dup: any = structuredClone(bundle);
...
test('WRONG-LENGTH SIGNATURE: a truncated Ed25519 leg refuses', async () => {
  const { bundle, context } = await buildV2();
  const truncated: any = structuredClone(bundle);
  const leg = truncated.signoffs[0].proof.signatures.find((s: any) => s.alg === 'Ed25519');
...
test('TAMPERED AFTER SIGNING: editing signed_at after signing does not affect the signoff signature (it only binds the context digest)', async () => {
  ...
  const { bundle, context } = await buildV2();
  const tampered: any = structuredClone(bundle);
