 ...
 */
export const OTEL_TOOL_AUTHORIZATION_VERSION = 'EMILIA-OTEL-TOOL-AUTHORIZATION-v0.1';
/**
...
export const TOOL_AUTHORIZATION_STATUS_VALUES = Object.freeze(Object.values(TOOL_AUTHORIZATION_STATUS));
/**
...
export const EVIDENCE_GRADE_VALUES = Object.freeze(Object.values(EVIDENCE_GRADE));
 ...
 */
function checkOpaqueToken(value, field) {
    if (typeof value !== 'string')
        return refuse(`${field}_not_a_string`);
    if (value.length === 0)
        return refuse(`${field}_empty`);
    if (value.length > MAX_VALUE_LENGTH)
        return refuse(`${field}_too_long`);
    if (value !== value.trim())
        return refuse(`${field}_has_surrounding_whitespace`);
    for (let i = 0; i < value.length; i += 1) {
        const code = value.charCodeAt(i);
        if (code < 0x20 || code > 0x7e)
            return refuse(`${field}_not_printable_ascii`);
    }
    return value;
}
function isRefusal(v) {
 ...
 */
export function buildToolAuthorizationAttributes(input, options = {}) {
    if (!isPlainObject(input))
        return refuse('input_not_an_object');
    const namespace = options.namespace ?? 'fallback';
    if (namespace !== 'registry' && namespace !== 'fallback' && namespace !== 'both') {
        return refuse('unknown_attribute_namespace');
    }
    const status = input.status;
    if (typeof status !== 'string')
        return refuse('status_not_a_string');
    if (!TOOL_AUTHORIZATION_STATUS_VALUES.includes(status))
        return refuse('unknown_status_value');
    const grade = input.evidence_grade;
    if (typeof grade !== 'string')
        return refuse('evidence_grade_not_a_string');
    if (!EVIDENCE_GRADE_VALUES.includes(grade))
        return refuse('unknown_evidence_grade_value');
    const optional = [
    ...
    if (grade === EVIDENCE_GRADE.INDEPENDENTLY_VERIFIABLE) {
        if (!resolved.evidenceDigest)
            return refuse('independently_verifiable_without_evidence_digest');
        if (!resolved.evidenceFormat)
            return refuse('independently_verifiable_without_evidence_format');
        if (!resolved.evidenceLocator)
            return refuse('independently_verifiable_without_evidence_locator');
    }
    const attributes = {};
    const emitRegistry = namespace === 'registry' || namespace === 'both';
    const emitFallback = namespace === 'fallback' || namespace === 'both';
    for (const slot of Object.keys(REGISTRY_ATTRIBUTE_KEYS)) {
        const value = resolved[slot];
        if (value === undefined)
            continue;
        if (emitRegistry)
            attributes[REGISTRY_ATTRIBUTE_KEYS[slot]] = value;
        if (emitFallback)
            attributes[FALLBACK_ATTRIBUTE_KEYS[slot]] = value;
    }
 ...
 */
export function setToolAuthorizationAttributes(span, input, options = {}) {
    const built = buildToolAuthorizationAttributes(input, options);
    if (!built.ok)
        return built;
    if (!span)
        return { ok: true, attributes: built.attributes, written: false };
    try {
 ...
 */
export async function resolveActiveSpan() {
    try {
        ...
        if (!trace || typeof trace.getActiveSpan !== 'function')
            return null;
        const span = trace.getActiveSpan();
        if (!span)
            return null;
        if (typeof span.setAttributes !== 'function' && typeof span.setAttribute !== 'function')
            return null;
        return span;
    }
    catch {
...
function withEvidence(status, grade, evidence) {
    const out = { status, evidence_grade: grade };
    if (evidence) {
        if (evidence.evidence_digest != null)
            out.evidence_digest = evidence.evidence_digest;
        if (evidence.evidence_format != null)
            out.evidence_format = evidence.evidence_format;
        if (evidence.evidence_locator != null)
            out.evidence_locator = evidence.evidence_locator;
        if (evidence.action_digest != null)
            out.action_digest = evidence.action_digest;
        if (evidence.policy_digest != null)
            out.policy_digest = evidence.policy_digest;
    }
    return out;
}
/**
 ...
 */
export function mapOpenAIAgentsDecision(decision, evidence) {
    if (typeof decision !== 'string')
        return refuse('openai_agents_decision_not_a_string');
    switch (decision) {
        case 'approve':
            return {
                ok: true,
                input: withEvidence(TOOL_AUTHORIZATION_STATUS.AUTHORIZED_IN_SCOPE, EVIDENCE_GRADE.INDEPENDENTLY_VERIFIABLE, evidence),
            };
 ...
 */
export function mapLangGraphDecision(decision, responseType, evidence) {
    if (typeof decision !== 'string')
        return refuse('langgraph_decision_not_a_string');
    ...
    switch (decision) {
        case 'resume':
            if (responseType === 'edit') {
                return {
                    ok: true,
                    input: withEvidence(TOOL_AUTHORIZATION_STATUS.EDITED_THEN_APPROVED, EVIDENCE_GRADE.INDEPENDENTLY_VERIFIABLE, evidence),
                };
            }
            return {
                ok: true,
        ...
        case 'reauthorize':
            ...
            return {
                ok: true,
                input: withEvidence(TOOL_AUTHORIZATION_STATUS.AUTHORIZED_OUT_OF_SCOPE, EVIDENCE_GRADE.SELF_ATTESTED, evidence),
            };
        case 'pass':
            ...
            return {
                ok: true,
                input: withEvidence(TOOL_AUTHORIZATION_STATUS.NO_AUTHORIZATION_STEP, EVIDENCE_GRADE.SELF_ATTESTED, evidence),
            };
        default:
 ...
 * rejected.
 */
export function mapLangChainDecision(allowed, receiptBound, evidence) {
    if (typeof allowed !== 'boolean')
        return refuse('langchain_allowed_not_a_boolean');
    if (typeof receiptBound !== 'boolean')
        return refuse('langchain_receipt_bound_not_a_boolean');
    if (!allowed) {
        return {
            ok: true,
            input: withEvidence(TOOL_AUTHORIZATION_STATUS.REJECTED, EVIDENCE_GRADE.SELF_ATTESTED, evidence),
 ...
 *   deny                                           -> rejected
 */
export function mapMcpGuardDecision(guardDecision, facts, evidence) {
    if (typeof guardDecision !== 'string')
        return refuse('mcp_guard_decision_not_a_string');
    if (!isPlainObject(facts))
        return refuse('mcp_guard_facts_not_an_object');
    const irreversible = facts.irreversible;
    const receiptConsumed = facts.receipt_consumed;
    if (typeof irreversible !== 'boolean')
        return refuse('mcp_guard_irreversible_not_a_boolean');
    if (typeof receiptConsumed !== 'boolean')
        return refuse('mcp_guard_receipt_consumed_not_a_boolean');
 ...
 */
export function mapClaudeCodeHookDecision(permissionDecision, evidence) {
    if (typeof permissionDecision !== 'string')
        return refuse('claude_code_decision_not_a_string');
 ...
 */
export async function emitToolAuthorization(telemetry, mapped, context = {}) {
    const options = telemetry ?? {};
    if (options.enabled === false)
        return { ok: false, reason: 'telemetry_disabled' };
    if (!mapped || mapped.ok !== true) {
        const result = { ok: false, reason: mapped?.ok === false ? mapped.reason : 'mapper_returned_no_input' };
        try {
            options.onEmit?.(result, context);
        }
        catch { /* telemetry never breaks the caller */ }
        return result;
    }
    let span = options.span ?? null;
    if (!span && typeof options.spanFor === 'function') {
        try {
            span = options.spanFor(context);
        }
        catch {
            span = null;
        }
