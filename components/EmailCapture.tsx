...
export default function EmailCapture({
  ...
  return (
    <section style={{ ...styles.section, paddingTop: 40, paddingBottom: 72, borderTop: `1px solid ${color.border}` }}>
      {status === 'done' ? (
        <div>
          <div style={{ ...eyebrowStyle, color: color.green }}>You're in</div>
          ...
          <p style={subStyle}>
            {result?.id != null ? (
              <>
                Founding member{' '}
                <strong style={{ color: accent, fontFamily: font.mono }}>#{result.id}</strong>. We'll only
                reach out when there's something worth your time.
              </>
            ) : (
              <>We'll only reach out when there's something worth your time.</>
            )}
          </p>
        </div>
      ) : (
