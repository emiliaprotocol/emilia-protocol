...
try {
  ...
  fail(2, `Cannot read receipt at ${receiptPath}: ${(e as Error).message}`);
}
...
if (explicitKey) {
  ...
  try {
    ...
    fail(2, `Could not fetch keys from ${keysUrl}: ${(e as Error).message}. Pass public-key explicitly.`);
  }
}
...
let result: ReceiptVerificationResult | null = null;
for (const key of candidates) {
  try {
    const r = verifyReceipt(receipt, key) as ReceiptVerificationResult;
