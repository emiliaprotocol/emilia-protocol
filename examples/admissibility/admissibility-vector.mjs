...
// Generated from admissibility-vector.mts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
//
...
export function run() {
    ...
    const staleComps = baseComponents().map((c) => c.type === 'authorization_receipt' ? { ...c, issued_at: '2026-07-02T11:00:00Z' } : c); // 60 min > 900 s
    ...
    const revokedComps = baseComponents().map((c) => c.type === 'recourse_reference' ? { ...c, revoked: true } : c);
    ...
    const forkComps = baseComponents().map((c) => c.type === 'policy_permit' ? { ...c, action_digest: 'e'.repeat(64) } : c);
    ...
    const denialComps = baseComponents().map((c) => c.type === 'authorization_receipt' ? { ...c, outcome: 'deny' } : c);
    ...
    const brokenComps = baseComponents().map((c) => c.type === 'authorization_receipt' ? { ...c, verified: false } : c);
...
console.log('ADMISSIBILITY VECTOR OK —', Object.values(results).map((v) => `${v.reliance_purpose ?? 'none'}:${v.verdict}`).join('  '));
