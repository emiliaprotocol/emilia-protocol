...
export function computeGatewayAction(request, { requestedAt = TIMES.requested, policy = DEMO_POLICY } = {}) {
  ...
  if (request.command !== 'move_container') throw new Error('command is not supported');
  nonEmptyString(request.container_id, 'container_id');
  if (request.from_zone !== policy.constraints.allowed_from_zone) throw new Error('from_zone is outside local policy');
  if (request.to_zone !== policy.constraints.allowed_to_zone) throw new Error('to_zone is outside local policy');
...
export function createApprovalAuthority(options: any = {}) {
...
function authorizationVerifier({ action, policy, verification, revokedEvidenceDigests }: any) {
  return (receipt: any) => {
    ...
    let receiptActionDigest: string | null = null;
    try { receiptActionDigest = artifactDigest(receipt?.action); } catch { /* refusal below */ }
    const artifactId = (() => {
      try { return artifactDigest(receipt); } catch { return null; }
    })();
    return {
 ...
 */
function refusal(reason: string, base: any = null) {
  return {
    allow: false,
    verdict: 'refuse',
    reason,
...
export function createReceivingGateway({
  policy = DEMO_POLICY,
  verification,
  challengeBackend = createMemoryBackend(),
  actionBackend = createMemoryBackend(),
  revokedEvidenceDigests = new Set(),
  now = () => TIMES.evaluated,
}: any = {}) {
  if (!verification) throw new Error('Gateway B requires pinned verification material');
  if (!(revokedEvidenceDigests instanceof Set)) throw new Error('Gateway B requires explicit revocation state');
  ...
  async function evaluate({ action, challenge: challengeDocument, receipt }) {
    ...
    let firstUse;
    try { firstUse = await actionStore.consume(`dmsc-action:${actionDigest}`); } catch {
      return refusal('action_consumption_storage_unavailable', base);
    }
    if (firstUse !== true) return refusal('action_already_consumed', base);
 ...
 */
export function verifyAuditBundle(bundle: any, {
  policy = DEMO_POLICY,
  verification,
  pinnedGatewayDecisionKey,
  revokedEvidenceDigests = new Set(),
}: any = {}) {
  try {
    if (!bundle || typeof bundle !== 'object') throw new Error('bundle missing');
    if (!(revokedEvidenceDigests instanceof Set)) throw new Error('revocation state missing');
    const actionDigest = artifactDigest(bundle.action);
    if (bundle.graph?.action_digest !== actionDigest) throw new Error('graph/action mismatch');
...
async function validFlow(authority) {
  ...
  const auditBundle = result.allow ? {
    ...
    graph: clone((result as any).graph),
    decision: clone((result as any).decision),
  } : null;
  return { gateway, prepared, receipt, result, auditBundle };
}

export async function runCrossGatewayDemo({ print = false }: any = {}) {
  ...
  const unavailableChallengeBackend: any = {
    durable: false,
    addIfAbsent: (key: any, value: any) => memory.addIfAbsent(key, value),
    compareAndSet: async (): Promise<boolean> => { throw new Error('simulated challenge-store outage'); },
    deleteIfValue: (key: any, expected: any) => memory.deleteIfValue(key, expected),
    has: (key: any) => memory.has(key),
  };
  ...
  if (print) {
    const line = (status: string, label: string, detail: string | null | undefined = '') => console.log(`${status.padEnd(8)} ${label}${detail ? ` - ${detail}` : ''}`);
    ...
    console.log('');
    line(valid.result.allow ? 'ALLOW' : 'REFUSE', 'pinned Class-A approval for the exact port action', valid.result.allow ? (valid.result as any).action_digest : null);
