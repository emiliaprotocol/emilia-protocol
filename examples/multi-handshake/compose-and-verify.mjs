...
// Generated from compose-and-verify.mts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
//
...
import { verifyQuorum, verifyWebAuthnSignoff, canonicalize, contextChainHash, } from '../../packages/verify/index.js';
...
const at = (min, sec = 0) => new Date(Date.UTC(2026, 5, 11, 0, min, sec)).toISOString();
const addSec = (iso, sec) => new Date(Date.parse(iso) + sec * 1000).toISOString();
const b64uSha256 = (utf8) => createHash('sha256').update(utf8, 'utf8').digest().toString('base64url');
// ---------------------------------------------------------------------------
...
function makeAuthenticator() {
    ...
    return {
        ...
        assert(challengeB64u) {
            counter += 1;
            const clientDataBytes = Buffer.from(JSON.stringify({ type: 'webauthn.get', challenge: challengeB64u, origin: ORIGIN }), 'utf8');
...
class QuorumComposer {
    ...
    trail;
    challenges;
    constructor({ policy, policyId, actionHash, initiator, enrollment }) {
     ...
     */
    issueChallenge({ approver, nowIso, validitySec = 600 }) {
     ...
     */
    admit(member, nowIso) {
        const refuse = (reason, ch) => {
            if (ch && ch.state === 'issued')
                ch.state = 'void';
        ...
        if (!ch)
            return refuse('unknown_challenge');
        if (ch.state !== 'issued')
            return refuse('challenge_reused', ch);
        if (Date.parse(nowIso) > ch.expiresAtMs)
            return refuse('stale_challenge', ch);
        if (canonicalize(ctx) !== canonicalize(ch.context))
            return refuse('context_mismatch', ch);
        // 2. Action binding: the exact action hash, nothing else.
        if (ctx.action_hash !== this.actionHash)
            return refuse('action_mismatch', ch);
        // 3. Initiator MUST NOT fill an approver slot (Q3; the base draft's
        //    SelfApprovalImpossible at the composition layer).
        if (ctx.approver === this.initiator)
            return refuse('self_approval', ch);
        ...
        if (this.trail.length >= this.policy.required)
            return refuse('quorum_already_satisfied', ch);
        ...
        if (this.trail.length > 0) {
            ...
            if (!(t - first <= this.policy.window_sec * 1000))
                return refuse('window_exceeded', ch);
            if (!(t > last))
                return refuse('non_increasing_time', ch);
        }
        ...
        if (!enrolledKey)
            return refuse('not_enrolled', ch);
        if (member.approver_public_key !== enrolledKey)
            return refuse('key_not_enrolled', ch);
...
const enrollment = new Map([...devices].map(([id, d]) => [id, d.publicKeySpkiB64u]));
...
function handshakeFor(approverId, role, device, nowIso, opts = {}) {
...
{
    const m = handshakeFor('ep:approver:cfo_bram', 'chief_financial_officer', devices.get('ep:approver:cfo_bram'), at(0, 30));
    const r = composer.admit(m, at(0, 45));
    console.log(`1) out-of-order signature      -> stage=${r.stage} reason=${r.reason}`);
    check(!r.admitted && r.reason === 'out_of_order', 'refused at admission with out_of_order (never appended)');
}
// ADMISSION 1: the Treasury Controller fills declared slot 1.
let controllerMember;
{
    controllerMember = handshakeFor('ep:approver:controller_ada', 'treasury_controller', devices.get('ep:approver:controller_ada'), at(1, 0));
...
// device assertion for the next slot. Refused BEFORE appending.
{
    ...
    const r = composer.admit(m, at(1, 40));
    console.log(`\n2) initiator self-approval     -> stage=${r.stage} reason=${r.reason}`);
    check(!r.admitted && r.reason === 'self_approval', 'refused at admission with self_approval (initiator may not fill an approver slot)');
}
...
{
    const r = composer.admit(controllerMember, at(2, 0));
    console.log(`\n3) replayed member handshake   -> stage=${r.stage} reason=${r.reason}`);
    check(!r.admitted && r.reason === 'challenge_reused', 'refused at admission with challenge_reused (one-time nonce already consumed)');
    const stale = handshakeFor('ep:approver:cfo_bram', 'chief_financial_officer', devices.get('ep:approver:cfo_bram'), at(2, 10), { validitySec: 60 });
    const r2 = composer.admit(stale, at(3, 20)); // presented 70s after issue, 60s validity
    console.log(`   stale-challenge variant     -> stage=${r2.stage} reason=${r2.reason}`);
    check(!r2.admitted && r2.reason === 'stale_challenge', 'refused at admission with stale_challenge (validity window elapsed)');
}
...
{
    const m = handshakeFor('ep:approver:cfo_bram', 'chief_financial_officer', rogueDevice, at(3, 30), { claimKey: enrollment.get('ep:approver:cfo_bram') });
    const r = composer.admit(m, at(3, 40));
    console.log(`\n4) off-roster key              -> stage=${r.stage} reason=${r.reason}`);
    check(!r.admitted && r.reason === 'invalid_signature', 'refused at admission with invalid_signature (assertion does not verify against the enrolled key)');
}
...
{
    const m = handshakeFor('ep:approver:cfo_bram', 'chief_financial_officer', devices.get('ep:approver:cfo_bram'), at(4, 0));
...
// key it carries, but its (role, approver) is not an eligible slot.
{
    ...
    check(v.valid === false && v.checks.roles_admitted === false, 'real verifier refuses the forced trail (roles_admitted=false), even though admission was bypassed');
}
