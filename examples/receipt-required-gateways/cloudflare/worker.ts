...
type Env = {
  ...
  EP_ACTION: string;
  EP_ACTION_HASH?: string;
  EP_AUTHORIZATION_ENDPOINT: string;
  EP_REQUIRED_FIELDS: string;
  EP_CAID_SELECTOR_FIELD?: string;
  EP_MAX_AGE_SEC?: string;
  EP_MAX_BODY_BYTES?: string;
  EP_TRUSTED_KEYS: string;
  RECEIPT_CONSUMPTION: DurableObjectNamespace;
};
...
function isCanonicalizable(value: unknown): boolean {
  ...
  if (value && typeof value === 'object') return Object.values(value).every(isCanonicalizable);
  return false;
}
...
function parseJsonArray(value: string, name: string): string[] {
  if (!strictJsonGate(value).ok) throw new Error(`${name}_invalid_json`);
...
function parseCarrier(carrier: string): ReceiptDocument | null {
  if (!/^[A-Za-z0-9+/_-]+={0,2}$/.test(carrier) || carrier.length % 4 === 1) return null;
  const mixedAlphabets = /[-_]/.test(carrier) && /[+/]/.test(carrier);
  if (mixedAlphabets) return null;
  try {
    ...
    let binaryCanonical = '';
    for (const byte of bytes) binaryCanonical += String.fromCharCode(byte);
...
async function projectJsonAction(request: Request): Promise<Record<string, unknown>> {
  const contentType = request.headers.get('content-type')?.split(';', 1)[0].trim().toLowerCase();
  if (contentType !== 'application/json') throw new Error('observed_action_invalid');
  const text = await request.clone().text();
  if (!strictJsonGate(text).ok) throw new Error('observed_action_invalid');
  const value = JSON.parse(text);
  if (!value || typeof value !== 'object' || Array.isArray(value)) throw new Error('observed_action_invalid');
  return value as Record<string, unknown>;
}

function positiveInteger(value: string | undefined, fallback: number): number {
  const parsed = value === undefined ? fallback : Number(value);
  if (!Number.isSafeInteger(parsed) || parsed <= 0) throw new Error('positive_integer_required');
  return parsed;
}

function problemResponse(decision: Extract<Awaited<ReturnType<ReturnType<typeof createReceiptRequiredEdgeHandler>>>, { ok: false }>) {
...
export class ReceiptConsumption {
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    let body: { receipt_id?: unknown; action?: unknown };
    try { body = await request.json(); } catch { return Response.json({ consumed: false }, { status: 400 }); }
  ...
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    const authorize = createReceiptRequiredEdgeHandler({
      ...
      async verifyReceipt(carrier, context) {
        ...
        if (payload?.claim?.action_type !== context.action) return { ok: false, reason: 'action_mismatch' };
        ...
        const claimedHash = typeof payload.claim.action_hash === 'string'
          ...
          : '';
        if (claimedHash !== computedHash) return { ok: false, reason: 'signed_action_hash_mismatch' };
        ...
        if (caidField && (typeof signedAction[caidField] !== 'string'
            || !caidPattern.test(signedAction[caidField] as string))) {
          return { ok: false, reason: 'caid_binding_invalid' };
        }
        let valid = false;
        for (const key of trustedKeys) {
          const result = await verifyReceiptWeb(document as Record<string, unknown>, key);
          if (result.valid) { valid = true; break; }
        }
        if (!valid) return { ok: false, reason: 'untrusted_or_invalid_signature' };
      ...
      async consume(receiptId, context) {
        ...
        if (!response.ok) throw new Error('durable consumption unavailable');
    ...
    if (decision.ok === false) return problemResponse(decision);

    let upstreamOrigin: URL;
    try { upstreamOrigin = new URL(env.UPSTREAM_ORIGIN); } catch { return new Response('gateway misconfigured', { status: 503 }); }
    ...
    for (const name of decision.upstream.remove_headers) headers.delete(name);
    for (const [name, value] of Object.entries(decision.upstream.set_headers)) headers.set(name, value);
