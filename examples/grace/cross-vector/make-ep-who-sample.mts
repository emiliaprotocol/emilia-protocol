...
const sha256hex = (buf: any) => crypto.createHash('sha256').update(buf).digest('hex');
...
console.log('receipt_id      :', receipt.receipt_id ?? (receipt as any).payload?.receipt_id);
console.log('subject digest  :', receipt.action_hash ?? (receipt as any).payload?.action_hash);
