...
const keyFor = (label: string) => crypto.createPrivateKey({
  key: Buffer.concat([PKCS8, crypto.createHash('sha256').update(`ep:grace-vector:v1:${label}`).digest()]),
  format: 'der', type: 'pkcs8',
});
const pub = (k: any) => crypto.createPublicKey(k).export({ type: 'spki', format: 'der' }).toString('base64url');
...
const mkSigned = (payload: any, key: any, keyField: string) => {
  const artifact: any = { payload, sig: crypto.sign(null, Buffer.from(canonicalize(payload), 'utf8'), key).toString('base64url') };
...
const presented = (evaluatePresentation(challenge, graph, CURTAILMENT_SETTLEMENT_POLICY, { verifiers, as_of: AS_OF, consumedNonces: nonces })) as { verdict: string, reasons: any[], next_challenge: any, result?: any, replay_digest?: any };
...
const settled = checkSettlementConsumption(settlementClaim, consumed as Set<string>);
if (!settled.settled) throw new Error('first settlement refused: ' + settled.reason);
const doubleSell = checkSettlementConsumption(settlementClaim, consumed as Set<string>);
