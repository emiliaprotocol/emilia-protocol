...
// Generated from agentic-audit-trail-demo.mts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
/**
...
const BASE = Date.now();
let tOffsetMs = 0;
const now = () => BASE + tOffsetMs;
const advance = (ms = 1000) => { tOffsetMs += ms; };
...
const canon = (v) => (v == null ? JSON.stringify(v)
    : Array.isArray(v) ? `[${v.map(canon).join(',')}]`
        : typeof v === 'object' ? `{${Object.keys(v).sort().map((k) => JSON.stringify(k) + ':' + canon(v[k])).join(',')}}`
            : JSON.stringify(v));
const HASH_FOR = (actionType) => crypto.createHash('sha256').update(canon({ action_type: actionType }), 'utf8').digest('hex');
function makeIssuerKey() {
...
function mintReceipt(privateKey, payload) {
...
async function main() {
    ...
    async function present(receipt, args) {
        ...
        try {
            return await gate.check({ ...args, receipt });
        }
        finally {
