...
export async function runConservedAdmissionLab() {
    ...
    async function admitAt(gateway, side, receipt, action = EXACT_ACTION) {
        const outcome = await gateway.gate.run({ selector: { ...SELECTOR }, receipt, observedAction: action }, executorAt(side));
        ...
        cases.push({
            ...
            records: {
                ...
                joined_by: {
                    ...
                    same_action_digest: handoff.transferRecord.action_hash === bAdmit.observed_action_hash
    ...
    return {
        ...
        scenario: 'A field robot with an outstanding single-use authorization for one valve shutoff roams from Gateway A\'s domain to Gateway B\'s while the authorization is live.',
        reference: 'draft-dunbar-dmsc-gw-scenarios-gap-analysis-04, Section 7.8 (Conserved Admission Across Gateway Boundaries)',
        property: 'At-most-once admission of a specific action across independently operated gateways; not exactly-once physical execution.',
        action: EXACT_ACTION,
        action_hash: actionHash,
        executions,
        cases,
...
function print(result) {
    ...
    for (const [index, item] of result.cases.entries()) {
        ...
        if (item.records?.joined_by)
            console.log(`   transfer and admission records join by action digest + handoff id: ${item.records.joined_by.same_action_digest ? 'yes' : 'NO'}`);
