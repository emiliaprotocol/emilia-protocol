...
// Generated from derf-receipt-demo.mts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
//
...
async function type(s, { d = 12, nl = true } = {}) {
    if (FAST) {
...
async function beat(t) { console.log(); rule(); await type('  ' + bold(t), { d: 8 }); rule(); await sleep(300); }
...
function mint(a, ah) {
    ...
    const receipt = { receipt_id: 'ep:receipt:rx-renewal-55019', action: a, action_hash: ah, contexts: [ctx], signoffs: [{ context_hash: `sha256:${d}`, key_class: 'A', approver_key_id: 'ep:key:pharmacist-jchen#1', signed_at: '2026-07-08T15:04:06Z', webauthn: webauthn(d) }], consumption: { nonce: 'n-consume', state: 'COMMITTED', committed_at: '2026-07-08T15:04:07Z' } };
...
async function main() {
    ...
    await sleep(200);
    const r1 = verifyTrustReceipt(receipt, PINNED);
    console.log();
    for (const [k, label] of [['action_hash', 'action bound to the signed bytes'], ['signoff_signatures', 'named human\'s device signature'], ['inclusion', 'transparency-log inclusion proof'], ['checkpoint_signature', 'log checkpoint signature']]) {
        console.log('    ' + (r1.checks[k] ? green('✓') : red('✗')) + '  ' + dim(label));
        await sleep(140);
    }
    console.log();
    ...
    const r2 = verifyTrustReceipt(tampered, PINNED);
    await sleep(200);
    console.log();
    console.log('    ' + (r2.checks.action_hash ? green('✓') : red('✗')) + '  ' + dim('action bound to the signed bytes'));
    await sleep(200);
    ...
    const r3 = verifyTrustReceipt(receipt, { approverKeys: {}, logPublicKey: logKey.pub, rpId: 'www.emiliaprotocol.ai' });
    await sleep(200);
    console.log();
    ...
    if (!ok) {
        console.error(red('DEMO INVARIANT FAILED'));
        process.exit(1);
    }
}
