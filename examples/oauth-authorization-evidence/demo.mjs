...
import { actionHash, canonicalize, generateEd25519KeyPair, issueTrustReceipt, publicKeyToSpkiB64u, softwareSignerFromPrivateKey, } from '../../packages/issue/index.js';
...
async function verifyDetachedEvidenceJws(evidence, publicKey) {
    ...
    if (parts.length !== 3 || parts[1] !== '')
        throw new Error('as_signature is not a detached compact JWS');
    const payload = Buffer.from(canonicalize(evidenceSignatureInput(evidence)), 'utf8');
    ...
    if (result.protectedHeader.kid !== AS_KEY_ID)
        throw new Error('unexpected AS evidence key id');
    return true;
}
function createDirectorySnapshot({ entry, rootPrivateKey, now, directoryId = DIRECTORY_ID, sequence = DIRECTORY_SEQUENCE, }) {
    const payload = {
...
function verifyDirectorySnapshot(snapshot, trust, now) {
    if (!snapshot || typeof snapshot !== 'object')
        throw new Error('missing approver directory snapshot');
    const { payload, signature } = snapshot;
    if (!payload || !signature || signature.alg !== 'Ed25519')
        throw new Error('malformed approver directory snapshot');
    const root = trust?.pinnedDirectoryRoots?.[signature.kid];
    if (typeof root !== 'string')
        throw new Error('directory root is not pinned');
    const publicKey = crypto.createPublicKey({
        key: Buffer.from(root, 'base64url'),
        format: 'der',
        type: 'spki',
    });
    const signatureOk = crypto.verify(null, Buffer.from(canonicalize(payload), 'utf8'), publicKey, Buffer.from(signature.value || '', 'base64url'));
    if (!signatureOk)
        throw new Error('approver directory signature is invalid');
 ...
 */
export async function verifyOAuthAuthorizationEvidence({ accessToken, expectedAction, artifactStore, approverDirectory, trust, now = new Date(), } = {}) {
    const checks = {
        access_token: false,
    ...
    try {
        if (!(now instanceof Date) || !Number.isFinite(now.getTime()))
            return refusal(checks, 'invalid_verification_time');
        if (!expectedAction || typeof expectedAction !== 'object')
            return refusal(checks, 'missing_expected_action');
        if (typeof accessToken !== 'string' || !accessToken)
            return refusal(checks, 'missing_access_token');
        ...
        if (token.protectedHeader.kid !== AS_KEY_ID)
            return refusal(checks, 'unexpected_as_key');
        ...
        const matches = Array.isArray(details)
            ...
            : [];
        if (matches.length !== 1 || !matches[0].evidence)
            return refusal(checks, 'missing_or_ambiguous_authorization_evidence');
        ...
        if (!receipt || receiptReference(receipt) !== reference)
            return refusal(checks, 'evidence_reference_digest_mismatch');
        ...
        if (!receiptReport.valid)
            return refusal(checks, `ep_receipt_refused:${receiptReport.errors[0] || 'invalid'}`);
        checks.ep_receipt = true;
        checks.action_binding = receipt.action_hash === actionHash(expectedAction);
        if (!checks.action_binding)
            return refusal(checks, 'authorized_action_does_not_match_requested_action');
        checks.subject_binding = token.payload.sub === expectedAction.initiator;
        if (!checks.subject_binding)
            return refusal(checks, 'token_subject_does_not_match_action_initiator');
