...
} from '../../packages/verify/src/portable-state-handoff.js';
...
test('independent Continuum producer and EMILIA recipient agree on bytes, bindings, and terminal receipt', async () => {
  ...
  const boundary = new InMemoryRecipientStateBoundary({
    authorizeImport(expected, evidence) {
      if ((evidence as { caid?: string })?.caid !== expected.caid) {
  ...
  const receipt = await importPortableState(one, {
    ...
    source_authority_verifier: {
      async verify(expected, evidence) {
        if ((evidence as { caid?: string; consumed?: boolean })?.caid !== expected.caid
            || (evidence as { consumed?: boolean })?.consumed !== true) {
  ...
  assert.deepEqual(
    await verifyPortableStateImportReceiptForManifest(receipt, one.manifest, pins),
    { valid: true, reasons: [] },
  );
});
