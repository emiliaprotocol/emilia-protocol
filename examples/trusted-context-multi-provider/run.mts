...
} from '@emilia-protocol/gate/trusted-context/sheesh';
import {
  ZEP_PROVIDER_ID,
  ZEP_SOURCE_PROFILE,
...
} from '@emilia-protocol/gate/trusted-context/zep';
...
export function runTrustedContextMultiProviderDemo() {
  ...
  const fullPolicy = (sourceProfile: string, contextFrameProfile: string) => ({
    adapterKeys,
  ...
  const sheeshFull = verifyMemoryProjectionRecordV1(
    sheesh.record,
    sheesh.verificationMaterial,
    fullPolicy(SHEESH_SOURCE_PROFILE, SHEESH_CONTEXT_FRAME_PROFILE),
  );
  const zepFull = verifyMemoryProjectionRecordV1(
    zep.record,
    zep.verificationMaterial,
    fullPolicy(ZEP_SOURCE_PROFILE, ZEP_CONTEXT_FRAME_PROFILE),
  );
