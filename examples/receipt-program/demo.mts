...
function resolveCaid(observed) {
  ...
  const computed = computeCaid(materialAction, { suite: 'jcs-sha256', definitions, enumSnapshots: REGISTRY_ENUM_SNAPSHOTS });
  if (!computed.caid) throw new Error(`CAID refused: ${computed.refusals?.join(',') ?? 'unknown reason'}`);
...
if (!parent.secret) throw new Error('parent capability secret missing');
...
if (!child.ok) throw new Error(`delegation refused: ${child.reason}`);
if (!child.secret) throw new Error('child capability secret missing');
...
if (!run.ok || !verified.ok) throw new Error(`receipt program failed: ${run.reason || verified.reason}`);
...
for (const step of run.certificate.steps) console.log(`Step ${step.sequence}: ${step.opcode}`);
