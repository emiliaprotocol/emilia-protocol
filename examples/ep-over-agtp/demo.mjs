...
const verify = (pub, obj, sigB64u) => crypto.verify(null, Buffer.from(jcs(obj), 'utf8'), pub, Buffer.from(sigB64u, 'base64url'));
// ── Keys ─────────────────────────────────────────────────────────────────────
...
const epOk = verify(approverKey.publicKey, agtpMessage.authorization.credential.payload, agtpMessage.authorization.credential.signature.value);
...
const epAfterTamper = verify(approverKey.publicKey, tamperedAuth.authorization.credential.payload, tamperedAuth.authorization.credential.signature.value);
