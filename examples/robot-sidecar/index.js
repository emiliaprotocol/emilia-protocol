...
export class EdgeActuatorGate {
    ...
    permit(command = {}) {
        if (!this.envelope)
            return { allow: false, reason: 'no_envelope' };
        if (this.revoked)
        ...
        if (w.not_before && nowSec < w.not_before)
            return { allow: false, reason: 'before_window' };
        if (w.not_after && nowSec > w.not_after)
            return { allow: false, reason: 'expired' };
        const s = this.envelope.scope;
        if (s.target_set && command.target && !s.target_set.includes(command.target))
            return { allow: false, reason: 'out_of_target_set' };
        if (s.allowed_actions && !s.allowed_actions.includes(command.action))
            return { allow: false, reason: 'action_not_in_envelope' };
        if (s.bounds?.max_reach_cm != null && command.reach_cm != null && command.reach_cm > s.bounds.max_reach_cm)
            return { allow: false, reason: 'exceeds_bounds' };
