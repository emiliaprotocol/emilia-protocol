...
import { ACTION_ESCROW_CONTRACTOR_EVIDENCE_PACKAGE_VERSION, } from '../../packages/gate/action-escrow-evidence.js';
...
test('keeps document, exact-action approvals, and external custody visibly separate', async () => {
    ...
    assert.equal(scenario.view.project_record.authorizes_action, false);
    assert.equal(scenario.view.project_record.establishes_acceptance, false);
    assert.equal(scenario.view.release.action.project_record_snapshot_digest, scenario.view.project_record.snapshot_digest);
    ...
    assert.equal(scenario.view.document.verification.document_as_payment_refusal, 'resolution_profile_invalid');
});
test('uses the shipped DAB, Action Escrow kernel, and portable evidence package', async () => {
    ...
    assert.equal(scenario.bundle.version, ACTION_ESCROW_CONTRACTOR_EVIDENCE_PACKAGE_VERSION);
    ...
    assert.equal(scenario.bundleProjectRecordAttack.valid, false);
    assert.equal(scenario.bundleProjectRecordAttack.reason, 'project_record_bytes_mismatch');
    assert.equal(Object.hasOwn(scenario.bundle.document, 'content_base64'), false);
    assert.equal(scenario.bundle.project_record.provider, 'procore');
    assert.equal(scenario.bundle.project_record.snapshot_digest, scenario.view.project_record.snapshot_digest);
    assert.equal(scenario.bundle.agreement_acceptances.length, 2);
    assert.equal(scenario.bundle.release_approvals.length, 2);
});
test('refuses project-source context relabeling even after package rehash', async () => {
    const scenario = await scenarioPromise;
    assert.equal(scenario.bundleProjectContextAttack.valid, false);
    assert.equal(scenario.bundleProjectContextAttack.reason, 'project_record_verification_failed');
});
test('releases exactly once and refuses every required mutation', async () => {
    ...
    assert.equal(scenario.view.attacks.every((attack) => (attack.refused && attack.reason === attack.expected_reason)), true);
});
test('preserves approve, decline, reject, and amend as distinct signed outcomes', async () => {
    ...
    assert.equal(scenario.view.outcomes.filter((outcome) => outcome.release_authorized).length, 1);
    assert.equal(scenario.view.outcomes.find((outcome) => outcome.outcome === 'approve')
        ?.release_authorized, true);
});
test('labels external custody as simulated and never assigns custody to EMILIA', async () => {
    const scenario = await scenarioPromise;
    assert.equal(scenario.view.custodian.provider.provider_mode, 'SIMULATED_LOCAL_PROVIDER');
    assert.equal(scenario.view.custodian.provider.emilia_holds_funds, false);
    assert.equal(scenario.view.custodian.provider.license_reference, 'SIMULATED-NOT-A-LICENSE');
});
