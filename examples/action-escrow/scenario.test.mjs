...
test('keeps document, exact-action approvals, and external custody visibly separate', async () => {
  ...
  assert.equal(
    scenario.view.release.action.project_record_snapshot_digest,
    scenario.view.project_record.snapshot_digest,
  );
...
test('uses the shipped DAB, Action Escrow kernel, and portable evidence package', async () => {
  ...
  assert.equal(
    scenario.bundleProjectRecordAttack.reason,
    'project_record_bytes_mismatch',
  );
  ...
  assert.equal(
    scenario.bundle.project_record.snapshot_digest,
    scenario.view.project_record.snapshot_digest,
  );
...
test('refuses project-source context relabeling even after package rehash', async () => {
  ...
  assert.equal(
    scenario.bundleProjectContextAttack.reason,
