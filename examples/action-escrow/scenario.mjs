...
import { computeReleaseActionDigest, signDocumentActionBinding, verifyDocumentActionBinding, } from '../../packages/verify/document-action-binding.js';
import { RESOLUTION_CONTEXT_TYPE, RESOLUTION_VERSION, computeResolutionChallenge, computeResolutionResponseHash, verifyResolutionReceipt, } from '../../packages/verify/resolution.js';
import { ACTION_ESCROW_PROFILE_VERSION, createActionEscrowReleaseBindingMoment, computeActionEscrowReleaseBindingMomentDigest, computeActionEscrowResolutionNonce, createActionEscrowKernel, } from '../../packages/gate/action-escrow.js';
import { ACTION_ESCROW_CONTRACTOR_TEMPLATE_VERSION, computeActionEscrowAgreementDigest, createActionEscrowContractorDocumentBindingVerifier, } from '../../packages/gate/action-escrow-verifiers.js';
import { ACTION_ESCROW_CONTRACTOR_EVIDENCE_PACKAGE_VERSION, verifyActionEscrowEvidencePackage, } from '../../packages/gate/action-escrow-evidence.js';
import { assembleActionEscrowContractorEvidencePackage, } from '../../packages/gate/action-escrow-package.js';
import { createActionEscrowCustodianBridge, createActionEscrowCustodianStatementVerifier, } from '../../packages/gate/action-escrow-custodian.js';
import { createActionEscrowStatePackageVerifier, signActionEscrowStateStatement, } from '../../packages/gate/action-escrow-state.js';
...
import { createProcoreChangeOrderAdapter, verifyProcoreChangeOrderEvidence, } from '../../lib/integrations/action-escrow/procore-change-order.js';
/**
...
function invariant(condition, message) {
    if (!condition)
        throw new Error(`Action Escrow invariant failed: ${message}`);
}
...
async function fetchSimulatedProcoreEvidence() {
    ...
    invariant(evidence.kind === 'evidence_ready', 'simulated Procore adapter must return a complete stable snapshot');
    invariant(evidence.evidence.authorizes_action === false
        && evidence.evidence.establishes_acceptance === false, 'project source evidence must never become acceptance or release authority');
    return evidence;
}
async function fetchSimulatedAcrobatEvidence(document) {
    ...
    const fakeFetch = async (url) => {
        ...
        if (requestUrl.pathname.endsWith('/combinedDocument')) {
            invariant(requestUrl.searchParams.get('versionId')
                === agreementEvents.events[0].versionId, 'Acrobat simulation must fetch the authoritative document version');
    ...
    invariant(Buffer.from(evidence.document_bytes).equals(document.bytes), 'DAB must receive the exact provider-refetched PDF bytes');
    return evidence;
}
...
function releaseAction({ terms, document, materialTermsDigest, completionDigest, profileDigest, projectRecordDigest, }) {
    return {
...
function makeAgreementAcceptance({ party, key, bindings, documentDigest, }) {
...
function resolutionOutcome(outcome, detail) {
    if (outcome === 'approved')
        return { outcome, selected_option: 0 };
    if (outcome === 'declined')
        return { outcome };
    if (outcome === 'rejected') {
        return {
...
function makeResolution({ party, key, outcome, bindings, action, detail, }) {
...
function verifyResolutionArtifact(artifact, { party, key, bindings, action, }) {
    const bindingInput = actionEscrowResolutionBindingInput(bindings, action);
...
function makeFundingStatement({ key, bindings, terms, }) {
    return signReceipt(key.privateKey, {
...
function attackResult({ id, title, layer, mutation, result, expectedReason, detail, }) {
...
function buildView({ terms, mapping, projectRecord, document, binding, documentVerification, completion, action, acceptances, approvals, approvalVerification, outcomes, custodian, fundingStatement, fundingVerification, releaseVerification, releaseResult, replayResult, attacks, evidencePackage, packageVerification, }) {
    return {
...
export async function runActionEscrowScenario() {
    ...
    const projectRecordBytes = Buffer.from(canonicalize(projectRecord.evidence), 'utf8');
    ...
    const action = releaseAction({
        terms,
        document,
        materialTermsDigest: sha256Canonical(dabMaterialTerms(terms, projectRecord.material_source_digest).sort((left, right) => (left.term_id < right.term_id ? -1 : left.term_id > right.term_id ? 1 : 0))),
    ...
    function verifyCompletionArtifact(artifact, expected) {
        ...
        const claim = artifact?.payload?.claim;
        const signer = artifact?.payload?.signer;
        const valid = Boolean(cryptoResult.valid
    ...
    const kernelDocumentBindingVerifier = createActionEscrowContractorDocumentBindingVerifier({
        issuerKeys: {
            [keys.mapping.keyId]: {
                issuer_id: MAPPING_ISSUER_ID,
        ...
        resolveDocumentBytes: async (expected) => (expected.agreement_id === AGREEMENT_ID
    ...
    const kernel = createActionEscrowKernel({
        ...
        verifyAgreementAcceptance: async (artifact, expected) => (verifyAcceptanceArtifact(artifact, expected)),
        verifyMilestoneEvidence: async (artifact, expected) => (verifyCompletionArtifact(artifact, expected)),
        ...
        verifyProviderStatement: async (artifact, expected) => (artifact?.payload?.statement_type === 'release'
    ...
    invariant(milestoneAttackResult.code === 'milestone_evidence_invalid', `tampered milestone evidence refusal: ${milestoneAttackResult.code}`);
    ...
    invariant(documentAsPaymentResult.code === 'resolution_profile_invalid', 'document acceptance must not become payment authorization');
    ...
    for (const outcome of ['decline', 'reject', 'amend']) {
        ...
        invariant(outcomeResults[outcome].code === 'resolution_not_approved', `${outcome} must not authorize the original release`);
    }
    ...
    invariant(signerAttackResult.code === 'resolution_verification_refused', `wrong signer refusal: ${signerAttackResult.code}`);
    ...
    invariant(releaseResult.ok && releaseResult.code === 'release_committed', `release: ${releaseResult.code} (calls=${custodian.state.releaseCalls}, reconciliations=${custodian.state.reconciliationCalls})`);
    ...
    invariant(!replayResult.ok && replayResult.code === 'release_already_applied', `replay refusal: ${replayResult.code}`);
    invariant(custodian.state.releaseCalls === 1, 'replay must not reach the custodian');
    const releaseVerification = await verifyCustodianObservation(custodian.state.releaseStatement, {
        ...kernelBindings,
        provider_id: CUSTODIAN_PROVIDER_ID,
    ...
    invariant(attacks.every((attack) => attack.refused && attack.reason === attack.expected_reason), `every attack must refuse as expected: ${JSON.stringify(attacks)}`);
    ...
    invariant(approvalVerification.homeowner.authorizes_action
        && approvalVerification.contractor.authorizes_action, 'both exact release approvals must verify');
    ...
    const evidencePackage = assembleActionEscrowContractorEvidencePackage({
        ...
        projectRecordBytes,
        projectRecordFileName: 'action-escrow-AGR-KITCHEN-2048-CCO-017-project-record.json',
        projectRecordProvider: 'procore',
        projectRecordSnapshotDigest: projectRecord.material_source_digest,
    }, {
        now: FIXED_NOW_MS,
    });
    invariant(evidencePackage.version === ACTION_ESCROW_CONTRACTOR_EVIDENCE_PACKAGE_VERSION, 'download must use the contractor Action Escrow evidence-package profile');
    ...
    const packageVerificationOptions = {
        ...
        verifyBinding: async (artifact, expected) => {
            ...
            return result.document_digest === expected.expectedDocumentDigest
                ? {
                    ...result,
                    project_record_snapshot_digest: artifact?.release_action?.template?.project_record_snapshot_digest,
                }
                : { ...result, valid: false, reason: 'document_digest_mismatch' };
        },
        verifyProjectRecord: async (record, expected) => (expected.provider === 'procore'
        ...
        verifyReleaseApproval: async (resolution, expected) => {
            ...
            const admission = expected.stateRecord?.snapshot?.operations
                ?.filter((entry) => entry.operation === 'approve_release')?.[expected.approvalIndex];
            return {
                valid: verification.valid,
                authorizes_action: verification.authorizes_action,
                outcome: verification.outcome,
    ...
    const packageVerification = await verifyActionEscrowEvidencePackage(evidencePackage, packageVerificationOptions);
    invariant(packageVerification.valid, `portable evidence package must verify (${packageVerification.reason})`);
    ...
    const projectRecordPackageAttack = await verifyActionEscrowEvidencePackage(evidencePackage, {
        ...packageVerificationOptions,
        projectRecordBytes: mutatedProjectRecordBytes,
    });
    invariant(!projectRecordPackageAttack.valid
        && projectRecordPackageAttack.reason === 'project_record_bytes_mismatch', 'portable package must refuse substituted project-record bytes');
    const contextMutatedRecord = clone(projectRecord.evidence);
    contextMutatedRecord.project_id = 'project:attacker-substitution';
    const contextMutatedBytes = Buffer.from(canonicalize(contextMutatedRecord), 'utf8');
    ...
    const { package_digest: _oldContextPackageDigest, ...contextMutatedPackageScope } = contextMutatedPackage;
    contextMutatedPackage.package_digest =
        sha256Canonical(contextMutatedPackageScope);
    const projectContextPackageAttack = await verifyActionEscrowEvidencePackage(contextMutatedPackage, {
        ...packageVerificationOptions,
        projectRecordBytes: contextMutatedBytes,
    });
    invariant(!projectContextPackageAttack.valid
        && projectContextPackageAttack.reason
            === 'project_record_verification_failed', 'portable package must refuse project-source context relabeling');
    ...
    const documentVerification = {
        ...
        checks: {
            ...
            both_agreement_acceptances_verified: documentAcceptanceVerification.homeowner.valid
                && documentAcceptanceVerification.contractor.valid,
            signed_document_is_not_payment_authority: documentAsPaymentResult.code === 'resolution_profile_invalid',
        },
    };
