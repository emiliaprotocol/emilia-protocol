...
function deterministicDemoKey(label) {
  ...
  const publicKeyObject = crypto.createPublicKey(privateKey as any);
...
async function fetchSimulatedAcrobatEvidence(document) {
  ...
  invariant(
    Buffer.from((evidence as any).document_bytes).equals(document.bytes),
...
export async function runActionEscrowScenario(): Promise<any> {
  const keys = {
  ...
  const binding = signDocumentActionBinding({
    ...
    agreement_id: AGREEMENT_ID,
    document: {
      bytes: (esignEvidence as any).document_bytes,
      media_type: 'application/pdf',
    },
    material_terms: dabMaterialTerms(terms, projectRecord.material_source_digest) as any,
    release_action_template: action,
    parties,
  ...
  const dabVerification = verifyDocumentActionBinding(binding, dabOptions as any);
  ...
  const outcomeResults: Record<string, any> = {};
  ...
  (mutatedBinding as any).material_terms
    .find((term: any) => term.term_id === 'release.amount').value = '28400.00';
  const materialTermsAttack = verifyDocumentActionBinding(mutatedBinding as any, dabOptions as any);
  ...
  const stateStatement = signActionEscrowStateStatement({
    ...
    bindingDigest: kernelBindings.document_action_binding_digest as string,
    actionDigest: kernelBindings.release_action_digest as string,
  ...
  const evidencePackage = assembleActionEscrowContractorEvidencePackage({
    ...
    documentExecution: {
      ...
      evidence: (esignEvidence as any).evidence,
    },
  ...
  const documentVerification = {
    ...
    authorizes_payment: false,
    dab: dabVerification,
    esign_evidence: (esignEvidence as any).evidence,
