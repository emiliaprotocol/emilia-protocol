...
function paymentSnapshot(args: unknown): Readonly<Record<string, any>> {
...
export function actionForCall(tool: string, action: string, args: any = {}): string {
  if (tool === 'release_payment') return bindExecutorAction(action, paymentSnapshot(args));
...
export function makeGuardedServer({ tool, perform = async (_name, args) => ({ ran: true, ...args }) }: {
  tool: string;
  perform?: (name: string, args: Readonly<Record<string, any>>) => any;
}): any {
    ...
    if (!gate) {
      gate = makeReceiptGate({
        action: req.match.tool === 'release_payment'
          ? (snapshot: any) => bindExecutorAction(req.action_type, snapshot)
          : req.action_type,
        allowInlineKey: true,
    ...
    try {
      res = await gate.run(receipt, { target }, async () => {
        if (isPayment) {
          ...
          let currentAction;
          try { currentAction = actionForCall(name, req.action_type, args); } catch {
            throw new PaymentArgumentsChanged();
          }
          if (currentAction !== action) throw new PaymentArgumentsChanged();
        }
        return { ...await perform(name, snapshot), action };
      });
    } catch (error) {
      if (error instanceof PaymentArgumentsChanged) return refuse('payment_arguments_changed');
      throw error;
    }
...
export async function runDemo({ title, tool, args, approver, agentLine }: { title: string; tool: string; args: any; approver: string; agentLine: string }): Promise<void> {
  ...
  show(res);
  if (res.status === 200) line(`     mock tool performed; receipt ${res.body.evidence.receipt_id} verified under demo trust pins`);
  await pause(900);
