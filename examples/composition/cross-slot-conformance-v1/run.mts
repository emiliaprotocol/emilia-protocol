...
type ResultStatus = 'pass' | 'fail' | 'not_evaluated' | 'unsupported' | 'indeterminate';
...
function artifact(value: RecordValue): RecordValue {
...
function slot(
  name: 'can' | 'who' | 'what' | 'audit',
  profile: string,
  nativePayload: RecordValue,
): RecordValue {
...
function buildBaseBundle(): RecordValue {
  const slots = {
  ...
  const names = Object.keys(slots) as Array<keyof typeof slots>;
  ...
  return {
    ...
    reporting: {
      ...
      reported_native_results: Object.fromEntries(
        Object.entries(slots).map(([name, entry]) => [name, entry.native_result]),
      ),
    },
  };
}
...
interface CaseDefinition {
  ...
  title: string;
  pair_id: string | null;
  variant: 'positive' | 'negative' | 'condition_removed';
  expected_terminal: ResultStatus;
  expected_check: string;
  expected_native_results: Record<string, ResultStatus>;
  expected_join_results: Record<string, ResultStatus>;
  bundle: RecordValue;
}

interface NegativeDefinition {
  id: string;
  title: string;
  expectedTerminal: ResultStatus;
  expectedCheck: string;
  mutate: (bundle: RecordValue) => void;
  repair?: (bundle: RecordValue) => void;
}

const NEGATIVES: NegativeDefinition[] = [
  {
    id: 'COMP-BIND-01',
...
function expectedNativeResults(bundle: RecordValue): Record<string, ResultStatus> {
  return Object.fromEntries(
    Object.entries(bundle.slots).map(([name, entry]: [string, any]) => [
      name,
      entry.native_result as ResultStatus,
    ]),
  );
}

function expectedJoinResults(
  expectedCheck: string,
  expectedTerminal: ResultStatus,
): Record<string, ResultStatus> {
  const expected = Object.fromEntries(
    JOIN_CHECK_IDS.map((id) => [
      id,
...
function caseDefinition(
  value: Omit<CaseDefinition, 'expected_native_results' | 'expected_join_results'>,
): CaseDefinition {
  return {
    ...value,
...
export function buildCatalog(): CaseDefinition[] {
...
function check(id: string, status: ResultStatus, detail: string): RecordValue {
  return { id, status, detail };
}

function verifyArtifactBytes(bundle: RecordValue): RecordValue {
  const failures: string[] = [];
  for (const [name, entry] of Object.entries(bundle.slots) as Array<[string, RecordValue]>) {
  ...
  return check(
    ...
    failures.length === 0 ? 'all native artifact bytes match their pinned digests'
      : `artifact digest mismatch: ${failures.join(',')}`,
  );
}

function verifySubjectBytes(bundle: RecordValue): RecordValue {
  ...
  return check(
    'subject.action_bytes',
    actual === bundle.subject.action_digest && contentMatches ? 'pass' : 'fail',
    'supplied action bytes must match both the visible action and pinned digest',
  );
}

function verifyDigestContext(bundle: RecordValue): RecordValue {
  const compatibleContext = (value: RecordValue): string => {
    const copy = clone(value);
    delete copy.representation;
    return canonical(copy);
  };
  const expected = compatibleContext(bundle.subject.digest_context);
  const incompatible = Object.values(bundle.slots).filter(
    (entry: any) => compatibleContext(entry.subject.digest_context) !== expected,
  );
  return check(
    'join.digest_context',
    incompatible.length === 0 ? 'pass' : 'indeterminate',
    incompatible.length === 0 ? 'all slots declare the same digest context'
      : 'at least one slot uses an incompatible profile or action projection',
  );
}

function verifyRepresentation(bundle: RecordValue): RecordValue {
  const expected = bundle.subject.digest_context.representation;
  const mismatch = Object.values(bundle.slots).some(
    (entry: any) => entry.subject.digest_context.representation !== expected,
  );
  return check(
    'join.digest_representation',
    mismatch ? 'fail' : 'pass',
...
function verifyCrossReferences(bundle: RecordValue): RecordValue {
  let mismatch = false;
  for (const entry of Object.values(bundle.slots) as RecordValue[]) {
  ...
  return check(
    'join.protected_cross_reference',
    mismatch ? 'fail' : 'pass',
    mismatch ? 'protected cross-reference does not identify supplied target bytes'
      : 'all protected cross-references match supplied target bytes',
  );
}

function verifyAdditionalBindingContext(bundle: RecordValue): RecordValue {
  const incomplete = Object.values(bundle.slots).flatMap(
    (entry: any) => entry.additional_bindings,
  ).some((binding: any) => typeof binding.purpose !== 'string'
    || typeof binding.context !== 'string'
    || typeof binding.digest !== 'string');
  return check(
    'join.additional_binding_context',
    incomplete ? 'fail' : 'pass',
    incomplete ? 'an additional binding is missing purpose, context, or digest'
      : 'additional bindings carry complete declared context',
  );
}

function verifyBindingSemantics(bundle: RecordValue): RecordValue {
  const bindings = Object.values(bundle.slots).flatMap(
    (entry: any) => entry.additional_bindings,
  );
  const required = bundle.policy.required_binding_purposes;
  const uninterpretedRequired = required.some((purpose: string) => {
    const matches = bindings.filter((binding: any) => binding.purpose === purpose);
    return matches.length === 0 || matches.every((binding: any) => binding.understood !== true);
  });
  return {
    ...check(
      'policy.binding_semantics',
      uninterpretedRequired ? 'unsupported' : 'pass',
      uninterpretedRequired
...
function verifyFieldBasis(bundle: RecordValue): RecordValue {
  const missing = Object.values(bundle.slots).some(
    (entry: any) => Object.values(entry.fields).some(
      (field: any) => typeof field.basis !== 'string' || field.basis.length === 0,
    ),
  );
  return check(
    'join.field_basis',
    missing ? 'indeterminate' : 'pass',
    missing ? 'joined field basis is absent' : 'joined fields declare their bases',
  );
}

function verifyFieldMapping(bundle: RecordValue, fieldBasis: RecordValue): RecordValue {
  ...
  const amountBases = [...new Set(
    Object.values(bundle.slots).map((entry: any) => entry.fields.amount.basis),
  )];
  if (amountBases.length <= 1) return check('join.field_mapping', 'pass', 'joined amount bases match');
  const mapped = bundle.mappings.some((mapping: any) => {
    const pair = new Set([mapping.from_basis, mapping.to_basis]);
    return amountBases.every((basis) => pair.has(basis));
  });
  return check(
    'join.field_mapping',
    mapped ? 'pass' : 'indeterminate',
    mapped ? 'incompatible bases have a pinned mapping' : 'incompatible bases lack a pinned mapping',
  );
}

function verifyResultSeparation(bundle: RecordValue): RecordValue {
  return check(
    'report.result_separation',
    bundle.reporting.native_results_separate === true ? 'pass' : 'fail',
    'native slot and cross-slot results must remain separately named',
  );
}

function verifyResultPreservation(bundle: RecordValue): RecordValue {
  return check(
    'report.result_preservation',
    bundle.reporting.composition_overrides_native === false ? 'pass' : 'fail',
    'composition must not upgrade, weaken, or overwrite a native result',
  );
}

function verifyExactActionJoin(bundle: RecordValue): RecordValue {
  const mismatch = Object.values(bundle.slots).some(
    (entry: any) => entry.subject.action_digest !== bundle.subject.action_digest,
  );
  return check(
    'join.exact_action',
    mismatch ? 'fail' : 'pass',
    mismatch ? 'populated slots do not identify the same exact action'
      : 'all populated slots identify the same exact action',
  );
}

function verifyNotEvaluatedPreservation(bundle: RecordValue): RecordValue {
  const mismatch = Object.entries(bundle.slots).some(([name, entry]: [string, any]) => (
    entry.native_result === 'not_evaluated'
    && bundle.reporting.reported_native_results[name] !== 'not_evaluated'
  ));
  return check(
    'report.not_evaluated_preservation',
    mismatch ? 'fail' : 'pass',
    mismatch ? 'not_evaluated native result was relabeled as a verifier failure'
      : 'not_evaluated native results remain not_evaluated',
  );
}

function verifyRequiredProfiles(bundle: RecordValue): RecordValue {
  const supported = new Set(bundle.policy.supported_profiles);
  const unknown = bundle.policy.required_profiles.filter((profile: string) => !supported.has(profile));
  return check(
    'policy.required_profile',
    unknown.length === 0 ? 'pass' : 'unsupported',
    unknown.length === 0 ? 'all required profiles are supported'
      : `unsupported required profiles: ${unknown.join(',')}`,
  );
}

function terminalStatus(results: RecordValue[]): ResultStatus {
  const statuses = results.map((entry) => entry.status as ResultStatus);
  for (const candidate of ['fail', 'unsupported', 'indeterminate', 'not_evaluated'] as ResultStatus[]) {
    if (statuses.includes(candidate)) return candidate;
  }
  return 'pass';
}

function divergenceFor(id: string, bundle: RecordValue): RecordValue {
  const slots = Object.entries(bundle.slots) as Array<[string, RecordValue]>;
  const base = (
    field: string,
    expected: unknown,
    actual: unknown,
    expectedBasis: unknown = null,
    actualBasis: unknown = null,
  ) => ({
    field,
  ...
  switch (id) {
    case 'bundle.artifact_bytes': {
      const mismatch = slots.find(([, entry]) => (
        sha256(Buffer.from(entry.artifact.bytes_b64u, 'base64url')) !== entry.artifact.sha256
      ));
      return base(
        `slots.${mismatch?.[0] ?? 'unknown'}.artifact.sha256`,
        mismatch?.[1].artifact.sha256,
        mismatch ? sha256(Buffer.from(mismatch[1].artifact.bytes_b64u, 'base64url')) : null,
        'declared artifact digest',
        'digest recomputed from supplied bytes',
      );
    }
    case 'subject.action_bytes':
      return base(
        'subject.action_digest',
        bundle.subject.action_digest,
        sha256(Buffer.from(canonical(bundle.subject.action))),
        canonical(bundle.subject.digest_context),
        canonical(bundle.subject.digest_context),
      );
    case 'join.digest_context': {
      ...
      return base(
        `slots.${mismatch?.[0] ?? 'unknown'}.subject.digest_context`,
        bundle.subject.digest_context,
        mismatch?.[1].subject.digest_context,
        'subject digest context',
        'slot digest context',
      );
    }
    case 'join.digest_representation': {
      ...
      return base(
        `slots.${mismatch?.[0] ?? 'unknown'}.subject.digest_context.representation`,
        expected,
        mismatch?.[1].subject.digest_context.representation,
        'subject digest representation',
        'slot digest representation',
      );
    }
    case 'join.protected_cross_reference': {
      const mismatch = slots.find(([, entry]) => (
        !bundle.slots[entry.protected_cross_reference.target_slot]
      ...
      return base(
        `slots.${mismatch?.[0] ?? 'unknown'}.protected_cross_reference.artifact_digest`,
        target ? bundle.slots[target]?.artifact.sha256 : null,
        mismatch?.[1].protected_cross_reference.artifact_digest,
        'supplied target artifact bytes',
        `protected reference to ${target ?? 'unknown target'}`,
      );
    }
    case 'join.additional_binding_context': {
      const owner = slots.find(([, entry]) => entry.additional_bindings.some(
        (binding: RecordValue) => typeof binding.purpose !== 'string'
          || typeof binding.context !== 'string'
          || typeof binding.digest !== 'string',
      ));
      const binding = owner?.[1].additional_bindings.find(
        (value: RecordValue) => typeof value.purpose !== 'string'
          || typeof value.context !== 'string'
          || typeof value.digest !== 'string',
      );
      return base(
        `slots.${owner?.[0] ?? 'unknown'}.additional_bindings`,
        ['purpose', 'context', 'digest'],
        binding,
        'required binding members',
        'supplied binding members',
      );
    }
    case 'policy.binding_semantics': {
      const required = bundle.policy.required_binding_purposes.find((purpose: string) => (
        slots.flatMap(([, entry]) => entry.additional_bindings)
          .filter((binding: RecordValue) => binding.purpose === purpose)
          .every((binding: RecordValue) => binding.understood !== true)
      ));
      return base(
        'policy.required_binding_purposes',
        'understood semantics',
        'present_uninterpreted',
        required,
        required,
      );
    }
    case 'join.field_basis': {
      const owner = slots.find(([, entry]) => Object.values(entry.fields).some(
        (field: any) => typeof field.basis !== 'string' || field.basis.length === 0,
      ));
      const fieldName = owner && Object.entries(owner[1].fields).find(
        ([, field]: [string, any]) => typeof field.basis !== 'string' || field.basis.length === 0,
      )?.[0];
      return base(
        `slots.${owner?.[0] ?? 'unknown'}.fields.${fieldName ?? 'unknown'}.basis`,
        'declared basis',
        null,
    ...
    case 'join.field_mapping': {
      ...
      return base(
        'slots.*.fields.amount.basis',
        'same basis or digest-pinned mapping',
        actual,
        'declared comparison basis',
        'supplied per-slot bases',
      );
    }
    ...
    case 'join.exact_action': {
      ...
      return base(
        `slots.${mismatch?.[0] ?? 'unknown'}.subject.action_digest`,
        bundle.subject.action_digest,
        mismatch?.[1].subject.action_digest,
        canonical(bundle.subject.digest_context),
        mismatch ? canonical(mismatch[1].subject.digest_context) : null,
      );
    }
    case 'report.not_evaluated_preservation': {
      const mismatch = slots.find(([name, entry]) => (
        entry.native_result === 'not_evaluated'
        && bundle.reporting.reported_native_results[name] !== 'not_evaluated'
      ));
      return base(
        `reporting.reported_native_results.${mismatch?.[0] ?? 'unknown'}`,
        'not_evaluated',
        mismatch ? bundle.reporting.reported_native_results[mismatch[0]] : null,
        'native slot result',
        'composition report',
      );
    }
    case 'policy.required_profile': {
      const supported = new Set(bundle.policy.supported_profiles);
      const unknown = bundle.policy.required_profiles.find((profile: string) => !supported.has(profile));
      return base(
        'policy.required_profiles',
        bundle.policy.supported_profiles,
        unknown,
        'implementation-supported profiles',
        'required profile',
      );
    }
    default:
...
export function evaluateCase(item: CaseDefinition): RecordValue {
  try {
    ...
    if (invalidVocabulary) throw new Error(`invalid result vocabulary: ${invalidVocabulary.status}`);
    const nativeResults = (Object.entries(item.bundle.slots) as Array<[string, RecordValue]>).map(
      ([name, entry]) => ({
        slot: name,
...
export function runSuite(): RecordValue {
  ...
  const reportResults: RecordValue[] = results.map<RecordValue>((result, index) => {
    const item = cases[index];
    const inputArtifactDigests = {
      action: item.bundle.subject.action_artifact.sha256,
      ...Object.fromEntries(
        Object.entries(item.bundle.slots).map(([name, entry]: [string, any]) => [
          name,
    ...
    return {
      ...
      native_results: result.native_results.map((entry: RecordValue) => ({
        ...entry,
        expected_result: item.expected_native_results[entry.slot],
      })),
      join_results: result.join_results.map((entry: RecordValue) => ({
        ...entry,
  ...
  const checks = cases.map((item, index) => {
    ...
    const nativeResultsMatch = result.native_results.every(
      (entry: RecordValue) => entry.native_result === entry.expected_result,
    );
    const joinResultsMatch = result.join_results.every(
      (entry: RecordValue) => entry.status === entry.expected_result,
    );
    return {
      id: item.id,
      pair_id: item.pair_id,
...
function emitArtifacts(run: RecordValue): void {
  ...
  const checksums = files.map(([name]) => (
    `${fileDigest(relative(ROOT, resolve(HERE, name))).slice(7)}  ${name}`
  )).join('\n');
...
function main(): void {
  ...
  process.stdout.write(
    `${run.report.passed ? 'CROSS-SLOT CONFORMANCE PASS' : 'CROSS-SLOT CONFORMANCE FAIL'} — `
