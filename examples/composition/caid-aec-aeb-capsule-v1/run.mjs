...
const CAPSULE_PUBLIC_KEY = crypto.createPublicKey(CAPSULE_PRIVATE_KEY);
...
function actionDefinitions() {
    ...
    if (!Array.isArray(registry.types))
        throw new Error('CAID action registry has no types array');
...
function nativeEvidence(id, actionDigestValue = PRIMARY.digest, acceptance = 'CURRENT') {
    return [
        {
...
function aebEvidence(id, options = {}) {
    return [
        {
...
function prior(id, custody = 'INVOKING', providerOutcome = 'INDETERMINATE', effectRelation = 'INDETERMINATE') {
    return {
        operation_id: `operation:${id}`,
        caid: PRIMARY.caid,
        normalized_action_digest: PRIMARY.digest,
...
function capsuleDisposition(kind, reasonCode) {
    if (kind === 'confirmed') {
        return {
            decision: 'accept',
            approver: 'human',
...
function buildCapsule(id, kind, reasonCode) {
    ...
    if (!capsuleId)
        throw new Error(`could not compute Capsule id for ${id}`);
    return {
        spec_version: capsuleWithoutId.spec_version,
        format_version: capsuleWithoutId.format_version,
        capsule_id: capsuleId,
        ...Object.fromEntries(Object.entries(capsuleWithoutId)
...
function hasFloat(value) {
    if (typeof value === 'number')
        return !Number.isSafeInteger(value);
    if (Array.isArray(value))
        return value.some(hasFloat);
    if (isRecord(value))
        return Object.values(value).some(hasFloat);
    return false;
}
function class1Finding(id, ok, detail) {
 ...
 */
export function verifyCapsuleClass1Candidate(statement) {
...
function expected(actionLinkage, evidenceSatisfaction, decision, admission, outcome, verifierStatus = 'PASS', reasonCodes = []) {
    return {
    ...
    return [
        ...
        },
        {
            ...
            aeb: aebCase('already_consumed_replay', {
                reservation: {
                    atomicity: 'local_atomic',
                    prior_operations: [prior('already_consumed_replay', 'TERMINAL', 'COMMITTED', 'OBSERVED_AS_REQUESTED')],
                    consumed_native_replay_units: [],
                },
            }),
            required_bindings: ['capsule.class1'],
...
function mapAdmission(item, aeb) {
    if (item.id === 'already_consumed_replay')
        return 'REFUSED_ALREADY_CONSUMED';
    if (item.id === 'timeout_before_dispatch')
        return 'REFUSED_TIMEOUT_BEFORE_DISPATCH';
    if (aeb.reservation === 'CONSUMED')
        return 'CONSUMED';
    if (aeb.decision === 'ADMIT')
        return 'ADMITTED';
    return 'REFUSED';
}
function mapOutcome(admission, aeb) {
    ...
    if (admission.startsWith('REFUSED'))
        return 'NONE';
    if (aeb.effect_relation === 'DIVERGED')
        return 'DIVERGENT';
    if (aeb.decision === 'INDETERMINATE' || aeb.effect_relation === 'INDETERMINATE') {
        return 'INDETERMINATE';
    }
    if (aeb.effect_relation === 'OBSERVED_AS_REQUESTED')
        return 'OBSERVED_AS_REQUESTED';
    return 'NONE';
}
function structuredRefusal(item, nativeResults, capsuleResult, unsupported) {
    return {
...
export function evaluateCase(item) {
    try {
        ...
        const unsupported = item.required_bindings.find((binding) => !['capsule.class1', 'capsule.class2'].includes(binding));
        if (unsupported)
            return structuredRefusal(item, nativeResults, capsuleResult, unsupported);
...
function verifyExpected(item, result) {
    ...
    const reasonsMatch = expectedReasons.every((reason) => result.reason_codes.includes(reason));
...
function main() {
    ...
    process.stdout.write(`${run.report.passed ? 'COMPOSITION VECTOR PASS' : 'COMPOSITION VECTOR FAIL'} — `
