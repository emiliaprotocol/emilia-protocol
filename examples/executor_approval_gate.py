...
"""executor_approval_gate — vendor-neutral irreversible-action gate for EMILIA.
...
Offline verification needs the verifier on the path:
    ...
    PYTHONPATH=packages/python-verify python examples/executor_approval_gate.py
...
logger = logging.getLogger("emilia.executor_approval_gate")
...
class VerifiedReceipt:
    ...
    #                                | "anchor_required" | "expired" | "not_yet_valid"
...
class EmiliaGuard:
    """Vendor-neutral guard for an agent executor.
    ...
    def __init__(
        ...
        max_receipt_age_s: Optional[int] = 900,
    ):
        ...
        self.max_receipt_age_s = max_receipt_age_s
    ...
    def guard(
        ...
        idem = f"executor-approval-{uuid.uuid4()}"
    ...
    #
    # ⚠ BLOCKS. Use this only in a worker / batch job, NEVER inside a live tool
...
def _parse_rfc3339_utc(value: Any) -> Optional[datetime]:
    """Parse an RFC 3339 instant and require an explicit UTC offset."""
...
# Explicit adapter mapping between the example tool name and the protocol-native
...
def _verify_evidence_offline(
    ...
    max_receipt_age_s: Optional[int] = None,
) -> VerifiedReceipt:
    ...
    Order (every step fails CLOSED):
      ...
      - signed issued_at outside freshness policy      -> "expired" / "not_yet_valid"
    ...
    # FRESHNESS: the relying party can impose a short validity window over the
    # signed `issued_at`. This does not trust a server status string and does not
    # rewrite the receipt; it evaluates signed evidence against local policy.
    # A missing/malformed timestamp fails closed whenever freshness is required.
    if max_receipt_age_s is not None:
        if isinstance(max_receipt_age_s, bool) or max_receipt_age_s <= 0:
            return VerifiedReceipt(
                verified=False,
                status="invalid_freshness_policy",
                detail="max_receipt_age_s must be a positive integer",
                checks=checks,
                raw=evidence,
            )
        now = (
            _parse_rfc3339_utc(evaluated_at)
            if evaluated_at is not None
            else datetime.now(timezone.utc)
        )
        issued_at = _parse_rfc3339_utc(_signed_payload(document).get("issued_at"))
        if now is None or issued_at is None:
            return VerifiedReceipt(
                verified=False,
                status="invalid_freshness",
                detail="freshness policy requires signed RFC 3339 issued_at and a valid evaluation time",
                checks=checks,
                raw=evidence,
            )
        age_s = (now - issued_at).total_seconds()
        if age_s < 0:
            return VerifiedReceipt(
                verified=False,
                status="not_yet_valid",
                detail="signed receipt issued_at is in the future",
                checks=checks,
                raw=evidence,
            )
        if age_s > max_receipt_age_s:
            return VerifiedReceipt(
                verified=False,
                status="expired",
                detail=f"signed receipt is older than the {max_receipt_age_s}s relying-party window",
                checks=checks,
                raw=evidence,
            )
...
# ── OpenAI-compatible tool schema. Register with the proposing agent. ────────
# The model may propose this tool instead of executing an irreversible action;
...
def dispatch_emilia_tool(
    ...
    """Run the gate for one proposed irreversible tool call.
    ...
    if not wait:
        ...
        return {
            ...
            "reason": "named human signoff required — awaiting signed decision evidence",
            "receipt_id": res.receipt_id,
...
async def resume_release_large_payment(
    ...
    return {
        "status": "executed",
        "executed": True,
        "reason": "authorization receipt VERIFIED offline (operator signer pinned, request bound, fresh, single-use)",
        "receipt_id": receipt_id,
...
# ── Wiring into an OpenAI-compatible proposing-agent loop ──────────────────
# The model may propose the approval tool, but the executor remains responsible
# for complete mediation. The irreversible callback must be reachable only from
# the verified resume path, never from a model-selected tool branch.
#
...
#   # result["proceed"] is true ONLY if the pinned authorization receipt VERIFIED OFFLINE.
#
...
if __name__ == "__main__":
    ...
    #     python examples/executor_approval_gate.py
