...
import {
  createGate,
  createEg1Harness,
  evaluateOriginLabelAssertions,
  hashCanonical,
  MemoryConsumptionStore,
  ORIGIN_LABELS_VERSION,
...
const CAID_DEFINITIONS = JSON.parse(
  readFileSync(resolve(HERE, 'fixtures/caid-definitions.json'), 'utf8'),
).types;
...
function toCents(amount) {
  ...
  if (!m) throw new Error(`fixture amount is not a 2-decimal string: ${amount}`);
  const sign = m[1] === '-' ? -1 : 1;
...
function resolveItem(item) {
  ...
  if (item.kind === 'deposit_in_transit' || item.kind === 'outstanding_check') {
    ...
    if (ledger) evidence.push(`gl:${item.account_id}:${item.id}`);
    else reasons.push(`no_ledger_entry:${item.account_id}:${item.id}`);
    if (cleared) evidence.push(`bank-subsequent:${item.account_id}:${item.id}`);
    else reasons.push(`no_subsequent_bank_line:${item.account_id}:${item.id}`);
    ...
    if (bankLine) evidence.push(`bank-period:${item.account_id}:${item.id}`);
    else reasons.push(`no_period_bank_line:${item.account_id}:${item.id}`);
    ...
    if (receiving) evidence.push(`bank-subsequent:${item.account_id}:${item.id}`);
    else reasons.push(`no_subsequent_bank_line:${item.account_id}:${item.id}`);
    if (sourceLedger || sourceBank) evidence.push(`source-document:${sourceId}:${item.id}`);
    else reasons.push(`no_counterpart_document:${sourceId}:${item.id}`);
  } else {
...
export async function runDemo() {
  ...
  //    material equals the authorized one, which is the join.
  {
    ...
    const outcome = await procedureGate.run(
      { selector: { ...PROCEDURE_SELECTOR }, receipt, observedAction: { ...PROCEDURE_ACTION, accounts: [...PROCEDURE_ACTION.accounts] } },
      async () => {
    ...
    if (!outcome.ok || tieout === null) throw new Error('through-case did not admit');
    ...
    const replay = await procedureGate.run(
      { selector: { ...PROCEDURE_SELECTOR }, receipt, observedAction: { ...PROCEDURE_ACTION, accounts: [...PROCEDURE_ACTION.accounts] } },
      async () => {
        procedureExecutions += 1;
    ...
    const caidRecheck = verifyCaid(
      { ...PROCEDURE_ACTION, accounts: [...PROCEDURE_ACTION.accounts] },
      procedureCaid,
      { definitions: CAID_DEFINITIONS },
    );
    cases.push({
      ...
      execution: {
        ...
        bound_to_admitted_decision:
          typeof outcome.execution?.authorizes_decision === 'string'
  ...
  //    self-disposition: the sign-off boundary demands a ceremony.
  {
    ...
    const agentAttempt = await signoffGate.run(
      { selector: { ...DISPOSITION_SELECTOR }, receipt: null, observedAction: { ...DISPOSITION_ACTION } },
      async () => {
  ...
  //    cross-path consistency check refuses the upgrade.
  {
    ...
    cases.push({
      ...
      item_status_after: requireFound(
        settledTieout.items.find((i) => i.id === 'TRF-0912'),
        'TRF-0912',
      ).status,
  ...
  //    scope Y, and the CAID no longer recomputes.
  {
    ...
    const outcome = await procedureGate.run(
      { selector: { ...PROCEDURE_SELECTOR }, receipt, observedAction: substituted },
      async () => {
        procedureExecutions += 1;
  ...
  //    disposition records the exception, it does not clear it.
  {
    ...
    const outcome = await signoffGate.run(
      { selector: { ...DISPOSITION_SELECTOR }, receipt, observedAction: { ...DISPOSITION_ACTION } },
      async () => {
        dispositionExecutions += 1;
        return { dispositioned: true };
      },
    );
    if (!outcome.ok) throw new Error('sign-off ceremony did not admit');
    dispositions.push({
  ...
  let committedWorkpaper: string | null = null;
  try {
...
function printDemo(result) {
  ...
  for (const [index, c] of result.cases.entries()) {
    ...
    console.log(`   ${c.title}`);
    if (c.authorization) console.log(`   authorization: ${c.authorization.verdict}`);
    if (c.origin_labels) console.log(`   origin labels: ${c.origin_labels.verdict} (${c.origin_labels.label_count} labeled paths)`);
    if (c.honest_derivation) console.log(`   honest derivation: ${c.honest_derivation.verdict} (${c.honest_derivation.reason})`);
    if (c.laundered_relabel) console.log(`   laundered relabel: ${c.laundered_relabel.verdict} (${c.laundered_relabel.reason})`);
    if (c.admission) console.log(`   admission: ${c.admission.verdict}${c.admission.reason ? ` (${c.admission.reason})` : ''}`);
    if (c.admission?.replay_refused) console.log(`   replay of consumed receipt: refused (${c.admission.replay_refused.reason})`);
    if (c.execution) console.log(`   execution: ${c.execution.verdict}`);
    if (c.signoff) console.log(`   sign-off: ${c.signoff.verdict} by ${(c.signoff.approvers ?? []).join(', ')}`);
    if (c.finalization) console.log(`   finalization: ${c.finalization.verdict}${c.finalization.reason ? ` (${c.finalization.reason})` : ''}`);
    if (c.agent_self_disposition) console.log(`   agent self-disposition: ${c.agent_self_disposition.verdict} (${c.agent_self_disposition.reason})`);
    if (c.item_status_after) console.log(`   TRF-0912 status after: ${c.item_status_after}`);
