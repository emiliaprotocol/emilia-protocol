...
test('audit-procedure evidence demo: composition, refusals, and deterministic workpaper', async () => {
    ...
    assert.equal(laundering.honest_derivation.reason, 'origin_trust_floor_violation:/bank/RESV-003/subsequent/TRF-0912');
    assert.equal(laundering.laundered_relabel.verdict, 'refused');
    assert.equal(laundering.laundered_relabel.reason, 'value_origin_conflict:/bank/RESV-003/subsequent/TRF-0912');
