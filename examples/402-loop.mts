...
interface IssueReceiptOptions {
  tamper?: boolean;
}

function issueReceipt(action: string, { tamper = false }: IssueReceiptOptions = {}): any {
...
async function callAction(doc: any): Promise<any> {
  if (DEMO_URL) {
...
const show = (r: any): void => line(`   <- ${r.status} ${r.status === 402 ? r.body.title : 'OK'}${r.body.rejected ? ` (${r.body.rejected.reason})` : ''}`);
