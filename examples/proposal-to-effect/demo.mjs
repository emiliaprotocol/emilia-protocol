...
const aebStore = {
    ...
    async reserve(key, replayKeys) {
        if (states.has(key))
            return false;
        if (replayKeys.some((replayKey) => nativeReplay.has(replayKey)))
...
const consequenceStore = {
    ...
    async reserve(binding) {
        if (attempts.has(binding.attempt_id))
            return { reserved: false, reason: 'consequence_attempt_conflict' };
    ...
    async read(binding) {
        ...
        if (!row || row.binding.tenant_id !== binding.tenant_id
            ...
            || row.binding.request_digest !== binding.request_digest)
            return null;
...
const controller = createProposalToEffect({
    ...
    aeb: {
        ...
        currentStatusResolver: ({ leg }) => ({ artifact_ref: leg.artifact_ref }),
        statusVerifier: ({ status_artifact, expected }) => ({
