...
} from '../../packages/gate/reports/external-verification.js';
...
try {
  ...
  const missing = new Map(good); missing.delete([...good.keys()][0]);
  assert.throws(() => compareSuiteResults(suite, missing), (e) => e instanceof Refusal && e.reason === 'missing_vector_ids');
  const extra = new Map(good); extra.set('vector_that_does_not_exist', true);
