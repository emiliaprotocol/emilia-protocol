...
export function loadResults(resultsPath) {
  ...
  for (let i = 0; i < json.length; i++) {
    ...
    const value = entry[kind as keyof typeof entry];
...
export function resolveResultsFiles(specs: any) {
  const files: string[] = [];
 ...
 */
function parseRunnerOutput(output: string, suites: any): any[] {
  const perSuite: any[] = [];
  let cursor = 0;
...
function main() {
  ...
  if (modeA) {
    ...
    suiteEntries.sort((a: any, b: any) => a.suite.file.localeCompare(b.suite.file));
