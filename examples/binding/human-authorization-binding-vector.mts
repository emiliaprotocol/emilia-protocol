...
export function verifyBinding(hostRec: any, art: any, policy: any = {}) {
  const checks = {
    b1_digest: false,
    artifact_sig: false,
    b2_action: false,
  ...
  try {
    ...
    const enrollmentSignatureValid = crypto.verify(
      null,
      canon(enrolled.payload),
      enrollmentKey,
      Buffer.from(enrolled.sig, 'base64url'),
    );
  ...
  try {
    ...
    const roleSignatureValid = crypto.verify(
      null,
      canon(roles.payload),
      roleKey,
      Buffer.from(roles.sig, 'base64url'),
    );
...
if (verifyBinding(host, tampered, acceptancePolicy).verified) throw new Error('B1 not enforced');
...
if (verifyBinding(hostOther, other, acceptancePolicy).verified) throw new Error('B2 not enforced');
...
const collapsedRoleResult = verifyBinding(
  { ...host, principal_role_evidence: collapsedRoleEvidence },
  artifact,
  acceptancePolicy,
);
...
console.error(
  'BINDING VECTOR OK — B1 digest, B2 action, B3 discovery!=trust, '
