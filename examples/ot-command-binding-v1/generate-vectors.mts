...
function mutation(action: any, field: string, value: unknown) {
    ...
    vectors: [
      ...
      },
      {
        ...
        profile_scope: {
          ...
          select_operate_requirement:
            'A future profile must bind SELECT and OPERATE to one authorization and enforce the arm timer in one state machine.',
        },
        no_ack_variant: {
          action: dnp3NoAck,
...
if (process.argv[1] && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
  ...
  if (process.argv.includes('--write')) {
    ...
    if (actual !== expected) throw new Error('OT command-binding vectors are stale; run with --write');
