...
import { TRANSPORT_PROFILES, commandDigest, decodeDnp3ControlRelay, decodeModbusWriteRegister, dnp3ControlRelayAction, encodeDnp3ControlRelay, encodeModbusWriteRegister, encodeOpcuaCall, extractOpcuaAuthorization, modbusWriteRegisterAction, } from './commands.mjs';
import { EXACT_COMMANDS, createEnforcementPoint, runOtCommandBindingLab, } from './scenario.mjs';
const LAB = await runOtCommandBindingLab();
const [DERIVATION, AUTHORIZED, DRIFT, UNRESOLVED, SPENT] = LAB.scenes;
const sceneById = (id) => LAB.scenes.find((scene) => scene.id === id);
// ---------------------------------------------------------------------------
...
test('a DNP3 CROB digest moves with its index and its control code', () => {
    ...
    assert.notEqual(commandDigest(dnp3ControlRelayAction({ ...link, index: 8, controlCode: 'LATCH_ON' })), commandDigest(base));
    assert.notEqual(commandDigest(dnp3ControlRelayAction({ ...link, index: 7, controlCode: 'LATCH_OFF' })), commandDigest(base));
});
...
test('the two out-of-band transports carry no authorization on the wire', () => {
    const outOfBand = AUTHORIZED.transports.filter((entry) => entry.binding_mode === 'out-of-band-digest');
    ...
    const inline = AUTHORIZED.transports.filter((entry) => entry.binding_mode === 'inline');
...
test('a valve address off by one never reaches the device', () => {
    const registerDrift = DRIFT.cases.find((item) => item.transport === 'modbus-tcp' && item.drifted_field === 'register');
...
test('a guarded command with no authorization at all is refused before the device', async () => {
    ...
    const result = await point.gate.run({ selector: { ...point.selector }, receipt: null, observedAction: point.action }, async () => point.device.apply(point.action));
...
test('an inline OPC-UA envelope is not a trust shortcut around the action binding', () => {
    const opcua = sceneById('drift-refused').cases.filter((item) => item.transport === 'opc-ua');
...
test('the lab reports one device command per enforcement point in the authorized scene', () => {
    const total = AUTHORIZED.transports.reduce((sum, entry) => sum + entry.device_command_count, 0);
