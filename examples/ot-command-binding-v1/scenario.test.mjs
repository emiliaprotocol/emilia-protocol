...
test('a DNP3 CROB digest binds the complete control octet, timing, count, and function', () => {
    ...
    assert.notEqual(commandDigest(dnp3ControlRelayAction({ ...link, index: 8, applicationFunction: 5, controlOctet: 0x03, operationCount: 1, onTimeMs: 0, offTimeMs: 0 })), commandDigest(base));
    assert.notEqual(commandDigest(dnp3ControlRelayAction({ ...link, index: 7, applicationFunction: 5, controlOctet: 0x43, operationCount: 1, onTimeMs: 0, offTimeMs: 0 })), commandDigest(base));
    for (const variant of [
...
test('a valve address off by one never reaches the device', () => {
    const registerDrift = DRIFT.cases.find((item) => item.transport === 'modbus-tcp' && item.drifted_field === 'protocol_address');
