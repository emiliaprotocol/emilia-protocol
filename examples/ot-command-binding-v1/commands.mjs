 ...
 */
export function modbusWriteRegisterAction({ site, device, unitId, register, value, }) {
...
export function dnp3ControlRelayAction({ site, device, outstationAddress, index, controlCode, count = 1, }) {
...
const DNP3_CONTROL_CODE_NAMES = Object.freeze(Object.fromEntries(Object.entries(DNP3_CONTROL_CODES).map(([name, code]) => [code, name])));
...
export function encodeDnp3ControlRelay(action, { sequence = 0 } = {}) {
    ...
    if (controlCode === undefined)
        throw new TypeError(`unsupported DNP3 control code ${action.control_code}`);
...
export function decodeDnp3ControlRelay(hex, link) {
    ...
    if (fragment.readUInt8(1) !== DNP3_FC_DIRECT_OPERATE)
    ...
    if (controlCode === undefined)
        throw new TypeError('unrecognized DNP3 control code');
