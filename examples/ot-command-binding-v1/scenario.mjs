 ...
 */
import { createGate, createEg1Harness, createEvidenceLog, createDurableConsumptionStore, createMemoryBackend, verifyExecutionBinding, } from '../../packages/gate/index.js';
...
import { BOUND_FIELDS, OT_COMMAND_BINDING_VERSION, TRANSPORT_PROFILES, commandDigest, commandDigestHex, decodeDnp3ControlRelay, decodeModbusWriteRegister, decodeOpcuaCall, dnp3ControlRelayAction, encodeDnp3ControlRelay, encodeModbusWriteRegister, encodeOpcuaCall, extractOpcuaAuthorization, modbusWriteRegisterAction, opcuaCallMethodAction, } from './commands.mjs';
...
function actionPackEntry(transport, action = EXACT_COMMANDS[transport]) {
...
const TRANSPORT_OPS = Object.freeze({
    'modbus-tcp': {
        encode: (action, opts = {}) => encodeModbusWriteRegister(action, opts),
        wireOf: (encoded) => encoded.hex,
        decode: (wire, link) => decodeModbusWriteRegister(wire, link),
        inlineAuthorization: () => null,
    },
    dnp3: {
        encode: (action, opts = {}) => encodeDnp3ControlRelay(action, opts),
        wireOf: (encoded) => encoded.hex,
        decode: (wire, link) => decodeDnp3ControlRelay(wire, link),
        inlineAuthorization: () => null,
    },
    'opc-ua': {
        encode: (action, opts = {}) => encodeOpcuaCall(action, opts),
        wireOf: (encoded) => encoded.request,
        decode: (wire, link) => decodeOpcuaCall(wire, link),
        inlineAuthorization: (wire) => extractOpcuaAuthorization(wire),
    },
});
/**
 ...
 */
export function createOutOfBandAuthorizationIndex({ defaultRequestContext = null } = {}) {
    const byAttempt = new Map();
    return {
        hold(action, receipt, { attemptRef = receipt?.payload?.receipt_id, requestContext = defaultRequestContext, } = {}) {
            const digest = commandDigestHex(action);
            requireAttemptRef(attemptRef);
            requireRequestContext(requestContext);
            if (byAttempt.has(attemptRef))
                throw new TypeError('attempt reference already held');
        ...
        lookup(attemptRef, digest, requestContext = defaultRequestContext) {
            if (!attemptRef)
                return null;
...
function createDevice() {
    ...
    return {
        ...
        async apply(action) {
            ...
            if (quiet) {
                const error = new Error('plc_response_timeout');
 ...
 */
export function createEnforcementPoint(transport, { startMs = T0, action = EXACT_COMMANDS[transport], } = {}) {
    ...
    return {
        ...
        action,
        advanceSeconds(seconds) { clock.ms += seconds * 1000; },
        ...
        async storeState(consumptionKey) { return backend.get(consumptionKey); },
    };
}
/**
 ...
 */
async function dispatch(point, { action, receipt, attemptRef = receipt?.payload?.receipt_id ?? null, attemptContext = point.attempt_context, encodeOptions = {}, }) {
    ...
    let terminal = null;
    try {
        outcome = await point.gate.run({ selector: { ...point.selector }, receipt: presented, observedAction }, async () => {
            const result = await point.device.apply(observedAction);
            if (point.transport === 'dnp3' && observedAction.application_function === 6) {
                const error = new Error('dnp3_direct_operate_no_ack');
...
function driftVariants(transport) {
...
function sceneCanonicalDerivation() {
    const transports = [];
    ...
    for (const transport of Object.keys(EXACT_COMMANDS)) {
        ...
        const drift = driftVariants(transport).map((variant) => {
...
// Scene 2 — authorized
// ---------------------------------------------------------------------------
async function sceneAuthorized() {
    const transports = [];
    for (const transport of Object.keys(EXACT_COMMANDS)) {
        const point = createEnforcementPoint(transport);
        const authorization = point.authorize();
        ...
        transports.push({
            ...
            execution_binds_authorization: result.execution?.authorizes_decision === result.packet?.summary?.decision_hash,
...
// ---------------------------------------------------------------------------
async function sceneDriftRefused() {
    const cases = [];
    for (const transport of Object.keys(EXACT_COMMANDS)) {
...
// ---------------------------------------------------------------------------
async function sceneUnresolvedAfterDispatch() {
    ...
    const executionRecords = point.evidenceLog.all().filter((record) => record.kind === 'execution');
    const indeterminateRecord = executionRecords.find((record) => record.outcome === 'indeterminate') ?? null;
...
async function sceneSpentOnce() {
    ...
    const ageSecondsAtReplay = Math.round((spentPoint.clock.ms - Date.parse(spent.payload.created_at)) / 1000);
    ...
    return {
        ...
        stale_but_unspent: {
            max_age_sec: MAX_AGE_SEC,
            receipt_id: staleId,
            age_seconds_at_use: Math.round((stalePoint.clock.ms - Date.parse(stale.payload.created_at)) / 1000),
...
export async function runOtCommandBindingLab() {
    const scenes = [
    ...
    return Object.freeze({
        ...
        scenario: 'One exact-action authorization bound to industrial control commands across three transports with different envelope capacity.',
        transports: TRANSPORT_PROFILES,
        scenes,
        invariant: 'The authorization binds to the canonical digest of the exact command. Where the wire can carry it, it rides along; where it cannot, it is held out of band and keyed to that same digest.',
    });
}
