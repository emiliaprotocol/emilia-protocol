 ...
 */
export const DNP3_CONTROL_FLAGS: Record<string, number> = Object.freeze({
  CLEAR: 0x20,
  QUEUE_OBSOLETE: 0x10,
});
...
export const DNP3_TRIP_CLOSE_CODES: Record<string, number> = Object.freeze({
  NUL: 0x00,
  CLOSE: 0x40,
  TRIP: 0x80,
  RESERVED: 0xc0,
 ...
 */
export function commandDigestHex(action: any): string {
  return hashCanonical(action);
}

/** Display form of the same digest. */
export function commandDigest(action: any): string {
  return `sha256:${commandDigestHex(action)}`;
}

function requireSafeInt(value: any, name: string, min: number, max: number): number {
...
function requireString(value: any, name: string): string {
...
export function opcuaCallMethodAction({
  site,
  device,
  objectId,
  methodId,
 ...
 */
export function encodeModbusWriteRegister(action: any, { transactionId = 1 }: any = {}): any {
...
const DNP3_FRAGMENT_OCTETS =
  DNP3_APPLICATION_HEADER_OCTETS + DNP3_OBJECT_HEADER_OCTETS + DNP3_CROB_OCTETS;

function requireDnp3ControlOctet(value: any): number {
 ...
 */
export function encodeDnp3ControlRelay(action: any, { sequence = 0 }: any = {}): any {
 ...
 */
export function encodeOpcuaCall(action: any, { receipt = null, requestHandle = 1 }: any = {}): any {
  if (action?.transport !== 'opc-ua') throw new TypeError('encodeOpcuaCall requires an opc-ua action');
  const request = {
    service: 'Call',
    request_header: {
...
export function decodeOpcuaCall(request: any, link: any): any {
  if (request?.service !== 'Call') throw new TypeError('unexpected OPC-UA service');
  ...
  if (!call || !argument) throw new TypeError('malformed OPC-UA Call request');
...
export function extractOpcuaAuthorization(request: any): any {
