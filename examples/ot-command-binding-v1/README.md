...
held out of band. A conduit-established attempt reference selects one held
authorization, and the holder releases it only when its canonical action digest
matches the command decoded from the wire. The digest is not secret and is not
the request-instance key. That distinction lets two authorizations for the same
exact action coexist. **The gate authorizes the digest; the wire carries only
the native command.**
...
- The transport encoders and decoders are minimal models written for this
  ...
  protocol specification. A deployment can render its own site-specific
  operator label for a protocol address, but that label is not the signed wire
  field.
