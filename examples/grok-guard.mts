 ...
 *   node examples/grok-guard.mts                            # pay an $82k invoice → needs signoff
 *   node examples/grok-guard.mts "refund $30 to order 8812" # low-risk → flows freely
 *
 * (A harmless MODULE_TYPELESS warning may print — Node auto-detecting the engine
 *  as ESM. Ignore it, or run with `node --no-warnings examples/grok-guard.mts`.)
 */
...
function toGuardInput(name: string, args: any): any {
...
async function humanSignoff(
  name: string,
  args: any,
  reasons: string[]
): Promise<{ approved: boolean; signer?: string }> {
...
function execute(name: string, args: any): any {
...
async function callGrok(messages: any[]): Promise<any> {
...
async function main(): Promise<void> {
  ...
  const messages: any[] = [
    { role: 'system', content: 'You are an autonomous treasury agent. Use the tools to complete the task; pay the full amount due.' } as any,
  ...
  for (let turn = 0; turn < 8; turn++) {
    ...
    for (const tc of msg.tool_calls) {
      ...
      if (!guardInput) {
        ...
        const d = evaluateGuardPolicy(guardInput as any);
