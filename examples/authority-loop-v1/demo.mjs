...
import { CAPABILITY_CAID_SCOPE_PROFILE, createDefaultActionRiskManifest, createEg1Harness, createGate, createMemoryCapabilityStore, createReceiptProgramKernel, createRuntimeMonitor, mintCapabilityReceipt, verifyReceiptProgramCertificate, } from '../../packages/gate/index.js';
...
function resolveCaid(observed) {
    ...
    if (!computed.caid)
        throw new Error(`receipt_program:mandate_refused:${computed.refusals?.join(',') ?? 'unknown reason'}`);
...
function expect(condition, message) {
    if (!condition)
        throw new Error(`authority loop expectation failed: ${message}`);
}
...
const allowRun = await kernel.run(bookingRequest(flight340, caid340, unattended.capabilityReceipt, unattended.secret, 'book-flight-340'), airline);
...
const askRefusal = await kernel.run(bookingRequest(flight620, caid620, unattended.capabilityReceipt, unattended.secret, 'book-flight-620'), airline);
expect(!askRefusal.ok && askRefusal.outcome === 'refused' && askRefusal.reason === 'budget_exceeded', `scene 2 refusal ${askRefusal.outcome}: ${askRefusal.reason}`);
...
const approvedRun = await kernel.run(bookingRequest(flight620, caid620, approval620.capabilityReceipt, approval620.secret, 'book-flight-620'), airline);
...
const replay = await kernel.run(bookingRequest(flight620, caid620, approval620.capabilityReceipt, approval620.secret, 'book-flight-620'), airline);
expect(!replay.ok && replay.outcome === 'refused' && replay.reason === 'operation_already_committed', `scene 2 replay ${replay.outcome}: ${replay.reason}`);
...
const refuseRun = await kernel.run(bookingRequest(alcoholPurchase, agentClaimedCaid, unattended.capabilityReceipt, unattended.secret, 'buy-alcohol-89'), airline);
expect(!refuseRun.ok && refuseRun.outcome === 'refused'
    && refuseRun.reason === 'mandate_refused:unknown_action_type', `scene 3 refusal ${refuseRun.outcome}: ${refuseRun.reason}`);
expect(verifyCertificate(refuseRun).ok, 'scene 3 refusal certificate verification');
expect(capabilityStore.getState('cap_mandate_unattended').consumed_amount === beforeRefusal, 'scene 3 must not consume any budget');
...
const indeterminateRun = await kernel.run(bookingRequest(flight120, caid120, unattended.capabilityReceipt, unattended.secret, 'book-flight-120'), async (_authorization, _operation) => {
    providerCalls += 1;
...
expect(!indeterminateRun.ok && indeterminateRun.outcome === 'indeterminate'
    && indeterminateRun.reason === 'effect_indeterminate', `scene 4 outcome ${indeterminateRun.outcome}: ${indeterminateRun.reason}`);
...
expect(capabilityStore.getOperation('bk_2026_0120').outcome === 'indeterminate', 'scene 4 operation recorded as indeterminate');
...
const blindRetry = await kernel.run(bookingRequest(flight120, caid120, unattended.capabilityReceipt, unattended.secret, 'book-flight-120'), airline);
expect(!blindRetry.ok && blindRetry.outcome === 'refused' && blindRetry.reason === 'operation_already_committed', `scene 4 retry ${blindRetry.outcome}: ${blindRetry.reason}`);
