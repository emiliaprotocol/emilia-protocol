...
const edVerify = (spkiB64u) => (bytes, sig) => {
  try { return crypto.verify(null, bytes, pubKey(spkiB64u), Buffer.from(sig, 'base64url')); } catch { return false; }
};
const es256Verify = (spkiB64u) => (bytes, sig) => {
  try { return crypto.verify('sha256', bytes, { key: pubKey(spkiB64u), dsaEncoding: 'ieee-p1363' }, Buffer.from(sig, 'base64url')); } catch { return false; }
};
...
function checkBinding(binding, leg, legDigestHex, verify) {
  if (binding?.['@type'] !== 'EP-CAID-BINDING-v1' || binding.leg !== leg) return null;
  const legD = hexOf(binding.leg_digest);
  const caid = hexOf(binding.caid);
  if (!legD || !caid || legD !== legDigestHex) return null;
...
export const crosswalkVerifiers = {
  ...
  'ap2-cart-mandate': (ev) => {
    try {
      ...
      if (header.alg !== 'ES256' || !es256Verify(PINNED.ap2)(Buffer.from(parts[0] + '.' + parts[1], 'utf8'), parts[2])) return NO;
      const checkoutDigestHex = sha256hex(canon(ev.cart));
      if (payload.cart_hash !== 'sha256:' + checkoutDigestHex) return NO; // mandate must carry the presented cart's own digest
  ...
  'scitt-statement': (ev) => {
    try {
      ...
      if (ev.statement.protected?.alg !== 'EdDSA'
          || !edVerify(PINNED.scitt)(Buffer.from(canon(unsigned), 'utf8'), signature)) return NO;
      const caid = checkBinding(ev.binding, 'scitt-statement', sha256hex(canon(ev.statement)), edVerify(PINNED.scitt));
      if (!caid || ev.statement.protected.subject !== 'sha256:' + caid) return NO; // subject and binding must name the same action
 ...
 * @param {any} [action]
 */
export function buildCrosswalkChain({ epLeg, ap2Leg, scittLeg }: any, action: any = ACTION) {
  const components: any[] = [];
  if (epLeg) components.push({ type: 'ep-action', label: 'EP canonical action digest', evidence: epLeg });
  if (ap2Leg) components.push({ type: 'ap2-cart-mandate', label: 'AP2-shaped cart mandate', evidence: ap2Leg });
  if (scittLeg) components.push({ type: 'scitt-statement', label: 'SCITT-style signed statement', evidence: scittLeg });
...
function show(title, res, expectAllow) {
  console.log(`\n${C.b}${title}${C.x}`);
  for (const c of res.components) console.log(`  ${c.valid && c.bound ? C.g + '✓' : C.r + '✗'}${C.x} ${c.label} ${C.d}(${c.type})${c.reason ? ' — ' + c.reason : ''}${C.x}`);
  ...
  if (!res.allow) res.reasons.forEach((x) => console.log(`    ${C.d}· ${x}${C.x}`));
  if (!asExpected) process.exitCode = 1;
}
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  console.log(`  ${C.d}Leg A native digest (EP)         ${C.x} ${(hexOf(legA.binding.leg_digest) as string).slice(0, 32)}… ${C.d}== CAID${C.x}`);
  console.log(`  ${C.d}Leg B native digest (AP2-shaped) ${C.x} ${(hexOf(legB.binding.leg_digest) as string).slice(0, 32)}… ${C.d}(its own cart form)${C.x}`);
  console.log(`  ${C.d}Leg C native digest (SCITT-style)${C.x} ${(hexOf(legC.binding.leg_digest) as string).slice(0, 32)}… ${C.d}(statement digest)${C.x}`);
