...
// Generated from run-lit.mts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
...
const litKeyId = (publicKey) => `sha256:${sha256hex(publicKey.export({ type: 'spki', format: 'der' }))}`;
...
function verifyPresentation(evidenceBytes, { pinnedKeys, actionInFront, expectedNonce, now, maxAgeMs }) {
    ...
    try {
        ...
        return { valid: false, reason: 'payload_unparseable' };
    }
    ...
    if (!publicKey)
        return { valid: false, reason: 'key_id_not_pinned' };
    ...
    const ageMs = (new Date(now).getTime()) - (new Date(claims.presented_at).getTime());
