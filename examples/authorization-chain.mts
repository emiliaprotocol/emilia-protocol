...
function approveOnDevice({ role, approver, issuedAt }: { role: string; approver: string; issuedAt: string }): any {
...
function mintPermit(actionDigestStr: string): any {
...
const permitVerifier = (ev: any): any => {
  let ok = false;
  try {
...
// ── Build the chain ──────────────────────────────────────────────────────────
const makeChain = (permitDigest: string): any => ({
...
const noHuman = makeChain('sha256:' + ACTION_HASH); noHuman.components = noHuman.components.filter((c) => c.type !== 'ep-quorum');
