...
import { ACTA_COMPONENT_TYPE, PAYMENT_RELEASE_MAPPING_PROFILE, PAYMENT_RELEASE_MAPPING_PROFILE_HASH, artifactDigest, computeActaActionRef, mapPaymentReleaseCaid, } from './acta-profile.mjs';
...
export function signActaPayload(payload, privateKey) {
    return {
        ...
        signature: {
            ...
            sig: crypto.sign(null, Buffer.from(canonicalize(payload), 'utf8'), privateKey).toString('hex'),
        },
    };
}
...
export async function buildActaEpFixture() {
    ...
    if (!mapped.ok)
        throw new Error(`CAID mapping failed: ${mapped.reasons.join(', ')}`);
    const actaPayload = {
...
export async function runActaEpJoinDemo() {
    ...
    embeddedKeyChain.components[0].evidence = signActaPayload(embeddedKeyPayload, fixture.actaPrivateKey);
