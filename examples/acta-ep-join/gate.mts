...
function componentsOfType(chain: unknown, type: string): any[] {
  const c = chain as any;
  if (!Array.isArray(c?.components)) return [];
  return c.components.filter((component: any) => component?.type === type);
}

function refusingVerifier(reason: string): () => any {
...
export function createActaEpExecutionGate({
  ...
  async function run(request: any = {}, effect: any): Promise<any> {
