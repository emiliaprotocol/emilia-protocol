...
function hasEmbeddedVerificationKey(value: unknown): boolean {
  const stack = [value];
...
function decodePinnedEd25519Key(value: unknown): crypto.KeyObject | null {
...
function activePinnedKey(entry: unknown, kid: unknown, at: number): boolean {
...
function freshRegistrySnapshot(policy: unknown, at: number): boolean {
  if (!isRecord(policy)) return false;
  const checked = strictInstantMs((policy as any).registry_checked_at);
  return Number.isFinite(at) && Number.isFinite(checked)
    && Number.isInteger((policy as any).max_registry_age_sec)
    && (policy as any).max_registry_age_sec >= 0
    && checked <= at && at - checked <= (policy as any).max_registry_age_sec * 1000;
}

export function artifactDigest(value: unknown): string | null {
  try {
...
export function mapPaymentReleaseCaid(action: unknown, policy: any = {}): any {
...
export function computeActaActionRef(evaluation: unknown): string | null {
...
export function createActaDecisionVerifier({
  ...
  expectedActionRef?: unknown;
  expectedHumanAuthorizationDigest?: unknown;
  policy?: unknown;
} = {}): (evidence: unknown, ctx?: Record<string, unknown>) => any {
  ...
  return (evidence: unknown, ctx: Record<string, unknown> = {}): any => {
    ...
    try {
      ...
      const pp = pinnedPolicy as any;
      if (!Number.isFinite(at) || !Number.isFinite(issued)
          || !Number.isInteger(pp.max_age_sec) || pp.max_age_sec < 0
          || issued > at || at - issued > pp.max_age_sec * 1000) {
      ...
      if (payload.type !== ACTA_RECEIPT_TYPE
          || payload.tool_name !== pp.expected_tool_name
          || payload.decision !== pp.expected_decision
          || payload.policy_digest !== pp.expected_policy_digest) {
        return fail('ACTA decision is outside the relying-party policy');
      }
      if (!HEX_256.test((expectedActionRef as string) ?? '') || payload.action_ref !== expectedActionRef) {
