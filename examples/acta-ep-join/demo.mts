...
export function signActaPayload(payload: any, privateKey: crypto.KeyObject): any {
  return {
...
function buildChain(action: any, actaReceipt: any, epReceipt: any): any {
  return {
    '@version': 'EP-AEC-v1',
    action: clone(action),
...
export async function buildActaEpFixture(): Promise<any> {
  const action = {
    ep_version: '1.0',
...
export function gateFor(fixture: any, overrides: any = {}): any {
...
async function mustNotExecute(): Promise<never> {
  throw new Error('refused scenario reached the protected effect');
}

export async function runActaEpJoinDemo(): Promise<any> {
  ...
  const changedChain = { ...(clone(fixture.chain) as any), action: changedAction };
  (changedChain as any).action_digest = `sha256:${actionDigest(changedAction)}`;
  ...
  const roleSubstitutionChain = clone(fixture.chain) as any;
  ...
  const swappedReceiptChain = clone(fixture.chain) as any;
  ...
  const embeddedKeyChain = clone(fixture.chain) as any;
