...
// Generated from acta-profile.mts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
...
export const PAYMENT_RELEASE_MAPPING_PROFILE_HASH = mappingProfileHash(PAYMENT_RELEASE_MAPPING_PROFILE);
...
function hasEmbeddedVerificationKey(value) {
    ...
    while (stack.length) {
        ...
        for (const [key, child] of Object.entries(current)) {
            if (EMBEDDED_KEY_FIELD.test(key))
                return true;
...
function activePinnedKey(entry, kid, at) {
    if (!isRecord(entry) || entry.status !== 'active' || entry.issuer_id !== kid)
        return false;
 ...
 */
export function createActaDecisionVerifier({ expectedActionRef, expectedHumanAuthorizationDigest, policy, } = {}) {
    ...
    return (evidence, ctx = {}) => {
        ...
        try {
            if (!isRecord(pinnedPolicy))
                return fail('missing relying-party ACTA profile');
            ...
            if (!publicKey)
                return fail('pinned ACTA key is not an Ed25519 SPKI key');
