...
// Generated from adverse-determination.mts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
//
...
import { buildDavinciPasReviewBinding, digestPasValue, verifyDavinciPasReviewBinding, } from '../lib/health/davinci-pas-binding.js';
...
import { canonicalize as gateCanonicalize, capabilityActionDigest, createGate, createMemoryCapabilityStore, executeWithCapability, hashCanonical, mintCapabilityReceipt, mintDeviceSignoff, reconcileCapabilityOperation, CAPABILITY_SCOPE_PROFILE, } from '../packages/gate/index.js';
import { buildAssurancePackage, renderAssuranceWorkpaper, reperformAssurancePackage, } from '../packages/gate/reports/assurance-package.js';
...
function pasClaimResponse(reviewActionCode, rationale) {
    return {
 ...
 */
function buildDeterminationReceipt(binding, nonce) {
    const action = {
        ep_version: '1.0',
...
const reviewerAuthority = (over = {}) => signAuthorityProof({
...
const PUBLISHED_PROFILE = JSON.parse(readFileSync(new URL('../public/schemas/reliance-profiles/cms-prior-auth.v1.json', import.meta.url), 'utf8'));
...
function createProviderPortal() {
    ...
    return {
        ...
        async execute(action, ctx) {
            ...
            if (!operationId)
                throw new TypeError('provider operation_id is required');
            ...
            const error = new Error('provider portal response lost after the determination was written');
            error.code = 'PROVIDER_RESPONSE_LOST';
            throw error;
        },
        signedEvidence(operationId) {
            const record = written.get(operationId);
            if (!record)
                throw new Error('provider portal has no written determination for that operation');
            const body = {
...
function verifyDeliveryEvidence(portal) {
    return (evidence, ctx) => {
        if (!evidence?.body || !evidence?.signature)
            return { valid: false };
        if (evidence.body['@version'] !== PROVIDER_EVIDENCE_VERSION
            ...
            || evidence.body.action_digest !== ctx.action_digest)
            return { valid: false };
        const authentic = crypto.verify(null, Buffer.from(gateCanonicalize(evidence.body), 'utf8'), crypto.createPublicKey({ key: Buffer.from(portal.pinnedPublicKey, 'base64url'), format: 'der', type: 'spki' }), Buffer.from(evidence.signature.value, 'base64url'));
        if (!authentic)
            return { valid: false };
        return {
            valid: true,
...
async function main() {
    ...
    line(`        → ok: ${noReviewer.ok}   reasons: ${noReviewer.reasons.join(', ')}`);
    ...
    line(`        → ok: ${approvalPath.ok}   decision_outcome: ${approvalPath.binding?.action.decision_outcome}`);
    ...
    if (!built.ok)
        throw new Error(`binding failed: ${built.reasons.join(',')}`);
    ...
    const trust = verifyTrustReceipt(receipt, { approverKeys: APPROVER_KEYS, logPublicKey: logKey.pub, rpId: RP_ID, allowedOrigins: ALLOWED_ORIGINS, now: NOW });
    const consumedActions = new Set(); // the RP's own atomic state
    const consumedCaids = new Set(); // the determination fence
    const relianceOpts = {
        approverKeys: APPROVER_KEYS,
        logPublicKey: logKey.pub,
        rpId: RP_ID,
        allowedOrigins: ALLOWED_ORIGINS,
        isConsumed: ({ action_hash }) => consumedActions.has(action_hash),
    };
    ...
    for (const l of wrap(rationaleTamper.reasons.join(', '), 56))
        line(`            ${l}`);
    await pause(2000);
    ...
    for (const l of wrap(reviewerTamper.reasons.join(', '), 56))
        line(`            ${l}`);
    ...
    const issue = () => executeWithCapability({
        ...
        now: () => NOW,
        executeAction: (a, ctx) => portal.execute(a, ctx),
    });
    ...
    line(`        reconcileCapabilityOperation(...) → ok: ${reconciled.ok}  outcome: ${reconciled.outcome}`);
    ...
    const invariants = [
        ['gate refuses an adverse determination with no reviewer evidence', noReviewer.ok === false && noReviewer.reasons.includes('adverse_reviewer_required')],
        ['an approval builds without reviewer evidence', approvalPath.ok === true && approvalPath.binding.action.decision_outcome === 'approved'],
        ...
        ['authenticated evidence reconciles the operation', reconciled.ok === true && reconciled.outcome === 'executed'],
