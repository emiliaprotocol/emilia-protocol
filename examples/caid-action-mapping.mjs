...
const corpus = JSON.parse(readFileSync(
  new URL('../caid/conformance/mapping-vectors.json', import.meta.url),
  'utf8',
));
...
function signedBytes(payload) {
  ...
  if (!encoded.ok) throw new Error(`native payload is not canonicalizable: ${encoded.refusals.join(',')}`);
...
export function runCaidActionMappingDemo() {
  ...
  const checkoutEnvelope = signNative(
    corpus.sources['ap2-order'],
    checkoutKeys.privateKey,
    'example-ap2-shaped-checkout-v1',
  );

  const epNative = verifyNative(
    epEnvelope,
    epKeys.publicKey,
    'example-ep-action-v1',
    epProfile.source_format,
  );
  const checkoutNative = verifyNative(
    checkoutEnvelope,
    checkoutKeys.publicKey,
    'example-ap2-shaped-checkout-v1',
    checkoutProfile.source_format,
  );
  ...
  const tamperedNative = verifyNative(
    tampered,
    checkoutKeys.publicKey,
    'example-ap2-shaped-checkout-v1',
    checkoutProfile.source_format,
  ...
  const wrongMerchantEnvelope = signNative(
    wrongMerchantPayload,
    checkoutKeys.privateKey,
    'example-ap2-shaped-checkout-v1',
  );
  const wrongMerchantNative = verifyNative(
    wrongMerchantEnvelope,
    checkoutKeys.publicKey,
    'example-ap2-shaped-checkout-v1',
    checkoutProfile.source_format,
  );
  ...
  const shadowNative = verifyNative(
    unsignedShadow,
    checkoutKeys.publicKey,
    'example-ap2-shaped-checkout-v1',
    checkoutProfile.source_format,
