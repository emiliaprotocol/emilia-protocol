...
class SignoffStore {
  ...
  request(receiptId: string, call: any): string {
  ...
  approve(id: string): void { const s = this.pending.get(id); if (s) s.status = 'approved'; }

  status(id: string): string | undefined { return this.pending.get(id)?.status; }
}

// Map a tool call to the real policy engine; null actionType = read-only/allow.
function decide(call: any): any {
