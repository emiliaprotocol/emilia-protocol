...
function machineAction(operationId: string, palletId: string): MachineAction {
...
function verifyControlTransition(input: any) {
...
function controlAuthorization(actionDigest: string) {
  return {
    authenticated: true,
...
export async function runSupplyChainAuthorityDemo() {
  ...
  if (!domain.ok) throw new Error('control-domain registration failed');
  ...
  if (!exactReservation.ok) throw new Error(`exact reservation failed: ${exactReservation.reason}`);
  ...
  if (!exactEntry.ok) throw new Error(`provider entry failed: ${exactEntry.reason}`);
  ...
  if (!exactCommit.ok) throw new Error(`exact commit failed: ${exactCommit.reason}`);
  ...
  const substitution = verifyCapabilityScope(
    capability.capabilityReceipt.capability,
    substitutedAction,
    substitutionBase.operation_id,
  );
  ...
  if (!pendingReservation.ok) throw new Error(`pending reservation failed: ${pendingReservation.reason}`);
