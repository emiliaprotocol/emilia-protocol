...
// Generated from 402-loop.mts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
//
...
function issueReceipt(action, { tamper = false } = {}) {
    ...
    if (tamper)
        doc.payload = { ...payload, subject: 'agent:attacker' }; // mutate a signed field after signing
    return doc;
}
...
async function callAction(doc) {
    if (DEMO_URL) {
        ...
        if (doc)
            headers['X-EMILIA-Receipt'] = Buffer.from(JSON.stringify(doc)).toString('base64');
        const r = await fetch(DEMO_URL, { method: 'POST', headers, body: '{}' });
        return { status: r.status, body: await r.json().catch(() => ({})) };
    }
    if (!doc)
        return { status: 402, body: receiptChallenge(ACTION, 'No EMILIA receipt presented.') };
...
show(res);
if (res.status === 200)
    line(`   action performed; service retains evidence ${res.body.evidence?.receipt_id} (signer ${res.body.evidence?.signer})\n`);
