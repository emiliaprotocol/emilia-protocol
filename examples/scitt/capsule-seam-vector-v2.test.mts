...
import {
  ...
  denied,
  vectorJson,
} from './capsule-seam-vector-v2.mjs';
...
function decodeProtected(statement: Uint8Array): Map<unknown, unknown> {
  ...
  const protectedBytes = (body.value as unknown[])[0];
  assert.equal(protectedBytes instanceof Uint8Array, true);
  const protectedHeader = decodeDeterministicCbor8949(protectedBytes as Uint8Array, {
    textKeysOnly: false,
  });
...
test('v2 approved and denied statements pass the shipped fail-closed profile verifier', () => {
  for (const fixture of [approved, denied]) {
    ...
    assert.equal(verified.identity?.authorization_payload_digest,
      fixture.authorization_payload_digest);
  }
});
...
test('v2 statements carry the exact EP profile headers and versioned action grammar', () => {
  for (const fixture of [approved, denied]) {
    ...
    assert.equal((claims as Map<unknown, unknown>).get(CWT_CLAIM_ISS), fixture.iss);
    assert.equal((claims as Map<unknown, unknown>).get(CWT_CLAIM_SUB), fixture.sub);
