...
// Generated from capsule-seam-vector-v2.mts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
/**
...
import { buildEpScittSignedStatement, canonicalize, receiptActionCaid, verifyEpScittSignedStatement, } from '../../packages/verify/index.js';
...
const ACTION_IDENTITY = must(receiptActionCaid(ACTION), 'action identity');
export const ACTION_CAID = ACTION_IDENTITY.caid;
export const SUBJECT_DIGEST = ACTION_IDENTITY.digest;
function makeReceipt(receiptId, decision, signer = receiptIssuer) {
    const payload = {
    ...
    const signature = crypto.sign(null, Buffer.from(canonicalize(payload), 'utf8'), signer.privateKey);
    return {
...
export const approved = buildFixture(makeReceipt('ep:receipt:capsule-seam-v2-approved-0001', {
    outcome: 'allow_with_signoff',
    approver: 'jane.doe@yourco.example',
    assurance: 'class_a',
}));
export const denied = buildFixture(makeReceipt('ep:receipt:capsule-seam-v2-denied-0001', {
    outcome: 'deny',
    approver: null,
    reason: 'no_human_authorizer',
}));
export function verifyCapsuleBinding(fixture, input) {
    ...
    const decision = verified.receipt?.payload?.context?.authority_decision;
...
function verifyReceiptActionBinding(fixture, action) {
    const verified = verifyEpScittSignedStatement(fixture.statement, fixture.pins);
    if (!verified.valid)
        return { accepted: false, reason: verified.reason ?? 'statement_invalid' };
...
function negativeCases() {
    ...
    const forgedReceipt = makeReceipt('ep:receipt:capsule-seam-v2-forged-0001', {
        outcome: 'allow_with_signoff',
        approver: 'mallory@example.invalid',
        assurance: 'untrusted',
    }, otherReceiptIssuer);
    const forgedBuilt = must(buildEpScittSignedStatement(forgedReceipt, {
        statementPrivateKey: statementIssuer.privateKey,
        kid: STATEMENT_KID,
        iss: STATEMENT_ISS,
...
function portableFixture(fixture) {
    const { statement: _statement, pins: _pins, ...portable } = fixture;
    return portable;
}
export function vectorJson() {
    const negatives = negativeCases();
    assert.equal(negatives.every((entry) => entry.enforced), true, JSON.stringify(negatives, null, 2));
    return {
...
function main() {
    ...
    if (process.argv.includes('--check')) {
        ...
        assert.deepEqual(vector, frozen, 'capsule seam v2 vector drifted; inspect and re-pin deliberately with --emit');
    }
