...
// Generated from registration-event-order.test.mts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
import assert from 'node:assert/strict';
import test from 'node:test';
import { assessRegistrationEventOrder } from './registration-event-order.mjs';
function entry(log_id, index, operation_id, record_digest) {
    return {
        log_id,
        index,
        operation_id,
...
function terminal(log_id, index, operation_id, record_digest, outcome = 'EFFECT_CONFIRMED') {
    return {
        log_id,
        index,
        operation_id,
...
test('same sequencer plus entry-only records proves registration order, not effect order', () => {
    assert.deepEqual(assessRegistrationEventOrder({
...
test('a terminal for another operation cannot establish effect order', () => {
    assert.equal(assessRegistrationEventOrder({
        first: {
            entry: firstEntry,
