...
function buildMerkleLevels(leaves: any[]) {
  if (!leaves.length) return [[sha256(Buffer.alloc(0))]];
  const levels: Buffer[][] = [leaves.map((leaf) => Buffer.from(leaf))];
...
function inclusionPath(levels: Buffer[][], index: number) {
  const path: Array<{ position: string; hash: string }> = [];
  let cursor = index;
...
function buildMockTransparencyReceipt({ entries, entryIndex, serviceKeys, issuedAt = new Date().toISOString() }: { entries: any[]; entryIndex: number; serviceKeys: any; issuedAt?: string }) {
...
export {
  MOCK_RECEIPT_CONTENT_TYPE,
  MOCK_RECEIPT_PROFILE,
  buildMockTransparencyReceipt,
  createMockScrapiRegistry,
  createMockScrapiTransparencyService,
