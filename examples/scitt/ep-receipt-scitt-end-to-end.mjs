...
// Generated from ep-receipt-scitt-end-to-end.mts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
//
...
import { buildArtifacts, registerAndResolveScrapi, verifyProfileArtifacts, } from './ep-receipt-scitt-conformance.mjs';
import { verifyScittReceiptByVds } from './scitt-receipt-dispatch.mjs';
import { createMockScrapiRegistry, verifyMockTransparencyReceipt, } from './mock-scrapi-transparency-service.mjs';
...
async function runEndToEnd({ scittUrl = process.env.SCITT_URL || process.env.SCITT_TS_URL || '', useMockFallback = true, receiptProfiles = new Map(), fetchImpl = fetch, wait = undefined, maxPolls = 8, } = {}) {
    ...
    if (!targetUrl)
        throw new Error('SCITT_URL is required when mock fallback is disabled');
    ...
    const transparencyChecks = target === 'mock' && receipt
        ...
        : (() => {
            ...
            return [check('external_receipt_native_verification', 'returned RFC 9942 Receipt verifies under the pinned native VDS profile', registration.ok && native.native_verification === 'VERIFIED', `vds=${native.vds ?? 'unknown'} profile=${native.profile_id ?? 'unconfigured'} ${native.reasons.join(',')}`)];
        })();
    return {
        target,
        targetUrl,
...
export { postSignedStatement, runEndToEnd, };
