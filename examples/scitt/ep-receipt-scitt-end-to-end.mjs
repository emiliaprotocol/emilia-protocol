...
import { buildArtifacts, verifyProfileArtifacts, } from './ep-receipt-scitt-conformance.mjs';
...
async function runEndToEnd({ scittUrl = process.env.SCITT_URL || process.env.SCITT_TS_URL || '', useMockFallback = true } = {}) {
    ...
    const transparencyChecks = target === 'mock' && receipt
        ...
        : [
            check('external_registration', 'external SCRAPI-compatible target accepted the Signed Statement', registration.ok, `${registration.status} ${registration.contentType}`),
            check('external_receipt_verifier_needed', 'returned transparency receipt requires a target-specific verifier before claiming inclusion verification', true, 'not a SCITT WG conformance claim'),
        ];
    return {
        target,
        targetUrl,
