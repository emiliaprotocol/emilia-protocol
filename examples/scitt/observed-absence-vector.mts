...
const canon = (o: any) => Buffer.from(canonicalize(o), 'utf8');
...
const pub = crypto.createPublicKey(key as any).export({ type: 'spki', format: 'der' }).toString('base64url');
...
export function verifyObservedAbsence(doc: any, pinnedObserverKeys: string[] = []) {
...
if (!pos.accepted) throw new Error('positive case failed');
const tampered = JSON.parse(JSON.stringify(signed));
tampered.payload.search.as_of = '2026-07-03T12:00:00Z';
if (verifyObservedAbsence(tampered, [pub]).verified) throw new Error('tamper not caught');
const asserted = { payload: { ...statement, search: undefined }, sig: signed.sig, observer_key: pub };
if (verifyObservedAbsence(asserted, [pub]).verified) throw new Error('absence-without-search accepted');
