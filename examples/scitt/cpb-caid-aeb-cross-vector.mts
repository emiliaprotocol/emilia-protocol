...
function verifyCpbTypedReference({ reference, artifact, registry }) {
  ...
  if (!encoded.ok) return { binding: 'FAILED', reasons: ['artifact_not_canonicalizable'] };
...
export function evaluateCpbCaidAebComposition(vector) {
  ...
  if (cpb.binding === 'FAILED' || caidMatch === 'MISMATCH') satisfaction = 'UNSATISFIED';
...
function main() {
  ...
  if (result.cpb_binding !== 'VERIFIED'
    ...
    || result.aeb_evidence_satisfaction !== 'SATISFIED') process.exit(1);
}
