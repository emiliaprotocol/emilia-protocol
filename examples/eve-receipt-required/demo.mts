...
function humanApproves(actionType: string): any {
...
console.log(`1. no receipt          -> ${r1.ok ? 'RAN (BUG!)' : `BLOCKED (${(r1 as {ok:false,status:any,body:any}).status}) — funds NOT moved`}`);

const receipt = humanApproves(bound);
const r2 = await releaseFundsGate.run(receipt, { target: DEST }, doRelease);
console.log(`2. human signs         -> ${r2.ok ? `RAN ONCE — released $${(r2 as {ok:true,result:any}).result.amount} to ${DEST}` : `BLOCKED (${((r2 as {ok:false,body:any}).body)?.rejected?.reason})`}`);

const r3 = await releaseFundsGate.run(receipt, { target: DEST }, doRelease);
console.log(`3. replay same receipt -> ${r3.ok ? 'RAN (BUG!)' : `BLOCKED (${((r3 as {ok:false,body:any}).body).rejected.reason})`}`);
...
console.log(`4. receipt for another acct -> ${r4.ok ? 'RAN (BUG!)' : `BLOCKED (${((r4 as {ok:false,body:any}).body).rejected.reason})`}`);
