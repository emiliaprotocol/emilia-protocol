...
export const AUTHORITY_OPERATIONS_INTERFACE_VERSION =
  'EMILIA-AUTHORITY-OPERATIONS-REFERENCE-v0';
...
export type AuthorityPolicy = {
  ...
  action_type: string;
  mode: 'AUTOMATIC_WITHIN_ENVELOPE' | 'FRESH_AUTHORIZATION_REQUIRED' | 'PROHIBITED';
  evaluator_profile: string;
  outside_envelope: 'FRESH_AUTHORIZATION_REQUIRED' | 'REFUSE';
...
export type AuthorizationEvidence = {
  ...
  verification: 'VERIFIED' | 'REJECTED' | 'INDETERMINATE';
  subjects: string[];
};
...
export type AuthorityOperation = {
  '@version': typeof AUTHORITY_OPERATIONS_INTERFACE_VERSION;
  operation_id: string;
  action: JsonObject;
  action_digest: string;
  policy: AuthorityPolicy;
  policy_result: 'WITHIN_ENVELOPE' | 'OUTSIDE_ENVELOPE' | 'INDETERMINATE';
  authorization: {
    state: 'NOT_REQUIRED' | 'REQUIRED' | 'SATISFIED' | 'INDETERMINATE';
    required_evidence: EvidenceRequirement[];
    accepted_evidence: AuthorizationEvidence[];
  };
  execution: {
    state: 'READY' | 'AUTHORIZATION_REQUIRED' | 'RESERVED' | 'PROVIDER_ENTERED'
      | 'COMMITTED' | 'PROVEN_NOT_COMMITTED' | 'INDETERMINATE' | 'REFUSED';
    provider_attempt: 'NOT_ENTERED' | 'ENTERED';
    outcome: 'NOT_ASSESSED' | 'COMMITTED' | 'PROVEN_NOT_COMMITTED' | 'INDETERMINATE';
    resulting_state: JsonObject | null;
  };
  control_state: 'ACTIVE' | 'FROZEN';
...
function exactAction(value: unknown): value is JsonObject {
...
function validateRequirements(value: unknown): value is EvidenceRequirement[] {
...
function validatePolicy(value: unknown, actionType: string): asserts value is AuthorityPolicy {
...
function event(operation: AuthorityOperation, at: string, type: string, details: Partial<AuthorityEvent> = {}): void {
  if (!iso(at)) throw new TypeError('authority_operations:timestamp_invalid');
...
function deriveInitialState(policy: AuthorityPolicy, result: AuthorityOperation['policy_result']): {
  authorization: AuthorityOperation['authorization']['state'];
  execution: AuthorityOperation['execution']['state'];
  reason: string | null;
} {
...
export function createAuthorityOperation(input: {
  ...
  action: JsonObject;
  policy: AuthorityPolicy;
  policy_result: AuthorityOperation['policy_result'];
  observed_at: string;
}): AuthorityOperation {
...
function evidenceReason(operation: AuthorityOperation, evidence: AuthorizationEvidence): string | null {
  ...
  if (evidence.action_digest !== operation.action_digest) return 'action_binding_mismatch';
  if (evidence.verification !== 'VERIFIED') return 'evidence_not_verified';
  const requirement = operation.authorization.required_evidence.find(
    (candidate) => candidate.evidence_type === evidence.evidence_type && candidate.role === evidence.role,
  );
  if (!requirement) return 'evidence_requirement_mismatch';
  if (new Set(evidence.subjects).size < requirement.minimum) return 'insufficient_distinct_subjects';
  return null;
}

function assertOperationIntegrity(value: unknown): asserts value is AuthorityOperation {
  if (!isPlainObject(value)
...
function allRequirementsSatisfied(operation: AuthorityOperation): boolean {
  return operation.authorization.required_evidence.every((requirement) =>
    operation.authorization.accepted_evidence.some((evidence) =>
...
export function presentAuthorizationEvidence(
  current: AuthorityOperation,
  ...
  if (!operation.authorization.accepted_evidence.some(
...
export function reserveAuthorityOperation(current: AuthorityOperation, at: string): AuthorityOperation {
  assertOperationIntegrity(current);
  const operation = copy(current);
  if (operation.control_state !== 'ACTIVE') throw new Error('authority_operations:control_domain_frozen');
...
export function beginProviderEntry(current: AuthorityOperation, at: string): AuthorityOperation {
  assertOperationIntegrity(current);
  const operation = copy(current);
  if (operation.control_state !== 'ACTIVE') throw new Error('authority_operations:control_domain_frozen');
...
export function recordProviderOutcome(
  current: AuthorityOperation,
  outcome: {
    value: 'COMMITTED' | 'PROVEN_NOT_COMMITTED' | 'INDETERMINATE';
...
export function intervene(
  current: AuthorityOperation,
  intervention: {
    type: 'CANCEL_BEFORE_ENTRY' | 'FREEZE_NEW_ADMISSIONS';
    actor_id: string;
    reason: string;
  },
  at: string,
...
function availableInterventions(operation: AuthorityOperation): string[] {
  const result: string[] = [];
  if (operation.execution.provider_attempt === 'NOT_ENTERED'
      && !['REFUSED', 'COMMITTED', 'PROVEN_NOT_COMMITTED', 'INDETERMINATE'].includes(operation.execution.state)) {
    result.push('cancel_before_entry');
  }
  if (operation.control_state === 'ACTIVE') result.push('freeze_new_admissions');
...
export function projectAuthorityOperation(current: AuthorityOperation): JsonObject {
  assertOperationIntegrity(current);
  const operation = copy(current);
