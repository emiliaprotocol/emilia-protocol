...
import { AUTONOMY_CONTROL_PLANE_VERSION, CAPABILITY_ALLOWANCE_SCOPE_PROFILE, capabilityActionDigest, canonicalize, compileAutonomyControlPlaneProfile, createDefaultActionRiskManifest, createEg1Harness, createGate, createMemoryCapabilityStore, createMemoryTrustProgramStore, createRuntimeMonitor, createTrustProgramKernel, executeWithCapability, mintCapabilityReceipt, } from '../../packages/gate/index.js';
...
export function selfModificationDigest(value) {
    const bytes = typeof value === 'string' || Buffer.isBuffer(value)
        ? value
        : canonicalize(value);
    return `sha256:${createHash('sha256').update(bytes).digest('hex')}`;
}
function exactActionMaterial(action) {
    return {
...
function resolveActionCaid(action) {
...
function exactActionDigest(action) {
    return capabilityActionDigest(exactActionMaterial(action));
}
function changeDigestMatches(action) {
...
function objectiveCaid(logicalAgentId, policyDigest) {
    ...
    if (!result.caid)
        throw new Error(`objective CAID refused: ${result.refusals?.join(',')}`);
    return result.caid;
}
function makePromotionAction({ phase, operationId, changedPaths, baseDigest, candidateDigest, evaluatorProfileDigest, policyDigest, }) {
    ...
    if (!action.caid)
        throw new Error('self-modification action CAID refused');
    return Object.freeze(action);
}
function controlPlaneOverlap(changedPaths) {
    return changedPaths.some((path) => PROTECTED_PATH_PREFIXES.some((prefix) => prefix.endsWith('/') ? path.startsWith(prefix) : path === prefix || path.startsWith(`${prefix}.`)));
}
function publicKeyFingerprint(publicKey) {
    return selfModificationDigest(publicKey.export({ type: 'spki', format: 'der' }));
}
function signedArtifact(challenge, requirement, role, claims, evidenceId) {
    const body = {
...
function verifySignedArtifact(artifact, trustedRoles) {
    if (!artifact || typeof artifact !== 'object' || typeof artifact.key_id !== 'string'
        || typeof artifact.signature !== 'string')
        return { ok: false, reason: 'evidence_shape_invalid' };
    const publicKey = trustedRoles.get(artifact.key_id);
    if (!publicKey)
        return { ok: false, reason: 'evidence_key_untrusted' };
    const { key_id: _keyId, signature, ...body } = artifact;
    let valid = false;
    try {
        valid = verify(null, Buffer.from(canonicalize(body)), publicKey, Buffer.from(signature, 'base64url'));
    }
    catch {
        valid = false;
    }
...
function resultFromEvidence(body) {
    return {
        valid: true,
        reason: null,
...
export function createSelfModificationHarness({ changedPaths = ['src/optimizer.ts'], provider = async (action) => ({
    ...
    if (!advanced.ok)
        throw new Error(`allowance status initialization failed: ${advanced.reason}`);
    ...
    function makeVerifier(expectedAction) {
        return async ({ artifact, requirement }) => {
            const signed = verifySignedArtifact(artifact, trustedRoles);
            if (!signed.ok)
                return { valid: false, reason: signed.reason };
    ...
    async function run({ action: presentedAction = action, operationId = presentedAction.operation_id, fitnessClaims = {}, } = {}) {
        ...
        const kernel = createTrustProgramKernel({
            ...
            actionBindingVerifier: ({ action: observed, expectedCaid, expectedActionDigest }) => (observed?.caid === expectedCaid
        ...
        if (!started.ok)
            return { ok: false, outcome: 'refused', reason: started.reason };
        ...
        for (const stage of promotionProgram.stages) {
            for (const requirement of stage.requirements) {
                ...
                if (!challenge.ok)
                    return { ok: false, outcome: 'refused', reason: challenge.reason };
                let claims = { status: 'current' };
                if (requirement.verifier_profile === 'ep-execution-binding:v1')
                    claims = child.change;
                if (requirement.verifier_profile === 'agent-fitness-bench:v1')
                    claims = {
                ...
                if (!role)
                    return { ok: false, outcome: 'refused', reason: 'evidence_role_unavailable' };
                const artifact = signedArtifact(challenge, requirement, role, claims, `evidence:${operationId}:${requirement.requirement_id}`);
                ...
                if (!admitted.ok)
                    return { ok: false, outcome: 'refused', reason: admitted.reason };
            }
        }
        ...
        if (!claim.ok)
            return { ok: false, outcome: 'refused', reason: claim.reason };
        ...
        if (!finalized.ok)
            return { ok: false, outcome: 'indeterminate', reason: finalized.reason };
        return {
