...
// Generated from provider.mts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
import { createPublicKey, generateKeyPairSync, sign, verify, } from 'node:crypto';
import { canonicalize, hashCanonical, } from '../../packages/gate/index.js';
...
export function createSignedMockProvider({ providerId = 'mock-bank.example', now = () => '2026-07-19T04:00:05.000Z', } = {}) {
    ...
    function signEvidence({ operationId, action } = {}) {
        ...
        if (!committed)
            throw new Error('provider has no committed effect for operation');
        ...
        return {
            ...
            signature: {
                ...
                value: sign(null, Buffer.from(canonicalize(body), 'utf8'), privateKey).toString('base64url'),
            },
        };
    }
    return {
...
export function verifySignedProviderEvidence(evidence, { pinnedProviderKey, expectedProviderId, expectedOperationId, expectedAction, } = {}) {
    ...
    let authentic = false;
    try {
        authentic = verify(null, Buffer.from(canonicalize(evidence.body), 'utf8'), createPublicKey({
    ...
    if (!authentic)
        throw new Error('provider evidence signature invalid');
    ...
    const expectedEffect = effectProjection(expectedAction, `provider-effect:${expectedOperationId}`);
