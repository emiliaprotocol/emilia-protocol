...
function receipt(nonce, amount) {
    ...
    const d1 = sha(canon(c1));
    const d2 = sha(canon(c2));
    const so = [
...
{
    const r = receipt('exp', 30000);
    decisions.push({ decision_id: 'PA-expired-auth', action: act(r, 30000), receipt: r, authority_proof: authority({ validity: { from: '2025-01-01T00:00:00.000Z', to: '2026-06-01T00:00:00.000Z' } }), revocation_state: fresh, consumption: { consumed: false }, stated_verdict: 'do_not_rely_authority_expired' });
}
...
{
    const r = receipt('over', 120000);
    decisions.push({ decision_id: 'PA-over-ceiling', action: act(r, 120000), receipt: r, authority_proof: authority(), revocation_state: fresh, consumption: { consumed: false }, stated_verdict: 'rely' });
}
