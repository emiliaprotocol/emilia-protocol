...
const REGISTRY_DEFINITIONS = JSON.parse(fs.readFileSync(
...
function monotonicCounterStore() {
  ...
  return {
    async advance(key, value) {
      ...
      if (!Number.isSafeInteger(value) || value <= previous) return false;
...
function durableAuditLog() {
  ...
  return createAtomicEvidenceLog(backend, {
    ...
    recordIdFactory: (() => `regulatory-mobile-audit-${String(++next).padStart(6, '0')}`) as () => `${string}-${string}-${string}-${string}-${string}`,
  });
}
...
function buildSyntheticSystemOfRecord() {
  ...
  if (!computed.caid) throw new Error(`synthetic CAID refused: ${(computed.refusals || []).join(', ')}`);
  const policy = {
    policy_id: POLICY_ID,
  ...
  return {
    ...
    applyVerifiedDecision({ result, receipt, evidenceReport }: any) {
      ...
      (record as any).approved_by = result.approver_id;
      (record as any).approval_context_hash = result.context_hash;
      return { applied: true, status: record.status };
    },
  };
}

function makeSyntheticAppAttestHarness(platformRoot: any) {
  const tokenBody = ({ requestHash, counter }: any) => ({
    app_id: APP_ID,
  ...
  const issueToken = ({ requestHash, counter }: any) => {
  ...
  const verifier = createAppleAppAttestVerifier({
    ...
    async verifyAssertion({ assertionObject, clientDataHash, appId, keyId, environment }: any) {
      try {
        const token = JSON.parse((assertionObject as unknown as Buffer).toString('utf8'));
...
function parseClientData(receipt) {
  try {
    return JSON.parse(Buffer.from(
      receipt.signoffs[0].webauthn.client_data_json,
      'base64url',
    ).toString('utf8'));
  } catch {
    return null;
...
function verifyAuditJoin(evidence, executionRecord) {
  ...
  if (!verifyEvidenceRecord(audit, { atomicRequired: true })) return false;
...
export function verifyRegulatoryEvidence(evidence, trustBundle) {
  ...
  try {
    ...
    const enrollment = profile.enrollments?.find(
      (item) => item.device_key_id === context?.mobile_binding?.device_key_id,
    );
    ...
    checks.receipt_execution_join = executionRecord.receipt_id === receipt.receipt_id
      ...
      && executionRecord.attestation_format === (
        context.mobile_binding.platform === 'ios'
          ? 'apple-app-attest'
...
export async function buildSyntheticRegulatoryDemo() {
  ...
  const controller = createGovernmentMobileController({
    ...
    resolveRequest: (input: any) => systemOfRecord.resolve(input) as Promise<Record<string, unknown>>,
    authorize: (input: any) => caller.subject === 'ep:service:synthetic-utilization-review'
  ...
  const challenge = issued.challenge as any;
  ...
  if (!offlineReport.valid) throw new Error('locally generated regulatory evidence did not verify');
  ...
  const freshAttestationReplay = structuredClone(response) as any;
