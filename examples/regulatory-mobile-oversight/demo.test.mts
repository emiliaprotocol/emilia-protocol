...
test('deterministic mobile fixture exercises the regulator evidence path', () => {
  ...
  assert.deepEqual(Object.values(demo.offlineReport.checks),
    Array(Object.keys(demo.offlineReport.checks).length).fill(true));
});

test('action and presentation substitution both refuse offline', () => {
  const action = structuredClone(demo.evidence);
  (action.receipt.action as any).units_approved = '999';
...
test('evidence never supplies its own reviewer or operator trust keys', () => {
  ...
  wrongPins.approver_keys[demo.evidence.execution_record.device_key_id as string].public_key =
  ...
  missingOperatorKey.execution_record_keys = {} as any;
...
test('unknown package and trust claims are refused instead of ignored', () => {
  ...
  (evidence as any).regulator_approved = true;
  ...
  (trust as any).operator_is_infallible = true;
...
test('runtime statement and audit record cannot be detached or relabeled', () => {
  ...
  (audit.audit_record as any).action_hash = `sha256:${'00'.repeat(32)}`;
...
test('the report separates direct verification, operator assertions, and nonclaims', () => {
  ...
  assert.ok(demo.offlineReport.directly_recomputed_offline.includes(
    'Class-A passkey signature under an out-of-band pinned reviewer key',
  ));
  assert.ok(demo.offlineReport.operator_attested_not_independently_replayed.includes(
