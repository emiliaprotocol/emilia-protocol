 ...
 */
import { createGate, createEg1Harness, } from '../../packages/gate/index.js';
...
export async function runGoogleCloudRelianceLab() {
    ...
    const cases = [];
    cases.push(await expectRefusal('local-controls-allow-but-evidence-missing', 'IAM and content controls allow; customer evidence is absent', main, () => callMcpTool(main)));
    cases.push(await expectRefusal('single-approver-cannot-fill-two-person-rule', 'one human cannot satisfy the customer-pinned quorum', main, () => callMcpTool(main, EXACT_ACTION, main.harness.mint({ outcome: 'allow_with_signoff' }))));
    const weakerAction = { ...EXACT_ACTION, role: 'roles/viewer' };
    const drift = makeSubject({ action: weakerAction, idPrefix: 'gcp_drift' });
    cases.push(await expectRefusal('receipt-for-viewer-cannot-grant-owner', 'evidence for roles/viewer cannot authorize roles/owner', drift, () => callMcpTool(drift, EXACT_ACTION, drift.harness.mint({ outcome: 'allow_with_signoff', quorum: QUORUM }))));
    cases.push(await expectRefusal('tampered-receipt-refused', 'a signed receipt altered after issuance is not evidence', main, () => callMcpTool(main, EXACT_ACTION, main.harness.mint({
    ...
    cases.push({
        ...
        execution_binds_authorization: accepted._emilia?.execution?.authorizes_decision
            === accepted._emilia?.reliance?.summary?.decision_hash,
    });
    cases.push(await expectRefusal('accepted-receipt-replay-refused', 'the same accepted evidence cannot drive a second mutation', main, () => callMcpTool(main, EXACT_ACTION, receipt)));
    return {
        '@version': LAB_VERSION,
