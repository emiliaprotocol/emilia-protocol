...
function refusal(id: any, title: any, error: any, callsBefore: any, callsAfter: any) {
  return {
    id,
    title,
...
async function expectRefusal(id: any, title: any, subject: any, invoke: any) {
 ...
 */
async function callMcpTool(subject: any, action: any = EXACT_ACTION, receipt: any = null) {
  const result = await subject.tool({
    ...paramsFor(action),
    ...(receipt ? { _emilia_receipt: receipt } : {}),
  });
  if (result?.isError) {
    const error = new Error(result._emilia?.reason || 'mcp_tool_refused') as Error & { status?: unknown, gate?: unknown, challenge?: unknown };
...
function paramsFor(action: any = EXACT_ACTION) {
  return {
...
export async function runGoogleCloudRelianceLab() {
  ...
  const weakerAction = { ...EXACT_ACTION, role: 'roles/viewer' } as any;
