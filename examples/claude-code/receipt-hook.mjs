...
// Generated from receipt-hook.mts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
/**
...
if (input?.tool_name !== 'Bash' || !command)
    process.exit(0);
if (![...DANGEROUS, ...extra].some((re) => re.test(command)))
    process.exit(0); // safe: untouched
...
let doc = null;
try {
    doc = JSON.parse(fs.readFileSync(receiptPath, 'utf8'));
}
catch { /* absent */ }
if (!doc)
    block(`EMILIA: blocked — dangerous command, no authorization receipt at ${receiptPath}.\n${challenge}`);
const res = verifyEmiliaReceipt(doc, { trustedKeys, action: ACTION, maxAgeSec: 900 });
if (!res.ok)
    block(`EMILIA: blocked — receipt rejected (${res.reason}${res.detail ? `: ${res.detail}` : ''}). The receipt must bind THIS exact command.\n${challenge}`);
