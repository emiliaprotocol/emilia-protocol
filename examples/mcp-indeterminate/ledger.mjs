...
// Generated from ledger.mts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
//
...
function fsyncAppend(path, line) {
...
export class Stores {
    ...
    constructor(dir) {
        ...
        if (!existsSync(this.watermarkPath)) {
            writeFileSync(this.watermarkPath, JSON.stringify({ complete_through_seq: 0, at: new Date(0).toISOString() }), 'utf8');
        }
    }
    // -- boundary journal -----------------------------------------------------
    boundaryEntries() {
        return readJsonl(this.boundaryPath);
    }
    /** Last recorded state for a replay unit, or null if never seen. */
    latestFor(replayUnit) {
        let found = null;
        for (const entry of this.boundaryEntries()) {
            if (entry.replay_unit === replayUnit)
                found = entry;
        }
        return found;
    }
    ...
    // -- provider record ------------------------------------------------------
    providerEntries() {
        return readJsonl(this.providerPath);
    }
    watermark() {
        return JSON.parse(readFileSync(this.watermarkPath, 'utf8'));
    }
    /**
    ...
    applyEffect(input) {
        ...
        const entry = { seq, ...input };
        fsyncAppend(this.providerPath, `${JSON.stringify(entry)}\n`);
        writeFileSync(this.watermarkPath, JSON.stringify({ complete_through_seq: seq, at: input.at }), 'utf8');
    ...
    advanceWatermark(at) {
        ...
        const wm = { complete_through_seq: seq, at };
 ...
 */
export function verifyProviderStatement(statement, pinnedPublicKeyB64) {
    ...
    try {
        ...
        ok = verify(null, Buffer.from(canonical(s.statement), 'utf8'), pub, Buffer.from(s.signature, 'base64'));
    }
    catch {
        return { ok: false, refusals: ['provider_statement_signature_unverifiable'] };
    }
    if (!ok)
        return { ok: false, refusals: ['provider_statement_signature_invalid'] };
