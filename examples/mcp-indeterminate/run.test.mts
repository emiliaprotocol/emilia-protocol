...
test('a provider statement under an unpinned key is refused', () => {
  const out = verifyProviderStatement(
    { statement: { operation_id: 'x', found: null, watermark: {} }, signature: 'AA', public_key_spki_b64: 'BB' },
    'CC',
  );
  assert.equal(out.ok, false);
  if (out.ok) return;
...
test('every vector in the pack parses and maps to an AEB state', () => {
  const pack = JSON.parse(readFileSync(join(here, 'vectors.v1.json'), 'utf8')) as {
    vectors: Array<{ id: string; result?: { _meta?: Record<string, unknown> } }>;
    aeb_mapping: unknown[];
  };
...
test('the four AEB-04 states named in the task are all covered', () => {
  ...
  for (const state of ['EXECUTED', 'FAILED', 'INDETERMINATE', 'REQUIRES_NEW_ADMISSION']) {
    assert.ok(covered.has(state as never), `${state} is not in the mapping`);
  }
