...
if (!caidResult.caid) throw new Error(`caid refused: ${(caidResult.refusals ?? []).join(',')}`);
const CAID = caidResult.caid;

const derived = deriveReplayUnit({ authority_instance_digest: AUTHORITY, caid: CAID });
if (!derived.ok) throw new Error(`derivation refused: ${derived.refusals.join(',')}`);
...
function callRequest(meta: Record<string, unknown> | null) {
  return {
    jsonrpc: '2.0',
    id: 1,
    method: 'tools/call',
...
const vectors = [
  {
    ...
    summary:
      'The clean path. An authoritative response matched to the exact action. '
  ...
  },
  {
    ...
    aeb_locator:
      'draft-schrock-action-evidence-boundary-04 section 5.10, restart promotion of a '
      + 'DISPATCH_PENDING record with no authoritative terminal record',
    summary:
      'The state MCP cannot express today. The server restarted holding a dispatch '
      ...
      content: [
        {
          type: 'text',
          text:
            'outcome is indeterminate: this replay unit has a dispatch with no terminal '
    ...
    host_obligation:
      'MUST NOT call release_payment again for a second effect. MUST call the tool named '
  ...
  },
  {
    ...
    aeb_locator:
      'draft-schrock-action-evidence-boundary-04 section 5.11: reconciliation MUST NOT '
      + 'resurrect the original authorization or silently release its one-time replay unit',
    summary:
      'Reconciliation moved INDETERMINATE to FAILED. The absence of a provider entry is '
      ...
      content: [
        {
          type: 'text',
          text:
            'reconciled: no effect landed, and the provider record is complete through '
    ...
    host_obligation:
      'MUST NOT reuse this replay unit or this authorization. A later attempt is a new '
  ...
  },
  {
    ...
    aeb_state: 'INDETERMINATE',
    aeb_locator:
      'draft-schrock-action-evidence-boundary-04 section 5.11: missing, stale, conflicting, '
      + 'unauthenticated or action-mismatched observations MUST leave the operation INDETERMINATE',
    summary:
      'Reconciliation ran and did not settle anything: the provider saw no entry but its '
  ...
  },
  {
    ...
    summary:
      'The harm, demonstrated. Two tools/call exchanges exactly as MCP defines them today, '
    ...
    first_attempt: {
      note:
        'The server applied the effect and then the process died. No JSON-RPC result was '
    ...
    host_obligation:
      'None available. The host behaved correctly given the vocabulary it has. That is the '
  ...
  },
  {
    ...
    aeb_locator:
      'draft-schrock-action-evidence-boundary-04 section 5.8: the key MUST be derived from '
      + 'verified evidence or the executor-owned observed action and operation identifier, '
      + 'never from a presenter-selected decoy',
    summary:
      'A replay unit that is not the derivation over the authority and the frozen action. '
...
const pack = {
  ...
  what_this_is:
    'A mapping of draft-schrock-action-evidence-boundary-04 sections 5.10 and 5.11 onto a '
  ...
  mcp: {
    ...
    meta_prefix_note:
      'A prefix whose second label is modelcontextprotocol or mcp is reserved for MCP use; '
...
for (const vector of vectors) {
  const result = (vector as { result?: { _meta?: Record<string, unknown> } }).result;
