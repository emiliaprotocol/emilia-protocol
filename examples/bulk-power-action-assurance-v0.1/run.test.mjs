...
import { createEquipmentEligibilityGuard, createFixture, runProfile, PROFILE_VERSION, } from './run.mjs';
...
test('the policy source lock identifies the exact final published bytes', () => {
    ...
    assert.equal(sourceLock.primary_source.citation, 'Executive Order 14421; 91 FR 55995; FR Doc. 2026-17843');
    ...
    assert.equal(sourceLock.primary_source.sha256, '3492b0c9049ee532e58719dedc070ae454d93b0070f71518282eec7ae0a39dbb');
...
test('the reference profile reports every implemented case as passing', async () => {
    ...
    for (const entry of report.cases)
        assert.equal(entry.passed, true, entry.id);
});
...
test('provider-entry status evidence reaches the effect and hash-chained execution detail', async () => {
    ...
    assert.equal(effectEvidence.guards.some((entry) => entry.kind === 'equipment_status'), true);
    assert.equal(effectEvidence.guards.some((entry) => entry.kind === 'organization_status'), true);
    assert.equal(result.provider_entry_evidence.guards.some((entry) => entry.kind === 'equipment_status'), true);
    assert.deepEqual(result.raw.execution.detail.provider_entry_evidence, result.provider_entry_evidence);
});
test('equipment status age policy accepts only bounded safe integers', () => {
    const resolveStatus = async () => { throw new Error('not called'); };
    assert.throws(() => createEquipmentEligibilityGuard({ resolveStatus, maxAgeMs: -1 }), /maxAgeMs must be a safe integer from 0 through 60000/);
    assert.throws(() => createEquipmentEligibilityGuard({ resolveStatus, maxAgeMs: 60_001 }), /maxAgeMs must be a safe integer from 0 through 60000/);
    for (const invalid of [Number.NaN, Number.POSITIVE_INFINITY]) {
        assert.throws(() => createEquipmentEligibilityGuard({ resolveStatus, maxAgeMs: invalid }), /maxAgeMs must be a safe integer from 0 through 60000/);
    }
    assert.doesNotThrow(() => createEquipmentEligibilityGuard({ resolveStatus, maxAgeMs: 60_000 }));
});
