...
} from '../ot-command-binding-v1/commands.mjs';
...
export type EquipmentStatus = Readonly<{
  subject_type: 'equipment';
  subject_id: string;
  status: 'ACTIVE' | 'PREQUALIFIED' | 'RESTRICTED' | 'REVOKED';
  source_id: string;
  ...
  observed_at: string;
  expires_at: string;
  applicable_asset_ids: readonly string[];
  applicable_operation_types: readonly string[];
...
function exactAction(overrides: Obj = {}): Obj {
...
function actionFromWire(template: Obj, transactionId = 1): Obj {
...
function currentStatus(action: Obj, overrides: Obj = {}): EquipmentStatus {
...
function validStringList(value: unknown, { maxItems = 100, maxBytes = 512 } = {}): value is string[] {
 ...
 */
export function createEquipmentEligibilityGuard({
  resolveStatus,
  sourceId = STATUS_SOURCE_ID,
  issuerKeyId = STATUS_ISSUER_KEY_ID,
  maxAgeMs = 5_000,
  now = Date.now,
}: {
  resolveStatus: (input: Readonly<{ action: Obj; context: ProviderEntryContext }>) =>
    EquipmentStatus | Promise<EquipmentStatus>;
  sourceId?: string;
  issuerKeyId?: string;
  maxAgeMs?: number;
  now?: () => number;
  ...
  return async (context) => {
    const action = context.observed_action as Obj | null;
    if (!action) return { ok: false, reason: 'equipment_action_missing', status: 409, reservation: 'hold' };
...
function manifest(action: Obj): Obj {
...
function verifyControlTransition(input: Obj) {
  return {
    authenticated: true,
    authorized: true,
...
export async function createFixture({
  ...
  freezeDuringStatusCheck = false,
}: {
  action?: Obj;
  status?: EquipmentStatus | null;
  statusUnavailable?: boolean;
  freezeDuringStatusCheck?: boolean;
} = {}) {
  ...
  if (!registered.ok) throw new Error(`control-domain registration failed: ${registered.reason}`);

  const actionContractGuard: ProviderEntryGuard = (context) => {
    const candidate = context.observed_action as Obj | null;
    if (!candidate) {
  ...
  const equipmentGuard = createEquipmentEligibilityGuard({
    maxAgeMs: 60_000,
    resolveStatus: async () => {
      if (statusUnavailable) throw new Error('status source unavailable');
  ...
  const organizationGuard = createOrganizationStatusProviderEntryGuard({
    ...
    resolveStatus: async () => {
      if (freezeDuringStatusCheck && !freezeCommitted) {
        ...
        if (!frozen.ok) throw new Error(`control-domain freeze failed: ${frozen.reason}`);
      }
      return {
  ...
  const providerEntryGuard = composeProviderEntryGuards(
    actionContractGuard,
    organizationGuard,
    equipmentGuard,
  );
  ...
  async function attempt({
    ...
    effect = async () => ({ acknowledged: true }),
  }: {
    presentedAction?: Obj;
    ...
    try {
      ...
      return {
        ...
        provider_entry_evidence:
          error.emiliaGateOutcome.provider_entry_evidence
  ...
  return {
    ...
    setStatus(next: EquipmentStatus) { equipmentStatus = next; },
  };
}

function caseResult(id: string, passed: boolean, observed: Obj) {
...
export async function runProfile() {
  ...
  const admittedEquipmentStatus = admittedGuards.find((entry: Obj) => entry.kind === 'equipment_status');
  cases.push(caseResult('EXACT-CURRENT-ACTION-ADMITTED',
    admitted.ok && positive.providerEntries === 1
      && admittedEquipmentStatus
      && admittedGuards.some((entry: Obj) => entry.kind === 'organization_status'), {
      allowed: admitted.ok,
      reason: admitted.reason,
      provider_entries: positive.providerEntries,
      equipment_status_digest:
        admittedEquipmentStatus?.source_artifact_digest ?? null,
    }));
  ...
  cases.push(caseResult('ASSET-SUBSTITUTION-REFUSED',
    assetRefusal.reason === 'capability_action_out_of_scope'
  ...
  cases.push(caseResult('PARAMETER-SUBSTITUTION-REFUSED',
    parameterRefusal.reason === 'capability_action_out_of_scope'
  ...
  cases.push(caseResult('ADMISSION-DOMAIN-SUBSTITUTION-REFUSED',
    domainRefusal.reason === 'capability_action_out_of_scope'
  ...
  cases.push(caseResult('FIRMWARE-DRIFT-REFUSED',
    firmwareRefusal.reason === 'capability_action_out_of_scope'
  ...
  cases.push(caseResult('MODBUS-TRANSACTION-ID-IS-NONMATERIAL',
    capabilityActionDigest(txActionA) === capabilityActionDigest(txActionB), {
  ...
  cases.push(caseResult('WIRE-OPERATION-LABELS-ARE-CONDUIT-DERIVED',
    conduitDerived.action_type === 'bulk-power.control-command.1'
  ...
  const invalidDigestResults: Obj[] = [];
  ...
  cases.push(caseResult('MALFORMED-ACTION-DIGESTS-REFUSED',
    invalidDigestResults.every((entry) => entry.reason === 'action_digest_field_invalid'
  ...
  cases.push(caseResult('POLICY-SUBSTITUTION-REFUSED',
    policyRefusal.reason === 'action_policy_mismatch' && policyFixture.providerEntries === 0, {
  ...
  cases.push(caseResult('PROVIDER-SUBSTITUTION-REFUSED',
    providerRefusal.reason === 'action_provider_mismatch' && providerFixture.providerEntries === 0, {
  ...
  cases.push(caseResult('OUTCOME-PROFILE-SUBSTITUTION-REFUSED',
    outcomeProfileRefusal.reason === 'action_outcome_evidence_profile_mismatch'
  ...
  cases.push(caseResult('CONTROL-EPOCH-MISMATCH-REFUSED',
    epochRefusal.reason === 'action_control_epoch_mismatch' && epochFixture.providerEntries === 0, {
  ...
  cases.push(caseResult('EXPIRED-ACTION-REFUSED',
    expired.reason === 'action_expired'
  ...
  cases.push(caseResult('STALE-STATUS-REFUSED',
    stale.reason === 'equipment_status_stale' && staleFixture.providerEntries === 0, {
      reason: stale.reason,
  ...
  cases.push(caseResult('FUTURE-STATUS-REFUSED',
    futureStatus.reason === 'equipment_status_stale'
  ...
  cases.push(caseResult('REVOKED-STATUS-REFUSED',
    revoked.reason === 'equipment_status_revoked' && revokedFixture.providerEntries === 0, {
  ...
  cases.push(caseResult('UNAVAILABLE-STATUS-REFUSED',
    unavailable.reason === 'equipment_status_unavailable' && unavailableFixture.providerEntries === 0, {
  ...
  cases.push(caseResult('UNTRUSTED-STATUS-SOURCE-REFUSED',
    untrusted.reason === 'equipment_status_source_untrusted' && untrustedFixture.providerEntries === 0, {
  ...
  cases.push(caseResult('STATUS-DIGEST-SUBSTITUTION-REFUSED',
    wrongDigest.reason === 'equipment_status_digest_mismatch' && wrongDigestFixture.providerEntries === 0, {
  ...
  cases.push(caseResult('MALFORMED-STATUS-SCOPE-REFUSED',
    malformedScope.reason === 'equipment_status_schema_invalid'
  ...
  cases.push(caseResult('CACHED-ACTIVE-CANNOT-BEAT-FREEZE',
    frozen.reason === 'capability_control_domain_frozen' && frozenFixture.providerEntries === 0, {
      reason: frozen.reason,
      provider_entries: frozenFixture.providerEntries,
      control_domain_status:
        frozenFixture.capabilityStore.getControlDomain(frozenFixture.action.admission_domain_id)?.status ?? null,
    }));

  const replayFixture = await createFixture();
  const first = await replayFixture.attempt();
  const replay = await replayFixture.attempt();
  cases.push(caseResult('REPLAY-REFUSED',
    first.ok && !replay.ok && replayFixture.providerEntries === 1, {
  ...
  cases.push(caseResult('MISSING-AUTHORITY-REFUSED',
    tunnelOnly.ok === false && tunnelOnlyEffects === 0, {
  ...
  const reconciledOperation = uncertainFixture.capabilityStore.getOperation(
    uncertainFixture.action.operation_id,
  );
  const reconciledCapability = uncertainFixture.capabilityStore.getState(
    uncertainFixture.capabilityId,
  );
  cases.push(caseResult('LOST-RESPONSE-IS-INDETERMINATE-AND-CONSUMED',
    uncertain.terminal_outcome === 'indeterminate'
