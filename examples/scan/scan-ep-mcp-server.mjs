...
// Generated from scan-ep-mcp-server.mts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
//
...
for (const r of truePositives)
    console.log(`  ${r.action.name}  ->  ${r.classification.assurance_class}  (${r.classification.category})`);
console.log('\nMutating, unrecognized -> defaulted FAIL-CLOSED for human review:');
for (const r of rep.results.filter((r) => r.classification.decision === 'review_fail_closed'))
    console.log(`  ${r.action.name}`);
console.log('\nHONEST false positives (flagged by keyword, a reviewer downgrades — the tool proposes, you decide):');
for (const [name, why] of Object.entries(KNOWN_FALSE_POSITIVES))
