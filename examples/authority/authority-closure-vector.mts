...
function keyFromSeedHex(hex: string): crypto.KeyObject {
...
function log(title: string): void { console.log(`\n=== ${title} ===`); }
...
const accepted = verifyAuthorityProof(proof, {
  ...
  pinnedRegistryKeys: [{ issuer_id: (proof as any).authority_id as string, public_key: registryPub }],
  expectMinEpoch: 17,
});
