...
import { makeReceiptGate, findActionRequirement, RECEIPT_REQUIRED_STATUS, } from '@emilia-protocol/require-receipt';
...
export async function dispatch(name, args = {}, receipt = null) {
    ...
    const r = await gateFor(req, assurance).run(receipt, { target: args?.table }, async () => performDangerousAction(name, args));
    if (r.ok) {
