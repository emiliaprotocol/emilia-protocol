...
// Generated from example-dangerous-action.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
//
...
import { makeReceiptGate, resolveActionRequirement, bindToolAction, snapshotToolArguments, RECEIPT_REQUIRED_STATUS, } from '@emilia-protocol/require-receipt';
...
const allowInlineKey = () => demoModeAvailable() && /^(1|true)$/i.test(process.env.EMILIA_ALLOW_INLINE_KEY || '');
...
function productionAssuranceConfiguration() {
    try {
        ...
        if (!approverKeys || typeof approverKeys !== 'object' || Array.isArray(approverKeys)
            || !Object.keys(approverKeys).length || !rpId || !allowedOrigins.length)
            return null;
...
async function assuranceConfiguration(requiredClass) {
    if (requiredClass === 'software')
        return {};
...
// response was lost.
function performDangerousAction(name, material) {
...
function gateFor(boundAction, req, assurance) {
...
function refused(status, reason, detail) {
...
function caseInsensitiveNearMatch(name) {
...
export async function dispatch(name, args = {}, receipt = null) {
    ...
    if (!manifestActions().some((entry) => entry?.match?.tool === name)) {
        ...
        return refused(403, 'action_not_in_manifest', `No agent-actions.json entry matches tool "${name}", so this dispatcher `
    ...
    if (resolution.status !== 'one') {
        ...
        return refused(403, resolution.status === 'conflict' || resolution.status === 'none'
            ? 'manifest_selector_conflict'
            : 'manifest_selector_ambiguous', `agent-actions.json does not name one policy for { protocol: "mcp", tool: "${name}" }`
    ...
    try {
        ...
        return refused(400, 'action_binding_invalid', 'Tool arguments are not finite JSON, so they cannot be bound to a receipt.');
    }
    ...
    if (!trustedKeys().length && !allowInlineKey()) {
        return refused(500, 'receipt_enforcement_misconfigured', 'Set EMILIA_TRUSTED_KEYS to the issuer key(s) you trust; '
    ...
    if (!Array.isArray(req.execution_binding?.required_fields)
        ...
        return refused(500, 'manifest_execution_binding_missing', `agent-actions.json entry "${req.id}" is receipt_required but declares no `
    ...
    if (!assurance) {
        return refused(500, 'receipt_assurance_misconfigured', 'Configure EMILIA_APPROVER_KEYS_JSON, EMILIA_RP_ID, and '
    ...
    const r = await gateFor(boundAction, req, assurance).run(receipt, { observedAction: material }, async () => performDangerousAction(name, material));
    if (r.ok) {
 ...
 */
export function receiptRequestFor(name, args = {}) {
    const resolution = resolveActionRequirement(MANIFEST, { protocol: 'mcp', tool: name });
    if (resolution.status !== 'one')
        return null;
