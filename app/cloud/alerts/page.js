...
const s = {
  ...
  card: { background: '#111827', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 10, padding: 24, marginBottom: 16 },
  alertCard: (severity) => {
    const borderMap = { critical: 'rgba(248,113,113,0.3)', high: 'rgba(248,113,113,0.15)', medium: 'rgba(212,175,85,0.15)', low: 'rgba(74,144,217,0.1)' };
    return { ...{ background: '#111827', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 10, padding: 20, marginBottom: 12 }, borderLeft: `3px solid ${borderMap[severity] || 'rgba(255,255,255,0.06)'}` };
  },
  ...
  btnSmall: { padding: '6px 14px', borderRadius: 6, fontFamily: "'IBM Plex Mono', monospace", fontSize: 10, fontWeight: 700, letterSpacing: 1, textTransform: 'uppercase', border: '1px solid rgba(74,144,217,0.3)', cursor: 'pointer', background: 'transparent', color: '#4a90d9' },
      ...
      <div style={s.container}>
        ...
        <div style={s.grid}>
          ...
          <div style={s.statCard}>
            ...
            <div style={{ ...s.statValue, color: '#d4af55' }}>{counts.acknowledged}</div>
          </div>
          <div style={s.statCard}>
        ...
        {filtered.map(a => (
          <div key={a.id} style={s.alertCard(a.severity)}>
            ...
            <div style={s.alertMeta}>
              ...
              {a.handshake && <span style={{ ...s.mono, color: '#4a90d9' }}>HS: {a.handshake}</span>}
              {a.policy && <span style={{ ...s.mono, color: '#d4af55' }}>Policy: {a.policy}</span>}
            </div>
            {a.status === 'active' && (
