...
export default function AuditExportPage() {
  ...
  const s = {
    ...
    eyebrow: { fontFamily: "'JetBrains Mono', monospace", fontSize: 11, letterSpacing: 3, textTransform: 'uppercase', color: '#ffd700', marginBottom: 8 },
    h1: { fontFamily: "'Outfit', sans-serif", fontSize: 28, fontWeight: 900, letterSpacing: -0.5, marginBottom: 8 },
    subtitle: { fontSize: 14, color: '#7a809a', marginBottom: 32 },
    card: { background: '#0e1120', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 12, padding: 24, marginBottom: 24 },
    label: { display: 'block', fontSize: 12, fontWeight: 600, color: '#7a809a', marginBottom: 6, fontFamily: "'JetBrains Mono', monospace", letterSpacing: 0.5 },
    input: { width: '100%', padding: '10px 14px', borderRadius: 8, border: '1px solid rgba(255,255,255,0.08)', background: '#0a0c18', color: '#e8eaf0', fontSize: 14, fontFamily: 'inherit', outline: 'none', boxSizing: 'border-box' },
    select: { width: '100%', padding: '10px 14px', borderRadius: 8, border: '1px solid rgba(255,255,255,0.08)', background: '#0a0c18', color: '#e8eaf0', fontSize: 14, fontFamily: 'inherit', outline: 'none', boxSizing: 'border-box', appearance: 'none' },
    btn: { padding: '10px 24px', borderRadius: 8, fontFamily: "'JetBrains Mono', monospace", fontSize: 12, fontWeight: 700, letterSpacing: 1, textTransform: 'uppercase', border: 'none', cursor: 'pointer' },
    mono: { fontFamily: "'JetBrains Mono', monospace", fontSize: 12 },
    th: { padding: '10px 14px', textAlign: 'left', fontSize: 11, fontWeight: 600, color: '#4a4f6a', fontFamily: "'JetBrains Mono', monospace", letterSpacing: 1, textTransform: 'uppercase', borderBottom: '1px solid rgba(255,255,255,0.06)' },
    td: { padding: '10px 14px', fontSize: 13, borderBottom: '1px solid rgba(255,255,255,0.03)' },
  };

  const integrityBadge = (status) => {
    const colors = { verified: '#00ff88', mismatch: '#f87171', pending: '#ffd700' };
    return (
    ...
    <div style={s.page}>
      <div style={s.container}>
        ...
        <div style={s.card}>
          ...
          <button onClick={handleGenerate} disabled={generating} style={{ ...s.btn, background: generating ? '#1a1e30' : '#ffd700', color: generating ? '#4a4f6a' : '#05060a', marginTop: 20 }}>
          ...
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              ...
              <tbody>
                {reports.map(r => (
                  <tr key={r.id}>
                    <td style={{ ...s.td, ...s.mono, color: '#00d4ff' }}>{r.id}</td>
                    ...
                    <td style={s.td}>
                      {r.status === 'complete' ? (
                        <button style={{ ...s.btn, padding: '4px 12px', background: 'transparent', color: '#00d4ff', border: '1px solid rgba(0,212,255,0.3)', fontSize: 11 }}>Download</button>
                      ) : (
                        <span style={{ ...s.mono, color: '#ffd700' }}>Processing...</span>
                      )}
                    </td>
                  </tr>
                ))}
