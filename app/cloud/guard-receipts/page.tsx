...
export default async function GuardReceiptsPage(): Promise<React.ReactElement> {
  ...
  const authWithTenant = auth as { error: string | null; tenantId: string };
  if (typeof authWithTenant.tenantId !== 'string' || authWithTenant.tenantId.length === 0) {
    return <SignInRequired />;
  }
  ...
  const { receipts, error } = (await loadTenantGuardReceipts({
    ...
    tenantId: authWithTenant.tenantId,
    log: logger,
  })) as { receipts: Array<{ receipt_id: string; action_type: string; status: string; enforcement_mode: string; adapter: string | null; created_at: string }>; error: string | null };
