...
export default function BlogTwoPersonRulePage(): React.ReactElement {
  useEffect(() => {
  ...
  return (
    <div style={styles.page}>
      ...
      <section style={{ ...styles.section, paddingTop: 0, paddingBottom: 48 }}>
        ...
        <p className="ep-reveal" style={styles.body}>
          To earn that claim, the verification has to be unambiguous enough that separate verifiers reach the same result. EMILIA ships JavaScript, Python, and Go reference verifiers in one repository, and a cross-language conformance suite feeds the same adversarial vectors through all three. They agree on authorization receipts, on device signoffs, and on multi-party quorum: the accept cases pass and each reject case (under threshold, duplicate human, out of order, action mismatch, expired window, one bad signature, wrong role) is refused identically. That is a cross-language consistency check, not a clean-room independent-implementation claim, and it runs on every change.
        </p>
      </section>
