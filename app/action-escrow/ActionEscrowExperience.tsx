...
function short(value: unknown, start = 12, end = 8): string {
  if (typeof value !== 'string' || value.length <= start + end + 3) return String(value) || 'not available';
...
function Status({ pass, children, tone = 'green' }: { pass?: boolean; children: React.ReactNode; tone?: string }): React.JSX.Element {
...
function IntegrationRow({ row, index, phase }: { row: any; index: number; phase: number }): React.JSX.Element {
...
export default function ActionEscrowExperience({ data }: { data: any }): React.JSX.Element {
  const [activeOutcome, setActiveOutcome] = useState<string>('approve');
  const [phase, setPhase] = useState<number>(data.integration_rows.length - 1);
