...
function bytesToB64u(bytes: Uint8Array | ArrayLike<number>): string {
  let bin = '';
...
function rawP256ToDer(raw: Uint8Array): Uint8Array {
  const enc = (v) => {
    let i = 0;
...
function buildContext(approver: string, host: string): any {
  return {
...
// ── enrollment ────────────────────────────────────────────────────────────────
async function enrollReal(approver: string, host: string): Promise<any> {
  ...
  if ((cred.response as any).getPublicKeyAlgorithm?.() !== -7) {
    throw new Error('This device did not offer an ES256 passkey.');
  }
  const spki = (cred.response as any).getPublicKey?.();
...
async function enrollSim(): Promise<any> {
...
async function signReal(context: any, cred: any, host: string): Promise<any> {
  ...
  const r = assertion.response as any;
  return {
    context,
    webauthn: {
...
async function signSim(context: any, cred: any, host: string): Promise<any> {
  ...
  const rawSig = new Uint8Array(
    await crypto.subtle.sign({ name: 'ECDSA', hash: 'SHA-256' }, cred.privateKey, signedData as BufferSource),
  );
  return {
    context,
    webauthn: {
...
export default function TryPage(): React.ReactElement {
  const [approver, setApprover] = useState('');
  const [phase, setPhase] = useState('intro'); // intro · ready · signed · forged
  const [mode, setMode] = useState<string | null>(null); // 'real' | 'sim'
  ...
  const [result, setResult] = useState<any>(null); // valid signoff verify
  const [forged, setForged] = useState<any>(null); // { signoff, result }
  ...
  const begin = useCallback(async (useSim: boolean): Promise<void> => {
    setBusy(true);
    setError(null);
    try {
      const h = getHost();
...
// ── small components ──────────────────────────────────────────────────────────
function Step({ n, label }: { n: string | number; label: string }): React.ReactElement {
  return (
...
function ActionCard({ context }: { context: any }): React.ReactElement {
  const rows = [
...
function VerifyPanel({ ok, checks, title, meaning, forged }: { ok: boolean; checks: any; title: string; meaning: string; forged?: boolean }): React.ReactElement {
...
// ── style atoms ───────────────────────────────────────────────────────────────
const card: React.CSSProperties = { background: color.card, border: `1px solid ${color.border}`, borderRadius: radius.base, padding: '24px 24px' };
const lbl: React.CSSProperties = { display: 'block', fontSize: 12, fontWeight: 600, color: color.t2, marginBottom: 6, fontFamily: font.mono, letterSpacing: 0.5 };
const input: React.CSSProperties = { width: '100%', padding: '12px 14px', borderRadius: radius.base, border: `1px solid ${color.inputBorder}`, background: color.card, color: color.t1, fontSize: 15, fontFamily: 'inherit', outline: 'none', marginBottom: 18, boxSizing: 'border-box' };
const primaryBtn = (busy: boolean): React.CSSProperties => ({ width: '100%', background: color.t1, color: '#fff', border: 'none', borderRadius: radius.sm, padding: '14px 24px', fontFamily: font.sans, fontWeight: 600, fontSize: 15, cursor: busy ? 'wait' : 'pointer', opacity: busy ? 0.6 : 1 });
const secondaryBtn: React.CSSProperties = { background: 'transparent', color: color.t1, border: `1px solid ${color.borderHover}`, borderRadius: radius.sm, padding: '11px 18px', fontFamily: font.sans, fontWeight: 600, fontSize: 14, cursor: 'pointer' };
const ghostBtn: React.CSSProperties = { display: 'block', width: '100%', background: 'none', border: 'none', color: color.t3, fontSize: 13, cursor: 'pointer', fontFamily: font.sans, marginTop: 12, textAlign: 'center', textDecoration: 'underline' };
const ghostBtn2: React.CSSProperties = { background: 'none', border: 'none', color: color.t3, fontSize: 14, cursor: 'pointer', fontFamily: font.sans, padding: '11px 8px' };
const lnk: React.CSSProperties = { color: color.blue, textDecoration: 'none' };
