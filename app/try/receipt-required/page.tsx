...
const now = (): string => new Date().toLocaleTimeString([], { hour12: false, hour: '2-digit', minute: '2-digit', second: '2-digit' });
...
export default function ReceiptRequiredTryPage(): React.ReactElement {
  ...
  const [busy, setBusy] = useState<'missing' | 'sign' | 'run' | 'replay' | 'forge' | null>(null);
  const [receipt, setReceipt] = useState<any>(null);
  const [evidence, setEvidence] = useState<any>(null);
  const [challenge, setChallenge] = useState<any>(null);
  const [copied, setCopied] = useState(false);
  const [feed, setFeed] = useState<Array<{ at: string; kind: string; text: string }>>([
...
function PanelTop({ label, value }: { label: string; value: string }): React.ReactElement {
  return (
...
function Line({ children, muted, danger }: { children: React.ReactNode; muted?: boolean; danger?: boolean }): React.ReactElement {
  return (
...
function Strike({ strike, state, index, active }: { strike: any; state: any; index: number; active: boolean }): React.ReactElement {
...
function tone(kind: string): React.CSSProperties {
...
function gateShadow(kind: string): React.CSSProperties {
...
function ringTone(kind: string): React.CSSProperties {
...
function strikeTone(status: string): React.CSSProperties {
