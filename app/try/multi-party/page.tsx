...
const b64u = (bytes: Uint8Array | ArrayLike<number>): string => btoa(String.fromCharCode(...Array.from(bytes))).replace(/\+/g, '-').replace(/\//g, '_').replace(/=+$/, '');
function canonicalize(v: any): string {
  if (v === null || typeof v !== 'object') return JSON.stringify(v);
  if (Array.isArray(v)) return `[${v.map(canonicalize).join(',')}]`;
  return `{${Object.keys(v).sort().map((k) => JSON.stringify(k) + ':' + canonicalize(v[k])).join(',')}}`;
}
const sha256 = async (bytes: Uint8Array): Promise<Uint8Array> => new Uint8Array(await crypto.subtle.digest('SHA-256', bytes as BufferSource));
...
async function signSim(context: any): Promise<{ approver_public_key: string; signoff: any }> {
  ...
  const rawSig = new Uint8Array(await crypto.subtle.sign({ name: 'ECDSA', hash: 'SHA-256' }, pair.privateKey, signedData as BufferSource));
  return {
...
export default function MultiPartyDemo(): React.ReactElement {
  const [actionHash, setActionHash] = useState<string | null>(null);
  const [members, setMembers] = useState<Array<{ role: string; approver_public_key: string; signoff: any }>>([]);
  const [result, setResult] = useState<{ valid: boolean; checks: Record<string, boolean>; members: any[] } | null>(null);
  ...
  const mkContext = (slot: typeof ROSTER[0], h: string, idx: number): any => ({
  ...
  const buildQuorum = (mem: any[], h: string): any => ({
  ...
  async function signNext(): Promise<void> {
    setBusy(true);
    try {
  ...
  async function cheatDuplicate(): Promise<void> {
    setBusy(true);
    try {
      const h = await ensureHash();
      const po = ROSTER[0];
  ...
  function reset(): void { setMembers([]); setResult(null); setCheated(false); }
...
function btn(c: string, ghost?: boolean): React.CSSProperties {
  return {
