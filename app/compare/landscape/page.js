    ...
    <div style={styles.page}>
      ...
      <section style={{ ...styles.section, paddingTop: 0, paddingBottom: 56 }}>
        ...
        <ul className="ep-reveal" style={styles.list}>
          ...
          <li><strong style={{ color: color.t1 }}>Formal verification.</strong> The protocol core is machine-checked — 26 TLA+ theorems + 35 Alloy facts, run in CI on every change to the formal models. The receipt cluster proves tamper-evidence; none claim machine-checked proofs of the protocol itself.</li>
          <li><strong style={{ color: color.t1 }}>Three cross-language verifiers, proven to agree.</strong> JavaScript, Python, and Go reference verifiers (one team's ports in one repository, a consistency check rather than independent reimplementations; independent implementations remain future interoperability evidence) agree on the canonical adversarial conformance vectors, re-proven on every push. Publishing a public cross-language conformance suite at all is uncommon in this landscape (corrections welcome). The vectors are public (<a href="https://github.com/emiliaprotocol/emilia-protocol/blob/main/CONFORMANCE.md" style={{ color: color.blue, textDecoration: 'none' }}>CONFORMANCE.md</a>) — any project here, including the ones above, is welcome to conform against them.</li>
