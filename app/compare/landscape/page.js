    ...
    <div style={styles.page}>
      ...
      <section style={{ ...styles.section, paddingTop: 0, paddingBottom: 56 }}>
        ...
        <ul className="ep-reveal" style={styles.list}>
          ...
          <li><strong style={{ color: color.t1 }}>Three cross-language verifiers, proven to agree.</strong> JavaScript, Python, and Go reference verifiers (one team's ports in one repository, a consistency check rather than independent reimplementations; separately, an externally authored from-spec Rust implementation (source public) agrees on all 162 published vectors, with construction independence the implementer's attestation, auditable in the public source) agree on the canonical adversarial conformance vectors, re-proven on every push. Publishing a public cross-language conformance suite at all is uncommon in this landscape (corrections welcome). The vectors are public (<a href="https://github.com/emiliaprotocol/emilia-protocol/blob/main/CONFORMANCE.md" style={{ color: color.blue, textDecoration: 'none' }}>CONFORMANCE.md</a>) — any project here, including the ones above, is welcome to conform against them.</li>
