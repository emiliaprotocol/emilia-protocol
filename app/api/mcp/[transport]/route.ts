...
const text = (obj: unknown): { content: { type: 'text'; text: string }[] } => ({
  content: [{ type: 'text', text: JSON.stringify(obj, null, 2) }],
});

type McpHandler = (request: Request, context?: any) => Promise<Response>;

const handler: McpHandler = createMcpHandler(
  (server) => {
    server.registerTool(
      'ep_verify_receipt',
      {
...
export async function POST(request: Request, context?: { params: Promise<{ transport: string }> }): Promise<Response> {
