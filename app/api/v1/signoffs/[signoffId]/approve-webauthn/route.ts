...
function submittedChallenge(assertion: any): string | null {
  try {
...
export async function POST(
  ...
  try {
    ...
    const parsed = await readLimitedJson(request, MAX_WEBAUTHN_APPROVE_BYTES, { invalidValue: {} } as any);
    if (!parsed.ok) {
    ...
    try {
      ...
      await supabase.from('signoff_metrics').upsert({
        ...
        approver_id: body.approver_id,
        signed_at: decidedAt,
        time_to_sign_ms: renderedAt ? Math.max(0, (signedAt as any) - (renderedAt as any)) : null,
        decision,
        key_class: 'A',
