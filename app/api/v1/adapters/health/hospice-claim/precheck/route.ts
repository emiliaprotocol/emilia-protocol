...
function safeRequirements(...candidates: unknown[]): string[] {
  const candidate = candidates.find((value) => Array.isArray(value)) as unknown[] | undefined;
...
// The request body and the action derived from it are untrusted, dynamically
// shaped JSON — typed `any` deliberately, matching every other field/property
// access below that assumes no particular shape until validateActionShape
// has run.
function actionFromBody(body: any): any {
...
function containsProhibitedPhi(
  value: unknown,
  depth = 0,
...
function validateActionShape(action: any): { code: string; detail: string } | null {
...
function tenantBoundAction(
  auth: any,
  body: any,
  action: any,
): { error?: { status: number; code: string; detail: string }; action?: any } {
...
function engineContractFailure(): NextResponse {
  return problem(503, 'program_integrity_engine_unavailable', 'Program integrity decision service is unavailable');
}

function responseForPrecheck(prepared: any, checked: any, authorization: unknown): NextResponse {
