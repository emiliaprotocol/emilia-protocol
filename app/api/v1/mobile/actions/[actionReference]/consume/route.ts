...
export async function POST(
  ...
  try {
    ...
    const parsed = await readLimitedJson(request, 8 * 1024, { invalidValue: null } as any) as
      | { ok: true; value: any }
    ...
    if (result.ok !== true) {
      // consumeMobileAction returns Json (Record<string, unknown>) — reason is
      // genuinely dynamic RPC-sourced data, so it is read once and cast here
      // rather than casting at every access point below.
      const reason = result.reason as string;
      return mobileProblem(
        ['already_consumed', 'superseded'].includes(reason) ? 409 : 422,
        reason || 'consumption_refused',
