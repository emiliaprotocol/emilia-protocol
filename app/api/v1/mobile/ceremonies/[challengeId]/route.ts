...
export async function GET(
  ...
  try {
    ...
    const result = await lookupMobileCeremonyResult(supabase, {
      ...
      // MobileSession.device_key_id is nullable (a session paired before
      // device-key binding existed); lookupMobileCeremonyResult already
      // guards for a non-string/empty deviceKeyId at runtime and returns
      // null, so a null value here is handled, just not reflected in its
      // (stricter) declared parameter type.
      deviceKeyId: session.device_key_id as string,
      challengeId,
    });
