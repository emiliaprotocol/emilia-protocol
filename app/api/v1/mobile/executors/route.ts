...
const MEMBERS: Set<string> = new Set(['executor_id', 'key_id', 'public_key']);
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    try { requirePermission(auth as any, 'admin'); } catch {
    ...
    const parsed = await readLimitedJson(request, 16 * 1024, { invalidValue: null } as any) as
      | { ok: true; value: any }
