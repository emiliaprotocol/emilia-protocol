...
export async function GET(request) {
  try {
    ...
    if (!networkLimit.allowed) return mobileProblem(429, 'rate_limited', 'Too many mobile history requests');
    ...
    if (!sessionLimit.allowed) return mobileProblem(429, 'rate_limited', 'Too many mobile history requests');
    ...
    return mobileJson({
      ...
      actions: actions.map((item) => mobileActionView(item, { includePassport: true })),
    }, { headers: { 'cache-control': 'no-store' } });
  } catch (error) {
    logger.error('[mobile] history failed', error);
    return mobileProblem(503, 'mobile_history_unavailable', 'Mobile action history unavailable');
  }
}
