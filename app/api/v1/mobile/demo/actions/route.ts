...
const MEMBERS: Set<string> = new Set(['approver_id', 'scenario']);
...
function controlledPresentation(action: Record<string, any>, details: Record<string, any>): Record<string, any> {
  return {
...
function scenario(name: string, actionReference: string, approverId: string): DemoScenario {
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    try { requirePermission(auth as any, 'write'); } catch {
      return mobileProblem(403, 'insufficient_permission', 'A write-capable organization key is required');
    }
    const limited = await checkRateLimit(authEntityId(auth as any), 'protocol_write');
    ...
    await createDemoAction(getGuardedClient(), {
      ...
      entity_ref: authEntityId(auth as any),
