 ...
 */
type EnrollmentBasisResult = {
  error?: { status: number; code: string; detail: string };
  basis?: 'directory' | 'operator_attested';
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const parsed = await readLimitedJson(
      request,
      MAX_WEBAUTHN_REGISTER_VERIFY_BYTES,
      { invalidValue: {} } as any,
    ...
    const basisResolution = await resolveEnrollmentBasis(supabase, organizationId, body.approver_id) as EnrollmentBasisResult;
    ...
    const { data: registration, error: registrationErr } = await supabase.rpc(
      'complete_webauthn_registration_atomic',
      {
        ...
        p_credential: {
          ...
          attested_by: authEntityId(auth as any),
