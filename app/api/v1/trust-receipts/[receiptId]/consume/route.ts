...
export async function POST(
  ...
  try {
    ...
    const parsed = await readLimitedJson(request, MAX_TRUST_RECEIPT_CONSUME_BYTES, { invalidValue: {} } as any);
    if (!parsed.ok) {
    ...
    let authorityFacts: any = null;
    const registryBindingsForConsume: any[] = [];
    ...
    if (base.signoff_required || base.quorum_policy) {
      ...
      if (base.quorum_policy) {
        ...
        const rosterRoleByApprover = new Map<string, any>(
          (Array.isArray(base.quorum_policy.approvers) ? base.quorum_policy.approvers : [])
            .filter((entry: any) => entry?.approver && entry?.role)
            .map((entry: any) => [entry.approver, entry.role]),
        );
        ...
        const credentialIds = rosterDecisions
          ...
          .filter(Boolean);
        let credsByCredentialId: Record<string, any> = {};
        ...
        const authorizedDecisions: Record<string, any>[] = [];
        ...
        if (isTierQuorumEnforced() && base.signoff_tier === 'dual') {
          ...
          const validBindingsByApprover = new Map<string, any>();
