...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const parsed = await readLimitedJson(request, MAX_TRUST_RECEIPT_CREATE_BYTES, { invalidValue: {} } as any);
    if (!parsed.ok) {
    ...
    const baseDecision = evaluateGuardPolicy({
      ...
      targetChangedFields: changedFields as string[],
      amount: body.amount,
      currency: body.currency,
    ...
    const decision: {
      ...
      aml_signals?: string[];
    } = applyEnforcementMode(baseDecision, mode);
    ...
    if (isRulesEngineV0Enabled()) {
      try {
        const rulesEngineInput: Parameters<typeof evaluateRulesEngineV0>[0] = {
