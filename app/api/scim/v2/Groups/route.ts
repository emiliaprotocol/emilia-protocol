...
const GROUP_FILTER_COLUMN: Record<string, string> = { displayName: 'display_name', externalId: 'external_id', id: 'id' };
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  const filter = parseFilter(url.searchParams.get('filter')) as ScimFilter | null;
  ...
  try {
    ...
    if (filter) {
      const column = GROUP_FILTER_COLUMN[filter.attribute as string];
    ...
    const resources = (count === 0 ? [] : (data || [])).map((row: any) => toScimGroup(row, base));
