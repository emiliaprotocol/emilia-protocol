...
type HandshakeCreateValidation =
  | {
      valid: true;
      data: {
        mode: string;
...
// lib/handshake/create.js documents these shapes via JSDoc @param/@typedef
// comments, not real exported TS types (and its untyped destructured
// parameter defaults make the *inferred* input type narrower than the
// documented contract), so both are redeclared here from the JSDoc contract
// rather than imported.
type InitiateHandshakeInput = {
  mode: string;
  policy_id: string;
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const { valid, data, errors } = validateHandshakeCreate(body) as HandshakeCreateValidation;
    if (!valid) {
    ...
    const initiators = data.parties.filter((p: any) => p.role === 'initiator');
    ...
    const result = (await initiateHandshake({
      actor: actorId,
      ...data,
    } as any)) as InitiateHandshakeResult;
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const result: any = await listHandshakes(filters, actorId);
