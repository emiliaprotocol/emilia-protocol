...
export async function POST(
  ...
  try {
    ...
    if (!valid) {
      ...
      return epError(EP_ERROR_CODES.INVALID_INPUT, (errors as string[]).join('; '));
    }

    // ── Manual validation (belt-and-suspenders fallback) ──────────────
    const validation = validatePresentBody(body);
    if (!validation.valid) {
      return epError(EP_ERROR_CODES.INVALID_INPUT, (validation as { valid: false; error: string }).error);
    }
