...
// readLimitedJson's inferred return type doesn't discriminate cleanly on `ok`
// (lib/http/body-limit.js is still untyped) — pin the real contract here and
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const parsed = (await readLimitedJson(request, MAX_BODY_BYTES)) as ReadLimitedJsonResult;
    ...
    const response: { receipt: any; entity_score: any; warnings?: any } = {
