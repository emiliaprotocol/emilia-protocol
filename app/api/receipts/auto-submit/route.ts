...
type AutoSubmitAuthResult = {
  actor?: AutoSubmitActor;
  error?: string;
};
...
type ReadEpJsonResult =
  | { ok: true; value: any }
  | { ok: false; response: NextResponse; error?: { code?: string; [key: string]: any } };
 ...
 *
 */
async function authenticateAutoSubmit(request: NextRequest): Promise<AutoSubmitAuthResult> {
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    // -----------------------------------------------------------------
    const validated: { index: number; idempotency_key: string; receipt: Record<string, any> }[] = [];
    ...
    // -----------------------------------------------------------------
    const receiptResults: { receipt_id: any; idempotency_key: string; deduplicated: boolean }[] = [];
    const canonicalErrors: Record<string, any>[] = [];
 ...
 *
 * @param receipt - The validated receipt object
 * @returns Hex-encoded SHA-256 hash
 */
function computeIdempotencyKey(receipt: Record<string, any>): string {
  const parts = [
    receipt.entity_id || '',
 ...
 *
 * @param value
 * @returns the sanitized plain object, or null
 */
function sanitizeJson(value: any): Record<string, any> | null {
