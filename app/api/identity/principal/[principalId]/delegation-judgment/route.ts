...
type JudgmentGrade = 'excellent' | 'good' | 'fair' | 'poor';

// Grade thresholds
function computeGrade(score: number): JudgmentGrade {
...
function buildInterpretation(
  ...
  grade: JudgmentGrade,
  agentsAuthorized: number,
  activeAgents: number,
  goodOutcomeRate: number | null,
  totalSignals: number
): string {
...
export async function GET(
  ...
  try {
    ...
    let avgAgentConfidence: number | null = null;
    if (delegationRows && delegationRows.length > 0) {
      const agentEntityIds: string[] = [...new Set<string>(delegationRows.map((d: any) => d.agent_entity_id))];
