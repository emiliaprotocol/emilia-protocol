...
import { BillingServiceError, reconcileAuthorityMonitoring } from '@/lib/works/billing-service';
import { isWorksV0Enabled } from '@/lib/works/env';
import { retrieveStripeSubscription } from '@/lib/works/stripe-gateway';
...
export async function POST(request: NextRequest, { params }: Context) {
  ...
  try {
    ...
    const entitlement = await reconcileAuthorityMonitoring({
      recordId,
      ownerToken,
    ...
    return epProblem(500, 'authority_billing_internal_error', 'Monitoring reconciliation failed.');
  }
}
