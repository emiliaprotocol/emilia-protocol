...
import { validateAssignmentCommand } from '@/lib/works/workflow-model';
...
export async function POST(request: NextRequest, { params }: RouteContext) {
  try {
    ...
    const command = validateAssignmentCommand(parsed.value);
    ...
    const result = await createSupabaseWorksWorkflowStore()
      .commandAssignment(auth.actor.ownerEntityId, id, command.value);
