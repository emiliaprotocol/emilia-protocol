 ...
 */
async function consumeSamlResponse(
  tenant: string,
  replayKey: string,
): Promise<'fresh' | 'replayed' | 'unavailable'> {
  try {
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  // readLimitedText's own return type is presently inferred (its source
  // module has not been given explicit type annotations yet), which loses
  // the `ok`-discriminated union shape its JSDoc has always documented. Pin
  // the real, unchanged contract here so this call site narrows correctly.
  const read = (await readLimitedText(request, MAX_SAML_ACS_BYTES)) as
    | { ok: true; text: string }
  ...
  res.cookies.set(SESSION_COOKIE, token, SESSION_COOKIE_OPTIONS as any);
...
async function resolveDirectory(
  tenant: string,
  profile: any,
): Promise<{ matched: boolean; active: boolean; user_id?: string; error?: string }> {
  try {
    const supabase = getGuardedClient();
