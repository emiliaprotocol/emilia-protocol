...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    doc = await discover(issuer.url as string);
  } catch {
  ...
  const token = await mintSession({
    ...
    subject: verdict.subject as string,
    email: verdict.email,
    protocol: 'oidc',
    directory,
  });
  ...
  res.cookies.set(SESSION_COOKIE, token, SESSION_COOKIE_OPTIONS as Partial<ResponseCookie>);
