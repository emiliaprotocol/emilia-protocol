...
export async function GET(request: NextRequest): Promise<Response> {
  ...
  return Response.json({
    ...
    expires_at: new Date((session.exp as number) * 1000).toISOString(),
  });
}
...
export async function DELETE(request: NextRequest): Promise<Response> {
  ...
  if (session?.jti) {
    await revokeSession(session.jti, {
      subject: session.sub as any,
      tenant: session.tenant as any,
      ...
      expiresAt: (session.exp ? new Date((session.exp as number) * 1000).toISOString() : null) as any,
    });
  }
...
export async function POST(request: NextRequest): Promise<Response> {
  ...
  const session = await readSessionFromRequest(request);
  if (!session) return epProblem(401, 'no_session', 'No valid EP session');
  const ok = await revokeAllSessionsForSubject(session.sub as string, session.tenant as any);
