 ...
 */
function sanitizePostgrestInput(str: any): string {
...
function enrichWithMaterializedTrust(
  results: any,
  { minConfidence, rankBy }: { minConfidence?: string | null; rankBy?: string | null }
): any[] {
  let enriched = (results || []).map((e: any) => {
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const minScore = parseFloat(searchParams.get('min_score') as string) || 0;
    ...
    const limit = Math.min(parseInt(searchParams.get('limit') as string) || 20, 50);
