...
export async function GET(_request: any, { params }: { params: Promise<{ scenarioId: string }> }) {
  ...
  const verdict: any = s.mode === 'enforced'
    ? evaluateGuardPolicy({
...
function bump(iso: string, seconds: number): string {
