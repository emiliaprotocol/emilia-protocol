...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    // `any` and cast the result to the documented ok/error union.
    const parsed = (await (readLimitedJson as any)(request, MAX_TRUST_GATE_BYTES, { invalidValue: {} })) as
      | { ok: true; value: any }
    ...
    let handshakeVerified: boolean | null = null;
    let handshakeBinding: any = null;
    ...
    let decision: 'allow' | 'review' | 'deny' = reasons.length === 0 ? 'allow' : 'deny';
    ...
    const extensions: {
      action: any;
      display_name: any;
      delegation_verified?: boolean | null;
      value_threshold?: { value_usd: any; escalated_to_strict: boolean };
      handshake_verified?: boolean | null;
      _note?: string;
      commit_ref?: any;
    } = {
      action,
    ...
    return NextResponse.json(buildTrustDecision({
      decision,
      entityId: result.entity_id || entity_id,
      policyUsed: policy,
      confidence: conf,
      reasons,
      warnings,
