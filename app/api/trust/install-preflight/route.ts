...
export async function POST(request: NextRequest): Promise<Response> {
  try {
    ...
    const parsed = (await readEpJson(request, MAX_BODY_BYTES, {})) as
    ...
    return NextResponse.json(buildTrustDecision({
      decision,
      entityId: result.entity_id || body.entity_id,
      policyUsed: pr?.policyName || 'standard',
      confidence: result.confidence,
      reasons,
      warnings: pr?.warnings || [],
      contextUsed: typeof result.contextUsed === 'object' ? result.contextUsed : undefined,
      profileSummary: {
        confidence: result.confidence,
