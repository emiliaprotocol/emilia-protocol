...
export async function POST(request: NextRequest): Promise<Response> {
  try {
    ...
    // readEpJson's return shape isn't a native TS annotation yet; cast to its
    // documented ok/error union rather than the loosely-inferred one.
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const supabase = getGuardedClient();
    // verifyCommitmentProof's return shape varies by verification outcome
    // (valid vs. each failure reason carries different fields); consumed here
    // across all of those shapes rather than narrowed to one.
    const result: any = await verifyCommitmentProof(proofId, supabase);
