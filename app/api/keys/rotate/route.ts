...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    if (auth.error) {
      const authError = auth as { error: string; code: string; status: number };
      return epProblem(authError.status, authError.code, authError.error);
    }
    ...
    const apiKey = (request.headers.get('authorization') as string).replace('Bearer ', '');
