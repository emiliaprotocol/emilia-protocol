 ...
 */
function approxWidth(text: string, perChar = 6.2, pad = 10): number {
 ...
 */
async function resolveCapability(entityId: string): Promise<Record<string, any>> {
...
function renderSvg({ entityId, capabilityOn }: Record<string, any>): string {
  const left = 'EMILIA';
