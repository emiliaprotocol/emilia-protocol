...
export async function POST(request: NextRequest, { params }: { params: Promise<{ policyId: string }> }) {
  try {
    const auth = await authenticateCloudRequest(request) as { tenantId: string; environment: string; permissions: string[]; keyId: string; operatorId?: string; principalId?: string } | null;
    ...
    const { data: rollout, error: insertErr } = await supabase
      .rpc('activate_policy_rollout_authorized', {
        ...
        p_authority_ids: (authorization as any).authorityIds,
