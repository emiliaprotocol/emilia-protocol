...
function redactForDisplay(value: any, depth = 0): any {
...
function ResultPanel({ title, data, status }: { title: string; data: any; status: 'success' | 'error' }): React.ReactElement | null {
  if (!data) return null;
...
export default function PlaygroundPage(): React.ReactElement {
  ...
  const [result, setResult] = useState<{ data: any; status: 'success' | 'error'; title: string } | null>(null);
  ...
  async function createEntity(name: string): Promise<void> {
  ...
  async function getProfile(entityId: string): Promise<void> {
  ...
  async function consumeHandshake(hsId: string): Promise<void> {
  ...
  async function verifyReceipt(): Promise<void> {
    ...
    const receiptId = (receipts[0] as any)?.payload?.receipt_id || (receipts[0] as any)?.receipt_id;
    if (receiptId) {
