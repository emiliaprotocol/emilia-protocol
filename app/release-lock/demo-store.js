...
function mergeCeremonyState(parsed, ceremony) {
...
export function createDemoReleaseLock(input) {
...
export function enrollDemoCredential(role) {
...
export function recordDemoApproval(ceremony, role, bindings) {
  const current = readDemoState();
...
export function advanceDemoMilestone() {
  const current = readDemoState();
...
export function buildAmendedDemoState(current) {
  const amendedLock = {
    ...current.lock,
...
export function amendDemoReleaseLock() {
  return writeDemoState(buildAmendedDemoState(readDemoState()));
}

export function subscribeToDemoState(callback) {
  if (typeof window === 'undefined') return () => {};

  const onCustomEvent = (event) => callback(event.detail || readDemoState());
...
export function buildDemoEvidencePackage() {
