...
function CeremonyLedger({ lock, state }) {
...
function PartyMatrix({ lock, state }) {
...
function CeremonyApproval({
  ...
  const approval = ceremonyState.approvals[role];
  const otherRole = role === 'customer' ? 'contractor' : 'customer';
  const otherApproved = Boolean(ceremonyState.approvals[otherRole]);
...
export default function ReleaseLockExperience({
  ...
  const [mirrorOpen, setMirrorOpen] = useState(null);
