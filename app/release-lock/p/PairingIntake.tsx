...
interface PairingIntakeProps {
  lockId?: string;
  role?: string;
  round?: string;
}

export default function PairingIntake({ lockId, role, round }: PairingIntakeProps) {
  const demo = isReleaseLockDemoMode();
  const eligible = !demo
    && Boolean(lockId)
    && ['contractor', 'customer'].includes(role || '')
    && ['co-accepted', 'draw-release'].includes(round || '');
  const [status, setStatus] = useState<'opening' | 'unavailable'>(eligible ? 'opening' : 'unavailable');
