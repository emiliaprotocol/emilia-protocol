...
export function releaseLockRoundSegment(ceremony: string): string {
...
export const RELEASE_LOCK_ENDPOINTS = Object.freeze({
  ...
  get: (lockId: unknown) => `/${encodeSegment(lockId)}/view`,
  registerOptions: (lockId: unknown) => `/${encodeSegment(lockId)}/registration/options`,
  registerVerify: (lockId: unknown) => `/${encodeSegment(lockId)}/registration/verify`,
  resolutionOptions: (lockId: unknown, round: unknown) => (
    `/${encodeSegment(lockId)}/rounds/${encodeSegment(round)}/action-check/options`
  ),
  resolutionSubmit: (lockId: unknown, round: unknown) => (
    `/${encodeSegment(lockId)}/rounds/${encodeSegment(round)}/approvals`
  ),
  createPairing: (lockId: unknown, round: unknown) => (
    `/${encodeSegment(lockId)}/rounds/${encodeSegment(round)}/pairings`
  ),
  evidence: (lockId: unknown) => `/${encodeSegment(lockId)}/evidence`,
  participantEvidence: (lockId: unknown) => `/${encodeSegment(lockId)}/participant-evidence`,
  stageDraw: (lockId: unknown) => `/${encodeSegment(lockId)}/draw-release`,
  amend: (lockId: unknown) => `/${encodeSegment(lockId)}/amendments`,
});

export function isReleaseLockDemoMode(): boolean {
  return !API_BASE;
}

export function isReleaseLockDemoPilotToken(value: unknown): boolean {
...
function displayMaterial(value: unknown, fallback: string): string {
...
function participantApproval(decisions: any[], round: string, role: string): any {
  const decision = decisions.find(
    (entry: any) => entry.round === round && entry.role === role && entry.invalidated !== true,
  );
...
export function normalizeReleaseLockParticipantView(view: any): any {
...
export async function createReleaseLock({ pilotToken, lock }: { pilotToken: unknown; lock: any }): Promise<any> {
...
function safeCleanPath(value: unknown, lockId: string): string {
  if (!value) return `/release-lock/${encodeSegment(lockId)}`;
  try {
    const parsed = new URL(String(value), 'https://release-lock.invalid');
...
export async function exchangeReleaseLockCapability(input: unknown): Promise<any> {
  const inputObj = input as any;
  const token = typeof input === 'string' ? input : inputObj?.token;
  if (isReleaseLockDemoMode()) {
...
export async function createReleaseLockPairing(lockId: string, ceremony: string): Promise<any> {
  if (isReleaseLockDemoMode()) {
    return {
...
export async function exchangeReleaseLockPairing({
  ...
  role,
  round,
}: { token: unknown; lockId: string; role: string; round: string }): Promise<any> {
...
export async function getReleaseLock(lockId: string): Promise<any> {
  if (isReleaseLockDemoMode()) {
...
export async function getRegistrationOptions(lockId: string, payload: any): Promise<any> {
  if (isReleaseLockDemoMode()) {
    const state = typeof window === 'undefined' ? null : readDemoState();
    return {
      demo: true,
...
export async function verifyRegistration(lockId: string, payload: any): Promise<any> {
  if (isReleaseLockDemoMode()) {
...
export async function getResolutionOptions(lockId: string, payload: any): Promise<any> {
  if (isReleaseLockDemoMode()) {
    return {
      demo: true,
...
export async function submitResolution(lockId: string, payload: any): Promise<any> {
  if (isReleaseLockDemoMode()) {
...
export async function getReleaseLockEvidence(lockId: string): Promise<Blob> {
  if (isReleaseLockDemoMode()) {
