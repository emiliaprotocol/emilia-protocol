  ...
  return (
    ...
    <nav style={{
          ...
          <a href="/" style={{ display: 'flex', alignItems: 'center', gap: 10, textDecoration: 'none' }}>
            <svg width="28" height="28" viewBox="0 0 34 34" fill="none"><rect x="7" y="5" width="2.5" height="24" rx="1.25" fill="url(#ng2)"/><rect x="9.5" y="5" width="16" height="2.5" rx="1.25" fill="#60a5fa"/><rect x="9.5" y="15.5" width="12" height="2.5" rx="1.25" fill="#f59e0b"/><rect x="9.5" y="26.5" width="14" height="2.5" rx="1.25" fill="#60a5fa"/><defs><linearGradient id="ng2" x1="8" y1="5" x2="8" y2="29"><stop offset="0%" stopColor="#60a5fa"/><stop offset="100%" stopColor="#f59e0b"/></linearGradient></defs></svg>
            <span style={{ fontFamily: 'monospace', fontWeight: 700, fontSize: 14, letterSpacing: 3, color: '#e8e6e3', textTransform: 'uppercase' }}>EMILIA</span>
          </a>
          <div style={{ display: 'flex', alignItems: 'center', gap: 24 }}>
            {[
              ['/', 'Home'], ['/quickstart.html', 'Quickstart'], ['/demo.html', 'Demo'],
              ['/spec', 'Spec'], ['/operators.html', 'Operators'], ['/appeal', 'Appeal'],
              ['https://github.com/emiliaprotocol/emilia-protocol', 'GitHub'],
            ].map(([href, label]) => (
              <a key={label} href={href} target={href.startsWith('http') ? '_blank' : undefined}
                style={{ fontFamily: 'monospace', fontSize: 11, letterSpacing: 1, textTransform: 'uppercase',
                  color: label === 'Appeal' ? '#00d4ff' : '#4a4f6a', textDecoration: 'none' }}>{label}</a>
            ))}
            <a href="/apply" style={{ background: '#00d4ff', color: '#0a0b14', padding: '8px 18px', borderRadius: 8,
              textDecoration: 'none', fontSize: 12, fontFamily: 'monospace', fontWeight: 700, letterSpacing: 1, textTransform: 'uppercase' }}>Apply to Review</a>
          </div>
        </nav>
    <div style={s.page}>
