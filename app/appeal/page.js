...
export default function AppealPage() {
  const [mode, setMode] = useState('lookup'); // lookup | report | status
  ...
  const s = {
    ...
    value: { fontSize: 16, color: '#e8e6e3', fontWeight: 600 },
  ...
  return (
    <div style={s.page}>
      <div style={s.container}>
        ...
        <p style={s.sub}>
          Look up any entity's trust profile, report a trust issue, or check the status of a dispute.
          No account required. No API key needed. If you've been wronged by a trust decision, you can raise your hand here.
        </p>
          ...
          <div>
            ...
            {result?.type === 'dispute' && (
              <div style={s.card}>
                ...
                <div style={s.row}><span style={{ color: '#8b8fa3' }}>Status</span><span style={{ color: result.data.status === 'reversed' ? '#00ff88' : result.data.status === 'open' ? '#ffd700' : '#8b8fa3' }}>{result.data.status}</span></div>
                ...
                {result.data.resolution_rationale && <div style={{ marginTop: 12, fontSize: 13, color: '#8b8fa3', lineHeight: 1.6 }}>{result.data.resolution_rationale}</div>}
              </div>
            )}
          </div>
        )}
        ...
        <div style={{ marginTop: 48, padding: 24, background: 'rgba(255,255,255,0.02)', borderRadius: 12, border: '1px solid rgba(255,255,255,0.05)' }}>
          <div style={{ fontSize: 11, color: '#8b8fa3', fontFamily: 'monospace', letterSpacing: 1, marginBottom: 8 }}>HOW DISPUTES WORK</div>
          <div style={{ fontSize: 13, color: '#8b8fa3', lineHeight: 1.8 }}>
            ...
            6. The entity's trust profile <strong style={{ color: '#e8e6e3' }}>immediately recomputes</strong> to reflect the reversal.
          </div>
        </div>
      </div>
    </div>
