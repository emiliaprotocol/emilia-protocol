...
interface SandboxReport {
  headline: string;
  summary: { total_actions: number; would_require_signoff: number; would_deny: number; would_allow: number };
  samples?: Array<{ action_type: string; amount?: number; would_have: string; signoff_tier?: string }>;
  evidence_packet?: { gg1: { badge: string }; verification: { offline_command: string } };
  next_step: string;
}
 ...
 */
function samplesFor(vertical: string, orgId: any): Array<[string, string, Record<string, any>]> {
  if (vertical === 'fin') {
    return [
...
export default function SandboxPage(): React.ReactElement {
  ...
  const [creds, setCreds] = useState<SandboxCreds | null>(null); // { api_key, organization_id, ... }
  const [busy, setBusy] = useState('');
  const [ran, setRan] = useState<RanEntry[]>([]); // [{ label, decision }]
        ...
        <section style={card}>
          ...
          {!creds ? (
            <>
              ...
              <select id="v" value={vertical} onChange={(e) => setVertical(e.target.value)} style={{ ...input, appearance: 'auto' } as React.CSSProperties}>
          ...
          <section style={card}>
            ...
            <div style={{ display: 'grid', gap: 8 }}>
              {samples.map(([label, path, body]) => (
                <button key={label} onClick={() => runSample(label, path, body)} disabled={busy === label} style={sampleBtn as React.CSSProperties}>
...
function Stat({ n, label, accent = color.t1 }: { n: number; label: string; accent?: string }): React.ReactElement {
  return (
