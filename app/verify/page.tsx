...
function findKey(doc: unknown, names: string[]): string | null {
  if (typeof doc !== 'object' || !doc) return null;
  const d = doc as Record<string, unknown>;
  for (const n of names) {
    if (typeof d[n] === 'string') return d[n] as string;
    if (typeof d.context === 'object' && d.context && typeof (d.context as Record<string, unknown>)[n] === 'string') return (d.context as Record<string, unknown>)[n] as string;
    if (typeof d.signer === 'object' && d.signer && typeof (d.signer as Record<string, unknown>)[n] === 'string') return (d.signer as Record<string, unknown>)[n] as string;
  }
  return null;
}
...
function present(kind: string | null, result: any): { title: string; labels: Record<string, string>; rows: Array<[string, unknown]>; meaning: string } {
  if (kind === 'receipt') {
...
async function verifyAny(doc: unknown): Promise<{ kind: string | null; result?: unknown; needKey?: boolean }> {
  // EP-BUNDLE-v1
  ...
  if ((doc as any)?.context && (doc as any)?.webauthn) {
...
export default function VerifyPage(): React.ReactElement {
  ...
  const run = useCallback(async (raw: string): Promise<void> => {
    setBusy(true);
    setState(null);
    try {
      let doc;
...
function Result({ state }: { state: VerifyState }): React.ReactElement {
  if (state.error) {
    return (
  ...
  const view = present(kind || null, result);
  const ok = (result as any).valid;
  ...
  return (
    <div style={{ marginTop: 28, borderRadius: radius.base, border: `1px solid ${accent}`, overflow: 'hidden' }}>
      <div style={{ background: bg, padding: '22px 24px', borderBottom: `1px solid ${ok ? '#BBF7D0' : '#FECACA'}` }}>
        ...
        {!ok && (result as any).error && <p style={{ margin: '14px 0 0', fontSize: 13, color: color.red, lineHeight: 1.6, fontFamily: font.mono }}>{(result as any).error}</p>}
      </div>
