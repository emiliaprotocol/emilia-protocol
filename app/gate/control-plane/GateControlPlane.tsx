...
interface ControlPlaneScenario {
  action?: {
    action_type?: string;
    site_id?: string;
    target_kw?: number;
...
function short(value: unknown, start = 15, end = 10): string {
...
function State({ good, children }: StateProps): React.ReactElement {
  const Icon = good ? Check : X;
  return <span className={good ? styles.good : styles.bad}><Icon size={13} aria-hidden="true" />{children}</span>;
}

export default function GateControlPlane(): React.ReactElement {
  const [mode, setMode] = useState<string>('complete');
  const [data, setData] = useState<ControlPlaneScenario | null>(null);
  const [status, setStatus] = useState<string>('loading');
  const [error, setError] = useState<string>('');

  const loadScenario = useCallback(async (nextMode: string) => {
    ...
    return body as ControlPlaneScenario;
  }, []);

  const run = useCallback(async (nextMode: string) => {
    setStatus('loading');
    setError('');
    try {
  ...
  const setScenario = (nextMode: string) => { if (nextMode !== mode) setMode(nextMode); else run(nextMode); };
