  ...
  return (
    <div style={styles.page}>
      ...
      <section style={{ ...styles.section, paddingTop: 100, paddingBottom: 60 }}>
        <div style={styles.eyebrowBlue}>Partners</div>
        <h1 style={styles.h1}>Pilot EMILIA in a real high-risk workflow</h1>
        ...
        <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
          <a href="#inquiry" className="ep-cta" style={cta.primaryBlue}>Apply for a Pilot</a>
          <a href="mailto:team@emiliaprotocol.ai" className="ep-cta-secondary" style={cta.secondaryBlue}>Talk to the Team</a>
        </div>
        <p style={{ fontSize: 13, color: color.t3, marginTop: 16 }}>
          ...
          <div style={{ ...styles.card, marginBottom: 24 }}>
            {PILOT_STEPS.map((step, i) => (
              <div key={i} style={{ display: 'flex', gap: 12, marginBottom: i < PILOT_STEPS.length - 1 ? 12 : 0, alignItems: 'flex-start' }}>
                <span style={{ fontFamily: font.mono, fontSize: 12, color: color.blue, minWidth: 20, flexShrink: 0 }}>{String(i + 1).padStart(2, '0')}</span>
                <span style={{ fontSize: 15, color: color.t2, lineHeight: 1.7, marginBottom: 6 }}>{step}</span>
              </div>
            ))}
          </div>
          <a href="#inquiry" className="ep-cta" style={cta.primaryBlue}>Apply for a Pilot</a>
        ...
        <div style={grid.cols2}>
          {BENEFITS.map((b, i) => (
            <div key={i} style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
              <span style={{ color: color.blue, fontSize: 14, flexShrink: 0 }}>+</span>
              <span style={{ fontSize: 14, color: color.t2 }}>{b}</span>
            </div>
          ))}
        </div>
      </section>
      ...
      <section style={styles.section}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
          {['Government agencies with delegated authority', 'Banks and payment processors', 'Enterprise compliance teams', 'Agent orchestration platforms', 'CI/CD and DevSecOps pipelines', 'Identity and access management vendors', 'Regulated industry SaaS'].map(tag => (
            <span key={tag} className="ep-tag-blue" style={{ padding: '6px 14px', borderRadius: 100, border: `1px solid ${color.border}`, background: 'rgba(59,130,246,0.04)', fontSize: 13, color: color.blue }}>{tag}</span>
          ))}
        </div>
      </section>
      ...
      <section style={{ ...styles.sectionAlt, textAlign: 'center' }}>
        <div style={{ ...styles.section, maxWidth: 540 }}>
          <h2 style={{ ...styles.h2, fontSize: 28 }}>Ready to deploy control before action in a real workflow?</h2>
          <p style={styles.body}>
          ...
          <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
            <a href="#inquiry" className="ep-cta" style={cta.primaryBlue}>Apply for a Pilot</a>
            <a href="mailto:team@emiliaprotocol.ai" className="ep-cta-secondary" style={cta.secondaryBlue}>Request a Conversation</a>
          </div>
        </div>
      </section>
      ...
      <section style={styles.section}>
        ...
        {FAQS.map((faq, i) => (
          <div key={i} style={{ marginBottom: 24, paddingBottom: 24, borderBottom: i < FAQS.length - 1 ? `1px solid ${color.border}` : 'none' }}>
            <div style={{ fontSize: 16, fontWeight: 700, color: color.t1, marginBottom: 6 }}>{faq.q}</div>
            <div style={{ fontSize: 15, color: color.t2, lineHeight: 1.7 }}>{faq.a}</div>
          </div>
        ))}
      </section>

      {/* Inquiry form */}
      <section id="inquiry" style={styles.sectionAlt}>
        <div style={styles.section}>
          ...
          {submitted ? (
            ...
            <div style={styles.card}>
              ...
              <button className="ep-cta" onClick={handleSubmit} disabled={submitting || !form.name || !form.email} style={{ ...((!form.name || !form.email) ? cta.disabled : cta.primaryBlue), marginTop: 20, width: '100%', textAlign: 'center' }}>
