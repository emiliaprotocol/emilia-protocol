...
function SourceLink({ href, children = 'Open source' }) {
  return (
...
function GuaranteeCell({ guarantee, dimension }) {
...
function SourceDetail({ source, dimensions }) {
  return (
...
function GuaranteeMap({ snapshot }) {
  const [query, setQuery] = useState('');
  const [layer, setLayer] = useState('all');
  const [selectedId, setSelectedId] = useState('wimse-condition-bounded');
              ...
              return (
                <tr key={source.id} data-selected={expanded ? 'true' : undefined}>
                  <td className="obs-source-column">
                      <button type="button" className="obs-source-button" onClick={() => setSelectedId(expanded ? null : source.id)} aria-expanded={expanded}>
                  ...
                  <td className="obs-expand-column">
                    <button type="button" className="obs-icon-button" onClick={() => setSelectedId(expanded ? null : source.id)} aria-label={`${expanded ? 'Close' : 'Inspect'} ${source.short_name}`}>
...
export default function ObservatoryClient({ snapshot }) {
