 ...
 */
async function epFetch(path, opts?: { method?: string; auth?: boolean; body?: any }) {
  const options = opts || {};
...
const TOOLS = [
  ...
  },
  {
    ...
    description:
      'Ask the configured EMILIA service to verify a stored trust receipt and ' +
...
async function handleTool(name, args) {
  switch (name) {
    ...
    case 'ep_trust_evaluate': {
      const body: Record<string, any> = { entity_id: args.entity_id, policy: args.policy || 'standard' };
    ...
    case 'ep_search_entities': {
      ...
      const entities: Array<Record<string, any>> = data.entities || data.results || [];
    ...
    case 'ep_domain_score': {
      ...
      for (const [domain, score] of Object.entries(domains)) {
        ...
        if (s.completion_rate != null) out += `  Completion: ${s.completion_rate}%\n`;
        if (s.dispute_rate != null) out += `  Dispute rate: ${s.dispute_rate}%\n`;
      }
...
export function advertisedToolsFor({
  includeProtocolTools = false,
  includeRegistryTools = false,
} = {}) {
  if (includeRegistryTools) return TOOLS.map(withAnnotations);
  const names = new Set(ACTION_TOOL_NAMES);
  if (includeProtocolTools) {
    for (const name of PROTOCOL_TOOL_NAMES) names.add(name);
  }
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  const params = request.params as unknown as { name?: string; arguments?: Record<string, any> };
  ...
  try {
    ...
    const invoker = isMetaTool
      ? (a: Record<string, any>) => handleTool(name || '', a)
      : autoReceipt.wrap(name || '', (a: Record<string, any>) => handleTool(name || '', a));

    const result = await invoker(args || {});
    return { content: [{ type: 'text', text: result }] };
  } catch (err) {
    return { content: [{ type: 'text', text: `Error: ${(err as any).message}` }], isError: true };
  }
});
