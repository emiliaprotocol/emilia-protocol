// Generated from integration.test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
/**
...
describe.skipIf(SKIP)('ep_trust_profile', () => {
    ...
    it('propagates API error without stack trace', async () => {
        ...
        const result = await handleTool('ep_trust_profile', { entity_id: 'ghost' }).catch((e) => e.message);
...
describe.skipIf(SKIP)('error handling', () => {
    it('does not leak stack trace on 500', async () => {
        ...
        const result = await handleTool('ep_trust_profile', { entity_id: 'x' }).catch((e) => e.message);
    ...
    it('does not leak stack trace on 429', async () => {
        ...
        const result = await handleTool('ep_trust_evaluate', { entity_id: 'x' }).catch((e) => e.message);
        const msg = typeof result === 'string' ? result : result?.message ?? String(result);
        expect(msg).not.toMatch(/\s+at\s+\w/);
    });
