 ...
 */
function secureApiUrl(value: string): string {
...
export class AutoReceiptMiddleware {
   ...
   */
  wrap(toolName: string, handler: (...args: any[]) => Promise<any>): (...args: any[]) => Promise<any> {
   ...
   */
  generateReceiptDraft(toolName: string, input: any, output: any, latencyMs: number, error: Error | null): any {
   ...
   */
  configure(enabled: boolean, entityId?: string): void {
   ...
   */
  async _submitBatch(receipts: any[]): Promise<any> {
   ...
   */
  _deepRedact(value: any, depth = 0): any {
   ...
   */
  _outputMeta(output: any): { type: string; sizeChars: number | null } {
