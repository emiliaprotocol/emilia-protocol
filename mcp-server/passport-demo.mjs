...
// Generated from passport-demo.mts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
//
...
const TOOLS = [
    {
        ...
        description: '[SANDBOX DEMO — simulated payout, NO real funds move. Safe to call.] '
    ...
    },
    {
        ...
        description: '[SANDBOX DEMO — no real money moves.] Obtain an EMILIA Trust Receipt authorizing an action. The real policy engine '
...
server.setRequestHandler(CallToolRequestSchema, async (req) => {
    ...
    if (name === 'release_payment') {
        ...
        const v = verifyEmiliaReceipt(typedArgs.emilia_receipt, { trustedKeys: [SERVER_PUB], action: 'payment.release', maxAgeSec: 900 });
        if (!v.ok)
            return out({ status: 402, error: 'receipt_rejected', reason: v.reason, detail: 'Obtain a fresh, valid receipt via emilia_authorize.' });
        const receipt = typedArgs.emilia_receipt;
        return out({ status: 200, released: true, amount: typedArgs.amount, destination: typedArgs.destination, receipt_id: v.receipt_id, approved_by: receipt.payload?.claim?.context?.approver });
    }
    if (name === 'emilia_authorize') {
        const action = String(typedArgs.action || '');
        ...
        if (base.signoffRequired && !typedArgs.approver) {
        ...
        const receipt = mint(action, base.decision, context, typedArgs.approver);
        return out({ status: 200, authorized: true, decision: base.decision, approver: typedArgs.approver || null, emilia_receipt: receipt });
    }
    if (name === 'verify_receipt') {
        const v = verifyEmiliaReceipt(typedArgs.emilia_receipt, { trustedKeys: [SERVER_PUB], action: typedArgs.action || null, maxAgeSec: 900 });
        return out(v);
    }
