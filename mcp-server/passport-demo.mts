...
const SERVER_PUB = crypto.createPublicKey(privateKey as any).export({ type: 'spki', format: 'der' }).toString('base64url');
...
server.setRequestHandler(CallToolRequestSchema, async (req) => {
  ...
  if (name === 'release_payment') {
    ...
    const receipt = typedArgs.emilia_receipt as { payload?: { claim?: { context?: { approver?: string } } } };
  ...
  if (name === 'emilia_authorize') {
    ...
    const base = isPayment
      ? evaluateGuardPolicy({ actionType: GUARD_ACTION_TYPES.AI_AGENT_PAYMENT_ACTION, riskFlags: [] } as any)
    ...
    const receipt = mint(action, base.decision, context, (typedArgs.approver as string | undefined));
  ...
  if (name === 'verify_receipt') {
    const v = verifyEmiliaReceipt(typedArgs.emilia_receipt as object, { trustedKeys: [SERVER_PUB], action: (typedArgs.action as string | null) || null, maxAgeSec: 900 });
    return out(v);
  }
