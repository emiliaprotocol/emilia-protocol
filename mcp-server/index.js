...
/**
 * @param {string} path
 * @param {{ method?: string, auth?: boolean, body?: any }} [opts]
...
async function epFetch(path, opts) {
    ...
    if (raw) {
        ...
        if (!gate.ok)
            throw new Error(`EP API returned ambiguous or invalid JSON: ${gate.reason}`);
        data = JSON.parse(raw);
    }
    if (!res.ok)
...
const TOOLS = [
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    {
        ...
        description: 'Request authorization before an irreversible action: payments, wire transfers, fund releases, ' +
    ...
    },
    {
        ...
        description: 'Poll a pending authorization after ep_guard_action returns BLOCKED. Pass the ' +
    ...
    // PRIMARY: Trust profile (canonical read surface)
    {
        ...
        description: 'Get an entity\'s full trust profile. This is the CANONICAL way to check trust in EP. ' +
    ...
    // PRIMARY: Trust evaluation (policy consumption)
    {
        ...
        description: 'Evaluate an entity against a trust policy. Returns a Trust Decision (allow/review/deny) with specific failure reasons. ' +
    ...
    // PRIMARY: Submit receipt
    {
        ...
        description: 'Submit a transaction receipt to the EP ledger. Requires an API key. ' +
    ...
    // SECONDARY
    {
        ...
        description: 'Search the EP registry for entities by name, capability, or category. ' +
    ...
    },
    {
        ...
        description: 'Create a new entity in the EP registry. SIDE EFFECT: persists the entity ' +
    ...
    {
        name: 'ep_leaderboard',
        description: 'List the top entities ranked by trust confidence. Read-only. Returns up to ' +
    ...
    // DUE PROCESS
    {
        ...
        description: 'File a dispute against a receipt. Any affected party can challenge. ' +
    ...
    },
    {
        ...
        description: 'Look up the current state of a dispute by id. Read-only, public (no auth) — ' +
    ...
    },
    {
        ...
        description: 'Report a trust issue as a human. No authentication required. ' +
    ...
    // EP-SX: Software Trust
    {
        ...
        description: 'Appeal a dispute resolution. Only dispute participants can appeal. ' +
    ...
    },
    {
        ...
        description: 'BEFORE installing or enabling third-party software an agent depends on — an npm ' +
    ...
    // V1.0: Delegation
    {
        ...
        description: 'Create a delegation record. WRITE: persists to the EP ledger that a human or ' +
    ...
    // V1.0: Verify delegation
    {
        ...
        description: 'Verify that an agent currently holds a valid delegation from a principal for a specific action. ' +
    ...
    {
        name: 'ep_trust_gate',
        description: 'Trust gate: check if an entity meets the required trust threshold BEFORE executing a high-stakes action. ' +
    ...
    // V1.0: Batch receipt submission
    {
        ...
        description: 'Submit multiple receipts atomically. All receipts share the same submitter (your API key). ' +
    ...
    // Sprint 2: Auto-receipt configuration
    {
        ...
        description: 'Enable or disable automatic receipt generation for this session. ' +
    ...
    // V1.0: Domain score
    {
        ...
        description: 'Get an entity\'s trust score broken down by behavioral domain. ' +
    ...
    // Sprint 5A: Privacy-Preserving Commitment Proof layer
    {
        ...
        description: 'Generate a privacy-preserving commitment proof. Proves a trust claim (e.g., score > 0.85 in the ' +
        ...
        inputSchema: {
            ...
            properties: {
                ...
                claim_type: {
                    ...
                    description: 'score_above: global behavioral score > threshold. ' +
                        'domain_score_above: per-domain score > threshold (requires domain). ' +
                        'receipt_count_above: total receipts > threshold (integer count).',
                },
                threshold: {
                    type: 'number',
                    description: '0.0–1.0 for score claims (e.g. 0.85 = 85th percentile behavioral score). ' +
    ...
    },
    {
        ...
        description: 'Verify a privacy-preserving commitment proof by proof_id. ' +
    ...
    // Sprint 4A: Delegation judgment
    {
        ...
        description: 'Get a principal\'s delegation authority — how well they choose and authorize agents. ' +
    ...
    // EP Commit — signed authorization token for high-stakes machine actions
    {
        ...
        description: 'Issue a signed EP Commit before a high-stakes action. ' +
    ...
    },
    {
        ...
        description: 'Verify a pre-action commit — read-only, no side effects. Checks its signature, ' +
    ...
    },
    {
        ...
        description: 'Get the current state of a pre-action commit by id. Read-only. Returns one ' +
    ...
    },
    {
        ...
        description: 'Revoke an active pre-action commit before it is fulfilled or expires. ' +
    ...
    },
    {
        ...
        description: 'Bind a post-action receipt to a commit, completing the commit-execute-receipt cycle. ' +
    ...
    // EP Handshake — structured identity exchange
    {
        ...
        description: 'Initiate an EP Handshake — a structured identity exchange between parties. ' +
    ...
    },
    {
        ...
        description: 'Add an identity presentation (proof) to an active handshake. WRITE: appends ' +
    ...
    },
    {
        ...
        description: 'Evaluate all presentations in a handshake against its governing policy — read-only, ' +
    ...
    },
    {
        ...
        description: 'Get the full state of a handshake including parties, presentations, binding, and result. ' +
    ...
    },
    {
        ...
        description: 'Revoke an active handshake. Only parties to the handshake may revoke it. ' +
...
async function handleTool(name, args) {
    switch (name) {
        ...
        case 'ep_guard_action': {
            if (!API_KEY)
                return 'Error: EP_API_KEY required to guard actions. Set it in MCP server config.';
        ...
        case 'ep_check_signoff': {
            if (!API_KEY)
                return 'Error: EP_API_KEY required. Set it in MCP server config.';
        ...
        case 'ep_search_entities': {
            ...
            if (args.entity_type)
                params.set('type', args.entity_type);
        ...
        case 'ep_leaderboard': {
            ...
            if (!lb.length)
                return 'No entities in leaderboard yet.';
            return lb.map(e => {
                const conf = e.confidence || 'pending';
        ...
        case 'ep_dispute_file': {
            if (!API_KEY)
                return 'Error: EP_API_KEY required to file disputes.';
            const body = {
                receipt_id: args.receipt_id,
                reason: args.reason,
        ...
        case 'ep_appeal_dispute': {
            if (!API_KEY)
                return 'Error: EP_API_KEY required to file appeals.';
            const body = {
        ...
        case 'ep_principal_lookup': {
            ...
            if (data.error)
                return `Principal not found: ${args.principal_id}`;
            const p = data.principal;
            let out = `Principal: ${p.display_name} (${p.principal_id})\n`;
            out += `Type: ${p.principal_type} · Status: ${p.status}\n`;
            if (p.bootstrap_verified)
                out += `Bootstrap verified: yes\n`;
            if (data.entities?.length) {
                out += `\nControlled entities (${data.entities.length}):\n`;
                for (const e of data.entities)
                    out += `  ${e.display_name} (${e.entity_id}) — ${e.entity_type}\n`;
            }
            if (data.bindings?.length) {
                out += `\nIdentity bindings (${data.bindings.length}):\n`;
                for (const b of data.bindings)
                    out += `  ${b.binding_type}: ${b.binding_target} [${b.status}] provenance: ${b.provenance}\n`;
            }
            if (data.continuity_claims?.length) {
                out += `\nContinuity history (${data.continuity_claims.length}):\n`;
                for (const c of data.continuity_claims)
                    out += `  ${c.old_entity_id} → ${c.new_entity_id} (${c.reason}) [${c.status}]\n`;
            }
            return out;
        }
        case 'ep_lineage': {
            ...
            if (data.predecessors?.length) {
                ...
                for (const p of data.predecessors)
                    out += `  ← ${p.from} (${p.reason}) [${p.status}] transfer: ${p.transfer_policy || 'pending'}\n`;
            }
            else {
            ...
            if (data.successors?.length) {
                ...
                for (const s of data.successors)
                    out += `  → ${s.to} (${s.reason}) [${s.status}] transfer: ${s.transfer_policy || 'pending'}\n`;
            }
            else {
                out += `No successors.\n`;
            }
        ...
        case 'ep_list_policies': {
            ...
            for (const p of data.policies) {
                ...
                out += `  ${p.description}\n`;
                if (p.min_confidence)
                    out += `  min confidence: ${p.min_confidence}\n`;
            }
        ...
        case 'ep_create_delegation': {
            if (!API_KEY)
                return 'Error: EP_API_KEY required to create delegations.';
            const body = {
        ...
        case 'ep_verify_delegation': {
            ...
            if (data.principal_id)
                out += `Principal: ${data.principal_id}\n`;
            if (data.agent_entity_id)
                out += `Agent: ${data.agent_entity_id}\n`;
            if (data.scope)
                out += `Scope: ${typeof data.scope === 'object' ? JSON.stringify(data.scope) : String(data.scope)}\n`;
            if (data.expires_at)
        ...
        case 'ep_trust_gate': {
            ...
            out += `Policy: ${data.policy_used}\n`;
            out += `Confidence: ${data.confidence}\n`;
            if (data.delegation_verified != null)
                out += `Delegation: ${data.delegation_verified ? '✓ Verified' : '✗ Not verified'}\n`;
            if (data.reasons?.length) {
                out += `\nReasons:\n`;
                for (const r of data.reasons)
                    out += `  ${data.decision === 'allow' ? '✓' : '✗'} ${r}\n`;
            }
        ...
        case 'ep_batch_submit': {
            if (!API_KEY)
                return 'Error: EP_API_KEY required for batch submission.';
        ...
        case 'ep_domain_score': {
            ...
            if (args.domains?.length)
                params.set('domains', args.domains.join(','));
            ...
            for (const [domain, score] of Object.entries(domains)) {
                const s = score;
                out += `\n${domain}:\n`;
                out += `  Confidence: ${s.confidence ?? 'pending'}\n`;
                out += `  Evidence: ${s.evidence_count ?? 0} receipts\n`;
                if (s.completion_rate != null)
                    out += `  Completion: ${s.completion_rate}%\n`;
                if (s.dispute_rate != null)
                    out += `  Dispute rate: ${s.dispute_rate}%\n`;
            }
            if (!Object.keys(domains).length)
                out += 'No domain data available yet.\n';
            return out;
        }
        case 'ep_configure_auto_receipt': {
            ...
            if (args.enabled) {
                return (`Auto-receipt enabled for entity: ${args.entity_id}\n` +
        ...
        case 'ep_generate_zk_proof': {
            if (!API_KEY)
                return 'Error: EP_API_KEY required to generate commitment proofs.';
            const body = {
                entity_id: args.entity_id,
                claim: {
            ...
            try {
                ...
                if (err.message?.includes('CLAIM_NOT_PROVABLE') || err.message?.includes('claim_not_provable')) {
                    return (`Commitment Proof: Claim Not Provable\n` +
                        `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n` +
            ...
            if (data.claim.domain)
                out += ` (${data.claim.domain})`;
            out += `\n`;
            out += `Receipts:       ${data.receipt_count} (hidden from verifiers)\n`;
            out += `Expires:        ${data.expires_at}\n`;
            if (data.anchor_block)
                out += `Anchor:         ${data.anchor_block}\n`;
        ...
        case 'ep_issue_commit': {
            if (!API_KEY)
                return 'Error: EP_API_KEY required to issue commits.';
            const body = {
                action_type: args.action_type,
                entity_id: args.entity_id,
            ...
            if (commit.scope)
                out += `Scope:      ${typeof commit.scope === 'object' ? JSON.stringify(commit.scope) : commit.scope}\n`;
            if (commit.appeal_path)
                out += `Appeal:     ${commit.appeal_path}\n`;
            return out;
        }
        ...
        case 'ep_get_commit_status': {
            if (!API_KEY)
                return 'Error: EP_API_KEY required to get commit status.';
            ...
            if (commit.decision)
                out += `Decision:    ${commit.decision}\n`;
            if (commit.expires_at)
                out += `Expires:     ${commit.expires_at}\n`;
            if (commit.receipt_id)
                out += `Receipt:     ${commit.receipt_id}\n`;
            return out;
        }
        case 'ep_revoke_commit': {
            if (!API_KEY)
                return 'Error: EP_API_KEY required to revoke commits.';
        ...
        case 'ep_bind_receipt_to_commit': {
            if (!API_KEY)
                return 'Error: EP_API_KEY required to bind receipts.';
        ...
        case 'ep_initiate_handshake': {
            if (!API_KEY)
                return 'Error: EP_API_KEY required to initiate handshakes.';
            const body = {
                mode: args.mode,
        ...
        case 'ep_add_presentation': {
            if (!API_KEY)
                return 'Error: EP_API_KEY required to add presentations.';
            const body = {
        ...
        case 'ep_verify_handshake': {
            if (!API_KEY)
                return 'Error: EP_API_KEY required to verify handshakes.';
            ...
            if (data.reason_codes?.length) {
                out += `Reasons:\n`;
                for (const r of data.reason_codes)
                    out += `  ${r}\n`;
            }
            if (data.evaluated_at)
                out += `Evaluated at: ${data.evaluated_at}\n`;
            return out;
        }
        case 'ep_get_handshake': {
            if (!API_KEY)
                return 'Error: EP_API_KEY required to get handshake details.';
            ...
            if (data.result) {
                ...
                if (data.result.reason_codes?.length) {
                    for (const r of data.result.reason_codes)
                        out += `  ${r}\n`;
                }
            }
            return out;
        ...
        case 'ep_revoke_handshake': {
            if (!API_KEY)
                return 'Error: EP_API_KEY required to revoke handshakes.';
...
function formatTrustProfile(data) {
    ...
    if (p) {
        ...
        if (p.provenance) {
            ...
            out += `\nProvenance: ${tiers}\n`;
            if (p.provenance.bilateral_rate != null)
                out += `  Bilateral rate: ${p.provenance.bilateral_rate}%\n`;
        }
    }
...
server.setRequestHandler(ReadResourceRequestSchema, async (request) => {
    ...
    if (uri.startsWith('entity://')) {
        ...
        data = /** @type {{ entity_id: string, current_confidence?: number, effective_evidence_current?: number, historical_establishment?: boolean, compat_score?: number }} */ (await epFetch(`/api/trust/profile/${encodeURIComponent(id)}`));
        text = JSON.stringify({
