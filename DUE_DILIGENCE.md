...
| --- | --- | --- |
| Tests and build | [`lib/proof-stats.json`](lib/proof-stats.json) is the single source of truth for the automated test and file counts; read `tests.total` and `tests.files` there rather than a number transcribed here. The reviewed CI run completed test, build, typecheck, lint, Postgres integration, E2E, packaging, conformance, security-case, and product jobs. | Passing tests are scoped regression evidence, not whole-system proof or production adoption. |
| Executable security case | Claim count, evidence-file count, and the evidence-bundle SHA-256 are generated: read `securityCase` in [`lib/proof-stats.json`](lib/proof-stats.json), or `evidence_bundle_sha256` in [`security/security-case.json`](security/security-case.json). Any bundle hash transcribed into prose goes stale the next time an evidence file changes. Execution passed on the reviewed run. | This is repository-defined executable assurance, not an accredited audit. |
| Conformance | **21 suites and 335 vectors** across JavaScript, Python, and Go same-team ports. A separately authored Rust implementation remains pinned to 164 vectors and passes 359 hostile cases. | Same-team ports are not independent implementations. Strict clean-room construction acceptance remains false. |
...
| Repository controls | `main` requires 16 status contexts through a merge queue (merge commits), with conversation resolution, administrator enforcement, force-push disabled, and deletion disabled. The organization requires 2FA; immutable release-tag rules have no bypass actor. | No pull-request approval, Code Owner review, or signed commit is currently required. The organization domain is not GitHub-verified and governance remains single-maintainer. |
