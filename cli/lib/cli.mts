 ...
 * @param {string} [value]
 */
function line(stream: OutputStream, value: string = ''): void {
  stream.write(`${value}\n`);
}
 ...
 * @param {unknown} value
 */
function json(stream: OutputStream, value: unknown): void {
...
function usage(stream: OutputStream, env: NodeJS.ProcessEnv): void {
...
export function runOfflineVerifier(
  args: string[],
  { stdout = process.stdout, stderr = process.stderr }: { stdout?: OutputStream; stderr?: OutputStream } = {},
): number {
  ...
  return Number.isInteger(result.status) ? (result.status as number) : 1;
}

function requireApiKey(apiKey: string | undefined, stderr: OutputStream): apiKey is string {
