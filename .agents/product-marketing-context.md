...
| --- | --- | --- |
| **EMILIA Authority Brain / Gate Starter** | Free local product experience that discovers visible agent-action surfaces, proposes consequence classifications, names blind spots, and prepares one owner-selected MCP action for Gate integration and local refusal testing | Apache-2.0 acquisition surface; the generated starter is not an activated or completely mediated production control |
...
**Adoption promise:** After the exact `@emilia-protocol/mcp-guard@0.5.0` runtime is installed, `npx @emilia-protocol/scan@0.5.0 protect ./tools.json --action sendWire --apply --verify` creates a self-contained local Authority Map, a Gate Starter for the selected MCP action, and a four-case synthetic refusal check without an account, upload, telemetry, or remote asset. The user replaces `sendWire` with one exact declared tool name. The command launches no configured server and makes no production-protection claim. After inspecting the generated bytes, the owner can explicitly acknowledge that review and emit a privacy-bounded handoff without copying a digest by hand. The handoff reports only the completed local checks; it does not claim certification, active deployment security, credential isolation, durable state, or complete mediation.
