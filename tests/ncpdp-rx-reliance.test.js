...
const consentKey = ed(); const clinicalKey = ed(); const benefitKey = ed(); const payerKey = ed();
...
const CHALLENGE = {
  ...
  accepted_issuer_keys: [logKey.pub], accepted_consent_keys: [consentKey.pub], accepted_clinical_keys: [clinicalKey.pub], accepted_benefit_keys: [benefitKey.pub], accepted_payer_keys: [payerKey.pub],
};
...
function base() {
  return { '@type': 'EP-RX-RELIANCE-PACKET-v1', action: ACTION, receipt, authority_proof: authority(), patient_consent: consent(), clinical_evidence: clinical(), benefit_check: benefit(), revocation_state: { checked_at: '2026-07-08T14:00:00.000Z' }, determination: 'approve' };
}
function applyBreak(brk) {
  ...
  switch (brk) {
    ...
    case 'wrong_formulary_policy': return { ...p, benefit_check: benefit('sha256:' + sha('wrong')) };
    case 'stale_benefit': return { ...p, benefit_check: benefit(BENEFIT_POLICY_HASH, '2026-07-01T00:00:00.000Z') };
    case 'unsigned_benefit': return { ...p, benefit_check: { '@type': 'EP-RX-BENEFIT-v1', action_hash: receipt.action_hash, privacy_key_id: privacyKeyId, policy_hash: BENEFIT_POLICY_HASH, issued_at: '2026-07-08T14:00:00.000Z' } };
...
describe('EP-NCPDP-RX-RELIANCE branch coverage', () => {
  ...
  it('a benefit check citing a different formulary policy is policy_mismatch', () => {
    const p = { ...base(), benefit_check: benefit('sha256:' + sha('other-formulary')) };
  ...
  it('an impossible benefit-check date is stale rather than normalized by Date.parse', () => {
    ...
    const p = { ...base(), benefit_check: { ...benefit(), issued_at: '2026-02-30T12:00:00Z' } };
