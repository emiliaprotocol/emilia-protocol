...
describe('/spec source contract', () => {
  it('renders the current posted authorization-receipts revision from an existing file', () => {
    const source = 'standards/posted/draft-schrock-ep-authorization-receipts-06.md';

    expect(existsSync(resolve(ROOT, source))).toBe(true);
    expect(page).toContain("join(process.cwd(), 'standards', 'posted', 'draft-schrock-ep-authorization-receipts-06.md')");
    expect(page).toContain('DRAFT-SCHROCK-EP-AUTHORIZATION-RECEIPTS-06');
