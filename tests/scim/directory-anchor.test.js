...
function makeSupabase({ tokens = [], users = [], errors = {} } = {}) {
  const seeded = { scim_provisioning_tokens: tokens, scim_users: users };
  function match(row, state) {
    ...
    const b = {
      ...
      then(resolve, reject) {
        try {
          ...
          const rows = (seeded[table] || []).filter((r) => match(r, state));
...
describe('resolveEnrollmentBasis', () => {
  it('1: no tokens => operator_attested, hasDirectory false, raw storedApproverId', async () => {
    ...
    const r = await resolveEnrollmentBasis(sb, 'org_x', 'ep:approver:JChen');
  ...
  it('2: org-bound token + active match => directory, id pinned, normalized storedApproverId', async () => {
    ...
    const r = await resolveEnrollmentBasis(sb, 'org_x', 'cfo@corp.com');
    expect(r).toMatchObject({
      basis: 'directory',
  ...
  it('8: sticky — a REVOKED token still anchors; active user => directory, absent user => 403', async () => {
    const revokedToken = [{ organization_id: 'org_x', tenant_id: 't_x', revoked_at: '2020-01-01T00:00:00Z' }];
    ...
    const absent = makeSupabase({ tokens: revokedToken, users: [] });
  ...
  it('9: token lookup error => 503 directory_lookup_failed (fail closed, never downgrade)', async () => {
    const sb = makeSupabase({ errors: { scim_provisioning_tokens: { message: 'db down' } } });
    const r = await resolveEnrollmentBasis(sb, 'org_x', 'cfo@corp.com');
