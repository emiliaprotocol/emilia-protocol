...
function makeEstablished(overrides: any = {}): any {
  return {
...
describe('computeReceiptComposite — extended edge cases', () => {
  it('returns DEFAULT_SCORE when all values are null', () => {
    const result: number = computeReceiptComposite({
  ...
  it('clamps values above 100 to 100', () => {
    const result: number = computeReceiptComposite({ delivery_accuracy: 200 });
    expect(result).toBe(100);
  });

  it('clamps values below 0 to 0', () => {
    const result: number = computeReceiptComposite({ delivery_accuracy: -50 });
  ...
  it('ignores the consistency field (entity-level, not receipt-level)', () => {
    const withConsistency: number = computeReceiptComposite({
  ...
  it('single signal returns that signal normalized to 100 scale', () => {
    const result: number = computeReceiptComposite({ agent_satisfaction: 60 });
    expect(result).toBe(60);
  });

  it('handles receipt with only return_processing signal', () => {
    const result: number = computeReceiptComposite({ return_processing: 75 });
...
describe('computeEmiliaScore — extended legacy coverage', () => {
  it('handles null input (treated as empty array)', () => {
    const result: any = computeEmiliaScore(null);
  ...
  it('returns breakdown with all null signals for a single unestablished receipt with missing fields', () => {
    const result: any = computeEmiliaScore([{
      submitted_by: 'sub-1',
      submitter_score: 50,
  ...
  it('single-receipt consistency is exactly 50 (no variance possible)', () => {
    const result: any = computeEmiliaScore([makeEstablished({ composite_score: 80 })]);
  ...
  it('effectiveEvidence reflects sum of weights correctly for mixed establishment', () => {
    ...
    const result: any = computeEmiliaScore(receipts);
  ...
  it('uniqueSubmitters counts distinct submitted_by values', () => {
    const receipts: any[] = Array(6).fill(null).map((_, i: number) =>
  ...
  it('established requires both effectiveEvidence >= 5 AND uniqueSubmitters >= 3', () => {
    ...
    const receipts: any[] = Array(3).fill(null).map((_, i: number) =>
  ...
  it('score is clamped to [0, 100]', () => {
    const receipts: any[] = Array(20).fill(null).map((_, i: number) =>
      makeEstablished({
  ...
  it('effectiveEvidence is rounded to 2 decimal places in output', () => {
    const receipts: any[] = [makeEstablished({ submitter_score: 85, submitter_established: true, submitted_by: 's1' })];
    const result: any = computeEmiliaScore(receipts);
    const decimals: number = String(result.effectiveEvidence).split('.')[1]?.length ?? 0;
    expect(decimals).toBeLessThanOrEqual(2);
  });

  it('receipts with no composite_score excluded from consistency calculation', () => {
    const receipts: any[] = Array(5).fill(null).map((_, i: number) =>
...
describe('computeMatchScore — extended edge cases', () => {
  ...
  it('returns a value when score exactly meets minScore', () => {
    const result: any = computeMatchScore(0.5, 70, 70);
    expect(result).not.toBeNull();
    expect(result).toBeGreaterThan(0);
  });

  it('rounds to 3 decimal places', () => {
    const result: any = computeMatchScore(0.333, 66.7, 0);
    const decimals: number = String(result).split('.')[1]?.length ?? 0;
...
describe('computeTimeDecay — edge cases', () => {
  ...
  it('applies floor of 0.05 for very old receipts', () => {
    // 10 years ago
    const tenYearsAgo: string = new Date(Date.now() - 10 * 365 * 24 * 60 * 60 * 1000).toISOString();
    expect(computeTimeDecay(tenYearsAgo, Date.now())).toBe(0.05);
  });

  it('returns ~0.5 for receipts exactly 90 days old (half-life)', () => {
    const ninetyDaysAgo: string = new Date(Date.now() - 90 * 24 * 60 * 60 * 1000).toISOString();
    const result: number = computeTimeDecay(ninetyDaysAgo, Date.now());
  ...
  it('accepts a Date object as createdAt', () => {
    ...
    const result: number = computeTimeDecay(d, Date.now());
...
describe('computeScoresFromClaims — extended coverage', () => {
  ...
  it('delivery: delivered=true, on_time=true → 100', () => {
    const result: any = computeScoresFromClaims({ delivered: true, on_time: true });
    expect(result.delivery_accuracy).toBe(100);
  });

  it('delivery: delivered=true, on_time=false → 70', () => {
    const result: any = computeScoresFromClaims({ delivered: true, on_time: false });
    expect(result.delivery_accuracy).toBe(70);
  });

  it('delivery: delivered=false → 0', () => {
    const result: any = computeScoresFromClaims({ delivered: false });
    expect(result.delivery_accuracy).toBe(0);
  });

  it('delivery: delivered=true, no on_time → 80', () => {
    const result: any = computeScoresFromClaims({ delivered: true });
    expect(result.delivery_accuracy).toBe(80);
  });

  it('delivery: on_time with promised/actual — on time → 100', () => {
    const promised: string = new Date('2025-01-10T12:00:00Z').toISOString();
    const actual: string = new Date('2025-01-10T11:00:00Z').toISOString(); // early
    const result: any = computeScoresFromClaims({ delivered: true, on_time: { promised, actual } });
    expect(result.delivery_accuracy).toBe(100);
  });

  it('delivery: on_time with promised/actual — 1-24h delay → 85', () => {
    const promised: string = new Date('2025-01-10T12:00:00Z').toISOString();
    const actual: string = new Date('2025-01-10T18:00:00Z').toISOString(); // 6h late
    const result: any = computeScoresFromClaims({ delivered: true, on_time: { promised, actual } });
  ...
  it('delivery: on_time with promised/actual — 24-72h delay → 65', () => {
    const promised: string = new Date('2025-01-10T12:00:00Z').toISOString();
    const actual: string = new Date('2025-01-11T18:00:00Z').toISOString(); // 30h late
    const result: any = computeScoresFromClaims({ delivered: true, on_time: { promised, actual } });
  ...
  it('delivery: on_time with promised/actual — >72h delay → 40', () => {
    const promised: string = new Date('2025-01-10T12:00:00Z').toISOString();
    const actual: string = new Date('2025-01-15T12:00:00Z').toISOString(); // 5 days late
    const result: any = computeScoresFromClaims({ delivered: true, on_time: { promised, actual } });
    expect(result.delivery_accuracy).toBe(40);
  });

  it('price_integrity: object form with overcharge > 10% → 10', () => {
    const result: any = computeScoresFromClaims({
  ...
  it('return_accepted=false → return_processing=15', () => {
    const result: any = computeScoresFromClaims({ return_accepted: false });
    expect(result.return_processing).toBe(15);
  });

  it('return_accepted=true → return_processing=95', () => {
    const result: any = computeScoresFromClaims({ return_accepted: true });
    expect(result.return_processing).toBe(95);
  });

  it('as_described=false → product_accuracy=20', () => {
    const result: any = computeScoresFromClaims({ as_described: false });
...
describe('computeReceiptHash — additional coverage', () => {
  it('produces 64-char hex for a minimal receipt', async () => {
    const hash: string = await computeReceiptHash({
      entity_id: 'e1',
      submitted_by: 's1',
  ...
  it('different context produces different hash', async () => {
    ...
    const h1: string = await computeReceiptHash({ ...base, context: null });
    const h2: string = await computeReceiptHash({ ...base, context: { task_type: 'delivery' } });
  ...
  it('handles nested object in evidence field (canonical JSON)', async () => {
    ...
    const h1: string = await computeReceiptHash(receipt);
    const h2: string = await computeReceiptHash(receipt);
    expect(h1).toBe(h2);
  });
});
