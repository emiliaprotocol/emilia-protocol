...
function createTableSim() {
  ...
  function handleRpc(fnName, params) {
    ...
    if (fnName === 'create_handshake_atomic') {
      ...
      if (params.p_binding) {
        getTable('handshake_bindings').push({
          handshake_id,
          ...params.p_binding,
        });
      }
    ...
    if (fnName === 'present_handshake_writes') {
      ...
      // Update handshake status to pending_verification if still initiated
      const hs = getTable('handshakes').find((h) => h.handshake_id === params.p_handshake_id);
      if (hs && hs.status === 'initiated') {
        hs.status = 'pending_verification';
      }
      ...
      getTable('handshake_events').push({
        ...
        actor_id: params.p_actor_id || 'system',
        detail: params.p_event_detail || {},
        created_at: new Date().toISOString(),
        sequence_number: getTable('handshake_events').length + 1,
      });
    ...
    if (fnName === 'revoke_handshake_atomic') {
      ...
      if (['rejected', 'revoked', 'expired', 'consumed'].includes(hs.status)) {
        return Promise.resolve({ data: null, error: { code: 'P0001', message: 'HANDSHAKE_NOT_REVOCABLE' } });
      }
    ...
    if (fnName === 'verify_handshake_writes') {
      ...
      if (Array.isArray(params.p_party_updates)) {
        for (const pu of params.p_party_updates) {
          ...
          if (party) {
            party.verified_status = pu.verified_status;
          }
        }
      }

      // Consume binding if accepted
      if (params.p_consume_binding) {
        const binding = getTable('handshake_bindings').find((b) => b.handshake_id === params.p_handshake_id);
        if (binding) {
          binding.consumed_at = new Date().toISOString();
        }
      }
      ...
      getTable('handshake_verifications').push({
        ...
        reason_codes: params.p_reason_codes,
        assurance_achieved: params.p_assurance_achieved,
        policy_version: params.p_policy_version,
        binding_hash: params.p_binding_hash,
        policy_hash: params.p_policy_hash,
        created_at: new Date().toISOString(),
      });
    ...
    // Default: return success with null data
    return Promise.resolve({ data: null, error: null });
  }
