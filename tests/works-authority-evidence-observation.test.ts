...
describe('source-agnostic Authority Record evidence', () => {
  ...
  it('fails closed across malformed envelope, source, collector, status, and surface shapes', () => {
    const mutations: Array<(candidate: ReturnType<typeof observation>) => void> = [
      (candidate) => { candidate.observation_id = 'bad id'; },
      (candidate) => { candidate.source = null as any; },
      (candidate) => { (candidate.source as any).kind = 'mutable_prompt'; },
      (candidate) => { candidate.collector = null as any; },
      (candidate) => { (candidate.collector as any).private_key = 'secret'; },
      (candidate) => { (candidate.collector as any).version = 'latest'; },
      (candidate) => { candidate.status = 'SAFE' as any; },
      (candidate) => { candidate.surface_ids = []; },
      (candidate) => { candidate.surface_ids = ['machine-command', 'machine-command']; },
    ];

    for (const mutate of mutations) {
      const candidate = observation();
      mutate(candidate);
      expect(validateAuthorityEvidenceObservation(candidate).ok).toBe(false);
    }
  });
  ...
  it('produces stable exact-byte digests and source-agnostic freshness outcomes', () => {
    ...
    expect(evaluateAuthorityEvidenceFreshness(parsed.observation, {
      kind: 'resolved', identifier: '\u0000invalid',
    }, Date.parse('2026-08-20T00:00:00.000Z'))).toEqual({
      status: 'INDETERMINATE', reason: 'resolved_identifier_invalid',
    });
    expect(evaluateAuthorityEvidenceFreshness(parsed.observation, {
      kind: 'resolved', identifier: 'release:v1.2.3',
