...
describe('/spec source contract', () => {
  it('renders the current posted authorization-receipts revision from an existing file', () => {
    const source = 'standards/posted/draft-schrock-ep-authorization-receipts-09.xml';

    expect(existsSync(resolve(ROOT, source))).toBe(true);
    expect(page).toContain("join(process.cwd(), 'standards', 'posted', 'draft-schrock-ep-authorization-receipts-09.xml')");
    expect(page).toContain('DRAFT-SCHROCK-EP-AUTHORIZATION-RECEIPTS-09');
  ...
  it('places Receipts -09 at the start of the canonical path without overstating its claim', () => {
