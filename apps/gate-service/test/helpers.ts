...
export function receiptCarrier(receipt: any): string {
  return Buffer.from(JSON.stringify(receipt), 'utf8').toString('base64');
}

export function createActionStore(initialRecords: any[] = []) {
  ...
  return {
    ...
    async update(id, principalId, patch) {
      ...
      if (!current || current.principal_id !== principalId) return false;
    ...
    async get(id, principalId) {
      ...
      if (!record || record.principal_id !== principalId) return null;
    ...
    async transition(id, principalId, statuses, patch) {
      ...
      const current = records.get(id);
      if (!current || current.principal_id !== principalId || !statuses.includes(current.status)) return false;
      records.set(id, { ...current, ...structuredClone(patch) });
      return true;
    },
    async reconcileInterrupted({ action, statuses, patch }) {
      ...
      for (const [id, record] of records) {
        if (record.action !== action || !statuses.includes(record.status)) continue;
...
export function createDurableTestState({ initialActions = [] }: any = {}) {
...
function copyConnectorArgs(args: any): any {
  const copy = { ...args };
  delete copy.signal;
  return structuredClone(copy);
}

export async function createServiceFixture(testContext: any, {
  ...
  authorizeEvidence = async (principal: any, _operation: any, tenantId: any, gateId: any) => (
    principal.id === TEST_PRINCIPAL.id && tenantId === TEST_TENANT_ID && gateId === TEST_GATE_ID
  ),
  authenticateRequest = async (request: any) => {
    if (request.headers?.authorization === `Bearer ${TEST_API_TOKEN}`) return TEST_PRINCIPAL;
    if (request.headers?.authorization === `Bearer ${SECOND_API_TOKEN}`) return SECOND_PRINCIPAL;
    return null;
  },
  initialActions = [],
  connectorClose = null,
  ...
  if (evidenceVerify) state.evidenceLog.verify = evidenceVerify;
  ...
  const connector = {
    ...
    async deleteRepository(args) {
      ...
      if (deleteImpl) return deleteImpl(args, deleteCalls.length);
      return { status: 204 };
    },
