...
export class GateServiceConfigError extends Error {
    name = 'GateServiceConfigError';
    code = 'EMILIA_GATE_CONFIG_INVALID';
    reasons;
    constructor(reasons) {
...
export function validateGateServiceConfig(input) {
    const errors = [];
    if (!isObject(input))
        throw new GateServiceConfigError(['config_object_required']);
    for (const key of Object.keys(input)) {
        if (!ALLOWED_CONFIG_KEYS.has(key))
            errors.push(`unknown_config_key:${key}`);
    }
    if (input.allowInlineKey === true)
        errors.push('inline_receipt_keys_forbidden');
    ...
    if (!Array.isArray(trustedKeys))
        errors.push('trusted_keys_invalid');
    ...
    if (!isObject(approverKeys))
        errors.push('approver_keys_invalid');
    ...
    const maxReceiptBytes = boundedInteger(input.maxReceiptBytes, DEFAULT_MAX_RECEIPT_BYTES, 1024, 256 * 1024, 'max_receipt_bytes', errors);
    const connectorTimeoutMs = boundedInteger(input.connectorTimeoutMs, DEFAULT_CONNECTOR_TIMEOUT_MS, 100, 120_000, 'connector_timeout_ms', errors);
    const readinessTimeoutMs = boundedInteger(input.readinessTimeoutMs, DEFAULT_READINESS_TIMEOUT_MS, 100, 30_000, 'readiness_timeout_ms', errors);
    const now = input.now ?? Date.now;
    const idFactory = input.idFactory ?? (() => crypto.randomUUID());
    if (typeof now !== 'function')
        errors.push('now_function_required');
    if (typeof idFactory !== 'function')
        errors.push('id_factory_required');
    ...
    if (errors.length > 0)
        throw new GateServiceConfigError(errors);
...
export async function loadGateServiceConfig(file = process.env.EMILIA_GATE_CONFIG, { environment = process.env, productionFactory = null } = {}) {
    let candidate;
