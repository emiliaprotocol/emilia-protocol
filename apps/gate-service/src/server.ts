    ...
    (request, response) => {
      ...
      const complete = () => {
        ...
        if (lifecycle.active === 0) {
          for (const resolve of lifecycle.waiters) (resolve as any)();
...
function listenSettings(environment: Record<string, any>): any {
...
function settleWithin(promise: Promise<any>, milliseconds: number): Promise<any> {
...
function beginServerClose(server: any): void {
  if (!server.listening) return;
  server.close(() => {});
  server.closeIdleConnections?.();
}

function activeRequestsDrained(server: any): Promise<boolean> {
  const lifecycle = serverLifecycles.get(server);
  if (!lifecycle || lifecycle.active === 0) return Promise.resolve(true);
  return new Promise<boolean>((resolve: any) => lifecycle.waiters.add(() => resolve(true)));
}

export function shutdownGateService(
  { server, runtime }: any,
  {
    graceMs = server?.requestTimeout,
    adapterCloseTimeoutMs = runtime?.limits?.connectorTimeoutMs,
  }: any = {},
): Promise<any> {
    ...
    if (!shutdownPromise) {
      shutdownPromise = shutdownGateService(started, { graceMs, adapterCloseTimeoutMs })
        ...
        .catch((error) => {
          ...
          try { processLike.stderr?.write?.('EMILIA Gate service shutdown failed.\n'); } catch { /* no-op */ }
          throw error;
        });
    }
...
export async function startGateService({ environment = process.env }: any = {}) {
