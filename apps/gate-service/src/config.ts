...
function snapshotMap(value: Record<string, any>): Record<string, any> {
...
function boundedInteger(value: unknown, fallback: number, min: number, max: number, field: string, errors: string[]): number {
  const selected: number = value === undefined ? fallback : (value as any);
...
function scopedId(value: unknown, field: string, errors: string[]): any {
...
export class GateServiceConfigError extends Error {
  name: string = 'GateServiceConfigError';
  code: string = 'EMILIA_GATE_CONFIG_INVALID';
  reasons: string[];
...
export function validateGateServiceConfig(input: any) {
  const errors: string[] = [];
...
export async function loadGateServiceConfig(
  file: string | undefined = process.env.EMILIA_GATE_CONFIG,
  { environment = process.env, productionFactory = null }: any = {},
) {
  let candidate;
