...
function pgResult(result: unknown, operation: string, { rowsMatch = false }: any = {}): any {
...
function jsonObject(value: unknown, operation: string): Record<string, any> {
  let parsed: any = value;
  if (typeof value === 'string') {
    try { parsed = JSON.parse(value); } catch { throw new Error(`${operation}: malformed JSON record`); }
  }
  if (!plainObject(parsed as any)) throw new Error(`${operation}: malformed JSON record`);
  return structuredClone(parsed);
}

function safePatch(patch: unknown): Record<string, any> {
  const p = patch as any;
  if (!plainObject(p)) throw new TypeError('action patch must be an object');
  for (const key of ['id', 'action', 'principal_id', 'tenant_id', 'gate_id', 'target', 'created_at']) {
    if (Object.hasOwn(p, key)) throw new TypeError(`action patch cannot change ${key}`);
  }
 ...
 * }} [options]
 */
export function createPostgresActionStore({ query, tenantId, gateId, close = null }: any = {}) {
  ...
  return {
    ...
    async create(record) {
      ...
      const result = pgResult(
        await query(ACTION_STORE_SQL.create, [record.id, JSON.stringify(record)]),
        'action create',
      );
      if (result.rowCount > 1) throw new Error('action create affected multiple rows');
      return result.rowCount === 1;
    },
    async get(id, principalId) {
      const result = pgResult(
        await query(ACTION_STORE_SQL.get, [id, principalId, tenantId, gateId]),
        'action get',
        { rowsMatch: true },
      );
      if (result.rowCount > 1) throw new Error('action get returned multiple rows');
      return result.rowCount === 0 ? null : jsonObject(result.rows[0].record, 'action get');
    },
    async update(id, principalId, patch) {
      const result = pgResult(
        await query(ACTION_STORE_SQL.update, [
          id,
          principalId,
          tenantId,
          gateId,
      ...
      if (result.rowCount > 1) throw new Error('action update affected multiple rows');
      return result.rowCount === 1;
    },
    async transition(id, principalId, statuses, patch) {
      ...
      if (result.rowCount > 1) throw new Error('action transition affected multiple rows');
      return result.rowCount === 1;
    },
    ...
    async health() {
      ...
      if (result.rowCount !== 1) throw new Error('action health returned an invalid row count');
...
function evidenceForAction(records: any[], actionId: string): any[] {
  const decisions = new Set(records
    .filter((record) => record?.kind === 'decision' && record.selector?.action_id === actionId)
    .map((record) => record.hash));
  return records.filter((record) => (
    (record?.kind === 'decision' && record.selector?.action_id === actionId)
    || (record?.kind === 'execution' && decisions.has(record.authorizes_decision))
  ));
}

function createScopedEvidenceLog({ backend, streamId, tenantId, gateId }: any) {
...
function requiredEnvironment(environment: Record<string, any>, name: string, { max = 16_384 }: any = {}): string {
...
function optionalInteger(environment: Record<string, any>, name: string, { min, max }: any): number | undefined {
  if (environment[name] === undefined || environment[name] === '') return undefined;
  if (!/^[0-9]+$/.test(environment[name])) throw new Error(`${name}_invalid`);
  const value = Number(environment[name]);
  if (!Number.isSafeInteger(value) || value < min || value > max) throw new Error(`${name}_invalid`);
  return value;
}

function parseTrust(environment: Record<string, any>): Record<string, any> {
  const source = requiredEnvironment(environment, 'EMILIA_GATE_TRUST_JSON', { max: 1024 * 1024 });
  if (!strictJsonGate(source).ok) throw new Error('EMILIA_GATE_TRUST_JSON_invalid');
  let parsed;
  try { parsed = JSON.parse(source); } catch { throw new Error('EMILIA_GATE_TRUST_JSON_invalid'); }
  if (!plainObject(parsed)) throw new Error('EMILIA_GATE_TRUST_JSON_invalid');
  return parsed;
}

function parseAllowedRepositories(environment: Record<string, any>): Set<string> {
...
function optionalSiemForwarder(environment: Record<string, any>, fetchImpl: any): any {
  if (!environment.EMILIA_GATE_SIEM_URL) return null;
  let url;
  try { url = new URL(environment.EMILIA_GATE_SIEM_URL); } catch { throw new Error('EMILIA_GATE_SIEM_URL_invalid'); }
  ...
  return createSiemForwarder({
    ...
    sink: async (event) => {
      ...
      if (!response?.ok) throw new Error('siem_delivery_failed');
      try { Promise.resolve(response.body?.cancel?.()).catch(() => {}); } catch { /* no-op */ }
    },
  });
}

export async function createProductionGateConfig({
  ...
  return {
    ...
    authorizeAction: async (candidate, action, owner, repo) => (
      candidate?.id === principal.id
      && action === GITHUB_REPOSITORY_DELETE_ACTION
      && allowedRepositories.has(`${owner}/${repo}`.toLowerCase())
    ),
    authorizeEvidence: async (candidate, operation, tenant, gate, actionId) => (
      candidate?.id === principal.id
