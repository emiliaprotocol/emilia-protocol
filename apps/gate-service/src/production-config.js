...
function pgResult(result, operation, { rowsMatch = false } = {}) {
  if (!result || !Number.isInteger(result.rowCount) || result.rowCount < 0
      || !Array.isArray(result.rows) || (rowsMatch && result.rows.length !== result.rowCount)) {
...
function jsonObject(value, operation) {
  ...
  if (!plainObject(parsed)) throw new Error(`${operation}: malformed JSON record`);
  return structuredClone(parsed);
}

function safePatch(patch) {
  if (!plainObject(patch)) throw new TypeError('action patch must be an object');
  for (const key of ['id', 'action', 'principal_id', 'tenant_id', 'gate_id', 'target', 'created_at']) {
    if (Object.hasOwn(patch, key)) throw new TypeError(`action patch cannot change ${key}`);
  }
