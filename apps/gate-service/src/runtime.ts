...
function response(status: number, body: any, headers: Record<string, string> = {}): any {
 ...
 */
function closedError(status: number, code: string, id: string | null = null, state: string = status >= 500 ? 'failed' : 'refused'): any {
...
function jsonSnapshot(value: unknown): Record<string, any> {
...
function exactKeys(value: unknown, expected: readonly string[]): boolean {
  ...
  const actual = Object.keys(value as any).sort();
  const wanted = [...(expected as string[])].sort();
  return actual.length === wanted.length && actual.every((key, index) => key === wanted[index]);
}

function validateLocator(locator: unknown): any {
...
function validateDeleteRequest(body: unknown): any {
  const b = body as any;
  if (!isPlainObject(b)) return { ok: false, code: 'request_object_required' };
  if (!exactKeys(b, BODY_KEYS)) return { ok: false, code: 'request_fields_invalid' };
...
function validateResumeRequest(body: unknown): any {
  const b = body as any;
  if (!isPlainObject(b)) return { ok: false, code: 'request_object_required' };
  if (!exactKeys(b, RESUME_BODY_KEYS)) return { ok: false, code: 'request_fields_invalid' };
  if (b.action !== GITHUB_REPOSITORY_DELETE_ACTION) {
    return { ok: false, code: 'unsupported_action' };
  }
...
function boundedObservedText(value: unknown, field: string, max: number = 256): string {
...
export function observedGithubRepository(repository: unknown, locator: any): any {
...
function stableIdempotencyKey(receiptId: unknown): string {
...
function challengeBinding(id: string, observedAction: any): string {
  return hashCanonical({ action_id: id, observed_action: observedAction });
}

function publicActionRecord(record: unknown, expectedId: string, principalId: string, { tenantId, gateId }: any): any {
  ...
  if (!isPlainObject(rec) || rec.id !== expectedId
      ...
      || typeof rec.status !== 'string') return null;
  ...
  for (const field of [
    ...
    if (typeof rec[field] === 'string') projected[field] = rec[field];
  }
  ...
  if (isPlainObject(rec.observed_action)) {
    ...
    for (const field of [
      ...
      if (typeof rec.observed_action[field] === 'string') observed[field] = rec.observed_action[field];
    }
...
function challengeBody(gateBody: any, { id, observedAction, carrierInvalid, binding }: any): any {
  const gb = (gateBody ?? {}) as any;
...
function redactedEvidenceRecord(record: unknown, actionId: string, actionRecord: any): any {
  const rec = record as any;
  if (!isPlainObject(rec) || !Number.isSafeInteger(rec.seq)  || rec.seq < 0
      ...
      || (rec.prev_hash !== 'genesis' && !HEX_256.test(rec.prev_hash))) return null;
  ...
  if (typeof rec.allow === 'boolean') projected.allow = rec.allow;
  projected.action_id = actionId;
  return projected;
}

function verificationProjection(report: unknown): any {
  ...
  const projected: any = { ok: false, reason: 'evidence_verification_failed' };
  if (typeof rep.reason === 'string' && /^[A-Za-z0-9_:-]{1,128}$/.test(rep.reason)) {
    projected.reason = rep.reason;
  }
  if (Number.isSafeInteger(rep.at) && rep.at >= 0) projected.at = rep.at;
  return projected;
}

function unavailableBody(): any {
  return response(503, {
    status: 'unavailable',
...
export function createGateRuntime(inputConfig: any) {
  ...
  async function initialize() {
    ...
    initializePromise = (async () => {
      ...
      const updated = await actionStore.reconcileInterrupted({
        action: GITHUB_REPOSITORY_DELETE_ACTION,
        statuses: [...((INTERRUPTED_ACTION_STATUSES as any) as readonly string[])].map(s => String(s)),
  ...
  async function continueDelete({ actionRecord, principal, receiptCarrier = null }: { actionRecord: any, principal: any, receiptCarrier?: unknown }) {
   ...
   */
  async function executeDelete(request: any = {}) {
    ...
    if (!deleteRequest.ok) return closedError(400, deleteRequest.code);
   ...
   */
  async function resumeDelete(request: any = {}) {
    ...
    if (!resumeRequest.ok) return closedError(400, resumeRequest.code, id);
    let owned;
    try {
