...
import { createDefaultActionControlManifest, createTrustedActionFirewall, createProposalToEffectPostgresStore, createPostgresAebDurableConsumptionStore, } from '@emilia-protocol/gate';
import { createProposalToEffect } from '@emilia-protocol/gate/proposal-to-effect';
import { createPostgresProposalToEffectStatusHeadStore, createProposalToEffectStatusVerifier, } from '@emilia-protocol/gate/proposal-to-effect-status';
...
export function parseStrictEnvironmentJson(source, name) {
...
function required(environment, name, max = 1024 * 1024) {
...
function positiveInteger(environment, name, maximum = Number.MAX_SAFE_INTEGER) {
...
function githubSlug(value, name) {
...
export function createGitHubIssueSelector({ owner, repo, issueNumber, }) {
...
export function createGitHubIssueControlManifest({ owner, repo, issueNumber, }) {
...
function secretBytes(environment, name) {
...
function constantTimeSecret(expected, candidate) {
...
export function createAebGateConsumptionAdapter(store) {
    ...
    const key = (receiptKey) => `gate-receipt:${receiptKey}`;
    return Object.freeze({
        durable: true,
        ownershipFenced: true,
        permanentConsumption: true,
        async reserve(receiptKey) {
            return await store.reserve(key(receiptKey), [`gate-native:${receiptKey}`]) === 'RESERVED';
        },
        async commit(receiptKey) {
            return store.commit(key(receiptKey));
        },
        async release(receiptKey) {
            return store.release(key(receiptKey));
        },
        async consume(receiptKey) {
            const operationKey = key(receiptKey);
            if (await store.reserve(operationKey, [`gate-native:${receiptKey}`]) !== 'RESERVED')
                return false;
...
function canonicalizeAction(input) {
    ...
    if (!result.caid)
        throw new Error(`github_issue_action_invalid:${result.refusals?.join(',')}`);
    return { action: structuredClone(input), caid: result.caid };
}
function contextValue(storage) {
    const value = storage.getStore();
    if (!value)
        throw new Error('consequence_evidence_context_missing');
    return value;
}
function readinessPrincipal(result, expectedRecovery, expectedContract) {
...
async function verifyPoolPrincipal(pool, tenantId, expectedRecovery) {
    ...
    const aebPrincipal = readinessPrincipal(aeb, expectedRecovery, AEB_DATABASE_READINESS_CONTRACT);
    const ptePrincipal = readinessPrincipal(pte, expectedRecovery, PTE_DATABASE_READINESS_CONTRACT);
    return aebPrincipal && aebPrincipal === ptePrincipal ? aebPrincipal : null;
}
export async function verifyDatabasePrincipalSeparation({ executorPool, recoveryPool, tenantId, }) {
...
export async function createProductionConsequenceControlConfig({ environment = process.env, PoolClass = null, fetchImpl = globalThis.fetch, } = {}) {
    ...
    if (recoveryToken.length < RECOVERY_TOKEN_MIN)
        throw new Error('EMILIA_CONSEQUENCE_RECOVERY_TOKEN_invalid');
    ...
    const trust = parseStrictEnvironmentJson(required(environment, 'EMILIA_CONSEQUENCE_GATE_TRUST_JSON'), 'EMILIA_CONSEQUENCE_GATE_TRUST_JSON');
    const aebConfig = parseStrictEnvironmentJson(required(environment, 'EMILIA_CONSEQUENCE_AEB_CONFIG_JSON'), 'EMILIA_CONSEQUENCE_AEB_CONFIG_JSON');
    const statusConfig = parseStrictEnvironmentJson(required(environment, 'EMILIA_CONSEQUENCE_STATUS_CONFIG_JSON'), 'EMILIA_CONSEQUENCE_STATUS_CONFIG_JSON');
    ...
    const githubOwner = githubSlug(required(environment, 'EMILIA_CONSEQUENCE_GITHUB_OWNER', 100), 'EMILIA_CONSEQUENCE_GITHUB_OWNER');
    const githubRepo = githubSlug(required(environment, 'EMILIA_CONSEQUENCE_GITHUB_REPO', 100), 'EMILIA_CONSEQUENCE_GITHUB_REPO');
    ...
    const aebStore = createPostgresAebDurableConsumptionStore({
        ...
        tenantId,
        relyingPartyId,
        authorizeRecoveryClaim: async (claim) => (constantTimeSecret(recoveryToken, claim?.authorization)
    ...
    const consequenceStore = createProposalToEffectPostgresStore({
        ...
        authorize_recovery: async (snapshot) => {
    ...
    return {
        ...
        withEvidenceContext: async (input, work) => (storage.run(structuredClone(input), work)),
        readiness: async () => {
