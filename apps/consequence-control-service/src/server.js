...
function shutdownGraceMs(environment) {
...
function waitForDeadline(promise, timeoutMs) {
    if (timeoutMs <= 0)
...
export async function shutdownConsequenceControlService(started, { graceMs = 15_000 } = {}) {
    ...
    const shutdown = (async () => {
        ...
        try {
            ...
            if (!drained)
                ...
                started.server.closeIdleConnections?.();
            const serverDrained = await waitForDeadline(serverClosed, Math.max(0, deadline - Date.now()));
            if (!serverDrained)
                started.server.closeAllConnections?.();
        }
        finally {
...
export function installShutdownHandlers(started, { processLike = process, graceMs = 15_000, } = {}) {
...
export async function loadRuntimeConfig(file, { environment = process.env } = {}) {
...
export async function startConsequenceControlService({ environment = process.env, config, } = {}) {
    const selectedConfig = config ?? await loadRuntimeConfig(environment.EMILIA_CONSEQUENCE_CONFIG, { environment });
    ...
    try {
        ...
        return Object.freeze({
            ...started,
            shutdown: (options) => (shutdownConsequenceControlService(started, options)),
        });
    }
    catch (error) {
        if (server?.listening)
