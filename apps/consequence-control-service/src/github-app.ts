...
const OBSERVATION_SIGNATURE_DOMAIN =
  'EMILIA-CONSEQUENCE-ACTUATOR-OBSERVATION-v1';
...
const RECONCILIATION_SIGNATURE_DOMAIN =
...
export interface SignedConsequenceActuatorObservation {
  '@version': typeof CONSEQUENCE_ACTUATOR_OBSERVATION_VERSION;
  payload: ConsequenceActuatorObservationPayload;
  signature: {
    algorithm: 'Ed25519';
    key_id: string;
    value: string;
...
function requiredText(value: unknown, name: string, maximum = 4096): string {
...
function requiredIdentifier(value: unknown, name: string): string {
  ...
  if (!IDENTIFIER.test(text)) throw new TypeError(`${name}_invalid`);
  return text;
}

function requiredDigest(value: unknown, name: string): string {
  const text = requiredText(value, name, 71);
  if (!DIGEST.test(text)) throw new TypeError(`${name}_invalid`);
  return text;
}

function requiredCaid(value: unknown): string {
  const text = requiredText(value, 'caid', 512);
  if (!CAID.test(text)) throw new TypeError('caid_invalid');
  return text;
}
...
function normalizePublicKey(value: KeyMaterial): crypto.KeyObject {
  let key: crypto.KeyObject;
  try {
    key = value instanceof crypto.KeyObject
      ? (value.type === 'private' ? crypto.createPublicKey(value as any) : value)
...
function endpointUrl(value: unknown, allowInsecureLoopback: boolean): string {
  let url: URL;
  try {
...
async function boundedJson(response: any): Promise<JsonObject> {
  ...
  try {
    while (true) {
      ...
      if (!(chunk.value instanceof Uint8Array)) throw new Error('actuator_response_invalid');
      total += chunk.value.byteLength;
      if (total > MAX_RESPONSE_BYTES) throw new Error('actuator_response_too_large');
  ...
  if (!strictJsonGate(text).ok) throw new Error('actuator_response_invalid');
  const parsed = JSON.parse(text);
  if (!plainObject(parsed)) throw new Error('actuator_response_invalid');
  return parsed;
}

function observationSignatureInput(payload: unknown): Buffer {
...
function providerAttributionSignatureInput(payload: unknown): Buffer {
  return Buffer.concat([
...
function validObservationPayload(value: unknown): value is ConsequenceActuatorObservationPayload {
  if (!exactKeys(value, OBSERVATION_PAYLOAD_KEYS)) return false;
...
function verifyObservation(
...
): ConsequenceActuatorObservationPayload | null {
  ...
  if (signature.byteLength !== 64
      ...
      || !crypto.verify(
        null,
        observationSignatureInput(evidence.payload),
        publicKey,
        signature,
      )) {
    return null;
...
function reconciliationSignatureInput(evidence: unknown): Buffer {
  return Buffer.concat([
...
function reconciliationEvidenceDigest(value: JsonObject): string {
...
function verifyReconciliationObservation(
  candidate: unknown,
  expected: JsonObject,
  {
    keyId,
  ...
  if (signature.byteLength !== 64
      || !crypto.verify(
        null,
        reconciliationSignatureInput(candidate.evidence),
        publicKey,
        signature,
      )) {
    return { valid: false, reason: 'provider_evidence_signature_invalid' };
  ...
  if (evidence['@version'] !== CONSEQUENCE_ACTUATOR_OBSERVATION_VERSION
      ...
      || RECONCILIATION_EXPECTED_KEYS.some(
...
export function consequenceActuatorTargetDigest({
  providerId,
  providerAccountId,
  owner,
  repo,
  ...
  return digestAeb({
    ...
    provider_account_id: requiredIdentifier(
      providerAccountId,
      'provider_account_id',
    ),
    target: {
...
export function createConsequenceActuatorClient({
  ...
  const providerAccount = requiredIdentifier(
    providerAccountId,
    'provider_account_id',
  );
  ...
  async function verifyProviderEvidence({
    evidence,
    expected,
    action: candidate,
  }: any = {}) {
  ...
  async function effect({
    ...
    const actionDigest = requiredDigest(
      proposal.aeb_action_digest,
      'action_digest',
    );
    ...
    if (!Number.isSafeInteger(current)) throw new Error('actuator_clock_invalid');
    ...
    const payload: ConsequenceExecutionEnvelopePayload = {
    ...
    const attribution = JSON.parse(canonicalize({
      ...
      signature: {
        ...
        value: crypto.sign(
          null,
          providerAttributionSignatureInput(attributionPayload),
          signingKey,
        ).toString('base64url'),
      },
    }));
    ...
    const executionObservation = verifyObservation(
      body.observation,
      observationPins,
    );
    ...
    if (body.outcome === 'INDETERMINATE') {
      const error: any = new Error('actuator_provider_outcome_indeterminate');
