...
interface FetchPendingSignoffsArgs {
  baseUrl: string;
  token?: string;
...
export async function fetchPendingSignoffs({ baseUrl, token, fetchImpl = fetch }: FetchPendingSignoffsArgs) {
...
interface FetchSignoffOptionsArgs {
  baseUrl: string;
  challengeId: string;
  token?: string;
  fetchImpl?: typeof fetch;
...
export async function fetchSignoffOptions({ baseUrl, challengeId, token, fetchImpl = fetch }: FetchSignoffOptionsArgs) {
...
export async function submitSignoff({ baseUrl, challengeId, attestation, token, fetchImpl = fetch }: SubmitSignoffArgs) {
