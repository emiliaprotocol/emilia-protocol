...
const DEMO_PENDING: SignoffItem[] = [{
...
export default function App(): React.JSX.Element {
  const [pending, setPending] = useState<SignoffItem[]>(DEMO_PENDING);
  ...
  const approve = useCallback(async (item: SignoffItem): Promise<void> => {
    ...
    try {
      ...
      if (TOKEN) {
        ...
        setPending((p: SignoffItem[]) => p.filter((x: SignoffItem) => x.challenge_id !== item.challenge_id));
      } else {
        Alert.alert('Signed (demo)', 'A verifiable Class-A signoff was produced on-device.');
      }
    } catch (e) {
      Alert.alert('Not approved', String((e as any).message || e));
    } finally {
      setBusy(null);
    }
  }, []);
  ...
  return (
    <SafeAreaView style={styles.screen}>
      ...
      <FlatList
        data={pending}
        keyExtractor={(i: SignoffItem) => i.challenge_id}
        ListEmptyComponent={<Text style={styles.empty}>Nothing to approve.</Text>}
        renderItem={({ item }: { item: SignoffItem }) => (
