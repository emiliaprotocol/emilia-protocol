-- 134_gate_commit_atomic_binding.sql
--
-- A /api/trust/gate allow commit authorizes exactly one materially identical
-- high-stakes commit issuance. Validation and consumption must share one
-- database transaction; otherwise expiry, revocation, or replay can race the
-- route between a read and the consumed_gate_refs insert.
