...
CREATE INDEX IF NOT EXISTS idx_security_events_target ON security_events (target_type, target_id, created_at DESC);

ALTER TABLE security_events ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "service_role_bypass" ON security_events;
CREATE POLICY "service_role_bypass" ON security_events TO service_role USING (true) WITH CHECK (true);
...
CREATE TRIGGER enforce_security_events_no_update BEFORE UPDATE ON security_events FOR EACH ROW EXECUTE FUNCTION prevent_security_event_mutation();
DROP TRIGGER IF EXISTS enforce_security_events_no_delete ON security_events;
CREATE TRIGGER enforce_security_events_no_delete BEFORE DELETE ON security_events FOR EACH ROW EXECUTE FUNCTION prevent_security_event_mutation();
