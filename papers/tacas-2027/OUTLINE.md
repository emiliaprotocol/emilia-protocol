...
5. **Two conservation surfaces.** One bounded TLA+ model checks that child registration transfers rather than copies a parent's balance across three possible siblings in one authoritative state domain. A separate bounded model mirrors the pure `EP-AUTHORITY-PROGRAM-v1` series/parallel verification fold and requires stage-scoped capability narrowing plus authoritative parallel allocation. Neither model establishes conservation across independent stores, and the program verifier explicitly preserves `execution_proven = false`.
...
4. **TLA+ lifecycle and capability models — 2.5 pages**
   ...
   - `ep_capability.tla`: 12 invariants and five temporal properties under four capability atoms, three operation atoms, finite budgets, delegation depth, and ticks. The configuration can explore three sibling candidates in one authoritative state domain; it is not an unbounded fan-out proof.
   ...
   - The implementation bridge: parent-funded child creation, operation-to-child binding, active-child and provider-effect concurrency ceilings, orphan retention, and a deliberate two-store counterexample to the global-conservation overclaim.
...
7. **Cross-language conformance and CI coupling — 2.0 pages**
   - 21 suites, 335 vectors, three same-team reference ports.
...
10. **Related work and conclusion — 1.5 pages**
   - Authorization receipts, WebAuthn, in-toto layouts, SCITT, RATS/EAT, capability systems, and TLA+/Alloy/Tamarin industrial case studies.
   - AIP's ceiling-valued token budgets, PEDIGREE delegation, CB4A credential-custody patterns, condition-bounded workload keys, and affine LLM token budgets. Distinguish their claims from EMILIA's exact-action, running-balance, provider-entry, and post-entry-uncertainty surface.
...
- the 332-vector conformance manifest and all three port runners;
