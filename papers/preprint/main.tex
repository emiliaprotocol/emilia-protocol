...
%
% arXiv preprint source (LaTeX). Target primary category: cs.CR (Cryptography
% and Security); suggested cross-list: cs.CY (Computers and Society).
%
% Content is a faithful carriage of papers/authorization-receipts-preprint.md
% at repository HEAD. No claim in this .tex is stronger than that source.
...
%     (totals.suites=18, totals.vectors=251; manifest_sha256
%      1285c269b3d58e325c2432d59a95e598761abf3cf3c8f2933b82f21b59214888)
...
%   - Alloy 15 + 7 + 6 + 4 = 32 assertions (4 models) -> formal/PROOF_STATUS.md
%   - Rust external verifier commit 7faba360..., 164 vectors, 353+6 hostility
%     -> conformance/external/rust-cleanroom-jdieselny.v1.json
% NOTE: lib/proof-stats.json currently reports conformance.vectors=232, which is
% stale relative to the authoritative conformance-manifest.json (251). This .tex
% uses 251 (the manifest), matching the source preprint. Flagged for the lead.
%
...
set of individual Internet-Drafts (\code{draft-schrock-ep-authorization-receipts-06}
...
(Section 6).
\item A cross-language conformance battery (18 suites, 251 vectors) whose
...
The quorum construction and the protocol's relational structure are separately
checked in Alloy (6.2.0, SAT4J). One model proves fifteen assertions over the
...
observer-independent portability). A quorum model proves six assertions about the
two-person rule: no signer self-approves, no human and no key fills two approver
slots, the two-person rule holds under the modeled policy, and the ordered
approval chain is acyclic and linear. A delegation model proves four assertions
about capability delegation: the delegation chain is acyclic, delegation
identifiers are unique, no leaf capability is its own ancestor, and authority is
non-increasing from parent to child along the chain. Across the four models the
analyzer discharges thirty-two assertions with no counterexample in the bounded
scope. These relational checks treat Ed25519
...
battery of 18 suites comprising 251 vectors. The receipt vectors are self-contained
...
\emph{one repository}: they are same-team ports whose agreement across all 251
...
with JavaScript, Python, and Go same-team ports); the 18-suite / 251-vector
...
\bibitem{ar06} I. Schrock, ``Authorization Receipts for High-Risk Agent Actions,''
Internet-Draft \code{draft-schrock-ep-authorization-receipts-06}, work in progress,
July 2026.

\bibitem{quorum02} I. Schrock, ``Multi-Party Human Authorization (EP-QUORUM),''
Internet-Draft \code{draft-schrock-ep-quorum-02}, work in progress, July 2026.

\bibitem{aec02} I. Schrock, ``Authorization Evidence Chains: Composing Heterogeneous
Agent-Authorization Receipts (EP-AEC),'' Internet-Draft
\code{draft-schrock-ep-authorization-evidence-chain-02}, work in progress, July
2026.
