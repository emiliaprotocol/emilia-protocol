...
also characterize four load-bearing assumptions by concrete attacks, give an
acceptance-prefix integrity result with an explicit anti-backdating boundary, and
show how Ed25519 and ML-DSA context strings instantiate the construction. Four
...
\item an acceptance-prefix integrity theorem that survives later key reveal,
paired with an impossibility result for offline anti-backdating under ordinary
signatures alone;
...
\begin{itemize}[leftmargin=*]
\item adaptively query $\mathsf{Sign}_j(m)$ for any message under any
uncompromised approver key $j$;
...
The signing oracle models signer harvesting. It deliberately does not assume that
an honest approver signs only a semantically good request. The theorem asks a
narrower question: if a context is accepted under an unrevealed key, was that exact
context signed?
...
\subsection{Acceptance-prefix integrity and the anti-backdating boundary}

We call the following property \emph{acceptance-prefix integrity}. It is the
evidentiary forward-integrity statement supported by the present assumptions: a
key reveal after an already recorded acceptance does not change which exact
context had to be signed before that acceptance.
...
\Adv^{\mathrm{cr}}_H(\mathcal C_H)$, a signing-oracle query for the exact accepted
slot context occurred before $t_a$.
\end{lemma}

\begin{proof}
Stop the experiment at $t_a$. The later reveal is outside this prefix and cannot
affect its distribution. If the accepted message was not queried to the signing
oracle before $t_a$, the pair is an EUF-CMA forgery for $j$. If it was queried,
injective encoding and collision resistance make the queried context equal to
the accepted context. The later
...
$x^*=(b^*,v^*,n^*,w^*)$ under an unrevealed pinned key, but the signing oracle for
that key was never queried on $m_{b^*,v^*}$.
\end{definition}
...
identifiers and keys were accepted; a required unrevealed key did not sign its
accepted slot message; the two slots do not share byte-identical $b$; or the
...
signature under unrevealed key $pk_j$ either has no matching signing-oracle query,
in which case it is an EUF-CMA forgery for $j$, or has a matching query on the
same bytes. In the latter case the exact-context-binding lemma makes the accepted
...
The challenger records every signing query as a pair $(j,m)$ and every issued
...
The challenger aborts if two distinct canonical action encodings queried or
accepted during the experiment have the same hash. An algorithm $\mathcal C_H$
...
\paragraph{Game $G_2$: exclude accepted unqueried messages.}
For each unrevealed pinned key $j$, the challenger aborts if verification accepts
$(m,\sigma)$ under $pk_j$ while $(j,m)$ is absent from the signing-query table.
...
Forger B_j(pk*, Sign*):
    ...
    on A.Sign(j, m):       record (j,m); return Sign*(m)
    on A.Sign(k, m), k!=j: record (k,m); return Sign(sk_k,m)
    on A.Reveal(k), k!=j:  return sk_k
    on A.Reveal(j):        abort this fixed-j reduction
    on accepted (m,sigma) under pk_j before Reveal(j):
        if (j,m) is absent from the query table, output (m,sigma)
...
message was not queried to its oracle and the signature verifies, which is a
valid EUF-CMA forgery.
...
In $G_2$, every accepted slot under an unrevealed key has a matching signing query
on exactly the accepted message bytes. The challenger now associates that slot
with the corresponding recorded query. This is a conceptual rewrite and changes
no probability. By the closed-schema checks, injectivity of $\Enc$, and the
...
initiator-exclusion checks remain. In $G_3$ every accepted unrevealed slot is an
exact recorded signing query, so those checks also make quorum substitution
...
$G_1\to G_2$ & an unrevealed required key accepts an unqueried message & one explicit forger $\mathcal B_j$ per required key & $\sum_j\Adv_{\mathsf{Sig}}^{\mathrm{euf\mbox{-}cma}}(\mathcal B_j)$ \\
$G_2\to G_3$ & replace accepted slots by their recorded equal-byte queries & injectivity and the $G_1$ condition & $0$ \\
...
slot message was not queried to that key's signing oracle, its accepted signature
is a forgery. If both queries exist, exact-context binding makes both signed slot
...
$G_2$ differ only on an accepted signature under an unrevealed pinned key whose
message was not queried to that key's signing oracle. The reductions above bound
those differences by the collision and per-key EUF-CMA advantages. Game $G_3$ is
identical to $G_2$ but uses exact-context binding to name the matching signing
queries, so transplantation and the signature-dependent quorum conditions cannot
...
slot without a matching oracle query is a forgery under that slot's key. In
$G_3$, each matching query binds the exact accepted slot fields. The verifier
...
\midrule
\code{Human\_Sign\_Receipt}, \code{Signed} & $\mathsf{Sign}_j(m)$ query and query-table row & the attacker may harvest genuine signatures on chosen contexts \\
...
Core authenticity and cross-action lemmas & Games $G_1$--$G_3$ & exact accepted context has a prior matching signing query \\
Later-reveal prefix lemma & acceptance-prefix lemma & a later key reveal does not rewrite the already recorded trace prefix \\
