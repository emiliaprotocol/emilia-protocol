...
function evaluatorFor(testCase: any) {
...
function inputFor(testCase: any) {
  const value = structuredClone(vector.fixture);
  if (testCase.mutation === 'action_diff_digest') value.action.diff_digest = `sha256:${'e'.repeat(64)}`;
  if (testCase.mutation === 'projection_byte_length') value.evidence.projection_record.projection.byte_length += 1;
