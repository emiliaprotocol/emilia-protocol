...
// Generated from generate-evidence.mts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
...
{
    const s = newSigner();
    const { rec, PROTECTED } = record(s);
    add('accept_two_link_sha256_then_sha384', true, { evidence_record: rec, tsa_keys: pin(s), protected_hash: PROTECTED });
}
{
    const s = newSigner();
    const { rec, PROTECTED } = record(s, { renewalHashed: 'sha384:' + '0'.repeat(96) });
    add('reject_broken_renewal', false, { evidence_record: rec, tsa_keys: pin(s), protected_hash: PROTECTED });
}
{
    const s = newSigner();
    const { rec, PROTECTED } = record(s);
    add('reject_unpinned_tsa', false, { evidence_record: rec, tsa_keys: {}, protected_hash: PROTECTED });
}
{
    const s = newSigner();
    const { rec } = record(s);
    add('reject_protected_mismatch', false, { evidence_record: rec, tsa_keys: pin(s), protected_hash: 'sha256:' + 'cd'.repeat(32) });
}
{
    const s = newSigner();
    const { rec, PROTECTED } = record(s, { t0: t(3), t1: t(1) });
    add('reject_non_monotonic_time', false, { evidence_record: rec, tsa_keys: pin(s), protected_hash: PROTECTED });
}
const suite = {
