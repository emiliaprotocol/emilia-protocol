...
const V = [];
const add = (id, description, expectValid, reason, public_key, document) => V.push({ id, description, expect: { valid: expectValid }, ...(reason ? { reason } : {}), public_key, document });
// ── ACCEPT class ────────────────────────────────────────────────────────────
add('accept_minimal', 'Minimal valid receipt', true, null, KEY.pub, receipt({ receipt_id: 'tr_min', issuer: 'ep:demo', created_at: '2026-06-11T00:00:00Z' }));
add('accept_nested_context', 'Deeply nested payload — recursive canonicalization must match across implementations', true, null, KEY.pub, receipt({
...
// Key-order-independence: sign one object, present the SAME logical payload with keys reversed.
{
    ...
    add('accept_key_order_independent', 'Reordered keys verify identically (canonical JSON is order-independent)', true, null, KEY.pub, { '@version': 'EP-RECEIPT-v1', payload: reordered, signature: { algorithm: 'Ed25519', value: sig } });
}
...
add('reject_legacy_v1_anchor_by_default', 'A legacy EP-MERKLE-v1 (sorted-pair, unbound) anchor is refused by default, production requires v2. Verifiable only via an explicit allowLegacyMerkle opt-in (see the per-language unit tests).', false, null, KEY.pub, receipt({ receipt_id: 'tr_anchored', issuer: 'ep:demo' }, { anchor: goodAnchor }));
...
add('reject_missing_signature', 'Missing signature value is refused', false, 'missing_signature', KEY.pub, { '@version': 'EP-RECEIPT-v1', payload: { receipt_id: 'tr_nosig' }, signature: { algorithm: 'Ed25519' } });
{
...
add('reject_wrong_key', 'Valid signature, but verified against an unrelated public key', false, 'wrong_key', OTHER.pub, receipt({ receipt_id: 'tr_wrongkey', issuer: 'ep:demo' }));
add('reject_malformed_signature', 'Signature value is not a valid signature', false, 'malformed_signature', KEY.pub, { '@version': 'EP-RECEIPT-v1', payload: { receipt_id: 'tr_bad' }, signature: { algorithm: 'Ed25519', value: 'bm90LWEtc2lnbmF0dXJl' } });
add('reject_tampered_anchor', 'Receipt signature is valid but the Merkle proof does not reconstruct the claimed root', false, 'tampered_anchor', KEY.pub, receipt({ receipt_id: 'tr_badanchor', issuer: 'ep:demo' }, { anchor: { leaf_hash: leaf, merkle_proof: [{ hash: sha('evil-sibling'), position: 'right' }], merkle_root: root } }));
const out = {
    suite: 'EP-RECEIPT-v1',
