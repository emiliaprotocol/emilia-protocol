...
// Generated from generate-optin-profiles.mts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
//
...
import { ReferenceConsumptionTree, verifyConsumptionProof, } from '../../packages/verify/consumption-proof.js';
...
const addCurrency = (id, args, expect_status) => {
    ...
    if (got !== expect_status)
        throw new Error(`currency ${id}: generator got ${got}, expected ${expect_status}`);
...
const addInit = (id, expectValid, att) => {
    ...
    if (got !== expectValid)
        throw new Error(`initiator ${id}: generator ok=${got}, expected ${expectValid}`);
...
function makeBundle({ nonce = 'nonce-A', otherNonces = ['nonce-B', 'nonce-C'], m = 3, n = 6 } = {}) {
    const tb = new ReferenceConsumptionTree();
    for (const o of otherNonces)
        tb.insert(o);
    const ni = tb.prove(nonce);
    const ta = new ReferenceConsumptionTree();
    for (const o of otherNonces)
        ta.insert(o);
    ta.insert(nonce);
...
const addConsume = (id, expectValid, bundle) => {
    ...
    if (got !== expectValid)
        throw new Error(`consume ${id}: generator valid=${got}, expected ${expectValid}`);
...
{
    const b = makeBundle();
    const already = new ReferenceConsumptionTree();
...
{
    const b = makeBundle();
    const abs = new ReferenceConsumptionTree();
    abs.insert('nonce-B');
    abs.insert('nonce-C');
...
{
    const b = makeBundle();
    const forked = [...b._leaves];
    forked[0] = denseLeaf('rewritten-0');
...
function cosign(cpt, id, k) {
    const digest = witnessSigningDigest(cpt);
...
const addWitness = (id, expectValid, wq) => {
    ...
    if (got !== expectValid)
        throw new Error(`witness ${id}: generator ok=${got}, expected ${expectValid}`);
...
write('currency.v2.json', 'EP-CURRENCY-v1', 'Two-valued currency status. valid iff evaluateCurrency(currency.args).currency_at_T.status === currency.expect_status.', currencyVectors, '2.0.0');
write('initiator-attestation.v1.json', 'EP-INITIATOR-ATTESTATION-v1', 'Initiator attestation validation + hostile-text neutralization. valid iff validateInitiatorAttestation(initiator_attestation).ok === expect.valid.', initiatorVectors);
write('consumption-proof.v1.json', 'EP-SMT-CONSUME-v1', 'Sparse-Merkle-over-nonce one-time consumption. valid iff verifyConsumptionProof(consumption_proof).valid === expect.valid.', consumptionVectors);
write('witness.v1.json', 'EP-WITNESS-v1', 'Witness cosignature k-of-n quorum (seeded Ed25519; JS and Python verify the SAME committed bytes). valid iff requireWitnessQuorum(witness_quorum.checkpoint, witness_quorum.cosignatures, witness_quorum.pinned, witness_quorum.k).ok === expect.valid.', witnessVectors);
