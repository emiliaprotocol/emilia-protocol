...
function signContext(context: Obj, key: typeof TEST_KEYS[number]): Obj {
  ...
  return {
    ...
    signature: crypto.sign(
      null,
      Buffer.from(contextHash.slice('sha256:'.length), 'hex'),
      privateKey,
    ).toString('base64url'),
    key_class: 'B',
...
function fixture(input: {
  ...
  binding?: Obj | null;
  contexts?: Obj[];
  keys?: ReadonlyArray<(typeof TEST_KEYS)[number]>;
  requiredApprovals?: number;
  signoffCount?: number;
} = {}): { bundle: Obj; approver_keys: Obj } {
...
function baseCase(id: string, expect: Obj, replacement: Obj = {}): Obj {
  const built = fixture();
  return {
    id,
    bundle: built.bundle,
...
cases.push(baseCase('presenter-selected-authorization-instance', {
  ...
  expected_authorization_instance:
    `b64u:${Buffer.alloc(16, 0x42).toString('base64url')}`,
}));
{
...
await writeFile(
  new URL('./authorization-bundle.v1.json', import.meta.url),
