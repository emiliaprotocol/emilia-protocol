...
function pipeline(raw) {
  let value;
  try { value = JSON.parse(raw); } catch { return { ok: false, why: 'parse error' }; }
  const gate = strictParseGate(raw);
  if (!gate.ok) return { ok: false, why: gate.reason };
  if (!isCanonicalizable(value)) return { ok: false, why: 'outside EP I-JSON profile' };
  return { ok: true, digest: sha256Hex(canonicalize(value)) };
}

const V: any[] = [];
const digests: Record<string, string> = {}; // id -> digest, for alias assertions
function accept(id: string, note: string, raw: string) {
  const r = pipeline(raw);
  if (!r.ok) throw new Error(`accept vector ${id} refused by reference pipeline: ${r.why}`);
  digests[id] = (r as any).digest;
  V.push({ id, expect: { valid: true }, canonicalization: { input_json: raw, expected_digest: (r as any).digest, note } });
}
function reject(id, note, raw) {
  const r = pipeline(raw);
  if (r.ok) throw new Error(`reject vector ${id} was accepted by reference pipeline (digest ${r.digest})`);
  V.push({ id, expect: { valid: false }, canonicalization: { input_json: raw, note } });
}
const assertSame = (a, b) => { if (digests[a] !== digests[b]) throw new Error(`expected same digest: ${a} vs ${b}`); };
const assertDiff = (a, b) => { if (digests[a] === digests[b]) throw new Error(`expected distinct digests: ${a} vs ${b}`); };
