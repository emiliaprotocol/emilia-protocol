...
function member({ role, approver, issuedAt, actionHash = ACTION, wrongKey = false, malformSig = false }) {
...
add('accept_ordered_3of3', 'Ordered quorum: Program Officer -> Authorizing Official -> Inspector General, distinct humans, increasing time, all bound to the action', 'accept', true, {
  '@type': 'ep.quorum', action_hash: ACTION, policy: orderedPolicy,
  members: [
    member({ ...PO, issuedAt: t(1) }),
    member({ ...AO, issuedAt: t(2) }),
    member({ ...IG, issuedAt: t(3) }),
  ],
});
...
add('reject_under_threshold', 'Only 2 of a required 3 approvers signed', 'threshold', false, {
  '@type': 'ep.quorum', action_hash: ACTION, policy: orderedPolicy,
  members: [member({ ...PO, issuedAt: t(1) }), member({ ...AO, issuedAt: t(2) })],
});
...
add('reject_duplicate_human', 'The Program Officer also signs the Inspector General slot — one human, two slots', 'distinct-humans', false, {
  ...
  members: [
    member({ ...PO, issuedAt: t(1) }),
    member({ ...AO, issuedAt: t(2) }),
    member({ role: 'inspector_general', approver: PO.approver, issuedAt: t(3) }),
  ],
});
...
add('reject_out_of_order', 'Ordered quorum signed out of sequence (times not strictly increasing in role order)', 'order', false, {
  '@type': 'ep.quorum', action_hash: ACTION, policy: orderedPolicy,
  members: [
    member({ ...PO, issuedAt: t(3) }),
    member({ ...AO, issuedAt: t(2) }),
    member({ ...IG, issuedAt: t(4) }),
  ],
});
...
add('reject_action_mismatch', 'One approver signed a different action_hash than the quorum authorizes', 'action-binding', false, {
  ...
  members: [
    member({ ...PO, issuedAt: t(1) }),
    member({ ...AO, issuedAt: t(2) }),
    member({ ...IG, issuedAt: t(3), actionHash: 'f'.repeat(64) }),
  ],
});
...
add('reject_expired_window', 'Signatures span longer than the policy window_sec', 'window', false, {
  ...
  members: [
    member({ ...PO, issuedAt: t(1) }),
    member({ ...AO, issuedAt: t(2) }),
    member({ ...IG, issuedAt: t(5) }), // 4 min span > 60s
  ],
});
...
add('reject_one_bad_signature', 'One member assertion verifies against the wrong key', 'cryptographic', false, {
  ...
  members: [
    member({ ...PO, issuedAt: t(1) }),
    member({ ...AO, issuedAt: t(2), wrongKey: true }),
    member({ ...IG, issuedAt: t(3) }),
  ],
});
...
const suite = {
  ...
  description: 'Adversarial conformance vectors for EP multi-party quorum approval. Each member is a real Class-A WebAuthn assertion; the quorum predicate (all-signatures-valid, action-binding, distinct-humans, roles-admitted, threshold, order, window) is fail-closed. A conformant verifier MUST return expect.valid for every vector.',
  count: V.length,
  vectors: V,
};
