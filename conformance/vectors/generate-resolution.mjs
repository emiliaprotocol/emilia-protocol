...
// Generated from generate-resolution.mts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
...
import { RESOLUTION_CONTEXT_TYPE, RESOLUTION_VERSION, computeBindingMomentHash, computeResolutionResponseHash, } from '../../packages/verify/resolution.js';
 ...
 * }} [options]
 */
function makeReceipt(resolution, { contextOverrides = {}, sourceBindingMoment = bindingMoment, signedRpId = RP_ID, signedOrigin = 'https://www.emiliaprotocol.ai', crossOrigin = false, flags = 0x05, tamperAfterSign = null, } = {}) {
    const context = {
        ep_version: '1.0',
...
add('refuse_approved_without_option_binding', 'A valid approval is evidence but not authority until the relying party pins which option denotes this action.', 'option-action-binding', false, approved, { expected_selected_option: undefined }, 'authorization');
add('refuse_approved_wrong_option_binding', 'An approval for one envelope option cannot authorize the action mapped to another option.', 'option-action-binding', false, approved, { expected_selected_option: 1 }, 'authorization');
add('refuse_approved_without_nonce_pin', 'An authentic approval is not executable authority without a relying-party-pinned ceremony nonce.', 'replay-context', false, approved, { expected_nonce: undefined }, 'authorization');
add('refuse_approved_without_initiator_pin', 'An authentic approval is not executable authority without a relying-party-pinned initiator.', 'initiator-binding', false, approved, { expected_initiator: undefined }, 'authorization');
add('refuse_approved_without_evaluation_time', 'An authentic approval is not executable authority without a relying-party evaluation time inside the signed window.', 'lifecycle', false, approved, { evaluation_time: undefined }, 'authorization');
add('reject_outcome_relabel', 'A signed decline relabeled approved after the ceremony breaks the challenge binding.', 'cryptographic-binding', false, makeReceipt({ outcome: 'declined' }, { tamperAfterSign: { resolution: { outcome: 'approved', selected_option: 0 } } }));
...
add('reject_envelope_substitution', 'The receipt is presented with a different source envelope.', 'envelope-binding', false, approved, { binding_moment: substitutedMoment });
add('reject_action_substitution', 'The relying party expects a different action digest.', 'action-binding', false, approved, { expected_action_hash: `sha256:${'b'.repeat(64)}` });
add('reject_unpinned_principal_key', 'No role-pinned principal key is supplied.', 'authority', false, approved, { principal_keys: {} });
add('reject_cross_principal_key', 'The same SPKI pinned to another principal cannot establish this principal.', 'authority', false, approved, { principal_keys: { [KEY_ID]: { principal: 'ep:principal:mallory', public_key: PUBLIC_KEY } } });
add('reject_wrong_rp', 'The relying party expects a different WebAuthn RP ID.', 'audience', false, approved, { rp_id: 'other.example' });
add('reject_wrong_origin', 'A valid signature from an origin outside the relying-party allowlist is refused.', 'audience', false, makeReceipt({ outcome: 'approved', selected_option: 0 }, { signedOrigin: 'https://attacker.example' }));
add('reject_cross_origin_ceremony', 'A cross-origin WebAuthn ceremony is refused even when its origin string is allowlisted.', 'audience', false, makeReceipt({ outcome: 'approved', selected_option: 0 }, { crossOrigin: true }));
add('reject_expired_ceremony', 'Evaluation time is outside the signed ceremony window.', 'lifecycle', false, approved, { evaluation_time: '2026-07-14T06:00:00.000Z' });
add('reject_impossible_calendar_date', 'February 30 is not an RFC 3339 instant and must not normalize to March.', 'lifecycle', false, makeReceipt({ outcome: 'approved', selected_option: 0 }, {
...
add('reject_nonce_mismatch', 'The relying party expected a different single-ceremony nonce.', 'replay-context', false, approved, { expected_nonce: 'res_other' });
add('reject_initiator_mismatch', 'The artifact is replayed under a different initiating agent.', 'initiator-binding', false, approved, { expected_initiator: 'spiffe://operator.example/agent/other' });
const malformedMoment = structuredClone(bindingMoment);
const malformedHatches = malformedMoment.question.hatches;
delete malformedHatches.dialogue;
add('reject_malformed_binding_moment', 'A source envelope that violates the binding-moment grammar cannot gain authority through a valid signature.', 'envelope-grammar', false, makeReceipt({ outcome: 'rejected' }, { sourceBindingMoment: malformedMoment }), { binding_moment: malformedMoment });
const unsafeMoment = structuredClone(bindingMoment);
unsafeMoment.question.recommended_idx = 9007199254740992;
add('reject_noncanonical_binding_moment', 'A binding moment outside the cross-language safe-integer profile is refused before hashing.', 'canonicalization', false, makeReceipt({ outcome: 'rejected' }, { contextOverrides: { envelope_hash: `sha256:${'c'.repeat(64)}` } }), { binding_moment: unsafeMoment });
add('reject_approved_without_selection', 'Approved must identify one option from the envelope.', 'outcome-grammar', false, makeReceipt({ outcome: 'approved' }));
add('reject_approved_selection_out_of_range', 'Approved cannot select an option absent from the envelope.', 'outcome-grammar', false, makeReceipt({ outcome: 'approved', selected_option: 9 }));
add('reject_amended_without_response', 'Amended must bind the principal-authored answer.', 'outcome-grammar', false, makeReceipt({ outcome: 'amended' }));
add('reject_decline_with_successor', 'Declined cannot smuggle a successor pointer; the original question was accepted as posed.', 'outcome-grammar', false, makeReceipt({ outcome: 'declined', successor_envelope_hash: computeBindingMomentHash(successorMoment) }));
add('reject_self_successor', 'A rejected or amended envelope cannot name itself as its successor.', 'continuation-binding', false, makeReceipt({ outcome: 'rejected', successor_envelope_hash: computeBindingMomentHash(bindingMoment) }));
add('reject_unknown_outcome', 'An unregistered fifth outcome fails closed.', 'outcome-grammar', false, makeReceipt({ outcome: 'deferred' }));
