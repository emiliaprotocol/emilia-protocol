...
const hashPair = (a: string, b: string): string => { const s = [a, b].sort(); return sha(s[0] + s[1]); };
...
function keyFromSeed(byte: number): KeyPair {
  const priv = crypto.createPrivateKey({ key: Buffer.concat([PKCS8, Buffer.alloc(32, byte)]), format: 'der', type: 'pkcs8' });
  const pub = crypto.createPublicKey(priv as any).export({ type: 'spki', format: 'der' }).toString('base64url');
  return { priv, pub };
}
const KEY = keyFromSeed(0xA1);        // the canonical signer
const OTHER = keyFromSeed(0xB2);      // an unrelated key (wrong-key vector)
const sign = (payload: any, k: KeyPair = KEY): string => crypto.sign(null, Buffer.from(canon(payload), 'utf8'), k.priv).toString('base64url');
const receipt = (payload: any, extra: any = {}): any => ({ '@version': 'EP-RECEIPT-v1', payload, signature: { algorithm: 'Ed25519', value: sign(payload) }, ...extra });
...
const V: any[] = [];
const add = (id: string, description: string, expectValid: boolean, reason: any, public_key: string, document: any) =>
