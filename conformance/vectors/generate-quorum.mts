...
function member({
  ...
  wrongKey = false,
  malformSig = false,
  prevContextHash = null as string | null,
  prevSignoffHash = null as string | null,
  contextOverride = null as any,
  sharedSigner = null as any,
  ...
  wrongKey?: boolean;
  malformSig?: boolean;
  prevContextHash?: string | null;
  prevSignoffHash?: string | null;
  contextOverride?: any;
  sharedSigner?: any;
  initiator?: string;
  crossOrigin?: boolean;
  ...
  const context: any = contextOverride || {
  ...
  if (prevSignoffHash !== null) context.prev_signoff_hash = prevSignoffHash;
...
const add = (id, description, failure_class, valid, quorum) =>
  V.push({ id, description, failure_class, expect: { valid }, quorum });
...
for (let i = 0; i < ROSTER.length; i++) {
  ...
  if (i > 0) context.prev_context_hash = chainHash(reverseContexts[i - 1]);
  reverseContexts.push(context);
}
const reverseMembers: any[] = [];
for (let i = ROSTER.length - 1; i >= 0; i--) {
  reverseMembers[i] = member({ ...ROSTER[i], contextOverride: reverseContexts[i] });
}
for (const withProfile of [false, true]) {
  add(`reject_reverse_context_chain_${withProfile ? 'profile' : 'legacy'}`,
    'Real signatures made in reverse order over precomputable context links must not satisfy a causal-chain policy',
    'chain', false, { '@type': 'ep.quorum', action_hash: ACTION,
...
for (const [suffix, profile] of [['missing', undefined], ['unknown', 'EP-QUORUM-SIGNOFF-CHAIN-v999']]) {
  ...
  if (profile === undefined) delete q.policy.ordered_chain_profile;
  else q.policy.ordered_chain_profile = profile;
...
add('reject_first_predecessor_null', 'The first member must omit the predecessor field, not supply null', 'chain', false,
  { '@type': 'ep.quorum', action_hash: ACTION, policy: orderedTwoOfThreePolicy, members: mixed });
