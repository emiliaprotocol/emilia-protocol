...
async function mint({ issuedAt, expiresAt, committedAt, keyFrom, keyTo, requiredApprovals = 1 }) {
  ...
  const contexts = buildContexts({ action, policyHash: computePolicyHash({ policy_id: action.policy_id }), approvers: ['ep:approver:dir'], requiredApprovals: requiredApprovals as any, issuedAt, expiresAt });
...
{
  const m = await mint({ issuedAt: '2026-06-13T11:00:00.000Z', expiresAt: '2026-06-13T18:00:00.000Z', committedAt: '2026-06-13T11:30:00.000Z', keyFrom: '2026-01-01T00:00:00Z', keyTo: '2036-01-01T00:00:00Z', requiredApprovals: '2' as any });
