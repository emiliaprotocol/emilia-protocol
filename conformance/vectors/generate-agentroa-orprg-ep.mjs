...
// Generated from generate-agentroa-orprg-ep.mts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
//
...
import { computeOrprgActionDigest, ORPRG_ACTION_PROFILE, ORPRG_JSON_JCS_PROFILE, } from '../../packages/verify/orprg.js';
...
if (!computed.caid)
    throw new Error(`CAID generation failed: ${computed.refusals?.join(', ')}`);
...
const sha256 = (value) => `sha256:${crypto.createHash('sha256').update(canonicalize(value), 'utf8').digest('hex')}`;
function signAgentRoa(body, signer, key) {
    const sig = crypto.sign(null, Buffer.from(canonicalize(body), 'utf8'), key.privateKey).toString('base64url');
...
function signOrprg(receipt, key = orprgKey) {
    ...
    receipt.authenticity.signature = crypto.sign(null, Buffer.from(canonicalize(signed), 'utf8'), key.privateKey).toString('base64url');
    return receipt;
}
...
const valid = baseVector('accept_agentroa_orprg_ep_same_caid', 'A real AgentROA AER, concrete ORPRG PermitReceipt, and two-device EP quorum verify natively and bind the same relying-party-pinned CAID.');
const actionSubstitution = baseVector('reject_action_substitution', 'The ORPRG receipt is genuinely reissued for a different invocation input; native verification succeeds but CAID correlation refuses the splice.');
...
const wrongProfile = baseVector('reject_wrong_mapping_profile', 'All native signatures verify, but the relying party pins a different ORPRG-to-CAID mapping profile hash.');
...
const untrustedIssuer = baseVector('reject_untrusted_issuer', 'The ORPRG receipt carries a mathematically valid Ed25519 signature, but its issuer key is not in the relying party trust pins.');
untrustedIssuer.orprg.issuer_keys = {};
untrustedIssuer.expect = { valid: false, first_valid: false, reason: 'untrusted_issuer' };
const approvalStateOnly = baseVector('reject_approval_state_only_substitution', 'The signed AgentROA root says approval_state=granted, but no EP Class-A/quorum artifact is present to fill the human requirement.');
...
const replay = baseVector('reject_orprg_replay', 'The exact same ORPRG single-use nonce is presented twice to one atomic durable replay domain; only the first presentation can verify.');
replay.presentations = 2;
replay.expect = { valid: false, first_valid: true, reason: 'replay_refused' };
const missingRequirement = baseVector('reject_missing_relying_party_requirement', 'All artifacts verify and match, but presenter-carried evidence cannot choose its own sufficiency bar when the relying party supplied no requirement.');
