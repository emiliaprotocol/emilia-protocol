...
// Generated from generate-revtime.mts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
 ...
 * }} opts
 */
function revStatement({ signer, target = TARGET, actionHash = target.action_hash, reason = 'authority withdrawn', revokedAt = REVOKED_AT, revokerId = REVOKER, }) {
...
{
    const s = newSigner();
    addR('reject_unpinned_revoker', false, { target: TARGET, revocation: revStatement({ signer: s }), revoker_keys: {} });
}
{
    const s = newSigner();
    const o = newSigner();
    addR('reject_key_substitution', false, { target: TARGET, revocation: revStatement({ signer: s }), revoker_keys: pinR(o) });
}
{
    const s = newSigner();
    const other = { ...TARGET, target_id: 'rcpt_other' };
    addR('reject_different_target_id', false, { target: TARGET, revocation: revStatement({ signer: s, target: other }), revoker_keys: pinR(s) });
}
{
    const s = newSigner();
    addR('reject_revoke_a_for_b', false, { target: TARGET, revocation: revStatement({ signer: s, actionHash: 'sha256:' + 'b'.repeat(64) }), revoker_keys: pinR(s) });
}
{
    const s = newSigner();
    const st = revStatement({ signer: s });
    st['@version'] = 'EP-REVOCATION-v2';
    addR('reject_wrong_version', false, { target: TARGET, revocation: st, revoker_keys: pinR(s) });
}
{
    const s = newSigner();
    const st = revStatement({ signer: s });
    st.reason = 'tampered after signing';
    addR('reject_tampered_field', false, { target: TARGET, revocation: st, revoker_keys: pinR(s) });
}
{
    const s = newSigner();
    addR('reject_missing_revoked_at', false, { target: TARGET, revocation: revStatement({ signer: s, revokedAt: null }), revoker_keys: pinR(s) });
}
{
    const s = newSigner();
    addR('reject_future_effective_instant', false, { target: TARGET, revocation: revStatement({ signer: s, revokedAt: '2026-06-21T12:00:00.000Z' }), revoker_keys: pinR(s), now: '2026-06-20T12:00:00.000Z' });
}
{
    const s = newSigner();
    const malformed = { target_type: 'receipt', target_id: '', action_hash: 'not-a-digest' };
    addR('reject_malformed_target_shape', false, { target: malformed, revocation: revStatement({ signer: s, target: malformed }), revoker_keys: pinR(s) });
}
{
    const s = newSigner();
    const st = revStatement({ signer: s });
    st.proof.algorithm = 'ES256';
    addR('reject_algorithm_label_mismatch', false, { target: TARGET, revocation: st, revoker_keys: pinR(s) });
}
{
    const s = newSigner();
    const st = revStatement({ signer: s });
    st.proof.revoker_key_id = 'ep:revoker-key:sha256:' + 'f'.repeat(64);
    addR('reject_revoker_key_id_substitution', false, { target: TARGET, revocation: st, revoker_keys: pinR(s) });
}
{
    const s = newSigner();
    const st = revStatement({ signer: s });
    st.scope_note = 'unsigned';
    addR('reject_unsigned_top_level_member', false, { target: TARGET, revocation: st, revoker_keys: pinR(s) });
}
{
    const s = newSigner();
    const st = revStatement({ signer: s });
    st.proof.signed_payload_b64u = Buffer.from('unsigned').toString('base64url');
    addR('reject_unsigned_proof_member', false, { target: TARGET, revocation: st, revoker_keys: pinR(s) });
}
{
    const s = newSigner();
    addR('accept_old_terminal_revocation', true, { target: TARGET, revocation: revStatement({ signer: s, revokedAt: '2020-01-01T00:00:00.000Z' }), revoker_keys: pinR(s), max_age_seconds: 3600, now: '2026-06-20T12:00:00.000Z' });
}
{
    const s = newSigner();
    const st = revStatement({ signer: s });
...
{
    const s = newSigner();
    const hostileRevokerId = { tenant: REVOKER };
...
{
    const s = newSigner();
    addT('accept_pinned_hash_bounds', true, { time_attestation: timeAtt({ signer: s }), tsa_keys: pinT(s), expected_hash: HASH, not_before: '2026-06-01T00:00:00.000Z', not_after: '2026-07-01T00:00:00.000Z' });
}
{
    const s = newSigner();
    addT('reject_unpinned_tsa', false, { time_attestation: timeAtt({ signer: s }), tsa_keys: {} });
}
{
    const s = newSigner();
    const o = newSigner();
    addT('reject_tsa_key_substitution', false, { time_attestation: timeAtt({ signer: s }), tsa_keys: pinT(o) });
}
{
    const s = newSigner();
    const a = timeAtt({ signer: s });
    a.time = '2030-01-01T00:00:00.000Z';
    addT('reject_tampered_time', false, { time_attestation: a, tsa_keys: pinT(s) });
}
{
    const s = newSigner();
    addT('reject_wrong_covered_hash', false, { time_attestation: timeAtt({ signer: s }), tsa_keys: pinT(s), expected_hash: 'sha256:' + 'd'.repeat(64) });
}
{
    const s = newSigner();
    addT('reject_out_of_bounds_time', false, { time_attestation: timeAtt({ signer: s }), tsa_keys: pinT(s), not_after: '2026-06-01T00:00:00.000Z' });
}
const timeSuite = {
