...
// Generated from generate-jws.mts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
//
...
const V = [];
const add = (id, description, expectValid, reason, public_key, compact_jws) => V.push({ id, description, expect: { valid: expectValid }, ...(reason ? { reason } : {}), public_key, compact_jws });
// ── ACCEPT class ──────────────────────────────────────────────────────────────
add('accept_minimal', 'Minimal valid receipt JWS', true, null, KEY.pub, jws({ receipt_id: 'tr_min', issuer: 'ep:demo', created_at: '2026-06-11T00:00:00Z' }));
add('accept_nested_context', 'Deeply nested payload — JCS canonicalization must match across implementations', true, null, KEY.pub, jws({
...
add('accept_no_kid', 'Valid JWS with no kid in the protected header', true, null, KEY.pub, jws({ receipt_id: 'tr_nokid', issuer: 'ep:demo' }, { kid: null }));
// ── REJECT class (each targets one invariant) ────────────────────────────────
add('reject_tampered_payload', 'A payload byte was flipped after signing — signature no longer matches', false, 'tampered_payload', KEY.pub, jws({ receipt_id: 'tr_tamper', context: { amount: 82000 } }, {
    mutate: ([h, p, s]) => { const b = Buffer.from(p, 'base64url'); b[Math.floor(b.length / 2)] ^= 0x01; return [h, b.toString('base64url'), s]; },
}));
add('reject_wrong_key', 'Valid signature, but verified against an unrelated public key', false, 'wrong_key', OTHER.pub, jws({ receipt_id: 'tr_wrongkey', issuer: 'ep:demo' }));
add('reject_tampered_signature', 'The signature segment was altered', false, 'tampered_signature', KEY.pub, jws({ receipt_id: 'tr_badsig', issuer: 'ep:demo' }, {
    mutate: ([h, p, s]) => { const b = Buffer.from(s, 'base64url'); b[0] ^= 0xff; return [h, p, b.toString('base64url')]; },
}));
add('reject_unsupported_alg', 'Protected header alg is not EdDSA', false, 'unsupported_alg', KEY.pub, jws({ receipt_id: 'tr_alg', issuer: 'ep:demo' }, { header: { alg: 'HS256', typ: TYP } }));
add('reject_wrong_typ', 'Protected header typ is not application/ep-receipt+jws', false, 'wrong_typ', KEY.pub, jws({ receipt_id: 'tr_typ', issuer: 'ep:demo' }, { header: { alg: 'EdDSA', typ: 'application/jwt' } }));
