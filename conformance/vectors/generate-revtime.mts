...
{ const s = newSigner(); addR('reject_missing_revoked_at', false, { target: TARGET, revocation: revStatement({ signer: s, revokedAt: null as any }), revoker_keys: pinR(s) }); }
...
{ const s = newSigner(); const st = revStatement({ signer: s }); (st as any).scope_note = 'unsigned'; addR('reject_unsigned_top_level_member', false, { target: TARGET, revocation: st, revoker_keys: pinR(s) }); }
{ const s = newSigner(); const st = revStatement({ signer: s }); (st.proof as any).signed_payload_b64u = Buffer.from('unsigned').toString('base64url'); addR('reject_unsigned_proof_member', false, { target: TARGET, revocation: st, revoker_keys: pinR(s) }); }
...
{
  const s = newSigner();
  const hostileRevokerId = { tenant: REVOKER } as any;
