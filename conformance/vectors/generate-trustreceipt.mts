...
function classASigner({ approverKeyId, privateKey, signedAt, crossOrigin = false }) {
  return {
    approverKeyId, keyClass: 'A' as const, signedAt,
    signWebAuthn: (digest) => {
...
interface MintOptions {
  legacy?: boolean;
  downgradeClassA?: boolean;
  crossOrigin?: boolean;
  decision?: string;
}

async function mint(actionParams: Record<string, unknown>, { legacy = false, downgradeClassA = false, crossOrigin = false, decision }: MintOptions = {}) {
  ...
  if (decision !== undefined) {
    ...
    for (const context of contexts) (context as any).decision = decision;
  }
...
const V: any[] = [];
const add = (id: string, expectValid: boolean, trust_receipt: any, verification: any, verify_opts?: any) =>
...
{ // I-JSON gate: a non-representable number in signed material is rejected fail-closed
  const m = await mint({ amount: 82000, currency: 'USD' });
  (m.receipt.action.params as any).rate = 1e-7; // outside the EP canonicalization profile
...
{ // empty-path degenerate: an empty inclusion_path folds to leaf == root, which is
  ...
  const cp: { tree_size: number, root_hash: string, log_key_id: string, log_signature?: string } = { ...m.receipt.log_proof.checkpoint };
