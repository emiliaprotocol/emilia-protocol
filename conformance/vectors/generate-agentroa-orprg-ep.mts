...
function ed25519(seedByte: number) {
  ...
  return {
    ...
    spki: crypto.createPublicKey(privateKey as any)
...
function webauthnMember({ role, approver, key, nonce }) {
  const context = {
    ...
    action_hash: (computed.digest as string).slice('sha256:'.length),
...
function baseVector(id, description) {
  ...
  return {
    ...
    relying_party_requirement: REQUIREMENT as string | null,
    presentations: 1,
    agentroa,
    orprg,
    ...
    ep: buildEpQuorum() as { quorum: any, profile: any },
    mapping: {
      agentroa: {
    ...
    expect: { valid: true, first_valid: true, reason: null as string | null },
  };
}
...
(actionSubstitution.expect as any) = { valid: false, first_valid: false, reason: 'material_action_mismatch' };
...
(wrongProfile.expect as any) = { valid: false, first_valid: false, reason: 'mapping_profile_refused' };
...
(untrustedIssuer.orprg as any).issuer_keys = {};
(untrustedIssuer.expect as any) = { valid: false, first_valid: false, reason: 'untrusted_issuer' };
...
(approvalStateOnly.ep as any) = { quorum: null, profile: (valid.ep as any).profile };
(approvalStateOnly.expect as any) = { valid: false, first_valid: false, reason: 'human_evidence_missing' };
...
(replay.expect as any) = { valid: false, first_valid: true, reason: 'replay_refused' };
...
(missingRequirement as any).relying_party_requirement = null;
(missingRequirement.expect as any) = { valid: false, first_valid: false, reason: 'relying_party_requirement_missing' };
...
function normalizeP256Signatures(value: any, parentKey: string | null = null): any {
