...
// Generated from generate-trustreceipt.mts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
...
import { buildContexts, collectSignoffs, assembleAuthorizationReceipt, assembleAuthorizationReceiptLegacyV1, policyHash as computePolicyHash, generateEd25519KeyPair, softwareSignerFromPrivateKey, } from '../../packages/issue/index.js';
...
const V = [];
const add = (id, expectValid, trust_receipt, verification, verify_opts) => V.push({ id, expect: { valid: expectValid }, trust_receipt, verification, ...(verify_opts ? { verify_opts } : {}) });
...
{ // §6.2 migration: a v2 receipt is accepted (alg == EP-MERKLE-v2 by default)
    ...
    if (m.receipt.log_proof.alg !== 'EP-MERKLE-v2')
        throw new Error('expected v2 anchor from default issuance');
...
{ // legacy v1 anchor is REFUSED by default (no allowLegacyMerkle) — the core fix
    ...
    if (m.receipt.log_proof.alg)
        throw new Error('legacy v1 must not carry a v2 alg marker');
...
{ // empty-path degenerate: an empty inclusion_path folds to leaf == root, which is
    ...
    const m = await mint({ amount: 82000, currency: 'USD' });
    if (m.receipt.log_proof.inclusion_path.length !== 0)
        throw new Error('expected single-leaf receipt (empty inclusion_path)');
...
{ // Class-A downgrade: the pinned key entry is Class-A, but the signoff declares
    ...
    if (m.receipt.signoffs[0].key_class !== 'B')
        throw new Error('expected signoff to declare key_class B');
    if (m.receipt.signoffs[0].webauthn)
        throw new Error('expected a bare (non-WebAuthn) Class-B signoff');
    if (m.verification.approver_keys['ep:key:dir#1'].key_class !== 'A')
        throw new Error('expected the pinned key to be Class-A');
...
{ // Class-A escalation: the pinned directory intentionally carries no class,
    ...
    if (m.receipt.signoffs[0].key_class !== 'A')
        throw new Error('expected presenter-declared Class-A signoff');
