...
const results: Array<{ name: string; required: boolean; status: string; detail?: string }> = [];
let passed = 0;
let failed = 0;
let skipped = 0;

function check(name: string, required: boolean, ok: boolean, detail?: string): void {
...
async function fetchJSON(path: string): Promise<Record<string, any>> {
...
try {
  ...
  let testEntity: Record<string, any> | null = null;
  try {
  ...
  // ═══════════════════════════════════════════════════════════════
  let testReceipt: Record<string, any> | null = null;
  if (testEntity) {
    try {
  ...
  check('Trust Receipt format (EP-RECEIPT-v1)', true, hasReceiptFormat,
    hasReceiptFormat && testReceipt ? `receipt: ${testReceipt.payload?.receipt_id}` : 'Receipt format does not match EP-RECEIPT-v1 schema');
  ...
  check('Merkle anchor proof', false, hasAnchor,
    hasAnchor && testReceipt ? `chain: ${testReceipt.anchor?.chain}` : 'No anchor data in receipt');
