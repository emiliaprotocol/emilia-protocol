...
// Generated from result-contract.mts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
import { isDeepStrictEqual } from 'node:util';
import { EXACT_EXTERNAL_RESULT_KINDS, LIVE_SUITE_EXECUTION_FILES, } from './suites.mjs';
...
export function buildSuiteContract(suiteFile, suite, executionSuite = suite) {
    ...
    for (const [id, vector] of official) {
        ...
        if (!executionVector)
            throw new Error(`${suiteFile}: execution companion missing ${id}`);
...
export function compareResultRow(contract, row) {
    ...
    if (!expect)
        return { ok: false, detail: 'unknown vector id' };
...
export function validateResultRows(contract, rows) {
    ...
    for (const id of contract.expectations.keys()) {
        if (!byId.has(id))
            throw new Error(`${contract.suiteFile}: runner omitted ${id}`);
    }
    return byId;
}
