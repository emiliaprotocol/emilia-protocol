...
} from '../packages/verify/federation.js';
...
function makeOperator(operatorId: string) {
...
function issueReceipt(op: any, payload: any, signingKey?: any) {
...
function discoveryDoc(op: any, historical: any[] = []) {
  const doc: any = { version: '1.1', operator_id: op.operatorId, keys: { [op.operatorId]: { public_key: op.publicKeyB64u, algorithm: 'Ed25519' } } };
  if (historical.length) doc.historical_keys = { [op.operatorId]: historical.map((h: any) => ({ public_key: h.publicKeyB64u, algorithm: 'Ed25519', retired_at: '2026-01-01T00:00:00Z' })) };
  return doc;
}
...
async function liveProbe() {
  if (typeof fetch === 'undefined') { console.log('  ⊘ SKIP — no fetch available'); return; }
  let doc;
  try {
    const res = await fetch(`${base}/.well-known/ep-keys.json`, { signal: AbortSignal.timeout(8000) });
    if (!res.ok) { console.log(`  ⊘ SKIP — ep-keys.json returned HTTP ${res.status}`); return; }
    doc = await res.json();
  } catch (e) {
