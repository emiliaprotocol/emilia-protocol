#!/usr/bin/env node
// Generated from ep-conformance-test.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
/**
...
async function fetchJSON(path) {
    ...
    if (!res.ok)
        return { _error: `HTTP ${res.status}`, _status: res.status };
    return res.json();
}
console.log(`
...
try {
    ...
    check('Discovery (/.well-known/ep-trust.json)', true, hasTrust, hasTrust ? `operator: ${trust.operator_id}, protocol: ${trust.protocol_version}` : trust._error);
    // ═══════════════════════════════════════════════════════════════
    ...
    check('Key publication (/.well-known/ep-keys.json)', true, hasKeys, hasKeys ? `${keyCount} public key(s) published` : (keys._error || 'Invalid format'));
    // ═══════════════════════════════════════════════════════════════
    ...
    try {
        ...
        if (regRes.ok)
            testEntity = await regRes.json();
    }
    catch { /* entity creation may not be public */ }
    check('Entity registration', true, !!testEntity?.entity_id, testEntity?.entity_id ? `entity: ${testEntity.entity_id}` : 'Entity creation endpoint unavailable or failed');
    // ═══════════════════════════════════════════════════════════════
    ...
    if (testEntity) {
        try {
            ...
            if (entity2) {
                ...
                if (receiptRes.ok)
                    testReceipt = await receiptRes.json();
            }
        }
    ...
    check('Trust Receipt format (EP-RECEIPT-v1)', true, hasReceiptFormat, hasReceiptFormat && testReceipt ? `receipt: ${testReceipt.payload?.receipt_id}` : 'Receipt format does not match EP-RECEIPT-v1 schema');
    // ═══════════════════════════════════════════════════════════════
    // CHECK 5: Ed25519 signature on receipts
    // ═══════════════════════════════════════════════════════════════
    let sigValid = false;
    if (hasReceiptFormat && testReceipt && testReceipt.signature?.algorithm === 'Ed25519' && testReceipt.signature?.value) {
        try {
    ...
    check('Ed25519 receipt signature', true, sigValid, sigValid ? 'Signature verified against published public key' : 'Signature verification failed or key not discoverable');
    // ═══════════════════════════════════════════════════════════════
    ...
    if (testEntity) {
        ...
        check('Trust Profile schema', true, hasProfile, hasProfile ? `score: ${profile.score}, confidence: ${profile.confidence}` : 'Profile schema incomplete or endpoint unavailable');
    }
    else {
    ...
    check('Trust Decision schema', true, hasDecision, hasDecision ? 'Returns allow/review/deny decision' : 'Decision endpoint unavailable or non-conformant');
    // ═══════════════════════════════════════════════════════════════
    // OPTIONAL: Handshake extension (PIP-002)
    // ═══════════════════════════════════════════════════════════════
    ...
    check('Handshake extension (PIP-002)', false, hasHandshake, hasHandshake ? 'Handshake ceremony functional' : (extensions.includes('handshake') ? 'Declared but failed' : 'Not declared'));
    // ═══════════════════════════════════════════════════════════════
    // OPTIONAL: Merkle anchor proof
    // ═══════════════════════════════════════════════════════════════
    const hasAnchor = testReceipt?.anchor?.merkle_root && testReceipt?.anchor?.merkle_proof;
    check('Merkle anchor proof', false, hasAnchor, hasAnchor && testReceipt ? `chain: ${testReceipt.anchor?.chain}` : 'No anchor data in receipt');
}
catch (err) {
