...
import { HYBRID_RECEIPT_PROFILE, HYBRID_RECEIPT_REASONS, hybridSignedBytes, loadAgilityModule, verifyHybridReceipt, } from '../../packages/issue/dist/hybrid-issuance.js';
...
describe('EP-RECEIPT-HYBRID-v1 conformance vectors', () => {
    ...
    it('names every refusal from the published vocabulary', () => {
        const known = new Set(Object.values(HYBRID_RECEIPT_REASONS));
        for (const vector of vectors.vectors) {
            if (vector.expect.reason === null)
                continue;
...
describe('EP-RECEIPT-v1 verifier behaviour on a hybrid receipt', () => {
    const valid = vectors.vectors.find((v) => v.id === 'hybrid-valid');
    ...
    it('refuses the classical leg repackaged as an EP-RECEIPT-v1 receipt', () => {
        const repackaged = {
            ...
            signature: {
                algorithm: 'Ed25519',
                value: valid.receipt.signatures.find((s) => s.alg === 'Ed25519').sig,
...
describe('determinism', () => {
    it('regenerates byte for byte', () => {
        ...
        const output = execFileSync(process.execPath, ['conformance/hybrid-receipts/generate.mjs', '--check'], { cwd: root, encoding: 'utf8' });
