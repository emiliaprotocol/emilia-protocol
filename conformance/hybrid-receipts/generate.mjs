...
// Generated from generate.mts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
//
...
import { HYBRID_RECEIPT_PROFILE, HYBRID_RECEIPT_REQUIRED_ALGORITHMS, createHybridReceipt, hybridSignedBytes, } from '../../packages/issue/dist/hybrid-issuance.js';
...
function ed25519KeyPairFromSeed(label) {
...
async function mldsaKeyPairFromSeed(label) {
...
function legOf(doc, alg) {
    const leg = doc.signatures.find((s) => s.alg === alg);
    if (!leg)
        throw new Error(`generate: receipt is missing its ${alg} leg`);
    return leg;
}
// --- vector construction ----------------------------------------------------
async function build() {
    ...
    const vectors = [];
    const add = (id, description, doc, expect) => { vectors.push({ id, description, receipt: doc, expect }); };
    add('hybrid-valid', 'Both legs over the same canonical bytes, committed algorithm set intact.', receipt, { verified: true, reason: null, failed_algorithm: null });
    const strippedEd = clone(receipt);
    strippedEd.signatures = strippedEd.signatures.filter((s) => s.alg !== 'Ed25519');
    add('ed25519-leg-stripped', 'The classical leg is removed. The committed set still requires it.', strippedEd, { verified: false, reason: 'hybrid_leg_missing', failed_algorithm: 'Ed25519' });
    const strippedPq = clone(receipt);
    strippedPq.signatures = strippedPq.signatures.filter((s) => s.alg !== 'ML-DSA-65');
    add('ml-dsa-leg-stripped', 'The post-quantum leg is removed. This is the downgrade attack the profile exists to stop.', strippedPq, { verified: false, reason: 'hybrid_leg_missing', failed_algorithm: 'ML-DSA-65' });
    const narrowed = clone(strippedPq);
    narrowed.profile.required_algorithms = ['Ed25519'];
    add('ml-dsa-leg-stripped-and-set-narrowed', 'The stripped receipt also claims it only ever required Ed25519. Refused structurally; the surviving signature would not verify over the narrowed bytes either, because the required set is inside the signed material.', narrowed, { verified: false, reason: 'algorithm_set_mismatch', failed_algorithm: null });
    const differentBytes = clone(receipt);
    differentBytes.signatures = differentBytes.signatures.map((s) => (s.alg === 'ML-DSA-65' ? clone(legOf(other, 'ML-DSA-65')) : s));
    add('legs-over-different-bytes', 'A valid ML-DSA-65 leg from a receipt over a DIFFERENT payload is spliced in. Both legs must cover identical bytes.', differentBytes, { verified: false, reason: 'signature_invalid', failed_algorithm: 'ML-DSA-65' });
    const differentBytesEd = clone(receipt);
    differentBytesEd.signatures = differentBytesEd.signatures.map((s) => (s.alg === 'Ed25519' ? clone(legOf(other, 'Ed25519')) : s));
    add('classical-leg-over-different-bytes', 'The same splice in the other direction: a valid Ed25519 leg from another payload.', differentBytesEd, { verified: false, reason: 'signature_invalid', failed_algorithm: 'Ed25519' });
    const tampered = clone(receipt);
    tampered.payload.action.parameters.amount = '9999999.00';
    add('payload-tampered', 'The signed amount is changed. Both legs fail; neither can be repaired independently.', tampered, { verified: false, reason: 'signature_invalid', failed_algorithm: 'Ed25519' });
    ...
    add('duplicate-classical-leg', 'One algorithm presented twice. One verdict per algorithm, so a repeat is refused rather than counted.', duplicated, { verified: false, reason: 'duplicate_algorithm', failed_algorithm: 'Ed25519' });
    const extra = clone(receipt);
    extra.signatures = [...extra.signatures, { alg: 'RSA-PSS', sig: 'AAAA' }];
    add('algorithm-outside-committed-set', 'A leg for an algorithm the receipt never committed to. Refused, not ignored.', extra, { verified: false, reason: 'unexpected_algorithm', failed_algorithm: 'RSA-PSS' });
    const relabelled = clone(receipt);
    relabelled['@version'] = 'EP-RECEIPT-v1';
    add('hybrid-relabelled-as-classical', 'The hybrid document wearing the classical version marker. The hybrid verifier refuses an unknown profile; it never guesses at another format.', relabelled, { verified: false, reason: 'unknown_profile', failed_algorithm: null });
