...
const seedKey = (seed) => {
  ...
  return {
    privateKey: sk,
    publicKey: crypto.createPublicKey(sk.export({ type: 'pkcs8', format: 'pem' })),
  };
};
...
async function main() {
  ...
  for (const v of vectors) {
    ...
    const gotOutcome = 'outcome' in got ? got.outcome : undefined;
    if (v.expect.outcome && gotOutcome !== v.expect.outcome) {
      throw new Error(`${v.id}: expected outcome=${v.expect.outcome}, got ${gotOutcome} (${got.reasons.join(', ')})`);
    }
    ...
    v.observed = { outcome: gotOutcome, valid: got.valid, reasons: got.reasons };
    checked += 1;
  }
