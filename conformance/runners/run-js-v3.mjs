...
// Generated from run-js-v3.mts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
// Same-team reference runner for EP-CONFORMANCE-FILE-RUNNER-v3. The evaluator
// supplies execution-only cases under opaque handles; this runner never reads
// catalogue IDs or expectations.
...
if (!corpusGate.ok)
    throw new Error(`strict corpus JSON refused: ${corpusGate.reason}`);
if (corpus?.['@version'] !== 'EP-CLEAN-ROOM-EXECUTION-SUITE-v3'
    || corpus?.runner_protocol !== 'EP-CONFORMANCE-FILE-RUNNER-v3') {
    throw new Error('unsupported expectation-separated execution suite');
}
if (!Array.isArray(corpus.vectors))
    throw new Error('execution suite must contain vectors');
const vectors = corpus.vectors.map((entry) => {
    if (!entry || typeof entry !== 'object' || Array.isArray(entry)
        || Object.keys(entry).sort().join(',') !== 'handle,input'
        || typeof entry.handle !== 'string'
        || !entry.input || typeof entry.input !== 'object' || Array.isArray(entry.input)
        || Object.hasOwn(entry.input, 'id') || Object.hasOwn(entry.input, 'expect')) {
        throw new Error('malformed expectation-separated execution vector');
    }
    return { ...entry.input, id: entry.handle };
});
const suiteMetadata = corpus.suite && typeof corpus.suite === 'object'
    && !Array.isArray(corpus.suite) ? corpus.suite : {};
const common = suiteMetadata.common && typeof suiteMetadata.common === 'object'
    && !Array.isArray(suiteMetadata.common)
    ? suiteMetadata.common
    : {};
const suiteId = String(suiteMetadata.id || '');
...
function runCanonicalization(c) {
    ...
    let value;
    try {
        value = JSON.parse(c.input_json);
    }
    catch {
        return false;
    }
    if (!strictParseGate(c.input_json).ok)
        return false;
...
const out = vectors.map((v) => {
    ...
    if (v.kind === 'graph')
        return { id: v.id, ...outcomeGraphResult(v) };
    if (v.attestation && common.receipt)
        return { id: v.id, ...exactOutcomeResult(v) };
    if (v.document)
        return { id: v.id, ...validResult(verifyReceipt(v.document, v.public_key).valid) };
    ...
    if (v.signoff)
        return { id: v.id, ...validResult(verifyWebAuthnSignoff(v.signoff, v.approver_public_key, { rpId: v.rp_id, allowedOrigins: v.allowed_origins }).valid) };
    if (v.quorum)
        return { id: v.id, ...validResult(verifyQuorum(v.quorum, { rpId: 'emiliaprotocol.ai', allowedOrigins: ['https://www.emiliaprotocol.ai'] }).valid) };
    if (v.revocation) {
        return suiteId === 'EP-REVOCATION-v1'
    ...
    if (v.time_attestation)
        return { id: v.id, ...validResult(verifyTimeAttestation(v.time_attestation, { tsaKeys: v.tsa_keys, expectedHash: v.expected_hash, notBefore: v.not_before, notAfter: v.not_after }).valid) };
    if (v.trust_receipt)
        return { id: v.id, ...validResult(verifyTrustReceipt(v.trust_receipt, { approverKeys: v.verification.approver_keys, logPublicKey: v.verification.log_public_key, ...(v.verify_opts || {}) }).valid) };
    if (v.provenance_chain)
        return { id: v.id, ...validResult(verifyProvenanceOffline(v.provenance_chain, { delegationKeys: v.delegation_keys, rootVerification: v.root_verification, actionVerification: v.action_verification, now: v.now_ms }).valid) };
    if (v.evidence_record)
        return { id: v.id, ...validResult(verifyEvidenceRecord(v.evidence_record, { tsaKeys: v.tsa_keys, protectedHash: v.protected_hash }).valid) };
    if (v.canonicalization)
        return { id: v.id, ...validResult(runCanonicalization(v.canonicalization)) };
    // EP-CURRENCY-v1: return the computed status; the evaluator keeps the
    // catalogue's asserted status out of the execution input and compares it.
    if (v.currency)
        return {
            id: v.id,
            currency_status: evaluateCurrency(v.currency.args).currency_at_T.status,
        };
    // EP-INITIATOR-ATTESTATION-v1: valid iff the attestation validates (fail-closed).
    if (v.initiator_attestation)
        return { id: v.id, ...validResult(validateInitiatorAttestation(v.initiator_attestation).ok) };
    // EP-SMT-CONSUME-v1: valid iff the sparse-Merkle absent→present transition verifies.
    if (v.consumption_proof)
        return { id: v.id, ...validResult(verifyConsumptionProof(v.consumption_proof).valid) };
    // EP-WITNESS-v1: valid iff k distinct pinned witnesses validly cosigned the head.
    if (v.witness_quorum) {
        const w = v.witness_quorum;
        return { id: v.id, ...validResult(requireWitnessQuorum(w.checkpoint, w.cosignatures, w.pinned, w.k).ok) };
    }
    ...
    if (v.timestamp_proof !== undefined)
        return { id: v.id, ...validResult(verifyTimestampProof(v.timestamp_proof, v.expected_digest, v.pinned_tsa_keys).verified) };
...
process.stdout.write(JSON.stringify(out.map(({ id, ...result }) => ({
    handle: id,
    result,
}))));
