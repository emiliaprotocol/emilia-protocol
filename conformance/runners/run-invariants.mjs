...
// Generated from run-invariants.mts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
//
...
import { CAPABILITY_SCOPE_PROFILE, capabilityActionDigest, mintCapabilityReceipt, createMemoryCapabilityStore, } from '../../packages/gate/capability-receipt.js';
...
const { checkNoDuplicateResult, checkResultImmutability, checkNotExpired, checkBindingValid, } = await import('../../lib/handshake/invariants.js');
...
function mintAndRegister(store, { budget, expiryMs }) {
    ...
    if (!registered)
        throw new Error('capability registration failed in harness setup');
    return {
...
async function runCapabilityCase(kase) {
    ...
    for (let i = 0; i < kase.actions.length; i += 1) {
        ...
        if (a.do === 'reserve') {
            ...
            if (res.ok)
                tokens.set(a.operation, res.reservation_token);
        ...
        if (a.do === 'commit') {
            ...
            assertExpect(kase, i, res, a.expect);
            if (res.ok)
                committedAmt.set(capabilityId, (committedAmt.get(capabilityId) || 0) + op.amount);
...
async function main() {
        ...
        for (const kase of inv.cases) {
            ...
            try {
                if (inv.domain === 'capability')
                    await runCapabilityCase(kase);
                else if (inv.domain === 'handshake')
                    runHandshakeCase(kase);
                else
                    throw new Error(`unknown domain: ${inv.domain}`);
    ...
    if (asJson) {
        ...
        if (failures)
            console.log(`${failures} CROSS-PORT/CONFORMANCE DIVERGENCE(S) — investigate before merge.`);
    }
