...
const boundedCheckerCache = new Map();
...
function witnessDigest(value) {
    return `sha256:${crypto
        .createHash("sha256")
        .update(JSON.stringify(canonicalize(value)))
        .digest("hex")}`;
}
function runBoundedChecker(root, runner) {
    ...
    if (run.status !== 0) {
        throw new Error(`${runner}: bounded checker failed\n${run.stdout ?? ""}${run.stderr ?? ""}`);
    }
    let result;
    try {
        ...
        throw new Error(`${runner}: bounded checker emitted invalid JSON: ${error.message}`);
    }
    if (result.verified !== true ||
        result.method !== "bounded_exhaustive_state_exploration" ||
        typeof result.model !== "string" ||
        result.model.length === 0) {
        throw new Error(`${runner}: bounded checker did not report a verified exhaustive model`);
    }
    boundedCheckerCache.set(cacheKey, result);
    return result;
}
function runBoundedFormalTrace(root, trace, model) {
    ...
    const checks = trace.obligations.map((obligation) => {
        ...
        if (row?.verified !== true ||
            ...
            throw new Error(`${trace.id}: ${runner} did not verify ${obligation} with a concrete mutation counterexample`);
        }
        const mutationStatesChecked = row.mutation_states_checked;
        if (mutationStatesChecked !== undefined &&
            (!Number.isSafeInteger(mutationStatesChecked) ||
                mutationStatesChecked <= 0)) {
            throw new Error(`${trace.id}: ${runner} reported an invalid mutation state count for ${obligation}`);
        }
        return {
            obligation,
            ...
            mutation_counterexample_sha256: witnessDigest(row.mutation_counterexample),
        };
    });
    ...
    if (trace.kind === "paired_negative_control" &&
        ...
        throw new Error(`${trace.id}: paired negative control is not bound to a checked mutation obligation`);
    }
    return {
        status: trace.kind === "sound" ? "matched" : "counterexample_detected",
...
function buildModule(trace, model) {
    ...
    const formalActions = [
        ...
        ...trace.steps.map((step, index) => trace.kind === "paired_negative_control" &&
            index === trace.steps.length - 1
            ? trace.mutation.operator
...
function buildConfig(trace, model, sourceConfig) {
    const directives = [
        configPreamble(sourceConfig),
        "SPECIFICATION TraceSpec",
        "CHECK_DEADLOCK FALSE",
        ...(trace.kind === "sound" ? ["INVARIANT TraceProjection"] : []),
        ...trace.obligations.map((obligation) => `${model.obligation_types[obligation] ?? "INVARIANT"} ${obligation}`),
...
export function runFormalTrace(root, trace, model, tlcJar) {
    ...
    try {
        ...
        writeFileSync(configFile, buildConfig(trace, model, readFileSync(path.join(root, model.config), "utf8")), "utf8");
        ...
        if (!detected) {
            throw new Error(`${trace.id}: formal negative control did not falsify ${obligation}\n${output}`);
        }
        return {
