...
const AEC_ROLE_SUITE = JSON.parse(readFileSync(
  resolve(process.cwd(), 'conformance/vectors/aec-role.v1.json'),
  'utf8',
));
...
function vector(id: string): Obj {
  const found = AEC_ROLE_SUITE.vectors.find((entry: Obj) => entry.id === id);
  if (!found) throw new Error(`missing AEC role vector: ${id}`);
  return structuredClone(found);
}

type UuidLike = `${string}-${string}-${string}-${string}-${string}`;

function deterministicIds(prefix: string): () => UuidLike {
  let next = 0;
  return () => (
    `${prefix}-${String(++next).padStart(8, '0')}-0000-0000-000000000000`
  ) as UuidLike;
}

function deterministicTokens(prefix: string) {
  let next = 0;
...
function productionState(
  consumptionBackend: Obj,
  evidenceBackend: Obj,
  identity: string,
  streamId = 'aec-fleet-assurance',
) {
  return {
...
function productionGate({
  ...
  return createAECExecutionGate({
    ...
    keysByType,
    humanFloor: 'class_a',
    store: state.store as any,
    log: state.log as any,
...
function step(
  operator: string,
  accepted: boolean,
  projection: RuntimeStep['projection'],
): RuntimeStep {
  return { operator, accepted, projection };
}

function policyVerifier(evidence: Obj, context: Obj) {
...
function chainWithPolicy(v: Obj, key: string, policy: string) {
...
async function pinnedBoundaryScenario(): Promise<RuntimeScenarioResult> {
  ...
  const mutableVerifiers: Record<string, Function> = {
  ...
  mutableVerifiers.policy_decision = (_evidence: Obj, context: Obj) => ({
    valid: true,
    action_digest: `sha256:${actionDigest(context.action)}`,
  });
  ...
  const runtimeConfigRefused =
    transactionResult.reason === 'runtime_trust_configuration_refused'
...
async function ownerReservationScenario(): Promise<RuntimeScenarioResult> {
  const base = createMemoryBackend();
  const reservationOptions: unknown[] = [];
  const backend = {
    durable: true,
    addIfAbsent: async (key: string, value: string, options?: unknown) => {
      reservationOptions.push(options);
      return base.addIfAbsent(key, value);
    },
    compareAndSet: (key: string, expected: string, replacement: string) => (
      base.compareAndSet(key, expected, replacement)
    ),
    deleteIfValue: (key: string, expected: string) => base.deleteIfValue(key, expected),
  ...
  if (!reserved) throw new Error('initial owner reservation failed');
...
async function atomicEvidenceScenario(): Promise<RuntimeScenarioResult> {
  const base = createMemoryAtomicEvidenceBackend();
  let loseFirstAppendResponse = true;
  const responseLossBackend = {
    durable: true,
    readHead: (streamId: string) => base.readHead(streamId),
    getById: (streamId: string, recordId: string) => base.getById(streamId, recordId),
    readAll: (streamId: string) => base.readAll(streamId),
    async appendIfHead(streamId: string, expectedHeadHash: string | null, record: Obj) {
  ...
  const first = createAtomicEvidenceLog(responseLossBackend, {
    ...
    recordIdFactory: () => (
      'response-loss-record-0001-0000-000000000001' as UuidLike
    ),
  });
  const recovered = await first.record({ type: 'decision', allow: true });
  const persisted = await base.getById(
    'shared-response-loss',
    'response-loss-record-0001-0000-000000000001',
  );
  const recoveredSameRecord =
    recovered.record_id === 'response-loss-record-0001-0000-000000000001'
  ...
  const restarted = createAtomicEvidenceLog(responseLossBackend, {
    streamId: 'shared-response-loss',
    recordIdFactory: () => (
      'after-restart-record-0001-0000-000000000001' as UuidLike
    ),
  });
  ...
  let substituted: Obj | null = null;
  const substitutedBackend = {
    durable: true,
    readHead: async () => null,
    getById: async () => structuredClone(substituted),
    async appendIfHead(_streamId: string, _expectedHeadHash: string | null, record: Obj) {
      substituted = {
  ...
  const strict = createAtomicEvidenceLog(substitutedBackend, {
    ...
    recordIdFactory: () => (
      'substituted-readback-0001-0000-000000000001' as UuidLike
    ),
  });
...
async function reservationFailureScenario(): Promise<RuntimeScenarioResult> {
  const v = vector('accept_pinned_human_receipt');
  const base = createMemoryBackend();
  ...
  const reservationFailureRefused =
    result.reason === 'consumption_store_unavailable'
...
async function responseLossScenario(): Promise<RuntimeScenarioResult> {
  const v = vector('accept_pinned_human_receipt');
  ...
  const records = await (restarted.evidence as any).all();
  const indeterminateRecorded = records.some(
    (record: Obj) => record.outcome === 'indeterminate',
  );
  const replayRefused =
    replay.reason === 'replay_refused' && providerExecutions === 1;
...
async function replicaRestartScenario(): Promise<RuntimeScenarioResult> {
  const v = vector('accept_pinned_human_receipt');
  const consumptionBackend = durableConsumptionBackend();
  const evidenceBackend = durableEvidenceBackend();
  let providerExecutions = 0;
  ...
  if (!firstResult.ok) throw new Error('first replica did not execute');
  const expectedKey = `aec:action:${firstResult.result.action_digest}`;
  const canonicalActionKey =
    (await consumptionBackend.get(expectedKey)) === 'committed:v2';
  ...
  const decoyReplayRefused =
    decoyReplay.reason === 'replay_refused' && providerExecutions === 1;
  ...
  const restartReplayRefused =
    restartReplay.reason === 'replay_refused' && providerExecutions === 1;
...
export async function runAecExecutionFleetAssuranceScenario(
  scenario: string,
): Promise<RuntimeScenarioResult> {
  if (scenario === 'aec-fleet-pinned-boundary') return pinnedBoundaryScenario();
  if (scenario === 'aec-fleet-owner-reservation') return ownerReservationScenario();
  if (scenario === 'aec-fleet-atomic-evidence') return atomicEvidenceScenario();
  if (scenario === 'aec-fleet-reservation-failure') return reservationFailureScenario();
  if (scenario === 'aec-fleet-response-loss') return responseLossScenario();
  if (scenario === 'aec-fleet-replica-restart') return replicaRestartScenario();
