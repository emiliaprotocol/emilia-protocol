...
import {
  ...
  type ConsequenceActuatorRefusalReason,
...
function initialProjection(overrides: Projection = {}): Projection {
  return {
    decision: "NONE",
    envelope: "NONE",
    binding: "NONE",
...
function requireRefusal<TResult>(
  result: ConsequenceActuatorExecutionResult<TResult>,
  reason: ConsequenceActuatorRefusalReason,
  invoked: boolean,
): asserts result is Extract<
  ConsequenceActuatorExecutionResult<TResult>,
  { ok: false }
> {
  requireCondition(
    result.ok === false &&
    ...
    `expected ${reason} with invoked=${String(invoked)}`,
  );
}

function executionInput(envelope: unknown) {
  return {
    envelope,
...
async function runSoundScenario(
  scenario: string,
  ...
  const actuator = new ConsequenceActuator({
    ...
    perform: async (binding) => {
      ...
      requireCondition(
        providerCredential.token === "credential-owned-only-by-actuator",
        "actuator lost its provider credential",
      );
      requireCondition(
        Object.isFrozen(binding) && !Object.hasOwn(binding, "credential"),
        "provider callback received ambient credential or mutable binding",
      );
      requireCondition(
        ...
        "provider callback received a substituted execution binding",
      );
  ...
  const exactVerification = verifyConsequenceExecutionEnvelope(
    exactEnvelope,
    {
      pins: {
        tenantId: TENANT_ID,
  ...
  requireCondition(
    exactVerification.ok,
    "real signed exact-binding envelope did not verify",
  );

  const expiredEnvelope = signConsequenceExecutionEnvelope(
    payload({
  ...
  const actionSubstitution = signConsequenceExecutionEnvelope(
    payload({ action_digest: OTHER_ACTION_DIGEST }),
    {
      privateKey: signer.privateKey,
      keyId: ENVELOPE_KEY_ID,
    },
  ...
  const wrongAction = await actuator.execute(
    executionInput(actionSubstitution),
  );
  requireRefusal(wrongAction, "action_digest_mismatch", false);

  const targetSubstitution = signConsequenceExecutionEnvelope(
    payload({ target_digest: OTHER_TARGET_DIGEST }),
    {
      privateKey: signer.privateKey,
      keyId: ENVELOPE_KEY_ID,
    },
  );
  const wrongTarget = await actuator.execute(
    executionInput(targetSubstitution),
  );
  requireRefusal(wrongTarget, "target_mismatch", false);

  const tamperedEnvelope = structuredClone(
    exactEnvelope,
  ) as SignedConsequenceExecutionEnvelope;
  ...
  requireCondition(
    providerCalls === 0 && store.size === 0,
    "expired or substituted envelope reached provider or replay store",
  );

  const indeterminate = await actuator.execute(
    executionInput(exactEnvelope),
  );
  requireRefusal(
    indeterminate,
  ...
  requireCondition(
    providerCalls === 1 &&
      ...
      consumed.expiresAt === EXPIRES_AT,
    "indeterminate provider entry was not exact, actuator-only, and consumed",
  );
  ...
  requireCondition(
    providerCalls === 1 &&
      store.size === 1 &&
      store.snapshot(TENANT_ID, NONCE)?.outcome === "INDETERMINATE",
...
async function runNegativeControl(
  ...
  requireRefusal(
    directProviderBypass,
    "malformed_envelope",
    false,
  );
  requireCondition(
    providerCalls === 0 && store.size === 0,
    "runtime accepted UnsafeDirectProviderCall",
  );
...
export async function runCompleteMediationScenario(
  scenario: string,
): Promise<RuntimeScenarioResult> {
