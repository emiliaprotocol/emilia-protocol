...
function privateKeyFromSeed(character: string): crypto.KeyObject {
...
function publicKey(privateKey: crypto.KeyObject): string {
  return crypto
    .createPublicKey(
...
function denialAuthorizationOutputs(
  verifierResult: any,
  receipt: any,
): Record<string, boolean> {
  const contexts = Array.isArray(receipt?.contexts) ? receipt.contexts : [];
  const approvals = contexts.filter(
    (context: any) =>
 ...
 */
export function runSignedDenialRuntimeScenario(vector: any) {
  ...
  const decisionEvidenceVerified =
    verifierResult?.checks?.action_hash === true &&
    verifierResult?.checks?.context_commitments === true &&
    verifierResult?.checks?.signoff_signatures === true &&
  ...
  const authorizationOutputs = denialAuthorizationOutputs(
    verifierResult,
    vector.trust_receipt,
  );
  if (
    ...
    throw new Error(
      "public Trust Receipt verifier did not preserve denial evidence while refusing every authorization output",
    );
  }
  return {
...
export function runScopedAuthorityRuntimeScenarios() {
  ...
  if (
    acceptedProof.accepted !== true ||
    unpinnedIssuer.accepted !== false ||
    acceptedScope.authorized !== true ||
    Object.values(refusals).some(
      (refusal: any) => refusal.authorized !== false,
    )
  ) {
    throw new Error(
      "public authority entry points did not enforce the bounded pin/scope scenarios",
    );
  }
 ...
 */
export function createStandaloneChallengeProjection(
  action: any,
  policy: any,
  options: any,
) {
...
const TRUST_RECEIPT_SUITE = JSON.parse(
  fs.readFileSync(
    new URL("../../vectors/trust-receipt.exec.v1.json", import.meta.url),
    "utf8",
  ),
);
...
export async function runTwoClaimAssuranceScenario(
  scenario: string,
): Promise<RuntimeScenarioResult> {
  if (
    scenario === "signed-denial-evidence-preserved" ||
    ...
    const projection = {
      ...
      allAuthorizationOutputsRefused: Object.values(
  ...
  if (
    scenario === "scoped-authority-exact-match" ||
    ...
    let projection: Record<string, string | number | boolean>;
    if (refusal) {
      projection = {
        unpinnedIssuerRefused: result.unpinnedIssuer.accepted === false,
        everyScopeViolationRefused: Object.values(result.refusals).every(
          (verdict: any) => verdict.authorized === false,
        ),
        authorized: false,
      };
    } else {
    ...
    return {
      ...
      relation: relation(
        { evaluated_at: EVALUATED_AT, registry_head: SNAPSHOT.head },
        projection,
      ),
    };
  }
