...
// Generated from mobile-continuity.mts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
import crypto from "node:crypto";
import { buildMobileActionIdentity, buildMobileProviderOutcome, } from "../../../lib/mobile/action-continuity.js";
import { consumeMobileAction, markMobileActionIndeterminate, reconcileMobileActionOperation, } from "../../../lib/mobile/store.js";
...
function deterministicExecutorKey() {
...
function createDeterministicMobileBackend({ actionCaid, actionDigest, executorKeyId, executorPublicKey, }) {
    const state = {
        phase: "idle",
        tenant: "none",
        executor: "none",
    ...
    let invokedProjection = null;
    const supabase = {
        async rpc(name, args) {
            if (name === "consume_mobile_action") {
                const exactRequest = args.p_entity_ref === ENTITY_REF &&
            ...
            if (name === "mark_mobile_action_indeterminate") {
                if (args.p_entity_ref !== ENTITY_REF ||
            ...
            if (name === "reconcile_mobile_action_operation") {
                const exactEvidence = args.p_entity_ref === ENTITY_REF &&
                    args.p_operation_id === OPERATION_ID &&
                    args.p_executor_id === EXECUTOR_ID &&
                    args.p_executor_key_id === executorKeyId &&
                    args.p_outcome === "executed" &&
                    args.p_provider_evidence?.action_caid ===
                        actionCaid &&
                    args.p_provider_evidence
        ...
        from(table) {
            ...
            const builder = {
                ...
                async maybeSingle() {
                    const exactPin = table === "mobile_executor_keys" &&
...
async function reserve(backend) {
    return consumeMobileAction(backend.supabase, {
...
export async function runMobileContinuityScenario(scenario) {
    if (scenario === "mobile-timeout-reconcile") {
        ...
        const timedOut = await markMobileActionIndeterminate(backend.supabase, {
            entityRef: ENTITY_REF,
            operationId: OPERATION_ID,
        });
        if (timedOut.ok !== true)
        ...
        const reconciled = await reconcileMobileActionOperation(backend.supabase, {
            entityRef: ENTITY_REF,
            operation,
            evidence,
        });
    ...
    if (scenario === "mobile-replay-refused") {
        ...
        if (!providerProjection)
            throw new Error("provider invocation was not observed before timeout");
        const timeoutProjection = projection(backend.state);
        const replay = await consumeMobileAction(backend.supabase, {
            entityRef: ENTITY_REF,
            actionReference: ACTION_REFERENCE,
        ...
        if (replay.ok !== false || replay.reason !== "already_consumed") {
            throw new Error("mobile replay was not refused by the production consumption boundary");
        }
        return {
            scenario,
            steps: [
    ...
    throw new Error(`unsupported mobile continuity refinement scenario: ${scenario}`);
}
