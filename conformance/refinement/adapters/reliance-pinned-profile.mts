...
} from "../../../formal/reliance-pinned-profile.model.mjs";
...
function deterministicEd25519(seedByte: number) {
...
function leafHash(value: string) {
  return crypto
    .createHash("sha256")
...
const SCENARIOS: Readonly<Record<string, ScenarioContract>> = Object.freeze({
  "reliance-all-pinned": {
    obligation: null,
...
function abstractStateFor(obligation: string | null) {
...
function assembleRuntimeInput(obligation: string | null) {
  ...
  let revocationState: Record<string, any> = {
...
export async function runReliancePinnedProfileScenario(
  scenario: string,
  ...
  assertRuntime(
    runtime.verdict === contract.expectedVerdict,
    `${scenario} returned ${runtime.verdict}, expected ${contract.expectedVerdict}`,
  );
  assertRuntime(
    runtime.rely === formal.accepted,
    `${scenario} disagreed with the bounded reliance model`,
  );
  assertRuntime(
    formal.failed_obligation === contract.obligation,
    `${scenario} did not isolate ${contract.obligation}`,
  );
  ...
  const runtimeProjection = {
    ...
    failedObligation:
      runtime.rely === true ? "none" : failedObligation,
  };
  return {
    scenario,
    steps: [
