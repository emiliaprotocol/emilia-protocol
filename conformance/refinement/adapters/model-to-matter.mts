...
function createProfile() {
  return createModelToMatterProfile({
    ...
    accepted_issuers: Object.fromEntries(
      M2M_EVIDENCE_TYPES.map((type) => [
        type,
        [
          {
...
function claimsFor(type: string, action: any): Record<string, unknown> {
...
function signedEvidence(action: any, type: string) {
...
function evidenceSet(action: any, mismatchedType?: string, otherAction?: any) {
  return M2M_EVIDENCE_TYPES.map((type) =>
    type === mismatchedType
...
async function clearanceOnceScenario(): Promise<RuntimeScenarioResult> {
  ...
  if (
    ...
    throw new Error(
      `Model-to-Matter runtime did not enforce exact single-use clearance: ${JSON.stringify({
        first,
        replay,
        effectCalls,
        actualCaid,
...
async function mismatchRefusedScenario(): Promise<RuntimeScenarioResult> {
  const action = createAction();
  ...
  const graph = buildModelToMatterGraph(
    action,
    evidenceSet(action, "domain_screening", otherAction),
  );
  let effectCalls = 0;
  ...
  if (
    ...
    throw new Error(
      "Model-to-Matter mismatched evidence runtime attempt was not refused",
    );
  }
...
export async function runModelToMatterScenario(
  scenario: string,
