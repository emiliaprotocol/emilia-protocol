...
} from "../../../packages/gate/dist/action-escrow.js";
import {
  canonicalize,
  hashCanonical,
} from "../../../packages/gate/dist/execution-binding.js";
...
const PROFILE = Object.freeze({
  ...
  required_acceptance_party_ids: Object.freeze(
    PARTIES.map(({ party_id: partyId }) => partyId),
  ),
  required_release_approver_party_ids: Object.freeze(
    PARTIES.map(({ party_id: partyId }) => partyId),
  ),
...
function durableCasStore() {
  const values = new Map<string, { revision: number; value: string }>();
  return {
    durable: true,
    ...
    latestRecord() {
      ...
      return envelope
        ? (JSON.parse(envelope.value) as Record<string, any>)
        : null;
    },
  };
}

function common(idempotencyKey: string, overrides: Record<string, any> = {}) {
  return {
    agreement_digest: AGREEMENT_DIGEST,
...
function acceptanceArtifact(partyId: string) {
  return {
...
function resolution(partyId: string) {
...
function releaseStatement(providerIdempotencyKey: string) {
  return {
...
function verifierBindings(expected: Record<string, any>) {
  return {
...
function defaultVerifiers() {
  return {
    async verifyDocumentActionBinding(
    ...
    async verifyAgreementAcceptance(
      artifact: Record<string, any>,
      expected: Record<string, any>,
    ) {
      ...
      return {
        ...
        acceptance_digest: digest(
          artifact.party_id.endsWith("client") ? "2" : "3",
        ),
    ...
    async verifyResolutionReceipt(
      artifact: Record<string, any>,
      expected: Record<string, any>,
    ) {
      const context = artifact?.signoff?.context ?? {};
      const outcome = context.resolution?.outcome;
      const party = expected.parties.find(
        ({ party_id: partyId }: { party_id: string }) =>
          partyId === context.principal,
      );
      const valid =
        artifact?.profile === "EP-RESOLUTION-v1" &&
...
function projection(
  aeState: string,
  aeReleaseCount: number,
  aeDuplicateRefused: boolean,
) {
  return {
    aeState,
...
async function prepareRelease(kernel: Record<string, any>) {
  let result = (await kernel.create(
    common("create", {
  ...
  result = (await kernel.beginAcceptance(
    common("begin-acceptance"),
  )) as KernelResult;
  assertRuntime(
    result.ok && result.state === "awaiting_acceptance",
    "acceptance opening was refused",
  );

  for (const party of PARTIES) {
    result = (await kernel.acceptAgreement(
      common(`accept-${party.role}`, {
  ...
  result = (await kernel.requestFunding(
    common("request-funding"),
  )) as KernelResult;
  assertRuntime(
    result.ok && result.state === "awaiting_funding",
    "funding request was refused",
  );

  result = (await kernel.recordFunding(
    common("record-funding", {
      provider_statement: fundingStatement(),
    }),
  )) as KernelResult;
  assertRuntime(
    result.ok && result.state === "funded",
    "funding evidence was refused",
  );

  result = (await kernel.submitMilestone(
    common("submit-milestone", {
      milestone_evidence: milestoneEvidence(),
    }),
  )) as KernelResult;
  assertRuntime(
    result.ok && result.state === "milestone_submitted",
    "exact milestone evidence was refused",
  );

  for (const party of PARTIES) {
    const approval = (await kernel.approveRelease(
      common(`approve-${party.role}`, {
        party_id: party.party_id,
        resolution: resolution(party.party_id),
      }),
    )) as KernelResult;
    assertRuntime(
      approval.ok && approval.state === "milestone_submitted",
      `release approval failed for ${party.role}`,
    );
  }
}
...
export async function runActionEscrowScenario(
  ...
  if (
    !["escrow-release-once", "escrow-duplicate-release-refused"].includes(
      scenario,
    )
  ) {
    throw new Error(
      `unsupported action escrow refinement scenario: ${scenario}`,
    );
  }
  ...
  const provider = {
    async release(request: Record<string, any>) {
      releaseCalls += 1;
    ...
    async getRelease(request: Record<string, any>) {
      getReleaseCalls += 1;
      return {
        authenticated: true,
        statement: releaseStatement(request.idempotency_key),
  ...
  assertRuntime(
    kernel.ready,
    `kernel configuration refused: ${kernel.configuration.reason}`,
  );
  ...
  const released = (await kernel.release(
    common("release-operation"),
  )) as KernelResult;
  assertRuntime(
    observedReservedState,
    "provider did not observe the durable release reservation",
  );
  assertRuntime(
    released.ok &&
      ...
      releaseCalls === 1 &&
      getReleaseCalls === 1,
    "the exact milestone was not released exactly once",
  );
  steps.push(
    {
  ...
  if (scenario === "escrow-duplicate-release-refused") {
    const duplicate = (await kernel.release(
      common("release-operation-replay"),
    )) as KernelResult;
    assertRuntime(
      !duplicate.ok &&
        ...
        releaseCalls === 1 &&
        getReleaseCalls === 1,
      "duplicate release was not refused before provider reinvocation",
    );
    steps.push({
