...
// Generated from aec.mts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
...
function exactRoleScenario() {
    ...
    if (result.allow !== true ||
        ...
        result.components.some((component) => component.valid !== true || component.bound !== true)) {
...
function substitutedRoleScenario() {
    const result = verifyAuthorizationChain({
        "@version": AEC_VERSION,
        action: ACTION,
    ...
    const evidenceWasCryptographicallyAdmitted = result.components?.length === 2 &&
        result.components.every((component) => component.valid === true && component.bound === true);
    if (!evidenceWasCryptographicallyAdmitted ||
        result.allow !== false ||
        result.satisfied !== false) {
        throw new Error("AEC substituted-role runtime attempt was not refused at the requirement boundary");
    }
    return {
...
export async function runAecScenario(scenario) {
    if (scenario === "aec-exact-role-map")
        return exactRoleScenario();
