...
function assertRuntime(condition: unknown, message: string): asserts condition {
  if (!condition) throw new Error(`revocation refinement failed: ${message}`);
}

function revokerKeyId(publicKeyB64u: string) {
...
function projection(
  now: number,
  revoked: boolean,
  lastRevocationVerdict: "none" | "accepted" | "refused",
) {
  return {
    now,
    revoked,
...
export async function runRevocationScenario(
  scenario: string,
): Promise<RuntimeScenarioResult> {
  if (
    !["revocation-effective-terminal", "revocation-future-refused"].includes(
      scenario,
    )
  ) {
  ...
  if (scenario === "revocation-future-refused") {
    const futureStatement = buildStatement(
      privateKey,
      publicKeyB64u,
      FUTURE_AT,
    );
