...
// Generated from grace-curtailment.mts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
...
const action = Object.freeze(createCurtailmentAction({
...
function ed25519FromSeed(keyId, byte) {
...
function approval({ approver, role, index, }) {
    ...
    const clientData = Buffer.from(JSON.stringify({
        type: "webauthn.get",
        challenge: Buffer.from(graceDigest(context).slice("sha256:".length), "hex").toString("base64url"),
        origin: ORIGIN,
        crossOrigin: false,
    }), "utf8");
...
function assertRuntime(condition, message) {
    if (!condition)
        throw new Error(`GRACE refinement failed: ${message}`);
}
export async function runGraceScenario(scenario) {
    if (!["grace-settle-once", "grace-indeterminate-replay-refused"].includes(scenario)) {
    ...
    if (scenario === "grace-settle-once") {
        ...
        assertRuntime(result.ok &&
            result.verdict === "executed_measured_settled" &&
            state.actuator.invocationCount() === 1 &&
            settlements === 1, "the authorized curtailment did not dispatch, meter, and settle exactly once");
        steps.push({
    ...
    assertRuntime(!first.ok &&
        first.verdict === "execution_indeterminate" &&
        first.retry_safe === false &&
        dispatchAttempts === 1 &&
        settlements === 0, "lost dispatch response was not fenced as indeterminate");
    steps.push({
    ...
    assertRuntime(!replay.ok &&
        replay.verdict === "refuse_replay" &&
        dispatchAttempts === 1 &&
        state.actuator.invocationCount() === 1 &&
        settlements === 0, "indeterminate dispatch replay reached the actuator or settlement");
    steps.push({
