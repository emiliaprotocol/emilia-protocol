...
// Generated from mobile-enrollment.mts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
import crypto from "node:crypto";
import { buildMobileEnrollmentBinding, createMobileEnrollmentService, MOBILE_ENROLLMENT_CHALLENGE_VERSION, MOBILE_ENROLLMENT_VERSION, } from "../../../packages/mobile/enrollment.js";
...
function enrollmentProjection({ state, rows, activations, replayRefused = false, }) {
    return {
...
function verifiedEnrollmentRows(value) {
    if (value === null || typeof value !== "object" || Array.isArray(value)) {
        return [];
    }
    const enrollment = value;
    const rows = [];
    if (typeof enrollment.credential_id === "string" &&
        typeof enrollment.public_key_spki === "string") {
        rows.push("webauthn");
    }
    if (typeof enrollment.attestation_key_id === "string" &&
        typeof enrollment.platform_public_key === "string") {
        rows.push("platform");
    }
    return rows.sort();
}
function enrollmentRuntime({ activationMutation = "none", }) {
    ...
    const directory = activationMutation === "drop_platform_row"
        ? {
            durable: true,
            async enrollAtomically(item) {
                const { attestation_key_id: _attestationKeyId, platform_public_key: _platformPublicKey, ...webauthnOnly } = item.enrollment;
    ...
    const service = createMobileEnrollmentService({
        ...
        verifyPasskeyRegistration: async (request) => {
            if (request.expectedChallenge !== expectedChallenge.challenge ||
        ...
        authorizeEnrollment: async (request) => request.operation === "mobile.enrollment.complete" &&
...
export async function runMobileEnrollmentScenario(scenario) {
    ...
    if (scenario === "mobile-enroll-incomplete-at-activation-refused") {
        ...
        if (result.ok !== false ||
            ...
            throw new Error("valid enrollment evidence did not reach and fail the two-row activation boundary");
        }
        return {
            scenario,
            steps: [
    ...
    throw new Error(`unsupported mobile enrollment refinement scenario: ${scenario}`);
}
