...
// Generated from network-witness.mts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
import crypto from "node:crypto";
import { acceptNetworkWitnessStatement, createMemoryWitnessSequenceStore, signNetworkWitnessStatement, } from "../../../packages/gate/dist/network-witness.js";
...
function exactStreamId(tenant, gate, witness, capturePoint) {
    return [tenant, gate, witness, capturePoint].map(lengthPrefixed).join("|");
}
function exactStreamIdFromVerified(tenant, gate, verifiedStreamId) {
    const parts = verifiedStreamId.split("\0");
    assertRuntime(parts.length === 2 && parts.every((part) => part.length > 0), "verified witness stream did not contain exactly witness and capture point");
    return exactStreamId(tenant, gate, parts[0], parts[1]);
}
function scopedSequenceStore(memoryStore, tenant, gate) {
    return {
        durable: true,
        advance: (verifiedStreamId, sequence, statementDigest) => memoryStore.advance(exactStreamIdFromVerified(tenant, gate, verifiedStreamId), sequence, statementDigest),
    };
}
...
function statement(privateKey, sequence, { actionDigest = ACTION, event = "request_observed", direction = "ingress", observedAt = "2026-07-16T19:59:30.000Z", witnessId = WITNESS_A, capturePointId = CAPTURE_A, } = {}) {
...
function projection(headSequence, poisoned, lastWitnessVerdict) {
    return {
        headSequence,
        poisoned,
        lastWitnessVerdict,
...
async function runCoordinateIsolationScenario(privateKey, publicKeyB64u) {
    ...
    const expectedKeys = coordinates.map(({ tenant, gate, witness, capture }) => exactStreamId(tenant, gate, witness, capture));
    assertRuntime(new Set(expectedKeys).size === expectedKeys.length, "one-coordinate stream fixtures did not produce distinct exact keys");
    const steps = [];
    for (const [index, coordinate] of coordinates.entries()) {
        ...
        const accepted = await acceptNetworkWitnessStatement(signed, {
            ...
            now: NOW,
            sequenceStore: scopedSequenceStore(memoryStore, coordinate.tenant, coordinate.gate),
        });
        assertRuntime(accepted.accepted &&
            accepted.consumed &&
            accepted.sequence === 1 &&
            accepted.witness_id === coordinate.witness &&
            accepted.capture_point_id === coordinate.capture, `${coordinate.operator} was refused: ${accepted.reason}`);
        const snapshotKeys = new Set(memoryStore.snapshot().map(({ stream_id: streamId }) => streamId));
        assertRuntime(snapshotKeys.size === index + 1 &&
            expectedKeys.slice(0, index + 1).every((key) => snapshotKeys.has(key)), `${coordinate.operator} did not advance only its exact tenant/gate/witness/capture stream`);
        steps.push({
...
export async function runNetworkWitnessScenario(scenario) {
    if (![
        "network-witness-coordinate-isolation",
        "network-witness-monotonic-poison",
        "network-witness-poison-clear-refused",
    ].includes(scenario)) {
        throw new Error(`unsupported network witness refinement scenario: ${scenario}`);
    }
    ...
    assertRuntime(acceptedFirst.accepted &&
        acceptedFirst.consumed &&
        acceptedFirst.sequence === 1, `first statement was refused: ${acceptedFirst.reason}`);
    steps.push({
        operator: "AcceptWitness(WitnessASeq1)",
        accepted: true,
    ...
    assertRuntime(acceptedSecond.accepted &&
        acceptedSecond.consumed &&
        acceptedSecond.sequence === 2, `monotonic statement was refused: ${acceptedSecond.reason}`);
    steps.push({
    ...
    assertRuntime(!poisoned.accepted &&
        !poisoned.consumed &&
        poisoned.reason === "sequence_equivocation", "same-sequence conflicting signed statement did not poison the stream");
    const poisonedSnapshot = memoryStore
        .snapshot()
        .find(({ stream_id: streamId }) => streamId === exactStreamId(TENANT_A, GATE_A, WITNESS_A, CAPTURE_A));
    assertRuntime(poisonedSnapshot?.sequence === 2 && poisonedSnapshot.equivocated === true, "sequence store did not persist the poisoned head");
    steps.push({
    ...
    if (scenario === "network-witness-poison-clear-refused") {
        ...
        assertRuntime(!clearAttempt.accepted &&
            !clearAttempt.consumed &&
            clearAttempt.reason === "sequence_equivocation", "later signed statement cleared or bypassed permanent stream poison");
        const finalSnapshot = memoryStore
            .snapshot()
            .find(({ stream_id: streamId }) => streamId === exactStreamId(TENANT_A, GATE_A, WITNESS_A, CAPTURE_A));
        assertRuntime(finalSnapshot?.sequence === 2 && finalSnapshot.equivocated === true, "poison-clear attempt changed the accepted stream head");
        steps.push({
