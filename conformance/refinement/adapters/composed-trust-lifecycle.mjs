...
import { assertAebExecutionStatus, runConsequenceLifecycleScenario, } from "./consequence-lifecycle.mjs";
...
function accepted(steps, operator, state, projection) {
    steps.push({
        operator,
        accepted: true,
...
function refused(steps, operator, state, projection = {}) {
    steps.push({
        operator,
        accepted: false,
        projection: { phase: state.phase, ...projection },
...
async function exerciseRemedyKernel() {
    ...
    const kernel = createRemedyProgramKernel({
        ...
        now: () => NOW,
        verifyOriginalEffect: ({ original: candidate }) => ({
            ok: true,
            ...candidate,
            evidence_digest: candidate.terminal_evidence_digest,
        }),
        verifyRevocation: ({ evidence, expected }) => ({
            ok: true,
        ...
        verifyRemedyOutcome: ({ evidence, outcome, expected, }) => ({
            ok: true,
            ...evidence,
    ...
    requireRuntime(revoked.ok &&
        revoked.state?.status === "effect_executed" &&
        revoked.state.original.outcome === "executed", `late revocation rewrote the original effect: ${revoked.reason}`);
    const dispute = {
    ...
    requireRuntime(!originalAuthority.ok &&
        originalAuthority.reason === "remedy_must_be_compensating", "original authority was accepted as remedy authority");
    const authorized = await kernel.authorizeRemedy({
        ...context,
        authorization,
    });
    requireRuntime(authorized.ok && authorized.state?.status === "remedy_authorized", `separate remedy authorization failed: ${authorized.reason}`);
    ...
    requireRuntime(uncertain.ok && uncertain.state?.status === "remedy_indeterminate", `remedy timeout was not fenced: ${uncertain.reason}`);
    const replay = await kernel.claimRemedy({
        ...context,
        remedyOperationId: authorization.remedy_operation_id,
        claimToken: "worker:formal",
    });
    requireRuntime(!replay.ok && replay.reason === "remedy_indeterminate", "indeterminate remedy was blindly replayable");
    ...
    requireRuntime(reconciled.ok &&
        reconciled.state?.status === "remedied" &&
        reconciled.state.original.outcome === "executed", `authenticated remedy reconciliation failed: ${reconciled.reason}`);
    return { originalAuthorityRefused: true, replayRefused: true };
}
let preflight = null;
async function ensureComposedRuntimeEvidence() {
    if (!preflight) {
        preflight = (async () => {
            bindExactCaid();
            ...
            await runConsequenceLifecycleScenario("consequence-indeterminate-reconcile");
...
function fullLifecycleSteps() {
    ...
    accepted(steps, "ReconcileRemedyExecuted", state, {
        remedyReconciliationAuthenticated: state.remedyReconciliationAuthenticated,
        remedyEffect: state.remedyEffect,
        originalEffect: state.originalEffect,
    });
    return steps;
}
function stateAfter(operator) {
    const state = initialState();
...
export async function runComposedTrustLifecycleScenario(scenario) {
    ...
    if (scenario === "composed-blind-replay-mutation") {
        await runConsequenceLifecycleScenario("consequence-blind-replay-refused");
    ...
    throw new Error(`unsupported composed trust lifecycle refinement scenario: ${scenario}`);
}
