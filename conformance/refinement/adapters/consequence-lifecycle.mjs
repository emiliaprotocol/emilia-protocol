...
// Generated from consequence-lifecycle.mts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
import crypto from "node:crypto";
import { readFileSync } from "node:fs";
import { createEg1Harness, createTrustedActionFirewall, EG1_DEFAULT_SELECTOR, } from "../../../packages/gate/dist/index.js";
...
const EFFECT_DIGEST_CORRECT = "sha256:5a008d03443c110476b4a05a78e5faf8076f3524d59d432eee26fc8f6c8eabef";
...
const AEC_SUITE = JSON.parse(readFileSync(new URL("../../../conformance/vectors/aec-role.v1.json", import.meta.url), "utf8"));
const AEC_VECTOR = structuredClone(AEC_SUITE.vectors.find((candidate) => candidate.id === "accept_pinned_human_receipt"));
function ensure(condition, message) {
    if (!condition)
        throw new Error(message);
}
function projection(executeStage, executeInvokeCount, executeEscrowState, reconciledBy = NO_EVIDENCE, effectDigest = NO_DIGEST) {
    return {
        executeStage,
        executeInvokeCount,
        executeEscrowState,
...
function createAebFixture(action) {
    const adapter = {
        id: "formal:human",
        version: "1",
        verifyNative({ artifact, status, trust_roots: trustRoots }) {
        ...
        mapAction({ artifact, native, expected_action: expectedAction, }) {
            return {
...
function createAttemptStore() {
    ...
    const events = [];
    const keyFor = (tenantId, attemptId) => `${tenantId}\0${attemptId}`;
    return {
        durable: true,
        ownershipFenced: true,
        compareAndSwap: true,
        ...
        async transition(input) {
            ...
            const allowed = (input.expected_state === "RESERVED" &&
        ...
        async reconcile(input) {
            ...
            const exact = entry.tenant_id === input.evidence.tenant_id &&
...
function providerEvidence(proposal, attempt, overrides = {}) {
    return {
        authenticated: true,
...
function createRuntimeFixture() {
    ...
    ensure(digestAeb(EFFECT) === EFFECT_DIGEST_CORRECT, "pinned effect digest drifted");
    ...
    const controller = createProposalToEffect({
        ...
        aeb: {
            ...
            currentStatusResolver: async ({ leg }) => aeb.statuses[leg.artifact_ref],
            statusVerifier: async ({ status_artifact: status }) => ({
                valid: true,
            ...
            verify_provider_evidence: async ({ evidence, expected }) => {
                ...
                else if (evidence?.effect_digest !== derivedEffectDigest ||
                    evidence?.effect_digest !== EFFECT_DIGEST_CORRECT) {
    ...
    ensure(proposal["@version"] === PROPOSAL_TO_EFFECT_VERSION, "proposal-to-effect prepare did not return the production version");
    return {
        aeb,
        aebStore,
        aecGate,
...
async function enterIndeterminate(fixture) {
    ...
    try {
        ...
        }, async ({ action }) => fixture.aecGate.run({
        ...
        throw new Error(`indeterminate provider execution unexpectedly returned: ${JSON.stringify(result)}`);
    }
    catch (error) {
        ...
        ensure(error?.emiliaGateOutcome?.outcome === "indeterminate", `Gate did not classify the lost provider response as indeterminate: ${JSON.stringify({
    ...
    ensure(fixture.attemptStore.events.join(",") ===
        "RESERVED,RESERVED->INVOKING,INVOKING->INDETERMINATE", "proposal-to-effect did not reserve, invoke, and freeze in order");
    ensure(fixture.aecGate.evidence
        .all()
        .some((entry) => entry.outcome === "indeterminate"), "AEC did not record the provider outcome as indeterminate");
    return { attempt, providerCalls };
}
function lifecycleStepsThroughIndeterminate() {
    return [
        {
...
async function refuseInvalidAdmissions(fixture) {
    let providerCalls = 0;
    const wrongCaid = await fixture.controller.execute({
        ...
        wrongCaid.reason === "aeb_evaluation_binding_mismatch", "wrong-CAID admission was not refused");
    ...
    ensure(unauthenticated?.ok === false && typeof unauthenticated.reason === "string", "unauthenticated admission was not refused");
    ensure(providerCalls === 0, "invalid admission crossed the effect boundary");
}
async function refuseBlindReplay(fixture, providerCalls) {
    ...
    ensure(replay.reason === "aeb_consumption_conflict", `blind replay refusal drifted: ${replay.reason}`);
    ensure(providerCalls === 1, "blind replay invoked the provider");
    return providerCalls;
}
export async function runConsequenceLifecycleScenario(scenario) {
    if (scenario !== "consequence-indeterminate-reconcile" &&
        scenario !== "consequence-blind-replay-refused") {
        throw new Error(`unknown consequence lifecycle refinement scenario: ${scenario}`);
    }
    ...
    const wrongCaid = await fixture.controller.reconcile({
        ...
        wrongCaid.reason === "provider_evidence_binding_mismatch", "wrong-CAID reconciliation was not refused");
    steps.push({
    ...
    ensure(wrongOperation?.ok === false &&
        wrongOperation.reason === "provider_evidence_binding_mismatch", "wrong-operation reconciliation was not refused");
    steps.push({
    ...
    ensure(wrongDigest?.ok === false &&
        wrongDigest.reason === "provider_effect_digest_mismatch", "wrong effect digest was not refused");
    steps.push({
    ...
    ensure(unauthenticated?.ok === false &&
        unauthenticated.reason === "provider_authentication_required", "unauthenticated reconciliation was not refused");
    steps.push({
    ...
    ensure(reconciled?.ok === true && reconciled.state === "COMMITTED", "authenticated exact reconciliation did not commit");
    ensure(reconciled.evidence_digest === EFFECT_DIGEST_CORRECT, "terminal reconciliation did not bind the exact effect digest");
    ensure([...fixture.aebStore.states.values()].every((state) => state === "CONSUMED"), "AEB authorization was not consumed after exact reconciliation");
    const storedAttempt = fixture.attemptStore.entries.get(`${entered.attempt.tenant_id}\0${entered.attempt.attempt_id}`);
    ensure(storedAttempt?.state === "COMMITTED" &&
        storedAttempt.evidence?.evidence_id === RECONCILIATION_EVIDENCE_ID &&
        storedAttempt.evidence?.evidence_digest === EFFECT_DIGEST_CORRECT, "durable attempt did not retain exact authenticated reconciliation");
    ensure(providerCalls === 1, "reconciliation re-invoked the provider");
    steps.push({
        operator: "ReconcileExact(ReconcileExecute)",
        accepted: true,
        projection: projection("EFFECT", 1, "CONSUMED", RECONCILIATION_EVIDENCE_ID, EFFECT_DIGEST_CORRECT),
    });
    return { scenario, steps };
}
