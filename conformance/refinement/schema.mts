...
export function validateTraceManifest(input: unknown): TraceManifest {
  ...
  for (const [modelPath, rawModel] of Object.entries(input.models)) {
    ...
    if (!Array.isArray(requiredActions)) {
      throw new Error(
        `models.${modelPath}.required_actions must be an array`,
      );
    }
    for (const [index, action] of requiredActions.entries()) {
      requireAction(
        action,
        `models.${modelPath}.required_actions[${index}]`,
      );
      if (!Object.hasOwn(actions, action)) {
        throw new Error(
          `models.${modelPath}.required_actions[${index}] is not registered in actions`,
        );
      }
    }
  ...
  for (const [modelPath, model] of Object.entries(models)) {
    ...
    const missing = model.required_actions.filter(
      (action) => !covered.has(action),
    );
    if (missing.length > 0) {
      throw new Error(
        `models.${modelPath}.required_actions are not trace-covered: ${missing.join(", ")}`,
      );
    }
  }
