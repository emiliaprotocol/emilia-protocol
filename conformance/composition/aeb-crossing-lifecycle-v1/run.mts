...
function sorted(value: any): Json {
  ...
  if (value !== null && typeof value === 'object') {
    return Object.fromEntries(Object.keys(value).sort().map((key) => [key, sorted(value[key])])) as Json;
  }
...
interface NativeFixture {
  native_system: 'oauth-transaction-challenge' | 'oasnt';
  adapter: AebAdapter;
  adapter_id: string;
  ...
  trust_roots: unknown[];
  role: string;
  subject_kind: 'human' | 'organization';
  profile_id: string;
...
function oauthFixture(): NativeFixture {
  ...
  const trust_roots: OAuthTransactionChallengeTrustRoot[] = [
    {
  ...
  return {
    ...
    artifact: {
      ...
      access_token_jwt: compactEs256(
        ...
        accessClaims,
        authorizationServerKey.privateKey,
      ),
    },
    status: {
...
function oasntFixture(): NativeFixture {
  const action = {
  ...
  const config: OasntAdapterConfig = {
  ...
  const trustRoot: OasntTrustRoot = {
...
function evaluateFixture(native: NativeFixture, operationId: string) {
  ...
  const entries: Record<string, AebRegistryEntry> = {
    ...
    [`role:${native.role}`]: registryEntry(
      `role:${native.role}`,
      'evidence-role',
...
function aebStore() {
  ...
  return {
    ...
    async reserve(key: string, replayKeys: readonly string[]) {
      ...
      if (replayKeys.some((replayKey) => replayOwners.has(replayKey))) return 'NATIVE_REPLAY_CONFLICT' as const;
...
function boundaryHarness(
  fixture: ReturnType<typeof evaluateFixture>,
  options: {
    store?: ReturnType<typeof aebStore>;
  ...
  const boundary = createConsequenceBoundary({
    ...
    invoke: async (context) => {
      providerCalls += 1;
      if (options.invoke) return options.invoke(context);
      return {
...
function runInput(fixture: ReturnType<typeof evaluateFixture>, action: unknown = fixture.native.action) {
  return {
...
async function validCrossingRecord(source: ReturnType<typeof evaluateFixture>) {
...
export async function buildReferenceReport() {
  ...
  cases.push(result(
    'OAUTH-ISSUED-ARTIFACT-THROUGH',
    'positive',
    oauthResult.state === 'EXECUTED' && oauthRun.providerCalls() === 1,
    'EXECUTED with one provider call',
    {
  ...
  cases.push(result(
    'HUMAN-AUTHORIZATION-THROUGH',
    'positive',
    humanResult.state === 'EXECUTED' && humanRun.providerCalls() === 1,
    'EXECUTED with one provider call',
    {
  ...
  cases.push(result(
    ...
    'hostile',
    wrapperFirstResult.state === 'EXECUTED'
      && wrapperSecondResult.state === 'REFUSED'
      && wrapperSecondResult.reason === 'native_replay_conflict'
      && wrapperSecond.providerCalls() === 0,
    'native_replay_conflict before provider entry',
    {
  ...
  cases.push(result(
    ...
    'hostile',
    concurrentRun.providerCalls() === 1
      && concurrentResults.filter((entry) => entry.state === 'EXECUTED').length === 1
      && concurrentResults.filter((entry) => entry.state === 'REFUSED').length === 1,
    'one EXECUTED, one REFUSED, one provider call',
    {
  ...
  cases.push(result(
    'EXECUTOR-OBSERVED-SUBSTITUTION-REFUSED',
    'hostile',
    substitutionResult.state === 'REFUSED'
      && substitutionResult.reason === 'exact_action_binding_mismatch'
      && substitutionRun.providerCalls() === 0,
    'exact_action_binding_mismatch before provider entry',
    {
  ...
  cases.push(result(
    ...
    'boundary',
    lostResult.state === 'INDETERMINATE'
      && lostResult.invoked === true
      && lostResult.retry_allowed === false
      && lostRun.providerCalls() === 1,
    'INDETERMINATE with retry refused',
    {
  ...
  cases.push(result(
    'BLIND-RETRY-REFUSED',
    'hostile',
    blindRetry.state === 'REFUSED' && lostRun.providerCalls() === 1,
    'REFUSED without another provider call',
    {
  ...
  cases.push(result(
    ...
    'positive',
    reconciled.state === 'EXECUTED' && lostRun.providerCalls() === 1,
    'EXECUTED by reconciliation with original provider-call count unchanged',
    { state: reconciled.state, provider_calls: lostRun.providerCalls() },
  ));
  ...
  cases.push(result(
    'CROSSING-RECORD-NONAUTHORIZING',
    'boundary',
    crossingResult.state === 'REFUSED' && crossingRun.providerCalls() === 0,
  ...
  cases.push(result(
    'NATIVE-PROVENANCE-PRESERVED',
    'boundary',
    oauthLeg?.adapter_id === OAUTH_TXN_CHALLENGE_AEB_ADAPTER_ID
      ...
      && oauthLeg?.replay_unit !== humanLeg?.replay_unit,
    'distinct native adapters, roles, and replay units remain visible',
    {
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  if (output) writeFileSync(resolve(output), `${JSON.stringify(sorted(report), null, 2)}\n`, 'utf8');
  else process.stdout.write(`${JSON.stringify(sorted(report), null, 2)}\n`);
