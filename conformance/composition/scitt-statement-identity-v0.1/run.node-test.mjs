...
test('substitutions fail while a valid re-encoding gets the correct reason', () => {
    ...
    assert.equal(byId['FALSE-TAMPERING-REASON-REFUSED'].observed.classification, 'same_signing_input_different_envelope');
});
...
test('the SCITT identity composition runner is mandatory in CI', () => {
    ...
    assert.match(packageJson.scripts['conformance:composition:scitt-statement-identity'], /npm run check:scitt-statement-identity-standalone/);
});
