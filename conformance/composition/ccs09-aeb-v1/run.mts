...
import {
  ...
  type CcsV13Receipt,
} from '../../../packages/verify/dist/aeb-ccs-adapter.js';
...
type Check = {
  id: string;
  layer: 'RECEIPT' | 'OPTIONAL-MAPPING' | 'RP-POLICY';
  description: string;
  passed: boolean;
  observed: unknown;
};
...
function fixture(params: Record<string, unknown> = { left: 19, right: 23 }) {
...
function resign(receipt: CcsV13Receipt, changes: Record<string, unknown>): CcsV13Receipt {
  const fields = { ...receipt, ...changes } as Record<string, unknown>;
  delete fields.signature;
  return signReceipt(fields as Omit<CcsV13Receipt, 'signature'>);
}

function signatureValid(receipt: CcsV13Receipt | Record<string, unknown>): boolean {
  const fields: Record<string, unknown> = { ...receipt };
  delete fields.signature;
  return crypto.verify(null, Buffer.from(canonicalizeFiniteJson(fields), 'utf8'),
    crypto.createPublicKey(PRIVATE_KEY), Buffer.from(String(receipt.signature), 'hex'));
}

function altered(value: unknown): unknown {
  if (typeof value === 'number') return value + 1;
  if (typeof value === 'string') return value.length ? (value[0] === 'a' ? 'b' : 'a') + value.slice(1) : 'changed';
...
export function runSuite() {
  ...
  const verify = (artifact: unknown) => f.adapter.verifyNative({ ...f.input, artifact });
  const map = (artifact: unknown, expected_action: unknown = f.action, pin = f.profile) =>
    f.adapter.mapAction({ ...f.input, artifact, expected_action, profile: pin, native: verify(artifact) });
  ...
  const absentMapping = f.adapter.mapAction({
    ...f.input, native, profile: undefined as unknown as AebPinnedProfile,
  });
  ...
  const statusResult = (changes: Record<string, unknown>) =>
    f.adapter.verifyNative({ ...f.input, status: { ...f.input.status, ...changes } });
  ...
  const deletedField = { ...f.allowReceipt } as Record<string, unknown>;
  delete deletedField.rule_summary;
  delete deletedField.signature;
  const missingField = signReceipt(deletedField as Omit<CcsV13Receipt, 'signature'>);
  ...
  const checks: Check[] = [
    check('CCS09-SHAPE', 'RECEIPT', 'The local fixture has the 22-field v1.3 shape, not the distinct PyPI receipt_version shape.',
      Object.keys(f.allowReceipt).length === 22 && !('receipt_version' in f.allowReceipt), Object.keys(f.allowReceipt)),
    check('CCS09-SIGNED-21-FIELDS', 'RECEIPT', 'Ed25519 authenticates all fields except the detached signature.',
      signatureValid(f.allowReceipt), { signed_fields: 21, signature_bytes: Buffer.from(f.allowReceipt.signature, 'hex').length }),
    check('CCS09-NATIVE-WITHOUT-MAPPING', 'RECEIPT', 'Native receipt acceptance does not require an AEB profile or expected action.',
      noMappingNative.native_verification === 'VERIFIED' && noMappingNative.acceptance === 'ACCEPTED', noMappingNative),
    check('CCS09-OPTIONAL-EXACT-MAP', 'OPTIONAL-MAPPING', 'An explicitly selected pinned profile maps the exact tool and arguments.',
      mapped.mapping === 'MATCH', mapped),
    check('CCS09-ABSENT-MAPPING', 'OPTIONAL-MAPPING', 'Without an opted-in mapping profile the receipt remains valid but no AEB MATCH is inferred.',
      absentMapping.mapping === 'INDETERMINATE' && noMappingNative.acceptance === 'ACCEPTED', absentMapping),
    check('CCS09-MAPPING-PIN-DRIFT', 'OPTIONAL-MAPPING', 'Changed mapping pins refuse correlation.',
      map(f.allowReceipt, f.action, badProfile).mapping === 'INDETERMINATE', map(f.allowReceipt, f.action, badProfile)),
    check('CCS09-ARGUMENT-SUBSTITUTION', 'OPTIONAL-MAPPING', 'Different executor-owned arguments do not match.',
      map(f.allowReceipt, wrongAction).mapping === 'MISMATCH', map(f.allowReceipt, wrongAction)),
    check('CCS09-FULL-ACTION-DIGEST', 'OPTIONAL-MAPPING', 'Changing the full signed digest while retaining its short prefix refuses the match.',
      verify(fullDigestChanged).native_verification === 'VERIFIED' && map(fullDigestChanged).mapping === 'MISMATCH', map(fullDigestChanged)),
    check('CCS09-SIGNATURE-TAMPER', 'RECEIPT', 'A response digest cannot be changed without invalidating the signature.',
      signatureTamper.native_verification === 'FAILED', signatureTamper),
    check('CCS09-UNTRUSTED-KEY', 'RECEIPT', 'A different relying-party key cannot validate this signature.',
      wrongKey.native_verification === 'FAILED', wrongKey),
    check('CCS09-AUDIENCE', 'RP-POLICY', 'A cryptographically valid receipt for another audience is rejected.',
      audience.native_verification === 'VERIFIED' && audience.acceptance === 'REJECTED', audience),
    check('CCS09-EXPIRY', 'RP-POLICY', 'A receipt outside its validity window is rejected.',
      expired.native_verification === 'VERIFIED' && expired.acceptance === 'REJECTED', expired),
    check('CCS09-NOT-YET-VALID', 'RP-POLICY', 'A future receipt is rejected before its allowed clock-skew window.',
      future.native_verification === 'VERIFIED' && future.acceptance === 'REJECTED', future),
    check('CCS09-DENY', 'RP-POLICY', 'A valid deny remains rejected and cannot yield a match.',
      deny.native_verification === 'VERIFIED' && deny.acceptance === 'REJECTED' && map(f.denyReceipt).mapping === 'INDETERMINATE', deny),
    check('CCS09-ESCALATE', 'RP-POLICY', 'A valid escalate remains indeterminate and cannot yield a match.',
      escalate.native_verification === 'VERIFIED' && escalate.acceptance === 'INDETERMINATE' && map(escalateReceipt).mapping === 'INDETERMINATE', escalate),
    check('CCS09-CONSUMED-STATUS', 'RP-POLICY', 'A supplied consumed status refuses receipt acceptance; no durable store is claimed.',
      consumed.acceptance === 'REJECTED' && consumed.reasons.includes('evidence_consumed'), consumed),
    check('CCS09-UNKNOWN-STATUS', 'RP-POLICY', 'Unavailable status cannot be promoted into acceptance.',
      unknown.acceptance === 'INDETERMINATE', unknown),
    check('CCS09-REVOKED-STATUS', 'RP-POLICY', 'A supplied revoked status refuses acceptance.',
      revoked.acceptance === 'REJECTED', revoked),
    check('CCS09-MACHINE-POLICY-ROLE', 'RP-POLICY', 'This pinned profile emits only system machine-policy evidence, never human approval or authorization.',
      native.evidence_role === 'machine-policy-decision' && native.subject.kind === 'system'
        && !('authorized' in native) && !('executed' in native), { evidence_role: native.evidence_role, subject: native.subject }),
    check('CCS09-EVERY-FIELD-TAMPER', 'RECEIPT', 'Mutating each of the 22 fields separately invalidates cryptography and native verification.',
      fieldMutations.every((entry) => !entry.signature_valid && entry.native_verification === 'FAILED'), fieldMutations),
    check('CCS09-CLOSED-KEYSET-EXTRA', 'RECEIPT', 'Even a correctly signed extra field is outside this closed compatibility profile.',
      signatureValid(extraField) && verify(extraField).native_verification === 'FAILED', verify(extraField)),
    check('CCS09-CLOSED-KEYSET-MISSING', 'RECEIPT', 'Even a correctly signed receipt missing a required field is refused.',
      signatureValid(missingField) && verify(missingField).native_verification === 'FAILED', verify(missingField)),
    check('CCS09-UNICODE-SIGNED-TEXT', 'RECEIPT', 'UTF-8 signed metadata round-trips through canonicalization and Ed25519.',
      unicodeCanonicalBytes === unicodeGoldenBytes && signatureValid(unicodeReceipt)
        && unicodeNative.native_verification === 'VERIFIED' && unicodeNative.acceptance === 'ACCEPTED',
      { native: unicodeNative, canonical_utf8_hex: unicodeCanonicalBytes, expected_utf8_hex: unicodeGoldenBytes }),
    check('CCS09-UNICODE-ARGUMENT-BOUNDARY', 'OPTIONAL-MAPPING', 'Valid signed Unicode arguments are outside the existing ASCII-only mapping subset.',
      unicodeArgsNative.acceptance === 'ACCEPTED' && unicodeArgsMapping.mapping === 'INDETERMINATE', { native: unicodeArgsNative, mapping: unicodeArgsMapping }),
    check('CCS09-FRACTIONAL-ARGUMENT-BOUNDARY', 'OPTIONAL-MAPPING', 'Valid signed fractional arguments are outside the existing safe-integer mapping subset.',
      fractionNative.acceptance === 'ACCEPTED' && fractionMapping.mapping === 'INDETERMINATE', { native: fractionNative, mapping: fractionMapping }),
    check('CCS09-OBJECT-ORDER', 'RECEIPT', 'Object insertion order does not change canonical signed bytes.',
      signatureValid(reordered) && verify(reordered).native_verification === 'VERIFIED', verify(reordered)),
  ];
  const report = {
...
function main() {
  ...
  if (process.argv.includes('--check')) {
    if (readFileSync(REPORT_PATH, 'utf8') !== renderedReport) throw new Error('CCS-09 compatibility report is stale');
    if (readFileSync(SAMPLE_PATH, 'utf8') !== renderedSample) throw new Error('CCS-09 compatibility fixtures are stale');
  }
