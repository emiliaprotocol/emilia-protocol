...
} from '../../../packages/gate/consequence-envelope.js';
import {
  createGraceReferenceInput,
  createGraceReferenceRuntime,
  executeGraceReferenceInput,
...
function materialAction(action: Obj) {
  return {
    action_type: 'grid.curtailment.1',
      ...
      return {
        ...
        replay_unit: digestAeb({
          ...
          approvals: artifact.authorizationEvidence
            .map((entry: Obj) => digestAeb(entry.signoff))
            .sort(),
        }),
...
async function consequenceEnvelope(capacityUnits: string): Promise<ConsequenceEnvelopeBoundary> {
...
function input(fixture: ReturnType<typeof evaluationFixture>, action: unknown = fixture.action) {
  return {
...
function graceResultSummary(value: unknown) {
...
function harness(
  ...
  envelope: ConsequenceEnvelopeBoundary,
  options: { lose_response?: boolean } = {},
) {
  ...
  const boundary = createConsequenceBoundary({
    ...
    invoke: async () => {
      providerCalls += 1;
      lastResult = await executeGraceReferenceInput(
        fixture.grace,
        createGraceReferenceRuntime(),
      );
      if (options.lose_response) throw new Error('synthetic_response_lost');
      return {
...
export async function buildReferenceReport() {
  ...
  cases.push(caseResult(
    'GRACE-THROUGH-CANONICAL-BOUNDARY',
    'positive',
    throughResult.state === 'EXECUTED'
      && through.providerCalls() === 1
      && (through.lastResult() as Obj)?.ok === true
      && throughEnvelope.snapshot().committed_units === '18000000',
    'one synthetic GRACE execution under one 18 MW aggregate reservation',
    {
      state: throughResult.state,
      provider_calls: through.providerCalls(),
      grace_verdict: (through.lastResult() as Obj)?.verdict ?? null,
  ...
  cases.push(caseResult(
    ...
    'hostile',
    exhaustedResult.state === 'REFUSED'
      && exhaustedResult.reason === 'consequence_envelope_capacity_exceeded'
      && exhausted.providerCalls() === 0,
    'capacity refusal before provider entry',
    {
  ...
  cases.push(caseResult(
    ...
    substitutionResult.state === 'REFUSED'
      ...
      && substitution.providerCalls() === 0,
    'exact-action refusal before provider entry',
    {
  ...
  cases.push(caseResult(
    ...
    lostResult.state === 'INDETERMINATE'
      ...
      && lostEnvelope.snapshot().committed_units === '18000000',
    'INDETERMINATE, no blind retry, capacity remains unavailable',
    {
  ...
  cases.push(caseResult(
    ...
    'positive',
    reconciled.state === 'EXECUTED' && lost.providerCalls() === 1,
    'terminal reconciliation with the original provider-call count unchanged',
    { state: reconciled.state, provider_calls: lost.providerCalls() },
  ));
  ...
  cases.push(caseResult(
    ...
    'boundary',
    impact.ok === true && impact.impact_units === 18000000n,
    'impact derives only from the material requested curtailment',
    { impact_watts: impact.ok ? impact.impact_units.toString() : null },
  ));
