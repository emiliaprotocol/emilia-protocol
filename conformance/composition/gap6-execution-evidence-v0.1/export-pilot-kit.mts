...
import {
  buildAuditWorkpaper,
  renderMarkdown as renderAuditWorkpaper,
} from '../../../packages/gate/reports/auditor-workpaper.js';
import {
  buildUnderwriterAttestation,
  renderMarkdown as renderUnderwriterAttestation,
} from '../../../packages/gate/reports/underwriter.js';
...
function availableOutput(requested: string): string {
  if (!existsSync(requested) || readdirSync(requested).length === 0) return requested;
...
function pilotReport(result: Awaited<ReturnType<typeof runProfile>>): string {
...
export async function exportPilotKit(requestedOutput = DEFAULT_OUTPUT) {
  ...
  if (instants.length === 0) throw new Error('pilot kit export: Gate emitted no dated evidence');
  ...
  const manifest = {
    ...
    source_tree_clean: execFileSync(
      'git', ['status', '--porcelain', '--untracked-files=no'], { cwd: HERE, encoding: 'utf8' },
    ).trim().length === 0,
...
function cliOutput(): string | null {
  const flag = process.argv.indexOf('--output');
  if (flag === -1) return null;
  const value = process.argv[flag + 1];
  if (!value) throw new Error('pilot kit export: --output requires a directory');
  return value;
}
