...
// Generated from verify-pilot-kit.mts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
...
import { FIELD_ORIGIN_CLAIM_BOUNDARY, createGate, hashCanonical, MemoryConsumptionStore, verifyFieldOriginEvidence, } from '../../../packages/gate/index.js';
...
export async function verifyPilotKit(bundleDirectory) {
    ...
    if (!/^[0-9a-f]{40}$/.test(manifest?.source_commit ?? '')) {
        ...
        try {
            ...
            if (verifierCommit !== manifest.source_commit)
                failures.push('source_commit_mismatch');
        }
        catch {
    ...
    for (const entry of listed) {
        ...
        let bytes;
        try {
            bytes = readFileSync(resolve(bundle, entry.name));
        }
        catch {
    ...
    const executionProgramVerification = readJson(resolve(bundle, 'bounded-execution-program-verification.json'));
    ...
    if (!fieldOrigin.accepted)
        failures.push(`field_origin_refused:${fieldOrigin.reason}`);
    ...
    try {
        ...
        if (!decision.allow)
            failures.push(`offline_gate_refused:${decision.reason}`);
        if (!decision.evidence?.field_origin_program_binding?.program_digest) {
            failures.push('bounded_execution_program_binding_missing');
        }
    }
    catch (error) {
        failures.push(`offline_gate_verification_failed:${String(error?.message ?? error)}`);
    }
    ...
    if (!Array.isArray(log))
        failures.push('gate_evidence_log_invalid');
    else {
    ...
    const observedReasons = new Set(Array.isArray(report?.cases) ? report.cases.map((item) => item.boundary_reason) : []);
    for (const reason of requiredReasons) {
        if (!observedReasons.has(reason))
            failures.push(`m01_case_missing:${reason}`);
    }
...
export function parseBundleDirectory(args) {
