...
import {
  ...
  type OasntTrustRoot,
} from '../../../packages/verify/aeb-oasnt-adapter.js';
...
function canonicalPythonSubset(value: unknown): string {
  if (Array.isArray(value)) return `[${value.map(canonicalPythonSubset).join(',')}]`;
  if (value !== null && typeof value === 'object') {
    return `{${Object.keys(value).sort().map((key) => (
      `${JSON.stringify(key)}:${canonicalPythonSubset((value as Record<string, unknown>)[key])}`
    )).join(',')}}`;
  }
...
function signCcsArtifact(artifact: CcsPyPiArtifact): CcsPyPiArtifact {
...
function ccsArtifact(traceId = '0011223344556677'): CcsPyPiArtifact {
  const fixture = JSON.parse(readFileSync(
    resolve(ROOT, 'interop/ccs-aeb/fixtures/ccs-verifier-pypi-1.1.0.json'),
    'utf8',
  )) as CcsPyPiArtifact;
...
const PUBLISHED_OASNT_ROOT: OasntTrustRoot = Object.freeze({
...
function oasntThumbprint(jwk: { kty: string; crv: string; x: string; y: string }): string {
...
export function assertContractLock(): {
  ...
  lock_digest: string;
  files: { path: string; expected: string; actual: string; matched: boolean }[];
} {
  const lock = JSON.parse(readFileSync(LOCK_PATH, 'utf8')) as {
    contract_version: string;
    verify_package_version: string;
...
function buildComposition() {
  const ccsConfig: CcsAebAdapterConfig = {
  ...
  const ccsRoot: CcsAebHmacTrustRoot = {
  ...
  const oasntConfig: OasntAdapterConfig = {
  ...
  const publishedOasntConfig: OasntAdapterConfig = {
    ...oasntConfig,
  ...
  const ccsProfile = profile(
    'ccs-native-action',
    CCS_CAID_MAPPER_ID,
    CCS_CAID_MAPPING_VERSION,
    createCcsNativeActionDefinition(ACTION_TYPE),
    [
  ...
  const oasntProfile = profile(
    'oasnt-native-action',
    OASNT_CAID_MAPPER_ID,
    OASNT_CAID_MAPPING_VERSION,
    createOasntActionDefinition(ACTION_TYPE, false),
    ['token.int', 'token.cnf.jkt', 'token.jti', 'token.iat', 'token.exp', 'token.rqf'],
  );
  const ccsInput = {
    artifact: ccs,
  ...
  const oasntPin: AebPinnedAdapter = {
  ...
  const entries = {
    'mapping:ccs-native-action': registryEntry(
      'mapping:ccs-native-action', 'mapping-profile', { profile_digest: ccsProfile.profile_digest },
    ),
    'mapping:oasnt-native-action': registryEntry(
      'mapping:oasnt-native-action', 'mapping-profile', { profile_digest: oasntProfile.profile_digest },
    ),
  ...
  const computed = computeCaid(ACTION, {
    suite: 'jcs-sha256',
    definitions: (oasntProfile.definition as Record<string, any>).definitions,
  });
  if (!('caid' in computed) || typeof computed.caid !== 'string') throw new Error('shared CAID failed');
  ...
  const makeEvaluation = (operationId: string) => evaluateAebEvidence({
    config,
    adapters,
...
export function runSuite(input: RunnerInput) {
  const runner = {
  ...
  const sourceProtocolChecks: Check[] = [
    check('CCS-PINNED-HMAC-ACCEPT', 'CCS', 'Pinned CCS 1.1.0 distribution/runtime 0.4.1 HMAC artifact verifies as machine-policy evidence.',
      result.ccsNative.native_verification === 'VERIFIED' && result.ccsNative.acceptance === 'ACCEPTED',
      { verification: result.ccsNative.native_verification, acceptance: result.ccsNative.acceptance }),
    check('CCS-MUTATED-PARAMS-REFUSE', 'CCS', 'Mutation after receipt creation is refused by the pinned CCS adapter.',
      result.tamperedCcsNative.acceptance === 'REJECTED',
      { acceptance: result.tamperedCcsNative.acceptance, reasons: result.tamperedCcsNative.reasons }),
    ...
    check('OASNT-ACTION-SUBSTITUTION-REFUSE', 'OASNT', 'Changing the expected action makes the published token fail closed.',
      result.changedOasntNative.acceptance === 'REJECTED',
      { acceptance: result.changedOasntNative.acceptance, reasons: result.changedOasntNative.reasons }),
    check('OASNT-NATIVE-ACTION-MAP', 'OASNT', 'OASNT maps the signed native action to the pinned AEB profile.',
      result.oasntMapping.mapping === 'MATCH',
      { mapping: result.oasntMapping.mapping, caid: result.oasntMapping.caid }),
  ];
  const aebCompositionChecks: Check[] = [
    check('AEB-CONTRACT-LOCK', 'AEB', 'The compatibility lock matches AEB-ADAPTER-v1 contract bytes.',
      lock.valid, { lock_digest: lock.lock_digest, files: lock.files }),
    check('AEB-COMPOSE-SAME-ACTION', 'AEB', 'Both native legs independently map to one CAID and action digest.',
      result.ccsMapping.mapping === 'MATCH'
        && result.oasntMapping.mapping === 'MATCH'
    ...
    check('AEB-COMPOSE-ROLES-PRESERVED', 'AEB', 'Machine policy and human authorization remain separate required roles.',
      result.first.valid && result.first.record.legs.map((entry) => entry.evidence_role).sort().join(',')
        === 'human-authorization,machine-policy-decision',
      { roles: result.first.record.legs.map((entry) => entry.evidence_role).sort() }),
    check('AEB-COMPOSE-ADMISSION', 'AEB', 'A satisfied signed evaluation authorizes one reserved provider admission.',
      result.firstVerification.execution_authorizing && result.firstAdmission.state === 'AUTHORIZED',
      { verdict: result.first.record.verdict, admission: result.firstAdmission.state }),
    check('AEB-COMPOSE-APPROVE-A-EXECUTE-B', 'AEB', 'A signed evaluation for action A cannot authorize changed action B.',
      !result.substitutionVerification.execution_authorizing,
      { execution_authorizing: result.substitutionVerification.execution_authorizing, reasons: result.substitutionVerification.reasons }),
    check('AEB-COMPOSE-INDETERMINATE', 'AEB', 'A lost provider outcome remains reconciliation-required rather than guessed.',
      result.firstOutcome.state === 'RECONCILIATION_REQUIRED',
      { state: result.firstOutcome.state }),
    check('AEB-COMPOSE-REPLAY-AFTER-INDETERMINATE', 'AEB', 'A second operation cannot re-admit the same native authority after an indeterminate attempt.',
      result.secondVerification.execution_authorizing
        && result.replayAdmission.state === 'REFUSED'
        && result.replayAdmission.reason === 'consumption_conflict',
      { state: result.replayAdmission.state, reason: result.replayAdmission.reason }),
  ];
