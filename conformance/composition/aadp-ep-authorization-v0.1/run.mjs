...
import { deriveAadpEpAuthorizationArtifact, matchAadpAuthorizationArtifact, verifyAadpEpAuthorizationArtifact, } from '../../../packages/verify/aadp-authorization-artifact.js';
...
export const MAPPING_PROFILE = 'https://emiliaprotocol.ai/profiles/aadp-ep-payment-release-v1';
...
export function mapAadpPaymentRelease(action) {
    return {
...
function profileInput({ request = BASE_REQUEST, bundle = FIXTURE.bundle, mappingProfile = MAPPING_PROFILE, mapAction = mapAadpPaymentRelease, options = bundleOptions(), } = {}) {
    return {
        bundle,
...
class AadpDraftLifecycle {
    ...
    propose(request, hook) {
        ...
        if (parsed.verdict !== 'MATCH')
            return { ok: false, decision: 'refused', reason: parsed.reason };
    ...
    decide({ request, approvalRef, presentedHook, verifiedHook, localPolicy = 'PERMIT', killSwitch = false, }) {
...
export function runComposition() {
    ...
    checks.push(check('AADP-EP-08', 'presenter-selected mapping-profile substitution is refused', profileResult, (value) => value.decision === 'refused' && value.reason === 'authorization_artifact_mismatch'));
    ...
    checks.push(check('AADP-EP-10', 'a valid EP artifact cannot overrule an AADP kill switch', frozenResult, (value) => value.decision === 'refused' && value.reason === 'kill_switch_active'));
