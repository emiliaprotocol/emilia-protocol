...
const VERIFY_PACKAGE = JSON.parse(readFileSync(
  new URL('../../../packages/verify/package.json', import.meta.url),
  'utf8',
));
...
const VERIFIER = Object.freeze({
  ...
  implementation: Object.freeze({
    ...
    digest: `sha256:${SOURCE_LOCK.emilia.runtime_files.find(
      (entry: any) => entry.path === 'packages/verify/src/authorization-bundle.ts',
    ).sha256}`,
  }),
});
...
function validWireRequest(value: unknown): value is Obj {
  ...
  if (!request
      ...
      || !validInstant(request.created_at)) return false;
  if (Object.hasOwn(request, 'approval_ref')
      && (typeof request.approval_ref !== 'string' || request.approval_ref.length === 0)) return false;
  return true;
}
...
function responseBase(request: any, verdict: string, reason: string, evidenceId: string): any {
  return {
    protocol: 'aadp/0.1',
...
function unreachableOutcome(reason: string, epInputObserved = true): any {
  return {
    ok: false,
    pdp_reachable: false,
    wire_response: null,
...
function malformedOutcome(value: unknown, evidenceId: string): any {
  ...
  if (typeof requestId === 'string' && UUID_RE.test(requestId)) response.request_id = requestId;
...
class AadpDraftLifecycle {
  ...
  propose(request: any, hook: unknown): any {
    ...
    if (parsed.verdict !== 'MATCH') {
      return wireOutcome(
        responseBase(request, 'deny', 'x-emilia.authorization_artifact_malformed', this.evidenceId()),
        false,
        true,
      );
    }
  ...
  decide({
    ...
    if (killSwitch === true) {
      ...
      return wireOutcome(
        responseBase(request, 'deny', 'kill_switch', this.evidenceId()),
        false,
        false,
      );
    }
    ...
    if (localPolicy !== 'PERMIT') {
      return wireOutcome(
        responseBase(request, 'deny', 'default_deny', this.evidenceId()),
        false,
        false,
      );
    }

    let profileVerdict: unknown;
    let profileReasons: unknown;
    let expectedArtifact: unknown;
    try {
      const verified = dataRecord(verifiedHook);
      if (!verified) return unreachableOutcome('native_authorization_artifact_unavailable');
    ...
    if (profileVerdict !== 'VERIFIED') {
      return wireOutcome(
        responseBase(request, 'deny', 'x-emilia.authorization_artifact_refused', this.evidenceId()),
        false,
        true,
      );
    }
    ...
    if (hookMatch.verdict !== 'MATCH') {
      return wireOutcome(
        responseBase(request, 'deny', 'x-emilia.authorization_artifact_mismatch', this.evidenceId()),
        false,
        true,
      );
    }
...
function observedProxy(value: unknown, onObserved: () => void): any {
...
export function runComposition(): any {
  ...
  checks.push(check('AADP-EP-16', 'mapping implementation and resolver substitution are refused', implementationResult,
    (value) => value.wire_verdict === 'deny'
      && value.reason === 'x-emilia.authorization_artifact_mismatch'));
  ...
  const allLockedHashes = [
    ...
    ...SOURCE_LOCK.onedoor.inspected_files.map((entry: any) => entry.sha256),
    ...SOURCE_LOCK.emilia.runtime_files.map((entry: any) => entry.sha256),
  ];
  ...
  checks.push(check('AADP-EP-22', 'the deterministic report binds every source-locked hash', {
    ...
    all_hashes_bound: allLockedHashes.every((hash: string) => lockedBytes.includes(hash)),
