...
test('the derived hook satisfies the checked-in closed JSON Schema', () => {
    const schema = JSON.parse(readFileSync(new URL('../../schemas/aadp-authorization-artifact.v1.schema.json', import.meta.url), 'utf8'));
...
test('the report digest covers the source lock and every declared hash', () => {
    ...
    const hashes = [
        ...
        ...sourceLock.onedoor.inspected_files.map((entry) => entry.sha256),
        ...sourceLock.emilia.runtime_files.map((entry) => entry.sha256),
    ];
    for (const hash of hashes)
        assert.ok(serialized.includes(hash), hash);
