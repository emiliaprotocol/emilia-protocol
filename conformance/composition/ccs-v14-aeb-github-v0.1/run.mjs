...
import { CCS_V14_AEB_CONFIG_VERSION, CCS_V14_AEB_TRUST_ROOT_VERSION, CCS_V14_CAID_MAPPER_ID, CCS_V14_CAID_MAPPING_VERSION, CCS_V14_SOURCE_LOCK, CCS_V14_VECTOR_COMMIT, CCS_V14_VECTOR_MANIFEST_SHA256, CCS_V14_VECTOR_REPOSITORY, createCcsV14AebActionDefinition, createCcsV14AebAdapter, } from '../../../packages/verify/aeb-ccs-v14-adapter.js';
...
function signReceipt(unsigned) {
    return {
        ...unsigned,
        signature: crypto.sign(null, Buffer.from(canonicalizeFiniteJson(unsigned), 'utf8'), CCS_PRIVATE).toString('base64'),
    };
}
...
function verifyUpstreamVector() {
    ...
    const signatureValid = crypto.verify(null, Buffer.from(canonicalizeFiniteJson(unsigned), 'utf8'), publicKey, Buffer.from(receipt.signature, 'base64'));
...
export async function buildReferenceReport() {
    ...
    cases.push(caseResult('CCS-V1.4-PUBLIC-VECTOR-PIN', 'source', upstream.raw_sha256 === '889855dc9fcebdb642bd7e0f369651015781b4c004227aef510feb1fb7cb4361'
        && upstream.signature_valid && upstream.args_digest_valid && upstream.response_hash_valid, 'byte-pinned public vector with valid Ed25519 and companion hashes', upstream));
    ...
    cases.push(caseResult('MISSING-EMILIA-AUTHORITY-REFUSED', 'boundary', noAuthorityResult.state === 'REFUSED'
        && noAuthorityResult.reason === 'local_authorization_denied'
        && noAuthorityHarness.providerCalls() === 0, 'CCS ALLOW alone cannot enter the provider', {
    ...
    cases.push(caseResult('INDETERMINATE-BLOCKS-BLIND-RETRY', 'boundary', first.state === 'INDETERMINATE' && first.retry_allowed === false
        && retry.state !== 'EXECUTED' && lostHarness.providerCalls() === 1, 'one provider entry, INDETERMINATE result, and no blind second entry', { first: first.state, retry: retry.state, provider_calls: lostHarness.providerCalls() }));
    const base = {
