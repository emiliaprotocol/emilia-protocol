...
import { CCS_V13_AEB_ADAPTER_ID, CCS_V13_AEB_ADAPTER_VERSION, CCS_V13_AEB_CONFIG_VERSION, CCS_V13_AEB_TRUST_ROOT_VERSION, CCS_V13_CAID_MAPPER_ID, CCS_V13_CAID_MAPPING_VERSION, CCS_V13_DRAFT_SHA256, CCS_V13_DRAFT_URL, CCS_V13_REFERENCE_CODEBERG_COMMIT, CCS_V13_REFERENCE_PYPI_VERSION, CCS_V13_SOURCE_LOCK, createCcsV13AebActionDefinition, createCcsV13AebAdapter, } from '../../../packages/verify/aeb-ccs-adapter.js';
...
export function runSuite() {
    ...
    const fullDigestMismatchUnsigned = { ...f.allowReceipt };
    ...
    const fullDigestMismatch = signReceipt(fullDigestMismatchUnsigned);
    ...
    const checks = [
        check('CCS-V13-SOURCE-PIN', 'CCS', 'The official CCS-05 bytes and current public implementation coordinates are pinned.', CCS_V13_DRAFT_SHA256 === 'c91f0fa31b1b9e5e2dfe79b99f3b554075d3a44d5309406e748b728f86767cb9', { source: CCS_V13_SOURCE_LOCK, draft_sha256: CCS_V13_DRAFT_SHA256 }),
        check('CCS-V13-SHAPE-SEPARATION', 'CCS', 'The v1.3 draft shape is not relabeled as the published package v1.1 receipt.', CCS_V13_REFERENCE_PYPI_VERSION === '1.1.20' && !('receipt_version' in f.allowReceipt), { package_version: CCS_V13_REFERENCE_PYPI_VERSION, receipt_fields: Object.keys(f.allowReceipt).length }),
        check('LIVE-SUM-EXECUTED', 'LIVE-RUN', 'The protected local operation executed and returned the response bound by the receipt.', f.response.result === 42 && f.allowReceipt.response_hash === prefixedJsonDigest(f.response), { response: f.response, response_hash: f.allowReceipt.response_hash }),
        check('CCS-V13-ED25519-ACCEPT', 'CCS', 'The 22-field receipt verifies under the relying-party-pinned Ed25519 issuer.', native.native_verification === 'VERIFIED' && native.acceptance === 'ACCEPTED', native),
        check('AEB-V13-EXACT-ACTION-MAP', 'AEB', 'The full receipt action digest maps to the executor-owned exact action and CAID.', mapping.mapping === 'MATCH', mapping),
        check('CCS-V13-SIGNATURE-TAMPER', 'CCS', 'Changing a signed response binding invalidates the receipt.', signatureTamperResult.native_verification === 'FAILED', signatureTamperResult),
        check('CCS-V13-UNTRUSTED-KEY', 'CCS', 'A relying-party root replacement cannot validate the receipt.', wrongKeyResult.native_verification === 'FAILED', wrongKeyResult),
        check('CCS-V13-AUDIENCE', 'CCS', 'A valid receipt is rejected for a different relying-party audience.', audienceResult.native_verification === 'VERIFIED' && audienceResult.acceptance === 'REJECTED', audienceResult),
        check('CCS-V13-EXPIRY', 'CCS', 'A signed receipt outside its freshness window is rejected.', expired.native_verification === 'VERIFIED' && expired.acceptance === 'REJECTED', expired),
        check('AEB-V13-PARAM-SUBSTITUTION', 'AEB', 'Changing executor-owned arguments produces an exact-action mismatch.', substituted.mapping === 'MISMATCH', substituted),
        check('AEB-V13-FULL-DIGEST', 'AEB', 'A signed action preserving the 16-hex prefix but changing the full digest is refused.', fullDigestMismatchNative.native_verification === 'VERIFIED' && fullDigestMismatchMapping.mapping === 'MISMATCH', fullDigestMismatchMapping),
        check('AEB-V13-REPLAY', 'AEB', 'Consumed status prevents a second acceptance of the receipt replay unit.', consumed.acceptance === 'REJECTED' && consumed.reasons.includes('evidence_consumed'), consumed),
        check('AEB-V13-STATUS-UNKNOWN', 'AEB', 'Unavailable authenticated status remains INDETERMINATE.', unavailable.acceptance === 'INDETERMINATE', unavailable),
        check('AEB-V13-DENY-NONAUTHORIZING', 'AEB', 'A native deny never becomes a mappable action admission.', denyNative.acceptance === 'REJECTED' && denyMapping.mapping === 'INDETERMINATE', { denyNative, denyMapping }),
    ];
    ...
    const report = {
        ...
        known_limits: [
            'This is an independent implementation of the CCS-05 v1.3 receipt profile, not a claim that ccs-verifier 1.1.20 emits the same shape.',
...
function main() {
    ...
    if (process.argv.includes('--check')) {
        if (readFileSync(REPORT_PATH, 'utf8') !== renderedReport)
            throw new Error('CCS-05 v1.3 report is stale');
        if (readFileSync(SAMPLE_PATH, 'utf8') !== renderedSample)
            throw new Error('CCS-05 v1.3 sample receipt set is stale');
    }
