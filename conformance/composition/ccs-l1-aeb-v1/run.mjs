...
const EMILIA_DERIVED_FIXTURE_PATH = resolve(ROOT, 'interop/ccs-aeb/fixtures/ccs-verifier-pypi-1.1.19-emilia-derived-reference-signed-001.json');
const UPSTREAM_STALE_FIXTURE_PATH = resolve(ROOT, 'interop/ccs-aeb/fixtures/ccs-verifier-pypi-1.1.19-upstream-stale-1.1.14-reference-signed-001.json');
...
function buildFixture() {
    const vector = JSON.parse(readFileSync(EMILIA_DERIVED_FIXTURE_PATH, 'utf8'));
    const upstreamStaleVector = JSON.parse(readFileSync(UPSTREAM_STALE_FIXTURE_PATH, 'utf8'));
    const config = {
...
export function runSuite() {
    ...
    const checks = [
        check('CCS-1.1.19-SOURCE-PIN', 'CCS', 'The exact upstream tag, commit, PyPI 1.1.19 artifacts, upstream-stale 1.1.14 vector, and EMILIA-derived 1.1.19 vector are pinned without relabeling provenance.', f.upstreamStaleVector.package_version === '1.1.14'
        ...
        check('CCS-1.1.19-ED25519-ACCEPT', 'CCS', 'The reference L1 signature verifies and its issuer key is accepted only under the relying-party pin.', native.native_verification === 'VERIFIED' && native.acceptance === 'ACCEPTED', { verification: native.native_verification, acceptance: native.acceptance }),
        check('CCS-1.1.19-SIGNATURE-TAMPER', 'CCS', 'A signature mutation is refused before semantic mapping.', invalidSignature.native_verification === 'FAILED', invalidSignature),
        check('CCS-1.1.19-UNTRUSTED-ISSUER', 'CCS', 'A self-consistent receipt not matching the relying-party key pin is rejected.', untrusted.native_verification === 'VERIFIED' && untrusted.acceptance === 'REJECTED', untrusted),
        check('CCS-1.1.19-EXPIRY', 'CCS', 'A correctly signed receipt past expires_at is rejected.', expired.native_verification === 'VERIFIED' && expired.acceptance === 'REJECTED', expired),
