...
import {
  ...
  CCS_L1_UPSTREAM_TAG,
  CCS_L1_UPSTREAM_TAG_GPG_SIGNED,
  CCS_L1_UPSTREAM_TAG_KIND,
...
const UPSTREAM_FIXTURE_PATH = resolve(
  ROOT,
  'interop/ccs-aeb/fixtures/ccs-verifier-pypi-1.1.20-upstream-reference-signed-001.json',
);
...
function buildFixture() {
  const vector = JSON.parse(readFileSync(UPSTREAM_FIXTURE_PATH, 'utf8')) as UpstreamVector;
...
export function runSuite() {
  ...
  const checks: Check[] = [
    check('CCS-1.1.20-SOURCE-PIN', 'CCS', 'The exact upstream vector, annotated tag object, commit, and PyPI 1.1.20 artifacts are pinned without treating the unsigned tag as publisher authentication.',
    ...
    check('CCS-1.1.20-ED25519-ACCEPT', 'CCS', 'The reference L1 signature verifies and its issuer key is accepted only under the relying-party pin.',
    ...
    check('CCS-1.1.20-SIGNATURE-TAMPER', 'CCS', 'A signature mutation is refused before semantic mapping.',
      invalidSignature.native_verification === 'FAILED', invalidSignature),
    check('CCS-1.1.20-UNTRUSTED-ISSUER', 'CCS', 'A self-consistent receipt not matching the relying-party key pin is rejected.',
      untrusted.native_verification === 'VERIFIED' && untrusted.acceptance === 'REJECTED', untrusted),
    check('CCS-1.1.20-EXPIRY', 'CCS', 'A correctly signed receipt past expires_at is rejected.',
