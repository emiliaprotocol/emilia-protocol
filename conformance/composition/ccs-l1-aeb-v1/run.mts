...
/** Source-locked CCS 1.1.19 L1 receipt to AEB exact-action mapping runner. */
...
import {
  ...
  CCS_L1_EMILIA_DERIVED_REFERENCE_VECTOR_SHA256,
  CCS_L1_UPSTREAM_STALE_REFERENCE_VECTOR_SHA256,
  CCS_L1_UPSTREAM_COMMIT_SHA,
  CCS_L1_UPSTREAM_REPOSITORY,
  CCS_L1_UPSTREAM_TAG,
  CCS_L1_UPSTREAM_TAG_OBJECT_SHA,
...
const EMILIA_DERIVED_FIXTURE_PATH = resolve(
  ROOT,
  'interop/ccs-aeb/fixtures/ccs-verifier-pypi-1.1.19-emilia-derived-reference-signed-001.json',
);
const UPSTREAM_STALE_FIXTURE_PATH = resolve(
  ROOT,
  'interop/ccs-aeb/fixtures/ccs-verifier-pypi-1.1.19-upstream-stale-1.1.14-reference-signed-001.json',
);
...
type EmiliaDerivedVector = {
  ...
  source: {
    ...
    commit_sha: string;
    pypi_sdist_sha256: string;
    pypi_wheel_sha256: string;
    upstream_stale_vector_sha256: string;
    generator: string;
  };
};

type UpstreamStaleVector = {
  package_version: string;
  receipt: CcsL1Receipt;
};
...
function buildFixture() {
  const vector = JSON.parse(readFileSync(EMILIA_DERIVED_FIXTURE_PATH, 'utf8')) as EmiliaDerivedVector;
  const upstreamStaleVector = JSON.parse(
    readFileSync(UPSTREAM_STALE_FIXTURE_PATH, 'utf8'),
  ) as UpstreamStaleVector;
...
export function runSuite() {
  ...
  const checks: Check[] = [
    check('CCS-1.1.19-SOURCE-PIN', 'CCS', 'The exact upstream tag, commit, PyPI 1.1.19 artifacts, upstream-stale 1.1.14 vector, and EMILIA-derived 1.1.19 vector are pinned without relabeling provenance.',
      f.upstreamStaleVector.package_version === '1.1.14'
        && f.upstreamStaleVector.receipt.rule_version === '1.1.14'
        && fileSha256(UPSTREAM_STALE_FIXTURE_PATH) === CCS_L1_UPSTREAM_STALE_REFERENCE_VECTOR_SHA256
        && f.vector.provenance === 'EMILIA-derived'
        && f.vector.package_version === CCS_L1_PYPI_DISTRIBUTION_VERSION
        && f.vector.receipt.rule_version === CCS_L1_PYPI_DISTRIBUTION_VERSION
        && f.vector.source.repository === CCS_L1_UPSTREAM_REPOSITORY
        && f.vector.source.tag === CCS_L1_UPSTREAM_TAG
        && f.vector.source.tag_object_sha === CCS_L1_UPSTREAM_TAG_OBJECT_SHA
        && f.vector.source.commit_sha === CCS_L1_UPSTREAM_COMMIT_SHA
        && f.vector.source.pypi_sdist_sha256 === CCS_L1_PYPI_SDIST_SHA256
        && f.vector.source.pypi_wheel_sha256 === CCS_L1_PYPI_WHEEL_SHA256
        && f.vector.source.upstream_stale_vector_sha256 === CCS_L1_UPSTREAM_STALE_REFERENCE_VECTOR_SHA256
        && f.vector.source.generator === 'interop/ccs-aeb/generate_l1_119_emilia_derived_fixture.mjs'
        && fileSha256(EMILIA_DERIVED_FIXTURE_PATH) === CCS_L1_EMILIA_DERIVED_REFERENCE_VECTOR_SHA256,
      {
        distribution: f.vector.package_version,
        derived_provenance: f.vector.provenance,
        ...
        upstream_stale_package_version: f.upstreamStaleVector.package_version,
        upstream_stale_rule_version: f.upstreamStaleVector.receipt.rule_version,
        upstream_stale_vector_sha256: fileSha256(UPSTREAM_STALE_FIXTURE_PATH),
        emilia_derived_vector_sha256: fileSha256(EMILIA_DERIVED_FIXTURE_PATH),
      }),
    check('CCS-1.1.19-ED25519-ACCEPT', 'CCS', 'The reference L1 signature verifies and its issuer key is accepted only under the relying-party pin.',
      native.native_verification === 'VERIFIED' && native.acceptance === 'ACCEPTED',
      { verification: native.native_verification, acceptance: native.acceptance }),
    check('CCS-1.1.19-SIGNATURE-TAMPER', 'CCS', 'A signature mutation is refused before semantic mapping.',
      invalidSignature.native_verification === 'FAILED', invalidSignature),
    check('CCS-1.1.19-UNTRUSTED-ISSUER', 'CCS', 'A self-consistent receipt not matching the relying-party key pin is rejected.',
      untrusted.native_verification === 'VERIFIED' && untrusted.acceptance === 'REJECTED', untrusted),
    check('CCS-1.1.19-EXPIRY', 'CCS', 'A correctly signed receipt past expires_at is rejected.',
      expired.native_verification === 'VERIFIED' && expired.acceptance === 'REJECTED', expired),
    check('AEB-EXACT-ACTION-MAP', 'AEB', 'The signed CCS action, tool, and full args_digest map to one executor-constructed CAID.',
      mapping.mapping === 'MATCH', mapping),
    check('AEB-ACTION-SUBSTITUTION', 'AEB', 'Changing the executor action arguments produces MISMATCH.',
      substitutedMapping.mapping === 'MISMATCH', substitutedMapping),
    check('AEB-STATUS-UNAVAILABLE', 'AEB', 'Unavailable authenticated status remains INDETERMINATE.',
  ...
  const report: Record<string, unknown> = {
    ...
    pins: {
      ...
      ccs_upstream_stale_reference_vector_sha256: CCS_L1_UPSTREAM_STALE_REFERENCE_VECTOR_SHA256,
      ccs_emilia_derived_reference_vector_sha256: CCS_L1_EMILIA_DERIVED_REFERENCE_VECTOR_SHA256,
    ...
    known_limits: [
      ...
      'The 1.1.19 source distribution bundles a stale 1.1.14 reference vector. Its exact bytes remain separately pinned; the adapter input is an explicitly labeled EMILIA-derived 1.1.19 fixture regenerated from the public seed, not an upstream vector.',
...
function main() {
  ...
  if (process.argv.includes('--write')) writeFileSync(REPORT_PATH, rendered);
