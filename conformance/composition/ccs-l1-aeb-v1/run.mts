...
/** Source-locked CCS 1.1.14 L1 receipt to AEB exact-action mapping runner. */
...
const FIXTURE_PATH = resolve(
  ROOT,
  'interop/ccs-aeb/fixtures/ccs-verifier-pypi-1.1.14-reference-signed-001.json',
);
...
function buildFixture() {
  const vector = JSON.parse(readFileSync(FIXTURE_PATH, 'utf8')) as ReferenceVector;
...
export function runSuite() {
  ...
  const checks: Check[] = [
    check('CCS-1.1.14-SOURCE-PIN', 'CCS', 'The exact published PyPI 1.1.14 release and reference vector are pinned.',
      f.vector.package_version === CCS_L1_PYPI_DISTRIBUTION_VERSION
        && fileSha256(FIXTURE_PATH) === CCS_L1_REFERENCE_VECTOR_SHA256,
      {
        distribution: f.vector.package_version,
        source_lock: CCS_L1_PYPI_SOURCE_LOCK,
        vector_sha256: fileSha256(FIXTURE_PATH),
      }),
    check('CCS-1.1.14-ED25519-ACCEPT', 'CCS', 'The reference L1 signature verifies and its issuer key is accepted only under the relying-party pin.',
    ...
    check('CCS-1.1.14-SIGNATURE-TAMPER', 'CCS', 'A signature mutation is refused before semantic mapping.',
      invalidSignature.native_verification === 'FAILED', invalidSignature),
    check('CCS-1.1.14-UNTRUSTED-ISSUER', 'CCS', 'A self-consistent receipt not matching the relying-party key pin is rejected.',
      untrusted.native_verification === 'VERIFIED' && untrusted.acceptance === 'REJECTED', untrusted),
    check('CCS-1.1.14-EXPIRY', 'CCS', 'A correctly signed receipt past expires_at is rejected.',
