...
import { InMemoryAebConsumptionStore, digestAeb, } from '../../../packages/verify/aeb-adapter-contract.js';
import { WAG_AEB_ADAPTER_VERSION, WAG_AEB_CONFIG_VERSION, WAG_CAID_MAPPER_ID, WAG_CAID_MAPPING_VERSION, WAG_DRAFT_REVISION, WAG_DRAFT_SOURCE_COMMIT, WAG_DRAFT_SOURCE_SHA256, WAG_DRAFT_TXT_SHA256, WAG_GRANT_TYPE, WAG_TRUST_ROOT_VERSION, createWagActionDefinition, createWagAebAdapter, } from '../../../packages/verify/aeb-wag-adapter.js';
...
function check(id, category, description, passed, observed) {
...
function mintGrant(overrides = {}, headerOverrides = {}) {
...
function input(value, action = expectedAction(value), status = STATUS) {
    return {
        artifact: value,
...
function buildResults() {
    ...
    function native(changed, action = expectedAction(validArtifact), status = STATUS) {
...
export function runSuite(inputValue) {
    const runner = {
    ...
    const checks = [
        check('WAG-00-SOURCE-PIN', 'source', 'The adapter pins the exact reviewed WAG -00 source and published bytes.', sourceLockCheck(), { revision: WAG_DRAFT_REVISION, source_commit: WAG_DRAFT_SOURCE_COMMIT }),
        check('WAG-VALID-TOKEN-ISSUANCE', 'positive', 'A valid per-tenancy WAG grant maps to one exact token-issuance CAID.', result.validNative.native_verification === 'VERIFIED'
            && result.validNative.acceptance === 'ACCEPTED'
            && result.validMapping.mapping === 'MATCH', { acceptance: result.validNative.acceptance, mapping: result.validMapping.mapping, caid: result.validMapping.caid }),
        check('WAG-ISSUER-SUBSTITUTION', 'hostile', 'A different issuer cannot use the pinned tenancy key.', result.issuer.acceptance === 'REJECTED' && result.issuer.reasons.includes('wag:issuer_mismatch'), { acceptance: result.issuer.acceptance, reasons: result.issuer.reasons }),
        check('WAG-TENANT-KEY-SUBSTITUTION', 'hostile', 'A presenter cannot substitute a root from another tenancy.', result.tenantKey.acceptance === 'REJECTED'
            && result.tenantKey.reasons.includes('wag:constructor_pin_mismatch'), { acceptance: result.tenantKey.acceptance, reasons: result.tenantKey.reasons }),
        check('WAG-UNSEEN-SUBJECT-ACCEPT', 'positive', 'A newly seen signed subject under the allowlisted issuer is accepted without per-agent registration.', result.childNative.acceptance === 'ACCEPTED'
            && result.childMapping.mapping === 'MATCH'
            && result.childMapping.caid !== result.validMapping.caid, { acceptance: result.childNative.acceptance, mapping: result.childMapping.mapping, caid: result.childMapping.caid }),
        check('WAG-SUBJECT-SUBSTITUTION', 'hostile', 'A child or sibling agent identifier cannot inherit the pinned subject implicitly.', result.subject.acceptance === 'REJECTED'
            && result.subject.reasons.includes('wag:token_request_projection_mismatch'), { acceptance: result.subject.acceptance, reasons: result.subject.reasons }),
        check('WAG-AUDIENCE-SUBSTITUTION', 'hostile', 'The grant must address the pinned authorization server.', result.audience.acceptance === 'REJECTED' && result.audience.reasons.includes('wag:audience_mismatch'), { acceptance: result.audience.acceptance, reasons: result.audience.reasons }),
        check('WAG-RESOURCE-SUBSTITUTION', 'hostile', 'Changing the observed RFC 8707 target resource refuses the token request.', result.resource.acceptance === 'REJECTED' && result.resource.reasons.includes('wag:resource_mismatch'), { acceptance: result.resource.acceptance, reasons: result.resource.reasons }),
        check('WAG-PROPERTY-SUBSTITUTION', 'hostile', 'A material signed Property mismatch refuses rather than granting local permission.', result.properties.acceptance === 'REJECTED'
            && result.properties.reasons.includes('wag:token_request_projection_mismatch'), { acceptance: result.properties.acceptance, reasons: result.properties.reasons }),
        check('WAG-EXPIRED-GRANT', 'hostile', 'An expired grant is rejected under the pinned clock policy.', result.expired.acceptance === 'REJECTED' && result.expired.reasons.includes('wag:grant_expired'), { acceptance: result.expired.acceptance, reasons: result.expired.reasons }),
        check('WAG-STATUS-UNAVAILABLE', 'hostile', 'Unavailable current status yields INDETERMINATE, never a guessed acceptance.', result.unavailable.acceptance === 'INDETERMINATE', { acceptance: result.unavailable.acceptance, reasons: result.unavailable.reasons }),
        check('WAG-REPLAY-IDENTITY', 'replay', 'Wrapper changes do not change the native replay identity.', result.validNative.replay_unit === result.wrapperReplay.replay_unit, { replay_unit: result.validNative.replay_unit }),
        check('WAG-SECOND-TOKEN-ISSUANCE', 'replay', 'The AEB consumption store refuses a second reservation of the same WAG replay unit.', result.firstAdmission === true && result.secondAdmission === false, { first_reserved: result.firstAdmission, second_reserved: result.secondAdmission }),
        check('WAG-DOWNSTREAM-ACTION-NON-SUBSTITUTION', 'boundary', 'WAG alone is indeterminate for a downstream tool action.', result.downstream.acceptance === 'INDETERMINATE'
            && result.downstream.reasons.includes('wag:does_not_bind_downstream_action'), { acceptance: result.downstream.acceptance, reasons: result.downstream.reasons }),
        check('WAG-NOT-HUMAN-APPROVAL', 'boundary', 'The evidence subject and role remain workload-only.', result.validNative.subject.kind === 'workload'
            && result.validNative.evidence_role === 'workload-authorization-grant', { subject_kind: result.validNative.subject.kind, evidence_role: result.validNative.evidence_role }),
    ];
    const catalog = JSON.parse(readFileSync(VECTOR_PATH, 'utf8'));
    const catalogIds = catalog.cases.map((entry) => entry.id).sort();
    const checkIds = checks.map((entry) => entry.id).sort();
    if (catalogIds.join('\0') !== checkIds.join('\0'))
        throw new Error('vector catalog and executable checks disagree');
    const reportBody = {
