...
import { canonicalReportBytes, runSuite, signReport, verifyReportSignature, } from './run.mjs';
...
test('the WAG profile pins the exact -00 source and all executable cases pass', () => {
    ...
    assert.equal(value.checks.every((entry) => entry.passed), true);
});
test('hostile cases cover every load-bearing WAG-to-AEB seam', () => {
    const ids = new Set(report().checks.map((entry) => entry.id));
    for (const id of [
...
test('the report preserves the workload-only and downstream non-substitution boundary', () => {
    ...
    const boundary = value.checks.find((entry) => entry.id === 'WAG-DOWNSTREAM-ACTION-NON-SUBSTITUTION');
