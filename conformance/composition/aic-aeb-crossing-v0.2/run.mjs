...
import { AIC_ADMISSION_DOMAIN_VERSION, AIC_CROSSING_MAX_STATUS_AGE_SECONDS, AIC_JWT_JKT_BOUND_CROSSING_MAPPING_PROFILE, AIC_JWT_SVID_SOURCE_VERIFICATION_PROFILE, AIC_X509_CREDENTIAL_BUNDLE_DIGEST_VERSION, AIC_X509_SPKI_BOUND_CROSSING_MAPPING_PROFILE, issueAicBoundCrossingRecord, mapAicJwtJktBoundCrossingAuthority, mapAicX509SpkiBoundCrossingAuthority, projectAicJwtToStrictJwtSvid, } from "../../../packages/verify/aeb-aic-crossing-adapter.js";
import { AEB_CROSSING_RECORD_REQUIRED_ALGORITHMS, AEB_CROSSING_RECORD_VERSION, verifyAebCrossingRecord, } from "../../../packages/verify/aeb-crossing-record.js";
...
const SOURCE_LOCK_DIGEST = `sha256:${crypto.hash("sha256", canonicalizeAeb(SOURCE_LOCK), "hex")}`;
...
function pinnedSourceDigest(repository, path, revision) {
    const repositories = SOURCE_LOCK.varwof.repositories.filter((entry) => entry.repository === repository
        && (revision === undefined || entry.revision === revision));
    assert.equal(repositories.length, 1, `ambiguous pinned repository ${repository}${revision ? `@${revision}` : ""}`);
    const file = repositories[0].inspected_files.find((entry) => entry.path === path);
...
const JWT_VERIFIER = Object.freeze({
    ...
    implementation_digest: digestAebTyped({
        validate_go_sha256: pinnedSourceDigest("https://github.com/varwof/types", "aicjwt/validate.go", "76f725ffc375ae7fda1f0255ea3e12a0074f6c4c"),
        keyhash_go_sha256: pinnedSourceDigest("https://github.com/varwof/types", "aicjwt/keyhash.go", "76f725ffc375ae7fda1f0255ea3e12a0074f6c4c"),
        gateway_bearer_sha256: pinnedSourceDigest("https://github.com/varwof/gateway-core", "jwt.go"),
        gateway_go_mod_sha256: pinnedSourceDigest("https://github.com/varwof/gateway-core", "go.mod"),
...
const X509_VERIFIER = Object.freeze({
    ...
    version: "source-lock-v0.2",
    implementation_digest: digestAebTyped({
        credential_bundle_sha256: pinnedSourceDigest("https://github.com/varwof/gateway-core", "credential_bundle.go"),
        gateway_go_mod_sha256: pinnedSourceDigest("https://github.com/varwof/gateway-core", "go.mod"),
        complete_inspection_set_digest: SOURCE_LOCK_DIGEST,
...
const REQUEST_BINDING = Object.freeze({
    ...
    projected_admission_domain_digest: digestAebTyped(BOUNDARY, AIC_ADMISSION_DOMAIN_VERSION),
});
const JWT_POLICY = Object.freeze({
    ...
    action_projection_profile_digest: REQUEST_BINDING.action_projection_profile_digest,
...
const JWT_PROJECTION_SOURCE_POLICY = Object.freeze({
    source_verification_profile_id: AIC_JWT_SVID_SOURCE_VERIFICATION_PROFILE,
    source_verification_profile_digest: MAPPING_PROFILE_DIGEST,
    trusted_issuer_trust_anchor_digests: [ISSUER_PIN],
    native_verifier: JWT_VERIFIER,
});
...
function crossingDraft(recordId) {
    return {
        record_id: recordId,
...
export async function buildReferenceReport() {
    ...
    const jkt = mapJkt();
    const x509 = mapX509();
    const jktRecord = await issue(jktInput(), RP_CONTEXT, "crossing:aic:jkt:0001");
    const jktCheck = await verify(jktRecord);
    cases.push(result("AIC-JWT-JKT-CROSSING", "positive", jkt.mapping_profile_id === AIC_JWT_JKT_BOUND_CROSSING_MAPPING_PROFILE
        && jktCheck.verified
    ...
    const x509Record = await issue(x509Input(), X509_RP_CONTEXT, "crossing:aic:x509:0001");
    const x509Check = await verify(x509Record);
    cases.push(result("AIC-X509-SPKI-CROSSING", "positive", x509.mapping_profile_id === AIC_X509_SPKI_BOUND_CROSSING_MAPPING_PROFILE
        && x509Check.verified
    ...
    cases.push(result("X509-DER-REPLAY-IDENTITY-STABLE", "mapping", x509.replay_unit === relabeledArtifact.replay_unit
        && x509.replay_unit === relabeledIssuer.replay_unit
        && x509.authority_instance_digest !== relabeledArtifact.authority_instance_digest
        && x509.authority_instance_digest !== relabeledIssuer.authority_instance_digest, "exact DER fixes replay identity while authenticated wrapper metadata remains instance-bound", {
        ...
        artifact_instance_changed: x509.authority_instance_digest !== relabeledArtifact.authority_instance_digest,
        issuer_instance_changed: x509.authority_instance_digest !== relabeledIssuer.authority_instance_digest,
    }));
    ...
    cases.push(result("SYNTHESIZED-X509-CARRIER-CONFUSION-REFUSED", "hostile", !synthesizedJwtAsX509.ok
        && synthesizedJwtAsX509.reason === "aic_carrier_provenance_unverifiable"
        && synthesizedJwtAsJwt.ok, "JWT-origin synthesized X.509 refuses the native X.509 path and remains JWT/JKT", {
        x509_path_ok: synthesizedJwtAsX509.ok,
        x509_path_reason: synthesizedJwtAsX509.ok ? null : synthesizedJwtAsX509.reason,
    ...
    const attackerAnchor = `sha256:${"a2".repeat(32)}`;
    ...
    cases.push(result("RP-POLICY-SELF-PIN-REFUSED", "hostile", !untrusted.ok && untrusted.reason === "aic_issuer_untrusted"
        && !selfPinned.ok && selfPinned.reason === "mapping_input_invalid", "native result cannot carry or replace relying-party trust pins", {
    ...
    cases.push(result("NATIVE-TYPE-CONFUSION", "hostile", !confused.ok && confused.reason === "aic_carrier_provenance_unverifiable", "aic_carrier_provenance_unverifiable", { ok: confused.ok, reason: confused.ok ? null : confused.reason }));
    ...
    const actionMismatch = mapAicJwtJktBoundCrossingAuthority(jktInput(), {
        ...RP_CONTEXT,
        action: {
            ...ACTION,
    ...
    cases.push(result("EXACT-ACTION-SUBSTITUTION-REFUSED", "hostile", !actionMismatch.ok
        && actionMismatch.reason === "aic_action_projection_mismatch", "aic_action_projection_mismatch", {
    ...
    cases.push(result("REQUEST-CAPABILITY-SUBSTITUTION-REFUSED", "hostile", !changedCapability.ok
        && changedCapability.reason === "aic_requested_capability_mismatch", "aic_requested_capability_mismatch", {
        ok: changedCapability.ok,
        reason: changedCapability.ok ? null : changedCapability.reason,
    }));
    const wrongDomain = mapAicJwtJktBoundCrossingAuthority(jktInput(), {
        ...RP_CONTEXT,
        admission_domain: {
            ...BOUNDARY,
    ...
    cases.push(result("RELYING-PARTY-DOMAIN-SUBSTITUTION-REFUSED", "hostile", !wrongDomain.ok
        && wrongDomain.reason === "aic_admission_domain_mismatch", "aic_admission_domain_mismatch", {
        ok: wrongDomain.ok,
    ...
    cases.push(result("JWT-AUDIENCE-SUBSTITUTION-REFUSED", "hostile", !wrongAudience.ok && wrongAudience.reason === "aic_audience_mismatch", "aic_audience_mismatch", {
        ok: wrongAudience.ok,
    ...
    cases.push(result("JWT-TEMPORAL-RELABELING-REFUSED", "hostile", !relabeledJwtValidity.ok
        && relabeledJwtValidity.reason === "aic_jwt_validity_mismatch", "signed compact-token iat/nbf/exp must exactly bind wrapper validity", {
    ...
    cases.push(result("STATUS-OBSERVATION-TIME-REFUSALS", "hostile", !stale.ok && stale.reason === "aic_status_observation_stale"
        && !future.ok && future.reason === "aic_status_observation_future", "stale and future observations both refuse", {
    ...
    const widenedProjection = projectAicJwtToStrictJwtSvid({
        ...
        source_verification_policy: JWT_PROJECTION_SOURCE_POLICY,
        ...widenedFreshness,
    });
    ...
    cases.push(result("NON-CURRENT-SOURCE-STATUS-REFUSED", "hostile", !revoked.ok && revoked.reason === "aic_status_not_current"
        && !unavailable.ok && unavailable.reason === "aic_status_not_current", "revoked and unavailable source status both refuse", {
    ...
    const issuerTimeDomainMismatch = await issueAicBoundCrossingRecord(jktInput(), RP_CONTEXT, {
        ...crossingDraft("crossing:aic:jkt:issuer-domain-mismatch"),
        boundary: {
            ...BOUNDARY,
            relying_party_id: "rp:other-company",
        },
    }, { signing_keys: [] });
    cases.push(result("AIC-ISSUER-DOMAIN-MISMATCH-REFUSED-BEFORE-SIGNING", "hostile", !issuerTimeDomainMismatch.ok
        && issuerTimeDomainMismatch.reason
            === "aic_crossing_issue_admission_domain_mismatch", "aic_crossing_issue_admission_domain_mismatch", {
        ok: issuerTimeDomainMismatch.ok,
        reason: issuerTimeDomainMismatch.ok
            ? null
            : issuerTimeDomainMismatch.reason,
    }));
    ...
    cases.push(result("SIGNED-BODY-RP-SUBSTITUTION-REFUSED", "boundary", !rpSubstitutedCheck.verified
        && rpSubstitutedCheck.reason === "signature_invalid", "signature_invalid", {
    ...
    const base = {
        ...
        known_limits: [
            ...
            "JWT-SVID workload projection uses a private source verifier that emits no crossing authority. The projection digest commits the accepted source evaluation, including verifier, source-policy, evidence, trust-anchor, status, validity, and freshness inputs.",
