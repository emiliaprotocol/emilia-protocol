...
import { assertAicInspectionBoundary, assertAicRawRepositoryFileUrl, } from "./verify-source-lock.mjs";
...
test("current AIC inspected-file URLs bind the declared repositories, revisions, and paths", () => {
    ...
    for (const repository of SOURCE_LOCK.varwof.repositories) {
        for (const inspectedFile of repository.inspected_files) {
            assertAicRawRepositoryFileUrl(repository.repository, repository.revision, inspectedFile.path, inspectedFile.url);
...
test("AIC source-lock validation rejects a raw URL with a different revision", () => {
    ...
    const differentRevision = repository.revision === "0".repeat(40)
        ? "1".repeat(40)
        : "0".repeat(40);
    const mismatchedUrl = inspectedFile.url.replace(`/${repository.revision}/`, `/${differentRevision}/`);
    assert.throws(() => assertAicRawRepositoryFileUrl(repository.repository, repository.revision, inspectedFile.path, mismatchedUrl), /does not match declared repository, revision, and path/);
});
test("AIC source-lock validation rejects a raw URL with a different path", () => {
    const repository = SOURCE_LOCK.varwof.repositories[0];
    const inspectedFile = repository.inspected_files[0];
    assert.throws(() => assertAicRawRepositoryFileUrl(repository.repository, repository.revision, inspectedFile.path, `${inspectedFile.url}.different`), /does not match declared repository, revision, and path/);
});
test("AIC source lock keeps carrier provenance and upstream wiring limits fail closed", () => {
    ...
    assert.throws(() => assertAicInspectionBoundary(relabeled), /inspection fact changed/);
    const wired = structuredClone(SOURCE_LOCK);
    wired.inspection.upstream_wiring.deployed_wiring_verified = true;
    assert.throws(() => assertAicInspectionBoundary(wired), /upstream wiring boundary changed/);
    ...
    assert.throws(() => assertAicInspectionBoundary(widenedSpki), /SPKI algorithm changed/);
});
...
test("pure JSON jkt and X.509 SPKI stay separate while both produce non-authorizing receipts", async () => {
    ...
    assert.equal(byId["SYNTHESIZED-X509-CARRIER-CONFUSION-REFUSED"].observed.x509_path_reason, "aic_carrier_provenance_unverifiable");
...
test("hostile native inputs fail before a crossing authority is emitted", async () => {
    ...
    assert.match(report.known_limits.join(" "), /native verifier result and relying-party policy are structurally separate/);
});
...
test("the report carries exact source revisions and calls an external run a reproduction", async () => {
    ...
    assert.equal(report.source_lock.inspection.gateway_bearer_bridge
        .non_test_verify_bearer_call_sites_observed, false);
    assert.equal(report.source_lock.inspection.upstream_wiring
        .cross_process_authenticated_provenance_wrapper_required, true);
