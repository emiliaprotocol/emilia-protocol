...
test("the kit keeps native formats separate while one verifier evaluates the record contract", async () => {
    ...
    assert.match(report.known_limits.join(" "), /do not claim native semantic equivalence/);
});
...
test("an evaluation is bound only when supplied and refused when unrelated", async () => {
    ...
    assert.match(report.known_limits.join(" "), /unverified pointer/);
});
test("an external execution self-describes as reproduction, not independent implementation", async () => {
    ...
    assert.match(report.reproduction_statement, /not an independent implementation/);
});
