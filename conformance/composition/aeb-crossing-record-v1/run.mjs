...
async function verify(value, keys = VERIFICATION_KEYS, evaluation) {
...
function evaluationFor(operationId, options = {}) {
    ...
    const registry = {
        ...
        epoch: 1,
        entries: {
            "mapping:conformance:crossing": evaluationRegistryEntry("mapping:conformance:crossing", "mapping-profile", { profile_digest: profile.profile_digest }),
    ...
    return evaluateAebEvidence({
        config,
        adapters: { [EVALUATION_ADAPTER_ID]: adapter },
...
function evaluationLifecycle(evaluation) {
    return {
...
export async function buildReferenceReport() {
    ...
    cases.push(result("EVALUATION-UNCHECKED-IS-INDETERMINATE", "boundary", verified.verified && verified.evaluation_binding === "INDETERMINATE", "verified; evaluation_binding INDETERMINATE without the evaluation", {
        verified: verified.verified,
    ...
    cases.push(result("EVALUATION-BOUND", "positive", bound.verified &&
        bound.evaluation_binding === "BOUND" &&
        evaluation.verdict === "SATISFIED", "verified; evaluation_binding BOUND", {
    ...
    cases.push(result("UNRELATED-EVALUATION-REFUSED", "hostile", unrelatedPointerOnly.evaluation_binding === "INDETERMINATE" &&
        !unrelatedCheck.verified &&
        unrelatedCheck.reason === "evaluation_operation_mismatch", "evaluation_operation_mismatch", {
    ...
    cases.push(result("EVALUATION-DIGEST-MISMATCH", "hostile", !digestMismatch.verified &&
        digestMismatch.reason === "evaluation_digest_mismatch", "evaluation_digest_mismatch", { verified: digestMismatch.verified, reason: digestMismatch.reason }));
    const unmatchedRecord = await issue(wimse, {
        lifecycle_records: evaluationLifecycle(evaluation),
    });
    const unmatched = await verify(unmatchedRecord, VERIFICATION_KEYS, evaluation);
    cases.push(result("NATIVE-AUTHORITY-NOT-IN-EVALUATION", "hostile", !unmatched.verified && unmatched.reason === "evaluation_authority_unmatched", "evaluation_authority_unmatched", { verified: unmatched.verified, reason: unmatched.reason }));
    ...
    const admittedOnRevokedCheck = await verify(admittedOnRevoked, VERIFICATION_KEYS, revoked);
    cases.push(result("ADMIT-UNSATISFIED-EVALUATION-REFUSED", "hostile", !admittedOnRevokedCheck.verified &&
        admittedOnRevokedCheck.reason === "evaluation_verdict_inconsistent", "evaluation_verdict_inconsistent", {
