...
function bcrAuthority(): CrossingNativeAuthority {
...
async function issue(
  nativeAuthority = wimseAuthority(),
...
async function refusedIssue(
  nativeAuthority: CrossingNativeAuthority,
  overrides: Record<string, unknown>,
): Promise<string> {
  try {
...
export async function buildReferenceReport() {
  ...
  cases.push(
    result(
      "MAPPING-WIMSE-OAUTH",
      "mapping",
      wimse.mapping_profile_id === WIMSE_OAUTH_CROSSING_MAPPING_PROFILE,
      WIMSE_OAUTH_CROSSING_MAPPING_PROFILE,
      {
  ...
  cases.push(
    result(
      "MAPPING-BCR",
      "mapping",
      bcr.mapping_profile_id === BCR_CROSSING_MAPPING_PROFILE,
      BCR_CROSSING_MAPPING_PROFILE,
      { mapping_profile: bcr.mapping_profile_id, replay_unit: bcr.replay_unit },
    ),
  );
  cases.push(
    result(
      ...
      "mapping",
      JSON.stringify(Object.keys(wimse).sort()) ===
        JSON.stringify(Object.keys(bcr).sort()) &&
        wimse.authority_instance_digest !== bcr.authority_instance_digest,
      "same projection keys; different native authority instance",
      {
        same_keys:
          JSON.stringify(Object.keys(wimse).sort()) ===
          JSON.stringify(Object.keys(bcr).sort()),
  ...
  cases.push(
    result(
      "HYBRID-OFFLINE-VERIFICATION",
      "positive",
      verified.verified && verified.execution_authorizing === false,
      "verified evidence; never fresh authority",
      {
  ...
  cases.push(
    result(
      ...
      "positive",
      bcrVerified.verified &&
        verified.verified &&
        record.body.contract_digest !== bcrRecord.body.contract_digest,
      "both verify under one record contract without native equivalence",
      {
  ...
  for (const [id, mutate, expected] of [
    ...
    cases.push(
      result(
        id,
        "hostile",
        check.verified === false && check.reason === expected,
        expected,
        { verified: check.verified, reason: check.reason },
      ),
    );
  }
  ...
  cases.push(
    result(
      "HYBRID-STRIPPING",
      "hostile",
      !stripping.verified && stripping.reason === "hybrid_leg_missing",
      "hybrid_leg_missing",
      { verified: stripping.verified, reason: stripping.reason },
    ),
  );
  ...
  cases.push(
    result(
      "WRONG-RELYING-PARTY-KEY",
      "hostile",
      !wrongKey.verified && wrongKey.reason === "signature_invalid",
      "signature_invalid",
      { verified: wrongKey.verified, reason: wrongKey.reason },
    ),
  );
  ...
  cases.push(
    result(
      ...
      staleCheck.verified &&
        ...
        stale.body.referee.admission === "REFUSE",
      "STALE plus REFUSE",
      {
  ...
  cases.push(
    result(
      "MISSING-ADMISSION-IS-NONAUTHORIZING",
      "boundary",
      missingCheck.verified &&
        missing.body.admission_reference.state === "MISSING" &&
        missing.body.referee.admission === "INDETERMINATE",
      "MISSING plus INDETERMINATE",
      {
  ...
  cases.push(
    result(
      "LOCAL-NARROWING-ALLOWED",
      "boundary",
      narrowedCheck.verified &&
        narrowed.body.native_authority.rp_acceptance === "ACCEPTED" &&
        narrowed.body.referee.admission === "REFUSE",
      "native ACCEPTED may become local REFUSE",
      {
  ...
  cases.push(
    result(
      "LOCAL-BROADENING-REFUSED",
      "hostile",
      broadened === "authority_broadened",
      "authority_broadened",
      { issue_result: broadened },
    ),
  );
  ...
  cases.push(
    result(
      "CARRIER-INJECTION-REFUSED",
      "hostile",
      !carrier.verified && carrier.reason === "malformed_record",
      "malformed_record",
      { verified: carrier.verified, reason: carrier.reason },
    ),
  );
...
if (
  ...
  if (process.argv.includes("--write-reference")) {
    writeFileSync(
      REFERENCE_PATH,
      `${JSON.stringify(sorted(await buildReferenceReport()), null, 2)}\n`,
      "utf8",
    );
  }
  if (output)
    writeFileSync(
      resolve(output),
      `${JSON.stringify(sorted(report), null, 2)}\n`,
      "utf8",
    );
