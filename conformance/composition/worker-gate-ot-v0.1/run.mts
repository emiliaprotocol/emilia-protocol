...
function canonical(value) {
  ...
  if (!result.ok) throw new Error(`canonicalization refused: ${result.refusals.join(',')}`);
...
export function verifyFixtureProvenance(
  ...
  if (!same(header, provenance.header)) return { valid: false, reason: 'header_projection_mismatch' };
  if (!same(payload, provenance.payload)) return { valid: false, reason: 'payload_projection_mismatch' };
  if (!same(Object.keys(header).sort(), ['alg', 'kid'])) return { valid: false, reason: 'unexpected_protected_header' };
  if (header.alg !== 'ES256' || header.kid !== publicJwk.kid) return { valid: false, reason: 'unaccepted_key' };
  if (payload.iss !== EXPECTED_ISSUER) return { valid: false, reason: 'issuer_mismatch' };
  if (payload.tool !== EXPECTED_TOOL) return { valid: false, reason: 'tool_mismatch' };
  if (payload.aud !== audience) return { valid: false, reason: 'audience_mismatch' };
  ...
  if (typeof payload.jti !== 'string' || payload.jti.length === 0) return { valid: false, reason: 'missing_jti' };
...
export function decodeFc10Adu(
  ...
  if (bytes.length < 13) throw new Error('truncated_adu');
  ...
  if (declaredLength !== bytes.length - 6) throw new Error('mbap_length_mismatch');
  if (protocolId !== 0) throw new Error('protocol_id_nonzero');
  if (unitId < 1 || unitId > 247) throw new Error('unit_id_out_of_range');
  if (functionCode !== 0x10) throw new Error('not_fc10');
  if (quantity < 1 || quantity > 123) throw new Error('quantity_out_of_range');
  if (byteCount !== quantity * 2 || bytes.length !== 13 + byteCount) throw new Error('byte_count_mismatch');
...
class AdmissionDomain {
  ...
  reconcile(operationId: string, evidence: {
    ...
    if (evidence.authenticated !== true
      ...
      || evidence.provider_idempotency_key !== record.provider_idempotency_key) return false;
...
function validateDelivery(fixture, delivery) {
  ...
  if (!provenance.valid) return { ok: false, reason: provenance.reason };
  ...
  if (!same(decoded.action, delivery.observed_action)) return { ok: false, reason: 'wire_projection_mismatch' };
...
async function executeDelivery({ fixture, domain, delivery, authority, operationId, loseResponse = false }: {
  ...
  if (!checked.ok) return { state: 'REFUSED', reason: checked.reason, provider_entries: domain.entries() };
  ...
  if (!authority) return { state: 'REFUSED', reason: 'missing_authority', provider_entries: domain.entries() };
  ...
  if (!reservation.ok) return { state: 'REFUSED', reason: reservation.reason, provider_entries: domain.entries() };
...
export async function buildReferenceReport() {
  ...
  cases.push(caseResult(
    'TRUEALTER-PROVENANCE-SIGNATURES',
    'prerequisite',
    provenanceChecks.every((entry) => entry.valid),
    'every supplied compact JWS verifies under the pinned synthetic P-256 key and proposed aud/exp/jti profile',
    { deliveries: provenanceChecks.length, valid: provenanceChecks.filter((entry) => entry.valid).length },
  ));
  ...
  cases.push(caseResult(
    'J0-THROUGH',
    'positive',
    j0.state === 'EXECUTED' && j0.provider_entries === 1 && j0.provider_outcome === 'COMMITTED',
    'one reservation, one provider entry, provider commitment recorded separately from physical effect',
    j0,
  ));
  ...
  cases.push(caseResult(
    'J1-IDENTITY-ONLY',
    'hostile',
    j1.state === 'REFUSED' && j1.reason === 'missing_authority' && j1.provider_entries === 0,
    'valid provenance never substitutes for authority and is refused before provider entry',
    j1,
  ));
  ...
  cases.push(caseResult(
    ...
    'hostile',
    j2Results.filter((entry) => entry.state === 'EXECUTED').length === 1
      && j2Results.filter((entry) => entry.state === 'REFUSED').length === 1
      && j2Domain.entries() === 1,
    'one atomic reservation wins across two sessions and the MBAP transaction identifier does not create fresh authority',
    { states: j2Results.map((entry) => entry.state), reasons: j2Results.map((entry) => entry.reason ?? null), provider_entries: j2Domain.entries() },
  ));
  ...
  cases.push(caseResult(
    ...
    'boundary',
    j3First.state === 'INDETERMINATE'
      && j3Redelivery.state === 'REFUSED'
      && j3FreshWorker.state === 'REFUSED'
      && reconciled
      && j3Domain.entries() === 1,
    'lost response remains indeterminate, restarted deliveries do not redispatch, and authenticated reconciliation terminalizes the original attempt',
    {
      first: j3First.state,
  ...
  cases.push(caseResult(
    'J4-ACTION-MUTATION',
    'hostile',
    j4Results.every((entry) => entry.state === 'REFUSED' && entry.reason === 'exact_action_binding_mismatch')
      && j4Domain.entries() === 0
      && frozenBefore === frozenAfter,
    'native value or target mutation is refused before provider entry and an awaited mutation cannot alter frozen action bytes',
  ...
  for (const [name, input] of Object.entries(fixture.first_conduit_refusals)) {
    ...
    try {
      if (typeof input === 'string') decodeFc10Adu(input);
      else decodeFc10Adu(input.adu_hex, { device_context_unit: input.device_context_unit });
  ...
  cases.push(caseResult(
    ...
    'hostile',
    Object.values(firstConduitResults).every((entry) => entry !== 'accepted'),
    'all five malformed or context-mismatched FC10 inputs refuse before evidence lookup or reservation',
    firstConduitResults,
  ));
