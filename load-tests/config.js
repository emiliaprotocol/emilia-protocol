...
function uuid4() {
    ...
    for (let i = 0; i < 36; i++) {
        if (i === 8 || i === 13 || i === 18 || i === 23) {
            ...
            u += hex[(Math.random() * 4 | 0) + 8];
        }
        else {
            u += hex[Math.random() * 16 | 0];
        }
    }
    return u;
}
...
export function presentBothParties(handshakeId) {
    ...
    http.post(`${BASE_URL}/api/handshake/${handshakeId}/present`, JSON.stringify(makePresentationPayload('responder')), { headers: RESPONDER_HEADERS });
}
/**
