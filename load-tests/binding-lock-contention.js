// Generated from binding-lock-contention.ts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
/**
...
import { BASE_URL, HEADERS, SLO, createHandshake, presentBothParties, verifyHandshake, } from './config.js';
// ── Custom metrics ───────────────────────────────────────────────────────────
...
export function setup() {
    ...
    if (error)
        throw new Error(`[setup] handshake create failed: ${error}`);
    ...
    for (let i = 0; i < poolSize; i++) {
        ...
        if (i % 10 === 0)
            console.log(`[setup] Created ${i}/${poolSize}...`);
    }
    ...
    if (cErr)
        throw new Error(`[setup] contention handshake failed: ${cErr}`);
