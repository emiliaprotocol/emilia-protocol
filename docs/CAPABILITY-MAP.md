...
| **Authorization Evidence Chain (EP-AEC)** — composes heterogeneous receipts (delegation, policy-permit, human-authorization) for ONE action into a single offline ALLOW/DENY; same-action binding (cross-binding defense) + requirement expression + pluggable verifiers | reference impl shipped; **JS/Python/Go agree** (11 vectors; tri-language, tracked outside the 8-suite conformance runner) | `packages/verify/evidence-chain.js` (+ `python-verify`/`go-verify`), `conformance/vectors/aec.json`, `examples/authorization-chain.mjs` | draft staged in `standards/` |
