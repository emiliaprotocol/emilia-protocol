...
| 8 | **Write-bypass prevention** | 10 | 10/10 | `getGuardedClient()` Proxy in `lib/write-guard.js` throws `WRITE_DISCIPLINE_VIOLATION` on direct trust-table mutation; verified by TLA+ `WriteBypassSafety` property; 0 violations in 5,378 automated test cases, with every platform-applicable case required to pass |
...
npm test
# Expected: 5,378 total cases, 0 failing; platform-specific cases may skip
