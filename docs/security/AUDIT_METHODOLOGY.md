...
| 8 | **Write-bypass prevention** | 10 | 10/10 | `getGuardedClient()` Proxy in `lib/write-guard.js` throws `WRITE_DISCIPLINE_VIOLATION` on direct trust-table mutation; verified by TLA+ `WriteBypassSafety` property; 0 violations in 5,400 automated test cases, with every platform-applicable case required to pass |
...
- **fast-check**: 19 property-based tests covering binding validation invariants, scoring monotonicity, delegation scope bounds, and nested-payload hash canonicalization (4 tests added post-audit to verify deepSortKeys correctness)
...
npm test
# Expected: 5,400 total cases, 0 failing; platform-specific cases may skip
