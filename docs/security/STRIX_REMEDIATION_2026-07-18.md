...
| Route authorization/write boundary | Audited handshake, webhook, dispute, and key-rotation routes use the guarded client and normalized actor identity. | `49f4466`; 484 tests passed in the worker lane; route-boundary sweep passes. |
...
3. Audited direct `auth.entity` uses are normalized through the identity helpers.
4. SSO service-role-derived and predictable fallback secrets are removed.
5. The audited direct environment reads in logger, SIEM, SSO, and WebAuthn code are centralized through `lib/env.js`.
...
7. The Supabase service client remains process-scoped, but the tree audit found no request-scoped auth/session mutation on that client. A per-request client factory remains a future defense-in-depth improvement, not a confirmed exploit closure.
8. Sensitive rate-limit categories now require durable Redis in production (or fail closed); the in-memory backend is limited to development/non-sensitive availability paths.
