...
- **Fully offline verification, in three independent languages.** JavaScript (`@emilia-protocol/verify` on npm), Python (`packages/python-verify`), and Go (`packages/go-verify`) are proven to agree on **8 adversarial cross-language conformance suites** (receipts, signoffs, quorum, revocation, time-attestation, trust-receipt, provenance, evidence-record) on every push — and were independently re-run and confirmed by an outside implementer in June 2026. That is the IETF bar for a standard: multiple independent interoperable implementations.
...
- **Permit Receipts (`draft-lee-orprg-permit-receipts`).** A **machine/policy** permit at the effect boundary (egress/API/key-release) that authorizes an effect *before commit* — explicitly with no human-approval semantics; the author notes human oversight "could be layered atop." EP is precisely that layer. The two independently converged on the same primitives (canonical action-digest, policy epoch, anti-replay, fail-closed), which is a signal the shape is right, not a collision.
- **EP-AEC (Authorization Evidence Chain), this work.** The composition layer for the whole cluster: a relying party verifies that, for one action, the delegation receipt + the policy permit + EP's human authorization all bind the *same* canonical action and each verify — one offline ALLOW/DENY. Introduces no new receipt; makes DRP, Permit Receipts, ACTA, and AgentROA compose. This is where EP stops being "one of N receipts" and becomes the verifier-side convergence point — implemented tri-language with conformance vectors.
