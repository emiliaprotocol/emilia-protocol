...
| Consent, signoff, issuer, annotation, or receipt-client callback mutates its argument object | Each callback receives a fresh copy; the private bound snapshot is the only object that may reach the MCP handler | `tests/mcp-guard-boundary.test.ts` |
| OpenAI runtime mutates an interruption while durable consumption commits | Burn the reserved authority, reject the drifted interruption, and never call runtime approve | `packages/openai-agents/index.test.ts` |
| Agent Inbox edits an approved action before LangGraph resumes | Bind the edit as a new exact action and require a fresh receipt; authority for the original action never transfers | `packages/langgraph/index.test.mjs` |
| Agent Inbox sends `response` or `ignore` | Pass only as non-authorizing control flow; never create execution authority | `packages/langgraph/index.test.mjs` |
...
| A human withdraws one approval while their identity credential remains valid | Refuse the exact receipt or standing grant named by the revocation; do not model withdrawal as credential compromise or revoke unrelated approvals | `tests/authorization-withdrawal-boundary.test.ts`, `packages/verify/consent-grant.test.ts` |
...
| Mutation returns but the executor supplies no independent action observation | Report `unobserved` and emit no execution attestation; never echo the approved plan as proof that it ran | `sdks/typescript/tests/client.test.ts`, `integrations/langchain-emilia/tests/test_client.py` |
