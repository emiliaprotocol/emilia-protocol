...
| Control Activities — segregation of duties | Divide key duties so no single individual controls all aspects of a transaction | EP-QUORUM enforces *distinct* approvers as a fail-closed predicate and bars self-approval (initiator ≠ approver) — provable, not asserted. |
