...
**Status:** repository reference implementation; not a deployment or standardization claim

**Package:** `@emilia-protocol/gate/receipt-program`

**Primary rule:** the receipt describes an exact bounded instruction; Gate owns
policy and enforcement control, while the Receipt Program owns the downstream
effect claim and owner-result evidence for its operation
...
those primitives into one bounded developer surface. Gate decides whether the
configured path may advance and fences one claim; the Receipt Program invokes
the protected executor and owns the resulting effect claim. The instruction
descriptor and certificate encoding are deterministic; external execution is
not.

It does not introduce another policy engine, ledger, wire-format token,
blockchain, or authorization format. Gate's claim token remains an operational
bearer capability, not a receipt or identity credential. A receipt program is a
frozen instruction whose downstream effect claim may be fenced once under the
relying party's existing Gate configuration. When the downstream owner result
has been verified, the configured signer succeeds, and the complete certificate
is appended to the evidence log, the output is an operator-signed certificate
that another party can verify offline. Signing or persistence failure is
returned as a typed failure without rewriting the recorded owner result or the
external effect.
...
flowchart LR
  ...
  G["Gate<br/>policy + enforcement controller"]
  B["Capability store<br/>reserve budget and operation"]
  E["Receipt Program effect owner<br/>protected executor + stable idempotency key"]
  C["OWNER RESULT<br/>executed or fenced indeterminate"]
...
| `EXECUTE` | As the selected downstream owner, invoke the configured effect once with frozen authorization/operation snapshots, an abort signal, and Gate's action-bound provider idempotency key. |
| `COMMIT` | Record the Receipt Program owner result and bounded spend as executed and append execution evidence. |
| `COMMIT_INDETERMINATE` | Record a fenced owner result and keep the spend and operation closed when provider entry occurred but the outcome cannot be proved. This is not an irreversible terminal conclusion. |
...
The claim token used between Gate and the selected Receipt Program worker is a
bearer capability: possession authorizes submission of the bound owner result.
It must be high entropy, confidential, excluded from logs, and scoped to the
exact operation. It does not prove worker identity, provider execution, legal
entitlement, or ownership of the receipt itself. Gate stores only its digest and
refuses another token for the same claimed operation.
...
3. canonical timestamps, the certificate state root, and program/result
   digests;
4. the exact opcode sequence for the declared owner result;
...
A comparison verdict over expected and observed fields is an assessment under a
named comparison profile. It may support policy, dispute, or audit, but it is
not an execution outcome and cannot replace owner-result evidence. Similarly,
an `overturned` receipt assessment means that a later assessment superseded an
earlier assessment. It does not alter the signed receipt or certificate and does
...
## Post-Effect Lifecycle and Remedies

The original receipt, certificate, and observed effect remain immutable.
Revocation before Gate issues the effect claim prevents that claim when current
authority is required. Revocation learned after claim or execution is late: it
may constrain future authority and become evidence in a dispute, but it does not
rewrite the original result.

A dispute is a challenge record, not a remedy decision. Every remedy is a fresh
compensating action with a new CAID, action digest, operation ID, authorization,
Gate claim, downstream owner result, and certificate or evidence package. A
compensating action may offset the original effect; it never turns the original
receipt into a refusal or the original execution into a non-event. See
[Lifecycle and Remedy Kernel](./LIFECYCLE-REMEDY-KERNEL.md).

## Failure behavior

| Failure point | Effect entered? | Recorded owner result | Replay authority |
| --- | ---: | --- | --- |
...
| Provider throws, times out, returns non-canonical data, or loses its response | Yes | `indeterminate` | The original operation remains consumed or ownership-fenced and cannot be blindly retried; only authenticated reconciliation may resolve the uncertainty. |
...
| Certificate signer fails | Determined by Gate | Gate owner result preserved plus `certificate_signing_failed`; no certificate | Determined by Gate's durable owner-result state. |
...
supports verification of integrity and internal binding under a pinned operator
key. It is not a
...
The certificate also does not prove that a comparison verdict is an execution
outcome, that an overturned assessment reversed an external effect, that a
dispute was adjudicated, that a remedy is legally enforceable, or that this
kernel is deployed, independently conformant, or standardized. It shows a
technically gated repository execution under the stated inputs and trust
configuration.
