...
| MCB-25 | Cause enough valid tokens to be consumed on one live connection that a capacity policy would evict an unexpired consumed record, then replay that token, if the stateless alternative is implemented | Replay is refused; capacity pressure refuses new work and never discards an unexpired record on a live connection |
| MCB-26 | Present an unexpired stateless token from a closed connection after an application or proxy handle has been recycled for a new TLS connection, if the stateless alternative is implemented | Connection identity is unique per TLS connection instance; the recycled handle does not revive the token |
| MCB-27 | Combine a valid HMAC tag from the current TLS endpoint with a valid workload signature from a different component, while providing no independently verified relationship between them | The channel-binding leg is `INDETERMINATE`; two valid cryptographic objects do not manufacture the deployment relationship |
| MCB-28 | Present a valid tag and workload signature when trusted deployment evidence positively identifies the tag generator as outside the deployment's permitted trust relationship with that signer | The presentation is rejected as a relationship mismatch, not accepted or downgraded to `INDETERMINATE` |
| MCB-29 | Use an attestation quote over public keys, nonces, ephemeral key shares, or transcript hashes as the only claim that the attested component held `K_bind` | The channel-binding leg is `INDETERMINATE`; public handshake binding alone is not treated as proof of exporter-key possession by the attested component |
| MCB-30 | Supply an exporter-derived comparison result or secret from a trusted proxy or sidecar over an authenticated interface that is not bound to the exact TLS connection and request | The handoff cannot satisfy the deployment relationship; the required binding is `INDETERMINATE` or rejected when a contradictory connection is identified |
| MCB-31 | Establish all three proofs, then substitute the CAID, audience, evidence transcript, nonce, or tag after either cryptographic proof was created | The altered presentation is rejected; a valid deployment relationship cannot bypass current-channel or signed-request linkage |
