...
| **Demand-side receipt requirement** | The network becomes unavoidable when services demand receipts, not merely when agents can issue them. | Put `@emilia-protocol/require-receipt` in one public API, MCP server, or high-risk tool endpoint and document the `428 EMILIA Receipt Required` loop (legacy 402 remains available for x402/AP2-compatible flows). |
