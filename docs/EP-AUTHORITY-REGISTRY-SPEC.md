...
| `assurance_class` | highest deployment-local authority tier the subject may act at. Existing stored values are `A`/`B`/`C` (ordered `C` < `B` < `A`). They are neither receipt key-custody classes nor a proposed cross-protocol assurance taxonomy. No letter migration is planned; portable policies should state verifier-visible proof predicates instead. The abandoned migration is recorded in `standards/archive/ASSURANCE-LETTER-MIGRATION-RETIRED.md`. |
