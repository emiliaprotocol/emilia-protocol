...
| `draft-nelson-agent-delegation-receipts-10` (DRP) | a user delegates bounded session authority to an agent; a Micro-Receipt is "a minimal Delegation Receipt covering a single action not included in the parent receipt's scope," and "the protocol makes every approval a signed, auditable artifact" — a defined, user-signed per-action approval | what its model does not carry is EP's organizational bundle: authority enrollment for the approver, initiator exclusion, distinct-human m-of-n quorum, and mandatory one-time consumption (the words quorum, m-of-n, and initiator do not appear in the document) | complementary layer, not overlap: DRP evidences the *delegating user's* consent within a session; EP evidences *organizational* accountable approval of one exact action (named approver under enrolled authority, EP-QUORUM-v1 m-of-n, EP-INITIATOR-ATTESTATION-v1 initiator exclusion, one-time consumption). A delegated agent that hits a receipt-required effect boundary presents the EP receipt; the shared digest joins the two records |
| `draft-farley-acta-signed-receipts-02` (ACTA) | a "format for recording machine-to-machine access control decisions" — Ed25519 (RFC 8032) over RFC 8785 canonical JSON, minimal disclosure, covering both synchronous policy checks and post-execution receipts (`tool_duration_ms` is "present only in post-execution receipts") | receipts "include an issued_at timestamp but do not include a nonce"; "implementations MAY add a nonce field" — no accountable-human ceremony and no mandatory one-time consumption, by design | same primitives, different question: ACTA records *what a policy engine decided* (before or after the tool runs); EP proves *a named accountable human authorized this exact action, once*. An ACTA record for a decision escalated to a human can carry the EP receipt digest as payload metadata; a consumed EP receipt plus the ACTA decision record is a clean pre/post pair |
