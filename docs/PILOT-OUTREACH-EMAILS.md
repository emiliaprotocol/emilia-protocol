# EP Pilot Outreach Emails

## A) Shopify DTC Merchant

**Subject:** Make your store trust-ready for AI shopping agents

Hi [Name],

I'm reaching out because we're building something that may matter soon for DTC brands before it becomes obvious.

EMILIA Protocol (EP) is an open protocol for making, explaining, challenging, and verifying trust decisions about agents, software, and machine counterparties. For merchants, it lets systems evaluate whether a store is trustworthy enough for a given context and policy. Instead of relying on generic reviews or platform-specific heuristics, EP produces machine-readable trust profiles from receipts, fulfillment outcomes, disputes, and policy evaluation.

For a Shopify brand, that means two immediate things:
1. AI shopping agents can evaluate your store before checkout
2. Your store can expose portable trust signals beyond any single platform

We're particularly interested in DTC because delivery accuracy, price integrity, return handling, and dispute behavior are measurable and high-value trust signals.

The pilot would be lightweight:
- Shopify webhook integration
- Trust profile generation
- Optional public trust profile endpoint
- Install-free, low-overhead initial evaluation

The upside is that your brand becomes one of the first merchants that is not just "reviewed," but machine-trustable.

If this is interesting, I'd love to show you a short demo.

Best,
Iman Schrock
EMILIA Protocol
team@emiliaprotocol.ai
emiliaprotocol.ai

---

## B) MCP Server Publisher

**Subject:** Add install preflight and trust evaluation to your MCP server

Hi [Name],

I'm reaching out because I think MCP servers are going to need something the ecosystem does not yet have: a portable trust-evaluation protocol.

EMILIA Protocol (EP) is an open protocol for making, explaining, challenging, and verifying trust decisions about agents, software, and machine counterparties. For MCP servers specifically, it provides trust profiles, policy evaluation, install preflight, disputes, and appeals. For MCP servers specifically, EP can answer the question:

**"Is this server safe enough to install or connect to in this context?"**

That matters because stars, tweets, and registry presence are not enough once agents and enterprises begin making software trust decisions automatically.

For an MCP server publisher, EP can provide:
- Install preflight evaluation
- Trust profile lookup
- Policy-based trust evaluation
- A dispute/appeal path if trust data is contested

We're looking for early pilot partners who want to help define what trust should look like for MCP ecosystems before it gets locked into proprietary heuristics.

If you're open to it, I'd love to show you the current MCP integration and discuss a pilot.

Best,
Iman Schrock
EMILIA Protocol
team@emiliaprotocol.ai
emiliaprotocol.ai

---

## C) GitHub App Developer

**Subject:** A better trust surface for GitHub App install decisions

Hi [Name],

I'm reaching out because GitHub Apps are one of the clearest places where software trust becomes a real operational problem.

EMILIA Protocol (EP) is an open protocol for making, explaining, challenging, and verifying trust decisions about agents, software, and machine counterparties. For GitHub Apps, the core use case is simple:

**"Should this app be trusted enough to install for this scope and policy?"**

That is bigger than reviews or publisher branding. It includes:
- Trust profile evaluation
- Install preflight
- Policy-aware decisions
- Challenge and appeal paths if trust is wrong or contested
- Eventual continuity/anti-whitewashing support across migrations

We think GitHub is one of the most important future trust surfaces because app install decisions are becoming more automated, more security-sensitive, and more consequential.

If you're open, I'd love to show you how EP could apply to GitHub App trust and explore a small pilot or design conversation.

Best,
Iman Schrock
EMILIA Protocol
team@emiliaprotocol.ai
emiliaprotocol.ai
