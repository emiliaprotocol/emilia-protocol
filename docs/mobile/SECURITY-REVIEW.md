...
| High | Android enrollment used a deployment-wide attestation identifier, so a synced passkey did not prove possession of the originally enrolled device key. | Enrollment now creates a non-exportable P-256 Android Keystore key, binds its SPKI and derived ID through Play Integrity, and requires its signature over every ceremony hash. The server verifies the enrolled key before accepting Play evidence. |
| High | A registration counter of zero was discarded as an implicit baseline, and the first assertion could establish its own starting point. | Enrollment persists the exact registration `sign_count` and seeds the WebAuthn counter row in the same transaction. The first and every later assertion must strictly advance it. |
| High | Native clients could hash presentation members they did not render. | `EP-MOBILE-PRESENTATION-v1` is closed and flat. Server, SQL, Swift, and Kotlin validators refuse unknown, nested, or unversioned fields before signing, and both review screens render every accepted field. This does not claim honest pixels or user perception. |
| High | Independent APK, Play, WebAuthn-origin, and Asset Links certificate values could drift. | One normalized certificate identity derives every server pin. Release CI compares the signed APK and AAB certificates to it, and production readiness requires the live Asset Links document to contain exactly that fingerprint. |
