...
**The path, framed as headroom — not a shipped feature.** The wire format is versioned (`EP-RECEIPT-v1`) and signatures carry key ids and an explicit `algorithm` field. That gives the format design headroom: a post-quantum signature suite (for example, ML-DSA) can be introduced as a new profile / receipt version without breaking existing verifiers, which continue to verify v1 receipts under their original suites. Re-anchoring historical checkpoints under post-quantum keys is a forward-compatible mitigation for evidence that must survive a cryptographically-relevant quantum computer. None of this is shipped today; it is roadmap and design intent, and it is honest to call it that.
