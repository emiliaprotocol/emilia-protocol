...
- **That the rendering was faithful.** A signature binds signed bytes; it does not establish what the person saw or understood. Separately verified presentation evidence can bind display bytes to the same action within its own trust boundary, but cannot prove what a compromised physical display showed. ([I-D -13, Section 13.4](https://datatracker.ietf.org/doc/html/draft-schrock-ep-authorization-receipts-13#section-13.4).)
- **A specific natural-person identity beyond the key-to-approver enrollment binding.** The receipt proves a *key* enrolled under a named approver signed. Proving that the named approver is a specific real-world human (biometric identity or KYC-grade identity proofing) is explicitly out of scope of this protocol; the current draft states this boundary directly in [Section 1.2](https://datatracker.ietf.org/doc/html/draft-schrock-ep-authorization-receipts-13#section-1.2), and enrollment binds key to approver in [Section 5.2](https://datatracker.ietf.org/doc/html/draft-schrock-ep-authorization-receipts-13#section-5.2). A key-discovery or identity layer can slot in at the enrollment boundary if a deployment needs stronger identity assurance. That is a layer above the receipt, not a property of it.
...
*References: [draft-schrock-ep-authorization-receipts-13](https://datatracker.ietf.org/doc/html/draft-schrock-ep-authorization-receipts-13) (current published I-D as checked 2026-09-25; its Section 13.13 consolidates these claim boundaries), `standards/staged/NEXT-AUTHORIZATION-RECEIPTS-13/UPLOAD-THIS/draft-schrock-ep-authorization-receipts-13.xml` (archive-identical submitted source), `packages/verify/` (offline verifier), and `docs/POST-QUANTUM-MIGRATION.md` (PQ analysis). Check the [live Datatracker record](https://datatracker.ietf.org/doc/draft-schrock-ep-authorization-receipts/) before citing a revision as current.*
