...
| RCM field | Content |
|---|---|
| **Control statement** | Each authorization receipt is consumed on first use and refuses every subsequent presentation (`replay_refused`). Consumption is keyed on the issuer-generated receipt ID (a receipt without one is refused). In multi-instance deployments, a shared durable store uses atomic insert-if-absent plus ownership-fenced compare-and-set and conditional delete. Reservations carry an opaque owner token, receive no TTL, and only that owner may commit or release them. A crash or response loss after effect invocation therefore freezes or burns the approval instead of silently reopening it. Store errors refuse the action; an expired-but-uncleaned committed row still refuses. In production, the Gate refuses to start without a provided store unless management explicitly acknowledges a single-instance deployment. |
| **Type** | Automated, preventive |
| **Frequency** | Continuous (every guarded request presenting a receipt) |
| **Where implemented** | Consumption check in `packages/gate/index.js`; store contract in `packages/gate/store.js` (`MemoryConsumptionStore`, `createDurableConsumptionStore`); durable reference backend in `packages/gate/store-postgres.js` (`EP-GATE-PG-CONSUMPTION-v2`) |
| **Test procedure** | 1. Re-perform: `node --test packages/gate/gate.test.js packages/gate/store-faults.test.js packages/gate/store-postgres.test.js`. The fault suite covers concurrent reservation, restart, response loss after commit, failure before commit, stale-owner transitions, and the no-reservation-TTL rule. 2. Inspect the Postgres statements and agree insert, commit, and release are each one conditional statement. 3. Re-perform EG-1: `cd packages/gate && node eg1.mjs --json`. 4. Inspect production configuration: agree a shared durable store is provisioned or management documented a single-instance exception. 5. Sample `replay_refused` and indeterminate-execution entries and agree their `receipt_id` to the original authorization. |
