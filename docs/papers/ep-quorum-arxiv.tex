...
adversarial conformance suite of eleven vectors that three independent
implementations (JavaScript, Python, Go) are required to agree on. We
...
\item
  \textbf{An adversarial conformance suite} (Section 6) of eleven vectors
  carrying real WebAuthn assertions, against which three independent
  implementations are required to agree, separating ``too strict''
...
eleven vectors that each carry real Class-A WebAuthn (ES256) assertions:
...
The suite is run against three independent implementations ---
JavaScript, Python, and Go --- which are required to agree on every
vector; divergence is a conformance defect. The ``accept'' vectors guard
