...
The current `@emilia-protocol/verify/aeb-wimse-oauth-adapter` export is the v2
request profile for WPT-02, HTTP Message Signatures -06, Workload Credentials
...
it must be reissued and reverified under v2. This document, its vectors, and
...
The current package export is v2 and does not run these cases as v1. The
