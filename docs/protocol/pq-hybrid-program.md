...
**Corrected 2026-08-18.** An earlier revision of this paragraph said no such path existed. That was wrong, and the correction matters more than the original claim. AWS KMS has offered generally available ML-DSA-65 signing since 2025-06-13 (`KeySpec: ML_DSA_65`, `SigningAlgorithm: ML_DSA_SHAKE_256`, `GetPublicKey` returning DER SPKI that an adapter must unwrap to the raw 1952-byte form EP pins). Google Cloud KMS reached GA in 2026 at the SOFTWARE protection level only, with hardware PQC on a 2028 roadmap, so adopting it would not move custody off software at all. HashiCorp Vault Enterprise Transit offers an `ml-dsa` key type inside its own software barrier. Self-operated appliances (Thales Luna, Entrust nShield, Utimaco, Securosys) ship ML-DSA in firmware. So the honest reason the PQ leg is software-held is ADOPTION, not availability.

**What adopting a managed KMS would and would not change.** It would move the PQ secret key out of EP's process and let `custody` report `'kms'`. It would change EP's FIPS posture by exactly nothing. The AWS KMS HSM validation (CMVP certificate 4884, FIPS 140-3 Level 3, validated 2024-11-18) does not list ML-DSA among its approved algorithms, so the truthful sentence is "executes on hardware that holds a FIPS 140-3 Level 3 module validation for other algorithms," never "runs inside a validated module." The distinction is not pedantic: the Marvell LiquidSecurity 2 security policy (CMVP 5220) lists ML-DSA as non-compliant and reports `fips_state = 0` when its ML-DSA signature generation is invoked, which is what invoking an unapproved algorithm inside a validated module actually does. Modules whose own certificates list ML-DSA as APPROVED do now exist (Luna T7, CMVP 5450, validated 2026-07-29; Kryptus ASI-HSM AHX5, CMVP 5282; Dell BSAFE for Java, CMVP 5361, a software module), and none of them is a managed cloud API. Also note that vendor claims of CAVP or ACVP ML-DSA algorithm certificates are a prerequisite for module validation, never a substitute for it.
