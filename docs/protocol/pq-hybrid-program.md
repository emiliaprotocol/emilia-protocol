...
The posture words, updated 2026-08-18 to match the code rather than the plan. `EP-RECEIPT-HYBRID-v1` is no longer opt-in-only: with a hybrid signer registered and custody permitting the PQ leg, the resolved default is now `dual`. Every OTHER hybrid profile remains opt-in. The software ML-DSA leg still does not satisfy kms/hsm custody, and that is now enforced rather than merely documented: `describeHybridCustodyPosture()` refuses a software PQ leg under gov-strict with a named reason. Since gov-strict is `EP_GOV_STRICT || isProduction()`, the practical consequence is that EP's own production cannot reach the `dual` default until a real custody path is adopted. That is the correct outcome, not a bug: the alternative would be an operator attestation about a custody boundary EP does not operate, which stays forbidden. Nothing here is FIPS validated.

The earned sentence: "hybrid post-quantum signatures available across every internally signed EP evidence surface, verified in JS, Python, and Go, dual-issued by default where custody permits, with the named boundaries above."

Remaining preconditions for hybrid to be on in an EP production deployment:
1. A non-software ML-DSA-65 custody path, adopted. The socket exists (`lib/pq-custody-external.ts` plus its conformance harness) and the availability question is answered, so this is now a procurement decision rather than a research question.
2. Relying parties on record verifying the hybrid leg. Open, and this one is commercial, not technical.
3. NOT "FIPS 204 errata settled". That precondition was wrongly worded and may never resolve, because NIST runs no errata process for FIPS 204; there is only an unofficial potential-updates list. Any future version of this precondition must name the specific pending item it waits on, or it is unfalsifiable and should be dropped.
