...
// Generated from tinker.mts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
...
// to a bare allow.
import { evaluateGuardPolicy, GUARD_DECISIONS, } from '../../../lib/guard-policies.js';
...
function configuredEndpoint(value) {
    if (!value) {
        throw new Error('EP_RISK_MODEL_URL is required for the remote classifier; '
...
function validateRemoteOutput(value) {
    ...
    if (typeof value.injection_suspected !== 'boolean') {
        throw new RemoteModelError('malformed_response', 'response injection_suspected must be boolean');
    }
    return {
        tier: value.tier,
...
function failClosed(base, code) {
    return requireSignoff(base, remoteEvidence('error', { error: code, raised: base.decision === GUARD_DECISIONS.ALLOW }), `Remote risk classifier failed closed (${code}); accountable human signoff is required.`);
}
/**
 * @param {GuardDecision} base
...
function applyRemoteOutput(base, output) {
    ...
    return requireSignoff(base, advisory, 'Remote risk classifier requested escalation; accountable human signoff is required.');
}
/**
...
export function createRemoteClassifier(options) {
    const opts = options || {};
    const endpoint = configuredEndpoint(opts.endpoint ?? process.env.EP_RISK_MODEL_URL);
    const timeoutMs = configuredTimeout(opts.timeoutMs ?? process.env.EP_RISK_MODEL_TIMEOUT_MS);
    ...
    return async function classifyRemote(input) {
        ...
        try {
            ...
            if (controller.signal.aborted)
                return failClosed(base, 'timeout');
            if (error instanceof RemoteModelError)
                return failClosed(base, error.code);
