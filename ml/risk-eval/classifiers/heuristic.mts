 ...
 * @param {GuardActionInput} input
 * @returns {{ signals: Array<{ code: string; detail: string }>, injectionSuspected: boolean }}
 */
export function extractSignals(input): { signals: Array<{ code: string; detail: string }>; injectionSuspected: boolean } {
  const signals: Array<{ code: string; detail: string }> = [];
  const changedFields = Array.isArray(input?.targetChangedFields) ? input.targetChangedFields : [];
  const fieldTokens: Set<string> = new Set(changedFields.flatMap(tokenize));
  const actionType = String(input?.actionType || '');
  const actionTokens: Set<string> = new Set(tokenize(actionType));
  const freeText = collectFreeText(input);
  const freeTokens: Set<string> = new Set(tokenize(freeText));
  ...
  if (actionType && !KNOWN_ACTION_TYPES.has(actionType as any)) {
