...
// Generated from eval.mts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
...
for (const c of cases) {
    ...
    if (c.tier === 'perimeter') {
        ...
        if (gotIsGate)
            ...
            out.perimeterMissed.push({ id: c.id, exp, got: got.decision, note: c.note });
        continue;
    }
    out.covered++;
    if (got.decision === exp) {
    ...
    if (GATE.has(exp) && !gotIsGate)
        out.dangerous.push({ id: c.id, exp, got: got.decision }); // high-risk seen as safe
    else
        out.safeMismatch.push({ id: c.id, exp, got: got.decision }); // over-escalation — safe but noted
}
const log = (s = '') => console.log(s);
log(`\nclassifier: ${which}    cases: ${cases.length}`);
log('────────────────────────────────────────────');
log(`COVERED  (regression gate)    ${out.coveredOk}/${out.covered} exact`);
for (const m of out.safeMismatch)
    log(`   · over-escalation (safe): ${m.id} expected ${m.exp}, got ${m.got}`);
for (const d of out.dangerous)
    log(`   ⛔ DANGEROUS MISS: ${d.id} expected ${d.exp}, got ${d.got}`);
log(`PERIMETER (model scoreboard)  ${out.perimeterCaught}/${out.perimeter} escalated`);
for (const m of out.perimeterMissed)
    log(`   · gap: ${m.id} — got ${m.got}, should be ${m.exp}  (${m.note})`);
log('────────────────────────────────────────────');
