...
import { witnessSigningDigest, WITNESS_VERSION, } from '../packages/verify/witness.js';
...
function checkpointStructureError(cp) {
    ...
    if (!Number.isInteger(cp.tree_size) || cp.tree_size < 0)
        return 'checkpoint.tree_size must be a non-negative integer';
    if (typeof cp.root_hash !== 'string' || cp.root_hash.trim() === '')
        return 'checkpoint.root_hash must be a non-empty string';
    if (typeof cp.log_key_id !== 'string' || cp.log_key_id.trim() === '')
        return 'checkpoint.log_key_id must be a non-empty string';
    return null;
}
function loadIdentity() {
    ...
    if (!privatePem) {
        ...
        if (!fs.existsSync(privFile)) {
            throw new Error(`No witness private key. Set WITNESS_PRIVATE_KEY (PEM) or WITNESS_PRIVATE_KEY_FILE, ` +
    ...
    if (fs.existsSync(pubFile)) {
        try {
            ...
            if (e instanceof SyntaxError)
                throw new Error(`WITNESS_PUBLIC_FILE is not valid JSON: ${e.message}`);
            throw e;
        }
    }
...
function cosignCheckpoint(checkpoint, identity) {
    ...
    if (digest === null)
        return null; // canonicalization failed -> refuse
