#!/usr/bin/env node
// Generated from equivocation-demo.mts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
/**
        ...
        const req = http.request({ host: '127.0.0.1', port, path: '/cosign', method: 'POST',
            ...
            res.on('end', () => {
                ...
                if (res.statusCode !== 200)
                    return reject(new Error(`cosign ${res.statusCode}: ${JSON.stringify(parsed)}`));
...
async function main() {
    ...
    try {
        ...
        for (const p of ports)
            honestCosigs.push(await cosign(p, headHonest));
        ...
        assert(c.overlapping_witness_ids.length === 1 && c.overlapping_witness_ids[0] === identities[1].witness_id, 'witness 2 must be named as having cosigned BOTH conflicting heads');
