// Generated from equivocation.node-test.mts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
/**
        ...
        const req = http.request({ host: '127.0.0.1', port, path: '/cosign', method: 'POST',
            ...
            res.on('end', () => {
                ...
                if (res.statusCode !== 200)
                    return reject(new Error(`cosign ${res.statusCode}`));
...
test('3 witnesses: honest head accepted, split view accepted per-verifier, gossip detects equivocation', async (t) => {
    ...
    for (const p of ports)
        honestCosigs.push(await cosign(p, honest));
    ...
    const det = detectEquivocation([{ checkpoint: headA, cosignatures: cosigsA, label: 'verifier-A' },
        { checkpoint: headB, cosignatures: cosigsB, label: 'verifier-B' }], pinned, K);
    ...
    assert.deepEqual(det.conflicts[0].overlapping_witness_ids, [identities[1].witness_id], 'straddling witness named as cosigning both heads');
    // CONTROL: agreeing views raise no false positive.
    const control = detectEquivocation([{ checkpoint: honest, cosignatures: honestCosigs.slice(0, 2), label: 'X' },
        { checkpoint: honest, cosignatures: honestCosigs.slice(1, 3), label: 'Y' }], pinned, K);
...
test('detector is fail-closed: a head that misses quorum cannot convict the log', async (t) => {
    ...
    const det = detectEquivocation([{ checkpoint: headA, cosignatures: cosigsA, label: 'A' },
        { checkpoint: headB, cosignatures: cosigsB, label: 'B' }], pinned, 2);
