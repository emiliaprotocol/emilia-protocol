#!/usr/bin/env node
// Generated from detect-equivocation.mts by scripts/build-standalone-runtimes.mjs. Do not edit.
/* eslint-disable */
/**
...
export function detectEquivocation(views, pinnedWitnessKeys, k) {
    ...
    for (const a of accepted) {
        ...
        if (!heads.has(a.root_hash))
            heads.set(a.root_hash, { labels: [], witnessSet: new Set() });
    ...
    for (const { log_key_id, tree_size, heads } of byPosition.values()) {
        ...
        for (const h of headArr)
            for (const w of h.witness_ids)
                witnessCount.set(w, (witnessCount.get(w) || 0) + 1);
